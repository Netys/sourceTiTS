package classes.Items.Transformatives
{
    ...
    public class CoCNeonPinkEgg extends ItemSlotClass
    {
		...
		public static function useNeonPinkEgg(pc:Creature, pregnantChange:Boolean = false):void
		{
			...
			if (pregnantChange) {
				buffer += ("\n\n<b>Your egg-stuffed ");
				if (pc.pregnancyData[0].pregnancyType == "CoCBunnyEggPregnancy") {
					buffer += ("womb ");
					if (pc.pregnancyData[3].pregnancyType == "CoCBunnyEggPregnancy") output("and ");
				}
				if (pc.pregnancyData[3].pregnancyType == "CoCBunnyEggPregnancy") output("backdoor ");
				if (pc.pregnancyData[0].pregnancyType == "CoCBunnyEggPregnancy" && pc.pregnancyData[3].pregnancyType == "CoCBunnyEggPregnancy") output("rumble");
				else buffer += ("rumbles");
				buffer += (" oddly, and you have a hunch that something's about to change</b>.");
			}
			//STATS CHANGURYUUUUU
			//Boost speed (max 80!)
			...
			if (changes < changeLimit && rand(5) == 0) {
				...
				if (pc.libido() < 60) {
					...
					if (!pc.hasCock() || (pc.isHerm() && rand(2) == 0)) {
						if (pc.libido() < 30) {
							...
							if (pc.cor() < 25) buffer += ("You're repulsed by such shameful thoughts.");
							else if (pc.cor() < 60) buffer += ("You worry that this place is really getting to you.");
							else if (pc.cor() < 90) buffer += ("You pant a little and wonder where the nearest fertile male is.");
							else buffer += ("You grunt and groan with desire and disappointment.  You should get bred soon!");
						}
					...
					if (pc.hasCock()) {
						if (pc.libido() < 30) {
							...
							if (rand(2) == 0) buffer += ("female hare until she's immobilized by all her eggs");
							else buffer += ("herm rabbit until her sack is so swollen that she's forced to masturbate over and over again just to regain mobility");
							buffer += (". ");
							if (pc.cor() < 25) buffer += ("You're repulsed by such shameful thoughts.");
							else if (pc.cor() < 50) buffer += ("You worry that this place is really getting to you.");
							else if (pc.cor() < 75) buffer += ("You pant a little and wonder where the nearest fertile female is.");
							else buffer += ("You grunt and groan with desire and disappointment.  Gods you need to fuck!");
						}
				...
				//Libido over 60? FUCK YEAH!
					...
					if (pc.skinType != GLOBAL.SKIN_TYPE_SKIN) buffer += (" through your " + pc.skinFurScales());
					buffer += (".  ");
					if (pc.hasCock() && !pc.isHerm()) buffer += ("Compression tightens down on [pc.eachCock] as it strains against your [pc.gear].  You struggle to fight down your heightened libido, but it's hard – so very hard.");
					else if (!pc.hasGenitals()) buffer += ("Sexual hunger seems to gnaw at your [pc.asshole], demanding it be filled, but you try to resist your heightened libido.  It's so very, very hard.");
					else if (pc.hasVagina() && !pc.isHerm()) buffer += ("Moisture grows between your rapidly-engorging vulva, making you squish and squirm as you try to fight down your heightened libido, but it's hard – so very hard.");
					else buffer += ("Steamy moisture and tight compression war for your awareness in your groin as [pc.eachCock] starts to strain against your [pc.gear].  Your vulva engorges with blood, growing slicker and wetter.  You try so hard to fight down your heightened libido, but it's so very, very hard.  The urge to breed lingers in your mind, threatening to rear its ugly head.");
				}
				//MEGALIBIDO
				else {
					...
					if (pc.cor() < 33) buffer += ("You sigh, trying not to give in completely.");
					else if (pc.cor() < 66) buffer += ("You pant and groan, not sure how long you'll even want to resist.");
					else {
						buffer += ("You smile and wonder if you can ");
						if (pc.libido() < 100) buffer += ("get your libido even higher.");
						else buffer += ("find someone to fuck right now.");
					}
				}
			}
			...
			//if (pc.sens < 60 && changes < changeLimit && rand(3) == 0) {
				...
				////(low)
				//if (rand(3) != 2) {
					//buffer += ("The feeling of small breezes blowing over your " + pc.skinDesc + " gets a little bit stronger.  How strange.  You pinch yourself and nearly jump when it hurts a tad more than you'd think. You've gotten more sensitive!", false);
					//dynStats("sen", 5);
				//}
				////(BIG boost 1/3 chance)
				//else {
					//dynStats("sen", 15);
					//buffer += ("Every movement of your body seems to bring heightened waves of sensation that make you woozy.  Your [pc.gear] rubs your [pc.nipples] deliciously", false);
					//if (pc.hasFuckableNipples()) {
						//buffer += (", sticking to the ", false);
						//if (pc.biggestLactation() > 2) buffer += ("milk-leaking nipple-twats", false);
						//else buffer += ("slippery nipple-twats", false);
					//}
					//else if (pc.biggestLactation() > 2) buffer += (", sliding over the milk-leaking teats with ease", false);
					//else buffer += (" catching on each of the hard nubs repeatedly", false);
					//buffer += (".  Meanwhile, your crotch... your crotch is filled with such heavenly sensations from ", false);
					//if (pc.gender == 1) {
						//buffer += (sMultiCockDesc() + " and your ", false);
						//if (pc.balls > 0) buffer += (ballsDescriptLight(), false);
						//else buffer += (assholeDescript(), false);
					//}
					//else if (pc.gender == 2) buffer += ("your [pc.vagina] and " + clitDescript(), false);
					//else if (pc.gender == 3) {
						//buffer += (sMultiCockDesc() + ", ", false);
						//if (pc.balls > 0) buffer += (ballsDescriptLight() + ", ", false);
						//buffer += (vaginaDescript(0) + ", and " + clitDescript(), false);
					//}
					////oh god genderless
					//else buffer += ("you " + assholeDescript(), false);
					//buffer += (" that you have to stay stock-still to keep yourself from falling down and masturbating on the spot.  Thankfully the orgy of tactile bliss fades after a minute, but you still feel way more sensitive than your previous norm.  This will take some getting used to!", false);
				//}
			//}
			...
			//-VAGs
			if (pc.hasVagina() && !pc.hasPerk("Bunny Eggs") && changes < changeLimit && rand(4) == 0 && pc.bunnyScore() > 3) {
				...
				buffer += ("(<b>Perk Gained: Bunny Eggs</b>)");
				pc.createPerk("Bunny Eggs", 0, 0, 0, 0, "Laying eggs has become a normal part of your bunny-body's routine.");
				changes++;
			}
			//Shrink Balls!
			...
			if ((pc.balls > 0 || pc.hasCock()) && pc.cumMultiplierRaw < 100 && rand(3) == 0 && changeLimit > 1) {
				...
				if (pc.balls > 0) {
					...
					//(medium cum quantity) < 250
						...
						if (pc.hasCock()) buffer += ("  In the process, you brush [pc.eachCock] and discover a bead of pre leaking at the tip.");
					}
					//(large cum quantity) < 750
					else if (pc.cumQ() < 750) {
						buffer += ("\n\nA strong contraction passes through your [pc.sack], almost painful in its intensity.  ");
						if (pc.hasCock()) buffer += ("[pc.EachCock] leaks and dribbles pre-cum down your " + pc.legs() + " as your body's cum production kicks up even higher.");
						else buffer += ("You wince, feeling pent up and yet unable to release.  You really wish you had a cock right about now.");
					}
					//(XL cum quantity) < 2000
						...
						if (pc.hasCock()) buffer += ("A thick trail of slime leaks from [pc.eachCock] down your " + pc.leg() + ", pooling below you.");
						else buffer += ("You grunt, feeling terribly pent-up and needing to release.  Maybe you should get a penis to go with these balls...");
						buffer += ("  It's quite obvious that your cum production has gone up again.");
					}
					//(XXL cum quantity)
					else {
						...
						if (pc.hasCock()) buffer += ("pre-cum explodes from [pc.eachCock], running down your " + pc.leg() + " and splattering into puddles that would shame the orgasms of lesser " + pc.mf("males", "persons") + ".  You rub yourself a few times, nearly starting to masturbate on the spot, but you control yourself and refrain for now.");
						else buffer += ("You pant and groan but the pleasure just turns to pain.  You're so backed up – if only you had some way to vent all your seed!");
					}
				}
			...
			if (pregnantChange) kGAMECLASS.eventBuffer += buffer;
			else output(buffer);
		}
	}	
}
