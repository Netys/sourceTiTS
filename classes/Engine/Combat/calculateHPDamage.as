package classes.Engine.Combat 
{
	...
	public function calculateHPDamage(target:Creature, attacker:Creature, damageResult:DamageResult, special:String = ""):void
	{
		...
		if (attacker != null && attacker.hasPerk("Sadist") && !target.isLustImmune) damageAfterResistances *= 1.2; // let's consider that lust immune foes are not sexy enough to get a thrill from hitting them
		...
		if (attacker != null && special == "melee" && attacker.hasPerk("Lunging Attacks") && target != attacker && defReduction > 0) defReduction /= 2;
		
		if (attacker != null && attacker.hasPerk("Precision") && target != attacker && defReduction > 0)
		{
		...
		if (damageAfterResistances <= target.HP())
		{
			...
			if (attacker != null && attacker.hasPerk("Sadist") && !target.isLustImmune) attacker.lust(Math.min(Math.max(damageAfterResistances / 10, 3), 30)); // isLustImmune is checked for target rather than for attacker intentionally
		...
		else
		{
			...
			if (attacker != null && attacker.hasPerk("Sadist") && !target.isLustImmune) attacker.lust(Math.min(Math.max(target.HP() / 10, 3), 30)); // isLustImmune is checked for target rather than for attacker intentionally
