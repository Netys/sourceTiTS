package classes.Characters.CoC 
{
	...
	public class CoCBasilisk extends Creature
	{
		...
		public static function basiliskSpeed(target:Creature, amount:Number = 0):void {
			if (!target.hasStatusEffect("Basilisk Slow")) target.createStatusEffect("Basilisk Slow", 0, 0, 0, 0, false, "Icon_Slow", "You are slowed down by basilisk gaze!");
		...
		private function compulsion(target:Creature):void {
			...
			if (target.hasPerk("Enlightened Nine-tails") && InCollection(target.eyeColor, kGAMECLASS.elderKitsuneEyeColors)) {
				...
				if (rand(target.willpower()) > willpower()) { // chance is really slim even on high levels, but why not to try?
					createStatusEffect("Blinded", 3, 0, 0, 0, false, "Blind", "Basilisk is dazzled under your gaze!", true, 0, 0xFF0000);
