package classes.Characters.CoC 
{
	...
	public class CoCTamani extends CoCGoblin
	{
		...
		//New Tease option:
		public function tamaniHypnoTease(target:Creature):void {
			...
			//REACTIONS
			//LOW HYPNO VALUE:
			if(int(kGAMECLASS.flags["COC.TAMANI_TIMES_HYPNOTISED"]) < 5) {
				selector = rand(3);
			...
			//MEDIUM HYPNO VALUE:
			else if(int(kGAMECLASS.flags["COC.TAMANI_TIMES_HYPNOTISED"]) < 10) {
				selector = rand(2);
				if(selector == 0) {
					output("With effort you manage to wrench your eyes away from the inviting folds of Tamani's vagina.  [pc.EachCock]");
					if(target.lust() > 80) output(" drips pre-cum");
					else if(target.lust() > 40) output(" grows harder");
					else output(" hardens");
					...
					if(target.lust() < 70) output("  Still, you don't want to fuck her right now!");
				}
				else {
					output("Struggling, you pull your eyes back into your head and away from Tamani's gorgeous slit.  You shudder, feeling [pc.eachCock]");
					if(target.lust() <= 41) output(" thicken perceptibly");
					else if(target.lust() <= 81) output(" twitch eagerly");
					...
					if(target.lust() < 70) output("  Regardless, you're resolute in your desire not to fuck her right now!");
				}
			}
			//HIGH HYPNO VALUE
			else {
				selector = rand(2);
				if(selector == 0) {
					output("You barely manage to step yourself from lunging forward to bury your mouth between your mistress's legs.  Hard and trembling between your [pc.legs], [pc.eachCock] aches with need.  You battle with the compulsion to kneel before your short, stacked mistress and perform your duties as her breeder husband.");
				}
				else {
					output("You wrench your gaze from the juicy mound before you with great difficulty.  The desire to submit to your wife and fuck her on the spot rages through your body, melting your resistance into liquid lust and pooling it in your groin.  ");
					output("[pc.EachCock] pulses and dribbles pre-cum, aching to do its duty and fire load after load into Tamani's perfect pussy.");
				}
			}
			applyDamage(new TypeCollection( { tease: (rand(target.libido() / 5) + 3 + (flags["COC.TAMANI_TIMES_HYPNOTISED"])) } ), this, target, "minimal");
		}
	}
}