	...
	if(target == pc) {
		...
		if(target.hairLength == 0) {
			...
			else if (target.earType == GLOBAL.TYPE_LAPINE || target.earType == GLOBAL.TYPE_LEITHAN)
			{
				...
				else output2(" A pair of floppy rabbit ears stick up from the top of your skull, flopping around as you walk.");
			}
			...
			else if (target.earType == GLOBAL.TYPE_DRACONIC)
			{
		...
		else if(target.armType == GLOBAL.TYPE_CANINE || target.armType == GLOBAL.TYPE_VULPINE) 
		{
			...
			if(target.armType == GLOBAL.TYPE_CANINE) output2(", just like one of the ausar");
			output2(".");
		}
