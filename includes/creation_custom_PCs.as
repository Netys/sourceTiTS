...
public function customPCCheck(TF:Boolean = false):Boolean
{
	...
	switch (short)
	{
		...
		case "etis":
			...
			pc.ballSizeRaw = (pc.isTaur() ? 2 : 6) * Math.PI;
			...
			if (!pc.hasPerk("Dragonfire")) pc.createPerk("Dragonfire", 0, 0, 0, 0, "Allows access to a dragon breath attack.");
			if (!pc.hasPerk("Androgyny")) pc.createPerk("Androgyny", 0, 0, 0, 0, "Your face is always androgynous.");
