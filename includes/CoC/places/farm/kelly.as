...
//
		...
		//{
			...
			//if (model.time.hours > 23) {
				...
				//if (flags["COC.KELT_BREAK_LEVEL"] >= 4) flags["COC.KELLY_DISOBEYING_COUNTER"]++;
			//}
...
public function showKeltKelly():void
{
	if (Flag("COC.KELT_BREAK_LEVEL") < 4) {
		showKelt();
		return;
	}
...
private function hasPinkEgg():Boolean {
	return (pc.hasItem(new CoCPinkEgg()) || pc.hasItem(new CoCPinkEggLarge()));
}
...
public function breakingKeltOptions():void {
	clearOutput();
	showKeltKelly();
	if((!pc.hasCock() && Flag("COC.KELT_BREAK_LEVEL") == 0) || Flag("COC.NEVER_RESIST_KELT") == 1 || Flag("COC.KELT_SUBMISSION") >= 40 || Flag("COC.KELT_MET") == 0) {
		keltEncounter();
		return;
	}
	if(flags["COC.KELT_BREAK_LEVEL"] > 0) {
		if(!pc.hasCock() && flags["COC.KELT_BREAK_LEVEL"] < 4) {
			output("You can't keep trying to break Kelt without the proper tool to do it with.");
			clearMenu();
	...
	output("Having met Kelt, you know he's liable to subject you to plenty of abuse in exchange for training.  Are you going to endure it, resist, or never resist?");
	clearMenu();
	addButton(0, "Endure", keltEncounter, null, "Endure", "Take it. For now.");
	addButton(1, "Resist", resistKeltsBSBreakHimIntro, null, "Resist", "Two can play this game...");
	addButton(2, "Never", neverBreakKeltIntoKelly, null, "Never resist", "Always endure.");
	addButton(4, "FIGHT!", fightToBeatKelt, null, "Fight", "Time to kick some arrogant ass.");
}

public function fightToBeatKelt():void {
	clearOutput();
	output("Deciding you've had enough with Kelt, you [pc.readyWeapon]. Kelt readies his bow. \"<i>Do you really think you can beat me, slut?</i>\"");
	flags["COC.KELT_KILL_PLAN"] = 1;
	...
	CombatManager.setHostileCharacters(kelt);
	CombatManager.victoryScene(fightToBeatKeltVictory);
	CombatManager.lossScene(keltFucksShitUp);
	CombatManager.displayLocation("KELT");
	clearMenu();
...
private function resistKeltsBSBreakHimIntro():void {
	...
	showKeltKelly();
	if(flags["COC.KELT_BREAK_LEVEL"] == 0) {
		output("You are more and more annoyed by Kelt's rudeness and dick-waving.  The centaur may be imposing at first and his archery skills are impressive, but you're sure that behind his false display of virility, there's nothing an experienced champion like you can't deal with.  With your superior strength and speed, you could probably take him by surprise and teach him a good lesson.  Of course, you won't ever be able to learn archery from him after that.");
		//[if (PC doesn't have items)
		if(!(pc.itemCount(new CoCSucMilk()) + pc.itemCount(new CoCSucMilkPure()) >= 15 || pc.itemCount(new CoCSucMilk()) + pc.itemCount(new CoCSucMilkPure()) >= 15 && hasPinkEgg())) {
			output(" Unfortunately, you don't have anything that could be useful to tame his arrogant maleness.  You want items that would make his disgracious horsecock and balls shrink.  A nice set of breasts on his human chest would be fine, too.  You know you're going to need A LOT of such items - or very potent ones.");
			clearMenu();
			addButton(0, "Next", farmExploreEncounter);
		}
		else {
	...
	else if(flags["COC.KELT_BREAK_LEVEL"] == 1) {
		output("You set out to go get Kelt, eager to teach this slut another lesson of your own.  You explore the farm for a bit before spotting the centaur behind the barn.  However, Kelt seems to have changed since last time: he somehow changed back his gender.  That rebellious little bitch!  Although he doesn't look as aggressively masculine as before, and his chest still bears some man-tits, he has gotten back his stern, rude face and you can clearly see a fat prick hanging from his backside.  It doesn't seem to be as big as it was before, though.  He must have grown it in a hurry.");
		//back to farm]
		//[if you don't have the items:
		if(!(pc.itemCount(new CoCSucMilk()) + pc.itemCount(new CoCSucMilkPure()) >= 10 || pc.itemCount(new CoCSucMilk()) + pc.itemCount(new CoCSucMilkPure()) >= 5 && hasPinkEgg())) {
	...
	//Third encounter
		...
		CombatManager.newGroundCombat();
		CombatManager.setFriendlyCharacters(pc);
		CombatManager.setHostileCharacters(new CoCKelt());
		CombatManager.victoryScene(breakingKeltNumeroThree);
		CombatManager.lossScene(keltFucksShitUp);
		CombatManager.displayLocation("KELT");
		clearMenu();
		addButton(0, "Next", CombatManager.beginCombat);
	...
	else if(flags["COC.KELT_BREAK_LEVEL"] == 3) {
		//spriteSelect(-1);
		finalKeltBreaking();
	}
	else {
...
private function neverBreakKeltIntoKelly():void {
	clearOutput();
	showKeltKelly();
	flags["COC.NEVER_RESIST_KELT"] = 1;
	output("You decide that trying to break Kelt is something you'd never want to do.  Besides, he's teaching you a useful skill, and there's just something charming about that bastard...");
	clearMenu();
	addButton(0, "Go To Kelt", keltEncounter);
	addButton(1, "Go Home", function():*{ processTime(10 + rand(10)); mainGameMenu(); } );
}
...
private function breakKeltGo():void {
	clearOutput();
	showKeltKelly();
	output("You approach the uppity centaur with glinting eyes, determined to take him down.  Kelt mistakes your anger for desire and sneers.");
	
	output("\n\n\"<i>What do you want, you little "+ pc.mf("sissy", "bitch") +"?  I'm done with you.  I'm already doing you a favor by teaching you a skill sluts like you will never use nor master.</i>\"");
	...
	output("\n\nWHAM!  ");
	if(pc.tallness < 70) output("You jump up, and y");
	else output("Y");
	output("our [pc.mainWeapon] hits Kelt's forehead, knocking him back.  He didn't even dodge or defend himself; the hit brought him down immediately.");
	...
	if(hasPinkEgg()) {
		output(", as well as the egg");
	}
	output(".  You grab a nearby keg and pour down vial after vial until it's half-full.  The creamy substance exudes strong female scents that arouse both of you.");
	
	if(hasPinkEgg()) {
		output("  You then throw the egg into the keg before stirring the whole mixture, grinning as you prepare Kelt a whole new meal.");
	}
	...
	if(!pc.isTaur()) output("pulling [pc.eachCock] out of your [pc.gear] and stroking yourself with your free hand");
	else {
		output("slapping your hardening dick");
		if(pc.cockTotal() > 1) output("s");
		output(" against your belly in anticipation");
	}
	output(".  You might as well add some flavor to the meal.");
	...
	output(" small droplets of pre-cum begin dripping out of your [pc.cocksLight] before being swallowed by the potent, feminine mixture down below.  The white spots are drowning in that milky goo, but you're confident that Kelt will be able to recognize the taste.");
	...
	output("\n\nWhen you've leaked a sufficient amount of pre, you stop masturbating.  [pc.EachCock] is already rock-hard, and you will have to put ");
	if(pc.cockTotal() == 1) output("it");
	else output("them");
	output(" to use later.  You lift the keg and walk up to Kelt.");
	...
	if(!pc.isTaur()) output("fully whip out your [pc.cock " + pc.biggestCockIndex()+ "] and shove it between her busty tits and slowly start grinding");
	else output("step over her, shoving your [pc.cock " + pc.biggestCockIndex()+ "] between her busty tits, grinding it through that smooth valley");
	output(".");
	...
	output("at your [pc.cock " + pc.biggestCockIndex()+ "], still pressing against her heavy chest. She looks back up, fear and shame in her eyes, then back at your [pc.cock " + pc.biggestCockIndex()+ "].  She then sticks her tongue out, understanding she has no choice but to endure this, and whimpers as she gets closer to your tip, giving it only a very quick lick.  Some time passes before she gives it another timid one.  Then another one, this time longer than the last.  Then a fourth one, bolder.  She goes little by little, but you can tell she's gradually getting accustomed to your own dick-scent.  You soon lose count of how often she licks your [pc.cock " + pc.biggestCockIndex()+ "], and she never stops, as if she were somehow drawn to it - maybe that succubi milk was just what she needed.");
	...
	if(pc.isTaur()) output("dance your hindlegs about in order to ");
	output("cockslap her.");
	...
	output("\n\nThe centauress nods weakly and proceeds to wrap her bountiful flesh orbs around your [pc.cock " + pc.biggestCockIndex()+ "].  She is as awkward as she was with her mouth, but you don't actually need her to fuck her tits.  Groaning in frustrated lust, you ");
	if(pc.isTaur()) output("step over her, moving your hips back and forth to thrust through her more-than-ample cleavage.");
	else output("grab her nipples and begin to thrust your pecker against her boob valley.");
	output("  She occasionally gives your tip a wet kiss, trying to make up for her mistakes.  You ignore her and thrust harder, her round melons squeezing your shaft rhythmically.");
	...
	if (hasPinkEgg()) {
		...
		pc.destroyItem(new CoCSucMilkPure(), 10 - pc.destroyItem(new CoCSucMilk(), 10));
	}
	else
	{
		pc.destroyItem(new CoCSucMilkPure(), 15 - pc.destroyItem(new CoCSucMilk(), 15));
	}
		
	pc.orgasm();
	pc.cor(5);
	flags["COC.KELT_BREAK_LEVEL"] = 1;
	validateKeltBreakLevel();
...
private function secondKeltBreaking():void {
	...
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(new CoCKelt());
	CombatManager.victoryScene(defeatKellyNDBREAKHIM);
	CombatManager.lossScene(keltFucksShitUp);
	CombatManager.displayLocation("KELT");
	clearMenu();
	addButton(0, "Next", CombatManager.beginCombat);
...
internal function defeatKellyNDBREAKHIM():void {
	clearOutput();
	//Cut these: You swing your [pc.mainWeapon], ready to use force against the restless centaur if necessary.
	//Cut these: \"<i>Easy now, okay? You don't have your bow, and you know what I can do with my [pc.mainWeapon]. Now if you just calm down I promise I'll be much nicer this time.</i>\"
	//lust/HP: 
	if(enemy.lustQ() > 99) output("Kelt moans, mauling at his mantits in his lust before he realizes what's going on");
	else output("Kelt groans, slumping slightly from all the damage you've done to him");
	output(".  You close in, saying, \"<i>Easy now, okay?  You know what I can do with my [pc.mainWeapon].  Now if you just calm down, I promise I'll be much nicer this time.</i>\"");
	...
	output("Kelt hardly shows any resistance, sometimes walking, sometimes letting himself be hauled like a beast of burden.  When you reach the barn, he's too tired to oppose you as you tie him up again.  When he's fully bound again, you remove your [pc.gear] and pull out your items, ready to prepare another 'meal' for the centaur.  You empty all your bottles in the keg you used last time; in your haste, some of your sweat drips into the container, but you shrug it off.  After all, the bitch is going to get accustomed to your bodily fluids in one way or another.");
	...
	output("\n\nThe centauress doesn't respond, still sobbing.  You grow impatient and slap her with your [pc.cock " + pc.biggestCockIndex()+ "].");
	
	output("\n\n\"<i>Answer, you wench!  Your rightful [pc.Master] asked you a question.  Do you like the name Kelly?</i>\"");
	...
	//(Silly:)
	//if(silly && amilyFollower() && amilyCorrupt()) output("\n\nSomewhere, Amily perks up, \"<i>[pc.Master]?</i>\"  How many cumbuckets do you need anyway?");
	...
	if(pc.isTaur()) output("push her underneath you towards");
	else output("pull her close to");
	output(" your erect [pc.cocksLight].");
	...
	if (hasPinkEgg()) {
		...
		pc.destroyItem(new CoCSucMilkPure(), 5 - pc.destroyItem(new CoCSucMilk(), 5));
	}
	else
	{
		pc.destroyItem(new CoCSucMilkPure(), 10 - pc.destroyItem(new CoCSucMilk(), 10));
	...
	pc.orgasm();
	pc.cor(10);
	flags["COC.KELT_BREAK_LEVEL"] = 2;
	validateKeltBreakLevel();
	processTime(45 + rand(20));
...
internal function breakingKeltNumeroThree():void {
	clearOutput();
	if(enemy.HP() <= 1) output("Slumping down, ");
	else output("Moaning, ");
	output("he doesn't attempt to escape or attack you.  Before he can do anything, you're already on him. His reactions appear to be a lot slower since he began discovering his new craving for your dick.  He just stares at you nervously, waiting for your next move.");
	...
	if(!pc.isTaur()) output(", sometimes with your [pc.cock] -");
	output(" and yell: \"<i>What's your name?</i>\"");
	...
	output("\n\nYou move [pc.oneCock] toward her face as you speak.  The centaur girl nods silently before seizing a handful of your junk and slowly stroking it.  Her hands move along your length, rubbing and massaging all the sensitive points.  She tries to use her mouth too but you don't let her; instead, you have her point your meat toward the old keg you used before.  Kelly's eyes widen slightly as she understands what you have in mind, but she doesn't slow down the dick-stroking pace.  You close your eyes and enjoy the sensation of Kelly's delicate palm and fingers handling your dick-flesh with great care.");
	
	output("\n\nYou briskly reach ");
	if(pc.isTaur()) output("down, pushing her");
	else output("forward to grab one of her tits and grope it");
	output(" convulsively as you feel your lust rise.  ");
	if(!pc.isTaur()) output("Soon one breast isn't enough to contain your lust-driven energy and you reach for the other one with your free hand.  Your constant nipple-teasing has an effect on");
	else output("Soon, you cannot contain your lust, and you begin to buck against her hands, whinnying with delight.  The rigid, pulsing dick-flesh in her mouth has an effect on");
	output(" Kelly at last: you can hear her moan and sigh, and her hands move erratically on your [pc.cocksLight], sometimes violently yanking it as she loses control.  This brings all kinds of painful yet pleasurable tingles to your body, and only entices you to feel her up even more.");
	
	output("\n\nA familiar warmth builds in your loins ");
	if(pc.balls > 0) output("and your [pc.balls] churn ");
	output("as you feel your climax coming.  ");
	if(!pc.isTaur()) output("You squeeze Kelly's boobs so harshly she screams in pain, and y");
	else output("Y");
	output("ou release a loud groan yourself.  You cum hard, your quivering [pc.cock " + pc.biggestCockIndex()+ "] spraying spooge in steady jets directly into the keg.  You maintain your grip on the centauress");
	if(!pc.isTaur()) output("' breasts");
	output(", heedless of her pain and only caring about your own orgasm.  You feel her soft hands squeezing your junk there and there in order to milk the last drop of cum and making sure none of it remains stuck in your urethra.  When you're done riding out your orgasm, you walk up to the keg and pour down the two bottles of Succubi Milk you have left.  The copious amount of baby-batter mixes with the alright-alabaster fluid to form a bubbling ivory mixture that you bring close to Kelly's lips.");
	...
	var milk:int = pc.destroyItem(new CoCSucMilk(), 5);
	if (milk < 10) pc.destroyItem(new CoCSucMilkPure(), 5 - milk);

	pc.orgasm();
	pc.cor(5);
	flags["COC.KELT_BREAK_LEVEL"] = 3;
	validateKeltBreakLevel();
...
private function finalKeltBreaking():void {
	clearOutput();
	output("You resolutely walk up to the centaur slut for her final lesson.  This time you don't even need to find her: she rushes to you on her own, her eyes glinting with need.");
	output("\n\n\"<i>[pc.Master], [pc.Master]!  You're finally here.  I-I haven't been sleeping well since you left.  I've been dreaming about you... about your cock.  I haven't been able to eat properly, it's like I'm in a state of constant hunger that can't be satisfied... except by you.</i>\"");
	...
	output("\n\nYou laugh triumphantly as you whip your [pc.cocksLight] out of your [pc.gear]; the slut is more than ready!");
	...
	if(pc.isTaur()) output("kneels down");
	else output("bends over");
	output(" to reach for your [pc.cock " + pc.biggestCockIndex()+ "].  She sucks with eagerness and passion; you don't even need to pull her head as it moves on its own around your length.  ");
	if(pc.balls > 0) output("Her free hands moves down on your groin and caresses your [pc.balls] with expert care and precision; obviously her previous experience in groping has had an effect on her current skills.  ");
	output("Her hunger for your meaty junk seems to be insatiable, for after moments of dick kissing and licking, she decides to take the entirety of your length.  Her eyes are voracious as her fat lips swallow inch after inch of dick-flesh.");
	
	output("\n\nYou savor this moment; your [pc.cock " + pc.biggestCockIndex()+ "] is literally radiating with pleasure, and you find yourself staggering multiple times, your body almost unable to bear the sheer pleasure provided by the needy centaur-slut.  Her fingers cling to your ");
	if(pc.isTaur()) output("hindleg haunches");
	else output("[pc.butt]");
	output(", always pulling your groin closer to her avid mouth as she relentlessly throatfucks herself on you.  Your [pc.cock " + pc.biggestCockIndex()+ "] is being squeezed and massaged in the most divine way as it knocks against her warm esophagus, and you're tempted to cum in her right now, right here. But you need to finish the 'lesson'; with great regret, you softly ");
	if(pc.isTaur()) output("back away");
	else output("push the centaur girl away");
	output(" and pull out of her mouth. Wailing with displeasure, the cock-slut decides to give your meat a couple of sad licks, disappointed to see her phallic prize withdrawing from her.  She quickly regains her enthusiasm, however.");
	
	output("\n\nYou don't even need to tell her to use her boobs; after a while of dick slurping, she wraps them around your rod and pumps ferociously, her eyes filled with raw need.  After being slathered with hot saliva from the long deepthroating, her breasts make all kind of squelching noises as they repeatedly squeeze your [pc.cock " + pc.biggestCockIndex()+ "].");
	if(!pc.isTaur()) output("You grope her bountiful tits and pull them closer to you as you fuck them.  ");
	output("Her breast valley feels almost as tight as a vagina; and due to the generous amounts of drool coating your lance, it is almost as wet.  Moreover, Kelly is now openly moaning from the tit-job, as if you were actually fucking her.  She obviously has no shame anymore in letting everyone in the farm know how much she enjoys grinding her soft melons against a stranger's junk.  These thoughts make you dreamingly wonder what her virgin pussy feels like, and make you even stiffer.  With a lust-filled grunt, you thrust deeper, your dick-flesh fighting for every inch between the girl's busty tits.  Your genitals are now dangerously pulsating in arousal, threatening to cum at any time.");
	...
	output("\n\n\"<i>How can I please my [pc.Master]?  Maybe you'd like your meat to be licked clean?</i>\"");
	...
	pc.orgasm();
	pc.cor(8);
	flags["COC.KELT_BREAK_LEVEL"] = 4;
	
	kelt.removeCocks();
	kelt.createVagina();
	kelt.vaginas[0].bonusCapacity = 500;
	kelt.vaginas[0].wetnessRaw = 4;
	kelt.vaginas[0].minLooseness = 4;
	kelt.vaginas[0].wetnessRaw = 4;
	kelt.ass.wetnessRaw = 1;
	kelt.ass.loosenessRaw = 1;
	kelt.femininity = 80;
	kelt.breastRows[0].breastRatingRaw = 5;
	kelt.tallness = 6 * 12 + 3;
	kelt.hairColor = "chestnut brown";
	kelt.tailType = GLOBAL.TYPE_DEMONIC;
...
private function keltFucksShitUpII():void {
	clearOutput();
	output("You awaken at the periphery of the farm, thankful to be alive. Kelt is nowhere to be seen. You have to wonder if Whitney saved you or the dumb beast was too stupid to finish you off. Whatever the case, you head back to camp to lick your wounds. ");
	if (flags["COC.KELT_KILL_PLAN"] != 1)
	{
		output("<b>The worst indignity of all is that he broke a lot of your succubi milks.</b>  He'll likely have regained some more of his maleness by the time you're ready to attempt teaching him another lesson.");
		pc.destroyItem(new CoCSucMilkPure(), 5 - pc.destroyItem(new CoCSucMilk(), 5));
		
		//Roll Kelt back one obedience level - at the worst he drops to the level of the first fight
		if (flags["COC.KELT_BREAK_LEVEL"] > 1)
		{
			flags["COC.KELT_BREAK_LEVEL"]--;
			validateKeltBreakLevel();
		}
	}
...
private function validateKeltBreakLevel():void
{
	var breakLevel:int = int(flags["COC.KELT_BREAK_LEVEL"]);
	kelt.femininity = 10 + 25 * breakLevel;
	
	if(breakLevel == 0)
		kelt.cocks[0].cLengthRaw = 30;
	else if (breakLevel == 1)
		kelt.cocks[0].cLengthRaw = 24;
	else 
		kelt.cocks[0].cLengthRaw = 12;
	
	kelt.breastRows[0].breastRatingRaw = breakLevel;
}

//Appearance
private function kellySeeAppearance():void {
	clearOutput();
	output("Kelly is a 6 foot 3 inch tall centauress with a svelte body and generous curves.  Her feminine face is pretty human in appearance, and her lovely traits would be adorably innocent if it weren't for her emerald eyes shining with lusty need whenever she looks at you.  Plump lips that seem to have been made for cock-sucking adorn her hungry mouth.  A long, single " + kelt.hairColor);
	//[chestnut brown/sable black/garish purple/bright pink/slutty blonde) 
	output(" braid hangs between her shoulders, giving you easy leverage when you have your way with her.  Her torso is likewise human-looking, only stopping below her navel where her equine part starts growing.  She stands on four horse hooves, her lower body looking almost like a horse's- aside from the pointed " + kelt.hairColor + " demon's tail which sprouts where her horse tail should, and her cute, pink, human-like asshole.");
	//[chestnut brown/sable black/garish purple/bright pink/slutty blonde]
	
	output("\n\nShe sports a pair of soft DD-cup breasts.  Each one is ornate with a 0.5 inch nipple, often rock-hard in arousal.");
	if (kelt.breastRows.length > 0) output("  Beneath those, she has a second row of jiggly tits, just aching to be squeezed.");
	//if (pregnancy.isPregnant) output("  Drops of milk often dribble from her tits, betraying her potent lactation.");
	//[enter vagina text] 
	output("\n\nKelly has a ");
	//[virgin]
	if(kelt.vaginalVirgin == 0) output("virgin, ");
	if(kelt.vaginas[0].type == GLOBAL.TYPE_HUMAN) output("pink pussy");
	else output("puffy, black mare-cunt");
	output(" placed below her hindquarters.  It is almost constantly dripping rivulets of moisture when you're around.  Its shining openness seems to welcome you in her warmth.");
	if (flags["COC.KELLY_HEAT_TIME"] > timeAsStamp && !kelt.isPregnant()) output("<b>  There is so much leaking from her that you think she might be in heat, rendering her more receptive to impregnation.</b>");
	...
	//{
		...
		//else
		//{
			//if (farm.farmCorruption.numTattoos("kelly") > 1) output("She has the following tattoos emblazoned across her body:\n");
			//else output("She has ");
//
			//if (flags[kFLAGS.KELLY_TATTOO_COLLARBONE] != 0) output(flags[kFLAGS.KELLY_TATTOO_COLLARBONE] + "\n");
			//if (flags[kFLAGS.KELLY_TATTOO_SHOULDERS] != 0) output(flags[kFLAGS.KELLY_TATTOO_SHOULDERS] + "\n");
			//if (flags[kFLAGS.KELLY_TATTOO_LOWERBACK] != 0) output(flags[kFLAGS.KELLY_TATTOO_LOWERBACK] + "\n");
			//if (flags[kFLAGS.KELLY_TATTOO_BUTT] != 0) output(flags[kFLAGS.KELLY_TATTOO_BUTT] + "\n");
			//output("\n");
		//}
	//}

	//Kids flavour text
	if(flags["COC.KELLY_KIDS"] > 0) {
		if(flags["COC.KELLY_KIDS"] == 1) {
			output("\nKelly's ");
			if(flags["COC.KELLY_FIRST_KID_GENDER"] == 1) output("son");
			else output("daughter");
			output(" is off in a separate field; still very young, you can see ");
			if(flags["COC.KELLY_FIRST_KID_GENDER"] == 1) output("him playing in the distance, lost in his own little world.");
			else output("her playing in the distance, lost in her own little world.");
		}
		else if(flags["COC.KELLY_KIDS"] == 2) output("\n\nThe " + num2Text(flags["COC.KELLY_KIDS"]) + " children you have had with Kelly are off in a separate field; the sound of their play drifts over the grasslands to your ears.  Kelly evidently prefers to keep her brood away from what you do with her, a remarkably sensible attitude coming from a centauress cumslut.  Maybe motherhood genuinely suits her?");
		else output("\n\nThe " + num2Text(flags["COC.KELLY_KIDS"]) + " children you have had with Kelly are as ever off in a separate field, the distance sounds of their acting out the wars and drama of childhood drifting out over the grasslands to your ears.");
		output("\n");
	}
	clearMenu();
...
private function approachKelly():void {
	...
	if (flags["COC.KELLY_PREGNANCY_INCUBATION"] < timeAsStamp)
	{
		kellyPopsOutARunt();
		return;
	}
	...
	if(Flag("COC.KELLY_VAGINALLY_FUCKED_COUNT") > 0 && Flag("COC.TIMES_PUNISHED_KELLY") == 0 && rand(3) == 0 && pc.hasCock()) {
		flags["COC.KELLY_DISOBEYING_COUNTER"] = 0;
		punishKelly();
		return;
	}
	
	//Descriptions and Flavour Text
	if (int(flags["COC.FARM_CORRUPTION_STARTED"]) == 0) output("You aren't welcome on the farm proper, but you can go visit Kelly's field.");
	
	//09:00-11:00, 2 or more children:
	if(hours >= 9 && hours <= 11 && flags["COC.KELLY_KIDS"] >= 2) {
		output("\n\nKelly is standing in the shadow of her barn, an expression of blissful contentment on her face as she nurses your ");
		if(flags["COC.KELLY_KIDS"] > 2) output("two youngest ");
		output("children with ");
		if(kelt.breastRows.length > 1) output("the top row of ");
		output("her milk-laden tits.  When she dreamily opens her eyes and sees you standing there, she nudges them off gently.");
	...
	else if(hours >= 15 && hours <= 16 && flags["COC.KELLY_KIDS"] >= 4) {
	...
	//Standard:
		...
		output("\n\n\"<i>[pc.Master], you've come to visit! Is it feeding time?</i>\"");
	}	
	clearMenu();
	addButton(0, "Appearance", kellySeeAppearance);
	if(pc.lust() < 33) output("\n<b>You aren't aroused enough to pursue sex with your toy right now.</b>");
	else addButton(1, "Sex", kellySexMenu);
	if(kelt.vaginas[0].type == 0) {
		if(pc.hasItem(new CoCEquinum())) {
	...
	else if(kelt.vaginas[0].type == GLOBAL.TYPE_EQUINE) {
	...
	if(pc.hasItem(new CoCCaninePepper())) {
		output("\nYou could give her a canine pepper");
		if(kelt.breastRows.length == 1) output(", but who knows how it will change her");
		output(".");
		addButton(6, "Give CanineP", giveKellyAPepper);
	}
	if(flags["COC.KELLY_VAGINALLY_FUCKED_COUNT"] > 0 && flags["COC.KELLY_DISOBEYING_COUNTER"] >= 3 && pc.hasCock()) {
	...
	if(flags["COC.TIMES_PUNISHED_KELLY"] > 0 && Flag("COC.KELLY_REWARD_COOLDOWN") == 0 && rand(3) == 0) {
...
private function kellySexMenu():void {
	...
	if (pc.hasCock() && pc.lust() >= 33) {
		if (pc.cockThatFits(kelt.vaginalCapacity()) >= 0 || kelt.vaginas[0].type == GLOBAL.TYPE_EQUINE) {
			if (kelt.isPregnant()) addButton(0,"Preg Fuck",kellyPregSex);
			else if (!pc.isTaur()) addButton(0,"Fuck Cunt",fuckKellysCunt);
			else addButton(0,"Fuck Cunt",taurOnTaurSexKelly);
			if (flags["COC.KELLY_VAGINALLY_FUCKED_COUNT"] == 0) addButton(0,"VirginFuck",takeKellysVirginity);
			if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) >= 2) addButton(1,"TentaFuck",tentaFuckKelly);
		}
		else output("\n<b>You're too big to fuck her vagina.</b>");
		if (kelt.breastRows.length == 1 && pc.smallestCockLength() > 18 && !pc.isTaur()) {
			output("\n<b>You're too big to fuck her tits.  Maybe if you gave her something to make her grow more...</b>");
		}
		else if (!pc.isTaur()) addButton(2,"Titfuck",kellyTitJob);
...
private function fuckKellysCunt():void {
	clearOutput();
	flags["COC.KELLY_VAGINALLY_FUCKED_COUNT"]++;
	var x:int = pc.cockThatFits(kelt.vaginalCapacity());
	if(x < 0) x = pc.smallestCockIndex();
	output("You strip out of your [pc.gear] in a hurry, intent on claiming your equine slut from behind.  As soon as [pc.eachCock] flops free, Kelly takes notice, her hand coming in front of her mouth in surprise.");
	
	output("\n\n\"<i>Are we... are you going to fuck me, [pc.Master]?</i>\"  Kelly asks excitedly, pawing at the ground with a forehoof.");
	
	output("\n\nSmiling roguishly, you nod, gesturing with a single digit that she should turn around.  Kelly immediately spins about, kicking up clods of dirt in her haste.  She coyly looks over her shoulder and asks, \"<i>Are you going to take my pussy today, [pc.Master]?</i>\"  She wiggles her powerful, equine backside back and forth as she continues, \"<i>My cunt could use another dose from your dick.</i>\"");
	...
	output("\n\nKelly moans, \"<i>Y-yes!  Yes, I'm a submissive, cock-loving slut!</i>\"  She swishes her tail fanning the scent of her moist slit everywhere.  \"<i>Please, [pc.Master], don't tease me any more!  I'm so ready for you, and you're so right!</i>\"");
	...
	if(pc.tallness < 72) {
		output("You turn around to get a stool, since your ");
		if(pc.tallness < 52) output("diminutive ");
		output("height makes reaching her pussy a difficult task.  ");
	}
	output("Holding your [pc.cock " + x + "] in one hand, you press your [pc.cockHead " + x + "] against her entrance.   The steamy cunt pulses wetly around your girth, slowly spreading its thick lips to accept your rigid endowment.");
	if(pc.cockTotal() > 1) {
		output("  Your other erection");
		if(pc.cockTotal() > 2) output("s hang");
		else output(" hangs");
		output(" down below, collecting her drippings and smearing across her belly.");
	}
	output("  You slowly slide inside the tight tunnel, feeling Kelly's powerful muscles squeeze delightfully around you, massaging your [pc.cock " + x + "] with slow undulations.");
	pc.cockChange();
	...
	if(pc.balls > 0) output(" and roiling balls");
	output(" leaves no doubt that climax is fast approaching.  Kelly cums first, splattering your [pc.hips] with her ");
	if (kelt.vaginas[0].type > 0) output("fragrant ");
	output("centaur-cum, soaking your [pc.legs] immediately.  Her pussy goes wild, the once-ordered spasms going wild, wringing your [pc.cock " + x + "] with uncoordinated motions.  A moment later, all those wild contractions sync up into intense, dick-massaging waves.  The power of her equine lower half becomes obvious when her twat seems to suck you inside, squeezing you off to orgasm.");
	...
	if(pc.cumQ() >= 500) output("  Her pussy seems like a soggy, cum-soaked sex-toy before you're even halfway done.");
	if(pc.cumQ() >= 1200) {
		output("  The sheer amount of spunk exploding into her womb actually rounds her belly a little");
		if(pc.cumQ() >= 4000) output(", until she's so pregnant with spooge that she can barely stand");
		output(".  ");
	}
	output("You lay immobile on her back, panting while her wonderful cunt drains you of every drop.");
	...
	//if (!pregnancy.isPregnant) output(" so the cum can turn you into a proper broodmare");
	//else 
	output(", maybe knit some for the kids you're going to be pumping out for me non-stop");
	output(".</i>\"");
	
	output("\n\nYou slide back off, landing with a fresh spring in your step.  Then, you pick up your [pc.gear] and head off to find Kelly's blanket - you need something to wipe all the cum and slime off your [pc.legs] with.");
	pc.orgasm();
	kellyPreggers();
...
private function taurOnTaurSexKelly():void {
	...
	var x:int = pc.cockThatFits(kelt.vaginalCapacity());
	if(x < 0) x = pc.smallestCockIndex();
	output("Admiring your [pc.gear], you slowly remove the portions that cover your animal-like bottom half, wondering how you could ever handle having to wear pants.  Letting [pc.eachCock] swing free below you, smacking against your belly when erect - now that's life.  That's how you were meant to be!  You go ahead and undo the top parts of your gear as well, not wanting anything to come between you and the slutty fem-centaur just a few feet away.  She's keeping her eyes down submissively, but she keeps casting glances under your lower half, licking her lips whenever she gets a good look at [pc.oneCock].");
	...
	output(", pressing it into her face, letting your sweat and the scent of your groin smother as effectively as your presence.  Her tongue extends, knowing what you want even before you have to ask.  She licks you, tentatively and then with increasing levels of confidence, smearing saliva all over your flesh as she licks up every drop of sweat, orally lubricating you with hot, wet tongue.");
	if(pc.balls > 0) output("  Your [pc.balls] tingle pleasantly from the attention, and you groan when she sucks one into her mouth, worshipping it with her lips.");
	...
	if(pc.cockTotal() > 1) output("s");
	output(".  You gesture for her to rise and turn around, which she does");
	if(pc.tallness < 60) output(" before sitting back down (she's too tall to take standing up) and lifting her tail");
	else output(" before lifting her tail");
	output(".  You prance forward, and with a mighty lurch, you mount your mare, climbing atop her, wrapping your hands around her middle for leverage.  She wiggles under your weight, but tries to hold her back-end still while you try to align yourself with her.  Getting in position isn't as easy as it was when you had a humanoid lower body, but when you do finally feel your [pc.cock " + x + "] butting up against that warm, wet orifice, the satisfaction is many times greater.");
	
	output("\n\nYou lurch forward, plunging yourself deep inside the engorged, ");
	if(kelt.vaginas[0].type == 0) output("pink pussy");
	else output("black horse-cunt");
	output(", sheathing yourself in that muscular, deep canal.  The lips cling snugly about your [sheath] as you slide into Kelly's velvet pussy, her inner walls squeezing rhythmically around you.  You adjust position, getting accustomed to mounting her like the mare she is, and get a handhold on her big, rounded breasts, squeezing her pebbly nipples between your thumb and forefinger.  She moans, placing her palms over your hands and pressing your harder into her chest, begging, \"<i>Yes!  Fuck me, [pc.Master]!  Give me your cum!</i>\"");
	pc.cockChange();
	...
	output(", splattering out in fresh waves from Kelly's ");
	if(kelt.vaginas[0].type > 0) output("musky ");
	output("womanhood.  With her body firmly under your own, where it belongs, you firmly assert your control, grabbing her hair in one hand and yanking.  You tug her head back as you bottom out, your body filled with the pleasure of mating like the beasts that you are.");
	
	output("\n\nKelly groans through her pain, \"<i>Ooohh, yes, give me your foal, [pc.Master]!  My pussy needs it!</i>\"  True to her words, her walls begin to tug at your [pc.cock " + x + "] with familiar, milking contractions, squeezing you from base to [pc.cockHead " + x + "] again and again.  Her fluids gush out everywhere, turning the ground underneath her to mud as she climaxes.  You pinch her nipples as the heat builds within you, and when you can take it no more, you reward your cum-slut with the liquid orgasm she so craved.  Jism basts her inner walls when you explode.  Again and again, your muscles clench with ecstatic blast, ejaculating thick flows of your lust straight into the cum-dumpster's spasming cunny.  ");
	if(pc.cumQ() >= 1000) {
		output("  Her belly grows round with the liquid weight of your spunk");
		if(pc.cumQ() >= 4000) output(", and when she grows too full to hold any more, waves of alabaster spooge roll out of her abused nether-lips, adding to the messy sex-puddle below");
		output(".  ");
	}
...
private function tentaFuckKelly():void {
	clearOutput();
	output("With a mischievous grin, you remove your [pc.gear], fully revealing your [pc.cocksLight].  The plant-like appendages wriggle around the horny centauress, inspecting her body from every angle.  You feel your own lust rising as the tentacle peckers grow harder, their green heads turning pink in arousal.  Kelly stammers, confused: \"<i>W-what are you-</i>\"");
	...
	output("\n\nSLAM!  You profit from the slut's babbling to shove your " + pc.cockDescript(one) + " right into her gaping mouth, forcefully filling her throat with dick-flesh.  Her lips instinctively cling to your shaft, sucking it like a popsicle.  Drops of pre-cum are quickly swallowed and ingested by the hungry whore as they escape your urethra.  You begin thrusting in and out of her warm mouth, but her lips squeeze your rod so tight that you have to fight for every inch.  As you stuff her full of your " + pc.cockDescript(one) + ", you can hear her endless muffled moans; the slut is obviously getting off to the taste of your meaty junk.  Completely driven with lust, the green tentacle prick is vibrating on its own as it fucks Kelly's throat, sending you soft, prickling sensations.  You can't help but release a grunt as the centaur slut fellates you so expertly.");
	output("\n\nKelly is doing an incredible job with her mouth, teasing you into oblivion.  Your " + pc.cockDescript(two) + " is convulsing recklessly in frustration, begging to be stuffed into some warm hole.  Therefore, you naturally direct it toward the centauress' wet snatch; it is easily spottable due to the regular streams of juices that ooze from it.  Moreover, the slut is incredibly turned on from sucking you off, and the fem-spunk trails are even thicker than usual.  It doesn't take long for your secondary tentacle dong to reach the entrance of her moist love-tunnel.  You grind the tip of your " + pc.cockDescript(two) + " against it, using her genitals as a masturbation aid.  When it is hard as bark and ready to go, you ram it through Kelly's fuck-hole, utterly piercing her from behind.");
	pc.cockChange();
	
	output("\n\nYou ream the centaur slut from both sides, making her whinny and thrash violently.  She tramples the floor with her hooves, arches her back and screams in ecstasy through the inches of dick-flesh blocking her throat; she is completely heedless of the world beside the two hard vegetal poles fully impaling her on either end.  You pump away in rhythm, synchronizing each thrust in both holes.  Her twat is almost inhumanly hot, and gloriously tight.  However, the slut tries her best to wrap her lips even tighter around your main flexible pecker, and her tongue is teasing your " + pc.cockDescript(one) + " in the most excruciating way.  You stuff the mare slut full of your green appendages, treating her body like a bipolar cocksleeve.");
	
	//[if (cocks > 2)
	if(three >= 0) {
		output("\n\nThe display of Kelly trembling in pleasure as you spit her like a pig on your duo of plant-like pricks is enticing, and your " + pc.cockDescript(three) + " is aching with need.  It almost moves on its own volition toward the woman's perky tits, ready for a good boobjob.  The supple green wang lodges itself between the busty melons; the slut instantly begins to squeeze her heavy globes around the rod of dick-flesh, fully entrapping it in her bosom.  Her tits are so tender and soft you feel like your tentacle pecker is about to melt.  Groaning in pleasure, you thrust your " + pc.cockDescript(three) + " up and down as the titty-fucked whore accompanies your motion.  You grind in rhythm so the slut doesn't forget your other tentacle dongs.  Fortunately, Kelly quickly picks up your intention and soon you can't tell which member is providing you with the most pleasure; her mouth, vagina and soft tits are all working in conjunction, and you are fully lost in a never-ending spiral of ecstasy.");
	}
	//if (cocks > 3)
	if(four >= 0) {
		output("\n\nYour " + pc.cockDescript(four) + " trembles furiously, unable to find some body part that isn't taken by its colleagues.  You grin wickedly as an idea comes to your mind.  The protruding appendage writhes as it threads its way toward Kelly's right hand; the slut is still focusing on your " + pc.cockDescript(three) + ", squeezing her tits as tightly as she can.  Needless to say, she is surprised when another tentacle shoves itself in the way, right under her palm.  She doesn't lose her composure, though; using her forearm to press on her chest, her hand quickly grabs hold of your tree-like pecker and begins stroking your length.  Although her handjob doesn't feel as warm and tight as the wet depths of her love-tunnel or her throat, or even the squelching contact of her tender boob-flesh, she teases you efficiently with her fingers.  Her hand massages your " + pc.cockDescript(four) + " and clutches it like a vice, milking some drops of pre-cum out of your shaft.  She takes care of the most sensitive points and often makes you moan when she squeezes the towering rod.");
	}
	//[if (cocks > 4)
	if(five >= 0) {
		output("\n\nKelly still has a free hand, and your " + pc.cockDescript(five) + " is left alone, flustered.  The squirming tentacle quickly finds a comfortable spot near her boob, but this time you don't need to do anything; the eager slut catches the vegetal rod and begins to stroke it furiously.  Her hands pump along your lengths in turn; both pricks are so hard that you don't know which one will cum first.  Her fingers move across the surface of your meaty green dongs with expert care, teasing you more and more.  You enjoy the double handjob, your duo of plant-like dongs ready to explode in a mess of cum.");
	}
	//[if (cocks > 5)
	if(pc.cockTotal(GLOBAL.TYPE_TENTACLE) > 5) {
...
private function giveKellyEquinum():void {
	clearOutput();
	output("Taking the long, flared vial in your hand, you get a wicked idea - what if you gave your personal mare-slave a cunt that was better suited to her depraved, animalistic shape?  What if you turned that pretty, pink pussy into a slobbering, black mare-cunt?");
	
	output("\n\nKelly sees you behind her and nervously asks, \"<i>[pc.Master]?  What are you doing?</i>\"");
	...
	output("\n\n\"<i>Oh, right,</i>\" Kelly demurely replies, lifting her tail to further expose her prominant, womanly folds, \"<i>Whatever you want, [pc.Master].</i>\"");
...
private function giveKellySuccubiMilk():void {
	clearOutput();
	output("You produce a vial of succubi milk and dangle it before Kelly, the ivory demon-juice sloshing audibly inside.  Kelly licks her lips immediately, running her hands across her perky nipples without meaning to.  She asks, \"<i>[pc.Master], is that what I think it is?</i>\"");
	...
	output("\n\nKelly finishes the drink with a self-satisfied smile, and she cheerful says, \"<i>Thank you for the treat, [pc.Master].</i>\"");
	
	output("\n\nYou swat her rump affectionately and nod, not deigning to give her the pleasure of your praise.");
	kelt.shiftVagina(0, GLOBAL.TYPE_HUMAN);
...
private function punishKelly():void {
	clearOutput();
	//First time: 
	if(IncrementFlag("COC.TIMES_PUNISHED_KELLY") == 1) {
		...
		output("\n\n\"<i>Like you wouldn't believe, [pc.Master] ...oooh,</i>\" she moans as you circle her clitoris, already bulging out of its hood.");
		...
		output("\n\n\"<i>It's why I need you, [pc.Master],</i>\" says Kelly between gasps. \"<i>Servicing you, being the best cumslut I can possibly be, that's what I exist for.  If I don't do it, I don't get satisfied.  Although...</i>\" She trails off.  You stop fingering her.");
		
		output("\n\n\"<i>Go on,</i>\" you say gently.");
		
		output("\n\n\"<i>Well,</i>\" she says hesitantly; you resume your soft, insistent pressure upon her wet lips as she talks. \"<i>I- I sometimes get off by rubbing myself against a tree or one of the standing stones.  It's not perfect, it's not as good as your cock, but when the urge gets unbearable... ohh... it does.  You aren't mad, are you [pc.Master]?</i>\" she says anxiously, peering back over her shoulder with wide, green eyes.");
	...
	//Repeat: (Suggested activations: PC does not have sex with Kelly for 3+ days, Kelly in heat but wasn't fucked during that period) 
		...
		output("You can see guilt written in mile high letters upon Kelly's face as she bounces to a halt in front of you.  You let her stew as, with a poker face, you begin your usual slow inspection of her, letting the aura of your closeness sink into her.  She tenses when you touch the fresh abrasions upon her butt, and you tut slowly.");
		output("\n\n\"<i>Have you been a bad little cumslut, Kelly?</i>\"");
		
		output("\n\n\"<i>Y-yes, [pc.Master], I have,</i>\" she whispers after a short pause.  You keep a firm hand pressed upon her ass.");
		
		output("\n\n\"<i>Do you need punishing?</i>\"");
		
		output("\n\n\"<i>Yes, [pc.Master], I do,</i>\" she says, slightly louder and straightening her back.  You smile softly.  Such a good girl.");
	}
	
	clearMenu();
	if(flags["COC.TIMES_RIDDEN_KELLY_FOR_PUNISHMENT"] > 0 && !(flags["COC.TEL_ADRE_KNOWN"] >= 1)) {
		output("You'd like to take Kelly for a ride, but you don't have any good ideas for public places to humiliate her.");
	}
...
private function getARimjobFromKelly():void {
	...
	if(x < 0) x = pc.smallestCockIndex();
	clearOutput();
	IncrementFlag("COC.TIMES_RIM_JOBBED_BY_KELLY");
	output("\"<i>You aren't worthy of my dick,</i>\" you say, as you slowly peel off your [pc.gear]; you do it facing away from Kelly so that she can take in your [pc.butt] as you slowly wriggle free of your underclothing.  \"<i>But I still need servicing.  So...</i>\"");
	
	//[Naga: 
	if(pc.isNaga()) output("\n\nYou slither into her and, wriggling upwards, beginning the process of wrapping your long, patterned tail around her powerful form.  Kelly murmurs disquietly but stays still as you weave under and over her; you stop level to her face, her entire frame swaddled in your coils.  You trace the line of her chin with your hand as you take a moment to enjoy the sensation of this big, soft creature against your warm scales, at your mercy; then you slowly rise above her until her face is level with your genital slit before, with sinuous grace, twisting your body so that her mouth is now facing your [pc.butt].");
	
	//[Centaur:]
	else if(pc.isTaur()) output("\n\nYou gesture at the ground.  \"<i>Lie down,</i>\" you command.  Kelly carefully folds her body up to do so, until her horse half is prostrate and only her human half is propped up.  You lead yourself back until your rear end is near to her face; grinning to yourself, you slowly brush your semi-erect [pc.cock " + x + "] against her soft face, teasing her with the idea that you've changed your mind; you cannot see her but from the small bitten off grunts and moans you know she's struggling with the urge to wrap her lips around it.  Finally, you carefully lean downwards, so that instead of your dick she's faced with your [pc.butt].");
	
	//[Biped/Other:
	else output("\n\nYou gesture at the ground.  \"<i>Lie down,</i>\" you command.  Carefully Kelly folds her body up to do so, until her horse half is prostrate and only her human half is propped up.  You step forwards and for a few moments slowly brush your semi-erect [pc.cock " + x + "] against her soft face, teasing her with the idea that you've changed your mind; she struggles with the urge to wrap her lips around it, emitting small bitten off grunts and moans to your ministrations.  Finally, with a grin you pull away and turn around, so that her mouth is now facing your [pc.butt].");
	
	output("\n\nYou flare your [pc.hips] and present your [pc.asshole] to her.  \"<i>So service me.</i>\"  Without waiting for a response you press your butthole onto her mouth.");
	
	output("\n\nShe is unpracticed at first; you feel her teeth rub against your hole and you hiss.  The mere sound of your annoyance makes her whimper around your [pc.ass] and her teeth seem to vanish, replaced with a sucking, eager wetness.  ");
	if(pc.isNaga()) output("With reptilian litheness, you slowly lean backwards and press your hands onto the back of her head, pushing her face further between your cheeks.  ");
	else if(!pc.isTaur()) output("You lean backwards and press your hands onto the back of her head, pushing her face further between your cheeks.  ");
	output("You laugh at the sensation of her tongue as it pushes into your anal passage, slathering it with worshipful attention, perhaps hoping the sooner she gets you off the sooner this can end.");
	//[Not Centaur: 
	if(!pc.isTaur()) output("  You smirk and grip her hair, moving her head in rhythm with your own movements.");
	else output("  You gyrate your [pc.butt] this way and that, moving her head in rhythm with your own movements.");
	
	//[Pussy: 
	if(pc.hasVagina()) output("\n\nYou feel thin fingers brush against your [pc.vagina], slowly rubbing against the sensitive folds of your labia before circling your [pc.clit] in virtually the same motion you used upon her earlier.  You decide to allow this; she's learnt a valuable lesson you weren't even aware you were teaching, and the dual movements of her tongue in your ass and her warm fingers slicking across your wet pussy are delightful.");
	
	output("\n\nHer tongue probes deeper and deeper into you, her plump lips pressing into your [pc.asshole] until she touches something sensitive.  Your [pc.cock " + x + "] immediately reacts, twitching upwards restlessly.");
	...
	if(pc.cockTotal() > 1) output("s harden");
	else output(" hardens");
	output(" until after minutes of blissful pleasure you are bulging with need.  Her saliva coats your opening as you clench your muscles and trap her tongue deep in you before roughly rubbing your [pc.ass] against her, face fucking her for all you are worth; the soft, slippery sensation inside you is immense and you pick up the pace.");
	if(pc.isNaga()) output("  You clench her almost bone crushingly tight in your coils as rut overtakes you.");
	
	output("\n\n\"<i>Oh, you're soooo good at that,</i>\" you sigh.  \"<i>You are just as good an asslicker as you are a cocksucker.  Maybe we'll do it this way every time from now on.  Would you like that? Worshipping your [pc.Master]'s ass for your cumfix?  Should I keep you around just for when I need the sweat cleaned from my crack");
	if(pc.balls > 0) output(" and balls");
	if(pc.hasVagina()) output(" and pussy");
	output("?  I can't say it's something you don't deserve for being such a shameless slut, and I don't doubt you'd enjoy every minute of it.</i>\"");
	...
	if(pc.hasVagina()) output("; she slides as many fingers as she can into your [pc.vagina] and frigs you as best she can");
	output(".  Grunting, you reach your peak.  Your [pc.cock " + x + "] opens wide and you ejaculate streams of cum, splattering into the dust in front of you, your whole body clenching and seizing around the warm, wet muscle wedged in your [pc.butt].");
	//[Pussy:]
	if(pc.hasVagina()) output("  Your female sex orgasms in tandem, your wet flesh seizing up around Kelly's fingers and spattering her arm and chest with girl juices.");
...
private function rideKellyForPunishment():void {
	...
	if(x < 0) x = pc.smallestCockIndex();
	clearOutput();
	flags["COC.TIMES_RIDDEN_KELLY_FOR_PUNISHMENT"]++;
	//First Time: 
	if(flags["COC.TIMES_RIDDEN_KELLY_FOR_PUNISHMENT"] == 1) 
	{
		output("You stroke Kelly's side thoughtfully.  What would be a good punishment for a centaur? Several ideas immediately occur, but all require... equipment.  You tap your chin as a plan of action falls into place.");
		...
		if(pc.tallness < 78 && !pc.isTaur()) 
		{
			...
			//[Naga:]
			if(pc.isNaga()) output("  Your reptilian half is hardly ideal for riding a horse but eventually you reach a happy compromise; you perch upon her with your long tail wrapped around her barrel-like lower body.");
			//[Drider:]
			else if(pc.isDrider()) output("  Your spider half is hardly ideal for riding a horse but eventually you reach a happy compromise; you perch upon her with your long, chitinous legs wrapped carefully around her barrel-like lower body.  You must look utterly bizarre but feel reasonably comfortable.");
			output("  You lean in close to her human half and wrap your arms around her waist, drinking in the sweet scent of her pale skin; you know your own smell and power is pressing upon her senses and she whimpers softly as you slide your hands up her stomach until they are clutching her ");
			if(flags["COC.KELLY_BONUS_TIT_ROWS"] > 0) output("top row of ");
			output("breasts.");
			...
			if(flags["COC.KELLY_LACTATING"] > 0) output(" hard enough to produce thin arcs of milk");
			output(".  The centauress gasps, her breath already ragged. \"<i>...I've got to hold onto something.  Now, giddy up!</i>\" Kelly jolts into action, stumbles slightly at the extra weight, before righting and trotting in the direction of the farm; you keep yourself steady by maintaining a firm grip upon her breasts.");
			//[PC is riding: 
			output("\n\nAlthough it's only a short trip, your [pc.butt] is plenty sore by the time you get to the farm, and you KNOW you're going to need better equipment if you're to do this properly.  You bring Kelly to a halt with a \"<i>woah there</i>\" and a tug upon her ponytail.  Then, you leave her behind the barn as you search the barn for what you need, hoping Whitney doesn't find you.");
		}
		...
		else 
		{
			...
			if(flags["COC.KELLY_LACTATING"] > 0) 
			{
				if(flags["COC.KELLY_BONUS_TIT_ROWS"] > 0) output("All four of h");
				else output("H");
				output("er swollen teats immediately begin to dribble milk, and she moans.  ");
			}
			output("\"<i>So we will have to find some other way you can serve me.  We're going to go across to the farm.  You remember where that is, don't you?  It's where I taught you who you really are.  We're going to learn some more lessons there.  Won't that be fun?</i>\" You let go and set off at a ");
			if(!pc.isTaur()) output("march");
			else output("canter");
			output(" and after a pause she follows; deliberately led towards your destination.");
		...
		output("\n\n\"<i>For most it's... taboo, [pc.Master],</i>\" she answers lowly.  She makes a shuddering sigh as you fix stirrups to her and begin to fit her head with the leather straps of a harness.  \"<i>Something you would only allow a close friend or lover to do.  Even then, never with... with gear.  That is the invention of biped races for their animals, and to wear it is... is...</i>\"");
		...
		if(pc.tallness < 78 && !pc.isTaur()) 
		{
			...
			if(pc.isBiped()) output(", set your feet into the stirrups");
			output(" and enjoy the ride.  The experience is greatly more comfortable and you luxuriate in the breeze upon your " + pc.skinFurScales() + ", the gentle, comforting undulation of the farm and grasslands around you, and increasingly the smell of sweat and horniness rising off the rosy skin of your pony bitch.  Her boobs bounce up and down with her own movements, accentuated by having her arms tied behind her, and although she seems to resolve to suffer in silence to begin with, the air in front of you is soon full of muffled whines and moans.  You make a 'tchk-tchk' noise and dig into her side, urging her to go faster.  Cows raise their heads from their grazing to stare at you incuriously as you gallop past; an utterly stunned Whitney watches you from her place underneath the tree, her book forgotten in her hands.  You wonder if she can recognise your mount, and you grin at the thought.");
			
			output("\n\nEventually you arrive back at camp, and not a moment too soon.  Your nostrils are full of the smell of Kelly, and the sight of her hot, moist skin has you practically chomping at the bit yourself.  You slide off her, stagger slightly as you rip off your [pc.gear], and then without any further ado grip her hindquarters and sink your straining [pc.cock " + x + "] straight into her cunt.  It is absolutely soaked with arousal, and you encounter virtually no resistance as you thrust your hips into her soft butt, immediately beginning to fuck her as hard as you can.  In your sex-daze you realize you are still clutching the reigns; you pull at them intermittently as you push as much of yourself into the centauress's hot, grasping warmth as you can, making her rear, squeal, and then scream around her gag to the dual sensation of her [pc.Master]'s dick burying itself into her and the harness pulling tight upon her head and bit, thrusting back into you as best she can.");
			
			output("\n\nNeither of you are in any condition to keep this up for long, and after five minutes of fevered fucking, you groan as you clutch Kelly's ass hard, clenching and pouring yourself into her, your [pc.cock " + x + "] surging line after line of cum into her.  She moans breathily and happily to the heavenly sensation of being filled with your seed, her vagina milking you for every drop it can get.  You don't even know how many times she orgasmed; her pussy dribbled and gushed the entire time you were in her.");
		}
		//[PC not rider:
		else 
		{
			...
			if(!pc.isTaur()) 
			{
				output("\n\nEventually you arrive back at camp, and not a moment too soon.  Your nostrils are full of the smell of Kelly, the sight of her hot, moist skin and you are practically champing at the bit yourself.  You bring her to a halt, rip off your [pc.gear], and then without any further ado grip her hindquarters and sink your straining [pc.cock " + x + "] straight into her cunt.  It is absolutely soaked with arousal and you encounter virtually no resistance as you thrust your hips into her soft butt and immediately begin to fuck her as hard as you can.  In your sex daze, you realize you are still clutching the reins; you pull at them intermittently as you push as much of yourself in the centauress hot, grasping warmth as you can, the force making her rear.  She squeals, then screams around her gag to the dual sensation of her [pc.Master]'s dick burying itself in her and the harness pulling tight upon her head and bit, thrusting back into you as best she can.");
				output("\n\nNeither of you are in any condition to keep this up for long, and after five minutes of fevered fucking, you groan as you clutch Kelly's ass firmly and pour yourself into her, your [pc.cock " + x + "] surging line after line of cum into her.  She moans breathily and happily to the heavenly sensation of being filled with your seed, her vagina milking you for every drop it can get.  You don't even know how many times she orgasmed; her pussy dribbled and gushed the entire time you were inside her.");
			}
			//Centaur: 
			else 
			{
				output("\n\nEventually you arrive back at camp, and not a moment too soon.  Your nostrils are full of the smell of Kelly, the sight of her hot, moist skin and you are practically champing at the bit yourself; even her rippling horse body speaks directly to your own, urging you irresistibly to mount her.  You bring her to a shuddering halt, rip off your [pc.gear], and then without any further ado rear onto her back and sink your straining [pc.cock " + x + "] straight into her cunt.  It is absolutely soaked with arousal and you encounter virtually no resistance as you thrust your hips into her soft butt and immediately begin to fuck her as hard as you can.  In your sex daze you realize you are still clutching the reigns; you pull at them intermittently as you push as much of yourself in the centauress hot, grasping warmth as you can, making her rear, squealing and then screaming around her gag to the dual sensation of her [pc.Master]'s dick burying itself in her and the harness pulling tight upon her head and bit, thrusting back into you as best she can.");
				output("\n\nYou wish you could live forever in these luscious moments; it is so indefinably right to be reared over your eager submissive, encompassing her with your own flesh as you penetrate her deep wetness, both your human and horse parts cry out for it; mounting her, breeding her, imprinting yourself upon her and making her yours.  Neither of you are in any condition to keep it up for long though, and after five minutes of this fevered fucking you groan as you clutch Kelly's breasts hard and pour yourself into her, your [pc.cock " + x + "] surging line after line of cum into her.  She moans breathily and happily to the heavenly sensation of being filled with your seed, her vagina milking you for every drop it can get.  You don't even know how many times she orgasmed; her pussy dribbled and gushed the entire time you were inside her.");
			}
		}
		kellyPreggers();
		//All go to: 
		...
		output("\n\n\"<i>Th-that was a good lesson, [pc.Master],</i>\" she says eventually, her mouth open and heaving in breath.  \"<i>I think I learnt a lot.  My favorite bit was where you fucked me silly at the end, though.  Could we just skip to that next time?</i>\"  You laugh and say as long as she's a good little cumslut from now on, you will always skip to that part.  Otherwise... you nod at the pile of riding gear... you'll have to teach her more lessons... and the next one will be harder.  She swallows, not taking her eyes off the gear.");
		
		output("\n\n\"<i>I understand, [pc.Master].  I'm so lucky to have you to teach me how to be good!</i>\" You send her on her way with a swat on the backside.");
		//(\"<i>Horsewhip</i>\" option opened in main \"<i>Punish</i>\" menu)
	}
	//First repeat: 
	else if(flags["COC.TIMES_RIDDEN_KELLY_FOR_PUNISHMENT"] == 2) {
		output("Without another word, you stride over to the stones underneath which you secreted the riding gear.  Kelly watches you with deep apprehension, and then bursts into protests as you return, carrying the heap of clinking leather and metal.");
		
		output("\n\n\"<i>No, [pc.Master] please no, I don't want that, I've learnt my lesson, I'll be good for now on, punish me any way you see fit but not that...</i>\"");
		...
		if(flags["COC.KELLY_BONUS_TIT_ROWS"] > 0) output("  Just as well there were four of them, eh?");
		output("</i>\" You step back to admire the effect.  Kelly stares back at you behind her leather straps and ball gag, her tasselled boobs forced outwards by her arms bound behind her back.  You laugh and stroke her face; she closes her eyes and bends into it, enjoying the simple motion of her [pc.Master]'s hand.");
		...
		//PC not rider: 
			...
			output("You grab hold of the reigns and stride off; after a short jerk Kelly follows behind you.  \"<i>We're heading to the desert,</i>\" you announce.  \"<i>I think you can maybe guess where.  Remember; flick those legs high, prance a little, make those tassels whirl.  You represent your [pc.Master], and you don't want " + pc.mf("him","her") + " to look bad, do you?</i>\"  There's a muffled whimper in response.");
			...
			output("\n\nUpon the close streets though everyone stares; even for as liberal a place as Tel'Adre the sight of a " + pc.race() + " leading a practically naked bound and gagged centauress draws a slight hush and open mouths.  The centaurs you canter past in particular look absolutely gobsmacked; by turns unbelieving, disgusted, or shocked into laughter.  Nobody attempts to stop you though and you affect to be completely unaware of the attention; you after all are a champion upon an important errand.  But you know the attention and pure humiliation is soaking into Kelly when she makes a sobbing, muffled moan.");
		...
		output("\n\nThis final humiliation has her breathing wetly and volubly into her gag by the time you are back to camp, somewhere between pants of desire and sobs of misery.  You strip her of her gear and to show you can be a kind [pc.Master] too you let her suck you off, the hard, shame-tinted memory of the last couple of hours suffixed by the joy of what she does best.");
		//[Not centaur: 
		if(!pc.isTaur()) output("   You sigh and enjoy the sight of her pretty head bobbing over your groin, your [pc.cock " + x + "] encapsulated in soft shifting blankets of wet mouth flesh, your ears filled with her slurps and muffled sighs.  You are quickly pushed towards a new high by the worshipful treatment; you give her what she wants and cum in her mouth, letting her savor your texture and flavor; she closes her eyes and swallows, orgasm shuddering through her as it hits her stomach.  You smile and hold her face as she basks in an afterglow earned from pure debasement.");
		else output("  You sigh and enjoy your [pc.cock " + x + "] encapsulated in soft shifting blankets of wet mouth flesh, your ears filled with her slurps and muffled sighs.  You are quickly pushed towards a new high by the worshipful treatment; you give her what she wants and cum in her mouth, letting her savor your texture and flavor; she closes her eyes and swallows, orgasm shuddering through her as it hits her stomach.  You smile, turn and hold her face as she basks in an afterglow earned from pure debasement.");
	...
	//Second+ repeat: 
		...
		if(pc.tallness < 78 && !pc.isTaur())
		{
			...
			output("\n\nUpon the close streets though everyone stares; even for as liberal a place as Tel'Adre the sight of a " + pc.race() + " riding a practically naked bound and gagged centauress draws a slight hush and open mouths, even among the ones who have already seen this display.  The centaur you canter past in particular look absolutely gobsmacked; by turns unbelieving, disgusted, or shocked into laughter.  Nobody attempts to stop you though and you affect to be completely unaware of the attention; you after all are a champion upon an important errand.  But you know the attention and pure humiliation are soaking into Kelly when she makes a sobbing, muffled moan.");
		...
		//PC not rider: 
			...
			output("\n\nUpon the close streets though everyone stares; even for as liberal a place as Tel'Adre the sight of a " + pc.race() + " leading a practically naked bound and gagged centauress around like a show horse draws a slight hush and open mouths, even among the ones who have already seen this display.  The centaur you canter past in particular look absolutely gobsmacked; by turns unbelieving, disgusted, or shocked into laughter.  Nobody attempts to stop you though and you affect to be completely unaware of the attention; you after all are a champion upon an important errand.  But you know the attention and pure humiliation are soaking into Kelly when she makes a sobbing, muffled moan.");
		...
		//All go to: 
		output("\n\nThis final humiliation has her breathing wetly and volubly into her gag by the time you are back to camp, somewhere between pants of desire and sobs of misery.  You strip her of her gear, throw the butt plug away and to show you can be a kind [pc.Master] too you let her suck you off, the hard, shame-tinted memory of the last couple of hours suffixed by the joy of what she does best.");
		//[Not centaur: 
		if(!pc.isTaur()) output("  You sigh and enjoy the sight of her pretty head bobbing over your groin, your [pc.cock " + x + "] encapsulated in soft shifting blankets of wet mouth flesh, your ears filled with her slurps and muffled sighs.  You are quickly pushed towards a new high by the worshipful treatment; you give her what she wants and cum in her mouth, letting her savor your texture and flavor; she closes her eyes and swallows, orgasm shuddering through her as it hits her stomach.  You smile and hold her face as she basks in an afterglow earned from pure debasement.");
		//[Centaur:]
		else output("  You sigh and enjoy your [pc.cock " + x + "] encapsulated in soft shifting blankets of wet mouth flesh, your ears filled with her slurps and muffled sighs.  You are quickly pushed towards a new high by the worshipful treatment; you give her what she wants and cum in her mouth, letting her savor your texture and flavor; she closes her eyes and swallows, orgasm shuddering through her as it hits her stomach.  You smile, turn and hold her face as she basks in an afterglow earned from pure debasement.");
...
private function takeKellysVirginity():void {
	...
	output("You tell Kelly that you intend to come claim the prize you broke her for - her virgin ");
	if(kelt.vaginas[0].type == GLOBAL.TYPE_EQUINE) output("horse-");
	output("cunt.");
	...
	if(kelt.vaginas[0].type == GLOBAL.TYPE_EQUINE) output("lust-swollen, black-lipped fuck-hole");
	else output("blooming, pink-tinged pussy");
	output(" to your roving eyes.  She asks, \"<i>Is th-th-this good, [pc.Master]?</i>\"");
	...
	if(kelt.vaginas[0].type == GLOBAL.TYPE_EQUINE) output(", equine");
	output(" cunt until her hymen is easily visible.  You rub every sensitive fold that you can get to");
	
	if(kelt.vaginas[0].type == GLOBAL.TYPE_EQUINE) output(", taking time to squeeze her plush, onyx vulva teasingly.  Kelly whinnies in response, overtaken by her potent equine instincts.");
	else output(", taking time to fondle her slick labia.  Kelly moans in response, overtaken by the sensations you're forcing through her.");
	...
	if(!pc.isTaur()) output("get down behind her and line your [pc.cock " + x + "] with her dribbling lips.");
	else output("lie down behind her, sliding your hindlegs behind hers in order to properly align your [pc.cock " + x + "] with her dribbling lips.");
	output("  The first contact of [pc.cockHead " + x + "] on her feminine mound sends an electric current of pleasure through your member, one that has you immediately and heedlessly pushing forward, sliding your full, sensitive phallus as deeply into that slick opening as you can.  That turns out not to be very far, as you almost immediately hit her virginal hymen.  It blocks off the rest of that hot, dripping hole almost spitefully.");
	
	output("\n\nYou squeeze Kelly's ");
	if(pc.isTaur()) output("breast as you snuggle up behind her, centaur to centaur,");
	else output("ass");
	output(" and laugh, \"<i>Here it comes, you eager whore!</i>\"  Then, you spear through her entrance with one powerful thrust.  In that second, her pussy somehow gets even hotter and warmer around your girth, and you sink yourself as deeply as you can");
	if(pc.cockVolume(x) <= kelt.vaginalCapacity())
	{
		output(", nestling your [sheath] up against her ");
		if(kelt.vaginas[0].type == GLOBAL.TYPE_EQUINE) output("plump, equine cunt-lips");
		else output("slippery, pink cunt-lips");
	}
	else output(", pressing your [pc.cockHead " + x + "] up against the barrier her cervix makes");
	output(".  A pinkish froth oozes out around you, tinted by the few drops of blood that claiming her generated.  Kelly made a pained face as you took her, but now, she's back to panting and moaning like a whore.");
	pc.cockChange();
	
	output("\n\nYou pull on her tail as you begin to slide in and out, asking, \"<i>Did you like that, slut?  Does your cunt like being broken in by a real " + pc.mf("man","breeder") + "'s cock?</i>\"");
	
	output("\n\nKelly moans, \"<i>Y-yes!  My... pussy...  My pussy feels so good!  It's so good to be yours, [pc.Master]!</i>\"  Her eyes cross slightly as her pussy begins to squeeze down around you, driven by instinctual desires the lust-crazed centauress barely understands.  She's been so conditioned to obey that when her body tells her to do something (like milk your [pc.cock " + x + "]), she doesn't question - she simply, deliciously complies, her pussy growing ever-wetter from her submissiveness.");
	
	output("\n\nWith the pain of her deflowering little more than a memory, Kelly is free to give in to the pleasure you're forcing through her powerful, centaur frame.  She cums almost immediately, moist fluids spattering your groin, the fruit of her passion mixed with all the wetness-enhancing drugs you force-fed her when you created her.  Her channel's gentle milking motions accelerate, squeezing tighter and tighter around you, more forcefully.  It feels good, but you endure, sawing your [pc.hips] in and out of her again and again, determined to enjoy this pleasure as long as you can.");
	
	output("\n\nKelly squirms and moans, shuddering again and again as you drive her to orgasm after orgasm.  After a few climaxes as juicy as the first, Kelly's eyes roll back and her tongue lolls out. She stops talking, simply grunting every time you slam against her and moaning with each draw back.  You fuck her until you're sure her body will never forget the feeling of wrapping around your [pc.cock " + x + "], and then you fuck her some more.  You fuck her until her human half flops down on the ground like the rest of her, limply spasming while her well-plowed vagina nervelessly spasms around you.");
	...
	if(pc.cumQ() >= 500) {
		output("  You even have enough jism to make sure that some is squirting into her womb and drizzling out of her entrance, mixing with her clear fluids to make a runny, off-wet mess down her rump.");
		if(pc.cumQ() >= 1500) output("  In no time flat, her belly has begun to round slightly, puffed up with spunk.");
		if(pc.cumQ() >= 3500) output("  That doesn't stop you from seeding it even further, pumping her poor horse-gut until it jiggles and jostles with your spermy cargo.  There's no way she'll forget this.");
	}
	
	output("\n\nYou pull out with a self-satisfied sigh, admiring ");
	if(kelt.vaginas[0].type == 0) output("the redness of her well-used cunt.");
	else output("the shining juices that drip from her oh-so-equine entrance, gilding the smooth onyx lips with a slimey sheen.");
	output("  Kelly has passed out in a puddle of her own making, and you leave her like that, getting dressed without a backwards glance.");
	kellyPreggers();
	pc.orgasm();
...
private function giveKellyAPepper():void {
	clearOutput();
	//First: 
	if(Flag("COC.KELLY_TIMES_PEPPERED") == 0) {
		output("You instruct Kelly to open wide; you've got something that will allow her to serve her [pc.Master] even better.  You imagine she once saw Whitney farm these things, but if your centauress slave knows its properties she doesn't show it; dutifully she opens her mouth, making an \"<i>aaaaaaah</i>\" noise until you pop the glossy red fruit onto her tongue.  You stand back and watch as she munches thoughtfully and then swallows.");
		
		output("\n\n\"<i>S'quite nice.  Spicy.  I don't know what it's supposed to do th-oh.  Ohhhh...</i>\" She closes her eyes and moans as flesh burgeons and balloons underneath her tits.  Nubs form in the softness and then darken, leaving her finally with a second row of breasts- smaller than her first but still at least C cups.  You smirk and take a few steps back and forth to properly take in her new curves; the soft obscenity of it is perfect for a slaveslut like her, and [pc.eachCock] begins to harden at the thought of sliding it between those four pillows.");
		...
		output("\n\n\"<i>[pc.Master], please...</i>\" she moans.  You draw into her, cup one of her new breasts and gently play with a nipple until you can hear the steady drip of her pussy juices on the dirt behind her, before saying maybe later; she needs to get used to her new body and you don't have time to satisfy the needs of a slut right now.  You turn and leave with a satisfied smile, the sound of the fitful hooves and whimper of a centaur impossibly turned on and incapable of doing anything about it in your ears.");
		//[Second row of breasts added]
		//[Kelta 15% more likely to conceive for 2 days]
		kelt.createBreastRow();
		kelt.breastRows[1].breastRatingRaw = 5;
		flags["COC.KELLY_HEAT_TIME"] = timeAsStamp + 48 * 60;
	}
	//Repeat:
	else {
		output("You grin and dangle the pepper in front of Kelly's face wordlessly.  The centauress knows what's coming and color spreads across her cheeks; however, she still opens wide and accepts the \"<i>gift</i>\", chewing slowly and swallowing.");
		//If no second row:
		if(kelt.breastRows.length == 1) {
			output("\n\nShe closes her eyes and moans as flesh burgeons and balloons underneath her tits.  Nubs form in the softness and then darken, leaving her finally with a second row of breasts- smaller than her first but still at least C cups.  You smirk and take a few steps back and forth to properly take in her new curves; the soft obscenity of it is perfect for a slaveslut like her and [pc.eachCock] begins to harden at the thought of sinking it between those four pillows.");
...
private function kellyTitJob():void {
	...
	//Dick >17 inches, Kelly has four breasts:
		...
		output("\n\n\"<i>So that's why you made me grow four of them!</i>\" Of course, you say; your slaves have to be suitably shaped to be able to properly service a [pc.Master] as mighty as you.  Now, you say as you hold her by the shoulders and begin to move your [pc.cock " + x + "] up and down her long, twin valley, hush and properly attend to your cumslut duties.");
		output("\n\nYou sigh as Kelly obediently clasps her top breasts and squashes her softness into your length, encapsulating it in her warm, yielding flesh.  You clasp her bottom boobs and do the same; whilst not as big as her first row, there is still more than enough plush fatness there to wrap your girth in warmth.  Pre leaks from your tip to the shifting, pillowy pleasure inundating your hard cock, slicking Kelly's tender skin with your scent; you slowly pull your [pc.cock " + x + "] all the way down and then up again, ensuring the full valley is given the lubrication you need to pick up the pace.  The centauress coos as you grip her harder and begin to fuck her pillows, her eyes closed as she is swallowed by your presence and feral smell, each second you spend using her like this making her more and more sensitive and pliant to your touch.");
...
private function kellyPreggers():void {
	if (flags["COC.KELLY_PREGNANCY_INCUBATION"] != undefined) return;
	var x:int = Math.round(pc.cumQ() / 20);
	if (x > 80) x = 80;
	if (flags["COC.KELLY_HEAT_TIME"] > timeAsStamp) x += 15;
	if (rand(100) + 1 <= 80) {
		//pregnancy.knockUpForce(PregnancyStore.PREGNANCY_PLAYER, PregnancyStore.INCUBATION_CENTAUR - 84);
		flags["COC.KELLY_PREGNANCY_INCUBATION"] = timeAsStamp + 336 * 60;
...
private function kellyPregSex():void {
	clearOutput();
	var x:int;
	if(kelt.vaginas[0].type == GLOBAL.TYPE_EQUINE) x = pc.biggestCockIndex();
	else x = pc.cockThatFits(kelt.vaginalCapacity());
	if(x < 0) x = pc.smallestCockIndex();
	...
	output("\n\nYou remove your [pc.gear] and watch your [pc.cocksLight] grow into a rock-hard erect state under the aphrodisiac scent; you're ready to stuff the breeding mare before you.  Driven mad with lust, you jump the centaur's behind and slap her angrily, making her moan and snort in submissive pleasure.  You can't take it anymore and with feverish excitement, you grab hold of [pc.oneCock] and plunge it deep into Kelly's dark recesses.");
	pc.cockChange();
...
public function kellyPopsOutARunt():void {
	output("<b><u>As you visit the barn where your centaur slave usually resides, you see something unusual...</u></b>");
	...
	output("\n\n\"<i>[pc.name]! You're here! Say hi to your new kid!</i>\"");
	...
	if (int(flags["COC.KELLY_KIDS"]) == 0) flags["COC.KELLY_FIRST_KID_GENDER"] = gender;
	IncrementFlag("COC.KELLY_KIDS");
	if (gender == 1) IncrementFlag("COC.KELLY_KIDS_MALE");
	flags["COC.KELLY_PREGNANCY_INCUBATION"] = undefined;
	
	addNextButton(approachKelly);
}
...
private function talkNHandToKelly():void {
	...
	if(flags["COC.KELLY_TALK_N_HAND_TIMES"] == 0) {
		output("\"<i>Talk?  Oh, that's the least interesting thing I can do with my mouth, [pc.Master],</i>\" Kelly purrs.  That may be, you say, but she can amuse you with her hands whilst she amuses you with her voice.  The centaur gets the picture and grins.  ");
		//Not Centaur: 
		if(!pc.isTaur()) {
			output("She helps you take off the lower half of your [pc.gear] as you set yourself down and then wraps her warm hand around ");
			if(pc.cockTotal() == 1) output("[pc.oneCock], gently tugging it.  It quickly hardens with the pleasing friction she coils around it.");
			else output("first your [pc.cock], then your [cock 2], gently tugging until they are both hard from the pleasing friction she coils around them.");
		}
		//Centaur:
		else {
			output("You leisurely clop yourself around so your back end is facing her and sigh as ");
			if(pc.cockTotal() == 1) output("a warm hands wraps itself around your [pc.cock] and then begins to gently tug, quickly hardening it with pleasing coils of friction.");
			else output("a warm hand wraps around first your [pc.cock] and then your [cock 2] before beginning to gently tug, both quickly hardening from the pleasing coils of friction.");
		}
	}
	//Repeat: 
	else {
		output("You tell her you'd like to... talk.");
		output("\n\nGrinning, your centaur slave ");
		if(!pc.isTaur()) {
			output("slides off the lower half of your [pc.gear] as you set yourself down and then grasps your cock");
			if(pc.cockTotal() > 1) output("s");
			output(" with her soft hands.  You sigh as she begins to gently pump, making ");
			if(pc.cockTotal() > 1) output("them");
			else output("it");
			output(" harden quickly.");
		}
		else {
			output("trots around behind you.  You sigh as ");
			if(pc.cockTotal() == 1) output("a warm hands wraps itself around your [pc.cock] and then begins to gently tug, quickly hardening it with pleasing coils of friction.");
			else output("a warm hand wraps around first your [pc.cock] and then your [cock 2] before beginning to gently tug, both quickly hardening from the pleasing coils of friction.");
		}
	}
	flags["COC.KELLY_TALK_N_HAND_TIMES"]++;
	//(Randomly generated)
	var temp:int = rand(5);
	//1.
	if(temp == 0) {
		output("\n\nYou ask what happened to the wives she mentioned she had.");
		output("\n\n\"<i>Who?</i>\" Kelly says, stopping her pumping for a moment.  \"<i>Oh, those two.  I wouldn't worry about them, [pc.Master].  I never told them where I worked, because I never wanted them to know I worked for a woman.  Funny, isn't it.");
		if(!pc.hasVagina()) {
			//Male: 
			output("  Now I </i>do<i> work for a man, and I spend most days polishing his [pc.cock " + pc.biggestCockIndex()+ "]!");
		}
		//Futa:
		else output("  Now I am a woman, I work for another woman, and I spend most days polishing her [pc.cock " + pc.biggestCockIndex()+ "]!");
		output("</i>\"  She laughs musically as she continues to rub your straining meat.  <i>\"They'll be alright, [pc.Master].  They were never bound to me as tightly as I am to you, I was never as strong or as clever as you- and I was always waiting for someone to make me into a submissive cockslave, I just never realized it.  They'll probably shake out of it and go their separate ways eventually, find new lives.  Unless...</i>\"  She pauses again. \"<i>Unless you want me to go find them, [pc.Master]?  Make them part of your glorious harem instead?  Make them see how inferior a [pc.Master] I really was?</i>\"  You tell her to keep pumping, close your eyes and imagine not one but three mare sluts serving you: imagining three plump, hungry mouths slathering up and down your [pc.cock " + pc.biggestCockIndex()+ "] at the same time, ");
		//1< cock:
		if(pc.cockTotal() > 1) output("imagining thrusting every single one of your cocks into a wet, willing hole, ");
		//vagina:
		else if(pc.hasVagina()) output("imagining one of them tongue-fucking your [pc.vagina] as you buck into another's tight cunt, ");
	...
	//2. 
		...
		output("\n\n\"<i>So,</i>\" says Kelly with feigned casualness as she masturbates you.  \"<i>When can I move in with you, [pc.Master]?  I don't mind if you've got other sluts, you know.  I know someone as virile as you needs many.</i>\"  You sigh, enjoying the coils of pleasure around your [pc.cocks], receding then tightening, wondering how you'll field this one.  Your rising seed pushes open a black doorway in your mind and you smirk.");
	...
	//5.
		...
		if (!whitneyCorrupt())
		{
			...
			output(".");
			output("\n\n\"<i>But you'll show her one day, right [pc.Master]?  Oh, what I'd give to see her grovelling in the dirt and licking your [pc.feet].  You'll make her do that one day, won't you [pc.Master]?  On that day... you'll remember to bring me along, right?</i>\"  She looks up at you, grinning, malicious fire in her green eyes.  You feel dizzy with your intense orgasm and are slightly taken aback there is still so much spite left in your cute pony cock-sleeve.  When you've recovered, you say maybe one day, but for now she's not finished with her job.");
			
			output("\n\n\"<i>Oh!  Sorry [pc.Master], please excuse me.</i>\"  She bends in and sends her tongue looping and curling deliciously over your aching, oozing [pc.cockHead] until it is gleaming.  You redress and give her a harder slap than usual on the ass as you leave.");
		}
		else
		{
			output("\n\n“<i>[pc.Master],</i>” trills Kelly, drawing the last syllable out, her tone one of mocking pique underpinned by dark glee. “<i>You promised me you would let me be there when you made Whitney yours!</i>”");

			output("\n\nYou reply stiffly that you promised no such thing. You want to add more but you are lost in the shift of her hand around your [pc.cock], making both your sex and your mind feel red, sweet and full.");

			output("\n\n“<i>You are so mean, [pc.Master]. I guess I will just have to imagine what it was like.</i>” Her voice has lowered to a playful husk. “<i>Did she resist? Did she spend days and weeks pretending she didn’t feel the creep of your influence, that she didn’t feel her heart quicken when you gave her orders? That she didn’t even notice you or the way you had taken possession of her home? I bet she did – stuck up bitch spent her whole life acting that way. Bet it became a struggle when heat began to run to her pussy every time you passed her.</i>”");
			...
			output(". The smooth woggle of flesh around your [pc.cock] coils you expertly through your exultant orgasm, wringing out every last drop before plump, wet lips come to rest on your [pc.cockHeadBiggest], kissing and licking it quite clean. Kelly looks extremely pleased with herself, glancing up at you once she’s done.");
			...
			output("\n\n“<i>Of course,</i>” replies Kelly innocently. “<i>Right hands are important things to have. I am certain that Mistress Whitney makes a very effective right hand for you, [pc.Master].</i>” You redress and give her a harder slap than usual on the ass as you leave.");
		}
	}
	pc.orgasm();
...
private function rewardKelly():void {
	flags["COC.KELLY_REWARD_COOLDOWN"] = 1;
	clearOutput();
	//First time: 
	if(Flag("COC.KELLY_TIMES_REWARDED") == 0) {
		output("There seems to be an added bounce to Kelly's canter today.  She prances a bit playfully in front of you, grinning, whipping her demonic tail so that the high, sweet smell of her gushing pussy is in the air.  You realize she is calling your attention to her backside, which is unmarked.");
		output("\n\n\"<i>I've been good, [pc.Master].  So very, very... good.</i>\"  Color is high in her cheeks, and although she's smiling proudly there is real desperation burning in her green eyes as she gazes at you.  \"<i>We'll fuck now, right?  It's so hard being good.  After a while I can't think of anything but your cock and how wonderful it tastes and feels.  Last night it was so bad I blew my own fingers thinking about you...</i>\"  She demonstrates graphically.  \"<i>But I didn't cum.  I've learnt my lesson [pc.Master], now please fuck me!</i>\"");
		output("\n\nIt's difficult not to laugh at this earnest outpouring, and yet you can't help but feel a fond glow for your centaur slave, who has fought against her slutty nature to keep herself locked into a constant state of burning arousal just to please you.  Maybe it's time for a bit of kind [pc.Master]-ing and provide her with some sort of reward, aside from fucking her rotten.");
	}
	//Repeat:
	else {
		output("Kelly holds her head up proudly as you inspect her, running your hand slowly over her backside.   It's nice and smooth.  You smile at her benevolently and tell her that since she's been such a good cumslut, you're going to reward her.  She claps excitedly.");
		output("  \"<i>Hurray!</i>\"");
	}
	Flag("COC.KELLY_TIMES_REWARDED", 1, true);
	...
	//[chestnut brown/sable black/garish purple/bright pink/slutty blonde) 
	if(pc.cockThatFits(kelt.vaginalCapacity()) >= 0 && pc.hasCock()) addButton(0,"Applesauce",giveKellyAppleSauce);
	if (pc.hasItem(new CoCDyeBlack()) && kelt.hairColor != "sable black") addButton(1, "Black Dye", dyeKellysBitchAssHair, new CoCDyeBlack());
	if (pc.hasItem(new CoCDyeBlond()) && kelt.hairColor != "slutty blonde") addButton(2, "Blond Dye", dyeKellysBitchAssHair, new CoCDyeBlond());
	if (pc.hasItem(new CoCDyePurple()) && kelt.hairColor != "garish purple") addButton(3, "Purple Dye", dyeKellysBitchAssHair, new CoCDyePurple());
	if (pc.hasItem(new CoCDyePink()) && kelt.hairColor != "bright pink") addButton(4, "Pink Dye", dyeKellysBitchAssHair, new CoCDyePink());
	if (pc.hasItem(new CoCDyeBrown()) && kelt.hairColor != "chestnut brown") addButton(5, "Brown Dye", dyeKellysBitchAssHair, new CoCDyeBrown());
...
private function dyeKellysBitchAssHair(color:ItemSlotClass):void {
	clearOutput();
	output("You tell her you've brought her a gift as you rummage around in your pockets.  Kelly looks apprehensive but pleasant surprise forms on her face when she catches the small vial of dye you throw at her.");
	output("\n\n\"<i>Oh wow, thanks [pc.Master]! It's been ages since I did my hair.");
	
	if(IncrementFlag("COC.KELLY_TIMES_DIED_HAIR") == 1) {
		output("</i>\"\n\nShe stops and thinks.  \"<i>In fact, I don't think I've ever done my hair.  ");
	}
	output("This'll be so much fun!</i>\"  You walk over to the barn with her and pour some water into a wide bucket.  She lets down her braid and you spend a pleasant quarter of an hour or so helping her work the sharp-smelling substance into her long, luscious hair.  When you're done, you step back to admire the effect.  Amazingly the color is not only sinking into her hair but also into her tail, her demon spade slowly bleaching with the hue you chose.");
	
	//Black dye:
	if(color is CoCDyeBlack) {
		output("\n\nKelly whips her jet black hair to look at it, trotting back and forth and considering, swishing her equally black tail.\n\n\"<i>Mmm.  Not sure how much I like this, [pc.Master],</i>\" she says eventually.  \"<i>It's very... severe, isn't it?</i>\"  You say you picked it because you think it will contrast well with the substance she's most often covered in.  Kelly laughs fondly as she begins the long process of retying her braid.\n\n\"<i>Oh, [pc.Master].  Always thinking two moves ahead.  Thank you for your reward!</i>\"");
		kelt.hairColor = "sable black";
		pc.destroyItem(color);
	}
	//Blonde dye: 
	else if(color is CoCDyeBlond) {
		...
		output("\n\n\"<i>This feels... right, [pc.Master].  Very right.</i>\"  She looks at you lustfully.  \"<i>Wanna see if we have more fun?</i>\"  You say you'll definitely be back later to test that theory out.");
		...
		pc.destroyItem(color);
		kelt.hairColor = "slutty blonde";
	}
	//Purple dye: 
	else if(color is CoCDyePurple) {
		...
		output("\n\n\"<i>Like the goblins, I guess?  I don't know how much I like this color, [pc.Master].</i>\"  You say it'll serve as a constant reminder to her that she's a cock hungry breeding machine.  Kelly nods thoughtfully as she begins the long process of retying her braid.");
		output("\n\n\"<i>You're right [pc.Master], I would hate to ever forget that.  Thank you for your reward!</i>\"");
		kelt.hairColor = "garish purple";
		pc.destroyItem(color);
	}
	//Pink dye:
	else if(color is CoCDyePink) {
		...
		output("\n\n\"<i>Thank you for your reward, [pc.Master]!</i>\"");
		pc.destroyItem(color);
		kelt.hairColor = "bright pink";
	}
	//Brown dye: Kelly smiles as she examines the regained chestnut brown color of her hair and tail.
	else if(color is CoCDyeBrown) {
		...
		output("\n\n\"<i>Of course, [pc.Master].  Thank you for your reward!</i>\"");
		kelt.hairColor = "chestnut brown";
		pc.destroyItem(color);
	}
...
private function giveKellyAppleSauce():void {
	clearOutput();
	//First time:
	if(Flag("COC.KELLY_TIMES_APPLESAUCED") == 0) {
		output("You tap your chin idly and ask Kelly what she likes eating.  Aside from dick, you add, rolling your eyes as she opens her mouth eagerly.");
		...
		output("\n\nAt least your jerk off-ing is put to good use these days, you say kindly.  \"<i>That's nice of you to say, [pc.Master],</i>\" replies the centaur solemnly.  She's wringing her hands, her thoughts elsewhere.");
		if (!whitneyCorrupt()) output(" \"<i>I'd like to go and check on it, but... I don't think Whitney likes me much anymore.  I'd hate to cause more trouble than I already have.</i>\"");
		else output("“<i>I'd like to go and check on it, but... I’d hate to get into Mistress Whitney’s bad books.  I’ve caused enough trouble already.</i>”");
		output(" You 'pfft' and wave your hand airily to show what YOU think of Whitney.  You tell her to stay put - as her treat, you'll go check on the orchard and bring back some apples for her, if there are any.");
		output("\n\n\"<i>You will?  I- well, thank you so much [pc.Master], but you'll be careful, won't you?</i>\" she says fretfully, still wringing her hands.  You go into her barn, grab a metal bowl and then stride off gallantly, as if setting off to take on Lethice herself, when in fact your epic quest involves jumping over a fence and walking 200 yards through some light woodland.");
		...
		if(!pc.isTaur()) {
			output("You remove the bottom half of your [pc.gear] and grip your [pc.cock].  You close your eyes, begin to stroke yourself and sigh as you think of Kelly- her fine, bountiful curves, her cute, dimpling face, how she unconsciously licks her cock-sucking lips when she sees you coming, how she begs for it, how sweetly she moans when you thrust deep into her tight, wet holes... your body clenches as you orgasm, spurting streamer after streamer of spunk into the apple mash.");
			if(pc.hasVagina()) output("\n\nYour [pc.vagina] quivers and clenches, and gasping with the dual orgasm which is clutching your body you stand over the bowl and let some of your femcum drip into it.  Taste my summer flavor Kelly, you think wildly, and laugh with a slightly manic edge.");
			//[High cum:]
			if(pc.cumQ() >= 750) output("  The bowl quickly begins to brim as your [pc.cock] torrents out cum, and eventually you have to turn away, grunting out the last jets of filth into the undergrowth.");
		}
		//[Centaur:]
		else {
			output("You position your lower half over the bowl.  Your [pc.cock] is already semi-turgid with the idea of what you're going to do, and it begins to bulge as you close your eyes, sigh and think of Kelly- her fine, bountiful curves, her cute, dimpling face, how she unconsciously licks her cock-sucking lips when she sees you coming, how she begs for it, how sweetly she moans when you thrust deep into her tight, wet holes... your body clenches as you orgasm, spurting streamer after streamer of spunk into the apple mash.");
			if(pc.hasVagina()) output("\n\nYour [pc.vagina] quivers and clenches, and gasping with the dual orgasm which is clutching your body you stand over the bowl and let some of your femcum drip into it.  Taste my summer flavor Kelly, you think wildly, and laugh with a slightly manic edge.");
			//[High cum:
			if(pc.cumQ() >= 750) output("  Your [pc.cock] torrents out cum, and eventually you have to clop away, grunting out the last jets of filth onto the ground to avoid overflowing the bowl.");
		}
		
		output("\n\nYou exhale, set yourself down and enjoy the afterglow at leisure in the peaceful woodland glade, taking the time to recover as you work your musky, gooey sex into the mixture using the stone.  Eventually you're left with a gooey, yellow green mash.  Kelly green, you think, and laugh.  The mixed aroma of sex and fructose hovers in your nose.  Satisfied, you pick yourself up and take the bowl, brimming with apple sauce a la [pc.name], back to the fields.  Kelly is upon you the moment you come back in sight, whinnying with concern.");
		...
		output("\n\n\"<i>Oh, thank you so much, [pc.Master]!  I could blow you a thousand times and it wouldn't be enough to repay you, I'm not worthy of a [pc.Master] who is as brave and noble as you, I- wait, what's this?</i>\"  She looks at the gloop in the bowl you've given her in bewilderment.  Apples, you say, grinning knowingly.  \"<i>But it smells more like... like... ooh.</i>\"  Her pupils dilate and her mouth opens, saliva forming in her mouth.  Your grin widening, you slide an arm around her human waist, drinking in her warm, horny scent as you lead her unresistingly to the barn.");
	}
	//Repeat:
	else {
		...
		if(flags["COC.KELLY_KIDS"] > 1) output("  Somebody has even cleared an oblong patch of earth alongside the trees- a piece of card stuck into it is scrawled with a childish hand which declares it to be “CAROTS”.  You marvel at it all.  Who would have imagined your lust for horse pussy would have turned a pointless asshole into a loving mother who gardens with her children?  Really, you deserve a medal for your efforts.");
		...
		if(!pc.isTaur()) {
			output("grasp your semi-turgid cock and drift into a fond reverie of Kelly- the way her boobs bounce when she canters towards you, the way her green eyes burn when they fall upon your [pc.eachCock], the way she gasps when you slap her ass, how sweetly she moans when you thrust deep into her tight, wet holes, the galaxy of filthy things you still have left to do to her....  Your body clenches as you orgasm, spurting streamer after streamer of spunk into the apple mash.");
			if(pc.hasVagina()) output("  Your [pc.vagina] quivers and clenches, and once again, gasping with the dual orgasm which is clutching your body, you stand over the bowl and let some of your femcum drip into it.");
			//[High cum:
			if(pc.cumQ() >= 750) output("  The bowl quickly begins to brim as your [pc.cock] torrents out cum, and eventually you have to turn away, grunting out the last jets of filth into the undergrowth.");
		}
		//[Centaur:
		else {
			output("position your lower half over the bowl, your [pc.cock] already semi-turgid as you drift into a fond reverie of Kelly- the way her boobs bounce when she canters towards you, the way her green eyes burn when they fall upon your [pc.eachCock], how sweetly she moans when you thrust deep into her tight, wet holes, the galaxy of filthy things you still have left to do to her....  Your body clenches as you orgasm, spurting streamer after streamer of spunk into the apple mash.");
			if(pc.hasVagina()) output("  Your [pc.vagina] quivers and clenches, and once again, gasping with the dual orgasm which is clutching your body, you stand over the bowl and let some of your femcum drip into it.");
			//High cum:
			if(pc.cumQ() >= 750) output("  Your [pc.cock] torrents out cum, and eventually you have to clop away, grunting out the last jets of filth onto the ground to avoid overflowing the bowl.");
		}
		output("\n\nYou exhale, set yourself down and enjoy the afterglow at leisure in the peaceful woodland glade, taking the time to recover as you work your musky, gooey sex into the mixture using the stone.  Eventually you're left with a gooey, yellow green mash- kelly green.  The mixed aroma of sex and fructose hovers in your nose as you pick yourself up and take the bowl, brimming with apple sauce a la [pc.name], back to the fields.  There's a wide and knowing grin on Kelly's face when you swagger back up to her.");
			
		output("\n\n\"<i>What have you made, [pc.Master]?  Something for the Tel'Adre fair?");
		if(silly) output("  Parsley, sage, rosemary and c...");
		output("  Ooh!</i>\"  You slap her ass and grab her around her supple, human waist as you pass her.  Laughing, you walk into the barn together.");
	}
	//[merge]
	var x:int = pc.cockThatFits(kelt.vaginalCapacity());
	var y:int = pc.cockThatFits2(kelt.analCapacity());
	//Not Centaur: 
	if(!pc.isTaur()) {
		output("\n\nThe mere smell of the apple sauce has got Kelly's vagina dribbling, and in the close, warm environment of the barn the cloying, sweet smell radiating off her has got [pc.eachCock] rock hard again, and your arousal urges you to work quickly.  You lead her to a shelf roughly level with her collarbone and place the bowl of apples sauce on it.  Transfixed by it, swallowing audibly, the centaur reaches for it automatically.  You catch her hand.");
		output("\n\n\"<i>No.  Put them here.</i>\"  You set each one on either side of the bowl so she is clasping it.  \"<i>Don't lift it until you have to.  That's it...</i>\"  You watch, pulling off piece after piece of your [pc.gear], as she dips her head into the bowl, scoops up a long lap of the green mash with her tongue and guides it into her wet mouth.  Her throat works and she closes her eyes.  Behind her, the intermittent dripping of her femcum on the straw turns into a steady trickle and you shake off the last of your underclothes in a hurry, stride over to her hindquarters and push your [pc.cock " + x + "] against her sopping pussy gently, gripping her " + kelt.hairColor + " tail to her side so she doesn't inadvertently flick you with it.");
		output("\n\nShe is radiating heat and her wet depths suck at your [pc.cockHead " + x + "] deliciously.  Still, you hold back on your pulsing need to bury your cock into her, pressing at her entrance softly until you see her head go down again, and the sloppy sound of a centaur trying to draw as much apple sauce into her mouth as she can reaches your ears.  With a contented sigh, you slide your [pc.cock " + x + "] into her, exulting in the hot slickness which envelops your sex bit by bit.");
		if(pc.cocks[x].cockThickness >= 4) output("  Your girth spreads her fuck tunnel wide and you grunt at the delicious tightness of it as you pack her full of cock.");
		
		output("\n\n\"<i>Oh [pc.Master]...</i>\" Kelly groans, arching her head back from her feed.  A series of clenches grip your [pc.cock] and a sudden gush of fluid warms your ");
		if(pc.balls > 0) output("[pc.balls]");
		else output("thighs");
		output(".  Did she cum already?  It's fortunate you've already taken the edge off, because you're not sure how long you would otherwise be able to last against her impossibly turned on, sucking, clinching pussy.  As it is... you grip her huge, muscled backside and pull yourself almost all the way out of her fleshy pocket then spear your way in again, steadily in and then out, working your way into a slow, sensual rhythm.  Ahead of you Kelly's head sinks and bobs, and your ears are full of her wet, muffled chews, gulps and moans as she gorges herself on your sweet, sticky sex whilst you gently fuck her.");
		
		//1 < cock:
		if(y >= 0) {
			output("\n\nYour eyes fasten upon her tight, puckered asshole as lust grips you closer, a tiny puddle of human pink buried in her heaving brown horse flesh.  Your breath coming heavily now you withdraw your [pc.cock " + x + "] from her wet softness, dripping with her lubrication.  Quickly you push it against her puckered anus whilst lining up your " + pc.cockDescript(y) + " with her gaping vagina.");
			output("\n\nYou wait for Kelly's head to descend towards the bowl again and then thrust forward, penetrating both her holes at the same time.  She cries out from the intense sensation and then bucks back into you, helping you sink your [pc.cock " + x + "] deeper and deeper into her ass, spreading her deliciously tight anal passage whilst your " + pc.cockDescript(y) + " slides effortlessly into the fleshy folds of her cunt.  You slap her ass exultantly, making her squeak again, as you find your limit before resuming the same rhythm, fucking both her sopping vagina and tight, hot ass now with deep, slow strokes.");
		}
		...
		output("\n\nWith her head thrown back it's easy for you to grip her " + kelt.hairColor + " braid and use it for leverage as you fuck into her with everything you've got, bucking into her tight, hot flesh ");
		if(pc.balls > 0) output(", your [pc.balls] slapping into her thighs demandingly ");
		output("as you push towards your reward.  Panting, Kelly thrusts back into you as best she can, resisting your frenetic tugs to her hair so that the big, final slather of apple sauce can slide out of the bowl and into her mouth.  She screams around the wet, gooey mouthful and her vagina quivers and seizes your bulging [pc.cock " + x + "] in milking ripples");
		if(y >= 0) output(" as her anus clenches around your " + pc.cockDescript(x));
		output(".  With a wordless shout, you tense up and orgasm in tandem, surging line after line of glorious into her wet, churning depths, lost in the ageless sensation of filling a breeding slut's holes with thick seed for what seems like oceans of blissful time.");
	}
	//Centaur: 
	else {
		output("\n\nThe mere smell of the apple sauce has got Kelly's vagina dribbling, and in the close, warm environment of the barn the cloying, sweet smell radiating off her has got [pc.eachCock] rock hard again, and your arousal urges you to work quickly.  With a “hup” you mount her, settling yourself onto her warm, rippling back, surrounding her with your heavy flesh, smell and presence, your [pc.chest] pressing into her back as you bring the bowl around her head.  Transfixed by it, swallowing audibly, the centaur reaches for it automatically.");
		output("\n\nYou tut mockingly and pull it away until she lowers her hands, then bring it in close to her face.  Her hands fall on her erect nipples as she dips her head into the bowl, scoops up a long lap of the green mash with her tongue and guides it into her wet mouth.  Her throat works and she closes her eyes.  The intermittent dripping of her femcum on the straw behind you turns into a steady trickle and your own arousal urgent now, you push your [pc.cock " + x + "] against her sopping pussy gently.  She is radiating heat and her wet depths suck at your [pc.cockHead " + x + "] deliciously.");
		...
		if(pc.cocks[x].thickness() >= 4) output("  Your girth spreads her fuck tunnel wide and you grunt at the delicious tightness of it as you pack her full of cock.");
		
		output("\n\n\"<i>Oh [pc.Master]...</i>\" Kelly groans, arching her head back from her feed so it spoons into your neck.  A series of clenches grip your [pc.cock " + x + "] and a sudden gush of fluid warms your ");
		if(pc.balls > 0) output("[pc.balls]");
		else output("thighs");
		output(".  Did she cum already?  It's fortunate you've already taken the edge off, because you're not sure how long you would otherwise be able to last against her impossibly turned on, sucking, clinching pussy.  As it is, you grip her muscled backside with your own huge hindquarters and pull yourself almost all the way out of her fleshy pocket before spearing your way in again, steadily in and then out, working your way into a slow, sensual rhythm.  Kelly's head sinks and bobs in front of you, and your ears are full of her wet, muffled chews, gulps and moans as she gorges herself on your sweet, sticky sex whilst you gently hump her.");
		
		//1 < cock:
		if(y >= 0) {
			output("\n\nYou think about her tight, puckered asshole beneath you as lust grips you closer, a tiny puddle of human pink buried in her heaving brown horse flesh.  Your breath coming heavily now you withdraw your [pc.cock " + x + "] from her wet softness, dripping with her lubrication.  Quickly you push it against her puckered anus whilst lining up your " + pc.cockDescript(y) + " with her gaping vagina.  You wait for Kelly's head to descend towards the bowl again and then thrust forward, penetrating both her holes at the same time.  She cries out from the intense sensation and then bucks back into you, helping you sink your [pc.cock " + x + "] deeper and deeper into her ass, spreading her deliciously tight anal passage whilst your " + pc.cockDescript(y) + " slides effortlessly into the fleshy folds of her cunt.  You grab one of her soft tits and squeeze exultantly, making her squeak again, as you find your limit before resuming the same rhythm, fucking both her sopping vagina and tight, hot ass now with deep, slow strokes.");
		}
		...
		output("\n\nYou grip her " + kelt.hairColor + " braid and force her head to arch back before pushing the bowl against her lips, tipping the last of the green sweet down her gullet whilst you begin to fuck into her with everything you've got, bucking into her tight, hot flesh");
		if(pc.balls > 0) output(", your [pc.balls] slapping into her thighs demandingly");
		output(" as you push towards your reward.");
		output("\n\nPanting, Kelly thrusts back into you as best she can, resisting your frenetic tugs to her hair so that the big, final slather of apple sauce can slide out of the bowl and into her mouth.  She screams around the wet, gooey mouthful and her vagina quivers and seizes your bulging [pc.cock " + x + "] in milking ripples");
		if(y >= 0) output(" as her anus clenches around your " + pc.cockDescript(y));
		output(".  With a wordless shout, you tense up and orgasm in tandem, surging line after line of glorious into her wet, churning depths, lost to the ageless sensation of filling a breeding mare's holes with thick seed for what seems like blissful oceans of time.");
	}
	//[merge]
	...
	//First:
	if(Flag("COC.KELLY_TIMES_APPLESAUCED")  == 0) {
		output("\"<i>That was the best treat ever [pc.Master],</i>\" she murmurs when you break away, gazing into your eyes lovingly.  \"<i>I liked apples but never quite as much as that.  Wow.</i>\"  You hold her face and say she should look after that orchard: who knows, maybe she'll be good enough to deserve a second helping.  Kelly smiles eagerly.");
		output("\n\n\"<i>I will be [pc.Master], you watch!</i>\"  You dress yourself and amble out of the stable with her before sending her on her way with a loving, jiggling swat to her boobs.");
	}
	//Repeat:
	else {
		output("\"<i>That tastes better every time you make it [pc.Master],</i>\" she purrs when you break away, gazing into your eyes lovingly.  \"<i>You should go into business with it.</i>\"  You grin at the idea and say who knows?  Maybe Mareth will eventually behave well enough to appreciate it as much as she does.  \"<i>I long for that day every moment of every day,</i>\" your slave replies fervently, stroking the hand you're holding her face with.");
	...
	pc.orgasm();
	//dynStats("sen", -2);
	IncrementFlag("COC.KELLY_TIMES_APPLESAUCED");
	doNext(returnToCampUseOneHour);
}
...
private function kellyBJsAhoy():void {
	clearOutput();
	output("You step into Kelly, a question fading on her lips as you put your arms around her waist and answer it by drawing her face into yours.  You kiss her hungrily, pushing your tongue into her hot mouth.  It's almost an instinctive reaction - it's difficult to look at her face and not be drawn to her plump, pert lips, to not want to touch them, to use them.  And godsdamn, does she know how to use them.  She responds to your kiss in kind, humming blissfully as she eagerly accepts your tongue, rolling and curling it with her own, entwined like two lovers, drawing it further into her warm wetness as her pillowy boobs push into your [pc.chest], her sweet, horny smell invading your nostrils as her overfull lips mash into your own, rubbing at your philtrum gently.");
	output("\n\nYou are already ragingly hard, [pc.eachCock] throbbing to the idea of those warm, expert folds of flesh sliding over ");
	if(pc.cockTotal() == 1) output("it");
	else output("them");
	output(".  You pull away, catching Kelly's tongue with your own lips teasingly as you do, so you can watch her very slowly withdraw it, licking her lips so they glisten, her eyes shining with a depthless, green desire.  You murmur that it's time to put that mouth to use it was made for.  She's folding her legs beneath her before you've finished speaking, her hungry jade eyes never leaving yours.");
	
	//Not Centaur: 
	if(!pc.isTaur()) {
		output("\n\nYour [pc.cock " + pc.biggestCockIndex()+ "] strains outwards as you slide out of your [pc.gear], and you grin as you draw in close to Kelly's face, letting the heavy, demanding smell of your musk envelop her.  Her skin flushes and her breath comes more rapidly as you brush her cheek with it.  You smile and gently but firmly tell her to put her hands under her tits, close her eyes, open her mouth and then be still.  You admire the sight as you languidly rub yourself, your pretty centaur slave presenting her big round breasts to you and her tongue out, waiting to take your load whole.");
		output("\n\nYou step in and tease her with your [pc.cock " + pc.biggestCockIndex()+ "], softly brushing her face with your [pc.cockHeadBiggest], working the musky smell deep into her head.  She closes her eyes and breathes out deeply, enveloping your crotch in warm air.");
		if(pc.cocks[pc.biggestCockIndex()].cType == GLOBAL.TYPE_EQUINE) output("  You wonder whether the fact it is a horse cock, protruding proudly out of its sheath, makes it particularly difficult for her to resist, that it triggers some deep urge hardwired into her.  There's no way to really judge, but it seems like you've only just begun to tantalize her with it that the air below you is filled with bitten off grunts and moans, her mouth and head unconsciously bending towards your flared stallion prick as you rub it into her, always keeping it tantalizingly out of reach of her mouth.");
		output("  Finally, after about a minute of this tender torture, she groans from deep within her throat.");
		
		output("\n\n\"<i>Please [pc.Master], let me swallow you, let me taste your strength, let me feel like a slut should,</i>\" she begs.  \"<i>I'll make it so good for you you'll never want anyone else, just like I will never want anyone else.  Just... let me drink your cum!</i>\"  You let a long second go by.");
		
		output("\n\n\"<i>Go on then,</i>\" you say, trying to keep the amused tone out of your voice.  \"<i>Show me why you deserve it.  Nice and slow.</i>\"  Her hands still holding up her gorgeous fuck udders, Kelly lets out a small whinny of pleasure and bends into your crotch, kissing the base of your bulging [pc.cock " + pc.biggestCockIndex()+ "] longingly as she gets to work.");
		//If PC has anything else aside from cock:
		if(pc.hasVagina()) output("  You're in no hurry, and with a slight roll of your [pc.hips] you direct her to start at the back and work her way up.");
		
		//Vagina and balls: 
		if(pc.balls > 0 && pc.hasVagina()) {
			output("\n\nKelly brushes your [pc.balls] out of the way as she burrows deep into your groin, her hot tongue finally pressing against your [pc.vagina], oozing already in sympathy to the lust which has gripped your male sex.  She is a bit of a stranger to your female anatomy but she's a quick and eager learner, tracing your entrance with the tip of her tongue before pushing her plush lips into your opening, covering your outer lips and your clit in shifting softness as she burrows into your tunnel with her tongue, curling deep into you as she worships every inch she can reach with her flexible mouth muscle, lapping up every trace of sweet moisture she finds with vocal pleasure.");
			output("\n\nYou close your eyes and lose yourself for a while, very gently thrusting your [pc.hips] into her hot mouth as she makes your [pc.vagina] glow with pleasure, sending small spasms of ecstasy shooting through your core, keeping your [pc.cock " + pc.biggestCockIndex()+ "] straining.  It'd be so easy to ride her face to orgasm... but no, you signed up for the whole ten yards here and she isn't even close to being finished.  With a bit of encouragement with your hips, you get her to pull away from your beading muff and move on to your balls.  She sighs as she begins to lavish them with worshipful attention.");
			//[Normal balls:
			if(pc.ballSize() <= 8) {
				...
				if(pc.balls > 2) output(", then the third, then the fourth");
				output(", then back to the first one, a slow and sensual repetition.  She moans intermittently as she does it, and you're not sure if it's intentionally to send delightful shivers of sensation through your scrotum and up the spine of [pc.eachCock], or it's simply because the sheer degradation of the act fills her with deep, shameful lust.  Supporting her soft, ");
				//if (pregnancy.isPregnant) output("milk-laden ");
				output("breasts whilst slavishly polishing the [pc.balls] of the " + pc.mf("man","woman") + " who made her into, well, a ball-licking bitch.  Whatever the cause the result is the same: pleasure thrums through your groin and up your [pc.cock " + pc.biggestCockIndex()+ "], and you close your eyes, lost to the sensations of her skillful tongue.");
			}
			//[Huge balls:
			else {
				...
				if(pc.balls > 2) output(", then the third, then the fourth");
				output(", then back to the first one, a slow and sensual repetition.  The warm air is punctuated with desperate 'ahh, ahn, ahh's as she surrenders herself to the deep, shameful lust of the act, supporting her soft");
				//if (pregnancy.isPregnant) output(", milk laden");
				output(" breasts whilst slavishly polishing the [pc.balls] of the " + pc.mf("man","woman") + " who made her into, well, a ball-licking bitch.");
				//[Dominika dreams:
				if(flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] > 0 && flags["COC.UNKNOWN_FLAG_NUMBER_00150"] > 0) output("  You are gripped by a sudden but thrilling velvet-edged sense of déjà vu.  Has this scene not repeated somewhere else?");
			}
		}
		//Vagina:
		else if(pc.hasVagina()) {
			output("\n\nKelly's hot tongue presses against your [pc.vagina], oozing already in sympathy to the lust which has gripped your male sex.  She is a bit of a stranger to your female anatomy but she's a quick and eager learner, tracing your entrance with the tip of her tongue before pushing her plush lips into your opening, covering your outer lips and your clit in shifting softness as she burrows into your tunnel with her tongue, curling deep into you as she worships every inch she can reach with her flexible mouth muscle, lapping up every trace of sweet moisture she finds with vocal pleasure.");
			output("\n\nYou close your eyes and lose yourself for a while, very gently thrusting your [pc.hips] into her hot mouth as she makes your [pc.vagina] glow with pleasure, sending small spasms of ecstasy shooting through your core, keeping your [pc.cock " + pc.biggestCockIndex()+ "] straining.  It'd be so easy to ride her face to orgasm, but no, you signed up for the whole ten yards here and she isn't close to being finished.  With a bit of encouragement with your hips, you get her to pull away from your beading muff and move up to your cock.");
		}
		//Balls:
		else if(pc.balls > 0) {
			output("\n\nKelly's hot tongue presses against your [pc.balls] and you sigh as she begins to lavish them with worshipful attention.  ");
			//[Normal balls:]
			if(pc.ballSize() <= 8) {
				output("After curling her tongue here and there over each sensitive orb, bathing them in saliva, she envelops each one in her mouth, sucking gently first one, then the second");
				if(pc.balls >= 4) output(", then the third, then the fourth");
				output(", then back to the first one, a slow and sensual repetition.  She moans intermittently as she does it, and you're not sure if it's intentionally to send delightful shivers of sensation through your scrotum and up the spine of [pc.eachCock], or it's simply because the sheer degradation of the act, supporting her soft, ");
				//if (pregnancy.isPregnant) output("milk-laden ");
				output("breasts whilst slavishly polishing the [pc.balls] of the " + pc.mf("man","woman") + " who made her into, well, a ball-licking bitch, fills her with deep, shameful lust.  Whatever the cause, the result is the same: pleasure thrums through your groin and up your [pc.cock " + pc.biggestCockIndex()+ "], and you close your eyes, lost to the sensations of her skillful tongue.");
			}
			//Huge balls:
			else {
				output("After curling her tongue here and there over each sensitive orb, bathing them in saliva, she attempts to envelop one in her mouth.  She can't though; your testicles, bulging and tight with arousal, are simply too big.  After a short pause she goes back to licking them, lapping at their surface tenderly, licking all around each one for every trace of salt and musk, wetly caressing first one, then the second");
				if(pc.balls >= 4) output(", then the third, then the fourth");
				output(", then back to the first one, a slow and sensual repetition.  The warm air is punctuated with desperate 'ahh, ahn, ahh's as she surrenders herself to the deep, shameful lust of the act, supporting her soft ");
				//if (pregnancy.isPregnant) output(", milk laden");
				output(" breasts whilst slavishly polishing the [pc.balls] of the " + pc.mf("man","woman") + " who made her into, well, a ball-licking bitch.");
		...
		//[merge]
		output("\n\nBreathing heavily, she presses her lips against the base of your [pc.cock " + pc.biggestCockIndex()+ "] again before slowly working her way up, licking here and kissing softly there as she goes.  ");
		if(pc.biggestCockLength() > 15) output("It's a long way up and the effect is like a lift slowly rising, turning on a glow of lights on each level as it goes.  ");
		output("Finally she reaches your [pc.cockHeadBiggest], and after resting her lips on your crown for a short time, moving them ever-so-gently to tantalize the most sensitive part of your bulging erection, she opens wide and envelops you, gripping the end of your dick in a soft, wet cave of shifting pleasure.  She slowly impales herself on it, her plump lips encapsulating more and more of your dick in sucking mouth flesh, and then it recedes, eventually even retreating from the crown as she goes back to licking and kissing the length.");
		output("\n\nYou look down with a mix of bemusement and amazement.  She doesn't return your gaze, her eyes closed and her hands on her boobs.  She continues to suck, lick and kiss your [pc.cock " + pc.biggestCockIndex()+ "], slathering every inch she can reach with attention.  She really isn't trying to get any kind of reaction out of you.  She is completely bent upon worshipping your cock.  She finds a vein bulging with blue urgency and traces it upwards; the act makes you sigh and dribble pre-cum with pleasure.  Rising upwards, Kelly spreads her masterful tongue across your cum-slit and whimpers as the musky taste hits the back of her throat.");
		...
		output("\n\n\"<i>It's lovely that you're such a bitch you can cum just from lapping up the smallest trickle of my weakest juices,</i>\" you say, letting amusement soak into your words.  \"<i>But we aren't here for you.  What can you do for me?</i>\"  The centaur only has eyes for your prick, licking her plump lips instinctively as she gathers her breath, her ardour and hunger not dimmed in the slightest.  She swathes your [pc.cockHeadBiggest] in her mouth again, her head bobbing purposefully as your length is swallowed by the roiling wetness again, this time working rhythmically, ribbing it with pliant pleasure.  You sigh as you begin to lose yourself in the soft but purposeful motion, revolving your hips now with the pulse of her movements.");
		output("\n\nBit by bit, she takes more and more of your [pc.cock " + pc.biggestCockIndex()+ "] into her milking redness.");
		if(pc.biggestCockLength() > 12) output("  She can't take all of your dick but she tries her best, thrusting her head down as she crams her mouth with your thick hardness until you are touching her tonsils.  Lust crowding you now, you take hold of her head and force her further down your [pc.cock " + pc.biggestCockIndex()+ "], gently but purposefully pushing further into her tight throat with each thrust.  You manage to hold back on your urge to simply face fuck her as hard as you can, instead taking hold of the base of her braid and moving her back and forth on your length as sensually as you can, drawing your dick back from the delicious tightness of her throat so she can breathe deep and spend a moment worshipping your [pc.cockHeadBiggest] with her tongue and lips, eliciting a moaned 'mmm' from her each time before penetrating her gullet again.");
		//[Cock <12: 
		else output("  Her mouth is a good fit for your dick and you soon find your limit, touching the back of her throat with each gentle thrust.  You manage to hold back on your urge to simply face fuck her as hard as you can, instead taking hold of the base of her braid and moving her back and forth on your length as sensually as you can, drawing your dick back to the entrance of her mouth so she can worship your [pc.cockHeadBiggest] with her tongue and lips, eliciting a moaned 'mmm' or from her each time before thrusting deep into her maw again.");
		output("\n\nShe runs her tongue forward along the underbelly of your [pc.cock " + pc.biggestCockIndex()+ "] as you thrust inwards and then backwards as you withdraw, creating the sweetest friction imaginable, and you find you are breathing hard, running close to the wind now as you thrust into her sucking, milking mouth harder and harder.  The more vigorously you do it, the louder Kelly's muffled sighs and slurps of pleasure become.");
		...
		if(pc.balls > 0) output(", as you feel your [pc.balls] clench");
		output(", you pull out, grip your [pc.cock " + pc.biggestCockIndex()+ "] at the base, point it at your cumslut's big creamy tits and groan as your bulging cock tenses.  She 'aww's in deep disappointment as the dick is torn away from her but she obediently presents her fat, pert breasts, and her breathy sigh is what you hear as orgasm clenches you.  Your cock slit dilates and you spurt out rope after rope of jizz");
		if(kelt.breastRows.length > 1) output(" onto all four of her tits");
		output(".  The sensation of deep release is ecstatic, the sight of your cock cream pasting your slave's softness delicious.");
		if(pc.cumQ() >= 750) output("\n\nAs ever you can barely control yourself once you get going, and it feels like you cum for minutes on end, painting not just her chest but her face and stomach, not stopping until she is absolutely caked and dripping with your potent sex.");
		if(pc.hasVagina()) output("\n\nYour female sex clenches and orgasms in sympathy, and you feel fluid dribble down your [pc.hips] as you continue to paint Kelly.  Your only dim regret is that there's no way of soaking her with that, too.");
		output("  Finally you are spent.");
	}
	//Centaur:
	else {
		output("\n\nYou feel your [pc.cock " + pc.biggestCockIndex()+ "] straining downwards as you slide out of your [pc.gear], and you grin as you draw in close to Kelly, letting the smell of your demanding musk envelop her as you stand over her, drooping your cock against her face.  Her skin flushes and her breath comes more rapidly as you brush her cheek with it.  You smile, step back and gently but firmly tell her to put her hands under her tits, close her eyes, open her mouth and then be still.");
		output("\n\nYou admire the sight for a moment.  Blood surges into your [pc.cock " + pc.biggestCockIndex()+ "] as you look at your pretty centaur slave presenting her big, round breasts to you with her tongue out, waiting patiently to take your load whole.  You clop in and tease her again with your [pc.cock " + pc.biggestCockIndex()+ "], softly brushing her face with your [pc.cockHeadBiggest], working the musky smell deep into her head.  She closes her eyes and breathes out deeply, enveloping your crotch in warm air.");
		//Horse cock:
		if(pc.cocks[pc.biggestCockIndex()].cType == GLOBAL.TYPE_EQUINE) output("\n\nYou wonder whether the fact that you are a centaur and it is a horse cock, protruding proudly out of its sheath, makes it particularly difficult for her to resist, that your animal musk triggers some deep urge hardwired into her.  There's no way to really judge, but it seems like you've only just begun to tantalize her with it that the air below your barrel-like body is filled with bitten off grunts and moans as you rub her with your flared stallion prick, always keeping it tantalizingly out of reach of her mouth.");
		output("  Finally, after about a minute of this tender torture, you hear her groan from deep within her throat.");
		
		output("\"<i>Please [pc.Master], let me swallow you, let me taste your strength, let me feel like a slut should,</i>\" she begs. \"<i>I'll make it so good for you you'll never want anyone else, just like I will never want anyone else.  Just... let me drink your cum!</i>\"");
		output("\n\nYou let a long second go by.");
		output("\n\n\"<i>Go on then,</i>\" you say, trying to keep the amused tone out of your voice. \"<i>Show me why you deserve it.  Nice and slow.  And keep your hands where they are.</i>\"  Kelly lets out a small whinny of pleasure and bends into your crotch, kissing the base of your [pc.cock " + pc.biggestCockIndex()+ "] longingly as she gets to work.");
		//If PC has anything else aside from a single cock:
		if(pc.cockTotal() > 1 || pc.hasVagina()) output("  You're in no hurry, and with a slight roll of your [pc.hips] you direct her to start at the front and work her way down.");
		...
		if(pc.hasVagina() && pc.balls > 0) {
			...
			if(pc.ballSize() <= 8) {
				...
				output(", then back to the first one, a slow and sensual repetition.  She moans intermittently as she does it, and you're not sure if it's intentionally to send delightful shivers of sensation through your scrotum and up the spine of [pc.eachCock], or it's simply because the sheer degradation of the act fills her with deep, shameful lust- supporting her soft");
				//if (pregnancy.isPregnant) output(", milk-laden");
				output(" breasts whilst slavishly polishing the [pc.balls] of the centaur who made her into, well, a ball-licking bitch.  Whatever the cause the result is the same.  Pleasure thrums through your groin and up your [pc.cock " + pc.biggestCockIndex()+ "] and you close your eyes, lost to the sensations of her skillful tongue.");
			}
			//[Huge balls:
			else {
				output("\n\nAfter curling her tongue here and there over each sensitive orb, bathing them in saliva, she attempts to envelop one in her mouth.  She can't though - your testicles, bulging and tight with arousal, are simply too big.  After a short pause she goes back to licking them, lapping at their surface tenderly, licking all around each one for every trace of salt and musk, wetly caressing first one, then the second");
				...
				output(", then back to the first one, a slow and sensual repetition.  The warm air is punctuated with desperate 'ahh, ahn, ahh's as she surrenders herself to the deep, shameful lust of the act, supporting her soft");
				//if (pregnancy.isPregnant) output(", milk laden");
				output(" breasts whilst slavishly polishing the [pc.balls] of the centaur who made her into, well, a ball-licking bitch.");
				if(flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] > 0 && flags["COC.UNKNOWN_FLAG_NUMBER_00150"] > 0) output("  You are gripped by a sudden but thrilling, velvet-edged sense of déjà vu.  Has this scene not repeated somewhere else?");
			}
		}
		//Vagina: 
		...
		//Balls: 
			...
			if(pc.ballSize() <= 8) {
				...
				output(", then back to the first one, a slow and sensual repetition.  She moans intermittently as she does it, and you're not sure if it's intentionally to send delightful shivers of sensation through your scrotum and up the spine of [pc.eachCock], or it's simply because the sheer degradation of the act, supporting her soft");
				//if (pregnancy.isPregnant) output(", milk-laden");
				output(" breasts whilst slavishly polishing the [pc.balls] of the centaur who made her into, well, a ball-licking bitch, fills her with deep, shameful lust.  Whatever the cause the result is the same; pleasure thrums through your groin and up your [pc.cock " + pc.biggestCockIndex()+ "] and you close your eyes, lost to the sensations of her skillful tongue.");
			}
			//[Huge balls:
			else {
				output("  After curling her tongue here and there over each sensitive orb, bathing them in saliva, she attempts to envelop one in her mouth.  She can't though - your testicles, bulging and tight with arousal, are simply too big.  After a short pause, she goes back to licking them, lapping at their surface tenderly, licking all around each one for every trace of salt and musk, wetly caressing first one, then the second");
		...
		//[merge]
		output("\n\nBreathing heavily, she presses her lips against the base of your [pc.cock " + pc.biggestCockIndex()+ "] again before slowly working her way down, licking here and kissing softly there as she goes.");
		//[Cock >15 inches:
		if(pc.biggestCockLength() > 15) {
			output("  It's a long way down and the effect is like a lift slowly descending, turning on a glow of lights on each level as it goes.");
		}
		output("  Finally she reaches your [pc.cockHeadBiggest], and after resting her lips on your crown for a short time, moving them ever-so-gently to tantalize the most sensitive part of your bulging erection, she opens wide and envelops you, gripping the end of your dick in a soft, wet cave of shifting pleasure.  She slowly impales herself on it, her plump lips encapsulating more and more of your dick in sucking mouth flesh... and then it recedes, eventually even retreating from the crown as she goes back to licking and kissing the length.");
		...
		output("\n\n\"<i>It's lovely that you're such a bitch you can cum just from lapping up the smallest trickle of my weakest juices,</i>\" you say, letting amusement soak into your words.  \"<i>But we aren't here for you.  What can you do for me?</i>\"  After a short pause, Kelly responds by swathing your [pc.cockHeadBiggest] in her mouth again, her head bobbing purposefully as your length is swallowed by the roiling wetness again, this time working rhythmically, ribbing it with pliant pleasure.  You sigh as you begin to lose yourself in the soft but assiduous motion, revolving your hips now with the pulse of her movements.");
		output("\n\nBit by bit, she takes more and more of your [pc.cock " + pc.biggestCockIndex()+ "] into her milking redness.  ");
		//[Cock >12:
		if(pc.biggestCockLength() > 12) {
			output("  She can't take all of your dick but she tries her best, thrusting her head down as she crams her mouth with your thick hardness until you are touching her tonsils.  Lust crowding you now, you thrust your back haunches forward, forcing her further down your [pc.cock " + pc.biggestCockIndex()+ "], gently but purposefully pushing further into her tight throat with each thrust.  You manage to hold back on your urge to simply face fuck her as hard as you can, instead moving her back and forth on your length as sensually as you can, drawing your dick back from the delicious tightness of her throat so she can breathe deep and spend a moment worshipping your [pc.cockHeadBiggest] with her tongue and lips, eliciting a moaned “mmm” from her each time before penetrating her gullet again.");
		}
		//[Cock <12: 
		else {
			output("  Her mouth is a good fit for your dick and you soon find your limit, touching the back of her throat with each gentle thrust.  You manage to hold back on your urge to simply face fuck her as hard as you can, instead moving your haunches back and forth as sensually as you can, drawing your dick back to the entrance of her mouth so she can worship your [pc.cockHeadBiggest] with her tongue and lips, eliciting a moaned “mmm” from her each time before thrusting deep into her maw again.");
		}
		...
		if(pc.balls > 0) output(", as you feel your [pc.balls] clench");
		output(", you pull out and point your [pc.cock " + pc.biggestCockIndex()+ "] downwards.  She 'aww's in deep disappointment as the dick is torn away from her, but you shush her.");
		...
		if(pc.hasVagina()) output("pussy-licking, ");
		if(pc.balls > 0) output("ball-sucking ");
		output("cock addict, you will have to do a good job of catching it on those whore pillows of yours.</i>\"  With that, you thrust your hindquarters forward.  Her breathy groan is what you hear as orgasm clenches you.");
		output("\n\nYour cock slit dilates and you spurt out rope after rope of jizz");
		if(kelt.breastRows.length > 1) output(" onto all four of her tits");
		output(".  The sensation of deep release is ecstatic, the sound of your cock cream spattering onto your slave's softness delicious.");
		//[Vagina:
		if(pc.hasVagina()) output("  Your female sex clenches and orgasms in sympathy, and you feel fluid dribble down your [pc.hips] as you continue to paint Kelly.  Your only dim regret is that there's no way of soaking her with that, too.");
		output("\n\nFinally you are spent.  You clop around to admire your handiwork.");
		//Low cum:
		if(pc.cumQ() < 750) output("  Kelly has done a sterling job of catching your load; you've frosted her boobs with long strokes quite convincingly.");
		else {
			output("  As ever, your virility has gotten the better of you.  Kelly looks like she's in an ecstatic trance.  You've completely pasted not just ");
			if(kelt.breastRows.length > 1) output("all four of ");
			output("her boobs with alabaster ooze, but her stomach, face and hair.  She's holding a reservoir of the stuff in her cleavage.");
		}
	}
	//[merge]
	output("\n\n\"<i>Thank you for using me, [pc.Master],</i>\" she says once you're finished, grinning happily as she gazes at the off-white goo splattered across her breasts, her cheeks rosy with exertion.  \"<i>But, couldn't you have made it in my mouth?  I would have swallowed it all down, you know.  I'm a good girl.</i>\"");
	output("\n\nYou laugh as you settle yourself down facing towards her");
	if(!pc.isTaur()) output(", still clutching your pleasantly aching prick");
	output(", saying you know she is, but it's a lot more pleasing to you to see her get her fix this way.  Quickly cottoning on, the centaur stares at you with her lustful green eyes as she lifts a cum-spattered boob to her mouth.");
	...
	if(kelt.breastRows.length > 1) output("\n\nShe can't reach her lower row of tits with her tongue, so she simply runs her hands over them, sighing as she rubs your musk into her horny skin and hard nipples before running her tongue across her palms.");
	output("  She leaves a trail of cum leaking down a nipple until at last, closing her eyes, she tenses and orgasms with a muffled sigh as her wet tongue pushes over her erect nub and guides the last of your fix into her mouth.");
	
	//[Libido <50: 
	if(pc.libido() < 50) {
		output("\n\nYou stroke yourself languidly to the obscene display and your [pc.cock " + pc.biggestCockIndex()+ "] is hard again by the time she's finished.  It'd be so easy to stay here and do it with her again, and again, and again... but no, there are plenty of other places you need to be and you are quite spent.  You say goodbye to your cumslut centaur with a fond kiss on the forehead and a slap on the ass before leaving the farm behind you.");
	}
	//Libido >50:
	else {
		output("\n\nYou stroke yourself languidly to the obscene display, and your [pc.cock " + pc.biggestCockIndex()+ "] is straining urgently again before she's even half done.  Although your intention was to leave once she had finished, you find yourself letting her gaze hungrily into your eyes again, letting her press her hands on your [pc.chest]");
		if(pc.hasVagina()) output(", letting her sensually lick you out again until your [pc.vagina] is gleaming with arousal");
		if(pc.balls > 0) output(", letting her needily suckle on your [pc.balls] again until they are inflamed with renewed pressure");
		output(", letting her engulf your [pc.cock " + pc.biggestCockIndex()+ "] with her plump lips again, letting her soft, muffled moans fill your ears as her head bobs over your groin until you clench and cum again, jetting every last drop you have remaining into that beautiful, filthy, juicy little mouth.  Kelly swallows it all down, and goes on licking your clean [pc.cockHeadBiggest] until you order her with a slight groan to stop.");
		
		output("\n\n\"<i>See?  I'm a good girl, [pc.Master].  Never doubt it!</i>\" she says with a wide grin, putting her arms around your waist and gazing up at you, profoundly proud of herself.  You suppose you should be mad at her for taking advantage of your out-of-control libido, but it's difficult to get really angry with a slave who sucks dick quite as well as she does.  After you've rested for a short while, you say goodbye to your cumslut centaur with a fond kiss on the forehead and a slap on the ass, before leaving the farm behind you.");
	}
	pc.orgasm();
	//dynStats("sen", -2);
	processTime(40 + rand(20));
