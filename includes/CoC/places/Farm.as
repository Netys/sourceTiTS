...
import classes.Items.Transformatives.CoCCaninePepperBlack;
...
public function farmExploreEncounter():void {
	...
	if(flags["COC.WHITNEY_MET"] < 3 || flags["COC.WHITNEY_MET"] == undefined) {
		whitneySprite()
		if(flags["COC.WHITNEY_MET"] == undefined) {
			flags["COC.WHITNEY_MET"] = 1;
			output("You find a quaint farmhouse on the far shores of the lake.  Around the homestead are a range of gardens, filled with delicious fruits and vegetables.  Your belly rumbles, aching with hunger, as you approach the dwelling.  A figure in a pepper patch rises up to greet you, waving you over.\n\nYou do your best to conceal your surprise as you realize the farmer is a woman... with fur and canine-like features!  She giggles happily and beckons you over, \"<i>Welcome stranger, it sure is pleasant to see a new face 'round here.  My name's Whitney, and it's mighty fine I don't have to pitchfork you like most guests!</i>\"  She fills you in about the lake and her farm, telling you how the demons can't seem to stay close for long, and monsters always seem weaker the few times they have approached her farm.  Whitney flushes and rapidly changes subject, \"<i>I've got to get back to work, but you help yourself to the peppers, hun!</i>\"\n\n");
		}
		else {
			output("You stumble across Whitney's farm again.  The anthropomorphic canine woman gives you a friendly wave and tosses you another Canine pepper.\n\n");
			flags["COC.WHITNEY_MET"]++;
			if(flags["COC.WHITNEY_MET"] >= 3) output("<b>You've been to the farm enough to easily find it.  You can return by selecting it from the places menu (and will no longer encounter it during random lake exploration)</b>.\n\n");
		}
	...
	//Repeat Offender
		...
		if(flags["COC.KELT_BREAK_LEVEL"] >= 4 && flags["COC.WHITNEY_FLIPPED_OUT_OVER_KELLY"] == undefined) {
			...
			flags["COC.WHITNEY_FLIPPED_OUT_OVER_KELLY"] = 1;
		}
		...
		if(flags["COC.BREAST_MILKER_INSTALLED"] != undefined) {
			if (pc.milkFullness < 75) addDisabledButton(2, "Get Milked", "Get Milked", "Your [pc.nipples] are currently too sore to be milked.  You'll have to wait a while.");
		...
		//if(flags["COC.MARBLE_RAPE_ATTEMPTED"] == undefined && flags["COC.MARBLE_DISABLED"] == undefined && flags["COC.MARBLE_MET"] != undefined && (flags["COC.MARBLE_WARNING"] == undefined || flags["COC.MARBLE_WARNING"] == 0)) {
...
private function talkWhitney():void {
	...
	{
		flags["COC.WHITNEY_TALK_MURBLE_AND_KELT"] = 1;
		
		output("You find Whitney in her usual spot underneath her tree, nose in book. She smiles at you distractedly as you approach.");
	...
	if (flags["COC.TEL_ADRE_KNOWN"] >= 1 && flags["COC.WHITNEY_TALK_TELADRE"] == undefined && flags["COC.WHITNEY_TALK_MURBLE_AND_KELT"] == 1)
	{
		flags["COC.WHITNEY_TALK_TELADRE"] = 1;
		
		output("You find the dog woman sitting on a stool, engaged in hand-milking a cow. She looks up sharply at your approach but smiles readily enough when she sees it’s you.");
	...
	if (flags["COC.EXPLORED_MOUNTAIN"] != undefined && flags["COC.WHITNEY_TALK_HIGH_MOUNTAIN"] == undefined && flags["COC.WHITNEY_TALK_TELADRE"] == 1)
	{
		flags["COC.WHITNEY_TALK_HIGH_MOUNTAIN"] = 1;
		
		output("You find Whitney outside the milking shed, carefully writing labels at a trestle table and sticking them on large bottles of milk.");
		//[PC has used milking device:
		if (flags["COC.WHITNEY_FARM_BREAST_MILKED"] != undefined)
		{
			output(" You are uncomfortably aware of the number of them which are labelled ‘[pc.name]’, and a charged memory of strong suction on your [pc.nipples] comes back to you.");
		}

		//if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00331] > 0 && flags[kFLAGS.ISABELLA_MILKED_YET] != -1)
		//{
			//// Jojo only
			//output(" At the far end there is a small alabaster cluster labelled ‘Jojo’.");
		//}
		//else if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00331] == 0 && flags[kFLAGS.ISABELLA_MILKED_YET] == -1)
		//{
			//// Isabella Only
			//output(" At the far end there is a small alabaster cluster labelled ‘Isabella’.");
		//}
		//else if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00331] > 0 && flags[kFLAGS.ISABELLA_MILKED_YET] == -1)
		//{
			//// Both
			//output(" At the far end there is one alabaster cluster labelled ‘Jojo’, another labelled ‘Isabella’.");
		//}
			
		output(" You ask her who she sells it all to.");
		...
		// [PC has used milking device:
		if (flags["COC.WHITNEY_FARM_BREAST_MILKED"] != undefined && pc.race() == "human")
		{
		...
		if (flags["COC.WHITNEY_FARM_COCK_MILKED"] != undefined)
		{			
	...
	// Subsequent visit
	if (flags["COC.WHITNEY_TALK_DUNGEON_FOLLOWUP"] == undefined && flags["COC.WHITNEY_TALK_DUNGEON"] == 1)
	{
		flags["COC.WHITNEY_TALK_DUNGEON_FOLLOWUP"] = 1;
		
		output("You find Whitney hard at work in the pepper patch. You approach her cautiously, but when she sees you she hails you brightly.");

		output("\n\n“<i>Hey there [pc.name]! I’m afraid I’m almost done here fer the day - can’t get your hands dirty this time, I’m afraid!</i>” You hold the gate open as she hauls a paper bag full of peppers over and plonks it on the ground. As you are closing it again a hand falls on yours.");

		output("\n\n“<i>Listen [pc.name],</i>” says Whitney hesitantly, “<i>I reckon I might come across as a bit... distant sometimes, but you know I 'preciate you coming down here to talk all the time, right? I like that a lot. Nobody 'round here is exactly a great conversationalist, and it’s nice to have someone who jus' listens. Particularly if they’re off savin' the world rest of the time.</i>” Slightly taken aback by her sincerity, you say it’s no big deal; you like hanging around the farm with her, too. She smiles broadly at that, and then with a nod of her head invites you to walk with her down to the storage barn. As delicately as you can, you ask her if she left Tel’Adre because of what happened to her husband.");
	...
	if(rand(4) == 0 && flags["COC.BREAST_MILKER_INSTALLED"] == undefined) {
		if(pc.hasKeyItem("Breast Milker")) {
			output("Before you can say much of anything, Whitney exclaims, \"<i>My stars! Is that one of them demon's milking machines?</i>\"\n\n");
			output("You nod and tell her how you liberated it from the demonic factory and explain that even though it should be fully functional, it'll need to connect to some other machinery to work, and it's way more than any one person could handle.\n\n");
			...
			output("Do you give the breast milker to Whitney for her to hook up?");
			addButton(0, "Yes", whitneyMilkerHookup);
			addButton(1, "No", whitneyMilkerRefusal);
			return;
		}
		else if(pc.milkQ() > 1000) {
			output("Whitney gives you a bemused look when you settle down for a chat.\n\n");
			output("\"<i>Ya might wanna get that looked at darlin',</i>\" she says, gesturing at milky wetness dripping from ");
			if(pc.isChestExposed()) output("your [pc.breasts]");
			...
			output("Whitney starts to giggle, but immediately stifles it, apologizing, \"<i>Ah'm sorry, I didn't mean nothing by it.  I know a few folks who LOVE being a walking milk-fountain.  If ya like, I could probably get you set up with your own milking equipment.   You'd be able to keep up with the... fluid accumulations that seem to be troubling you.   I'd even be able to toss you a few gems if you produce enough.</i>\"\n\n");
			output("It almost sounds too good to be true.   The farmer-girl nods, reading your expression quite clearly, \"<i>Yes, there is a bit of a catch.  I'll need 250 gems for the parts to get this all set up.   Equipment like this isn't cheap.   Whaddya say, hun?   I understand if you don't want to – you can always just wait for the milk to stop.</i>\"\n\n");
			if(pc.credits >= 2500) {
				output("Do you purchase a breast-milker from Whitney for 250 gems?");
				addButton(0, "Yes", breastMilkerPurchase);
				addButton(1, "No", breastMilkerNoPurchase);
			}
			else {
	...
	if(rand(4) == 0 && flags["COC.COCK_MILKER_INSTALLED"] == undefined && pc.hasKeyItem("Cock Milker")) {
		output("Before you can say much of anything, Whitney exclaims, \"<i>My stars! Is that one of them demon's milking machines?</i>\"\n\n");
	...
	//[GENERIC TALK]
	output("You tell her of your recent trials and tribulations ");
	if(pc.cor() > 50) output("or at least the parts you think she would want to hear ");
	output("and she listens attentively, chiming in with witty quips and comfort when appropriate.  When you finish she tells you ");
	if(!flags["COC.FACTORY_SHUTDOWN"] == 1) output("how well the farm has been going");
...
public function workFarm():void {
	...
	//if(flags["COC.MARBLE_DISABLED"] == undefined && flags["COC.MARBLE_WITHDRAWAL"] >= 0) {
		//if(flags["COC.MARBLE_MET"] == 1) marbleScene.addictedEncounterHappy();
		//else marbleScene.encounterMarbleAshamedAddiction();
		//return;
	//}
	////1/3 chance of marblez
	//if(rand(3) == 0 && pc.findStatusAffect(StatusAffects.NoMoreMarble) < 0 && pc.findStatusAffect(StatusAffects.Marble) > 0) {
		////Rapez Override normal
		//if(pc.findStatusAffect(StatusAffects.MarbleRapeAttempted) >= 0 || flags[kFLAGS.MARBLE_WARNING] == 3) {
			//marbleScene.marbleAfterRapeBattle();
			//pc.createStatusAffect(StatusAffects.NoMoreMarble,0,0,0,0);
			//return;
		//}
		////Angry meeting
		//if(flags["COC.MARBLE_WARNING""] == 1) {
			//marbleScene.marbleWarningStateMeeting();
			//return;
		//}
		//if(pc.findStatusAffect(StatusAffects.Marble) >= 0) {
			////Pre-addiction events(explore events take 1 hour, working ones take 3)
			//if(pc.statusAffectv3(StatusAffects.Marble) == 0) {
				//marbling = rand(2);
	...
	whitneySprite()
	output("You ask Whitney if she could use help with anything and she points towards the pepper fields, \"<i>Ya mind gathering up some peppers for an hour or two?  I'm gonna need a few for supper tonight.  I'll even let you keep the best one!</i>\"\n\n");
	output("You nod and borrow a basket, and set off towards the fields.  The next two hours are a blur of sweat and hard work as you prowl between the rows of plants, picking as many ripe red peppers as you can find.  When you finish, you drop the basket by Whitney's door, but not before taking your pepper.\n");
	//(75% chance normal pepper, 25% chance \"<i>rare</i>\" pepper)
	var pepper:Number = rand(4);
	var itype:ItemSlotClass;
	if(pepper <= 2) itype = new CoCCaninePepper();
	else {
		temp = rand(5);
		//-Oversized Pepper (+size, thickness)
		if(temp == 0) itype = new CoCCaninePepperLarge();
		//-Double Pepper (+grows second cock or changes two cocks to dogcocks)
		if(temp == 1) itype = new CoCCaninePepperDouble();
		//-Black Pepper (Dark Fur, +corruption/libido)
		if (temp == 2) itype = new CoCCaninePepperBlack();
		//-Knotty Pepper (+Knot + Cum Multiplier)
		if(temp == 3) itype = new CoCCaninePepperKnotty();
		//-Bulbous Pepper (+ball size or fresh balls)
		if(temp == 4) itype = new CoCCaninePepperBulby();
	}
...
public function exploreFarm():void {
	...
	////Marble after-rape
	//if(pc.findStatusAffect(StatusAffects.MarbleRapeAttempted) >= 0 && pc.findStatusAffect(StatusAffects.NoMoreMarble) < 0) {
		//marbleScene.marbleAfterRapeBattle();
		//pc.createStatusAffect(StatusAffects.NoMoreMarble,0,0,0,0);
		//return;
	//}
	////FIND CARROT!
	//if(kGAMECLASS.nieveHoliday() && flags[kFLAGS.NIEVE_STAGE] == 3 && pc.hasKeyItem("Carrot") < 0) {
		//kGAMECLASS.findACarrot();
		//return;
	//}
	////Free Isabella Milkings!
	//if(pc.hasCock() && flags[kFLAGS.FOUND_ISABELLA_AT_FARM_TODAY] == 0 && flags[kFLAGS.ISABELLA_MILKED_YET] < 0 && kGAMECLASS.isabellaFollowerScene.isabellaFollower() && flags[kFLAGS.ISABELLA_MILK_COOLDOWN] == 0 && rand(2) == 0) {
		//kGAMECLASS.isabellaFollowerScene.findIzzyMilking();
		//return;
	//}
	////Meet Marble First Time
	//if(pc.findStatusAffect(StatusAffects.Marble) < 0 && pc.findStatusAffect(StatusAffects.NoMoreMarble) < 0) {
		//doNext(returnToCampUseOneHour);
	...
	if(explore == 0) {
		whitneySprite()
		output("You run around the farm, keeping an eye for any monsters or oddities around Whitney's property.  Eventually the she-dog joins you, and the two of you have a good time pushing your speed to its limits.  ");
		//Less than 30 speed (+2 speed)
		if (pc.RQ() < 30) {
			pc.slowStatGain("reflexes", 1);
			//dynStats("spe", 2);
			output("Whitney easily outpaces you, leaving you so far behind that she laps around the farm twice for each pass you make.");
		}
		//Less than 50 speed (+1 speed)
		else if(pc.RQ() < 50) {
			pc.slowStatGain("reflexes", 0.5);
			//dynStats("spe", 1);
			output("Whitney is still faster than you, and manages to get far enough ahead of you to disappear from time to time.");
		}
		//Less than 70 speed (+.75 speed)
		else if(pc.RQ() < 70) {
			pc.slowStatGain("reflexes", 0.33);
			//dynStats("spe", .75);
			output("Whitney and you are evenly matched, and the two of you run together for a while, each pushing yourself harder in an effort to best the other.");
		}
		//Else (+.5 speed)
		else {
	...
	//Cows
	if(explore == 2) {
		output("Your explorations take you to the cow pasture. There's no bull here, so the cows are all placidly grazing, building up milk for Whitney to gather. One turns to face you, and you get the odd feeling that it's trying to tell you something.\n\n");
		//[if have a horse dick and rape-high Lust] 
		if(pc.hasCock(GLOBAL.TYPE_EQUINE)) {
			output("Unbidden, the notion that a cow-slit would probably feel pretty good wrapped around your horse-prick flickers through your mind.  ");
			//[if high corruption]
			if(pc.cor() > 60) output("It makes you smile.");
			if(pc.cor() < 30) output("It disgusts you.");
			//[else/mid-corruption] 
			else if(pc.cor() <= 60) output("You aren't sure how you feel about that.");
			//[continue paragraph condition] 
			output("  It for certain would get Whitney chasing you off with a pitchfork.\n\n");
			pc.lust(10);
		}
		//[if no horse dick, a deep cow or horse vag, and in heat]
		else if(pc.hasStatusEffect("Heat")) {
			output("Maybe it wants a bull? You do, one with long, thick dick-meat to satisfy your starving fuck-hole...\n\n");
			pc.lust(15);
		}
		//- [if no dick, not in heat, but overfull with milk]
		else if(pc.milkFullness > 75) {
			output("\"<i>Maybe she wants to be milked?</i>\" you think. You certainly do.\n\n");
			pc.lust(3);
		}
...
public function getMilked():void {
	...
	output("The barn looms tall ahead of you as you step into its shadow.  ");
	if(flags["COC.WHITNEY_FARM_BREAST_MILKED"] == undefined) {
		if(pc.cor() < 50) output("You shiver nervously when you step inside.");
		else output("You smile eagerly as you walk inside.");
		output("  The barn is filled with the earthy smells of earth, wood, and grease.  It's clean for the most part, though the floor is just packed dirt and the stalls look old and well-used.  A bank of machinery along the wall thrums and pulses as if it's a living creature.  Hoses and cables run from it in a dozen places, disappearing into the walls.   There is even a set of stout wooden doorways along the west wall.  That must be where the farm's intelligent denizens stay.  You notice each of the stalls have name-plates on them, and there is even one that says [pc.name].  It must be for you.\n\n");
	}
	//Repeat
	else {
		output("You walk over to the barn, eagerly anticipating the opportunity to get milked.  ");
		////If ilk withdrawl or high lactation no dicks
		//if (	pc.findStatusAffect(StatusAffects.LactationReduction) >= 0 && pc.totalCocks() == 0) output("  Your [pc.nipples] are engorged and ready to be taken care of.", false);
		////If cocks
		//else 
		if(pc.hasCock()) {
			output("Your [pc.cocks] erect");
			if(pc.totalCocks() > 1) output("s");
			output(" and throb", false);
			if(pc.totalCocks() == 1) output("s");
			output(" with desire.  ");
		}
		//If both
		//if(pc.findStatusAffect(StatusAffects.LactationReduction) >= 0 && pc.cockTotal() > 0) {
			//output("Your [pc.nipples] and [pc.cocksLight] grow", false);
			//output(" hard and ready of ", false);
			//output("their", false);
			//output(" own volition.  ", false);
		//}
		output("The doors part easily, and you breeze into your stall in a rush.\n\n");
	}
	//Step into harness – first time only
	if(flags["COC.WHITNEY_FARM_BREAST_MILKED"] == undefined) {
		output("A harness hangs limply in the stall, there to hold the occupant in place while they are milked of every last drop.  You exhale slowly and force yourself to step into it.  As you puzzle out the straps, it gets easier and easier to get the rest of the harness into place.  As you snap the last one into position, machinery whirs and pulls it tight, lifting you off the ground and suspending you, facedown.  The breast milk pumps pulse and vibrate on a tray below you, twitching slightly as you hear the machinery activate.\n\n");
	}
	//REPEAT
	else {
		output("You easily attach the familiar harnesses and lift up into position, hearing the machinery activate automatically. ");
	}
	
	var application:Number = rand(3);
	//Super huge nips scene
	if(pc.nippleLength() >= 3 && rand(2) == 0) application = 3;
	//Apply
	if(flags["COC.WHITNEY_FARM_BREAST_MILKED"] == undefined || application == 0) {
		flags["COC.WHITNEY_FARM_BREAST_MILKED"] == 0;
	...
	//Apply repeat alternate
		...
		output("You stretch down and grab onto the suction cups, pulling them up to your eager [pc.nipples].  They latch on, slapping tight against you as the vacuum pressure seals them tightly against your body.  You can feel your [pc.nipples] pulling tight, nearly doubling in size from the intense pressure.  ");
		if(pc.nippleLength() >= 3) output("They nearly burst the tubes designed to milk them by virtue of their sheer size.  ");
		output("The sensitive flesh of your " + pc.allBreastsDescript() + " fill with a burgeoning pressure that centers around the tubes connected to your nips.\n\n");
	}
	//Version 3
	else if(application == 2) {
		output("Despite the tightness of your harness, you manage to reach down to grab the clear cups of the breast milker.  The cups twitch and move in your hands as you bring them up, ready to milk you.  You begin holding them against your [pc.nipples] and with a sudden lurch the suction pulls against you, pressing the breast-milker's cups tightly against your chest, stretching your [pc.nipples] to nearly twice their normal length.  You feel a building pressure as the machine sucks you relentlessly, drawing your milk to the surface.\n\n");
	}
	//Version 4 huge nips
	if(application == 3) {
		output("In spite of the tightness of your harness, you collect the suction cups and bring them up to your huge nipples, letting the machine pull them into the tight cups with agonizing slowness.  In spite of the large size of your aureola, the machine slowly sucks you inside, the tightness serving only to arouse you further.  The suction pulls the walls of the nipple-tubes tight against your nipples, turning them purple as they swell up like dicks.  Drops of milk leak from the tips as your body lets your milk down, letting it flow through your imprisoned nipples towards its release.\n\n");
		pc.lust(10);
	}
	//Milksplosion Texts
	//Lactation * breastSize x 10 (milkPerBreast) determines scene
	...
	if(pc.lactationQ() < 50) {
		pc.lust(10);
		if(milksplosion == 0) {
			output("A few drops of milk bud on the tips of your [pc.nipples], growing larger as they roll down to the edge of the tube.  It feels as if a ");
			if(pc.totalBreasts() == 2) output("pair");
			else if(pc.totalBreasts() == 4) output("quartet");
			else output("group");
			output(" of internal floodgates are opening, and thin streams of milk erupt, spraying to the noisy suckling tubes.   The milk is sucked away before it can build up, leaving you to wonder just how much you're managing to produce.   The milking goes on for the better part of an hour, though you stop producing long before it's over.  By the time the milker shuts off and the harness releases you, your breasts ache terribly.\n\n");
		}
		//Lightish2
	...
	//Medium (+30 lust)
		...
		if(milksplosion == 2) {
			...
			output(" from the sensation.  Over the next hour you're drained totally dry, until the only answer to the machine's effort is a tiny trickle of whiteness.  The harness gently lowers you to the ground and releases you, leaving you feeling sore.\n\n");
		}
	}
	//High Output (+ 40 lust)
...
public function cockPumping():void {
	var cumQ:Number = pc.cumQ() * (rand(10) + 90) / 100;
	clearOutput();
	//First time barn entrance
	if(flags["COC.WHITNEY_FARM_COCK_MILKED"] == undefined) {
		output("The barn looms tall ahead of you as you step into its shadow.  ", false);
		if(pc.cor() < 50) output("You shiver nervously when you step inside.  ", false);
		else output("You smile eagerly as you walk inside.  ", false);
		output("The barn is filled with the earthy smells of earth, wood, and grease.  It's clean for the most part, though the floor is just packed dirt and the stalls look old and well-used.  A bank of machinery along the wall thrums and pulses as if it's a living creature.  Hoses and cables run from it in a dozen places, disappearing into the walls.   There is even a set of stout wooden doorways along the west wall.  That must be where the farm's intelligent denizens stay.  You notice each of the stalls have name-plates on them, and there is even one that says [pc.name].  It must be for you.\n\n", false);
	}
	//Repeat
	else {
		output("You walk over to the barn, eagerly anticipating your session with the 'milker'.  ", false);
		output("Your [pc.cocksLight] erect", false);
		if(pc.cockTotal() == 1) output("s", false);
		output(" and throb", false);
		if(pc.cockTotal() == 1) output("s", false);
		output(" with building desire.  It's as if ", false);
		if(pc.cockTotal() == 1) output("it", false);
		else output("they", false);
		output(" know", false);
		if(pc.cockTotal() == 1) output("s", false);
		output(" all too well what's to come.  The doors part easily, and you breeze into your stall in a rush.\n\n", false);
	}
	//First time using cock milker – factory install
	if(flags["COC.COCK_MILKER_INSTALLED"] == undefined) {
		output("<b>There is a note tacked to the door from Whitney:</b>\n\n<i>I got your machine all rigged up, and even made a few modifications to bring it up to speed with the machinery I use on the bulls.  Enjoy your 'milkings', and don't overdo it – there are certain messes even a farm-girl like me WON'T clean up.</i>\n\n", false);
		//Step into harness – first time only
		output("A harness hangs limply in the stall, there to hold the occupant in place while they are drained of every ounce of 'milk'.  You exhale slowly and force yourself to step into it.  As you puzzle out the straps, it gets easier and easier to get the rest into place.  You snap the last one in place, and machinery whirs and pulls them tight, lifting you off the ground and suspending you, facedown.  A hatch in the far wall slides open, revealing a mass of tubes, wires, and machinery.  A ", false);
		if(pc.cockTotal() == 1) output("single hose wriggles out from the hole, squirming across the floor.  It stops and lifts up, slowly homing in your horny shaft.\n\n", false);
		else if(pc.cockTotal() == 2) output("pair of hoses wriggle out from the hole, squirming across the floor.  They stop and lift up, slowly homing in your horny shafts.\n\n", false);
		else output("group of hoses wriggle out from the hole, squirming across the floor.  They stop and lift up, slowly homing in your horny shafts.\n\n", false);
		flags["COC.WHITNEY_FARM_COCK_MILKED"] = 1;
	}
	//REPEAT
	else {
		output("You easily attach the harnesses and lift up into position, hearing the machinery activate automatically.  The hungry ", false);
		if(pc.cockTotal() == 1) output("hose wriggles out from the wall, curving up towards your crotch. ", false);
		else output("hoses wriggle out from the wall, curving up towards your crotch. ", false);
		//Count repeat milkings! Wootles for arbitrary stats
		flags["COC.WHITNEY_FARM_COCK_MILKED"]++;
	}
	//Small/Medium/Large, 2x each
	if(pc.cockVolume(0) < 20) {
		//Small 1 
		if(rand(2) == 0) {
			if(pc.cockTotal() == 1) output("The hose's 'tip' is many times longer than your member, and about thrice as thick.  The 'opening' is filled with something wet and pink.  It squirms and wriggles, looking very much alive.  Where did Whitney get this stuff?    It tentatively presses against your " + pc.cockDescript(0) + ", sucking wetly as the material inside slurps you down.  The feeling is immediate and intense, like being surrounded by hundreds of eager tongues, all writhing together along your length.\n\n", false);
			else output("The hoses' 'tips' are many times longer than your members, and about thrice as thick.  The openings are each filled with something wet and pink.  They squirm and wriggle, looking very much alive.  Where did Whitney get this stuff?   They tentatively press against your [pc.cocksLight], sucking wetly as the material inside slurps you down.  The feeling is immediate and intense, like being surrounded by hundreds of eager tongues, all writhing together along your lengths at once.\n\n", false);
		}
		//Small 2
		else {
			if(pc.cockTotal() == 1) output("The hose's opening, while much larger than your " + pc.cockDescript(0) + ", is packed full of some slimy looking pink flesh.  It wriggles hungrily as it snakes up, noisily slobbering over your tip.  Your body, tired of the foreplay, thrusts forwards, rocking the harness back and forth.  The effort is wasted, as the hose stops and slowly repositions itself under you, having to reset before it can begin 'milking'.  You sigh and hold perfectly still as it bumps back against you.    Your muscles twitch but you hold steady as it slowly slides forwards, effortlessly taking your " + pc.cockDescript(0) + " inside.  The stimulation is intense and immediate.  The suction kicks in, making you swell larger than normal.  You feel like every inch is covered in tiny wet massaging fingers and tongues.\n\n", false);
			else output("The hoses' openings, while much larger than your [pc.cocksLight], are packed full of some slimy looking pink flesh.  They wriggles hungrily as they snake up, noisily slobbering over your tip.  Your body, tired of the foreplay, thrusts forwards, rocking the harness back and forth.  The effort is wasted, as the hoses stop and slowly reposition themselves under you, having to reset before they can begin 'milking'.  You sigh and hold perfectly still as they bump back against you.    Your muscles twitch but you hold steady as they slowly slide forward, effortlessly taking your " + pc.multiCockDescript() + " inside.  The stimulation is intense and immediate.  The suction kicks in, making you swell larger than normal.  You feel like every inch is covered in tiny wet massaging fingers and tongues.\n\n", false);
		}
	}
	else if(pc.cockVolume(0) < 70) {
		//Medium 1
		if(rand(2) == 0) {
			if(pc.cockTotal() == 1) {
				output("The mechanical snake's receptacle yawns open, showing an interior filled with wriggling pink material and dripping mucusy wetness.  It wiggles, closing the distance to your " + pc.cockDescript(0) + " in a flash.  Pausing just below your " + pc.cockHead() + ", it slowly flows upward, engulfing you with slick pleasure.  It sucks hard, sliding up ", false);
				if(pc.hasKnot(0)) output("past your knot", false);
				else output("to your base", false);
				output(" and massaging you with hundreds of lubricated internal protrusions.\n\n", false);
			}
			else {
				output("The mechanical snake's receptacles yawn open, showing an interior filled with wriggling pink material and dripping mucusy wetness.  They wiggle, closing the distance to your [pc.cocksLight] in a flash.  Pausing just below your " + pc.cockHead() + "s, they slowly flow upward, engulfing you with slick pleasure.  They suck hard, sliding up ", false);
				output("to your base", false);
				output(" and massage you with hundreds of lubricated internal protrusions.\n\n", false);
			}
		}
		//medium 2
		else {
			if(pc.cockTotal() == 1) output("The dick-milker's opening looks well suited to take your " + pc.cockDescript(0) + ", just about a perfect match for someone hung like you.  As it closes with your groin, you get a better look at the internals.  It looks... alive.  The inside is filled with slime-slick flesh, pinkish in color and jiggling hungrily.  What the hell did Whitney put inside this thing anyway?  The dripping opening rubs against you tentatively, wetting your " + pc.cockDescript(0) + " with slipperiness.  You resist the urge to push forwards, knowing it would be futile with the restraints binding you so tightly.  It sucks your " + pc.cockHead() + " inside, making you groan and swell as your maleness is slowly pulled inside the mechanized fuck-hole.   Squelching noises echo through the barn, mixed with pleasured moans.\n\n", false);
			else output("The dick-milkers' openings look well suited to take your [pc.cocksLight], just about a perfect match for someone hung like you.  As they close with your groin, you get a better look at the internals.  They look... alive.  The inside is filled with slime-slick flesh, pinkish in color and jiggling hungrily.  What the hell did Whitney put inside these things anyway?  The dripping openings rub against you tentatively, wetting your [pc.cocksLight] with slipperiness.  You resist the urge to push forwards, knowing it would be futile with the restraints binding you so tightly.  They suck your " + pc.cockHead() + "s inside, making you groan and swell as your multitude of malenesses is slowly pulled inside the mechanized fuck-hole.   Squelching noises echo through the barn, mixed with pleasured moans.\n\n", false);
		}
	}
	else {
		//large 1
		if(rand(2) == 0) {
			if(pc.cockTotal() == 1) output("The hose's hole spreads wide as it closes in on your crotch, revealing the organic pinkish interior.   Somehow you doubt it'll be able to take all of your mammoth member, but the artificial pussy seems intent on trying.   It pushes against your " + pc.cockDescript(0) + "'s " + pc.cockHead() + ", stretching wide and struggling to fit.  The feelings of slippery flesh writhing about you overwhelms your body, making you go limp in the harness.   The suction slowly pulls you inside, an inch at a time, while the soft 'flesh' of the device hugs your " + pc.cockDescript(0) + " with its many wriggling 'tongues'.\n\n", false);
			else output("The hoses' holes spread wide as they close in on your crotch, revealing their organic pinkish interiors.   Somehow you doubt they'll be able to take all of your mammoth members, but the artificial pussies seem intent on trying.   They push against your [pc.cocksLight]' " + pc.cockHead() + "s, stretching wide and struggling to fit.  The feelings of slippery flesh writhing about you overwhelms your body, making you go limp in the harness.   The suction slowly pulls you inside, an inch at a time, while the soft 'flesh' of the devices hugs your " + pc.multiCockDescript() + " with its many wriggling 'tongues'.\n\n", false);
		}
		//large 2
		else {
			if(pc.cockTotal() == 1) output("The milker's opening dilates wide, struggling to spread enough to accommodate your sheer size.  It closes the gap, waving back and forth like a cobra.  It pauses, brushing its squirming interior against your " + pc.cockHead() + " experimentally.  You squirm as its lubricants begin coating you, but do your best to hold still for the organic-looking pump mechanism.  It starts pushing against you, almost painfully, as your distended cock resists fitting into the tubular hole.  With a sigh of anguished pleasure you're crammed into the tube, squeezed tight by wiggling tendrils. The ever-increasing suction pulls the tube further and further down your length.  It bottoms out, pressing tightly against your crotch and squeezing the entirety of your " + pc.cockDescript(0) + " with inhuman flesh.\n\n", false);
			else output("The milkers' openings dilate wide, struggling to spread enough to accommodate your sheer size.  They close the gap, waving back and forth like a cobra.  Pausing, they brush their squirming interiors against your [pc.cocksLight] experimentally.  You squirm as the lubricants begin coating you, but do your best to hold still for the organic-looking pump mechanisms.  They start pushing against you, almost painfully, as your distended cocks resist fitting into the tubular holes.  With a sigh of anguished pleasure you're crammed into the tubes, squeezed tight by wiggling tendrils.  The ever-increasing suction pulls the tubes further and further down your lengths.  They bottom out, pressing tightly against your crotch and squeezing the entirety of your [pc.cocksLight] with inhuman flesh.\n\n", false);
		}
	}
	
	//MILKIN ACTION GO
	if(rand(2) == 0) {
		output("You squirm against your restraints, openly moaning and twitching your hips, trying to enhance the sensations further.  All you feel is hot wet warmth surrounding and caressing you, stroking you to new heights of pleasure.  Gods above and below, you could lose yourself to this!  You don't want to be released, or to orgasm, or even for the machine to slow down.  You only want MORE!   The heat in your groin gets increasingly worse as your pre-cum starts dripping, turning into a steady flow.  Noisy slurping noises greet your ears as the machinery sucks your pre-seed down the tubes.  The machine immediately backs off a notch, keeping you on the edge of release, much to your simultaneous delight and frustration.\n\n", false);
	}
	//Ver2
	else {
		output("You moan with lust, pushed beyond the limits of sensation to a mixture of heaven and hell.   Thankfully the harness holds your body still as you writhe inside the straps, utterly restrained but instinctively trying to thrust into the source of your good feelings.  The milking devices suck and stroke with feverish intensity, making you bigger and harder than you've ever been before.  You can practically feel the pre-cum dripping from your urethras, and you can definitely hear it being sucked down the tubes.  You feel release building, but the machinery somehow senses it, and backs off, keeping you on the edge of orgasm for as long as possible.\n\n", false); 
	}
	//(OPTIONAL ANAL PROBE)
	if(pc.hasKeyItem("Cock Milker: Anal Attachment") >= 0) {
		if(pc.cockTotal() == 1) output("A sudden splash of wetness hits your backside, instantly soaking your " + pc.skin() + " with slippery lubricants.  You feel something narrow touch against your " + pc.assholeDescript() + ", workings its way past the clenching muscles and into your body.  The violation would bother you more if it didn't seem to amplify the feelings radiating down your shaft, but then it starts to change.  You feel it swelling up inside, somehow inflating.  It stretches you wide, pressing against the knot of your prostate, milking out a thick flow of cum.   It pushes tighter and tighter, making you drip and drizzle in a steady flow before deflating, allowing your abused body to relax.  You sigh, but are kept rock hard by the feverish milking of your dick.\n\nEventually the probe inflates again, squeezing out another drizzle of thick spooge, the hose on your " + pc.cockDescript(0) + " noisily sucking it down.  You're kept like that for quite a while – never feeling a true orgasm, but forced to expel cum over and over by always increasing waves of pressure in your ass.  You babble incoherently, pleasure-drunk, not even noticing a green light on the far side of the wall come on.  One thing you do notice is that the cock-tube isn't slowing down its ministrations.  You're finally allowed to cum!\n\n", false);
		else output("A sudden splash of wetness hits your backside, instantly soaking your " + pc.skin() + " with slippery lubricants.  You feel something narrow touch against your " + pc.assholeDescript() + ", workings its way past the clenching muscles and into your body.  The violation would bother you more if it didn't seem to amplify the feelings radiating down your shafts, but then it starts to change.  You feel it swelling up inside, somehow inflating.  It stretches you wide, pressing against the knot of your prostate, milking out a thick flow of cum.   It pushes tighter and tighter, making you drip and drizzle in a steady flow before deflating, allowing your abused body to relax.  You sigh, but are kept rock hard by the feverish milking of your dicks.\n\nEventually the probe inflates again, squeezing out another drizzle of thick spooge, the hoses on your [pc.cocksLight] noisily sucking it down.  You're kept like that for quite a while – never feeling a true orgasm, but forced to expel cum over and over by always increasing waves of pressure in your ass.  You babble incoherently, pleasure-drunk, not even noticing a green light on the far side of the wall come on.  One thing you do notice is that the cock-tubes are no longer slowing down their ministrations.  You're finally allowed to cum!\n\n", false);
	}
	//(ELSE)
	else {
		if(pc.cockTotal() == 1) output("You're kept on the edge of orgasm for the better part of an hour.   Rhythmic contractions squeeze through the flesh-tube wrapped around your manhood, keeping it painfully hard and dribbling, always backing off before you can truly cum.  You thrash in your harness wildly, insane with need and nearly frothing at the mouth.  The licking tongues never stop, slithering across you in between every wave of mechanized suction that pulls on your member.   You babble incoherently, pleasure-drunk, not even noticing a green light on the far side of the wall turning on.  One thing you do notice is that the cock-tube isn't slowing down its ministrations.  You're finally allowed to cum!\n\n", false);
	...
	if(pc.cumQ() >= 50 && pc.energy() <= 0 && flags["COC.USED_MILKER_TODAY"] > 0) {
		...
		if(cumQ < 1000) {
			...
			if(cumQ > 500) output("thick ", false);
			output("spurts of spunk.  Wriggling cilia convulse around you, licking your head as it flares wide, filling with blood and pouring out cum.  You moan and scream with delight, babbling happily as you watch your cum wick up the clear tube and into the machinery in the wall.  All too soon the pleasure comes to an end; the harness lowers you to the ground and releases you, but the squishy walls of your demon machine lover don't cease their ministrations on your tender [pc.cockNoun]", false);
			if(pc.cockTotal() > 1) output("s", false);
			output(".\n\n", false);
						
			output("A loud 'ding' sounds ", false);
			//[(if enough for payout)
			if(cumQ > 100) output("and a clatter of gems drop into the plate ", false);
			output("as the machine measures your contribution, but it barely registers in the back of your mind as your [pc.cocksLight] harden again from the unending, expertly-given stimulation of the animate tubes.  You try to pull the milker off your [pc.cocksLight], but with ", false);
			//[(big dix)
			if(pc.biggestCockVolume() > 70) {
				output("the end", false);
				if(pc.cockTotal() > 1) output("s", false);
				output(" wrapped so snugly around your huge member", false);
				output("s", false);
				output(" and ", false);
			}
			output("how tired you are, you can't break the suction.  Every time you get a good grip and start to pull, the suction shoots a shiver of pleasure down " + pc.cocksDescriptLight() + " and robs you of your focus and muscle control.  Practically helpless against the tubes, you feel your [pc.cocksLight] get harder and harder, leaking out more pre-cum.\n\n", false);
		}
		//(big skeet, ordinarily would be helper-assisted)
		else {
			output("An eruption of jism boils out, splattering with loud wet noises into the tube.  You can feel the pressure build up, pushing back against your [pc.cocksLight] as the machinery fails to keep up with the gushes of white fluid erupting inside it.  Your eyes cross from the pleasure and you start drooling all over yourself, not even noticing that you're about to blast the milker off your member.  The machinery grinds as it struggles to keep up, then pops off as it overflows with semen.  You don't even notice, instead focusing on the feeling of ejecting sticky globs from your [pc.cocksLight].\n\n", false);

			output("Jism splatters on the floor below you as you discharge rivers of spunk.  Liters upon liters of the stuff spill out of your throbbing [pc.cocksLight], coating the outside of the ejected suction tubes to match the inside.  Eventually you spend your reserve and the cum flow reduces to a trickle.  Stupid with pleasure, you don't notice the suction tube", false);
			if(pc.cockTotal() > 1) output("s", false);
			output(" wriggling back into place on your [pc.cocksLight] as the harness lets you down into the puddle you made.  Only when the velvet walls of the demon machine resume their caresses do you take something as amiss with the situation.\n\n", false);
			
			output("A loud 'ding' sounds and a clatter of gems drop into the plate as the machine measures your contribution, but it barely registers in the back of your mind as " + pc.cocksDescriptLight() + " hardens again from the unending, expertly-given stimulation of the animate tube", false);
			if(pc.cockTotal() > 1) output("s", false);
			output(".  You try to pull the milker off your [pc.cocksLight], but the outside of the tubing is slick with your semen", false);
			//[(big dix)
			if(pc.biggestCockVolume() > 70) {
				output(", and the end", false);
				if(pc.cockTotal() > 1) output("s", false);
				output(" wrap too snugly around your swollen member", false);
				if(pc.cockTotal() > 1) output("s", false);
			}
			output(".  Every time you get a good grip and start to pull, a shiver of pleasure shoots down " + pc.cocksDescriptLight() + " and robs you of your focus and muscle control.  Practically helpless against the tubes, you feel " + pc.cocksDescriptLight() + " get harder and harder as the machine sucks out more pre-cum along with the milky aftershocks of your last massive orgasm.\n\n", false);
		}
		//TO BAD ENDAGE
	...
	cumQ = int(cumQ);
	if(cumQ > 100) output(cumQ/1000 + " Ls.  ", false);
	else output(cumQ + " mLs.  ", false);
	...
	if(flags["COC.WHITNEY_GEMS_PAID_THIS_WEEK"] >= cap) {
		payout = 0;
		output("It also shows a warning: <b>FUNDS EXHAUSTED.</b>  ", false);
	}
	//Reduce payout if it would push past
	else if(flags["COC.WHITNEY_GEMS_PAID_THIS_WEEK"] + payout >= cap) {
		payout = cap - flags["COC.WHITNEY_GEMS_PAID_THIS_WEEK"];
		output("It also shows a warning: <b>Not enough gems for full payment.  GEMS NOW EXHAUSTED.</b>  ", false);
	}	
	if(payout > 0) {
		if (pc.cumQ() < 1000) pc.cumMultiplierRaw += 0.5;
		if (payout == 1) output(num2Text(payout) + " gem rolls ", false);
		else output(num2Text(payout) + " gems roll", false);
	
	output("out into a collection plate.  Whitney really put a lot of work into this! ", false);
	
		pc.credits += payout * 10;
		flags["COC.WHITNEY_GEMS_PAID_THIS_WEEK"] += payout;
		//statScreenRefresh();
	}
	//if (pc.countCockSocks("gilded") > 0) {
		//
		//var gems:int = rand(2) + pc.countCockSocks("gilded");
		//
		//if (pc.cumQ() > 1000)
			//gems *= 1.5;
		//
		...
		//}
		//
		//output("ou see a few sparkling gems in your trail of cum on the floor. You reach down and pick up all " + gems + " of them</b>, and then you are", false);
		//
		//pc.gems += gems;
		//
	//}
...
private function cowBadEnd1():void {
	...
	output("...finding someone to fuck you silly while you get something to eat.  It's so simple!  You crawl out of your stall, feeling your ponderous breasts jiggle as they drag on the ground.  A trail of white cream clearly marks your passage out into the fields, eventually passing beyond the edges of Whitney's farm and into the mountains.");
	doNext(cowBadEnd2);
}

private function cowBadEnd2():void {
	clearOutput();
	output("A day later, you slosh back into the barn with your breasts painfully full and ready to be emptied.  Your cum-filled belly joins your tits in dragging on the floor while you daydream about your time in the minotaur caves last night.  The first one claimed your " + pc.assholeDescript() + ", brutalizing it over and over while the beast emptied his vast reservoirs of seed into your rectum.  Before the beast's equine-like cock could be stuffed halfway inside you, a slightly smaller minotaur was in front of your face, smearing your lips with a thick coating of his tasty pre.  Like a good cow, you opened wide for the bull and gratefully accepted his thick loads, feeling the narcotic-like spunk shut down your ability to think about anything but sex.\n\n", false);
	output("You moo happily, shivering at the intensity of the fond memories and in anticipation of tonight's activities.  But first you need to get your swollen nipples emptied!  The straps to the harness hang there in front of you – how do you work them again?  Your happy moos turn to moos of confusion and stress, but thankfully Whitney pads into the barn before you have a chance to panic.  She looks down and sighs, filling you with confusion.  You weren't a bad cow, were you?\n\n", false);
	output("The anthropomorphic country-girl gives you a gentle pat on the head as she works the straps, talking to herself as she does, \"<i>Don't worry girl, I'm not mad.  I reckon I should've given you a stronger warning is all.  But now the damage is done – what's left of your brains was mushed up by all that milking.  Don't worry none honey, there will always be a spot in my stall for you.  I'll just have to make sure you don't get out with the animals again, won't I?</i>\"\n\n", false);
	output("You nod happily, determined to find someone or something that can stuff you as full as your bulls did.  Whitney gives a resigned sigh as she turns the machine on, watching your determined stare melt into a dopey painting of relief...", false);
	badEnd();
}
private function milkerBadEnd1():void {
	var cumQ:Number = pc.cumQ();
	clearOutput();
	output("As you roll onto your back the machine delivers an encore performance on your swollen [pc.cocksLight], down to the very same orgasm denial.  Wracked by pleasure but now freed to move, you find yourself trying to thrust aganst the tubes, as best as your feeble grip and exhaustion will allow, in order to try to override the machine's will and achieve release.  Nevertheless, the suction expertly mainpulates your arousal and you can do little except endure it as another oversized batch of cum wells up", false);
	//[(balls)
	if(pc.balls > 0) output(" in your " + pc.ballsDescript(), false);
	output(".  As before, the machine works you on and off for roughly an hour before the wall light once again turns green and the suction on your [pc.cocksLight] intensifies.  Your hips take over as you buck against the machine and push out another ", false);
	if(cumQ < 100) output("trickle", false);
	else if(cumQ < 600) output("stream", false);
	else output("torrent", false);
	output(" of jism.", false);
	//[(big skeet)
	if(cumQ > 1000) {
		output("  Unlike before, a fleshy valve flap pops open in the rear of the suction cup", false);
		if(pc.cockTotal() > 1) output("s", false);
		output(", and the pressure of your ejaculation forces squirts of cum out of the valve with every spasm, dribbling sticky whiteness back onto your crotch.", false);
	}
	output("  The orgasm winds down but still the suction and the massage do not.\n\n", false);
	
	output("Another 'ding'", false);
	//[(payment-worthy skeet)
	if(cumQ > 100) output(" and another clatter of gems", false);
	output(" reaches your ears as your second deposit is massed, but your attention is drawn to the new tube", false);
	///(if vag/nipplecunts/lactation)
	var tubes:Boolean = false;
	if(pc.hasVagina() || pc.hasFuckableNipples() || pc.isLactating()) {
		output("s", false);
		tubes = true;
	}
	output(" snaking out of the wall toward you.  Spent as you are, you can only manage a token resistance as ", false);
	if(!tubes) output("it", false);
	else output("one of them", false);
	output(" inserts a rod-like end into your mouth and begins trickling salty fluids down your throat.", false);
	//[(if vag)  
	if(pc.hasVagina()) {
		output("  Another tube, likewise capped with a thick, firm end instead of a suction cup, reaches your " + pc.vaginaDescript(0) + " and pushes inside, secreting more of the fluid.  ", false);
		pc.cuntChange(0,15,true);
		//[(bigclit)
		if(pc.clitLength > 3) output("A smaller suction tube fastens over your " + pc.clitDescript() + " as it rouses from the vaginal stimulation.", false);
	}
	//[(lactation, no nipplecunts)
	if(pc.isLactating() && !pc.hasFuckableNipples()) {
		output("  Some tubes of the cup-type latch onto your [pc.nipples], then begin suction of their own. You feel your breastmilk shift toward the nipples, then begin to dribble into the tubes.", false);
	}
	//[(nipplecunts)
	if(pc.hasFuckableNipples()) {
		output("  More tubes snake up to your " + pc.biggestBreastDescript() + " and work their way into your [pc.nipples].", false);
	}
	output("  " + pc.cocksDescriptLight() + " becomes enormously hard again in a matter of seconds as you absorb the secretions.  You quickly deduce what it is you're tasting; the demon-designed tubing is feeding your own semen back into you, laced with some kind of chemical!\n\n", false);
	
	output("Under the effects of this new drug, " + pc.cocksDescript() + " inflates, engorging with blood until it's much larger than usual", false);
	//[(balls)
	if(pc.balls > 0) output(" and your " + pc.sackDescript() + " stretches as your [pc.balls] nearly double in size", false);
	output(".  Another orgasm passes, this time unhindered as the machine does not slow its efforts in the slightest.  More than twice the previous quantity of semen shoots out of your cock", false);
	if(pc.cockTotal() > 1) output("s", false);
	output(", thanks assuredly to the drug", false);
	//[(balls)
	if(pc.balls > 0) output(" and the new volume of your monstrous testicles", false);
	//[(big skeet)
	if(cumQ > 1000) output(", spraying out of the overflow valve and coating your entire lower body", false);
	output(".", false);
	//[(if vag)
	if(pc.hasVagina()) {
		output("  Your cunt spasms around its invader, milking the pseudo-cock and drawing the tainted semen it emits even deeper into your womb", false);
		//[(big clit)
		if(pc.clitLength > 3) output(", and your clitoris twitches under the cup, threatening to black you out from the pleasure", false);
		output(".", false);
	}
	//[(lactation) 
	if(pc.biggestTitSize() > 1) {
		output("  Your milk sprays out of your [pc.nipples]", false);
		//[(nipplecunts)
		if(pc.hasFuckableNipples()) output(" and rings raise from the bases of the shafts inside your nipples, forming a seal,", false);
		output(" as suction begins to whisk it away.", false);
	}
	output("  A minute after your release, the flow in the other direction intensifies, pouring more fluid into you than before", false);
	//[(lactation)
	if(pc.isLactating()) output(", the drug now tasting of the milk mixed in", false);
	output(". Another chorus sounds from the counter and you hear some gems tinkling as they roll off the plate and onto the ground.\n\n", false);
	...
	if(cor() > 90) {
		...
		output(" to the edge of orgasm and then backs off.  All the pumping in the world can't seem to get you over it, and your frustration mounts as the minutes roll by.  A sharper pressure than usual forms at the base of your [pc.cocksLight] as you fruitlessly grind into the tubes, wishing more than anything to climax; the green light on the wall steadfastedly refuses to light up, mocking your attempts.  From the new pressure an odd strength flows through the rest of your body, increasing your sexual fervor and giving you a second wind, and you finally manage to get a good grip on the tubes, nearly slamming them back and forth on your shaft", false);
		if(pc.cockTotal() > 1) output("s", false);
		output(".  This damn demon machine can't stop you from getting off anymore; with a violent shove you bury " + pc.cocksDescript() + " into the mouth", false);
		if(pc.cockTotal() > 1) output("s", false);
		output(" as your orgasm lets fly!  A truly miraculous amount of semen erupts from your [pc.cocksLight], blowing off the covers of the overflow valves entirely. Jizz shoots across the room as you feel the bulge of pressure slide up your shaft and out of your urethra; a large pink crystal falls out of an open valve-hole and onto the wet floor with a 'plop'.  ", false);
	}
	output("It is at this point Whitney walks in.\n\n", false);
	...
	//[(corruption > 90, high demonscore)
		...
		output("  Still filled with the residual effects of your sudden burst of energy, you pull off the tubes one by one and, taking a moment to snatch up the pink crystal, saunter over to her. Her eyes widen at the simultaneously horrifying and comical sight of you approaching with lust in your eyes and ", false);
		//[(onecox)
		if(pc.cockTotal() == 1) output("a gigantic, erect penis bobbing in front of you and throwing off your balance", false);
		//(multi)
		else output("multiple gigantic, erect penises bobbing in front of you and dragging between your legs, head pinned against the floor", false);
		output(".  Just in time she backs up and pulls a pitchfork down from the wall. \"<i>I expect I was wrong about you when we met, " + pc.short + ".  Get on out of here now and never come back or I'll make sure you never go anywhere again.</i>\"  Scowling, you break off your advance and head toward the door.  Though you definitely want to fuck her, giving yourself a chance to adjust to your magnificent new body might not be a bad idea either.  After all, no matter what she says you can always come back... meanwhile you vow to find something or someone to rape or turn into your personal cock-milker.  With an amazing ", false);
...
private function centaurToysHoooooo():void {
	...
	output("She clears her throat awkwardly.  \"<i>Uh, well... A pair of centaurs that lived here the last couple of years ran off, looks like.  Gone to get married or something, by the sound of things.  So, hey, wanna help me solve a little problem I'm having?</i>\"\n\n");

	output("You shrug. Sure.\n\n");

	output("\"<i>See, the two of them left some rather personal belongings behind,</i>\" Whitney says, swinging the barn door open.  Inside, lying in the middle of a stall are what looks like a tall, slender totem with a giant rubber horsecock sticking out of it and a fake mare standing on reinforced wooden legs with a glistening, horsey onahole between its thighs.  Oh, my.  \"<i>So, [pc.name], since you look to have the right build for 'em... they're yours.</i>\"\n\n");
