import classes.Characters.CoC.CoCLethice;
import classes.Engine.Combat.applyDamage;
import classes.Engine.Combat.DamageTypes.TypeCollection;
import classes.GameData.CombatManager;
import classes.GLOBAL;
import classes.Items.Melee.CoCLethiceWhip;
import classes.Util.*;
...
public function encounterLethice():void {
	...
	else
	{
		...
		addButton(1, "Consort", joinLethice);
	}
}

private function goFight():void
{
	output("<i>“So be it.”</i> Lethice rises from her throne, pacing pensively back and forth. Wings unfurl from behind her back, casting you into the darkness of her demonic shadow. <i>“You seek a fight with the Queen of Mareth, and you shall have one.”</i> With feline grace, she launches herself dozens of feet into the air, flips forward, and snaps her wings out to catch her midair. <i>“My pets! My lovelies! Watch what this mortal’s foolishness earns " + pc.mf("him","her") + "!”</i>");
	output("\n\nScattered applause breaks out as Lethice wings around to engage you.");
	if(pc.canFly()) output(" Good thing she’s not the only one that can fly!");
	output("\n\n<b>It’s a fight!</b>");
	
	var m:CoCLethice = new CoCLethice();
	if(!pc.canFly()) m.createPerk("Melee Immune", 0, 0, 0, 0);
	
	CombatManager.newGroundCombat();
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(m);
	CombatManager.victoryScene(handleLeticeMaybePCVictory);
	CombatManager.lossScene(CoCLethicePCLoss);
	CombatManager.displayLocation("LETHICE");
	
	clearMenu();
	addButton(0, "Next", CombatManager.beginCombat);
}

private function handleLeticeMaybePCVictory():void
{
	var lethice:CoCLethice = enemy as CoCLethice;
	if (lethice.fightPhase == 1) {
		lethice.phase1Ends(lethice.HP() < 1);
		return;
	}
	if (lethice.fightPhase == 2) {
		lethice.phase2Ends(lethice.HP() < 1);
		return;
	}
	CoCLethicePCVictory();
}
...
public function CoCLethicePCVictory():void
{
	clearOutput();
	flags["COC.MINOTAURKINGS_TOUCH"] = undefined;
	flags["COC.LETHICE_DEFEATED"] = 1;
	var hpVictory:Boolean = enemy.HP() < 1;
	if(hpVictory)
	{
		output("<i>“No,”</i> Lethice cries, dropping onto all fours, barely able to support herself. <i>“How c-can this be...”</i> She shakes her head, tries to stand, and collapses once more. <i>“My reign was to be eternal and unquestioned.”</i> Hatred burns in her eyes. <i>“You ruined it all.”</i> She spits on the ground, then struggles not to collapse into it.\n\n");
	}
	else
	{
		output("<i>“No,”</i> Lethice cries, shaking her head in denial. <i>“How c-can this be? How can you do this to me.”</i> She shudders, dropping onto all fours, barely keeping a hand out of her cunt by sheer force of will. It dances across the slip of fabric at her crotch, threatening to rebel at any moment. <i>“I’m not some ditsy, succubus slut.”</i> A whole-body shiver wracks her from head to toe. <i>“Gods, am I?”</i>\n\n");
	}
	...
	clearMenu();
	addDisabledButton(0, "Plow Her", "Plow Her", "This scene requites you to have cock.");
	addDisabledButton(1, "Queen Her", "Queen Her", "This scene requites you to have vagina.");
	//addDisabledButton(5, "Redemption", "Redemption", "This scene requites you to shutdown factory safely and visit Marae after.");
	
	if(pc.hasCock())
		addButton(0, "Plow Her", plowLethice);
	if(pc.hasVagina())
		addButton(1,"Queen Her",queenLethice);
	addButton(2, "Boob Play", boobPlay, hpVictory);
	
	addButton(3, "Kill Her", killLethicePrompt);
	addButton(4, "Leave Her", leaveHer);
	
	//var hasLethicite:Boolean = pc.hasKeyItem("Sheila's Lethicite") || pc.hasKeyItem("Stone Statue Lethicite");
	//if (hasLethicite && flags["COC.MET_MARAE"] > 0 && flags["COC.FACTORY_SHUTDOWN"] == 1)
...
private function plowLethice():void
{
	...
	output(" [pc.cocks] pushing uncomfortably against your [pc.gear]. Deciding she’s had enough of an ass-beating, you shrug out of your gear and grab your prick, letting it flop mightily between Lethice’s up-turned cheeks. She gasps, clawing her fingers into the seat of her throne but finding no purchase as your [pc.hips] bear down on her. After the spanking you’ve given her, though, Lethice is too lust-addled and weak-kneed to do anything but whimper and lift her spaded tail like a bitch in heat, all but inviting you to plow your cock into her sweet demon’s cunny.");
	if(pc.cocks.length == 1)
	{
		output("\n\nUnless you have better plans...");
		clearMenu();
		addButton(0, "Fuck Pussy", plowLethicePussy);
		addButton(1, "Fuck Butt", plowLethiceButt);
		return;
	}
	output("\n\nOf course, with more than one cock, you can easily take advantage of everything the queen bitch has to offer...");
	clearMenu();
	addButton(0, "Next", plowLethiceDouble);
}

private function plowLethicePussy():void
{
	clearOutput();
	output("Lethice is primed and ready, spanked into submission twice over and drooling with her eagerness for a victorious fucking. You have to wonder when the last time this haughty bitch was put on the bottom - how long has she gotten to play top dog, with nobody to tell her otherwise?");
	output("\n\nGrinning to yourself, you decide it’s high time you put Lethice in her place. You grab your [pc.cock] and drag it through the sodden valley between her legs, slathering yourself in the steaming fuck-juice her first orgasm’s left behind before bringing your [pc.cockHeadBiggest] to rest firmly between the puffy lips of Lethice’s pink fuckhole.");
...
private function plowLethiceButt():void
{
	...
	output("\n\nWith a contemptuous smirk, you push Lethice off her throne once and for all, letting the sodomized slut marinate in the pool of cum you’ve left on the floor. Content in your final victory, you swing yourself into her throne and prop your [pc.feet] up on the Demon Queen’s prostrate back, smearing her face down in your seed. The demons look positively");
	if(pc.cor() >= 66)
	{
		output(" gleeful");
	}
...
private function plowLethiceDouble():void
{
	clearOutput();
	output("You smirk down at Lethice’s winged back and reddened ass, letting your multiple cocks tell her what’s in store. Your [pc.cock] presses against the tight, dark ring of her asshole while the");
	...
	output(" member sloppily nestles between the lips of her pussy, slathering its crown with demonic juices. Lethice can only groan and weakly struggle against your firm, dominant hand when she realizes what you intend.");
	...
	if(pc.isBiped())
	{
		output(" and prop your [pc.feet] up on the Demon Queen’s prostrate back, smearing her face down in your seed");
	}
	output(".");
	output("\n\nAnd now, you’ve got a choice to make...");
	theChoiceDunDunDun();
...
private function queenLethice():void
{
	clearOutput();
	output("The Demon Queen is defeated. Some part of you knew this was where it had to end ever since you first heard of Lethice: with one of you on her knees, completely at the mercy of the other. Now, though... She looks almost... normal, in a way. Her default form, such as it is, seems like nothing more than a tall, lanky girl with horns - she doesn’t even sport the huge, drooling cock and tits of most of her slack-jawed minions standing around. If she closed her eyes and furled back her wings, you could almost imagine her as any of the priestesses from Ingnam.");
	...
	output(" forward, pulling your [pc.gear] off as you loom over the defeated demoness.");
	...
	{
		output(" cupping a tit");
	}
	else
	{
		output(" pinching one of your nipples");
	}
	output(" and arching your back as you feel her long, sinuous muscle slithering inside you like a snake - or a tentacle! Her talents as a shapeshifter become readily apparent as her tongue grows thicker and thicker, yet no less dexterous, spreading your pussy-lips and straining your slit with growing girth.");
	...
	if(pc.hasCock())
	{
		output(" and dragging your [pc.cock] across her face, smearing pre all through her hair");
	}
	output(". She keeps wriggling under you, trying to rest herself free of your pin, but you’re relentless in your determination: she’s not moving from this spot until you cum all over her face!");
	...
	output(". Lethice is lying stunned on the ground, eyes rolled back in her head and face utterly smeared with your orgasmic juices. That’s a good look for you, you decide. Content in your final victory, you swing yourself into her throne");
	if(pc.isBiped())
	{
		output(" and prop your feet up on the Demon Queen’s prostrate back, smearing her face down in your seed");
	}
	output(".");
	output("\n\nAnd now, you’ve got a choice to make...");
	theChoiceDunDunDun();
...
{
	clearOutput();
	output("So this is the great and powerful Demon Queen, brought low");
	if(!hpVictory)
	{
		output(" by your superior sexual acumen");
	}
	else
	{
		output(" by the might of your [pc.mainWeapon]");
	}
	output(". She looks almost... normal, in a way. Her default form, such as it is, seems like nothing more than a tall, lanky girl with horns - she doesn’t even sport the huge, drooling cock and tits of most of her slack-jawed minions standing around. Strange, for a shapeshifter... maybe you can force her to fix that before you reap the fruits of your conquest!");
	output("\n\nYou grab the defeated demoness’s shoulders and");
	if(pc.cor() >= 66)
	{
		output(" throw her roughly onto her back");
	}
	else
	{
		output(" push her onto her back");
	}
	output(". The way she’s dressed, the way she looks so unsettlingly human... Lethice reminds you of an innocent maiden, locking her legs together and lying unresistant before you - like a girl on her wedding night. You lick your lips and tear away the tainted churchwear she’s clad herself in, revealing in full the pert mounds of her small breasts, and the puffy pink nipples at their peaks.");
	...
	if(pc.hasCock())
	{
		output(" They’re easily big enough to fuck now!");
	}
	...
	if(pc.hasCock())
	{
		...
		output("\n\nThankfully, you’re pretty sure the demoness’s grossly swollen tits are more than enough to keep her weighted to the ground. She’s probably put on fifty pounds of boob now, both so full of corrupted milk. You give her a gloating grin and reach up to tweak her nipples, making Lethice cry out around your [pc.cockHeadBiggest]. Now this is more like it!");
	...
	else
	{
		...
		output(" bowels");
		output(", pumping out of the Demon Queen like cum out of a real cock. It feels... good. <b>Really</b> good! The milk might be cold, but you feel a heat start spreading through your loins, spurring you to start really moving - to start taking all the pleasure that Lethice’s mutable body can give you!");
		output("\n\n");
		if(pc.hasVagina())
		{
			output("One cock’s not enough for you now. Not at the end of your adventure, after everything this bitch and her minions have put you through. You need <b>more</b>! You hike");
			if(!pc.isNaga())
			{
				output(" a leg");
			}
			...
			output(" over Lethice’s body, starting to ride her sideways. The feeling of her nipple-cock twisting around inside you, gushing milk all the harder, but it’s not what you’re looking for. Instead, you reach back and grab the other dick-nipple, angling it towards your [pc.asshole]. Like the first, it’s slicked with plenty of tainted milk, making the entrance as easy as you’d like. You give out a shuddering sigh as you sink down on both of the Demon Queen’s dicks, filling your holes with throbbing, milky meat.\n\n");
		}
		output("You start to move, grinding against Lethice’s jiggling rack and drawing her dick-nipple");
		if(pc.hasVagina())
		{
			output("s");
		}
		output(" through you, again and again. The demons around you hoot and holler, driven wild with lust as you rape their queen, and you can’t help but smile.");
		if(pc.cor() >= 66)
		{
			output(" You’re finally where you belong!");
		}
		else
		{
			output(" Even the demons recognize what you’ve done! Now the only question is what to do with them afterwards...");
		}
		output(" For now, though, you sate yourself by riding faster, bouncing on Lethice’s plump chest. Faster and faster, you piston your [pc.hips] until you and Lethice both are moaning with pleasure, your bodies quaking with growing ecstasy.");
		output("\n\nGiving little concern to your corrupted partner’s pleasure, you ride her dick-nipple");
		if(pc.hasVagina())
		{
			output("s");
		}
		output(" as hard and fast as you can, letting purple milk spray from your hole as Lethice’s milky tits cum their corrupted load inside you. She screams, back arching and wings beating like hammers against the stone. But that doesn’t matter to you - not right now, not when you’re so close! You shove the demoness down, pinning her between your pounding hips and the unrelenting stone.");
		...
		output(" from the Demon Queen’s swollen breast");
		if(pc.hasVagina())
		{
			output("s");
		}
		output(", gushing their lactic load inside you. You keep pistoning your hips as fast as you can, dragging out every iota of pleasure that you can before the climax passes over you, leaving you panting and gasping and leaking corrupted fluids from your hole");
		if(pc.hasVagina())
		{
			output("s");
		}
...
{
	clearOutput();
	output("Looking down at the");
	if(hpVictory)
	{
		output(" battered and beaten demon, once so full of haughty pride and self-assured in her power, fills you with a sense of pity.");
	}
	else
	{
		output(" horny, desperate demon before you fills you with a sense of pity.");
	}
	output(" She’s defenseless now - the greatest of the demons has fallen at your hand, but to what end? What about all the other countless demons swarming the vast palace, and the world outside. Merely killing Lethice accomplishes precious little. Another will rise in her place, won’t they?");
...
{
	clearOutput();
	output("<b>A day of travel later...</b>");
...
{
	clearOutput();
	output("What name do you give the new woman you and Marae have made?");
	this.displayInput();
	clearMenu();
...
private function redemptionIIIGoName():void
{
	...
	flags["COC.LETHICE_NAME"] = this.userInterface.textInput.text;
	this.removeInput();
	
	clearOutput();
	if(flags["COC.LETHICE_NAME"] == "Lethice")
	{
		output("You decide to tell her the truth - to give her back the name she bore as the Demon Queen. Trying to hide who and what she was from her - and from the world - will do you no good. The rest of the demons, and those who fight against them, deserve to see that there is hope. That even the Demon Queen could be made whole again.");
...
{
	clearOutput();
	output("You spend some time there on the beach, comforting " + flags["COC.LETHICE_NAME"] + ". Eventually, as the hazy sky begins to darken, you take her hand and board your small boat, pushing off towards land. A new day will soon dawn on Mareth, and with it, you introduce a new hope: a way to fight back against the demons.");
	output("\n\nIt soon comes to light just why " + flags["COC.LETHICE_NAME"] + "’s rebirth was so costly, so tiring, to your patron goddess. Not a week after your victory at the mountain fortress, you and your new companion are ambushed by demons - assassins, bent on exacting vengeance for their queen’s humiliation. Instead, they end up on their knees before you, beaten as all who stood before you always are. This time it’s different, though. This time, rather than the choice of carnal punishment or death, you find yourself given a third option.");
	output("\n\nThe demons carried a few shards of Lethicite, which " + flags["COC.LETHICE_NAME"] + " quickly finds, drawn to the power of it like a moth to flame. Before you have time to realize what she intends, " + flags["COC.LETHICE_NAME"] + " grabs the brilliant pink stones and raises one of them aloft. Her arms and hands begin to glow, radiating that same awesome viridian power that Marae had shown you before, and in a flash of light one of the demons crumples to the ground, orgasming and writhing as the seething corruption in his veins is torn from his body in one last demonic ejaculation. The pool of cum he leaves catches flame, burning away to smoke and cinders a moment later. Corruption eradicated, utterly and completely.");
	...
	output("\n\nWhen she’s finished, " + flags["COC.LETHICE_NAME"] + " collapses to her knees, gasping, chest heaving against her comfortable clothes - she has long since abandoned the twisted habit of her demonic reign. You put a hand on her shoulder, expecting her to be almost ready to collapse as Marae was... but instead, she turns to you with a huge smile. " + flags["COC.LETHICE_NAME"] + " throws her arms around you, laughing like a giddy child.");
	...
	output("\n\n<b>THE END</b>");
	flags["COC.GAME_END"] = GAME_END_REDEMPTION;
	clearMenu();
	//addButton(0, "Next", postEndingChoices);
}

private function joinLethice():void
{
	clearOutput();
	output("Stepping towards the infamous Demon Queen, you");
	if(!pc.hasWeapon())
	{
		output(" raise your empty hands");
	}
	else
	{
		output(" set aside your [pc.mainWeapon]");
	}
	output(" and smile. Lethice cocks a bemused brow, crossing her legs and tracing her slender fingers around the rim of her Lethicite-filled goblet. None of her remaining minions move to stop you, allowing you to approach their queen, right to the foot of her throne.");
	...
	if(flags["COC.DRIDERINCUBUS_KILLED"] != 1 && flags["COC.MINOTAURKING_KILLED"] != 1)
	{
		output(" humiliated");
	}
	else
	...
	output(" by your hand back to back. Compared to you, they were nothing. Pathetic, really. Are they the best the mighty demons have to offer?");
	...
	if(flags["COC.DRIDERINCUBUS_KILLED"] == 1 || flags["COC.MINOTAURKING_KILLED"] == 1)
	{
		output(" Carved a bloody swathe");
	}
	else
	{
		output(" Fucked your way");
	}
	output(" through my keep. And all this to... what? Become my new bodyguard?”</i>");
	output("\n\n<i>“Something like that,”</i> you answer, grabbing the demoness by the hips and yanking her close, tight against your chest so that you can feel her pert breasts hot against your [pc.gear]. She gasps with surprise, but the moment of weakness is just that: momentary. In the blink of an eyes, she’s right back to unflappable, regal dominatrix. One of her legs hikes up around your hip, pressing her crotch against yours, and her hands reach up to cup your cheeks.");
	...
	output("\n\nAn explosion of sensation erupts through your [tongue], spreading as a tingling warmth all through your mouth, throat, and beyond. You’ve barely gotten your tongue out of your mouth before the alien feeling has spread through your entire body, infusing you with renewed lust above and beyond anything you’ve felt for the statuesque demoness. Like a " + pc.mf("man","woman") + " starving, you dig in with desperate abandon, munching that demonic rug with everything you have.");
	...
	clearMenu();
	addButton(0,"Next",joinLethiceII);
}

private function joinLethiceII():void
{
	clearOutput();
	output("<b>Time passes in Mareth...</b>");
	...
	flags["COC.GAME_END"] = GAME_END_CONSORT;
	clearMenu();
	//addButton(0, "Next", postEndingChoices);
}

public function CoCLethicePCLoss():void
{
	clearOutput();
	output("You fall.");
	...
	if(!hpVictory)
	{
		output(" by your own rampant sexual desires");
	}
	else
	{
		output(" beneath the demoness’s merciless assault. She’s more powerful than you could have ever imagined, far more so than a wisp of a girl like her has any right to be");
	}
	output(". Gasping, you slump");
	if(!pc.isBiped())
	{
		output(" down");
	}
	...
	{
		output(" to your knees");
	}
	output(",");
	if(pc.hasWeapon())
	{
		output(" your [pc.mainWeapon] clattering away across the stone");
	}
	else
	{
		output(" too weak to even ball your fists");
	}
	output(" as the victorious demon strides towards you.");
	...
	output("\n\n<i>“So powerful, so self-righteous... and here you are, kneeling before me like so many before you. Still, I have to give you credit, Champion. You got so much further than most:");
	if (enemy.lustQ() > 80 && enemy.HPQ() < 20 && (enemy as CoCLethice).fightPhase == 3) {
		output(" you was really close.");
	}
	else if (enemy.lustQ() > 60 && enemy.HPQ() < 30 && (enemy as CoCLethice).fightPhase == 3) {
		output(" it was a fight to remember.");
	}
	else if ((enemy as CoCLethice).fightPhase == 3) {
		output(" I had to really fight, not just play.");
	}
	else {
		output(" I almost broke a sweat breaking you.");
		if(enemy.lust() < 50 && enemy.HPQ() > 50 && (enemy as CoCLethice).fightPhase != 3) output(" Almost.");
	}
	output("”</i> She licks her lips, and gives your chin a sharp release. <i>“I think you deserve a special... reward... for your efforts, Champion. Oh, except that’s not what you are anymore, is it? A Champion doesn’t bow down, hoping a pretty little demon doesn’t rip " + pc.mf("his", "her") + " soul out, does " + pc.mf("he", "she") + "? No, that’s something I’d expect my dog to do. Is that what you are: a dog?”</i>");
	output("\n\nYou look away from Lethice, trying");
	if(hpVictory)
	{
		output(" to hide just how turned on she’s left you... how close you are to orgasm, just in need of her lurid touch");
	}
	else
	{
		output(" not to let her see how weak you are, how easy it really would be for her to kill you... or worse");
	}
	output(". The Demon Queen just smiles all the wider as she turns from you, striding towards her throne. She stops after a few long-legged paces, casting a gaze over her shoulder to you. <i>“What are you waiting for, </i>dog<i>? Come along...”</i>");
...
private function woofwoof():void
{
	...
	if(pc.cockTotal(GLOBAL.TYPE_CANINE))
	{
		output(" newly-grown doggy cock");
	}
	else
	{
		output(" thick, throbbing dog-member");
	}
	output(" as your swollen knot is yanked from the dragon-girl’s hole. You tumble across the floor with a pathetic whine, drooling spunk from your twitching member. Your beloved Mistress gives you a disapproving scowl as you recover.");
	...
	{
		output("Your predecessor destroyed our welcoming facility. Otherwise I’m sure a certain imp would have revealed the truth to you long ago, before joining you with our breeding machines.");
	}
	else
	{
		output("How did you escape from the factory, I wonder? And so clean and pure, too... your belly’s not yet swollen with imps, and your breasts are so small. Did you slip through");
		if(flags["COC.ZETAZ_DEFEATED_AND_KILLED"] != 1)
		{
			output(" Zetaz");
		}
		else
		{
			output(" the new Warden");
		}
		output("’s fingers, I wonder? Tsk, more punishments to deal out, I suppose. Isn’t that right, " + pc.mf("boy","girl") + "?”</i>");
	}
	pc.lust(pc.lustMax(), true);
	pc.intelligence(pc.intelligence() / 5);
	pc.libido(pc.libidoMax(), true);
	pc.cor(70);
	//dynStats("lus=", pc.maxLust(), "int/", 5, "lib", 100, "sens", 50, "cor", 70, "resisted", false);
	clearMenu();
...
{
	clearOutput();
	output("You bark excitedly, ready to be your loving, forgiving mistress’s means of meeting out punishment. The Champion’s eyes turn to you, and her jaw drops like an anchor.");
	output("\n\n<i>“No... [pc.name]? [pc.name] is that you?”</i> she gasps, staring wide-eyed. <i>“Oh, gods, no...”</i>");
	output("\n\nYou cock your head aside, confused. That’s not your name. You’re Champ. You’re Mistress’s favorite puppy-" + pc.mf("stud","slut") + ".");
	...
	output("\n\nMistress gives your leash a gentle tug, drawing your attention back. <i>“Poor thing. My precious Champ’s starting to get confused. I think we need to shut this pretty little bitch up, don’t we " + pc.mf("boy","girl") + "? Or better yet, make her scream.”</i>");
	...
	output("\n\n<i>“Good " + pc.mf("boy","girl") + ", Champ,”</i> your Mistress murmurs, and as you give a glance over your shoulder, you’re deeply, deeply pleased to see that one of her hands has slipped beneath her priestly garb to pleasure herself. She approves!");
...
private function goKillYourself():void {
	clearOutput();
	output("There is nothing worse than being humiliated and turned into a corrupted, lustful dog who serves only the Queen of the Demons. Death is a better option for sure. Reaching your shaking hand into your pack, you pull out a cyanide pill and pop it into your mouth then swallow. Your mouth foams as your heart stops beating.");
	...
	//pc.takeDamage(pc.HP + 9999, false);
	applyDamage(new TypeCollection( { truedamage : pc.HP() + 9999 } ), pc, pc);
	badEnd();
}
...
{
	clearMenu();
	addButton(0, "Kill Her", killLethicePrompt);
	addButton(1, "Leave Her", leaveHer);
}

private function killLethicePrompt():void {
	//if (pc.weaponVerb == "slash" || pc.weaponVerb == "cleave" || pc.weaponVerb == "keen cut" || pc.weaponVerb == "stab") { // not wanna bother with this shit
		//clearOutput();
		//output("How shall you end Lethice? Do you snap her neck and make it quick? Or do you chop her head off and scare the demons?");
		//addButton(0, "Snap Her Neck", killLethice, 0, null, null, "Snap her neck and make it quick.");
		//addButton(1, "Behead Her", killLethice, 1, null, null, "Off with her head!\n\n(Warning: Contains gore!)");
	//}
	//else {
		killLethice();
	//}
...
private function killLethice(method:int = 0):void
{
	clearOutput();
	if (method == 0) {
		output("Lethice probably deserves the worst sorts of torture imaginable, but you don’t have time to dawdle. You snap her neck and drop her twitching body to the ground. ");
		flags["COC.LETHICE_KILLED"] = 1;
	}
	else if (method == 1) {
		output("Lethice probably deserves a punishment that would fit her foul deeds indeed. You raise your [pc.mainWeapon] and slice through her neck, causing her head to fall to the floor and demonic blood spills forth. You pick up your prize and show it to the demons to let them know not to mess with you. ");
		//awardAchievement("Off With Her Head!", kACHIEVEMENTS.GENERAL_OFF_WITH_HER_HEAD, true, true, true);
		flags["COC.LETHICE_KILLED"] = 2; // 2 indicates Lethice beheaded
	}
	output("The assembled demons scatter at the sight, fearful they’ll fall next - and rightly so. So long as they remain, there’s always the chance another will take her place.");
	postTheChoice();
}
...
private function postTheChoice():void
{
	...
	output("\n\nJust... Something is bothering you... Is something wrong?");
	flags["COC.PLOT_END"] = 1;
	pc.inventory.push(new CoCLethiceWhip());
	clearMenu();
	addButton(0, "Go Home", goHome);
	addButton(1, "ClaimFort", claimFortress);
	addButton(2, "ABORT NOW", abortVRAfterVictory);
}
...
private function goHomeII():void
{
	...
	output("\n\nNo room for loyalty among demons, you suppose. Still, with the information in these books, you can go home. Taking them back to your camp, you work the rituals on the portal to Ingnam, and before you know it, the portal’s hazy mists resolve into a perfectly circle opening, one that leads to a familiar cave.");
	output("\n\nWill you seek vengeance or justice with your return?");
	clearMenu();
...
{
	clearOutput();
	output("<b>Later...\n</b>Tales of the Champion’s return are rarely told, not in the open anyway. " + pc.mf("He","She") + " blew into town with all the subtlety of a cyclone, laying waste to the village’s elders and all who dared defend them. Every villager remembers that night, the night that their leadership was put to the torch - literally. Each of them was left to die inside his own burning mans, bathed in the scorching heat until naught remained but ashes and memories.");
	output("\n\nAfter that night, there wasn’t a rebellious soul left alive to oppose to the Champion’s rule. " + pc.mf("He","She") + " became the defacto " + pc.mf("King","Queen") + " of the village, the breaker of the old ways and forger of the new.");
	if(pc.cor() <= 33)
	{
		output(" Despite the fear and terror of " + pc.mf("his","her") + " arrival, " + pc.mf("he","she") + " proved a fair and wise monarch. The village soon became a town, and that town a city. Prosperity came to those who lived in the valley, guided by the strength of its unshakable ruler.");
	}
	else if(pc.cor() <= 66)
	{
		output(" Despite the fear and terror of " + pc.mf("his","her") + " arrival, " + pc.mf("he","she") + " proved a competent, if harsh monarch. The village’s prosperity was marred only by its limited liberties. Still, it was soon a town, and later a city. Most folk were willing to sacrifice their freedoms for the safety " + pc.mf("he","she") + " provided.");
	}
	else
	{
		output(" Despite the fear and terror of " + pc.mf("his","her") + " arrival, " + pc.mf("he","she") + " proved a capable monarch in spite of " + pc.mf("his","her") + " rapacious tastes. The villagers dared not protest when their once-Champion treated their sons and daughters as little more than slaves. They feared to question where the fruits " + pc.mf("he","she") + " provided came from, and why they felt so strange after eating them. Of course, these concerns did not stop them from breeding. Soon, nothing could. As the population of the valley swelled, the Champion amassed an army, one large enough to rival the nearby countries that had ignored it for so long.");
	}
	clearMenu();
	//addButton(0, "Next", postEndingChoices);
}
...
{
	clearOutput();
	output("<b>Later...</b>\nTales of the Champions arrival spread far and wide. None expected " + pc.mf("his","her") + " return, and yet " + pc.mf("he","she") + " arrived all the same, weapon in hand before the council chambers, demanding the corrupt old men who once lead to face justice for their betrayal. At " + pc.mf("his","her") + " feet were strange creatures, beings the Champion faced and defeated, brought to Ingnam’s world to testify to just how rotten the village’s core had become. Once they said their piece, " + pc.mf("he","she"));
	if(pc.cor() >= 50)
	{
		output(" put an end to their rank existence right there.");
	}
	else
	{
		output(" sent them through a glowing portal of " + pc.mf("his","her") + " own creation, never to be seen again.");
	}
	...
	if(pc.cor() < 50)
	{
		output(" In " + pc.mf("his","her") + " wisdom, " + pc.mf("he","she") + " stopped the townsfolk from lynching their traitorous leaders right there. Instead, they were banished, forever expelled from the lands of Ingnam on pain of death.");
	}
	else
	{
		output(" The townsfolk lynched their traitorous leaders right there and then under the watchful eyes of the Champion. " + pc.mf("He","She") + " nodded approvingly when they gave their last twitches, seemingly happy to let small-town justice run its course.");
	}
	output("\n\nLife continued on. A new council was chosen, but it was Champion [pc.name] that most people followed. " + pc.mf("He","She") + " had left on the cusp of adulthood and returned a grizzled");
	output(" warrior. With a seemingly infallable champion there to defend it");
	if(companionsCount() > 0)
	{
		output(" to say nothing of " + pc.mf("his","her") + " bizarre friends");
	}
	output(", Ingnam prospered. The tiny village soon grew into a bustling town, and later a city.");
	output("\n\nWhen age finally claimed the unexpected " + pc.mf("hero", "heroine") + ", a stone statue of immense proportions was erected so that future generations could forever live under the protection of their greatest hero.");
	clearMenu();
	//addButton(0, "Next", postEndingChoices);
}
...
{
	clearOutput();
	output("This fortress would be a fitting seat to your power. But the question remains, what would you do from such a fortress? You could gather the pure forces of the land together into one army, and from here purge the demon’s corrupted taint from the land, or you could set yourself up in Lethice’s place. Many demons could be turned to serve you, now that you think about it. Surely you could rule better than she!");
	clearMenu();
...
private function HowFuckedIsThePlayerOMeter():Number
{
	...
	if(flags["COC.LETHICE_KILLED"] == 0)
		levelOfFuckedness += 25;
	if(flags["COC.MINOTAURKING_KILLED"] == 0)
		levelOfFuckedness += 10;
	if(flags["COC.DRIDERINCUBUS_KILLED"] == 0)
		levelOfFuckedness += 10;
	if(flags["COC.D3_CENTAUR_DEFEATED"] != 1)
		levelOfFuckedness += 10;
	if(flags["COC.D3_GARDENER_DEFEATED"] != 3)
		levelOfFuckedness += 10;
	if(flags["COC.D1_OMNIBUS_KILLED"] != 1)
		levelOfFuckedness += 10;
	if(flags["COC.ZETAZ_DEFEATED_AND_KILLED"] != 1)
		levelOfFuckedness += 10;
	if(flags["COC.HARPY_QUEEN_EXECUTED"] != 1)
		levelOfFuckedness += 25;
	//if(getGame().camp.campCorruptJojo())
		//levelOfFuckedness += 10;
	if(flags["COC.MARCUS_CHOISE"] == -1)
		levelOfFuckedness += 10;
	if(flags["COC.AMILY_FOLLOWER"] == 2)
		levelOfFuckedness += 10;
	if(flags["COC.PUMPKIN_FUCK_YEAR_DONE"] > 0)
		levelOfFuckedness += 10;
	...
	//if(getGame().camp.ceraphIsFollower())
		//levelOfFuckedness -= 5;
	if(flags["COC.OWCA_SACRIFICE_DISABLED"] == 1)
		levelOfFuckedness -= 10;
	if(flags["COC.SAND_WITCHES_FRIENDLY"] == 1)
		levelOfFuckedness -= 10;
	if(flags["COC.URTA_QUEST_STATUS"] == 1)
		levelOfFuckedness -= 30;
	if(flags["COC.HELSPAWN_NAME"] != undefined)
		levelOfFuckedness -= 10;
	if(flags["COC.FARM_CORRUPTION_FIRST_DATE"] != undefined)
		levelOfFuckedness += 20;
	if(flags["COC.KID_A_XP"] >= 100)
		levelOfFuckedness -= 10;
	if(flags["COC.FUCK_FLOWER_LEVEL"] > 3)
		levelOfFuckedness += 15;
	if(flags["COC.EDRYN_NUMBER_OF_KIDS"] != 0)
		levelOfFuckedness -= 5;
	if(flags["COC.COTTON_KID_COUNT"] != 0)
		levelOfFuckedness -= 5;
	if(flags["COC.ANT_KIDS"] > 0)
		levelOfFuckedness -= 20;
	...
	if(flags["COC.RATHAZUL_IN_CAMP"] == 1)
		levelOfFuckedness -= 5;
	if (flags["COC.CORRUPTED_GLADES_DESTROYED"] >= 100)
		levelOfFuckedness -= 10;
...
{
	//if(pc.statusEffectv1(StatusEffects.Birthed) > 0)
		//return true;
	if(flags["COC.AMILY_BIRTH_TOTAL"] > 0 || flags["COC.PC_TIMES_BIRTHED_AMILYKIDS"] > 0)
		return true;
	if(flags["COC.ANEMONE_KID"] > 0)
		return true;
	if(flags["COC.BEHEMOTH_CHILDREN"] > 0)
		return true;
	...
	if(flags["COC.EDRYN_NUMBER_OF_KIDS"] > 0)
		return true;
	//if(getGame().emberScene.emberChildren() > 0 || flags["COC.EMBER_EGGS"] > 0)
		//return true;
	//if (getGame().isabellaScene.totalIsabellaChildren() > 0)
		//return true;
	if(flags["COC.IZMA_CHILDREN_SHARKGIRLS"] > 0 || flags["COC.IZMA_CHILDREN_TIGERSHARKS"] > 0)
		return true;
	if (flags["COC.JOJO_LITTERS"] > 0)
		return true;
	if(flags["COC.KELLY_KIDS_MALE"] > 0 || flags["COC.KELLY_KIDS"] > 0)
		return true;
	//if (getGame().kihaFollower.totalKihaChildren() > 0)
		//return true;
	if(flags["COC.MARBLE_KIDS"] > 0)
		return true;
	if(flags["COC.MINERVA_CHILDREN"] > 0 || flags["COC.TIMES_BIRTHED_SHARPIES"] > 0)
		return true;
	if(flags["COC.ANT_KIDS"] > 0)
		return true;
	if(flags["COC.PHYLLA_DRIDER_BABIES_COUNT"] > 0)
		return true;
	if(flags["COC.SHEILA_JOEYS"] > 0)
		return true;
	if(flags["COC.SHEILA_IMPS"] > 0)
		return true;
	if(flags["COC.SOPHIE_ADULT_KID_COUNT"] > 0 || flags["COC.SOPHIE_DAUGHTER_MATURITY_COUNTER"] > 0)
		return true;
	if(flags["COC.SOPHIE_EGGS_LAID"] > 0)
		return true;
	if(flags["COC.TAMANI_NUMBER_OF_DAUGHTERS"] > 0)
		return true;
...
private function purgeShit():void
{
	...
	if(levelOfFuckedness <= 50) {
		...
		if (flags["COC.FACTORY_SHUTDOWN"] != 2)
			output(" Marae reclaimed the full network of her roots and single-handedly purged the last holdouts of corruption from the lands, choking it out of her world like weeds from a garden.");
		else
			output(" Marae fought for a time, but even corrupted goddesses cannot hold back your tide of purity. Her fall was the biggest blight on your campaign, a sad footnote on an otherwise successful reclamation.");
		output("\n\n<b>You ended the demon threat forever and saved Mareth.</b>");
		flags["COC.GAME_END"] = GAME_END_PURGE_0_50;
	}
		...
		output("\n\n");
		if (flags["COC.FACTORY_SHUTDOWN"] != 2)
			output("Marae is a wonderful aid in your quest, always able to manifest in her newly purified roots and offer words of wisdom or encouragement. Her magical might is a rock you can lean upon when times are tough. You aren’t sure how long you could keep this up without her.");
		else
			output("Marae is always there, always lending her power to the demons, even when you dig her roots out of the ground with shovels and axes, burning the greasy lumber into nothingness. This task would be so much easier if the goddess was on your side, but still, you persevere.");
		...
		if (flags["COC.FACTORY_SHUTDOWN"] != 2)
			output(" Those few pockets that escaped your armies could not escape Marae’s all-encompassing roots. Mareth is purified once more.");
		else
			output(" A few places are tainted still, but it’s impossible to cleanse every cave. Instead, you make instructions to maintain the army. Should evidence of corruption arise anywhere, your forces will swiftly put it to flame.");
		output(" Peace returns to the land. You did it. You saved this world.");
		...
		flags["COC.GAME_END"] = GAME_END_PURGE_50_100;
	}
	else {
		...
		flags["COC.GAME_END"] = GAME_END_PURGE_100;
	}
	output("\n\n<b>THE END</b>");
	clearMenu();
	//addButton(0, "Next", postEndingChoices);
...
private function conquer():void
{
	...
	if(pc.cor() <= 33)
	{
		output("You could definitely run this place better than Lethice, and though she had many corrupted lieutenants, surely their worst depredations could be reigned in by your steady hand. You");
		if(companionsCount() > 0)
		{
			output(" gather your follower");
			if(companionsCount() > 1)
			{
				output("s");
			}
			output(" in the citadel itself and");
		}
		output(" familiarize yourself with the place. Rather than driving off the few remaining demons you find, you place them into your service, explaining that so long as they serve you loyally they may yet live.");
		output("\n\nWord gets out over the coming days about the new " + pc.mf("King","Queen") + " of the demons, and little by little, the masterless hordes find their way back.");
		if(flags["COC.LETHICE_KILLED"] == 0)
		{
			output(" Lethice herself is even brought back, though this time securely leashed and empty-eyed.");
		}
		output(" You order the worst of their factories shut down and put an end to attempts to locate Tel’Adre. No more will the demons be all-consuming force.");
	...
	else if(pc.cor() <= 66)
	{
		...
		if(companionsCount() > 0)
		{
			output("your followers from");
		}
		output(" your camp away from the old portal and into the fortress. With their organization broken, the demons will hardly be in any position to mount an organized invasion into your world, and once you cow them into submission, they’ll work for you. Luckily, a few hung around after the fight. Either they weren’t close enough to the throne to hear the commotion or they didn’t care.");
		...
		output("\n\nThe changes are slow. Years pass, but inevitably, you see the wisdom in becoming an immortal demon. How else will you save the poor wretches of this plane from themselves?");
		output("\n\n<b>You’ve won, but at what cost?</b>");
		flags["COC.GAME_END"] = GAME_END_CONQUER_MED;
	}
	else
	{
		output("For all her might, Lethice had no idea what she was doing. She subsumed herself in corruption to no effect, washing away her reason in order to consistently and constantly overindulge in pleasure.");
		...
		output(". With Lethice’s rule broken, there’s no danger of a demonic invasion of your homeland, and you’ll have any surviving rogues in hand soon enough.");
		...
		output("\n\nAge is your only problem, but your servants devise draughts to keep you young, treatments to keep your flesh as firm and vigorous as the day you stepped through the portal. <b>You rule Mareth as its ageless " + pc.mf("King","Queen") + ", a harsh monarch whose word is law and whose whims are made manifest.</b>");
		
		flags["COC.GAME_END"] = GAME_END_CONQUER_HIGH;
	}
	output("\n\n<b>THE END</b>");
	clearMenu();
	//addButton(0, "Next", postEndingChoices);
...
private function postEndingChoices():void {
	...
	clearMenu();
	if (flags["COC.GAME_END"] == GAME_END_CONSORT)
		addDisabledButton(1, "Continue", "Nuh-uh. Mareth is too fucked up to let you continue and it's all due to the consequences you've made.");
	else
		addButton(1, "Continue", postEndingReturn, null, "Continue", "Continue the game so you complete anything you've missed.");
}

private function postEndingReturn():void {
	clearOutput();
	output("You may have defeated Lethice and completed the main story but the fun isn't over! It's time for you to return to the game and begin a new era of Mareth.");
	
	//awardAchievement("Demon Slayer", kACHIEVEMENTS.STORY_FINALBOSS, true, true, false);
	//if (pc.level <= 1) awardAchievement("Ultimate Noob", kACHIEVEMENTS.CHALLENGE_ULTIMATE_NOOB, true, true, false); //Lethice beaten at level 1!
	pc.HP(pc.maxHP());
	...
	itemCollect([new CoCLethiceWhip()]);
}