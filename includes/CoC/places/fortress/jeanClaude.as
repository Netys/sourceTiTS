...
import classes.Characters.CoC.CoCJeanClaude;
...
public function gogoFuckTheseBasilisks():void
{
	...
	output("You breathe deep, steel yourself")
	if (pc.hasKeyItem("Laybans")) output(", put your shades on");
	output(" and stride out onto the gantry. The plan is simple. You’ll keep your eyes up and march across as if you belong there, as if you are on official business.");
	if (pc.demonScore() > 4) output("  You can’t be mistaken for anything but a demon, after all.");
	output("  If or when the lizards realize who you are, you’ll make a break for it, and hopefully by then you’ll be in reach of the exit. It’s straight-forward, it’s elegant, it will work, whatever your hammering pulse has to say on the matter.");

	output("\n\nYou try and not look down as you proceed, back straight and as aloof as you can.");
	
	if (pc.isNaga()) output("  You’re probably imagining things but the passage of your sinuous form over the wire surface seems absurdly loud, an endless rubbing of sandpaper which seems to fill the hall.");
	else if (pc.isGoo()) output("  Godsdamn this form... you try not to think as you slide across the wire surface, intensely aware of the slimy sound you make as you go, of the steady drip of your fluids through the mesh, trying not to think of it splashing down on curious, blunt heads.");
	else output("  Clack, clack, clack. You are intensely aware of the sound of your [pc.feet] on the wire surface. You close your eyes and tell yourself you’re only imagining that it’s loud, that it echoes around the hall, that the basilisks would have to be deaf not to hear you.");
	...
	else
	{
		...
		if (pc.reflexes() < 40 && !CoCBasilisk.canLookIntoEyes(false))
		{
		...
		else
		{
			...
			if (!pc.canFly()) output(" almost");
			else output(" literally");
			output(" fly towards the door. A solitary basilisk manages to clamber onto the catwalk ahead of you- and you simply bull-rush straight through him, his deathly eyes useless against your screened vision. You think you feel the passage of sickle claws scything fruitlessly through the air inches away from your back as you bundle through the exit and into the empty observation booth. In an ecstasy of terror you fumble with the door and manage to slam it shut, just as several heavy bodies thud into the other side. You breathe a huge sigh of relief as your hands find a deadbolt and slide it across, sealing the basilisks off from you. They hammer at it for a while longer, shouting and hissing angrily, but eventually, whilst you’re still gathering your breath, they get fed up with it and leave.");
			...
			clearMenu();
			addButton(0, "Next", move, "COC FORTRESS MAGPIE HALL N");
		}
	}
}
...
public function capturedByBasilisks():void
{
	...
	if (pc.hasVagina()) output(" your [pc.vagina] beading with excitement");
	if (!pc.hasCock() && !pc.hasVagina()) output(" your loins aflame with excitement");
	output(" as you sink further into the yellow warmth, immersing yourself in it, so small and pliant and willing to be shaped by that fathomless, over-arching will, this huge masculine creature with a body of carved strength, with his feral, demanding smell surrounding you, with his big, luscious cocks....  The other basilisk’s spell is fading from your limbs, allowing you to move, but it is no respite at all- the more you stare into the eyes in front of you now, the weaker you feel, the more you want to puddle onto the floor and supplicate yourself before their power.");
	...
	if (pc.willpower() >= 40 || CoCBasilisk.canLookIntoEyes(false))
	{
...
{
	clearOutput();
	output("With all your willpower you tear away from the monster’s lidless gaze. His sharp claws graze your cheeks as you throw yourself backwards, turn and charge with a defiant cry into the ranks of reptiles which surround you. You have your [pc.mainWeapon] loose, and you swing it this way and that, adrenaline and desperation lending you strength as you send basilisks flying... but there are hundreds of them, and one of you, and too many places not to look. You strike, you punch, you scratch, you bite, but eventually your limbs tire; they seize up as you tear yourself away from one grey gaze only to find another, and the basilisks simply have to surround you, pack you tight with their bodies while one of them stares deep into your eyes, flooding your mind with a blaring, hypnotic wash. There’s a sound coming from somewhere: a strange, staccato bark. It’s only when the lizards return you frozen to the feet of their king that you realize what it is. The Rude King is laughing, sat on a table and doubled up with gulping mirth.");
...
private function challengeBasilisksGo():void
{
	clearOutput();
	output("With magnificent insolency you move directly to the someone who is clearly the basilisks leader.");
	...
	output("\n\nYou draw your weapon and set your jaw. You’ve gotten your one shot, and you can’t waste it.");
...
{
	clearOutput();
	output("You stare into the yellow, hypnotic eyes and for a moment see beyond them, see them as part of this being rather than the whole, see the arrogance that sits behind them, see, perhaps, a way out of this. With all your willpower you force your mouth to open, work your vocal chords- and make a 'pfft' noise.");
...
private function fightJeanClaude():void
{
	CombatManager.newGroundCombat();
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(new CoCJeanClaude());
	CombatManager.victoryScene(beatDaFuckhugeBasilisk);
	CombatManager.lossScene(fuckhugeBasiliskFuckedYouUp);
...
public function beatDaFuckhugeBasilisk():void
{
	...
	if (pc.mf("m", "f") == "f" && silly) output("  You raise your fists in glee and crow “<i>I AM THE LIZARD QUEEN!</i>” at the ceiling.");
	...
	output("\n\n“<i>Que- quels sont vos ordres, " + pc.mf("mon roi", "ma reine") + "?</i>” he whispers.\n\n");
...
private function goAway():void
{
	...
	flags["COC.D3_BASILISKS_REMOVED_FROM_MAGPIE_HALL"] = 1;
...
public function fuckhugeBasiliskFuckedYouUp():void
{
	clearOutput();
	
	if (pc.lustQ() >= 100)
	{
		output("You look down at the [pc.mainWeapon] in your hand in bleary confusion. Why are you holding it? Everything feels slow and close, your body almost painful with arousal and sensitivity, and you can’t push beyond it, can’t fathom where you are, what is happening, who you are. It’s the golden glow- it surrounds you and blots everything else out. When Jean-Claude steps in and almost kindly grabs your [pc.mainWeapon] out of your unresisting hands and takes you by the chin, you almost groan. So big, so powerful, so in control- everything you are not. You gladly stare into his maddening yellow eyes, gladly intensifying the feeling of delicious helplessness they give you.");
...
{
	if (pc.hasPerk("Basilisk Womb")) bwombBadEnd();
	else regularBadEnd();
}
...
private function regularBadEnd():void
{
	...
	output("The Rex’s yellow glow encompasses you, glaring out everything - your purpose, your memories, your sense of self, replaced only by the need to stare deeper and deeper into the pits of gold, the depthless lava of his will which rolls over you, heating you within and without. When he steps into you and puts his warm, smooth hand on your waist you almost flinch, you are that sensitive to his presence. He grins with the exhilaration of a conqueror as he draws you further into himself. The nagging undercurrents of your own body rise to the surface as you feel his warmth and smell his powerful musk;");
	if (pc.hasVagina()) output(" your [pc.vagina] beads with excitement");
	if (pc.hasCock() && pc.hasVagina()) output(" and");
	if (pc.hasCock()) output(" [pc.eachCock] hardens");
	if (pc.hasCock() || pc.hasVagina()) output(" as"); 
	output(" your body reacts uncontrollably to the presence of the being whose claws reach deeper and deeper into your mind, touching and triggering your basest desires.");
	if (pc.isNude()) output("  You realize vaguely you already have taken your clothes off, undoubtedly your own subconscious preparing you for this moment.");
	else output("  You really are uncomfortably hot underneath the glare of those eyes. Hardly even aware you’re doing it, you begin to take off your [pc.gear]. Piece after piece falls to the floor until you are naked before him.");
	...
	if (pc.skinType != GLOBAL.SKIN_TYPE_LATEX)
	{
		...
		if (pc.hasFur()) output("  Your fur drops away in ragged clumps as you drink."); 
		output("  Once you are finished another black egg is pushed into your hands, then another... you sigh as the viscous gloop slides down your throat and your flesh begins to thrum, your pores puckering and then shrinking into themselves as your skin begins to gleam with the clean starkness of rubber. You slowly examine yourself once the transformation is complete, marvelling at your living, latex flesh. Yes. This feels right.");
	
		output("\n\nJean-Claude watches, sharing in your admiration of your new body before stepping in to casually touch you himself. He slides his hand down your flank, rounding your [pc.butt] before suddenly digging his claws cruelly into your rubber flesh. This time you do flinch, unable to stop yourself gasping as the pain trades with your heavy arousal.");
	}
	else
	{
		output("\n\nHe slides his hand down your flank, rounding your [pc.butt] before suddenly digging his claws cruelly into your rubber flesh. This time you do flinch, unable to stop yourself gasping as the pain trades with your heavy arousal.");
	}
	...
	if (pc.hasVagina()) output(" your [pc.vagina] dribbling excitement");
	if (pc.hasVagina() && pc.hasCock()) output(" and");
	if (pc.hasCock()) output(" your [pc.cock] straining helplessly");
	output(" to this depth of slutty surrender you have plumbed, your neck muscles seizing up and your limbs tightening as the thoughts sink into you.");
	...
	if (pc.isBiped()) output("  The transformation isn’t quite complete; your mouth opens as a will not your own forces your arms and legs to open, leaving you completely exposed. It takes you a few seconds to realize that your mouth opening wasn’t your doing, either. It won’t close.");
	else output("  The transformation isn’t quite complete; your mouth opens as a will not your own forces your arms to open and your monstrous bottom half to take on a stance as close to complete submission as it will allow, your [pc.hips] flared to the world. It takes you a few seconds to realize that your mouth opening wasn’t your doing, either. It won’t close.");
	...
	{
		output("\n\nHe reaches down and, bending his claws away, puts his warm, smooth hand to your [pc.vagina]. Already aroused beyond measure, you seep femcum ardently as he circles your [pc.clit] and moves his digit down your channel. Grinning, he moves his hand upward, sliding over a nipple teasingly before pressing his finger against your mouth, wiping the black fluid onto your frozen lips, making you taste it.");
	...
	output(" push roughly against your [pc.asshole]");
	if (pc.hasVagina()) output(" and [pc.vagina]");
	output(", it disappears into the depths forever. Your mind is a blank canvas of pure, submissive pleasure and it, like your limitless future, is golden.");
...
private function bwombBadEnd():void
{
	...
	if (pc.vaginalCapacity(0) < 24)
	{
		output("\n\n“<i>Ungh! Tight little bitch,</i>” he gasps. He thrusts into you with wicked force and you gasp, grasping at his scaled shoulders, upon the threshold of pain and exquisite pleasure as he spreads your [pc.vagina] wide with his huge girth. “<i>Been saving yourself for this? Good girl.</i>”");
