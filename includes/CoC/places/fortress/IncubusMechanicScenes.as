...
{
	if (flags["COC.FACTORY_INCUBUS_DEFEATED"] > 0 || flags["COC.FACTORY_INCUBUS_BRIBED"] > 0) return true;
	return false;
}
...
public function incubusMechanicMeetAtElevator():void
{
	if (flags["COC.D3_MECHANIC_LAST_GREET"] == MECHNIC_NO_SELECTION || flags["COC.D3_MECHANIC_LAST_GREET"] == undefined)
	{
		output("\n\nA");
		if (metIncubusMechanicInD1()) output(" familiar,");
		output(" demonic mechanic lounges against a gleaming, metallic control panel");
		if (flags["COC.FACTORY_INCUBUS_BRIBED"] > 0) output(", a rolled-up hentai magazine tucked neatly into the chest-pocket of his overalls");
		if (metIncubusMechanicInD1()) output(". This is the same incubus you met back in the factory! He still sports his familiar, stained coveralls, and the crotch is as torn as ever, revealing a cock that looks like an odd hybrid of a tentacle, a bumpy demon-dong, and a stinger-ringed anemone cock. While you watch, it idly divides into two... four... eight narrow, writhing tendrils before merging back together into a three-inch-thick monster-cock.");
		else output(". He sports a pair of stained coveralls, with the crotch torn open, revealing a cock that looks like an odd hybrid of tentacle, a bumpy demon-dong, and a stinger-ringed anemone cock. While you watch, it idly divides into two... four... eight narrow, writhing tendriles before merging back together into a three-inch-thick monster-cock.")
		output(" It stiffens under your ocular attentions, and a timeless voice with the smoothness of silk, yet an undercurrent of rumbling gravel calls, \"<i>Eyes up here, Champion.</i>\" ");
		
		applyDamage(new TypeCollection( { tease : 5 + pc.libido() / 20 } ), pc, pc);

		output("\n\nScowling a little at being caught so openly studying his erection, you level your gaze at the haughty incubus.");

		output("\n\n\"<i>" + (metIncubusMechanicInD1() ? "Once more, you bumble" : "You've bumbled") + " into a hornet's nest, none the wiser for your previous experience. Did you expect not to get stung? Did you expect that you'd simply meander up to the queen without being torn apart by her loyal drones?</i>\" He smirks at the word 'drone', one corner of his mouth twisting in a way that stretches his infernal face into a handsome, sardonic smile. Smiling wider, he admits, \"<i>Well, you did get this far, and I'd rather not risk bruising my fantastic body in a fight, so how about this: I let you ride the lift down to safety, and you suck my cock - or give me 500 gems, I guess.</i>\"");

		output("\n\nHe's offering you a deal?");

		output("\n\nThe incredulity must be plain on your [pc.face], because his smile parts with an edge of worry. \"<i>Listen, I just like to get paid, slack off, and fuck. I'd rather not risk my life if I can help it. So how about it? You get a safe exit, and I get my dick sucked... or get a little richer, maybe.</i>\"");
		...
		addDisabledButton(0, "Pay Toll", "Pay Toll", "You do not have enough gems to pay the required toll!");
		if (pc.credits >= 5000) addButton(0, "Pay Toll", payDaToll);
		addButton(1, "Suck Dick", suckIncubusDick);
		addButton(2, "Fight", fightIncubusAtLift);
		if (pc.canFly()) addButton(3, "Flight", useOwnWings, undefined, "Flight", "You have your own [pc.wings], why bother with lift?");
	}
	else if (flags["COC.D3_MECHANIC_LAST_GREET"] == MECHANIC_SUCKED)
	{
		...
		addDisabledButton(0, "Pay Toll", "Pay Toll", "It appears that your only options are to walk away, suck his cock, or fight him.");
		addButton(1, "Suck Dick", suckIncubusDick);
		addButton(2, "Fight", fightIncubusAtLift);
		if (pc.canFly()) addButton(3, "Flight", useOwnWings, undefined, "Flight", "You have your own [pc.wings], why bother with lift?");
	}
	else if (flags["COC.D3_MECHANIC_LAST_GREET"] == MECHANIC_PAID)
	{
		output("\n\n\"<i>Back again?</i>\" the incubus queries while he reclines against the softly thrumming machinery. \"<i>I was just calibrating my baby here. Now she purrs like a kitten.</i>\" He affectionately pats the metal devices that power the lift. \"<i>If you want to use her, you're going to need to pay up, [pc.name]. 500 gems seems fitting recompense, but between you and me, I'd rather have you on your knees polishing my python here.</i>\" The mechanic's cock suddenly rears up, swaying back and forth in a decidedly snake-like motion, the light glistening off his purple-hued pecker as it stiffens encouragingly.");
	...
	else // if (flags["COC.D3_MECHANIC_LAST_GREET"] == MECHANIC_FOUGHT)
	{
		addButton(0, "Lift", useLiftPostDefeat);
		if (pc.canFly()) addButton(1, "Flight", useOwnWings, undefined, "Flight", "You have your own [pc.wings], why bother with lift?");
	}
}

public function fightIncubusAtLift():void {
	CombatManager.newGroundCombat();
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(new CoCIncubusMechanic());
	CombatManager.victoryScene(beatDaMechanic);
	CombatManager.lossScene(mechanicFuckedYouUp);
...
{
	flags["COC.D3_MECHANIC_LAST_GREET"] = MECHANIC_PAID;
...
{
	clearOutput();
	output("You spend a minute or two fiddling with the lift controls, trying to understand how, exactly, to control the contraption. When you think you've got a handle on the system, you tug on a lever with authority and hastily step onto the platform.");
...
private function useOwnWings():void
{
	clearOutput();
	output("Not bothering with slow lift, you just jump down from the platform");
	if(flags["COC.D3_MECHANIC_LAST_GREET"] != MECHANIC_FOUGHT) output(", followed by rather indifferent mechanic's gaze");
	output(", and spread your [pc.wings] to glide down.  You aren't bothered by a single harpy along the way. This section of the high mountains seems to have been cleansed of them.");
	
	output("\n\nYou head back to camp with all due haste.");
...
{
	flags["COC.D3_MECHANIC_LAST_GREET"] = MECHANIC_SUCKED;

	clearOutput();
	
	if (pc.credits < 5000) output("You spare a glance to your depleted pouches and admit, \"<i>I guess I'll suck your dick then.</i>\"");
	else output("You shrug and smile. \"<i>I guess now is as good a time as any to suck some dick, huh?</i>\"");
	...
	output("\n\nScowling a little, you go ahead and get down off your [pc.feet], bringing your [pc.face] level with the pulsating, sexual monstrosity that seems to be growing larger by the second. This close, you can make out every fast-pumping vein. His tumescent crown pulsates, the helmet thickening and darkening with pleased engorgement. The demonic nodules that line the head fill, becoming more distinct: so hard, sensitive, and ready to stimulate you with their exotic texture.");
	if (pc.hasVagina())
	{
		output(" You find your crotch becoming");
		if (pc.wetness() <= 2) output(" even more soaked than normal");
		else if (pc.wetness() <= 4) output(" wet with fresh lust");
		else output(" sticky with lust")
		output(" at his masculine scent and appearance.")
	}
	else if (pc.hasCock())
	{
		output(" You feel a heady, turgid warmth growing in your own crotch, mirroring the ardor displayed by the cock you're about to service.");
	}
	output(" A patch of neatly trimmed pubic hairs lead up his abdomen to a set well-defined muscles, half-hidden under his loose not-so-coveralls.");
...
public function beatDaMechanic():void
{
	var hpVictory:Boolean = enemy.HP() < 1;
	
	flags["COC.D3_MECHANIC_LAST_GREET"] = MECHANIC_FOUGHT;

	clearOutput();
	output("The mechanic collapses, his overalls");
	if (hpVictory) output(" falling off his prone from, shredded.");
	else output(" hanging open as he reaches down for his exposed, dangling cock to wildly masturbate.");
	output(" He seems surprisingly human like that, just as mortal as anyone. You could let him go, play with him some, or cleanse the world of another demon.");

	clearMenu();
	addButton(0, "Kill", killMechanic);
	addButton(1, "Let Go", letMechanicGo, hpVictory);
	if (pc.hasCock()) addButton(2, "Buttfuck", buttfuckTheMechanic, hpVictory);
	if (pc.hasVagina()) addButton(3, "Ride Cock", rideMechanicsCock);
}

private function killMechanic():void
{
	flags["COC.D3_MECHANIC_FIGHT_RESULT"] = MECHANIC_KILLED;
	clearOutput();
	output("He proves blessedly easy to kill, and you roll the body off the cliffs to avoid alerting any of Lethice's other ilk.\n\n");
...
{
	flags["COC.D3_MECHANIC_FIGHT_RESULT"] = MECHANIC_RELEASED;
	clearOutput();
	output("Feeling merciful, you point off the side of the cliff. <i>\"Leave this place and never return. If you do, I might have to kill you. I'd rather not shed any more blood than necessary, but I will do what I have to in order to cleanse this land. Leave and never return. Maybe you can find your own peace without driving everything around you to corruption.\"</i>");

	output("\n\nThe incubus");
	if (hpVictory) output(" slowly struggles to his feet");
	else output(" reluctantly stops masturbating");
	output(", nodding. His skin cracks as two leathery wings unfurl. <i>\"If that's how it has to be.... For what it's worth, I hope you win. Lethice is a bitch.\"</i> Slowly walking towards the cliff, he beats his wings and lifts off the ground. \"<i>Come visit me sometime. Maybe I can show you a good time as thanks.</i>\"");
...
{
	flags["COC.D3_MECHANIC_FIGHT_RESULT"] = MECHANIC_FUCKED;

	var x:int = pc.cockThatFits(enemy.analCapacity());
	if (x == -1) x = 0;

	clearOutput();
	output("The incubus may have done his best to seduce you with his");
	if (!hpVictory) output(" throbbing hard");
	else output(" gently bobbing");
	output(" dick, but you've got a better idea. You got more than one look at the curvature of his butt while you were fighting, tempting your burgeoning bulge to the point of distraction. You hastily remove your [pc.gear] and smile. [pc.EachCock] springs up, full and hard. You take");
	if (pc.cocks.length > 1) output(" one");
	else output(" it");
	output(" in hand, pumping slowly as you eye your tainted prey. He whimpers");
	if (hpVictory) output(" in need, and");
	else output(", but");
	output(" his cock seems to get even bigger and harder. The poor guy doesn't realize what a reaming he's in for.");
	...
	output("\n\nYou grab [pc.oneCock] with one hand and the incubus' waist with the other. Your heart is hammering in anticipation. Growling in the back of your throat, you align");
	if (silly) output(" tab C with slot buttfuck");
	else output(" your eager tool with the corrupted anus");
	output(" and steadily push. At first, his asshole shuts you out. The sphincter is just too tightly clenched for anything, let alone something so full and thick, to fit inside. You give up on the push and begin rocking your [pc.hips] back and forth, getting his soon-to-be-stretched hole used to the idea of having you inside him. A droplet of pre-cum escapes your [pc.cockHead " + x + "] as you work, smearing over him.");

	output("\n\nThe incubus's now-slickened pucker quivers, and that's all the opening you need. Your [pc.cockHead " + x + "] slips in during a moment of looseness. In that second, you turn his crinkly rosebud into a smoothly stretched o-ring. The incubus moans, though whether in pleasure or pain, you cannot tell. You slap his ass and feel him clamp down on you with renewed vigor, briefly arresting your progress, but he can't squeeze that hard all the time. His asshole relaxes after a few seconds and lets another inch in.");
	
	output("\n\n\"<i>Just relax and let it happen,</i>\" you instruct as you slowly squeeze another inch inside him. The incubus' insides are just so... so tight around your length and so warm compared to the outside air. You lean down over him, listening to him grunt, enjoying the feel of his entrance slowly slackening as he obeys. A huge blob of pre or cum (you can't be sure which) splatters into the growing puddle below him as your [pc.cockHead " + x + "] inexpertly milks his prostate. The incubus shakes from head to toe, and you feed yourself in to the halfway point.");
	...
	{
		output("\n\nYou pull");
		if (pc.hasKnot(x)) output(", popping your knot");
		output(" out after depositing your meager load into your demonic fuck-hole. One [pc.cumVisc] strand of [pc.cumColor] [pc.cumNoun] is leaking out over the demon's ballsack.");
	}
	//Medium cum exit
	else if (pc.cumQ() <= 1000)
	{
		output("\n\nYou pull");
		if (pc.hasKnot(x)) output(", popping your knot");
		output(" out of the demon's creampied fuck-hole with a smile. His stretched-out ass looks virtually glazed with [pc.cum], and you can see small rivers of it trickling down over his balls and slowly shrinking cock.");
	}
	//High cum exit
	else if (pc.cumQ() <= 2500)
	{
		output("\n\nYou pull");
		if (pc.hasKnot(x)) output(", popping your knot");
		output(" out of the demon's well-fucked asshole with a smile. As soon as the bulk of your dick unplugs him, a river of [pc.cum] spills out, washing over his balls and cock to drip down into his puddle. You smirk at the [pc.cumColor]-glazed demon. His butt is still full of your spunk.");
	}
	//Huge cum
	else
	{
		output("\n\nYou pull");
		if (pc.hasKnot(x)) output(", popping your knot");
		output(" out after depositing a massive, belly-bulging load directly into the demon's guts. A tidal wave of [pc.cum] bursts out of his over-pressurized asshole to fall on the ground, and as it dies off, small rivers of [pc.cumColor] spunk escape the lake in his ass to run down his balls and cock. His belly is");
		if (pc.cumQ() <= 5000) output(" pudgy");
		else if (pc.cumQ() <= 10000) output(" swollen");
		else output(" absolutely gravid");
		output(" with the distended weight of the [pc.cumNoun] packing his body. From time to time, fresh streamers of sperm squirt out of his thoroughly cream-pied backdoor.");
	}

	//No new paragraph
	output("The incubus slumps down into the puddled semen, gasping in exhaustion.");

	output("\n\nYou wipe your " + pc.cockDescript(x) + " off on his practically glowing bottom before turning to re-dress. When you turn back, he's gone, though a trail of [pc.cumColor] drops indicates the direction of his flight. It looks like he fled towards the cliff, perhaps growing wings. Oh well, that's one less obstacle between you and Lethice.\n\n");
...
{
	flags["COC.D3_MECHANIC_FIGHT_RESULT"] = MECHANIC_FUCKED_YOU;

	clearOutput();
	output("Well, he does have quite the thick, throbbing piece of male genitalia on display for you.... There's no sense in letting a good cock go to waste, right? You stalk over to him, looming over the defeated demon with a predatory grin. Your [pc.tongue] flicks out to lick your lips. Meanwhile, your lower ones go suddenly and irrevocably slick, excited and instantly ready for a helping of thick demon-cock. Your [pc.vagina] is getting warmer and warmer as you tease the poor demon, leaving him to wonder if you're going to kill him or fuck him. The worry is as obvious on his face as the upraised tumescence that jabs spear-like out from his crotch, aimed straight at you.");

	output("\n\nAbruptly, you drop your [pc.butt] down on top of him, sitting square on top of his chest");
	if (pc.hasTail()) output(", [pc.tails] draped across his face");
	output(", and casually begin to undress. Looking over your shoulders, you see hope in his soulless eyes. You tease, \"<i>I wouldn't want to stain my clothes when I finish you off, would I?</i>\" His dick wilts a little at that, and he looks about to faint. Giggling, you toss aside your [pc.gear] and grab hold of his mostly-hard cock, feeling the corrupted, textured nodules that encircle his shaft flaring up at your gentle squeeze. He firms up nicely in your grip, filling back to a pulsating, rigid hardness in seconds.");
...
{
	flags["COC.D3_MECHANIC_COCK_TYPE_SELECTION"] = cType;
	...
	else if (cType == MECHANIC_HORZ_GOG)
	{
		...
		enemy.cocks[0].cLengthRaw = 22;
		enemy.cocks[0].cThicknessRatioRaw = 1.2;
	}
	//Dogdick
	else if (cType == MECHANIC_DOG_COCK)
	{
		output("Giving the incubus's sweaty tool an affectionate squeeze, you decide, \"<i>Grow a dog-cock. Let's see if I can take the knot, shall we, pet?</i>\"");
		...
		enemy.cocks[0].cThicknessRatioRaw = 1;
		enemy.cocks[0].knotMultiplier = 3;
	}

	//ALL CONTINUE TO THIS:
	output("\n\nYou dare not delay any further. Your [pc.vagina] is so hot and slick with lube that it feels almost like it's steaming. It aches for penetration. Lifting yourself up off your [pc.legs], you guide your wanton honeypot up until it's poised just above the " + enemy.cockDescript());
	if (pc.wetness() <= 2) output(", letting your free-flowing pussyjuice wash over it, lubricating it with your glorious [pc.girlCum]");
	else if (pc.wetness() <= 3) output(", letting your oh-so-juicy vagina drip and dribble, soaking it with your copious [pc.girlCum]");
	else output(", letting your [pc.girlCum] drip over it");
	output(". You stuff two fingers inside and stroke deep before pulling them out and using them to spread you wide open. The " + enemy.cockDescript() + " pulsates with every beat of its demonic owner's heart, needing you every bit as much as you now need it.");

	output("\n\nLetting your [pc.legs] go slack, you press down upon the incubus' length. His " + enemy.cockHead() + " presses poignantly at your entrance, throbbing gently. Small blooms of warm tickle over your lips, warm and wet as he leaks his liquid excitement into your nethers, lubricating you further. You relax a bit more");

	var mCockArea:Number = enemy.cockVolume(0);
	var pCuntArea:Number = pc.vaginalCapacity();
	var tightFit:Boolean = false;

	//Easy fit -- Player cunt can fit at least 50% more cock
	if (pCuntArea >= mCockArea * 1.5)
	{
		output(" and feel the girthy boner start to slide deeper inside you. Your capacitative cunt hugs and squeezes tightly about its new best friend, guiding it deep into your heated channel. You slide on down, reveling in the blissful sensation of being properly filled for once, ecstatic as you realize just how hard this demon's dick is going to make you cum. You're so busy riding the thrill of pleasure that you barely notice when you bottom out above his balls");
		if (cType == MECHANIC_HORZ_GOG) output(", your belly bulging with his equine shape");
		else if (cType == MECHANIC_DOG_COCK) output(", your lips slurping noisily about his knot");
		output(".");
	}
	//Decent Fit - Player cunt can fit between 95% - 149% of the total cock area
	else if (pCuntArea >= (mCockArea * 0.95))
	{
		output(" and feel the girthy boner slowly start to slide through your clinging lips. He's so big and yet fits so well. It's like his " + enemy.cockDescript() + " was made to violate your channel - to slip and to slide through you while keeping you oh-so-pleasantly full. You're so busy enjoying the agonizingly slow pleasure that you barely notice when you bottom out above his balls");
		if (cType == MECHANIC_HORZ_GOG) output(", your belly bulging with his equine shape");
		else if (cType == MECHANIC_DOG_COCK) output(", your lips slurping noisily about his knot");
		output(".");
	}
	...
	{
		tightFit = true;
		output(" and feel the girthy boner slowly pry your lips further apart. He's so goddamn big that it feels like it's barely fitting inside. The first inch eventually slips into your tightly-clinging lips, forcing an inadvertent clench to run up the length of your channel. Your [pc.vagina] noisily squelches around the slowly inserting member. You're stretched so full, and you're loving every moment of it. His "+ enemy.cockDescript() +" is so goddamn filling, and you've only taken the first few inches! The feel of him sliding through you is so fulfilling, so all-consuming, that you barely notice your [pc.butt] touching down against his crotch");
		if (cType == MECHANIC_HORZ_GOG) output(", your belly bulging with his equine shape");
		else if (cType == MECHANIC_DOG_COCK) output(", your lips slurping noisily about his knot");
		output(".");
	}
	...
	{
		tightFit = true;
		output(" and feel the immensely girthy boner butt up against your lips, too wide to slip even the tiniest bit inside. Grunting in frustration, you grind against him, slicking his head further and applying more force. You push harder and harder, actually bending his rigid shaft slightly from the force. He groans in pain and pleasure. Suddenly, your voice joins his. His " + enemy.cockHead() + " finally powers through your sopping gates and into your tunnel, stretching it painfully wide as it goes. The intensity of the combined pain and pleasure shocks you so badly that you nearly faint, and your [pc.legs] go limp, forcing your body to slide down the belly-stretching meat-pole. You quiver and cry, sobbing words that sometimes sound like no and sometimes sound like yes, filled with more cock than you ever hoped to handle.");
	}
	pc.cuntChange(0, enemy.cockVolume(0), false, false, false);
	
	//Merge together
	output("\n\nYou hold yourself atop him and let your abdominal muscles flutter around the incubus' "+enemy.cockDescript() +" like an organic sextoy, getting used to holding him inside you. Turning to look back over your shoulders, you place a hand upon his chest feel his well-defined muscles. You stop to admire his nipple, tugging at it a little bit. He growls in irritation, but you just squeeze his dick with your");
	if (tightFit) output(" well-stretched");
	else output(" stuffed");
	output(" twat to silence him.");
	...
	if (flags["COC.FACTORY_INCUBUS_BRIBED"] > 0) output(" Nothing but hentai mags for you to drain your balls with....");
	output("</i>\"");
	...
	if (cType == MECHANIC_HORZ_GOG) output(", the head flaring to seal itself against your cervix");
	else if (cType == MECHANIC_DOG_COCK) output(", the knot popping in just in time to start bloating, locking you in place");
	output(", and jism begins to shoot out in waves.");
	if (!pc.isPregnant())
	{
		output(" Your empty womb drinks up the virile, seed-filled sludge with abandon.");
		if (cType == MECHANIC_HORZ_GOG) output(" After all, that bestial horse-cock doesn't really leave it anywhere else to go. The demonic animal-cunt just continues spurting straight on in to have its way with your eggs, and you're sure there isn't a contraceptive this side of Mareth that could stop you from having his imps.");
		else if (cType == MECHANIC_DOG_COCK) output(" After all, that immense knot doesn't really leave it anywhere else to go. Once every fold is bathed in alabaster doggie-cream, the continued ejaculations start pouring in after your eggs. His knot is so big and stretching you so wide; you don't think he would pop out even if he filled your belly to look nine months pregnant. Worse, or perhaps better, you're sure with how thick and virile his seed feels that you'll be getting pregnant with his imps from this.");
		//pc.knockUp(PregnancyStore.PREGNANCY_IMP, PregnancyStore.INCUBATION_IMP);
	}
	else
	{
		output(" Your blocked womb deflects the seed, forcing it all to stay in your channel");
		if (cType != MECHANIC_DOG_COCK) output(" and drip out over his balls, much to his chagrin.");
		else output(" and stretch you further.");
	}

	output("\n\nThe feeling triggers your own climax. Your [pc.vagina] clenches down around his " + enemy.cockDescript() + " and gushes out around it, splattering the demon's crotch and legs the whole way down to his hooves. Arcs of [pc.girlCum] actually jet across the air");
	if (pc.wetness() < 4) output(", even though you aren't normally a squirter");
	output(". The pleasure is so great that you can feel electric impulses arcing between neurons, short-circuiting your throughs into involuntary ecstasy. Your eyes roll back, and your [pc.hips] twitch and shake, vibrating around the object that brough you to this perfect");
	if (!pc.isPregnant()) output(" insemination");
	else output(" tryst");
	output(".");
...
public function mechanicFuckedYouUp():void
{
...
private function maleLossToMechanic(hpVictory:Boolean):void
{
	...
	if (!hpVictory)
	{
		...
		{
			output("\"<i>You're so... average. I suppose you'll do, if the succubi feel like having a drab, everyday orgasm.</i>\" The incubus shrugs. \"<i>A dick will do...</i>\"");
		}
		//Big cocks
		...
		{
			output("\"<i>Seriously?</i>\" the incubus asks. \"<i>How am I supposed to work with something so gargantuan?</i>\" He disdainfully pulls a vial from his pocket and upends it over your crotch. The effect is immediate. [pc.Eachcock] shrinks from ridiculously large down to horse-sized. <i>\"There, that's better.\"</i>");
			//Shrink down to 18" by 3"
			for each (var item:CockClass in pc.cocks) 
			{
				if(item.cLengthRaw > 18) item.cLengthRaw = 18;
				if(item.cThicknessRatioRaw > 1) item.cThicknessRatioRaw = 1;
			}
		}
		...
		if (pc.cocks.length == 1) output(" your");
		else output(" a");
		output(" length inside any hole and thrust until you're quivering in orgasmic release. That gorgeous demon-cock taunts you, swaying just out of reach.");
	}
	//HP Loss
	else
	{
		output("Dropping prone, you gasp for breath, struggling against your injured body to rise. Your muscles burn with the effort, exhausted from the fight, and ultimately, you fail to stand. Slumping down on the ground, you're forced to come to terms with your defeat. The incubus has won. You came all this way, gave your best, and got smacked down by a lower demon for all your troubles. Some champion you are.");

		output("\n\nLeering down at you, the incubus swiftly tears away your [pc.gear], his fingers suddenly razor-sharp claws. He is precise in his attentions, leaving your [pc.skin] pristine and unbroken. You get the impression he doesn't want damaged goods. He regards [pc.eachcock] with interest, judging them.");
		...
		{
			output("\n\n\"<i>Seriously?</i>\" the incubus asks. \"<i>How am I supposed to work with something so gargantuan?</i>\" He disdainfully pulls a vial from his pocket and upends it over your crotch. The effect is immediate. [pc.Eachcock] shrinks from ridiculously large down to horse-sized. <i>\"There, that's better.\"</i>");
			
			//Shrink down to 18" by 3"
			for each (var item2:CockClass in pc.cocks) 
			{
				if(item2.cLengthRaw > 18) item2.cLengthRaw = 18;
				if(item2.cThicknessRatioRaw > 1) item2.cThicknessRatioRaw = 1;
			}
		}
	...
	{
		output("\n\nThe incubus gathers your [pc.multiCocks] together in his hands, pressing them together into a column of compressed cock. His touch feels so nice that you piston your [pc.hips] against him, trying to fuck his hands, but he holds you firmly. A warm tightness, pleasant and yet entirely unyielding presses down against your tips, so tight that whimper at the slight pinch of pain. You crane your head, careful not to lose your mouth's grip on its leaking fixation. What you see nearly makes you spit it out.");
		...
		if (pc.cockTotal() != pc.cockTotal(GLOBAL.TYPE_DEMONIC)) output(" The skin of your transforming length is darkening to a purplish hue, and in the wake of the magical cock-ring, nodules are slowly emerging, growing into an imitation of the cock you're still suckling.");
		
		output(" The incubus chuckles, easily pushing the ring the rest of the way down, smiling at the way your demonic tip pulsates and leaks pre-cum. Once it's nestled firmly against your groin, your new master withdraws his hands (and his cock). <b>Your singular, demonic cock is impossibly hard.</b>");
	}
	else
	{
		//Single
...
{
	clearOutput();
	output("An agonizingly perfect pussy is suddenly pressing down on your cock, subsuming it in slick, hot depths that cling and squeeze with undulations so skillful you'd be cumming already if it weren't for the magic ring.");
...
{
	clearOutput();
	output("The champion finally got to cum once Lethice had her turn, but by that point, pussy was all " + pc.mf("he","she") + " could think about. Release came more frequently with the demon queen's permission. It didn't really matter, though. The champion was addicted to riding on the edge of climax, broken into nothing more than a demonic vibrator.");

	badEnd(); // G-G-G-GAMEOVER.
}
...
private function errybodyelseLossToMechanic(hpVictory:Boolean):void
{
	...
	//Genderless HP Loss
	else if (!pc.hasVagina() && hpVictory)
	{
		output("Collapsing onto the ground, you look up at the demonic aggressor with hate in your eyes, unwilling to admit defeat even when staring full-on into its face. The incubus, for his part, simply smiles down at you as he steps closer, claws clicking noisily on the smooth, tiled pavement.");

		output("\n\n\"<i>Is that all the mighty champion is capable of? Feeble resistance that ends itself before lust can be properly awakened? I am... disappointed in you,</i>\" the incubus admits as he kneels down to look at conquered prize. \"<i>Still, Lethice will be pleased to have you at hand at last. I think I might get to keep you.</i>\" He playfully manhandles his foot-long meat to smack against your [pc.face], forcing you to get a good feel for his tainted cock, to smell lurid, sexual scent.");

		output("\n\nIt makes your [pc.asshole] tingle hungrily. More hungrily than it's ever felt.");
		...
		output("\n\nWhile you are distracted, he unhooked your [pc.gear], casting it aside, already forgotten. Like this... he can take you more easily. No! You've got to fight this. He gently strokes your [pc.hair] and laughs. You clench needily, aching to feel him inside you. Acting while you're distracted, the incubus suddenly presses something smooth and cold against your exposed ass. Something flows out of it into you! It was a bottle. You squeeze down, trying to force it out, but it stays stubbornly inside you. The incubus pulls it out a moment later, but the damage is done. Increadible warmth gathers in your crotch. With it comes arousal. In seconds, you're panting, and lurid wetness spreads out from your crotch, leaking from <b>your new vagina.</b>");

		pc.createVagina();

		output("\n\nGods, your pussy is wet, but you've got to fight it! You have to hold on, maybe bite him or something to give you the time to recover and get back on the offensive! Yeah... biting might do the trick. You inhale to steel yourself for the oral assault, getting a nice, long whiff of his supremely potent pheromones in the process. Your [pc.clit] gets so stiff that you start writhing your [pc.hips] in order to squish your cunt-lips tight around. <i>Mmmm, gotta attack...</i> you think, but your heart just isn't in it anymore. You open your mouth, letting him push his tip inside, so that you can bite him, of course. Oh, the flavor is so intense... so strong and masculine and perfect on your tongue that you decide NOT to chomp down. Not yet. You can spend a little time recovering first, with his wonderfully tasty, throbbing-hot demon-cock on your happily licking tongue.");

		output("\n\nSoon, those licks turn to sucks, and those sucks into a devilishly pleasurable blowjob. One hand winds to your steaming box while the other gropes at your [pc.fullChest],");
		if (pc.hasFuckableNipples()) output(" slipping inside a [pc.nipple] to thrust just like your other hand. You work your upper and lower pussies like a woman possessed.");
		else
		{
			output(" pinching and tugging on");
			if (pc.totalNipples() > pc.totalBreasts()) output(" nipple after nipple, your myriad breastly endowments providing you with oh so many buttons to tend to");
			else output("your [pc.nipples] one after another");
		}
		output(" Alas, all good things must come to an end, and your lovely, lovely little lollipop is rudely pulled from your questing lips with a noisy pop.");

		output("\n\nYou give a mewl of disappointment and open your eyes to see the incubus standing back a few feet, smirking at you, his cock hard, jutting, and leaking. That simply won't do! A dick like that... it needs to be sucked... tended to... until it can cum! You struggle out of your [pc.gear] and up onto your [pc.feet], staggering towards it with your hand still firmly lodged in your [pc.vagina]. You've got to tempt him back to you, or catch him at the very least.");
	}
	//Lust Lawss
	else if (pc.hasVagina() && !hpVictory)
	{
		output("You stagger about drunkenly, stuffing your hand down your [pc.gear] to get at your needy, aching [pc.vagina]. You feel like a molten-hot font of lust and a delirious, jizz-craving slut all at the same time. Nothing short of hours of endlessly rutting on a virile, demonic cock for hours will quench the flames of your ardor and give you the pleasure you now crave. You whimper, looking up at the incubus imploringly as you try to get him to take you, yanking open your [pc.gear] to show off your splayed-open twat and busily pumping fingers. You even pull them out lick clean, hoping he'll enjoy the show as much as you enjoy your own taste.");
	}
	//HP Lawss:
	else
	{
		output("Collapsing onto the ground, you look up at the demonic aggressor with hate in your eyes, unwilling to admit defeat even when staring full-on into its face. The incubus, for his part, simply smiles down at you as he steps closer, claws clicking noisily on the smooth, tiled pavement.");
		output("\n\n\"<i>Is that all the mighty champion is capable of? Feeble resistance that ends itself before lust can be properly awakened? I am... disappointed in you,</i>\" the incubus admits as he kneels down to look at conquered prize. \"<i>Still, Lethice will be pleased to have you at hand at last. I think I might get to keep you.</i>\" He playfully manhandles his foot-long meat to smack against your [pc.face], forcing you to get a good feel for his tainted cock, to smell lurid, sexual scent.");
		output("\n\nIt makes your [pc.vagina] wet. Wetter than it really should be.");
		output("\n\nSmiling, the overall-clad corruptor drags his thick tool up your cheek and across your lips, leaking his slime as he goes, and he gleefully watches you try to maintain your disdain in the face of his lascivious influence. ");
		output("\n\nGods, your pussy is wet, but you've got to fight it! You have to hold on, maybe bite him or something to give you the time to recover and get back on the offensive! Yeah... biting might do the trick. You inhale to steel yourself for the oral assault, getting a nice, long whiff of his supremely potent pheromones in the process. Your [pc.clit] gets so stiff that you start writhing your [pc.hips] in order to squish your cunt-lips tight around. <i>Mmmm, gotta attack...</i> you think, but your heart just isn't in it anymore. You open your mouth, letting him push his tip inside, so that you can bite him, of course. Oh, the flavor is so intense... so strong and masculine and perfect on your tongue that you decide NOT to chomp down. Not yet. You can spend a little time recovering first, with his wonderfully tasty, throbbing-hot demon-cock on your happily licking tongue.");
		output("\n\nSoon, those licks turn to sucks, and those sucks into a devilishly pleasurable blowjob. One hand winds under your [pc.gear] to tend to your steaming box while the other gropes at your [pc.fullChest],");
	...
	output("\n\nThe source of your lust is soon bending double to fasten more bindings around your waist and [pc.hips], attacking them to more chains by the sounds of it. Soon after, the devices lift you up off the ground such that you");

	// 9999 not sure about these
	if (pc.isNaga()) output("r coils don't get sore");
	else if (!pc.isBiped()) output(" don't even need to support yourself anymore");
	else output(" knees don't get sore");
	output(". How wonderful! If you move just right, you can make the whole contraption sway slightly to get a proper dick-sucking rhythm going. His pearly pre-cream oozes onto your tongue, and you swallow as much of it as you can.");
	...
	output("\n\nHe smiles and affectionately tussles your [pc.hair]. \"<i>Silly girl. Your pussy wasn't meant to be fucked and filled.</i>\"");

	output("\n\nYou plead with red-rimmed eyes, imploring him to fuck your [pc.vagina]. Of course it is! It's all you want right now! It'll be all you ever want if he does it right!");
	...
	output("\n\nThe demon must be able to read the question in your eyes, because he nods understandingly. \"<i>Yes, little mortal, this draught is going to make you wetter and hornier than you can possibly image. That cute little twat you've got is going to be my own personal lubricant-fountain. I suspect you may even manage to learn how to bring yourself off by shifting your [pc.hips] to make your lips rub your [pc.clit] just right.</i>\" He shrugs. \"<i>I don't really care, so long as that cunt of yours stays wet enough to shame the goo-girls at the lake.</i>\" His hand ruffles your hair once more before he steps back to watch you drink this the milky, medicinal goo.");

	output("\n\nYou can already feel the first few swallows affecting you. Your [pc.skin] is flushing, worse than before, and your nether-lips are getting so wonderfully slippery that you can feel your moisture beading on your mons. Your [pc.clit] fares little better, swelling up nearly double its old size");
	if (pc.clitLength > 5) output(", even if that makes it truly monstrous indeed");
	output(". It drips long, sticky strands of girlish lust as you swallow, only becoming more soaked as time passes as your body becomes even more productive. A plaintive whimper escapes around the edges of your phallic gag, but your cry falls on deaf ears. The incubus is too busy fitting pipes together beneath you to care.");
