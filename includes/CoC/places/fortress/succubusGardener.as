import classes.Characters.CoC.CoCSuccubusGardener;
...
{
	if (flags["COC.D3_GARDENER_DEFEATED"] > 0) return true;
	return false;
}

public function gardenerKilled():Boolean
{
	if (flags["COC.D3_GARDENER_DEFEATED"] == 3) return true;
	return false;
}

public function gardenerFucked():Boolean
{
	if (flags["COC.D3_GARDENER_DEFEATED"] == 2) return true;
	return false;
}

public function gardenerLeft():Boolean
{
	if (flags["COC.D3_GARDENER_DEFEATED"] == 1) return true;
	return false;
}
...
public function fightTheGardener():void {
	...
	CombatManager.setHostileCharacters(new CoCSuccubusGardener());
	CombatManager.victoryScene(fuckUpTheGardener);
	CombatManager.lossScene(surrenderToTheGardener);
	CombatManager.displayLocation("SUCCUBUS\nGARDENER");
	CombatManager.beginCombat();
}

public function fuckUpTheGardener():void
{
	clearOutput();
	output("The succubus drops to her knees, grabbing a tentacle and thrusting it into wanton sex forcefully enough to make you wince. She doesn't seem to mind, in fact, her lips spread into a wordless 'o' of pleasure as she begins rocking back and forth atop, lost to her own lusts. She's in no state to stop you from moving on. Hell, she's probably going to be busy with the tentacles for a long, long time. You suppose you could try and put her mouth to use, but there are a LOT of tentacles awfully close. It might be best to move on or end her quickly.");

	clearMenu();
	addButton(0, "End Her", endGardener);
	addButton(1, "Leave", leaveGardener);
...
private function tentaFail():void
{
	...
	output("There's no way you're going to pass up an opportunity to put a succubus's lips to use. Pillowy");
	if (pc.hasVagina()) output(" cunt-kissers");
	else output(" cock-suckers");
	output(" like those are one in a million.");
	
	if (flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] > 0) output(" Perhaps Scylla's ruby mouth could give her a run for her money, but there's only one way to be sure.");
	//else if (fellatrixSucked() > 0) output(" Perhaps Dominika's all-consuming mouth could give her a run for her money, but there's only one way to be sure.");
	output(" You grab hold of the blubbering slut by her curled horns, admiring the way the tentacle-juice on her lips shines in the light, and pull her against your crotch, pressing her against");
	if (pc.hasCock()) output(" [pc.oneCock]");
	else if (pc.hasVagina()) output(" your [pc.vagina]");
	else output(" the sensitive skin of your groin");
	output(". She eagerly goes to work, moaning from the attentions of the tentacle down below, and licking with such fervor that you'd think this is what she wanted all along.");

	output("\n\nRocking your [pc.hips] to the pace of her short, frenzied lips, you lose yourself in the moment. Here in the garden with a succubus to serve your every whim, it's easy to just stop and enjoy the little things in life. Lethice can sit on her throne for the few minutes it will take you to");
	if (pc.hasCock()) output(" blow a wad down this slut's throat");
	else if (pc.hasVagina()) output(" cream yourself on this slut's face");
	else output(" teach this bitch how to pleasure an asshole");
	output(". None-too-quiet slurps hang in the steamy air, accompanied by your own " + pc.mf("grunts","coos") + " of enjoyment. You reach down to cup one of her breasts, ignorant of the shadows creeping nearer.");
	...
	if (!pc.hasCock()) output(" Its outline is clearly phallic, but how could such a huge thing ever fit inside you, let alone anyone?");
	else
	{
		output(" Its tip oozes lubricants, but the gaping pussy at its tip is big enough to hold six minotaurs' monster-cocks.");
		if (pc.biggestCockLength() < 36) output(" How could you ever hope to fill it?");
		else output(" How long were they preparing this for you?");
	}
	...
	if (!pc.hasCock()) // 100% gonna have a cooch by this point
	{
		output("\n\nYou can feel your");
		if (createdVag) output(" new");
		output(" entrance giving around the girthy monstrosity with each passing second. Wider and wider, your flesh opens up. It feels so big that you wonder if you're going to rip in half, but there is no discomfort, only the pleasure of accepting it's tumescent, inhuman length. You groan as your own fluids make themselves known, soaking your gaped lips in girlish cum. There's no way that you should be able to take this, let alone enjoy it, but you are. Oh gods, how you are!");
	...
	{
		output("\n\nThat monstrous, hungry-looking pussy wastes no time in descending towards your [pc.cocks].");
		if (pc.biggestCockLength() <= 12) output(" The ease with which you slide inside is no surprise, given the disparity in size.");
		else if (pc.biggestCockLength() <= 30) output(" The ease with which you enter is no real surprise, given that even your bloated length"+ ((pc.cocks.length > 1) ? "s are" : " is") +" small by comparison.");
		else if (pc.biggestCockLength() <= 48) output(" You slide in with ease, perfectly matched to the hungry fuck-tunnel in size.");
		else output(" Sliding in must take quite the effort. The orange-hued twat is drooling around you, slowly working down an inch at a time. It takes all of you, even if it has to stretch beyond all reason to do so.");
		output(" It feels good, better");
		if (pc.biggestCockLength() <= 12) output(" than a giant that looks that loose should. You'd swear it was tight little twat from how firmly it's squeezing you!");
		else if (pc.biggestCockLength() <= 30) output(" than you had thought at a glance. Not only is it the perfect size, but it seems to hug and touch every part of your maleness" + ((pc.cocks.length > 1) ? "es" : "") + " just right.");
		else if (pc.biggestCockLength() > 48) output(" than you would expect given how tightly-stretched it looks. You'd think it'd be pressing down painfully, but it feels tailor made to take you instead.");

		//Dicks need to grow
		if (pc.biggestCockLength() <= 30)
		{
			output("\n\nGlowing orange goo leaks from the oversized slit in thick beads. You briefly wonder what purpose the odd coloration could serve when the feeling of your cock"+ ((pc.cocks.length > 1) ? "s" : "") +" stretching answers. It's making you grow bigger, somehow! The sensation is akin to stretching long-dormant muscles... an subtle flexing of unrealized potential that makes you aware of just how much you can do, or in this case, how big you can get. Your vision swims as your body works to keep up with the sudden change, and you close your eyes to keep from sicking up.");

			output("\n\nThe tentacle starts sliding up and down. At first, it's motions are slow and languid, giving you plenty of time to feel the supple slickness of its interior texture against you, but as your mass increases, so too does the speed of its up-and-down pumping motion. Even when it's pushing down, it somehow maintains a gentle suction that the velvety walls are tight against you, allowing you to subsume yourself in slippery cunt. The bigger you get, the better it feels. You aren't sure whether you're simply feeling more and more pussy at once or if you're somehow getting more sensitive, but it's enough to make your eyes roll back and your [pc.cocks] practically piss pre-cum.");

			output("\n\nWhimpering from more ecstasy than you know what to do with, you feel yourself surge in size again. It's impossible to tell just how big you've gotten with your view forced to look straight ahead. All you can be sure of is that it's getting bigger, and you're feeling a LOT of pussy. By comparison, the dozens of other tentacles surrounding you are barely there. It's like your cock" + ((pc.cocks.length > 1) ? "s are" : " is") + " the only part of you worth feeling, and the rest of you is just floating in a void - a sensation that strengthens with every inch of length and girth that you gain.");

			output("\n\nCumming almost comes as a surprise to you, but there's only so much pleasure a body can receive before it helplessly and completely cumming, spraying thick wads of jism into the tentacle's hungry folds with instinctive convulsions. Every squirt leaves you a few inches longer and a good deal thicker. Only after emptying every ounce of cum from your [pc.balls] does the feverish expansion finally halt.");

			output("\n\nUnfortunately, the sensations don't. Even though the tentacle has stopped moving to digest its meal, you're still hyper-aware of the feeling of its slick folds against you. There's enough sensitivity in your enhanced package that unmoving pressure is giving your nervous system more feedback than your old dick's most vigorous fuck. The glowing, orange goo wasn't just making you bigger! It was stimulating nerve growth in your [pc.cocks], making you so sensitive that you can't help but stay hard, even after cumming.");
		...
		{
			output("\n\nGlowing orange goo leaks out around your base" + ((pc.cocks.length > 1) ? "s":"") + ". That must be what passes for the tentacle's girlcum. It feels like normal lubricant, at least. It doesn't burn or numb you, and it certainly doesn't seem to be making you grow any more. Wait... you close your eyes and focus on the feel of it. It's... it's better than normal lubricant! You're not sure why or how, but it feels subtly slicker than a normal pussy, and not in a bad 'no friction' kind of way. Every fold and silken caress still rubs your nerves just right. How can it be so slick and yet so powerfully soft and gripping?");

			output("\n\nThe inside of the tentacle contracts slightly, yet the accompanying pleasure is anything but slight. It feels like the whole world tightens against your [pc.cocks], cradling them in cotton softness girded with velvet silk. You moan and cum, flooding the tube with the creamy goo of your release. Your body tries to thrust, but you're held completely immobile, forced to climax to the tentacle's tune. The ropes of pure pleasure are soon squirting to the timing of the gloriously textured fuck-tunnel, and it is only when your orgasm ends that you realize what the orange goo is doing to you; it's making you more sensitive.");

			output("\n\nYou don't go soft after you finish squirting. You don't even get to catch your breath. The orange channel might be holding still as it digests your load, but your dick" + ((pc.cocks.length > 1) ? "s are" : " is") + " still getting more sensitive. You don't even need motion for the clutching walls to arouse you any more, and once it starts moving... you're not sure you'll be able to handle it.");

			output("\n\nA few seconds later, you find out that you can't handle it when the tentacle strokes you. The wave of skittering, electric pleasure overloads your mind, and breakers for your consciousness shut it down while your dick" + ((pc.cocks.length > 1) ? "s throb" : "throbs") +", loving every moment of it.");
...
private function endGardener():void
{
	clearOutput();
	output("You circle behind her an put and end to her evil while she is busy with her pet, then turn to walk away. When you look back over your shoulder, her body is gone. Nothing remains but an empty pathway.\n\n");

	flags["COC.D3_GARDENER_DEFEATED"] = GARDENER_KILLED;
	processTime(5 + rand(10));
	CombatManager.genericVictory();
}

private function leaveGardener():void
{
	clearOutput();
	output("Figuring that the succubus's pets can keep her busy indefinitely, you turn away. A shriek of pleasure draws your attention back, and you turn in time to see dozens of coiling, leafy masses encircling her every limb, bodily carrying her into a wall of twisting, leaking cocks and pussies. Her orifices are stuffed with pumping lengths that froth with spit and girlcum, and her eyes, equal parts alarmed and aroused, widen before disappearing into the forest of green.\n\n");

	flags["COC.D3_GARDENER_DEFEATED"] = GARDENER_LEFT;
	processTime(5 + rand(10));
	CombatManager.genericVictory();
}

public function surrenderToTheGardener():void
{
	...
	if (pc.hasCock() && !pc.hasVagina())
	{
		maleLossToGardener(hpVictory);
	}
	
	// Genderless
	if (!pc.hasCock() && !pc.hasVagina())
	{
		femGenderlessLossToGardener(hpVictory);
	}
	...
	if (pc.hasVagina() && pc.hasCock())
	{
		if (rand(2) == 0) femGenderlessLossToGardener(hpVictory);
		else maleLossToGardener(hpVictory);
	}
}

private function femGenderlessLossToGardener(hpVictory:Boolean):void
{
	clearOutput();
	output("You drop to the ground, defeated and degraded");
	
	if (hpVictory) output(", not even able to push your bruised body up onto its [pc.feet].");
	else output(", too turned on to try and stop your hands from diving into your needy crotch.");
	...
	if (flags["COC.FACTORY_SHUTDOWN"] == 2) output(" destroy");
	else output(" shut down");
	output(" our factory would fall to a simple succubus and her lowly garden?\"</i> She strides over to you, hips wiggling in pleasure, and kneels alongside you, running her hand over your abdomen like a girl handling a new pet for the first time. <i>\"You're going to be so useful in my garden!\"</i>");
	
	output("\n\nHer garden?");
	if (!hpVictory) 
	{
		output(" You cast a furtive glance towards the writhing vines.");
		if (pc.hasVagina()) output(" Your underthings were already swampy with need, but the sight of watching those eager vines squirm elicits fresh trickles of lube to course down your [pc.legs].");
		else output(" You were already horny, but you find yourself wishing you had a pussy, just so you could take even more of them inside you.");
	}
	else output(" You cast a worried look towards the writhing vines. They seem so eager, yet they hold back for now, perhaps waiting for their mistress to finish. No... you can't go down like this!");
	
	output("\n\nGently removing your [pc.gear] a piece at a time, the succubus coos, <i>\"Are you looking forward to it yet, [pc.name]? Being wholly, totally embraced, every hole filled with pulsing, eager lengths?\"</i> She strokes your cheek in a gesture that you would mistake for affection from anyone but a demon.");
	if (pc.hasVagina()) output(" Against your better judgment, you're starting to get wet from the promises of her words. You know you're being given to a neverending hell of sexual stimulation, but you can't stop your body's libido from responding to the idea.");
	output(" <i>\"Yeah, you want it, doncha?\"</i> Slim fingers deftly press at your");
	if (!pc.hasVagina())
	{
		output(" bare crotch, sinking into a crease that didn't exist a moment before. Her skilled touches and dark powers mold your body like a sculptor's clay, crafting a magnificent cunt on your crotch while simultaneously bringing it to a dripping wet state.");
		pc.createVagina();
	}
	else
	{
		if (pc.wetness() <= 2) output(" moistening");
		else output(" soaked");
		output(" crease, and it's all you can do not to cry out and thrust back against her.");
	}
	
	output("\n\nThe succubus idly toys with your [pc.vagina], sometimes rubbing her thumb");
	if (pc.balls > 0) output(" across your [pc.sack],");
	else if (pc.hasCock()) output(" across the underside of your [pc.cock],");
	else output(" against your [pc.clit],");
	output(" confiding aloud, <i>\"To be honest, I'm a little jealous of you.\"</i>");
	...
	output("\n\nSmiling sadly, the succubus pulls her fingers away. A web of your own clear lubricant hangs suspended between her fingers and your moistened box for a moment, then snaps. <i>\"You're going to be a wonderful mother my for pets, I just know it.\"</i> She loops an arm under your shoulder and lifts you to your [pc.feet]. <i>\"Now, lets get you what you want, dear.\"</i>");
...
{
	clearOutput();
	output("Your fall doesn't take you very far. A cushion of interceding plants stops you long before any harm could befall you, holding you nearly horizontally above the rich, loamy earth. There's a bit of a salty-sweat scent in the air. You can't quite identify it, but then again, you don't really have time to. Bands of floral flesh are circling around your arms and [pc.legs]. They aren't constrictive. As a matter of fact, they're kind of comforting - like being surrounded in a lovers arms. Of course, these arms conform perfectly to your flesh, pressing down on you from all sides.");
	
	output("\n\nIt grows dark as the tentacles adjust to your presence within their midst. Those not actively engaged in sliding across your [pc.skinFurScales] are straightening once more, blocking the light and warmth of the sun out for what is perhaps the last time. Bands of slick, wet plant roll across your shoulders and [pc.chest], enveloping them much like your limbs a moment before. A narrow band slides over your collarband and worms around your neck into an impromptu collar, tight enough that it's impossible to ignore but loose enough to allow you to breathe.");
	
	output("\n\nWetness drizzles your [pc.vagina]");
	if (pc.hasCock()) output(" and [pc.cocks]");
	output(" with foreign wetness. The vines' lubricant is surprisingly warm on your skin, and the feeling of rivulets of it running down the crack of your [pc.butt] and over your [pc.asshole] is surprisingly pleasant.");
	if (pc.isBiped()) output(" Your [pc.legs] are eased open, but who are you to stop them? You're as wet as anything, and if spreading wide is the next step towards scratching your itch, so be it.");
	output(" Your [pc.clit] must look like a");
	if (pc.clitLength > 6) output(" glistening, cum-soaked shaft");
	else if (pc.clitLength > 3) output(" tiny, cum-soaked cock");
	else if (pc.clitLength > 1) output(" swollen, lube-lacquered gumball");
	else output(" glistening, pink target");
	output(" by now. You roll your [pc.hips] as much as your position will allow. Your passion inflamed pussy has thus far avoided the tentacles' affections, though it's the one part that wants them more than anything.");
	...
	if (pc.hasCock())
	{
		output("\n\n[pc.EachCock], long ignored and pulsing ribbons of off-white lust, is snapped up by what feels like a custom-made sleeve. You can hear the noisy slurp of it vacuuming up your length");
		if (pc.cocks.length > 1) output("s"); 
		output(" over the lurid squishing sounds coming from your cunt, and the two sensations combine into a riot of overwhelming sensory data. You're fucking and being fucked to two different, incongruent tempos and unable to make sense of it in the slightest.");
	}
	
	//Back to normal.
	output("\n\nMore cum splatters across the exterior of your womanhood a moment before an enclosure envelops your [pc.clit].");
	if (pc.clitLength > 6) output(" It stretches some around the cock-sized button");
	else if (pc.clitLength > 3) output(" It has no trouble handling your plus-sized button");
	else output(" Your tiny button seems dwarfed by the tube enveloping it");
	output(", but that doesn't make it feel heavenly. It gently pulsates around your buzzer, rippling and squeezing, rhythmically whipping electric lashes of sensation into your fuck-crazed body one after another.");
	
	output("\n\nThe fleshy spear ravaging your honeypot quivers and slows, then thrusts hard enough to rock your body");
	if (pc.biggestTitSize() > 2) output(" and jiggle your tits");
	output(", pressing itself flat against your cervix before finally exploding, creaming your insides with a thick layer of molten-hot seed. You would groan or whimper if your throat wasn't crammed so full of plantcock. Instead, your eyes roll back under their fluttering lids, revealing a narrow band of white as you lose all control. Your body shakes like a seizure victim's, all except for the muscles down south. They're too busy clenching and squeezing the lengthy intrudor, wringing every drop of lust-imbued cum");
	if (pc.isPregnant()) output(" to splatter against your occupied womb.");
	else output(" to fill your womb, to make it round with life so that others can feel what you're feeling.");
	
	output("\n\nYour girlhoney squirts, splattering over the ground.");
	if (pc.wetness() < 4) output(" You may not normally be capable of such soaking orgasms, but that seems of little concern to you.");
	output(" You're a fountain of wetness now, fit only for cumming and taking cum in. The thought triggers a second orgasm on the back of the first while the spent tentacle exits.");
...
private function maleLossToGardener(hpVictory:Boolean):void
{
	clearOutput();
	output("You drop to the ground, defeated and degraded");
	if (hpVictory) output(", not even able to push your bruised body up onto its [pc.feet].");
	else output(", too turned on to try and stop your hands from rushing to your crotch. Why bother?");
	
	output("\n\n<i>\"Oooh, has the mighty champion finally fallen? To think, the one who wiped out our factory installation would fall prey to simple tentacle shrubs and a lowly succubus.\"</i> She strides over to you, her hips wiggling with pleasure. Her foot presses down on your crotch, rubbing your [pc.cock] through your [pc.gear], her eyes twinkling as you writhe at her touches. <i>\"Hmmm... would you rather be my little pet doggie? Following me around on a leash for walkies in the garden, or do you think that this-\"</i> She presses down harder on your crotch. <i>\"-would be a good source of protein for my other pets?\"</i> Gesturing at a nearby tendril's slobbering slit, she smiles. <i>\"Either is good for me.\"</i>");
...
{
	clearOutput();
	output("<i>\"I'll be your doggie,\"</i> you gasp with an accompanying needy arch of your back.");
	
	output("\n\nPressing down on your [pc.cock] once more, the succubus giggles, <i>\"Oh, you've chosen wisely, my little pet. Oh... oh my, has pet got a bone?\"</i> She bends over, stripping your [pc.gear] with effortless efficiency to expose the eagerly pulsating length.");
	if (pc.biggestCockLength() >= 18) output(" <i>\"Oh, it's quite the big bone, isn't it? You're going to have an awful hard time dragging that around on all fours, [puppy]. Why, I bet the moment you see a bitch, you're going to be humping the ground, practically immobilized by it.");
	else if (pc.biggestCockLength() >= 8)
	{
		output(" <i>\"Oh, that's quite the nice bone you've got there, isn't it? Why, everyone is going to see how big you are down there, even while you're walking on all fours. I suppose you'd be good for breeding the bitches");
		if (pc.cockTotal(GLOBAL.TYPE_CANINE) == 0) output(", once you have an appropriately-shaped cock");
		output(".");
	}
	else
	{
		output(" Oh, that's quite the well-formed bone you've got there, isn't it? I could probably take you to shows, and the judges would comment on how well it matches your form. You'd like that, wouldn't you? Rolling over on your back and letting some official-looking demon measure your cock.");
	}
	output("\"</i>");
	
	output("\n\nA high-pitched whine of agreement automatically slips off your tongue - anything to get her to continue her ministrations on your crotch. You hold your hands up like paws and splay your [pc.legs] for easier access, humping the 'v' between her big and index toes, rubbing yourself off on the bottom of her foot, knowing full well that's the best the demon is likely to give you.");
	
	output("\n\n<i>\"Goood " + pc.mf("boy", "girl") + ",\"</i> she coos, twisting her foot to rub her heel against your [pc.sheath]. <i>\"You're just so eager to cum that you'll do anything, won't you?\"</i>");
	...
	output("\n\nThe succubus' heel rubs at your [pc.sheath]");
	if (pc.balls > 0) output(", just above your [pc.balls],");
	output(" once more, cooing, <i>\"I knew you'd train well. Now, I'm going to lead you to your new home, and then you'll get to blow all that nasty, pent-up spunk all over my hand. But you have to be good until then, okay?\"</i>");
	
	output("\n\nYou... you're not sure you can! You're so hot and so close! Pearls of clear pre are oozing out of your [pc.cocks] like water from a leaky faucet, and all you'd need to do is grab hold of her leg and just hump away.");
	...
	if (!pc.hasMuzzle()) output(", heedless of the fact that you seem to be able to reach a little farther with every one you consume");
	output(", slobbering some in your vigor to get more. All the while, the depression fills higher. The sound of gushing fluid tapers down to pitter-patters of the last droplets impacting the impromptu bowl, accompanied by a moan from the demon. She's clearly enjoyed preparing your little treat.");
	
	output("\n\n<i>\"Eat up, doggy.\"</i> Her foot disappears.");
	
	output("\n\nYou look down at your drooling rod");
	if (pc.cocks.length > 1) output("s");
	output(", then at the puddle of sap-laced milk, then at your rod again, then finally back at the puddle before getting up onto all");
	if (pc.isNaga()) output(" threes");
	else if (pc.isTaur()) output(" your [pc.legs]");
	else output(" fours");
	output(".");
	if (!pc.isTaur()) output(" You scrabble over, unusued to having to move in such a way,");
	else output(" You gallop over and drop down on your forelegs");
	output(" and plant your [pc.face] right into the succubus' milk. It's a little thicker than milk normally is and stickier too, like honey, but it's also sweeter. The ambrosial fluid fills your mouth with scintillating, indescribable flavor that sets");
	if (pc.tailType != 0)
	{
		output(" your tail");
		if (pc.tailType == 13 && pc.tailVenom > 1) output("s");
	}
	else output(" your stubby, new tail");
	output(" wagging.");
	if (pc.tailType != 2) output(" Never mind that it gets longer and fuzzier with every gulp or that your");
	else output(" Never mind that your");
	output(" tongue is swelling up, wide, flat, and perfect for panting, licking, and lapping up a drink.");
	pc.tailType = 2;
	...
	output("\n\n<i>\"Good [pc.boyGirl],\"</i> she coos again, and you hear the tone more than the words, hear the praise layered into them, understand that you've made her happy. Your tail flails back and forth in excitement, and your obedient cock");
	if (pc.cocks.length > 1) output("s thrum");
	else output(" thrums");
	output(" with ready excitement. A thin trail of pre-spooge rolls out of you, trailing on the ground behind you with each happy twitch of your bitch-");
	if (pc.biggestCockLength() >= 12) output("breaker");
	else output("boner");
	output(". You hope she doesn't mind, but you have a hunch that she won't mind you being THAT kind of messy puppy.");
	...
	//Taurs
	if (pc.hasLegFlag(GLOBAL.FLAG_HOOVES) || pc.isDrider())
	{
		...
		if (pc.isDrider()) output(" sharp clacks of your spidery claws");
		else output(" clip-clop of your hooves");
		output(" fading into the near silent pats of padded paws. Your hands are changing too: the fingers are shortening, but your nails are growing. Fur grows out over them, and your spine shifts, all so that you can follow on all fours that much easier.");
	}
	else
	{
	//Nontaurs
		output("\n\nThe longer you follow, the easier it gets to stay on her heels.");
		if (pc.isGoo()) output(" Your gooey undercarriage easily divides into two hind legs, slapping wetly with regularity as you plod along.");
		else if (pc.isNaga()) output(" Your slithering tail twitches, stopping you for a second, but then it splits into two limbs, each firmer and more canine than the snake-like appendage you once had. They wobble unsteadily for a second before jumping into action.");
		else output(" Your [pc.legs] shorten as you plod along, slowing you momentarily as they change. You lift your knees up so that you can stand on your [pc.feet] once more, only they're soft, padded paws now, tipped with short nails.");
		output(" Your hands follow suit. First, pads form on your palms, making it easier to support your weight. Then, your fingers shorten. It's not entirely an unpleasant sensation, as it makes moving towards your home that much easier.");
...
{
	clearOutput();
	output("Your kennel is amazing! It's the perfect size for you to crawl into, there's holes in it so that you don't get too hot while you wait to be let out. There's even one big enough for you to slip");
	if (pc.cocks.length > 1) output(" a");
	else output(" your");
	output(" cock through. That hole is on the back wall. You can't see what's over there, but your nose can smell pussies - some of them bitch-pussies, and lots of spilled cum. Your old mind would've identified it as a glory-hole, but it's harder to focus on concepts like that now. ");
	...
	if (!pc.hasFur()) output(" Was that there before?");
	output(" It feels very right, and in your excitement, it's hard not to hump her leg. It must be obvious to her, because your dick");
	if (pc.cocks.length > 1) output("s are");
	else output(" is");
	output(" bobbing beneath you, and she gestures for you to roll onto your back.");

	output("\n\n<i>\"You've listened so well today, doggie. Time for your treat.\"</i>");

	output("\n\nOhhhh, she sounds so happy! You roll over, panting in excitement, your pre oozing into your fur in reckless drops, and look up at her. She ruffles your belly, making you wiggle in happiness, before reach down to [pc.oneCock] and wrapping her hand around it. Ohhh, it's so smooth and soft. You can feel every crease in her hand as it slides around you, and the wetness of your eagerness has you irrevocably lubricated. She doesn't stroke just yet, instead rubbing her thumb back and forth beneath your [pc.cockHead " + pc.biggestCockIndex() + "] while cradling your tool.");

	output("\n\nMistress asks, <i>\"Who's a good puppy?\"</i>");

	output("\n\nYou bark, and her other hand ruffles your [pc.hair], playfully scratching behind your ears. She squeezes while stroking slowly and repeats the question.");
	...
	if (pc.cocks.length > 1) output("s");
	output(" trembling and spurting out ribbons of pre. Whining in the back of your throat, you look at her, hoping she can read the affection in your eyes, wanting her to see just how completely contented you are with this state of affairs.");
	...
	if (pc.cumQ() >= 2500) output(" A few thick ropes shoot past your head to cream the wall, eliciting a pleased giggle from your owner as she squeezes yet more from your length.");
	output(" You yip with glee as you cream for Mistress' fingers, but eventually, your climax trails off into a dribbly white mess, some of which clings to the demoness' nails like beads of ivory dew.");
...
private function maleLossFeeder():void
{
	...
	output("\n\nSomething slick and smooth wraps around your [pc.leg], moving slowly and patiently but with great strength. You can feel the firmness of its grip tightening while a second latches on. The twinned tentacles circle up your body in alternating corkscrew patterns, gliding across your [pc.skinFurScales] with self-lubricated ease. You try to scrabble away, but the tendrils merely lift you aloft like a child would a toy. Meanwhile, the succubus merely flicks");
	if (pc.cocks.length > 1) output(" [pc.oneCock].");
	...
	output("\n\nYou... you're not sure. You noisily gulp down the proffered, delectable goo to buy time to think. It doesn't just taste good; it feels good in your belly as well. It fills you up in ways that food never could. Trickles of that delight radiate out from your slowly-filling gut. They make your [pc.skin] seem to fizzle with raw energy, particularly in your crotch. It's impossible to ignore the way that [pc.eachCock] is swelling or how wonderful the currents of moist air against it feel. ");

	output("\n\nRunning her finger down");
	if (pc.cocks.length == 1) output(" its");
	else output(" one's");
	output(" twitching length, the succubus giggles. <i>\"Can you feel it? Can you feel your body just welling up with cum? I suppose I should have mentioned that, huh?\"</i> She runs her finger back the other way, and your muscles clench, squeezing out a big, fat drop of pre from your cock, dripping down to hit");
	if (pc.biggestTitSize() >= 3) output(" the underside of your breasts");
	else output(" your chin");
	output(". You whimper with need. It's impossible not to. Hell, your [pc.cocks]");
	if (pc.cocks.length == 1) output(" is");
	else output(" are");
	output(" flexing, and if you had anything to brace off of, you'd be thrusting into the first orifice you could find.");
	...
	output("\n\nA breeze caresses your [pc.leg] as the succubus casually removes your [pc.gear]. It feels like oiled fingertips sliding over your flesh. Your [pc.nipples] are like live wires, and your [pc.cocks].... Well, you're leaking pre in thick, solid ropes now, an act that feels like halfway between a good tonguing and orgasm itself. Your lips are slick with the tentacle's fluid and tingling like sex-organs of their own. Sealing them tight, you moan, gurgle, and swallow. Attempts to beg for more have fallen away. This is enough, and if it keeps up, you're going to cum, touches or no.");
...
private function maleLossFeederII():void
{
	...
	{
		output(" swiftly engulfs your meager offering");
		if (pc.cocks.length > 1) output("s");
	}
	else if (pc.biggestCockLength() < 12)
	{
		output(" rapidly devours your plus-sized length");
		if (pc.cocks.length > 1) output("s");
	}
	else if (pc.biggestCockLength() < 24)
	{
		output(" devours your immense offering");
		if (pc.cocks.length > 1) output("s");
		output(" with steady determination");
	}
	else
	{
		output(" slowly encircles your cum-spouting length");
		if (pc.cocks.length > 1) output("s");
		output(" with inhuman elasticity");
	}
	output(". The interior is covered with tiny, wiggling nubs that wiggle into action on contact with your trembling member");
	if (pc.cocks.length > 1) output("s");
	output(". It's like every single nerve has its own skilled masseuse giving it a rubdown, coaxing more pleasure from every square inch of skin than you thought it capable of producing. You cry out, totally overwhelmed, only to be silenced by a familiar presense in your mouth.");
	...
	if (pc.tailType != 0) output(" past your tail to slither");
	output(" between your cheeks.");
	if (pc.analVirgin) output(" The virginal tightness of your pucker holds the tentacle at bay at first. Still, it's no match for the combination of slippery plant fluid and constant pressure.");
	else if (pc.ass.looseness() <= 2) output(" The tentacle has little problem slipping through your tight backdoor, slick as it is with fluid and backed by constant pressure.");
	else output(" The tentacle has little difficulty squeezing into your well-loosened ass. Even without its constant, firm pressure and slippery fluid, it could enter with ease.");
	output(" You gasp at the odd, room-temperature feel of an intruder squirming through your rearmost passage. It's entirely unexpected, but once its mass slithers across your prostate, squeezing even bigger blobs of cum from your rapt, attentive body, you forgive its intrusion.");
