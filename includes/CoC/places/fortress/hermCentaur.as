import classes.Characters.CoC.CoCHermCentaur;
...
public function encounterThePony():void
{
	...
	var hermCentaur:CoCHermCentaur = new CoCHermCentaur();
	CombatManager.newGroundCombat();
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(hermCentaur);
	CombatManager.victoryScene(beatThePony);
	CombatManager.lossScene(inSovietCoCPonyRidesYou);
	CombatManager.displayLocation("HERM\nCENTAUR");
	
	if (pc.vaginalCapacity() < hermCentaur.biggestCockVolume() 
		&& pc.analCapacity() < hermCentaur.biggestCockVolume())
...
public function beatThePony():void
{
	var hpVictory:Boolean = enemy.HP() < 1;
	clearOutput();
	output("With a heavy 'thud', the demonic red-head slumps onto her side in the grass.  Her tail flails fitfully")
	if (hpVictory) output(" as she tries to rise");
	else output(" as she tries to stretch for her pulsating, cum-dripping member");
	output(", but it's a futile gesture.  She doesn't have the power to face you on her own terms anymore, and she watches you with a");
	if (hpVictory) output(" resigned glare");
	else output(" lustful glare");
	output(" once she realizes she's utterly spent.");
	...
	if (pc.hasCock()) addButton(0, "Fuck Her", maleFuckHermCentaur);
	if (pc.hasVagina()) addButton(1, "Get Fucked", femFuckHermCentaur);
	addButton(2, "Leave", letHermCentaurGo);
	addButton(3, "Kill", killHermCentaur);
}

private function killHermCentaur():void
{
	clearOutput();
	output("Sighing, you advance on the downed demon, trying to ignore the horrified look in her eyes as you end her.");
	flags["COC.D3_CENTAUR_DEFEATED"] = CENTAUR_KILLED;
	processTime(5 + rand(10));
	CombatManager.genericVictory();
}

private function letHermCentaurGo():void
{
	clearOutput();
	output("You dust off your [pc.gear] and wave nonchalantly at the equine demoness.  She slowly staggers up, watching you warily.  Dismissively, you tell her to leave before she earns your ire - her queen will be dealt with soon enough.  She clops off in a huff towards the exit.");
	flags["COC.D3_CENTAUR_DEFEATED"] = CENTAUR_RELEASED;
	processTime(5 + rand(10));
	CombatManager.genericVictory();
}

private function maleFuckHermCentaur():void
{
	flags["COC.D3_CENTAUR_DEFEATED"] = CENTAUR_FUCKED;

	var y:int = pc.cockThatFits(enemy.vaginalCapacity());

	if (y < 0) y = pc.smallestCockIndex();

	clearOutput();
	output("You shrug out of your [pc.gear] as you close with the defeated beast-woman.  Looking at her like this, she almost seems more animal than woman.  Her hundreds of pounds of muscular equine flesh twist and contort as you approach, shifting her bulk to allow you unimpeded access to either of her trembling, moist genitals.  Glancing up to her pale, freckled face, you trap her green eyes with your own and say, \"<i>" + ((pc.cor() > 50) ? "I'm gonna wreck it!" : "We do this my way.") + "</i>\"");
	...
	if (pc.cumQ() >= 15000) output(", practically dragging her gut on the stones as she runs");
	output(".  Well, she might have gotten away, but she won't be able to stop you from getting to Lethice.  Indeed, it seems she was running for the exit.\n\n");
...
private function femFuckHermCentaur():void
{
	flags["COC.D3_CENTAUR_DEFEATED"] = CENTAUR_FUCKED;

	clearOutput();
	output("Tossing aside your [pc.gear], you close in on the defeated equine with a lusty gaze on your [pc.face].  The defeated demon looks up at you with the reluctant anticipation of one who's getting what they want in EXACTLY the wrong way.  You swat the rebellious mare's flank, disdainfully sliding your [pc.foot] across her gushing gash to watch her twitch and moan like the beast she is.  Of course the demontaur would enjoy such debasement.  You prace forward, being sure to jostle her gurgling balls, and enjoy the spurts of lurid white liquid that escape her tremendous, pulsating member.");
	...
	output("\n\nFinally, a demon with some common sense!  You idly trace your [pc.foot] along the underside of her leaky horse-cock while pondering this newest development.");
	output("  She'll need to match her size to your [pc.vagina] for sure,");
	output(" but what kind of cock do you want her to have?");
...
private function femFuckHerII(cockType:int):void
{
	...
	if (cockType == TENTACOCK)
	{
		...
		if (pc.clitLength >= 6) output(" - you swear it spends as much time caressing your [pc.clit] as squeezing it off");
		output(".  You buck your hips, dragging the butt-piercing wang along for the ride as you start to fuck in earnest.");
	...
	{
		output("You give her throbbing maleness an affection pat and proclaim, \"<i>Make it a dog-cock, one that fit to knot properly.  Then we can fuck.</i>\"");

		output("\n\nThe light in the room seems to dim, drawn into the demon's onyx-black horn, accompanied by the sound of flesh shifting, creaking and thrumming as it takes on new shape.  You feel something pointy and hot brush your [pc.foot], and as the light returns, you're treated to the sight of a swollen dog-cock leaking its thick pre-creme onto you.  Where once an equine cock jutted from a horse-like sheath, now there is the rounded bulb of a distinctly canine cock.  Above the deliciously thick knot, the rest of the shaft is blessedly thick, tapering only near the very tip.  Best of all, it looks able to be faced in any direction, so taking it should be no problem.");

		output("\n\nYou lay down under the centaur, your torso nestled firmly between her forelegs.  Her animalistic body's muscles and heavy heartbeats are surprisingly soothing to you as you position yourself, aligning the doggie dick to take you from behind.  Breathy heavy with anticipation and lust, you fairly ooze with lady-lube, your [pc.vagina] drizzling the bitch-cock with your lusty lubricants.  You rub that hot member around your outer lips to tease yourself, only growing wetter with each passing press, prod, and near-penetration.");
		...
		output("  The pleasure of being so fully stretched is enough to overwhelm any pain at such an act, and you switch over to smaller, faster strokes the leave most of the animal-cock inside you.  Bouncing atop the centaur's balls with a dopey grin on your [pc.face], you sigh and moan.  This is how you're going to take down these demons, one fuck at a time!  The thought gives your " + pc.skin() + " a fresh flush, and you pinch your nipple while imaging mounting a demon queen atop an orgy of her defeated lieutenants.");
		...
		output("\n\nEventually, the semen stops squirting into your willing depths, but the knot holds you firm, forcing you to adapt to being cum-pregnant for what seems like hours.  You lounge with the demon still inside you, half-conscious and cumming with every sudden shift while you wait on the expanded knot to contract.  The tainted centaur tires of this in time and tries to rise, lifting you into the air by your [pc.butt] for a few seconds until she pops free.");
...
public function inSovietCoCPonyRidesYou():void
{
	var hpVictory:Boolean = pc.HP() < 1;
	
	if (pc.hasCock() && pc.hasVagina())
	{
		if (rand(2) == 0) maleLossHermCentaur(hpVictory);
		else femLossHermCentaur(hpVictory);
	}

	if (pc.hasCock() && !pc.hasVagina()) maleLossHermCentaur(hpVictory);
	else if (pc.hasVagina() && !pc.hasCock()) femLossHermCentaur(hpVictory);
	else if (!pc.hasVagina() && !pc.hasCock()) femLossHermCentaur(hpVictory);
}

private function femLossHermCentaur(hpVictory:Boolean):void
{
	clearOutput();
	output("Dropping down, you");
	if (hpVictory) output(" try to pull yourself up with on your palms, utterly defeated in spite of the centaur's complete lack of physical attacks.");
	else 
	{
		output("put your rump up in the air, fully exposing your [pc.crotch] to the pervy, sexy equine.");
	}
	output(" She smirks as she trots up, commenting, \"<i>That's more like it, slut.  I don't know how you got this far, but your quest ends here.</i>\"  She casually reaches down and grabs you by the scruff of the neck, hauling you bodily up until your head nestles between her tits");
	if (pc.tallness >= 7 * 12) output(", even though your [pc.feet] still touch the ground");
	output(".  Brushing through your hair, a hand gently caresses your back as its owner coos, \"<i>You'll make a fine mare for my stable, but lets get you in the right mindset first...</i>\"");
	...
	output("\n\nThat hot horsecock is rammed into you, until you feel the folded skin of her sheath on your lips and the heavy weight of her squirming testes bouncing off your [pc.butt].  The proud shape of her flare is clearly visible on your stretched-out tummy, but it doesn't hurt.  All you feel is glorious fullness and the ache of so much pleasure that you fear you'll cum and pass out, all at the same time.  The delicious thickness inside you bubbles and grows, thickening with each beat of the demon's heart, and you realize she's as close as you, perhaps closer.  Timidly, you caress her thick flare through your taut skin, garnering a spurt of hot pre and a matched moan of contentment from your gargantuan lover.");
...
private function femLossIIHermCentaur():void
{
	...
	output("\n\nYou slump down as you're dragged through dirt, over carpets, and through halls.  Various demons leer at you as you pass, snickering at the fallen champion, now little more than a willing mare.  The humiliation makes your cheeks go red but your [pc.vagina] drool. Some part of you is clearly enjoying it, though you aren't sure if it's a natural enjoyment or something this creature has done to you.  You sigh and shrug, raising your hands to fondle your breasts as you're dragged to your new home.  You may as well enjoy yourself.");

	clearMenu();
	addButton(0, "Next", femLossIIIHermCentaur);
}

private function femLossIIIHermCentaur():void
{
	clearOutput();
	output("<b>Months later...</b>");
...
private function maleLossHermCentaur(hpVictory:Boolean):void
{
	clearOutput();
	...
	if (hpVictory)
	{
		output(" try to pull yourself up with on your palms, utterly defeated in spite of the centaur's complete lack of physical attacks.");
	}
	else
	{
		output(" try to pull [pc.oneCock] out of your [pc.gear] in an attempt to masturbate.  You're beaten, you just want to fuck or be fucked until you're both covered in a sheen of lady-spunk or your own thick cum.");
	}
	output(" She smirks as she trots up, commenting, \"<i>That's more like it, slut.  I don't know how you got so far thinking you were some kind of " + pc.mf("stud","queen bitch") + ".</i>\"  She casually reaches down and grabs you by the scruff of the neck, hauling you bodily up until your head nestles between her tits");
	if (pc.tallness >= 7 * 12) output(", even though your [pc.feet] still touch the ground");
	output(".  Brushing through your hair, a hand gently caresses your back as its owner coos, \"<i>I'm not sure you have what it takes to be one of my stallions...</i>\"");
	...
	output("\n\nThat burgeoning heat that's soothing away your worries builds hotter and higher with every swallow, but it isn't until the lactation slows to a trickle that you notice something else - the warmth is gathering in your groin.  [pc.EachCock] is so hard, so throbbingly turgid, that it presses painfully at your [pc.gear], dribbling so much pre-cum that feel it'll start to run down your [pc.legs] any second now.");
	if (pc.hasVagina()) output(" Your [pc.vagina] fares no better, puffing up and drooling like its big brother while your [pc.clit] grows taut and sensitive.");
	output(" That wonderful warmth excites you sexually, moulding your loins into a simmering blaze of desire just looking for a something to cool its ache.");

	output("\n\nYou're moved to the other side, with a fresh, juicy nipple for you to nuzzle.  You do so without thought, effortlessly clasping your lips in a tight seal around the heavenly nub and suckling, even as you rest your cheek against the pale, freckled skin of the melon-sized tit.  The fluid begins to flow almost effortlessly, flooding out in response to the slightest hint of suction from your hungry mouth.  Bubbling, almost boiling over, your lust mounts to higher and higher levels, your loins seeming to simmer with molten desire to breed.  In response, you begin peeling away your [pc.gear].  You know this delicious milk will be gone soon, and when you finish, you'll need to mount something... hard.");
	...
	output("\n\nIn light of your hapless situation, you cling tight and watch as you're given a rampant, dick-squeezingly tight tour of the demons' headquarters. You're taken past masturbating succubi, fapping incubi, and even snickering imps.  All manner of demons sneer at you, the once champion, now a slave to a centaur's sodden snatch.  The sheer, debauched exhibition of it sends a spike of pleasure to your [pc.balls], the upwelling of desire burning with wet heat that seems to dwarf the roaring furnace that is the demon's thighs.  Your burgeoning orgasmic sensation is brought to a screeching halt along with your mistress when she comes to an abrupt stop.");

	output("\n\nThen, someone else joins you on the centaur's back - it's an omnibus");
	if (flags["COC.D1_OMNIBUS_KILLED"] != 1) output(", and a familiar one at that");
	output(".  She wears a black reflective bra, though you swear you can see pink protrusions under the shiny fabric, wriggling around constantly.  As expected with such dress, her nipples are hard as bullets.  A bulbous, ten-inch penis bobs between her thighs, thickening nicely as it fills to its full size, kissing your lips with a moist bead of pre as it fully erects.  The omnibus");
	if (flags["COC.D1_OMNIBUS_KILLED"] != 1) output(" overseer asks, \"<i>Remember me?  I thought it only proper that I thank you for letting me live.  Now, be a good boy and suck my dick - show all everyone just what a horny, helplessly fuckable toy you are.</i>\"");
	else output(" asks, \"<i>I hope you don't mind slurping down a little cock, because you're going to anyway.</i>\"");
	
	output("\n\nThe second herm grabs you by the");
	if (pc.hasHorns()) output(" [pc.horns]");
	else output(" head");
	output(" and sidles forward, ramming her glans into your mouth and down the back of your throat.  You can feel her glans pulsing deep inside you, and as soon as the herm starts moaning, the centaur starts trotting forward again.  Amazingly, having her holding you up makes it easy to maintain your position and keep your dick bouncing in that hot twat. Fuck it! You're too horny to care anymore.");

	output("\n\nSlurping hard, you lick and suck the dick in your throat, so busy tending to the omnibus that you barely the notice the throbbing ache in your loins.  You cast lusty, inviting eyes around at every feminine form you see during journey to the stables, located in some backwater, seemingly forgotten part of the fortress.  You quiver with need and explode in passion, moaning into the Omnibus's cock as your " + pc.cockDescript(y) + " spends its lust into the oozing horse-pussy, wasting your champion-seed to father nothing more than a batch of shiftless imps.");
