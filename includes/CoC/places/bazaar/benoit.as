...
public function benoitMF(stringM:String,stringF:String):String {
	if (Flag("COC.BENOIT_STATUS") == 1 || Flag("COC.BENOIT_STATUS") == 2 || Flag("COC.BENOIT_STATUS") == 3) return stringF;
	return stringM;
}
private function benoitLover():Boolean {
	return Flag("COC.BENOIT_TIMES_SEXED_FEMPCS") >= 2;
}
public function benoitAffection(changes:Number = 0):Number {
	if(changes != 0) {
		Flag("COC.BENOIT_AFFECTION", changes, true);
		if(Flag("COC.BENOIT_AFFECTION") > 100) flags["COC.BENOIT_AFFECTION"] = 100;
		else if(Flag("COC.BENOIT_AFFECTION") < 0) flags["COC.BENOIT_AFFECTION"] = 0;
	}
	return Flag("COC.BENOIT_AFFECTION");
}

private function benoitKnocksUpPCCheck():void {
	//Convert old basi's to real basi's!
	//if (pc.pregnancyType == PregnancyStore.PREGNANCY_BASILISK && pc.hasPerk("Basilisk Womb")) pc.knockUpForce(PregnancyStore.PREGNANCY_BENOIT, pc.pregnancyIncubation);
	//Knock up chances:
	if ((pc.hasPregnancyOfType("CoCOviElixEggs") || pc.hasPerk("Harpy Womb") || pc.hasPerk("Oviposition") || pc.hasPerk("Basilisk Womb")) && (!pc.isPregnant() || pc.hasPregnancyOfType("CoCOviElixEggs"))) {
		if (pc.hasPerk("Basilisk Womb") && Flag("COC.BENOIT_TESTED_BASILISK_WOMB") == 1) {
			if (!pc.hasPregnancyOfType("CoCOviElixEggs") || !pc.isPregnant()) {
				//pc.knockUp(PregnancyStore.PREGNANCY_BENOIT, PregnancyStore.INCUBATION_BASILISK);
			}
			//if (pc.isPregnant()) pc.knockUpForce(PregnancyStore.PREGNANCY_BENOIT, pc.pregnancyIncubation);
		}
		else {
...
{
	if (Flag("COC.BENOIT_STATUS") == 0) return false;
	if (Flag("COC.FEMOIT_EGGS") > 0) return true;
	return false;
}
...
{
	if (Flag("COC.BENOIT_STATUS") == 0) return false;
	if (Flag("COC.FEMOIT_EGGS") >= 1 && Flag("COC.FEMOIT_EGGS") <= 4) return true;
	return false;
}
...
{
	if (Flag("COC.BENOIT_STATUS") == 0) return false;
	if (Flag("COC.FEMOIT_EGGS") >= 5 && Flag("COC.FEMOIT_EGGS") <= 8) return true;
	return false;
}
...
{
	if (Flag("COC.BENOIT_STATUS") == 0) return false;
	if (Flag("COC.FEMOIT_EGGS") >= 9 && Flag("COC.FEMOIT_EGGS") <= 12) return true;
	return false;
}
...
{
	if (Flag("COC.BENOIT_STATUS") == 0) return false;
	if (Flag("COC.FEMOIT_EGGS") >= 13) return true;
	return false;
}
...
{
	if (Flag("COC.FEMOIT_EGGS") != 0)
	{
		flags["COC.FEMOIT_EGGS_LAID"] += flags["COC.FEMOIT_EGGS"];
		flags["COC.FEMOIT_EGGS"] = 0;
...
public function benoitIntro():void {
	...
	else if(Flag("COC.BENOIT_SUGGEST_UNLOCKED") == 0 && pc.hasVagina() && (pc.hasStatusEffect("Heat") || pc.hasPregnancyOfType("CoCOviElixEggs") || pc.hasPerk("Harpy Womb") || pc.hasPerk("Oviposition")) && (pc.hasPregnancyOfType("CoCOviElixEggs") || !pc.isPregnant()) && (Flag("COC.BENOIT_STATUS") == 0 || Flag("COC.BENOIT_STATUS") == 3)) {
		benoitAndFemPCTalkAboutEggings();
	}
	else if (Flag("COC.FEMOIT_NEXTDAY_EVENT_DONE") == 1 && Flag("COC.FEMOIT_NEXTDAY_EVENT") <= days && Flag("COC.BENOIT_STATUS") == 0)
	{
		femoitNextDayEvent();
	}
	else if (benoitInClutch() && Flag("COC.FEMOIT_READY_FOR_EGGS") == 0 && (Flag("COC.BENOIT_STATUS") == 1 || Flag("COC.BENOIT_STATUS") == 2))
	{
	...
	else if (benoitPreggers() && Flag("COC.FEMOIT_INCUBATION") < 0)
	{
		...
		{
			output("When you enter the stall, you notice that Benoite's stomach is flat again. She gives you a toothy grin when you enter her stall. \"<i>I have good newz [pc.name]! You are a fazer! " + String(flags["COC.FEMOIT_EGGS"]) + " timez over in fact! Oh I wish zat you could have helped; but I was ztrong, ztrong enough get through it on my own.</i>\" she gushes, speaking at a mile-a-minute. She's clearly pleased with herself; and your virility.");
	...
	else if (!benoitInClutch() && !benoitPreggers() && (days - Flag("COC.FEMOIT_NEXTDAY_EVENT") >= 30) && (Flag("COC.BENOIT_STATUS") > 0))
	{
		if (Flag("COC.FEMOIT_FIRST_CLUTCH_MISSED") == 0)
		{
			Flag("COC.FEMOIT_FIRST_CLUTCH_MISSED", 1, true);
	...
	else
	{
		...
		if(benoitAffection() <= 10) output("Once again, you carefully enter the gloom of Benoit's salvage shop.  The proprietor sniffs the air as you enter, and then looks at you sourly.  \"<i>Well?</i>\" " + benoitMF("he","she") + " rasps.");
		...
		else if(benoitAffection() < 35) output("Once again, you carefully enter the gloom of Benoit's salvage shop.  The proprietor sniffs the air as you enter, and then relaxes in " + benoitMF("his","her") + " seat.  \"<i>Allo again, " + pc.short + ".  What brings you 'ere?</i>\"");
		//Subsequent Visit, Affection 35+ but pre-lover/pre-fem: 
		else
		{
			output("Once again, you carefully enter the gloom of "+ benoitMF("Benoit", "Benoite") +"'s salvage shop.  The proprietor sniffs the air as you enter, and then smiles widely.  \"<i>If it isn't my favorite customer!  Do not 'ang around out zere, [pc.name]; please, come in and let us, 'ow you say, chew ze fat.</i>\"");
	...
	//Basilisk Womb
	if(Flag("COC.BENOIT_WOMB_TALK_UNLOCKED") == 1 && !pc.hasPerk("Basilisk Womb") && Flag("COC.BENOIT_TESTED_BASILISK_WOMB") == 0 && Flag("COC.BENOIT_STATUS") == 0) addButton(4, "Basil. Womb", tryToConvertToBassyWomb);
	
	//if (pc.hasCock() && Flag("COC.BENOIT_STATUS") > 0 && pc.lust() >= 33) addButton(6, "Sex", femoitSexIntro);
}
...
private function buyOrSellExplanationFirstTime():void {
	Flag("COC.BENOIT_EXPLAINED_SHOP", 1);
...
public function benoitsBuyMenu():void {
	clearOutput();
	clearMenu();
	if (Flag("COC.BENOIT_EXPLAINED_SHOP") == 0) {
	...
	if (flags["COC.BENOIT_STATUS"] == 1) 
	{
		buyMod = 1.66;
	}
	
	shopkeep = new CoCTrader();
	shopkeep.short = "Benoit";
	shopkeep.sellMarkup = buyMod;
	shopkeep.buyMarkdown = (flags["COC.BENOIT_EGGS"] > 0 || flags["COC.BENOIT_STATUS"] > 0) ? 0.5 : .33;
	shopkeep.typesBought = [GLOBAL.ALL];
	shopkeep.keeperBuy = "\"<i>Some may call zis junk,</i>\" says " + benoitMF("Benoit", "Benoite") + ", indicating " + benoitMF("his", "her") + " latest wares.  \"<i>Me... I call it garbage.</i>\"";
	shopkeep.inventory = [
		new (getDefinitionByName(flags["COC.BENOIT_1"]))(),
		new (getDefinitionByName(flags["COC.BENOIT_2"]))(),
...
private function benoitSellMenu():void {
	...
	shopkeep = new CoCTrader();
	shopkeep.short = "Benoit";
	shopkeep.sellMarkup = 3;
	shopkeep.buyMarkdown = (flags["COC.BENOIT_EGGS"] > 0 || flags["COC.BENOIT_STATUS"] > 0) ? 0.5 : .33;
	shopkeep.typesBought = [GLOBAL.ALL];
	shopkeep.keeperSell = "\"<i>Let us feel what you are trying to palm off upon me zis time, zen,</i>\" sighs Benoit" + benoitMF("", "e") + ", sitting down and opening " + benoitMF("his","her") + " hand to you.\n";
	shopkeepBackFunctor = benoitIntro;
	sellItem();
}
...
public function updateBenoitInventory():void {
	...
	flags["COC.BENOIT_2"] = getQualifiedClassName(item);
	
	//Slot 3 Any one of the following: Maid's Clothes, Wizard Robes, Tough Silk, Slutty Swimwear, Goo Chunk, Chitin Plate
	item = RandomInCollection(CoCWizardRobes, CoCSluttySwimwear, CoCGreenGel, CoCBlackChitin); // CoCToughSpiderSilk, CoCDragonScale? sort of gamebreaking
	...
	flags["COC.BENOIT_3"] = getQualifiedClassName(item);
...
private function buyFlintlock():void {
	clearOutput();
	output("You wander " + benoitMF("Benoit", "Benoite") + "'s shop for a good while as you're searching for something interesting until you spot something interesting.");
	...
	output("\n\nTime to test this weapon out.  You aim the pistol at one of the empty tin cans and pull the trigger.  A round launches from the pistol and hits the tin can, knocking it off shelf.  " + benoitMF("Benoit", "Benoite") + " looks in surprise and says, \"<i>It works?  500 gems and it can be yours.</i>\"");
	output("\n\nDo you buy the gun?  ");
	clearMenu();
...
private function buyFlintlockConfirmation():void {
	...
	output("\"<i>Here you go.  I have no need for zis,</i>\" " + benoitMF("Benoit", "Benoite") + " says.");
...
//private function buyBackpack():void {
	//clearOutput();
	//output("You ask " + benoitMF("Benoit", "Benoite") + " if " + benoitMF("he", "she") + " has a backpack to spare.");
	...
	//menu();
	//if (pc.keyItemv1("Backpack") < 3) addButton(0, "Small", buyBackpackConfirmation, 3, "Small", 200, "Grants additional three slots. \n\nCost: 200 gems");
	//if (pc.keyItemv1("Backpack") < 4) addButton(1, "Medium", buyBackpackConfirmation, 4, "Medium", 600, "Grants additional four slots. \n\nCost: 600 gems");
	//if (pc.keyItemv1("Backpack") < 5) addButton(2, "Large", buyBackpackConfirmation, 5, "Large", 1200, "Grants additional five slots. \n\nCost: 1200 gems");
...
//private function buyBackpackConfirmation(size:int = 3, sizeDesc:String = "Small", price:int = 200):void {
	...
	//}
	//output("\"<i>Here you go.  I have no need for zis,</i>\" " + benoitMF("Benoit", "Benoite") + " says.");
	//if (pc.hasKeyItem("Backpack") >= 0) {
		//output("\n\n<b>(Key Item Upgraded: " + sizeDesc + " Backpack! You now have " + num2Text(size - pc.keyItemv1("Backpack")) + " extra inventory slots");
		//pc.addKeyValue("Backpack", 1, size - pc.keyItemv1("Backpack"));
		//output(" for a total of " + num2Text(inventory.getMaxSlots()) + " slots.)</b>");
	//}
	//else {
		//output("\n\n<b>(Key Item Gained: " + sizeDesc + " Backpack! You now have " + num2Text(size) + " extra inventory slots");
		//pc.createKeyItem("Backpack", size, 0, 0, 0);
		//output(" for a total of " + num2Text(inventory.getMaxSlots()) + " slots.)</b>");
	//}
	//pc.gems -= price;
	//statScreenRefresh();
...
private function talkToBenoit():void {
	...
	if(((flags["COC.BENOIT_TIMES_SEXED_FEMPCS"] > 2 && pc.IQ() >= 60 && flags["COC.BENOIT_WOMB_TALK_UNLOCKED"] == 0) || flags["COC.BENOIT_TIMES_SEXED_FEMPCS"] == 2) && pc.hasVagina()) {
		output("You ask " + benoitMF("Benoit","Benoite") + " if " + benoitMF("he","she") + " has ever thought about trying to do something to help his people's plight.");
	...
	//Subsequent Talk
		...
		if(benoitLover() && benoitMF("he","she") == "he" && pc.hasVagina()) {
		...
		//(Randomly generated)
		if(choice == 0) {
			output("You ask if all basilisks talk as " + benoitMF("he","she") + " does.");
			...
			output("You ask " + benoitMF("Benoit","Benoite") + " about the dog.");
			...
			output("You ask " + benoitMF("him","her") + " about the sign above the shop.");
			...
			output("You ask if " + benoitMF("he","she") + " has always been blind.");
			...
			output("You ask " + benoitMF("Benoit","Benoite") + " for any rumors going around.");
			//[Deep cave cleared: 
			...
			output("You ask if " + benoitMF("he","she") + "'s ever had any trouble with the demons who frequent the Bazaar.");
			...
			output("You ask " + benoitMF("Benoit","Benoite") + " what results when basilisks mate with harpies.");
			...
			output("You ask if " + benoitMF("Benoit","Benoite") + " really can tell who you are just by smell.");
			
			if(pc.race() == "human") output("\n\n\"<i>Certainly!</i>\" " + benoitMF("he","she") + " smiles.  \"<i>Ze smell of shaved monkey is distinctive.  I get very few 'uman customers, you know.</i>\"  The basilisk scratches " + benoitMF("his","her") + " jaw absent-mindedly.  \"<i>If you do not mind me saying so, [pc.name], you also smell... different.  Like you do not really belong 'ere.  In ze nicest possible way, of course.</i>\"");
			...
			//[(male Benoit only)
			if(benoitMF("he","she") == "he") output("  There's a hint of longing jealousy in the basilisk's voice and when " + benoitMF("he","she") + " lapses into silence you decide not to push the subject.");
		}
		...
		//(Randomly generated.  Added to normal talk options after PC has had sex with Benoit two times or more regardless of womb quest.)
			...
			output("You ask Benoit if " + benoitMF("he","she") + " really, <b>really</b> can tell who you are just by smell.");
...
private function benoitAndFemPCTalkAboutEggings():void {
	...
	output("" + benoitMF("Benoit","Benoite") + " seems very on edge today.  " + benoitMF("He","She") + " paces up and down, returns your greeting with a stiff nod, and fiddles endlessly with the stock in reach of " + benoitMF("his","her") + " counter as you search for a topic.  You ask if there's something wrong.");
...
private function eggySuggest():void {
	...
	if(flags["COC.BENOIT_TIMES_SEXED_FEMPCS"] > 0) {
		if(pc.isTaur()) {
			output("You silently reach across the counter to squeeze " + benoitMF("his","her") + " hands again.  " + benoitMF("Benoit","Benoite") + " grins with deep affection at you and, hand in hand, the two of you quietly debunk to the store room again.");
			output("\n\nOnce again, you carefully inch your blind charge to a clear cranny and push " + benoitMF("him","her") + " against a wooden wall, standing back to slowly peel off your [pc.gear].  You grin as you ostentatiously drop each piece onto the packed earth, allowing " + benoitMF("him","her") + " to guess what it is by the sound it makes.  " + benoitMF("His","Her") + " breathing comes heavier as your undergarments make a feathery sound as they fall.  As you take " + benoitMF("his","her") + " hands and lay them upon your naked [pc.skinFurScales], you think about how you want to go about this.");
		}
		else {
			output("You silently reach across the counter to squeeze " + benoitMF("his","her") + " hands again.  " + benoitMF("Benoit","Benoite") + " beams at you and, hand in hand and without a word, the two of you depart to the store room again.");
	...
	if(pc.isTaur()) { // WTF? taur scene is actually doubled, since non-taur one have taur chesks everywhere
		//Suggest: 
		output("You reach your fingers across the counter and lightly touch " + benoitMF("Benoit","Benoite") + "'s hands, saying you don't mind working out a few natural urges, if " + benoitMF("he","she") + "'s in the mood.");
		...
		output("\n\n" + benoitMF("His","Her") + " warm fingers travel over your body, brushing over your [pc.face], your [pc.belly], your [pc.hips]; you feel like you're being read like a book in Braille.  “Good Gods,” " + benoitMF("he","she") + " murmurs as " + benoitMF("his","her") + " hands lead back onto your flanks.  “Good Gods!” " + benoitMF("he","she") + " cries out as " + benoitMF("he","she") + " follows you all the way back to your mighty, powerful rear.  “I knew you were a taur because of all ze tramping,” " + benoitMF("he","she") + " says, rubbing your side back and forth in wonder.  “But to know it and actually feel it, zey are somesing very different.” " + benoitMF("He","She") + " sighs.  “I 'ope you do not mind zis being a bit... awkward, but I am guessing you are probably used to zat by now, yes?”");
		// Herm: 
		if(pc.isHerm()) {
			output("\n\n" + benoitMF("His","Her") + " hands travel down your behind until, with a sharp intake of breath, " + benoitMF("he","she") + " touches [pc.oneCock].  “Aren't you just full of surprises,” " + benoitMF("he","she") + " says dryly.  After a pause, " + benoitMF("he","she") + " slowly wraps " + benoitMF("his","her") + " dry, smooth grasp around your semi-erect cock and moves it up and down, rubbing and coiling you until you are straining.");
		...
		//[Taur: 
		if (pc.isTaur()) output("\n\n" + benoitMF("His", "Her") + " warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  \"<i>Good Gods,</i>\" " + benoitMF("he", "she") + " murmurs as " + benoitMF("his", "her") + " hands lead back onto your flanks.  \"<i>Good Gods!</i>\" " + benoitMF("he", "she") + " cries out as " + benoitMF("he", "she") + " follows you all the way back to your mighty, powerful rear.  \"<i>I knew you were a taur because of all ze tramping,</i>\" " + benoitMF("he", "she") + " says, rubbing your flank back and forth in wonder.  \"<i>But to know it and actually feel it, zey are very different.</i>\"  " + benoitMF("He", "She") + " sighs.  \"<i>Zis is going to be a bit... awkward, but I am guessing you are all too used to zat by now, yes?</i>\"");
		//[Demon: 
		else if(pc.hasHorns(GLOBAL.TYPE_DEMONIC) && pc.hasTail(GLOBAL.TYPE_DEMONIC) && pc.demonScore() >= 3) output("\n\n" + benoitMF("His","Her") + " warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  " + benoitMF("He","She") + " touches your horns and pauses; " + benoitMF("he","she") + " reaches around, finds and grips your tail, running " + benoitMF("his","her") + " pads up to the spaded point.  \"<i>So,</i>\" " + benoitMF("he","she") + " says quietly.  \"<i>You are one of zem.</i>\"  " + benoitMF("He","She") + " is silent for a while before finding a warm smile.  \"<i>But I am being silly.  I know you are different inside.</i>\"");
		//[Dog enough for ears and tail: 
		else if(pc.earType == GLOBAL.TYPE_CANINE && pc.hasTail(GLOBAL.TYPE_CANINE) && pc.canineScore() >= 3) output("\n\n" + benoitMF("His","Her") + " warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  " + benoitMF("He","She") + " grins as " + benoitMF("he","she") + " finds your floppy ears and outright laughs when " + benoitMF("he","she") + " reaches around and touches your tail. \"<i>I like dogs, but not ZAT much, [pc.name],</i>\" " + benoitMF("he","she") + " laughs.");
		//[Reptile/Naga: 
		else if(pc.skinType == GLOBAL.SKIN_TYPE_SCALES && (pc.lizardScore() >= 3 /*|| pc.nagaScore() >= 3*/ || pc.dragonScore() >= 3)) output("\n\n" + benoitMF("His","Her") + " warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  " + benoitMF("He","She") + " starts slightly when " + benoitMF("he","she") + " touches your scales, and then caresses the reptilian parts of your body with increasing interest.  \"<i>I cannot believe I did not realize you were a sister of ze scales,</i>\" " + benoitMF("he","she") + " says huskily.  \"<i>Zat is very... interesting.</i>\"  You can see real arousal in the tense lines of " + benoitMF("his","her") + " face now.");
		...
		else if(pc.isDrider()) output("\n\n" + benoitMF("His","Her") + " warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  \"<i>Good Gods,</i>\" " + benoitMF("he","she") + " murmurs as " + benoitMF("his","her") + " hands lead back onto your tough exoskeleton. \"<i>Good Gods!</i>\" " + benoitMF("he","she") + " cries out as " + benoitMF("he","she") + " follows your bulging abdomen all the way back to your spinnerets. \"<i>I knew you were a spider because of all ze click-clacking,</i>\" " + benoitMF("he","she") + " says, " + benoitMF("his","her") + " fingers feeling around one of your intricate, many-jointed legs in wonder.  \"<i>But to know it and actually feel it, zey are very different.</i>\"");
		//[Slime: 
		else if(pc.isGoo()) output("\n\n" + benoitMF("His","Her") + " warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  \"<i>I knew you were different from ze squishy sounds you made,</i>\" " + benoitMF("he","she") + " murmurs as " + benoitMF("his","her") + " hands sink into your soft, amorphous mass.  \"<i>But zis is...good Gods, zis is strange.  And zis doesn't 'urt you at all?</i>\" " + benoitMF("he","she") + " asks incredulously as " + benoitMF("he","she") + " gently pokes a finger into you.  You answer " + benoitMF("his","her") + " question by giggling.  \"<i>Zat must come in very useful,</i>\" " + benoitMF("he","she") + " says, shaking " + benoitMF("his","her") + " head in wonder.  You push yourself slowly up " + benoitMF("his","her") + " arms and tell " + benoitMF("him","her") + " " + benoitMF("he","she") + " has no idea.");
		else output("\n\n" + benoitMF("His", "Her") + " warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  \"<i>You 'umans are so squishy, fuzzy and 'ot,</i>\" " + benoitMF("he", "she") + " says huskily. \"<i>'Ow can you stand it?</i>\"");
		
		if(pc.hasCock()) {
			output("\n\n" + benoitMF("His","Her") + " hands travel downwards until, with a sharp intake of breath, " + benoitMF("he","she") + " touches [pc.oneCock].  \"<i>Aren't you just full of surprises,</i>\" " + benoitMF("he","she") + " says dryly.  After a pause, " + benoitMF("he","she") + " slowly wraps " + benoitMF("his","her") + " smooth hand around your semi-erect cock and moves it up and down, rubbing and coiling you until you are straining."); 
			...
			else output("  This is evidently an uncanny experience for " + benoitMF("him","her") + ", the alien nature of it deepening as " + benoitMF("his","her") + " hands moves along your male sex. \"<i>'Oly Gods, [pc.name]; you are a monster,</i>\" " + benoitMF("he","she") + " says thickly.  You smile and decide it's time to do some feeling yourself; you grasp and pinch at " + benoitMF("his","her") + " tight, supple behind through " + benoitMF("his","her") + " trousers, making " + benoitMF("him","her") + " gasp as you move into " + benoitMF("him","her") + ".");
		}
		output("\n\n" + benoitMF("His","Her") + " hands trail further down, moving into your inner thighs, exploring your soft flesh until " + benoitMF("he","she") + " touches your lips.  You close your eyes and sigh as " + benoitMF("he","she") + " slowly parts them with " + benoitMF("his","her") + " smooth fingers and slides into your [pc.vagina].  Although " + benoitMF("his","her") + " breath is becoming increasingly heavy " + benoitMF("he","she") + " also seems genuinely curious about you; with surprising gentleness " + benoitMF("his","her") + " fingers travel over and around your moistening sex, exploring your every fold, working deeper and deeper as " + benoitMF("he","she") + " does.  You let " + benoitMF("him","her") + " know what pleases you by sighing when " + benoitMF("he","she") + " touches a sweet spot, moving deliberately with " + benoitMF("his","her") + " finger's motions so " + benoitMF("he","she") + " may give them better attention.  " + benoitMF("He","She") + " soon finds your [pc.clit], beginning to bulge with need; slowly " + benoitMF("he","she") + " circles it and then flicks at it, gently frigging you.");
		...
		output("\n\nYou keep at this for what feels like hours, stopping and starting, pulling and pushing deliberately to keep the basilisk you have backed into a corner under your control.  The pace of the fuck is clearly agonising for " + benoitMF("Benoit","Benoite") + "; " + benoitMF("he","she") + " pants, tenses and gasps to the wet movement of your [pc.vagina], " + benoitMF("his","her") + " face and chest heaving with extreme arousal, but " + benoitMF("he","she") + " lets you stay in command, eventually giving up on anything but lying back and letting you have your way with " + benoitMF("him","her") + ".  To give " + benoitMF("him","her") + " something to do, you grab " + benoitMF("his","her") + " hands and lay them upon your [pc.chest]; after exploring your flesh for a time, " + benoitMF("he","she") + " finds your [pc.nipple]s and begins to knead them, placing each nub between two fingers and rubbing them insistently.");
		if(pc.lactationQ() > 100) {
			output("  You moan as your breasts reward " + benoitMF("his","her") + " attention with spurts of milk; " + benoitMF("he","she") + " starts in genuine amazement when they ");
			if(!pc.isTaur()) output("hit " + benoitMF("him","her") + " in the face");
			else output("trickle over " + benoitMF("his","her") + " fingers");
			output(".  \"<i>Mammals are so damn weird,</i>\" " + benoitMF("he","she") + " mutters, making you giggle.");
		}
...
private function firstTimeAfterBoningEncounterBenoit():void {
	...
	output("\n\n\"<i>'Allo again, [pc.name]</i>\"!</i>\" " + benoitMF("he","she") + " says brightly.  \"<i>'Ow pleasant it is to see you.  'Ow are we zis very fine day?</i>\"  There's something imperceptibly different about " + benoitMF("Benoit","Benoite") + " today and it takes you a moment to work out what it is.  " + benoitMF("He","She") + " doesn't seem quite as shabby as " + benoitMF("he","she") + " did before; " + benoitMF("his","her") + " scales gleam dully and you wonder if " + benoitMF("he","she") + "'s taken a bath recently.  There's something else, too.");
...
private function repeatSexWithBenoitLetHim():void {
	clearOutput();
	if(pc.isTaur()) {
		output("\n\nFor the moment you don't do anything; you simply stand back and let " + benoitMF("his","her") + " hands slowly move across your frame.  One of " + benoitMF("his","her") + " hands comes to rest upon your [pc.nipples]; as " + benoitMF("he","she") + " gently teases and kneads the soft, sensitive flesh " + benoitMF("his","her") + " other hand drift downwards, across your [pc.belly], around your waist and down your velvety back.  Although " + benoitMF("he","she") + " is familiar with your frame by now " + benoitMF("Benoit","Benoite") + " never seems to stop being enthralled by your body; there is an unconscious frown of concentration on " + benoitMF("his","her") + " face as " + benoitMF("his","her") + " smooth hands move across your warm [pc.skinFurScales], as if " + benoitMF("he","she") + " were mapping you in " + benoitMF("his","her") + " mind's eye.");
		...
		output("\n\nBenoit surges forwards, grasps you by the [pc.butt], turns and pinions you to the wall.  Your laughs turn to gasps as with an almost pained growl, " + benoitMF("Benoit","Benoite") + " thrusts himself straight into your moist twat.  You can only clutch at the wall of the wagon as " + benoitMF("he","she") + " begins to fuck you with abandon, pushing your human front forwards as " + benoitMF("he","she") + " levers your [pc.hips] upwards with each thrust.  " + benoitMF("He","She") + " is clumsy with lust as well as blind, uncaring of anything but the need to clench your body and hilt himself in your depths, unable to take hold of himself with your pussy juices coating " + benoitMF("his","her") + " nose and burning an unstoppable path to " + benoitMF("his","her") + " brain.  " + benoitMF("His","Her") + " pheromone driven callousness and the discomfort of the wood against your hands ");
		if(pc.biggestTitSize() >= 1) output("and [pc.fullChest] ");
		output("augments the roughness of the fuck and you find yourself getting carried along with it, thrusting your powerful rump back against the basilisk and clenching " + benoitMF("him","her") + " deeper into you, delighting in " + benoitMF("his","her") + " firm, dense mass pounding into you.  Your [pc.vagina] dribbles juices around " + benoitMF("his","her") + " impatient, straining dick, practically foaming as " + benoitMF("his","her") + " smooth length rubs forcefully all along your sensitive tunnel.");
		pc.cuntChange(0,14,true,true,false);
		...
		output("\n\nHe pulls out of you, trailing threads of semen as " + benoitMF("he","she") + " goes, before slowly turning you around, painstakingly feeling around for a clear space with " + benoitMF("his","her") + " feet, and then putting you down, gently but firmly positioning you on your lower body.  " + benoitMF("His","Her") + " hands descend upon your [pc.butt]; grinning, you poke it outwards invitingly, sighing with mock frustration as the blind basilisk painstakingly lines up " + benoitMF("his","her") + " next shot with your drooling, sticky sex.");
		...
		output("For the moment you don't do anything; you simply stand back and let " + benoitMF("his","her") + " hands slowly move across your frame.  One of " + benoitMF("his","her") + " hands comes to rest upon your [pc.nipple]; as " + benoitMF("he","she") + " gently teases and kneads the soft, sensitive flesh " + benoitMF("his","her") + " other hand drift downwards, across your belly, around over the crack of your [pc.butt] then down to cup your behind.  Although " + benoitMF("he","she") + " is familiar with your frame by now, " + benoitMF("Benoit","Benoite") + " never seems to stop being enthralled by your body; there is an unconscious frown of concentration on " + benoitMF("his","her") + " face as " + benoitMF("his","her") + " smooth hands move across your warm skin, as if " + benoitMF("he","she") + " were mapping you in " + benoitMF("his","her") + " mind's eye.");
		
		output("\n\nThis slow, gentle pressure is all very well, but you can't help but wonder if you can't awaken something a bit more bestial in the timid basilisk.  The thought of making " + benoitMF("him","her") + " lose " + benoitMF("his","her") + " self-control over you causes you to grin, unseen, and tenderly but firmly you put your hands on " + benoitMF("his","her") + " claws and tell " + benoitMF("him","her") + " to stop.  " + benoitMF("He","She") + " looks at you in puzzlement as you shift your bodies around, your hands sliding over " + benoitMF("his","her") + " shoulders and chest as you change position so that it is you with your back to the wall.  " + benoitMF("He","She") + " begins to move " + benoitMF("his","her") + " hands again and you tut mockingly, telling " + benoitMF("him","her") + " to be still for now.  Smiling, you begin to give " + benoitMF("him","her") + " some of " + benoitMF("his","her") + " own treatment; your hands drift softly over " + benoitMF("his","her") + " tight, smooth flesh, working down " + benoitMF("his","her") + " washboard stomach until they reach " + benoitMF("his","her") + " long johns.  You slowly unbuckle them and let them fall, releasing " + benoitMF("his","her") + " long, thin erection.  With one hand you circle the base gently; even touching this least sensitive part of " + benoitMF("his","her") + " dick makes " + benoitMF("him","her") + " grunt with need, the thump of " + benoitMF("his","her") + " heart reverberating through " + benoitMF("his","her") + " scales, and " + benoitMF("he","she") + " involuntarily thrusts forwards, trying to get more of your hand upon " + benoitMF("him","her") + ".  Again, you tell " + benoitMF("him","her") + " to be still.  You continue to almost-masturbate " + benoitMF("him","her") + ", your one hand rubbing the very base of " + benoitMF("his","her") + " cock and the slit from which it thrusts, whilst with the other you reach down and touch your own slickening [pc.vagina].  You moan exaggeratedly as you dip first one finger and then two into your honey pot, gently frigging your [pc.clit] until you are in full spate, dripping your fluids onto the packed earth beneath you.  " + benoitMF("Benoit","Benoite") + " is clenching " + benoitMF("his","her") + " pointed teeth, trembling slightly like a pipe about to burst, as you lift your coated fingers up and smear your essence over " + benoitMF("his","her") + " incredibly receptive nostrils.  " + benoitMF("His","Her") + " tail thrashes fitfully as you continue to torment " + benoitMF("his","her") + " dick, just barely touching " + benoitMF("his","her") + " purple tip before returning to " + benoitMF("his","her") + " base.  All " + benoitMF("he","she") + " can smell is your own arousal as you begin to talk huskily, saying you've been a naughty girl playing in the mountains, you've teased and mocked and run away from every creature you've found but now a big bad basilisk has got you cornered, and what is the big bad basilisk going to do now it's got this naughty girl all to itself...?");
		
		output("\n\nBenoit surges forward, grasps you by the [pc.butt] and pinions you to the wall.  Your laughs turn to gasps as " + benoitMF("he","she") + " lifts your teasing hands upwards, pinning them out of the way with the mass of " + benoitMF("his","her") + " body and with an almost pained growl, thrusts himself straight into your moist twat.  You can only clutch at " + benoitMF("his","her") + " back as " + benoitMF("he","she") + " begins to fuck you with complete abandon, smacking your back into the wall of the wagon and levering your [pc.hips] upwards with each thrust.  " + benoitMF("He","She") + " is clumsy with lust as well as blind, uncaring of anything but the need to clench your body and hilt himself in your depths, unable to take hold of himself with your pussy juices coating " + benoitMF("his","her") + " nose and burning an unstoppable path to " + benoitMF("his","her") + " brain.  " + benoitMF("His","Her") + " pheromone-driven callousness and the discomfort of the wood rubbing against your back and butt augments the roughness of the fuck and you find yourself getting carried along with it, wrapping your thighs around the basilisk's thin waist and clenching " + benoitMF("him","her") + " deeper into you, delighting in " + benoitMF("his","her") + " firm, dense mass pounding into you.  Your [pc.vagina] dribbles juices around " + benoitMF("his","her") + " impatient, straining dick, practically foaming as " + benoitMF("his","her") + " smooth length rubs forcefully all along your sensitive tunnel.");
		...
		output(".  " + benoitMF("His","Her") + " hands descend upon your [pc.butt]; grinning, you poke it outwards invitingly, sighing with affected frustration as the blind basilisk painstakingly lines up " + benoitMF("his","her") + " next shot with your drooling, sticky sex.");
		
		output("\n\nWhen " + benoitMF("he","she") + " slides into you " + benoitMF("he","she") + " does so with virtually no resistance whatsoever; the two of you have turned your pussy into an inviting, gooey sleeve.  ");
		if(pc.vaginalCapacity() < 30) output("You don't mind; your tight sex is a perfect fit for " + benoitMF("his","her") + " smooth, thin dick, and you work with " + benoitMF("him","her") + " as " + benoitMF("he","she") + " thrusts, pulling and pushing your walls in time with " + benoitMF("his","her") + " length, your lubrication allowing " + benoitMF("him","her") + " to quickly increase the tempo until the two of you are once again bucking against each other gleefully, your fluids spattering against each other.");
		else output("Although " + benoitMF("he","she") + " is long, " + benoitMF("he","she") + " barely even touches the sides of your encompassing twat.  The sensation isn't great for you until " + benoitMF("he","she") + " really starts to go to town, ramming into you with all " + benoitMF("he","she") + "'s got, beating a wet staccato against your [pc.butt].  You send a hand roaming back around and begin to finger your [pc.clit] as " + benoitMF("he","she") + " sheaths himself in you, " + benoitMF("his","her") + " fluids running down your fingers as you work your slick, bulging pleasure button with increasing delight.");
		pc.cuntChange(0,10,true,true,false);
		output("  The difference in the position makes " + benoitMF("his","her") + " dick bend into you at an angle, stroking a neglected spot which soon has you gasping with need.  " + benoitMF("Benoit","Benoite") + " is not as maddened as " + benoitMF("he","she") + " was the first time, and " + benoitMF("he","she") + " has the composure to draw himself out; " + benoitMF("he","she") + " slows himself down and then back up again, fucking you magisterially, withdrawing himself almost completely before slamming firmly back in, stopping whilst hilted in you until you beg and moan for " + benoitMF("him","her") + " to continue.  You give yourself up to the dominant rut you've awoken in " + benoitMF("him","her") + ", thrusting back into " + benoitMF("him","her") + " as you are fucked up to a second ecstatic height and then a third; everything disappearing underneath a timeless red haze, of being on your hands and knees with your [pc.butt] in the air and being given what a female animal needs.");
	...
	flags["COC.BENOIT_TIMES_SEXED_FEMPCS"]++;
	benoitKnocksUpPCCheck();
	benoitAffection(2);
	pc.orgasm();
	doNext(returnToCampUseOneHour);
...
private function repeatBenoitFuckTakeCharge():void {
	clearOutput();
	if(pc.isTaur()) {
		output("nFor the moment you don't do anything; you simply stand back and let " + benoitMF("his","her") + " hands slowly move across your frame.  One of " + benoitMF("his","her") + " hands comes to rest upon your [pc.nipple]; as " + benoitMF("he","she") + " gently teases and kneads the soft, sensitive flesh " + benoitMF("his","her") + " other hand drift downwards, across your belly, then slowly back along your vast tauric frame.  Although " + benoitMF("he","she") + " is familiar with your frame by now " + benoitMF("Benoit","Benoite") + " never seems to stop being enthralled by your body; there is an unconscious frown of concentration on " + benoitMF("his","her") + " face as " + benoitMF("his","her") + " smooth hands move across your warm [pc.skinFurScales], as if " + benoitMF("he","she") + " were mapping you in " + benoitMF("his","her") + " mind's eye.");
		...
		output("\n\n");
		//[Small capacity:]
		if(pc.vaginalCapacity() < 30) output("You slide back down onto " + benoitMF("him","her") + ", cooing this time as you feel " + benoitMF("him","her") + " bottom out with several inches of " + benoitMF("him","her") + " still outside of you.  " + benoitMF("His","Her") + " thin, long length could not be more perfect for your tight sex; " + benoitMF("he","she") + " rubs your walls up and down as you dreamily thrust in and out of " + benoitMF("his","her") + " body, sending waves of pleasure flowing through you.");
		//Large capacity: 
		...
		//begin copypasta
		output("" + benoitMF("His","Her") + " hands trail further down, moving into your inner thighs, exploring your soft flesh until " + benoitMF("he","she") + " touches your lips.  You close your eyes and sigh as " + benoitMF("he","she") + " slowly parts them with " + benoitMF("his","her") + " smooth fingers and slides into your [pc.vagina].  Although " + benoitMF("his","her") + " breath is becoming increasingly heavy " + benoitMF("he","she") + " also seems genuinely curious about you; with surprising gentleness " + benoitMF("his","her") + " fingers travel over and around your moistening sex, exploring your every fold, working deeper and deeper as " + benoitMF("he","she") + " does.  You let " + benoitMF("him","her") + " know what pleases you by sighing when " + benoitMF("he","she") + " touches a sweet spot, moving deliberately with " + benoitMF("his","her") + " finger's motions so " + benoitMF("he","she") + " may give them better attention.  " + benoitMF("He","She") + " soon finds your [pc.clit], beginning to bulge with need; slowly " + benoitMF("he","she") + " circles it and then flicks at it, gently frigging you.");
		if(pc.hasStatusEffect("Heat")) output("  By now your vagina is practically gushing, your body's own deep-seated pheromone need stoked to blazing heights by the basilisk's gentle, painstaking exploration.  You cannot stop yourself thrusting against " + benoitMF("his","her") + " soaked hand, announcing how badly you want this with heavy moans.");
...
private function tryToConvertToBassyWomb():void {
	...
	if(!(pc.hasItem(new CoCOvipositionElixir(),2) && pc.hasItem(new CoCReptilum()) && pc.hasItem(new CoCGoblinAle()))) {
		output("You don't have the necessary ingredients to attempt this yet.  You recall " + benoitMF("Benoit", "Benoite") + " mentioning that you would need Reptilum, two Ovi Elixirs, and Goblin Ale.");
		clearMenu();
	...
	//Ingredients in inventory: 
		...
		output("\n\n\"<i>[pc.name]?</i>\" says " + benoitMF("Benoit","Benoite") + " nervously.  \"<i>Are you all right?  Shall I call ze sawbones? I will call ze sawbones.  'E is mostly good at taking people apart and putting zem back togezzer again, but I am sure 'e can give you a good purgative if we rush...</i>\"  You toy with the idea of staging a dramatic allergic reaction, but deciding not to fray " + benoitMF("his","her") + " nerves any further you tell " + benoitMF("him","her") + " you feel absolutely fine.  Indeed, stroking your belly, you are almost certain that it worked.");
		...
		if(pc.lust() >= 33) {
			...
			if(pc.tallness <= 78 && !pc.isTaur()) output("  The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and hoists you over " + benoitMF("his","her") + " shoulder.  You squeal in mock terror as " + benoitMF("he","she") + " hauls you as fast as " + benoitMF("he","she") + " can into the back room, knocking over half " + benoitMF("his","her") + " stock as " + benoitMF("he","she") + " does.");
			else output("  The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and frenetically attempts to hoist you over " + benoitMF("his", "her") + " shoulder.  You are far too big for " + benoitMF("him", "her") + " though; after several valiant attempts, " + benoitMF("he", "she") + " collapses against a shelf.  Laughing, you pick the stricken, panting reptile up, hoist " + benoitMF("him", "her") + " over your own shoulder, and navigate a path into the back room.");
			clearMenu();
			addButton(0, "Next", suggestSexAfterBasiWombed, false);
			return;
		}
		else output("  You grin and say you're not even sure it worked... but you'll be back at some point to try it out, and " + benoitMF("he", "she") + "'d better be ready for when you do.  You gently pry yourself out of " + benoitMF("his", "her") + " grip and leave as deliberately as you can, aware of the beguiling, invisible scent you are leaving for the stunned, silent basilisk to simmer in.");
		clearMenu();
...
private function suggestSexAfterBasiWombed(later:Boolean = true):void {
	...
	if(later) {
		output("You smile coquettishly and ask if " + benoitMF("he","she") + "'s interested in taking your new body for a... test drive.  ");
		//(6'6 or less: 
		if(pc.tallness <= 78 && !pc.isTaur()) output("The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and hoists you over " + benoitMF("his","her") + " shoulder.  You squeal in mock terror as " + benoitMF("he","she") + " hauls you as fast as " + benoitMF("he","she") + " can into the back room, knocking over half " + benoitMF("his","her") + " stock as " + benoitMF("he","she") + " does.");
		else output("The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and frenetically attempts to hoist you over " + benoitMF("his","her") + " shoulder.  You are far too big for " + benoitMF("him","her") + " though; after several valiant attempts, " + benoitMF("he","she") + " collapses against a shelf.  Laughing, you pick the stricken, panting reptile up, hoist " + benoitMF("him","her") + " over your own shoulder, and navigate a path into the back room.");
		output("\n\n");
	...
	if(!pc.isTaur()) {
		...
		else output("" + benoitMF("Benoit","Benoite") + " staggers into the back room, bumping into several things as " + benoitMF("he","she") + " sets you down, thankfully onto a clear space on the ground.  " + benoitMF("He","She") + " leans into you, " + benoitMF("his","her") + " weight pushing you onto the ground, hands flying over your [pc.gear], fumbling off clasps and belts when " + benoitMF("he","she") + " finds them.  Now that your gut has settled you feel imbued with an odd, ripe sensation; your belly bulges ever-so-slightly with unfertilized eggs and you feel red, soft and ready.  You work with " + benoitMF("him","her") + ", wriggling out of your clothes and ripping " + benoitMF("his","her") + " own trousers off, spreading your [pc.hips] eagerly as " + benoitMF("his","her") + " long, smooth prick springs to attention.");
		
		output("\n\nThe basilisk needs no further invitation.  In a moment " + benoitMF("he","she") + " is upon you, " + benoitMF("his","her") + " tight, muscled chest pressed against your [pc.chest], " + benoitMF("his","her") + " flat stomach rubbing over your own fertile belly and the head of " + benoitMF("his","her") + " dick pushed against your moist lips.");
		if(pc.hasCock()) output("  " + benoitMF("He","She") + " deliberately rubs himself up and down your body, and the [pc.cock] trapped between your warm bodies quickly hardens against the warm, smooth friction.");
		output("  Despite " + benoitMF("his","her") + " blindness, " + benoitMF("he","she") + " slides straight into your moist depths, making you coo as " + benoitMF("his","her") + " hard, smooth spur glides across your sensitive walls, slowly bringing himself out before thrusting himself in again, working more and more of " + benoitMF("his","her") + " length into you.  Never quite able to control himself around your body, it's obvious in the strain in " + benoitMF("his","her") + " face and the raggedness of " + benoitMF("his","her") + " breath against your skin that " + benoitMF("he","she") + " is exercising every restraint " + benoitMF("he","she") + " has not to fuck you into the ground; " + benoitMF("he","she") + " pushes " + benoitMF("his","her") + " dick upwards with each return thrust to bump deliberately against your [pc.clit], sending irresistible spasms of pleasure chiming through you.  Pushed inexorably upwards you curl an arm around " + benoitMF("his","her") + " neck, kiss " + benoitMF("his","her") + " nose and grit your teeth, then whisper into " + benoitMF("his","her") + " ear to stop holding back.  " + benoitMF("Benoit","Benoite") + " pauses for a moment to gather " + benoitMF("his","her") + " breath, hilted entirely in your wet cunt, then hooks " + benoitMF("his","her") + " hips around yours, entrapping you around " + benoitMF("his","her") + " body, before beginning to fuck your softened, ripe body like a jackhammer.  " + benoitMF("He","She") + " pounds into you with everything " + benoitMF("he","she") + "'s got, clenching you as your gushing [pc.vagina] deliriously spatters fluids across your entwined bodies.  Lost in rut now, " + benoitMF("Benoit","Benoite") + " licks your face with " + benoitMF("his","her") + " long tongue, the soft, sticky pressure against your reddened cheeks only seeming to make the contrasting sensation of " + benoitMF("his","her") + " long prick taking you deep even more overwhelming.");
		pc.cuntChange(0,14,true,true,false);
		...
		output("\n\nThe basilisk needs no further invitation.  In a moment " + benoitMF("he","she") + " is upon you, " + benoitMF("his","her") + " tight, muscled chest is pressed against your [pc.ass], " + benoitMF("his","her") + " hands moving over your own fertile belly and the head of " + benoitMF("his","her") + " dick pushed against your moist [pc.vagina].");
		//[Herm:
		if(pc.hasCock()) output("  " + benoitMF("He","She") + " deliberately moves " + benoitMF("his","her") + " hand over [pc.oneCock] before trapping it in " + benoitMF("his","her") + " warm grasp.   It quickly hardens against " + benoitMF("his","her") + " warm, smooth friction.");
		output("  Used to your body now despite " + benoitMF("his","her") + " blindness, " + benoitMF("he","she") + " slides straight into your moist depths, making you coo as " + benoitMF("his","her") + " hard, smooth spur glides across your sensitive walls, slowly bringing himself out before thrusting himself in again, working more and more of " + benoitMF("his","her") + " long length into you.   Never quite able to control himself around your body, it's obvious in the strain of the muscles pressed against you and the raggedness of " + benoitMF("his","her") + " breath upon your skin that " + benoitMF("he","she") + " is exercising every restraint " + benoitMF("he","she") + " has not to fuck you into the ground; " + benoitMF("he","she") + " pushes " + benoitMF("his","her") + " dick downwards with each return thrust to bump deliberately against your [pc.clit], sending irresistible spasms of pleasure chiming through you.  You slowly move forwards until your arms are braced against the wall, before gritting over your shoulder to " + benoitMF("him","her") + " to stop holding back.  " + benoitMF("Benoit","Benoite") + " pauses for a moment to gather " + benoitMF("his","her") + " breath, hilted entirely in your wet cunt, then hooks " + benoitMF("his","her") + " strong arms around your back end, entrapping you around " + benoitMF("his","her") + " body, before beginning to fuck your softened, ripe body like a jackhammer.  " + benoitMF("He","She") + " pounds into you with everything " + benoitMF("he","she") + "'s got, clenching you as your gushing [pc.vagina] deliriously spatters fluids across your entwined bodies."); 
		pc.cuntChange(0,14,true,true,false);
...
//
	//output("\n\n\"<i>What is it?!</i>\" " + benoitMF("he","she") + " snarls, displaying " + benoitMF("his","her") + " fangs when " + benoitMF("he","she") + " pops " + benoitMF("his","her") + " head irritably out of the door.  " + benoitMF("He","She") + " stops and inhales through " + benoitMF("his","her") + " nose, starting lightly when " + benoitMF("he","she") + " recognizes your scent.  \"<i>Oops!  [pc.name], I am zo sorry, I did not think it would be you.  But why are you here at such an early hour?</i>\"");
	//
	...
	//output(".  Well, " + benoitMF("he","she") + " will be when this ");
	...
	//}
	//pc.orgasm();
	//pc.knockUpForce(); //Clear Pregnancy
	//flags["COC.BENOIT_EGGS"] += Math.floor(pc.totalFertility() / 10);
	////doNext(1);
//}
...
public function femoitInitialTalk():void
{
	...
	output("You ask " + benoitMF("Benoit","Benoite") + " if " + benoitMF("he","she") + " has ever thought about trying to do something to help " + benoitMF("his","her") + " people's plight.");
...
/*
	if (pc.hasItem(consumables.BIMBOLQ))
	{
...
//
	//// Ingredients not in inventory
	//if (!pc.hasItem(consumables.P_S_MLK, 2) || !pc.hasItem(consumables.L_PNKEG) || !pc.hasItem(consumables.OVIELIX) || !pc.hasItem(consumables.REPTLUM))
	//{
...
//
		//flags["COC.FEMOIT_NEXTDAY_EVENT"] = this.getGame().model.time.days + 1;
...
//
	//if (pc.isTaur()) output("\n\nHer warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  \"<i>Good Gods,</i>\" she murmurs as her hands lead back onto your flanks.  \"<i>Good Gods!</i>\" she cries out as she follows you all the way back to your mighty, powerful rear.  \"<i>I knew you were a centaur because of all ze clopping,</i>\" she says, rubbing your side back and forth in wonder.  \"<i>But to know it and actually feel it, zey are very different.</i>\"  She sighs.  \"<i>Zis is going to be... awkward, but I guess you are probably used to zat by now, yes?</i>\"");
	//else if (pc.isDrider()) output("\n\nHer warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  \"<i>Good Gods,</i>\" she murmurs as her hands lead back onto your abdomen. \"<i>Good Gods!</i>\" she cries out as she follows your bulging abdomen all the way back to your spinnerets. \"<i>I knew you were a spider because of all ze click clacking,</i>\" she says, her fingers feeling around one of your intricate, many-jointed legs in wonder . \"<i>But to know it and actually feel it, zey are very different.</i>\"");
	//else if (pc.demonScore() > 4) output("\n\nHer warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  She touches your horns and pauses; she reaches around, finds and grips your tail, running her grasp up to the spaded point. \"<i>So,</i>\" she says quietly. \"<i>You are one of zem.</i>\" She is silent for a while before finding a warm smile. \"<i>But I am being zilly.  I know you are different inside.</i>\"")
	//else if (pc.dogScore() >= 4 && pc.earType == 2 && pc.tailType == 2) output("\n\nHer warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  She grins as she finds your floppy ears and outright laughs when she reaches around and touches your tail.  \"<i>I like dogs but not ZAT much, \"<i>[pc.name],</i>\" she giggles.  \"<i>No wonder Pierre 'as been acting jealous recently.</i>\"");
	//else if ((pc.bunnyScore() >= 4 && pc.earType == 7 && pc.tailType == 10) || (pc.catScore() >= 4 && pc.earType == 5 && pc.tailType == 8)) output("\n\nHer warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  She grins as she finds your ears, outright laughs when she reaches around and touches your soft tail.  \"<i>I always wondered why Pierre gets all excited when 'e sees you,</i>\" she giggles.");
	//else if (pc.harpyScore() >= 4 && pc.wingType != 0 && pc.armType == 1) output("\n\nHer warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  She finds your wings and follows them up as far as she can reach; she carefully shifts her feet forward to touch at your own clawed toes.  \"<i>So zis is what irony is,</i>\" she murmurs, a smile playing on her lips as she touches your shoulder.  \"<i>My saviour is an 'arpy, come to ravish me.</i>\"");
	//else if (pc.beeScore() >= 4 && pc.wingType != 0 && pc.lowerBody == 7) output("\n\nHer warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  She finds your diaphanous wings and follows them up as far as she can reach, her grip on your sensitive membranes making you twitch a bit; then she sends her hands trailing down your carapace-armored limbs. \"<i>I always sought you just liked wearing big boots,</i>\" she murmurs. \"<i>But zis is actually a part of you?  'Ow... interesting.</i>\"");
	//else if (pc.gooScore() >= 4 && pc.skinType == 3) output("\n\nHer warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  \"<i>I knew you were different from ze squishy sounds you made,</i>\" she murmurs as her hands sink into your soft, amorphous mass.  \"<i>But zis is... good Gods, zis is strange.  And zis doesn't 'urt you at all?</i>\" she asks incredulously as she gently pokes a finger into you.  You answer her question by laughing.  \"<i>Zat must come in very useful,</i>\" she says.  You push yourself slowly up her arms and tell her she has no idea.");
	//else if (pc.skinType == 2) output("\n\nHer warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  She starts slightly when she touches your scales, and then caresses the reptilian parts of your body with increasing interest.  \"<i>You didn't do zis just for me, did you [pc.name]?</i>\" she murmurs.  \"<i>I 'ave to admit - it feels very good.</i>\"");
	//else output("\n\nHer warm fingers travel over your body, brushing over your face, your belly, your [pc.hips]; you feel as though you're being read like a book.  \"<i>You 'umans are so squishy, fuzzy and 'ot,</i>\" she giggles huskily. \"<i>'Ow can you stand it?</i>\"");
//
...
//
		...
		//pc.orgasm();
	//}
	//else if (benoitRegularPreggers() && (!pc.isTaur() || (pc.isTaur() && (pc.tallness * (5/6) < pc.cocks[pc.longestCock()].cLength()))))
	//{
		//clearOutput();
...
//
	...
	//if (flags["COC.FEMOIT_EGGS"] > 1) output("s");
