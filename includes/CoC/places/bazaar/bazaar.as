...
//private function theSlipperySqueeze():void {
	...
	//}
	////Cock Milker
	//else if(pc.hasKeyItem("Cock Milker") < 0 && flags[kFLAGS.JOEY_OFFERED_MILKER] == 0 && pc.hasKeyItem("Cock Milker - Installed At Whitney's Farm") < 0) {
	...
	////(Repeat visit: 
	//else {
		//output("Joey purses his glossed lips when you enter and coyly cocks one of his ears as he says, \"<i>Welcome back to 'The Slippery Squeeze', " + pc.short + ".  Would you like me to give you a nice, salty rub-down?  Or would you prefer Sara do it?  Just remember, she can't make her own 'oil' like I can.</i>\"\n\n", false);
	...
	//}
	//if(flags[kFLAGS.JOEY_OFFERED_MILKER] > 0 && pc.hasKeyItem("Cock Milker") < 0 && pc.hasKeyItem("Cock Milker - Installed At Whitney's Farm") < 0)
	...
	//if(isEaster()) {
		...
		//if(pc.gems < 20) output("  You can't afford it.");
...
//private function joeyAndrogyny():void {
	//clearOutput();
	//if(pc.gems < 500) {
		//output("You haven't got enough gems for that treatment!", false);
		//doNext(theSlipperySqueeze);
		//return;
	//}
	//pc.gems -= 500;
	//statScreenRefresh();
	//output("Joey takes your hand in his and insistently pulls towards the back rooms.  \"<i>Once we have this all done ", false);
	//if(pc.gender == 1) output("you'll be able to be as cute as me!", false);
	//else if(pc.gender == 2) output("you'll be able to be as masculine as you want!", false);
	//else output("you'll be able to be as cute as me or as masculine as you want!", false);
	...
	//
	//output("Thanking the cute bunny-boy for his help, you hand over the payment and head back to check on camp.", false);
...
//private function joeyMassage():void {
	//clearOutput();
	//if(pc.gems < 10) {
		//output("Joey frowns when you realize you don't have the 10 gems.  He apologizes, \"<i>I'm sorry, " + pc.short + " but I can't give freebies - our special potions cost us plenty.", false);
		//doNext(enterTheBazaar);
		//return;
	//}
	//pc.slimeFeed();
	...
	//if(pc.hasCock()) output("  Would you like one?", false);
	//output("</i>\"", false);
	//if(pc.hasCock()) {
...
//private function joeysMassageWifNoExtraJizz():void {
	//clearOutput();
	////(Continue as NoWang)
	//output("The rabbit-eared fem-boy climbs back onto the table and strokes himself a few times over your " + assDescript() + "; the first drops of his 'special oil' feel hot as they land on the curves of your butt cheeks.  He climbs over you, touching himself just enough to stay hard while his cum-drooling cock stops dripping and starts genuinely leaking.  A long trail of bunny-spunk is dripped onto your " + assDescript() + " until you're glazed with thick ropes of it.  You spot his discarded thong on the floor and giggle as you feel him flip around to put his cute bunny-butt on your shoulders.  His spunk immediately runs down your spine, even as his hands smear it all over your " + pc.skinDesc + ".\n\n", false);
	//output("The massage heads back towards your [pc.ass]; Joey's hands fill with your flesh as he fondles and strokes, spreading the jism into every nook and cranny, even your " + assholeDescript(), false);
	//if(pc.hasVagina()) output(" and " + vaginaDescript(0), false);
	...
	//
	//output("Joey stands and turns, his cum drooling into your " + hairDescript() + " as he asks, \"<i>Would you roll over for me?</i>\"  Eager for more of his skilled massage and impelled by your own growing arousal, you roll over, letting his jism drip onto your face.  He sits down gently, resting his barely-felt weight atop your bellybutton, and in no time, his fingers are smearing more of his fragrant, constantly-leaking goo over your " + chestDesc() + ".  The long-eared fem-boy rubs around your [pc.nipples], trailing delicate circles that make the perky, pink flesh harden and glisten.\n\n", false);
	...
	//output("You catch a view of your hand as it ", false);
	//if(!pc.hasFuckableNipples()) output("tweaks and teases", false);
	//else output("sinks a finger deep inside", false);
	...
	//if(pc.hasCock()) {
		...
		//if(pc.totalCocks() > 1) {
			//output("  He takes your " + cockDescript(1) + " with his free hand and pulls it over, giving it an affectionate, loving smooch.", false);
			//if(pc.totalCocks() > 2) {
				//output("  The process is repeated", false);
				//if(pc.totalCocks() > 3) output(" as necessary until " + sMultiCockDesc() + " is coated with dick-drenching bunny spit.", false);
				//else output(", coating your " + cockDescript(2) + " with dick-drenching bunny spit.", false);
			//}
		//}
		//output("  A moment later, ", false);
		//if(pc.balls > 0) output("his hand firmly rubs your " + sackDescript() + ", and ", false);
		//output("gentle fingertips are probing between your cheeks, rubbing his dripping seed against the pucker of your [pc.asshole].\n\n", false);
	//}
	////(FEM/GENDERLESS) 
	//else {
		//if(pc.hasVagina()) output("presses his ruby lips into the glistening delta of your mons", false);
		//else output("presses a finger against the semen-soaked ring of your " + assholeDescript(), false);
	...
	//if(!pc.hasCock()) {
		////(Genderless orgasm)
		//if(pc.gender == 0) output("Though his single finger makes you burn with passion, Joey's second slides effortlessly after it, filling your [pc.asshole] with another of his cum-soaked digits.  His warm jism slides down the crack between the fingers, slowly pooling in your backdoor.  Once you've adjusted, he continues to the massage, stroking and bumping your interior with the confident, practiced strokes of a professional. You pull hard on your tortured [pc.nipples], egging up the gradual upwelling of pleasure while you lick the bunny-cream from your lips.  A moment later, the no-longer-offending digits press hard on a sensitive spot, and you're arching your back, screaming with pleasure.\n\n", false);
		////(Female orgasm)
		//else {
			//output("Though his spit and cum-lubed tongue is quite skilled, deftly tasting your labia and channel, the bunny adds a pair of fingers to the mix, pulling the musky tunnel wide and letting more of his slippery seed inside you.  He uses it like lube, sliding his digits around while he sucks your [pc.clit] ", false);
			//if(pc.clitLength >= 4) output("like a practiced whore fellating a john.", false);
			//else output("like a professional pussy-licker.", false);
	...
	//else {
		...
		//if(pc.cumQ() >= 700) {
			//output("  A smile widens your " + pc.face() + " when you see the ", false);
			//if(pc.cumQ() >= 2000) output("massive, jiggling belly you've given him.", false);
			//else output("little paunch on his belly jiggle.", false);
			//output("  He won't be hungry for some time.", false);
		//}
	...
	//pc.orgasm();
	//if(pc.lib > 20) dynStats("lib", -.5);
	//if(pc.lib > 80) dynStats("lib", -1);
	//if(pc.lib > 60) dynStats("lib", -1);
...
//private function joeysMassageWithEXTRASpooge():void {
	...
	//if(pc.balls == 0) output("inside you", false);
	//else output("in your " + ballsDescriptLight(), false);
	//output(" that reminds you of the sensation just before orgasm.", false);
	//if(pc.balls > 0) output("  They even feel a little bigger.", false);
	//output("\n\n", false);
	//
	...
	//if(pc.cockTotal() > 1) output("s like fiddles", false);
	//else output(" like a fiddle", false);
	...
	//if(pc.balls > 0) output("Your balls pull and relax and pull tight, bouncing below you over and over, all while working hard to produce more juice for you to gush.  ", false);
	...
	//
	//output("Joey giggles, turning about to put his lotion-dispenser on the small of your back, and you feel the hot bunny-spoo pour onto you in a wave.   It's gathered up and pushed up your back.  Globs of it roll down your side, sliding over the semi-waterproofed sheets to mix with your growing spunk-lake.  With the slippery-seed aiding Joey's massage, he somehow manages to work out even more of your tension.  By this point your muscles feel like jello melting in the afternoon sun.  You feel like you could melt into your cum, and you sigh in bliss", false);
	//if(pc.cumQ() >= 1500) output(".  The sound of your rivers of spooge splattering against the floor barely registers, but a part of you is proud to be so incredibly fertile", false);
	//output(".\n\n", false);
	//
	...
	//if(pc.cumQ() >= 1500) output("  Joey's jaw drops from the unexpectedly large volume you're producing, and he watches in awe as each huge globule of cum rolls off you towards the floor.", false);
	//output("\n\n", false);
	//
	//output("The bunny-boy springs back atop you, landing hard just below your hips.  His dripping seed washes over " + sMultiCockDesc() + ", and the sudden onslaught of fresh, liquid warmth on your groin sets off a small explosion of jism that splatters into your chin, leaving a long trail of slime behind like a snail.  The long-eared girly-boy smiles and shifts to rub his small cock against your [pc.cock], frotting you aggressively while you both spray cum like faucets with the knobs torn off.  You don't mind that he seems to have forgotten the massage, and you run your hands up and down your " + chestDesc() + " to smear the heavy loads ", false);
	//if(pc.skinType != SKIN_TYPE_FUR) output("over your " + pc.skinDesc, false);
	//else output("through your fur", false);
	//output(".\n\n", false);
	//
	//output("The special potion makes it so your orgasm is long and languid, oozing out for minutes instead of seconds, and the small chamber is filled with breathy, exultant cries of passion.  Joey leans forward and grabs hold of your [pc.nipples], ", false);
	//if(!pc.hasFuckableNipples()) output("squeezing and tugging on them", false);
	//else output("sliding cum-coated fingers into their slippery depths", false);
	...
	//if(pc.cumQ() >= 1500) output("  You really flooded the room though, so you'd best walk on your tip-toes until we can get a succubi or a goblin in here to clean up.", false);
	//output("  Come back ANY time, " + pc.short + ".  I'd love to help you release again.</i>\"\n\n", false);
	//
...
//private function joeyBigBalls():void {
	...
	//if(flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00348] == 0) {
		...
		//
		//output("Why is it that no one ever desperately requires your assistance with a mundane task?  Sighing, you judge", false);
		//if(cor() > 60) output(", with no more than a bit of wishful thinking,", false);
		...
		//if(cor() > 70) output("; you won't get to watch him fountaining all that pearly spunk like a perverted statue", false);
		//output(".  What do you decide?", false);
...
//private function suckOffJoeysGardenHose():void {
	...
	//output("Joey's rouged lips silently mouth, \"<i>Oh gods,</i>\" as he delivers the next, even bigger rope to your eager maw, stuffing your mouth so quickly that his jism squirts from the corners before you can react.  You shiver from the semen overload", false);
	//if(pc.minotaurAddicted()) output(" and find yourself wishing you had one of these potions to use on a minotaur.  It would be divine!", false);
	//else output(" and keep at it.  The knowledge that you're helping a friend is almost as filling as his delicious cum.", false);
	...
	//if(cor() < 33) output("all for a good cause, right?", false);
	//else if(cor() < 66) output("also kind of fun, in a naughty way.", false);
	//else output("exactly the kind of nasty stuff that gets your blood pumping.", false);
	...
	//output("Joey blushes again and begins looking for a mop.", false);
	//dynStats("lus", 70);
	//doNext(returnToCampUseOneHour);
//}
//
...
//private function gretasGarments():void {
	...
	//else {
		//if(flags[kFLAGS.FOUND_SOCKS] == 2 && pc.cocks.length > 0 && pc.hasSockRoom()) addButton(1,"Browse Socks",browseDemSocksSon);
		//if(pc.hasSock()) addButton(2,"Remove Sock",takeOffDatSock);
	//}
...
//private function cockSelectionMenu():void {
	//menu();
	//if((flags[kFLAGS.SOCK_HOLDING] == "amaranthine" && pc.gems >= 1000) || (flags[kFLAGS.SOCK_HOLDING] == "gilded" && pc.gems >= 3000) || (flags[kFLAGS.SOCK_HOLDING] == "cobalt" && pc.gems >= 250) || (flags[kFLAGS.SOCK_HOLDING] == "scarlet" && pc.gems >= 250) || (flags[kFLAGS.SOCK_HOLDING] == "viridian" && pc.gems >= 1000) || (flags[kFLAGS.SOCK_HOLDING] == "cockring" && pc.gems >= 100) || (flags[kFLAGS.SOCK_HOLDING] == "alabaster" && pc.gems >= 25) || (flags[kFLAGS.SOCK_HOLDING] == "wool" && pc.gems >= 10) || (flags[kFLAGS.SOCK_HOLDING] == "green" && pc.gems >= 500) || (flags[kFLAGS.SOCK_HOLDING] == "red" && pc.gems >= 500) || (flags[kFLAGS.SOCK_HOLDING] == "blue" && pc.gems >= 500)) addButton(0,"Buy",pickACockForSock);
...
//
	//if(flags[kFLAGS.SOCK_HOLDING] == "amaranthine") pc.gems -= 1000;
//
	//if(flags[kFLAGS.SOCK_HOLDING] == "gilded") pc.gems -= 3000;
//
//
	//if(flags[kFLAGS.SOCK_HOLDING] == "cobalt") {
		//pc.gems -= 250;
		//// if(pc.findPerk(PerkLib.PhallicRestraint) < 0) pc.createPerk(PerkLib.PhallicRestraint,0,0,0,0);
	//}
//
	//if(flags[kFLAGS.SOCK_HOLDING] == "scarlet") {
		////if(pc.findPerk(PerkLib.PhallicPotential) < 0) pc.createPerk(PerkLib.PhallicPotential,0,0,0,0);
		//pc.gems -= 250;
	//}
//
	//if(flags[kFLAGS.SOCK_HOLDING] == "viridian") {
		//if(pc.findPerk(PerkLib.LustyRegeneration) < 0) {
			//pc.createPerk(PerkLib.LustyRegeneration,0,0,0,0);
			//pc.gems -= 1000;
		//}
		//else {
			//conflict = true;
...
//
	//if(flags[kFLAGS.SOCK_HOLDING] == "cockring") {
		//pc.gems -= 100;
		//if(pc.findPerk(PerkLib.PentUp) < 0) pc.createPerk(PerkLib.PentUp,10,0,0,0);
		//else pc.addPerkValue(PerkLib.PentUp,1,5);
	//}
//
	//if(flags[kFLAGS.SOCK_HOLDING] == "alabaster") pc.gems -= 25;
//
	//if(flags[kFLAGS.SOCK_HOLDING] == "wool") pc.gems -= 10;
//
	//output("You nod to the busty succubus and strip off your [pc.gear], revealing your naked body.  Greta's eyes light up as she looks over your body with barely-contained lust.  Finally her eyes settle onto your " + cockDescript(target) + ", and she licks her lips.  ");
//
	//if (!conflict) { // There's no conflict. DO IT!!!
		//pc.cocks[target].sock = flags[kFLAGS.SOCK_HOLDING];
		//statScreenRefresh();
//
...
//private function removeTargettedSock(index:int):void {
	...
	//output("\n\n\"<i>Ah, all right then,</i>\" she says smoothly, setting aside her knitting needles.  \"<i>Making room for a new sock, or just looking to get rid of this one?  No matter, it's a simple counterspell.</i>\"  Greta stands up from her chair, though she's only on her feet for a moment before she kneels down in front of you, placing one hand under your " + cockDescript(index) + ".  With her free hand, she runs a little circle around your " + pc.cockHead(index) + ", muttering something under her breath.");
	//
	...
	//if(extra && storage == "cockring") {
		//if(pc.perkv1(PerkLib.PentUp) >= 10) pc.addPerkValue(PerkLib.PentUp,1,-5);
		//else pc.setPerkValue(PerkLib.PentUp,1,10);
	//}
	//else {
		//if(storage == "gilded") {
			//pc.removePerk(PerkLib.MidasCock);
		//}
		//if(storage == "cobalt") {
			//pc.removePerk(PerkLib.PhallicRestraint);
		//}
		//if(storage == "scarlet") {
			//pc.removePerk(PerkLib.PhallicPotential);
		//}
		//if(storage == "viridian") {
			//pc.removePerk(PerkLib.LustyRegeneration);
		//}
		//if(storage == "cockring") {
			//pc.removePerk(PerkLib.PentUp);
		//}
	//}	
...
//private function eggsInButt(eggButt:Boolean = false):void {
	...
	//if(pc.hasCock()) {
		//if(pc.biggestCockArea() >= 30) output(", especially compared to you.");
		//else if(pc.smallestCockArea() < 6) output(", of course, if he's considered tiny, what does that make you?");
	//}
	//else output(".");
