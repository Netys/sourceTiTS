...
public function TelAdreWeaponShop():void {
	...
	//if (pc.hasKeyItem("Sheila's Lethicite") >= 0 || flags[kFLAGS.SHEILA_LETHICITE_FORGE_DAY] > 0) {
...
private function TelAdreWeaponShopList():void {
	...
	shopkeep.inventory = [new CoCWingstick(), new CoCClaymore(), new CoCWarhammer(), new CoCKatana(), new CoCSpear(), new CoCWhip(), new CoCWizardStaff(), new CoCSpikedGauntlet(), new CoCDagger(), new CoCScimitar(), new CoCMace(), new CoCFlail()];
...
//private function weaponBuy(itype:ItemType):void {
	//clearOutput();
	//spriteSelect(80);
	//output("The gruff metal-working husky gives you a slight nod and slams the weapon down on the edge of his stand.  He grunts, \"<i>That'll be " + itype.value + " gems.</i>\"", false);
...
//private function forgeScarredBlade():void {
	//if (pc.hasKeyItem("Sheila's Lethicite") >= 0) forgeScarredBladeStart();
