...
public function showScylla():void {
	userInterface.showName("\nSCYLLA");
	
	//if (flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] > 3) spriteSelect(82);
	//else spriteSelect(51);
}

public function get scylla():PregnancyPlaceholder // since there are no TFs... screw making actual character
{
...
public function scyllaBarSelectAction():void {
	...
	if (pc.cocks.length > 0) {
		if (pc.longestCockLength() >= 12) {
			if (flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] == undefined) {
				scyllaAction = SCYLLA_ACTION_FIRST_TALK;
				return;
			}
			if (flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] == 1 && rand(5) == 0) {
				scyllaAction = SCYLLA_ACTION_ROUND_TWO;
				return;
			}
			if (flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] == 2 && rand(5) == 0) {
				scyllaAction = SCYLLA_ACTION_ROUND_THREE;
				return;
			}
			if (flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] == 3 && rand(5) == 0) {
				scyllaAction = SCYLLA_ACTION_ROUND_FOUR;
				return;
			}
			if (flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] == 5 && rand(5) == 0) {
				scyllaAction = SCYLLA_ACTION_MEET_CATS;
				return;
			}
			if (flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] >= 4 && (hours == 18 || hours == 19)) {
				scyllaAction = SCYLLA_ACTION_ADICTS_ANON;
				return;
			}
			if (flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] >= 2 && flags["COC.FED_SCYLLA_TODAY"] != days && hours >= 7 && hours <= 11) {
	...
	trace("Scylla state: " + flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] + ", action: " + scyllaAction);
		...
		if (!urtaAtBar() || flags["COC.URTA_ANGRY_AT_PC_COUNTDOWN"] > 0) return;
		if (flags["COC.URTA_TIME_SINCE_LAST_CAME"] > timeAsStamp || int(flags["COC.URTA_COMFORTABLE_WITH_OWN_BODY"]) <= 2 || int(flags["COC.TIMES_FUCKED_URTA"]) == 0) return;
			//She only fucks Scylla if she's horny and you've fucked her enough to make her comfortable
		if (int(flags["COC.UNKNOWN_FLAG_NUMBER_00143"]) == 0) { //Never fucked Scylla before
			if (!urtaDrunk()) return; //So she has to be drunk
		}
		else if (urtaDrunk() && pc.balls == 0) return; //Otherwise she has to be sober and you need to have balls (I'm not sure why, but it is so)
		...
		if (rand(2) == 0 && flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] >= 3 && pc.hasKeyItem("Opal Ring") < 0 && urtaAtBar() && pc.longestCockLength() >= 8 && flags["COC.UNKNOWN_FLAG_NUMBER_00143"] > 0) scyllaAction = SCYLLA_ACTION_FURRY_FOURSOME;
	}
}
...
private function scyllaIIIBellyChat():void {
	showScylla();
	clearOutput();
	output("Off-handedly, you ask how her thirst has been. She seemed to be, ahem, retaining a lot of fluid when you last left her, but now...\n\n");

	output("Scylla's stomach moans audibly and she blushes, putting a hand over it. \"<i>Ah ha, yes. It has been a trial. Thankfully, awfully kind people like yourself have been helping me when the hunger becomes unbearable. I can't really stomach normal food or drink anymore, but it's okay.</i>\" She bites the side of her plump lip and glances aside. \"<i>Besides... I really... like... the taste,</i>\" she whispers, half to herself.\n\n");

	output("The nun shakes her head. \"<i>But it hasn't been the same lately. The thirst comes back almost immediately. I can't keep it in my body long enough before digestion kicks in.</i>\"\n\n");

	output("You look at her closer and you think you can guess where all the extra protein has been going lately.  Her always lustrous hair is practically alive, sparkling twinkles of light reflecting off her long, jet curls. Her skin looks even softer than before, flush and creamy with an ageless smoothness. A thin but sturdy layer of muscle has taken definition under her buxom flesh, making her back straighter and her lean arms more graceful.\n\n");

	output("Knowing you're unlikely to discover any more on that topic, you turn the subject to her breathing...");

	processTime(2);
	clearMenu();
	addButton(0, "Next", scyllaIIIHOWZUBREETH);
}
...
private function scyllaLevelIIIRoundIIIFIGHT():void {
	...
	pc.cor(2);
	pc.cumMultiplierRaw += 2;
	if (pc.balls > 0) pc.ballSizeRaw += 2;
	clearMenu();
	addButton(0, "Next", scyllaIIIFinisher);
}

private function scyllaIIIFinisher():void {
	showScylla();
	clearOutput();
	output("When you finally rouse from your slumber, you understand why all the guys downstairs in the Wet Bitch looked like they'd been sucked dry. Your body is sore and you feel like you haven't had anything to drink in months. Scylla is next to you, plump and happy. She's managed to replace her irredeemably stained robe with a new, larger one, but it too is on the verge of tearing. Though her digestion seems to have handled most of your cum, her breasts have kept some of their weight, each as large as a basketball. Her belly has the slightest paunch, but apparently this is enough to sate her thirst because she gives you an affectionate pat on the shoulder and a smile so bright that the candles in the room seem dim.\n\n");

	output("\"<i>Thank you so much,</i>\" she gushes. \"<i>Once again, you come to my rescue and teach me how to overcome my trials. You truly are my guardian angel. One of these days, I hope I'll be able to return the favor.</i>\" She favors you with a moist kiss on the forehead and springs from bed, energized and ready to do some good. You groan, roll over, and go back to sleep.");
...
private function scyllaWatchThirdPartOhShitSon():void {
	...
	//Go to epilogue
	clearMenu();
	addButton(0, "Next", scyllaPtIVEpilogue);
}
...
private function scyllaPtIVKissPtII():void {
	...
	var x:Number = pc.biggestCockIndex();
	clearOutput();
	output("When you break your second kiss, tummy full of Scylla's cream, you find yourself entrapped by her mammoth chest. The stimulation of your nursing seems to have caused her tits to overcompensate, inflating with milk far in excess to what your small stomach could drain. You are literally encased between her breasts, the pliant but firm flesh engulfing everything from your shoulders down in her snug cleavage. You shift uncomfortably for a moment, until you realize not all of your body has shrunk. You shift, no longer able to resist your erection's pressure, and shed your [pc.gear]. With your shrunken body's child-like dimensions, [pc.eachCock] is virtually as thick as your torso. The weight would knock you flat on your back if Scylla's tits weren't wrapped around you in their tight embrace.  [pc.EachCock] strains just under her nose, pulsing at the tightness around you.\n\n");

	output("Scylla strokes [pc.eachCock] with her palms and cheeks, reveling in the heat of your panting chest between her tits. \"<i>We still have to get that nasty poison out of your precious body, little one. Don't worry; let Scylla kiss it all better.</i>\" She purses her lips and plants kisses from shaft to head, pausing at your crown. Blushing, she lowers her mouth to the shaft again and pokes out her tongue.\n\n");

	output("Scylla's tongue is as crimson as her lips and moist with the hungry drool in her mouth. She strokes your " + pc.cockDescript(x) + " with her delicately pointed tip, tracing the swell of your cock-head bashfully. She smiles as she opens her mouth broadly and stretches her tongue out further. You've felt it curl around your " + pc.cockDescript(x) + " before, but having not seen it until now, you never appreciated how long it must be to fit all the way around. At its base, the nun's tongue is three inches wide and a serpentine eight inches long. She giggles in delight at the look of shock on your adolescent face. Scylla dips her tongue into her cleavage and draws it up along your [pc.cocks], moving in long, slow motions, soft, wet muscle curling around you and drinking in the flavor of your sex. She presses her arms on either side of her love-pillows as she laps at your " + pc.cockDescript(x) + ", and her tits squeeze [pc.eachCock] with liquid tightness. Her tongue wraps around your cock, spiraling up and around, corkscrewing your head as the pressure from her mountainous breasts redoubles and squishes the breath from your chest.\n\n");

	output("You begin to feel light headed from the blood trapped in [pc.eachCock] just as she finally releases the squeezing embrace. [pc.EachCock] can take no more; your orgasm robs your muscles of control, your whole body shivers against Scylla's chest.  She hungrily coaxes your cum upward with her coiled tongue, and places a dainty kiss on your tip just as it bursts from you. She drinks load after load, yummy noises gurgling from her throat while her tongue laps up the leaking sperm that runs in thick rivulets from her mouth.\n\n");
	//[2+ dicks only] -
	if (pc.totalCocks() > 1) {
		if (pc.totalCocks() == 2) output("Your other cock doesn't have the nun's eager mouth tending to it, but the pressure of her colossal tit fucking coaxes it to cum all the same. Jizz fountains in hot streams of white lust that splatter across the nun's blissful face and into her coal-dark hair. Gobs of it roll across her engorged tits, soaking you in the press of her cleavage as it rolls inward and leaks goopy tendrils that paint her alabaster skin whiter still. She runs her hands over the cum, gathering it between her fingers and massaging it into her flesh, every sensation drunk from the feeling of your sticky orgasm on her body.\n\n");
		else output("Your other cocks don't have the nun's eager mouth tending to them, but the pressure of her colossal tit fucking coaxes it to cum all the same. Jizz fountains in hot streams of white lust that splatter across the nun's blissful face and into her coal-dark hair. Gobs of it roll across her engorged tits, soaking you in the press of her cleavage as it rolls inward and leaks goopy tendrils that paint her alabaster skin. She runs her hands over the [pc.cum], gathering it between her fingers and massaging it into her flesh, every sensation drunk from the feeling of your [pc.cumVisc] orgasm on her body.\n\n");
	}
	...
	//[Next]
	clearMenu();
	addButton(0, "Next", scyllaPtIVKissPtIII);
}
...
private function scyllaPtIVKissPtIII():void {
	...
	clearOutput();
	var x:Number = pc.biggestCockIndex();
	output("Scrambling up and out of your titty cocoon, you pull your tiny body onto the top of her jiggling mounds. Scylla laughs at the feeling of having you crawl over her body, and watches your progress with amusement as she lazily slurps down your slowing loads. With an expression of ecstatic agony, you buck your hips in an attempt to fuck the nun's face without any leverage. The cum on her body makes your perch slippery, however, and you begin to slide backward before she catches you with sure hands and draws you closer. Each time you thrust, she pulls you a little closer, a few more inches of your " + pc.cockDescript(x) + " jamming into her mouth. In your state, your strength is nothing next to hers, but she plays along, making surprised gasps and moans as you \"ram\" yourself deeper. When you get close enough for your short arms to reach her head, you grab onto her palm-sized horn nubs. She gives you complete control of her head as you jerk her back and forth, lips, mouth, and throat sliding along your length.\n\n");

	if (pc.balls > 0) {
		output("Scylla's long, winding tongue seeks out your " + pc.sackDescript() + " and licks it ravenously, rolling each testicle around to reignite your flagging orgasm. She looks up and meets your eyes. Drinking in your adolescent expression of helpless pleasure, she blushes a vivid pink, shivering in her own orgasm. She can't stop herself from letting her tongue travel up your taint to your " + pc.assholeDescript() + ". You gasp in juvenile surprise as she digs her fingers into her nipple lips, masturbating furiously at your cute expression. Without preamble, she thrusts her tongue into your ass and presses against your immature prostate, making you squeak in another helpless orgasm. You pull weakly at her horns and she complies hungrily, swallowing the rest of your " + pc.cockDescript(x) + " in one gulp just in time for your renewed balls to flood her throat with your seed. You tremble uncontrollably, eyes clamped shut, sitting atop Scylla's tits with her head buried in your crotch.  Her arms curl around your back, hugging you as she drinks deeply. As the nun's thirst drains you, a cold sensation works its way through your ball sack and down your cock. When it finally bursts into her stomach, you gasp as blazing warmth fills you.\n\n");
		output("Your [pc.balls] pump out the last of your cum, and Scylla gulps it happily before she puts her hands under each of your arms and pulls you up, [pc.eachCock] flopping sloppily against her face. She's smiling with puffed-out cheeks when she sets you down gently on the bed and strokes your young face with her fingertips. She winks at you and stoops down to the unconscious imp. She pins his mouth open with her forefinger and thumb, and she purses her bloated lips into a narrow opening. A thick stream of your cum slowly drips down toward the imp Lord's waiting gullet.  In the cum you can see thin streaks of a greenish fluid which you realize must be the venom he'd injected you with!  Indeed, you begin to feel pinpricks along your fingertips and toes as your strength ebbs back into your limbs. As you gradually start to regain your size, the agonizingly long stream of cum from Scylla's mouth reaches the imp's own. He begins to choke as it touches his throat, and Scylla swiftly lunges forward to plant a kiss on the demon, holding his mouth open with hers. The nun's cheeks empty, and the imp swallows in reflex at the flood filling his throat. Scylla breaks the kiss with a loud, wet, \"<i>Muah!</i>\" and grins mischievously at you. The imp coughs and gags, but the cum-coated poison swiftly courses through his system and he shrinks on the spot. He diminishes from four feet down to one, his aura of infernal power broken as his strength is siphoned away. Scylla picks him up like a doll and thrusts him into her cleavage for safe-keeping.  His struggles are so pathetic that they barely make her love-pillows jiggle at all.");
	}
	else {
		output("Scylla's long, winding tongue seeks out the base of your shaft and licks it ravenously, pumping your rod to reignite your flagging orgasm. She looks up and meets your eyes. Drinking in your adolescent expression of helpless pleasure, she blushes a vivid pink, shivering in her own orgasm. She can't stop herself from letting her tongue travel up your taint to your " + pc.assholeDescript() + ". You gasp in juvenile surprise as she digs her fingers into her nipple lips, masturbating furiously at your cute expression. Without preamble, she thrusts her tongue into your ass and presses against your immature prostate, making you squeak in another helpless orgasm. You pull weakly at her horns and she complies hungrily, swallowing the rest of your " + pc.cockDescript(x) + " in one gulp just in time for you to flood her throat with your seed. You tremble uncontrollably, eyes clamped shut, sitting atop Scylla's tits with her head buried in your crotch.  Her arms curl around your back, hugging you as she drinks deeply. As the nun's thirst drains you, a cold sensation works its way through your cock. When it finally bursts into her stomach, you gasp as blazing warmth fills you.\n\n");
		output("Your loins pump out the last of your cum, and Scylla gulps it happily before she puts her hands under each of your arms and pulls you up, [pc.eachCock] flopping sloppily against her face. She's smiling with puffed-out cheeks when she sets you down gently on the bed and strokes your young face with her fingertips. She winks at you and stoops down to the unconscious imp. She pins his mouth open with her forefinger and thumb, and she purses her bloated lips into a narrow opening. A thick stream of your cum slowly drips down toward the imp Lord's waiting gullet.  In the cum you can see thin streaks of a greenish fluid which you realize must be the venom he'd injected you with!  Indeed, you begin to feel pinpricks along your fingertips and toes as your strength ebbs back into your limbs. As you gradually start to regain your size, the agonizingly long stream of cum from Scylla's mouth reaches the imp's own. He begins to choke as it touches his throat, and Scylla swiftly lunges forward to plant a kiss on the demon, holding his mouth open with hers. The nun's cheeks empty, and the imp swallows in reflex at the flood filling his throat. Scylla breaks the kiss with a loud, wet, \"<i>Muah!</i>\" and grins mischievously at you. The imp coughs and gags, but the cum-coated poison swiftly courses through his system and he shrinks on the spot. He diminishes from four feet down to one, his aura of infernal power broken as his strength is siphoned away. Scylla picks him up like a doll and thrusts him into her cleavage for safe-keeping.  His struggles are so pathetic that they barely make her love-pillows jiggle at all.");
	}
...
public function scyllaAdictsAnonV():void {
	...
	clearMenu();
	addButton(0, "Next", function():*{ processTime(10 + rand(10)); mainGameMenu(); } );
	if (int(flags["COC.TIMES_MET_SCYLLA_IN_ADDICTION_GROUP"]) == 0) scyllaPtVFirstTime();
	else scyllaPtVRepeat();
	//Set scylla progression if not set yet!
	if (flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] < 5) flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] = 5;
}
...
private function scyllaPtVFirstTime():void {
	...
	//(First time)
	clearOutput();
	//output(images.showImage("scylla-help-round-five-firt-time"), false);
	output("You spot Scylla across the common room of the Wet Bitch, but she seems preoccupied. The nun is fetching a tray of assorted drinks and shots from the bar, which strikes you as strange since you haven't seen her order anything here that wasn't drowned in milk and cream. You slink back into the crowd and keep a curious eye on her. After marshalling the drinks, she hefts the tray and very carefully takes it upstairs.  She bumps one of the doors open with her plump butt and walks inside, before giving the door another rump bump to close it.  You quickly creep forward and hook a finger into the frame, keeping the door open an inch.\n\n");
	...
	output("You almost don't notice the third girl until what you'd assumed to be a very finely crafted salt shaker moves across the table. It's a fairy, robed in crystal that sparkles on her pale skin like morning dew. Her movements, however, are leaden, her gossamer wings drag limply behind her. She stumbles to a shot glass, and with one hand rubbing her temple, she leans down to takes a drink from a glass larger than her head. She groans and plops down, rubbing her eyes regretfully.\n\n");

	output("Scylla's soft enthusiasm seems unfazed by her companions' dour attitudes. She takes a creamy liquor and sips it before starting. \"<i>I'm so very glad everyone could make it today. I am excited to begin, but perhaps it would be best to do some introductions? If it's all right with everyone, I will go first.  My name is Scylla and I am a nun in the service of the gods. I grew up very far away, but I live in Tel'Adre now, between my missionary expeditions.</i>\" She nods at the fairy and smiles so wide that she actually squeaks.\n\n");

	output("The fairy shakes her head slowly and rubs a spot between her eyes. \"<i>Not so loud. My name's... ugh... Pastie. I'm from the forest and I really need to be sleeping off this hangover,</i>\" she mutters, her voice thin and high, like wind across reeds.\n\n");

	output("Scylla turns her gaze to the goblin, who snorts and begins chewing on her toothpick. \"<i>What's it matter what my name is, I'm always just gonna be one of 'Tamani's daughters,' the dumb slut.</i>\" Her small, pointed nose makes the goblin's voice nasally and her habit of talking through sharp teeth gives her a coarse accent.\n\n");

	output("The nun puts a hand over her bulbous lips. \"<i>Oh dear, please don't speak of your mother that way! I am sure she loves you very much. We care deeply about you here; this is a safe place.  Won't you please share your name with the group?</i>\" She leans forward, her eyelids fluttering hopefully.\n\n");

	output("\"<i>Bleh, enough with tha 'sisterhood' crap,</i>\" the goblin responds, waving a hand dismissively in front of Scylla's face. \"<i>Fine, ya fat tub, I'm Abylon. But call me Abby or I'm outta here.</i>\" She kicks the air with her suspended leg and grabs another beer.");

	processTime(12);
	clearMenu();
	addButton(0, "Next", scyllaPtVFirstTimeII);
}
...
private function scyllaPtVRepeat():void {
	showScylla();
	clearOutput();
	output("Scylla's at the bar, filling up another large tray of drinks. Judging by how many she's got, you assume she's giving the addiction counseling meeting another shot. The nun is murmuring to herself, possibly rehearsing a sermon or maybe just working up the strength to be the pillar the other girls need. She heads upstairs and you ponder what to do as you watch her go. Will you join them?\n\n");
	processTime(2);
	//[Share][Take Advantage][Leave]
	clearMenu();
	addButton(0, "Share", scyllaPtVShare);
...
private function scyllaPtVTakeAdvantage():void {
	showScylla();
	IncrementFlag("COC.TIMES_MET_SCYLLA_IN_ADDICTION_GROUP");
	IncrementFlag("COC.TIMES_SCYLLA_ADDICT_GROUP_EXPLOITED");
	if (pc.hasStatusEffect("Exgartuan") && pc.statusEffectv2("Exgartuan") == 0) {
	...
	clearOutput();
	//output(images.showImage("scylla-help-round-five-take-advantage"), false);
	//[Corruption up]
	pc.cor(1);
	if (pc.cor() < 30) pc.cor(2);
	if (pc.cor() < 60) pc.cor(1);

	var x:Number = pc.biggestCockIndex();

	output("You decide this is too good to pass up, so you step away and duck into an empty room. You whip out your " + pc.cockDescript(x) + " and begin masturbating, careful to stop before you cum. You take a dollop of pre from the tip and dab it behind your ears, to make sure the musky scent will be obvious. Then, you slip back into your clothes and adjust your package to make the bulge unmistakable. Suitably prepared, you go to the ladies' door and knock crisply.\n\n");

	if (pc.balls > 0) {
		output("Scylla opens the door curiously and smiles when she sees you. \"<i>Oh, " + pc.short + ", what a pleasant surprise! I'm sorry, but do you mind waiting a bit? I'd love to talk with you, but I'm in the middle of a... the middle of...</i>\" she trails off, the smell of your pre-cum floating into her nostrils and starting to shut off the nun's higher brain functions. Her eyes drop to your crotch and her stomach rumbles loudly as she stares at the outline of your " + pc.cockDescript(x) + "  and [pc.balls]. You grin and gently push her backwards, stepping into the room. Scylla follows after like a lost puppy, licking her lips without realizing it. \"<i>This... um, this is " + pc.short + ", a friend of, uh, mine,</i>\" she explains, stumbling through the introduction. Abylon regards you distrustfully but she bites her lips and rubs her legs together through her armor when your musk hits her. Pastie is too drunk to notice the sudden pressure in the room.\n\n");
		output("No sense in playing coy when you're the drug in a room full of addicts. You brazenly whip out your " + pc.cockDescript(x) + " and spread your arms. \"<i>Ladies,</i>\" you grin, in invitation. The nun shakes her head, trying to clear her mind and failing, giggling involuntarily. The goblin trembles, her fists balled into pale knuckles, but then she slumps, defeated.  \"<i>Can't fight nature,</i>\" she grumbles, weakly. \"<i>Like mother, like daughter.</i>\" Pastie hiccups, trying to figure out what's happening.\n\n");
		output("Scylla reaches out for your shaft and wraps long, thin fingers around it, almost worshipfully. This provokes an indignant cry from Abby, who crosses the distance between you faster than you would've believed. \"<i>Hands off, ya big bitch. That's my baby-batter.</i>\" She grabs your [pc.balls] possessively, a little harder than you'd like. \"<i>You'll just waste it in yer fat mouth anyway. Dumb humans don't even know where ta take a cream pie- it's amazin' ya ever get knocked up!</i>\"\n\n");
		...
		output("\"<i>Like fuck yer goin' first,</i>\" Abby shoots back, her grip on your balls so painful now that you hiss through your teeth. The goblin grabs a beer bottle and smashes it on the table, brandishing it at Scylla. \"<i>Go ahead, cunt, put those bimbo cock-sucking lips on my baby-daddy's dick. See what happens.</i>\" The jagged glass is perilously close to your shaft, and you don't dare to move while the two have a tug of war with your junk. Maybe this wasn't the best idea you've ever had.\n\n");
		output("Pastie finally figures out what's going on through her stupor and flies between the three of you, settling atop your " + pc.cockDescript(x) + ", tiny stiletto heels digging into soft flesh like pins. \"<i>Don't fight, you two, this thing's gross,</i>\" she argues. Pouting, she stomps her foot down and the heel punches into your cockhead, drawing a scream from you that only makes Scylla and Abby tug all the harder.\n\n");
		output("\"<i>Abylon, please,</i>\" Scylla starts, but the use of her given name infuriates the goblin girl, and she hurls the broken bottle at the nun, who ducks. It shatters on the wall, sending glass shards flying, several of them cutting your cock and giving you a half-dozen scrapes. The goblin releases your scrotum and dives at the nun, snarling. They roll in a tangled cat fight, leather and cloth shredding under addiction-fueled claws. You swat Pastie from your dick and run out of the room, gingerly tending to your tormented groin.\n\n");
	}
	else {
		output("Scylla opens the door curiously and smiles when she sees you. \"<i>Oh, " + pc.short + ", what a pleasant surprise! I'm sorry, but do you mind waiting a bit? I'd love to talk with you, but I'm in the middle of a... the middle of...</i>\" she trails off, the smell of your pre-cum floating into her nostrils and starting to shut off the nun's higher brain functions. Her eyes drop to your crotch and her stomach rumbles loudly as she stares at the outline of your " + pc.cockDescript(x) + ". You grin and gently push her backwards, stepping into the room. Scylla follows after like a lost puppy, licking her lips without realizing it. \"<i>This... um, this is " + pc.short + ", a friend of, uh, mine,</i>\" she explains, stumbling through the introduction. Abylon regards you distrustfully but she bites her lips and rubs her legs together through her armor when your musk hits her. Pastie is too drunk to notice the sudden pressure in the room.\n\n");
		output("No sense in playing coy when you're the drug in a room full of addicts. You brazenly whip out your " + pc.cockDescript(x) + " and spread your arms. \"<i>Ladies,</i>\" you grin, in invitation. The nun shakes her head, trying to clear her mind and failing, giggling involuntarily. The goblin trembles, her fists balled into pale knuckles, but then she slumps, defeated.  \"<i>Can't fight nature,</i>\" she grumbles, weakly. \"<i>Like mother, like daughter.</i>\" Pastie hiccups, trying to figure out what's happening.\n\n");
		output("Scylla reaches out for your shaft almost worshipfully with her long, thin fingers. This provokes an indignant cry from Abby, who crosses the distance between you faster than you would've believed. \"<i>Hands off, ya big bitch. That's my baby-batter.</i>\" She grabs your " + pc.cockDescript(x) + " possessively, a little harder than you'd like. \"<i>You'll just waste it in yer fat mouth anyway. Dumb humans don't even know where ta take a cream pie- it's amazin' ya ever get knocked up!</i>\"\n\n");
		output("\"<i>You don't understand, I need this,</i>\" the nun pleads, her hands tightening on your cock, nails starting to dig in as she jerks you, groin-first closer to her. \"<i>You can have some after, I promise, but I need it in my mouth. I'm just... so... hungry.</i>\" Under her robes, her nipples bulge, lip-shaped against the black velvet.\n\n");
		output("\"<i>Like fuck yer goin' first,</i>\" Abby shoots back, her grip on your base so painful now that you hiss through your teeth. The goblin grabs a beer bottle and smashes it on the table, brandishing it at Scylla. \"<i>Go ahead, cunt, put those bimbo cock-sucking lips on my baby-daddy's dick. See what happens.</i>\" The jagged glass is perilously close to your shaft, and you don't dare to move while the two have a tug of war with your junk. Maybe this wasn't the best idea you've ever had.\n\n");
		output("Pastie finally figures out what's going on through her stupor and flies between the three of you, settling atop your " + pc.cockDescript(x) + ", tiny stiletto heels digging into soft flesh like pins. \"<i>Don't fight, you two, this thing's gross,</i>\" she argues. Pouting, she stomps her foot down and the heel punches into your cockhead, drawing a scream from you that only makes Scylla and Abby tug all the harder.\n\n");
	...
	processTime(15);
	applyDamage(new TypeCollection( { truehp : 5 } ), scylla, pc);
...
private function scyllaPtVShare():void {
	showScylla();
	clearOutput();
	//output(images.showImage("scylla-help-round-five-share"), false);
	//FIRST TIEM
	if (int(flags["COC.SCYLLA_TIMES_SHARED_IN_ADDICT_GROUP"]) == 0) {
		output("With a blush of embarrassment, you push the door open and step inside. \"<i>I'm " + pc.short + " and I'm an addict,</i>\" you admit, keeping your eyes cast down.\n\n");

		output("The girls look surprised by your entry, but Scylla is welcoming all the same. \"<i>Oh, " + pc.short + ", I'm so glad you could join us after all,</i>\" she fibs, covering up for the fact that you were obviously spying on them. \"<i>Pastie, Abby, this is my dear friend. Please, take a seat here,</i>\" she offers you her chair and daintily takes a place on the edge of the room's bed. \"<i>Please, tell us more about yourself.</i>\"\n\n");
	}
	//[Share]	(Repeat)
	else {
		output("You knock on the door and poke your head in. As they look up, you see an empty chair's been pulled up to the table this time. It seems they were expecting - or perhaps hoping - you'd come again. Abylon still wears her carefully crafted mask of mocking apathy, but you think you catch a glimmer in her eyes that even she's not fully aware of. She scratches the tip of her nose and tosses you a curt 'hey' gesture with her wrist. Pastie is already wasted, but only on liquor. Is it just your imagination, or are the fairy's crystalline clothes a little sluttier than they were last time? Scylla seems happy to see you, though there are little crinkles of need at the edges of her smile.\n\n");

		output("You resolve to be on your best behavior, mentally noting that your 'best' isn't what it used to be. What would you like to talk about today?\n\n");
	}
	//Increment times met share group
	IncrementFlag("COC.SCYLLA_TIMES_SHARED_IN_ADDICT_GROUP");
	//Increment overall meetings
	IncrementFlag("COC.TIMES_MET_SCYLLA_IN_ADDICTION_GROUP");
	output("What addiction would you like to discuss?");
	//Set choices
	
	if (flags["COC.MINOTAUR_CUM_ADDICTION_TRACKER"] >= 50 || pc.hasPerk("Minotaur Cum Addict") || pc.isDependant(Creature.DEPENDANT_CUM))
		addButton(0, "Cum", scyllaPtVCum);
	else addDisabledButton(0, "Cum");
	
	if ((pc.hasPerk("Marble's Milk") || pc.statusEffectv3("Marble") > 0) && !pc.hasPerk("Marble Resistant"))
		addButton(1, "Milk", scyllaPtVMilk);
	else addDisabledButton(1, "Milk");
	
	if (pc.slut() >= 85 || pc.lustMin() >= 20)
		addButton(2, "Sex", scyllaPtVSex);
	else addDisabledButton(2, "Sex");
	
	if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) >= 2)
		addButton(3, "Tentacles", shareTentaclesWithScylla);
	else addDisabledButton(3, "Tentacles");
...
private function scyllaPtVNoDiscussion():void {
	showScylla();
	clearOutput();
	output("You fail to come up with a suitable explanation for your presence and politely excuse yourself.  AWKWARD!");
	processTime(2);
	clearMenu();
	addButton(0, "Next", function():*{ processTime(10 + rand(10)); mainGameMenu(); } );
}
...
private function scyllaPtVCum():void {
	...
	processTime(8);
	clearMenu();
	addButton(0, "Next", scyllaPtVCumII);
}

private function scyllaPtVCumII():void {
	showScylla();
	clearOutput();
	//output(images.showImage("scylla-help-round-five-jizz-pt-two"), false);
	var x:Number = pc.biggestCockIndex();
...
private function scyllaPtVSexIII():void {
	...
	if (pc.balls > 0) {
		...
		output("Scylla releases the vacuum-like suction of her scarlet-lacquored lips and rises to nurse the last few loads out of you, suckling serenely, satisfied at the restraint you and Abby showed. The goblin merely laughs her ass off at Pastie's plastered form, writhing on the table, soaked inside and out, sloppy like a used condom. Abyon slaps the nun on the back and chuckles, \"<i>Gotta give it to ya, fatty, ya know a good time.</i>\" Scylla fills her cheeks with your final load and spins around, grabbing the goblin's shoulders and pressing a deep kiss onto her, bulbous lips sealing around the lower half of the green girl's face. Abby tries to speak and Scylla empties her cheeks, snowballing your cum and her saliva into the unsuspecting goblin's mouth. Surprised, she swallows reflexively and almost chokes on the volume of gooey cum that pours down her throat. The nun ends the kiss with a wet \"<i>muah!</i>\" and smiles.\n\n");
	}
	output("\"<i>Please stop calling me fat,</i>\" she asks, innocently. \"<i>You know, if that's all right with you.</i>\"\n\n");
	processTime(7);
	pc.orgasm();
	//[Next]
	clearMenu();
	addButton(0, "Next", scyllaPtVSexIV);
}

private function scyllaPtVSexIV():void {
	showScylla();
	clearOutput();
	//output(images.showImage("scylla-help-round-five-sex-pt-four"), false);
	output("When the four of you are ready to leave, Scylla gives you a big hug and thanks you for coming. Pastie, barely able to keep her overinflated form aloft, gives you a drunk kiss on the nose before slowly flying away. Even Abby gives you a friendly smack on the ass, rubbing her own tender rump gingerly. While your travels have guaranteed you'll always - and frequently - need a release, you don't think you're as much a slave to it as you used to be.");
	//[Libido Down]
	processTime(3);
	pc.slowStatGain("l", -1);
	clearMenu();
...
private function addictionAdvantageMultiDickGreatTimeParty():void {
	clearOutput();
	//output(images.showImage("scylla-help-round-five-multi-cock-one"), false);
	showScylla();
	//[First time]
	if (flags["COC.UNKNOWN_FLAG_NUMBER_00144"] == undefined) {
		flags["COC.UNKNOWN_FLAG_NUMBER_00144"] = 1;
		output("You decide this is too good to pass by, so you step up to the ladies' room and knock crisply. Scylla opens the door curiously and smiles when she sees you. \"<i>Oh, " + pc.short + ", what a pleasant surprise! We were just in the middle of a... the middle of...</i>\" she trails off, noticing the look of wicked relish on your face and you push the seven-foot girl aside, stepping inside confidently. Her eyes drop to your crotch and her stomach rumbles loudly as she stares at the over-stuffed mass of cocks straining against your [pc.gear]. Licking her lips without realizing it, the nun keeps trying to stay focused. \"<i>This... um... what was I...</i>?\" she mutters, unable to keep her thoughts together. Abylon regards you distrustfully but she bites her lips and rubs her legs together through her armor when your musk hits her. Pastie is too drunk to notice the sudden pressure in the room.\n\n");
...
private function addictionScyllaTakeAdvantageDicksBonusAndEpilogue():void {
	...
	if (select == 6) {
		...
		var type:int = Mutator.guessCockType(pc);
		if (type == GLOBAL.TYPE_DEMONIC) type = GLOBAL.TYPE_HUMAN;
		var temp:int = pc.cocks.length;
		while(temp > 0) {
			temp--;
			if (pc.cocks[temp].cType == GLOBAL.TYPE_DEMONIC) {
				pc.shiftCock(temp, type);
				pc.cor( -3);
				break
			}
		}
...
private function Scylla6YellForHelp():void {
	...
	processTime(5);
	clearMenu();
	addButton(0, "Truth", Scylla6AfterYellTruth);
	addButton(1, "Lie", Scylla6AfterYellLie);
}
...
private function Scylla6AfterYellTruth():void {
	showScylla();
	clearOutput();
	output("\"<i>These drifters attacked us in broad daylight,</i>\" you spit in disgust. \"<i>Lock them up, banish them from the city, do whatever it is you do. I never want to see these furry freaks again.</i>\"\n\n");
...
private function Scylla6NoMilkGetHelp():void {
	showScylla();
	clearOutput();
	output("You fill your lungs and scream for help as loudly as you can. The cats around Scylla hiss and leap to their feet, trying to find you, cursing. A tall, black-furred girl tries to flee down the alleyway, but the damage is done. The clattering rush of the city guard surrounds the group from both directions and the cats find themselves trapped. A few try to bolt between the guards, but a well-placed centaur kick knocks one into an unconscious slump and the others quail from the guards in a scared huddle.\n\n");
	...
	processTime(4);
	clearMenu();
	addButton(0, "Truth", Scylla6NoMilkTruth);
	addButton(1, "Lie", Scylla6NoMilkLie);
}
...
private function Scylla6NoMilkSubmitII():void {
	...
	//[Next]
	clearMenu();
	addButton(0, "Next", Scylla6NoMilkSubmitIII);
}
...
private function Scylla6NoMilkSubmitIII():void {
	...
	pc.orgasm();
	clearMenu();
	addButton(0, "Next", Scylla6NoMilkSubmitIV);
}
...
private function Scylla6NoMilkSubmitIV():void {
	...
	flags["COC.KATHERINE_MET_SCYLLA"] = 1;
	pc.createKeyItem("Silver Kitty-Bell", 0, 0, 0, 0);
	pc.credits += 500;
	processTime(5);
	clearMenu();
	addButton(0, "Next", function():*{ processTime(10 + rand(10)); mainGameMenu(); } );
...
private function Scylla6NoMilkRAPETHEWORLD():void {
	//spriteSelect(59);
	clearOutput();
	output("You decide to have a little fun with this situation, so you remain hidden around the corner while the cats mill around Scylla. It's not long before their thirst overwhelms their caution and the felines sink to their knees, nuzzling Scylla's bare chest, small tongues lacquering her mountainous breasts in saliva. Her skin shimmers in the alley's waning light and the nun moans, struggling weakly against her bonds, her milk-laden tits jiggling tantalizingly. You strip off your [pc.gear] and lightly grasp your cock, stroking slowly as you wait for the opportune moment.\n\n");
...
private function shareTentaclesWithScylla():void {
	...
	//[Cocks under 5' long:
	if (pc.longestCockLength() < 60) {
		output("\n\nScylla meets your gaze, her pure, sapphire eyes peering through her long, elegant lashes.  She opens her mouth once more to speak, perhaps to try to dissuade you from giving into temptation, perhaps to ask if she could be first.  The words die on her lips as the seething thirst inside her boils up, dulling the clarity of her eyes with a decidedly impure hunger.  She whispers a prayer and you can feel your body reacting to the nun's lustful desire, her corrupted supplications flowing across your flesh like sanctified oil.  [pc.EachCock] pulse as blood surges through their lengths, growing longer and thicker with every heartbeat.  They expand and swell until each is at least five feet long, resembling true tentacles more than the simple cocks they once were.  You marvel at the famished depravity of the holy woman, but you must admit, her charity seems boundless.");
		//[Next]
	}
	processTime(4);
	clearMenu();
...
private function shareTentaclesWithScylla2():void {
	...
	//[Six cocks:
	if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) >= 6) output("  Another encircles Pastie, thick flesh lifting the limp fairy off the carpet while a sixth rubs the underside of its crest against her squirming face.");
	...
	//[Next]
	//dynStats("lib", .5, "lus=", 100, "cor", .25, "resisted");
...
private function shareTentaclesWithScylla3():void {
	clearOutput();
	//output(images.showImage("scylla-share-tentacle-addiction-three"), false);
	output("With your members in place, you take a moment to drink in the sensations pulsing through your flesh.  Scylla appears almost relieved as her crimson puckers wrap around the peaks of your undulating tentacles, their lengths coiled around her statuesque limbs and squeezing her inflated breasts a little tighter with every inch she sucks down.");
	//[four cocks:
	if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) >= 4) output("  Abylon's hips roll as she struggles for breath, transfixed atop your shafts, stiletto-heeled boots uselessly kicking air while her mouth works silently, speechless at the strength of your sinuous poles.");
	//[Six cocks:
	if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) >= 6) output("  Pastie, wriggling in her twin sheathes, shudders with ecstatic bliss, her every sensation overwhelmed by the pressing confines of your voracious cocks.");
	output("  Just getting your writhing host into place has ignited the fuse of your crescendo, globs of pre-cum leaking down every shaft in anticipation.  Flesh throbbing, you curl a lip and abandon all restraint.");
	...
	processTime(9);
	clearMenu();
	addButton(0,"Next",shareTentaclesWithScylla4);
}

//[Next]
private function shareTentaclesWithScylla4():void {
	clearOutput();
	output("As your orgasm wanes and you are able to catch your breath, the firmness gradually ebbs from your tentacle dicks.  Uncoiling them, your excess length shrinks back into your loins, restoring you, more or less, to normal.  Scylla slumps against the side of the bed, her eyes closed in a private communion, beatific face plastered and dripping with your seed.");
	//[four cocks:
	if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) >= 4) output("  Lowering the goblin girl into the nun's pillowy cleavage, she tries to make a snide remark, but can't find the strength and simply snuggles into the ample flesh of Scylla's inflated bosom.  Thick, creamy milk leaks from the nun's well-nourished nipple-lips, warm drool glazing Abby's beryl skin.");
	//[six cocks:
	if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) >= 6) output("  Scooping Pastie from the ground, you rest her atop the room's table, her body wobbling like an overfilled water balloon.  It's just as well that she's filled with a half gallon of spunk – she's far too drunk and high to fly without hurting herself.");
	output("  Rather than linger, you take your leave, the restless heat of your indulgence lingering in the air as you close the door, avoiding the curious glances of tavern patrons on the way out.");
	if (pc.maxCum() < 28000) pc.cumMultiplierRaw += 2;
...
private function feedScyllaSomeJizzDatJunkieNeedsIt():void {
	...
	if (flags["COC.TIMES_SOLO_FED_NUN"] == 1) {
		...
		//[balls:
		if (pc.balls > 0) output("  Your scrotum inflates ever larger until the pressure of your [pc.gear] becomes unbearable.");
		output("  You hastily strip and guide the pliant, hunger-dazed woman to the bed.  She proves to be further gone than you would've guessed, sinking down sluggishly and laying on her back with a hot, breathless sigh.  Her head droops over the edge of the bed, a dreamy smile playing across her bimbo-thick lips as voluminous ebony curls spill from her habit.  Your abdomen clenches in icy tightness as your loins thicken with a need nearly as great as the one dominating the overwrought maiden on the bed.");
		if (pc.balls > 0) output("  The orbs swinging beneath your [pc.cocks] continue their growth, threatening to immobilize you if you don't find release quickly.  Every motion sends your spunk-laden reservoirs sloshing and wobbling, the ponderous ballsac now hanging mere inches from the floor.");

		output("\n\nStanding at the edge of the bed, you lift your [pc.cock " + pc.biggestCockIndex() + "] to the level of the virgin's head, holding your swollen tip inches from her panting mouth.  For the nun, everything has vanished but your turgid shaft and the creamy promise within.  With a moan of yearning desire, she circles her whorish pucker into a wanton O and presses the pliant, rosy flesh at the tip of your flushed crest.  The shameless prioress' obscene kiss sends tendrils of muscle-clenching bliss through your loins, and you can't help but thrust forward, your eager motion parting her garnet lips and slipping your pulsing cockhead into the virgin's needy mouth.  With voracious relish, she begins suckling at your [pc.cock " + pc.biggestCockIndex() + "] with a hungering devotion that borders on worship.  Her tongue swirls over and under your sloping shaft, licking and stroking every sensitive nerve in your trembling flesh.  Her cheeks go concave as she slurps greedily your long inches, her kiss reaching further, drawing more of your shaft into her.  You rock back and forth, letting the reclining maiden polish your receptive tip until a dangerous tingle thrills down your spine, warning you of the tensing in your loins.");
		//[balls:]
		if (pc.balls > 0) output("  The churning burden heaving in your testes has grown large enough to put you off-balance, forcing you to adjust your stance to accommodate their barrel-sized girth.");
		...
		//[Feed] (repeatable)
		output("Suggesting the two of you retire to her room, she leads you up the stairs, closing the door behind you.  As the door clicks shut, you embrace the nun with a kiss, the sweet warmth of her pliant lips echoing through your bones as a tightness forms in your [pc.gear].  The embrace turns impassioned, your tongue sweeping into her eager mouth as the swelling force of her silent prayers creep into your flesh.  Her long, mischievous tongue curls around you while a cold tickle of liquid weight swells in your abdomen.");
		//[Balls:
		if (pc.balls > 0) output("  The seething sensation in your loins begins to empty into your [pc.balls], your cum factories lurching to life with a heavy, sinking sensation.  Your [pc.gear] bulge as they grow, filling with your potent seed.");
		...
		//[balls:
		if (pc.balls > 0) output("  Your scrotum inflates ever larger until the pressure of your [pc.gear] becomes delicious.");
		output("  She helps you strip and guides you to her bed with a coy smile.  Sinking down eagerly, she lays on her back with a hot, breathless sigh, just as before.  Her head droops over the edge of the bed, a dreamy smile playing across her bimbo-thick lips as voluminous ebony curls spill from her habit.  Your abdomen clenches in icy tightness as your loins thicken with a need nearly as great as her own.");
		if (pc.balls > 0) output("  The orbs swinging beneath your [pc.cocks] continue their growth, every motion sending your spunk-laden reservoirs sloshing and wobbling.");

		output("\n\nStanding at the edge of the bed, you lift your [pc.cock " + pc.biggestCockIndex() + "] to the level of the virgin's head, holding your swollen tip inches from her panting mouth.  For the nun, everything has vanished but you and her, alone together in a moment that stretches on forever.  With a yearning moan, she circles her whorish pucker into a wanton O and presses the pliant, rosy flesh at the tip of your flushed crest.  The shameless prioress' obscene kiss sends tendrils of muscle-clenching bliss through your loins.  Eagerly, you thrust forward, parting her garnet lips and slipping your pulsing cockhead into the virgin's needy mouth.  With voracious relish, she begins suckling at your [pc.cock " + pc.biggestCockIndex() + "] with a hungering devotion that borders on worship.  Her tongue swirls over and under your sloping shaft, licking and stroking every sensitive nerve in your trembling flesh.  You rock back and forth, letting the reclining maiden polish your receptive tip until a dangerous tingle thrills down your spine, warning you of the tensing in your loins.");
		if (pc.balls > 0) output("  The churning burden heaving in your testes has grown large enough to put you off-balance, forcing you to adjust your stance to accommodate their swelling girth.");
...
private function feedingScyllaCumStepTwo():void {
	clearOutput();
	//output(images.showImage("scylla-feed-jizz-solo-two"), false);
	showScylla();
	if (flags["COC.TIMES_SOLO_FED_NUN"] == 1) {
		output("Like a punch to the stomach, the nun's virile magic pours into you, a flux of hot and cold stimulating your prostate into an intoxicating overdrive.  ");
		//[balls:
		if (pc.balls > 0) output("The pressure on your corpulent balls is suffocating, the goblin-sized globes hanging from you literally vibrating under the tension of your imminent orgasm.  ");
		output("To avoid further waste, you begin sliding your meat deeper into the nun's mouth, feeding inch after inch until you hit the back of her throat and push further.  Reaching out, you grab the sides of the nun's head, tilting her jaw back to ease the relentless journey of your tremulous hydrant.  Her throat clenches down around you in muscled pulses, rings of tension milking your [pc.cock " + pc.biggestCockIndex() + "] with every gulping gasp the tainted nun takes.  Her neck visibly bulges as you grit your teeth and force your girth deeper into her welcoming body.");
		...
		output("\n\nThe deluge of your semen pours forth in globular torrents that stretch Scylla's jaw to its limits, bloated swells of cum distending your [pc.cock " + pc.biggestCockIndex() + "], while the prioress' throat fattens obscenely around your widening girth.  Pumping load after load directly into her tainted stomach, you are rewarded with the sight of her gut thickening into an expanding paunch.  With every fresh wave of cascading spunk, the nun's belly enlarges, her lean frame swallowed under the corpulent bulk of her cum-swollen heft.");
		//[Balls:
		if (pc.balls > 0) output("  Discharging their pressurized bounty in an unbound release, your over-inflated balls finally cease their incapacitating growth and begin to deflate by inches at a time.  When you take a shuddering breath, it's as if you had been underwater and finally broken the surface - gulping cool air into your lungs with heaving gasps of blessed relief.");
	}
	else {
		output("Like a bolstering surge, the nun's virile magic pours into you, a flux of hot and cold stimulating your prostate into an intoxicating overdrive.  ");
		//[balls:
		if (pc.balls > 0) output("The pressure on your corpulent balls is tremendous, the over-burdened globes hanging from you literally vibrating under the tension of your imminent orgasm.  ");
		output("Needing no further coaxing, you begin sliding your meat deeper into the nun's mouth, feeding inch after inch until you hit the back of her throat and push further.  Reaching out, you grab the sides of the nun's head, tilting her jaw back to ease the relentless journey of your tremulous hydrant.  Her throat clenches down around you in muscled pulses, rings of tension milking your [pc.cock " + pc.biggestCockIndex() + "] with every gulping gasp the impure nun takes.  Her neck visibly bulges as you grit your teeth and force your girth deeper into her welcoming body.");
		...
		output("\n\nThe deluge of your semen pours forth in globular torrents that stretch Scylla's jaw to its limits, bloated swells of cum distending your [pc.cock " + pc.biggestCockIndex() + "], the prioress' throat fattening obscenely around your widening girth.  Pumping load after load directly into her tainted stomach, you are rewarded with the sight of her gut thickening into an expanding paunch.  With every fresh wave of cascading spunk, the nun's belly enlarges, her lean frame swallowed under the corpulent bulk of her cum-swollen heft.");
		//[Balls:
		if (pc.balls > 0) output("  Discharging their pressurized bounty in an unbound release, your over-inflated balls finally cease their incapacitating growth and begin to deflate by inches at a time.");
	}
	pc.orgasm();
...
public function openTheDoorToFoursomeWivScyllaAndFurries():void
{
	clearOutput();
	showScylla();
	output("The door audibly creaks when you slip into the Wet Bitch’s back room, but the chamber’s occupants are none the wiser, too busy passionately rutting to hear something as subtle as a hinge’s protest. Scylla is on her knees (of course), her breasts ballooned to gargantuan sizes, big enough that the sit on the floor with her while two white-furred girls pound away at her noisily slurping lipples.");
	...
	output("\n\nScylla’s fluttering eyes spot you from between the curvy hermaphrodites. Her tongue laps hungrily at her lips before she stops herself, blushing. <i>“[pc.name]! I... ooh... didn’t... uh... oh my! You l-look so pent up!”</i>");
	output("\n\nYou shed your [pc.gear] halfway through her stammered apology-turned-invitation, giving her a good long look at your [pc.cocks]. Luminous, innocent eyes fix on your libertine display, filling with hunger by the second. Her mouth falls open on its own, her tongue rolling out like a lush red carpet, an invitation if you’ve ever seen one.");
	...
	//First time
	if (int(flags["COC.SCYLLA_FURRY_FOURSOME_COUNT"]) == 0)
	{
		output("\n\nShe greets you in between grunts of pleasure, <i>“I’m Snow, and the whimpering slut over there is Winter. Nice to finally meet you, [pc.name].”</i>");
		output("\n\nYou nod and step a little closer. Scylla’s tongue stretches out to greet your [pc.cockHead " + pc.biggestCockIndex() + "], lapping wetly against it. You struggle not to groan too lewdly in the presence of the more composed mouse.");
		output("\n\n<i>“Awww, don’t be shy,”</i> Snow coos, swatting at your [pc.butt] with her whip-like tail until your hips hitch forward. <i>“She told us about you earlier.”</i> The mouse pauses, wiggling her tail between your cheeks until it’s pressing on your asshole. <i>“Usually I reserve this kind of treatment for Winter, but we can’t have you backing out after coming this far, can we?”</i> You lurch forward from the near-intrusion, burying more than a few inches into Scylla’s sucking gullet.");
		output("\n\nShe hums happily, eyes falling closed to focus on the feelings coming from her three mouths.");
	}
	else
	{
		output("\n\nThe cocky rodent, Snow, greets you in between grunts of pleasure, <i>“Hello again, [pc.name]. You’re just in time - she hasn’t had any cum yet, and she wouldn’t shut up about wanting your dick in her mouth until we both started plugging her together. Some nun.”</i>");
		output("\n\nYou step a little closer, allowing Scylla’s tongue to stretch out to caress your [pc.cockHead " + pc.biggestCockIndex() + "]. She laps at you wetly, running an organ that seems more snake than muscle in lazy circles around your girth. You struggle not to groan too lewdly in presence of the two herms, silently hoping for a little encouragement from Snow again.");
		output("\n\nThe albino mouse has quite the twinkle in her red-hued eyes as she watches you, silently wrapping her rodentine tail around behind your [pc.butt]. She gently caresses each of your cheeks with it before threading in between to press gently on your [pc.asshole]. <i>“Need a little more encouragement, don’t you?”</i>");
		output("\n\nYou nod and thrust the first few inches of your [pc.cock " + pc.biggestCockIndex() + "] into Scylla’s mouth at the direction of her tail. The nun moans around your shaft and lets her eyes drift closed, overwhelmed by the feelings from three dick-stuffed mouths and her own unholy seed-hunger.");
	}
	output("\n\nSnow beams at you, still circling your [pc.asshole] while you feed the rest of your [pc.cock " + pc.biggestCockIndex() + "] into the nun’s mouth of your own accord.");
	...
	if (int(flags["COC.SCYLLA_FURRY_FOURSOME_COUNT"]) == 0) output("busts her nuts like a champ.");
	else if (int(flags["COC.SCYLLA_FURRY_FOURSOME_COUNT"]) <= 1) output("fills condoms like some kind of balloon-making machine.");
	else if (int(flags["COC.SCYLLA_FURRY_FOURSOME_COUNT"]) <= 2) output("busts condoms just about every other orgasm now.");
	else if (int(flags["COC.SCYLLA_FURRY_FOURSOME_COUNT"]) <= 3) output("busts condoms if she even thinks about using ‘em.");
	else output("can almost fill a bathtub nowadays. Fuck, I love watching her squirm while I tease those huge loads out of her.");
	output("”</i>");
	output("\n\nYou feel like you started cumming a little more... vigorously since your first visits with Scylla as well. Nothing wrong with a little bit more spunk though, right? You feed the last few inches of your length past the nun’s scarlet suckers, feeling your [pc.cockHead " + pc.biggestCockIndex() + "] snake down her seemingly endless throat. It’s hard to be concerned about something that feels so unequivocally good, particularly when you’re already started to leak pre-cum like a sieve into a wanton woman’s belly.");
	...
	if (pc.biggestCockLength() <= 12) output("a foot");
	else if (pc.biggestCockLength() < 24) output("over a foot");
	else output("a few feet");
	output(" of cock choking off her throat.");
	output("\n\nSnow, clearly feeling mischievous, draws most of the way out of chosen target, releasing a small avalanche of backed-up milk in the process. She gingerly guides her nearly three-foot pole over the nun’s leaking lips, brushing her quivering tip against the sensitive ruby pillows. <i>“Hey, if you use some of that magic on me, I’ll put it back in </i>and<i> milk [pc.name]’s prostate for you.”</i>");
	...
	output("\n\nSnow pushes her bulbous head halfway back in. <i>“Nope! Just think about it, you, making my huge dick squirt out rivers of that stuff you crave, and me, squeezing every... single... drop... from lover-" + pc.mf("boy","girl") + " here.”</i>");
	...
	output("\n\nYou aren’t far behind her. The moment her tail slips from your sore prostate and out of your [pc.asshole], your [pc.legs] give out, dropping you on a pile of clothes. Your [pc.cock " + pc.biggestCockIndex() + "] gives a few feeble squirts before ");
	if (pc.libido() >= 80 || pc.lustMin() >= 50) output("slapping into your belly, still hard but spent");
	else output("starting to go limp");
	output(". It feels like you’ve been cumming for hours, and you’re equal parts sore and exhausted as a consequence.");
	...
	pc.slowStatGain("l", 0.5);
	clearMenu();
	addButton(0, "Next", scyllaFurryFoursomePartDues);
}
...
private function scyllaFurryFoursomePartDues():void
{
	clearOutput();
	showScylla();
	output("A strong hand slaps down on your chest, rousing you from slumber. <i>“Whaa...?”</i>");
	...
	if (pc.cumMultiplierRaw < 19990) pc.cumMultiplierRaw += 1 + rand(5);
	if (pc.balls > 0 && pc.ballSizeRaw < 30) pc.ballSizeRaw++;
	IncrementFlag("COC.SCYLLA_FURRY_FOURSOME_COUNT");
	processTime(5);
	clearMenu();
	addButton(0, "Next", returnToCampUseOneHour);
}