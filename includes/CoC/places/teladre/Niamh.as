...
public function showNiamh():void {
	if (Flag("COC.MET_NIAMH") > 0) userInterface.showName("\nNIAMH");
	else userInterface.showName("\nBEER CAT");
}

public function telAdreNiamh():void {
	if(Flag("COC.MET_NIAMH") == 0) {
		...
		if(hours <= 8) output("massive N-cup breasts, spilling over the table");
		else if(hours <= 9) output("huge M-cup breasts, taking up much of the table");
		else if(hours <= 10) output("great L-cup breasts, advancing far onto the table");
		else if(hours <= 11) output("heavy K-cup breasts, lying on the table for support");
		else if(hours <= 12) output("hefty J-cup breasts, resting comfortably on the table");
		else if(hours <= 13) output("weighty I-cup breasts, dangling to lie on the table");
		else if(hours <= 14) output("squeezable H-cup breasts, settling burdensomely on her chest");
		else output("moderately manageable G-cups, trembling with every breath she takes");
		output(".  Smooth, touchable skin covers her chest, face, hands and presumably the rest of her; it seems her animalistic accoutrements stop at kitty ears and tail. She takes a deep swig of her drink, following it up with a cute little burp.");
	}
	//This scene only plays if the PC has either Talked to Niamh or has gotten at least 1 mug of Black Cat Beer from her.
		...
		if(hours <= 8) output("her titanic N-cup 'kegs' resting against the thankfully sturdy table in front of her.  Her teats are engorged with the beer she's cursed to carry, and the stuff dribbles out whenever she shifts.");
		else if(hours <= 14) {
			output("her ");
			if(hours <= 9) output("M-cup");
			else if(hours <= 10) output("L-cup");
			else if(hours <= 11) output("K-cup");
			else if(hours <= 12) output("J-cup");
			else if(hours <= 13) output("I-cup");
			else output("H-cup");
			output(" brew-burdened breasts have clearly gotten smaller as the day dragged on, although they're still uncomfortably sizeable and swollen.");
		}
		else output("her G-cup beer-filled boobs far smaller than their morning volume, though still impressive in stature.  You can follow the progress she's made throughout the day by the beer stains running along the wooden surface, trailing from all the way off the table and ending where her boobs now lie.");
	}
}
...
public function approachNiamh():void {
	clearOutput();
	showNiamh();
	//output(images.showImage("niamh-approach-in-bar"));
	if(rand(5) == 0 && Flag("COC.MET_NIAMH") > 0 && (pc.hasItem(new CoCBimboLiqueur()) || pc.hasItem(new CoCSucMilk())))
	{
...
public function getANiamhBeer():void {
	clearOutput();
	showNiamh();
	//output(images.showImage("niamh-get-beer"));
	output("\"<i>That'll be two gems,</i>\" she replies.  You hand over the payment, which she holds up to the light and scrutinizes thoroughly.  Satisfied, she drops the things into a sack at her waist.  \"<i>Sorry,</i>\" she mutters. \"<i>I do want to get this damned beer out o' me, but this is still my primary livelihood; I canno' afford to be cheated.  Anyway, here's your mug - you can keep that f'r another two gems; I get 'em cheap.  Go ahead and fill it up, but keep in mind what you're tweakin', right?</i>\"");
	//Feeder
	//if(pc.findPerk(PerkLib.Feeder) >= 0) output("  You understand exactly what she's going through; the weight of a huge pair of breasts, the sensitivity as they engorge with fluids, the near-maddening sensation of the contents sloshing around underneath your straining skin...");
	//(Player has G-cups or bigger;
	//else 
	if(pc.biggestTitSize() >= 15)
	{
		if(pc.lactationQ() == 0)
			output("  Not terribly surprising; you know your tits are sensitive, and you don't even make milk in them.");
		else
			output("  You can easily imagine just how much trouble she must have, especially given the bulk of her tits are fluid.");
	}
	else if(pc.lactationQ() > 0)
		output("  You know just how distracting it is to have your tits full of fluid waiting to get out.");

	//([if first time]
	if(Flag("COC.GOT_NIAMH_BEER") == 0)
	...
	if(Flag("COC.MET_NIAMH") > 0)
		...
		output("the cat-girl ");
	output("sighs in relief, biting her lower lip slightly.");
	//General Ending
	...
	pc.credits -= 20;
	IncrementFlag("COC.GOT_NIAMH_BEER");
	//[Here][To Go]
	clearMenu();
	addButton(0, "Here", drinkNiamhsBeerInTelAdre);
	addDisabledButton(1, "To Go", "To Go", "You're too poor to buy the mug.");
	if(pc.credits >= 20) addButton(1, "To Go", blackCatBeerToGo);
}
...
private function drinkNiamhsBeerInTelAdre():void {
	...
	clearMenu();
	showNiamh();
	if(pc.lactationQ() >= 300 && pc.biggestTitSize() >= 5 && rand(2) == 0 && IncrementFlag("COC.MET_NIAMH", false) > 0) {
		...
		output("\n\nYou start to stumble as your sense of balance shifts and you realize the heat in your body is expanding outwards.  Looking down you see it isn't the heat that's expanding, but your own bountiful tits.  Each of your [pc.fullChest] have gained two full cup sizes, and within seconds they've gained even more.  Beneath your clothes your areola darken in color and you can feel each nipple throbbing with the need to expel its milky contents.");
		output("\n\n\"<i>Oh me, ma darlin'.</i>\"  Niamh comments, \"<i>Those beauties o' yours look to be burstin' out o' ye...</i>\"  Your [pc.gear] falls open as your enlargening breasts swell too large to be contained.  Each of your nipples fatten up and begin squirting a liquid that looks unusual.");
		...
		if(checkDate(31, 12, 3)) {
			...
			if(pc.hasVagina()) output("pussy to wetten");
			if(pc.isHerm()) output(" and ");
			if(pc.hasCock()) output("[pc.eachCock] to stiffen");
			output(".  \"<i>Ye know, I got's an idea.  How's about ye stick by me for the next hour.  I'm sure ye must be diein' to drain those big'uns of yours.  We might as well sell your champagne ta the patrons.  Whaddaya say Lassie?</i>\"");
			//[SELL YOUR BOOZE]  [LEAVE]
		}
		// Saint Patrick's
		else if(checkDate(17, 2, 1)) {
...
private function blackCatBeerToGo():void {
	clearOutput();
	output("\"<i>That'll be two more gems, then,</i>\" the girl says.  \"<i>Though, if your hands get any more skillful I might have to pay you to take it,</i>\" she flirts.  You pass over two more gems, and she gratefully bags them and beams you a smile.");
	pc.credits -= 20;
	//both output
	if (hours <= 15) output("\n\n\"<i>Thanks for the business, " + pc.mfn("laddie", "lassie", "customer") + "!  Remember, Niamh sells her Black Cat Beer every day from 8 until 4.</i>\"  You nod your head in thanks and step away from the table.");
	//16:00 ending
		...
		output("\n\nThanks to your efforts, ");
		if(IncrementFlag("COC.MET_NIAMH", false) == 0) output("the cat girl's");
		else output("Niamh's");
		output(" breasts have had the last of their beer squeezed from them.  She sighs in relief, caressing her shrunken breasts; while still hovering at around G-cup size, they're much smaller than they are when the day starts for her.  \"<i>Me thanks for the business; ye got the last mug for today.  Still, I'll be here tomorrow, full as ever.</i>\" She sighs softly.  \"<i>I regret to say that Niamh's Black Cat Beer doesn't look to be going out of business anytime soon.</i>\"  She stands and gathers her coat, slipping her arms into the sleeves.  The nimble girl draws the garment across her buxom chest, buttoning the slightly strained buttons with deceptive ease.  Dressed properly, she starts away, a bag full of gems bouncing against her swaying hips. You watch her go, staring at her back until she walks out the door.");
	}
	output("\n\n");
	...
	itemCollect([new CoCBlackCatBeer()]);
}
...
private function talkToNiamh():void {
	clearOutput();
	//(first time only)
	if(Flag("COC.TALKED_NIAMH") == 0) {
		output("You ask the cat-girl to tell you about her past.");
	...
	clearMenu();
	addDisabledButton(0, "Get Beer", "Get Beer", "You're too poor for beer.");
	if(pc.credits >= 20) addButton(0, "Get Beer", getANiamhBeer);
	addButton(14, "Leave", leaveNiamh);
}
//Leave
private function leaveNiamh():void {
	clearOutput();
	showNiamh();
	output("You decide that you don't really want to talk to this strange cat-girl and, as politely as possible, excuse yourself.");
...
public function blackCatBeerEffects(player:PlayerCharacter,clearScreen:Boolean = true,newLine:Boolean = false):void {
	if (clearScreen) clearOutput();
	showNiamh();
	if(newLine) output("\n\n");
	output("Uncapping the mug, you swill the stuff down in a single swig, gasping as it burns a fiery trail into your belly.  It's rich and sweet, and damn, but it's strong stuff!");
	...
	// FIXME: not going to do it myself without proper debuff system
	//var lib:Number = 0;
	//if(pc.hasStatusEffect("Black Cat Beer")) {
		//if(100 - pc.libido() >= 10) libido() = 10;
		//else libido() = 100 - pc.libido();
		//pc.addStatusValue(StatusAffects.BlackCatBeer, 1, 4);
		//pc.addStatusValue(StatusAffects.BlackCatBeer, 2, libido());
		//output("\n\nDamn, it's even better with every extra drink!");
		//dynStats("spe", -1, "int", -1, "lib", lib, "lus", 30 + rand(pc.lib / 4));
	//}
	//else {
		//if(100 - pc.lib >= 10) lib = 10;
		//else lib = 100 - pc.lib;
		//pc.createStatusAffect(StatusAffects.BlackCatBeer, 8, lib, 0, 0);
		//dynStats("spe", -5, "int", -5, "lib", lib, "lus", 20 + rand(pc.lib / 4));
	//}
	//pc.slimeFeed();
}
...
//public function blackCatBeerExpires():void {
	//dynStats("spe", 4.5, "int", 4.5, "lib", (-1 * pc.statusAffectv2(StatusAffects.BlackCatBeer)));
	//pc.removeStatusAffect(StatusAffects.BlackCatBeer);
...
private function corruptOrBimboNiamhIntro():void {
	clearOutput();
	showNiamh();
	output("\"<i>Heya, favored customer,</i>\" the busty kitty greets as you approach.  \"<i>I have a favor to ask!  My drink's empty, and I get bored of me own brew,</i>\" Niamh explains.  \"<i>Would ya mind fetchin' me somethin', or are ye gonna force me to sample me own goods once more?</i>\"");
	...
	if(pc.hasItem(new CoCBimboLiqueur()))
		output("'bubbly' for her, in the form of the Bimbo Liqueur, or ");
	output("maybe even a Succubus Milk?  You'll have to consider your options.  On the other hand, of course, something that corruptive would probably mutate her booze-filled boobs, most likely forcing her out of Tel'Adre altogether... and may fall back on your own head, to boot.  What do you do?");
	...
	clearMenu();
	if (pc.hasItem(new CoCBimboLiqueur())) addButton(0, "Bimbo", giveNiamphBimboLiquer);
	if (pc.hasItem(new CoCSucMilk())) addButton(1, "S.Milk", giveNiamphSuccubiMilk);
	addButton(14, "Back", maybeLaterNiamh);
}
//[Maybe Later]
...
public function taintNiamh(bimbo:Boolean = false):void {
	clearOutput();
	showNiamh();
	output("You grab Niamh's glass and one more and head off, making sure to turn a corner before commandeering an empty table.  Plunking the mugs down in front of you, you mutter to yourself as she moves away.");
	//[if sucmilk]
	if(!bimbo) {
	...
	//[if bimbo liqueur]
		...
		output("\n\nHer expression brightens when she notices your approach, and she giggles when she regards the quarter-full mugs of liqueur.  \"<i>What, that's it?</i>\" she quips curiously.  \"<i>This stuff had better pack some punch, " + pc.mf("lad","lass") + ".</i>\"  You assure her that it will knock her socks off.");
		output("\n\nNiamh reaches for a mug, but you draw it away from her questing hand, instead sinking it into her considerable expanse of cleavage.  A devious grin flits across your features as she subsequently shivers from the cold glass between her fluid-filled bosom.  \"<i>Wh-why you...</i>\" she gasps, quickly snatching the drink and rubbing warmth back into her goosebumped flesh.  \"<i>You scoundrel...</i>\"");
	...
	//[if bimbo liquor]
		...
		flags["COC.NIAMH_STATUS"] = 2;
	}
	flags["COC.NIAMH_MOVED_OUT_COUNTER"] = timeAsStamp;
	pc.cor(10);
...
public function niamhPostTelAdreMoveOut():void {
	...
	flags["COC.NIAMH_MOVED_OUT_COUNTER"] = -1;
	clearOutput();
	showNiamh();
	output("Upon one of your routine searches, you're given pause as a half-coherent mess of a song comes tumbling across the grasses.  You move toward the cacophony of sound, fairly confident in what you will find.  Indeed, stumbling and shambling towards you comes a ");
	if(flags["COC.NIAMH_STATUS"] == 1)
		output("demonic-looking");
	else
		output("hourglass-shaped");
	output(" Niamh, and it's obvious she's completely hammered.  ");
	if(flags["COC.NIAMH_STATUS"] == 1)
		output("\"<i>W'hey there " + pc.mf("boyo", "big girl") + ", wanna...g'forrra goooo-gfh...time?</i>\"");
	else
		output("\"<i>W'heeeey, honey!  I never thought I'd see yaaaagain!</i>\"");
	output(" she slurs, struggling to stay upright between her impaired motor skills and her absolutely overfilled chest.  \"<i>Aaah've been drinkin' me 'shtash',</i>\" she explains, noticing your gaze.  \"<i>Canno' leddit go'tuh waste!</i>\" You nod your agreement, sneaking a glance behind her to observe the twin trails of ");
	if(flags["COC.NIAMH_STATUS"] == 1)
		output("dark");
	else
		output("light");
	output(" fluid she's leaving behind.");
	output("\n\n\"<i>Wha's yer name, by th' way?</i>\" she suddenly inquires, sobering just a bit.  You decide there's no harm in formally introducing yourself.  \"<i>[pc.name], eh?  Weeeeeird name,</i>\" she comments.");
	...
	addButton(4, "Leave", niamhCorruptMobileSnackTurnDown);
}
...
private function niamhCorruptedMobileSnackDrinkTime():void {
	...
	if(flags["COC.NIAMH_STATUS"] == 2) {
		...
		output("\n\nFinally, Niamh's boobgasm comes to an end and she falls back on her big jiggly butt.  \"<i>Like, that feels so much better than when I put my big ol' boob in my own mouth...  Hey, [pc.name]... you, like, okay?</i>\" she asks.  She half-crawls over to you, then giggles.  \"<i>You, like, got a big sloshy belly on you now, [pc.name]!  You look so funny with this giant gutty-gut.</i>\"");
...
public function bimboChampagne(player:PlayerCharacter,clearScreen:Boolean,intro:Boolean):void {
	if (clearScreen) clearOutput();
	showNiamh();
	if(intro) {
		if(pc.isBimbo(true)) {
			output("You could've swore the stuff worked when you saw Niamh do it to others, but for some reason, it had, like, no effect on you. How weird!");
		}
		else if(!pc.hasStatusEffect("Bimbo Champagne")) output("You uncork the bottle and breathe in the fizzy, spicy aroma of the sparkling liquor.  Breathing deeply, you open your mouth and begin pouring the ever-effervescent fluid inside.  It's sweet and slightly gooey, and the feel of it sliding down your throat is intensely... awesome?  Like, totally!");
...
public function bazaarNiamh():void {
	...
	//Bimbo Niamh:
	if(flags["COC.NIAMH_STATUS"] == 2) {
		if(IncrementFlag("COC.TIMES_NIAMH_BAZAAR_MET", false) == 0) {
			output("As you enter the bazaar, you hear a familiar voice calls out to you. \"<i>Hey!  [pc.name]!  Over here!  Like, it's me!</i>\"");
			output("You follow the sound and find Niamh seated in a corner of the bazaar.  She is totally naked save for a small strip of cloth around her loins; her huge breasts lie exposed for all the world to admire.  She's surrounded by a small entourage of assorted beings; some are either hoping to earn a drink or else are simply here to ogle the massively buxom catgirl, others have evidently already drunk their fill, given their swollen bellies, feminine forms and platinum blonde coloration.  She grins as you approach.");
			output("\"<i>Like, I was just roaming the plains, trying to, like, figure out where I was gonna go, and I found this place.  Isn't it, like, so neat here?</i>\"  She has apparently already forgotten that this was her destination all along!  \"<i>All these cool people who know how to have a totally good time!  Not like that dump in the desert, fulla buzzkills.  Yeah, I'm gonna, like, party all night and day here, [pc.name].</i>\" She giggles yet again.  \"<i>But you're still, like, my number one customer.  So... you, like, wanna do something?</i>\" she asks.");
		}
		//[Bizarre Bazaar, repeat encounter]
			...
			output("Surrounded by blonde-haired, horny women, herms and traps, many of them sporting stomachs swollen with Niamh's copious champagne lactation, the bimbo catgirl enthusiastically waves you over.  \"<i>[pc.name]!  Like, what can I do you for?</i>\" she giggles.");
		}
	}
	//Corrupt:
	else {
		//(General notification:
		output("Given the pile of snoozing bodies, you can probably hazard a guess as to where Niamh decided to set up shop.");
		//(Approach:
	...
	IncrementFlag("COC.TIMES_NIAMH_BAZAAR_MET");
	//[Bizarre Bazaar, beer purchase, bimbo and corrupted version]
	clearMenu();
	
	addDisabledButton(0, "Get A Drink", "Get A Drink", "You're too poor to get a drink.");
	addDisabledButton(1, "Tit-Fuck", "Tit-Fuck", "This scene requires you to have cock.");
	
	if (flags["COC.NIAMH_STATUS"] == 1) {
		addDisabledButton(0, "Get A Drink", "Get A Drink", "Succubus milk Niamh Not implemented yet.");
		addDisabledButton(1, "Tit-Fuck", "Tit-Fuck", "Succubus milk Niamh Not implemented yet.");
	} else {
		if (pc.credits >= 20) addButton(0, "Get A Drink", bazaardNiamhDrink);
		if (pc.hasCock()) addButton(1, "Tit-Fuck", bazaarNiamhSex);
	}
	addButton(14, "Back", enterTheBazaar);
}
...
private function bazaardNiamhDrink():void {
	...
	showNiamh();
	pc.credits -= 20;
	output("Approaching the fallen-from-grace (yet completely content) catgirl, you toss her a couple gems and request a glass of the house drink.");
	//Bimbo:
	if(flags["COC.NIAMH_STATUS"] == 2) {
		...
		addButton(0, "Bottle", getBimboChampFromNiamh);
	}
	//Corrupt
	else {
		output("Succubus milk Niamh Not implemented yet");
...
private function drinkFromZeTap():void {
	clearOutput();
	showNiamh();
	output("Your eyes lock onto her large, ever-dribbling nipples, the froth of spilt champagne caked on the table supporting her mammoth breasts, and you know you just have to have it from the source.  You push your way forward and seat yourself on a plush cushion that, from its many tears and suspicious-smelling stains, has seen better days, then grasp the huge boob in front of you, making the ditzy catgirl yowl in anticipation of pleasure.");
	...
	output("\n\nYou snap back to your senses with a jolt, finding yourself flat on your back and gazing up at the sky.  \"<i>Like, are you alright, [pc.name]?</i>\" Niamh asks, amused.  \"<i>My fizzy titties, like, really hit some people harder than they can, y'know, deal with.  You okay?</i>\"");
...
private function getBimboChampFromNiamh():void {
	clearOutput();
	showNiamh();
	output("You explain your need for a bottle of her champagne; the ditzy kitty takes several moments to register your words, then nods idly.  \"<i>Works fer me!</i>\" she responds, reaching behind her and rummaging in a large burlap bag.  She produces a flask from the sack's depths, unscrewing the cap and attempting to line it up with a nipple.  Try as she might, she just can't quite reach all the way, and with an airy giggle, she passes it to you instead.");
	...
	itemCollect([new CoCBimboChampagne()]);
}

//[Bazaar sex]
private function bazaarNiamhSex():void {
	...
	if (flags["COC.NIAMH_STATUS"] == 2)
	{
		...
		output("As you approach the bimbofied cat-girl, your eyes wander lustily toward her huge, beer-lactating tits.  Seeing you stare, she giggles and shakes her chest enticingly, letting the huge, liquid-filled mounds wobble.  \"<i>Like, like what you see, [pc.name]?</i>\" she purrs, her long black tail wagging excitedly behind her.");
		...
		output("\n\n\"<i>Thash was - HIC - shuper shexy, [pc.name],</i>\" the drunken keg-girl giggles, staggering to her feet and stumbling toward the Bazaar.  \"<i>Like, we shud do thish again shoon!</i>\" she adds, prancing out with her new pearl necklace on full display.");
		pc.orgasm();
		//dynStats("int", -.5, "sen", -2);
		pc.slowStatGain("i", -0.5);
	}
	else if (flags["COC.NIAMH_STATUS"] == 1)	// Succubi Milk
	{
...
//public function seanBimboBrewing():void {
	//clearOutput();
	//output("The uncharacteristically gentlemanly incubus raises his eyebrow as you produce an armful of filled flasks and drop them onto his mostly-empty table.  \"<i>I assume you'd like me to do something special with these,</i>\" he quips, somewhat dryly.");
...
//public function getBimboozeFromSean():void {
	//clearOutput();
	//output("Sean turns as you enter, managing a thin smile as he regards you.  \"<i>Excellent timing, I have just finished this!</i>\"");
	//output("He scoops a familiar-looking bottle from a tabletop, passing it off to you.  \"<i>Be somewhat careful with this,</i>\" he warns as you take it.  \"<i>The original's deleterious effect on the mind has been duplicated, I assure you.</i>\"\n\n");
	//output("You drop the vial into a pouch on your person.\n\n");
...
private function sellYourBooze():void {
	...
	showNiamh();
	pc.credits += 20;
	temp = Math.round(pc.lactationQ() / 100);
	if(temp > 40) temp = 40 + rand(3);
	pc.credits += Math.round(temp * 10);
	//If player has only two breasts.
	if(pc.bRows() == 1) output("With a chipper giggle Niamh pulls you closer to the bar.  With both hands she helps you heft your tits up and on to the counter.");
	//If player has four or six breasts.
	else output("Niamh giggles as you try to move your breasts closer to the bar to be milked by the patrons, but it's obvious only the top pair of your tits can easily rest on the counter top.  \"<i>Lassie, it might be unconventional, but givin' the circumstance mayhaps it'd be best if ye laid down on the bar.</i>\"");
	...
	if (pc.hasVagina()) output("  You feel your excitement dripping down the inside of your thighs as your pussy wettens.");
	pc.lust(10 + pc.libido() / 5);
	//dynStats("lus", 10 + pc.sens/5, "resisted", false);
	//[If player has cocks]
	if(pc.hasCock()) output("  You start to feel sympathetic sensations in [pc.eachCock] as the hands and paws of the bar goers milk your nipples.  With their fingers sliding over your sensitive nubs you can easily imagine them sliding up and down your cock");
	...
	//If lust is high
		...
		if(pc.hasVagina()) output("You can feel your labia dripping with need.  ");
		if(pc.hasCock()) {
			output("pre-cum drizzles down your [pc.cocksLight] as the need to fuck a pussy grows stronger.  A few wandering hands reveals some apparent interest amongst the folks in the bar in seeing your genitals as active as your nipples.");
		}
		//[FUCK THE BAR] [LEAVE]
...
private function barBeerOrgyTits():void {
	...
	if(pc.hasCock()) {
		...
		output(" discreet but it's impossible.  You try to push ");
		if(pc.cockTotal() == 1) output("it");
		else output("them");
		output(" down or aside, but other pairs of hands begin stroking you.");
	}
	...
	if(pc.buttRating() >= 20) output("  She slaps your ass to emphasize her meaning, and the action sends jiggling waves through each immense cheek.");
	...
	if(pc.biggestTitSize() >= 20) output("Your giant boobs are large enough to temporarily hang off the sides of your body as you're carried.");
	output("  Your [pc.butt] hits the table first as the crowd puts you down.  Some patrons begin sucking on your nipples directly.");
	// [IF player has four or more giant tits, but not hugely pregnant]
	if((!pc.isPregnant() || pc.bellyRatingMod < 50) && pc.bRows() > 1) output("  You are essentially a pile of " + boozeBoobsType() + " filled tits.");
	// [IF player is hugely pregnant with four or more tits]
	else if(pc.bRows() > 1) output("  You are essentially a giant pregnant belly surrounded by your [pc.chest] on the table, helpless to the sexual advances of the bar goers and you can't get enough of it.");

	//===============
	//Third Paragraph
	//===============
	//IF player has pussy and Urta is in the bar and sex with her is unlocked.
	if(hours < 15 && flags["COC.URTA_COMFORTABLE_WITH_OWN_BODY"] > 0 && pc.hasVagina()) {
		...
		if(pc.isPregnant()) output("  Some folks give yelps and cheers when they see the bulge that repeatedly forms in your abdomen each time Urta thrusts herself inside you.");
	...
	if(pc.isPregnant() && pc.bellyRatingMod >= 50) output("  \"<i>Oi!  Careful folks, this babe could blow at any time!</i>\"  Niamh pats your belly with chiding affection.");
	...
	if(pc.hasFuckableNipples()) output("  Eventually some of the randier drunks start whipping out their cocks, and begin pressing the heads against the openings of your nipples.  Within moments you feel your booze bloated breasts being fucked silly by " + num2Text(pc.totalNipples()) + " hard cocks.  Each thrust causes " + boozeBoobsType() + " to spill out around their shafts.");
	// ELSE[Character doesn't have fuckable nipples]
	else output("  Some of the hornier customers start shoving cocks of all shapes and sizes into the moist wet cleavages formed by yours and Niamh's breasts.");
	...
	if(pc.hasFuckableNipples()) output("  The cocks filling your nipples start pumping hot sperm into your breasts, mixing with your " + boozeBoobsType() + " and causing every nerve ending in your nipples to tingle.  Some of them ejaculate so much that it feels as if your breasts are swelling even larger, almost to the point of bursting before the cum spills out.");
	//IF [Player has cocks]
	if(pc.cockTotal() > 0) {
		output("  You feel as if your body is exploding everywhere.  Niamh gags momentarily as her mouth fills with your seed, and you realize your cock");
		if(pc.cockTotal() > 1) output("s have");
		else output(" has");
		output(" begun spurting ");
		if(pc.cockTotal() == 1) output("its ");
		else output("their ");
		output("jizz.  Niamh swallows it down her throat but not without some of it getting plastered across her face.");
		//IF [character is pregnant]
		if(pc.isPregnant() && pc.bellyRatingMod >= 50) output("  Globs of cum splatter across your gravid belly.");
	}
	...
	if(IncrementFlag("EDRYN", false) >= 5 && pc.isHerm() && hours == 14 && flags["COC.URTA_COMFORTABLE_WITH_OWN_BODY"] > 0) {
		...
		else output("  Urta's cock and Edryn's pussy pin you in place between them, and as Urta fucks you deeper her cock head forces your belly to bulge outwards until it presses against Edryn's ass.");
		...
		if(pc.bRows() == 2) output("  Your second row of tits get thrust out to either side every time Urta's torso presses your own against Edryn.");
		//IF [player has six giant breasts]
		else if(pc.bRows() == 3) output("  Your lowest row of tits slap against all three of your thighs as the love making becomes more brutal.");
		//IF [player has large balls]
		if(pc.balls > 0 && pc.ballSize() > 14) output("  Your hanging testicles bounce around between everyone's legs.");
		//IF [player has giant ass]
		if(pc.buttRating() >= 22) output("  The fat of your ass cheeks jiggles and quakes even as Urta practically stuffs her hips into your crack.  \"<i>Damn, you've got a bigger ass than Edryn,</i>\" she chides as she spanks you.  \"<i>Hey!</i>\"  Edryn calls out with a false tone of hurt pride.");
	...
	// IF [Urta but not Edryn is present in the bar and sex with her is unlocked and character has pussy]
		...
		if(pc.bRows() > 1) output("  Your rows of breasts shift around as your posture changes, making squelching sounds as they slide around.");
	...
	//Generic ending if the first two don't trigger
		...
		if(pc.cockTotal() == 1) output("  The six titted girl smiles at you, patting her belly while your cum runs down the inside of her thighs.  You only now wonder about how fertile she might be.");
		// IF [player has multiple cocks]
		else if(pc.cockTotal() > 1) output("  Various girls around the room are patting their bellies and giving you sultry looks while your cum continues to drip from their pussies.  You passively wonder if any of them might indeed be hauling around some larger pregnant bellies in a few days, but you try to push those thoughts from your head.  At the moment they seem happy.");
		// IF [player has pussy]
		if(pc.hasVagina()) {
			output("  You walk slightly bow legged out of the bar.  Cum is still dripping from your snatch and will likely continue to do so for a while.");
			//IF [player is not pregnant]
...
private function boozeBoobsType():String {
	// New Years
	if(checkDate(31, 12, 3)) return "champagne";
	// Saint Patrick's
	else if(checkDate(17, 2, 1)) return "Saint Patrick's Day lagar";
	// Christmas
	else if(checkDate(25, 12, 3)) return "egg nog";
	// Valentine's
	else if(checkDate(14, 2, 1)) return "wine";
	// Non-Holiday, Generic
	else return "beer";
}