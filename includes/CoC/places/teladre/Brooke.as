...
public function brookeAffection(arg:Number = 0):Number {
	Flag("COC.BROOKE_AFFECTION", arg, true);
	if(flags["COC.BROOKE_AFFECTION"] > 100) flags["COC.BROOKE_AFFECTION"] = 100;
	else if(flags["COC.BROOKE_AFFECTION"] < 0) flags["COC.BROOKE_AFFECTION"] = 0;
	if((pc.isTaur() || flags["COC.BROOKE_SHOWERED_WITH"] == 0) && flags["COC.BROOKE_AFFECTION"] > 20) flags["COC.BROOKE_AFFECTION"] = 20;
	return flags["COC.BROOKE_AFFECTION"];
}
...
public function meetBrookeFirstTime():void {
	...
	brookeAffection(5);
	flags["COC.BROOKE_MET_TODAY"] = days;
	flags["COC.BROOKE_MET"] = 1;
...
public function repeatChooseShower():void {
	...
	if(hours < 16 || hours > 18 || pc.tone < 30 || flags["COC.BROOKE_MET_TODAY"] == days) {
		...
		else if(Flag("COC.BROOKE_MEDIUM_SCENE") == 0) {
	...
	if(Flag("COC.BROOKE_GRUMPS_ABOUT_TA") == 0 && pc.tone >= 30) {
	...
	else if(brookeAffection() <= 39 && Flag("COC.BROOKE_SHOWERED_WITH") == 0) {
		mediumLowBrookeAffectionOneTime();
	}
	//Affection >=40, after first-time sex
	else if(brookeAffection() >= 40 && Flag("COC.BROOKE_MEDIUM_SCENE") > 0) {
		mediumBrookeAffectionMeetingAfterSex();
	}
	else if(brookeAffection() >= 40 && Flag("COC.BROOKE_MEDIUM_SCENE") == 0 && pc.tone >= 50 && (pc.hasVagina() || pc.cockThatFits(brookeCapacity()) >= 0)) {
...
public function lowAffectionBrookeMeeting():void {
	clearOutput();
	output("You head your way back to the showers, feeling the sweet ache of your muscles as you easily find your way there.  Brooke is there, already rinsing the day's work out of her muscles.  She hears you coming in, and turns to give you a warm greeting, which you happily return.");
...
public function mediumLowBrookeAffectionOneTime():void {
	clearOutput();
	output("Another good workout and another fine sheen of sweat to wash off.  The day hasn't been so bad so far, and you start to whistle to yourself as you head to the showers, dropping your [pc.gear] off in a locker beforehand.  Glancing out one of the windows, you guess that Brooke ought to be there around this time of day; and as you approach, you hear the sounds of rushing water.  Sure, it could actually be someone else, but you have a feeling it's the Shepherd girl.");

	output("\n\nSure enough, there she is, although she's looking a little... melancholy.  She's staring down at her feet, letting the water rush over her body, not really moving or anything.  One of her ears perks as she hears you coming, and she turns to face you.  <i>\"Oh,\"</i> she sighs, <i>\"hey, [pc.name].\"</i>");
...
public function helpBrookeOut():void {
	...
	if(pc.hasCock()) output(" and being so close to such a woman, you find it difficult to keep [pc.eachCock] from getting too excited");
	output(".");
	...
	output("\n\nEventually, Brooke, as she usually does, is the first to start the conversation.  <i>\"Hey, [pc.name],\"</i> she starts.  <i>\"You asked me once before if I was in the Tel'Adre watch, right?\"</i>  You confirm, but you remind her that she seemed rather evasive of the subject and assure her that she doesn't need to talk about it if she doesn't want to.  <i>\"Oh, no, that wasn't because of... well, it wasn't <b>entirely</b> because of the watch.  I don't have any bad blood with them or anything.\"</i>  You remain silent.");
	...
	if(pc.isHerm()) output("your loins beginning to stir, each and every part of you enticed");
	else if(pc.hasCock()) output("not caring about the stirring in [pc.eachCock] in the privacy of the stall");
	//[if {PC is female}]
	else if(pc.hasVagina()) output("feeling a fire stoke in your [pc.vagina] even under the constant water");
	output(" as you replay the memory of the Shepherd girl's taut, firm ass swaying, just for you.");
	...
	pc.lust(15 + pc.libido() / 20);
	flags["COC.BROOKE_SHOWERED_WITH"] = 1;
	brookeAffection(4);
...
public function mediumLowBrookeAffection():void {
	clearOutput();
	output("After another workout session, you head back to the showers, stopping at the lockers to set down your [pc.gear] and to grab a towel.  You hear the sound of rushing water as you approach and, as usual, you find Brooke in her usual stall.  She's busy lightly scrubbing at her fur, but she easily picks out the sound of your footsteps over the water.  <i>\"Hey, [pc.name]!\"</i> she says, turning to face you with a smile.  <i>\"Right on time, sweetheart.  I could use some company to talk to.  Care to hear a gal pal out?\"</i>");

	output("\n\nYou tell her that you'd be glad to.  You step into the stall next to her, lathering up your hands and begin to rub the suds into your [pc.hair].  She finishes doing the same to her own, before crossing her arms over the wall of the stall between you, facing you with a smile.  She waits patiently while you lather yourself – you could take this opportunity to start the conversation yourself, for once.");
	...
	if (brookeAffection() >= 40 && brookeAffection() <= 70 && Flag("COC.BROOKE_MEDIUM_SCENE") > 0) addButton(3, "Grope", gropeDatBrooke);
}
//Between 16:00 and 18:00, Affection >=40, after first-time sex
public function mediumBrookeAffectionMeetingAfterSex():void {
	clearOutput();
	output("After another workout session, you head back to the showers, stopping at the lockers to set down your [pc.gear] and to grab a towel.  You hear the sound of rushing water as you approach and, as usual, you find Brooke in her usual stall.  She's busy lightly scrubbing at her fur, but she easily picks out the sound of your footsteps over the water.  <i>\"Hey there, sexy!\"</i> she calls, turning to face you with a smile.  <i>\"You're just in time.  The water's plenty warm, but my stall is getting cold.  Care to help warm it up?\"</i>");
	...
	addButton(2,"Her Bod",brookesRockinBod);
	if(brookeAffection() >= 40 && Flag("COC.BROOKE_MEDIUM_SCENE") > 0) addButton(3,"Grope",gropeDatBrooke);
}

//[=Working Out=]
public function talkWithBrookeAboutWorkingOut():void {
	clearOutput();
	output("You ask Brooke how she manages to work out, given her condition.  You can understand the legs and the washboard abs, but she's still packing more heat in her arms and shoulders than most people do in their bodies.");
...
public function talkToBrookeAboutGymFolks():void {
	clearOutput();
	output("You ask about what she thinks of the other gym members.  Surely she's seen them as often as you have, given how frequently she visits the place.");
	...
	//[if {met Lottie}]
	if(Flag("COC.UNKNOWN_FLAG_NUMBER_00281") > 0) {
...
public function brookesRockinBod():void {
	clearOutput();
	output("You say that you recall her talking about wanting to get buff since a young age.  She even got a dumbbell for one of her birthdays, you recall.  What pushed her to want to become such a pinnacle of modern-day fitness?");

	output("\n\n<i>\"Ooh, keep talking sweet things, sweet thing,\"</i> she moans, her eyes drifting dreamily closed.  <i>\"I love it when a " + pc.mf("guy","girl") + " notices all my hard work.\"</i>  She stops, expecting you to keep admiring her body, when you remind her that you asked her a question first.  <i>\"You did?  Oh, right!  Why I... yeah, why I work out.");
...
public function gropeDatBrooke():void {
	clearOutput();
	//output(images.showImage("brooke-grope-her"));
	output("Brooke has an amazing body and she knows it.  She consciously chose to flaunt it in front of you by asking you to 'wash her back'.  The tightness of her muscles; the upright perkiness of her breasts; the taut, flawless features of her thighs... you bet if you smacked her ass, it'd be like slapping clay.");
	//[if (hasCock = true)]
	if(pc.hasCock()) output("  You can only imagine what it could feel like to drive your [pc.cock] between those cheeks, just inches away from you.  The thought makes [pc.eachCock] rouse.");
	output("  The tension is beginning to get a little unbearable and your resolve wavers.");
	...
	if(pc.hasCock()) output("[pc.eachCock]");
		...
		output("your pelvis");
		if(pc.hasVagina()) output(" and dragging her wet tail enticingly through the vulva of your [pc.vagina]");
	}
	output(".  <i>\"I'm game.  Hope you're ready, though; you're gonna need more than a few laps around the track to keep up with me.\"</i>");
	...
	addDisabledButton(0, "Penetrate", "Penetrate", "This scene requires you to have fitting cock.");
	addDisabledButton(1, "Anal", "Anal", "This scene requires you to have fitting cock.");
	addDisabledButton(2, "Tribadism", "Tribadism", "This scene requires you to have vagina.");
	addDisabledButton(4, "Get Laid", "Get Laid", "This scene requires you to have fitting cock.");
	addDisabledButton(5, "Double-Dick", "Double-Dick", "This scene requires you to have at least two fitting cocks.");
	
	if (pc.hasCock() && pc.cockThatFits(brookeCapacity()) >= 0) addButton(0, "Penetrate", penetrateBrooke);
	if (pc.hasCock() && pc.cockThatFits(brookeCapacity()) >= 0) addButton(1, "Anal", brookeAnal);
	if(pc.hasVagina()) addButton(2,"Tribadism",tribadism);
	addButton(3,"Go Down On",goDownOnBrooke);
	if(pc.hasCock() && pc.cockThatFits(brookeCapacity()) >= 0) addButton(4,"Get Laid",getLaidByBrooke);
	if(pc.cockThatFits(brookeCapacity()) >= 0 && pc.cockThatFits2(brookeCapacity()) >= 0) addButton(5,"Double-Dick",doubleDicked);
}
...
public function penetrateBrooke():void {
	...
	output("\n\n[pc.EachCock] rises, engorging with your lust, and it doesn't take long at all until you're ready to fuck Brooke properly.  You release one of her breasts, tickling your fingers down her stomach and her abdomen, over her pelvis and ultimately sliding one into her tunnel, feeling the eager heat and hearing the pleasured woofs of the dog before you.  Your other hand releases her other breast, and she whines pleadingly, feeling your other fingers slide down her side and grip onto her ass, squeezing hard and pressing the one cheek against your still-humping phallus.");
	...
	if(pc.balls > 0) output(" across your [pc.sack] and");
	output(" down your thighs that she's getting wet enough for the both of you all the while.");

	output("\n\nHer body jerks with everything you do to it: out, into your hands while you play with her tits, and onto your [pc.cock " + x + "] with every thrust into her tight tunnel you make.  Her whines and sighs quickly begin to rise and quicken, and with a few louder grunts, she cums onto you, clamping her cunt on you like a vice and doing her very best to coax everything out of you");
	if(pc.balls > 0) output("r [pc.balls]");
	output(".  You're not too far behind her, and with just a few more pumps, you're shooting your load deep into her, pumping and fucking the whole time, making sure to get it in deep.");
	if(y >= 0) output("  Your " + pc.cockDescript(y) + " shoots its own load up and onto her lower back and the base of her tail, snug between the cheeks of her ass");
	//[if (cocks >= 3)
	if(pc.cockTotal() > 1 && y >= 0) output(" while [pc.eachCock] sprays your jizz all over the place: the floor and the walls of the small stall in particular, but also on her ass, onto her tail, and down her legs.  Most of it washes into the drain beneath you, but it'll definitely take a bit of scrubbing for her to get really clean after this.");
	//[if (cocks = 2)
	if(pc.cockTotal() == 2 && y >= 0) output(".");
	if(pc.isHerm()) output("  Your [pc.vagina] cums along with the rest of you, spurting your femcum down your legs, washing the inside of your thighs in your juices.  The combined pleasure between your genitals makes your knees buckle – if only you had something, or someone, fucking you too!");

	output("\n\nYou two stay there, standing under the cold shower nozzle, basking in the glow of each other post-coitus.  You begin to slide out when she clenches hard onto your [pc.cock " + x + "].  <i>\"Just a minute longer,\"</i> she says, and you don't fight her.  You simply enjoy each other for a while, your stiff tool resting in her, and her just relaxing in your embrace.  Eventually, you soften and slip out, and she relents.  <i>\"Good as ever, sweet thing,\"</i> she says, and you return the compliment, giving her a loving, lingering kiss on her neck.");

	output("\n\nYou two spend the next few minutes keeping each other warm in the frozen cold shower water while you finish cleaning up.  You get to drying each other off, flirting and groping some more, but it doesn't progress any farther than that.  She kisses you one last time after you slip on your [pc.gear], and you both leave the gym, looking forward to next time.");
...
public function brookeAnal():void {
	...
	//[if {first time}]
	if(Flag("COC.TIMES_IN_BROOKES_BUTT") == 0) {
	...
	//[if {first time}]
	if(Flag("COC.TIMES_IN_BROOKES_BUTT") == 0) output("  You wonder – is this the first time she's ever had anal before?");
	else output("  She's tight as a vice, but you know she can take you.  She's just getting herself ready.");

	output("\n\nThe [pc.cockHead " + x + "] of your [pc.cock " + x + "] rubs against her rosebud in anticipation");
	if(pc.cockTotal() > 1) output(" while your other member entertains itself between her thighs, bumping and sliding pleasurably across her vulva");
	output(".  She instinctively clenches her ass, and you give her a moment to relax before you stroke her there again.  She loosens, and, almost teasingly now, you prod gently at her hole, implying but not acting just yet.  She takes deep breaths, working to steady herself.  <i>\"Okay,\"</i> she says, looking over her shoulder to you with a smile.");

	//output(images.showImage("brooke-gym-male-analbrooke"));

	output("\n\nYou plan to take it easy on her.  You realign yourself, pointing your [pc.cockHead " + x + "] right at her hole, and slowly begin to push yourself in.  On reflex, she inhales sharply, pressing her upper body into the wall some more.  You reassure her and push in a little deeper, sliding a quarter of your [pc.cock " + x + "] into her anus.  She gasps, but doesn't object.  Another push, sliding in to halfway.  A grunt, and she shuffles her feet slightly, getting more stability.  Another push, nearly to the base.  She gives out a whorish moan, and you wonder if she's at her comfort limit.  You push forward anyway, to your [pc.sheath], and gasp in pleasure and sigh with relief when she doesn't object.");

	output("\n\nYou stay still for a moment, basking in the heat of her ass as she completely takes your [pc.cock " + x + "].  The ring of her sphincter pulls and tightens around you, dragging the skin of your cock with each movement, and her inner muscles clench and massage every fiber of your hard meat.  You shuffle a bit, when Brooke interrupts you.  <i>\"Just a sec,\"</i> she says, rocking her hips some more, this way and that, trying to get a good feel for your dick in her ass.  You let her take her time – it's her ass you're fucking.  She eventually shuffles into a comfortable standing position, and with a nod, gives you the go ahead.");
	...
	output("; your cock stiffens");
	if(pc.isHerm()) output("; your lonely pussy clenches at nothing");
	output("; and you cum into her, blasting rope after rope of your jism inside her.");
	if(pc.cockTotal() > 1) output("  Your second dick flails and erupts with your [pc.cock " + x + "], spraying onto the wall in front of Brooke and across the lips of her sex.");
	//[if (cocks >= 3)]
	if(pc.cockTotal() > 2) output("  [pc.EachCock] blasts in time with your [pc.cock " + x + "], each of your dicks pointing in a different direction.  Your cum gets on everything: Brooke, yourself, the stall – some even reaches as high as the shower nozzle.");
	...
	//[if {first time}]
	if(Flag("COC.TIMES_IN_BROOKES_BUTT") == 0) output("<i>\"I never knew anal could be such a thrill!  We'll have to do that again sometime!\"</i>  You chuckle politely but deep down you're almost dreading your next anal session with her.  You'll have to build up your stamina some more to keep up with her.");
	else output("<i>\"Thanks for putting up with me being such a bossy bitch, [pc.name].  But it's your fault, you know.  You're the one who awakened the anal-hungry monster within me.\"</i>  You're aware of the beast you've created, you tell her, but truth be told, since your first session, it's been a bit easier to keep up with her.  She's just that good of a workout partner.");

	output("\n\nWith some reluctance, she eventually lets go of your spent, flaccid cock, conceding momentary defeat.  The water's long-since turned cold, and you spend the remainder of your time together washing and drying each other off.  She does all she can to entice you some more – she fondles your [pc.cock " + x + "] and kisses your neck sensually, eager for some more anal-play, but you're just too worn from her earlier energy.  She pouts, but gives in, finally allowing you to slip on your [pc.gear] and leave the gym in peace.");
	//Lust reduced to 0, fatigue increased by 15
	pc.orgasm();
	pc.energy( -15);
	Flag("COC.TIMES_IN_BROOKES_BUTT", 1, true);
	brookeAffection(10);
...
public function tribadism():void {
	clearOutput();
	//output(images.showImage("brooke-gym-female-tribadism"));
	...
	//[if (isHerm = true)
	if(pc.isHerm()) output(" your [pc.cock], impishly wishing it were put to use.  You have some other plans this time, though.");
	else output(" your " + pc.skinFurScales() + ".  The tone of her ass almost makes you wish you had a dick to fuck her with, but you're sure she'll give you a hell of a ride all the same.");
	...
	output("\n\nYour fingers continue to tease her: your index thrusts gently and shallowly into her pulling cunt, and you start to use the others to stroke and caress her vulva, pinching her lower lips between the lengths of your digits.  She groans into your mouth at your treatment, and her tongue spazzes and tenses over and over again.  She eventually breaks the kiss and looks you in the eye, her own eyes seductively narrowing and her mouth curled into a sexy grin.  <i>\"Just some foreplay today, [pc.name]?\"</i> she asks, roiling her hips into your touch.  <i>\"You're sure you're not up for anything a bit more adventurous?\"</i>");
	...
	output("\n\nYou close the gap between your bodies, pressing your [pc.chest] into Brooke's own set of tits; pushing your stomach against her tight six-pack; and sliding your [pc.legs] between her thighs until your [pc.vagina] comes into delicious contact with hers.  You both gasp slightly and you keep mostly still for a moment, only jostling a little to draw out the stimulation.  Brooke takes the initiative, and, with a long, sensuous drag, slides the lips of her tight pussy up and across yours, over your [pc.clit]");
	if(pc.isHerm()) output(" and all the way to the base of your now-straining, pre-leaking cock");
	output(".");

	output("\n\nThe electrifying feel snaps you out of it.  You hump her right back, drawing your genitals up and across hers, loving the way she bites her bottom lip from the feel.  Her arms slide down to your own [pc.butt], gripping your cheeks for some leverage");
	if(pc.tone >= 50) output(", and she coos sexily, gripping and pulling at each of your toned, chiseled asscheeks, even giving one as hard a slap as her hand can manage");
	output(".  You start to fuck against her in earnest, pressing and pumping, sliding and bucking against her, loving and living for the sounds and expressions she makes as you love her the way only another " + pc.mf("'woman'", "woman") + " can.");

	output("\n\n<i>\"Fuck yeah!\"</i> she enthuses, spurring you harder into action.  You lift her by her ass, speeding up your fucking, rutting against her vulva with your own [pc.vagina] as quick and as hard as you can.  There's nothing gentle about it: the way your two clits kiss against each other with each upswing sends waves and demands from your [pc.clit] to your head, and you eagerly obey them");
	if(!pc.hasCock()) output(".  Brooke moves her hands up to your tits, her hands groping onto your [pc.chest], her palms pressing onto your [pc.nipples] mindlessly, and her tongue lolls and flags with each buck up against her you make");
	//[if {is not a herm and is lactating}
	if(!pc.hasCock() && pc.lactationQ() >= 200) output(".  Your milk streams from your [pc.chest] as Brooke puts more of her weak pressure on them: it sprays from between her fingers and drips down the back of her hands on across her wrists");
	if(!pc.hasCock() && pc.hasFuckableNipples()) output(".  Feeling the texture of your [pc.nipples] under her hands snaps her out of her reverie enough to think to stimulate you further by pressing into each with a thumb, fucking into and against you with each of your excited thrusts");
	output(".");
	if(pc.hasCock()) output("  Brooke moves one hand to your [pc.chest] and wraps her other around your stiff [pc.cock], pumping it in time with your thrusts against her.");

	output("\n\nWith your whole body heaving into each of your thrusts, it's not long before you're brought to your peak.  Your moan with an animalistic abandon, pressing forward, letting your [pc.vagina] kiss with hers one final time as your juices gush forth, coating and soaking into hers");
	if(pc.isHerm()) output(".  Your [pc.cock] explodes along with its sister, spurting your cum high between your pressed bodies, coating the underside of your breasts and your stomachs, but mostly onto Brooke, who keeps your flow pointed towards her as she pumps you");
	else output(".  Brooke continues to drive against you as she mauls your breasts, using whatever strength is in her fingers to grope what she can, eager to reach her own peak not far off");
	output(".  Brooke's moans rise and climb in her own ecstasy, until she climaxes along with you, returning the favor and spraying your crotch with her fluids, coating your [pc.vagina] with her own gift.");
...
public function goDownOnBrooke():void {
	...
	output("\n\nYou comply, and you drop to your knees with a thud, pressing the flat of your tongue tightly against her button.  She hisses and arches some more, and her knees buckle, while you don't even move.  You wait until she's calmed down a bit before you lick upwards slowly, dragging your tastebuds across her clit, being careful not to overstimulate her too quickly.  From the sound of her voice, your efforts pay off, and she just groans and shudders above you ferociously.  <i>\"Fuck,\"</i> she breathes, and she places her hands in your [pc.hair], weakly trying to pull you into her crotch.");
	...
	if(pc.hasCock()) output("and [pc.eachCock] pulls forward, pointing towards her like a compass to magnetic north, leaking your pre at her scent and ready to fuck her, or something, anything.");
	else output("and you find yourself salivating – such a viable, powerful specimen like herself has you wishing you had a dick to fuck her with.");

	output("\n\nShe moans and rocks her body to her own rhythm, and you go with her, gripping onto her rock-hard calves and quadriceps.  Your upper lip brushes up and across her clit while your lower simply stimulates her.  Her hands grip onto your ears and drag through your [pc.hair] while she moans like a whore and grooves like an actress, and you're part co-star and part prop.");
	...
	//[if (hasCock = true)]
	if(pc.hasCock()) output("  [pc.EachCock] has hardly gone unnoticed this whole while, still standing at attention and begging, absolutely pleading for some action.");
	else output("  Your own [pc.vagina] stands out, puffy and needy against the cold air, and it's certainly not gone unnoticed.");
	output("  Brooke offers to return the favor, more than once, but you decline each time, knowing that if she did, it'd be a cycle between you two that would last for hours.  She understands, but promises you that she owes you one.");
...
public function getLaidByBrooke():void {
	...
	output("You step closer into her, wrapping your left arm around her torso diagonally, so that your hand rests on her right hip, and you reach over her chest with your right hand, grasping her left boob.  You pull her into you, and she coos at the affection, enjoying your loving embrace, but doesn't say anything, allowing you to have your way with her for now.");

	output("\n\nYou roll your hips into her tight, toned ass, ");
	if(pc.hasCock()) output("spearing your [pc.cock " + x + "] between her cheeks wickedly, feeling it rise effortlessly against her soft fur and the smooth, strong muscles of her glutes");
	//[if (hasCock = false)]
	else output("and with some clever maneuvering, you manage to wiggle her tail between your legs, and you feel the soft fur and the smooth, powerful muscles of her tail brush against your [pc.vagina], stimulating you easily");
	output(".  You kiss and nip at her neck, knowing just what she likes, and she moans, knowing that you like hearing her.");
	...
	if(pc.hasCock()) output("your [pc.cock " + x + "] continues to thrust and spear between her luscious ass, demanding attention be paid to it.  Brooke notices, and rocks her hips against you, squeezing your meat teasingly with each downstroke her ass makes on your " + pc.cockDescript(x));
	else output("your [pc.vagina] stokes with each swish her wet, heavy tail makes against you, hungering for something in it, and getting humped against by something as phallic as her tail just makes you hornier.  Brooke can tell from the way you keep dragging your [pc.hips] across her tail, and teases you however she can, by stiffening her tail and whipping it against your [leg]");
	output(".  <i>\"Excited?\"</i> she asks, grinning back at you.  You let your actions speak for themselves, and you hump against her butt some more, dragging out your pleasure.");
	...
	if(pc.hasCock()) output("horndog");
	else output("whore");
	output(" sometimes, [pc.name],\"</i> she says, turning herself around in your arms.  She reaches down between you, ");
	if(pc.hasCock()) output("stroking along [pc.eachCock], hissing inwardly at what she finds, loving the way her fingers, hurt as they are, just manage to glide across your skin");
	else output("stroking along your [pc.vagina], grinning at how your lower lips just manage to wrap around her fingers like they were made for each other.  Her thumb drags across your [pc.clit] intentionally, several times, and she smiles wider at every feminine grunt you make");
	output(".  <i>\"Feeling a little pent up, sweet thing?\"</i> she asks innocently, her fingers working harder on your genitals.  <i>\"It's okay.  Brooke's in a giving mood.\"</i>");

	output("\n\nShe leans in and gives you a quick, but fierce, kiss on the lips, sliding her tongue along your teeth just once before pulling away and lowering herself, sliding her eager tongue across your own neck, down your [pc.chest], across a [pc.nipple], and over your stomach.  She comes to a rest on her knees, bringing herself level with your [pc.hips]; she eyes");
	if(pc.hasCock()) output(" [pc.eachCock], now fully erect and at attention, just for her");
	else output(" your [pc.vagina], loving how you can hardly resist fucking your hips at nothing in anticipation");
	output(", and, licking her drooling lips, leans in.");

	//{male/herm scene}[if (cocks = 1)
	if (pc.hasCock()) {
		//output(images.showImage("brooke-gym-male-getLaid"));
		output("\n\nBrooke opens her mouth and breaths hotly on your [pc.cock " + x + "], hesitating just a moment to feel you jump in her hands, before taking you in.  She only wraps her lips around the crown, suckling gently on you, lapping at any precum that flows out.  Her tongue, though, runs out of her mouth unabated, lathering the underside of your [pc.cock " + x + "] with her saliva, preparing it for the journey it's about to take.");
		if(pc.cockTotal() == 2) output("  Brooke grips onto [pc.eachCock] gently, keeping them sturdy and pointing them right at her lips.  She pumps them alternately, watching how the skin glides with her fingers, while she ponders how precisely to pleasure you.  Throwing caution to the wind, she opens her mouth as wide as she can, and shoves both of [pc.eachCock] into her mouth at once, suckling and laving at your rods gently, preparing both themselves and herself for the journey they're about to take.");
		if(pc.cockTotal() >= 3) output("  Brooke isn't quite sure where to begin, staring at [pc.eachCock].  She grabs ahold of your [pc.cock " + x + "] with one hand, before grasping at your other phallus with another, and seeing your last cock hanging loose and alone, she bends down and leans in, using just her face to lift it and shuck it into her mouth – only by the tip, and very gently, but nonetheless eagerly, spurred on by your jungle of a crotch for her to explore and conquer.");
		if(pc.cockTotal() >= 4) output("  Still, she can't quite tear her eyes from the rest of your cold, wet, and alone penises, and she sighs, almost out of frustration, completely unsure how she's going to accomplish this.");
		if(pc.hasVagina()) output("\n\nDespite her task, she's fully aware of your feminine side, and gently leads one of her hands down the shaft of your [pc.cock " + x + "], reaching back and between your legs for your other half.  You gasp and buck as she finds your [pc.clit] first, and then thrusts her fingers in, up to the first set of knuckles.  She pumps them in time with the dick in her mouth, trying to build up a rhythm with you.");
		...
		if(pc.cockTotal() >= 3) output(" and all over the rest of her");
		output(".");

		output("\n\nShe moans and pushes herself deeper, swallowing more of your [pc.cock " + x + "] into her tight, slurping maw: her head bobs and her tongue tastes, sucking more of your meat into her, eager to please.  You pump your hips with her sucking, your fingers twirling and playing with her hair affectionately, ");
		if(pc.cockTotal() <= 2) output("enjoying the lips of your canine lover as they slide over [pc.eachCock] and how she seems to enjoy herself so thoroughly");
		else if(pc.cockTotal() >= 3) output("enjoying how she tries her damndest to make sure every little bit of you has enough attention and is pleased to the best of her ability, no matter how much effort she requires");
		output(".");
		if(!pc.isHerm()) output("\n\n");
		else {
			output("\n\nSuddenly, she pulls away, leaving [pc.eachCock] out in the cold air and the beating of the shower water.  You look at her questioningly, when she leans forward, ");
			if(pc.balls > 0) output("burying her muzzle underneath your [pc.sack] to push your [pc.balls] out of her way, ");
			output("and gets her tongue to work on your [pc.vagina].  You gasp, unintentionally squatting just a bit to get more of her.  Ever the multi-tasker, she keeps her left hand busy, stroking your [pc.cock " + x + "], making sure no part of you is ever left alone for long.\n\nShe pushes her nose up, rubbing the bridge of her long dog-nose against your [pc.clit], knowing how much that'd get to you.  From her position, she can't reach very deep into your tunnel with her tongue, but she certainly does what she can: she licks and digs at your vulva, tasting every drop of juice you drip, and allows you to fuck against the fine fur on her face as she services you.  With the work she does in your [pc.vagina] and with the way her expert hand continues to masturbate your [pc.cock " + x + "], your body is horribly confused in which way to thrust against, but the sensations lighting up your mind nonetheless make it worthwhile.\n\nEventually, Brooke pulls back, releasing your cock from her hand and stepping out from between your legs.  She rolls her head on her neck a bit – staying in such a way for so long must have been a strain – and gets right back to work, sucking your [pc.cock " + x + "] back into her mouth, deeper and faster this time.  ");
		}
		output("Brooke moans out loud, pressing herself farther in, loving the feel of your dick in her mouth as much as you do.  Your [pc.cock " + x + "] pulses and bounces with each of your heartbeats, and she can feel the jump in her mouth and across her tongue.  She does everything she can to enhance your pleasure, from pressing your [pc.cock " + x + "] against the roof of her mouth, to using the base of her tongue on your [pc.cockHead " + x + "] while she toys with your shaft with the tip, to leaning in and taking all of you in her mouth at once and staying there for a moment.  You let her know that her efforts are paying off with a wordless grunt and jerking your hips: you're close, and she can tell.");
		...
		//[if (cocks >= 3)]
		if(pc.cockTotal() >= 3) output(".  Her hands are practically a blur, up and down on [pc.eachCock] – she's pulled them each down and towards her, aiming them towards their proper target");
		//[if (isHerm = true)]
		if(pc.hasVagina()) output(".  Her fingers are straight as can be, pistoning into you roughly while her thumb frigs against your [pc.clit], and the feel of her enthusiastic fingers inside your [pc.vagina], massaging and caressing your walls, has you seeing stars");
		output(".  You're not entirely positive if she's giving you pleasure because she wants to, or if you're just some perverse toy for her to suck on.  Whatever the case, you're not complaining, that's for sure.  Your hands grip hard onto her head, harder than you intend to, but that seems only to spur her on.  It's not much longer until you cum from her expertise.");

		//[if (cumQuantity <= 349)]
		if(pc.cumQ() <= 349) output("\n\nYou blow your load into her sucking mouth, and she licks and laps at every drop.  Your [pc.cock " + x + "] shakes and explodes with your jizz, coating her hot mouth, but it's never quite enough for her: she doesn't stop her sucking, milking you of every drop of cum you can make.  You fill her mouth once, and she audibly swallows hard, and you fill it once more before you're depleted.  She only calms down with her sucking once she's beyond doubt that you gave her everything.");
		//[if (cumQuantity <= 999)]
		else if(pc.cumQ() <= 999) output("\n\nYou cum into her hungry mouth, and you do not disappoint the cum-hungry canine beneath you.  You coat her tongue with your seed, blowing out her cheeks with your sperm, and she swallows quickly before your next blast fills her mouth to the brim again.  You're a faucet, and she's your sink, taking every drop, clamping her lips securely on your [pc.cock " + x + "] to keep anything from spilling out.  Her throat works hard to keep up, but she manages, and by the time you're finally depleted, she's satisfied – if a little stuffed.");
		else output("\n\nBrooke worked so very hard for your seed, and you're not about to disappoint the cute canine sucking you off.  Your eruption knocks her off guard: your jizz shoots all over her mouth and straight down her throat.  When she's halfway done with one load, you deposit another, not giving her a hope to keep up with your output.  She clamps down on your [pc.cock " + x + "] hard, trying to not let anything escape, but it's hardly effective: seconds later, cum is leaking out from her mouth and out of her nose, before she relents and lets go, letting your [pc.cock] spray her in the face instead.  You cum and you cum, coating her face and her neck with your offering; she groans gutturally, half in love with the scenario and half disappointed she lost the competition between your dick and her throat.");
		//if (cocks >= 3)
		if(pc.cockTotal() >= 3) output("  She strokes along [pc.eachCock] while you orgasm, and the rest of you sprays with the one in her throat.  She does her best to keep [pc.eachCock] pointed somewhere at herself, letting you spray her with your sperm over and over, from different angles.  You're barely able to pay attention with the ecstasy flowing from your groin, but you could swear she was smiling the whole while, moaning along with your every blast.");
		if(pc.isHerm()) output("\n\nThe hand in your pussy is finally rewarded for its diligence, and gets a heaping blast of femcum all over Brooke's hand, flowing down her wrist and dripping off at her forearm.  Your [pc.vagina] pulls hard at her, milking her fingers for all the sperm it wishes it could get.  Brooke doesn't stop her thrusting, even while she tries to focus on [pc.eachCock], sympathetic to your feminine requirements.  You thank her by coming for her some more, soaking her arm even more than it was.");
	}
	//{female scene}
	else {
		//output(images.showImage("brooke-gym-female-getLaid"));
		output("\n\nShe doesn't waste any time and gets to work right away on your [pc.vagina].  She leans in and pulls her face up a bit, drawing her long nose across your [pc.clit] while her tongue goes to work piercing into your tunnel.  You gasp and buckle, one hand on her head and keeping her in place with the other reaching up and grabbing onto the wall of the stall for stability.  Brooke works quickly and deeply into you, and you can feel every twitch and turn of her muscle inside of you, and every bump and movement her nose makes against your stiff button.");

		output("\n\nJust as you pull her face into your crotch, she reaches up to grasp onto your [pc.butt] and pull you into her, as well as she can.  She alternates from fucking you hard with her face – moving quickly and harshly, side to side, front and back, thrusting her long, fat tongue and thrashing it every which way – and taking it a bit slowly, pursing her lips and sensually digging herself as deep into you as she can reach, kissing and loving you, making you squirm with carnal delight.");
		output("\n\nShe withdraws her tongue, making you whimper just a little, before she puts it back to work licking up between the lips of your sex and tapering it off by stroking your [pc.clit] with the tip of her tongue.  She presses and laps at it, ever so gently, and gradually puts more of her tongue onto it, pressing the flat of it onto your button.  She rises on her knees, sliding her tongue down your sensitive [pc.clit] until it's resting on the back of her long doggy-tongue.  From there, she takes long and laborious licks, angling her tongue to work between the folds of your [pc.vagina].  It's a creative trick and it makes you moan like a slut.");

		output("\n\nYou close your eyes and bask in the feel of the warm water washing you both while the toned Shepherd girl works your horny cunt between your legs.  Her tongue works wonders on you, touching and licking you in just the right ways, sending your mind off and letting your body do all the work.  You don't even notice when Brooke removes one of her hands from your [pc.butt], and it's not until her body starts rocking a bit harder against you that you look down, seeing her masturbate herself while she works to bring you to orgasm.");
		if(pc.hasFuckableNipples()) output("\n\nSeeing her frig herself along with you reminds you of your own 'alternative outlets'.  You keep your hand on her head, gently and reassuringly keeping her in place, while you use the other to toy with your [pc.nipple].  The tag-team of sensations between your sensitive [pc.vagina] getting expertly worked and the unusual penetration your fingers provide your [pc.chest] nearly sends you heaving, but stopping isn't even an option to you.  Your pump your fingers, feeling your [pc.nipple] clench and pull at them, and your breath nearly escapes you.");
	...
	if(pc.cockTotal() >= 3) output(", and letting the excess wash from her fur half-heartedly");
	output(".  Moments pass in a completely undecipherable blur to you, and when you can finally make sense of your surroundings, Brooke's standing up and has her tongue in your mouth.");
	...
	if(pc.hasCock()) output(", snowballing what she hasn't swallowed quite yet");
	output(".  She cradles you in her strong arms until she's done, and you pull away from the wall, still a bit wobbly.  <i>\"Was it good for you too, sweet thing?\"</i> she asks.  You barely nod.");
	...
	output("\n\nYou can't help but keep stealing glances at her naked, powerful form, and before you're done drying, you offer her to return the favor.  It's the least you can do, after all.  Brooke looks you over, smiling and biting her lower lip as she imagines it, but, with a frown, declines.  <i>\"Thanks, sweet thing,\"</i> she says, <i>\"but we'd probably just keep fuckin' and suckin' all day if I let you.\"</i>  You ask her what the harm in that is, and she playfully whips you with her towel.  <i>\"You owe me one, though, and don't you forget it.\"</i>");
 	...
 	output("\n\nClean as a whistle and with your [pc.gear] back on you, you wave farewell to your canine lover and leave the gym.");
	//Lust reduced to 0
	brookeAffection(10);
	pc.orgasm();
	//dynStats("sen", -2);
...
public function doubleDicked():void {
	...
	//if {all cockFits  >= 61}
	if(y < 0 || pc.cockThatFits(x) >= brookeCapacity()) {
		output("You hump against her, enjoying the warm, wet feel of her fur against your sensitive monsters.  Brooke moans out, enjoying the way you're using her body, but freezes slightly when one of them begins to press and ply against her butthole.  <i>\"Sorry, sweet thing, but maybe not this time.  Much as I'd love me some double-dickings, one of those things in my butt would be one too many.  I'm down for something else, though.\"</i>");
		clearMenu();
		addButton(0,"Next",gropeDatBrooke);
		return;
	}
	//output(images.showImage("brooke-gym-male-doubledicked"));
	output("You don't bother saying much and choose to just rub yourself against the wet, matted fur of her ass for a bit.  [pc.EachCock] begins to rise and stiffen, thawed by the heat of her body and the friction between you two: your [pc.cock " + x + "] drives itself sensually between her thighs and across her sex, while your " + pc.cockDescript(y) + " enjoys the warmth and coziness of her ass, rubbing across her taint and teasing her anal ring.  You don't make any attempts at either hole – that comes later.");

	output("\n\nBrooke giggles to herself, letting you jerk yourself off with her lower body.  The [pc.cock " + x + "] between her legs caresses and tickles against her pussy while another, " + pc.cockDescript(y) + " crawls devilishly up her butt and between her cheeks.  She squeezes down on both, forcing you to rub a bit harder, which was just the reaction she was aiming for.");

	if(pc.isNaga()) output("\n\n\"<i>Dunno if you reptiles got it better or worse,");
	else output("\n\n\"<i>Got yourself an exotic set, sweet thing,");
	output("\"</i> she mentions, almost casually, while you continue to masturbate yourself on her.  <i>\"");
	if(pc.isNaga()) output("Twice the dick means twice the fun... but twice the work, too.  I'm up to the challenge, though, if you are.");
	else output("Haven't actually seen anyone that's not half-snake sporting more than one.  Mind if I put 'em to the test?");
	output("\"</i>  She rocks her hips a little higher, brushing the ring of her anus across your " + pc.cockDescript(y) + " while driving your [pc.cock " + x + "] further across her labia.");
	//[if {not first time anal}
	if(Flag("COC.TIMES_IN_BROOKES_BUTT") > 0) output("  <i>\"Let's just hope this doesn't turn me any more into a buttfucking freak, huh?\"</i>");
	else output("  Brooke hesitates just a bit as she feels the heat of your dick against her asshole.  <i>\"Well, first time for everything,\"</i> she says to herself.");

	output("\n\nShe teases your dual penises, rubbing her wet holes against your [pc.cock " + x + "] and your " + pc.cockDescript(y) + ", intent on bringing you to full mast before taking any sort of plunge.  Your hands travel down to her toned, trim hips, and you grasp onto them sharply, helping her rock against you.  She 'mmm's at the feel of your rough hands on her muscles, and the way you continue to drive [pc.eachCock] against her.");
	if(pc.cockTotal() >= 3) output("  Your other dicks rise as well, though not as quickly as the first two, jealous that they were chosen instead.");

	output("\n\nYou slide your hands up, sliding your palms over every well-defined muscle and sinew, before they come to rest on her pert breasts, each of them filling your squeezing hands just right.  She sighs with delight while you press and massage against her wet chest, and leans back, resting her hips on [pc.eachCock] and her back onto your own [pc.chest].  She reaches back with her hands, stroking your face, and you kiss her cheek, prompting her to turn so you can kiss her properly.");

	output("\n\nYour tongues work together, arousing you both, preparing [pc.eachCock] for the plunge and preparing herself, sexually and mentally, for the strain she's volunteering to take for your pleasure.  You were both well along before you had started, though: you're not going to get much harder and she's not going to get much wetter (all things considered).  She disengages from you so she can turn herself around, straddling you once more, face-to-face.  Her hips rise along with your [pc.cock " + x + "] and your " + pc.cockDescript(y) + ", until one head is pushing against her impatient cunt while the other rests against her butthole.");

	output("\n\nAs ready as you are, you don't rush her.  You hold her by the ass once again, keeping her steady against your body, while she leans into your [pc.chest], using one hand to stabilize herself and the other to align your tools.  When they're both in position, she takes a deep breath, and starts to slide down.");

 	output("\n\nThe [pc.cock " + x + "] in her quim slides in tightly, but easily.  The " + pc.cockDescript(y) + " pressing against her rim, though, takes a bit more convincing.  She grits her teeth and closes her eyes in concentration, grinding her hips just slightly, back and forth, working the meat into her butt.  Once the " + pc.cockDescript(y) + " pops in, she lets out a gasp, opening her eyes a bit, before focusing once again to work more of it into her.  Her muscles relax and tense sporadically – ");
	//[if {first time anal}
	if(Flag("COC.TIMES_IN_BROOKES_BUTT") == 0) output("it's certainly not a workout she's accustomed to,");
	else output("no matter how much she loves getting fucked in the butt, it's still not a usual thing for her,");
	output(" and for every inch her ass takes, another simultaneously slides into her pussy, stuffing her fuller than she's ever felt.");

	output("\n\nYou can feel her glutes become rigid in your palms whenever she moves, the muscles practically turning to stone.  The pressure on your " + pc.cockDescript(y) + " is both stimulating and a bit painful; you whisper encouraging nothings to her while your hands work on her cheeks, trying to get them to ease whenever she slides down a bit more.  It takes some patience, but eventually, she gets you all into her, in both ends.");
	...
	if(pc.cockTotal() >= 3) output("The rest of you lamentably stands between you, leaking their pre uselessly between your bodies, pleading for some attention themselves.  ");
	if(pc.isHerm()) output("Your [pc.vagina] clenches in time with its neighbor – you can easily imagine what it might feel like to have two dicks in you at once.  You almost wish for it while you're waiting for Brooke's go-ahead.  ");
	output("Brooke's face contorts, not in pain but in concentration, while she fidgets with her ass and lower body, adjusting to both of her holes being so totally full.  After minutes of her positioning and repositioning herself, her eyes finally meet yours.  <i>\"Okay,\"</i> she whispers, <i>\"but... a bit gentler than what we're used to, okay?\"</i>");

	output("\n\nYou accede; it's the least you can do in return.  After all the effort she went through to right herself on you, you pull out from her, ruining all her hard work, and then thrust back in, slowly.  Her breath catches in her throat, but she doesn't object – you repeat yourself, pulling out just as far, and she shucks in some air once you thrust in.  Her pussy squelches and squeezes, but not anything like her anal ring: it's like she's both trying to keep you in and trying to keep you from entering her body, all at once.  Her muscles move and form around your [pc.cock " + x + "] and your " + pc.cockDescript(y) + " with each movement you make, doing their best to shape themselves around both of your intruders.");

	output("\n\nYour fucking keeps steady and slow.  ");
	if(Flag("COC.TIMES_IN_BROOKES_BUTT") == 0) output("Brooke's breathing slowly starts to even out, and she starts to moan enjoyably.  Maybe she's starting to like it?");
	else output("Brooke rocks her hips onto both of your cocks, alternating from taking one deeper than the other.  As much as she just loves sex in general, it's the alien feeling of being so full that keeps her from going any harder.");
	output("  She gasps and jerks with your every thrust, and you grunt, shoving yourself");
	if(pc.balls > 0) output(" down to your [pc.balls]");
	output(" deep into her.  You can practically feel your two dicks rubbing against each other inside her with each thrust.");

	output("\n\nHer strained grunts slowly turn into pleasured moans with each thrust, and you take that as your cue to pick up the pace just a bit.  It's still by no means an easy task, with the way she's putting so much pressure on you, but the ease of her muscles and the lubrication of the water make your thrusting somewhat smoother.  She buries her face into your [pc.chest] while you continue to brutalize her pelvis, muffling her pleasured noises with your body.");

	output("\n\nHer hands go from the wall for her balance, down to your [pc.butt], grabbing a pair of handfuls for herself.  For more leverage, she lifts her right leg and wraps it around your [pc.hips].");
	if(pc.cockTotal() >= 3) output("  Your extra equipment, painfully erect, squashes between your bodies, pointing right at the underside of her boobs and your own [pc.chest].");
	output("  You fuck her a bit more forcefully, but not any faster, for both your sakes.  She moans into you: her body's adjusted to the feel, and she's more receptive of your actions.");
	...
	output("\n\nThe intense pressure makes it just a little harder-going, but the feel of having her entire body trap all of you, milking you for the cum it craves, spurs you into your own throes of orgasm.  You grunt and lean forward, resting your head on her shoulder, as [pc.eachCock] sprays forth, blasting into her from every angle.  ");
	//[if (cumQuantity <= 349)]
	if(pc.cumQ() <= 349) output("Your jizz blasts into her, filling into her womb nice and comfortably and shooting deep into her bowels.  Your both shake: her from the feeling of getting two doses of your cum at once and you from having your [pc.cock " + x + "] and your " + pc.cockDescript(y) + " pleased so uniquely.  Your orgasm goes for two, three, and four blasts, before tapering off to nothing inside of her.");
	//[if (cumQuantity <= 999)
	else if(pc.cumQ() <= 999) output("You cum hard, shooting your potent spunk into her thirsty cunt and her slurping asshole.  Her orifices fill out quickly, stuffed full, but you don't relent, shooting more and more into her.  She groans out, feeling so much sperm pool inside her, and her breathing shortens while she tries to focus on the feeling.  By the time you've finally finished inside her, your creamy jizz leaks from each of her holes.");
	else output("Once your orgasm finally, powerfully overtakes you, a glimmer of a thought questions that maybe finishing inside her wasn't the best idea.  Your cum rockets into both her pockets and then some: the [pc.cock " + x + "] in her vagina fills it to the brim in just a few spurts, and the " + pc.cockDescript(y) + " in her ass shoots and shoots into the rest of her, distending even her tight stomach and ruining her form.  She grunts hard, gripping onto you while you fill her out, but the tightness from both your cocks makes keeping herself composed a harder task than she anticipated.  Your orgasm goes for what seems like minutes, and when you're finally done, she looks primed to pop like a balloon.");
	if(pc.cockTotal() >= 3) output("  Your [pc.eachCock] sprays from between you, launching its own jizz up and uselessly between you, forming a warm, sticky blanket between your stomachs.  Some of it even crawls up between her cleavage, sticking them together from the inside, and with her pressed against you like this, it can't wash off right away.");
	if(pc.isHerm()) output("\n\nYour [pc.vagina] pulses and cums, spraying its juices down your legs and splashing onto the floor.  With every thrust you make into her while your fairer sex orgasms, you try to imagine something in it, to stimulate it and draw yourself out longer – with both your hands occupied and Brooke focused on herself, your imagination is all you have.  It works well enough, and with every clench, your [pc.vagina] rocks just a little more.");

	output("\n\nThe two of you stay like that for just a moment, basking in your afterglow with each other.  Brooke's face is still lodged firmly in your [pc.chest], and, while you can still feel the errant twitch and squeeze from either part of her, her breathing calms and evens.  Her arms are wobbly and her legs are shaky from the unreal session between you");
	//[if (cumQuantity >= 1000)
	if(pc.cumQ() >= 10000) output(" – she's probably going to have trouble doing much of anything for a day or two until she 'digests' her load");
	output(".  Wearily, she pulls herself away, just enough to look you in the eye, and, without a moment, leans in to kiss you, tongue and all.");

	output("\n\nYou both stay like that in the shower, long after [pc.eachCock] has deflated and fallen limp.  It's not until the shower turns frigid that you decide to finish cleaning yourselves and get a move on out of the gym.  Despite how cold the water is, you keep each other warm; she gets to your hard-to-reach places and you reciprocate, getting at all the hard spots under her fur.");
	...
	//[if {first time anal}
	if(Flag("COC.TIMES_IN_BROOKES_BUTT") == 0) output("but Brooke eventually breaks the silence.  <i>\"Hot damn,\"</i> she finally says, rubbing her hands over her sore ass.  <i>\"Never knew anal sex could be such a trial.  And getting it in the cooch at the same time?  Man, out of this world.\"</i>  She drags her index finger down your [pc.chest] playfully, giving you a sultry smile.  <i>\"I hope you didn't think it was a one-time thing, sweet thing.  We're going to have to try it again sometime.\"</i>  You smile back at her and tell her you're looking forward to it – but in the back of your mind, you wonder if you might have just awakened some sort of beast.");
	else output("not that you need words.  Brooke tells you everything you need to hear by wrapping her towel around you both, under the pretense that she's helping you dry, but from the lewd way she's rubbing her body against you, and from the way her eyes narrow and curl, pursed lips coming within inches of your own, or your [pc.nipples], she's more than ready for a repeat performance.  From the stirring in [pc.eachCock], you're almost ready to take her up on that.  But, dredging up all the willpower you have, you manage to rebuff her – and her pleading dog eyes once you do.  You have other things to do today.");

	output("\n\nThe rest of your drying continues without much incident, and before long, you have your [pc.gear] back on you and you're out of the gym, heading back towards camp.");
	//Lust reduced to 0
	pc.orgasm();
	brookeAffection(10);
...
//flags["COC.BROOKE_MEDIUM_SCENE"] == 1
public function mediumAffectionOneTimeEvent():void {
	clearOutput();
	output("You wander your way back into the showers, stopping once again at the lockers to deposit your [pc.gear].  Right on time, you hear the tell-tale sound of rushing water just up ahead, and as usual, you see Brooke washing at her fur lightly in the first shower stall.");

	output("\n\nWith your first step, her ears perk, and she turns to you.  She smiles once more; you can't tell at this distance, but her smile looked a little... devious.  Or maybe playful.  You're not sure what the difference is.  <i>\"Hey, [pc.name],\"</i> she says, giving you a wink.  <i>\"The water's warm, but my stall is cold... care to come and warm it up?\"</i>");
	...
	output("\n\nShe gets to work right away, rubbing at your shoulders, soaking the water into your skin.  You don't know if she even has any soap on them, but you don't particularly care.  You return the motions, getting at her usual spots, just from a different perspective.  <i>\"I was on the jogging track earlier today,\"</i> she says, nearly a whisper, leaning in close, letting her breasts squeeze against your [pc.chest], her thighs pressed tightly together and leaning into yours, <i>\"and I thought to myself, 'man, I can't wait to talk to [pc.name] again today.  I should ask [pc.himHer] for another massage.  [pc.HeShe] really knows how to work a pair of shoulders.'\"</i>");
	//[if (hasCock = true)]
	if(pc.hasCock()) output("  You feel [pc.eachCock] beginning to wake from her proximity, and in just a few seconds, she'll know it.");
...
public function brookeSpecialMediumSceneContinued():void {
	clearOutput();
	var x:int = pc.cockThatFits(brookeCapacity());
	if(x < 0 && pc.hasCock()) x = pc.smallestCockIndex();

	flags["COC.BROOKE_MEDIUM_SCENE"] = 1;

	//{Male only}
	if (x >= 0 && !pc.hasVagina()) {
		//output(images.showImage("brooke-gym-male-specialscene"));
		output("Her grip isn't very strong on your shoulders, relying on keeping you pressed against the stall for any traction against your body.  She grinds herself against you, her firm, plush tits dragging over your [pc.chest] and her hot slit running along the skin of your [pc.cock] with each motion.  Her tongue doesn't stop, dancing energetically in your mouth, running and playing with your own with every motion.  You run your hands along the swell of her ass, groping and lifting, humping along with her, building the excitement between you both while avoiding penetration for now.");

		output("\n\nEventually, she breaks the kiss, pulling back for air, but she doesn't stop for a moment as she begins kissing your cheek, trailing herself down to her neck, tongue laving and sweeping across your wet skin.  Her hands trail to your sides, under your pits and across your ribs, stroking your skin delicately.  You do the same, moving your hands from her toned ass to her thighs, squeezing what you can as you sensuously rub down her legs.  Your [pc.cock " + x + "] continues to saw tantalizingly between her thighs");
		var y:int = pc.cockThatFits2(brookeCapacity());
		if(pc.cockTotal() == 2) output(" while your second phallus brushes higher, rubbing itself through the wet fur on her stomach between the gorge of her abs");
		//[if (cocks >= 3)
		if(y >= 0 && pc.cockTotal() >= 3) output(" while [pc.eachCock] finds themselves either driving themselves higher, through the wet fur on her stomach and across each of her tensed abs, or joining the first in between her legs, pressing and pulling with her needy humping");
		output(", and you can feel the heat, the need, pour from Brooke's pussy as she continues to grind on you.  You can hear her pant as she continues to kiss your neck, whimpering at your denial to penetrate her and moaning with each brush across her vulva.");

		output("\n\nShe needs it as badly as you do, and you're not about to deny it.  With your grip on her thighs, you bend your [pc.legs] just slightly, lining your [pc.cockHead " + x + "] up with her target.  She stops breathing, tensing slightly, waiting for your thrust, and you oblige.  Straightening your legs and pushing forward, you enter her, pushing in only slightly so you can adjust.");
		...
		if(pc.balls > 0) output("[pc.sack] resting gently against her crotch");
		else output("crotch resting gently against hers");
		output(", her fur lightly tingling the skin.");

		output("\n\nYou remain there for just a moment, embracing your lover in the shower, enjoying being inside her, especially after putting up with her teasing for so long.  You don't wait long, though, and begin to pull out just a bit before sliding back in.  The water makes it all very smooth, and you continue to gently saw into her this way.  <i>\"[pc.name]\"</i> she says, just barely loud enough over the running water.  You look down, and she looks up, your eyes meeting once again.  <i>\"I'm a big strong gal,\"</i> she says, smiling,<i>\" and you're a big strong guy.  Fuck me, [pc.name].  Don't be gentle.\"</i>");

		output("\n\nA part of you was wishing she'd say that.  You reach down, around her thighs once more, and you lift her off the ground.  She gasps in surprise, as, impaled on your [pc.cock " + x + "] and lifted off the ground, you walk forward and slam her into the other wall of the stall, rattling the whole setup.  With your new leverage, you pull your hips back and slam them back home, rocking the whole shower room.");

		output("\n\nBrooke nearly yelps out, but quickly bites her lip, trying to keep her composure as you pull back and thrust in once more.  You put everything you have into fucking the Shepherd girl: gripping her tightly, you pound into her wet, warm, inviting snatch, feeling every worked muscle lave and caress your [pc.cock " + x + "].");
		if(pc.balls > 0) output("  Your [pc.balls] swing away with you, slapping onto her pussy with every inward thrust.  You feel no pain, focused only on fucking this girl raw, knowing she's strong enough to take you.");
		if(pc.cockTotal() == 2) output("  Your other dick swings freely, cold and alone in the shower between you, occasionally spurting pre into the air and onto Brooke's fur.  It's quickly washed away with the water.");
		if(pc.cockTotal() >= 3) output("  Your other dicks swing freely, cold and alone in the shower between you, occasionally spurting pre into the air and onto Brooke's fur.  They're quickly washed away with the water.");
		...
		if(pc.hairLength >= 1) output(", running her fingers through your " + pc.hairDescript());
		output(".  <i>\"Fuck me, [pc.name]!  Just like that!  Give me that hard dick!  Fuck me!\"</i>");

		output("\n\nThe whole gym probably heard.  You might even have visitors by now.  You don't give the littlest damn.  You just grunt along with her, flexing your own stomach to reach up and drive more [pc.cock " + x + "] into her.  She grows louder and more active the farther up the wall you drive her, until eventually, she orgasms, holding you in her as she resists the urge to scream out.  She uses her hands to drive your face into her chest as her rocking begins to slow – even with her weak grip, she's still managing to pull you in fairly hard.");

		output("\n\nHer orgasm reminds you of your own, coming up very quickly.  She spends only a moment to herself as her world begins to calm down from the shattering orgasm you just gave her, and she resumes her rocking, intent on returning the favor.  <i>\"Come on, [pc.name],\"</i> she practically growls at you.  <i>\"Give it to me.  Fuck me harder.  I want your cum, [pc.name].  I want it all.  Make me your bitch, [pc.name].  Fuck me like a whore.  Fuck me, you sexy fucking beast, <b>fuck me!</b>\"</i>");

		output("\n\nYou put everything you have into bringing about your release.  Your muscles begin to ache.");
		if(pc.balls > 0) output("  Your [pc.balls] begin to churn.");
		output("  Your heart races, and your [pc.cock " + x + "] pulls and tightens in her pussy.  With some more encouraging, demanding words, and with a little more activity on her part, you finally cum, giving Brooke everything she asked for.");
		...
		else output("\n\nYou burst inside of her, immediately filling her to capacity with your intense orgasm.  Your cum can't blast through your [pc.cock " + x + "] into her fast enough, thickening your dick with each pulse as another glob passes through and into her hungry cunt.  Brooke easily gets what she wants, and then some: she grinds her hips greedily on yours, her pussy voracious for your seed, but despite her best efforts, a small trickle of semen escapes her tight pussy.  She nearly yells out again, feeling your seed sink deeper into her body with every pulse, cumming herself once more – and then once more.");
		if(pc.cockTotal() == 2) output("  Your other prick tenses along with the first, pointing up and along the tight abs of the Shepherd girl, jizz erupting from the tip and blasting her on the underside of her perky tits.  If she even notices, she certainly doesn't seem to care.");
		if(pc.cockTotal() >= 3) output("  [pc.EachCock] spurts and flails along with the first, painting the stall white with your warm sperm.  It lands on the walls and the floor mostly, but more than a good deal lands on Brooke and into her fur.  She's so out of it from her own orgasm that she barely even notices.");
	...
	//{Female only}
		...
		//output(images.showImage("brooke-gym-female-specialscene"));
		output("You can barely keep up with her sheer, wanton desire as her hands grope your body and her tongue conquers your mouth.  You can tell from her squeezing that she's trying to lift your left leg up and across her hips, but, unable to grip or lift with her hands, all she can do is grunt in frustration as she instead takes to humping into you.  You do her a favor and wrap your leg around her anyway.");

		output("\n\nHer right hand strokes up and down your [pc.hips] and [leg] while her other arm wraps around the small of your back, using her powerful forearms to pull you in close to her and press your abdomens together.  All the while she never stops humping, and her tongue doesn't stop fighting your own, forcing you both to breathe through your noses.  Her wet, pert C-cup breasts press against your own [pc.chest] – she mauls your whole body, giving you no quarter or leeway, absolutely <i>intent</i> on having each and every part of you.  You hardly resist, though, instead melting into every action and ministration the needy Shepherd girl makes, rocking back with her, pulling her closer.");

		output("\n\nBrooke suddenly pulls her mouth away, leaving you breathless and panting, but she never even slows – her tongue travels from your lips to your neck, and from your neck to your collarbone and down to your [pc.chest].  Her waist closes in, pressing her tight cunt against your lower stomach while her inner thighs clamp onto your right leg.  Her hungry mouth easily finds your left nipple while her hand pinches and plays with your right, and she begins to earnestly hump against your leg, rubbing you both in all the right areas.");
		...
		output("\n\nHer hands travel low, grasping and pulling at your [pc.butt].  You lean into her, melting back into her kiss, and she presses her lower stomach forward.  It's a little awkward from your standing positions, but she manages to hook herself around your pelvis, and with a few experimental humps, she finds your [pc.vagina] with her own tight, hot box, and from there, she starts roughly grinding herself against you.");
		...
		output("\n\nYou two share a more calm moment as she continues to make out with you in the shower.  When she pulls back, she looks deeply into your eyes, her brown irises sparkling.  She can't help the huge grin slowly spreading over her muzzle.  <i>\"Holy shit,\"</i> she eventually says – not exactly the most romantic phrase ever.  <i>\"You're amazing, [pc.name].\"</i>");
		...
		output("\n\nShe leans her face in close, looking at you almost predatorily.  <i>\"I might even take that as competition.  We'll see how long you last next time, [pc.name].\"</i>  You tell her that she's on, and she smiles, planting another, shorter, lovelier kiss on your lips.");
	...
	//{Herms only}
		...
		output("Your hands waste no time in feeling and groping all over your new canine lover's body, appreciating every nuance of the tight muscles you feel under her short fur.  Her strong shoulders; her thick upper-back; her taut lower-back; and her tight ass.  [pc.EachCock] rises in the excitement, reaching up between your bodies, scratching itself across her fine belly fur.  Your [pc.vagina] likewise quivers in excitement, readying itself, though in Brooke's ravaging passion, you wonder how you'll be able to satisfy all of yourself, if you even can.");
		...
		output("\n\nShe keeps you pinned against the stall wall as she lowers her body, bringing her head to your [pc.chest].  Eagerly, she begins licking the flesh of your breasts, using her tongue to heft and drag the fat of them with each lick, slowing and moaning every time she hits a [pc.nipple].");
		//[if (hasNippleCunts = true)]
		...
		//[if (isLactating = true)]
		if(pc.lactationQ() >= 200) output("\n\nYou feel your milk slosh in your [pc.chest] and trickle out from Brooke's insistency.  She doesn't realize it immediately, the milk diluting with the shower water, but soon enough, she pulls back, licking her lips inquisitively, her eyes squinted in thought.  She squeezes the nipple again, and her eyes widen when she sees the white fluid drip from you in response.  Grinning, she dives back in, hungry and intent for more.");

		output("  She finally drops her hands down, one wrapping and caressing carefully, diligently, around your [pc.cock " + x + "] while the other trails the cleft of your [pc.butt], a single claw running between them and towards your taint.  She doesn't waste a moment when she finds your [pc.vagina] instead of plain skin, apparently not unfamiliar with multitasking.");

		output("\n\nYou grind your hips on Brooke's hands as one glides easily up and down your lubricated [pc.cock " + x + "] while the other begins to gently pump into your [pc.vagina], all the while her tongue continues to bathe your [pc.chest].  You cradle her head in your own hands, unable to do much else with how she's treating your body.  Finally, she gets down to business, dropping to her knees and bringing herself face-to-face with your equipment.");

		//[if (cocks = 1)]
		if(pc.cockTotal() == 1) output("\n\nShe can hardly contain herself as she sets her tongue to work once again.  She opens wide and sucks you into her, clamping her mouth gently shut around your meat, minding her teeth.  She takes it slowly, in contrast to her apparent eagerness: she eases her head down, slowly taking it more and more, until her nose presses against your pelvis and she slowly begins to rise back up.  However, all the while, her tongue is going a mile a minute, pressing and playing, teasing and tasting, working every nerve you have in your sensitive [pc.cock " + x + "].  The attentions make you melt into her mouth, and the sounds of her 'mmm'ing with every pass just get you that much more excited.");
		//[if (cocks = 2)]
		if(pc.cockTotal() == 2) output("\n\nShe looks almost ecstatic, seeing [pc.eachCock] up close.  She keeps her left hand busy with your [pc.vagina], and puts her right back to work stroking your [pc.cock " + x + "] while she eagerly swallows your other penis.  Her left hand thrusts into you, pressing and searching your pussy, gently trying to find your G-spot.  Her right hand pumps your [pc.cock " + x + "] eagerly, sensuously sliding up and down, gripping the base as tight as she can in one motion then rubbing the head carefully and expertly the next.  Her mouth takes its time crawling down the length of your second erection, her head going slowly but her tongue excitedly wrapping and worshipping it on the inside.  Her expertise is impressive, really, and you voice your appreciation with dumb, happy moans as she does everything she can to you.");
		//[if (cocks >= 3)]
		if(pc.cockTotal() >= 3) output("\n\nShe looks at each of your [pc.eachCock], wide-eyed – she almost looks lost, staring, entranced by the multitude of dicks you're sporting.  She whimpers, nearly pulling her left hand away from your [pc.vagina] so she can use everything she has to pleasure your masculinity, but resolves instead to keep it there, knowing just how important it is to not ignore your femininity.  She gets to work with what she has: her right hand pumps your [pc.cock " + x + "] quickly, pleasurably and deliberately, while her mouth takes your second dick, her eagerness apparent, gliding as far down as she can and keeping herself there for but a moment, her tongue lathering and laving, worshipping as much as it can, before she pulls away reluctantly and takes your third prick into her mouth.  She attempts to pleasure as much of you as quickly as she can, her whole upper-body working overtime on your [pc.eachCock].  You're too pleasure-drunk to stop her, and from the look on her face, she's having the grandest time anyway.");
		if(pc.balls > 0) output("  Every once in a while, her head dips low, suckling on the meat she has, and she slips her tongue out, running across what she can't reach and cupping your [pc.sack] with the tip of her long, heavy dog tongue.  Occasionally, her left hand slips free from your snatch to get a quick fondle of your [pc.balls] before dutifully returning to work.");

		output("\n\nYou tremble and whimper, approaching your edge with the things Brooke is doing to you.  She's very skilled, and she uses everything she knows to bring you to your climax – seemingly, anyway.  She recognizes the telltale signs of your approaching orgasm, and she angrily stands up, bringing herself to your eye level.  <i>\"Don't be rude,\"</i> she chastises, reaching around behind you and gripping onto the stall wall you're leaning on.  She lifts one leg and wraps it around your [pc.hips], lining her hot, tight twat with your [pc.cock " + x + "] but refusing to make the first move.  <i>\"I know you weren't thinking of finishing without me.\"</i>  You assure her that you wouldn't dream of it.  <i>\"Good.  Get to work.  You have some ground to make up.  Show me what your hot bod can do.\"</i>");

		output("\n\nYou grin and take the invitation gladly.  You grip onto her own hips tightly, admiring the feel of her body under your palms, as you thrust into her.  She gasps and stutters, mouth dropping agape once your wet [pc.cock " + x + "] is thrust imperiously into her, through no small effort on your part.  Her bravado wavers and her arms buckle as her weak grip tries to keep her steady.  You, though, just stand there for a moment, enjoying the feel of her velvet cunt accepting you as it had.  She's tight, amazingly so – for someone with so much experience pleasuring a herm like yourself, she seems almost virginally tight.");
		...
		output("\n\nHer request not-forgotten, you put everything your body has into fucking the Shepherd girl.  You're determined to fuck her until she regrets doubting you.  Your hands travel down to her ass, gripping hard for leverage, as you pump your [pc.cock " + x + "] in and out with everything you have.  Brooke's grin fades underneath her swishing light-brown hair and beneath the sheen of the water still clinging to her face while you ravage her.  Her tongue lolls and spittle sometimes convulsively flings from her slack mouth.  Her hands move from the stall wall to your neck, and she looks down at you – her expression is difficult to read, as focused as she is on the pounding you're giving her.  Nonetheless, her brown eyes never leave yours.");

		output("\n\nHer breathing comes out ragged, rising in pitch the longer and harder you go without slowing the littlest bit.  Her breasts bounce with the rest of her, and eventually, she lifts her other leg, forcing you to carry all of her weight in your hands.  You grunt with her, fucking her, feeling her vice down on your [pc.cock " + x + "], her voice ascending the closer she gets.  <i>\"Fuck!\"</i> she says occasionally, not having the energy to say it much louder than her senseless moaning.");

		output("\n\n<i>\"Fuck yes, [pc.name],\"</i> she chants every once in a while.  <i>\"Fuck me, you dirty herm bitch.  Show me what you got.  Cum in me, [pc.name], fucking give me all you got.\"</i>  You say nothing in response, and simply keep doing what you're doing.");
		//[if (hasBalls = true)
		if(pc.balls > 0)
		{
			if (!pc.hasStatusEffect("Uniball"))
			{
				output("  Your [pc.sack] swings with every pump, slapping onto the firm skin of Brooke\'s vulva every time you bottom out.  You feel no pain, and in fact the smoothness of her fur around her genitals tickles the cum out of your [pc.balls] all the faster.");
			}
			else
			{
				output("  Your [pc.sack] is to tight and smooth to swing, but it still slaps against the firm skin of Brooke\'s vulva every time you bottom out.  The smoothness of her fur around her genitals tickles the cum out of your [pc.balls] all the faster.");
			}
		}

		if(pc.cockTotal() == 1) output("  You drill into that canine pussy, giving Brooke everything you have.  Your body burns and your cum churns the longer you go.  Her voice ascends from pitiful puppy whining to the jubilant cries of a woman well fucked, and the rise of her yells bring with them the rise of your cum.");
		if(pc.cockTotal() == 2) output("  Your second penis swings, cold and alone, between your body and Brooke's as you continue to saw into her with your [pc.cock " + x + "].  The tip points up to the cleft of her swinging, pounding breasts, ready to erupt and coat the underside of her tits with your offering.  She knows of it, and she does what she can to pleasure it, but even her well-defined six-pack can only do so much.");
		if(pc.cockTotal() >= 3) output("  Unfortunately, as much as both you and Brooke want to pleasure as much of you as possible, she can only do so much.  [pc.EachCock] thrusts into nothing between you, each of them stiff and pointed right up at her face – if they can't cum inside her, then they'll jealously paint her white on the outside.");

		output("\n\nShe howls to the ceiling as her body convulses, unable to contain herself anymore.  She orgasms hard, squeezing your [pc.cock " + x + "] hard, making your last few thrusts all the sweeter before you cum yourself.");

		//[if (cumNormal = true)]
		if(pc.cumQ() <= 250) output("  Your cum erupts into the Shepherd girl, pumping into her hard, and she continues to coax as much as she can with each ripple her orgasm gives her.  It's the grandest orgasm you've had in some time, and it takes a good few moments before you finally calm down.");
		//[if (cumMedium = true)]
		if(pc.cumQ() <= 500) output("  Your fluid pumps into her hungry cooch with abandon: each of your heartbeats begets a pulse through your [pc.cock " + x + "] and into her womb.  She continues to shout with each of her convulsions, her voice slowly and steadily descending as her own orgasm subsides.  It takes you a moment to calm down, yourself.");
		//[if (cumHigh = true)]
		else if(pc.cumQ() <= 1000) output("  Urgently, your cum floods into her, pumping into her with each heartbeat.  She feels it all and squirms with each new squirt she manages to pull out of you – she continues to rock her hips, her howling from her orgasm keeping up with your own.  She holds you tight with her powerful legs, intent on not letting a drop go.");
		//[if (cumVeryHigh = true)]
		else if(pc.cumQ() <= 3000) output("  You orgasm powerfully, the cum ripping through your [pc.cock " + x + "] and into her with a force she's never experienced before.  Her howling is cut short as she doubles over, pressing her tits into your face, her hips desperately rocking into yours every time you deposit another spurt.  Her stomach begins to lose some definition from the sheer amount of cum of pumping into her, but from how she continues to convulse even long after her initial orgasm, she seems to like it.");
		//[if (cumExtreme = true)]
		else output("  Your orgasm is undeniable, the initial blast into her womb barely even a hint of the torrent following behind it.  Her howl is cut short as she feels her womb fill so full, so immediately, and then she howls again, even louder, as you continue to relentlessly pump your semen into her.  Her stomach not only loses its six-pack definition, but it actually begins to balloon slightly, nearly suggesting that she's not all muscle.  Her orgasm either never stops, or she has more than a few – you're not positive.");
		//[if (cocks = 2)]
		if(pc.cockTotal() == 2) output("  Your second phallus erupts in time with its brother, spraying into the air uselessly, but coating the underside of Brooke's breasts just as it promised it would.  Your cum erupts in equal parts from both dicks, soaking into her fine fur, blending in with the white strip of fur running down her torso.  She'll definitely need a second rinsing after this.");
		//[if (cocks >= 3)]
		if(pc.cockTotal() >= 3) output("  [pc.EachCock] not inside her sprays out in time with their lucky brother, aimed right at her hunched, humping body and coating it all with your cum.  Her tight thighs, her taught belly, her ribs, her round tits, and even as high as her collarbone and her face each get a hefty blast of your cum, fulfilling their unspoken promise.");
		//if (isSquirter = false)
		if(pc.wetness() >= 4) output("  Your poor, lonely pussy erupts with the rest of you, coming in a torrent.  It robs whatever energy remains in your body for itself, and you buckle your knees, nearly falling with Brooke still in your arms.  You become aware of every sensation between your legs – from the water dripping down between your legs to the feel of your own heartbeat energizing your vulva.  You simultaneously crave something long and hard to drive into you, and want nothing of the sort, afraid the slightest of stimulation will send you reeling once more.");
		//[if (isSquirter = true)
		else output("  Whatever's left of your energy goes into your neglected cunt as it erupts itself, spraying its own juices down your legs and onto the shower floor.  Your pussy quivers and cums long and hard, squirting in time with [pc.eachCock], the warm fluids mingling and contrasting with the cooling shower water.  As it winds down with the rest of you, you're conflicted between reaching down and giving your feminine half some special attention, and simply not touching anything, afraid that the sensations would be too much for your addled body.");
		...
		output("\n\nFinally turning the frigidly cold water off, you each make a show of drying yourselves off.  Despite how bow-legged Brooke is moving and despite the wear your [pc.cock " + x + "] is feeling after such a workout... well, to put it one way, you can't wait until you meet her again after this.  Miraculously, you both manage to keep your hands to yourselves, and the rest of your drying ends without incident.  She blows you a kiss before she leaves the locker room, and, after a vigorous dry, you put your [pc.gear] back on and leave the gym.");
		// Lust reduced to 0, sensitivity decreased by 2, fatigue increased by 10
		pc.orgasm();
		//dynStats("sen", -2);
		pc.energy( -10);
...
public function specialHeckelAndBrookeEncounter():void {
	clearOutput();
	flags["COC.BROOKE_MET_TODAY"] = days;
	output("You stroll into the gym, looking for Heckel, but she's not at her usual routine, running laps around the gym.  The gym itself is a little bare; there aren't a lot of people using the machines dotted around the room.  Perfectly good and ready machines, barbells, bench-presses, et al sit ready and waiting for someone to test their mettle on them.");
...
public function leaveHeckelEncounter():void {
	clearOutput();
	output("Well, whatever.  You came here looking for Heckel, and she's not here, as far as you can tell.  You turn around and leave the gym, without investigating the cause of the commotion.");
...
public function nopeOutofBroke():void {
	clearOutput();
	output("You shake your head at Heckel, not feeling like playing second fiddle today.");
...
public function submitHeckelXBrookeThreesome():void {
	clearOutput();
	//output(images.showImage("brooke-gym-heckelbrooke3some"));
	output("Once you snap out of it, you comply, hastily removing your [pc.gear] and leaving you just as naked as her.  Although you never really agreed to something like this in the first place, you <b>did</b> come here to see Heckel for a reason, and seeing the two of them like they are <b>is</b> really hot...");
	...
	output("\n\n<i>\"Stop,\"</i> Heckel commands, and you both comply, out of breath and panting from your services to the top-dog of the day.  <i>\"Brooke, you stand.  [pc.name], stay down.\"</i>  You obey, curious as to what she has in mind, while Brooke stands, still a little wobbly from the workout before.  She's still wearing her workout shorts, and you can easily tell from your viewpoint that they've been soaked through with her arousal.  Whether or not Brooke has a submissive side, she's definitely getting off on the treatment.");

	output("\n\n<i>\"Take off her shorts for me, [pc.name].  Slowly.\"</i>  You do so, gripping onto the hem of Brooke's shorts and slowly pulling them down from her waist.  The juices from her puffed cunt drip down with the cloth, and immediately the heady smell of her arousal assaults your nostrils.  Heckel takes a long drag of the smell herself, even from where she's sitting.  <i>\"That's what I like,\"</i> she says lowly, and you can see Heckel's cock bulge even longer and thicker, with the pole reaching a good foot and a half and the knot billowing to the size of a melon.  <i>\"On all fours, bitch,\"</i> she commands, pointing to the floor, <i>\"and [pc.name], you sit on the bench.\"</i>");
	...
	output("\n\nAt that, Heckel pulls back, and, with lightning reflexes, lines her shot up with Brooke's waiting, hungry hole and thrusts forward.  No warning, no foreplay.  Brooke cries out in surprise, paralyzed with the sudden force and pleasure at finally being fucked by the alpha of the day.  She ignores your [pc.croch], too out of it from the sudden penetration to do as Heckel said.  In response, Heckel thrusts forward much harder, roughly shoving a good third of her dick into Brooke's cunt.  <i>\"<b>Now!</b>\"</i> she demands, pushing her skull forward into your crotch, and finally, Brooke gets the sense to obey.");

	output("\n\nResting her forearms on the bench for support, Brooke lunges forward, ");
	if(pc.hasCock()) output("engulfing your [pc.cock] into her warm, busy mouth");
	if(pc.cockTotal() == 2) output(" while she strokes your second dick with a free hand, wobbly balancing herself on her other, tired arm");
	if(pc.cockTotal() >= 3) output(" while she does her best to please your [pc.eachCock], massaging one with her fingers and shoving the others in her mouth, sometimes at once");
	if(pc.hasCock()) output(".  Brooke and Heckel move at the same pace: the Shepherd girl, eager to please, does all she can with her mouth, sucking, bathing, and worshiping your tool, ");
	if(!pc.hasCock() && pc.hasVagina()) output("burying her muzzle into your [pc.vagina] and immediately gets to work digging and searching your tunnel with her warm, moist, energetic tongue.  She licks, kisses, and worships everything from your vulva to your [pc.clit] and when she gets bored of that, she thrusts her whole face into you once more, finding some new depth neither of you thought she could reach.  All the ");
	output("while the hyena behind her pounds into her cunt with an unusual ferocity, atypical of how you know her.  Whether Heckel is just unusually aroused or she's taking the whole 'dominant' role more personally with Brooke, you're unsure and uncaring, if it means getting this sort of action.");
	if(pc.isHerm()) output("  Brooke, knowing of your duality, releases your cock from her mouth and dips lower, snaking her tongue below you and running it deliciously over your feminine sex.  Despite her weak hands, she does absolutely everything she can with what reach she's given, and you lean back, sliding your ass forward a bit to give her better reach to everything you have.");
	...
	if(pc.hasCock()) output("  You might have taken offense to the idea you're not as good a fuck if you weren't too focused on the amazing blowjob Brooke's giving you.");
	...
	if(pc.hasCock()) output("you erupt, spraying your cum directly into Brooke's thirsty throat.  Despite her own ecstasy, she knows to swallow to avoid choking, taking hurried breaths through her nostrils when she can while you coat her mouth white several times over.  Her mouth and her tongue are mostly still, barely conscious enough to move much less pleasure you farther.  You don't mind, and you cum and cum into her mouth regardless.");
	if(pc.hasVagina() && !pc.hasCock()) output("you cum into her mouth, spraying your feminine juices onto her tongue and wherever else Brooke didn't have the sense to cover.  It gets into her mouth; onto her muzzle; and down her face.  You coat her from your end as your pleased cunny quivers and cums again and again.  Brooke, for her part, swallows whatever happens to land into her maw, but she barely even seems to notice whenever your happen to get her anywhere else.");
...
public function brookeAndHeckelStuffPartThree():void {
	...
	output("\n\nReassured, you plant a kiss on her forehead, and shakily stand up from the bench.  Bow-legged, you make your way to your [pc.gear] and slip it back on.  You turn back to Brooke and Heckel, to see that Brooke's laid herself flat on the floor, waiting for Heckel's knot to shrink so she can make a break for it.  Based on what you can see from your new angle, that won't be for some time yet.");
...
public function brookeVHeckelBrookeWins2():void {
	clearOutput();
	//output(images.showImage("brooke-gym-heckelbrookedominance"));
	output("You and Brooke help Heckel into the locker room.  <i>\"[pc.name], close the door and strip down,\"</i> Brooke says, shouldering the rest of Heckel's weight as they lumber towards the bench.  Brooke whispers some sexy nothings into Heckel's ear while you do as you're asked, making sure there's some semblance of privacy in the wide-open locker room while you go about stripping off your [pc.gear] until you're in the nude.");
	...
	if(x >= 0) {
		...
		if (Flag("COC.TIMES_FUCKED_HECKEL_BLOWJOB") > 0 || Flag("COC.TIMES_FUCKED_HECKEL_ANAL") > 0)
		{
			...
			else output("  Every time you met Heckel, you had to take her dick up your ass - now, with the rare opportunity to return the favour, y");
		}
		else output("Y");
		output("ou savor every thrust into her tight velvet tunnel.  She's tight, far tighter than you had expected, but she's definitely no virgin either.  You draw your hands down to her hips, gripping and pulling delightedly at her tight muscles as you work her cunt, whetting your [pc.cock " + x + "] with the fruit you've been forbidden all this time.");
		//[if (cocks >= 2)
		if(pc.cockTotal() >= 2) output("  Your extra dick flops lamely up the crack of her ass, hotdogging between her cheeks with every thrust.  You consider the possibility of maybe double-dicking the cocky hyena, but you opt against it, not willing to break the good flow you've got between the three of you.");
		...
		output("\n\nYour arms and your back begin to strain a bit from having to balance both of the horny bitches as they bounce on you and on each other, but the rising pleasure in your [pc.cock " + x + "] overrides them both: you're not too far off, and from the rising grunts from Brooke, neither is she.  She finally lets go of Heckel's face, smiling playfully down on the hyena, not caring to wipe the sweat from her brow as she continues her ride.");
		...
		output("\n\nHeckel cries out shortly after Brooke and begins flooding the eager Shepherd girl's cunt with her cum, blasting her womb with her potent spunk again and again, her whole body shaking and shivering uselessly, a collective slave to her cock taking all the energy out of her.  Her pussy spasms and clamps down on your [pc.cock " + x + "] simultaneously, milking you for every drop you have, and with a few barbaric thrusts and a wordless cry, you oblige, shooting your own load into her hungry vagina.");
		if(pc.hasKnot(x)) output("  Likewise with Brooke, you pound your knot against Heckel's quim again and again, refusing to be denied entry, and with your orgasm, her pussy relents, taking in every inch of your [pc.cock " + x + "] with you.");
		output("  You cum and you cum, thrusting hard, making sure to drench every possible nook and cranny within Heckel's needy tunnel, seeding her deeply with your every thrust, and from how she coos and rocks in time with each of your pulses rather than her own as she does the same to Brooke, it's safe to say she enjoys it");
		//[if (isHerm = true)]
		if(pc.isHerm()) output(".  Your [pc.vagina] pulses in time with your [pc.cock " + x + "], spraying your feminine juices down the inside of your tired legs uselessly, lonely and longing for a cock of its own to take");
		if(pc.cockTotal() == 2) output(".  Your second dick flails and pulses, ejaculating in time with its lucky brother, shooting up and coating Heckel's back with your jism");
		if(pc.cockTotal() >= 3) output(" while [pc.eachCock] pumps yet more of your virile jism every which way, staining the tile floor and tagging the lockers surrounding you with your cum");
		output(".");
	}
	//{Females only}
	else {
		...
		output("\n\nOn your end, you find your experience with Heckel's tongue somewhat wanting.  You certainly can't fault her eagerness, but the amount of times she's put another woman's needs before her own is apparent in her amateurishness.  She licks and laps every which way, and while it's definitely pleasurable, she doesn't really know what to do and just sloppily goes at it.  You're not about to complain, though – for her faults, she's not a bad lay, and she certainly seems willing.  You continue to fuck her mouth with your [pc.vagina], cooing in encouragement whenever she does something right.");
