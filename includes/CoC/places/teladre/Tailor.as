...
//private function buyClothes(itype:ItemType):void {
	//clearOutput();
	//spriteSelect(61);
	//output("Victoria nods and pulls a measuring tape off her shoulder.  She moves around you with practiced ease, taking measurements from every conceivable angle.  Thanks to her small stature, it's quite easy for her to take your inseam measurement, though Vicky manages to ", false);
	//if(pc.biggestCockArea() > 30 || pc.totalCocks() > 1) output("fondle your bulging package", false);
	//else if(pc.hasVagina()) output("rub against your outer lips", false);
	//else output("slip a finger along your crotch", false);
	//output(" more than a few times.  You could swear you catch her licking her lips when she stands up, but she quickly turns away, saying, \"<i>I've got one in the back that should fit perfectly!  Be right with you!</i>\"\n\n", false);
	//output("She disappears in the back for a few moments, then returns with " + itype.longName + " that looks as if it were tailor-made for you.\n\n", false);
	//output("\"<i>" + itype.value + " gems and it can be yours,</i>\" she says.  ", false);
	//if(pc.gems < itype.value) {
		//output("You count out your gems and realize it's beyond your price range.", false);
		////Goto shop main menu
		//doNext(tailorShoppe);
		//return;
	//}
...
private function flirtWithVictoria():void {
	...
	if(pc.tallness >= 65)
		...
		output("your face.");
	output("  <i>\"Now then,  let's see what you've got!\"</i>  With practiced ease she works the bottom of your [pc.gear] off, revealing [pc.eachCock].  <i>\"Well, well. Looks like I was right about you from the start,\"</i> she says, licking her lips again.  <i>\"Just a taste first, I think...\"</i> Sticking her tongue out once more, she gives your rapidly stiffening dick a long, slow lick from the base up to the tip.  She closes her mouth just around your [pc.cock " + x + "], giving it a few rapid licks before pulling off with a pop.");
	...
	if(pc.cumQ() < 25)
		...
		if (silly)
			...
			output("Thick streams of your seed shoot out all around your dick as she continues to squeeze down on you, trying to keep as much of it inside her massively pregnant-looking belly as possible before she finally releases, squirting your dick back out of her with a pop, followed by a veritable geyser of white. Your entire lower body is painted white as it gushes out, soaking into your clothes and covering the floor as she visibly deflates.");
	}
