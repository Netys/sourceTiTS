...
public function showLoppe():void {
...
private function appearanceOfLoppe():void {
	clearOutput();
	showLoppe();
	if(Flag("COC.LOPPE_FURRY") == 0) {
		output("Loppe is a 6'2\" bunny-girl with deep brown eyes set in her pretty face.  Shoulder-length black hair and a pair of snow-white rabbit ears adorn the top of her head, tilted and lying backward to conceal the insides.  Curiously, her face is that of a normal human woman.");
...
private function noLoppeInterest():void {
	clearOutput();
	output("Dancers aren't really your thing, and you shake your head.");
	//(disable repeat and NPC)
	flags["COC.LOPPE_DISABLED"] = 1;
	processTime(1);
	clearMenu();
...
private function mebbeLaterToLoppe():void {
	clearOutput();
	output("You're not really in the mood for this right now, so you leave the bar.");
...
private function yesToMeetingLoppe():void {
	clearOutput();
	flags["COC.LOPPE_MET"] = 1;
	output("There's no harm in staying a while...");
...
private function yesToLoppeMeetingTwo():void {
	...
	//output(images.showImage("loppe-show-at-bar"));
	output("Suddenly the bar goes silent and the lights dim; a figure clad in a white mantle steps onto the stage; discarding the covering with a swift movement, it reveals the comely visage of a bunny-girl - a literal bunny-girl, in that apart from the obvious bunny ears and the hints of rabbit-like legs, she actually looks otherwise human despite the prevalence of anthropomorphic animals in this city.");
	...
	//[(corr < 40)]
	if(pc.cor() < 40 && !pc.isAss()) output(" as politely as possible. It felt nice to see a display like that here; makes it feel a little less like a wasteland of sex-mad monsters and a little more like home");
	output(".");
...
private function notReallyInterestedInLoppe():void {
	clearOutput();
	output("You decline, and the girl gives you a look of disappointment.");
...
private function sureBakeryWithLoppe():void {
	clearOutput();
	output("You consider the time of day, and the girl offering, and decide that it can't hurt.  Loppe smiles and takes your hand, leading you towards a nearby bakery.");
	...
	output("\n\nYou tell her that you aren't, you just didn't expect her to have such an appetite.  With a playful smile, you ask what other not-so-dainty secrets she's hiding; does she burp loudly to show she enjoyed her meal?  Loppe smiles mischievously, \"<i>Oh, my sweet " + pc.mf("boy", "girl") + "...  When I get my hands on something I like, I eat it up whole...</i>\"  Slowly, you feel a foot gently glide across your [pc.legs].");
	...
	output("\n\nLoppe giggles at your expression and answers the unspoken question.  \"<i>I do, [pc.name].  It's not really girl-on-girl, though.</i>\"  Loppe stays silent for a long moment, then sighs reluctantly and gently but firmly takes your hand and places it over her crotch, where you can feel a most unfeminine bulge. She holds it there, then lets you go, looking at you all the while.");
...
private function NoWayLoppe():void {
	clearOutput();
	output("Loppe lets out a breath she had obviously been holding in a bitter sigh, the ghost of the words, \"<i>I knew it</i>\" echoing faintly as she does so.  \"<i>Well, I understand.  I just had to give it a try all the same.  It was nice talking to you, [pc.name].  Might see you again, someday.  Sorry for wasting your time...</i>\"");
...
private function okayLoppeLetsGo():void {
	...
	output("\n\n\"<i>Hey, [pc.name],</i>\" she says quietly, \"<i>you're really " + pc.mf("handsome", "beautiful") + ", y'know?  And kind... how about a quick stop at my place, before we say goodbye?</i>\"");
...
private function noLoppesHouse():void {
	clearOutput();
	output("Loppe gives a disappointed sigh, but smiles at you all the same.  \"<i>I understand... if you ever feel like talking some more, you can find me in the gym.  I need to keep myself in tip-top shape for my dancing shows, after all.</i>\"");
...
private function yesLoppesHouse():void {
	clearOutput();
	//Loppe can now be found in the Gym
	output("The dancer smiles mischievously at you.  \"<i>Wonderful.  I'm going to show you just how great my body looks without this dress.  You wouldn't believe how much time I spend in the gym, working out.</i>\"  Loppe grabs her last cookie in one hand and your arm in the other, leading you away - presumably to her house.  Enroute, she giggles constantly, provoking a question from you.");
	...
	else if(pc.energy() >= 150 || pc.hasPerk("Enlightened Nine-tails") || pc.hasPerk("Corrupted Nine-tails")) {
...
public function loppeGenericMeetings():void {
	showLoppe();
	clearOutput();
	output("You decide to approach the bunny-girl.  Loppe smiles and wipes the sweat off her brow with the towel.  \"<i>Hey there, [pc.name], nice seeing you around here.  So... do you want to do something?  Talk, maybe?  Or go to my place for a 'workout'?</i>\" she asks with a smirk.");
	//Appearance
	//Talk
	//Sex
	...
	addButton(4, "Leave", TelAdreGymDesc);
...
private function talkWithLoppe():void {
	clearOutput();
	output("That 'workout' is going to have to be postponed; you have some questions.");
	...
	if (flags["COC.LOPPE_TIMES_SEXED"] != undefined) addButton(1, "Children", askLoppeAboutChildren);
	else addDisabledButton(1, "Children");
	//Gossip (after sexin' once)
	if (flags["COC.LOPPE_TIMES_SEXED"] != undefined) addButton(2, "Gossip", gossipWithLoppe);
	else addDisabledButton(2, "Gossip");
	//Working (after [Loppe]
	if (flags["COC.TIMES_ASKED_LOPPE_ABOUT_LOPPE"] != undefined) addButton(3, "Working", talkWithLoppeAboutWorking);
	else addDisabledButton(3, "Working");
	//Her Mom (available after choosing \"<i>Loppe</i>\" at least once)
	if (flags["COC.TIMES_ASKED_LOPPE_ABOUT_LOPPE"] != undefined) addButton(4, "Her Mom", chatWithLoppeAboutHerMom);
	else addDisabledButton(4, "Her Mom");
	//Her Village (after [Loppe]
	if (flags["COC.TIMES_ASKED_LOPPE_ABOUT_LOPPE"] != undefined) addButton(5, "Her Village", chatWithLoppeAboutLoppesVillage);
	else addDisabledButton(5, "Her Village");
	//The curse (unfinished; available after choosing \"<i>Loppe</i>\" at least once)
	//Fondle (after "Loppe"; see appropriate section)
	if (flags["COC.TIMES_ASKED_LOPPE_ABOUT_LOPPE"] != undefined && flags["COC.LOPPE_TIMES_SEXED"] != undefined) addButton(6, "Tease Her", fondleAndTease);
	else addDisabledButton(6, "Tease Her");
...
private function talkWithLoppeAboutLoppe():void {
	clearOutput();
	output("You tell the laquine that you're curious about her, that you'd like to know more about her: who she is, where she comes from, et cetera.");
	...
	//(if first dialogue)
	if(flags["COC.TIMES_ASKED_LOPPE_ABOUT_LOPPE"] == undefined) {
	...
	//set LoppeChat = 1
	flags["COC.TIMES_ASKED_LOPPE_ABOUT_LOPPE"] = 1;
	processTime(45);
	clearMenu();
	addButton(0, "Next", telAdreMenu);
}
...
private function askLoppeAboutChildren():void {
	clearOutput();
	output("You decide to ask Loppe for her thoughts on children.  Has she considered having any?");
	...
	output("\n\n\"<i>I think I already answered that.  I do love kids, and I would very much like to have my own; I can only hope that I'll be a mother - or maybe a father - half as good as my mom is, though.  But that's enough about me... what about you, [pc.name]?  Where do you stand on the subject?</i>\"");
...
private function noOpinionOnLoppeArt():void {
	clearOutput();
	output("You decline to answer, waving the question off.  Loppe raises an eyebrow at that.  \"<i>Haven't thought about it either, huh?</i>\"");
	output("\n\n\"<i>Just making conversation,</i>\" you reply.");

	output("\n\nLoppe nods, finishes her meal quietly, and departs.  \"<i>See you later, [pc.name].</i>\"");
	processTime(5);
	clearMenu();
	addButton(0, "Next", telAdreMenu);
}
...
private function loppeIDontReallyWantKidsYouStupidTwat():void {
	clearOutput();
	output("You confess you aren't really a 'kids' person; you don't think you could ever envision yourself as a parent, especially in this crazy world.");
...
private function loppeIWantKidsSoonOkayCanWeFuck():void {
	clearOutput();
	output("With a faint smile, you declare that having children is definitely in your future.  You want to be a parent; all you need is to find somebody who feels similar to you.");
	...
	output("\n\nLoppe grins at you.  \"<i>Oh... I don't know.  I'm sure a " + pc.mf("handsome", "beautiful") + " person like you has already drawn quite a few stares,</i>\" Loppe teases, batting her eyes at you.  \"<i>But, if the time is right, and you ask nicely... I'm pretty sure there's a certain bunny-girl around here who wouldn't mind...</i>\"");

	output("\n\nWell, you'll keep that in mind.  Realizing how much time has gone past, you politely excuse yourself.");

	output("\n\n\"<i>It was nice chatting with you, [pc.name].  Come visit me soon,</i>\" Loppe says, giving you a little peck on the cheek.");
	processTime(5);
	clearMenu();
	addButton(0, "Next", telAdreMenu);
}
...
private function gossipWithLoppe():void {
	clearOutput();
	output("You ask Loppe if she has any comments on the other residents of Tel'Adre.  The laquine smiles at you.  \"<i>It's not polite to pry into other people's lives, y'know?</i>\"");
	...
	if (flags["COC.NUMBER_OF_TIMES_MET_SCYLLA"] > 0) addButton(1, "Scylla", gossipWithLoppeAboutScylla);
	else addDisabledButton(1, "Scylla");
	//Jasun
	if (flags["COC.UNKNOWN_FLAG_NUMBER_00179"] > 0) addButton(2, "Jasun", gossipWithLoppeAboutJasun);
	else addDisabledButton(2, "Jasun");
	//Heckel
	if (flags["COC.MET_HECKEL"] > 0) addButton(3, "Heckel", gossipWithLoppeAboutHeckel);
	else addDisabledButton(3, "Heckel");
	//Edryn
	addButton(4, "Edryn", gossipWithLoppeAboutEdryn);
	//Lottie
	if (flags["COC.UNKNOWN_FLAG_NUMBER_00281"] > 0) addButton(5, "Lottie", gossipWithLoppeAboutLottie);
	else addDisabledButton(5, "Lottie");
	//Cotton
	if (flags["COC.COTTON_MET_FUCKED"] > 0) addButton(6, "Cotton", gossipWithLoppeAboutCotton);
	else addDisabledButton(6, "Cotton");
...
private function gossipWithLoppeAboutUrta():void {
	...
	if(!(flags["COC.TIMES_FUCKED_URTA"] > 0) || flags["COC.URTA_COMFORTABLE_WITH_OWN_BODY"] == -1) {
		output("\"<i>Ah, so you've met Watch Captain Urta?  She's famous around here, you know; people say she's a legendary bare-knuckle brawler and one of the toughest guards on the force.  The only problem is that she's not really that easy to approach... I guess she prefers to keep to herself.  Although she acts very friendly with that pretty centauress, Edryn.</i>\"");
	...
	else if(flags["COC.LOPPE_URTA_CHATS"] == undefined) {
		output("Loppe smirks at you.  \"<i>I heard she's been getting along nicely with a certain outsider; you wouldn't happen to know anything about that, would you, [pc.name]?</i>\"");
		//[It's Me] [No]
		processTime(1);
		clearMenu();
	...
	//(if LoppeUrtaKnowledge > 0)
		...
		output("\"<i>Urta again, huh?</i>\"  Loppe stares you, but her curious expression turns into one of smug triumph, like a cat that's caught a mouse.  \"<i>So... she has a big horse-cock.  I'm beginning to see a pattern here, sugar.   You like women with huge equine parts?  Is that why you hang out with me?  With Urta?</i>\"");
...
private function noLoppeWhosFuckingUrta():void {
	clearOutput();
	output("You shake your head, claiming that you have no idea what she's talking about.  Loppe looks at you, then shrugs, clearly not caring if you don't have any gossip on the subject to share.");
	//end scene
	processTime(3);
	clearMenu();
	addButton(0, "Next", telAdreMenu);
...
private function itsMeFuckingUrtaLoppe():void {
	...
	output("\n\nThe dancer pouts at you.  \"<i>That's not fair.  You wanted me to gossip, but you're not willing give up any gossip of your own?</i>\" She makes a motion of mock hurt.  \"<i>How could you take advantage of a poor laquine, [pc.name]?  Am I right, at least?  Is it big?</i>\"");
...
private function admitToLoppeThatYouLoveZeHorsecock():void {
	clearOutput();
	output("You look her in the eyes and tell her she's hit the nail on the head; you do have a fetish for chicks with dicks, and having a horsecock makes a dickgirl even sexier to you.");
...
private function dontLoppesHouse4Fucks():void {
	clearOutput();
	output("Loppe raspberries you.  \"<i>You tell me you love herms with big horse cocks and now you won't have sex with me?  You're one big tease, [pc.name].</i>\"");
...
private function denyToLoppeThatYouLoveZeHorsecock():void {
	clearOutput();
	output("Not in the mood to play along with Loppe's little game, you simply state that your reasons are your own and will stay that way for now.");
...
private function gossipWithLoppeAboutScylla():void {
	clearOutput();
	output("\"<i>The nun who likes sucking on dicks?</i>\" Loppe asks you.  \"<i>Yes, I've run into her at the Wet Bitch.  She offered to blow me too.  I was tempted, y'know?  She has those wonderful looking lips... nice big breasts too... but ultimately I wound up refusing.  It's just... not right.</i>\"");
...
private function gossipWithLoppeAboutHeckel():void {
	clearOutput();
	output("You ask if Loppe's ever had a run-in with a herm hyena who's usually running in the track at the gym.");
...
private function gossipWithLoppeAboutEdryn():void {
	clearOutput();
	output("\"<i>That pretty centauress watchwoman that hangs in the Wet Bitch when she's off duty?  Yes, it's no secret that she specializes in particular sorts of wetwork for those with... compatible endowments,</i>\" Loppe says, giving you a lecherous wink.");
	output("\n\nSomething in her look suggests a question to you.");
	output("\n\n\"<i>Well... maybe I've used her once or twice,</i>\" the dancer admits.  \"<i>But she costs 50 gems a ride and, frankly, she thinks I overdo things too much, so she wouldn't have me back even if I wanted to pay that much.</i>\"  Loppe looks rather sheepish as she smiles nervously, then looks right at you.  \"<i>Well... I shouldn't need her anyways, right?  Not as long as you come see me once in a while.</i>\"");
	processTime(4);
	clearMenu();
	addButton(0, "Next", telAdreMenu);
}
...
private function gossipWithLoppeAboutLottie():void {
	clearOutput();
	output("\"<i>That pig-girl that started hanging around the gym in the evenings?  I heard about her... I believe she wants to get fit,</i>\" Loppe comments conversationally.");
	//(if PC's training Lottie)
	if(flags["COC.UNKNOWN_FLAG_NUMBER_00299"] == 1) {
	...
	else if(flags["COC.UNKNOWN_FLAG_NUMBER_00281"] > 0) {
...
private function talkWithLoppeAboutWorking():void {
	clearOutput();
	output("Her curse naturally impacts her social life, but what about work?  How do she and her mother put food on the table?");
	...
	//set LoppeChat = 2
	flags["COC.TIMES_ASKED_LOPPE_ABOUT_LOPPE"] = 2;
	processTime(6);
	clearMenu();
	addButton(0, "Next", telAdreMenu);
}
...
private function chatWithLoppeAboutHerMom():void {
	clearOutput();
	//req LoppeChat > 0
	output("Loppe rubs her chin, deep in thought.  \"<i>Where do I start?  I guess by saying that my mom is a super mom.  She raised me all on her own, and she's always had time for me, despite having to keep up with her job.  I love her very much.</i>\"");
	...
	//(If PC is male:)
	if(!pc.isFeminine()) output("  Well, from what Loppe's told you, Uma wouldn't be attracted to you anyway.");
	output("\n\nLoppe giggles and grins. \"<i>Silly [pc.name].  I don't need several lovers.  Just one... the perfect one.  And I don't think my mom would be upset either way.  She knows I'm a big girl now.</i>\"");

	processTime(6);
	clearMenu();

	if (flags["COC.LOPPE_PC_MET_UMA"] == undefined)
	{
		...
		output("Loppe looks you over, thoughtful.  \"<i>To be honest... I don't think she would mind.  Want me to introduce you to her?  To become my 'official' " + pc.mf("boy", "girl") + "friend?  I know I certainly wouldn't mind having a 'serious' relationship with a cutie like you.</i>\"  The dancer winks at you. \"<i>How about it Sugar, do you wanna go visit my mom?</i>\"");
		
		addButton(0, "Visit Mom", firstVisitUmaPart1);
		addButton(1, "No", telAdreMenu);
	}
	else
	{
		addButton(0, "Next", telAdreMenu);
...
private function chatWithLoppeAboutLoppesVillage():void {
	clearOutput();
	output("You ask if Loppe would be willing to tell you about her village.");
...
private function loppeSexChoice(bakery:Boolean = false):void {
	...
	if(flags["COC.LOPPE_TIMES_SEXED"] == undefined) {
		//first-time only output, if not coming from bakery date
		if(!bakery) {
			output("That 'workout' seems like a nice option right now.");
			...
			//(else)
				...
				//[(lib>40)]
				if(pc.slut() >= 40) output(".. it almost sounds exciting.");
			}
		...
		output("It's not very difficult to reach Loppe's place; it happens to be located only a few blocks away from the gym, behind a small shop with the sign \"<i>Kemono's Oriental Clinic</i>\".");
		...
		//[(corr < 40)]
		if(pc.cor() < 40 && !pc.hasPerk("Corrupted Nine-tails") && !isBeyondMoralHorizon()) output("  You tell her they're quite nice... though privately you're curious what the interior looks like.");
		else output("  They don't seem to work very well - after all, <i>you're</i> here.");
		...
		//[(any cock)
		if(pc.hasCock()) output("You could just whip it out and ask what she thinks of it, compared to hers - it might lead to some cross words and crossed swords, though.  ");
		if(pc.hasVagina()) output("You could take her monster length vaginally, but if her libido is what she claims, you'll probably wind up quite stretched.  ");
		//[(boobs >= boobjob req)
		if(pc.biggestTitSize() >= 5) output("Her hardening horsecock looks like it would fit between your [pc.chest], an act likely to net you a messy demonstration of her enthusiasm.  ");
		output("There's always the option to receive her anally, though with her vaunted libido, you'd probably end up so flooded with her cum that it would wash from your mouth.  Or you could be a prick-tease and leave.");
		//Display sex options
		//[Cowgirl][Frot][TakeVaginal][Boobjob][TakeAnal][Bail]
	}
	//Repeat Intro (edited)
	else {
		output("Loppe's other 'workout' sounds pretty good right now.");
		...
		output("\n\nYou wonder if that was deliberate or not...");
		if(!pc.isNude()) output(" still, you follow suit, stripping off your [pc.gear] and presenting yourself to the sexy herm smiling at you.");
		...
		output("There's always the option to receive her anally, though with her vaunted libido, you'd probably end up so flooded with her cum that it would wash from your mouth.  Or you could be a prick-tease and leave.");
		//[(loppesexed)
		output("  Just jerking that cock and squeezing it so hard her balls swell up when she comes, though, that might be fun.  It'd be interesting to see how big they can get.");
	...
	addDisabledButton(0, "Cow-girl", "Cow-girl", "This scene requires you to have fitting cock.");
	addDisabledButton(1, "Get BJ", "Get BJ", "This scene requires you to have cock.");
	addDisabledButton(2, "TakeVaginal", "Take Vaginal", "This scene requires you to have vagina.");
	addDisabledButton(3, "Boob-job", "Boob-job", "This scene requires you to have large enough boobs.");
	...
	if (pc.hasVagina()) addButton(2, "TakeVaginal", getFuckedInYerTwatYaCunt);
	
	if (pc.biggestTitSize() >= 4) addButton(3, "Boob-job", boobjobLoppe);
	
	if (flags["COC.LOPPE_TIMES_SEXED"] != undefined) addButton(4, "SqueezeJob", loppeSqueezedickWhateverThatIs);
	
	if (pc.isTaur()) addButton(5, "TakeAnal", getAssFuckedByLoppeAsACentaur);
	else addButton(5, "TakeAnal", getButtFuckedNonHoarseByLoppe);
...
private function loppeRidesCocks():void {
	clearOutput();
	//output(images.showImage("loppe-rides-your-cocks"));
	output("Looking over the hermaphroditic, horse-cocked bunny-girl, you contemplate your options.  You settle yourself ");
	//[(not centaur)]
	if(!pc.isTaur()) output("on her bed, making yourself comfortable, and start suggestively stroking yourself");
	//(centaur)
	else output("cumbersomely on the floor, rolling over and spreading your hindlegs to expose yourself");
	output(".  Quietly, you ask Loppe how she would feel about indulging her feminine half.");
	...
	//(3+ fit cocks)
	else if(y >= 0 && pc.cockTotal() == 3) output("  \"<i>But I'm afraid I'll only be able to handle two of them.</i>\"");
	if(pc.cockTotal() > 1 && y >= 0) output("\n\nBetter than the usual reaction... at least she's willing to give more than one some love.");

	output("\n\nLoppe pushes you on your back and gently takes hold of ");
	if(y >= 0) output("two of ");
	output("your cock");
	if(y >= 0) output("s");
	output(", rubbing gently.  \"<i>Just sit back and leave everything in my capable hands, sugar.</i>\"");
	...
	if(y >= 0) output("s");
	output(" nice and slick.  You gasp in pleasure and surprise as you feel Loppe's erect cock sidle up to yours, helping lube you with its leaking pre. Raising up your hips, you clumsily try to ");
	if(y < 0) output("slide your shaft against her");
	else output("pinion her shaft between your");
	output(" own, shivering from the sensation of your sensitive " + pc.skin() + " against her proud horseflesh, already drooling even though you can plainly feel that it's only half-erect.");
	if(pc.balls > 0) output("  Your [pc.balls] gently brush and rub against her own swollen cum-factories, and you can't wait to empty your overfilled sac into her waiting womb.");

	output("\n\n\"<i>Okay, that's enough foreplay!</i>\" Loppe announces suddenly, eyeing your [pc.cock " + x + "] with a hunger that you never expected to see on her face.  Loppe quickly straddles you, aligning it with her pussy");
	if(y >= 0) output("; its neighbor is aimed for her tight rosebud");
	output(".  \"<i>Itadakimasu!</i>\" Loppe says, licking her lips as she finally slides you home.");
	...
	output(", yet squeezing you as tightly as a velvety vise.  In fact, she's gripping you so hard that you can't seem to push yourself any further into her depths; she's squeezing you much too tightly to move in or out, an outright amazing feat, if Loppe gets half as much action as she claims to.");
	...
	if(y >= 0) output("s");
	output(" with happy squeals of pleasure and happiness, softly accompanied by the wet sounds of your coupling.  You gasp and cry, savoring every motion as she squeezes and wriggles, her inner walls rippling so deliciously around your cock");
	if(y >= 0) output("s");
	output(", stroking them in the most delicious way.  You thrust your hips, trying to slip deeper inside her, but still she's holding you at bay.");
	...
	if(y >= 0) output(" and watertight ass");
	output(".  The sensation borders on the thin line between pleasure and pain, and you can't help but moan into Loppe's kiss; you wrap your ");
	if(pc.isDrider()) output("spindly legs");
	else if(pc.isTaur()) output("forelegs");
	else output("arms");
	output(" around the bunny-girl's ");
	if(Flag("COC.LOPPE_FURRY") == 0) output("silky-smooth");
	else output("softly furred");
	output(" back and hold her tight against you, her long horse-cock sandwiched between your bellies, smearing both of you with drooling precum, rivers of the stuff flowing out steadily with every thrust and grind the two of you make.");
	...
	//[(not horse)]
	if(!pc.isTaur()) {
		output("<b>You realize that if you keep holding onto Loppe, you're going to end up with a face covered in herm-cum - if you act quickly, though, you can avoid the impromptu facial.  You could even turn her hose of a cock back on her.</b>");
...
private function loppeRidesYouNoFaceJizz():void {
	...
	else output("hand lunges forward and places itself underneath, fingers curling around as you push it further up, aiming it over your shoulder.  You can feel it throbbing like mad against your fingers, can feel the impressive bulge of semen as it distends her urethra and surges up towards her flared tip.");
	output("  With an ululation of ecstasy, she literally explodes into orgasm, a great gout of cum visibly flying through the air with such force you can hear it splattering against the wall.  This is followed by a second eruption, then a third, a fourth... they just keep coming!  All the while, she bounces, moans, and writhes, her rapacious pussy gripping and squeezing");
	if(y >= 0) output(" while her ass suckles on your second cock with just as much eagerness");
	output(".");
...
private function loppeRidesPCCockFinal():void {
	...
	if(y >= 0 || pc.hasVagina()) {
		output(", while ");
		if(y>= 0) output("your second " + pc.cockDescript(y) + " just as easily pumps her perverted ass full of baby juice");
		if(y>= 0 && pc.hasVagina()) output(" and ");
		if(pc.hasVagina()) {
			output("your [pc.vagina] ");
			if(pc.wetness() < 3) output("drizzles");
			else if(pc.wetness() < 4) output("spatters");
			else output("gushes");
			output(" femcum onto the bed below you, forgotten during the act but not unsympathetic to her ministrations");
		}
	}
	output(".  You cry out as you cum long and hard, arching your back off of the bed in your pleasure before finishing and collapsing, gasping for breath as, above you, the laquine comes to her own climactic end.");
	...
	if(pc.isDrider() || pc.isTaur()) output("forelegs");
	else output("arms");
	output(" around her, pulling her into a post-coital cuddle.  Her wet cock quietly slaps against your belly as she embraces you back, clearly savoring the closeness the same way you are.  However, moments later, you feel Loppe's hips start to twitch, slowly building into more lustful thrustings, seconds before Loppe pushes herself back into a deliberately upright position, her pace building steadily.  With a lusty smile and a mischievous twinkle in her eyes, Loppe looks you straight in the eye as she proclaims, \"<i>It's time for round two, [pc.name]!</i>\"");

	//(High libido or High Min lust)
	if(pc.slut() >= 70 || pc.lustMin() >= 50) {
	...
	output("[pc.cocks] and slides down your body, pressing her soft, cum-slickened breasts against you.  You moan as the smooth curves of her chest press against [pc.eachCock]; the cum-slick ");
	if(Flag("COC.LOPPE_FURRY") == 0) output("skin");
	else output("fur");
	output(" give you the impression of being wrapped in the moist folds of a woman's sex.  As Loppe slides further down you feel her teasingly licking at the tip");
	if(y >= 0) output("s");
	output(" of your [pc.cocksLight].");

	output("\n\nThe skillful dance of her warm, wet tongue on the sensitive skin of your post-orgasmic member");
	if(y >= 0) output("s");
	output(" pull");
	if(y < 0) output("s");
	output(" an involuntary shudder from you; the erotic stimulation sends blood pulsing back into your veins, and your shaft");
	if(y >= 0) output("s rise");
	else output(" rises");
	output(" to full mast again, so swiftly ");
	if(y < 0) output("it almost smacks");
	else output("they almost smack");
	output(" the laquine on her nose, leaving you ready to be mounted by her once again.");
	...
	if(pc.lustMin() <= 30 && pc.slut() <= 50) output("; if I have to stop to tend to you after every coupling, I'll never get these empty");
	output(".</i>\"  She points with a smirk to her swollen balls.  Er... didn't she just blow her load? Her balls look at least as full as, if not fuller than, when you started, and they nearly slosh with eagerness to be rid of their cargo!");
...
private function loppeWorshipsDicks():void {
	clearOutput();
	//output(images.showImage("loppe-worships-your-cock"));
	var x:int = pc.biggestCockIndex();
	output("As your eyes sweep over the naked half-breed's form, they focus on her equine masculinity, already standing proud and eager at the prospect of sex.  Distinctly aware of your own male appendage");
	if(pc.cockTotal() > 1) output("s");
	...
	output("\n\nUsing her hands and arms, Loppe grinds her soft boobs against your [pc.cock " + x + "], milking a few more dollops of cum from you, slickening her breasts, as well as your saliva-coated prick.  You shudder and instinctively thrust your cock into her soft ");
	if(Flag("COC.LOPPE_FURRY") > 0) output("fluffy ");
	output("cleavage; her ");
	if(Flag("COC.LOPPE_FURRY") == 0) output("skin");
	else output("fur");
	output(" against your [pc.cock " + x + "] sends tingles of pleasure rippling through your loins.  The half-breed doesn't stop her ministrations, barely noticing your reaction.  All she does is giggle as you continue pumping more of your slick pre all over her breasts.  Murmuring and moaning appreciatively, you continue to thrust and grind, eagerly spilling pre-cum all over your herm lover's cleavage and midriff, though you can't prevent one sudden spurt from landing right on her face");
	//[(lib > 40)
	if(pc.slut() >= 40) output(" - nor would you want to; she looks just right covered in your fluids");
	output(".");
...
private function getFuckedInYerTwatYaCunt():void {
	clearOutput();
	//output(images.showImage("loppe-sticks-it-in-your-vagoo"));
	output("As you contemplate the delicious possibilities that the herm presents, your eyes are drawn to her admirable piece of horse-meat.  With a half-grin, you settle yourself on her bed, ");
	//[(mans)
	if(!pc.isTaur()) output("spreading your labia in an invitation that needs no words.");
	else output("exposing your rump and spreading your hind legs in your best effort to make her feel welcome.");

	output("\n\nLoppe smirks at you and winks.  \"<i>Don't worry; I'll treat you like a ");
	//[(femininity <= 40)
	if(pc.isFeminine()) output("princess.  A sexy, smoking hot princess");
	else output("prince.  A dashing, dandy prince");
	output(" that I intend to fill up with my laquine love.</i>\"  She wastes no time and pounces on the bed, ready to bury her face between your [pc.legs], licking her lips while taking deep breaths to inhale as much of your pheromones as she can.  You blink at the speed with which she moves, but smile, repositioning yourself to give her better access");
	//[(herm non-horse)
	if(pc.isHerm() && !pc.isTaur()) {
		output(", though she needs to move aside [pc.eachCock] ");
		if(pc.balls > 0) output("and [pc.balls] ");
		output("to really get at your cunt");
	}
	output(".");
	...
	//[(not horse and herm)
	if(!pc.isTaur() && pc.isHerm()) {
		output(", under your ");
		if(pc.balls > 0) output("[pc.sack]");
		else output("[pc.cock]");
		output(",");
	}
	output(" to lick and kiss your sensitive labia.  You arch your back and thrust your crotch at her, letting the laquine have your precious pussy to taste and suckle and ravage... she's pretty good with that tongue of hers.  Loppe lifts her head and wipes her face with her arm.  \"<i>I'm even better with this,</i>\" she says, mirroring your thoughts and holding her painfully erect equine-shaft.  You watch as it throbs, every vein bulging visibly, balls engorged as they churn up a load of cum to deposit in your waiting womb.  She ducks to take another lick, tasting you like one tastes a fine wine.");

	if(pc.vaginas[0].hymen) {
		output("\n\n\"<i>You taste like a virgin, [pc.name].  But that's impossible, right?  There's no way a hottie like you would let a silly horse-bunny like me be your first.</i>\"");
		//[(low corr)]
		if(pc.slut() < 50) output("  You look coy, for her benefit, and cast your eyes down shyly.");
		else output("  You do your best to warn her away from the topic with your eyes, eager to get on with the fuck.");
		//(not for horsez)
		if(!pc.isTaur()) output("\n\nLoppe gasps and persists.  \"<i>Wow... I'll actually get to be your first... are you sure that's okay?</i>\"");

		output("\n\nYou reach out ");
		if(pc.slut() < 50) output("and tap her playfully on the nose");
		else output("in your irritation and squeeze her face imploringly");
		output(".  Would you be here, with your clothes off in Loppe's bed, if you weren't okay with it?");
	...
	//[(not horse)
	if(!pc.isTaur()) output("  You wrap your [pc.legs] around her hips as best you can, thrusting in your efforts to meet her strong rhythm, feeling your cunt rippling around her wonderfully long, thick horsecock.");
	...
	if(pc.hasCock()) output(", precum begins to drool from your [pc.cocks]");
	output(".");
	...
	if(!pc.isTaur()) {
		...
		if(pc.isHerm()) output(" and smearing both your bellies with herm-pre");
		output(".");
	}
	//Centaur:
	else {
		output("\n\nYou find yourself pushing back against her, lifting your [pc.butt] to better receive Loppe's thrusts, even forcing her back as you do so.");
		...
		output("\n\nWith a pointed comment about her not being enough of a stallion to please a proper mare and a laugh for emphasis, you push your hips back with extra force, moaning at the feeling of her horse-cock inside your mare's cunt.  The lusty half-breed doesn't bother replying, she just thrusts against you as hard as she can, pistoning in and out of your [pc.vagina] with abandon.  Each slap of your hips sends pleasure rippling through you.  You gasp and moan, flicking your fingers across your [pc.nipples] for further stimulus.  You rut back against your hybrid herm lover as hard as you can... just a little more, oh, you're so close!");
	}
	...
	if(pc.hasCock()) {
		output(", neglected cock fountaining cum ");
		if(pc.cumQ() < 500) output("between you and painting you both");
		else output("gushing onto the bedsheets and smearing your belly");
		output(" with this aspect of your dual-sexed orgasm");
	}
	output(".");
	...
	if(!pc.isTaur() && pc.hasCock()) output(", giving you a quick peck on the cheek and licking a small gob of cum that seems to have stuck to your chin");
	output(".");
	...
	//[(low corr)
	if(pc.slut() < 50) output("a pleading look... and if there's maybe a trace of horror dancing in the depths of your eyes, well, that's understandable, isn't it?");
	else output("an incendiary glare, letting her know just what you think of her posturing.");
	output("  Loppe bursts out laughing.  \"<i>Just kidding, sugar.  If we go another round I think my dick might fall off.</i>\"");
	...
	if(silly) output("  \"<i>Da! Sandvich is kredit to team!</i>\"");
	...
	//[Libido >=50:
	if(pc.slut() >= 50 || pc.lustMin() >= 30) output("  Not that you didn't just titillate yourself thinking of it...");
...
private function getButtFuckedNonHoarseByLoppe():void {
	clearOutput();
	//output(images.showImage("loppe-sticks-it-in-your-butt"));
	output("You swallow as your eyes are drawn to the equine sausage jutting out between the bunny-girl's legs, and know you just have to have it.  Slowly, you spread yourself upon Loppe's bed, scooting up toward the head and pillows and kneeling, [pc.butt] in the air, inviting the laquine to do with you as she will.");
	...
	//(else if Anal Looseness < 3)
		...
		output("\n\nLoppe presses her flared head against your [pc.butt]; it takes a little work, but she manages to pop her tip inside your waiting hole.  You shiver and let out a purr of pleasure at being filled so...");
	...
	if(pc.hasVagina()) output(" pussy oozing femlube");
	output("... you beg her to go faster, to do it harder!");

	output("\n\nLoppe happily complies, thrusting herself deeper and stronger.  ");
	//[(cock)
	if(pc.hasCock()) output("One of her hands reaches around to grasp your pre-slickned [pc.cock " + pc.biggestCockIndex() + "], stroking you in time with her thrusts.  ");
	else if(pc.hasVagina()) output("One of her hands reaches around to tease your dripping [pc.vagina] and erect [pc.clit], dipping a finger in every time she bottoms out.  ");
	output("Wet slapping noises fill the room, and you gasp in pleasure each time her cum-churning balls slap against ");
	if(pc.balls > 0) output("yours");
	else output("your [pc.butt]");
	output(".  Loppe lays down on top of you, panting hotly on your neck, moaning in pleasure each time your sphincter contracts.");

	output("\n\nWith a groan, she jams herself as far in as she can, grasping your [pc.hips] tightly.  You can feel her balls churning against your butt, her throbbing dick seemingly growing fatter as cum travels up her urethra; finally, with a moan, she explodes inside you, filling you up with hot laquine cream.  You gasp and shudder as shot after shot of herm-spunk rushes in; you'd almost swear you can hear it slurping and splashing as it cascades into your belly, and you're positive you can feel your stomach start to distend from the glutinous mass of semen building up inside of you.  You struggle to hold back your own orgasm but finally lose the battle, crying out as it rocks through you.");
	if(pc.hasVagina()) {
		output("  A gush of sexual fluids from your crotch ");
		if(pc.wetness() < 3) output("spatters");
		else if(pc.wetness() < 4) output("splashes");
		else if(pc.wetness() < 5) output("drenches");
		else output("completely soaks");
		output(" the bed-linen under you.");
	}
	...
	output("\n\nStill... it was definitely an interesting ride.  \"<i>Aw, sugar, I'm sure it's not that bad.</i>\"  She gives your [pc.butt] a playful swat, prompting a jet of spunk to spill from your abused ass, and bites her lip apologetically.");
	...
	if(flags["COC.SATYR_KIDS"] > 0) output("You laugh softly; hopefully she'll never have to meet the creature that dispelled that same doubt in you.  ");
	output("\"<i>I think this is time for sleeping, not joking.</i>\"  With a trace of wounded dignity, she lays herself down and curls into your back, one arm wrapped around your gurgling belly.");
	...
	//(First Time)
	if(flags["COC.LOPPE_TIMES_SEXED"] == undefined) {
		output("Loppe laughs nervously.  \"<i>Sorry about that.  It's just that I usually can't bag a real cutie like you, and I think I might have gone a bit overboard... is there anyway I can make it up to you?</i>\"");
	...
	output("\n\nLoppe returns with your [pc.gear] in hand.  \"<i>Here you go, sugar,</i>\" she says, presenting them, then adds, \"<i>Err... you might want to get a bath before putting these on... would you like me to heat some water for you?</i>\"");
...
private function getAssFuckedByLoppeAsACentaur():void {
	clearOutput();
	//output(images.showImage("loppe-sticks-it-in-your-butt-you-centaur"));
	output("You swallow as your eyes are drawn to the equine sausage jutting out between the bunny-girl's legs, and know you just have to have it.  But is she confident in her ability, even with your shape?");
	...
	output("\n\nGood, because you feel like having her put that cock of hers to work on your [pc.ass].  You slowly circle her, lazily waving your tail.");
	...
	//[ass wet)
	if(pc.ass.wetness() > 1) output("  She doesn't even have to bother with lube; your ass is wet enough to accommodate her on its own.");

	output("\n\n\"<i>You might be even more eager than me - just give me a moment to set myself up,</i>\"  Loppe insists, giving your [pc.ass] an appreciative slap.  From behind you, you can hear her bustling around, muttering, \"<i>Where did I put my lube?</i>\"  A triumphant exclamation follows, and you hear the wet, sloppy sound of fluid being enthusiastically applied to a sizable cock as the laquine eagerly strokes herself off.  You can already imagine that thick shaft of hers piercing your interior, satiating your hungry bottom with the heavy load contained in those balls.");
	...
	if(pc.hasCock()) output(", feeling your erect [pc.cock] press against your underbelly, mashed between your centaur half and the soft carpet of Loppe's room.  You hope she doesn't mind a few stains...  ");
	//(else vag)
	...
	output("\n\nHer enthusiasm is contagious, and you find yourself bucking against her with almost as much eagerness as she's using to fuck your [pc.asshole].  The pleasure is exquisite; that flared tip of hers plugs your hole in a most wonderful way... it's almost like every thrust fills up a part of you that was missing, while every pull creates a vacuum that threatens to suck that big horse-dick right where it belongs.  ");
	//[PC has cock:
	if(pc.hasCock()) output("  The bumps against your prostate send waves of pleasure running through [pc.eachCock] and spreading along your body, until they reach your throat and come out as a ragged groan.  ");
	output("If she keeps this up for too much longer, you will pop any moment now...");

	output("\n\n\"<i>Ohhh!  G-gonna cum... [pc.name], your ass is incredible!</i>\"  Loppe moans, humping your butt like a herm possessed, delirious with her eagerness to climax.  Precum shoots in spurts easily equal to the orgasms of normal men and dickgirls; when she blows, it's going to be a big one...");
	...
	if(pc.hasCock()) output("  [pc.EachCock] spasms and unloads beneath you, smearing you and the carpet below and pooling thickly with fluid forced out by the prostate stimulation.");
	if(pc.hasVagina()) output("  Your vagina contracts, trying to grip at a phantom member as you spill your fluids on the carpet beneath you.");
	...
	if(pc.skinType != GLOBAL.SKIN_TYPE_SCALES) output(" have goosebumps all over your body");
	else output("r scales are practically standing on end");
	output(".");
	...
	//(High Libido)
		...
		output("\n\nLaquine fingers fix themselves to your [pc.ass] as if glued there and their bearer begins to rut you just as eagerly as before, too caught up in the fucking to care about something as trivial as defending her stallionhood with words.");
	}
	output("\n\n<b>One hour and some orgasms later...</b>");
	...
	output("\n\n\"<i>Hey there, [pc.name]; I see you're up now,</i>\"  Loppe calls to you in greeting as she comes through the door, hair wrapped up in a towel.  \"<i>Bath's free, so you might want to go and offload some of the cargo I gave you.</i>\"");
...
private function loppeSqueezedickWhateverThatIs():void {
	clearOutput();
	//output(images.showImage("loppe-orgazm-denial"));
	//(if LoppeDenial == 0)
	if(flags["COC.LOPPE_DENIAL_COUNTER"] == undefined) {
		output("After seeing Loppe's prodigious output, you wonder how she would handle not being able to orgasm.");
	...
	else if(flags["COC.LOPPE_DENIAL_COUNTER"] > timeAsStamp) {
	...
	output("\n\n\"<i>Ah!  [pc.name], I... I can't.  Ah!</i>\"  Loppe shudders, curling her toes and gripping her bed as a third orgasm rocks her.  The increased sensitivity of her prick contributes to a string of serial orgasms; you barely need to do anything but hang on for dear life as her body puts itself through a loop where she attempts to orgasm but fails with sensations so intense that she climaxes again.");
	...
	if(pc.cor() < 50 && !pc.isAss()) {
		output("  You loosen your grip; this was supposed to be pleasurable, not painful, so it's definitely time to stop.");
		//Goto [Let Go]
		clearMenu();
	...
	//(High Corruption, no new pg)
		...
		output("  You suppose you could let her go... on the other hand, it might be fun to see just how big she'll get if you keep holding her in.");
		clearMenu();
		addButton(0,"Let Go",letsLoppeGoCum);
		addButton(1,"Hold",superLoppeOrgasmDenialGo);
	}
...
private function letsLoppeGoCum():void {
	clearOutput();
	//output(images.showImage("loppe-comes-after-orgasm-denial"));
	output("As your fingers slacken on the laquine's straining shaft, they're practically pushed away by the violent force of long-delayed ejaculation, which rockets out of her horse-cock with immense speed and volume as you back away.  For an instant you almost think like she's going to punch a hole through the wall.  She screams, both in pleasure and relief as she fountains cum like a perverted geyser.  Spooge flies through the air, spilling all over the room; you manage to avoid the brunt of it, but some droplets inevitably hit you.");
	...
	flags["COC.LOPPE_DENIAL_COUNTER"] = timeAsStamp + 3 * 24 * 60;
...
private function superLoppeOrgasmDenialGo():void {
	clearOutput();
	//output(images.showImage("loppe-comes-after-moar-orgasm-denial"));
	output("For fear of not being able to hold her back anymore, you return both hands to tightly gripping her shaft.  Tears escape Loppe's eyes as she's denied yet again.  \"<i>Stop...</i>\" she says meekly.");
	...
	output("\n\n\"<i>[pc.name]... can you come closer... ?</i>\" you hear the sleepy voice of Loppe call after a few minutes.  Wondering what she wants, you lean in to hear.");
	...
	flags["COC.LOPPE_DENIAL_COUNTER"] = timeAsStamp + 5 * 24 * 60;
	//(very high corr)
	if(pc.cor() >= 90) {
		...
		flags["COC.LOPPE_DENIAL_COUNTER"] = timeAsStamp + 10 * 24 * 60;
	...
	output("\n\n\"<i>Bye, sugar!");
	if(pc.hasCock()) output("  Maybe next time I'll be the one to give you a bit of a holdout!");
	output("</i>\"");
...
private function boobjobLoppe():void {
	clearOutput();
	//output(images.showImage("loppe-give-her-a-titty-fuck"));
	output("You contemplate your options, unthinkingly rubbing your breasts.  As you do so, you notice your lover is observing your hands very keenly.  With a grin, you grope your tits and ask if Loppe likes your display.");
	...
	output("\n\nLoppe can't resist humping you slightly.  \"<i>Sugar, you're so soft...</i>\"  Smiling wanly, you grab your [pc.chest] and do your best to wrap around her horse-dick, pinning the shaft inside.  \"<i>I love it!  Don't stop!</i>\" the girl moans in pleasure.");

	output("\n\nEmboldened, you clasp your breasts firmer together and start to rise and fall, slowly scraping your " + pc.skin() + " up and down the half-breed's prick.  It feels so hot and firm against your chest, growing increasingly damp as Loppe oozes gouts as voluminous as the cumshots of ordinary men, making your breasts and her prick alike equally slippery and enabling you to stroke her faster and faster. \"<i>K-keep that up and I'm gonna shoot anytime,</i>\" Loppe groans, trying to hump into your cleavage.");
	...
	if((pc.physique() < 40 && !pc.isTaur()) || (pc.physique() < 25 && pc.isTaur())) {
		output("  Struggle as you may, you're powerless to stop the drooling laquine as she sends you crashing down on her bed.  The whole piece of furniture shakes ominously with the force of your impact, and for a moment you think it will break apart.");
		output("\n\nLeft a bit dizzied after this sudden turn of events, you yelp in surprise when a rock-hard, hot, throbbing horse-cock is shoved between your [pc.chest].  Loppe works over your boobs like a ravenous beast, not caring that she's spewing pre all over your face and even sometimes forcing you to taste her as she pushes her shaft up your mouth.  \"<i>So soft... so hot...</i>\" she moans as she continues to drool in lust.");
		...
		if(pc.hasTongueFlag(GLOBAL.FLAG_LONG)) output("looping your inhuman tongue around it and stroking for extra stimulus even while ");
		output("grabbing your breasts and squeezing the horsemeat between them.  The sooner you help her get over this frenzy, the sooner she'll get off of you... hopefully.  \"<i>Ah, sugar!  I knew you liked it rough!</i>\" your suddenly forceful lover shouts, ominously.");
		...
		if(silly) output(".  You can't help but think of Family Matters");
		output(".  You simply nod and groan, hands cradling your distended gut.  Loppe snuggles up against you, rubbing your bloated midriff.  \"<i>Sorry about that... hmm, how about we just cuddle and rest?</i>\"");
		...
		output("  You hold the laquine's arms and force her onto the floor; she lands with an audible \"<i>oomph</i>\" and you grin as you hold the eager herm down.");
		...
		output("\n\nLooking at your own [pc.skinFurScales] now off-white from the glazing she's given you inadvertently, you concede the point to yourself.  Still...");
		...
		//"You jerk, [pc.name]; now I have to wash this!"
		//Laughing, you blow her a kiss and make your way out of the house.
		//end scene
		output("\n\n");
		if(pc.cor() >= 60 || pc.isAss()) output("You don't really feel like it, but you figure you had better humor the greedy herm if you want to keep fucking her whenever you like.  ");
		output("Oh, well... you reach down and heft the surprised hybrid off of the floor and onto her spattered bed, then slide yourself into it as best you can, wrapping your arms around her.  She hugs you back, then yawning widely.  \"<i>Naptime...</i>\"");
		output("\n\nGiven she seems unlikely to let you go, you decide you may as well enjoy a brief doze too.");
	}
	output("\n\n<b>Two hours later...</b>");
	output("\n\nYou have the distinct feeling that someone is licking your [pc.nipples]... and when you finally open your eyes, you're introduced to a pair of rabbit-like ears.  Loppe looks up, licking a stray dollop of cum from her nose.  \"<i>Hey, sleepyhead!  I was just finishing cleaning you up.</i>\"");
	...
	output("\n\n\"<i>Anyway, I set your [pc.gear] over there.</i>\"  She points to a chair nearby.   You thank her and pick yourself out of bed ");
...
private function beATeaseAndLeaveLoppeAfterSexInvite():void {
	clearOutput();
	//output(images.showImage("loppe-teaze-and-leave"));
	output("Despite the hybrid beauty standing stark naked before you, you decide that you aren't in the mood after all.");

	output("\n\nLoppe shoots you a look of annoyance and disappointment.  \"<i>Well, if you really don't want to do anything, there's nothing I can do.  But nevertheless, that was very mean of you, [pc.name]; teasing a poor, innocent laquine with the promise of sex...</i>\"  She pouts exaggeratedly, possibly hoping you'll relent.");
	...
	output("\n\n\"<i>You're no fun, [pc.name],</i>\" she gripes, when you don't.  \"<i>Hopefully next time you'll be willing to do a lot more than just give me a peck on the lips.</i>\"");

	output("\n\nShe smiles wanly at you and walks to her closet, dressing herself in a comfy bathrobe.  \"<i>[pc.name], sorry for being a bad hostess, but can you show yourself out?  I kinda need to take care of a... thing.</i>\"");
...
//req LoppeChat > 1
private function fondleAndTease(out:Boolean = true):void {
	if(out) {
		clearOutput();
		output("You lean in over the table and tell Loppe you'd like to talk about her.  She looks vaguely confused.  \"<i>Well, I already told you about everything I can think of.  Was there something you wanted to explore in more detail?</i>\"");
...
private function teaseLoppeNSuck():void {
	clearOutput();
	output("You lean over the table and give the laquine a kiss.  Confused, but not displeased, Loppe eagerly returns it, then watches as you break the lip lock and start to slide yourself under the table.  \"<i>Umm... sugar?  What are you doing?</i>\"");
	...
	if(pc.hasTongueFlag(GLOBAL.FLAG_LONG)) output(", inhumanly long tongue looping out to coil wetly around the length");
	output(".  You suckle gently, letting the salty tang of hot spunk wash over your tastebuds, and then start to gulp and swallow your way down her prick's sizable length.");
