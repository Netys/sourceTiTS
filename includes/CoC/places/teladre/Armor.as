...
//private function armorBuy(itype:ItemType):void {
	...
	//clearOutput();
	//if (itype is Armor) { 
		//output("Yvonne gives you a serious look, then nods.  She pulls the armor off a rack and makes a few adjustments, banging away with her massive hammer to ensure a perfect fit.  The entire time, she's oblivious to the movements of her massive breasts, accidentally exposing her impressive nipples multiple times.\n\n", false);
		//output("She finishes and turns to you, smiling broadly, \"<i>Now, that will be " + itype.value + " gems, unless you want to change your mind?</i>\"", false);
	//}
	//else {
		//output("Yvonne gives you a serious look, then nods.  She pulls the shield off a rack and shows it to you.\n\n", false);
		//output("She smiles broadly, \"<i>Now, that will be " + itype.value + " gems, unless you want to change your mind?</i>\"", false);
//
	//}
...
private function fuckYvonneInZeBlacksmith():void {
	...
	if(pc.cockVolume(x) < 20) output("runt");
	else if(pc.cockVolume(x) < 40) output(pc.mf("boy","girl"));
	else if(pc.cockVolume(x) < 60) output("big " + pc.mf("boy","girl"));
	...
	output("\n\nYou sidle up to the larger woman and begin aligning [pc.oneCock] with her mammoth buns, the sweltering, pheromone-laced stink pouring from her body making it difficult not to fall on top of her and rut.  Her huge tits are squished against the floor, squeezing out obscenely to either side of the blacksmith's lithe, muscular torso.  When you push inside, her slick wet cunt squeezes your [pc.cock " + x + "] powerfully but affectionately.  Her potent vaginal muscles work your [pc.cock " + x + "] over, tightly embracing your turgid dickflesh as you begin to fuck her properly, plowing her sweat and love-juice soaked folds even while you struggle to reach up for her incredible breasts.");
	...
	if (pc.hasKnot(x)) {
		output("Your knot fills, locking you inside her, securely blocking any escape for the jizz you're now filling her depths with.  Yvonne sighs dreamily, \"<i>Just right...</i>\" while spunk slides into her birth canal to infiltrate her womb.");
		if (pc.cumQ() > 500)
			output("  The pearly goop spatters into her womb with egg-inseminating force, filling her beyond her wildest expectations.");
		if (pc.cumQ() > 1000)
		...
		if (pc.cumQ() > 2000)
			output("  Thanks to your knot, not a single drop escapes, and soon Yvonne's belly is as fat as her tits, cum-bloated in the extreme, a sloshing auburn tub packed with ivory sperm.");
	}
	else {
		output("Your jizz bubbles out to fill her depths, the spunk surging through her as it moves towards her womb.  Yvone sighs dreamily, \"<i>Ahhhh...</i>\" while you empty your [pc.balls] inside her unprotected womanhood.");
		if (pc.cumQ() > 500)
			output("  The pearly goop spatters into her uterus with egg-inseminating force, filling the bitch beyond her wildest expectations.");
		if (pc.cumQ() > 1000)
			output("  The blacksmith cries out in pain and pleasure, her belly rounding with your liquid, looking positively pregnant.  Her twat begins to dribble sperm, creampied beyond belief.");
		if (pc.cumQ() > 2000)
			output("  Unfortunately, as your virility makes itself known, Yvonne's body hits its limit, and jets of ivory spooge squirt from all sides of her cunny, dribbling into a pearly puddle on the floor.");
	}
	//still no new pg
	output("  With a thoroughly cream-stuffed twat beneath you, you ");
