...
//
	...
	//if (flags[kFLAGS.COTTON_HERBS_OFF] > 0) {
		//if (rand(5) == 0 || pc.cumQ() > rand(1000) || pc.virilityQ() >= 0.5) {
	...
	//else {
		...
		//if (flags[kFLAGS.COTTON_KID_COUNT] == 0) {
			//if (rand(5) == 0 || pc.cumQ() * pc.virilityQ() >= rand(1000)) {
				//pregnancy.knockUpForce(PregnancyStore.PREGNANCY_PLAYER, PregnancyStore.INCUBATION_COTTON);
			//}
		//}
		////NOT FIRST KID - LESS LUCKY!
		//else if (pc.cumQ() * pc.virilityQ() >= rand(1000)) {
			//pregnancy.knockUpForce(PregnancyStore.PREGNANCY_PLAYER, PregnancyStore.INCUBATION_COTTON);
		//}
	//}
}
	...
	if (pc.isPregnant() && pc.bellyRatingMod > 33)
	{
		...
		//switch (pregnancy.event) {
			...
			//case 4: output("Cotton smiles and greets you as you approach.  Gone is her usual top, instead she's chosen to show off her enormous, round brown belly with an incredibly short white tank top which might be confused with a bra if you didn't know any better.  Though you'd imagine her movement would be quite limited, she actually moves with her usual grace... until her belly collides with a row of water bottles set out on a nearby table, knocking them to the ground.");
					...
					//break;
			//default:
					output("You approach Cotton, who smiles and says, \"<i>Hey there my little pet, if you'd like a work out, just get changed and come on back.</i>\"\n\nDo you want to engage in yoga with her?");
		//}
	}
	processTime(1);
	cottonMenu();
...
private function cottonFucksYou():void {
	...
	if(!pc.hasVagina() && pc.hasCock()) {
		...
		pc.buttChange(72, true, true, false);
		output("\n\n");
	...
	//(Get fucked, as Female)
		...
		pc.cuntChange(0, 72, true, true, false);
		output("\n\n");
...
private function fuckCottonInShowerRepeat():void {
	...
	*/
	...
	if(pc.isTaur()) {
		...
		//if (pregnancy.event > 1) output("pulling you gently towards her bulging belly, which you can't help but put your hands against as you kiss her. The bulge is firm and solid, almost a drum of solid muscle, and you caress it as you and the mare-morph make out, eliciting soft murmurs of appreciation for your efforts.\n\n");
		//else 
		output("pulling you closer.\n\n");
		...
		//(if player is too small (under 4</i>\")
			...
			//if (pregnancy.event > 1) output("Don't worry pet, it's plenty adequate for me.  And you've got to admit,</i>\" she smirks, rubbing her round belly, \"<i>It gets the job done, doesn't it?");
			//else 
			output("Don't worry pet, it's plenty adequate for me.");
		...
		if(pc.cockVolume(x) >= 100) output("Oblivious to you, her own limp cock leaks cum down onto the bench as an orgasm rocks her body.");
		//else if (pregnancy.event > 1) output("Oblivious to you, her own cock twitches and spasms, letting loose a spray of cum across her swollen, pregnant belly, breasts and the bench as an orgasm rocks her body.");
		//else 
	...
	//(Repeat fuck her, for nagas)
		...
		if(pc.cockVolume(x) >= 100) {
			//(If PC has one cock)
			if(pc.cockTotal() == 1) output("  You feel your [pc.cock " + x + "] stirring beneath you, slowly coming to attention, though Cotton's remains curiously limp.  Your [pc.cockHead " + x + "] rubs against her belly, sending ripples of pleasure up your spine.", false);
			//(If PC has multiple cocks)
			else if(pc.cockTotal() > 1) output("  You feel your [pc.cocksLight] stirring beneath you, slowly coming to attention, though Cotton's remains curiously limp.  The group of cocks rubs together as you make out, sending ripples of pleasure up your spine.", false);
		}
		else {
			//(If PC has one cock)
		...
		if(pc.cockVolume(x) >= 100) output("Her own cock, still limp, visibly twitches and begins leaking cum onto her belly as an orgasm rocks her body.");
		//else if (pregnancy.event > 1) output("Her own cock twitches and spasms, letting loose a spray of cum across her swollen, pregnant belly, breasts and face as an orgasm rocks her body.");
		//else 
		output("Her own cock twitches and spasms, letting loose a spray of cum across her belly, breasts and face as an orgasm rocks her body.");
		//(If player is too big, add)
		if(pc.cockVolume(x) > 70) output("  Even in this pseudo-missionary position you can't stuff your entire enormous girth into her, but she doesn't care. Her eyes seem to roll back and forth like it's hard to focus, and her tongue drips water, sweat and saliva onto her body. She's panting for breath and you are able to make out faint, \"<i>Oh gods, oh gods</i>\" over the running water. You can even make out the shape of your cock in Cotton's toned stomach, pumping back and forth, which brings a smile to your face. Her back arches under the shower, her hair messily spread out beneath her and she bites her lip as another orgasm rocks her body.");
		output("\n\n");
	...
	//(Repeat Fuck Her, for humanoid bodies)
		...
		if(pc.cockVolume(x) >= 100) output("Her own cock, still limp, visibly twitches and begins leaking cum onto her belly as an orgasm rocks her body.");
		//else if (pregnancy.event > 1) output("Her own cock twitches and spasms, loosing a spray of cum across her swollen, pregnant belly, breasts and face as an orgasm rocks her body.");
		//else 
		output("Her own cock twitches and spasms, letting loose a spray of cum across her belly, breasts and face as an orgasm rocks her body.");
		//(If player is too big, add)
		if(pc.cockVolume(x) > 70) output("  Even in the missionary position you can't stuff your entire enormous girth into her, but she doesn't care. Her eyes seem to roll back and forth like it's hard to focus, and her tongue drips water, sweat and saliva onto her body. She's panting for breath and you are able to make out faint, \"<i>Oh gods, oh gods</i>\" over the running water. You can even make out the shape of your cock in Cotton's toned stomach, pumping back and forth, which brings a smile to your face. Her back arches under the shower, her hair messily spread out beneath her and she bites her lip as another orgasm rocks her body.");
		output("\n\n");
...
private function cottonFucksYouInShowerRepeat():void {
	...
	if(pc.isTaur()) {
		...
		if(pc.hasCock()) {
			if(pc.biggestCockVolume() > 70) output("  \"<i>Oh my, what's this?</i>\" She puts a hand under your enormous, yet embarrassingly limp cock and lifts it slightly. \"<i>My little pet has such a big dick... Just how I like it. Perhaps next time I'll get to try it out... but not today, hm? This is all about you right now.</i>\"");
		...
		if (pc.hasVagina()) pc.cuntChange(0, 72, true, true, false);
		else pc.buttChange(72, true, true, false);
		output("\n\n");
	...
	//(Repeat get fucked for humanoids and nagas)
		...
		if(pc.isNaga()) {
			...
			//if (pregnancy.event > 1) output("Moving quickly, you wrap your arms around Cotton's neck and coil your body around her waist, just below her round, pregnant belly, and down one leg.  You can feel her swollen midriff rubbing against the scales on your serpentine half with each motion you make.");
			//else 
			output("Moving quickly, you wrap your arms around Cotton's neck and coil your body around her waist and down one leg.");
		}
		//(If Humanoid body)
		else {
			...
			//if (pregnancy.event > 1) output("Moving quickly, you wrap your arms and legs around Cotton so you don't fall over, carefully avoiding her pregnant belly, which is no mean feat. Her belly is pressed into yours, letting you feel the solid weight of it, gentle kicks from your unborn child within.");
			//else 
			output("Moving quickly, you wrap your arms and legs around Cotton so you don't fall over.");
		}
...
//private function cottonPregnantAlertII():void {
	...
	//if (flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] > 0 || (telAdre.edryn.pregnancy.isPregnant && flags[kFLAGS.EDRYN_PREGNANT_AND_NOT_TOLD_PC_YET] != 0)) output("  Should have figured as much after you got Edryn pregnant.");
	...
	//if(flags[kFLAGS.EDRYN_NEVER_SEE_AGAIN] == 0) output("you'll stick with me through this, right?");
...
//private function beABadCottonDad():void {
	...
	//if(flags[kFLAGS.EDRYN_NEVER_SEE_AGAIN] == 0) output("and slaps you as hard as she can");
	//else output("and punches you as hard as she can");
	//output(". \"<i>Fuck you 'pet'.");
	//if(flags[kFLAGS.EDRYN_NEVER_SEE_AGAIN] > 0) output("  That was for Edryn too.");
...
//private function cottonContraceptionToggle():void {
	...
	//}
	//output("You ask the ");
	//if (pregnancy.isPregnant) output("pregnant ");
...
//
	...
	////(Cottonpreg:
	//if (pc.gender != 3 || pregnancy.isPregnant) output("virile");
	//else output("fertile");
...
//
	...
	//if(flags[kFLAGS.COTTON_KID_COUNT] < 3) output("Well that's true,</i>\" she admits.  \"<i>But I leave it up to you, pet.");
...
//private function visitCottonKids():void {
	...
	//if(flags[kFLAGS.COTTON_KID_COUNT] == 1) output("kid");
	//else output("kids");
	...
	//if(flags[kFLAGS.COTTON_KID_COUNT] == 1) {
		//if(flags[kFLAGS.COTTON_OLDEST_KID_GENDER] == 1) output("him");
		//else output("her");
	//}
	//else output("them");
...
//
	...
	//if(scene == 0) {
		//output("\n\nYour ");
		//if(flags[kFLAGS.COTTON_OLDEST_KID_GENDER] >= 2) output("baby girl is");
		//else output("baby boy is");
		//output(" sitting up in ");
		//if(flags[kFLAGS.COTTON_OLDEST_KID_GENDER] >= 2) output("her");
		//else output("his");
...
//
		...
		//if(flags[kFLAGS.COTTON_OLDEST_KID_GENDER] >= 2) output("she");
		//else output("he");
		...
		//if(flags[kFLAGS.COTTON_KID_COUNT] == 1) output("baby");
		//else output("babies");
...
//
		...
		//if(flags[kFLAGS.COTTON_OLDEST_KID_GENDER] == 1) output("colt");
		//else output("filly");
		...
		//if(flags[kFLAGS.COTTON_OLDEST_KID_GENDER] == 1) output("he");
		//else output("she");
...
//
		...
		//if(flags[kFLAGS.COTTON_KID_COUNT] > 1) {
			//output("\n\nYour other ");
			//if(flags[kFLAGS.COTTON_KID_COUNT] == 2) output("baby is");
		...
		//output("\n\nEventually, though, ");
		//if(flags[kFLAGS.COTTON_KID_COUNT] == 1) {
			//if(flags[kFLAGS.COTTON_OLDEST_KID_GENDER] >= 2) output("she grows tired");
			//else output("he grows tired");
		//}
		//else output("they grow tired");
		//output(" and, still giggling, settle");
		//if(flags[kFLAGS.COTTON_KID_COUNT] == 1) output("s");
...
//
		...
		//if(flags[kFLAGS.COTTON_KID_COUNT] == 1) {
			//if(flags[kFLAGS.COTTON_OLDEST_KID_GENDER] >= 2) output("she goes");
			//else output("he goes");
		//}
		//else output("they go");
	...
	//else if(scene == 1) {
		...
		//if(flags[kFLAGS.COTTON_KID_COUNT] == 1) output("your baby is");
...
//
	...
	//else if(scene == 2) {
		...
		//if(flags[kFLAGS.COTTON_OLDEST_KID_GENDER] >= 2) output("daughter");
		//else output("son");
		...
		//if(flags[kFLAGS.COTTON_KID_COUNT] > 1) output("  Fortunately, your other kids don't seem to be hungry, so it's just the one who needs feeding.");
//
		////(If PC lactates:
		//if(pc.lactationQ() >= 50) {
			//output("  You could probably offer to nurse your ");
			//if(flags[kFLAGS.COTTON_OLDEST_KID_GENDER] != 1) output("daughter");
			//else output("son");
	...
	//else {
		...
		//if(flags[kFLAGS.COTTON_OLDEST_KID_GENDER] >= 2) output("girl");
		//else output("he");
...
//private function feedYourCottonKids():void {
	...
	//if(flags[kFLAGS.COTTON_OLDEST_KID_GENDER] >= 2) output("filly ");
	//else output("cotton ");
...
//
	...
	//if(flags[kFLAGS.COTTON_KID_COUNT] == 1) output(" needs");
...
//
	...
	//if(flags[kFLAGS.COTTON_OLDEST_KID_GENDER] >= 2) output("her down for her nap, hm pet?</i>\"");
