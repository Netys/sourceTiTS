...
import classes.Items.Transformatives.CoCLaBova;
...
public function showLottie():void {
	//showLottie();
...
public function lottieAppearance(display:Boolean = true):Function {
	if (!pc.hasCock()) return null;
	if (flags["COC.MET_MARAE"]  == undefined) return null;
	if (flags["COC.LOTTIE_DISABLED"] == 1) return null;
	
	if (hours >= 15 && hours <= 23) {
		if(flags["COC.LOTTIE_MET"] == undefined) {
			...
			if(display) {
				//[RANDOM INTROS]
				//[Encouragement 0 – 30]
				if(flags["COC.LOTTIE_ENCOURAGEMENT"] < 30 && rand(6) == 0) {
					...
					if(silly) output("  What can you say? You love making bacon.");
				}
				//[Encouragement 0 – 30]
				else if(flags["COC.LOTTIE_ENCOURAGEMENT"] < 30 && rand(5) == 0) {
					...
					if(silly) output("  What can you say? Her pork buns are the best.");
				}
				//[REPEAT INTROS]
				else if(rand(2) == 0) {
					...
					if(flags["COC.LOTTIE_TONE"] <= 33) output("voluptuous");
					else if(flags["COC.LOTTIE_TONE"] <= 66) output("plump");
					else output("curvy");
					output(" pig girl jogs on the spot to wake herself up - unintentionally jiggling her " + lottieCup() + "-cup breasts and soft ass in the process. She walks over to the nearby cafe and sits down on her ");
					if(flags["COC.LOTTIE_TONE"] <= 33) output("bountiful");
					else if(flags["COC.LOTTIE_TONE"] <= 66) output("chubby");
					else output("cute");
					output(" rump.");
				}
				else {
					...
					if(flags["COC.LOTTIE_TONE"] <= 33) output("voluminous");
					else if(flags["COC.LOTTIE_TONE"] <= 66) output("chubby");
					else output("cute");
					output(" rump.");
				}
...
private function lottieCup():String {
	//(d/e/f/g)
	if(flags["COC.LOTTIE_TONE"] >= 75) return "D";
	else if(flags["COC.LOTTIE_TONE"] >= 50) return "E";
	else if(flags["COC.LOTTIE_TONE"] >= 25) return "F";
	else return "G";
}
...
public function encounterLottie(charge:Boolean = false):void {
	...
	if(flags["COC.LOTTIE_MET"] == 1) {
		...
		if(!pc.isNaga() && !pc.isTaur() && pc.tone >= 50) {
			...
			processTime(5);
			clearMenu();
			addButton(0, "Yes", acceptBeingLottiesHamSandwich);
			addButton(1, "No", declineBeingLottiesGymHamsandwich);
		}
		//[If Don't Meet Full Requirements]
		else {
			output("\"<i>Oh, uh... thanks! Hey, listen – I gotta go start my yoga class now, but it was pretty great letting it all out with you. We should run into each other again sometime!</i>\"  The girl flashes a grin and jogs towards the showers.  \"<i>My name's Lottie, by the way!</i>\"\n\n");
			processTime(5);
			clearMenu();
	...
	//[REPEAT ENCOUNTER]
		...
		output("You approach the pig girl ");
		if(flags["COC.LOTTIE_ENCOURAGEMENT"] >= 50) output("relaxing");
		else output("sitting timidly");
		output(" at a table, wearing her low-cut top and booty shorts that accentuate her ");
		if(flags["COC.LOTTIE_TONE"] <= 50) output("plump");
		else output("curvy");
		output(" figure.  ");
		if(flags["COC.LOTTIE_BIGGER_IS_BETTER"] != 1) output("Lightly sipping");
		else output("Slurping down");
		output(" what appears to be ");
		if(flags["COC.LOTTIE_BIGGER_IS_BETTER"] != 1) output("some juice");
		else output("a milkshake");
		output(", her eyes wander over the room when she spots you and waves");
		if(flags["COC.LOTTIE_ENCOURAGEMENT"] >= 70) output(" excitedly");
		output(".  You walk over to her and sit down, listening to Lottie update you on her progress.\n\n");

		//[Repeat If didn't meet requirements]
		if(flags["COC.LOTTIE_TRAINING_ACCEPTED"] == 0 && (pc.isNaga() || pc.isTaur() || pc.tone < 50)) {
			output("\"<i>Hey, " + pc.short + "! I...I'm really glad I talked about myself with you, you're a really great friend, y'know. I really think I can achieve something this time, if I work hard!</i>\" You smile and tell her how glad you are that she's doing this. Lottie responds with a delighted expression, and continues to talk. \"<i>Anyway, I've been trying to look for a personal trainer, but it's kinda difficult, heh. I mean... I'm sure there's lots of trainers here and all, but I – I'd prefer someone I know personally, right?</i>\" She looks up at you expectantly, but then looks back towards the floor. </i>\"But I guess we can't always have what we want.</i>\"\n\n");
			//(Note: No options but 'leave' are available until you meet requirements.)
			processTime(5);
			clearMenu();
			addButton(0, "Leave", TelAdreGymDesc);
			return;
		}
		//[Normal Encounter, If became met requirements]
		else if(flags["COC.LOTTIE_TRAINING_ACCEPTED"] == 0) {
			output("\"<i>Hey, " + pc.short + "! I was just... I...</i>\"Lottie seems to be at a loss of words, gaping at your new, chiselled physique. You smile and tell her how glad you are that she's doing this, thought it's a shame she hasn't found a personal trainer yet. Lottie responds with a hazy expression and an open mouth. \"<i>Y-yeah... it's a shame...</i>\" She begins, until snapping out of her daze and looking up excitedly at you. \"<i>I- Y'know, I don't know what you've been doing to yourself lately, but... you look a... you're really amazing right now, " + pc.mf("big guy", "babe") + "!</i>\" She squeals as you flex at her, giggling at your blatant attempts at showing off. You never really took her as one to appreciate muscles, but you figure her obsession towards the perfect body might extend to other people as well. You take one stern look at the chubby porcine in front of you and grin, asking if she wants you to be her personal trainer. She seems shocked at your proposal. </i>\"Are... are you serious?</i>\"\n\n");
			output("Well, are you?");
			//[Yes][No][God No]
			processTime(5);
			clearMenu();
			...
			if (silly && pc.cor() >= 80) addButton(2, "God No", gotInShapeComedyDeclineLottie);
			return;
		}
		//[Second Encounter, If fucked Lottie]
		else if(flags["COC.LOTTIE_MET"] == 2 && flags["COC.LOTTIE_SEX_OPTIONS_UNLOCKED"] > 0) {
			output("\"<i>Hey, " + pc.short + "! I...I really enjoyed what we did. God, if that's what it takes to get healthy I wouldn't mind! I think I'm falling in love with you already, haha!</i>\" She forces a giggle at her last sentence, albeit rather nervously. You raise an eyebrow, but decide not to push the issue further.");
		}
		//[Second Encounter, If didn't fuck Lottie]
		else if(flags["COC.LOTTIE_MET"] == 2) {
			output("\"<i>Hey, " + pc.short + "! I...I'm really glad we're doing this, y'know? I really think I can achieve something this time, if I work hard – and if you work hard with me!</i>\" You smile and tell her how glad you are that she's doing this. Lottie responds with a delighted expression as you both return to normal conversation.");
		...
		//[Encouragement moved from 79 to 80, first time]
			...
			flags["COC.LOTTIE_ENCOURAGEMENT_LAST"] = flags["COC.LOTTIE_ENCOURAGEMENT"];
			//, first time]
			if(flags["COC.LOTTIE_ENCOURAGEMENT_UP_80"] != 1) {
				flags["COC.LOTTIE_ENCOURAGEMENT_UP_80"] = 1;
				output("Lottie seems a lot more nervous than usual, curling her hair around her finger and fidgeting with the cup of tea in her hand. \"<i>H-hi... hi " + pc.short + ".</i>\" she squeaks, unable to form sentences under the immense amount of pressure she's put herself under. You place your hand on hers to relieve her own stress, but this seems to make her more nervous. \"<i>Oh! I... gosh, " + pc.short + ", I don't know what to say!</i>\" Lottie shakes her head several times, muttering a 'pull yourself together, Charlotte!' before starting again. \"<i>I like you! I really, really like you, " + pc.short + "!</i>\" She squeals at the top of her lungs, jumping up in excitement before immediately clapping her hands over her mouth at the realisation that she's caught the attention of the entire cafe. You silently walk over to her, placing her hands back down before pulling her into a warm embrace. \"<i>I... I...</i>\" Looking down, you brush the hair out of her eyes and place a small kiss on her forehead, calming her down considerably. \"<i>Do you like me too?</i>\" She asks. You smile, and hug her tighter than before, the scent of black cherries and roses lingering in the air. Her voice rings through your head while you embrace the girl.  Do you like me too?\n\n");
				output("You don't know the answer.");
			}
			//[Encouragement moved from 79 to 80, regular]
			else {
				output("\"<i>Hey, " + pc.short + ".Thanks for being... nicer, again. Sometimes I really worry about you, but... well; I'm fine with putting that behind us. I'm glad, y'know?</i>\" She throws you a smile, confident that you won't mistreat her again. You smile back, silently assuring her that you won't, today.");
			}
		}
		//[Normal Encounter, Encouragement 60 - 100]
		else if(flags["COC.LOTTIE_ENCOURAGEMENT"] >= 60) {
			//[Figure 0 – 20]
			if(flags["COC.LOTTIE_TONE"] <= 20 && flags["COC.LOTTIE_BIGGER_IS_BETTER"] != 1) output("\"<i>Oh, " + pc.short + "! Let's do our best today, I really wanna lose those pounds! I know I can do my best when I'm with you!</i>\" Lottie beams, her massive tits pushed against the table. She seems to be excited for what you've got in store today.");
			//[Figure 0 - 20, convinced her to accept body]
			else if(flags["COC.LOTTIE_TONE"] <= 20) output("\"<i>Hey, " + pc.mf("stud","cutie") + "! You shouldn't keep a big girl like me waiting; unless you wanna get crushed by these!</i>\" Lottie grins and grabs her luscious breasts firmly, giggling when she notices you're at full attention now. She seems to be excited for what you've got in store today.");
			//Figure 20 - 40, haven't convinced her to accept body]
			else if(flags["COC.LOTTIE_TONE"] <= 40 && flags["COC.LOTTIE_BIGGER_IS_BETTER"] == 0) output("\"<i>Ah, " + pc.short + "! We should probably get started soon; I'm gonna push myself to the limit today!</i>\" Lottie's arms are folded over her chest, subconsciously trying to hide her nervousness, though squishing her pillowy tits in the process. She seems to be excited for what you've got in store today.");
			//[Figure 20 - 40, convinced her to accept body]
			else if(flags["COC.LOTTIE_TONE"] <= 40) output("\"<i>Hey, " + pc.mf("stud","cutie") + "! You shouldn't keep me waiting; the only thing I'm losing right now is my patience!</i>\" Lottie grins and punches your arm playfully, squealing when you try to get her back. She seems to be excited for what you've got in store today.");
			//[Figure 40 - 60]
			else if(flags["COC.LOTTIE_TONE"] <= 60) output("\"<i>Hey! I was looking for you earlier, y'know? I can't wait to get started.</i>\" She looks happy that you've arrived. Lottie leans over to grab her beverage, giving you an excellent view of her supple mounds. She seems to be excited for what you've got in store today.");
			//[Figure 60 - 80]
			else if(flags["COC.LOTTIE_TONE"] <= 80) output("\"<i>Yo, " + pc.mf("big guy","babe") + "! I've been waiting for you, so you better be pumped! Ready to get started?</i>\" Lottie leans over, her creamy tits pushed against the table. She seems to be excited for what you've got in store today.");
			//[Figure 80 - 100]
			else output("\"<i>Hey, " + pc.mf("stud","cutie") + "! You shouldn't keep me waiting, or I'll put you through the burn twice as hard! You ready?</i>\" Lottie winks at you and punches your arm playfully. She seems to be excited for what you've got in store today.");
		}
		//[Normal Encounter, Encouragement 30 - 60]
		else if(flags["COC.LOTTIE_ENCOURAGEMENT"] >= 30) {
			//[Figure 0 - 20]
			if(flags["COC.LOTTIE_TONE"] <= 20) output("\"<i>Oh, " + pc.short + ", hey. I think we should try and get into it today. I don't think I tried hard enough last time, y'know?</i>\" Lottie leans over to grab her beverage, her massive tits pushed against the table. She seems to be intent on trying her best today.");
			//[Figure 20 - 40]
			else if(flags["COC.LOTTIE_TONE"] <= 40) output("\"<i>A-ah, " + pc.short + "! We should probably get started soon; I... I want to push myself to the limit today!</i>\" Lottie's arms are folded over her chest, subconsciously trying to hide her nervousness, though squishing her pillowy tits in the process. She seems curious as to what you both plan to do today.");
			//[Figure 40 - 60]
			else if(flags["COC.LOTTIE_TONE"] <= 60) output("\"<i>H-hey! I was looking for you earlier, y'know? I can't wait to get started.</i>\" She looks at ease now that you've arrived. Lottie leans over to grab her beverage, giving you an excellent view of her supple mounds. She seems curious as to what you both plan to do today.");
			//[Figure 60 - 80]
			else output("\"<i>Hello " + pc.short + "! I've... I've been waiting for you, so you better be ready, hey! Don't hold back!</i>\" Lottie leans over, her creamy tits pushed against the table. She seems to be excited for what you've got in store today.");
		}
		//[Normal Encounter, Encouragement 0 - 30]
		else {
			//[Normal Encounter, Figure 0 – 70]
			if(flags["COC.LOTTIE_TONE"] <= 70) output("\"<i>Oh, " + pc.short + "... I know I haven't been trying the way you want me to. I – I punished myself extra hard last night! Please, let me try again today!</i>\" Lottie leans over and looks at you desperately, her creamy tits pushed against the table. You can see they're redder than before. Has she been trying to slap her fat away? You turn away, not giving her the pleasure of looking at your face. You agree to help her – but don't expect to be looked at by the only person who can stand you if this goes on. Lottie squeals and manages to blurt out a 'th-thank you!' in response.");
			//[Normal Encounter, Figure 70 - 100]
			else output("\"<i>" + pc.short + "! I know you don't like looking at me, but I... I lost some more weight for you today!</i>\" Lottie removes her top and exposes her curvaceous body, plump tits in full view. You stare her down, and she smiles weakly, grateful for your patience. She waits for your response, but you turn away and tell her to cover up her disgusting body. \"<i>But I – oh god, I'm sorry, I'm so sorry, I'll try extra hard today, I'll try so so hard...</i>\" She babbles on while you go to the counter and order some food for yourself.");
		}
	}
	//[Appearance][Talk][Exercise][Give Item][Sex][Hug]
	clearMenu();
	...
	if (item != null) addButton(3, "Give Item", item);
	if (sex != null) addButton(4, "Sex", sex);
	if (hug != null) addButton(5, "Hug", hug);
	addButton(14, "Leave", TelAdreGymDesc);
	flags["COC.LOTTIE_ENCOURAGEMENT_LAST"] = lottieMorale();
	if (flags["COC.LOTTIE_TONE"] == 0) flags["COC.LOTTIE_TONE"] = lottieTone();
}
...
private function fourthMeetingChooseSexWithLottie():void {
	showLottie();
	clearOutput();
	IncrementFlag("COC.LOTTIE_SEX_OPTIONS_UNLOCKED");
	var x:Number = pc.cockThatFits(40);
	if(x < 0) x = 0;
	output("You tell her that you're fine with the idea, as long as she's willing to follow your lead. \"<i>W-wow, you really wanna... and with me? Okay, sure, lemme just uh, hang on...</i>\" She fumbles with her clothing, clearly trying to get undressed in public before you decide to stop her – there are far better places for this. Taking her by the hand, you lead her into one of the empty saunas, your erection stirring in your pants as you grow more and more eager to take her. \"<i>" + pc.short + "... I'm – I really don't know where to start, h-hey.</i>\" She throws an uneasy glance at you, her intent for you to take the lead apparent in her heavy breathing and wanting eyes. Not one to disappoint, you grab her by the hips and lean her against the wall, ripping off her clothes and revealing her velvety skin underneath. Reaching down, you grab a handful of her luxurious ass, squeezing and slapping it lightly. Lottie moans into your shoulder and reaches towards your groin, removing your [pc.gear]. She begins stroking your [pc.cock " + x + "] lightly, picking up the pace as you continue to work on her pert rump. You can't help but lust over her full figure, using one of your hands to firmly grope her pillowy jugs and reaching your other towards her glistening pussy. You move your hand over her cushiony rack before burying your head into her bosom and eagerly sucking on one of her nipples, fondling the other between your thumb and forefinger. Your other hand is already fast at work, fingering her hot cunt. You can feel her legs about to buckle beneath their own weight. Lottie moans in between short, hot breaths, \"<i>Y-you're... really amazing, " + pc.mf("guy","lady") + ".</i>\" You give her chubby ass a hard slap in agreement.  Lottie squeals with  glee, jerking your [pc.cock " + x + "] as it pokes into her soft stomach. The sensation of your tip rubbing against her velvety skin drives you wild.\n\n");
	output("You tell Lottie you've decided you're going to take her for a vigorous work out. She can only pant with anticipation.\n\n");
...
private function declineBeingLottiesGymHamsandwich():void {
	showLottie();
	clearOutput();
	output("\"<i>O-oh, um... okay. I understand! You're probably busy what with all that lifting you're getting up to, haha!</i>\" The pig girl chuckles nervously, shifting her towel to cover more of her soft cleavage.  \"<i>I'll uh – I'll go get outta your hair then. I've got some stuff to do at home.</i>\"  You watch as she jogs off through the door and out into Tel'Adre, towel and all.  You decide you better return home too, and head back towards the camp.");
	//[PC no longer encounters Lottie.]
	flags["COC.LOTTIE_DISABLED"] = 1;
	processTime(20 + rand(10));
	clearMenu();
	addButton(0, "Next", mainGameMenu);
}
...
//Followups to this: output("\"<i>Hey, " + pc.short + "! I was just... I...</i>\"Lottie seems to be at a loss of words, gaping at your new, chiselled physique. You smile and tell her how glad you are that she's doing this, thought it's a shame she hasn't found a personal trainer yet. Lottie responds with a hazy expression and an open mouth. \"<i>Y-yeah... it's a shame...</i>\" She begins, until snapping out of her daze and looking up excitedly at you. \"<i>I- Y'know, I don't know what you've been doing to yourself lately, but... you look a... you're really amazing right now, " + pc.mf("big guy","babe") + "!</i>\" She squeals as you flex at her, giggling at your blatant attempts at showing off. You never really took her as one to appreciate muscles, but you figure her obsession towards the perfect body might extend to other people as well. You take one stern look at the chubby porcine in front of you and grin, asking if she wants you to be her personal trainer. She seems shocked at your proposal. </i>\"Are... are you serious?</i>\"\n\n", false);
//If No]
private function gotInShapeButTurnedDownLottie():void {
	showLottie();
	output("\"<i>O-oh, um... okay. I understand! You're probably busy what with all that lifting you're getting up to, haha!</i>\" The pig girl chuckles nervously, shifting her towel to cover more of her soft cleavage. \"<i>I'll uh – I'll go get outta your hair then. I've got some stuff to do at home.</i>\" You watch as she jogs off through the door and out into Tel'Adre, towel and all. You decide you'd better return home too, and head back towards the camp.", false);
	//[PC no longer encounters Lottie.]
	flags["COC.LOTTIE_DISABLED"] = 1;
	processTime(20 + rand(10));
	clearMenu();
...
public function gotInShapeComedyDeclineLottie():void {
	showLottie();
	clearOutput();
	output("Haha, of course you fucking aren't, why would you be? Lottie stares at you, tears welling in her eyes. Oh, whoops – you must've said that out loud. Proceeding to not give two shits about the bawling pig-slut in front of you, you take a good squeeze of her cushiony tits before whipping your " + pc.cockDescript(pc.biggestCockIndex()) + " out and slapping her in the face with it.  \"<i>W-w-why... why are you... *sniff* p-please...!</i>\" The pig girl chokes in between sobs. It almost gives you a hard on - almost. You take one final step backwards, before announcing quite loudly to the other cafe-goers that you're going for a home run. Lottie squeaks before you take your " + pc.cockDescript(pc.biggestCockIndex()) + " and swing at her face, hitting dead on and managing to leave a bit of pre-cum on her newly-christened cheek. You stand triumphantly at your latest work of art, though Lottie seems to have other ideas, running out the door in complete misery, pre-cum and all. You decide you'd better return home too, and head back towards the camp – but not before spending the next hour or so chasing after Lottie and taking grabs at her plush ass.");
	//[PC no longer encounters Lottie.]
	flags["COC.LOTTIE_DISABLED"] = 1;
	processTime(20 + rand(10));
	clearMenu();
...
private function gotInShapeAcceptTrainingOffer():void {
	...
	flags["COC.LOTTIE_TRAINING_ACCEPTED"] = 1;
	clearOutput();
	output("\"<i>A-ah, really? Y'mean it? This isn't some trick or something, right? You ain't about to go ahead and flake out on me, are you?</i>\" You assure her you're serious if she is, though you make a mental note that she's more likely to flake out than you are, given her history. \"<i>Eeee! Thanks, " + pc.mf("big guy", "babe") + " – I can't wait to get started!</i>\" You chuckle, remarking on her enthusiasm. \"<i>Well " + pc.short + ", I'm excited because I think the two of us are about to become super close! Do you feel it? Do ya?</i>\" Lottie wiggles in excitement, loosening the already skimpy towel and revealing one of her perky nipples. You definitely feel something, that's for sure. She follows your eyes down to her breasts and reddens as she quickly covers herself up as best as she can - only succeeding in making her pillowy breasts push up against the towel, increasing her cleavage.\n\n");

	output("You take this opportunity to get a good look at Lottie. She stands at around 4'8</i>\", considerably short on average, ");
	if(pc.tallness > 60) output("and shorter than you");
	else if(pc.tallness > 53) output("but you can't judge, being roughly the same height");
	else output("taller than you");
...
private function drinkLustWithYourHamhock():void {
	showLottie();
	clearOutput();
	output("You decide to down the cup of pink liquid, savoring the sweet taste as it pours down your throat. Your thirst quenched, you start to walk back towards the track when you're suddenly overcome with a warm feeling that stretches down to your groin. [pc.EachCock] stirs from beneath your [pc.gear] – you need to fuck something, and fast.  \"<i>Uh, is something wrong?</i>\" Lottie glances over to you, noticing the bulge in your pants as it strains against your [pc.gear]. She looks up at your face, concerned about your well being. You grin and tell her you've had enough running.  Confused, Lottie asks what you mean.  You explain that you know a far more effective way to lose weight and stay in shape – in fact, it's how you got to be the way you are now.  You can see Lottie's eyes widen at the idea of such a technique, especially considering your own well-muscled physique.  \"<i>I'm all ears! What is it?</i>\"  She asks.  Clearly not getting the picture, you grab her hand and put it on your crotch.  Lottie quietly squeals and looks around to see if anyone noticed before looking back at you.  \"<i>Oh, um, I don't- I don't know about that, " + pc.short + "...</i>\" She mumbles, her hand still remaining on your growing hard-on.  It's perfectly fine - you've trained like this with hundreds of people, you lie.  Lottie swallows and looks down at the sound of 'hundreds'.  Not being one to question your methods, she begins to stroke your hardening length through your [pc.gear], using her palm to massage your shaft");
	//[if balls]
	if(pc.balls > 0) output(" while her fingers nervously trace around your balls");
	output(". You stop her for now, though not before softly squeezing her hand as it caresses your girth. You know where you two can go 'work out', you tell her. She nods, her eyes glued to your erection.\n\n");

	output("You take her by the hand and lead her into one of the empty saunas. \"<i>" + pc.short + "...</i>\" You grab her by the hips and lean her against the wall, throwing off her towel and revealing her velvety skin underneath. Reaching down, you grab a handful of her luxurious ass, squeezing and slapping it lightly. Lottie moans into your shoulder and reaches towards your groin, removing your [pc.gear]. She begins stroking [pc.oneCock] lightly, picking up the pace as you continue to work on her pert rump. You can't help but lust over her full figure, using one of your hands to firmly grope her pillowy jugs and reaching your other towards her glistening pussy. You move your hand over her cushiony rack, before burying your head into her bosom and eagerly sucking on one of her nipples, fondling the other between your thumb and forefinger. Your other hand is already fast at work, fingering her hot cunt. You can feel her legs about to buckle beneath their own weight. Lottie moans in between short, hot breaths, \"<i>Y-you're... really amazing, " + pc.mf("guy","lady") + ".</i>\"  You give her chubby ass a hard slap in agreement.  Lottie squeals in glee, jerking your [pc.cock] as it pokes into her soft stomach. The sensation of your tip rubbing against her velvety skin drives you wild.\n\n");
...
public function analLottieMilkshake():void {
	showLottie();
	clearOutput();
	//output(images.showImage("lottie-fuck-her-in-teh-butzor"));
	var x:Number = pc.cockThatFits(40);
	if(x < 0) x = 0;
	output("You brush her grip off of [pc.oneCock], only to turn her around and fit your girth between her large cheeks");
	if(pc.totalCocks() == 2) output(", managing to squeeze in your other cock along with it");
	else if(pc.totalCocks() > 2) output(", the rest of your dicks poking and prodding against her soft flesh");
	output(".  \"<i>You sure do like my butt, huh?</i>\"  She giggles softly, satisfied that at least some of her fat is being put to use.  You groan as she starts to move her ample hips, massaging your [pc.cock " + x + "] within her jiggling buttocks.  Your tip brushes against her asshole, smearing pre-cum all over the hot entrance.  Lottie starts to push back towards your cock, clearly overcome with lust.  You wait a few seconds as she aimlessly grinds against you before you spread her cheeks and plunge your [pc.cock " + x + "] into her tight backdoor. Lottie squeals against the wall in delight as you fill her stomach and begin pounding away at her relentlessly, her large tits bouncing rhythmically. She continues to drive herself into you, her tight, fiery insides squeezing your length whilst you watch her heavy butt ripple with every thrust.\n\n");

	output("Lottie is a complete mess, gasping and convulsing underneath you as you plunge into her depths. Her mouth hangs wide open as she holds on to one of her plush tits, using her free hand to pleasure herself. You take hold of her lavish buttcheeks and force your length in deep, bottoming out inside her hot ass. You remain inside her and begin gyrating your hips, rubbing against the tight walls of her interior. Her legs collapse underneath you and you wrap one hand around her pot belly to hold her in place, resuming your thrusting.\n\n");

	output("\"<i>A-aa-ah, I-I... AHH!</i>\" Lottie oinks noisily, her eyes rolling to the back of her head and her large thighs slapping against your own, pulling your [pc.cock " + x + "] further in. She shudders in orgasm");
	if(pc.balls > 0) output(", her soaked pussy rubbing against your balls");
	output(".\n\n");

	//[If Multi]
	if(pc.cockTotal() > 1) {
		output("Deciding you aren't done with her bouncy rear, you take ");
		if(pc.cockTotal() == 2) output("your other cock");
		else output("one of your cocks");
		output(" and push it against her puckered hole. \"<i>H-hey, wait a mome- OHH!</i>\" You ease yourself back into her, with ");
		if(pc.cockTotal() == 2) output("your two ");
		else output("two of your ");
		output(" throbbing cocks buried deep in her rectum. The pressure against your hot rods is immense, though you continue to slowly push and pull her sweaty ass against you, enjoying the feeling of her meaty fuckpillows as they massage your cock");
		if(pc.cockTotal() > 1) output("s");
		output(".\n\n");
	}
	if(pc.balls > 0) output("Your [pc.sack] begins to swell");
	else output("You feel yourself about to explode");
	output(" with the large amount of cum you've prepared to paint her inner walls with.  Your unrelenting fucking picks up the pace as you feel yourself coming to a climax.  Lottie's soft flesh starts to quiver, signaling her second orgasm.  You lift Lottie's plump body up straight and against yours as you groan loudly and thrust hard, the noisy broad ");
	if(pc.cumQ() >= 1000) output("overflowing");
	else if(pc.cumQ() >= 500) output("dripping");
	else output("filled", false);
	output(" with your hot, thick spunk. She squeals again as you bring her to orgasm, ramming herself into you to collect as much of your cum as possible. You continue to pound her bouncy buttocks as you release the rest of your seed inside of her");
	//[If large cum production]
	if(pc.cumQ() >= 500) output(", a heavy amount leaking from her ass");
	output(".  Pulling yourself free, you wipe yourself against her plush butt and give it one final slap before walking out of the sauna.  \"<i>I-I'll see you again, hey?</i>\" Lottie asks, collapsed onto the floor in a mess of sweat and drool. You give her a thumbs up before turning around and heading out of the gym, making sure to grab a ");
	if(silly) output("bacon ");
	output("shake on the way out.");
	//LOTTIE FIGURE +1, MUSCLE TONE +1, DRAINS PC'S LUST.
	lottieTone(1);
	if(flags["COC.LOTTIE_ANAL_TALKS"] == undefined) flags["COC.LOTTIE_ANAL_TALKS"] = 1;
	output(pc.modTone(1));
	pc.orgasm();
...
public function getSuckedOffByAHamSandwich():void {
	showLottie();
	clearOutput();
	//output(images.showImage("lottie-gives-you-a-blowjob"));
	var x:Number = pc.biggestCockIndex();

	output("You grab Lottie's shoulders and firmly move her down towards your crotch. She kneels down, face to face with your [pc.cock " + x + "].  Feeling her hot breath against your " + pc.skinFurScales() + ", you tell her to suck it.\n\n");

	//[If player cock is at or below 4']
	if(pc.cocks[x].cLength() <= 4) {
		output("She stares at your [pc.cock " + x + "] and then up at you. \"<i>I... oh, well I guess this is okay.</i>\" She lazily slurps your member into her mouth, and moans at the taste of your [pc.cock " + x + "]. Her agile tongue moves over your entire length, licking gently over your sensitive head and along your shaft. Your [pc.cock " + x + "] sits comfortably inside her warm hole, covered in her saliva");
		if(pc.cockTotal() > 1) {
			output(" while your other cock");
			if(pc.cockTotal() == 2) output(" pokes");
			else output("s poke");
			output(" against her chin");
		}
		output(".\n\n");
	}
	//[If the player has a cock between 5' – 12']
	else if(pc.cocks[x].cLength() <= 12) {
		output("She stares intently at your [pc.cock " + x + "] with hunger in her eyes. A small amount of drool escapes her luscious lips as she moves forward, kissing your ");
		if(pc.cockTotal() > 1) output(" biggest head lightly");
		else output(" head lightly");
		output(".  Her saliva leaks onto your [pc.cock " + x + "], though she is quick to lick it up while she works her tongue around the rest of your shaft");
		if(pc.cockTotal() > 1) output(", jerking off your " + pc.cockDescript(pc.biggestCockIndex2()) + " with both hands");
		output(".  She places small licks on the tip of your head, and then hungrily devours the rest of your length in one fell swoop.\n\n");
	}
	//[If the player has a cock equal to or over 13']
	else {
		output("She gasps as you lift your [pc.cock " + x + "] and slap her face. \"<i>It's... wow, " + pc.short + "!</i>\" You grin before poking her pudgy cheek with the tip of your ");
		if(pc.cockTotal() > 1) output("biggest erection");
		else output("erection");
		output(".  A small amount of drool escapes her luscious lips as she moves forward, kissing your head lightly. Her saliva leaks onto your [pc.cock " + x + "], though she is quick to lick it up while she works her tongue around the rest of your lengthy shaft");
		if(pc.cockTotal() > 1) output(" - using her hands to massage your leftover " + pc.cockDescript(pc.biggestCockIndex2()));
		output(". She places small licks on the tip of your head, and without warning stuffs as much of your thick girth as she can into her mouth, coughing around your [pc.cock " + x + "].\n\n");
	}

	output("She begins fellating your cock at an incredible rate, throatfucking herself on your " + pc.cockDescript(x));
	if(pc.balls > 0) output(", your sack slapping against her chin ");
	else if(pc.biggestTitSize() > 2) output(" while you play with your [pc.nipples]");
	output(". You remark on what a hungry little piggy she must be, and the chubby girl moans in response, the vibrations massaging your member. Relishing the feeling of your [pc.cock " + x + "] in between her hungry lips, you start to thrust into her warm mouth hole, grabbing onto her head as you pound her face against your groin.");
	if(pc.cockTotal() > 1) {
		if(pc.cockTotal() == 2) output("  Your extra cock flails wildly, slapping against her cheeks.");
		else if(pc.cockTotal() > 2) output("  Your extra cocks flail wildly, slapping against her cheeks.");
	}
	output("  Lottie's slick tongue desperately wraps itself around your length as you drive in and out of her fuckhole relentlessly, drool beginning to leak from her mouth. She");
	//[if Multi]
	if(pc.cockTotal() > 1) output(" stops rubbing your [pc.cock " + x + "] to ");
	//(Otherwise)
	else output(" reaches down towards her dripping pussy; plunging two fingers into her wetness while using the other hand to ");
	//If Balls]
	if(pc.balls > 0) output("knead your [pc.balls]");
	else output("reach around and squeeze your [pc.ass]");
	output(".  You grab the back of her head and pull her in further, bobbing her head while she slurps and sucks at your meatstick. The stimulation almost causes you to explode right then and there, but her bouncing tits give you an idea.\n\n", false);

	output("You slowly take your [pc.cock " + x + "] out of her hot mouth, a small *pop* breaking the silence as it escapes her plump, puckered lips. Grabbing one of her hefty tits in your hands, you rub your cockhead against her erect nipple, eliciting a small moan from the sweaty slut. You slap your [pc.cock " + x + "] against her sweaty chest a few times before burying it in between her cleavage, ");
	//[If the player has a cock equal to or over 13']
	if(pc.cocks[x].cLength() > 12) output("your sensitive tip brushing against her eager mouth");
	//Otherwise]
	else output("resting comfortably in between her cushiony fuckpillows");
	output(". Lottie wraps her tits around your length and begins working at pleasuring your every inch.\n\n");

	//[If the player has a cock equal to or over 13']
	if(pc.cocks[x].cLength() > 12) output("Licking at your cockhead, Lottie squishes her breasts against the rest of your member, making sure to lap up the stray beads of pre-cum squirting from your tip. Relishing the taste of your [pc.cock " + x + "] in her mouth she begins to suck at your head, flicking her tongue around your urethra and pressing her full lips against it, creating a vacuum of pleasure around your manhood.\n\n");

	//[If Multi]
	if(pc.cockTotal() > 1) {
		output("You decide to make use of ");
		if(pc.cockTotal() == 2) output("your");
		else output("an");
		output(" extra cock as you push it between her pouty lips, her face brushing against it while she tries to titfuck your main member. Looking up at you, Lottie descends on you and begins fellating your " + pc.cockDescript(pc.biggestCockIndex2()) + ", not forgetting to knead her fat tits into your [pc.cock " + x + "].\n\n");
	}

	output("Raising her body up and down, you watch as the pig-slut massages her rack tightly around your rod whilst you plunge between her cleavage, thrusting slightly to match her movements. \"<i>" + pc.short + ", p-please cum on me, god- I need this!</i>\" she moans, her saliva lubing her chest while she pumps your dick in between her melons");
	if(pc.cockTotal() > 1) output(" and chokes on your meat");
	output(".  You throb against her hot flesh, feeling your spunk bubbling up inside you.  Throwing your head back, you give one final thrust before pulling back and exploding all over your squishy slut, ", false);
	if(pc.cumQ() < 500) output("thick ropes of cum splattering all over her face and pooling in between her tits");
	else output("massive globules of cum splattering all over her face and painting her tits");
	output(".  She continues to move up and down against the thick shaft between her breasts, determined to drain you of your seed. She pulls you out of her bosom and plunges your member back into her mouth, suckling on it for any stray milk left. After a while, you groan as she pops you back out, collecting the ");
	if(pc.cumQ() > 500) output("incredible amounts of ");
	output("cum you've painted on her flesh and gulping it all down. \"<i>Thanks for the load, " + pc.mf("big guy","babe") + ",</i>\" she remarks, sucking gently on her fingers. \"<i>I'll... I'll see you again, hey!</i>\" You pat her on the head and begin to make your way out of the gym, making sure to grab a ");
	if(silly) output("bacon ");
	output("shake on the way out.");
	//LOTTIE FIGURE -1, MUSCLE TONE +1, DRAINS PC'S LUST.
	lottieTone(-1);
	if(flags["COC.LOTTIE_ORAL_TALKS"] == undefined) flags["COC.LOTTIE_ORAL_TALKS"] = 1;
	output(pc.modTone(1));
	pc.orgasm();
	processTime(45 + rand(10));
	clearMenu();
...
private function lotteryAppearance():void {
	...
	//[If Figure between 80 - 100]
		...
		output(". Physique-wise, Lottie is in excellent shape, her curvy figure drawing the eyes of many males in the vicinity. Her hard work has definitely paid off. Of the few pig people you have seen around Tel'Adre, she is definitely the most conventionally attractive by far. What fat she does have is focused on her perky d-cups, girly thighs and cute ass, her flat stomach showcasing her shapely abdominal muscles that are more than capable of supporting the weight of her upper body. Her heart-shaped face is complimented by her button nose and soft lips, framed by her pink-gray hair, which ends in small curls. A small, coiled tail rests above her rump.");
	}
	//Back to lottery menu
	clearMenu();
	addButton(0, "Next", encounterLottie);
}

public function lottieTone(dif:Number = 0):Number {
	if(dif == 0) return flags["COC.LOTTIE_TONE"];
	if(debug) output("\n<b>(DEBUG: Lottie Tone changed by " + dif + "!)</b>\n");
	flags["COC.LOTTIE_TONE"] += dif;
	if(flags["COC.LOTTIE_TONE"] > 100) flags["COC.LOTTIE_TONE"] = 100;
	else if(flags["COC.LOTTIE_TONE"] < 0) flags["COC.LOTTIE_TONE"] = 0;
	//Clear blowjob/anal change talk flags in case tone is getting out of whack.
	if(flags["COC.LOTTIE_ORAL_TALKS"] == 1) flags["COC.LOTTIE_ORAL_TALKS"] = 0;
	if(flags["COC.LOTTIE_ANAL_TALKS"] == 1) flags["COC.LOTTIE_ANAL_TALKS"] = 0;
	return flags["COC.LOTTIE_TONE"];
}
public function lottieMorale(dif:Number = 0):Number {
	if(dif == 0) return flags["COC.LOTTIE_ENCOURAGEMENT"];
	if(debug) output("\n<b>(DEBUG: Lottie Morale changed by " + dif + "!)</b>\n");
	flags["COC.LOTTIE_ENCOURAGEMENT"] += dif;
	if(flags["COC.LOTTIE_ENCOURAGEMENT"] > 100) flags["COC.LOTTIE_ENCOURAGEMENT"] = 100;
	else if(flags["COC.LOTTIE_ENCOURAGEMENT"] < 0) flags["COC.LOTTIE_ENCOURAGEMENT"] = 0;
	return flags["COC.LOTTIE_ENCOURAGEMENT"];
}
...
private function talkToPigSlut():void {
	...
	//FIRST TIMER SHIT
	//[If 1st Time Oral]
	if(flags["COC.LOTTIE_ORAL_TALKS"] == 1) {
		flags["COC.LOTTIE_ORAL_TALKS"] = 2;
		output("Her ears perk up at your eagerness to talk, but you can tell she's otherwise gloomy. You ask her what's wrong. \"<i>That obvious, huh?</i>\" Lottie looks down onto the table.  \"<i>Y'see, " + pc.mf("big guy","babe") + ", ever since I... you know, I've actually - I've gained weight!</i>\"  She stands up, her chubby cheeks puffed out in frustration. \"<i>I mean, i-it's not like I didn't enjoy it or anything, but... it seems to be having the opposite effect of what I want!</i>\" She's clearly annoyed, but you place your hand on her shoulder. Do you encourage her, or is it her own fault?");
		//[Encourage][Abuse]
		clearMenu();
	...
	//[If 1st Time Anal]
	else if(flags["COC.LOTTIE_ANAL_TALKS"] == 1) {
		flags["COC.LOTTIE_ANAL_TALKS"] = 2;
		output("Her ears perk up at your eagerness to talk, she seems to be pretty excited about something. You ask her what's on her mind. \"<i>That obvious, huh?</i>\" Lottie looks up at you and grins.  \"<i>Y'see, " + pc.mf("big guy","babe") + ", ever since we... you know, I've actually – I've lost weight!</i>\" She stands up, pumping her fist in the air and wiggling her plump ass. \"<i>I mean, I know it's not a big difference, but it's still a result! This is amazing!</i>\" She's clearly pleased with herself. You place a hand on her shoulder, and she beams at you. Do you encourage her, or break the bad news?");
		//[Encourage][Abuse]
		clearMenu();
	...
	else if(lottieMorale() <= 70 && flags["COC.LOTTIE_TONE"] - 5 > lottieTone()) {
		output("Her ears perk up at your eagerness to talk, but you can tell she's otherwise gloomy. You ask her what's wrong. \"<i>That obvious, huh?</i>\" Lottie looks down onto the table.  \"<i>Y'see, " + pc.mf("big guy","babe") + ", ever since we last met, I've actually - I've gained weight!</i>\" She stands up, her chubby cheeks puffed out in frustration. \"<i>I mean, i-it's not like I'm angry at you or anything, but... I'm just a little frustrated, that's all.</i>\" She's clearly more than a little frustrated, but you place your hand on her shoulder. Do you encourage her, or is it her own fault?");
		//[Encourage][Abuse]
		clearMenu();
	...
	else if(flags["COC.LOTTIE_TONE"] - 5 > lottieTone() && flags["COC.LOTTIE_BIGGER_IS_BETTER"] == 1) {
		output("Her ears perk up at your eagerness to talk, though she seems rather distracted by something. You ask her what's wrong. \"<i>Oh babe, it's really nothing...</i>\" Lottie looks up and smiles, but your questioning gaze earns another answer. \"<i>Dang, y'see " + pc.mf("big guy","babe") + ", ever since we last met, I've gone ahead and gained a bit more weight...</i>\" She remains in her chair, resting her face between her arms, though her chubby cheeks remain puffed out in frustration. \"<i>I mean, I love my body and I love you, but... it gets to me sometimes, y'know?</i>\" She's clearly more than a little frustrated, but you place your hand on her shoulder, rubbing it gently. Do you encourage her, or is it her own fault?", false);
		//[Encourage][Abuse]
		clearMenu();
	...
	else if(flags["COC.LOTTIE_TONE"] - 5 > lottieTone()) {
		output("Her ears perk up at your eagerness to talk, though she seems rather distracted by something. You ask her what's wrong. \"<i>Oh babe, it's really nothing...</i>\" Lottie looks up and smiles, but your questioning gaze earns another answer. \"<i>Dang, well, y'see " + pc.mf("big guy","babe") + ", ever since we last met, I've gone ahead and gained weight!</i>\" She stands up, her chubby cheeks puffed out in frustration. \"<i>I mean, i-it's not like I'm angry at you or anything, but... I'm just a little frustrated, that's all.</i>\" She's clearly more than a little frustrated, but you place your hand on her shoulder. Do you encourage her, or is it her own fault?", false);
		//[Encourage][Abuse]
		clearMenu();
	...
	else if(lottieMorale() <= 70 && flags["COC.LOTTIE_TONE"] < lottieTone() - 5) {
	...
	else if(flags["COC.LOTTIE_TONE"] < lottieTone() - 5) {
		output("Her ears perk up at your eagerness to talk; she seems to be pretty excited about something. You ask her what's on her mind.  \"<i>That obvious, huh?</i>\" Lottie looks up at you and grins. \"<i>Y'see babe, since the last time we met, I've actually – I've lost weight!</i>\" She stands up, pumping her fist in the air and wiggling her plump ass. \"<i>I mean, I know it's not a big difference, but it's still a result! This is amazing!</i>\" She's clearly pleased with herself. You place a hand on her shoulder, and she beams at you. Do you encourage her, or break the bad news?\n\n");
		clearMenu();
	...
	//[If Neither, Encouragement 70 - 100]
		...
		output("<b>SHIT BUGGED OUT SON.</b>");
		clearMenu();
		addButton(0, "Next", TelAdreGymDesc);
	}
	flags["COC.LOTTIE_TONE"] = lottieTone();
}
...
private function lottieLowMoraleWeightGainEncourage():void {
	showLottie();
	clearOutput();
	output("You tell her that she shouldn't cry, or worry – sometimes, these things happen; results don't magically appear in the first few weeks, right? Her weight is bound to fluctuate now and then, it's completely natural. She looks up at you, confused at your tone of voice, but relieved at the same time. \"<i>I... okay. Th-thank you, " + pc.short + ".</i>\" You nod and smile, waving it off as nothing. She sniffs, wiping away her tears and taking a deep breath. You relinquish your grip on her chin, and she relaxes considerably. Lottie continues to stay at ease for a few more minutes before you both decide to hit the track.\n\n");
	output("<b>...</b>After a good workout, you decide to return back to camp, exhausted.");
	//ENCOURAGEMENT +2
	lottieMorale(3);
	pc.energy( -10);
	processTime(50 + rand(10));
...
private function lottieMediumMoraleWeightGainEncourage():void {
	showLottie();
	clearOutput();
	output("You tell her it isn't her fault; these things happen all the time for a variety of reasons. Her weight is bound to fluctuate now and then, it's completely natural. She looks up at you, her eyes welling up slightly. \"<i>Is that true? It's completely natural?</i>\" You nod your head. \"<i>I'm glad. I'm so glad.</i>\" She sniffs, wiping away her tears and taking a deep breath. You rub her shoulder as she places her hand upon yours. Lottie continues to talk about her day for a few more minutes before you both decide to hit the track.\n\n");
	output("<b>...</b>\nAfter a good workout, you decide to return back to camp, exhausted.");
	//ENCOURAGEMENT +2
	lottieMorale(3);
	pc.energy( -10);
...
private function lottieHighMoraleWeightGainEncourage():void {
	showLottie();
	clearOutput();
	output("You tell her it's never her fault for something like that; and it's perfectly fine to be frustrated – as long as it doesn't detract from her main goal. Her weight is bound to fluctuate now and then, it's completely natural. She looks up at you, beaming at your kind words. \"<i>" + pc.short + ", you really do know how to make people feel better, y'know?</i>\" You nod your head in agreement – you know. \"<i>I'm glad. I'm so glad.</i>\" She sighs, taking a deep breath. You rub her shoulder as she places her hand upon yours. Lottie continues to talk about her day for a few more minutes before you both decide to hit the track.\n\n");
	output("<b>...</b>\nAfter a good workout, you decide to return back to camp, exhausted.");
	//ENCOURAGEMENT +2
	lottieMorale(3);
	pc.energy( -10);
...
private function lottieLowMoraleWeightLossEncourage():void {
	showLottie();
	clearOutput();
	output("You congratulate her on her success, in fact – you could almost say you are quite happy with her results. Lottie is visibly confused, not expecting you to react this way. Regardless, she takes the compliment for what it's worth and squeals happily, popping your finger into her warm mouth and suckling gently. Pulling your hand away, you pat her softly on the head. \"<i>Th-thank you... thanks, " + pc.short + ".</i>\"  You both share a smile, Lottie at ease with your unnaturally welcoming nature. You stand up, telling her you're going to go hit the track, if she's willing to join you – and of course, Lottie eagerly follows.\n\n");
	output("<b>...</b>\nAfter a good workout, you decide to return back to camp, exhausted.");
	//ENCOURAGEMENT +2
	lottieMorale(3);
	pc.energy( -10);
...
private function lottieHighMoraleFatYoullGetThereOneTimeEvent():void {
	...
	var x:Number = pc.cockThatFits(40);
	if(x < 0) x = 0;
	output("You tell Lottie that the way she looks right now shouldn't affect how she feels about herself as a person – her value isn't determined by her weight, and never will be. You pat her hand before giving her a thumbs up. You believe in her and you believe in her goal, whether it leads to a better appreciation for herself or her body. She's reassured by your words, stroking your hand gently. You make clear to the girl that you aren't going to leave her because of how she looks, because she's beautiful where it counts. Lottie melts, blushing hard at your compliments. \"<i>Oh, " + pc.short + ", sometimes I wish I could be as brave as you, y'know?</i>\" she exclaims, sighing politely. Continuing your rant, you explain how all she needs is a little self confidence to get her going – she can't be afraid to take leaps into the deep end to get what she wants, right? She giggles at your enthusiasm, though takes your words into consideration. \"<i>Self confidence, huh?</i>\" Lottie mutters to herself. \"<i>" + pc.short + "... could you come with me for a moment?</i>\"\n\n");

	output("You follow Lottie out into the pool area, waiting as she walks into the dressing room to get changed. When she comes out however, you're surprised to find that she hasn't changed into anything – in fact, she's removed all her clothes entirely!\n\n");

	output("The nude pig-girl stands in front of you, turning red at your hungry gaze. \"<i>I'm not comfortable with my body, but... if self confidence is the key to getting what I want, then... will you fuck me, " + pc.short + "?</i>\"\n\n");

	output("In only seconds flat you remove your clothes to jump the piggy, whipping out your [pc.cock " + x + "] and pushing it against Lottie's wet slit, the pig-girl lying on her back while her legs rest upon your shoulders. With a hearty thrust you drive yourself into her, eliciting a loud moan while you pork her insides with your meat. Her thick thighs slap against you as you bend over to take a lustful grab at her melons, sucking at her nipple and pinching the other, her sweet taste enveloping your mouth. Lottie buries you in her cleavage, yelping as you piledrive her tight pussy into submission – but you've got more in store. Flipping her over, Lottie lies face down with her soft butt jutting up in the air, your persistent fucking driving her wild. You continue gripping onto her tits as you pound her flesh, every thrust accompanied by her pleasured grunts. Back and forth you mount her until the familiar sensation in your loins signal your release. You increase your pace, pounding her pussy until you pull your head back and ejaculate inside her inner walls, slamming in harder every few seconds with each burst.");
	if(pc.cockTotal() > 1) {
		output("Your other cock");
		if(pc.cockTotal() == 2) output(" twitches ");
		else output("s twitch ");
		output("in ecstasy, releasing all over her plump butt.");
	}
	output("\n\n");

	output("Sweaty and panting, Lottie lies on the floor, exhausted and full of your spunk. You slap her jiggling ass before walking out of the pool area, making sure to grab a ");
	if(silly) output("bacon ");
	output("shake on the way out.");
	if(silly) output("  They're fucking delicious.");
	output("\n\n");
	//[Unlocks Quickie]
	output("(<i>'Quickie' has been unlocked from Lottie's sex menu.</i>)");
	flags["COC.LOTTIE_QUICKIE_ENABLED"] = 1;
	IncrementFlag("COC.LOTTIE_SEX_OPTIONS_UNLOCKED");
	pc.orgasm();
	//dynStats("sen", -3, "cor", -1);
	pc.cor( -1);
...
public function ewLottieNo():void {
	showLottie();
	clearOutput();
	output("You begin to talk about your story, about how you came to be in Mareth and how you got to be where you are now. You've encountered nasty things, and you've seen people become nasty things themselves. You also know that you've changed since then. You've become someone you didn't expect yourself to become in a million years – and whether it was for better or for worse was up to you to decide. You know you've been through hard times, but you've never once regretted the decisions you've made – because they led to her. To Lottie. The girl begins to tear up at your confession. \"<i>Oh, " + pc.short + ", I...</i>\" But then you laugh. You laugh harder than you ever have since you came into this hellhole. In fact, you'd be willing to bet it was the first time you've ever laughed in Mareth. Lottie's face immediately scrunches up as she begins to bawl, but you could give less of a fuck as you stand up and point at the pig-slut as cafe-goers look at you both in curiosity. \"<i>ATTENTION FURRY ASSHOLES</i>\" you yell, much to the disgust of the anthropomorphic animal-people staring in your direction. \"<i>THIS SLUT – \"<i>you point to Lottie. \"<i>- WON'T EAT MY HAM SANDWICHES.</i>\" An audible gasp is heard from the growing crowd. Lottie's buried her head in her hands but your tirade continues. Walking to the counter, you take one look at the fuzzy clerk before he nods and hands you the aforementioned sandwich. Weird, you don't remember him being fuzzy – you really need to find a pair of fucking glasses in this sand pit. Regardless, you stomp over to the pig-girl, pulling her out of her seat and forcefully shoving the meaty meal down her throat. She gags for a second before unwillingly swallowing her own species. You grab her cheeks and pull her face several inches towards yours. \"<i>You disgust me.</i>\" You tell her, before whipping your [pc.cock] out and fucking her ham and bread-filled mouth. </i>\"Here comes the mayonnaise train!</i>\" You yell, ejaculating straight down her gullet and into her flabby abyss.\n\n");

	output("For hours, people take turns pumping their seed into Lottie while you dance naked around her, chanting obscenities at the blurry people around you.\n\n");

	output("You really need glasses.");
...
private function leaveLottieInsteadOfYogaSex():void {
	showLottie();
	clearOutput();
	output("As excited as you are, your mind is telling you to leave. In blissful ignorance, you walk out the door of the cafe, through the pool area, into the main gym and out the main entrance. Once outside, you figure it's as good a time as any to take a friendly stroll back to the camp.\n\n");
	output("As you head home, you have the strangest feeling you've missed out on incredibly sexy sex.");
	processTime(50 + rand(10));
	clearMenu();
	addButton(0, "Next", mainGameMenu);
}
...
private function boastAtLottie():void {
	showLottie();
	clearOutput();
	output("You laugh as you pick up one of the larger ones and pump mercilessly, looking at her expectantly as if she had been failing out of laziness. She tries harder to pull the heavy object over her head, but eventually she begins to shake and drops it, landing in the sand with a ");
	if(silly) output("large *pomf =3*");
	else output("loud clonk");
	output(". She looks up at you and sniffles, but you just grunt in response, disappointed with her efforts. She sighs and tries her best to do it again.\n\n");

	output("After a few more attempts at the weights, she eventually buckles under the pressure and gives up, walking home by herself. You snort and continue a few more well-earned reps before dropping down and recharging for the night.");
	lottieMorale(-3);
	pc.energy( -10);
...
private function trainWithLottie():void {
	...
	output("After a few more attempts at the weights, you both decide you've had enough, going your separate ways and waving as you leave the gym.");
	lottieMorale(2);
	lottieTone(2);
	pc.slowStatGain("p", 0.1);
	pc.energy( -10);
	processTime(50 + rand(10));
	clearMenu();
	addButton(0, "Next", mainGameMenu);
...
private function sprintAndLeaveLottieBehind():void {
	...
	output("A few rounds later, you've exhausted yourself so you decide to return home. You notice that Lottie's nowhere in sight, but it's her loss - not yours.");
	//dynStats("tou", .2, "spe", .25);
	pc.slowStatGain("r", 0.45);
	lottieMorale(-3);
	pc.energy( -10);
	processTime(50 + rand(10));
	clearMenu();
...
private function jogWithLottie():void {
	showLottie();
	clearOutput();
	output("You decide that sprinting wouldn't really get either of you anywhere, though taking things a little faster wouldn't be any harm. You take the power walking to the next level with a quick jog, before reminding Lottie to take this as leisurely as possible - it isn't a race. Lottie quickly picks up the idea and starts to push herself further, trying her best to keep up with you, whilst maintaining a steady rhythm on the track. You both spend the good part of the evening jogging a fair distance, before deciding to retire for the night. You're both tired, but satisfied with today's efforts.\n\n");
	//dynStats("tou", .1, "spe", .1);
	pc.slowStatGain("r", 0.2);
	lottieMorale(2);
	lottieTone(2);
	pc.energy( -10);
	processTime(50 + rand(10));
...
private function sprintAndLeaveFitLottieBehind():void {
	...
	output("A few rounds later and you've exhausted yourself, so you decide to return home. You notice that Lottie's nowhere in sight, but it's her loss - not yours.");
	//dynStats("tou", .2, "spe", .25);
	lottieMorale(-3);
	pc.slowStatGain("r", 0.45);
	pc.energy( -10);
...
private function lottieYoga():void {
	...
	clearOutput();
	//[If Figure 0 – 100]
	output("You know that Lottie has an affinity for yoga, so you decide to enter the dimly lit room at the end of the pool. She gladly follows, bouncing along with you as you take two of the mats out. A class has already started, and you can see many people gathering around a tanned, four-armed woman performing warm-up exercises. Knowing you both have a little time before they break out into the more complicated stretches; you scan around the room for optimal places to sit down without blocking your view of the tetra-armed instructor. By the time you've searched the room, a few more people had already blocked off most of the available spots. You look over to see Lottie already spread out and lying on her stomach – she looks over to you and giggles before patting the vacant area next to her. Throwing a quick smile, you also notice a small unused area right next to the teacher – you'd be able to take it if you went by yourself. Should you go at it alone, or stretch with Lottie?", false);
	//[Alone] [Together]
	clearMenu();
...
private function lottieYogaAlone():void {
	showLottie();
	clearOutput();
	output("Taking your mat, you walk over towards the four-armed yoga trainer, sitting down only a meter or two away from her tanned, muscular body. In the corner of your eye, you can see Lottie visibly disheartened as she watches you gazing at the fit, caramel-colored woman in front of you. As you take your position, the intense smell of coffee and desert flowers hits you as the instructor lays on her back and pulls both of her legs up, giving you a clear view of her large thighs and puffy mound. You swear you can see her smiling as the other surrounding members begin to imitate her position. Not wanting to disappoint, you lie on your rear and tilt your head backwards, confronted by another pair of legs lifted into the air. You can see Lottie attempting the same position, only to lean to her right and fall over, earning an annoyed stare from the person next to her.\n\n");
	output("You continue to take on the vaguely sexual positions of the athletic yoga teacher, the class ending before you know it. Your head strangely dizzy from the smell of sweat and pheromones, you walk home by yourself, completely forgetting about Lottie.");
	Mutator.modThickness(pc, 30, 1);
	Mutator.modTone(pc, 60, 1);
	lottieMorale(-2);
	pc.energy( -5);
	processTime(50 + rand(10));
	clearMenu();
...
private function lottiePool():void {
	showLottie();
	clearOutput();
	output("You often find Lottie near the pool, so it's only natural to assume she might have a knack for it. You mention swimming to the pig-girl, her ears perking up at the idea of going in the water. \"<i>S-sure! I'll go get ready!</i>\"  Wondering what she might've meant, you wait by the side of the pool, your legs dipping into the cool, blue water. Soon after, Lottie runs out into the area, wearing the daring, black bikini she wore the first time you had met. Her jiggling breasts squish past the fabric, while her chubby butt and thighs hug tightly against the bottom piece. Feeling a familiar stretching in your groin, you look down to find your [pc.cocksLight] at full attention, hardly subtle against your [pc.gear]. A thought passes your mind to cover up, but before you can begin to reflect on the idea the bouncing girl sits her plump ass next to you. You wonder if it's easier to jump straight into the pool or to wait it out – hell, she might even be flattered by your reaction.", false);
	//[Jump][Wait]
	clearMenu();
...
private function jumpInPoolBeforeLottieSeesYourWingWang():void {
	showLottie();
	output("",true);
	output("You can't risk it, so you decide to take a plunge into the water – it's freezing at first, but you quickly warm up. Lottie takes your avoidance to heart and looks visibly discouraged, though you wonder what her reaction would've been if your cockrocket", false);
	if(pc.cockTotal() > 1) output("s");
	output(" decided to say hello. Needless to say, you decide it'd be better if you swam alone – nothing's wrong with the occasional chubby-assed slut wandering now and then, but it's a pain to not be distracted. Lottie wades at the other end of the pool, doing the occasional stretch before breaking into laps. You take a page from her book and begin to work back and forth from either end, making sure not to stare at Lottie's massive chest as it bounces into the air every so often after a stroke. After a considerable amount of time, you both finish considerably more exhausted than before. She seems to have lightened up since your initial reaction to her body, so you shrug it off and head home.");
	//ENCOURAGEMENT -1, FIGURE +2, MUSCLE TONE +2, LUST +9001
	Mutator.modTone(pc, 100, 2);
	pc.lust(40);
	lottieTone(2);
	lottieMorale(-1);
	pc.energy( -5);
...
private function waitAndShowLottieYourPoolBoner():void {
	showLottie();
	clearOutput();
	output("You figure it can't do much harm to let [pc.eachCock] fly, so you decide to leave it be, even leaning back so as to accentuate the hardening length. It doesn't take long for Lottie to notice, letting out a little gasp as she observes your ");
	if(pc.biggestCockVolume(false) >= 15) output("pride and joy");
	else if(pc.biggestCockVolume(false) < 5) output("secret shame");
	else output("painfully average prick");
	output(". You almost experience a tinge of regret from the now incredibly awkward silence, but it's thrown out the window as soon as you feel a soft hand grab your throbbing member. Looking up, Lottie's face has reddened considerably, but her curiosity overrides her shame as she begins to slowly massage you to full mast. \"<i>I... I did this?</i>\" she asks, facing you with large, wondering eyes. You grin and nod, taking her hand and helping her stroke it further, only to be met with a slap on your wrist as she removes her grip. You look up, frowning as Lottie laughs at the situation. \"<i>I'm – it's nice to know that someone likes me.</i>\" She stutters, smiling timidly. \"<i>But, um, we're here to exercise, " + pc.mf("big guy","babe") + ". We should do that.</i>\" You groan at the sudden cockblock, but you can't really argue. She takes your hand and leads you into the water, but not before you playfully grab a handful of her plush ass.\n\n");

	output("A while later and you've both finished off several laps of the pool, exhausted from your progress. Lottie flashes you a large smile before you part ways, leaving you still somewhat aroused from the experience.");
	Mutator.modTone(pc, 100, 2);
	pc.lust(80);
	lottieTone(2);
	lottieMorale(2);
...
public function hamSammitchTimeBIATCH():void {
	showLottie();
	clearOutput();
	if(flags["COC.LOTTIE_TALK_HAM_SANDWITCH"] == 0) {
		flags["COC.LOTTIE_TALK_HAM_SANDWITCH"]++;
		output("You walk over to the cafe, Lottie watching as you order a sandwich from their menu. In a matter of seconds the collie-like clerk pulls one up from the glass display and hands it over to you");
		if(silly) output(", mumbling something about lifetime members receiving free pig-related food items");
		output(". You exchange a smile before walking back to the table and handing Lottie the moist meal, explaining that you saw the ham sandwich the other day, figuring she might enjoy it. Lottie throws it on the floor. \"<i>What the heck is wrong with you?</i>\"\n\n");

		output("You reach for a high five that never comes.");
	}
	//[If Ham Sandwich, Repeat Encounter, High Corruption]
	else if(pc.cor() >= 75) {
		output("You walk over to the cafe, Lottie watching as you order a sandwich from their menu. In a matter of seconds the collie-like clerk pulls one up from the glass display and hands it over to you, mumbling something about giving a pig-girl a ham sandwich being a stupid fucking idea. You exchange a smile before quickly walking out of the cafe and into the currently empty pool area. You hastily remove your [pc.gear] and take out your [pc.cock], jamming it straight into the sandwich and fucking it relentlessly. In moments you cum between the velvety slices of ham, leaving a large puddle inside and leaving the rest of your spunk leaking out the back and pooling onto the floor. You dress yourself before walking back to the table and handing Lottie the incredibly moist meal, explaining that you saw the ham sandwich the other day, figuring she might enjoy it – and that you've taken care into adding your own 'special mayonnaise'. Lottie throws it on the floor. \"<i>Don't cum inside ham sandwiches, asshole!</i>\"\n\n");
		output("You reach for a high five that never comes.");
		pc.orgasm();
	}
	//[If Ham Sandwich, Repeat Encounter]
	else {
		output("You walk over to the cafe, Lottie watching as you order a sandwich from their menu. In a matter of seconds the collie-like clerk pulls one up from the glass display and hands it over to you, mumbling something about giving a pig-girl a ham sandwich being a stupid fucking idea. You exchange a smile before walking back to the table and handing Lottie the moist meal, explaining that you saw the ham sandwich the other day, figuring she might enjoy it. Lottie throws it on the floor. \"<i>Stop giving me ham sandwiches, asshole!</i>\"\n\n");
		output("You reach for a high five that never comes.");
	}
	pc.cor(1);
	processTime(50 + rand(10));
...
public function lottieFuck():void {
	...
	if(lottieMorale() <= 40) {
		output("You take Lottie's hand in your own, though the girl seems to flinch at your touch. \"<i>D-did I do something?</i>\" she asks, slowly recoiling into her seat. You choose not to answer, though the both of you know what's coming next. Silently, you increase your grip on Lottie, leading her out of her seat and into the pool area, checking for any bystanders before taking her inside one of the empty saunas. Lottie grows more and more anxious as you undress yourself in front of her, taking out [pc.oneCock] and stroking gently. She seems to shrink when you turn to her and ask her to do the same. \"<i>P-please, " + pc.short + "... I... I like what we're doing, but,</i>\" she turns away from your wanting gaze before muttering, \"<i>I want to do this with someone special.</i>\" You scowl – it looks like you're going to have to take things into your own hands again. Walking towards the pig-slut, you grab the hem of her top and pull down, ripping it off to reveal her lovepillows which are covered hastily by her arms. \"<i>N-no! I... please, please don't do this...</i>\" Grabbing your [pc.cock " + x + "], you step close enough for her to feel your hot breath against her skin. You rub the tip of your [pc.cock " + x + "] against her soft flesh, pre-cum leaking against her stomach as you thrust slowly. Your natural musk fills Lottie's button nose, and her increasingly shorter breaths give you the signal that she's ready to give up against your raging lust. \"<i>But " + pc.short + ", I... I love...</i>\" A loud slap is heard as you press and play with her bottom, grinding your length in between her creamy thighs. \"<i>I... love...</i>\" Slap!  You wail on her ass and thrust hard between her legs. \"<i>F-fuck! I love you so much!</i>\" the pink whore cries, pulling off her shorts and turning around, grabbing your [pc.cock " + x + "] and rubbing it against the entrance to her wetness. You grin at your success, but you're not going to satiate her needs yet. You continue to fuck her thighs, slapping her luscious butt every so often and earning a squeal in response. You lean over and grab one of her heaving breasts, groping it roughly to the sound of your pelvis slapping against her fat legs. The feeling of your [pc.cock " + x + "] being squeezed against Lottie's thighs almost sends you to climax, before you get a better idea.\n\n");

		output("You turn Lottie around to face you, telling her you've decided you're going to take her for a vigorous work out. She only pants with anticipation.");
		...
		if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) > 0 && flags["COC.LOTTIE_ORAL_TALKS"] == 2)
			addButton(2, "Tentacle", tentaDickLottieButtFuckbuttFuckButtFUCKBUTTFUCKBUTTFUCK);
		if (flags["COC.LOTTIE_DOGGY_STYLE_ENABLED"] > 0) 
			addButton(3, "Doggystyle", lottieRepeatDoggieStyle);
		if (flags["COC.LOTTIE_WALKIES_ENABLED"] > 0) 
			addButton(4, "Walkies", lottieRepeatWalkies);
		if (flags["COC.LOTTIE_THREESOME_ENABLED"] > 0) 
			addButton(5, "Threesome", lottieRepeatThreesome);
		if (flags["COC.LOTTIE_NETORARE_ENABLED"] > 0) 
			addButton(6, "Netorare", lottieRepeatSUPERNEATORARES);
		addButton(7, "30Minute", lottiesThirtyMinutePigGasm);
	}
	//[If Encouragement 30 – 60]
	else if(lottieMorale() <= 60) {
		output("You ask Lottie if she's keen for a little 'extracurricular workout', earning a giggle and a tilt of the head from the pig-girl. \"<i>What do you mean, " + pc.mf("big guy","babe") + "?</i>\" she asks. You sigh and stand up, revealing your hardening member");
		if(pc.cockTotal() > 1) output("s");
		output(". \"<i>I... oh.</i>\" Taking Lottie's hand, you quickly lead her out of the cafe and into the pool area, looking for an empty sauna to sneak into. Considering that the gym's pool is only used by a few semi-aquatic humanoids and the odd ");
		if(silly) output("street ");
		output("shark, your search isn't that difficult. Hopping into the toasty room, you quickly undress, Lottie still standing by the doorway. \"<i>H-hey, y'know, I didn't actually agree on this or a-anything...</i>\" she stutters, nervously fiddling with her top. Without missing a beat, you walk towards her and plant a kiss on her full lips, tasting her sweetness as you probe her mouth with your eager tongue. The girl melts underneath your touch, passionately returning your erotic display while you take the chance to slip Lottie out of her top, reaching into her shorts to feel between her creamy hips. She moans as you slowly push two fingers inside her moist wetness, simultaneously pulling out your [pc.cock " + x + "] and pushing her back against the wall. Lottie begins to jerk you off slowly, massaging the length of your [pc.cock " + x + "] with one hand while the other takes care at gently caressing your tip. You start to thrust into her hands on reflex while you claim her mouth, the feel of her sugary tongue wrapped around yours sending you both into fits of needy lust. You use your hands to reach around and squeeze her chubby derriere, lightly slapping it whilst a stray hand heads towards her source of hot desire. You slick your hand against her wetness, Lottie moaning in your mouth while you work towards bringing her to an early climax. However, the sudden roughness of your hands against her mound only causes a quick jerk to your [pc.cock " + x + "], her soft hands tightening against your shaft. You groan in response, removing yourself from the lips of her mouth and instead placing your own against her erect nipple, sucking forcefully. The pig-slut grabs your head and pushes you towards her heaving chest, relinquishing her grip on your member to hug you closer.\n\n");

		output("Not wanting to be left unsatisfied, you tell her you've decided you're going to take her for a vigorous work out. She can only pant with anticipation.");
		//[Anal][Oral]
		clearMenu();
		addButton(0, "Anal", analLottieMilkshake);
		addButton(1, "Oral", getSuckedOffByAHamSandwich);
		addButton(2, "30Minute", lottiesThirtyMinutePigGasm);
		if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) > 0 && flags["COC.LOTTIE_ORAL_TALKS"] == 2)
			addButton(3, "Tentacle", tentaDickLottieButtFuckbuttFuckButtFUCKBUTTFUCKBUTTFUCK);
	}
	//[If Encouragement 60 – 100]
	else {
		output("You take Lottie's hand in your own, causing a small blush from her in response. \"<i>" + pc.short + "?</i>\" she begins, only to be cut off by a small peck to the lips. Lottie reddens, giggling slightly and gazing around for any onlookers. \"<i>What's brought this on, babe?</i>\" She beams, twirling and caressing your fingers among her own. You tell her that you thought she just needed a little more 'special exercise' to help get the blood flowing – making sure to emphasise \"<i>special exercise</i>\" with exaggerated air quotes. She playfully pushes away your idiotic gesture and takes you out of your seat, leading the way into the pool area and into one of the empty saunas. Locking the door behind you, she turns around to face you and your hardening crotch. \"<i>I think you're getting a little over excited, babe,</i>\" she purrs, her confidence getting the better of you as she takes a step closer, but you're the one in charge. Grabbing Lottie by the hips, you twirl her around to face the wall, grinding your hardening length");
		if(pc.cockTotal() > 1) output("s");
		output(" against her plump asscheeks. \"<i>W-woah! I... well, okay...</i>\" she moans, reaching back to stroke your [pc.cock " + x + "] through your clothing while you grab a handful of her rosy ass, slapping and squishing it against your palm. You continue to hump relentlessly against Lottie's tight shorts, occasionally reaching around to feel her hot mound, causing the odd squeal or moan to emanate from her pouting lips. Eventually deciding clothes are the only thing between Lottie's lewd figure and your own [pc.cock " + x + "], you simultaneously remove your outfit while ripping off her sweaty top, causing her breasts to spill out into your eager, groping hand. Despite your sudden stripping, Lottie immediately gets back into position, placing your cock in between her pillow-like asscheeks and moaning while you flick and tease her swollen nipples, rocking back and forth against you.\n\n");

		output("Lottie whimpers in between short, hot breaths, \"<i>You're driving me crazy here, babe...</i>\" You give her chubby ass a hard slap in agreement. Lottie squeals in glee, jerking your [pc.cock " + x + "] as it pokes up through her ass. The sensation of your tip rubbing against her velvety skin drives you wild.\n\n");

		output("You turn Lottie around to face you, telling her you've decided you're going to take her for a vigorous work out. \"<i>I'll work you out, babe,</i>\" she moans in response.");
		...
		if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) > 0 && flags["COC.LOTTIE_ORAL_TALKS"] == 2)
			addButton(2, "Tentacle", tentaDickLottieButtFuckbuttFuckButtFUCKBUTTFUCKBUTTFUCK);
		if (flags["COC.LOTTIE_QUICKIE_ENABLED"] > 0) 
			addButton(3, "Quickie", lottieRepeatQuickie);
		if (flags["COC.LOTTIE_COWGIRL_ENABLED"] > 0) 
			addButton(4, "Cowgirl", lottieRepeatCowGirl);
		if (flags["COC.LOTTIE_REVERSE_COWGIRL_ENABLED"] > 0) 
			addButton(5, "R.Cowgirl", lottieRepeatBackwardsAssCowgirl);
		if (flags["COC.LOTTIE_FEMDOM_ENABLED"] > 0) 
...
public function lottieRepeatDoggieStyle():void {
	showLottie();
	clearOutput();
	//output(images.showImage("lottie-fuck-her-doggie-style-repeat"));
	var x:Number = pc.cockThatFits(40);
	if(x < 0) x = 0;
	output("A few minutes later and you've taken your whore to the washrooms. Mirrors line the sides of the walls to allow members to observe how far they've gone and how far they still need to go. It's entirely empty for the night, though you make sure to lock the door just in case. You turn around to see Lottie cowering in the corner. She knows what you want – and you swear on Mareth's giant tree titties you'll get it. With her naked body on full display, Lottie quietly moves down to the ground, positioning herself on all fours with her butt facing towards you. She looks behind herself to see your erect [pc.cock " + x + "] on full display, quickly squealing and looking away. You grin as you stroke yourself into fullness, walking over and placing yourself behind Lottie's full derriere. \"<i>" + pc.short + "?</i>\" she gently sobs, \"<i>P-please don't do this.</i>\" With that, you grab her ample thighs and thrust forward, driving your [pc.cock " + x + "] straight into her tense wetness.\n\n", false);
...
public function lottieRepeatWalkies():void {
	showLottie();
	clearOutput();
	//output(images.showImage("lottie-fuck-her-in-public-repeat"));
	var x:Number = pc.cockThatFits(40);
	if(x < 0) x = 0;
	output("A few minutes later and you've taken your whore outside of the gym and around to the back, where an equipment shed lies waiting. You make quick work of the lock, busting it open with a nearby metal pole. The two of you walk slowly inside, Lottie pausing behind you to gaze at the tools and exercise apparatuses littering the walls. Soon, you find what you're looking for. You've never been one to judge a person for their fetishes, but you make a mental note to thank the person who couldn't control theirs. A single leather collar attached to a leash dangles silently upon a hook attached to the walls of the shed. Grinning, you take it down, adjusting the length of the rope before handing it to Lottie. \"<i>W-what's this for, " + pc.mf("sir","ma'am") + "?</i>\" she mumbles, but you pay no attention to her questioning as you loosen the collar and push it down on the girl's neck, tightening it before she can protest. \"<i>N-no! What are yo-!</i>\" Slap! You pelt your hand against her soft cheek, causing her to jerk on the rope and fall to the floor, miserable and in pain. Your slut slowly looks up at you with watering eyes, a hand to her face to quell the stinging sensation. \"<i>I... I understand.</i>\" She mutters quietly, picking up the leash and placing it in your hands, before standing on her knees and adopting a begging position in front of you. You grunt as the girl pulls out her small tongue, reluctantly licking at your groin to satisfy the beast behind your clothes. Just like a whore to assume that's what you're here for. You walk away from her attempts to fellate you, pulling on the leash and yanking your girl forward. She gets up and starts walking behind you and out into the sunlight - you want the world to know how well you've trained your fat little piggy.\n\n");

	output("You look back to the shed to see if there's anything you might be missing. Weights, large poles, a whip, some rope, and – ah yes, the steel ring gag. 'Where there's a whip, there's a way', you chuckle to yourself as you take the strap and bring it over to Lottie. Your pig slut is naked and ready, waiting on your command as you pull out the ring-shaped device and stretch the strap over her head, placing the metal ring inside her mouth and forcing the pale slut to make a permanent 'O'-shape with her lips. You tighten the gag, causing Lottie to cough uncomfortably as drool forms at the base of her mouth. Taking the leash, you drag Lottie away from the shed, her large ass jiggling as she tries to catch up to you. You figure you'll take her to the backstreets of Tel'Adre – you want people to see your work of art, but you know only the scum of the city will truly appreciate her for what she is. You pull the leash, leading Lottie through an alleyway and into a smaller, dirtier part of the desert city, an enclosed area with several food stalls littered around.  A majority of the lower-class citizens wander these parts, shuffling about as they haggle, argue and steal – as is usual in the slums. Most people notice your pig-girl trotting behind you, though few offer a second glance. The majority stand and stare as you present your trophy – a few of the more courageous ones openly masturbate to the site, whereas the braver still walk up and slap your pig on the ass, eliciting a squeal or a moan. You decide to up the ante while you walk through the commons, placing a hand between Lottie's pale thighs and thrusting two fingers into her pussy, the heat enveloping your digits as you shamelessly fingerfuck the poor girl.\n\n", false);

	output("On your walk, you find yourself in a familiar location – the back alleyway of the Wet Bitch. Figuring it wouldn't hurt to take your slut inside; you jerk your hand forward, forcing Lottie to walk towards the door – when you recognize the face of a furiously masturbating fox herm.  Urta looks at you while she pumps her thick horsecock relentlessly, winking at the pig-slut you've currently got on a leash. She doesn't seem to recognize you again, which could either mean she's somehow contracted serious amnesia, or she's a lot drunker than usual. \"<i>");
	if(!silly) output("Heyyy, is she like *hic* your shex slave er shumthin?");
	else output("Heyyy, do I shmell bacon?  I fucking love bacon!  BACON BACON BACON!");
	output("</i>\" You decide to give the drunk vixen a treat. Pulling your slick fingers out of her cunt, you push Lottie towards the gray fox-woman, tugging on the leash to jerk her backwards onto the floor. As she tries to get up, the girl squeals as Urta's large equine member slaps Lottie on the nose. You take out your own [pc.cock " + x + "], barking at her to keep on all fours – as long as you own the backside, you couldn't care less what the fox-herm does to her face. Urta starts by rubbing the flat head of her length on the full lips of your slut, the scent of her intoxicating pre-cum enticing Lottie to place small licks along the wet tip. Not wanting to be left out, you forcefully grab her hips, gliding your [pc.cock " + x + "] straight into the slut's warm hole, groaning as you feel her walls tighten around you. You jerk as her hips suddenly thrust backwards, and you glance over to find Urta burying as much of her thick cock into the girl's mouth as she can. You both glance at each other, sharing a nod before you both plunge your lengths into the sobbing pig-whore, rocking back and forth. Urta moans while she holds onto the back of Lottie's head with one hand, fondling her own tits with the other. On the other end of your slave, you fuck relentlessly as you hold onto her fat asscheeks, panting heavily from the pleasure of them slapping against your groin. The fox-herm lets out a loud moan before pushing as much of Lottie's face into her groin as she can, her balls growing as she prepares to fill the slut's throat. You feel yourself about to explode as well, taking longer, and deeper thrusts into the girl before finally pushing forwards and filling her with your spunk, the fox-woman on the opposite end forcing her twitching, animalistic member into Lottie's throat and doing the same. The both of you slowly remove yourselves from your trophy, the pale, sweating girl now on the floor, thick seed leaking from both ends. With one more grunt you unload the rest on her ample butt, Urta painting her face.\n\n");

	output("You decide to leave Lottie there while Urta jerks off for another round at her mouth, heading back to the gym to grab a ");
	if(silly) output("bacon ");
	output("shake.");
	if(silly) output("  You sure do love spit roasting a pig, but nothing can beat a bacon shake.");
	pc.orgasm();
...
public function lottieRepeatCowGirl():void {
	showLottie();
	clearOutput();
	//output(images.showImage("lottie-fuck-her-cowgirl-repeat"));
	var x:Number = pc.cockThatFits(40);
	if(x < 0) x = 0;
	output("A few minutes later and you've both gone to the yoga rooms – you've already placed a mat down while you wait for Lottie to change. Your rosy lover bounces into the empty, dim lit room, her ample thighs squishing out of a pair of super tight shorts, while her breasts fill over a sports bra. You're immediately turned on by the incredibly erotic display, Lottie catching scent of your increasingly hardening member and giggling while she crushes her boobs together in front of you. \"<i>I'm guessing you like what you see?</i>\" she purrs, walking over to you and pushing you down upon the gym mat. Lottie makes quick work of your lower clothes, your [pc.cock " + x + "] bouncing out and at attention, which she grabs hastily as she begins to jerk you off. The pig-girl presses your member against her moist slit, grinding it back and forth while her moans fill the empty room. You grunt in pleasure, jerking your hips slightly to help the process along, to which she replies by bending down to press her overflowing tits against your chest. You take your hands and slap them upon her ass, groping and squeezing while she moans into you. The girl shudders in bliss as the grinding sends her into an early orgasm. \"<i>I... I've got something better for you, babe...</i>\" she pants as she pulls apart the opening in her shorts to reveal her blushing pussy. Keeping a firm grip on your [pc.cock " + x + "], Lottie lifts herself up and slowly lowers her body upon it, your tip parting her folds and entering her hot insides. You grunt at the sensation of her inner walls squeezing tightly against your member, ");
	if(pc.cockVolume(x) <= 40) output("her ample cheeks resting on your groin as you bottom out in her");
	else output("though you only reach so far before you've filled her up entirely with your length");
	output(". You decide to take matters into your own hands as you grab her thick hips and drive upwards, Lottie squealing as you begin to pump in and out of her. The pig-girl's creamy breasts pop out of her bra and jump to your thrusts, her fat legs hugging either side while she bounces upon you. \"<i>God, I-I love you! Unh, I feel so hot, babe!</i>\" She yells as you pork your lover enthusiastically. The slaps of her full buttcheeks hitting your groin pervade the air, your grunts accompanied by Lottie's orgasmic moaning.\n\n");
	...
	output("A while later, you've both said your goodbyes for the night. Before you leave, you make sure to head out to the cafe to buy yourself a ");
	if(silly) output("bacon ");
	output("shake.");
	if(silly) output("  You love bacon shakes.");
	pc.orgasm();
	//dynStats("sen", -2);
...
public function lottieRepeatQuickie():void {
	...
	output("The nude pig-girl stands in front of you, turning red at your hungry gaze. \"<i>Will you f-fuck my pussy, " + pc.short + "?</i>\"\n\n");

	output("In moments flat you remove your clothes to jump the piggy, whipping out your [pc.cock " + x + "] and pushing it against Lottie's wet slit, the pig-girl lying on her back while her legs rest upon your shoulders. With a hearty thrust you drive yourself into her, eliciting a loud moan while you pork her insides with your meat. Her thick thighs slap against you as you bend over to take a lustful grab at her melons, sucking at her nipple and pinching the other, her sweet taste enveloping your mouth. Lottie buries you in her cleavage, yelping as you piledrive her tight pussy into submission – but you've got more in store. Flipping her over, Lottie lies face down with her soft butt jutting up in the air, your persistent fucking driving her wild. You continue gripping onto her tits as you pound her flesh, every thrust accompanied by her pleasured grunts. Back and forth you mount her until the familiar sensation in your loins signal your release. You increase your pace, pounding her pussy until you pull your head back and ejaculate inside her inner walls, slamming in harder every few seconds with each burst.");
	if(pc.cockTotal() > 1) {
		output("  Your other cock");
		if(pc.cockTotal() == 2) output(" twitches ");
		else output("s twitch ");
...
public function lottieRepeatSUPERNEATORARES():void {
	...
	var x:Number = pc.cockThatFits(40);
	if(x < 0) x = 0;
	output("You both walk to the yoga rooms, to find Elle stretching in the empty area. She spots the both of you, waving hello -  but you all have other, dirtier intentions in mind. Elle strips down, her perky tits letting loose. You chuckle devilishly as you undress in front of your naked and wanting harem – but you only have eyes for one girl. You point at the spider-girl, informing the both of them that you only plan to fuck her. Lottie may have thought she could win you over with another slut, but you feel as if you should teach her a lesson about being too presumptuous about what you want. Elle walks towards you, her child-bearing hips swaying hypnotically as she ");
	if(pc.cockTotal() == 1) {
		output("begins to caress your [pc.cock " + x + "] with two of her hands, while the other sets to work on massaging your ");
		if(pc.balls > 0) output("balls");
		else if(pc.hasVagina()) output("pussy");
		else output("ass");
	}
	else  output("uses all four of her hands to handle [pc.eachCock], jerking each one with ease");
	...
	output("  Elle follows suit, groaning in pleasure as she reaches her own peak, her hole tightening around your length and trapping the rest of your spunk inside. The spinnerets upon her shoulder pulse in orgasm, spurting sticky globs of what appears to be thread up into the air and back down onto the pig, spunk pooling around her cleavage. You both relax, letting the spider-girl down off of you to collapse on the floor. You bend down, looking at Lottie before shoving your [pc.cock " + x + "] inside of her and letting one final spurt of your seed fill her up. \"<i>Satisfactory.</i>\" You mumble, redressing and heading out the door, making sure to grab a ");
	if(silly) output("bacon ");
	output("shake on the way out.");
	if(silly) output("  You only need one of those to satisfy you.");
	pc.orgasm();
...
public function lottieRepeatBackwardsAssCowgirl():void {
	...
	var x:Number = pc.cockThatFits(40);
	if(x < 0) x = 0;
	output("You both walk into the yoga area, met with the smell of sweat and flowers pervading the room. Lottie gently asks for you to lie down on your back, to which you comply quite easily. You're comfortable, folding your arms behind your head to give you leverage as you watch your lover walk slowly to you. Lottie turns around, her thick thighs straddling you as she sits down on your groin with her back facing your direction, giving you  a perfect view of her curvy body and plump ass. Your [pc.cock " + x + "] nestles gently between her rosy cheeks, pulsing with lustful intent as the girl grabs her hips and begins to pleasure you through her ass. The silence of the room breaks at your fevered pants and grunts as Lottie's skillful assjob sends you through fits of ecstasy, every curve of her bottom jiggling against the heat of your member. Her pace picks up while she bounces upon you, squeezing and squashing her perfect ass until one final lift in the air is met with her plunging your length up into her hot interior. The feeling of your molten cock inside her immediately sends Lottie into fits of orgasmic bliss, the intense heat filling the pig-girl with pleasure. Her attempts to continue are thwarted by your [pc.cock " + x + "] scraping against her inner walls, her thick thighs clenched together while she slowly moves up and down upon you. Deciding to take things into your own hands, you thrust upwards, almost pushing the girl off with your strength and causing Lottie to squeal as you begin pounding her irresistible hole. Her lush buttcheeks jiggle with every movement you make, the rising and falling of her body moving in accordance to your own will. You pump in and out, keeping a steady rhythm inside the hot and bothered woman.\n\n");
...
public function lottieRepeatFEMDOMFEMFDOM():void {
	...
	var x:Number = pc.cockThatFits(40);
	if(x < 0) x = 0;
	output("You both walk into the yoga area and are met with the smell of sweat and flowers pervading the room. You look around, unable to find your lover until –THUNK! A heavy weight is brought down upon your head, pulling you down to the ground and away from consciousness as everything fades to black.\n\n");
	...
	//[If Cum Production High]
		...
		output("Unfortunately for her, you have far too much built up to stop now. With another thrust, your [pc.cock " + x + "] pulses in delight, exploding in orgasm as ropes of spunk splatter upon Lottie's face and tits. The girl gasps, the sheer amount of your seed painting her white. \"<i>N-no, stop! Oh god!</i>\" she yelps, before her mouth is filled with your hot essence. She forces herself to swallow, coughing as you grin at the defeated slut. \"<i>O-okay, y-you win, " + pc.short + ".</i>\" she looks up at you and smiles, before untying your binds and setting you free. \"<i>But you have to admit, this was pretty fun, right?</i>\" You give her a thumbs up before walking out the door, leaving her to clean your thick mess. On the way out, a milkshake vendor waves over to you – it's the guy from the cafe. You buy a ");
		if(silly) output("bacon ");
		output("shake before deciding to head home.");
		if(silly) output("  You fucking love bacon shakes.");
	}
...
public function tentaDickLottieButtFuckbuttFuckButtFUCKBUTTFUCKBUTTFUCK():void {
	clearOutput();
	//output(images.showImage("lottie-fuck-her-with-tentacle-dick"));
	var x:Number = -1;
	var y:Number = -1;
	var z:Number = -1;
	var zz:Number = -1;
	...
	while(counter < pc.cockTotal()) {
		if(pc.cocks[counter].cType == GLOBAL.TYPE_TENTACLE) {
			if(x == -1) x = counter;
	...
	output("It isn't long before you've taken Lottie out to the shed behind the gym - due to the amount of visitors regularly coming in and out of the gym itself, the equipment seems to have been mostly removed - leaving bare shelves and a large concrete floor covered in spare gym mats.\n\n");

	output("The porcine girl takes the initiative as soon as you enter the room, stripping down and planting her cushiony ass upon the pile of mats in front of you.  \"<i>What're we gonna do in here, babe?</i>\" she asks, turning over onto her soft belly and rolling around, covering herself in an assortment of old, thin mattresses.  \"<i>Heh, I'm a pig in a blanket,</i>\" she squeaks, unaware of you slowly pacing towards her, your hand firmly stroking your [pc.cocksLight].  There's a time for games, you think, and a time for unleashing your myriad of undulating vine-beasts upon your curious pink slut.\n\n");
	output("Removing your [pc.gear], you let your [pc.cocksLight] hang loose, a single vine stretching outwards and slapping Lottie upon the ass to remind you of why you're here.  She squeals, trapped underneath the heap of mattresses and doing little to help the situation she's found herself in.  \"<i>H-hey, uh, I've kinda found myself in a little pickle, " + pc.mf("big guy","hon") + " - could you give me a hand?</i>\" Lottie chuckles, squirming around in an attempt to loosen herself from the pile, but inevitably failing.  You walk closer towards her, shifting away a few mattresses here and there to free her limbs.  She smiles apologetically as you lift up the last mattress, crawling back out of the pile only to squeak when you bring down the smallest - albeit, heaviest - mat onto her back, preventing her from moving any further and causing her face to be buried against the mattress below her, the girl's plump ass stretched directly up into the air.  The awkward position restricts the movement of anything outside of her forearms and neck, and the majority of the weight upon her back pushes her chest into her knees, disallowing her from stretching outwards.  Lottie turns her head to the side, giving her time to breathe and assess the situation.  She notices you're stark naked, her eyes drawn down to the [pc.cocksLight] laid before her.");
	if(lottieMorale() <= 30) output("  She gives you a gloomy stare, before turning her head back down and plunging it into the mattress, already accepting her defeat");
	else output("  She raises an eyebrow, though doesn't probe any further");
	output(".\n\n");

	output("You grasp the base of your [pc.cock " + x + "] as it stretches forward, gently caressing Lottie's face before exploring the rest of her body, idly stroking the smooth curves of her hips or brushing against her hardening nipples.  The pig-girl moans quietly from the light petting while the feeling of her soft skin excites your hardening length, reminding you of your true intentions.  You walk behind her, obscuring yourself from Lottie's peripheral vision and crouching down to inspect the view of her perfectly rounded ass pointed conveniently into the air.  [pc.EachCock] twitches in agitation, eager to plow the chubby slut trapped in front of you - and you aren't about to disappoint.\n\n");

	//[if a single, lonely tentapenis]
	output("You firmly squeeze your ");
	if(y != -1) output("first ");
	output("writhing member as it stretches out, snaking towards Lottie's genitals and probing her moistened lips.  The tip of your vine-like length wriggles vertically along her eager hole, vibrating against her hardening clit and causing the sweating pig-whore to oink in pleasure.  She struggles to move her arms towards her aching need, desperate to relieve herself from the thick, stretching snake pushing against her slick cunt.  Without a word of warning, the hardened tentacle presses the side of its slick length firmly against her entrance, before whipping itself backwards and plunging into her heated hole.\n\n");

	output("Lottie groans from pleasure while your thickened, snake-like cock lies pulsing inside her, trapped within the tightening walls of her womb.  You force the bestial length to curl back outwards, before pumping back with incredible force - plowing your girl with fevered abandon as the insatiable vine pushes her face into the mattress soaked with her saliva.\n\n");

	//[if extra cock] //this is to be added to the standard text
	if(y >= 0) {
		output("Seeing that you have extra junk left to be used, and she has some vacant holes, you quickly remedy this problematic situation; guiding your " + pc.cockDescript(y) + " toward her moist butthole, you press the tip of your tree-like beast against her plush buttocks, literally stabbing her flesh with your penile flora.  The butt-slut doesn't seem to care: if anything, she squirms all the more, visibly aroused.  Her ass seems to stretch on its own as it braces itself for the imminent butt-devastation.  You welcome Lottie's efforts to comply to your anal needs and with a single thrust, you push ");
		if(pc.cockVolume(y) <= 80) output("the entirety of your " + pc.cockDescript(y), false);
		else output("as many inches of your " + pc.cockDescript(y), false);
		output(" as you can, tearing her insides as your mammoth makes room for itself.  The piggy girl squeals, the intense and rough friction proving too much for her sensitive anal receptors.  You push further, making sure her colon is completely stuffed with vine-like cock.  You rest there for a while, letting Lottie thrash wildly as she futilely tries to accommodate to the beast filling her interior.  It feels good, but you rapidly lose patience and start sliding your rubbery dong in and out of her butthole, smearing her anal entrance with your own pre-cum, making further thrusts easier and faster.  You butt-fuck the pig-girl until her anal walls feel numb from the raw violation, and you butt-fuck her a little more.\n\n");

		//[if another extra cock]//this is to be added to the standard text
		if(z >= 0) {
			output("Her constant moaning and screaming reminds you that one of her holes at least isn't being put to use.  Her eyes widen in awe as she sees your " + pc.cockDescript(z) + " inexorably moving to her waiting mouth, her lips forming a wide O as if to greet your tentacle pecker.  Without a word, you plunge your beast in her mouth, burying your prick until she's about to choke.  Fortunately she's used to blowjobs, and you have no trouble fitting a decent amount of inches down her throat.  You can see her neck bulge from the obscene intrusion, and the tip of your " + pc.cockDescript(z) + " swells until her windpipe looks ready to explode.  She can't say anything, but the way her eyes are staring at you tell you more than anything about how she feels.", false);
			if(lottieMorale() < 40) output("  This pig slut really loves to be used like that!");
			else output("  Your pig lover really loves what you're doing to her!");
			output("  A bunch of tight fuck-holes, each taking care of your tentacle pricks in its own way, all different but all so wonderful!  A horny girl begging to be rammed with cock in her every hole... You almost feel bad for using her like a broken fuck-toy, but she is clearly asking for it.  She keeps twisting in every direction as you savagely triple-penetrate her, her silent moans of pleasure being muffled by the beast lodging in her mouth.\n\n");
			//[if another extra cock]//to be added to standard text
			if(zz >= 0) {
				output("Surely her twat must have some room left for more cock?  Before you even think of it, your free tentacle prick is already squirming its way to the piggy's love-tunnel, trying to fit as its companion ruthlessly rams her fuckhole.  You feel her whole body clench as you press the tip of your " + pc.cockDescript(zz) + " between her netherlips, stretching them beyond their normal capacity as you keep pushing forward.  If her mouth wasn't so full of dick, you'd be sure she would scream like a harpy in heat.  Once the tip manages to get through her tight vaginal entrance, the rest easily follows, and you effortlessly slide your dickflesh down her now-gaping cunt.  You synchronize your pricks penetrating her cuntwardly, literally ravaging her pussy as you pork her insides with your genitals.\n\n", false);
				//[if another extra cock]//to be added to standard text
				if(zzz >= 0) {
					output("Since her dripping twat has been able to take some extra dick, why not try her backside?  Your " + pc.cockDescript(zzz) + " wriggles joyfully as it wraps around Lottie's plump rump, ready to test her anal capacity.  Fortunately, its rubbery texture allows it to be compressed as it grinds against your " + pc.cockDescript(y) + ", sliding toward the pig-girl's wet anus.  Your plant-like dong bounces back against her entrance numerous times, but you know she is being stretched wider at every attempt.  You don't know how many times you poke her pucker with the erect tip of your " + pc.cockDescript(zzz) + ", and you can't imagine how the teasing stimulation feels for the poor prisoner of your tentacle embrace.  At last, with a brutal thrust, your flexible dong somehow manages to clear a path for additional butt-penetration.  Your twin erect pricks are incredibly squeezed against her tight anal walls and her body is about to be torn in half from all the rods impaling her from many directions at once.  That doesn't prevent you from thrusting back and forth very carefully through her colon, never completely pulling out.\n\n");
					//[if another extra cock]//to be added to standard text
					if(zzzz >= 0) {
						output("No matter how hard you try, you can't fit any more cock in her toy-like body. Your " + pc.cockDescript(zzzz) + " is quivering in frustration, until you notice her fleshy orbs bouncing up and down repeatedly on her chest as you ruthlessly pound her from every hole.  This might be the right occasion to get a good titfuck: your " + pc.cockDescript(zzzz) + " slowly slithers along her sweaty body and slides between her sizable fuckpillows; although the sensation isn't as hot and amazingly rough as the sharp tingles of pleasure received from the dongs pounding her ass, your rod is being subtly tickled and teased by her supple flesh.  Already aroused from the thorough multi-penetration, you start dripping pre-cum all over her torso, smearing her whole body with your sap.  This makes her breasts all the more slick and slippery, and needless to say, the sensation isn't lost on your " + pc.cockDescript(zzzz) + "; it seems to grow even bigger and becomes pinkish, the sudden color change betraying its arousal. You keep fucking her boobs until they are literally glistening from all the juices you've spilled on them.\n\n");

						//[if another extra cock]//to be added to standard text
						if(zzzzz >= 0) {
							output("As you idly pump Lottie's body with all your junk, you mentally count all the extra ways her body could be used, since you have some tentacle dong left.  Mouth, cunt, ass, tits... wait!  Her hands are still free!  By the way they are twitching convulsively, you can clearly tell they are aching for some dick to handle.  You graciously comply to her cocky needs and shove your " + pc.cockDescript(zzzzz) + " against her palm, coating it with your pre-cum.  She instinctively clings to your rod of flesh as if her life depended on it and starts jerking you off, ferociously rubbing your dickflesh and using a mix of her sweat and your pre-cum as lube.", false);
							//[if another extra cock]
							if(zzzzzz >= 0) output("  Seeing the effect your junk had on her, you move your " + pc.cockDescript(zzzzzz) + " to her remaining hand and she grips it likewise.");
							output("  She keeps stroking you in a compulsive way, her hands clearly acting under the effect of her tremendous lust.");
							output("\n\n", false);
							//[if another extra cock]//to be added to standard text
							if(zzzzzzz >= 0) {
								output("With another cock left unsatisfied, you place your thick, tentacled member against the soft flesh of Lottie's ass, driving it to and fro as the sensation of her chubby flesh drives you wild.\n\n");
...
public function lottiesThirtyMinutePigGasm():void {
	clearOutput();
	var x:Number = pc.biggestCockIndex();
	output("\"<i>Hey, babe!</i>\"  Lottie greets you in her usual, cheerful fashion.  Though her smile is booming with assumed confidence, the look in her eyes tells a different story.  She has sad, dependent eyes.  She looks up to you!  Then again, why shouldn't she?  You are her personal trainer after all.\n\n");

	output("Smiling back, you give her a warm greeting.  After the pleasantries are dealt with, you instruct her that it is time for some pre-workout stretches.  Ever eager to please you, Lottie begins.  She spreads her legs as far as she can and then leans down in an attempt to touch her toes.  As she bends over, you get a glimpse of her spacious ass.  Baby got back!  While she fights to touch her toes, her chubby ass wiggles and jiggles; it's the very definition of the term \"<i>more cushion for the pushin'.</i>\"  The more you watch, the more blood rushes to your [pc.cock " + x + "].  You bite your lip as sweat beads on your skin; you NEED to tame that ass!\n\n");

	output("Lottie is completely oblivious to that fact that you are ogling her plump rump.  Her face wrenches in frustration as she strains.  Then... victory!  She managed to touch her toes!  A relieved smile crosses her face until... SMACK!  Lottie recoils in pain from the sudden stinging slap to her rear.  She tries to unbend, but your forceful left hand keeps her in place.  Glancing back as much as she can, she notices first your missing [pc.gear] and then your erect, throbbing cock.  Trembling a little in apprehension, she tries to form a question, but only incoherent stammering comes out of her mouth.\n\n");

	output("\"<i>Time to pork this bitch!</i>\" you exclaim in triumph.  You forcibly yank down the skimpy bottoms that barely contained her bodacious booty.  Smiling lecherously, you flop [pc.oneCock] between her pink cheeks.  Now firmly holding her hips, you stroke your manhood between them.  Lottie doesn't so much as put up a fight.  Instead, she trembles and oinks like the pig she is.\n\n");

	output("Looking down at her with a look most wicked, you immediately stop stroking your length between her chunky cheeks.  Giving her no warning at all, you promptly slam your [pc.cock " + x + "] into her piggy depths, forcibly parting her labia.  Lottie squeals from the sudden impact as her cunt muscles tense around you, giving you unbridled pleasure.  Panting heavily, she just stands there and takes it.  Her face is flustered, blushing rosily as her tongue just hangs out of her mouth, but it matters little to you that she enjoys herself.  FUCK!  Her cunt is a natural born cock milker.  It churns and squeezes your [pc.cock " + x + "], encouraging you to pound her, and you happily oblige.  Grasping her hips for leverage, you pummel her porky pussy for several minutes until your loins unleash a thick jet of jism, completely coating her insides white.\n\n");

	output("Standing victorious, you attempt to pull out, but you can't.  Her pussy has clamped down on you so hard that you can't withdraw.  You are in no rush, however. Your cock is still hard and her body is plenty warm.  Smirking, you just stand and enjoy the feeling of her womb.  Then suddenly, Lottie cries at the top of her lungs.  Your attempt to pull out seems to have triggered her orgasm.  A cataract of womanly juices cascades out of her cunt.  She gushes, and gushes, and gushes... after five minutes, her orgasm still hasn't ceased!  The ground where you two stand looks like a small puddle thanks to her constant stream of femspray.  At ten minutes, she still hasn't stopped!  When will the end be?  Why all this pig glee?  Yo man, what happened?  Lottie's breathing heavily and oinking as loudly as possible.  Tired and frustrated, you just stand there while her orgasm continues unabated.  After about twenty minutes more, Lottie's chubby cunt finally releases you and she falls forward onto her face in the puddle of pork drippings, eyes rolled into her head and a smile splitting her face.\n\n");

	output("DAMN!  Did this pig bitch really just have a half-hour orgasm?  You're a little jealous.");
	pc.orgasm();
	processTime(50 + rand(10));
	clearMenu();
	addButton(0, "Next", mainGameMenu);
