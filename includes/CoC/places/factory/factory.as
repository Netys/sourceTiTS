...
private function DemonFactoryBadEndGeneric():void {
	...
	if(pc.cocks.length == 0 || pc.biggestTitSize() <= 1) {
		output("A small inner voice pipes up to remind you that you don't have ");
		if(pc.cocks.length == 0) {
			output("anything in your groin to suck on");
			if(pc.biggestTitSize() <= 1) output(" or ");
		}
		if(pc.biggestTitSize() <= 1) output("any adornments on your chest");
	...
	else output("multitude of ");
	output(" clear hoses coming away from your cow-like chest udders.  ");
	if(pc.milkQ() < 750) output("Creamy white milk is flowing in a steady stream up the tubes and away from you.  ");
	else output("The hoses bulge obscenely as they struggle to keep up with the torrents of creamy-white milk you're producing.  ");  
...
public function DemonFactoryFoyer():Boolean {	
	...
	if (!pc.hasKeyItem("Factory Iron Key")) {
		output("\n\n<b>The door to north is locked.</b>");
