...
public function pcDefeatsBrigid():void {
	clearOutput();
	output("The harpy jailer collapses, ");
	if (enemy.lustQ() >= 100) output("too turned on");
	else output("too badly beaten");
	output(" to continue the fight.  You quickly snatch the keys from inside her shield, and hook them onto your own belt.");
	//(New Key Item: Harpy Key A)
	pc.createKeyItem("Harpy Key A");
	flags["COC.HEL_BRIGID_DEFEATED"] = 1;
...
public function pcDefeatedByBrigid():void {
	...
	if (flags["COC.HEL_HARPY_QUEEN_DEFEATED"] == undefined) {
		output("\"<i>Tsk tsk tsk,</i>\" the harpy jailer croons, looming over you as you slump to the ground. \"<i>You shouldn't have messed with me, bitch!</i>\" she snaps, giving you a rough kick to the side. \"<i>Now, let's see what Mother has to say about this...</i>\"");
		//(Go to \"<i>Harpy Breeding Slut</i>\" Bad End)
		addButton(0, "Next", harpyQueenBeatsUpPCBadEnd);
	}
	else {
		output("\"<i>That's it?!</i>\" Brigid screams, as you collapse in front of her.  \"<i>A weak little piece of trash like you took out our queen?</i>\"");

		output("\n\n\"<i>[pc.name]!</i>\" calls a voice from behind you.  Hel bounds down the stairs, summoned by the sound of the fight; when she spies your defeated form, she draws her sword and throws herself at the harpy jailer, completely clearing the ground in her rage.  The scene that plays out next passes through your dazed mind agonizingly, like a nightmare.");
		...
		addButton(0, "Next", brigitPostQueenDeathBadEndII);
	}
}
...
private function brigitPostQueenDeathBadEndII():void {
	...
	if (!pc.hasVagina() || pc.isPregnant()) {
		...
		if (pc.cockTotal() > 1 || pc.cumQ() > 1000) output("  [pc.EachCock] holds forth for a while, until Kiri's ass, pussy, and tailfeathers are soaked with oozing, [pc.cumColor] [pc.cumNoun].");
		output("  Next to you, the salamander on the other table climaxes as well, dumping a load into Hel that sizzles when it leaks onto his fiery, iron-bound tail.  Your former lover squirms and shivers as the scalding-hot load fills her cunt, and her own prick twitches weakly, depositing a small string of bubbling jism on the stone table.");

		output("\n\n\"<i>Oh dad... Hel... I'm sorry,</i>\" Kiri whimpers, unable to pull her eyes away from them as your cock dribbles the last of your seed into her.  The bound man's single eye widens as he hears the name of his partner, and his face deforms with rage as a roar pours out of him with enough volume to shake the room.  Kiri buries her face in your [pc.chest] and shudders as the prisoner bellows his frustration; Hel simply looks left and right, confused and anxious as the echoing roar dies off; Brigid pushes her toy pussy away and rises from her stool.");
	...
	//(else if not visibly pregnant F or H)
		...
		if (pc.hasCock()) output(" as the phoenix's cunt slides onto your [pc.cock " + pc.smallestCockIndex() + "]");
		output(".  Brigid makes her way up the stairs, leaving you and the others at the mercy of the assorted horde.  The harpy rides your mouth greedily, knocking your head into the hard stone floor, and the last thing you can see around her feathery thighs before you slip into unconsciousness again is Kiri being pulled roughly from her resting place atop Hel by a mean-looking, wide-hipped harpy matron who takes her place, sitting sideways and drawing her talons sadistically across Hel's vulnerable chest to smear Kiri's seed around...");
