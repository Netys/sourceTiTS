...
public function EventsAdopterHellNotify(deltaT:uint, doOut:Boolean = true):void {
	////////////////////////////////////////////////////
	...
	if (pc.hasPerk("Futa Form")) { //Futa checks
		...
		if (!pc.hasCock() && !pc.hasStatusEffect("Goo Crotch")) { //(Dick regrowth)
			...
			eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>As time passes, your loins grow itchy for a moment.  A split-second later, a column of flesh erupts from your crotch.  Your new [pc.cock] pulses happily.";
			//if (pc.balls == 0) {
		...
		if (pc.longestCockLength() < tmp && !pc.hasStatusEffect("Goo Crotch")) { //(Dick rebiggening) 
			eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>As time passes, your cock engorges, flooding with blood and growing until it's at 8 inches long.  You really have no control over your dick.</b>";
		...
		if (!pc.hasStatusEffect("Gel Body") && pc.breastRows.length > 0 && pc.breastRows[0].breastRatingRaw < tmp) { //Tits!
			...
			if (pc.isBimbo())
				eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>Your tits get nice and full again.  You'll have lots of fun now that your breasts are back to being big, swollen knockers!</b>";
			else eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>Your " + pc.breastDescript(0) + " have partially regained their former bimbo-like size.  It looks like you'll be stuck with large, sensitive breasts forever, but at least it'll help you tease your enemies into submission!</b>";
			pc.slowStatGain("i", -1);
			pc.lust(15);
		}
		if (!pc.hasVagina() && !pc.hasStatusEffect("Goo Crotch")) { //Vagoo
			...
			if (pc.isBimbo())
				eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>Your crotch is like, all itchy an' stuff.  Damn!  There's a wet little slit opening up, and it's all tingly!  It feels so good, why would you have ever gotten rid of it?</b>";
			else eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>Your crotch tingles for a second, and when you reach down to feel, your [pc.legs] fold underneath you, limp.  You've got a vagina - the damned thing won't go away and it feels twice as sensitive this time.  Fucking bimbo liquer.</b>";
			pc.slowStatGain("i", -1);
			pc.lust(15);
		}
	}
	...
	if (pc.hasPerk("Bimbo Body") || pc.hasStatusEffect("Bimbo Champagne")) { //Bimbo checks
		...
		if (pc.breastRows[0].breastRatingRaw < tmp && !pc.hasStatusEffect("Gel Body")) { //Tits!
			pc.breastRows[0].breastRatingRaw = tmp;
			if (pc.isBimbo())
				eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>Your boobies like, get all big an' wobbly again!  You'll have lots of fun now that your tits are back to being big, yummy knockers!</b>";
			else eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>Your " + pc.breastDescript(0) + " have partially regained their former bimbo-like size.  It looks like you'll be stuck with large, sensitive breasts forever, but at least it'll help you tease your enemies into submission!</b>";
			pc.slowStatGain("i", -1);
		}
		if (!pc.hasVagina() && !pc.hasStatusEffect("Goo Crotch")) { //Vagoo
	...
	if (pc.hasPerk("Bro Body")) { //Bro checks
		...
		if (!pc.hasCock() && !pc.hasStatusEffect("Goo Crotch")) { //(Dick regrowth) 
			pc.createCock(tmp);
			pc.shiftCock(0, Mutator.guessCockType(pc));
			eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>As time passes, your loins grow itchy for a moment. A split-second later, a column of flesh erupts from your crotch.  Your new, " + tmp + "-inch cock pulses happily.";
			if (pc.balls == 0) {
				eventBuffer += "  A pair of heavy balls drop into place below it, churning to produce cum.";
				pc.balls = 2;
				pc.ballSizeRaw = 3 * Math.PI;
		...
		if (!pc.hasStatusEffect("Goo Crotch") && pc.longestCockLength() < tmp) { //(Dick rebiggening)
			eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>As time passes, your [pc.cock] engorges, flooding with blood and growing until it's at " + tmp + " inches long.  ";
		...
		if (pc.balls == 0 && !pc.hasStatusEffect("Goo Crotch")) { //(Balls regrowth)
			eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>As time passes, a pressure in your loins intensifies to near painful levels. The skin beneath [pc.eachCock] grows loose and floppy, and then two testicles roll down to fill your scrotum.</b>";
			pc.balls = 2;
	...
	if (pc.lizardScore() < 3 && pc.dragonScore() < 3 && pc.harpyScore() < 3 && pc.hasPerk("Oviposition") && !pc.hasPerk("Basilisk Womb")) { //--Lose Oviposition perk if lizard score gets below 3.
		eventBuffer += "\n\n" + logTimeStamp("passive") + "Another change in your uterus ripples through your reproductive systems. Somehow you know you've lost a little bit of reptilian reproductive ability.\n(<b>Perk Lost: Oviposition</b>)";
		pc.removePerk("Oviposition");
	}
	if (pc.bunnyScore() < 3 && pc.hasPerk("Bunny Eggs")) { //--Lose Oviposition perk if bunny score gets below 3.
		eventBuffer += "\n\n" + logTimeStamp("passive") + "Another change in your uterus ripples through your reproductive systems. Somehow you know you've lost your ability to spontaneously lay eggs.\n(<b>Perk Lost: Bunny Eggs</b>)";
		pc.removePerk("Bunny Eggs");
	}
	
	if ((pc.hasPerk("Oviposition") || pc.hasPerk("Bunny Eggs")) && pc.hasVagina() && pc.fertility() > 0 && (days - int(flags["OVIPOSITION_LAST_CYCLE"]) >= Math.max(int(30 / pc.fertility()), 3)) && !pc.isPregnant() && flags["OVIPOSITION_LAST_CYCLE"] != days) {
		if (PregnancyManager.findHandler("CoCOviElixEggs").tryKnockUp(pc, pc, rand(pc.vaginas.length))) {
			eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>Somehow you know that eggs have begun to form inside you. You wonder how long it will be before they start to show?</b>";
			if(pc.hasPerk("Oviposition")) pc.addPerkValue("Oviposition", 1, 1); //Count times eggpregged this way in perk.
			flags["OVIPOSITION_LAST_CYCLE"] = days;
		} else {
			eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>Oviposition failed!</b>";
			flags["OVIPOSITION_LAST_CYCLE"] = days;
		}
	}
	...
	///////////////////////////////////////
	if ((pc.hasVaginaType(GLOBAL.TYPE_CANINE) || pc.hasVaginaType(GLOBAL.TYPE_VULPINE) || pc.hasVaginaType(GLOBAL.TYPE_FELINE) || pc.hasVaginaType(GLOBAL.TYPE_EQUINE) || pc.hasVaginaType(GLOBAL.TYPE_LEITHAN)) // regulars heats for appropriate morphs... should be actually some function or list
		&& pc.fertility() > 0 && (days - int(flags["HEAT_LAST_CYCLE"]) >= Math.max(int(30 / pc.fertility()), 3)) && !pc.isPregnant() && flags["HEAT_LAST_CYCLE"] != days) { // same schedule with oviposition for correct overlaps
	...
	if (pc.hasStatusEffect("Heat") && pc.statusEffectv3("Heat") == 0) {
		if (pc.isPregnant() || pc.fertility() <= 0) {
			eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>Your heat is suddenly gone.</b>";
		...
		if (!pc.hasVagina() && pc.statusEffectv4("Heat") == 0) {
			eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>Your heat is gone with your vagina.</b>";
			pc.removeStatusEffect("Heat");
		}
	}
	if (pc.hasStatusEffect("Rut") && !pc.hasCock()) {
		eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>Your rut is gone with your maleness.</b>";
		pc.removeStatusEffect("Rut");
	}
	if (pc.hasStatusEffect("Rut") && pc.virility() <= 0) {
		eventBuffer += "\n\n" + logTimeStamp("passive") + "<b>Your rut is suddenly gone.</b>";
		pc.removeStatusEffect("Rut");
	}
