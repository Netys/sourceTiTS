...
//trace("Marble Stats: Aff-"+pc.statusEffectv1("Marble") +" Add-" + pc.statusEffectv2("Marble") + " isA-" + pc.statusEffectv3("Marble") + ".");
...
//const MARBLE_MILKED_BEFORE:int = 1165;
		...
		{
			pregnancy = new PregnancyStore("COC.MARBLE_PREGNANCY_TYPE", "COC.MARBLE_PREGNANCY_INCUBATION", 0, 0);
		...
		public function timeChange():Boolean
		{
			...
			if (pc.findStatusEffect(StatusEffects.CampMarble) >= 0) {
				//Marble stuff pt I
				if (flags["COC.MARBLE_RATHAZUL_COUNTER_1"] > 0) {
					flags["COC.MARBLE_RATHAZUL_COUNTER_1"]--;
					//Stick it at 1 so I can trigger it off the camp screen.
					if (flags["COC.MARBLE_RATHAZUL_COUNTER_1"] <= 1) flags["COC.MARBLE_RATHAZUL_COUNTER_1"] = 1;
				}
				//Counting down to purity!
				if (flags["COC.MARBLE_PURIFICATION_STAGE"] == 0) {
					if (flags["COC.MARBLE_COUNTUP_TO_PURIFYING"] < 200) flags["COC.MARBLE_COUNTUP_TO_PURIFYING"]++;
				}
				//Counter 2!
				if (flags["COC.MARBLE_RATHAZUL_COUNTER_2"] > 0) {
					flags["COC.MARBLE_RATHAZUL_COUNTER_2"]--;
					//Stick it at 1 so I can trigger it off the camp screen.
					if (flags["COC.MARBLE_RATHAZUL_COUNTER_2"] <= 1) flags["COC.MARBLE_RATHAZUL_COUNTER_2"] = 1;
				}
				//Post purification nursitude
				if (flags["COC.MARBLE_PURIFICATION_STAGE"] >= 5) {
					flags["COC.MARBLE_TIME_SINCE_NURSED_IN_HOURS"]++;
					if (flags["COC.MARBLE_TIME_SINCE_NURSED_IN_HOURS"] > 1000) flags["COC.MARBLE_TIME_SINCE_NURSED_IN_HOURS"] = 1000;
				}
				//Reset Marble corruption warning
				if (flags["COC.MARBLE_WARNED_ABOUT_CORRUPTION"] == 1 && cor() < 50) flags["COC.MARBLE_WARNED_ABOUT_CORRUPTION"] = 0;
				//Lactation whoopie!
				if (flags["COC.MARBLE_TIME_SINCE_NURSED_IN_HOURS"] < 100) flags["COC.MARBLE_TIME_SINCE_NURSED_IN_HOURS"]++;
				//Increment Marble's Lust
				if (flags["COC.MARBLE_LUST"] < -100) flags["COC.MARBLE_LUST"] = -100;
				if (rand(2) == 0) flags["COC.MARBLE_LUST"] += 1;
				if (flags["COC.MARBLE_4"] > 50) flags["COC.MARBLE_LUST"] += .3;
				if (flags["COC.MARBLE_4"] > 70) flags["COC.MARBLE_LUST"] += .3;
				//If bitch-bin is in construction
				if (flags["COC.MARBLE_NURSERY_CONSTRUCTION"] > 0 && flags["COC.MARBLE_NURSERY_CONSTRUCTION"] < 100 && marbleAtCamp()) {
					flags["COC.MARBLE_NURSERY_CONSTRUCTION"]++;
					if (flags["COC.MARBLE_NURSERY_CONSTRUCTION"] >= 100) {
						spriteSelect(41);
						output("\n<b>Marble lets you know that she's finished building a rather secure nursery for your coming offspring.</b>\n");
						needNext = true;
						flags["COC.MARBLE_NURSERY_CONSTRUCTION"] = 100;
					}
				}
				//Marble find items check
				else if (pc.findStatusEffect("MarbleHasItem") < 0 && marbleAtCamp()) {
					// Off cooldown
					if (pc.findStatusEffect("MarbleItemCooldown") < 0) {
						if (rand(10) == 0) {
							spriteSelect(41);
							output("\n<b>You find a note from Marble back at camp, letting you know that she has an item for you!</b>\n");
							pc.createStatusEffect("MarbleItemCooldown",(24 + rand(24)),0,0,0);
							pc.createStatusEffect("MarbleHasItem",rand(10),0,0,0);
							needNext = true;
						}
					}
				}
			...
			if (pc.findStatusEffect("MarbleItemCooldown") >= 0) {
				pc.addStatusValue("MarbleItemCooldown",1,-1);
				if (pc.statusEffectv1("MarbleItemCooldown") < 1) pc.removeStatusEffect("MarbleItemCooldown");
			}
			if (pc.findStatusEffect("Infested") < 0) flags["COC.MARBLE_GROSSED_OUT_BECAUSE_WORM_INFESTATION"] = 0;
			if (pc.findStatusEffect("Marble"sMilk) >= 0 && pc.findPerk(PerkLib.MarblesMilk) < 0) {
				//Decrement time remaining by 1		
				pc.addStatusValue("Marble"sMilk,1,-1);
				//Remove the status and stat boosts when time runs out on the milk
				if (pc.statusEffectv1("Marble"sMilk) <= 0) {
					needNext = true;
					dynStats("str", (-1 * pc.statusEffectv2("Marble"sMilk)),"tou", (-1 * pc.statusEffectv3("Marble"sMilk)));
					pc.removeStatusEffect("Marble"sMilk);
					//Text for when Marble's Milk effect wears off:
					//[addiction is 10 or less] 
					if (pc.statusEffectv2("Marble") <= 10) output("\nYou feel the euphoria from drinking Marble's milk fade from you. Only now that it's gone do you notice that it was actually making you tougher.\n");
					//[addiction is 11-30]
					else if (pc.statusEffectv2("Marble") <= 30) output("\nYou feel a slight sense of loss as the euphoria from Marble's milk fades.  You kinda want to drink more, but the desire is not overpowering.\n");
					//[addiction is 31-50, player is not addicted]
					else if (pc.statusEffectv2("Marble") <= 50) output("\nYou shiver slightly as the euphoria from Marble's milk fades.  You really feel like suckling her breasts again.\n");
					//IF ADDICTED
					if (pc.statusEffectv3("Marble") > 0) {
						//If player is under bottled milk effects
						if (pc.findStatusEffect(StatusEffects.BottledMilk) >= 0) {
							...
							pc.createStatusEffect("Marble"Withdrawl,0,0,0,0);
			...
			if (pc.statusEffectv3("Marble") > 0 && pc.findPerk(PerkLib.MarbleResistant) < 0 && pc.findPerk(PerkLib.MarblesMilk) < 0 && pc.statusEffectv2("Marble") > 25) {
				//If player does not have marble's milk or bottled milk, go into withdrawl
				if (pc.findStatusEffect("Marble"sMilk) < 0 && pc.findStatusEffect(StatusEffects.BottledMilk) < 0) {
					//If player is not yet in withdrawl
					if (pc.findStatusEffect("Marble"Withdrawl) < 0) {
			...
			if (pc.statusEffectv3("Marble") <= 0) {
				if (pc.statusEffectv2("Marble") > 60) pc.changeStatusValue("Marble", 2, 60);
			}
			//Withdrawl removal if you get unaddicted.
			if (pc.findStatusEffect("Marble"Withdrawl) >= 0) {
				if (pc.statusEffectv2("Marble") <= 25) {
					pc.removeStatusEffect("Marble"Withdrawl);
					dynStats("tou", 5, "int", 5);
					output("\nYou no longer feel the symptoms of withdrawal.\n");
					needNext = true;
				}
				//Remove withdrawl if you have bottled milk affect
				else if (pc.findStatusEffect(StatusEffects.BottledMilk) >= 0) {
					output("\nYou no longer feel the symptoms of withdrawal.\n");
					needNext = true;
					pc.removeStatusEffect("Marble"Withdrawl);
					dynStats("tou", 5, "int", 5);
			...
			if (pc.findStatusEffect(StatusEffects.BottledMilk) >= 0) {
				pc.addStatusValue(StatusEffects.BottledMilk,1,-1);
				if (pc.statusEffectv1(StatusEffects.BottledMilk) <= 0) pc.removeStatusEffect(StatusEffects.BottledMilk);
			}
			if (model.time.hours > 23) {
				flags["COC.MARBLE_PLAYED_WITH_KIDS_TODAY"] = 0;
				if (pc.findStatusEffect("Marble") >= 0 && pc.statusEffectv2("Marble") > 0) marbleStatusChange(0,-1);
			}
			return needNext;
		}
		...
		public function timeChangeLarge():Boolean {
			...
			if (pc.statusEffectv3("Marble") > 0 && pc.statusEffectv2("Marble") < 25 && pc.findPerk(PerkLib.MarblesMilk) < 0 && pc.findPerk(PerkLib.MarbleResistant) < 0 && model.time.hours == 6) {
				spriteSelect(41);
				...
				output("You find Marble in her room.  When you come in she looks up at you and starts.  \"<i>What happened?</i>\" she asks, \"<i>Something about you is completely different from before...</i>\"  You explain to her that you've gotten over your addiction and no longer crave her milk.\n");
				//(reduce corr by 5)
				dynStats("cor", -5);
				...
				pc.createPerk(PerkLib.MarbleResistant,0,0,0,0);
				//After player ends Addiction:
				//Marble liked you addicted
				if (pc.statusEffectv3("Marble") == 1) {
					//Affection 0-29, version 1
					if (pc.statusEffectv1("Marble") < 30) {
						...
						pc.createStatusEffect(StatusEffects.NoMoreMarble,0,0,0,0);
					}
					//Affection 30-89, version 1
					if (pc.statusEffectv1("Marble") >= 30 && pc.statusEffectv1("Marble") < 90) {
					...
					if (pc.statusEffectv1("Marble") >= 90) {
						...
						pc.createStatusEffect(StatusEffects.CampMarble, 0, 0, 0, 0);
						flags["COC.FOLLOWER_AT_FARM_MARBLE"] = 0;
						if (kGAMECLASS.isabellaFollowerScene.isabellaFollower() && flags["COC.FOLLOWER_AT_FARM_ISABELLA"] == 0) flags["COC.ISABELLA_MURBLE_BLEH"] = 1;
						//if amily is there, tag it for freakout
						if (flags["COC.AMILY_FOLLOWER"] > 0 && flags["COC.FOLLOWER_AT_FARM_AMILY"] == 0) {
							flags["COC.MARBLE_OR_AMILY_FIRST_FOR_FREAKOUT"] = 2;
						}
						else flags["COC.MARBLE_OR_AMILY_FIRST_FOR_FREAKOUT"] = 1;
						//if Izma is there, tag for freakout!
						if (flags["COC.UNKNOWN_FLAG_NUMBER_00238"] == 1 && flags["COC.FOLLOWER_AT_FARM_IZMA"] == 0) {
							flags["COC.UNKNOWN_FLAG_NUMBER_00237"] = 1;
						}
						pc.createStatusEffect(StatusEffects.NoMoreMarble,0,0,0,0);
					}
				}
				//Ashamed Marble
					...
					if (pc.statusEffectv1("Marble") < 30) {
						...
						output("The piece of paper looks like a page torn from a book.  It looks like an entry from an encyclopedia of sorts, it reads in formal script:\n");
						output("<b><u>Codex: Lacta Bovine</u></b>\n");
						output("Description: <i>A race of all female bovine-morphs, more commonly called cow-girls.  They appear as tall well endowed women, with numerous bovine characteristics.  Generally they have bovine horns, ears, tail, and legs.  Like all minotaurs, they are very strong and resilient, however, they are unusually sensitive compared to their relatives.</i>\n");
						output("Skin and Fur: <i>The skin tone for these creatures is very close to being human, their fur more closely follows the common minotaur fur colors: brown, black or white with brown spots.</i>\n");
						output("Behavior: <i>The behavior of Lacta Minotaurs varies greatly between each individual.  The only major unifying piece of behavior is their desire to give milk to almost any living creature, and their high libido, common to all corrupted creatures.</i>\n");
						output("Special abilities: <i>A lightly corrupted creature, most of the corruption is centered on their breast milk.  It is addictive to those that drink it repeatedly, eventually making them dependent on the one from whom it was drank from.  The milk also strengthens the drinker, makes them better able to find the one who nursed them, and grants limited powers of control over them to the Lacta Minotaur that nursed them.  Finally, the breasts of Lacta Minotaurs are incredibly resilient, healing from almost any damage, even being cut off; they are able to produce milk for their entire life without fail.</i>\n");
						...
						flags["COC.CODEX_ENTRY_LABOVINES"] = 1;
						output("\n\n<b>New codex entry unlocked!</b>");
						pc.createStatusEffect(StatusEffects.NoMoreMarble,0,0,0,0);
					}
					//Affection 30-69, version 2
					if (pc.statusEffectv1("Marble") >= 30 && pc.statusEffectv1("Marble") < 70) {
					...
					if (pc.statusEffectv1("Marble") >= 70) {
			...
			if (pc.statusEffectv3("Marble") > 0 && pc.statusEffectv2("Marble") >= 100 && pc.findPerk(PerkLib.MarblesMilk) < 0 && pc.findPerk(PerkLib.MarbleResistant) < 0 && model.time.hours == 6) {
				spriteSelect(41);
				...
				output("You find Marble in her room.  When you come in she looks up at you and smiles deeply.  \"<i>What happened?</i>\" she asks, \"<i>Something about you feels so wonderful and right.</i>\"  You explain to her that you've finally become entirely dependent on her milk.\n");
				...
				pc.createPerk(PerkLib.MarblesMilk,0,0,0,0);
				//Clear withdrawl
				if (pc.findStatusEffect("Marble"Withdrawl) >= 0) {
					pc.removeStatusEffect("Marble"Withdrawl);
					dynStats("tou", 5, "int", 5);
				...
				if (pc.findStatusEffect("Marble"sMilk) >= 0) {
					pc.removeStatusEffect("Marble"sMilk);
				}
				...
				if (pc.statusEffectv3("Marble") == 1) {
					...
					if (pc.statusEffectv1("Marble") < 49) {
					...
					//High affection
					if (pc.statusEffectv1("Marble") >= 80) {
			...
			if (checkedMarbleMilk++ == 0 && model.time.hours == 6 && pc.findPerk(PerkLib.MarblesMilk) >= 0) {
				//In prison
				if (prison.inPrison) {
					output("\nYou get up and complain about not getting your daily dose of Marble's milk. ");
					if (flags["COC.PRISON_BITCHED_ABOUT_MARBLE_MILK"] == 0) {
						output("Yet oddly, you are still fine. Perhaps addiction to Lacta Bovine milk is a conspiracy? You curse the codex entry for deceiving you about the properties of her milk!");
					}
					output("\n");
					flags["COC.PRISON_BITCHED_ABOUT_MARBLE_MILK"]++;
					addNextButton(playerMenu);
					return true;
				}
				//Marble is at camp
				...
				//Marble isn't at camp
					...
					if (pc.findStatusEffect(StatusEffects.NoMoreMarble) < 0) {
...
override public function marbleFollower():Boolean {
	return pc.findStatusEffect(StatusEffects.CampMarble) >= 0;
}
...
public function marbleAtCamp():Boolean
{
	if (marbleFollower())
	{
		if (flags["COC.MARBLE_RATHAZUL_COUNTER_2"] > 0) return false;
		else return true;
	}
	return false;
}
...
{
	if (flags["COC.MARBLE_PURIFIED"] > 0)
	{
		if (flags["COC.MARBLE_BREAST_SIZE"] == 0) return "DD cup";
		else if (flags["COC.MARBLE_BREAST_SIZE"] == 1) return"G cup";
		else if (flags["COC.MARBLE_BREAST_SIZE"] == 2) return "HH cup";
		else return "J cup";
	}
	else
	{
		if (flags["COC.MARBLE_4"] > 30) return "HH cup";
		else return "G cup"
	}
}
...
public function encounterMarbleInitially():void {
	...
	output("\"<i>My name's Marble, what's yours?</i>\" she asks you.  You introduce yourself and exchange a few pleasantries before she asks how she can help you.  You tell her that you actually came to help her, explaining that Whitney said she could use a gentle touch.  \"<i>Oh that would be nice</i>\", she says \"<i>Spending the night connected to the milking machine was a mistake, and now I need something gentle.</i>\"  How will you help her?");
...
private function leaveNonFriendsMarble():void {
	clearOutput();
	spriteSelect(41);
	output("Smiling politely and just as insincerely as Marble, you beg her pardon and excuse yourself.");
...
private function leaveNonFriendsMarble4EVERRRR():void {
	...
	spriteSelect(41);
	pc.createStatusEffect(StatusEffects.NoMoreMarble,0,0,0,0);
	flags["COC.MARBLE_WARNING"] = 2;
	output("Answering the cow-girl with a blank look, you shake your head and walk away, resolving to avoid Marble from now on.");
...
private function apologizetoWalkingTitsIMEANMARBLE():void {
	clearOutput();
	spriteSelect(41);
	output("Wanting to make up for before, you apologize for your behavior and ask Marble if there is a way you could make it up to her.  She's pleasantly surprised by your answer, and after a few moments of contemplation says, \"<i>Well, all right then.  My breasts are still a bit sore - after all, I have to milk them every day - so do you think you could give them that personal touch?</i>\"  You figured she would ask this of you... quite the one-track mind.");
	...
	//Basic scene 
	output("You give chase after the bovine woman, wandering around the many plants in search of the runaway.  Her constant giggling makes sure you know you're going in the right direction, but sometimes she likes to double back or make false trails so the game is more interesting.  ");
	//[(intelligence check; <15, 15-40, 41+) 
	if (pc.inte < 15) output("Eventually you find Marble stopped, looking towards you with her hands in the air saying, \"<i>You caught me!  Come here.</i>\"  She beckons you towards her chest, and you don't make her wait.");
	else if (pc.inte < 40) output("Eventually you find Marble stopped and waiting for you.  She puts her hands in the air and says, \"<i>You caught me!</i>\"  It's fairly clear she's given herself up, but when she folds her hands in front of her chest and presses her breasts together, then tells you to come over, you aren't complaining.");
	...
	flags["COC.MARBLE_WARNING"] = 0;
	//(apply the stat effect 'Marble's Milk' to the player)
	applyMarblesMilk();
	dynStats("lib", .2, "lus", (5 + pc.lib/10));
	HPChange(100,false);
	pc.changeFatigue(-50);
...
private function stayForFights():void {
	clearOutput();
	spriteSelect(41);
	output("You fold your arms over your chest and scowl as Marble trudges back over the fields carrying a huge hammer.  Part of you feels terribly juvenile to be solving an argument with violence - but the other part is cheering at the opportunity to put the bossy cow in her place.");
	//go to battle
...
private function getOutOfDodge():void {
	clearOutput();
	spriteSelect(41);
	output("The hell you will... the truth is the truth no matter how many talking hammers show up.  Catharsis completed, you leave the farm and its cows behind.");
	//makes the battle available as the next Marble encounter, as if PC had raped her
	flags["COC.MARBLE_WARNING"] = 3;
	addNextButton(returnToCampUseOneHour);
}
...
internal function marbleFightWin():void {
	...
	clearOutput();
	//Win by hp
	if (monster.HP() <= 1) output("Marble falls to the ground defeated; she looks up at you helplessly, wondering what you're going to do next.  ");
	//win by lust
	else output("Marble collapses and looks at you longingly, pulling up her skirt with a look of desperation in her eyes.  ");
	//after the lust+HP defeat scenes if the player wins
	output("You've gathered a bit of a crowd around you now, thanks to the noise of this cow clunking around with her huge hooves and hammer.  It might not be a terribly good idea to rape Marble...  you'd have to drag her up to her room just to avoid interruption and Whitney would likely find out and be upset.  What do you do?");
	//Options, rape in room, milk (Spy's submission - not included yet) and, don't rape.
	var feed:Function = null;
	if (pc.findPerk(PerkLib.Feeder) >= 0 || pc.lactationQ() > 200) feed = forceFeedMarble;
...
internal function marbleFightLose():void {
	...
	clearOutput();
	//lose by hp
	if (pc.HP() <= 1) output("After a few too many blows to the head, you topple over to the ground.  ");
	//lose by lust
	else output("Overcome by desire, you fall to your knees, and start masturbating furiously.  Disgusted with you, Marble hits you upside the head once more, knocking you over.  ");
	output("She leans in close to your head and whispers \"<i>Don't ever come near me again, or I will crush your head with this hammer.</i>\"  She stands up and walks away from you as you pass out from your head injuries.  ");	
...
private function rapeMarbleInHerRoom():void {
	clearOutput();
	spriteSelect(41);
	output("You aren't going to give up on this opportunity, but you don't want to have an audience either.  So you drag Marble and her hammer back to her room, and throw Marble onto her bed, grabbing and twisting her nipples, causing her to cry out in pain and pleasure.");
	//continue onto original rape
	spriteSelect(41);
	output("  You suddenly grab at her breasts and squeeze them roughly, at which point she screams and ");
	output("tries to slap you.  You easily duck under her hand and start twisting her nipples.  She squeals and begins to go limp under your painful ministrations.  You move her around and force her to kneel, pushing her face down into her bed.  Keeping one of your hands on her nipple, you pull down her skirt and expose her beautiful womanhood and asshole.\n\n");
	//dicked players
	if (pc.cocks.length > 0) {
		output("Chuckling to yourself, you free your " + pc.multiCockDescriptLight() + " from your [pc.gear].  You spend a moment to ask the helpless cow-girl if she is ready, her only response being a whimper, before ");
...
private function forceFeedMarble():void {
	...
	if (pc.findPerk(PerkLib.Feeder) >= 0) output("You bet this cow-girl loves to get milked and nursed on just like you, but how often does she get to taste the creamy sweetness of milk?  Having her suck on your own leaky tits would be doing her a favor, right?  You approach the defeated cow-girl; her eyes are still locked onto you, wondering what you're going to do next.  Well, not that you can do much with this crowd watching you...\n\n");
	output("You never really noticed how many people live on this farm until now.  They're all probably expecting you to rape her - and not all of them are reconciled to the idea, judging by the looks you're getting.  What the hell, you might as well continue your business with these on-lookers around anyway... you're like 99% sure you won't be in trouble for feeding a cow.");
	output("\n\nYou remove the top half of your [pc.gear], showing everyone your [pc.chest]; a few cat calls and wolf whistles come from your spectators.  You do your best to ignore them... right now your world is just you and Marble.  You kneel down next to the cow-girl and sit her up, lifting her head up to your chest.");
	...
	output(", but she keeps them tightly closed, refusing to drink your sweet milk.");
	...
	pc.addStatusValue(StatusEffects.Feeder,1,1);
	pc.changeStatusValue(StatusEffects.Feeder,2,0);
	combat.cleanupAfterCombat();
}
...
private function resistMarbleInitially():void {
	...
	//(player chose resist)
	clearOutput();
	output("Surprised by your resistance, she pulls back and apologizes for being presumptuous.  ");
...
private function rapeMarble():void {
	spriteSelect(41);
	clearOutput();
	output("You decide that rather than helping her, you are going to roughly manhandle her breasts and rape her.  You suddenly grab at her breasts and squeeze them roughly, at which point she screams and slaps you.  While you are still reeling from the blow, she uses a surprising amount of strength to force you out the door.  She slams it behind you and yells, \"<i>Don't you ever come back!</i>\" through the door. You hear her start to cry as you walk away.  Aw well, you didn't like her anyway.", true);
...
public function encounterMarbleExploring():void {
	spriteSelect(41);
	clearOutput();
	output("While wandering around the farm, you meet the cow-girl Marble heading towards the barn.  ");
	//[player height < 5 feet]
	if (pc.tallness < 60) output("Marble gives her customary greeting of hugging you to her breast before telling you that she is off to get milked at the barn.  ");
	//[player hight >= 5 feet]
	else output("You exchange a quick greeting before Marble tells you that she is off to get milked at the barn.  ");
	//[affection <30]
	if (pc.statusEffectv1("Marble") < 30) {
	...
	//[affection >=30]
		...
		if (pc.statusEffectv2("Marble") < 40) {
...
private function drinkMarbleMilk():void {
	spriteSelect(41);
	clearOutput();
	output("Beaming, Marble leads you back to her room and sits down on the bed.  She invites you onto her lap and lets you start sucking at one of her nipples.  The moment that wonderful taste meets your tongue, you start gulping down the milk with reckless abandon. She sighs in pleasure in response.  From time to time, Marble gets you to switch nipples, all the while gently stroking your head");
	//[player has animal ears]
	if (pc.earType > EARS_HUMAN) output(" and occasionally scratching behind your ears");
	output(".  ");
	output("Once you've had your fill, you pull back and the two of you smile at each other.  \"<i>It's really nice for you isn't it sweetie?  Nice for me too to have someone like you that can give a good suck on my itching nipples.</i>\"\n\n");
	...
	if (pc.statusEffectv2("Marble") >= 50) {
	...
	if (pc.statusEffectv2("Marble") < 50)
	output("After drinking Marble's milk, a feeling of euphoria spreads through you as you leave the farm in high spirits.");
	applyMarblesMilk();
	addNextButton(returnToCampUseOneHour);
}
...
public function encounterMarbleExploring2():void {
	spriteSelect(41);
	clearOutput();
	output("You decide to pay Marble a visit at her room.  ");
	//[player height <5]
	if (pc.tallness < 60) output("As you step into her room, she eagerly rushes over and hugs you to her breast. \"<i>You're as cute as ever, sweetie!</i>\"  ");
	output("She is happy to see you and treats you to a small meal while you have a pleasant chat.  ");
	...
	else output("While you talk mostly about unimportant things, there is some discussion about the world and the dangers within.");
	//[addiction >30]
	if (pc.statusEffectv2("Marble") > 30) {
	...
	//[affection <60]
	if (pc.statusEffectv1("Marble") < 60) {
		...
		if (pc.inte < 30) dynStats("int", 4);
		else if (pc.inte < 40) dynStats("int", 2);
		else if (pc.inte < 60) dynStats("int", 1);
		addNextButton(returnToCampUseOneHour);
	}
	else {
		//[affection >=60, player has not had sex with Marble]
		if (pc.findStatusEffect(StatusEffects.FuckedMarble) < 0) {
...
private function turnDownMarbleSexFirstTime():void {
	spriteSelect(41);
	clearOutput();
	output("She stares at you for a few moments as your refusal sinks in.  \"<i>So you don't feel the same way about me...  I'm sorry, I won't ever ask you again,</i>\" she says sadly.  \"<i>Maybe I'll see you later.</i>\" She directs you out the door.  You realize that refusing her will permanently affect your relationship.");
	//(affection drops to 50, it can no longer be raised above 50)
	pc.changeStatusValue("Marble", 1, 50);
	//(increase player inte)
	dynStats("int", 4);
	addNextButton(returnToCampUseOneHour);
}
...
private function turnDownMarbleSexRepeat():void {
	spriteSelect(41);
	clearOutput();
	output("She looks disappointed at your refusal but quickly brightens up and says, \"<i>Ok sweetie, next time then.</i>\" On that note, you bid farewell to the pretty cow-girl and return to your camp.");
...
private function AcceptMarblesSexualAdvances():void {
	...
	standardSex(true);
	if (pc.findStatusEffect(StatusEffects.FuckedMarble) < 0) pc.createStatusEffect(StatusEffects.FuckedMarble,0,0,0,0);
...
public function helpMarble1():void {
	spriteSelect(41);
	clearOutput();
	output("\"<i>You know, Marble is moving some produce right now. How about you go help her out?</i>\" Whitney suggests.  You agree to help the well-endowed anthropomorph and Whitney directs you to the storage shed.  You arrive to find that Marble is quite busy carrying stacks of crates into the barn.  She gives you a smile when she sees you and calls out, \"<i>Hey, sweetie!  Nice to see you.</i>\"  When you tell her you came to help her smile broadens.  \"<i>Oh, I'd love to have some help.  It'll save me some trips if you give me a hand,</i>\" she says happily before putting on a serious face and continuing, \"<i>but don't strain yourself sweetie, these are heavy. I don't want you to get hurt.</i>\"  With that, you get to work with her.\n\n");
	//[player str <20] 
	if (pc.str < 20) output("Unfortunately, the crates are quite heavy and you end up having to stick with small ones to keep up with Marble's pace.  She doesn't appear to mind, just enjoying having someone to talk to while she works, even if it doesn't save her many trips.\n\n");
	//[player str >=20, <50]
	if (pc.str >= 20 && pc.str < 50) output("You try your best, but for every crate you carry, Marble caries three. She doesn't mind though, since you'll end up saving her a quarter of the trips she would have had to make.\n\n");
	//[player str >=50, <80]
	if (pc.str >= 50 && pc.str < 80) output("You put your back into the job and manage to match Marble in her efforts.  She is really impressed with your strength, and together you can cut the number of trips needed in half.\n\n");
	//[player str >=80]
	if (pc.str >= 80) output("Marble may be strong, but you are stronger.  She is amazed as you manage to take even more crates at a time than she can, only held back by the number you can balance.  Thanks to your efforts, the chore only takes a third of the number of trips it normally would have taken.\n\n");
	output("After a little while, you notice that Marble is walking with an almost mesmerizing sway in her hips as she carries the crates; it is rather hard to take your eyes off her.  ");
	if (afterMarbleHelp()) return;
	output("When the two of you finish and you start to leave, Marble calls out to you, \"<i>Wait, let me give you something!</i>\" You turn and look back at her as she rushes up to you.  Smiling brilliantly, the cow-girl hands you a bottle full of warm milk, \"<i>My gift to you for your help, fresh from the source.</i>\" she says, patting her sizable chest.\n\n");
	//(increase player str)
	dynStats("str", 1);
	//(increase affection by one tenth the player's str)
	marbleStatusChange(int(pc.str/10),0);
	//(increase player lust)
	dynStats("lus", 10);
	//(player receives a bottle of Marble's milk)
	inventory.takeItem(consumables.M__MILK, returnToCampUseOneHour);
}

//Help out Marble, version 2 (can occur anytime before Marble knows about her milk):
public function helpMarble2():void {
	spriteSelect(41);
	output("You run into Whitney at the farm, and ask if there's something you could do.\n\n", true);
	output("\"<i>I've got it; you can help Marble do some weeding.  She's in the field over there right now,</i>\" Whitney says, pointing to a nearby pasture.  Nodding to her, you set off to help the pretty cow-girl with her chores.  It takes you a while to find her, but you eventually find Marble bent over with her rump in the air.  Once you get closer you realize that she is munching on a weed.  \"<i>Oh!</i>\" she exclaims, noticing you.  She hurriedly straightens up and looks around a little embarrassed.  \"<i>Hi there sweetie, what are you doing here?</i>\"  You explain that Whitney suggested you could help her with the weeding.  \"<i>Oh!</i>\" she exclaims again, \"<i>I guess that would be nice, but don't stare at my bum too much while I'm eating, ok?</i>\"  You agree and set to work.\n\n");
	//[player spd <20] 
	if (pc.spe < 20) output("Even though Marble often stops to munch on a weed, she is still able to get more weeds then you do.  Despite her size, she can move surprisingly fast.  Regardless, she enjoys simply having you there while she works, and you get to enjoy the view.\n\n");
	//[player spd >=20, <50]
	if (pc.spe >= 20 && pc.spe < 50) output("You put in a good effort at cleaning out the weeds, and Marble often gives you a good look at her rear when she finds a tasty looking weed.\n\n");
	//[player spd >=50, <80]
	if (pc.spe >= 50 && pc.spe < 80) output("Moving quickly through the fields, you surprise Marble with your speed so much that she jokingly pouts that you're getting to all the tasty weeds before she has a chance to eat them.  You still end up getting a few good views of her ass.\n\n");
	//[player spd >=80] 
	if (pc.spe >= 80) output("Weeding the field is a breeze for you, going fast enough that you're able to bring weeds to Marble faster than she can eat them.  In the end, you do almost all the work yourself.  She does reward you with a good view for your efforts.\n\n");
	//(increase player spd)
	dynStats("spe", 1.5);
	//(increase affection by one tenth the player's spd)
	marbleStatusChange(int(pc.spe/10),0);
	//(increase player lust)
	dynStats("lus", 10);
	if (afterMarbleHelp()) return;
	output("When the two of you finish and you start to leave, Marble calls out to you, \"<i>Wait, let me give you something!</i>\" You turn and look back at her as she rushes up to you.  Smiling brilliantly, the cow-girl hands you a bottle full of warm milk, \"<i>My gift to you for your help, fresh from the source,</i>\" she says, patting her sizable chest.\n\n");
...
private function afterMarbleHelp():Boolean {
	...
	if (pc.statusEffectv2("Marble") >= 40) {
		...
		if (pc.inte < 40 && rand(2) == 0) {
			output("You find that the more time you spend being around Marble, the thirstier you grow for her milk.  Finally, as the two of you are finishing, you are unable to take it any longer and beg Marble to let you drink her milk.  After a moment, your words sink in and she blushes deeply.  \"<i>Ok sweetie, since you helped me out and all, let's go back to my room.</i>\"  You enter into her pleasant room once again.  She invites you onto her lap and lets you start sucking at one of her nipples.  The moment that wonderful taste meets your tongue, you start gulping down the milk without abandon. She sighs in pleasure in response.  From time to time, Marble gets you to switch nipples, all the while gently stroking your head and occasionally scratching behind your ears.\n\n");
			output("Once you've had your fill, you pull back and the two of you smile at each other.  \"<i>It's really nice for you isn't it sweetie?  Nice for me too to have someone like you that can give a good suck on my sensitive nipples.</i>\"\n\n");
		...
		//[player succeeds the int check] 
			...
			output("While you're working, you are continually plagued by the thought of drinking from Marble's breasts, but you're able to keep those thoughts at bay and continue working normally.\n\n");
		}
	}
	return false;
}
...
private function withdrawlFarmVisit():void {
	spriteSelect(41);
	output("You visit Whitney's farm once again. She quickly sees the tell-tale signs of your need and lets you know where Marble is.\n\n");
...
public function addictedEncounterHappy(clearS:Boolean = true):void {
	...
	if (pc.findStatusEffect("MalonVisitedPostAddiction") < 0) {
		output("You find Marble coming out of the barn, holding one of her bottles of milk.  When she spots you, she hurries over and hands you the bottle.  \"<i>I want to find something out. Can you drink from that bottle?</i>\" she asks.  Do you drink it?");
		//- player chooses yes/no
		doYesNo(playerAgreesToDrinkMarbleMilkBottled,playerRefusesToDrinkBottledMilk);
		pc.createStatusEffect("MalonVisitedPostAddiction",0,0,0,0);
	}
	//Return visits
	else {
		//Addiction event version 1:
		if (rand(2) == 0) {
			output("You find Marble in her room, softly humming while reading a book on her bed.  You walk up to her, and without looking away from her book she says, \"<i>I can smell your need, sweetie. Are you ready for your drink?</i>\" She sets the book down and turns to you, her hands under her breasts as she leans forward.\n\n");
			//- inte check to avoid immediately drinking, if succeeded: 
			if (pc.inte >= 40) {
				output("Will you drink her milk?");
				//- player chooses yes/no
		...
		//Addiction event version 2:
			...
			output("You find Marble in the midst of one of her chores.  She smiles at you and says that if you help her with her chores, she will give you a bottle of milk to soothe your nerves.  Do you do it for the milk, Marble, or refuse?");
...
private function playerAgreesToDrinkMarbleMilkBottled():void {
	...
	if (pc.findStatusEffect(StatusEffects.BottledMilk) >= 0) {
		pc.addStatusValue(StatusEffects.BottledMilk,1,(3+rand(6)));
	}
	else pc.createStatusEffect(StatusEffects.BottledMilk,8,0,0,0);
	addNextButton(returnToCampUseOneHour);
}
...
private function playerRefusesToDrinkBottledMilk():void {
	...
	//If the player is addicted, this item negates the withdrawal effects for a few hours (suggest 6), there will need to be a check here to make sure the withdrawal effect doesn't reactivate while the player is under the effect of 'Marble's Milk'.
	if (pc.findStatusEffect(StatusEffects.BottledMilk) >= 0) {
		pc.addStatusValue(StatusEffects.BottledMilk,1,(1+rand(6)));
	}
	else pc.createStatusEffect(StatusEffects.BottledMilk,3,0,0,0);
	addNextButton(returnToCampUseOneHour);
}
...
private function marbleChoreRefusal():void {
	spriteSelect(41);
	clearOutput();
	output("You angrily tell her that you aren't going to work for her milk and turn away, leaving her visibly upset.  Your body seems to be upset at your refusal too, feeling painful all over.  Fortunately, you also feel a temporary reprieve from the symptoms of your withdrawal.");
...
public function marbleEncounterAddictedNonWithdrawl():void {
	spriteSelect(41);
	clearOutput();
	output("You decide to pay Marble a visit, as it would be nice to spend some time with her while you aren't in withdrawal.  You find her in her room reading a book.  She looks up at you surprised and says, \"<i>You don't look like you need milk right now.  What's up, sweetie?</i>\"  You tell her that you just wanted to spend some time together, and not worry about milk.  She laughs at you and says, \"<i>Sweetie, you'll always be thinking about milk; but I'm fine with pretending for a while.</i>\"  The two of you enjoy a meal together in her room.");
	pc.refillHunger(30);
	output("\n\n");
	if (pc.statusEffectv1("Marble") >= 80) {
		output("As you eat, she looks deeply into your eyes for a moment. You think she is going to say something, but she shakes her head and avoids your questions about it for the rest of your time together.\n\n");
	}
	output("After you finish, she thanks you for treating her to your company and asks you to come back soon.  You return to your camp, knowing you will probably be seeing her again soon for something less pleasant.");
	//(increase affection by 10)
	marbleStatusChange(5,0);
	addNextButton(returnToCampUseOneHour);
}
...
private function withdrawalDelay():void {
	if (pc.findStatusEffect(StatusEffects.BottledMilk) >= 0) {
		...
		pc.createStatusEffect(StatusEffects.BottledMilk,3,0,0,0);
	}
...
private function refuseMarblesMilkAddiction():void {
	spriteSelect(41);
	clearOutput();
	output("You look at Marble and refuse to do as she says.  She looks at you in surprise and asks why. You tell her you can't bear to talk about her like that, and that if you have to make her feel bad to get over this need, it's not worth it.  After a moment to let your words sink in, she rushes over to you and ");
	if (pc.tallness < 60) output("hugs you to her chest, ");
	else output("gives you a big hug, ");
	output("all the while saying how wonderful you are.  The bottle ends up getting dumped on the ground during the embrace anyway, but neither of you care to notice until afterwards. But then, it doesn't matter anyway; you'll be fine for at least a little while. Right now, you just want to enjoy Marble's warm form wrapped around you.");
	//(increase affection by 5)
	marbleStatusChange(5,0);
	//(delay withdrawal for a few hours)
	if (pc.findStatusEffect(StatusEffects.BottledMilk) >= 0) {
...
private function ashamedAddictionBegForIt():void {
	spriteSelect(41);
	clearOutput();
	output("You look at her in horror at the suggestion of wasting her delicious milk in such a way. You snatch the milk bottle and hold it tightly to your chest.  You beg her not to talk about it like that and not to throw her milk away so easily.  She seems to be even more upset by your declaration and grabs hold of your hands.  Marble looks into your eyes for a moment and tells you that there is always hope to change before she runs off.  You are left with the milk bottle, but you think that you can wait until later to drink it.  It just felt right to make that bold declaration and it seems to have made you feel better, if only for now.");
	//(player gets a bottle of Marble's Milk)
	inventory.takeItem(consumables.M__MILK, returnToCampUseOneHour);
	//(delay withdrawal for a few hours)
	if (pc.findStatusEffect(StatusEffects.BottledMilk) >= 0) {
...
public function marbleEncounterAddictedNonWithdrawlAshamed():void {
	spriteSelect(41);
	clearOutput();
	output("You decide to pay Marble a visit, as it would be nice to spend some time with her while you aren't in withdrawal.  You find her in her room and she looks at you ");
	//[affection >= 30]
	if (pc.statusEffectv1("Marble") >= 30) output("worriedly ");
	output("for a moment before it dawns on her that you aren't shaking.\n\n");
	//[affection >= 30] 
	if (pc.statusEffectv1("Marble") >= 30) output("\"<i>Sweetie, w");
	else output("\"<i>W");
	output("hy are you here if you don't need my milk?</i>\"  You explain that you just want to enjoy her company like you used to.  She gives a genuine smile that probably hasn't been on her face for a while, and the two of you have a meal together in her room. ");
	pc.refillHunger(30, false);
	output("\n\n");
	//[affection is 80 or more]
	if (pc.statusEffectv1("Marble") >= 80) {
		output("While you're eating, Marble looks into your eyes deeply for a moment and you think she is going to say something, but she shakes her head and seems more reserved for the rest of your time together.\n\n");
	}
	output("After you finish, she thanks you for thinking of her like this, even with what you are going through.  You return to your camp, knowing you will probably be seeing her again soon for something less pleasant.");
	//increase affection by 10)
	marbleStatusChange(10,0);
	addNextButton(returnToCampUseOneHour);
}
...
public function postAddictionFarmMornings():void {
	...
	clearOutput();
	output("You hurry over to the farm to get your daily dose of Marble's milk.  It takes an hour of your day, but your body is satisfied.");
	//(increase player corr by 2 if corr is under 30, otherwise increase corr by 1 up to a max of 40)
	pc.refillHunger(20);
	pc.slimeFeed();
	if (cor() < 40) {
		if (cor() < 30) dynStats("cor", 1);
		dynStats("cor", 1);
	}
	//(event takes an hour)
	addNextButton(returnToCampUseOneHour);
...
public function postAddictionFarmExplorings():void {
	...
	if (pc.findPerk(PerkLib.MarbleResistant) >= 0) output("uncomfortably ");
	output("aware of the smell of Marble's milk.");
	dynStats("int", 1);
	addNextButton(returnToCampUseOneHour);
}
//Murble farm talks
private function extendedMurbelFarmTalkz():void {
	...
	switch (flags[kFLAGS.MURBLE_FARM_TALK_LEVELS]) {
		case 0:
			output("During your talk, Marble asks where you're from.");
			//[if PC is human] 
			if (pc.race() == "human") {
				output("  \"<i>The only other human I've ever met is that wandering trader Giacomo,</i>\" she tells you \"<i>but he doesn't really talk about himself.  Maybe you could tell me about humans?  I was wondering where they live and what kind of people they are.</i>\"");
				//[if PC is shorter then 5 feet] 
				if (pc.tallness < 60) output("  Her eyes light up. \"<i>Are they all as cute as you?</i>\"");
				//[if PC is taller then 6'6" feet] 
				else if (pc.tallness > 78) output("  \"<i>Are most of you this tall?</i>\"");
			}
			//[if PC is cow-girl/cowboi]
			else if (pc.race() == "cow-morph" || pc.race() == "cow-boy") {
				output("  \"<i>It's so nice to see another of my kind,</i>\" she tells you, \"<i>I haven't seen any since I left home.  Where are you from?</i>\"");
			}
			//[if PC is a dogmorph] 
			else if (pc.race() == "dog-morph") output("  \"<i>I've seen lots of dog-morphs before, are you from Barkersvile?</i>\"");
			//[if PC is a centaur]
			else if (pc.isTaur()) output("  \"<i>I've seen a few centaurs before, but they don't seem to have regular homes.  They're nomads, wandering the plains.  Are you the same?</i>\"");
			//[if PC is not human, cow-girl/cowboi, dogmorph, or centaur] 
			else output("  \"<i>It's very rare that we get a " + pc.race() + " here.  Are you from around these parts?</i>\"");
		...
		//--- Second Conversation ---
		case 1:
			output("After you've been talking about inconsequential things for a few minutes, Marble asks you, \"<i>Sweetie, do you ever miss your home?  Do you ever wish you could be back with your parents?</i>\"");
			//[PC is pure] 
			...
			else if (cor() < 66) output("\n\nYou pause a moment before telling her that you used to think about them all the time, but you've since tried to push them from your mind so that you can focus on why you came to this world.");
			else output("You chuckle and say that you hardly think of them with all the other fun things to think about.  Then you pause and say that it is in the moments that you do think about them that keep you from forgetting who you once were.");
		...
		case 3:
			//why she left
			output("Marble looks over you a moment before saying, \"<i>You've mentioned this 'mission' a few times now; I guess that's why you left home?</i>\"  You nod.  \"<i>Fulfilling a duty, huh?  My story isn't quite the same as yours, and certainly not so noble!</i>\"  She laughs at this.");
			//[if PC is under 5 feet tall]
			if (pc.tallness < 60) output("  \"<i>Sorry, sorry, it's just that you're so cute!  I can't picture you as some great hero leaving behind home and family for a mission.</i>\"  She wipes a few tears from her face.");
			//[if PC is over 7 feet tall]
			else if (pc.tallness > 84) output("  \"<i>I'm sure a giant like you was the perfect choice for this mission.</i>\"");
			...
			if (pc.statusEffectv1("Marble") >= 60) output("  \"<i>Even though the start of the trip didn't turn out all that well, I think I found what I was looking for.</i>\"");
		...
		case 4:
			...
			output("After telling Marble about your explorations of the world thus far, Marble offers to tell you a bit more about her own journey.  \"<i>My family actually lives on the other side of the mountains to the south.</i>\"  She pauses, then chuckles.  \"<i>Or is it to the north?  You know how hard it is to describe direction over long distances, don't you?</i>\" You nod; while you've never had any problem with it back home, you can well imagine how the magic of this realm would eliminate the need for such facility.");
		...
		case 5:
			...
			output("This time it's you who spends a fair bit talking about your own love life back home - or lack thereof, as it seemed to you sometimes.  After you finish, you notice that Marble is looking off to the side.  She turns back to you and thanks you for sharing.  You ask her if she doesn't mind continuing where you left off last time.  \"<i>");
			if (pc.findPerk(PerkLib.HistorySlut) >= 0 || pc.findPerk(PerkLib.HistoryWhore) >= 0) output("Well, you certainly gave an arousing description.  I think I can share something in return.");
			else if (cor() < 33) output("Sweetie, after you told me such a nice story, of course I will.");
			else if (cor() < 66) output("Sweetie, I can tell that you care a great deal about what happened, even if you try to hide it. Of course I'll share.");
			else output("Well, you certainly gave an arousing description.  I think I can share something in return.");
			output("  After my falling out with Ansgar, I noticed that I really missed nursing him more than anything else.</i>\"  She stops for a moment, shaking her head.  \"<i>That's not quite right.  It's more that I needed to keep nursing; it just felt like everything was wrong with me if I wasn't nursing something.</i>\"");
		...
		default:
			...
			if (pc.findStatusEffect(StatusEffects.CampMarble) < 0) output(", except that mission of yours that's so important to you");
			output(".</i>\"");
			...
			if (pc.findStatusEffect(StatusEffects.CampMarble) < 0) {
				output("  I may not show it, but I'm actually really lonely on the inside.");
				//[if addiction quest is active, and Marble is ashamed of her milk]
				if (pc.statusEffectv3("Marble") == 2) output("  I really just wanted someone who loved me and loved nursing from my breast.  I had no idea what that would do to you.");
				//[if addiction quest is active, and Marble is happy with her milk]
				else if (pc.statusEffectv3("Marble") == 1) output("  But I think that I've found the one who will change that.  Right sweetie?");
				//[if addiction quest has not yet triggered]
				else output("  Though, I'm not giving up hope yet.");
				output("</i>\"");
			}
			else output("  Until I met you, I was actually really lonely on the inside.</i>\"");
...
public function postAddictionFarmHelpings():void {
	spriteSelect(41);
	clearOutput();
	output("Smiling, Whitney suggests that you go help Marble out with her chores.  You readily agree and go out to meet with her.  Afterwards, Marble offers you a bottle of her milk.  ");
	//[if player is no longer addicted]
	if (pc.findPerk(PerkLib.MarbleResistant) >= 0) output("She assures you that you can't get addicted to it again if you don't drink her milk directly from her breasts.");
...
public function postAddictionCampMornings(extra:Boolean = true):void {
	...
	if (pc.lib < 40) dynStats("lib", .1);
	pc.refillHunger(20);
	pc.slimeFeed();
	if (!extra) return;
	//(if the player has less than 5 bottles of milk in their inventory or storage containers)
	if (!pc.hasItem(consumables.M__MILK, 5)) {
		output("\n\nAs you are about to leave, Marble hands you a bottle of her milk.  ");
		//[if the player is no longer addicted]
		if (pc.findPerk(PerkLib.MarbleResistant) >= 0) output("She assures you that you'll be fine as long as you don't drink directly from her breasts.");
...
private function standardSex(newpage:Boolean = true):void {
	...
	if (pc.gender > 0) {
		output("Marble smiles at you and leads you towards her bed.  She gets you to sit on one end while she moves to the head. As she sits down, she slowly starts to remove her clothes:  First she pulls off her top and gives you a full view of her breasts, rubbing and caressing them before running one hand down to her skirt and slipping it off.  She pulls her tail up between her breasts and gives you a coy smile as she slips the ribbon on it off. She is now completely naked.  \"<i>Now it's your turn,</i>\" she tells you with a smile.\n\n");
		//(player is wearing fetish gear)		
		if (pc.armorName == "bondage patient clothes" ||
		   pc.armorName == "crotch-revealing clothes" || pc.armorName == "cute servant's clothes" ||
		   pc.armorName == "maid's clothes" || pc.armorName == "servant's clothes") {
			output("You smirk at her and demonstrate just how easy it is to get at your genitals while wearing this outfit.  But you decide to not disappoint her, making a show of removing your [pc.gear] and pretending to have some modestly when you show off your ");
			//[player has at least one dick]
			if (pc.hasCock) {
				output(pc.multiCockDescriptLight(), false);
				//player has at least one dick and a vagina]
				if (pc.vaginas.length > 0) output(" and your ");
			}
			//[player has a vagina]
			if (pc.vaginas.length > 0) output(pc.vaginaDescript(0), false);
			output(".  You also make sure that there is no lingering clothing on your body for her, just as she did for you.\n\n");
		}
		//(player is not wearing fetish gear)
		else {
			output("You smile in return and begin removing your [pc.gear] from your body.  You start by slowly slipping off your top to reveal your " + pc.allBreastsDescript() + ", ");
			//[player's breasts are at least B cup]
			if (pc.biggestTitSize() >= 2) output("running one of your hands down them before continuing.");
			//[player has small or no breasts]
			else output("running one of your hands over them before continuing on.");
			output("  You slip open the lower half of your clothes, revealing your ");
			if (pc.hasCock) {
				output(pc.multiCockDescriptLight(), false);
				//player has at least one dick and a vagina]
				if (pc.vaginas.length > 0) output(" and your ");
			...
			//[player has a vagina]
			if (pc.vaginas.length > 0) output(pc.vaginaDescript(0), false);
			output(".  You then remove any other clothing, leaving your body bare.\n\n");
		}
		//(player is a herm)
		if (pc.isHerm()) {
			output("Marble's eyes widen as you show off your equipment.  \"<i>You have both?</i>\" she says in awe. \"<i>I'm not sure if I can pleasure both parts of you at the same time... which would you prefer to use?</i>\"\n\nWhich of your genitals do you want to have sex with?");
		...
		//male
		if (pc.gender == 1) marbleSex2Continued(1);
		//female
		if (pc.gender == 2) marbleSex2Continued(2);
	}
	addNextButton(returnToCampUseOneHour);
}
...
public function marbleSex2Continued(genders:int):void {
	...
	//(player is male)
	if (genders == 1) {
		output("Marble leans back and invites you to come.  With a coy grin, you slowly crawl towards her, each movement bringing the two of you closer until you are on top of her body. She puts her hands around you and lies back completely.  You carefully line up your " + pc.cockDescript(0) + " and thrust into her warm snatch.  ");
		//[player is less than 8 inches in length]
		if (pc.cocks[0].cLength() < 8) output("You easily slide all the way into her depths before beginning your hungry thrusts.  ");
		//[player is more than 8 inches in length]
		else output("She is unable to take all of you, but you're hardly discouraged as you begin your hungry thrusts.  ");
		output("She gasps slightly and her arms clamp down on you, ");
		//[player is between 4 and 5 feet in height]
		if (pc.tallness < 60 && pc.tallness >= 48) output("keeping your head tightly locked between her breasts.  ");
		//[player is not between 4 and 5 feet in height] 
		else output("keeping your body tightly locked against her.  ");
		output("Her tight grip does nothing to slow your thrusts, only helping to bring the both of you closer and closer to sweet release.  Finally, you push into her as far as you can");
		//[player has a knot and is not more than 8 inches long]
		if (pc.cocks[0].cLength() < 8 && pc.hasKnot(0)) output(", and with a pop, your knot slips inside of her");
		output(".  Deep within her, your " + pc.cockDescript(0) + " explodes.  Within moments, Marble feels her orgasm too, her grip intensifying immensely.  You hear her give a sigh that sounds almost like a moo and her grip relents.  The two of you lay there panting, your " + pc.cockDescript(0) + " still inside her.\n\n");
	}
	//(player is female, I hope I did this right, I'm not familiar with the exact mechanics of female/female toys)
	else {
		output("Marble grins at you and reaches into the drawers of the mini-dresser next to her bed, pulling out a long, double-ended dildo. She slips one half into her own hole and invites you over to her.  With a coy grin, you slowly crawl towards her, each movement bringing the two of you closer until you slip your " +  pc.vaginaDescript(0) + " onto the other end of the dildo.  Sighing, Marble lies down underneath you and begins to rock back and forth.  You bring your own ministrations to the mix in response. Soon, the two of you are panting and moaning against each other with the dildo sliding around between you.  Suddenly, she locks her arms around you tightly and ");
		//[player is between 4 and 5 feet in height]
		if (pc.tallness < 60 && pc.tallness >= 48) output("squeezes your head into her breasts.  ");
		//[player is not between 4 and 5 feet in height]
		else output("pulls you securely against her.  ");
		output("Marble shudders with pleasure and redoubles her efforts at moving the dildo, quickly pushing you over the edge as well");
		//[player has at least one dick]
		if (pc.hasCock) output(".  Your " + pc.multiCockDescriptLight() + " erupts while clamped between your bodies");
		output(".  Her grip soon relents and the two of you lay there panting, the dildo still connecting you.  You can hear Marble say under her breath \"<i>That was way better than on my own...</i>\"\n\n");
		//(remove vaginal virginity?  "Your hymen is torn...")
		pc.cuntChange(8,true);
	}
	//(first time sex)
	if (pc.findStatusEffect(StatusEffects.FuckedMarble) < 0) {
		output("After a few minutes pass, Marble breaks the silence. \"<i>Sweetie, that was wonderful. You're really special to me, yah know?  Please remember that.</i>\"  You know that your relationship is special too; you won't forget Marble anytime soon.");
		pc.createStatusEffect(StatusEffects.FuckedMarble,0,0,0,0);
	}
	//(repeat sex)
	else {
		output("Marble sighs and gives you a big smile. \"<i>Sweetie, you're just as wonderful to be with as ever.  I'm always looking forward to our times together,</i>\" she tells you.  You would be inclined to agree with her.");
	}
	pc.orgasm();
	//– return to trigger
}
...
private function marbleMilkSex(newpage:Boolean = true):void {
	...
	output("Drinking her milk has filled you with an intense need, and you can see that need in Marble's eyes too.  You have no choice; you are going to have sex with her.\n\n");
	//[player is wearing fetish gear]
	if (pc.armorName == "bondage patient clothes" ||
		   pc.armorName == "crotch-revealing clothes" || pc.armorName == "cute servant's clothes" ||
		   pc.armorName == "maid's clothes" || pc.armorName == "servant's clothes") {
		 output("She moves first to get a good view of your equipment.  ");
	}
	//[player is not wearing fetish gear]
	else {
		output("She moves first and pulls open your [pc.gear] to get a view of your equipment.  ");
	}
	//(player is male, or a herm)
	if (pc.cockTotal() > 0) {
		//[player has only human or animal dicks]
		if (pc.cockTotal() == pc.horseCocks() + pc.dogCocks() + pc.normalCocks()) {
			output("Marble seems pleased at the sight of your " + pc.multiCockDescriptLight() + " and she pushes you back onto her bed.  She removes her own skirt and stops for a moment at her tail.  Giggling slightly, she uses her ribbon-tied tail to brush at your " + pc.multiCockDescriptLight() + " before climbing on top of you and slipping her legs to your sides.\n\n");
		}
		//[player has at least one of a different kind of dick]
		else {
			output("Marble gives a long hard look before pointing at your ");
			temp = pc.cocks.length;
			while(temp > 0) {
				temp--;
				if (pc.cocks[temp].cockType.Index > 2) {
					output(pc.cockDescript(temp), false);
					temp = -1;
				}
			}
			output(".  \"<i>Sweetie, what the heck is that?</i>\" she asks in an unsure tone. You smile at her and tell her that she should touch it.  After a moment, Marble reaches out and seems to visibly relax after feeling its texture and stiffness.  She pushes you back onto her bed before slipping free of her own garments and climbing on top of you.  She gives you a sly smile as she puts her legs on your sides.\n\n");
		}
		//[after either]
		//[player has more than one dick]
		if (pc.cockTotal() > 1) output("\"<i>So, which one is your favorite?</i>\"  Marble asks you while taking measure of your " + pc.multiCockDescriptLight() + ".  Before you have a chance to answer, Marble grabs a hold of your central " + pc.cockDescript(0) + " and says \"<i>I'll bet it's this one.</i>\"  ");
		output("Without much hesitation, Marble lifts herself up, and impales her moist lips upon your " + pc.cockDescript(0) + ".  The two of you gasp as you sink into her interior.  You feel each inch slowly flow inside her until ");
		//[dick(0) is less than 8 inches long]
		if (pc.cocks[0].cLength() < 8) output("every bit of your " + pc.cockDescript(0) + " is deep inside.  ");
		// //[dick(0) is more than 8 inches long]
		else output("Marble bottoms out at eight inches and you can go no further.  ");
		output("With a delighted shiver, Marble starts to push herself up and down on you, her movements growing more and more frantic over time.  You try to slow her down but she seems to be beyond the capability of listening to you now, giving only louder and more frantic moans of pleasure.  Sooner than you would have preferred, you explode inside her. At that moment, Marble gives a final moan that sounds almost exactly like a moo.  She slows down, seeming to have already reached her peak.\n\n");
		output("You can see that Marble is quite pleased and satisfied after that milking and sex combo session.  She rolls off you onto her bed and is soon asleep.  It takes you a few moments to get cleaned up, still in a slight daze after that frantic lovemaking.  As you depart, you give a final glance to Marble and see her still dozed off on her bed in a slightly lopsided position.");
	}
	//(player is female)
	else if (pc.vaginas.length > 0) {
		//[player has sizable breast, C or bigger]
		if (pc.biggestTitSize() > 2) {
			output("Marble runs her fingers along your " + pc.vaginaDescript(0) + " for a moment before slipping a hand onto your " + pc.biggestBreastSizeDescript() + ".  She smiles at you and says \"<i>I think that maybe we should play up here. You did enjoy mine, so I think I'll enjoy yours.</i>\"  She helps you slips off your garments and lies you down on her bed, quickly taking her place on top of you.  She sticks her tongue out before putting her mouth to one of your " + pc.nippleDescript(0) + "s and giving it a series of gentle licks.  Taking a moment to lick her lips, Marble starts to rub, caress and lick each of your " + pc.allBreastsDescript() + " in turn with her hands and face, always gently and always lovingly.  Marble seems to be quite familiar with these techniques; she has probably practiced on herself many times before.\n\n");
		}
		//[player has smaller then C breasts]
		else {
			output("She teases your " + pc.vaginaDescript(0) + " with her fingers for a moment before putting her hand on one of your " + pc.biggestBreastSizeDescript() + ".  \"<i>I'm not sure what to do with teats this small, but I'll do my best. Though I feel kinda sorry for you, sweetie.</i>\"  You open your mouth to make an indignant response, but Marble puts a figure on your lips to shush you. She gives you an intense stare and tells you in no uncertain terms that since you enjoyed her breasts so much, it's only fair that she gets to experience your " + pc.allBreastsDescript() + ".  She then sticks out her tongue at you before giving your " + pc.nippleDescript(0) + " a lick and rub down, which she extends to the ");
			if (pc.totalNipples() > 2) output("other");
			else output("rest");
			output(".\n\n");
		}
		//[after either]
		output("After finishing up");
		//[if player has more than two breasts]
		if (pc.totalBreasts() > 2) output(" with each of your " + pc.allBreastsDescript(), false);
		else output(" with each of your " + pc.breastDescript(0), false);
		output(", Marble looks up at you and sticks her tongue out again.  \"<i>Now it's time for the main course!</i>\" she whispers before lowering her head down to one of your " + pc.nippleDescript(0) + "s and closing her mouth around it. She soon begins to suckle your nipple.\n\n");
		//[if player has nipplecunts]
		if (pc.hasFuckableNipples()) {
			output("As Marble's tongue rubs against your " + pc.nippleDescript(0) + " it slips inside, much to her surprise.  She pulls back for a moment and gives you a confused look.  You tell her it isn't a problem; it only makes you happier. Understanding, Marble quickly resumes her efforts.  Her talented tongue fucks your " + pc.nippleDescript(0) + " in earnest.");
			//[player is lactating]
			if (pc.biggestLactation() > 1) output("  The whole while, Marble continues to gulp down your milk.");
		}
		//[if player is lactating]
		else if (pc.biggestLactation() > 1) output("Her suckling soon brings a stream of milk to her lips that she gulps down eagerly.  You give a contented sigh, but are soon brought out of your revelry as her sucking becomes more stimulating and intense.");
		//[player is neither lactating or has nipplecunts]
		if (pc.biggestLactation() <= 1 && !pc.hasFuckableNipples()) output("Of course, since you aren't lactating, nothing but your " + pc.nippleDescript(0) + " reaches her lips.  Marble doesn't seem to be put off by this and still gives you a thoroughly enjoyable experience.");
		output("\n\n");
		output("After a while, Marble pulls back from your " + pc.nippleDescript(0) + " and tells you, \"<i>Sweetie, I know how annoying it is to only have one of your nipples serviced, so I will be sure to finish the job.</i>\"  She dives right into your ");
		if (pc.totalNipples() > 2) output("next");
		else output("other");
		output(" " + pc.nippleDescript(0) + " and starts the experience all over again.  By the end, her efforts have successfully rung an orgasm from you. After cleaning up a bit, Marble sends you off in high spirits.");
	}
	pc.orgasm();
}
...
public function marbleAddictionSex(newpage:Boolean = true):void {
	...
	output("You rock against each other, your hands working to remove Marble's clothes while her hands work at yours.  As your respective clothing falls to the floor, ");
	//[player is under 5.5 feet in height]
	if (pc.tallness < 66) output("Marble bends over and kisses you deeply on the lips, a kiss that you eagerly return.  She lifts you up into the air, her lips still locked on yours, and holds you tightly against her body. A full minute passes before she lowers you down so your bodies can be joined.\n\n");
	//[player is between 5.5 feet and 6.5 feet in height]
	else if (pc.tallness < 78) output("Marble lowers her head and locks her lips with yours, a show of passion that you eagerly return.  The two of you stay like that for a while, time seeming to standing still.  Then Marble breaks the kiss and looks deeply into your eyes as a sly grin spreads across her face.\n\n");
	//[player is over 6.5 feet in height]
	else output("Marble grabs a hold of your head and pulls your mouth to hers for a passionate kiss.  You are only too eager to comply and join in.  Even without her vice grip on your head, it feels like you couldn't possibly leave her wonderful mouth. But you eventually push back and look into her eyes intensely.\n\n");
	//[player has at least one dick]
	if (pc.hasCock) {
		output("The feeling of need is almost palpable within the two of you, and when the time comes, neither of you hesitate. You and Marble push yourselves together, forcing your " + pc.cockDescript(0) + " deep into the soft folds of Marble's sex.  The two of you shudder and bask for a moment in the wonderful feeling of being connected, before gently starting to rock against one another.  Little else matters to you right now other than the warm, loving body that your arms are wrapped tightly around");
		//[player height is between 4 feet and 5 feet]
		if (pc.tallness >= 48 && pc.tallness <= 60) output(", the breasts your head is clamped between,");
		output(" and the warm folds where your " + pc.cockDescript(0) + " is embedded.  You can't help but think about how wonderful a person Marble is, and you bet that Marble feels the same way about you.  \"<i>I love you sweetie.</i>\" Marble says to you gently.  You assure her that you feel the same, and at the moment, nothing else matters.\n\n");
	}
	//[player only has a vagina]
	else if (pc.vaginas.length > 0) {
		output("The feeling of need is almost palpable within the two of you.  Marble giggles and reaches behind her to grab something.  She shows you a long double-ended dildo and explains it will connect the two of you.  She pushes one end of it into your " + pc.vaginaDescript(0) + " and the other end into the folds of her own sex.  The two of you shudder and bask for a moment in the wonderful feeling of being connected, before gently starting to rock against one another.  Little else matters to you right now other than the warm, loving body that your arms are wrapped tightly around");
		//[player height is between 4 feet and 5 feet]
		if (pc.tallness >= 48 && pc.tallness <= 60) output(" and the breasts your head is clamped between");
		output(".  You can't help but think about how wonderful a person Marble is, and you bet that Marble feels the same way about you.  \"<i>I love you sweetie.</i>\" Marble says to you gently.  You assure her that you feel the same, and at the moment, nothing else matters.");
		//(remove vaginal virginity? "Your hymen is torn...")
		pc.cuntChange(8, true);
		output("\n\n");
	}
	//[after either]
	output("Long after the two of you finish your lovemaking, you still continue to hold on to one another.  Eventually Marble ");
	//[player height is under 5.5 feet]
	if (pc.tallness < 66) output("sets you down and ");
	output("pushes you to arms length and asks you, \"<i>If it's all right with you, could we live together?</i>\"  You hesitate, before explaining that you don't think that's such a good idea. You explain that you aren't from this world, why you came here, and what you've found since doing so.  Marble looks at you for a moment before letting go of you and going over to her bed. Effortlessly, she lifts it up off the ground and turns to you.  \"<i>I can pull my weight, so don't you dare think I can't help you do something so important, champion.</i>\"  She says, setting the bed back down.  She barely is able to keep a straight face as she says \"<i>champion</i>\".  You smile and wonder how could you possibly turn down someone so wonderful?  \"<i>Well I guess that settles it, then,</i>\" she says happily, \"<i>I'll move right in.</i>\"");
...
public function marbleStatusChange(affection:Number, addiction:Number, isAddicted:Number = -1):void {
	if (pc.findStatusEffect("Marble") < 0) pc.createStatusEffect("Marble",0,0,0,40);
	//Values only change if not brought to conclusion
	if (pc.findPerk(PerkLib.MarblesMilk) < 0 && pc.findPerk(PerkLib.MarbleResistant) < 0) {
		pc.addStatusValue("Marble",1,affection);
		pc.addStatusValue("Marble",2,addiction);
	}
	if (isAddicted != -1) pc.changeStatusValue("Marble", 3, isAddicted);
	
	trace("Marble Affection: " + pc.statusEffectv1("Marble"));
	trace("Marble Addiction: " + pc.statusEffectv2("Marble"));
}
...
private function applyMarblesMilk():void {
	...
	if (pc.findStatusEffect("Marble"sMilk) < 0) {
		pc.createStatusEffect("Marble"sMilk,12,0,0,0);
		if (pc.str + 5 > 100) {
			str = 100 - pc.str;
			if (str < 0) str = 0;
		}
		...
		pc.changeStatusValue("Marble"sMilk,2,str);
		pc.changeStatusValue("Marble"sMilk,3,tou);
	}
	else {
		pc.addStatusValue("Marble"sMilk,1,12);
	}
	//Prevent duration from going to high.
	if (pc.statusEffectv1("Marble"sMilk) > 36) pc.changeStatusValue("Marble"sMilk,1,36);
...
I will refer to it as pc.statusEffectv4("Marble"), replace it with what you will use.
...
public function interactWithMarbleAtCamp():void {
	...
	clearOutput();
	if (flags["COC.MARBLE_PURIFICATION_STAGE"] != 2) output("How will you interact with Marble?");
	//Reminder to visit Rathazul
	else
	{
		output("<i>\"Hello, sweetie; have you had a chance to talk to that alchemist yet? I’m so close to being pure at last, but I need his help if I’m to get over this last hurdle.\"</i>",true);
	}
	...
	if (pc.itemCount(consumables.M__MILK) < 5) milkEvent = gotMilk;
	//Determine if marble has an item for the player
	if (pc.findStatusEffect("MarbleHasItem") >= 0) gatherEvent = marbleGathered;
	if (flags["COC.MARBLE_KIDS"] > 0) playtime = marbleKidsPlaytime;

	if (flags["COC.MARBLE_PURIFICATION_STAGE"] == 3 && flags["COC.MARBLE_RATHAZUL_COUNTER_1"] == 0) {
		marbleGoEvent = marblePurification.murbleShouldGoRunAlongAndGetHerCuntySisterSoTheyCanBeCuntsTogether;
		goEventString = "Go";
	}
	else if (flags["COC.MARBLE_PURIFICATION_STAGE"] >= 5) 
	{
	...
	//Sex
	if (pc.lust >= 33) sexEvent = marbleSexExpanded;
	clearMenu();
...
private function marbleTalkOverhaul():void {
	clearOutput();
	output("What do you want to discuss with Marble?");
	clearMenu();
	addButton(0,"Info",marbleInfo);
	if ( (flags["COC.MARBLE_PURIFICATION_STAGE"] < 5 && flags["COC.MARBLE_LUST"] >= 20) || (flags["COC.MARBLE_PURIFICATION_STAGE"] >= 5 && flags["COC.MARBLE_TIME_SINCE_NURSED_IN_HOURS"] >= 4) )
	{
		output( "\n\n<b>Marble is fidgeting around uncomfortably, perhaps she needs to be milked?</b>" );
		addButton( 2, "Milking", milkMarble );
	}
	if (cor() < 50 && flags["COC.MARBLE_4"] < 60) addButton(1, "Just Talk", talkWithMarbleAtCamp);
	//if (pc.level >= 10) addButton(3, "Her Hammer", hammerQuest); //Start hammer quest
	addButton(14,"Back",interactWithMarbleAtCamp);
	if (flags["COC.SLEEP_WITH"] != "Marble") addButton(4,"Sleep With",marbleSleepToggle);
	else addButton(4,"Sleep Alone",marbleSleepToggle);
}

private function marbleSleepToggle():void {
	clearOutput();
	if (flags["COC.SLEEP_WITH"] != "Marble") {
		output("Marble says, \"<i>That sounds lovely, Sweetie.</i>\"");
		flags["COC.SLEEP_WITH"] = "Marble";
	}
	else {
		output("Marble folds her arms across her bountiful bosom and sighs, \"<i>If you say so, Sweetie.</i>\"");
...
private function marbleKidsPlaytime():void {
	...
	if (flags["COC.MARBLE_KIDS"] > 1) choices[choices.length] = 2;
	if (flags["COC.MARBLE_KIDS"] > 1) choices[choices.length] = 3;
	if (flags["COC.MARBLE_KIDS"] > 1) choices[choices.length] = 4;
	if (flags["COC.MARBLE_KIDS"] > 1) choices[choices.length] = 5;
	var select:int = choices[rand(choices.length)];
	if (flags["COC.KID_SITTER"] == 2 && flags["COC.ANEMONE_KID"] > 0) select = 6;
	//Bovine Saviour (Z)
	...
	if (select == 1) {
		...
		{
			output("Your daughter immediately clops up to you and jumps into your arms.  \"<i>" + pc.mf("Daddy, daddy","Mommy, mommy") + "!  Play with me!</i>\"  You laugh at the girl in your arms, so young and yet full of energy.  She proceeds to rope you into a grand adventure, where you play the parts of the one who searches for a hero to save their camp, and the monster that has to be stopped, and also the monster's hostage that needs to be saved, while she is the great bovine saviour that carries out this grand quest.");
			output("\n\nThe game finally ends with the bovine saviour victorious over the horde of ravenous monsters  - somewhere along the line it was decided that hostages were no fun and single combat wasn't impressive enough, and you exhausted your theatrical skills to provide a bevy of satisfyingly scary faces and growls - and the people of the land (you) rejoicing.  You're left a bit tired afterwards, but you certainly don't regret having played with your child.");
		}
	...
	if (select == 2) {
		output("When you approach the nursery, the faces of your kids immediately light up and as one they cry out, \"<i>" + pc.mf("Dad","Mom") + "!  Tell us a story!</i>\"  They crowd around you, excited at the prospect of hearing of your adventures.  It would seem that, once again, the kids will be the ones that decide what will be happening in your time with them.");
		...
		//([corruption check, <=40] 
		if (cor() < 66) output("  You do make sure to skip over the items of a more explicit nature throughout.");
		output("  Eventually your story comes to an end, and you bid farewell to your kids.");
	...
	//requires at least 2 kids
	if (select == 4) {
		output("You notice Marble is sitting on some wood logs with your children.  Curious, you decide to check what exactly is going on.");
		...
		if (flags["COC.MARBLE_KIDS"] == 2) output("both");
		else output("about half of your children");
		output(" have at least some knack for it.");
	...
	if (select == 5) {
		output("Marble looks up when you approach and gives you a weary smile.  \"<i>Hello, sweetie.</i>\"");
		...
		output("\n\n\"<i>" + pc.mf("Daddy","Mommy") + "!  Is this it?  Is this it?</i>\"");
		...
		output("\n\nYour kids are just a little bit tired from all the work and it doesn't seem they'll pester Marble much more today.  One of them even grabs you, complaining, \"<i>" + pc.mf("Daddy","Mommy") + ", I'm sleepy!</i>\"  You pat her head and smile, telling her that you'll be home soon enough.");
		...
		//[Height below 5 feet]
		if (pc.tallness < 60) output("grabs you and immediately lifts you up to her chest, giving you a big hug.");
		//[Height above 5 feet]
		else output("embraces you strongly, pressing her warm body against yours.");
		output("\n\n\"<i>Thank you sweetie.  I love you.</i>\"\n\n");
		//increase fatigue by 15, decrease lust by 10, and decrease libido by 1
		pc.changeFatigue(15);
		//Takes 1-2 hours?
		inventory.takeItem(consumables.W_FRUIT, returnToCampUseOneHour);
		//end event
		return;
	}
	if (select == 6) {
	...
	if (flags["COC.MARBLE_PLAYED_WITH_KIDS_TODAY"] > 0) dynStats("lus", -10);
...
private function breakUpWithMarble():void {
	spriteSelect(41);
	clearOutput();
	output("Are you sure you want to break up with Marble?  You won't be able to get her back if you do so.");
...
private function definitelyBreakUpWithWithMarble():void {
	spriteSelect(41);
	clearOutput();
	output("You approach Marble and tell her that you need a bit of space and some time away from her.  She freezes at your words, and asks you to repeat yourself.  You do so and she nods, her face almost impassive.  \"<i>I guess I'll go back to the farm.  ");
	if (pc.findPerk(PerkLib.MarblesMilk) >= 0)
		output("I'll see you there when you need my milk, alright?");
	else
		output("I'll see you later then.");
	output("</i>\"  Marble collects her things");
	if (flags["COC.MARBLE_KIDS"] == 1) output(" and her child");
	else if (flags["COC.MARBLE_KIDS"] > 1) output(" and her children");
	output(", and leaves the camp behind.");
	addNextButton(playerMenu);
	flags["COC.BROKE_UP_WITH_MARBLE"] = 1;
	pc.removeStatusEffect(StatusEffects.CampMarble);
	pc.removeStatusEffect(StatusEffects.NoMoreMarble);
}
...
private function talkWithMarbleAtCamp():void {
	spriteSelect(41);
	if (pc.findStatusEffect("Marble"Specials) < 0) {
		pc.createStatusEffect("Marble"Specials,0,0,0,0);
	}
	addNextButton(playerMenu);
	//I don't understand your code enough yet to come up with the conditions for these ifs:
	//General thoughts:
	output("You call Marble over and the two of you sit down on some rocks to chat.  After thinking a bit, you sort through everything that has happened in this strange land, trying to come up with a new story for your bovine friend.  ",true);
	...
	if (flags["COC.MET_MARAE"] > 0 && flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] < 1)
	{
		output("You tell Marble about your visit with the Goddess Marae.  Marble is very interested in the story and listens closely to your every word.  \"<i>To think that there is still a pure Goddess in this world...</i>\" she says in wonder afterward, \"<i>But what's happening to her is so sad.  We should definitely help her if we can.</i>\"",false);
		//Level up!
		flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] = 1;
	}
	//The player has discovered the factory
	else if (flags["COC.FACTORY_FOUND"] > 0 && flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] < 2)
	{
		output("You tell Marble you found a demonic factory and relate everything you know about it.  \"<i>Be careful in there,</i>\" Marble tells you, \"<i>I'm certain that place will consume you if you're unprepared.</i>\"");
		flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] = 2;		
	}
	//The player has cleared the factory and shut it down
	else if (flags["COC.FACTORY_SHUTDOWN"] > 0 && flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] < 3)
	{
		output("You tell Marble about what you found inside the factory.  She is horrified at what was being done to the other champions and assures you that no one should ever <i>belong</i> in a place like that. You continue and tell of the overseer and her fate. Marble reacts with surprise, "); 
		if (pc.findPerk(PerkLib.OmnibusGift) >= 0)
			output("and hopes that you've learned your lesson about accepting <i>gifts</i> from demons.  ");
		else 
			output("but concludes that what you did was probably for the best.  At least you didn't fall for her trick.  ");
		output("Finally, you tell her how you shut down the factory and what happened to the captured champions.  Marble is shocked that some stayed, but says she doesn't think there is much the two of you could do to help them if they're already addicted. \"<i>You should probably just leave them be, for now.</i>\"");
	...
	else if (flags["COC.MARAE_QUEST_COMPLETE"] > 0 && flags["COC.FACTORY_SHUTDOWN"] == 1 && flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] < 4)
	{
		output("Marble is very happy to hear you helped Marae.  With the factory taken care of and Marae's corruption postponed for some time, the both of you will probably sleep a little easier tonight. ");
		flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] = 4;

	}
	//The player has met the corrupted Marae after blowing the storage tanks
	else if (flags["COC.MARAE_QUEST_COMPLETE"] >= 1 && flags["COC.FACTORY_SHUTDOWN"] == 2 && flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] < 4)
	{
		output("Your story about what had happened to Marae seems to have shaken up Marble a little.  Though, you notice that she seems to be getting more and more aroused as you relate your story.  ");
		if (pc.findPerk(PerkLib.MaraesGiftFertility) >= 0)
			output("You continue and tell her how your attempt to get Marae's Lethicite turned out.  Marble can't believe you tried that, but when she hears what happened next, her eyes go wide and she actually starts masturbating in front of you.  At the end of your tale, however, Marble looks more concerned than aroused.  She hopes you won't have too much trouble with pregnancies. That seemed to have killed the mood for her, too.");
		...
		else if (pc.findPerk(PerkLib.MaraesGiftStud) >= 0)
		...
		if (pc.lust<50) dynStats("lus", 35);
		if (pc.findPerk(PerkLib.MaraesGiftFertility) < 0 && pc.findPerk(PerkLib.MaraesGiftStud) < 0) {
			output("You finish your tale by recounting how you ran away.  She isn't really sure how to respond to your decision, but Marble does thank you for not leaving her behind and joining Marae.");
		}
		flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] = 4;
	}
	//(story) Talk about Zetaz's unexplored lair(Z)
	else if (flags["COC.DISCOVERED_DUNGEON_2_ZETAZ"] > 0 && flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] < 5) {
		output("You tell Marble about the cave you found deep in the forest.  For some reason, this seems to bother Marble, but when you ask her what is wrong, she claims it's just a feeling and tells you not to worry about it.  Maybe you should make sure you're ready before you go back there?");
		flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] = 5;
	}
	//story) Talk about Tel'Adre(Z)
	else if (pc.statusEffectv1(StatusEffects.TelAdre) >= 1 && flags[kFLAGS.MARBLE_TELADRE_STORY] == 0) {
		output("This time you've got quite a tale to tell Marble: the last free city in the world, Tel'Adre.  She is quite enthralled by the tale of the many buildings and people who live in its confines, managing to hide away from the demons thanks to the mages in the great tower, but is somewhat saddened when you tell her about how empty much of the city was, and oddly confused about the guards you met at the entrance.  In the end, she seems to decide to check it out on her own, although she doesn't say so openly.");
		flags[kFLAGS.MARBLE_TELADRE_STORY] = 1;
	}
	//(story) Talk about defeating Zetaz(Z)
	else if (flags["COC.DEFEATED_ZETAZ"] > 0 && flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] < 6) {
		output("You tell Marble about your run-in with an old foe in the forest cave.  She tenses up at first, but the obvious fact that you've made it out of there alright relaxes her.  Marble admits that for some reason she felt like you were in great danger, but couldn't figure out why she felt that.  \"<i>Anyway, I'm glad that you managed to take care of your old nemesis... that map sounds interesting, too.  Think it really leads to the demon lord's lair?  It is a bit overwhelming, sweetie, to think that you might actually defeat the demon lord!  Part of me wants to beg you not to go, but I know how driven you are, and I won't stop you from going to fulfill your mission.  Just promise me that you'll make absolutely sure you're ready before you follow that map, okay?</i>\"");
		flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] = 6;
	}
	//(story) Talk about finding Lethice's stronghold
	else if (flags["COC.D3_DISCOVERED"] > 0 && flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] < 7) {
		output("You tell Marble about how you've found the stronghold where Lethice might reside.  She looks at you with a worried look on her face.  Marble admits that for some reason she felt like you were in great danger, but couldn't figure out why she felt that.  \"<i>Be careful, sweetie.  It looks like it might be very dangerous there.  Who knows what might happen if you fall victim?  All hope would be lost.</i>\"");
		flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] = 7;
	}
	//(story) Talk after defeating Lethice
	else if (flags["COC.LETHICE_DEFEATED"] > 0 && flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] < 8) {
		output("You tell Marble about how you've fought and defeated Lethice the Demon Queen.  She looks at you and smiles.  \"<i>You've done well, sweetie.  You have managed to stop the corruption.</i>\"");
		flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] = 8;
	}
	else if (flags["COC.CORRUPTED_MARAE_KILLED"] > 0 && flags[kFLAGS.MARBLE_CORRUPT_MARAE_STORY] == 0) {
		output("You tell Marble about how you've managed to slay Marae. She drops her jaws in surprise. \"<i>Sweetie... REALLY? You've managed to kill a goddess, didn't you?</i>\" You tell her that you did manage to defeat the corrupted goddess for real. She says, \"<i>If you've managed to defeat a powerful being, that means you could dominate anything, sweetie!</i>\"\n\n")
		if (pc.armorName == "tentacled bark armor")
		{
			output("You tell her about how you've brought the bark platings infested with tentacles and got Rathazul to shape it into armor. The cowgirl takes a glance at your bizarre armour. \"<i>That armour you're wearing... There are tentacles in there. Are you okay with having tentacles, sweetie?</i>\" she says. ")
			output("You assure her that it's entirely up to you. \"<i>Thanks for telling me the story, sweetie,</i>\" she says.")
		}
		...
		output("You tell Marble about how you've managed to best Marae. She drops her jaws in surprise. \"<i>Sweetie... REALLY? You've managed to beat a goddess in a duel, didn't you?</i>\" You tell her that you did manage to defeat the goddess for real and she rewarded you with bark platings. She says, \"<i>If you've managed to defeat a powerful being, that means you could dominate anything, sweetie!</i>\"\n\n")
		if (pc.armorName == "divine bark armor")
		{
			output("You tell her about how you've brought the bark platings and got Rathazul to shape it into armor. The cowgirl takes a glance at your armor. ");
		}
		output("\"<i>Sweetie, I am proud of you. Thank you for telling me the story</i>\" she says. ");
		flags[kFLAGS.MARBLE_CORRUPT_MARAE_STORY] = -1;
	}
	//If talked about nothing!
	else output("You have no new stories to share with Marble, so you chat for a bit about inconsequential things.");
	...
	output("The topic of conversation turns to your mission and you ask Marble what she thinks you should be doing next.  ");
	//If (player has not yet met Marae)
	if (flags["COC.MET_MARAE"] <= 0) {
		output("\"<i>Well sweetie, I guess you should start with getting to know the place a little better.  Why don't you look some more around the lake outside the farm?  I think that's the safest place to start.</i>\"",false);
	}
	else if (flags["COC.MET_MARAE"] > 0 && flags["COC.FACTORY_FOUND"] <= 0)
	{
		//check if the player is far too weak to go to the factory
		if (pc.level < 3) output("\"<i>I think we should help out Marae and shut down that factory she mentioned was in the mountains, but I don't think you're ready to go into the Mountains yet. They can be brutal - get a little more practice, and make sure you've got a good weapon.</i>\"");
		else output("\"<i>I think we should help out Marae and shut down that factory she mentioned was in the mountains. I have no idea what will happen in there, though, so make sure you're as ready as you can be before you go.</i>\"\n\n");
	}
	//Player has found factory but not shut it down.
	else if (flags["COC.FACTORY_FOUND"] >= 1 && flags["COC.FACTORY_SHUTDOWN"] <= 0)
		output("\"<i>You still haven't shut down the factory yet, have you?</i>\"  You shake your head.  \"<i>Well then go do it!</i>\"");		
	//(player has completed the factory but has not returned to Marae)
	else if (flags["COC.FACTORY_SHUTDOWN"] >= 1 && flags["COC.MARAE_QUEST_COMPLETE"] <= 0 && flags["COC.MET_MARAE_CORRUPTED"] <= 0 && flags["COC.CORRUPTED_MARAE_KILLED"] <= 0)
		output("\"<i>You haven't gone back to Marae yet have you?</i>\"  You shake your head.  \"<i>Well then go see her!  I'm sure she really wants to thank you.</i>\"");
	//If PC has not yet discovered Zetaz's lair or Tel'Adre (Z)
	else if (flags["COC.DISCOVERED_DUNGEON_2_ZETAZ"] == 0 || pc.statusEffectv1(StatusEffects.TelAdre) == 0)
		output("\"<i>Well sweetie, maybe you should explore the world more?  From what I've heard from the centaurs who stop by the farm, I'm sure you could find something interesting in the deep forest, or maybe the desert,</i>\" she suggests.");
	//(suggestion) If PC has found Tel'Adre, but not Zetaz's lair(Z)
	else if (flags["COC.DISCOVERED_DUNGEON_2_ZETAZ"] == 0 && pc.statusEffectv1(StatusEffects.TelAdre) >= 1)
		output("\"<i>I was thinking that the deep woods probably still hide secrets from you, sweetie... a few times I've been there, I've noticed large numbers of imps carrying supplies.  Think there could be something to that?</i>\" she asks.");
	...
	else if (flags["COC.DISCOVERED_DUNGEON_2_ZETAZ"] > 0 && flags["COC.DEFEATED_ZETAZ"] == 0)
		output("\"<i>Well, you haven't finished exploring that cave in the woods yet, have you?  Why don't you go there?</i>\" she offers.  She still seems to be a bit bothered by something, but doesn't voice it.");
	else if (flags["COC.DEFEATED_ZETAZ"] > 0 && flags["COC.D3_DISCOVERED"] == 0)
		output("\"<i>Well, have you tried exploring the high mountains?  The map looks like it might lead there but be careful as there may be something dangerous there,</i>\" she pleads.");
	else if (flags["COC.D3_DISCOVERED"] > 0 && flags["COC.LETHICE_DEFEATED"] == 0)
		output("\"<i>Well sweetie, this is it. Your mission has been to stop the demons, Lethice is their leader. Once you're ready, you need to go to their high mountain lair and take them down. Make sure you've finished everything else you want to do before you face Lethice, you won't get a second chance.</i>\"");
	//Nothing to advise!
	else  output("\"<i>Sweetie, you've finished the main story of the game!</i>\" she says to you winking. \"<i>Why not explore? There's a lot in here that isn't a part of the main story.  You could also try changing your form and see how that changes each encounter.  Just don't get caught or lose your head, ok sweetie?<i/>\"");
	//else output("\"<i>Sweetie, you've finished all that there is in the main story of the game so far.  You'll have to wait for Fenoxo to add more,</i>\" she says to you winking. \"<i>In the meantime, explore! There's a lot in here that isn't a part of the main story.  You could also try changing your form and see how that changes each encounter.  Just don't get caught or lose your head, ok sweetie?<i/>\"");
	addNextButton(interactWithMarbleAtCamp);
}
...
private function gotMilk():void {
	spriteSelect(41);
	addNextButton(returnToCampUseOneHour);
	output("You ask Marble for a bottle of her milk, and she happily hands you one.  ", true);
	inventory.takeItem(consumables.M__MILK, returnToCampUseOneHour);
}
...
private function marbleGathered():void {
	...
	output("You ask Marble about any supplies she might have found.  She smiles and hands you her latest find.\n\n", true);
	...
	if (flags["COC.MARBLE_PURIFICATION_STAGE"] != 1 && pc.statusEffectv1("MarbleHasItem") <= 4)
		inventory.takeItem(consumables.VITAL_T, returnToCampUseOneHour);
	//Tanned Leather clothes, armor, def: 5 (three days)
	else if (flags["COC.MARBLE_PURIFICATION_STAGE"] != 1 && pc.statusEffectv1("MarbleHasItem") <= 7)
		inventory.takeItem(armors.LEATHRA, returnToCampUseOneHour);
	...
	else inventory.takeItem(consumables.LABOVA_, returnToCampUseOneHour);
	pc.removeStatusEffect("MarbleHasItem");
}

//Gives general info on how Marble works, and what she can do for the player
private function marbleInfo():void {
	spriteSelect(41);
	output("Marble is a loyal friend and lover who has decided to help you with your quest.  She can be interacted with while she is at camp.  ",true);
	output("She can share some of her thoughts and give advice on your current situation, or supply you with bottles of her milk and other useful items that she has found while scavenging.  You can also get Marble to consume some of the items you find.\n\n");
	//explain morning drinking sessions if the player is an addict
	if (pc.findPerk(PerkLib.MarblesMilk) >= 0)
	{
		output("She will nurse you every morning automatically to satisfy your addiction.  ",false);
		//otherwise tell the player that drinking bottles of her milk is safe
	} 
	else
	{
		if (flags["COC.MARBLE_PURIFIED"] == 0) output("So long as you don't drink milk from Marble's breasts again, you don't have to worry about getting addicted.  ");
		else output("Now that she's been purified, you can drink as much of her milk as you like without fear.  ");
	}
	output("Once camp improvements have been implemented, you will be able to get her to work on upgrading the camp instead of searching for supplies.");
	addNextButton(interactWithMarbleAtCamp);
}

private function canGiveItem():Boolean {
	if (pc.hasItem(consumables.OVIELIX, 1)) return true;
	if (flags["COC.MARBLE_DICK_TYPE"] == 0)
		if (pc.hasItem(consumables.P_DRAFT, 1)) return true;
	else if (flags["COC.MARBLE_DICK_TYPE"] > 0) {
		if (pc.hasItem(consumables.PINKEGG, 1)) return true;
		if (pc.hasItem(consumables.L_PNKEG, 1)) return true;
	}
	if (pc.hasItem(consumables.PROBOVA, 1) && flags["COC.MARBLE_BOVA_LEVEL"] < 2) return true;
	if (pc.hasItem(consumables.P_LBOVA, 1) && flags["COC.MARBLE_PURIFICATION_STAGE"] == 1) return true;
	if (flags["COC.MARBLE_PURIFICATION_STAGE"] >= 5) {
		if (pc.hasItem(consumables.P_S_MLK, 1)) return true;
		if (pc.hasItem(consumables.REDUCTO, 1)) return true;
		if (pc.hasItem(consumables.LACTAID, 1)) return true;
	}
	else if (pc.findPerk(PerkLib.MarblesMilk) >= 0) return true;
	return false;
}

public function giveItem():void {
	clearOutput();
	output("What item do you want to give Marble?");
	clearMenu();
	if (pc.hasItem(consumables.OVIELIX, 1)) addButton(1, "OviElixir", marbleOvulatesLikeMadDawg);
	if (flags["COC.MARBLE_DICK_TYPE"] == 0)
		if (pc.hasItem(consumables.P_DRAFT, 1)) addButton(2, "P.Incub Dr", MarbleDigsDraftsYo);
	else if (flags["COC.MARBLE_DICK_TYPE"] > 0) {
		if (pc.hasItem(consumables.PINKEGG, 1)) addButton(3, "Pink Egg", MarblepinkEgg);
		if (pc.hasItem(consumables.L_PNKEG, 1)) addButton(4, "L.Pink Egg", MarbleLPinkEgg);
	}
	if (pc.hasItem(consumables.PROBOVA, 1) && flags["COC.MARBLE_BOVA_LEVEL"] < 2) addButton(5, "Pro Bova", giveMarbleTheProBovas4Sho);
	if (pc.hasItem(consumables.P_LBOVA, 1) && flags["COC.MARBLE_PURIFICATION_STAGE"] == 1) addButton(6, "P.LaBova", marblePurification.giveMarblePureQuestLabova);
	if (flags["COC.MARBLE_PURIFICATION_STAGE"] >= 5) {
		if (pc.hasItem(consumables.P_S_MLK, 1)) addButton(7, "P.SuccMilk", marblePurification.purifiedSuccubusMilkForPureMarbles);
		if (pc.hasItem(consumables.REDUCTO, 1)) addButton(8, "ReductoBust", marblePurification.pureMurbleUsesReducto);
		if (pc.hasItem(consumables.LACTAID, 1)) addButton(0, "Lactaid", marblePurification.lactaidForPureMurble);
	}
	else if (pc.findPerk(PerkLib.MarblesMilk) >= 0) addButton(0, "Lactaid", giveMarbleLactaid);
...
public function giveMarbleLactaid():void {
	spriteSelect(41);
	//Lactaid
	pc.consumeItem(consumables.LACTAID);
	//Give Marble a dose of Lactaid to gain powerful but temporary boost to your stats, it takes an hour to do.
	output("You show Marble the bottle of Lactaid that you found and ask her if she could drink it.  She gives you a knowing smile. \"<i>So you want a special treat?  Ok sweetie, give it here.</i>\"  She takes the bottle from you and easily drinks the liquid.  Within moments, she gasps and ", true);
	if (pc.tallness<60) output("lifts you up to her chest before saying breathlessly saying \"<i>Drink now!</i>\"  You waste no time obliging her.\n\n");
	else if (pc.tallness > 72) output("breathlessly says\"<i>Drink now!</i>\"  You waste no time and descend upon her breasts.\n\n");
	else output("breathlessly says, \"<i>Drink now!</i>\"  You quickly move forward to get a drink.\n\n");
	output("What meets your lips surprises you; it feels tastier, healthier, and just plain better than normal.  With an effort, you manage to empty her breasts, even though they seemed to have been filled with more milk than usual.  You feel even more powerful than normal and the whole world seems clearer.  However, Marble seems a little disappointed; it seems the Lactaid's effects on her were only temporary.\n\n");
...
private function marbleCampSexNew():void {
	spriteSelect(41);
	if (pc.findStatusEffect("Infested") >= 0) {
		output("  You call Marble over and ask her if she can give you some release.  She smiles at you and gently grips your " + pc.cockDescript(0) + " in one of her hands before recoiling in horror.  \"<i>Uh, why don't you take care of that problem of yours in your cock first, sweetie?  Then I'll help you get release.</i>\"  It looks like Marble isn't willing to help you get release while you have worms infecting your cock.");
		return;
	}
	//Marble isn't willing to pleasure the PC if her lust is lower than 0
	if (flags["COC.MARBLE_LUST"] < 0) {
		output("  You call Marble over to see if she can help you get some release.  She looks at you sourly before saying, \"<i>I'm not in the mood right now, but feel free to use the toy in my bag on yourself.</i>\"  Well, that's generous... er, wait, did she just tell you to go fuck yourself?");
		//end event
		return;
	}
	flags["COC.MARBLE_LUST"] = 0;
	//Non nagas && nontaurs
	if (!pc.isNaga()) {
		//Feeding
		if (pc.findPerk(PerkLib.MarblesMilk) >= 0 && pc.gender > 0 && rand(2) == 0) {
			...
			if (pc.tallness < 66) output("right breast.");
			//[height above 5' 7''] 
			else output("left breast.");
			//[+ if height above 8' 6'']
			if (pc.tallness > 90) output("  You do need to bend over in order to access her breast as well as allow her access to your groin, but while slightly uncomfortable, your plan is worth the effort.");
			output("\n\nAt the same time, Marble's right hand is slowly moving down your abdomen, lingering a bit just over your [pc.hips], as she awaits your 'answer'.  Gently, you grab Marble's breast with both hands and slowly guide the " + marbleNip() + " to your thirsty lips.");
			...
			//( [Male/Herm] 
			if (pc.hasCock()) {
				output("She chuckles as her hand reaches your " + pc.multiCockDescriptLight() + ".  \"<i>You seem quite ready and eager for something, sweetie.</i>\"");
			}
			else {
				output("She chuckles as her hand reaches your " + pc.vaginaDescript(0) + ".  ");
				//[Vaginal wetness 0-1] 
				if (pc.wetness() < 2) output("\"<i>Oh, sweetie, this just won't do!  It's like you're not looking forward to this at all!</i>\""); 
					//[Vaginal wetness 2-3] 
				else if (pc.wetness() < 4) output("\"<i>It seems you're quite in need of something down here, sweetie...</i>\"");
				//[Vaginal wetness 4]
				else if (pc.wetness() < 5) output("\"<i>My, my, sweetie; this is so wet... are you sure we should stimulate it more?</i>\"");
				//[Vaginal Wetness 5] 
				else output("\"<i>Oh my, sweetie; it seems like you need to be toweled off...</i>\"");
			}
			...
			//[Male/herm] 
			if (pc.hasCock()) {
				output("\n\nHer hand continues stroking your " + pc.cockDescript(0) + ", very gently at first, but once you intensify your suckling, she gasps slightly and gives your member an appreciative squeeze, increasing the intensity.  You buck your hips slightly but she just strokes your " + pc.hairDescript() + " as if to calm you down.");
				output("\n\n\"<i>It doesn't hurt, right?  I want to make you feel good, sweetie...  If you keep sucking me, I'll make you feel very, very good.</i>\"");
				
				output("\n\nYou gently moan into her breast in response, causing her to giggle as she plays with the tip of your " + pc.cockDescript(0) + ".  She returns to the pumping motion and increases the pace just as you decide you need something to grab onto - and the most pleasant things at hand are her breasts.  Marble gasps in surprise and moans as you rub her breasts while suckling, and starts jerking you off even faster in response.");
				//( [Female Marble]
				if (flags["COC.MARBLE_DICK_LENGTH"] == 0) output("  Her thighs seem to be rubbing together, and the place you're lying on feels a tiny bit wet, so you assume it isn't only you who's getting more and more excited by this.");
				else output("  Her thighs are gently rubbing beneath you, and you feel something hard poking you in the back.  You smile around the nipple you have in your mouth.");
				//[If balls] : A
				if (pc.balls > 0) output("  As her hand reaches the base of your cock, she stops stroking it to take a moment and gently roll your " + pc.ballsDescriptLight() + " with her fingers.  \"<i>Are these ready to shoot out for me soon, sweetie?</i>\""); 
				//([If pussy] : 
				if (pc.hasVagina()) output("  From the base of your cock, her hand moves even lower, gently rubbing your " + pc.clitDescript() + ".  \"<i>This seems to also require some attention...  Maybe later, sweetie, if you do a good job nursing from me.  But right now, I want to see you shoot off,</i>\" she says, as her hand moves back to your " + pc.multiCockDescriptLight() + ".");
				output("\n\nShe starts tugging at and stroking your " + pc.cockDescript(0) + ", eager to make you cum now.  You buck your hips again and she responds more agressively this time, pushing you back down with her breasts as her touch starts to become unbearable.  \"<i>You're close, aren't you, sweetie?  It's okay, I want to see you cum.</i>\"");
				...
				if (pc.cumQ() < 500) output("  You cover her hand as she keeps jerking you off, squeezing out tiny little droplets once you're done ejaculating.");
				else if (pc.cumQ() < 1000) output("  Your load is as large as always, and ropes of cum fly into the air, covering her arm and your abdomen... and you swear one string even lands of her right breast along the way.  \"<i>That's it, sweetie, shoot all you have.</i>\"");
				else output("  You pretty much cover yourself, Marble, and the surrounding ground with your enormous white load of cum.  She seems numbed from the sheer output you're capable of.  \"<i>Uh... sweetie...</i>\"");
				output("\n\n\"<i>Well... that was fun.</i>\"  Marble slowly pulls you away from her breast");
				if (pc.cumQ() >= 1000) output(", then wipes the nipple clean on the other");
				output(".  \"<i>Now, sweetie, if you'd be so kind to suck on the other one...</i>\"");
			}
			//[Female] 
			else {
				output("\n\nHer hand moves onto your pleasure button, starting to gently rub your [pc.clit] as her milk flows into your mouth.  When you intensify your suckling, she gasps slightly and slides a finger into your " + pc.vaginaDescript(0) + ", pumping it in and out as her palm has its way with your [pc.clit].  You buck your hips slightly but she just strokes your " + pc.hairDescript() + " as if to calm you down.");
				output("\n\n\"<i>That feels good, doesn't it, sweetie?  You should suck my breasts some more, I'd really appreciate that.</i>\"");
				output("\n\nYou gently moan into her nipple in response, causing her to giggle as she plays with the tip your " + pc.clitDescript() + ".");
				//([Normal clit] 
				...
				output("  She returns to pumping her fingers in and out of your pussy and, just as she increases the pace, you decide you need something to grab onto - and the most pleasant things at hand are her breasts.  Marble gasps in surprise and moans as you rub her breasts while suckling, and starts fingering you even faster in response.");
				//( [Female Marble] 
				if (flags["COC.MARBLE_DICK_LENGTH"] == 0) output("  Her thighs seem to be rubbing together, and the place you're lying on feels a tiny bit wet, so you assume it isn't only you who's getting more and more excited by this.");
				else output("  Her thighs are gently rubbing beneath you, and you feel something hard poking you in the back.  You smile around the nipple you have in your mouth.");

				output("\n\nYou're at the edge already, and the fingering of your " + pc.vaginaDescript(0) + " as well as the rubbing motion against your [pc.clit] soon send you over it.  Thrashing in Marble's embrace, you release your feminine juices all over her hand.");
				if (pc.wetness() >= 4) output("  Messy as it is, Marble never stops, wanting to make the orgasm as intense as possible.");
				//([Squirter]
				if (pc.wetness() > 4) output("  Soon, you're shooting your femcum all over her arm, and she giggles at the intensity and visibility of your orgasm.");
			...
			pc.slimeFeed();
			flags["COC.MARBLE_LUST"] += 20;
		}
		//ORAL
		else if (rand(2) == 0 && pc.gender > 0) {
			clearOutput();
			...
			output("You walk up to your big-breasted lover, a feverish look in your eyes.  \"<i>Hmm... looks like you need to release some tension, don't you sweetie?</i>\"  You nod in approbation, ");
			if (pc.hasCock()) output("a tent growing in your " + pc.armorName);
			else if (pc.hasVagina()) output("a moistness growing under your " + pc.armorName);
			output(" as you keep staring at her tender eyes; you instinctively know she's going to handle your junk with care.");
			...
			if (pc.hasCock()) {
				output("[pc.eachCock] throbs");
			}			
			else {
				output("your " + pc.clitDescript() + " quivers in arousal");
			}
			output(".\n\n");
			output("Marble gives you a kinky look and coos, \"<i>Do you like what you see?  Because I sure do like what I see.</i>\"  Her hand goes to your crotch and delivers some teasing squeezes until you can't do anything but moan under her gentle fondling.  She smiles at you, happy to see the effect her efforts have on you.  \"<i>Relax, sweetie... I really believe you need some more love...</i>\"  You trill in excitement as she starts going down on you.");
			output("\n\nThe contact of her hands has been replaced with a sensation of sweet warmth.  Marble effortlessly licks your groin, ");
			if (pc.hasVagina() && !pc.hasCock()) output("probing her tongue in your " + pc.vaginaDescript(0) + ", licking you in the most appropriate places");
			else output("wrapping her tongue around your rod of dickflesh, squeezing and pumping beads of pre-cum up your shaft");
			output(".  She's doing an amazing job, and you thrash wildly under her smothering tongue assaults.");
			//([vag only/has a cock]
			output("  Marble keeps ");
			if (pc.hasVagina() && !pc.hasCock()) output("poking your cunt");
			else output("suckling the juices out of your urethra and literally absorbing the drops of pre-cum as they spurt out of the tip of your junk.");
			//[if multicock]
			if (pc.cockTotal() > 1) output("  She alternatively sucks and jerks every cock you have, making sure there isn't a single meat-tower that isn't being taken care of.");
			if (pc.balls > 0) output("  Her hands softly rub your [pc.balls], making them churn and swell in pure arousal.");
			...
			if (pc.gender == 2) output("tongue-fuck");
			else output("deepthroat");
			output(" you.  Your hips thrust back and forth, enjoying the fluid motion of her tongue working over your wet, saliva-slathered genitals.");
			//[if cock] 
			if (pc.hasCock()) {
				output("  Her lips tightly enclose your " + pc.cockDescript(0));
				if (pc.cockTotal() > 1) output(" while she strokes another");
				output(", acting as an organic cock-ring.  She looks up to you as you watch her lips undulating under the repeated pulsations of your spooge-pumping " + pc.cockDescript(0) + ".  These cocksuckers are teasing you in the most infuriating way, and you savagely shove more of your prick down her throat, stretching her jaw and making her moan in delight, although her cries of pleasure are muffled by the pecker stuffing her mouth.");
			}
			//[if vagina]
			else output("  Her flexible, yet firm tongue really feels like a dong and it keeps provoking you with its prolonged digging. With a mighty thrust, you shove her against your " + pc.vaginaDescript() + ", forcing the entirety of her oral muscle inside.");
			...
			//[if cock]
			if (pc.hasCock()) {
				output("\n\nYour [pc.cock] palpitates, tip twitching on its own against Marble's throat as it liberates your milky essence.  Marble eagerly gulps, drinking with avidity that reminds you of how you ");
				if (pc.findPerk(PerkLib.MarblesMilk) >= 0) output("usually");
				else output("used to");
				output(" suckle her tits.");
				//[if high cum production] 
				if (pc.cumQ() >= 500) output("  Your " + pc.cockDescript(0) + " keeps spouting more seed inside her, efficiently distending her belly");
				//[if cum production is massive]until she looks ready to give birth.[/]
				if (pc.cumQ() > 1500) output(" until she looks ready to give birth.");
				output("  Her lips cling to your cum-tube as if her life depended on it; she sucks viciously, milking your junk of every drop of its baby-batter.");
			}
			//[if vagina]
			else output("\n\nYour " + pc.vaginaDescript(0) + " quivers and vibrates before releasing a vigorous discharge of fem-spunk, splattering Marble's lips and mouth; she drinks all your juices with an insatiable cum-thirst.  Her tongue explores your insides as if to make sure there's no girlcum left to absorb.");
			...
			output("\"<i>You feeling antsy, sweetie?</i>\" Marble asks as you walk up to her.  \"<i>Don't worry; I'm ready and able to help.</i>\"  ", true);
			//Select one option based on the sum of the player's and Marble's Corr, similar to the one in regular masterbation
			if (cor() + flags["COC.MARBLE_4"] < 66) {
				output("The two of you find a secluded place in the camp and slip off your clothes.\n\n");
			} 
			else if (cor() + flags["COC.MARBLE_4"] < 132) {
				output("The two of you strip down right there in the middle of camp.\n\n");
			} 
			else {
				output("You both look around for another member to join in. No one jumps at the offer, so the two of you strip down.\n\n");
			}
			//all the foreplay here
			output("Marble smiles at you and playfully pushes her sizable breasts together a few times before ");
			//Marble titfucks the players upper body, two variations, boobs and no
			if (pc.biggestTitSize() > 3) {
				output("pressing them against your own " + pc.biggestBreastSizeDescript() + ".  She giggles slightly as she puts one of them between her mounds and rubs them up and down around it, teasing your " + pc.nippleDescript(0) + " with her own " + marbleNips() + ", a trickle of milk leaking out onto your locked chests.");
			} else {
				output("pressing them against your chest.  She rubs them up and down against you as she moves around, making sure she has rubbed each part of your upper body at least three times over.  She is especially happy with moving your head around between her breasts.");
			}
			//If Player has at least one dick, Marble titfucks that
			if (pc.cocks.length > 0) {
				output("\n\nShe moves down to your crotch and slips your " + pc.cockDescript(0) + " between her breasts.  She smiles at you and says, \"<i>This is what you were hoping for, right?</i>\" before squeezing them together tightly and pushing your " + pc.cockDescript(0) + " in and out.  You sigh in pleasure and smile down at Marble as she pumps you between her breasts.  But it seems Marble is unsatisfied with only that; she quickens her pace and starts moving each of her breasts up and down opposite each other.");
				//Multiple dick side note
				if (pc.cocks.length > 1) {
					output("  Yet still unsatisfied, Marble engulfs and rub the rest of your " + pc.multiCockDescriptLight() + " within her ample mounds.  ");
				}
			}
			//If Player has a vagina, Marble titfucks that
			if (pc.vaginas.length > 0) {
				output("\n\nShe slides down your body to rub and caress your " + pc.vaginaDescript(0) + " with her breasts, sometimes pushing one of her " + marbleNips() + " inside your " + pc.vaginaDescript(0) + " and pulling a small gasp from you.  ");
				//If clit is too small to be titfucked...
				if (pc.clitLength < 2) {
					output("  Marble runs down each of your legs once with her breasts before unexpectedly plunging two of her fingers inside your " + pc.vaginaDescript(0) + ".");
				} 
				else {
					output("  Marble then pulls your " + pc.clitDescript() + " between her breasts and does her best to pump and stimulate it as though it were a cock.");
				}
			}
			//If Player is genderless, Marble improvises
			if (pc.gender == 0) {
				output("  She then says, \"<i>I think I've found a way to make you cum, sweetie,</i>\" with a sly expression.  She moves down from your upper body and starts rubbing and caressing your lower body too. It is extremely arousing, but it doesn't seem to be enough to push you over the edge.  Marble stands up, kisses you and rubs a single finger against the smooth skin of your groin.  You are so close to going over the edge.  Then one of her fingers pushes up against your " + pc.assholeDescript() + ", causing you to go rigid from the new stimulation.");
			}
			//Sweet release
			...
			if (pc.cocks.length > 0) {
				...
				if (pc.cumQ() < 25) output("  In moments, a small amount of your cum is covering Marble's breasts.");
				else if (pc.cumQ() < 250) output("  After a few moments, a fair amount of your cum is covering Marble's breasts and face.");
				else if (pc.cumQ() < 500) output("  You keep cumming and cumming, until eventually Marble is completely drenched in your cum.");
				else output("  Your seemingly endless orgasm eventually results in the two of you becoming completely drenched in your cum, as well as a large part of the ground around you.");
			}
			...
			if (pc.vaginas.length > 0) {
				...
				if (pc.vaginas[0].wetness() < VAGINA_WETNESS_WET) output("  As you recover, you see that a small amount of your girly fluids has leaked onto Marble's breasts.");
				else if (pc.vaginas[0].wetness() <= VAGINA_WETNESS_SLICK) output("  As you recover, you see that Marble has been covered fairly liberally with your girly fluids.");
				else if (pc.vaginas[0].wetness() <  5) output("  As you recover, you see that Marble is decently drenched in your girly fluids.");
				else output("  It takes you a minute to recover, but upon doing so, you blush and realize just how soaked with fluid you've become.");
			}
			//finally some philosophy on what an orgasm would feel like for someone who is genderless
			if (pc.gender == 0) output("  It's not quite the same as when you had genitals, but it is undeniably an orgasm.  While it is essentially a pointless orgasm, it's also almost pure in a way.  There is not a feeling inside you but the explosion of pleasure.");
	...
	//NAGAS
		...
		if (pc.lib >= 50) output("  Quietly, you reach down and hook your fingers into the hem of her clothing, allowing her to pull it half off with her next shift and expose her pussy");
		if (flags["COC.MARBLE_DICK_LENGTH"] > 0) output(" and " + marbleCock());
		output(".  \"<i>Why, you shameless " + pc.mf("pervert","hussy") + "!</i>\" the cow-girl says in mock-offense, winking at you.");
		...
		if (pc.gender > 0) {
			...
			if (pc.hasCock()) {
				output(pc.multiCockDescriptLight());
				if (pc.hasVagina()) output(" and ");
			}
			if (pc.hasVagina()) output(pc.vaginaDescript());
			output(".  \"<i>Well, sweetie, looks like I don't have to go digging for treasure; it's already right here for me,</i>\" she whispers to you.  With one hand she ");
			if (pc.hasCock()) output("grabs your shaft");
			else output("penetrates your pussy");
			output(", while using her long tongue to lick the ");
			if (pc.hasCock()) output("tip");
			else output("clit");
			output(".  You cry out in pleasure, and start to wrap your long tail about Marble's shapely body.  You can't hold back for long, and soon reach orgasmic bliss, all while tightly clinging to your cow-girl lover.");
			if (pc.hasCock()) output("  [pc.EachCock] unleashes a load of semen right into her mouth, which she swallows with pink-faced, wanton gusto.");
			if (pc.hasVagina()) output("  Your [pussy] runs and squirts as you moan, drooling your juices into the little valley where Marble's breasts press against your scales.");
		} 
		else {
			output("\n\nWhen she reaches where your genitals would be - if you had any - you're panting with pleasure.  Marble gives you a smile and whispers, \"<i>Don't worry sweetie, I know just how to make you cum.</i>\"  She then hugs your body tightly, reaching her arms around and squeezing and slapping your [pc.butt], while slipping a finger into your " + pc.assholeDescript() + ".  You gasp at the surprise, but are soon panting even harder.  The fact that she is still rubbing her breasts on your abdomen even as she teases you helps even more, and you can't help but coil your lower body around Marble, rubbing every part of it on her.  In moments, you cry out in bliss as your unusual anal orgasm washes over you.");
		}
...
private function marbleNips():String {
	if (pc.findStatusEffect("Marble"Specials) < 0) {
		pc.createStatusEffect("Marble"Specials,0,1,0,0);
	}
	//Set nipples to 1 if uninitialized
	if (pc.statusEffectv2("Marble"Specials) == 0) pc.changeStatusValue("Marble"Specials,2,1);
	if (pc.statusEffectv2("Marble"Specials) == 1) return "nipples";
	if (pc.statusEffectv2("Marble"Specials) == 4) return "quad-nipples";
...
private function marbleCock():String {
	...
	if (int(Math.random()*3) == 0) {
		if (flags["COC.MARBLE_DICK_LENGTH"] < 3) descript = "squat ";
		if (flags["COC.MARBLE_DICK_LENGTH"] >= 3 && flags["COC.MARBLE_DICK_LENGTH"] < 5) descript = "short ";
		if (flags["COC.MARBLE_DICK_LENGTH"] >= 5 && flags["COC.MARBLE_DICK_LENGTH"] < 7) descript = "average ";
		if (flags["COC.MARBLE_DICK_LENGTH"] >= 7 && flags["COC.MARBLE_DICK_LENGTH"] < 9) descript = "long ";
		if (flags["COC.MARBLE_DICK_LENGTH"] >= 10 && flags["COC.MARBLE_DICK_LENGTH"] < 13) descript = "huge ";
		if (flags["COC.MARBLE_DICK_LENGTH"] >= 13 && flags["COC.MARBLE_DICK_LENGTH"] < 18) descript = "massive ";
		if (flags["COC.MARBLE_DICK_LENGTH"] >= 18 && flags["COC.MARBLE_DICK_LENGTH"] < 30) descript = "enormous ";
		if (flags["COC.MARBLE_DICK_LENGTH"] >= 30) descript = "towering ";
	}

	//Discuss girth one in 3 times.
	if (int(Math.random()*3) == 0) {
		//narrow, thin, ample, broad, distended, voluminous
		if (flags["COC.MARBLE_DICK_THICKNESS"] <= .75) descript += "narrow ";
		if (flags["COC.MARBLE_DICK_THICKNESS"] > 1 && flags["COC.MARBLE_DICK_THICKNESS"] <= 1.4) descript += "ample ";
		if (flags["COC.MARBLE_DICK_THICKNESS"] > 1.4 && flags["COC.MARBLE_DICK_THICKNESS"] <= 2) descript += "broad ";
		if (flags["COC.MARBLE_DICK_THICKNESS"] > 2 && flags["COC.MARBLE_DICK_THICKNESS"] <= 3.5) descript += "fat ";
		if (flags["COC.MARBLE_DICK_THICKNESS"] > 3.5) descript += "distended ";
	}
...
private function rapeDAHMARBLEZ():void {
	...
	clearOutput();
	output("You decide that rather than helping her, you are going to roughly manhandle her breasts and rape her.  You suddenly grab at her breasts and squeeze them roughly, at which point she screams and ");
	...
	if (raped) {
		...
		//wrapping things up
		output("You close your eyes and revel in the moment before feeling Marble roll over and grab one of your hands.  You open your eyes just in time to see a big hammer-head flying towards your face...  ");
...
private function marbleRapeCorruptFantasy():void {
	spriteSelect(41);
	output("Marble is helpless before your onslaught with your superior position, and you find it immensely enjoyable to have someone trapped under you like this.  You start to fantasize just what it would be like if everyone were like this to you, just from being in your presence.  You imagine a sea of asses and pussies all stuck up in the air for you to rape at your leisure, and none of the owners able to do a damn thing about it.\n\n");
	//do they really want to have this fantasy?  How far are they gone?
	if (cor()>=66) {
		output("You shake your head, clearing the bizarre fantasy from your mind and returning to the more immediate enjoyment.\n\n");
	} 
	else {
		output("You smile to yourself and think, \"<i>One day,</i>\" before returning to the more immediate enjoyment.\n\n");
	}
}
...
public function marbleAfterRapeBattle():void {
	spriteSelect(41);
	output("While exploring the farm, you notice the cow-girl that hit you earlier, Marble, coming out of the barn.  You could try confronting her if you want to, or you could just avoid her from now on.", true);
...
private function marbleAfterRapeYes():void {
	spriteSelect(41);
	//If choice was yes 
	output("Deciding to deal with her, you move towards the barn.  However, Marble spots you on your way over and quickly disappears inside.  Just as you get to the entrance, she re-emerges with a large two handed hammer in hand.  \"<i>Leave right now, or this hammer is going into your head,</i>\" she tells you with an angry look in her eyes and drops into a combat stance.  Will you fight her?", true);
...
private function marbleAfterRapeStartFight():void {
	spriteSelect(41);
	output("You drop into your own combat stance; it's time to get even with her for last time.  ", true);
...
private function marbleAfterRapeNo():void {
	spriteSelect(41);
	output("You shake your head. It's just not worth the headache to deal with this cow.  You turn around and leave; you aren't going to be seeing her anymore.", true);
	addNextButton(returnToCampUseOneHour);
}
...
public function marbleBadEndFollowup():void {
	...
	var morph:String = pc.race(); //Now uses actual race.
	...
	if (pc.tallness < 54) approxHeight = "very short";
	else if (pc.tallness < 66) approxHeight = "short";
	else if (pc.tallness < 72) approxHeight = "average";
	else if (pc.tallness < 84) approxHeight = "tall";
	...
	//BEGIN BAD-ENDNESS
	output("Over time, the two of you learn to get along and accept the way things have gone.  Before long, the two of you become close friends, then even lovers.  At the same time, you learn the ways of life on the farm and adjust to your new life successfully.  As the months pass, things remain much the same from day to day. Until nearly a year later...\n\n\n");
	//SHIFT ABOVE TO END OF RELEVANT BAD ENDS
	output("The young champion moved quickly towards the barn; he was really looking forward to seeing this 'person he might know' as that dog-girl had described.  Maybe it was another one of the champions!  He thought as he hurried over.\n\n");
	//does he find a human?
	if (morph=="human") {
		output("His eyes light up as he spots a very attractive ");
		//does that person have a noteworthy stature?
		if (approxHeight!="average height")
			output(approxHeight + " ");
		output(pc.mf("male", "female") + " human leaning against one of the barn doors in a farm hand outfit.  It looks like " + pc.mf("he", "she") + "'s taking a break from something.  With a big smile on his face, the young champion strides up to " + pc.mf("him", "her") + " and says, \"<i>Hello there, it's rare to see a human in this world.</i>\"  The other looks up at him for a few moments before smiling in recognition and saying \"<i>So they picked you to go after me, huh?</i>\"  The young champion stared at the " + pc.manWoman() + " for a few moments, unable to answer, this was better than he could have ever hoped for!  He was looking at last year's champion, " + pc.short + "!\n\n");
		output("\"<i>I'm so glad to have found a friend like you in this world, " + pc.short + ". ");
	}
	//that's no human!
	else {
		output("However, he is rather surprised to see a " + pc.mf("male", "female") + " " + morph + " leaning against the barn wall next to the entrance.  The young champion approaches the " + morph + " with a little trepidation and calls out to them <i>\"Hello there, uh, Whitney said I might know you.</i>\"  The " + morph + " looks up at him for a few moments before smiling in recognition and saying \"<i>So they picked you next, huh?</i>\"  The young champion starts at this declaration.  \"<i>How do you know who I am?</i>\" he demands indignantly, \"<i>What do you know about my home?</i>\" the other chuckles and says, \"<i>I'm " + pc.short + ", despite my form, I'm still the same person.  Being able to change is one of the joys of this messed up world.</i>\"  The young champion looks uncertainly at them for a few moments before realizing that the " + morph + " is telling the truth!\n\n");
		output("\"<i>You really are " + pc.short + "!  I can't believe you left your humanity behind, but I'm glad to have found a friend in this world.  ");
	}
	//does he comment on the player's height?
	if (approxHeight=="very short" || approxHeight=="short") {
		output("Though I see you've gotten a little shorter then I saw you last.  ");
	} 
	else if (approxHeight=="tall" || approxHeight=="very tall") {
		output("You've gotten bigger since I saw you last.  ");
	}
	output("Could you join me at my camp?  </i>\" the younger champion asks, \"<i>Your experience would help me out a great deal, and I'd be glad to have another fighter on my side.</i>\"  The older champion shakes " + pc.mf("his", "her") + " head and replies, \"<i>I'm sorry, but I abandoned my quest some time ago.  There is simply something here that is more important to me than that.</i>\"  The younger champion looks at " + pc.mf("him", "her") + " in alarm and asks \"<i>What could be more important than our mission?</i>\"\n\n");
	output( "At that moment, an earthly female voice calls out from inside the barn.  \"<i>Sweetie, is that someone you know?</i>\"  The older champion smiles and replies to the voice, \"<i>Yes Marble, it is.  You should come out and meet him.</i>\"  <i>\"Alright,</i>\" the voice calls back.  ");
	//could the player get Marble pregnant?
	if (pc.gender == 1 || pc.isHerm()) {
		output("A tall female cow-girl then steps out of the barn entrance.  The young champion notes just how pretty she is, if a bit imposing, but he also quickly notices that she is quite clearly pregnant.  \n\n");
		output("<i>\"Marble,</i>\" the older champion says to her, \"<i>this is a friend, he's actually from my village.  This is Marble, she is the reason I left my mission.  I care about her too much, and she is carrying my child.</i>\"  ");
		//mysterious baby?
		if (pc.isHerm() && pc.mf("m", "f") == "f" && pc.biggestCockArea() < 15) {
			output("\"<i>Wait, your child?!  How?</i>\" the younger champion stammers. The older one grins at him and says, \"<i>So I'm the first you've met?  Well then...</i>\" before dropping " + pc.mf("his", "her") + " pants and revealing " + pc.mf("his", "her") + " " + pc.multiCockDescriptLight() + " and " + pc.mf("his", "her") + " " + pc.vaginaDescript(0) + " to him.  The younger champion's jaw drops and he simply stares at " + pc.mf("him", "her") + " for a time, during which " + pc.mf("he", "she") + " giggles at him and says, \"<i>They're fully functional too.  Would you like to see them in action?</i>\"  The younger champion finds he is only able to stammer out an incoherent answer to this.  \"<i>I think I'll take that as a 'yes'. Don't worry, it'll be fun.</i>\" the older champion says, leading the young one along with Marble back inside the barn.  \"<i>Afterwards, maybe I can give you some advice on dealing with this world.</i>\"  ");
		}
		//if this isn't a herm, then met the family
		else {
			output("\"<i>Really?  Well I guess I should congratulate you on your new family,</i>\" the young champion says with a little uncertainty.  \"<i>Oh, it's not really a new family,<i>\" the other responds as a pair of little girls that look very much like little Marbles come running out of the barn, one chasing the other.  \"<i>Oh wow, you've found a nice family to join,</i>\" the younger champion says, watching the girls run off towards the farm house.  \"<i>Were those twins?</i>\"  \"<i>No,</i>\" the older champion responds, \"<i>Mili is several weeks older then Aura, but they're both my children.</i>\"  The younger champion stares at him incredulously before stammering \"<i>But how?  They're at least a few years old!</i>\"  The older champion puts his arm around the younger one's shoulder and leads him and Marble inside the barn telling him.  \"<i>You have a lot to learn about this world if you're going to last.</i>\"  ");
		}
		//girls and those without naughty bits go here 
	}
	else {
		output("A tall female cow-girl then steps out of the barn entrance.  The young champion notes just how pretty she is, if a bit imposing.\n\n");
		output("<i>\"Marble,</i.>\" the older champion says to her, \"<i>this is the new champion, he's actually from my village.  This is Marble, she is the reason I left my mission.  I care about her too much.</i>\"  The younger champion looks at the other in confusion.  \"<i>What do you mean by that?<i>\" he asks " + pc.mf("him", "her") + ".  Marble giggles, and invites the older champion to suckle her nipples, which they eagerly begin doing.  \"<i>I- what?  Huh?!</i>\" the younger champion stammers at the sight, unable to believe his eyes.  \"<i>Like my sweetie here said, we have a special relationship,</i>\" she tells him smiling, easily recognizing the tell-tale signs of arousal at what he was seeing.  \"<i>Does your relationship extend to, um, other things?</i>\" he asks a little nervously.  The older champion turns away from Marble's breast and walks over to the younger one, putting " + pc.mf("his", "her") + " arm around the young champion's shoulders.  With a little milk still dripping from " + pc.mf("his", "her") + " mouth, the older champion leads the younger one inside the barn with Marble, saying \"<i>Yes, would you care to join us?</i>\"  ");
	}
	//If Marble is a pusher, she starts to tempt this champion
	if (pc.statusEffectv3("Marble") == 1) {
		output("After a moment, \"<i>By the way,</i>\" Marble's voice can be heard from inside saying, \"<i>do you like milk?\"</i>");
		//dun dun dun!  That was dumb; I apologize for the lame joke.
	}
	//wrap things up
	output("\n\n");
	output("You've abandoned your quest due to your inability to refuse Marble, thanks to the effect her milk has on you.  However, you may have a chance to help the next one complete his quest, or maybe you won't.  As it stands now, this is where your story ends.");
	getGame().gameOver();
}
	...
	if (pregnancy.type == PregnancyStore.PREGNANCY_PLAYER) {
		...
		if (flags["COC.MARBLE_PURIFIED"] > 0 && rand(2) == 0)
		{
			...
			if (flags["COC.MARBLE_BOYS"] ==0)
			{
				...
				output("You ask her why she hesitated like that.  \"<i>Oh?</i>\" she starts, looking up at you, \"<i>Sorry sweetie, it's just that I never thought I would give birth to a boy.  It just never occurred to me.</i>\"  Then her expression changes.");
			}
			else
			{
			...
			flags["COC.MARBLE_BOYS"]++;  //again, n is the flag for the number of male kids Marble has had
		} 
		else
		{
			...
			if (flags["COC.MARBLE_DICK_TYPE"] > 0) output(", but you notice that she does not have a cock of any kind. It seems that trait isn't passed on");
			output(".  The little girl's face is a really pretty one; you're sure that she'll grow up to be like her mom.  You hand Marble the child and she puts the crying child to her chest. The little girl stops crying at once and starts eagerly gobbling down Marble's milk.\n\n");
			//If (PC is addicted to Marble)
			if (pc.findPerk(PerkLib.MarblesMilk) >= 0) {
				output("\"<i>Don't worry sweetie,</i>\" Marble tells you, \"<i>somehow I know that she won't get addicted.\"</i>  ");
			} 
			else {
				output("\"<i>Oh my,\"</i> Marble says to you, \"<i>It's just as wonderful as when you suckled me when my milk was addictive; I'd forgotten the feeling.\"</i>  ");
			}
			output("Marble looks at her other breast a moment before looking at you and saying \"<i>Still, I think I could use you on the other side.</i>\" You oblige her by suckling her other breast.\n\n");
			//since the PC must either be addicted, or have removed Marble's addictive trait in order to father a child with her, there is no need for a check for addiction here!
			output("Once the two of you have had your fill, Marble puts the child inside the nursery.  The little girl is already starting to look like she is a few years old, and is already trotting around on her little hoofs.  Marble turns to look at you and asks \"<i>Ok, I'll name her ");
			...
			if (flags["COC.MARBLE_KIDS"] == 0) {
				...
				output(", yes, that's a good name for her.</i>\"");

			}
			else {
				output("...hmm, well Sweetie, I can't think of a good name right now, I'll figure one out tomorrow.</i>\"");
			}
		}
		//cow-girl child is added to the nursery, her name is set to \"<i>childName</i>\"
		flags["COC.MARBLE_KIDS"]++;
		addNextButton(playerMenu);
	}
	//Eggs
...
public function marbleNightSleepFlavor():Boolean {
	...
	if (flags["COC.MARBLE_NURSERY_CONSTRUCTION"] == 0 && pc.pregnancyType == PregnancyStore.PREGNANCY_MARBLE && (pc.pregnancyIncubation <= 128)) {
		output("<b>Citing your pregnant belly, Marble informs you she'll be getting to work on building a nursery for your coming cow-child soon.</b>\n\n");
		flags["COC.MARBLE_NURSERY_CONSTRUCTION"]++;
	}
	//PREGGO ALERTZ
	switch (pregnancy.eventTriggered()) {
		case 2: output("<b>You notice that Marble seems to have gained some weight.</b>\n\n"); //3 days in
				break;
		case 3: output("<b>Marble's belly has gotten a fair bit bigger; she may be pregnant.</b>\n\n"); //8 days in
				break;
		case 4: output("<b>Marble's belly has gotten bigger; she is obviously pregnant."); //12 days in
				...
				if (flags["COC.MARBLE_NURSERY_CONSTRUCTION"] == 0 && pregnancy.type != PregnancyStore.PREGNANCY_OVIELIXIR_EGGS) {
					flags["COC.MARBLE_NURSERY_CONSTRUCTION"]++;
					output("  She lets you know that she'll be busy working on a nursery for her child for a while.");
				}
				output("</b>\n\n");
				break;
		case 5: output("<b>Marble's pregnancy has advanced further still, though the structure of her body keeps it from slowing her down.</b>\n\n"); //18 days in
				break;
		case 6: output("<b>Marble is probably getting close to giving birth, as her belly has gotten very large.</b>\n\n"); //24 days in
		default:
	}
	...
	if (pc.findStatusEffect("Infested") >= 0) {
		if (flags["COC.MARBLE_GROSSED_OUT_BECAUSE_WORM_INFESTATION"] != 1) {
			output("As you go to lie down for the night, you find that Marble has gotten there before you.  You lie down next to her, but you can't help wishing that the two of you could have sex.  Suddenly, you feel Marble's hand grab ahold of your " + pc.cockDescript(0) + ".  She gives a bit of a squeal as one of the worms starts to move around inside the shaft.  She pulls open the blankets and looks closely at your dick for a few moments before picking up her bedroll and moving to a different part of the camp saying, \"<i>" + pc.short + ", you get rid of those or I'm not going to sleep next to you or with you.</i>\"\n\n");
			flags["COC.MARBLE_GROSSED_OUT_BECAUSE_WORM_INFESTATION"] = 1;
		}
		return false;
	}
	...
	if (pc.gender == 0) {
		output("As you and Marble lie down for the night, she whispers into your ear, \"<i>Sweetie, if it isn't too much trouble, I'd really like it if you would grow a penis for me.  If you can't do that, then could you at least get a vagina?  I'll understand if it's too much trouble, I just wanted to get that off my chest...</i>\" After a moment of getting comfortable, the two of you go to sleep.\n\n");
		return false;
	}
	else if (flags["COC.MARBLE_LUST"] < 50 || pc.gender == 0) {
		output("As you go to lie down for the night, you find that Marble has gotten there before you.  ");
		if (flags["COC.MARBLE_LUST"] < 0) output("She makes a point of turning to face away from you when you climb into your bedroll.  Looks like she's still annoyed at you.");
		else if (flags["COC.MARBLE_LUST"] < 20) output("She lies next to you contentedly when you climb into your bedroll.");
		else if (flags["COC.MARBLE_LUST"] < 30) output("Marble gives a soft sigh when you climb into your bedroll.");
		else if (flags["COC.MARBLE_LUST"] < 40) output("Marble fidgets around a bit when you climb into your bedroll.");
		else output("Marble seems rather worked up when you climb into your bedroll.");
		if (pc.lust < 33 && flags["COC.MARBLE_LUST"] < 20) output("  After a few moments of getting comfortable, you drift off to sleep.");
		else if (pc.lust < 33) {
			output("  You wonder for a moment if Marble is maybe looking for something more from you.");
			//(50% chance of this happening)
			if (rand(2) == 0) {
				marbleCuddlin();
			}
			else output("  However, you're not aroused enough to consider sex.");
		}
		else {
			output("  Do you ask Marble if she'd like to have sex?");
			//[Ask][Don't Ask]
	...
	//Replaces laying down texts when Marble is at or over 50 lust
		...
		output("You lie down and notice that Marble hasn't gone to bed yet.  You decide not worry about it too much until you hear Marble come clopping up outside.  \"<i>Sweetie?</i>\" she calls out to you as she crawls in next to you.  \"<i>Sweetie, I'm feeling really horny ");
		//if (PC's lust is under minimum to request sex)
		if (pc.lust < 33) {
			output("and I know you really aren't in the mood, but could you please help me out?</i>\"  You sit up and ask her what it is.  She looks at you slightly embarrassed before asking you, \"<i>Could you please ");
			//If (Marble is a herm)
			if (flags["COC.MARBLE_DICK_TYPE"] > 0)
				output("suck my " + marbleCock() + "?  I really, really need it.</i>\"");
			else
				output("lick my womanhood?  I really want to feel your tongue bring me to release...</i>\"");
			...
			if (pc.isNaga() && flags["COC.MARBLE_DICK_LENGTH"] > 0) tailJob = giveMarbleTailjobRelease;
			simpleChoices("Yes", marbleNomNoms, "No", playerMenu, "Tailjob", tailJob, "", null, "", null);
		}
		else {
			output(".  I can tell you want this too, so can we please have sex?</i>\"  You sit up and consider her request.  Will you have sex with Marble?  If not, you could always offer to give her oral instead.");
...
private function pcPregWithMarblesKids():Boolean {
	return pc.pregnancyType == PregnancyStore.PREGNANCY_MARBLE && pc.pregnancyIncubation <= 280;
...
private function marbleCuddlin():void {
	output("  Before too long, Marble reaches over and ");
	...
	if (!pcPregWithMarblesKids() && !marblePregWithPCKids()) {
		...
		if (pc.tallness <= 78) output("pulls your head on top of her chest.  She idly strokes your " + pc.hairDescript() + " and says, \"<i>Sweetie, how about I be your pillow for the night?</i>\"  You happily agree, and soon you drift off to the sound of her steady breathing while you lay between her sizable breasts.");
		else output("wraps her arms around you, gently pulling you against her.  \"<i>Sweetie, do you mind if we stay like this for a while?</i>\"  There is no way you could refuse, and before long, the two of you drift off to sleep in each other's arms.");
	}
	//Preg-cuddles
	else {
		//if (only Marble is heavily pregnant)
		if (marblePregWithPCKids() && !pcPregWithMarblesKids())
			output("gently pulls your head to her swollen belly.  \"<i>Listen, sweetie,</i>\" she whispers, \"<i>you can hear our child's heartbeat when it's quiet like this...</i>\"  You hear a gentle thump accompanying Marble's own powerful heartbeat, along with your child's occasional squirms inside Marble.  You sigh and snuggle up against Marble's chest, rubbing her belly with your hand before drifting off to sleep.");
		//PC be preggers only
		else if (pcPregWithMarblesKids())
			output("gently pets your swollen belly.  \"<i>Sweetie,</i>\" she whispers, \"<i>I can feel our child.</i>\"  She puts her furry ears to your growing belly, gently rubbing it all the while.  After a few moments she snuggles up against your body and the two of you drift off to sleep.");
		else
			output("pulls your hand to her belly, and puts her own onto yours.  \"<i>The miracle of two lovers carrying each other's child,</i>\" she says.  It certainly isn't something you ever expected to happen, but that doesn't make it any less wonderful.  The two of you drift off to sleep together, hands on each other.");
	}
}
//Ask for sex
private function atNightAskMarbleForSomeSexMaybe():void {
	clearOutput();
	output("You turn to face Marble, and ");
	//if (Marble's lust is negative)
	if (flags["COC.MARBLE_LUST"] < 0) {
		output("see the annoyed back of her head.  Oh right, she's probably not in the mood for this.  You'll have to find another way of satisfying your needs tomorrow.");
	}
	else if (flags["COC.MARBLE_LUST"] < 20) {
		output("gently ask her if she wants to fool around.  After a moment you hear her breathe out and she says, \"<i>Sorry sweetie, I'm not feeling up to it right now.</i>\"");
		//Marble cuddles the PC
		marbleCuddlin();
	} 
	else {
		output("gently ask her if she wants to fool around.  After a moment you hear her breathe out and, with a lewd voice, she says, \"<i>I was worried you'd never ask, sweetie...</i>\"");
		//trigger Marble sex
...
private function marbleNightSexIntro(clear:Boolean = true):void {
	...
	//This scene is for non corrupted Marble, and low corruption pc.  Until I add more scenes, just use it for everything.
	//Try to find a dick that fits.
	var x:Number = pc.cockThatFits(marbleCuntCapacity());
	if (x < 0) x = 0;
	//No sex if dude and too big!
	else if (pc.gender == 1 && pc.cockVolume(x) > marbleCuntCapacity()) {
		output("You lie down next to her, but you can't help wishing that the two of you could have sex.  Suddenly, you feel Marble's hand grab a hold of your " + pc.cockDescript(x) + ".  She sighs and says, \"<i>Sweetie, I'm afraid that you won't fit inside me.  Do you think you could get a little smaller, just for me?</i>\"  After a moment of getting comfortable, the two of you go to sleep.");
		addNextButton(playerMenu);
	}
	//Ok lets light this fire!
	output("At that moment, you feel Marble breathe in your ear, \"<i>I know you want this, don't you sweetie?</i>\"\n\n");
	//If (PC is under 5 feet tall) {
	if (pc.tallness < 60) {
		output("Marble rolls you onto your back and pulls your head up onto her ample cleavage.  \"<i>I want it too. Just relax and let me take care of this.</i>\"  Marble keeps your head locked in her cleavage as she uses her other hand to slowly free the two of you of your garments.\n\n");
		output("Your head rests comfortably between Marble's breasts the entire time, her warm breath blowing softly on your hair.  You feel almost completely relaxed, yet incredibly turned on at the same time.  For some reason, it just feels right to lay there and breathe in her scent, enjoying the wonderful love pillows she is forcing you to relax between.  She runs her hand down your " + pc.biggestBreastSizeDescript() + ", gently cooing to you.  Her hand slips down to your ");
		//If (PC has dick) {
		if (pc.cockTotal() > 0) {
			output(pc.multiCockDescriptLight(), false);
		...
		//If (PC has vag) 
		if (pc.hasVagina()) output(pc.vaginaDescript(), false);
		output(" and gently caresses you.  She whispers into your ear, \"<i>Soon sweetie, soon we'll both get what we want.</i>\"\n\n");
		output("Finished with removing your clothes and her own, Marble releases her grip on your head and turns your body over.\n\n");
	} 
	else 
	{
		output("She reaches over and puts her hand onto your waist, gently slipping her hand inside your undergarments to touch your ");
		//If (PC has dick) {
		if (pc.cockTotal() > 0) {
			output(pc.multiCockDescriptLight(), false);
			//If (PC both dick and vag) 
		...
		output(" while you slip your own hand into her clothes and ");
		//If (Marble is a herm) {
		if (flags["COC.MARBLE_DICK_TYPE"] > 0) {
			output("gently rub her " + marbleCock() + ".  After a moment, you move your hand down lower and ");
		}
		output(" run your fingers down her moist womanhood.  The two of you quiver under each other's ministrations.  Both of you work to quickly free yourselves from your clothing, growing increasingly excited at where things are going.  Free of your garments, you move overtop of Marble.\n\n");
	}
	//If (player is a herm, and their dick can fit inside Marble)
	if (pc.isHerm()) {
		if (pc.cockVolume(x) <= marbleCuntCapacity()) {
			output("\"<i>So sweetie, what part will you be using today?</i>\" she asks you.");
			...
			output("Marble says, \"<i>Well sweetie, since that " + pc.cockDescript(x) + " of yours is too big to fit inside me, let's put that " + pc.vaginaDescript(0) + " to use.</i>\"\n\n");
			//GIRL SECKS GOES HERE
			addNextButton(marbleNightSexChicks);
		}
	}
	else {
		//Go to next scene based on the PC's and Marble's attributes
		//DUDEZ
		if (pc.gender == 1) addNextButton(marbleNightSexDudes);
		//CHICKZ
		else addNextButton(marbleNightSexChicks);
	}
}
...
private function marbleNightSexDudes():void {
	...
	flags["COC.MARBLE_LUST"] = 0;
	clearOutput();
	output("You gently lift yourself up and slide the tip of your " + pc.cockDescript(x) + " into her waiting hole.  Ever so slowly, you push yourself further and further in.  Each inch gained brings gentle moans and coos from Marble, encouraging you onward.  ");
	//Can the PC's main cock go all the way?
	//If (cock 0 is longer than 8 inches) 
	if (pc.cocks[x].cLength() > 8) {
		output("There is only one thing that will stop your advance: when you reach the end of the line.  \"<i>Keep going sweetie, there is room left.  Make sure you fill me all the way.</i>\"  With a groan, your " + pc.cockDescript(x) + " hits the furthest part of Marble's confines and you can go no further.\n\n");
	}
	//otherwise, Marble doesn't mind, she'll hold all of them instead
	else {
		output("Nothing will stop your advance; your cock will be fully enveloped inside Marble's body.  \"<i>That's right sweetie... Ah, let me fully envelop you.</i>\"  With a sigh of satisfaction, your bodies come completely together.\n\n");
	}
	output("You look Marble in the eye and the two of you relish the moment; you are two lovers, connected in the most enjoyable of ways.  ");
	//Is Marble going to take the initiative here?
	if (cor() + pc.lib < flags["COC.MARBLE_4"] + 10) {
		output("You get so caught up in the moment that Marble takes the initiative. \"<i>Now, now, it's not nice to keep me waiting, sweetie,</i>\" she says before lifting up your hips and repeatedly inserting you inside her at a fair pace.  You're caught off guard and fall forward, ");
		//Do you fall into the boobies?
		if (pc.tallness < 66) {
			output("straight back into her ample bosom.  Marble giggles at your predicament, but doesn't stop moving your hips.  Rather than do anything more about it, you decide to just let Marble have her fun. After all, it's not like you aren't enjoying the situation.  You can already feel yourself getting close to release, and from the sound of Marble's breathing, she is too.\n\n");
		}
		//nah, but if you have boobies, maybe they hit hers?
		else if (pc.tallness < 84) {
			output("your " + pc.biggestBreastSizeDescript() + " pressing against her own lovely pillows.  Marble giggles slightly before giving you a kiss on the lips.  She continues moving your hips the whole time.  Of course, as you start to near your peak, you get your control back and start thrusting into her of your own accord.  You can hear Marble beginning to breathe faster as she moves her hands onto your back.  She holds you tightly and gives you another kiss.\n\n");
		}
		//Fun, the PC's breasts fall onto Marble's Face!  If they have them, anyway...
		else {
			if (pc.biggestTitSize() >= 4) {
				output("landing your " + pc.biggestBreastSizeDescript() + " into Marble's face!  You hear a slight muffled cry underneath you as Marble takes her hands off your hips and lifts you up off of her.  ");
			} 
			else {
				output("crashing down on Marble and doing a good job of flattening her breasts under your size.  \"<i>Hey!</i>\" she cries out, letting go of your hips to lift your body off of her.  ");
			}
		}
		output("You look down at her annoyed expression, before grinning at her and beginning to impale her moist and wonderful snatch yourself.  In a moment, her expression changes from a pouty face to one of ecstasy.  The two of you are nearing your peaks now.\n\n");
	}
	//nope the PC is the one
	else 
	{
		output("It isn't enough to hold you back for more than a moment, though, and soon you're thrusting deep inside her at a comfortable rhythm for the two of you.  You smile at her and she smiles back as you start fondling and playing with Marble's lovely pillows.  She starts to run her fingers onto her love button, crying out in pleasure as you continue to thrust your " + pc.cockDescript(x) + " inside her.\n\n");
		//If (PC's breasts are B cups or bigger)
		if (pc.biggestTitSize() >= 2) output("You pull your hands back up and begin to run them over your " + pc.biggestBreastSizeDescript() + ", toying and teasing your nipples as your " + pc.cockDescript(x) + " continues to pound away at Marble's confines.  Marble soon has her hands on the breasts you vacated, clearly enjoying teasing herself just as much as you are.  The two of you cry out in pleasure from the stimulation; you can feel your orgasm fast approaching.\n\n");
		else output("The sensations are quickly becoming too much for you, and you're unable to do anything but focus on the feeling of your " + pc.cockDescript(x) + " inside her wonderful confines.  Marble grins at your difficulties and teases, \"<i>Am I just too much for you, sweetie?  Ah!</i>\" She gasps; it seems that you're becoming too much for her too.\n\n");
	}
	//ORGASMO
	output("You gasp as you feel your insides churning and the walls of Marble's sex collapsing tightly down on you.  Once again, you've brought each other to the point of blissful orgasm around the same time.  ");
	//let's talk about the PC's cum production
	if (pc.cumQ() < 250) {
		output("Marble cries out in joy as your " + pc.cockDescript(x) + " unleashes its load within her insides.  ");
	} 
	else {
		output("Marble gasps in both pleasure and pain as you fill up her insides to almost bursting, and large amounts of your jizz spill out around her slit.  ");
	}
	...
	if (pc.totalCocks() > 1) 
	{
		output("Of course, ");
		if (pc.totalCocks() > 2) { 
			output("the rest of your " + pc.multiCockDescriptLight(), false);
		} 
		else { 
			output("your other tool");
		}
		output(" unleashes its own load, liberally covering both of you.  ");
	}
	//does Marble spray her cock?
	if (flags["COC.MARBLE_DICK_TYPE"] > 0) {
		output("Trapped between your bodies, Marble's " + marbleCock() + " spasms and sprays the two of you.");
	}
...
private function marbleNightSexChicks():void {
	...
	clearOutput();
	if (flags["COC.MARBLE_DICK_TYPE"] == 0) {
		output("You gently lift yourself up as Marble produces her double dildo and slips it inside her womanhood.  Smiling at her, you slowly lower your body and impale your " + pc.vaginaDescript(0) + " onto the artificial rod.  ");
		//Loosen PC's vagina if they are really tight, also removes virginity
		pc.cuntChange(14,true);
		//(Message about vagina becoming looser)
		output("After adjusting to the feeling of the shaft inside you, you can't deny what a wonderful feeling it is to be connected like this.  ");
		//Does Marble take the initiative?
		if (cor() + pc.lib < flags["COC.MARBLE_4"] + 10) {
			output("Marble gives you a grin and starts lifting your hips up and down, while simultaneously bouncing her own hips.  The combination results in both your " + pc.vaginaDescript(0) + " and Marble's being pistoned by her double dildo.  The two of you start moaning in unison from the overwhelming feelings, leaving you unable to do anything but let her buckle and bounce you around like a rough wagon, and you love every second of it.\n\n");
			output("\"<i>Oh sweetie, am I going too fast for you?  Here, let me give you a chance to catch your breath.</i>\"  She offers as she slows down her rapid movement, letting your mind settle back into place.\n\n");
			//If (PC's breasts are D cup or bigger)
			if (pc.biggestTitSize() >= 4) {
				output("Marble takes her hands off your hips and lets you start to rise and fall on her in your own time.  She still rises up to meet you as you descend and lowers herself down each time you rise, matching your timing, but her hands have a much more pressing concern: your " + pc.biggestBreastSizeDescript() + ".  Marble starts working her hands expertly to tease and play with your " + pc.biggestBreastSizeDescript() + ". She seems to know all the right ways to move her fingers, clearly having a lot of experience with her own, and you can't help but moan at the attention.  She giggles and pants in pleasure herself; it seems she may be enjoying this play just as much as you!\n\n");
			} 
			else {
				output("Marble decides it would be a good time to start playing with your " + pc.biggestBreastSizeDescript() + " and starts teasing a " + pc.nippleDescript(0) + " with one of her hands as the other continues to gently lift you up and down.  You moan a little at her ministrations as she smiles at you and says, \"<i>You know, sweetie, maybe those cute little boobies of yours could stand to get a little bigger.  I'm sure I could do some great things to you then.</i>\" With a grin still on her face, she gasps and the two of you feel your climaxes approaching.\n\n");
			}
			...
			output("\"<i>Sweetie!</i>\" Marble gasps as you start to play with her breasts.  ");
			//Does Marble help out, or does she go for the PC's breasts?
			if (pc.biggestTitSize() >= 4) {
				output("She reaches her hands up and starts to play with your " + pc.biggestBreastSizeDescript() + ", flicking and toying with them at the same pace that you play with hers.  The two of you start a friendly little contest to see who can stimulate the other's chest better; all the while, you continue to rise and fall above her.\n\n");
			} 
			else {
				output("Marble reaches her hands up and grabs onto yours, pulling them down tightly against her chest.  She gets you to play with each of her " + marbleNips() + " in turn, making sure that you flick them and tease them in just the right way to get her to cry out in ecstasy again and again.\n\n");
			}
			output("Within moments, your rapid pistoning on the dildo is pushing the two of you closer and closer to the edge.  ");
		}
		output("The two of you cry out in orgasm as your " + pc.vaginaDescript(0) + " clamps down on one end of her dildo and her womanhood clamps down on the other end.  ");
		//Does this PC have a cock too?
		//If (PC has at least 1 cock)
		if (pc.hasCock) {
			output("At the same time, your " + pc.multiCockDescriptLight() + " sprays its own load onto Marble's chest and stomach.  ");
		}
		output("The two of you give one more shudder from the wonderful stimulation, before collapsing on top of one another.  ");
	}
	//PC is female, Marble is a herm
	else {
		pc.slimeFeed();
		output("You gently lift yourself up into the air and guide your " + pc.vaginaDescript(0) + " to the tip of her waiting " + marbleCock() + ".  You slowly lower your " + pc.vaginaDescript(0) + " onto her quivering " + marbleCock() + ", gaining inch by inch ");
		//is she satisfied?
		//If (player can fit all of her cock and PC is not a virgin) {
		if (pc.vaginalCapacity() > flags["COC.MARBLE_DICK_LENGTH"] * flags["COC.MARBLE_DICK_THICKNESS"]) {
			output("until she is fully within you.  The two of you shudder in pleasure, and Marble says, \"<i>Oh sweetie, you got it all in there.  Being inside another woman like this is something I'll never really get used to, but I can't deny how good it feels.</i>\"\n\n");
		}
		//Marble seems to think you can take more than maybe you should be able to take
		else {
			output("until you bottom out and can take no more of her.  \"<i>No more?</i>\" Marble asks you a little sadly, \"<i>You sure you can't take a little more?</i>\" she adds as she starts pushing you down onto her " + marbleCock() + ". You are forced to stretch to accommodate more of her and you shudder slightly from the strain.  ");
			//(Message about vagina becoming looser)
			output("Marble grins at you and says, \"<i>See?  I knew you that you could fit a little more.</i>\"\n\n");
		}
		output("After adjusting to the feeling of her inside you, you can't deny what a wonderful feeling it is to be connected like this.  ");
		//Does Marble take the initiative?
		if (cor() + pc.lib < flags["COC.MARBLE_4"] + 10) {
			output("Marble gives you a grin and starts lifting your hips up and down, while simultaneously bouncing her own hips.  The combination results in your " + pc.vaginaDescript(0) + " being pistoned by her " + marbleCock() + ".  The two of you start moaning in unison from the overwhelming feelings, leaving you unable to do anything but let her buckle and bounce you around like a rough wagon, and you love every second of it.\n\n");
			output("\"<i>Oh sweetie, am I going too fast for you?  Here, let me give you a chance to catch your breath.</i>\"  She offers as she slows down her rapid movement, letting your mind settle back into place.  ");
			if (pc.biggestTitSize() >= 4) {
				output("Marble takes her hands off your hips and lets you start to rise and fall on her in your own time.  She still rises up to meet you as you descend and lowers herself down each time you rise, matching your timing, but her hands have a much more pressing concern: your " + pc.biggestBreastSizeDescript() + ".  Marble starts working her hands expertly to tease and play with your " + pc.biggestBreastSizeDescript() + ". She seems to know all the right ways to move her fingers, clearly having a lot of experience with her own, and you can't help but moan at the attention.  She giggles and pants in pleasure herself; it seems she may be enjoying this play just as much as you!\n\n");
			} 
		...
		//no, no, the PC is much hornier than she is
			...
			output("Within moments, your rapid pistoning on her " + marbleCock() + " is pushing the two of you closer and closer to the edge.  ");
		}
		output("The two of you cry out in orgasm as your " + pc.vaginaDescript(0) + " clamps down on her " + marbleCock() + " as it fills you with its seed.  ");
		//Does this PC have a cock too?
		if (pc.hasCock) {
			output("At the same time, ");
			if (pc.totalCocks() > 1) output("each of ");
			output("your " + pc.multiCockDescriptLight() + " sprays its own load onto Marble's chest and stomach.  ");
		}
		output("The two of you give one more shudder from the wonderful stimulation, before collapsing on top of one another.  ");
		//Pregnancy chance for PC, ¼ their fertility
		pc.knockUp(PregnancyStore.PREGNANCY_MARBLE, PregnancyStore.INCUBATION_MARBLE, 150);
		pc.cuntChange(flags["COC.MARBLE_DICK_THICKNESS"] * flags["COC.MARBLE_DICK_LENGTH"], true);
	}
	marbleSexFinish();
}
...
private function marblePreggoChance(preggerMult:Number):void {
	...
	//Count cum quantity
	preggerOdds += pc.cumQ() / 100;
	//Capped at 20
	if (preggerOdds > 20) preggerOdds = 20;
	//Fertility+ perk bumps odds to 25.
	if (pc.findPerk(PerkLib.FertilityPlus) >= 0) preggerOdds += 5;
	if (pc.findPerk(PerkLib.FertilityMinus) >= 0 && pc.lib < 25) preggerOdds -= 5;
	//If has 'stud perk' almost always get her pregnant
	if (pc.findPerk(PerkLib.MaraesGiftStud) >= 0) preggerOdds += 25;
	preggerOdds *= preggerMult;
	//GET HER PREGNANT
	trace("MARBLE PREGGO ODDS: " + preggerOdds);
	
	if (rand(100) < preggerOdds && (pc.findPerk(PerkLib.MarblesMilk) >= 0 || flags["COC.MARBLE_PURIFICATION_STAGE"] >= 5)) {
...
public function MarbleDigsDraftsYo():void {
	spriteSelect(41);
	clearOutput();
	pc.consumeItem(consumables.P_DRAFT);
	//gives Marble a 7 inch long, 2 inch thick dick by default, no other effect.  Can only be given while she does not have a dick.
	output("You hand Marble the bottle.  She looks at it for a moment before ");
	if (flags["COC.MARBLE_4"] > 60) {
		output("giving you a smile and saying, \"<i>So you want me to partake in corruption, and to have a nice cock to stick my sweet with?</i>\"  You tell her that the bottle has been purified, so it won't give corruption, but otherwise, you're hoping it gives her the <i>additional</i> effect.  She grins at you and downs the bottle.\n\n");
	} 
	else {
		output("looking at you uncertainly and saying, \"<i>Uh, sweetie, I'm not going to drink this demon stuff.  It'll warp my body, and I think this will give me a cock...</i>\"  You assure her that the draft has been purified, so it won't warp her body or corrupt her... aside from that last effect she mentioned, but that's what you want her to get.  She sighs before yielding, and says, \"<i>Ok sweetie, if you really want me to have one, I'll take it for you.</i>\"  She takes a deep breath before drinking the bottle, and grimaces at the taste.\n\n");
	}
	output("Marble drops the potion and grabs at her crotch.  Turning away from you, she gives an excited cry that is a mix between a moo and a moan as her body convulses slightly.  After a moment, she turns back to you and shows her new 7\" long and 2\" wide human cock.  She gives you a ");
	if (flags["COC.MARBLE_4"] > 50) output("big ");
	else output("nervous ");
	output("smile.  <b>Marble is now a hermaphrodite.</b>  ");
	addNextButton(returnToCampUseOneHour);
	flags["COC.MARBLE_DICK_TYPE"] = 1;
	flags["COC.MARBLE_DICK_LENGTH"] = 7;
	flags["COC.MARBLE_DICK_THICKNESS"] = 2;
	
}

//Pink egg or large pink egg
public function MarblepinkEgg():void {
	pc.consumeItem(consumables.PINKEGG);
	MarblePEggEffects();
}
public function MarbleLPinkEgg():void {
	pc.consumeItem(consumables.L_PNKEG);
	MarblePEggEffects();
}
...
//removes her dick, no other effect.  Can only be given if Marble has a dick.
	output("You hand Marble the pink egg.  She looks at it thoughtfully for a moment before her eyes light up in recognition.  \"<i>This is one of those magic eggs made from those egg elixirs.  If I remember right, this one removes the male traits of those who eat them.  ");
	if (flags["COC.MARBLE_4"] > 50 ) {
		output("So my sweet, tired of my cock?  Well, what if I like it?  Do you really want me to get rid of it?</i>\"  You assure her that yes, you do want her to get rid of it.  She frowns at you in annoyance, but ultimately agrees to take the egg.  \"<i>Eventually you'll want it back, I'm sure of it,</i>\" she says just before eating the egg.\n\n");
	} 
	else {
		output("So does this mean you don't want me to have a cock anymore, sweetie?</i>\"  You assure her that yes, you don't want her to have it anymore.  She seems relieved by this and admits, \"<i>Thank you sweetie, I don't think I really liked having it,</i>\" before eating the egg.\n\n");
	}
	output("She lifts up her skirt and the two of you watch as her " + marbleCock() + " is absorbed back into her body.  <b>Marble is now purely female.</b>  ");
	flags["COC.MARBLE_DICK_TYPE"] = 0;
	flags["COC.MARBLE_DICK_LENGTH"] = 0;
	flags["COC.MARBLE_DICK_THICKNESS"] = 0;
	addNextButton(returnToCampUseOneHour);
}
...
public function marbleOvulatesLikeMadDawg():void {
	...
	if (flags["COC.MARBLE_NURSERY_CONSTRUCTION"] < 100 && pregnancy.type == PregnancyStore.PREGNANCY_PLAYER) {
		output("Marble refuses to take the elixir to speed up her pregnancy until after she has finished the nursery. ");
		addNextButton(returnToCampUseOneHour);
		return;
	}
	else if (pregnancy.isPregnant) {
		output("After taking the elixir, Marble's belly rumbles slightly.  You can both tell that her pregnancy has advanced.  ");
		...
		output("After taking the elixir, Marble tells you that her belly feels full.  It sounds like she'll be laying eggs soon.  ");
		pregnancy.knockUpForce(PregnancyStore.PREGNANCY_OVIELIXIR_EGGS, PregnancyStore.INCUBATION_OVIELIXIR_EGGS + 46);
	}
	pc.consumeItem(consumables.OVIELIX);
	addNextButton(returnToCampUseOneHour);
}
...
public function marbleCuntCapacity():Number {
	var size:Number = 36;
	if (flags["COC.MARBLE_BOVA_LEVEL"] >= 1) {
		size += 10;
	}
	if (flags["COC.MARBLE_BOVA_LEVEL"] >= 2) size += 20;
	if (flags["COC.MARBLE_KIDS"] > 0) size += 14;
	return size;
}

private function giveMurbleProBova():void {
	clearOutput();
	if (flags["COC.MARBLE_BOVA_LEVEL"] == 0) {
		output("You hand Marble the bottle of ProBova.  She considers it for a few moments before looking at you and saying, \"<i>Judging by the label, this is probably going to make me even more cowlike then I already am, sweetie.  I'm not really opposed to the idea, but it will be permanent.  Are you sure you want me to drink it?</i>\"");
		//player chooses yes/no
		doYesNo(giveMarbleTheProBovas4Sho, giveItem);
	}
	else {
		output("You hand Marble another bottle of the ProBova.  She studies your face seriously for a few moments before saying, \"<i>Now, you're absolutely sure that you want me to drink this?  Remember that whatever happens will likely be permanent.</i>\"");
		//player chooses yes/no
		doYesNo(giveMarbleTheProBovas4Sho, giveItem);
	}
}
...
public function giveMarbleTheProBovas4Sho():void {
	clearOutput();
	spriteSelect(41);
	pc.consumeItem(consumables.PROBOVA);
	
	if (flags["COC.MARBLE_BOVA_LEVEL"] == 0) {
		output("Marble nods and downs the contents.  Nothing happens for a moment... then she gasps and grabs at her chest, stumbling forward slightly.  The cow-girl straightens up and releases her grip, then pulls open her top to look at her breasts.  <b>Each is now decorated with sets of four nipples, like the teats of a cow.  She has also gained about 4 inches in height, judging against the backdrop of the camp.</b>  Marble takes a few minutes to test her new nipples, squeezing them gently and sighing as dribbles of milk decorate her areolae in fours instead of one, then looks at you and says, \"<i>This isn't really so bad.  Actually, it feels nice.  If you find another dose, I'm willing to drink it - just to see what happens, of course.</i>\"");
	...
	//end event
		...
		flags["COC.MARBLE_BOVA_LEVEL"] = 2;
	}
	addNextButton(returnToCampUseOneHour);
}

private function marbleNip():String {
	if (flags["COC.MARBLE_BOVA_LEVEL"] > 0) return "quad-nipple";
	else return "nipple";
}
...
private function marbleAppearance():void {
	clearOutput();
	spriteSelect(41);
	output(images.showImage("monster-marble"));
	//Gives Marble's appearance screen, some of these values change depending 
	//on her level of corruption.
	output("Marble is a ");
	if (flags["COC.MARBLE_BOVA_LEVEL"] == 0) output("6 foot 4 ");
	else if (flags["COC.MARBLE_BOVA_LEVEL"] == 1) output("6 foot 8 ");
	else output("7 foot 4 ");
	output("inch tall cow-girl, and while she isn't very fast, she has very strong muscles; though they are difficult to spot under her skin's layer of fat. <b>She is currently wearing farm-hand clothes and using a large two-handed hammer as a weapon.</b>  ");
	if (flags["COC.MARBLE_BOVA_LEVEL"] <= 1) output("She has a fairly normal soft human face, with light-colored skin.  ");
	else output("She has a soft face that is a mix of bovine and human features, with thick brown fur covering her body.  ");

	output("Her shoulder-length brown hair is parted by a pair of rounded cow-ears that stick out sideways from her head.  ");
	
	if (flags["COC.MARBLE_4"] <= 20) output("Two small horns grow from her forehead, similar in size and appearance to those on a young female bovine.  ");
	else if (flags["COC.MARBLE_4"] <= 50) output("Two medium sized horns grow from her forehead, similar in size and appearance to those on a female bovine.  ");
	else output("Two fairly large horns grow from her forehead, similar in appearance to those on a female bovine.  ");
	output("She has wide womanly thighs that draw the attention of those around her, and her large butt fills out her clothing nicely.  A long cow-tail with a puffy tip swishes back and forth between her legs, as if swatting at flies. A pretty bow has been tied to her tail.  Two legs grow down from her waist");
	
	if (flags["COC.MARBLE_BOVA_LEVEL"] <=1) output(", human until about half-way down her thigh.  The lower portion of her legs is covered in thick dark brown fur and ends in a pair of bestial hooves.\n\n",false);
	else output(" that are oddly jointed and end in a pair of bestial hooves.\n\n",false);

	if (flags["COC.MARBLE_4"] <=15) output("She has two pillowy breasts, each supporting a 0.4-inch lactating " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + " bra.\n\n");
	else if (flags["COC.MARBLE_4"]<=25) output("She has two large breasts, each supporting a 0.6-inch lactating " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + ".\n\n");
	else if (flags["COC.MARBLE_4"]<=35) output("She has two basketball-sized breasts, each supporting a 0.8-inch milk-seeping " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + " bra.\n\n");
	else output("She has two basketball-sized breasts, each supporting a 1-inch milk-seeping " + marbleNip() + ".  She could easily fill an " + marbleBreastSize() + " bra.\n\n",false);
	...
	switch (pregnancy.event) {
		case 2: output("It looks like she's put on some weight since coming to your camp.\n\n"); //3 to 8 days in
				break;
		case 3: output("Her belly has swollen a bit; she may be pregnant.\n\n"); //8 to 12 days in
				break;
		case 4: output("Her belly is obviously swollen; she is almost certainly pregnant.\n\n"); //12 to 18 days in
				break;
		case 5: output("Her belly is very swollen; she is very pregnant.\n\n"); //18 to 24 days in
				break;
		case 6: output("Her belly is extremely swollen and occasionally quivers when whatever she is pregnant with moves around.\n\n"); //24+ days in
		default:
	}
	
	if ( (flags["COC.MARBLE_PURIFICATION_STAGE"] < 5 && flags["COC.MARBLE_LUST"] >= 20) || (flags["COC.MARBLE_PURIFICATION_STAGE"] >= 5 && flags["COC.MARBLE_TIME_SINCE_NURSED_IN_HOURS"] >= 4) )
	{
		output( "<b>Marble is fidgeting around uncomfortably, perhaps she needs to be milked?</b>\n\n" );
	}
	
	if (flags["COC.MARBLE_DICK_TYPE"] > 0) output("She has grown a " + marbleCock() + " since you brought her to camp. It is " + num2Text(int(flags["COC.MARBLE_DICK_LENGTH"])) + " inches long and " + num2Text(int(flags["COC.MARBLE_DICK_THICKNESS"])) + " inches thick.\n\n");
	
	if (flags["COC.MARBLE_4"]<=50) output("She has a pussy, with a 0.5 inch clit.\n\n");
	else if (flags["COC.MARBLE_4"] <=75) output("She has a cunt, with a 0.6 inch clit.  You can see moisture gleaming from it.\n\n");
	else output("She has a fuck-hole, with a 0.7 inch clit.  Moisture gleams in her cunt, its lips slightly parted.\n\n");
	//since I assume Marble may become ass fucked in the future, Boolean Marble.analVirgin:
	if (pc.statusEffectv1("Marble"Specials) == 0) output("She has one virgin bum-hole, placed between her squeezable butt-cheeks where, you presume, it belongs.");
	else output("She has one bum-hole, placed between her squeezable butt-cheeks where, you presume, it belongs.");
	addNextButton(interactWithMarbleAtCamp);
}
...
private function giveMarbleTailjobRelease():void {
	clearOutput();
	output("You tell Marble that while you're not in a mood to suck her off today, you're not just going to leave her alone with it, either, and ask for her permission to do an exotic alternative.  Slowly, she nods, apparently not so sure about what you mean or whether she should agree.");
	...
	if (cor() > 70) {
		...
		if (pc.tongueType == TONGUE_SNAKE || pc.hasLongTongue()) output("  Your tongue runs along the length of the end of your tail, tasting both Marble's feminine secretions and her semen.  She gives you a smoldering gaze as you lick her juices up.  You grin at her.");
		//(Demon Tongue) 
		if (pc.hasLongTongue()) output("  You decide to put on a show for Marble, moving your tail as you drop out a large piece of your inhumanly long tongue, licking up her secretions sensuously while staring at her.  She quickly blushes under your gaze.");
...
{
	clearOutput();
	if ( flags["COC.MARBLE_MILKED_BEFORE"] < 1 )
	{
		output( "With an unusually bright look in her eye, Marble comes up to you and asks, \"<i>Hey sweetie, I was just about to go get milked when I realized that you've never actually been with me for it.  How would you feel about getting a tour of the setup that Whitney has for me?</i>\"  Then a mischievous look crosses her face, \"<i>Maybe we could even do something a little... intimate too?</i>\"" );
		
		output( "\n\nThere is no harm in checking out her stall, at least.  You agree to go with her." );
		
		output( "\n\nThe two of you hold hands and quickly warp over to Whitney's farm and, with a wave to the proprietor, Marble escorts you to the barn.  The smells of the place are as familiar as ever, but this particular trip has an unusual feel to it since you're going to visit a room you've never been in before." );
		...
		output( "\n\nYour amorous guide gives an animated rundown of the uses of her pieces of furniture, the chair is what she usually sits in while reading, the bar if she's got stiff legs, and the bench if she's really worn out.  Once finished, you do notice there is a book sitting on the table right now and ask Marble what she's reading.  She uncharacteristically bites her thumb a bit nervously and says in a low voice that it's the book she was reading while doing research when you were addicted to her." );
		
		output( "\n\nNow very curious, you ask her how it is, and how close she is to finishing it.  \"<i>It's a bit embarrassing sweetie, but I've actually barely started it.  Unlike my younger sister, I never started reading until I needed something to do while being milked, and I'm terrible at it.  It can take me months to get through just one book, even if I'm spending several hours reading every day...</i>\"" );
		
		output( "\n\nYou apologize for bringing it up and suggest that maybe you should get down to the milking?" );
		
		output( "\n\nShe perks up instantly and pulls down " );
		if ( flags["COC.MARBLE_BOVA_LEVEL"] < 1 ) output( "two" );
		else output( "eight" );
		output( " suction cups off of the milker machine and carefully attaches them to her nipples, then flips a switch, causing it to come to life.  In an instant, milk starts flowing out of her " + marbleNips() + " into the cups and then up the clear tubes into the machine.  She lets out a relaxed sigh, then turns her gaze back to you." );
		
		output( "\n\n\"<i>Well sweetie, is there something you've got in mind you'd like to do?</i>\"" );
		
		flags["COC.MARBLE_MILKED_BEFORE"] = 1;
	}
	else
	{
		output( "Sensing that Marble looks like she is going to get milked soon, you offer to escort her to her stall personally." );
		
		output( "\n\nAs usual, Marble is pleased at the prospect and agrees instantly, and the two of you head over to her small homely space in Whitney's operation." );
		
		output( "\n\nOnce Marble has herself settled in and hooked up, she turns to you and asks if you've got something in mind." );
	}
	
	if ( flags["COC.MARBLE_PURIFICATION_STAGE"] >= 5 ) flags["COC.MARBLE_TIME_SINCE_NURSED_IN_HOURS"] = 0;
	clearMenu();
	addButton( 0, "Cunnilingus", milkMarbleCunnilingling );
	if ( pc.cockThatFits(marbleCuntCapacity()) >= 0 && (pc.isBiped() || pc.isNaga() || pc.isGoo()) ) addButton( 1, "Fuck Her", milkMarbleFuckDatCowPussy );
	if ( pc.hasVagina() && flags["COC.MARBLE_DICK_TYPE"] > 0 && (pc.isBiped() || pc.isGoo()) ) addButton( 2, "Take Her", milkMarbleTakeHerDick );
...
private function milkMarbleCunnilingling():void
{
	...
	output( "You suggest that maybe she'd like it if you licked her vagina while being milked." );
	
	output( "\n\nA huge smile spreads across her face, \"<i>Oh sweetie, that sounds like it would be wonderful!  You're the best; you know that?</i>\"" );
	
	output( "\n\nShe takes a seat on the edge of the chair in the middle of the room and spreads her legs wide.  Her skirt slips up her " );
	if ( flags["COC.MARBLE_BOVA_LEVEL"] < 1 ) output( "semi-human" );
	else output( "inhuman" );
	output( " legs, giving you a clear look at her waiting womanhood.  With a coy sideways glance, she wiggles her fingers in invitation for you to go down on her." );
	
	output( "\n\nIn order to get the show on the road, you " );
	if ( pc.isBiped() ) output( "get down on your knees, and slowly crawl forward" );
	else output( "lower your body close to the ground, and slowly inch forward" );
	output( " like an animal who's found something they want.  When you get close to her, your lover runs a finger up the inside of her thigh as her tail sneaks its way up around her waist and drapes itself over your prize." );
	
	output( "\n\nArriving at a hard brown hoof, you wrap your arm about her coarsely furred leg and rest your head against the bestial appendage.  You stroke and run your fingers through her fur to a soft giggle above you while the machine continues to pump and whirl.  You tease her a bit more before raising yourself up, bit by bit, until you're at eye level with her tail-covered opening. A small tuft of hair decorated with a pink bow is all that hides your goal.  No, it's a present, gift wrapped for you." );
	
	output( "\n\nYou take hold of two strands of the bow and tell Marble she shouldn't have given you such a gift." );
	
	output( "\n\n\"<i>Oh, not at all sweetie.  You deserve it!</i>\"" );
	
	output( "\n\nYou slip the bow open, unwrapping your present, and pull her tail aside to reveal her moist and inviting opening and exclaim that it was what you always wanted!" );
	
	output( "\n\n\"<i>Oh, I'm so glad you like it.  Why don't you try it out right now?</i>\"" );
	
	output( "\n\nShe doesn't have to give you the suggestion twice.  You dive in at once, sticking your [tongue] deep into her recesses, and tasting her cow-girl nectar.  Her folds accept you easily, and an eager moan slips out of the woman above you.  Her fingers run over your [pc.hair] while you withdraw from your initial sortie to scout out the entrance more." );
	
	output( "\n\nIn response to your tugging and toying with her outer folds, Marble's hips jerk forward slightly, and you see her love button pulse and engorge itself.  There is a happy moan, and her tail manages to find itself flicking your face.  You swat the pesky wrappings away and continue your efforts at bringing your mate to the height of pleasure.  The next target: her clit." );
	
	output( "\n\nThere is a long drawn out cry of joy when you bring your [tongue] to the source of her womanly pleasure.  " );
	if ( flags["COC.MARBLE_DICK_TYPE"] > 0 ) output( "With the first flick across its pink flesh, Marble grabs ahold of her "+ marbleCock() +", and tightly squeezes it, slowly pushing her hand down it." );
	else output( "With the first flick across its pink flesh, Marble grabs onto an arm of her chair, gripping it tightly." );
	output( "  You can't help but laugh and ask her if your treatment is too much, suggesting that maybe you should save playing more for later." );
	
	output( "\n\n\"<i>No more teasing me, [pc.name], bite me now!</i>\"" );
	
	output( "\n\nHow can you refuse such a request?" );
	
	output( "\n\n\"<i>Ahhhhooooo!</i>\"  The wonderful bovine women gives a grateful cry of pleasure as your teeth nip into her nub of engorged skin and a burst of liquid sprays out onto your chin.  " );
	if ( flags["COC.MARBLE_DICK_TYPE"] > 0 ) outputText ("Of course, there is also a spurt of white which sprays out over her lap and your head.  " );
	output( "Obviously, while the milkers don't normally get her aroused, they can seriously amplify the effects of any pleasure she gets." );
	...
	output( "\n\n\"<i>Oh, I'll be looking forward to it.  See you later sweetie!</i>\"  She gives you a slightly awkward peck on the cheek around the milking tubes, that sends you into a blissful state." );
	
	flags["COC.MARBLE_LUST"] = 10;
	var pLust:int = int( 20 + ((pc.lib + cor()) / 10) );
	dynStats( "lus", pLust );
	dynStats( "sen", 1 );
	addNextButton(camp.returnToCampUseTwoHours);
}


private function milkMarbleFuckDatCowPussy():void
{
	var x:int = pc.cockThatFits( marbleCuntCapacity() );
	clearOutput();
	
	output( "You wonder aloud if maybe she'd be interested in a little intercourse action. You'd be perfectly prepared to provide for her." );
	
	output( "\n\n\"<i>Hmm, well, your offer is tempting, but I'm going to have to see what's available before I make a final decision.</i>\"" );
	
	output( "\n\nWith a flourish, you remove your [pc.gear] and do a little sweep over your " + pc.cockDescript(x) + ", asking her if everything is to her needs." );
	
	if ((pc.findPerk(PerkLib.BulgeArmor) >= 0 || pc.modArmorName == "backless female teacher's clothes" || pc.modArmorName == "bridle bit and saddle set" || pc.modArmorName == "headdress, necklaces, and many body-chains" || pc.modArmorName == "bondage patient clothes" || pc.modArmorName == "crotch-revealing clothes" || pc.modArmorName == "cute servant's clothes" || pc.modArmorName == "maid's clothes" || pc.modArmorName == "servant's clothes") && pc.hasCock())
	{
		output( "\n\n\"<i>I appreciate the little show, but I could already see you just fine.  Come closer so I can get a better idea.</i>\"" );
	}
	else output( "\n\n\"<i>Hmm, yes, I think that will work.  Come closer so I can make a more thorough inspection.</i>\"" );

	output( "\n\nYou step forward and allow your lover to get \"acquainted\" with your goods." );
	
	output( "\n\nShe squats down and makes a show of looking you over carefully, humming to herself along the way.  Her breath tickling your skin slightly and quickly bringing you to full hardness.  With one finger, she taps your tip a few times, then runs it along your glans.  Finally she finishes her examination of your manhood by gripping it in her hand and giving it a few experimental pumps." );
	
	output( "\n\nShe stands back up and looks at you with a serious expression on her face.  \"<i>Your penis has met my expectations, and I do believe that I will be taking it into my vagina.  Please lay down on the bench over there and we can begi-hahahahaha!</i>\"  Her face finally cracks and she bursts out laughing over the little game the two of you have been playing." );
	
	if ( pc.tallness < 68 )
	{
		output( "\n\n\"<i>Oh sweetie.</i>\"  She pulls you into a deep hug against her chest.  \"<i>It's too much!  Hah, hahahahaha!  No more joking around.  I need you inside me now.</i>\"  She carefully carries you over to the bench and lays you down on your back, then straddles the bench above your " + pc.cockDescript(x) + ".  \"<i>Are you ready for me?</i>\" she calls out in a deep husky voice, breasts heaving, tail swishing, and all the while the milker machine continues to hum above her." );
	}
	else
	{
		output( "\n\nYou pull her into a deep hug as she continues to her hysterics and gently caress her hair.  You assure her that if the games are too much, you can skip straight to the action." );
		
		output( "\n\nAfter a moment she settles down and returns the embrace.  \"<i>Hmmhmm, yes.  No more joking around.  I need you inside me now.</i>\"" );
		
		output( "\n\nYou break away from her and casually walk over to the bench and lay down on top of it.  Marble follows immediately behind you, and before you've even finished laying down, she's already standing above with a hungry look on her face, breasts heaving, and tail swishing." );
		
		output( "\n\n\"<i>Are you ready for me?</i>\" she asks in a deep husky voice." );
	}
	
	output( "\n\nWith a grin, you tell her you're always ready for her." );
	
	output( "\n\nThe words have barely left your mouth before Marble drops her waist and engulfs your " + pc.cockDescript(x) + ", swallowing it up to the base in a single motion.  \"<i>Ohhhhhh, yes!</i>\"  Marble happily rubs her belly with one hand, while another cubs her breast.  She shakes her hips from side to side a couple times, savoring the feeling of your cock rubbing back and forth across her insides." );
	
	output( "\n\nThen both hands come down and push down on your stomach, and she begins a rapid forward and backward rocking motion on your member, keeping every bit of your length deep inside her, putting it into a spinning trip around her passage while she starts giving low happy moans of pleasure.  You look up at your lover and get a good view of the expression of bliss upon her face, but also get a chance to see the milk draining from her chest." );
	
	output( "\n\nWith each push and pull on your member, there is a rapid spurt of milk that the machine is only just able to keep up with.  After a moment, you realize that it isn't actually able to keep up, her milk is coming out too fast for the milker!  You try to warn your bovine lover, but it's too late.  With a pop, the suction cups fly off of Marble's breasts, spraying milk everywhere, and there is a bang from the machine as it shuts down." );
	
	output( "\n\n\"<i>Oh no!  Damn it, now what?</i>\" Marble stops moving and looks around worried." );
	
	if ( pc.findPerk(PerkLib.MarblesMilk) >= 0 )
	{
		output( "\n\nYou quickly sit up and take it upon yourself to continue the efforts of the milking machine, bringing with your actions a surprised gasp, followed by a contented sigh, and Marble resuming the movements of her hips.  The ambrosia from her breasts increases in quantity once more, and it actually feels like it's being pumped into your mouth.  You're happy to rise to the challenge of drinking it all while Marble continues to ride your body." );
		...
		output( "\n\nAlmost an hour later, the flow from both of Marble's breasts has slowed to a trickle, and both of you have cum at least three times in a haze of both sexual and chemical pleasure.  Marble gives a contented sigh and informs you that she thinks she'll be taking a nap now, wondering whether or not she should make a habit of breaking the milker, given what happens afterwards." );
		pc.orgasm();
		pc.orgasm();
		pc.orgasm();
		dynStats( "lus", 20 );
		flags["COC.MARBLE_LUST"] = 15;
		dynStats( "lib", .3 );
		if ( cor() < flags["COC.MARBLE_4"] ) dynStats( "cor", 1 );
		dynStats( "tou", .1 );
		dynStats( "str", .1 );
		dynStats( "sen", -2 );
		marblePreggoChance(1);
	}
	else if ( flags["COC.MARBLE_4"] > 30 )
	{
		output( "\n\nThe haze given off by Marble's tainted fluids alights a familiar craving at the back of your mind, but you push it back and reach your hands up to her breasts and pinch her nipples.  Marble tenses for a moment, and looks down at you confused.  You tell her that you'll try to keep her entertained while you wait for Whitney to fix things." );
		...
		output( "\n\nAfter rubbing against you for a bit, the stimulation on your length just isn't enough for you anymore, and try to start thrusting up into your lover's passage.  Marble seems to be in agreement in what she wants, and her motions switch from a gentle rocking into meeting each of your thrusts with a pumping of her own, trying to drain your manhood while you try to drain her breasts." );
		
		output( "\n\nFinally the two of you cry out together in orgasmic pleasure, and you spray out your seed into her passage, mixing with a rush of her own fluids into a slurry of sex juices sloshing around your " + pc.cockDescript(x) + ".  The two of you settle down, with Marble laying down on you contentedly for a moment." );
		
		output( "\n\nSuddenly a voice with a southern drawl call out from above you, \"<i>You kids havin a lots of fun down there?</i>\"" );
		
		output( "\n\nThe two of you stare at each other in surprise and tense up." );
		
		output( "\n\n\"<i>Next time, please try to be a bit gentler on the equipment, if ya don't mind.  I'd rather not have to do fix it every time you two get frisky.</i>\"  There is a slam of metal on metal as Whitney finishes working on the broken milker and it whirls back to life." );
		
		output( "\n\nYou and Marble each look each other in the eye and burst out laughing.  You then help her reattach the cups and excuse yourself.  Just as you're leaving, Marble blows you a kiss and says, \"<i>We should definitely do this again sometime.  Let's just give Whitney a break first.</i>\"" );
		
		pc.orgasm();
		flags["COC.MARBLE_LUST"] = 5;
		dynStats( "sen", -2 );
	}
	else
	{
		output( "\n\nYou assure Marble that it will be fine and grab a bucket that was sitting under the bench and offer to continue her milking by hand.  She looks like she is going to object for a moment before nodding.  You help her turn herself around while keeping your manhood firmly embedded within her.  She puts the bucket down at the end of the bench, and you reach your hands around her to grip her massive mammaries." );
		
		output( "\n\nYou roll the soft pliable flesh around in your hands for a few moments, garnering appreciative moans from their owner.  Then you start pinching and pulling on her " + marbleNips() + ", sending a jolt through her body that makes her tail shoot straight up.  Almost instantly after that, she resumes rubbing herself against your manhood, sending it sweeping through her insides once more while her delightfully shaped rear-end slides around against your waist." );
		
		output( "\n\nYou give a gasp of pleasure of your own and try your best to properly milk her nipples in the same way that cows back home were milked, in spite of how roughly your milkee is fucking right now.  If anything, being milked has made her more aggressive, and her tail is swishing around madly." );
		
		output( "\n\nIt takes a few minutes of effort, but when you find yourself finally closing with the peak of an orgasm, there is a spray followed by a ping. You finally manage to get some milk flowing from Marble's breasts, just in time for her to scream in pleasure and drive herself back into you more forcefully than before.  Her tail shoots straight up again, twitches twice, and you're thrown over the edge as well." );
		
		output( "\n\nThe two of you cum together at least three times over the next hour or so, and while your hand milking certainly isn't as efficient as Whitney's machine, it manages well enough in satisfying your bovine lover.  When you go to tell the farmer what happened to her device, she is rather surprised at just how nonchalant and pleasant the two of you were about it.  As she put it, \"<i>Most folks generally feel bad when they go round breaking other people's things.</i>\"  Though, you're certain she knew what you'd been up to." );
...
private function milkMarbleTakeHerDick():void
{
	...
	output( "You ask her if maybe she wants to penetrate you while she is using that milking machine." );
	
	output( "\n\nShe looks thoughtful for a moment, tapping her finger against her lips while her tail swishes back and forth.  With the tubes coming off of her nipples, it actually looks rather comical." );
	
	output( "\n\nSpotting your look, she frowns at you, \"<i>Oh, you think this is silly do you, [pc.name]?  I'll have you know that I'm seriously trying to figure out how to make this work.</i>\"  She indicates her rather physical attachment to the room, and the furnishings it has available to it." );
	
	output( "\n\nYou give it a little thought, then suggest that you could lean up against that railing while she fucks you from behind." );
	
	output( "\n\n\"<i>Take you from behind, huh?  I'd really rather look into your eyes while I'm having sex, but there aren't many options here, are there?</i>\"" );
	
	output( "\n\nTo silence her complaining, you quickly strip" );
	if ( pc.tallness < 56 ) output( ", grab a box," );
	output( " and lean against the railing, then shake your [pc.ass] in her direction with your vagina on full display, wet with arousal.  You look back at her to see if you're having the desired effect." );
	
	output( "\n\nMarble is licking her lips with a nervous look on her face while holding her tail in her hand and running her fingers through it's brush.  \"<i>You make a very persuasive argument, sweetie.  Plus, if you're looking at me like that, I guess I really don't have anything to complain about, do I?</i>\"" );
	
	output( "\n\nShe strips off her short skirt and takes up a position behind you, hands on your hips and erect cock pressed against your entrance.  You look down between your legs and catch sight of her tail hanging down between her legs, the barest hint of movement to it.  \"<i>Well sweetie, are you ready for me?</i>\"" );
	
	output( "\n\nYou retort that if you weren't, you wouldn't have suggested this in the first place." );
	
	output( "\n\n\"<i>Heh, good point.  Alright, here I go...</i>\"  She slowly works her way inside you, inch by inch.  " );
	if ( pc.vaginalCapacity() < flags["COC.MARBLE_DICK_LENGTH"] * flags["COC.MARBLE_DICK_THICKNESS"] ) output( "With each new bit of length inserted, she waits a few moments for you to adjust to her size, letting you fill out to accommodate her.  " );
	output( "With the final bit of her inserted into you, her tail suddenly shoots straight up out of your sight, and she lets out a long exaggerated sigh of pleasure as the milking machine continues to whirl above her." );
	pc.cuntChange( 8, true, true, false );
	
	output( "\n\n\"<i>Oh sweetie, you feel so wonderful.  I actually feel a bit strange right now, like I want to....</i>\"  She starts thrusting into your entrance with great gusto, eliciting a surprised gasp from you; Marble isn't normally this energetic when it comes to sex!  Being milked is probably making her more aggressive than usual, though given how good she is making you feel, you can't really complain." );
	
	output( "\n\nShe starts playing with your backside with her hand, massaging your " );
	if ( pc.buttRating <= 4 && !pc.isGoo() ) output( "taut" );
	else output( "jiggling" );
	output( " flesh while her " + num2Text(Math.round(flags["COC.MARBLE_DICK_LENGTH"])) + " inch length continues to push and pull into your womanhood.  Egged on by her enthusiasm, you start putting in some effort into the sex yourself by pushing your backside tight against her with each push forward and shaking it back and forth with each pull out of your interior." );
	
	output( "\n\nThe stimulation sends Marble wild, and in short order she is flooding you with her hermaphroditic seed.  However, she doesn't slow down at all and simply continues to lovingly abuse your backside and mound of pleasure.  You look down between your legs once more to see her tail has started swinging lengthwise with her body, every last inch of her body and mind are fixed on breeding with you, triggering a triumphant laugh from you as you cum yourself." );
	
	output( "\n\nNearly an hour later, Marble finally calms down and looks down at what she has managed to do to you.  \"<i>Oh my!  Sweetie, I'm sorry, I'm not sure what came over me...</i>\" she freezes when she sees your happy expression, and you inform her that you rather liked seeing that side of her. The two of you will have to do this again some time." );
...
private function milkMarbleOnTheBar():void
{
	...
	output( "You suggest that you'd be interested in playing with her while she's draped over the bar on the one side of the room." );
	
	output( "\n\nShe narrows her eyes at you, considering the proposition.  \"<i>I don't know sweetie, doesn't really sound like something I'd like that much.  I'd like to be in a position where I can do more than just take it...</i>\"" );
	
	output( "\n\nYou assure her that you'll make it good enough that it will be worth while to let you have complete control." );
	
	output( "\n\nAfter a few more moments of thought, Marble gives you a very serious look, \"<i>You don't get complete control, I'll be giving the instructions here, oh, and you'd better do a good job.</i>\"  Suddenly she smiles, \"<i>Otherwise, I'll just have to turn things around!</i>\"" );
	
	output( "\n\nYou wonder if maybe you want her to turn things around while she rests her hands on the bar, then drapes herself over it with her breasts swinging underneath.  She turns her head towards you and swings her ribbon decorated tail back and forth a few times.  You'd best not keep her waiting when she actually took you up on your request, so you strip down and step up behind her." );
	
	output( "\n\n\"<i>Alright sweetie, time to get me ready.  Let's see how you do.</i>\"  She turns her head back, and simply waits for you to start while the milking machine continues to hum above her." );
	
	output( "\n\nYou start by lifting up her skirt to expose her round ass to the air and the lips of her womanhood below it.  Of course, there is also the base of her tail just above it which continues to swish back and forth somewhat indignantly overtop of her openings.  Next, you take a handful of her generous asscheeks, rolling them around in your hands and kneading the soft bouncy flesh.  Though not as expansive as her bust by any means, Marble undeniably has quite the pleasant backside." );
	
	output( "\n\n\"<i>Hmm,</i>\" comes a pleased moan from the cowgirl in front of you, and her tail lifts up ever so slightly as the first signs of moisture and arousal form around her womanhood." );
	
	output( "\n\nNext, you run a finger across her entrance, teasing her ever so slightly.  A small jolt flies through her body, and her tail shoots straight up." );
	
	output( "\n\n\"<i>Ah, that's a good start sweetie, now, put a finger inside me.</i>\"" );
	
	output( "\n\nThe words are barely out of her mouth, when you push your index finger into her lower lips.  There is a low squelch while it slips inside, which starts again when you stir your finger around in a few circles.  This causes Marble's tail to curl around your hand and try to pull your finger out.  Once a little ways out, her tail tries to force your hand back into her waiting snatch." );
	
	output( "\n\n\"<i>Maybe you need my help after all.  Want to let my tail do the talking sweetie?  Let me show you how I like being penetrated.  Then again, your tongue might be nice to feel next too...\"</i>" );
	
	if ( pc.cockThatFits(marbleCuntCapacity()) > 0 ) output( "\n\nYou idly consider grabbing and pulling on her tail, so you could have a chance to take charge, though Marble would probably try and take control in that case for sure..." );
	
	clearMenu();
	addButton( 0, "Follow Tail", milkMarbleBarFollowTail );
	addButton( 1, "Cunnilingus", milkMarbleBarCunnilingling );
	if ( pc.cockThatFits(marbleCuntCapacity()) >= 0 ) addButton( 2, "Pull Tail", milkMarbleBarPullTail );
...
{
	var x:int = pc.cockThatFits( marbleCuntCapacity() );
	var y:int = pc.cockThatFits2( marbleCuntCapacity() );
	clearOutput();
	
	output( "You ask Marble where her tail will lead you." );
	
	output( "\n\nShe gives a low chuckle, \"<i>Why don't you wait and find out?</i>\"" );
	
	if ( x >= 0 )
	{
		output( "\n\nHer ribbon adorned, bovine appendage slowly unwraps from your wrist and instead searches around your crotch for something else.  Eventually the tuff comes into contact with your " + pc.cockDescript(x) + ", and runs its fur across your length lovingly for a few times, then curls around your base and pulls you in towards Marble's waiting womanhood." );
		
		output( "\n\n\"<i>Mhehehe, found it!</i>\"" );
		
		output( "\n\nIt was obvious that you couldn't hide from her for long." );
		
		output( "\n\n\"<i>Oh I know sweetie, but it's always so much fun looking.</i>\"" );
		
		output( "\n\nSliding into her interior is a slow but enjoyable process.  Each inch you pass in makes you want to just slam yourself home in an instant, but you wait to let Marble's tail guide the way.  Finally you're fully inside her, and she lets out a long soft moan of pleasure, followed by shaking her hips side to side a few times." );
		if ( y >= 0 ) output( "  You find your " + pc.cockDescript(y) + " slides up comfortably in between her cheeks, and squeeze them together a bit to give it a nice tight valley of soft flesh to run through." );
		
		output( "\n\nYour guide string tightens up a bit in time with the end of your lover's cry of pleasure.  \"<i>Hmmm, mine, all mine.</i>\" You barely catch Marble whisper under her breath, then her tail relaxes a bit and pushes back against your waist.  Understanding the direction, you pull your cock back out of its \"owner's\" home.  Though you haven't gone far before she tightens on you once more, and you realize that you were suppose to switch directions again. You focus your attention once more on the fluff that's showing you the way." );
		
		output( "\n\nEverything centers onto three things: the feel of aroused cunt surrounding your shaft, rippling and squelching with each slow plunge and retreat from its depths, the soft springy ass and hip curves under your hands that runs oh so pleasantly under your hands, and of course, Marble's tail curled around the base of your " + pc.cockDescript(x) + ".  The sensation of her thin appendage, soaked and sweat yet still soft with fur, constantly quivering and twitching in time with the movements of your two bodies, draws your entire focus." );
	}
	else
	{
		...
		output( "\n\nYou ask Marble if there is any chance you've found what she wanted?" );
		
		output( "\n\n\"<i>Why yes sweetie.  What gave it away?</i>\"" );
		
		output( "\n\nIt was just a hunch." );
		
		output( "\n\nYou adjust your grip on her personal toy and press one end against her waiting womanhood.  Just as you're about to ask her if she's ready for it, her tail suddenly jerks forward, sliding the tool inside her.  She gasps while the false cock quickly slides up to its centerpiece into her depths, then giggles when you try to pull it back out. Unfortunately, you find she's clapped down on it hard." );
		
		output( "\n\n\"<i>What's wrong sweetie?  You haven't managed to get stuck have you?  Here, let me give you a hand....</i>\"" );
		
		output( "\n\nAbruptly, her tail reverses it's pressure and allows you to extract her toy from its sheath - at least a little ways.  Then, you flip directions yourself and push it back in to the hilt.  There is a sharp intake of breath from your lovely lover, and you give her rear a small smack.  In response she tightens around your wrist, and it sounds like she might be about to get mad at you.  Instead she chuckles a bit." );
		
		output( "\n\n\"<i>Alright, no more teasing.  You're more than just a plaything after all.  I've got to let you do something.</i>\"" );
		
		output( "\n\nFinally, she allows you to freely build her pleasure and tease her body.  You can focus on bringing her to blissful sexual release by playing with her wonderful plump rump, twisting and pumping her with her dildo to the sound of lewd squelching that can just be heard over the hum of the milker and Marble's moans of pleasure, and lastly, the quivering of her tail.  The tail that serves as a subtle guide to the ways of your lover's tunnel." );
	}
	
	output( "\n\nYou're so engaged into the activity, that you barely register a long low call of ecstasy come out of cow-girl mate.  That same cry that is something between a moo and a long sigh that you've come to know very well.  At the same time, she manages to knock the hoses off her breasts, and there is a splash as her milk hits the floor of the stall, which steadily slows to a trickle, then only a few drops." );
	
	if ( x > 0 )
	{
		output( "\n\nFinally, the grip on your root relaxes, and you're finally able to explode inside her. Her tail slowly slips off of you and falls limp next to the connection between your bodies.  Several gasps and groans come out of your lips, and you ask Marble if she was satisfied." );
	}
	else
	{
		output( "\n\nThe hold on your hand tightens to a death grip, and you have no choice but to let go of her dildo and desperately struggle to free your constricted limb from her tail.  When you finally do manage to free yourself, you start rubbing your tingling fingers and ask Marble if putting your hand to sleep was a sign that she was satisfied." );
	}
	
	output( "\n\nYour lover girl straightens up and turns around to face you.  She fixes you with a stern look for a few moments before pulling you into a deep french kiss.  \"<i>Well, if you're always that good at following directions, I'm not sure how I wouldn't be.</i>\"" );
	...
	{
		var pLust:int = int( 10 + pc.lib / 10 );
		dynStats( "lus", pLust );
	}
	addNextButton(camp.returnToCampUseTwoHours);
}
...
private function milkMarbleBarCunnilingling():void
{
	...
	output( "You ask Marble to release your wrist so you can get into a better position for taking care of her.  You're feeling an ache to harvest her other womanly fluid." );
	
	output( "\n\n\"<i>Of course sweetie, I'm feeling very wet down there.  I definitely think I'm going to need you to take care of that.</i>\"" );
	
	output( "\n\nShe released you, and allows you to kneel down behind her and get a good whiff of her rich womanly scent.  It throws your mind into a haze for a few moments, and you're brought back out with a swish of wind as her tail swings side to side over her entrance, the base lifted up eagerly.  You rest your head to the side of her entrance, and lean against the part of her body where ass meets thigh while letting one hand play across the other leg." );
	
	output( "\n\nAfter a moment's climb, you find your fingers curling around the root of her bottom's bestial extension.  An idea strikes you, and you comment on how you need to brush her off before you start harvesting." );
	
	output( "\n\n\"<i>Oh?  What do you mean by that?</i>\" comes a somewhat bemused response." );
	
	output( "\n\nYou don't reply, instead you make a small ring with your thumb and finger and slide her tail through the hole as it flicks and swings around.  When you reach the thick hair at the end, you tighten your grip and direct the brush to the entrance of her womanhood." );
	...
	output( "\n\nYou nuzzle in against her soft bum once more, and bat at her tail playfully for a few moments." );
	
	output( "\n\n\"<i>Hmm, I'd say that you've made standing like this worth the while, [pc.name].  I think I might be willing to go like this again, if you're able to keep up this level of performance.</i>\"" );
	
	output( "\n\nYou laugh and assure Marble that when it comes to her, you'll always be giving it your all.  Just before you leave, an impish thought passes through your head for a moment, and you slip her tail's brush to Marble's entrance and push it inside a short ways.  Marble starts from the unexpected sensation, and you explain that you're only cleaning up some stray drops of fluid that might still be hiding away." );
	
	output( "\n\nAfter a moment, her tail flicks back out of her passage and she shakes her head at you.  \"<i>Thank you for the thought, sweetie, but please don't get me worked up again now, unless you're going to take care of me again.</i>\"" );
	
	output( "\n\nAlright, you'll end the teasing, for now." );
...
private function milkMarbleBarPullTail():void
{
	...
	output( "Probably against your better judgment, you decide to take charge and grab onto Marble's tail, roughly untwisting it off of you and yanking it upwards." );
	
	output( "\n\n\"<i>Ow!  What the hell sweetie?!</i>\"" );
	
	output( "\n\nYou ignore her protests and smack her ass once, then roughly penetrate her with your " + pc.cockDescript(x) + " while yelling at her to take it." );
	
	output( "\n\nThere is a loud clop as Marble stomps her hoof on the ground angrily.  With a huff, she lifts herself up on the bar and kicks back with both her legs, sending you flying onto your back and knocking the wind from your lungs.  You gasp for breath a few times, then look up to see Marble looming over you with a wicked smile on her face." );
	
	output( "\n\n\"<i>I warned you that I'd be taking charge if you couldn't behave yourself, didn't I [pc.name]?</i>\"  She sits down on your stomach, and presses her hand against your [pc.chest], squeezing you somewhat painfully.  \"<i>Now you're going to be punished!</i>\"  Given her expression, you get the impression that maybe she actually wanted you to misbehave!" );
	
	output( "\n\nYour aggressive mate moves forward and straddles your face, shaking her snatch back and forth a few times just above your head.  A few drops of her lubricant drip down onto you, followed by a flick across the nose, delivered by the tail you just grabbed.  At the same time, you're bathed in the deep scent of her aroused sex." );
	if ( flags["COC.MARBLE_DICK_TYPE"] > 0 ) output( "  She grabs her erect member, and rubs it against your cheek a few times, spreading around some of her pre-cum." );
	
	output( "\n\n\"<i>You like it when I take charge, don't you?  Hmmhmm, yes...</i>\"" );
	
	if ( flags["COC.MARBLE_DICK_TYPE"] == 0 )
	{
		output( "\n\nShe drops her hips and roughly forces her womanhood down onto your face, cutting off your ability to breathe.  You sputter and gasp underneath her at the sudden suffocation, though it lasts only a few seconds before she releases you." );
		
		output( "\n\n\"<i>Aww, was that too rough?  Don't worry.  I'll be a bit more gentle.</i>\"" );
		
		output( "\n\nMarble then settles herself down a bit more gently, this time giving you plenty of room to breathe while also keeping your mouth tightly locked against her slick passage." );
		
		output( "\n\n\"<i>Alright, let's put that rude tongue of yours to a much better job.  Lick me.</i>\"" );
		
		output( "\n\nYou do as you're bid and get an approving smile in response.  Digging deeper inside her, you feel under her folds and plunge into her depths.  " );
		if (pc.tongueType > TONGUE_HUMAN) output( "You take full advantage of your inhumanly long tongue to fill her up, running across her passage all the way to the entrance to her womb and then back to her lower lips." );
		else output( "You send your tongue as far into her passage as it will go and wriggle around her lower lips, trying to please your aggressive bovine lover to the best of your abilities." );
	}
	else
	{
		output( "\n\nShe forces your mouth open and pushes her hips forward, roughly shoving her " + num2Text(Math.round(flags["COC.MARBLE_DICK_LENGTH"])) + "-inch cock into your hapless mouth and down your throat.  You choke and sputter in surprise at the sudden suffocation, but it only lasts a few seconds before she pulls back out." );
		
		output( "\n\n\"<i>Aww, was that too rough?  Don't worry, I'll be a bit more gentle.</i>\"" );
		
		output( "\n\nMarble then thrusts forward much more gently, giving you plenty of time to get use to the sensation of her member inside your mouth.  She also is sure to keep herself well outside of your throat." );
		
		output( "\n\n\"<i>Alright, let's put that rude mouth of yours to a much better job.  Suck me.</i>\"" );
		
		output( "\n\nYou do as you're bid, and get an approving smile in response.  You wrap tightly around her with your lips and slide your tongue up and down the bottom of her shaft while also playing across the urethra.  She pumps herself into you a few times, and you get a taste of some of her pre-cum." );
	}
	
	output( "\n\nMarble lifts her hips back up after a few moments, with a curt nod.  \"<i>Now, you've got to take care of my tail.  It stings after you pulled on it like that, I want you to apologize to it and kiss it better.</i>\"  She then turns around and gives you a good view of that very tail, which seems to be swinging back and forth rather half-heartedly." );
	
	output( "\n\nThere, there, you tell the part of your mate's body you hurt before, gently stroking the short furred appendage.  Then you lean forward and repeatedly kiss it, running from the thick tuff on the tip, all the way up to the base where her spine meets the round cheeks of her ass.  When you reach the root, a quiver shoots through Marble's body, and she suddenly grabs you and forces her tail into your mouth.  You gasp in surprise and look up at your lover inquisitively." );
	
	output( "\n\n\"<i>Hmm, sorry sweetie, I just thought I'd try something.  It didn't feel the way I expect, but I really like the way you look with my tail in your mouth like that.  If you suck on it for a bit, I might be able to forgive you for yanking it before.</i>\"" );
	
	output( "\n\nYou run your tongue around the sweaty fur and get an approving look from your bovine lover.  Then you shake your jaw back and forth a few times and slowly let her bush slide out of your mouth.  As the last of the hair slips out, you give a small kiss to the tip." );
	
	output( "\n\n\"<i>Did you like that sweetie?" );
	if ( silly() ) output( "  Want to brush your teeth with my tail next time?" );
	output( "</i>\"  She nods and considers you for a moment.  \"<i>Well, I think I'm feeling a lot better now.  I think I can forgive you for what you did.  In fact, you did such a good job that I think I'll give you a reward!</i>\"" );
	
	output( "\n\nWith a smile, you ask her what she's got planned for you." );
	
	output( "\n\nShe repositions herself overtop of your waist and eyes your male member.  \"<i>Obviously this thing here is in need of some treatment, since you were so eager to try and take charge with it before.</i>\"  She drops down and brings the flaps of her womanhood into contact with the front of your shaft and runs them over it a couple of times.  \"<i>Look how hard you are.  You really want me to take you inside me, don't you?  You want so much to give complete control of your long, hard, dick, to me to use as I see fit.</i>\"" );
	
	output( "\n\nShe doesn't let you respond.  She's already decided what is going to happen, and that's for you to go inside her and be used by her as she sees fit.  She strokes your " + pc.cockDescript(x) + " with one finger, then pushes it into her waiting cunt.  You slip easily into her very well lubricated passage and groan as it clamps down on you.  Every part of your shaft is being tightly wrapped in the ring of her vaginal muscles, and you can't help but groan in pleasure." );
	
	output( "\n\nShe shakes her hips side to side a few times, spinning your shaft around her interior, then stops and considers you.  \"<i>You know, I was thinking that maybe I'm giving you too much of a reward, or maybe not enough of one?</i>\"  The same wicked look when she first knocked you over crosses her eyes once more.  \"<i>Let's try something new...</i>\"  She turns away from you and puts her hand somewhere behind her, outside of your view." );
	
	output( "\n\nAfter a moment, you suddenly feel something pushing up against the entrance to your [pc.vagOrAss]!  You try to ask what Marble has planned, but the pressure forces something bushy into your hole.  She just stuck her tail into your [pc.vagOrAss]!" );
	
	output( "\n\n\"<i>Well sweetie, what do you think?</i>\"  It quivers a few times as Marble resumes the rocking of her wide womanly hips.  \"<i>Never mind, if you don't like it, well you can just try and end this as soon as possible.  If you do like it, then I guess...</i>\"  She leans forward as she switches from a swinging movement to a bounce on your " + pc.cockDescript(x) + ", and her furry tufft of a backend appendage starts to push its way in deeper sliding easily thanks to your saliva on it.  \"<i>You'll just have to bear it as long as you can!</i>\"" );
	
	output( "\n\nYou've never seen Marble this aggressive before, but the cause is rather obvious: the whirring milking machine that's currently attached to her breasts.  Obviously giving milk is triggering some kind of dominance instinct, and it's incredibly sexy!" );
	
	output( "\n\nYour lover laughs uproariously as she continues to ride you hard and fast, bringing up the pleasure in your lower body just as hard and fast.  Whether or not you want it, you can feel an orgasm rapidly building up within you, easily detectable to your rider from the increased rate of your gasps and cries of pleasure.  For her part, Marble is rapidly approaching her peak as well, and with a practiced rhythm, the two of you cum in unison as your lover's tail pops out of your [pc.vagOrAss]." );
	
	output( "\n\nThe bovine woman gasps a few times above you in exhaustion, her great breasts heaving with each breath.  She then reaches down and pulls your head up and cradles it to her chest, smiling very happily.  \"<i>Well sweetie, if you ever want me to play rough again, you know what to do.  I'm looking forward to any other adventures the two of us have in the bedroom, are you?</i>\"" );
	
	output( "\n\nYou snuggle up to her bosom, thinking something very similar.  Things certainly haven't been dull while you've been in Marble's milking stall, that's for certain." );
...
private function milkMarbleLeaveAfterBar():void
{
	...
	output( "You don't answer Marble and instead continue to finger her womanhood until she cries out in orgasm.  You tell her you'd be happy to help her out again whenever she'd like someone to accompany her to get milked." );
	
	output( "\n\nShe is a bit disappointed that you didn't go further after getting her on the bar, but doesn't have a problem with you leaving.  She does tell you that she's looking forward to having you in her milker again some time." );
	
	var pLust:int = int( 5 + pc.lib / 20 );
	dynStats( "lus", pLust );
	flags["COC.MARBLE_LUST"] += 5;
	addNextButton(camp.returnToCampUseTwoHours);
}
...
{
	clearOutput();
	output( "You shake your head and say that you think you'll be fine." );
	
	output( "\n\n\"<i>Oh, alright then sweetie, I'll see you later.</i>\"  She takes a seat in the chair in the middle of the room and picks up where she left off in her book.  You excuse yourself and return back to camp." );
...
private function hammerQuest():void {
	clearOutput();
	output("You ask Marble about her hammer.");
	...
	flags["COC.MARBLE_HAMMER_QUEST_PROGRESS"] = 1;
}