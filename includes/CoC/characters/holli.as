import classes.Characters.CoC.CoCHolli;
...
public function get holli():CoCHolli {
	return new CoCHolli();
}

public function followerCampMenuBlurbHolli(showInteractButton:Boolean):void {
	if (flags["COC.FUCK_FLOWER_KILLED"] == 1) return;
	
	if (flags["COC.FUCK_FLOWER_LEVEL"] == 4) {
		if (showInteractButton) addButton(followerBtnNum++, "Holli", function():* { processTime(2); treeMenu() });
	}
	else if (flags["COC.FUCK_FLOWER_LEVEL"] > 0) {
		if (showInteractButton) addButton(followerBtnNum++, "Sprout", function():* { processTime(2); treeMenu() });
	}
}
		...
		if (flags["COC.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE"] > 0) { //Marae met 2nd time?
			if (int(flags["COC.FUCK_FLOWER_KILLED"]) == 0) { //If flower hasn't been burned down yet
				if (int(flags["COC.FUCK_FLOWER_LEVEL"]) < 4 && int(flags["COC.FUCK_FLOWER_GROWTH_COUNTER"]) < 1000) { //Grow flower if it isn't fully grown.
					IncrementFlag("COC.FUCK_FLOWER_GROWTH_COUNTER");
				}
			}
		}
...
private var HolliTimePassedNotifyHook: * = HolliTimePassedNotifyGrapple();
private function HolliTimePassedNotifyGrapple():* { 
		timeChangeListeners.push(HolliTimePassedNotify);
		loverCampMenuBlurb.push(followerCampMenuBlurbHolli);
	}
	
private function fightHolli():void {
	flags["COC.FOUGHT_HOLLI"] = 1;
	CombatManager.newGroundCombat();
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(new CoCHolli());
	CombatManager.victoryScene(defeatHolli);
	CombatManager.lossScene(enjoyYourBadEndBIYAAAATCH);
	CombatManager.displayLocation("HOLLI");
	CombatManager.beginCombat();
}

public function treeMenu(display:Boolean = true):void {
	if (display) clearOutput();
	if (flags["COC.FOUGHT_HOLLI"] > 0) {
		//If You Fight, Run, and Come Back To Her in the Menu -Z
		output("The demon tree is still here, and the face peeking from it still stares daggers at you even from a distance.  It looks like forgiveness won't be forthcoming.");
		...
		} else addDisabledButton(3, "Torch It", "Torch It", "This option requires you to have access to some fire magic.");
		
		if (pc.hasCock() && pc.lust() >= 33) addButton(0, "Fuck Flower", fuckTheFlower);
		else addDisabledButton(0, "Fuck Flower", "Fuck Flower", "This option requires you to have cock and sufficient arousal.");
		addButton(1, "Drink Sap", drinkThePlantGirlsSap);
		if (pc.hasVagina() && pc.lust() >= 33) addButton(2, "Ride Tentacle", rideTheWalrusP3);
		else addDisabledButton(2, "Ride Tentacle", "Ride Tentacle", "This option requires you to have vagina and sufficient arousal.");
		addButton(14, "Back", campFollowersMenu);
	}
	else {
		//Camp Menu (edited)
		if (display) {
			output("Walking over to the edge of your camp, you approach the familiar demon-tree.  It looks innocent enough from a distance, but the pulsing purple veins in the green leaves show its true nature.  If they didn't, the tentacles cleverly camouflaged in the foliage certainly would!  Once you get closer, the bark splits like a pair of silken curtains, and a female form exposes itself from inside the interior.  Heavy, dripping tits and a fully-unfolded female flower dominate her form, but you drag your eyes up to her own, staring into Holli's golden-brown pupils and black sclera as you greet her.");
			if (flags["COC.HOLLI_SUBMISSIVE"] == 1) {
				output("\n\n\"<i>Welcome back, [pc.name].  ");
				if (flags["COC.TIMES_FUCKED_FLOWER"] > 0 && rand(3) == 0) output("Did you want to fertilize me again?  My flower is ready and waiting...");
				else if (flags["COC.TIMES_RIDDEN_FLOWER"] > 0 && rand(3) == 0) output("Did you want to use my tentacles again?  They've been restless without you...");
				else if (rand(2) == 0) output("Did you come for a drink?  I've built up lots of nectar for you.  Surely, a sip would be refreshing for you...");
				else output("How may I serve you?  Come, give me a command so that I can fulfill it for you.");
				output("</i>\"");
				
			}
			else {
				output("\n\n\"<i>Welcome back, [pc.name].  ");
				if (flags["COC.TIMES_FUCKED_FLOWER"] > 0 && rand(3) == 0) output("Did you want to fertilize me again?  Gardening is such exhausting work, but so rewarding...");
				else if (flags["COC.TIMES_RIDDEN_FLOWER"] > 0 && rand(2) == 0) output("Did you come back to play again?  My tentacles are so restless.");
				else if (rand(2) == 0) output("Did you come for a drink?  I've built up lots of nectar for you.  Surely a hungry adventurer like yourself needs a sip, right?");
				else output("Do you want to play?  Come on, taste Mother Marae's finest creation.");
				output("</i>\"");
			}
			//Extra line on description:  
			if (flags["COC.HOLLI_FRUIT"] > 0) {
				if (flags["COC.HOLLI_FRUIT"] == 1) output("\n\nA large, purple fruit hangs from Holli's branches, twisting softly in the breeze.  It looks succulent and ripe.");
				else output("\n\n" + num2Text(flags["COC.HOLLI_FRUIT"], true) + " large, purple fruits hang from Holli's branches, twisting softly in the breeze.  They look succulent and ripe.");
				//Optional Addition: 
				if (flags["COC.HOLLI_FRUIT_EXPLAINED"] > 0) output("  A very small, strange part of you actually feels proud to have made something like that with the tree-woman.");
			}
		}
		...
		if (int(flags["COC.HOLLI_SUBMISSIVE"]) == 0) {
			...
			clearMenu();
			if (pc.hasCock() && pc.lust() >= 33) addButton(0, "Fuck Holli", fuckHolliInZeFlowerPuss);
			else addDisabledButton(0, "Fuck Holli", "Fuck Holli", "This option requires you to have cock.");
			addButton(1, "Drink Sap", haveAMapleSyrupSnack);
			if (pc.hasVagina() && pc.lust() >= 33) addButton(2, "Ride Tentacles", level4RideHollisTentacruels);
			else addDisabledButton(2, "Ride Tentacles", "Ride Tentacles", "This option requires you to have vagina.");
			if (flags["COC.HOLLI_DEFENSE_ON"] == 1)
				addButton(3, "Stop Guarding", askHolliToWatch);
			else addButton(3, "Guard Camp", askHolliToWatch);
			if (flags["COC.HOLLI_FRUIT"] > 0) addButton(4, "Eat A Fruit", eatHolliFruit);
			else addDisabledButton(4, "Eat A Fruit", "Eat A Fruit", "There are no fruits now.");
			addButton(14, "Leave", campFollowersMenu);
		}
		else {
			if (pc.hasCock() && pc.lust() >= 33) addButton(0, "Fuck Holli", holliGetsDickDommed);
			else addDisabledButton(0, "Fuck Holli", "Fuck Holli", "This option requires you to have cock.");
			
			if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) >= 10 && pc.lust() >= 33) addButton(1, "TentacleBone", fullOnTentacleTasticGangBangForHolli);
			else addDisabledButton(1, "TentacleBone", "Tentacle Bone", "This option requires you to have at least 10 tentacle cocks.");
			
			if (pc.hasVagina() && pc.lust() >= 33) addButton(2, "Ride Holli", vaginalDomHollisTentacruels);
			else addDisabledButton(2, "Ride Holli", "Ride Holli", "This option requires you to have vagina.");
		...
		if (int(flags["COC.FOLLOWER_AT_FARM_HOLLI"]) == 0 && flags["COC.FARM_CORRUPTION_STARTED"] == 1) addButton(6, "Farm Help", helpWithFarm);
	}
}
...
private function helpWithFarm():void
{
	...
	output("You ask if a plant goddess might not be able to help the farm you recently acquired.");
...
private function noPlzDontFuckWithFarm():void
{
	...
	output("“<i>Such a fickle mortal,</i>” she sighs. “<i>But I am a kind goddess. Ask me for it anytime, assuming you’ve... performed the rites.</i>”");
...
private function fertilizeHolli(cock:Boolean = true):void {
	...
	if (cock && pc.hasCock()) {
		...
		odds += pc.cumQ() * pc.virility() / 300;
		if (odds > 40) odds = 40;
		if (pc.hasPerk("Marae's Gift: Stud")) odds += 10;
		if (pc.hasPerk("Fera's Boon: Alpha")) odds += 10;
		if (pc.hasPerk("Elven Bounty")) odds += 2;
	}
	if (!cock && pc.hasVagina()) {
		if (pc.fertility() <= 0) return;
		
		odds += pc.fertility() * 10;
		if (odds > 40) odds = 40;
		if (pc.hasPerk("Marae's Gift: Fertility")) odds += 10;
		if (pc.hasPerk("Fera's Boon: Breeding Bitch")) odds += 10;
		if (pc.hasPerk("Elven Bounty")) odds += 2;
	}
	if (odds >= rand(101)) {
		flags["COC.HOLLI_FRUIT"] += 1 + rand(Math.floor(odds/10));
		trace("Holli pregged!");
	}
}
...
public function getASprout():void {
	...
	if (flags["COC.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE"] == 2) output("  She even promised to plant a tree in your camp...");
	output("  You look closer and see purple veins on the undersides of the leaves, throbbing with what can only be the tainted liquids that suffuse much of this planet.  Fully grown, it might spread its corruption to your camp.\n\n<b>Do you destroy it?</b>");
	flags["COC.FUCK_FLOWER_LEVEL"] = 1;
	flags["COC.FUCK_FLOWER_GROWTH_COUNTER"] = 0;
	//[Yes] [No]
	clearMenu();
...
private function destroyDatFukkinTree():void {
	clearOutput();
	output("You reach down and begin to excavate meticulously around the tainted sapling.  It has one central root that seems to go down forever, but after you've uncovered half a foot of taproot, you grab hold of it and yank.  At first, your straining seems unnoticed by the plant, the leaves rustling as it shakes and strains under your grip.  Then, all at once, it gives with a violent SNAP.  You tumble end over end, clutching to the demon-flora tightly to your [pc.chest].  Sighing and grumbling, you get up and toss the leafy demon into your fire.");
	...
	//(-5 corruption)
	pc.cor( -5);
	flags["COC.FUCK_FLOWER_KILLED"] = 1;
	processTime(15);
	addNextButton();
}
...
private function letZeFuckingSproutLive():void {
	clearOutput();
	output("Looking down at the sapling, you stay your wrath.  It may be corrupt, but it hasn't done anything to harm you just yet.  You give it a little pat on the uppermost leaves and leave it be.  It's not like it's going anywhere.");
	output("\n\n(<b>'Plant' added to your followers menu</b>.  It's too small to know what it will grow into yet.  You can currently remove it at your leisure.)");
	processTime(5);
	addNextButton();
}
...
public function fuckPlantGrowsToLevel2():void {
	...
	if (!pc.hasGenitals()) output("  Sucks to be it!  You don't waste time with stupid shit, like genitals.");
	output("  Is this Marae's idea of a gift?");
	if (pc.hasGenitals()) output("  Well, you could give it a whirl... just being around it seems to slowly turn you on.");
	output("  Of course, destroying it would be the safest option.");
	pc.lust(33);
	if (silly) output("\n\n<b>What do?</b>");
	else output("\n\n<b>What do you do?</b>");
	//[Fuck It] [Ride Stamen] [Do Nothing] [Destroy It]
	processTime(5);
	clearMenu();
	if (pc.hasCock() && pc.cockThatFits(cockVolume(100)) >= 0) addButton(0, "Fuck It", fuckFuckingFuckFlowerP2);
	else addDisabledButton(0, "Fuck It", "Fuck It", "This option requires you to have fitting cock.");
	if (pc.hasVagina()) addButton(1, "Ride Stamen", rideDatFuckingFukkFlowerP2);
	else addDisabledButton(1, "Ride Stamen", "Ride Stamen", "This option requires you to have vagina.");
	addButton(2, "Do Nothing", mainGameMenu);
...
private function fuckFuckingFuckFlowerP2():void {
	...
	output("Undoing your [pc.gear], you approach the pulsating plant with one thought in mind: fucking it.  At the sight of [pc.eachCock], the petals flutter happily, their surface glimmering with moisture in an instant, slick and wet for your pleasure.  You lean down to inhale the sweet yet sensual nature of the flower's aroma, letting it tickle your nostrils as you idly fondle [pc.oneCock].  Your maleness quickly assumes your favorite state - hard and sensitive, ready to plunge into the nearest set of wet lips with reckless abandon.  At the same time, you can see the wriggling tendrils inside the flower's vase waving about hungrily.");
	output("\n\nYou laugh in amusement; who are you to deny it the creamy treat it so richly deserves?  Letting go of your " + pc.cockDescript(x) + ", you reach around the soft petals to the harder, central bud and slide yourself through the honeyed embraced into the pulsating, tentacled flower-core.  Juicy warmth snuggles about your dick, the velvet-soft chamber immediately tightening up while the petals fold in, crossing over one another until they form a tube long enough to swallow you whole.  Gentle suction begins to rhythmically tug at your [pc.cockHead " + x + "], swelling you larger and more sensitive inside.  Immediately, the tiny tendrils you saw swarming around earlier reach up to massage your cock, slithering across your skin like tiny, sinuous snakes.");
	output("\n\nIt's enough to make your [pc.legs] quake, and you slowly slide to the ground.  The flower, now a tightly-sealed bulb, sucks hard enough to stay latched onto you as you recline.  It makes the stem double over, but the resilient plant seems to handle the strain with ease.  Rivulets of clear slime escape from the tight seal to run down your ");
	...
	output(".  With so much blood trapped in your " + pc.cockDescript(x) + ", it's bigger and harder than ever, twitching valiantly against its restraints with every beat of your heart.");
	output("\n\nOne of the tentacles circles your [pc.cockHead " + x + "] for a moment, and without much warning or pause, it lances forward to bury itself into your vulnerable cum-slit.  Your preconceptions are turned on their heads when there's a complete lack of pain from the abrupt penetration.  If anything, it actually feels kind of good... like there's a warm, slippery finger caressing your penis from the inside out.  It slithers in and out of you, pumping you inside, burrowing pleasure into your center even while stroking you from without.  Your body, burning from the exertion of fucking the flower's pod and the constantly-rising tidal wave of lust, begins to twitch spasmodically.");
	output("\n\nThe internal suction spikes as your dick bloats larger, and with a roar of bestial pleasure, you arch your back, hands pushing your " + pc.cockDescript(x) + " as deep into the plant's gullet as possible.  ");
	if (pc.balls > 0) output("Your [pc.balls] gurgle, a surprisingly happy sound, and cling tight to your loins.  ");
	...
	if (pc.cumQ() > 500) output("  You pour forth your prodigious essence without pause or fail, and the plant immediately begins to drink it down, the stem bulging with rounded balls of sperm that it carries underground.");
	if (pc.cumQ() > 1500) output("  Soon, the meager stem cannot swallow it all.  Spunk froths and squirts out around the base of your tool for a moment, and then a veritable tide of the stuff gushes out to soak  you.");
	...
	pc.orgasm();
	pc.cor(1);
	flags["COC.TIMES_FUCKED_FLOWER"] = 1;
	if (flags["COC.FUCK_FLOWER_GROWTH_COUNTER"] < 1000) flags["COC.FUCK_FLOWER_GROWTH_COUNTER"] += 4;
	pc.energy( -20);
	addNextButton();
}
...
private function rideDatFuckingFukkFlowerP2():void {
	clearOutput();
	output("You shuck your [pc.gear] and slowly ease yourself towards the violet-hued creature.  As soon as you get within a few feet, the flower's bulb angles toward you, the petals wiggling slightly along with the stamen inside.  A few of them even stretch out towards you like longing fingertips, shaking slightly from the effort before sliding back inside.  The dull ache in your [pc.vagina] grows a few degrees hotter at the display, your body growing flush with excitement for the coming copulation.  Throwing caution to the wind, you move forward with a confident sway of your [pc.hips].  It's time to get pollinated.");
	...
	output("\n\nDriven mercilessly by the sudden onslaught of sensation, you flop back onto the ground, but the flower comes with you, latched onto your [pc.hips] with no sign of releasing.  The swirling tentacles twist around inside you in a tornado of ecstasy, and the pleasure, that awful, unstoppable pleasure, twists your muscles into knots.  They instinctively convulse to raise your lower lips higher, as if that would somehow propagate even deeper penetration.  One of the tentacles retracts, drawing a burst of slick cunt-lube with it.  Then, it twists around your [pc.clit], tightening up on the over-sensitive bud until it feels like it could pop.");
	if (pc.clitLength > 4) output("  Your heavy buzzer is so big that the flexible plant-cock has plenty of room to curl around it, and if you could see it, your clit would look almost like a candy-cane (if one set of stripes was jerking off the cane beneath).");
	if (pc.cockTotal() > 0) output("  [pc.EachCock] soon has its own partner twisting around it, a pussy-slicked vine to jerk and twine about its girth.");
	output("\n\nWith nothing to occupy your roving hands, you let them slide up your " + pc.skinFurScales() + " to your ");
	if (pc.bRows() > 1) output("lowest row of tits, cupping them before groping to your top row of breasts");
	else output("[pc.chest]");
	output(", caressing the");
	if (pc.biggestTitSize() > 1) output("ir");
	output(" swell as you search for the lust-tightened skin of your [pc.nipples].  They seem almost as sensitive as little clits, and as soon as you find them, you immediately set to circling them slowly, occasionally tugging and twisting when a particularly powerful sensation works through your pleasure-wracked body.");
	if (pc.lactationQ() > 50) {
		output("  One after the other, they begin to ");
		if (pc.lactationQ() < 150) output("trickle");
		else if (pc.lactationQ() < 500) output("leak");
		else output("spray torrents of");
		output(" milk.");
	}
	output("  You moan in unabashed delight, your mouth hanging open in a mindless, blissful 'o' in between vocalizations.  If your friends back home could see you now, writhing in the dirt, face open in simple, animal pleasure, and perhaps worst of all, fucking a plant, you'd die of ");
	if (pc.exhibitionism() < 66 && pc.slut() < 100) output("embarrassment");
	else output("lust");
	output(".");
	...
	if (pc.cumQ() < 50) output("drizzle");
	else if (pc.cumQ() < 250) output("stream");
	else if (pc.cumQ() < 750) output("spray");
	else output("torrent");
...
public function flowerGrowsToP3():void {
	clearOutput();
	output("Again, you hear odd noises coming from a corner of your camp - the corner where you let that tainted shrubbery grow.");
	if (flags["COC.TIMES_FUCKED_FLOWER"] + flags["COC.TIMES_RIDDEN_FLOWER"] > 0) output("  The fucking probably only helped it to grow faster.");
	output("  The distinct sound of wood bowing and creaking under new weight catches your ears, and well before you can get to the plant, you can see a leafy canopy stretching towards the sky.  <b>It's grown into a small tree, with bark, leaves, and everything!</b>  Warily, you round one of the ancient standing stones to get a better look.");
	...
	if (getKnownFireMagic() != null) {
		output(", though you suppose you could burn it down with magical fire if you set your mind to it");
		addButton(3, "Torch It", torchP3Tree);
	} else addDisabledButton(3, "Torch It", "Torch It", "You need some special kind of fire to burn it now.");
	
	output(".\n\n<b>What do you do?</b>");
	
	if (pc.hasCock() && pc.lust() >= 33) addButton(0, "Fuck Flower", fuckTheFlower);
	else addDisabledButton(0, "Fuck Flower", "Fuck Flower", "This option requires you to have cock.");
	
	addButton(1, "Drink Sap", drinkThePlantGirlsSap);
	
	if (pc.hasVagina() && pc.lust() >= 33) addButton(2, "Ride Tentacle", rideTheWalrusP3);
	else addDisabledButton(2, "Ride Tentacle", "Ride Tentacle", "This option requires you to have vagina.");
	
	addButton(4, "Leave It", mainGameMenu);
}
...
private function fuckTheFlower():void {
	...
	if (pc.hasVagina()) output("  Wetness builds in your [pc.vagina] as well, but you're focusing on your maleness just now.  Your neglected twat will have to wait its turn.");
	output("  A feverish sweat breaks out on your brow, though whether from your own arousal, the plant's heat, or some chemical in its juices, you cannot tell.");
	
	output("\n\nBefore you can pull back, the hiss of something cutting through the air with lightning speed hits your eardrums.  Almost at the same time, tentacles (stamen?) snap around your neck, head, and ears, going taut in an instant.  Your " + pc.face() + " is pulled roughly into the glossy hole, the soft folds of the petals smearing your nose and mouth with their saccharine sweetness.  You cannot help but breathe in more deeply as you struggle for your freedom, and with every struggle, every struggling gasp, [pc.eachCock] grows hotter and harder, pulsating with tactile hunger.  With each moment that passes the motivation for your struggles shifts from freedom to fucking.  You don't care about getting away so much as getting your dick in this aromatic fuck-pot.");
	
	output("\n\nYou claw the whip-like vines from your face in a frenzy and fall back, tumbling end over when the tension suddenly gives way.  Landing flat on your back and gasping, you immediately try to rise, spurred on by the white-hot desire burning in your groin.  Tentacles drop down as you climb to your [pc.feet], and they loop under your arms, around your shoulders, and even over your [pc.chest].  They hoist you up before you can get your [pc.feet], another vine twisting your wrists together behind you, immobile.  You foolishly pump your hips forwards, struggling to reach the plant's puckered opening, but the attempt bears no fruit.");
	
	output("\n\nRacking your brain for ideas, you momentarily slump into a temporary acceptance of your position, sagging heavily into the strong, fibrous tentacles.  Perhaps that's what the unnatural creature wants, for as soon as you let it have control, the tendrils swing you closer, close enough for [pc.oneCock] to kiss the warm, wet hole.  Momentum carries you away from the promise of greater pleasure, your " + pc.cockDescript(x) + "'s tip nicely moistened by lubricants and its own heavy bead of liquid pleasure.  When you swing back, a few inches are lucky enough to nestle inside, and you find yourself moaning without meaning to.");
	
	output("\n\nYou're swung to and fro, held aloft by a shrubbery harness seemingly intent on teasing you.  First, you get a taste of heaven, but only on your [pc.cockHead " + x + "].  Then, you're left to flutter in the breeze, the only sexual sensation available the cool wind fluttering across your whetted tip.  Over and over, the plant teases you until you lose track of time.  The closest thing to a watch you have is the gentle throb in your [pc.balls] and its gradually strengthening intensity.  Oh, how it aches!  You feel taut and stretched like a harp string being tuned, only the musician just keeps cranking away, ratcheting up the pressure until you feel about to snap.  Your one comfort is the momentary bliss of the flower's sweet kiss, an exciting pleasure that's slowly being turned into an aching hell.");
	output("\n\nDuring a particularly shameless moan, you realize that you're dipping a little deeper with each pendulous stream.  Luckily, the corrupt tree's arboreal lust must be rising like your own, and after a few slow swings, your " + pc.cockDescript(x) + " is slipping three quarters of the way in");
	if (pc.cockVolume(x) >= cockVolume(150)) output(", sliding down the trunk as easily if the entire inside was a giant pussy just waiting to get fucked");
	output(".");
	if (pc.cockTotal() > 1) {
		output("  Your other penis");
		if (pc.cockTotal() > 2) output("es rub");
		else output(" rubs");
		output(" sensually across the moist outer blossom, drooling their clear pre-cum over the petals.");
	}
	output("  Schliiick.  You slide out.  Slorp-pop.  You slide in, this time to the ");
	if (pc.hasSheath()) output("sheath");
	else output("base");
	if (pc.balls > 0) output(" with your gurgling balls slapping against the flower's edge");
	output(", accompanied by a small squirt of sweet lubricants.");
	output("\n\nYou've been kept on edge for so long that even this brief taste of liquid-soft snugness has your body clenching and warm bliss mounting inside of you.  You wrap as much of your [pc.legs] around the trunk below as possible and give into your pleasure, thrusting your " + pc.cockDescript(x) + " against the petalled muff each time the vines draw you away.  You rut like a demon's pet, half-mad with pleasure and desire so powerful it obviates rational thought.  The ecstasy rises like an all consuming tide, tinting your vision pink as your eyes roll back and [pc.eachCock] thickens.");
	
	output("\n\nJizz blasts out of your [pc.cocks] with uncommon force, painting the flower's interior with white");
	if (pc.cockTotal() > 1) output(", as your multiple endowments allow you to drizzle the purple bloom's exterior with alabaster cream");
	output(".");
	if (pc.cumQ() > 500) output("  Pearly ropes big enough to fill a flask lurch out from your cum-slit, propelled by your quivering orgasmic muscles deep into the trunk, and you swear you can hear the pitch of your sexual pumping rising in tone, changed by the flooded vessel.");
	if (pc.cumQ() > 1500) output("  A wave of your thick gunk rolls out from under your pumping cock - you've filled the tree beyond even its capacity, and the thick puddle forming beneath your [pc.feet] is all the proof you need of your virility.");
	if (pc.hasVagina()) {
		output("  Your hungry cunt, long denied in this exchange, trembles as it ");
		if (!pc.isSquirter()) output("leaks");
		else output("sprays");
		output(" your own female fluids in sympathy.");
	}
	output("  After what feels like forever, the spurting seed no longer flows from your long-abused dick, and as your member softens, the tentacles swing you away, setting you down softly.");
	
	output("\n\nYou sigh, utterly exhausted and drained of sperm.  What a rush!  Your [pc.gear] slips on easily after that ordeal, though your muscles are a bit sore.");
	processTime(25 + rand(10));
	pc.orgasm();
	pc.cor(1);
	pc.energy( -20);
...
private function drinkThePlantGirlsSap():void {
	...
	if (silly) output(", tree hugger that you are");
	output(".  The thick 'milk' quickly fills your body with energy, though it runs out nearly as soon as it started.");
...
private function rideTheWalrusP3():void {
	...
	output("You eye the tentacles hiding amongst the leafy branches above, writhing obscenely around each other as they snake through the foilage.  You shed your [pc.gear] into a worthless pile and bare your body, cupping your [pc.chest] enticingly as you whistle up at the tree.  Your calls seem to fall on deaf ears; the whirling vines stay put up in their perch.  They're ignoring your delicious, feminine flesh!  Irritated, you stomp up to the trunk's flower and plunge your fist inside it.  Fresh fluid gushes out over your armor, releasing a spray of sweet, musky lubricants.  You blindly feel around inside the pliant plant-pussy with your fingers, giggling to yourself whenever the soft cavity clenches tightly to your arm.  Whatever this is, it can feel pleasure.");
	...
	output("\n\nWith a sweet sigh, you spread your arms and let the surprisingly talented feelers take you.  One wraps around your midsection on its way down to your claimed [pc.vagina], cinching around you with gentle tightness.  A trio of verdant vines hooks under your armpits, two on the wet side, and one on your dry, unscented arm.  They really like the smell of cunt, even their own.  A mass of them gathers in and around your [pc.legs], and as soon as they wrap you up tight, the whole group of them begins to lift you up.  A tremor of worry runs through your stomach, but a slow, gentle prodding in your [pc.vagina] does much to assuage your worries.  Another, smaller cock circles around its lucky brother, pressing its tip insistently at your [pc.clit] while the first slowly penetrates you.");
	//cuntChange:15
	pc.cuntChange(0, cockVolume(15), true, true, false);
	output("\n\nYour [pc.nipples] go hard; whether from the penetration, reek of sex, or body-caressing vines, you cannot say.  A few more loops of plant-cock surround you, to the point where you're beginning to wonder if this is what a snake's prey feels like, bound up with smooth coils that cling tightly to every curve of your body.  Pumping slowly, the tentacle you eased into your [pc.vagina] begins to fuck you with meticulous slowness.  The surface of the slick shaft turns glossy with your lust and wriggles pleasantly inside you, twirling around in a ballet of unnatural motions that make you feel like you're atop a tornado of sexual pleasure.  Between that and the numerous coils gliding across your sensitive nipples, you throw back your head and moan with unrestrained delight.");
	...
	output("\n\nAt some point, you are set back down on the ground and sprayed with a fresh batch of seed.  You rest there for a while, until you can catch your breath and move without pain.  When you start trying to clean up, you realize the puddle of cum is completely gone, likely swallowed back up the tree's hungry roots, and the only mess left is your sodden [pc.skinFurScales].  You get dressed and stumble away a little drunkenly, your [pc.vagina] raw and oozing.");
...
private function torchP3Tree():void {
	...
	output("This has gone on long enough!  This thing cannot continue to grow in your camp any longer, and you have just the means to remove it: fire.  ");
	//[(nerdfire)
	if ((pc.perkv1("Psionic Affinity") & KBIT_SPELL_WHITEFIRE) > 0 || isNineTails(pc)) output("Closing your eyes to focus, you gather your energies, and unleash your white, supernatural flames upon the thing.");
	//(bromancefire)
	else if (pc.hasPerk("Fire Lord")) output("Sucking in your breath, you inflate your chest and stir the embers of the jaguar demon's gift.  Dredging up an enormous pine-green fireball, you exhale and launch it at the tree.");
	else output("Sucking in your breath, you inflate your chest and stir the embers of your fiery gift.  Dredging up an enormous fireball, you exhale and launch it at the tree.");
	output("  It goes up by the torch, though the fire starts to fade sooner than you would expect.  You huff, but repeat the action, layering a new blaze atop the previous one; you put all your concentration into this new conflagration, stoking the inferno with every ounce of your ");
	if ((pc.perkv1("Psionic Affinity") & KBIT_SPELL_WHITEFIRE) > 0 || isNineTails(pc)) output("magical will");
	else output("breath");
	output(".");
...
public function JojoTransformAndRollOut():void {
	...
	output("\"<i>[pc.name], come here please.</i>\"");
	...
	if (pc.cor() < 25) output("Not willing to lie to your friend, you also admit that she seems to have changed after your actions in the factory - that, though you were trying to render the tanks permanently inoperable, the spillage you created pushed the cthonic spirit over the edge.");
	else output("Though, you do leave out the precise, needless details of what caused the change.");
	output("  The monk's eyes widen in surprise and surmise.");

	output("\n\n\"<i>Unbelievable...</i>\" he blurts.  \"<i>");
	if (flags["COC.TIMES_FUCKED_FLOWER"] + flags["COC.TIMES_RIDDEN_FLOWER"] == 0) output("To think that I must now consider Marae herself my enemy.  Forgive me, but this place is not safe for us anymore if it has attracted the attention of such a creature.  For now, I will return to the forest.  I know that you believe your mission keeps you here, but I urge you to stay away from that thing and watch the portal from an adequate distance.");
	//(totally deflowered that flower)
	else output("And even knowing that this could be the case, you still chose to fornicate with it?  [pc.name]... I cannot say that I am not disappointed.  This place is not safe for me anymore.  For now, I will return to the forest.");
	output("</i>\"");
	
	output("Jojo looks at you sadly; any argument you try to make is met with a resigned shake of his head.  \"<i>If the creature truly is the demonic offspring of a corrupted earth spirit, doubtless it will be powerful and the wrath of its parent severe.");
	if (pc.level < 20) output("  Nonetheless, if you find yourself in a position where you must confront this demon or perish, call upon me.  I will render what aid I can, though I prefer it not be in vain.");
	else output("  Though, a mortal of your stature and resources may be able to destroy her and escape the reprisal.  I would counsel against direct confrontation until you are sure of victory, but if you must fight, then call upon me and you will have what aid I can give.");
	output("</i>\"  So saying, he presses a small talisman into your hand.");
	
	output("\"<i>Until then, you will find me in the forest if you need my assistance.</i>\"  Jojo makes a polite bow and departs.");
	output("(<b>Jojo is no longer a companion!</b>)");
	//no mo' Jojo; ho made schmo go
	//return Jojo to forest, get key item Small Talisman
	flags["COC.JOJO_IN_CAMP"] = 0;
	pc.createKeyItem("Jojo's Talisman", 0, 0, 0, 0);
...
private function stayQuietWhileAmilyBitchesAboutFuckingArborday():void {
	clearOutput();
	output("Amily berates you for some while longer, then eyeballs you.  Just as you begin to speak, she cuts you off again.");
	...
	flags["COC.AMILY_FOLLOWER"] = 0;
	//Set - amily flipped her shit
	flags["COC.AMILY_TREE_FLIPOUT"] = 1;
	flags["COC.AMILY_CORRUPTION_WARNING"] = 0;
	//Enable village encounters
	flags["COC.AMILY_VILLAGE_ENCOUNTERS_DISABLED"] = 0;
...
private function slapAmilysWhoreFace():void {
	clearOutput();
	output("You slap Amily across the ");
	if (int(flags["COC.AMILY_NOT_FURRY"]) == 0) output("nose");
	else output("face");
	...
	if (int(flags["COC.AMILY_NOT_FURRY"]) == 0) output("mouse-");
	output("girl says, eyes wet with tears, \"<i>but I don't.  Goodbye.</i>\"");
...
private function fuckHolliInZeFlowerPuss():void {
	...
	if (pc.tallness < 84) output("little");
	else output("big");
	output(" " + pc.mf("boy","girl") + ",</i>\" the arboreal tramp suggests with a husky coo, \"<i>I got you some sugar.</i>\"  She pinches her nipples and gasps, dark streams of sugary juices dripping wetly into the soil as she beckons you closer.");
	output("\n\nYou confidently advance, phallus first, and press your erection into the syrup-spurting valley, getting fluid all over yourself in the process.  Wet, sticky squishes greet your ears as [pc.oneCock] is enveloped in the jiggling G-cups, webs of amber fluid hanging between your loins and the hefty globes as they're lifted away, stroking along your sensitive length.");
	if (pc.cockTotal() > 1) {
		if (pc.cockTotal() == 2) output("  Your other penis splays");
		else output("The rest of your penises splay");
		output(" to the side, smearing furrows of the stuff around as they slide over the butter-smooth skin.");
	}
	output(" The demon dryad's hand reaches below to ");
	if (pc.balls > 0) output("cradle your [pc.balls], gently massaging your sack");
	else {
		output("gently caress your ");
		if (pc.hasSheath(x)) output("sheath");
		else output("loins");
	}
	output(" encouragingly.  You respond by edging your [pc.hips] upward, more effectively presenting your shaft to be pleasured.");
	
	output("\n\nSighing happily, the immobile beauty ");
	if (pc.cockVolume(x) < cockVolume(100)) output("leans down and kisses the tip, opening her glossy green puckers to slurp it down, sucking just hard enough to make your [pc.cockHead " + x + "] tingle with erotic warmth.");
	else output("kisses up and down your huge, towering shaft, slobbering her glossy green puckers all over your urethral bulge, making it even more of a wet mess.");
	output("  You grab hold of the edges of her tits and squeeze, smushing them tight around yourself and sliding them up and down.  The plant-woman's twinkling golden eyes meet your own while she pleasures your " + pc.cockDescript(x) + ", and her hands come to rest over top of your own, assisting you with the quick tit-fuck.");
	...
	output(", slapping wetly against you.  Soon, the slick texture of her soft melons has you throbbing eagerly and dribbling hot trails of pre-cum.  You're going to blow any second now, and the hot bloom of pleasure within you is spreading further and further.  Those gold and black eyes look up imploringly while the pale green cheeks hollow hungrily, and you release ecstatically, arching your back as you pound the heavy tits down your length.");
	
	output("\n\nA geyser of spunk rockets out into ");
	if (pc.cockVolume(x) >= cockVolume(100)) output("the air, where it hangs for a moment before splattering down atop the plant-girl's leaf-colored hair and face, forming a gossamer shroud.");
	else output("her mouth, which works noisily to swallow the hot load even as you inject it into her.  After a few swallows, she leans back and lets you shoot it up into the air, so that it can fall down over her face and hair, into a gossamer shroud.");
	//highish cum:  
	if (pc.cumQ() >= 500) output("  You keep blasting heavy ropes of seed until her body and trunk are painted with goo, and then you cum some more, smattering spunk until she looks more like a gooey waterfall than a fey creature.");
	if (pc.cumQ() >= 1500) output("  Before you know it, there's a lake around the two of you and a small river running downhill away from camp.");
	output("  A mischievous grin breaks out under the goopy facial, followed by an unnaturally long tongue that pulls gobs of the stuff into the dryad's hungry mouth.  Once she can see again, the girl gives her tits a squeeze and affectionately kisses your cock, practically worshipping it with sloppy-sweet licks.");

	output("\n\nYou break the embrace with a happy sigh and admire the state of your cock: it drips with milky syrup and still pulses with the pleasant aftershocks of your climax.  The pressure of your lust is dimmed, though a hint of it remains in your [pc.balls], and you're still fairly hard.");
	...
	if (pc.tallness < 60) output("tits");
	else output("arms");
	output(".  [pc.EachCock], instead of slamming into the bark painfully, slots into something warm and wet, pulsating and pliant.  A clingy, tight hold that ripples gently around your member");
	if (pc.cockTotal() > 1) output("s, massaging the bundle of cocks with uncommon ferver");
	else output(", massing your tool with uncommon fervor");
	output(".  Your grunt of irritation vanishes as soon as your tender sex is forced into the heavenly hole, and with the soft breasts pressing into ");
	if (pc.biggestTitSize() >= 2) output("your own");
	else output("your [pc.chest]");
	output(", you give up and kiss the dryad full on the lips, biting the lower one roughly in irritation.");
	
	output("\n\nHolli deprived you of choice, but if you have to fuck, it's going to be by your rules.  You slide your fingers through the seam between her hips and the tree, finding a squeezable, pert behind to grab hold of.  Then, you pull your hips back, and still biting her lip, you slam them home, powerfully delivering your [pc.cocks] into her slobbering, nectar-filled gash.  You let her lip go and menacingly growl at her, plowing her fields with animalistic vigor.");
	output("\n\n\"<i>F-f-fuck yeah!  Ooooh...</i>\" Holli moans, clinging tightly to you as she pouts her lower lip at you, \"<i>Take me!  Seed me!  Seed me, please, [pc.name]!</i>\"");
	output("\n\nYou snarl in irritation at the dryad's pathetic pillow talk, and tweak her nipple, turning her pleading into in a high pitched whine.  Meanwhile, the slick, outer petals have spread out around your [pc.hips], closing neatly behind your [pc.butt].  It limits your range of motion, but you don't have to pull back THAT far to give this pussy a proper reaming.  You fuck it faster and harder, hitting that tight twat hard enough to rustle the leaves above.  She gushes sweet nectar down your [pc.legs] with each clap of hip to hip, and soon her accommodating interior is quivering uncontrollably, actually spurting out jets of fragrant nectar with each cock-devouring push.");
	
	output("\n\nHolli's black-ringed eyes roll back, her mouth quivering wordlessly.  The sight is enough to drive you to a release of your own, and you vent your [pc.balls] into the quavering honeypot. Kissing her again, you enjoy the moment, your hips slowly pumping in time with the muscular contractions that squirt ever more of your seed into her furrow.  The petals that were constraining you open wide, oozing their matching moisture in fat drops.  The moment is as messy as a mud wrestling competition in the rain, a sexual frenzy of sordid, splattering proportions.");
...
private function haveAMapleSyrupSnack():void {
	clearOutput();
	output("You approach Holli with a hungry smile plastered across your [pc.face], licking your chops as you eye the arboreal slut with unrestrained desire.  Of course, your gaze fixes on her heavy chest, with its distended, dripping nipples.  They leak sweet sap as you stare; the demon seems eager to feel your mouth on her slick, almond-colored buds.  Looking at you ");
	if (int(flags["COC.HOLLI_SUBMISSIVE"]) == 0) output("knowingly, Holli asks, \"<i>Come for a little pick-me-up?  Well, go on; drink deeply of my nectar, </i>champion.\"  The last word is inundated with malicious, mocking tones.");
	else output("eagerly, Holli asks, \"<i>Care for a little pick-me-up?  They're getting very engorged, and I'm sure it would be a refreshing snack for you.</i>\"");
	output("\n\nGrowling faintly, you snake your arm in along her side and curl it behind her back, pulling her forward and out.  Holli gasps in surprise, but doesn't struggle.  A drop of her syrupy sap splashes on your [pc.gear], trickling out with greater rapidity from the dryad's sweet tits.  You crane your neck to take one in your mouth, and immediately begin to suckle it down.  The fluid is more like maple syrup than milk, and so potent it fizzles on your tongue, bursting with flavor.  With your free hand, you squeeze the rounded tit and increase the flow.");
	...
	output("\n\nThe nipple pops out of your lips with a weak spray of 'milk', and its owner shudders slightly, her flower blooming against your [pc.legs], weakly grasping.  You let your restraining arm go a bit lower to squeeze Holli's cute little butt.  Her smooth skin feels perfect in your hand, though you can feel that a little lower, her flesh joins with the wood of the tree, becoming quite rough.  How odd.");
	output("\n\nYou quickly tire of fondling the proud creature's body and return to drinking her treasure straight from the tap, suckling hungrily to expedite the process.  Holli's pants can be heard above you, and the slick grinding of her petals over your [pc.hips] adds to the eroticism in the air.  Your own blood is pumping harder and faster, flushing your " + pc.skin() + ".  Drinking deeper, you realize you feel vibrantly alive, completely and utterly suffused with energy.  You swallow the last of Holli's syrup and shake slightly, breaking away from the intimate embrace you shared.");
	//pc.refillHunger(10);
...
private function level4RideHollisTentacruels():void {
	...
	output("\n\nYou fold your arms impatiently across your [pc.chest] and nod, waiting for her to get her phallic tendrils in position.  It takes you completely off guard when a half-dozen tentacles snap across your front and another bundle sweeps your [pc.legs] out, spinning you to look face down at the ground, supported horizontally on a bed of taut, straining tentacles.  More loop over you, under you, and around you, cinching tight to your wrists and [pc.feet].  They pull them out while other prehensile plants swing down to support you, occasionally looping around you, their moist skin easily gliding over your " + pc.skinFurScales() + ".  All that, and she's not even fucking you yet!");
	...
	if (!pc.isLactating()) output("droplets of [pc.milk] dripping from your nipples.  When did you start lactating?");
	else output("steady streams of [pc.milk] erupting from your nipples.  When did you start lactating this hard?");
	...
	if (pc.legCount < 2) output("you'd be walking bowlegged for a week, if you had legs.");
	else output("you'll be walking bowlegged for a week!");
...
private function eatHolliFruit():void {
	clearOutput();
	output("You reach up into Holli's branches and pluck ");
	if (flags["COC.HOLLI_FRUIT"] == 1) output("the fruit");
	else output("one of the fruits");
	output(".  ");
	if (flags["COC.HOLLI_FRUIT_EXPLAINED"] == 0) {
		output("Holli smiles over at you and asks, \"<i>Enjoying the fruits of our union?</i>\"");
		...
		flags["COC.HOLLI_FRUIT_EXPLAINED"]++;
	}
	output("Biting into it, sweet juices seem to explode from the flesh, dribbling down your chin.  It tastes like a dessert and you chow down, happily munching away.  In no time flat, you're down to just a core.  You toss it and wipe your [pc.face] clean, then burp.  Damn, that was good! ");
	//pc.refillHunger(25);
	//TF CHANCES
	if (rand(2) == 0 && pc.cockTotal() > pc.cockTotal(GLOBAL.TYPE_TENTACLE)) {
		var choices:Array = [];
		temp = 0;
		while(temp < pc.cockTotal()) {
			if (pc.cocks[temp].cType != GLOBAL.TYPE_TENTACLE && pc.cockTypeUnlocked(temp, GLOBAL.TYPE_TENTACLE)) choices[choices.length] = temp;
			temp++;
		}
		temp = choices[rand(choices.length)];
		output("\n\nYour " + num2Ordinal(temp + 1) + " penis itches, and you idly scratch at it.  As you do, it begins to grow longer and longer, all the way to the ground before you realize something is wrong.  You pull open your [pc.gear] and look down, discovering your " + pc.cockDescript(temp) + " has become a tentacle!  As you watch, it shortens back up; it's colored green except for a purplish head, and evidence seems to suggest you can make it stretch out at will.  <b>You now have a");
		if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) > 0) output("nother");
		output(" tentacle-cock!</b>");
		pc.shiftCock(temp, GLOBAL.TYPE_TENTACLE);
		//dynStats("sen", 3, "lus", 10);
		pc.lust(10);
	}
	
	flags["COC.HOLLI_FRUIT"]--;
	pc.cor(1);
	pc.slowStatGain("p", 1);
	//if (pc.str < 50) dynStats("tou", 1);
...
private function askHolliToWatch():void {
	clearOutput();
	if (int(flags["COC.HOLLI_DEFENSE_ON"]) == 0) {
		output("You ask Holli if she would mind using her tentacles to help guard the camp at night.  She rolls her eyes and taunts, \"<i>I don't know; maybe if you get down on your knees and beg me.  ...And I mean </i>really<i> beg, with plenty of whimpering.  Lick my roots too.  After all, I should get something out of this.</i>\"");
		...
		if (flags["COC.THREATENED_HOLLI"] > 0) output("<b>  You could always just slap the cunt.</b>");
		...
		if (int(flags["COC.THREATENED_HOLLI"]) == 0) addButton(1, "Threaten", threatenHolli);
		else addButton(1, "Slap Her", slapDatHo);
		addButton(2, "Assert Self", domUpSomeHolli);
		addButton(14, "Back", treeMenu);
	}
	//Stop Guarding (edited)(C)
	else {
		output("You tell Holli you'd like her to stop watching at night.  She laughs, \"<i>You just want a chance to come back and beg some more, don't you?  I thought you were supposed to be tough shit, not a simpering little submissive.</i>\"");
		flags["COC.HOLLI_DEFENSE_ON"] = 0;
		addNextButton(treeMenu);
	}
}
...
private function begHolli4Watches():void {
	clearOutput();
	output("You swallow your pride and drop down prostrate on the ground.  Sighing, you grudgingly say, \"<i>Please, Holli, guard my camp at night.  I don't want to be raped by monsters.</i>\"");
...
private function domUpSomeHolli():void {
	...
	if(pc.isGoo()) domPowah *= 0.75;
	else if(pc.isTaur()) domPowah *= 1.75 + ((pc.legCount - 4) * 0.125);
	else if(pc.isNaga()) domPowah *= 1.5 + ((pc.legCount - 1) * 0.125);
	else if (pc.isDrider()) domPowah *= 1.25 + (pc.legCount * 0.05);
	
	if (isNineTails(pc)) domPowah += 5;
	if (pc.hasHorns()) domPowah += 3;
	if (pc.cor() > 66) domPowah += 2;
	domPowah += pc.level;
	processTime(5);
	//{fail} 
	if (domPowah < 20) {
...
private function askBrokenHolliToGuard():void {
	clearOutput();
	output("Holli bows her head when you mention her guarding camp.  She asks, \"<i>Shall I keep watch for foes to drain or let them into camp for you to have the pleasure of dealing with, [pc.name]?  <b>I've been ");
	if (flags["COC.HOLLI_DEFENSE_ON"] == 1) output("keeping watch");
	else output("sleeping, instead");
	output(".</i>\"");
	
	//[Guard] [Don't Guard] [Back]
	clearMenu();
	if (flags["COC.HOLLI_DEFENSE_ON"] == 1) addButton(1, "Don't Guard", toggleBrokenHolliGuard);
...
private function toggleBrokenHolliGuard():void {
	clearOutput();
	if (int(flags["COC.HOLLI_DEFENSE_ON"]) == 0) {
		output("\"<i>All right, I'll get to have some fun then,</i>\" Holli quips while rubbing her palms together in anticipation.");
		output("\n\nYou suppose you can sleep better with your pet demoness taking on the lesser whelps of this land.");
		flags["COC.HOLLI_DEFENSE_ON"] = 1;
	}
	//Guard Off
	else {
		output("\"<i>All right, they're yours to smite or play with,</i>\" Holli says with a wink.  You give her a glare, to which she responds with a shrug.  \"<i>What?  I know how it is!  You mortals can't fight worth a damn when you're sleeping.</i>\"");
...
private function holliGetsDickDommed():void {
	clearOutput();
	output("Tossing your [pc.gear] aside, you expose your [pc.multiCocks].  Holli's eyes fixate on [pc.oneCock], her attention rapturously contained by a single phallus.  You grip your maleness in hand and bounce it up and down into your other palm.  The solid slaps of genital impacts echo off the surrounding rocks, each sending a pleasant tingle of pleasure through the root of your manhood.  Before long, you're completely erect and throbbingly hard, bouncing unbidden with each and every thrum of your heart.");
	...
	output("\n\nHanging out of her tree, Holli reaches for [pc.oneCock], but stops just short.  \"<i>Can I pleasure it, please?</i>\"  She's looking up at you hopefully, like a kid hoping for a treat.");
	output("\n\nSo long as she continues to be good, you don't mind letting her have a few licks.  You nod to the black-eyed beauty and rock forward, butting your [pc.cockHead " + pc.biggestCockIndex() + "] right into her soft lips.  Holli squeaks in surprise and grabs hold, firmly gripping your [pc.cock " + pc.biggestCockIndex() + "] in her capable fingers.  She makes to kiss it, but you pump yourself forward again.  The first few inches sink into her mouth before she can react.  This time, all she manages is a muffled groan, lips thoroughly snared with dick.  Her distraught vocalizations die down as she begins to accept her role, gently sliding your [pc.cock " + pc.biggestCockIndex() + "] down to take in as much as she can");
	if (pc.biggestCockVolume() > cockVolume(80)) output(", jaw stretching obscenely");
	output(".");
	...
	if (pc.biggestCockVolume() <= cockVolume(80)) {
		output(", swallowed to the ");
		if (pc.balls > 0) output("[pc.balls]");
		else output("[pc.sheath]");
		output(".");
	}
	else output(", swallowed as deep as Holli's distended neck can handle.");
	output("  Gurgling, the demonic dryad flutters her lengthy eyelashes and looks up at your eyes, twinkling with happiness at her servitude.  You wiggle your crotch around, and it twists your [pc.cock " + pc.biggestCockIndex() + "] around her mouth and throat.  The tainted tree's tongue massages the underside of your length, embracing you with subtle moisture and frictionless pressure.");
	output("\n\nBubbling sexual pleasure begins to build up inside you, making your ");
	if (pc.balls > 0) output("[pc.sack] feel taut and ready to burst");
	else output("body feel strung tight like an over-tuned guitar");
	output(".  Holli pulls back, inciting a snarl of displeasure to vibrate from you, but as soon as she's withdrawn, her hands clasp onto the slick shaft and begin to pump it with vigorous strokes.  Her mouth settles back into place around your [pc.cockHead " + pc.biggestCockIndex() + "], circling it with her tongue while she bobs up and down on it, snaring your most sensitive place with overwhelming sensations.");
	output("\n\nYou grab hold of the hungry cock-sucker by her neck and push her back, up into her tree, releasing your [pc.cock " + pc.biggestCockIndex() + "] before it can blow.  It's a close thing - you're rigid and bobbing, muscles clenching involuntary right on the edge.  Holli makes a choking protestation as she's forced into place but doesn't struggle, fearful of your wrath.  You let the hungry slut go, unharmed but chastised by the rough treatment.  Your cum is destined for her twat, not to be wasted in her slutty noise-hole.");
	
	output("\n\nStepping up, you ram your rigid phallus deep into the slick flower at Holli's waist.");
	if (pc.biggestCockVolume() > cockVolume(80)) output("  It bends slightly as the immense girth is forced inside her, aiming down, deep inside the trunk.");
	output("  Holli flops into your [pc.chest], twitching weakly and breathing heavily.  \"<i>Fffuck, so good!</i>\" she pants, nuzzling her cheek into your collarbone while her hands play across your shoulders.  You ruthlessly mount her flowery mound with fast motions.  Juices squirt out around your member at the apex of every stroke, raining down over the surrounding ground.  You grab Holli's deep green hair and pull her back.  Her face is wrecked by bliss, her mouth and eyes moving in uncoordinated twitches.");
	output("\n\nYou kiss your planty pet forcefully, and she yields to your impressive dominance, putty in your hands.  Her body quavers against you, and then, she's moaning into your lips, forgetting the kiss as climax wracks her frail, human-like half.  Nectar drips down your [pc.legs] in a steady drizzle, sweet, lubricating juices that fill the air with a flowery, feminine musk.  You pull away from her mouth's incessant utterances and watch her cum, held in your arms.  The pleasant caresses of the dryad's inner walls squeeze down with pliant pressure, rolling over your [pc.cock " + pc.biggestCockIndex() + "] in slow waves, hungrily sucking at you and ready for sperm.");
	output("\n\nThe abruptness of your orgasm startles you.  One moment there's the sanguine ecstasy of the surrounding fuck-flesh, and the next, there's an explosive volcano of lust boiling out of your [pc.cocks], geysering fountains of fluid.");
	if (pc.cockTotal() > 1) {
		output("  Excess jism spurts onto Holli's jiggling udders from below, spotting the viridian mounds with white.");
		if (pc.cumQ() >= 500) output("  The constant sprays turn them white in little time, dripping from the peaks of her nipples along with her sticky sap.");
	}
	if (pc.cumQ() > 1000) output("  A trickle of sperm washes out of the demoness's entrance, somehow escaping the heavy suction.");
	if (pc.cumQ() > 2000) output("  A moment later, that trickle turns into a river.");
	if (pc.cumQ() > 3000) output("  You swear that her trunk must be absolutely stuffed with spunk.");
	if (pc.cumQ() > 5000) output("  A spooge-spout sprays out as the pressure goes through the roof, confirming your suspicions in the messiest way possible.");
	output("  Holli's tentacles cum with her, spraying her seed through her canopy in aimless ropes.  Most of it sticks to her branches and leaves, but small amounts of it trickle down to spatter the ground.  One of them lands on the back of your hand.  You frown and push it up against the tree-woman's mouth.  She licks you clean without question, still dazed from her orgasm.");
	output("\n\nRemoving yourself from the gaping flowerpot, you admire the fruits of your labors.  Holli breathily says, \"<i>Th-thank you, [pc.name]...  So good...</i>\"");
	output("\n\nYou crook a finger and point down at the slop of sperm and nectar glazing your [pc.cock " + pc.biggestCockIndex() + "].  Getting the idea, she leans back out and opens wide.  You get a thorough cock polishing from the much meeker plant-woman.  She swallows it all and withdraws, offering, \"<i>If you ever want another round... my flower is yours.</i>\"");
	processTime(25 + rand(10));
	pc.orgasm();
...
private function fullOnTentacleTasticGangBangForHolli():void {
	...
	if (silly) output(", inflatable-arm, flailing tube-cocks");
	output(".  Below the orgy of arboreal affection, Holli's slick petals have spread wide.  You can actually see her interior twitching voraciously.  It opens and closes with pulsing hunger, vibrantly engorged.  As you watch, the fourth of your members lances forward of its own volition, spearing straight for that welcoming hole with no guidance from you.  It hits like a missile.  Two feet of that cock is gone in a moment, swallowed deep in the cunt-flower's bottomless depths.  A gush of girl-cum spurts out around the intruding member, and you can feel the muffled moan vibrate through the mouth-bound cock as she gives in to the four-way fucking.");
	...
	output("\n\nA tremendous upwelling of excitement surges through your meager frame, and you feel insignificant in the face of it, like your body is simply too small and simple for you to endure such agonizingly intense pleasure.  Your penises thrust and move of their own accord as your bliss builds, eyes rolled back into your head.  Holli watches you with a dopey, pleasure-dazed look in her eyes as she's thoroughly fucked, smiling faintly when she sees your [pc.hips] begin to shake as orgasm overtakes you.");
	if (pc.balls > 0) output("  Your [pc.balls] draw up tight to your groin and squeeze, rhythmically discharging their heavy loads.");
	...
	if (pc.cumQ() > 500) output(", and the next");
	if (pc.cumQ() > 1400) output("... and the next");
	output(".  Another jizz-load reaches one of your crowns, and you fire a goopy load into Holli's hair from one of the hand-bound shafts.  The other side follows, spraying a torrent of milky creme onto her face, practically gluing the dryad's eyes shut with semen.  The twin-tit-fucking tendrils ejaculate next.  The first one is barely seen, as it's lodged deep between her gorgeous g-cups, but you can feel it.  The warmth floods her breasts with subtle, wet heat, making your other tentacle cum all that much harder.  It spurts hard enough that the load deflects off her neck and chin and right back down onto the top of her tits, glazing both.");
	output("\n\nHolli climaxes again when your load passes into her vagina, deforming the petals slightly as it goes.  Her cunt-tentacles are similarly stretched by the force of your orgasm, distended by spermy knots that erupt inside them.  Jizz and girl-cum drip from the numerous twats, all five of them.  Those pussies aren't simply idly filled either.  They hungrily massage you throughout their own moments of bliss, actually providing a modicum of suction to draw even more spunk from your [pc.balls].  You roar in triumph and thrust all ten at once, pushing them as deep into their chosen locations as you ride out of the ecstasy, spurting and shaking.");
...
private function vaginalDomHollisTentacruels():void {
	...
	output("\n\nHolli nods again, more enthusiastically this time.  The tentacle lodged in your [pc.vagina] retracts slightly, but only for a moment.  It returns to your twat with a vengeance, plunging up hard.  You gasp, \"<i>O-oh!  Go... go slower!</i>\"");
	...
	output("\n\n\"<i>I told you, slower,</i>\" you say, punctuating the sentence with another heavy cock-slap.  Holli works her jaw in irritation, but lowers her eyes against the inferno in your steely gaze.  \"<i>Better.  Now, kiss and make up, you two.</i>\"  You cram the tender cock right back into her noise hole and command, \"<i>Giddyup, little pony - you've still got a [pc.master] to serve.</i>\"");
	output("\n\nThe undulations down below resume, this time at a pleasantly languid pace.  You sigh in contentment, finally able to enjoy the slow fuck.  You grope at your [pc.fullChest]");
	if (!pc.hasFuckableNipples() && pc.lactationQ() < 50) output(" and pay special attention to your [pc.nipples], tugging them in time with every sluggish thrust in your slit.");
	else if (pc.lactationQ() >= 50) {
		output(" and sigh at the ");
		if (pc.lactationQ() < 150) output("squirts");
		else if (pc.lactationQ() < 800) output("sprays");
		else if (pc.lactationQ() < 2000) output("outpouring");
		else output("thick fountains");
		output(" of [pc.milk] that escape with every tug, timed to match the sluggish thrusts at your slit.");
	}
	else output(" and sink your fingers into your [pc.nipples], fucking them to the sluggish tempo of the thrusting tentacle.");
	output("  A loud groan escapes your mouth, and you rock your [pc.hips] against the undulating tendril.  Your body's eagerness for more pleasure is unparalleled, but you control yourself as efficiently as you control the dryad.");
	...
	output("\n\nMoving quicker now, Holli's tentacle begins to pound you faster, pistoning with greater and greater enthusiasm.  You let it - you're achingly wet by this point, and a fast, fervent fuck is exactly what your [pc.vagina] needs.  \"<i>Harder!</i>\" you urge, and your obedient tree-slut obeys.  The thick-tipped tendril smacks into your cervix, filling your channel completely with its thick length.  Its shaft arches up, grinding over your [pc.clit]");
	if (pc.clitLength >= 3.5) output(" for a moment before wrapping around and jerking on it");
	output(".");
	...
	output("\n\nYou whisper into her ear, \"<i>Go on, slut, cum.</i>\"  You stroke her faster, so fast your arms are starting to burn from the effort.  \"<i>Cum for [pc.master], go on.  Spooge all over your pretty face and hair.  Pump all that nasty cum straight down your throat.</i>\"  Holli's eyes begin to roll back, and you throatily purr, \"<i>Bust a nut in my cunt, little slut.</i>\"");
	...
	output("\n\nA warm wave washes into your womb at the same time, easily injected past your cervix's unresisting entrance.  Some of it sputters out to the sides to further lubricate your tunnel.  It sets you off like a bomb, and you fall back onto the ground, trying to lift your [pc.hips] higher.  With gravity assisting, the jism floods your uterus even faster.  The squeezing tightness of your involuntary muscle spasms don't help.  If anything, your body is holding even more of the spooge inside, forcing you to take another few blasts of jizz straight into your well-packed reproductive system.");
	output("\n\nThe still spurting cock begins to withdraw, dragging its over-sized head through your sensitive, quavering gates.  It bumps your tender clit on the way, soaking it with a fresh flow of semen.  Then it ascends into the canopy, leaving your [pc.vagina] empty of everything but the gallon of creme it left behind.  You hum in satisfaction and try to stand.  Your [pc.legs] wobble and give out, dropping you back ");
	if (!pc.isSquirter()) output("onto the ground");
	else output("into the leavings of your squirting orgasm");
...
private function threatenHolli():void {
	clearOutput();
	output("You snarl and threaten to burn the barked bitch to the ground if she won't make herself useful after you ask politely.");
	output("\n\n\"<i>You threaten me with fire?!  Now?  [pc.name], you could have uprooted my sapling and tossed it in the fire.  You could have made a bonfire to char my flower to cinders.  You could even have called upon magic to torch my tree while it was still young.  Now?  My bark is stronger than iron and flush with the power of an invigorated goddess.  You had opportunities to rid yourself of me before Marae had invested this much time and energy into your gift.  Trying to destroy such a personal present now would draw her attention... and her ire.</i>\"");
	
	output("\n\nThat is a compelling point.  ");
	if (pc.level < 30) output("Mighty as you are, Marae is likely still beyond your power.  Perhaps you should just ignore this demon for the time being.");
	else output("Marae is likely beyond your power yet, but... you feel like she might not pressure you too hard if you can make a suitably credible threat.");
	...
	if ((flags["COC.JOJO_IN_CAMP"] == 1 && flags["COC.JOJO_NIGHT_WATCH"] == 1) || flags["COC.KIHA_CAMP_WATCH"] == 1 && followerKiha() || flags["COC.DEFENSE_CANOPY"] == 1 || flags["COC.ANEMONE_WATCH"] > 0) {
...
private function cutHerDown():void {
	clearOutput();
	output("\"<i>Are you serious?  You really want to be broken that badly, my little toy?</i>\"  The demon folds her arms over her hefty chest, her earlier, suggestive posture completely absent now.  \"<i>You know my bark is as strong as any armor, right?  That Marae herself - the land itself - bolsters me?  Take that back right now, and I'll let you return to my good graces... if you put your face in the dirt and plead for the duty to pleasure me whenever I want.</i>\"");
	
	//(if PC weapon = Large Axe)
	if (pc.meleeWeapon.longName.indexOf("axe") != -1) {
...
private function callDatJojo():void {
	...
	var hol:CoCHolli = holli;
	hol.createStatusEffect("JojoIsAssisting");
	CombatManager.newGroundCombat();
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(hol);
	CombatManager.victoryScene(defeatHolli);
	CombatManager.lossScene(enjoyYourBadEndBIYAAAATCH);
	CombatManager.displayLocation("HOLLI");
	CombatManager.beginCombat();
...
internal function defeatHolli():void {
	...
	flags["COC.FUCK_FLOWER_KILLED"] = 1;
	//Win without Jojo -Z
	if (!enemy.hasStatusEffect("JojoIsAssisting")) {
		output("Weary, the succubus godling folds into her tree, unwilling to allow you the sight of her defeat.  With her energy suppressed, you're free to ");
		if (pc.meleeWeapon.longName.indexOf("axe") == -1) output("fetch an axe and ");
		output("hack at her trunk, laboriously peeling bark and making wedge-shaped cuts in it as you try to fell the abomination.  It takes nearly eight hours, but eventually the gnarled tree topples.  No sooner does it fall than it shrivels and turns to ash.");
		processTime(7 * 60 + rand(2 * 60));
	}
	//Win with Jojo -Z
	else {
		output("The fire-blackened mouse cheers at you as the succubus groans and retreats completely into her bark.  \"<i>Well done, [pc.name]!  Help me carry tinder and we can burn this abomination to the ground!</i>\"");
	...
	pc.createKeyItem("Holli's Ashes", 0, 0, 0, 0);
...
private function recruitJojoToCamp():void {
	clearOutput();
	output("Jojo nods eagerly at you when you invite him to share your camp.  \"<i>Excellent idea.  If you'll excuse me, I wish to attempt to purify the spot where the creature grew, and then I will fetch my things.</i>\"");
...
private function dontRecruitJojoToCamp():void {
	clearOutput();
	output("You hold your tongue, allowing Jojo to make his way back to the forest.");
	output("\n\n(Key Item Gained: Holli's Ashes)");
	pc.createKeyItem("Holli's Ashes",0,0,0,0);
...
internal function enjoyYourBadEndBIYAAAATCH():void {
	clearOutput();
	output("Screeching in triumph, the demon holds your defeated form aloft and begins to shake you.  ");
	if (enemy.hasStatusEffect("JojoIsAssisting")) {
		output("Jojo drops the firewood he's carrying and attempts to strike her, but she swats him aside brutally with a branch, leaving him clutching his stomach and retching as he stumbles away; she returns her attention to you, completely fixated.  Her oozing sap runs in streams as she heals the damage from the dwindling flames.  ");
	}
	output("\"<i>Oh, my little meat...</i>\" Holli coos dangerously.  \"<i>Was I not clear enough before?  You exist to worship me.  If you won't give your mind to me, I'll have your body, instead.</i>\"  A floral tendril descends from her upper branches, slipping into your mouth and beginning to drool a nectar down your throat.  Forced to drink or drown, you gulp unhappily, feeling increasingly light-headed with each swallow.");
	
	//FOOOOARK - choose with RNG for herms
	//branch of penis-having
	if (pc.hasCock() || (rand(2) == 0 && pc.isHerm())) {
		output("\n\nShe strips off your [pc.gear] haphazardly, rending and shredding with vicious tugs and leaving deep lines where the clasps bite into your " + pc.skinFurScales() + " before parting.  [pc.oneCock] exposed, she lowers her mouth to the [pc.cockHead " + pc.biggestCockIndex() + "] and");
		if (pc.cocks[pc.biggestCockIndex()].thickness() > 6) output(", stretching her jaw wide open in a way that makes you wince,");
		output(" engulfs you.  Holli's tongue plays over your slit and she looks up at you for just a moment to be sure you're watching.");
		
		output("\n\nOnce she's captured your attention, she smiles disarmingly around your shaft... until you feel a prick at the end.  A small stem slides into your cockhole, and you freeze in panic.  Holli's eyes dance with amusement at your discomfort, and she begins to work her lips up and down the [pc.cock " + pc.biggestCockIndex() + "], giggling into it every time the insertion scrapes your inside and you flinch.  It's hardly more than a few minutes of this internal and external stimulation before your doped-up mind is at the brink of climax, and your dick twitches eagerly.  Holli responds to this, halting her current plunge and watching you again.");
		...
		if (pc.biggestCockLength() > 24) output("her neck ripples and extends grotesquely, and her head begins to migrate down all " + num2Text(Math.round(pc.biggestCockLength())) + " of your prick with alarming speed, pushing the stem inside along with it.  ");
		else output("she sinks her head down to the base of your prick in one quick thrust, bringing the stem with it.  ");
		output("The sensation of having your entire [pc.cock " + pc.biggestCockIndex() + "] penetrated and sucked into her hot, wet, clinging throat pushes you back toward the edge and over, and it literally becomes a race for Holli to reach bottom before your nerves fire.  Her wet lips impact your crotch with a lewd splatter, but her tongue keeps going deep into your body, stopping only once it reaches your prostate.  A small bulge fires down the thin stem, forcing through your slit and into the shaft to lodge in your gland.  Holli's mouth and tongue withdraw once it is in place, and she grabs your cock in her hand, masturbating you vigorously.");
		
		output("\n\nIt's too much; your climax comes in a flood");
		if (pc.balls > 0) output(", and your [pc.balls] draw close to your body to unload");
		output(".  Your eyes roll as you flex and flex, but nothing comes out.  As you conclude your dry orgasm, you begin to feel a pressure on your prostate.");
		
		output("\n\nHolli smiles.  \"<i>That's right, my toy... water my little seed.  Let it grow.</i>\"  The swelling increases, and you shiver as the pressure on your prostate forces out another squirt of fluid... which also never sees the light of day.  Each miniature orgasm causes the swelling to increase, which provokes yet another.  As you reach the conceivable limit of what your prostate could produce, hoping desperately for a respite, you feel a pincushion's worth of jabs in it, followed by a suffusion of fluttering warmth and growth, and then your agonizingly pleasurable squirts begin again.  You can actually feel a pressure finally begin to ascend your dick, and anticipate the release coming.  Closer and closer it creeps, right up to the tip, until what must be the first drop is peeking out.  You look down, and Holli's eyes join yours, to behold... a small green nub.  She smiles, leans down, and gingerly kisses it; a visible vibration travels back down your body and your prostate feels like it's overheating.  The sprout explodes from the end of your [pc.cock " + pc.biggestCockIndex() + "], a fleshy tentacle that lashes in the air, swaying your shaft wildly and drooling your cum from an open slit in its end.  More buds can be felt inside now, ");
		if (pc.balls > 0) output("migrating the other direction into your [pc.balls], entangling and infiltrating them, filling them with the same warmth, then pressing out against your sack.  ");
		else output("exploring the depths of your body and curling back against the surface of your " + pc.skin() + ".  ");
		output("Your stomach turns as the sprouts push their way through, crawling from your crotch down the outside of your [pc.legs] to root in the dry earth.  The shoots thicken into fibrous roots, forming a trunk that supports you and pushes you closer to Holli and lower to the ground, until your writhing, tentacle-tipped prick is lined up with her pussy flower and within her easy reach.");
		
		output("\n\n\"<i>Foolish little meat,</i>\" the demon smiles down at you, \"<i>I <b>am</b> Mareth.  There is a part of me in everything born in this land.  And now, in you as well.</i>\"  Your tendril reaches for Holli's nethers at her unspoken command, dragging your [pc.cock " + pc.biggestCockIndex() + "] into her wet, lush depths as she wraps her arms around you...");
	...
	//branch of vagina-having
		...
		output("\n\nShe pulls impatiently at your [pc.gear], ripping the fastenings and tearing it off of you to expose your crotch.  With a caress between your legs, she murmurs, \"<i>Oh, my pet, we are going to plant something beautiful in this little furrow of yours.</i>\"  Raising you above her head");
		if (pc.isTaur()) output(" and turning you around");
		output(", the demon buries her face in your [pc.vagina], penetrating it with her wet, flexible tongue.");
		...
		if (pc.biggestTitSize() > 1) output("wrapping around and under your breasts to keep them permanently jutting out in a lurid display");
		else output("tracing a lattice of slender, criss-crossing ivy over your chest that hangs just far enough over your [pc.nipples] that a stiff breeze will drag it into them");
		output(", then continuing over your back and up your neck.  The tendrils thicken and multiply, occluding your forearms and lower [pc.legs] in a casing of young bark and leaf buds, and Holli's own release you as yours take over the duty of holding you in place to receive whatever pleasure she forces upon you.  You're left in a salacious pose when the growth subsides, suspended several feet off the ground with your arms held over your head, your back arched in a memorial of your incredible orgasm, ");
		if (pc.biggestTitSize() > 1) output("your breasts cupped and pressed together by vines, ");
		if (!pc.isNaga() && !pc.isGoo()) output("your thighs spread wide open, ");
		output("and your nectar-drooling flower-sex exposed with your [pc.clit]");
		if (pc.hasCock()) output(" and [pc.eachCock]");
		output(" bobbing in a state of permanent semi-arousal.");
		...
		//dynStats("lib", 5, "sen", 20);
		pc.slowStatGain("l", 5);
		output("\n\nShe pulls your [pc.gear] free without care, breaking the fastenings in her haste.  A raised eyebrow meets your bare crotch, but Holli doesn't pause for long.  \"<i>My simple little toy... if it were so easy to keep out the corruption of this land, the demons would be overthrown tomorrow.</i>\"  Revising her plans, the demon turns you around and slides a finger between your asscheeks, bending close.  ");
		if (pc.ass.wetness() > 0) output("Betraying your depravity, the slick crack opens wide and your greedy, wet asshole practically welcomes the demon's entire hand, sucking it up and making you shiver with repressed delight.  \"<i>Oh, mother!  It seems as though this is not a new lesson to you, is it?  My little whore, will you beg and moan as I fill you?</i>\"");
		else if (pc.analCapacity() < cockVolume(30)) output("With some difficulty, she forces it into your pucker.  \"<i>I think we'll just use this as our inroads, instead.  It may be slow going, but the journey is half the fun.</i>\"");
		else output("Despite your clenching, your loose hole proves no impediment to the demon's exploration.  \"<i>Ah, how roomy you are!  I think I'll move right in.</i>\"");
		...
		//[(iff buttslut)
		if (pc.ass.wetness() > 0) output("Try as you might, you can't see far enough down your back to determine what the demon has done.  The question answers itself, though, when she presses a shaft almost as thick as her leg into your [pc.butt], nosing it into your wet, receptive asshole.  Despite yourself, you shiver in anticipation at being filled so completely.  ");
		else if (pc.analCapacity() < cockVolume(30)) output("Your jaw hits your collarbone as Holli angles the florid head of a long, narrow penis into your field of view.  Judging by how far it comes up her chest, it's almost a foot and half long, and it's drooling sticky sap all over your back.  With a twisted smile, the demon pushes it along your spine until it reaches your [pc.butt], then shoves it toward the hole.  ");
		else output("The demon presses up to your back, brushing her nipples against your skin and shoving her new growth between your thighs.  Looking down, you can see a fat, green shaft sticking from your crotch, glistening and dripping with sticky slime.  With another moan, Holli pulls it back from between your legs, then pushes apart your asscheeks and plunges it into your [pc.asshole].  ");
		output("The sap oozing from her member coats your passage, ");
		if (pc.ass.wetness() > 0) output("mingling with your own lubrication and ]easing her thrusts.  Holli's tempo picks up quickly, and soon you can feel her twitch inside you as she nears her climax.");
		
		output("\n\n\"<i>O-oh, my little toy, be excited,</i>\" she pants.  \"<i>I'm about to give you your purpose in this woh-world!</i>\"  With a shriek from the demon, Holli's shaft erupts and fills you with another wave of liquid warmth.  As she rests, panting, on your back, your [pc.asshole] itches with arousal; your own unreached orgasm hangs out of reach, maddening you.  Your demon lover pulls her rapidly-deflating tool from your backside, ");
		//[(buttslut or loose)
		if (pc.ass.wetness() > 0 || pc.analCapacity() > cockVolume(30)) output("spilling a wave of her deposited fluids down your [pc.legs].  ");
		else output("and your once-tight pucker reflexively squeezes shut as best it can, trapping all but a few trickles of Holli's deposit inside you.  ");
		output("The freshly-grown shaft between Holli's legs begins to shrivel, withering and browning like a cut flower.  With an expression of mild disdain, she pulls it from her body and casts it aside, where it crumbles to pieces.");
		
		output("\n\nThe fluids in your gut begin to mingle, sending a disquieting pinprick sensation through the skin");
		if (pc.hasFur() || pc.hasScales()) output(" under your " + pc.skinFurScales());
		output(".  Panic intensifies as the sensation quickly focuses on your crotch, and when the tendrils holding you push you away from the demon's tree, your curiosity gets the better of you.  You look over your shoulder again, trying to see what's happening.  As you strain to watch, you feel hundreds of tiny, sharp pinches and small vines begin to creep from your body, just below your ass.  The vines merge and thread into thick trunks as they get closer to the ground, pushing deep into the dirt and mooring you to the spot.  Arousal builds in you as they join the web of roots linking the demon Marae and Holli, as does a swelling at your crotch.  Another cluster of thin vines push free from your groin, growing upward in a lattice and lashing your arms into place, ");
		if (pc.biggestTitSize() < 1) output("folded atop your chest");
		else output("crooked at your sides with your hands resting atop your [pc.chest]");
		output(".  They continue past your face, reaching over your head to form a canopy similar to Holli's own.  Struggle as you may, when they're done growing you're left unable to budge and locked in a rather artful pose, head turned over your shoulder with your [pc.ass] thrust out");
		if (pc.biggestTitSize() >= 1) output(", your breasts exposed from the sides,");
		output(" and your [pc.legs] concealed below your buttcheeks by a thick tree trunk.");
...
private function holliPlaysWithPenisesBadEnd():void {
	...
	if (pc.biggestTitSize() < 1) output("masculine shape, it bears");
	else output("feminine shape, it nonetheless bears");
	output(pc.multiCockDescript() + ", standing erect and hard as iron; the brown-haired girl blushes and looks away.");
	...
	output("\n\nThe dryad gestures once again to her companion's [pc.cocks], causing the brown-haired girl's mouth to hang open a little.  \"<i>I can think of few better ways to demonstrate your vitality,</i>\" Holli coos.  \"<i>Do not feel indebted to us, but if you would share of yourself, we would be grateful.  And, in truth, is it not a hint of longing I see in your eyes?  Come, join with " + pc.mf("him","her") + " for a while and clear your head.</i>\"");
	if (pc.smallestCockVolume() > cockVolume(36)) output("  As the dryad speaks, the silent figure's [pc.cock " + pc.smallestCockIndex() + "] shudders and begins to shrink, dwindling to a size that would fit " + pc.mf("him","her") + " prospective lover.");
	...
	output("\n\n\"<i>I'm not sure...</i>\" the girl begins.  With a grin, Holli cuts her off by pushing her makeshift swing forward, impaling the Champion's pussy on the [pc.cock " + pc.smallestCockIndex() + "].  \"<i>Ahh!</i>\" she shrieks, as the branches begin to sway of their own accord in time with their master's pleasure.  A small trickle of blood seeps through the leaves, dripping onto the ground.");
	...
	output("\n\nBy the time the girl reaches her third orgasm, her partners are tense and near the breaking point; with an shudder and a stilling of the shaking branches, the silent one opens " + pc.mf("his", "her") + " mouth in mute ecstasy and ejaculates, ");
	if (pc.cocks.length > 1) output("spraying the girl's stomach and tits with waste seed from " + pc.mf("his", "her") + " unused manhood and ");
	output(" filling the Champion's pussy with so much semen it cascades from her stretched hole as Holli continues to pump.");

	output("\n\nThe dryad herself soon gives a cry and pushes her tits into the brown-haired girl's back as her climax comes.  A bulge ascends the green shaft, disappearing into the beat-up cunt of the Champion, and a trickle of golden sap leaks back down Holli's cock.  Pulling free, the dryad wipes her member off and folds it back into its bud; it shrinks and dwindles until nothing is left but her clitoris.  The silent one's branches lower the girl to the ground, " + pc.mf("his","her") + " always-erect prick");
	if (pc.cockTotal() > 1) output("s");
	output(" drooling aftershocks onto her breasts.");
...
private function girlsGetANiceManToBadEnd():void {
	...
	//[(if tits)
	if (pc.biggestTitSize() > 1) output("[pc.chest] cupped by vines and held out toward the young man, ");
	else output("chest crisscrossed with a mesh of fine vines, ");
	output("its legs elevated and spread apart to display a ");
	if (pc.balls > 0 && pc.ballDiameter() > 10 && pc.isHerm()) output(pc.multiCockDescript() + " with [pc.balls] swinging below.");
	//(if H and ballsize < 10)
	else if (pc.balls > 0 && pc.ballDiameter() <= 10 && pc.isHerm()) output(pc.multiCockDescript() + " and [pc.balls] that hang slightly over a [pc.vagina].");
	//(if H and noballs)
	else if (pc.isHerm()) output(pc.multiCockDescript() + " and a [pc.vagina].");
	else output("a wet, drooling cunt that practically begs to be filled.");
	output("  Though its eyes are covered, the figure's mouth works constantly, biting and licking its lips as if beset by otherworldly amounts of lust.  Holli beckons the youth toward this tree.");
	...
	if (pc.ballDiameter() > 10 && pc.balls > 0) output("lifting the hefty testicles to reveal a wet, eager pussy.");
	else output("spreading the labia and exposing the eager, clenching hole inside.");
	...
	if (pc.biggestTitSize() >= 1) {
		output("reaches up and grabs two handfuls of breast, passing his thumb over a nipple and watching the ");
		if (!pc.isHerm()) output("woman");
		else output("hermaphrodite");
		output("'s mouth open and close in bliss.  ");
	}
	else output("leans down and kisses the [woman/hermaphrodite]'s smooth chest, causing her to arch her back in pleasure.  ");
	if (pc.hasCock()) output("With a nervous glance downward, he slides one hand to her [pc.cock " + pc.smallestCockIndex() + "].  \"<i>It's not gay if it's a chick's,</i>\" he mumbles to himself, and then begins to stroke haltingly, playing his finger along the urethra and drawing a squirt of precum from his shivering partner.  ");
	else {
		output("His hand slides to the [pc.clit] above her wet fuck-hole; collecting some of the frothing lube from their wet sex, ");
		if (pc.clitLength < 6) output("he smears it over the little button and then begins to trace up and down the underside with a finger.  ");
		else output("he coats the swollen organ with it and begins to jerk it off valiantly, like a cock.  ");
		output("\"<i>Does that feel good?</i>\" he asks, upping the pace of his thrusts.  ");
	}
	output("As if in reply, the ");
	if (!pc.isHerm()) output("woman");
	else output("hermaphrodite");
	output(" in the tree arches her back and her [pc.vagina] clamps down on his tool");
	if (pc.hasCock()) {
		output("; ");
		if (pc.balls > 0) output("her balls draw closer to her body and ");
		output("her trembling dick");
		if (pc.cockTotal() == 1) output(" throbs");
		else output("s throb");
		output(" as she nears climax");
	}
	output(".");
	...
	if (pc.hasCock()) {
		...
		if (pc.cockTotal() > 1) output("s");
		output(" toward her face; ");
		if (pc.biggestCockVolume() < cockVolume(20)) {
			output("semen spurts and dribbles down her [pc.chest], pooling in the combined laps and mingling with the sticky lube ringing her vagina.  ");
			if (pc.cumQ() >= 500) output("So much so that by the time she finishes her climax, both her partner's legs and her trunk are covered in a sheen of milky white seed.  ");
		}
		else if (pc.biggestCockVolume() < cockVolume(80)) {
			output("her mouth is now full of cockhead, and she gulps as eagerly as she spurts, drooling lines of white from the corners of her lips.  ");
			//[(big skeet)]
			if (pc.cumQ() >= 500) output("The waves of seed don't stop coming, pouring into her and swelling her belly out until its round bulge pushes the cock away from her mouth and the remaining squirts fall on her [pc.chest].  ");
		}
		//else cock biggest > 80)
		else {
			output("her body is easily dwarfed by her male sex's massive size, and globs of sperm shoot into her canopy to splatter among her leaves and branches.  ");
			if (pc.cumQ() >= 1000) output("The overproductive hermaphrodite's body pushes out so much seed that the entire canopy soon turns white and drips constantly as though it were in a tropical rainforest.  ");
		}
	}
	//(vagina and no cock)
	else {
		output("\n\nA ");
		if (pc.wetness() < 3) output("gush");
		else if (pc.wetness() < 4) output("rush");
		else output("torrent");
		output(" of female orgasm pours from her pussy, and her mouth opens wide in pleasure.  The tree branches surrounding the young man seem to close in, pushing him toward her.  Getting the message, he leans in and kisses his lover; her tongue probes his mouth with abandon as her body shakes out its fit and her [pc.vagina] wrings his prick.  ");
	}
	output("Pushed past his limit by her lewd display, the young man groans, shudders, and begins to unload his pent-up cargo.  Falling on top of his partner's slick ");
	if (pc.cockTotal() == 1) output("cock");
	else if (pc.cockTotal() > 1) output("cocks");
	else output("chest");
	output(", he goes limp and slack-jawed as his dick squirts, filling her vagina.");
	...
	output("'s stomach.  \"<i>In fact, she seems to have enjoyed it so much that she would have me bestow a blessing on you.</i>\"");
...
private function holliAndGenderlessSittingInATree():void {
	...
	if (!pc.hasFur() && !pc.hasScales()) output(pc.skinTone + "-colored");
	else output("covered in " + pc.skinFurScales());
	output(" and wrapping its body around the tree in a way that makes determining its sex impossible.  No... it's not wrapping around the tree, but embedded in it up to the hips!");
	...
	output("\n\nHolli begins undressing her prospective lover.  \"<i>[pc.name] is a bit of a weird one.  It has no sex we could use.  Customarily, we can choose whether we want to manifest as male, female, or both... [pc.name] chose to be neither, and turned its back on the whole affair of sexual interplay - symbolically and literally.</i>\"");
	...
	if (pc.hasScales()) output("a quiver of arousal shakes its scales");
	else if (pc.hasFur()) output("its fur begins to prick up with arousal");
	else output("a blush of lust colors its " + pc.skinFurScales());
	output("...");
...
public function amilyComesBack():void {
	clearOutput();
	output("Amily arrives with her belongings over her shoulder and a smile on her face.  \"<i>I knew you'd do the right thing, [pc.name].  I'll get my nest set back up.</i>\"\n\n(<b>Amily has moved back in!  She can be found in the lovers tab.</b>)");
