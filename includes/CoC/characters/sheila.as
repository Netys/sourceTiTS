import classes.Characters.CoC.CoCScarredBladeAnimated;
import classes.Characters.CoC.CoCSheila;
...
public function showSheila():void {
	userInterface.showName("\nSHEILA");
}

public function get sheila():CoCSheila
{
	return new CoCSheila();
}

public function sheilaCombatStart():void {
	CombatManager.newGroundCombat();
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(sheila);
	CombatManager.victoryScene(flags["COC.SHEILA_DEMON"] == 1 ? beatUpDemonSheila : sheilaGotWhomped);
	CombatManager.lossScene(flags["COC.SHEILA_DEMON"] == 1 ? loseToSheila : getBeatUpBySheila);
	CombatManager.displayLocation("SHEILA");
	CombatManager.beginCombat();
}

private function sheilaPreg(reducedChance:Boolean = false):void {
	if(pc.virility() <= 0) return;
	if (flags["COC.INCUBATION_SHIELA"] != undefined) return;
	...
	if (rand(100) <= chance) {
		flags["COC.INCUBATION_SHIELA"] = timeAsStamp + 72 * 60; // 72 hours incubation time
	}
	trace("Sheila preg chance: " + chance + ", result: " + (flags["COC.INCUBATION_SHIELA"] != undefined));
}

public function sheilaCorruption(arg:Number = 0):Number {
	if (flags["COC.SHEILA_CORRUPTION"] == undefined) flags["COC.SHEILA_CORRUPTION"] = 30;
	
	//if (flags["COC.SHEILA_CORRUPTION"] + arg > 100) flags["COC.SHEILA_CORRUPTION"] = 100;
	//else 
	if (flags["COC.SHEILA_CORRUPTION"] + arg < 0) flags["COC.SHEILA_CORRUPTION"] = 0;
	else flags["COC.SHEILA_CORRUPTION"] += arg;
	return flags["COC.SHEILA_CORRUPTION"];
}

//Sheila's cup function!
public function sheilaCup():String {
	if (flags["COC.SHEILA_CORRUPTION"]  < 10) return "tiny, almost-absent";
	else if (flags["COC.SHEILA_CORRUPTION"] < 20) return "A-cup";
	else if (flags["COC.SHEILA_CORRUPTION"] < 30) return "B-cup";
	else if (flags["COC.SHEILA_CORRUPTION"] < 40) return "C-cup";
	else if (flags["COC.SHEILA_CORRUPTION"] < 50) return "D-cup";
	else if (flags["COC.SHEILA_CORRUPTION"] < 60) return "DD-cup";
	else if (flags["COC.SHEILA_CORRUPTION"] < 70) return "E-cup";
	else if (flags["COC.SHEILA_CORRUPTION"] < 80) return "EE-cup";
	else if (flags["COC.SHEILA_CORRUPTION"] < 90) return "F-cup";
	else if (flags["COC.SHEILA_CORRUPTION"] < 100) return "FF-cup";
	else return "G-cup";
}

public function sheilaEncounterRouter():void {
	trace("Sheila Encounter:");
	trace("Sheila Corruption: " + Math.round(sheilaCorruption() * 10) / 10 + " Sheila Preg: " + (flags["COC.INCUBATION_SHIELA"] - timeAsStamp) + " Sheila XP: " + flags["COC.SHEILA_XP"]);
	//UNFUCK FLAGS
	if (sheilaCorruption() >= 100 && flags["COC.SHEILA_DEMON"] != 1) flags["COC.SHEILA_DEMON"] = 1;
	if (flags["COC.SHEILA_DEMON"] == 1) flags["COC.SHEILA_XP"] = 4;
	
	//ACTUALLY PICK ENCOUNTER!
	if (flags["COC.SHEILA_DEMON"] != 1) {
		//PREG SHIT
		if (flags["COC.INCUBATION_SHIELA"] != undefined && flags["COC.INCUBATION_SHIELA"] < timeAsStamp && flags["COC.SHEILA_XP"] >= -2) {
			if (!(flags["COC.SHEILA_JOEYS"] > 0)) normalSheilaPregNotifNumberOne();
			else normalSheilaPregNotifREPEATEDED();
			return;
		}
		if (flags["COC.SHEILA_XP"] == undefined) sheilaFirstEncounter();
		else if (flags["COC.SHEILA_XP"] == -1 ) sheilaApology();
		//XP-2 Sheila's apology (sheila xp = -2 and demon sheila = 0):
		else if (flags["COC.SHEILA_XP"] == -2) sheilaIsSorrySheRapedYou();
		//XP-3 or XP-4 She-mad (Sheila XP <= -3 and demon sheila = 0):
		else if (flags["COC.SHEILA_XP"] == -3 || flags["COC.SHEILA_XP"] == -4) sheilaReallyMad();
		//XP1: Reconciliation encounter (sheila xp = 1 and demon sheila = 0):
		else if (flags["COC.SHEILA_XP"] == 1) sheilaReconcile();
		//XP2: Familiarizing (Sheila XP = 2; or Sheila XP = 3 AND time =/= 20:00 and demon sheila = 0):
		else if (flags["COC.SHEILA_XP"] == 2 || (flags["COC.SHEILA_XP"] == 3 && hours != 20)) sheilaGettingFamiliar();
		//XP3: Sexy Time (sheila xp = 3 AND time = 20:00 and demon sheila = 0):
		else if (flags["COC.SHEILA_XP"] == 3 && hours == 20) sheilaXPThreeSexyTime();
		//XP4: Fancy meeting you here (sheila xp >= 4 and demon sheila = 0):
		else if (flags["COC.SHEILA_XP"] == 4) fuckBuddySheilaMeeting();
		else {
			output("A BUG HAPPENED.  YOUR SHEILA_XP: " + flags["COC.SHEILA_XP"] + ". <b>Value should be between -3 to 4.</b>");
	...
	//DEMONS!
		...
		if (flags["COC.INCUBATION_SHIELA"] != undefined && flags["COC.INCUBATION_SHIELA"] <= timeAsStamp) demonImpChildren();
...
private function sheilaCorruptionUpdate():void {
	...
	else if (pc.cor() > sheilaCorruption()) {
		//Big change, dump ten.
		if (pc.cor() - sheilaCorruption() >= 10) {
			pc.cor( -10);
			sheilaCorruption(10);
		}
		else {
			...
			while(Math.round(pc.cor()) != Math.round(sheilaCorruption())) {
				pc.cor(-1);
				if (Math.round(pc.cor()) <= Math.round(sheilaCorruption())) {
				   break;
				}
				else sheilaCorruption(1);
			}
			pc.cor(Math.round(pc.cor()), true);
		}
	}
	else if (pc.cor() < sheilaCorruption()) {
		//Big Change!
		if (sheilaCorruption() - pc.cor() >= 10) {
			pc.cor(10);
			sheilaCorruption( -10);
		}
		else {
	...
	if (flags["COC.SHEILA_DEMON"] == 1) pc.cor(1);
}
...
private function sheila1stEncStayHidden():void {
	clearOutput();
	output("You regulate your breathing and hold yourself still to avoid rustling anything softly, waiting for the creature to finish its business and leave.");
...
private function sheila1ndEncLookCloserPtII():void {
	...
	//[(libido>=50)
	if (pc.slut() >= 50) output("  Noting you still checking out her body, she smirks and says, \"<i>Enjoying your lookie, then?  Up here, cobber,</i>\" causing you to snap your head up sheepishly.");
	output("  You nod at the strange woman and introduce yourself.");
	if (pc.short != "Sheila") output("  \"<i>[pc.name], eh?  My mates just call me Sheila.  Bunch of jokers, the bastards.</i>\"");
	else output("  \"<i>You too, eh?  Sheila's what my mates call me for a nickname.  Don't suppose you got yours from a bunch of adolescent-minded wankers, though.</i>\"");
	...
	//[(high demon score)
	if (pc.demonScore() >= 4) output("  I mean, take a squizz at yourself.  It's like you're not even trying to hide it; I've seen imps that look more like normal people.");
	output("  Forget it.</i>\"  You deny the accusation, but she continues to glare at you.");
...
private function sheila1ndEncLookCloserPtIILeave():void {
	clearOutput();
	output("\"<i>Good onya.  Maybe I'll see you around.</i>\"  With that, you take your leave of the strange woman.");
	//set sheila xp = 1
	flags["COC.SHEILA_XP"] = 1;
	clearMenu();
	addButton(0, "Next", function():*{ processTime(10 + rand(10)); mainGameMenu(); } );
}
...
private function sheila1ndEncLookCloserPtIITalk():void {
	...
	//[Leave][Fight]
	clearMenu();
	addButton(0, "Fight", sheilaCombatStart);
	addButton(1, "Leave", sheila1ndEncLookCloserPtIITalkLeave);
}

//[XP0 - Look Closer - Talk - Leave]
private function sheila1ndEncLookCloserPtIITalkLeave():void {
	clearOutput();
	output("Wordlessly, you get up and back away from the woman... though not without a dirty glance.  She follows you with her eyes until you judge yourself far enough away to turn your back to her.");
	//go to camp
	clearMenu();
	addButton(0, "Next", function():*{ processTime(10 + rand(10)); mainGameMenu(); } );
}
...
private function apologyAvoidSheila():void {
	clearOutput();
	output("You shake your head at the leery girl, and tell her you'd be just as happy as she would if you never saw each other again.");
...
private function apologySheilaApology():void {
	clearOutput();
	output("You scratch your " + pc.skinFurScales() + " absently behind an ear, not sure how to broach the topic of reconciliation when you feel like she was the aggressor, but mumble something about being sorry things started off on the wrong foot.");
...
private function apologySheilaSlapSult():void {
	clearOutput();
	output("You say nothing as she finishes with the snare and stands back up.");
	...
	output("\n\n\"<i>Fuck you, arsehole!</i>\" she counters, breaking in.  \"<i>I'm not about to stand here and take a bollocking from some mullet who rolls up and expects everyone " + pc.mf("he","she") + " meets to fall over " + pc.mf("his","her") + " charming self like " + pc.mf("he","she") + "'s the storybook hero" + pc.mf("","ine") + "!  If you want a girl who'll be your soulmate at the word 'hello' you can go look for sloppy seconds from one of those goblin slags!</i>\"");
	...
	flags["COC.SHEILA_XP"] = -3;
	addButton(0, "Next", sheilaCombatStart);
}

//[XP-1 - Nothing]
private function apologySheilaSayNothing():void {
	clearOutput();
	output("You decide to keep your own counsel for now, folding your arms defensively and frowning.");
...
private function sheilaIsSorryButLeaveMeAlone():void {
	clearOutput();
	output("You shake your head and turn away, dismissing Sheila with a pointed gesture.  \"<i>H-hey!</i>\" the girl calls angrily from behind you.  \"<i>Dammit, I said I was sorry, you bastard!  Screw you!  See if I talk to you again!</i>\"");
	//set sheilapreg = -1
	flags["COC.SHEILA_DISABLED"] = 1;
	clearMenu();
	addButton(0, "Next", function():*{ processTime(10 + rand(10)); mainGameMenu(); } );
...
private function sheilaIsSorryAndYouForgive():void {
	...
	if (flags["COC.SHEILA_JOEYS"] > 0) flags["COC.SHEILA_XP"] = 4;
	else flags["COC.SHEILA_XP"] = 1;
	clearMenu();
	addButton(0, "Next", function():*{ processTime(10 + rand(10)); mainGameMenu(); } );
}
...
private function sheilaPologyFight():void {
	clearOutput();
	output("Saying nothing, you [pc.readyMelee] and take a swing!  Sheila starts a bit and dodges just in time, then with a look of resignation, raises her fists as well.  \"<i>That'll be right, I guess.  For what it's worth, I really am sorry.</i>\"");
...
private function sheilaPologyCastArouse():void {
	...
	output("\n\nBetter and better.");
	applyDamage(new TypeCollection( { tease : 1 + pc.libido() / 5 } ), sheila, pc);
	//[Let Her][Leave]
	clearMenu();
...
private function castArouseAndLeaveSheila():void {
	clearOutput();
	output("You shake your head sadly.  \"<i>I'm sorry, I haven't the time right now.</i>\"");
	...
	flags["COC.SHEILA_CORRUPTION"] = 100;
...
private function castArouseAndLetHer():void {
	clearOutput();
	output("As soon as you nod, the girl bounds to her feet and jumps you, driving you slightly downward in her ardor.");
	//increase PC corruption +10, output loss scene consonant with player's sex, set sheila xp = 4
	pc.cor(10);
	flags["COC.SHEILA_XP"] = 4;
...
private function sheilaReallyMad():void {
	clearOutput();
	output("A rustle in the bush claims your attention, and you turn your head to see Sheila, standing up suddenly.  Her eyes narrow when she recognizes you, and she takes off running!");
	...
	if (flags["COC.SHEILA_CORRUPTION"] >= 90) output("It's so hard to remember myself anymore, and the taint you've put in my body goes crazy when you touch me.  Do you want me to get on my hands and knees?</i>\"  The woman lowers herself to the ground.  \"<i>I beg you.  ");
	output("Please, please leave me alone.");
	if (flags["COC.SHEILA_XP"] == -3) output("  I know you... you're not as bad as the rest of the monsters out here, so... please.");
	output("</i>\"");
	//[Stand][Leave]
	clearMenu();
...
private function sheilaReallyMadLeave():void {
	clearOutput();
	output("Huffing, you throw the animals down and turn away.  What a display.");
...
private function sheilaReallyMadStandGround():void {
	...
	//[(normal mode)]
	if (!silly) {
		output("You throw the animals behind you and fold your arms.");
		output("\n\nSheila shivers, then speaks.  \"<i>Alright, mate.  If that's the way it has to be.</i>\"  She raises her fists half-heartedly.");
	}
	//(silly mode)
	else {
		output("Throwing the bundle to the side dramatically, you call the name of your stand!  Sheila's eyes widen as your body surges with renewed energy, and she jumps back as a ghostly other self materializes next to you.");
...
private function sheilaReconcile():void {
	clearOutput();
	output("\"<i>Oy, it's you,</i>\" sings out a voice behind you.  \"<i>Back again?</i>\"");
...
private function sheilaReconcileDunWanna():void {
	clearOutput();
	output("Raising your hands in a palms-out gesture, you decline.  Sheila colors a bit, misinterpreting your reluctance.  \"<i>Hey, it's not like I'm cracking onto you or anything.  I was just looking for a reason to eat half a rabbit myself, is all.</i>\"");
...
private function sheilaReconcileKay():void {
	clearOutput();
	output("Some fresh meat does sound pretty tempting.  You nod at the woman and she climbs to her feet.  \"<i>Beaut!  Do me the favor and fossick about for some tinder.</i>\"");
	//--Next--
	clearMenu();
...
private function sheilaReconcileKay2():void {
	clearOutput();
	output("Soon enough you've got a firepit going in a small clearing.  \"<i>Ahh, classic bush theater,</i>\" Sheila says, admiring it.  The woman dextrously spits two liberated and skinned rabbit quarters on a long stick and twirls them over the lapping flame, and soon the air is full of the sizzle and pop of meat juices dripping on the hot stones and the simple tune your cook hums.  You stare at ");
	if (pc.slut() < 50) output("the beckoning hunks of meat");
	else output("her rising and falling breasts under the pretext of watching her work");
	output(", until she happens to look up and catch your eye.  \"<i>Sorry, no parsley,</i>\" she says, breaking into an obliviously happy smile.  Seems like her temperament preternaturally suits this lifestyle.");
	...
	//[(pc is kangaroo)
	if (pc.race() == "kangaroo-morph") output(" - except you, I suppose");
	output(".  We trade our goods in prearranged op shops and dead drops but avoid bringing outsiders into our own settlements, even when they're right corkers.</i>\"");
	...
	//[(lib >50)]
	if (pc.slut() >= 50) output(" like yourself - I saw you perving -");
	output(" make us a little hornier, and evil buggers make us a bit more evil.  Our bodies tend to change on their own to reflect how tainted we are... which is as much as saying there are a sad lot among us who flirt with demonic energy to up our sizes to suit our egos.  But versus the sexual appetite and limitless rapacity of the demons themselves...?</i>\"");
	...
	//set sheila xp to 2
	flags["COC.SHEILA_XP"] = 2;
	clearMenu();
	addButton(0, "Next", function():*{ processTime(45 + rand(10)); mainGameMenu(); } );
}

//XP2: Familiarizing (Sheila XP = 2; or Sheila XP = 3 AND time =/= 20:00 and demon sheila = 0):
private function sheilaGettingFamiliar():void {
	clearOutput();
	output("\"<i>Hey, it's [pc.name].</i>\"");
...
private function sheilaFriendlyNoQuestions():void {
	clearOutput();
	output("You decide to hold your tongue and simply mimic her pose as best you can, relaxing on the grass.  The sun is hot but its angle not insistent here, thanks to the shelter from a broad tree nearby.  Its soft rays almost reduce you to sleep over the next forty-five minutes before your companion speaks, rousing you.");
	
	output("\n\n\"<i>Didn't take ya for a quiet " + pc.mf("bloke", "sheila") + " when we met, [pc.name]... but I find you peaceful company.</i>\"");
	...
	flags["COC.SHEILA_XP"] = 3;
	clearMenu();
	addButton(0, "Next", function():*{ processTime(25 + rand(10)); mainGameMenu(); } );
}

//[XP2 - Come Here Often?]
private function sheilaFriendlyComeHereOften():void {
	clearOutput();
	output("You ask her why she enjoys being on the plains so much.");
...
private function sheilaFriendlyHerPeoples():void {
	clearOutput();
	output("You lie down on the grass yourself, balance the parcel on your stomach, and ask about her people.");
	...
	if (pc.isMasculine()) output("  She looks absently at your crotch.");
	...
	if (pc.slut() < 50) {
		...
		output("\n\n\"<i>Anyway,</i>\" she resumes, sobering up, \"<i>it's no more precarious than anyone else's situation as long as the demons control the skies and poison the world.</i>\"  She climbs to her feet and picks up the bundle of caught game.  \"<i>You're making me wanna check on my mob, [pc.name].  I'll catch you later.</i>\"");
	}
	//(lib >=50)
	else {
		...
		if (pc.hasCock()) output(", and you raise the kids at home?  Or put them with a nanny");
		output("?</i>\"");
		
		output("\n\n\"<i>Yeah, I suppose that'll work");
		if (pc.hasCock()) output(", and I'll turn the kids over to");
		output(" - no, I mean, ");
		if (!pc.hasCock()) output("if I decide");
		else output("I'm not ready for");
		output(" - er... fuck <b>you!</b></i>\"  This last exclamation comes in response to your pealing laughter.  \"<i>Wanker!</i>\"");
	...
	flags["COC.SHEILA_XP"] = 3;
	clearMenu();
	addButton(0, "Next", function():*{ processTime(35 + rand(10)); mainGameMenu(); } );
}
...
public function sheilaXPThreeSexyTime():void {
	clearOutput();
	output("You prowl through the twilit grasslands, lazily looking for anything interesting before the bleak, red moon brings on the night and its restrictions.  Nothing much catches your eye, except for a rounded shape cast in silhouette against the horizon.  Initially taking it for a rock, you're surprised when it shivers at the sound of your approach!  It unbends and raises a head, turning toward you and revealing the flushed face of the bush girl, Sheila.  Her eyes quickly change from terror to relief as they settle on you, and she cries out.  \"<i>[pc.name]!  Oh thank god; please help me!</i>\"");
	output("\n\n\"<i>What's wrong?</i>\"");
	...
	//[(lib < 50)]
	if (pc.slut() < 50) output("\n\nAn imp?  Really?  Your incredulity must show on your face, because Sheila squeezes harder.  \"<i>Please, [pc.name]!  Don't leave me alone... I don't want to be ruined by an imp just because he got lucky!</i>\"  You idly lay a hand on her head as you consider.");
	//(else lib >=50)
	else {
...
private function sheilaXPThreeSexyTimeDitchHer():void {
	clearOutput();
	output("You shake your head, pulling away from the woman and telling her she'll be fine.  \"<i>No!</i>\" she shrieks.  \"<i>You gotta help me!</i>\"");
...
private function sheilaXPThreeSexyTimeSex():void {
	clearOutput();
	output("Sheila's eyes widen with your smile, and she pulls back from you.  \"<i>That's, um...</i>\"");
...
private function sheilaXPThreeSexyTimePostSexLetHerBe():void {
	clearOutput();
	output("Pulling away from you, Sheila gathers her clothing and departs, looking satisfied.  The glance she gives you over her shoulder is even a little warmer than before.");
	//i had snoo-snoo!
	flags["COC.SHEILA_XP"] = 4;
	clearMenu();
...
private function shielaXPThreeSexyTimePostSexStay():void {
	clearOutput();
	flags["COC.SHEILA_XP"] = 4;
	output("You grab her tail, provoking a curious, but not discouraging, look from the woman who was planning to spend the night alone.  \"<i>What is it, [pc.name]?</i>\"");
	...
	output("\n\nYou lie with the woman, stroking her skin as she sighs and hums softly, for almost an hour.  Just when you feel like you should mention arranging a place to sleep and a watch order, Sheila's fingers begin to roam over you.  \"<i>[pc.name]...</i>\" she whispers, \"<i>I'm...</i>\"");
...
private function shielaXPThreeSexyTimePostSexStayII():void {
	clearOutput();
	//(has cockfit 32)
	var x:int = pc.cockThatFits(sheila.vaginalCapacity());
	if (pc.hasCock() && x >= 0) {
		output("Her hand slips down to your groin and, overcoming its trepidation, grabs your [pc.cock " + x + "] and begins to pump languidly, slowly stimulating blood back into it.  \"<i>I don't know what's come over me,</i>\" she says, giggling.  \"<i>Maybe I'm just greedy, but... I want you again.</i>\"  She pushes you");
		//[(horse)]
		if (pc.isTaur()) output(" and you playfully roll");
		output(" onto your back; Sheila slips her leg over you");
		if (pc.isTaur()) output("r own");
		output(", positioning herself atop your crotch and guiding your tip to her pussy.  It pokes through the labia slightly, welcomed with warm wetness.  \"<i>Is... this ok, [pc.name]?  Can I?</i>\"");
		...
		output("\n\nShe slumps against you and swivels, stirring herself with your tool.  \"<i>[pc.name], you're awful... making me beg for it,</i>\" Sheila says, kissing you");
		//[(not horse)]
		if (!pc.isTaur()) output(" just above one [pc.nipple]");
		else output(" on the thigh");
		output(".  \"<i>I'm gonna have to punish you for it, you dag.</i>\"  Excusing herself thusly, she sits up and begins to slide up and down your [pc.cockNounSimple " + x + "].  ");
		//[(not horse)]
		if (!pc.isTaur()) output("Your hands squeeze her hips almost of their own volition, helping her slam down with force, and her head rolls back on her neck.  ");
		output("Her ");
		//[(sheila corruption < 30)]
		if (flags["COC.SHEILA_CORRUPTION"] < 30) output("little breasts rise and fall with increasing frequency atop her ribcage as her pace picks up and her breaths come short and shallow.");
		else if (flags["COC.SHEILA_CORRUPTION"] > 60) output("huge tits jiggle magnificently as she rocks, nearly slapping her in the face as she plunges down and then falling back into place with waves of rippling flesh.");
		else output("breasts quiver invitingly as she pumps your cock, but before you can think of grabbing one yourself, both of her hands are squeezing and caressing them, drawing sighs from the woman as she toys with her erect nipples.");
		...
		//(horse)
		if (pc.isTaur()) output("barrel");
		else output("chest");
		output(" in her soft hair - but her hips keep pumping, incredibly.  ");
		//(pc corr >= 90 or > sheila corruption)
		if (pc.cor() >= 90 || pc.cor() > flags["COC.SHEILA_CORRUPTION"]) {
			output("  The tight, rippling muscles of her pussy clamp down on you, nearly holding your cock in place as she tries to ride you still but only manages to jerk your dick up and down and squeeze pre-cum from it.  \"<i>S-so full,</i>\" Sheila groans, \"<i>did you just get bigger in me?</i>\"  Her lungs inflate as she sucks in her breath, and her hard nipples push into your " + pc.skinFurScales() + ".");
		}
		else if (pc.cor() < flags["COC.SHEILA_CORRUPTION"]) output("  Her pussy clamps down around you, the rippling muscles so tight that she wouldn't even be able to pump if it weren't for the hot river of juice you feel around your tool.  \"<i>A-ah,</i>\" Sheila groans, \"<i>you thrust too!  Rail me silly!</i>\"  She tries to push against you with her hands, to return herself to an upright position, but can barely pull her chest apart from you - her nipples just narrowly recede from the surface of your " + pc.skinFurScales() + " as her efforts peak.");
		//(else if magically equal but < 90)
		else output("  Her pussy squirts and sucks alternately, trying to find purchase on your cock even as her pumping hips deny it; the sensation drives you wild.  \"<i>[pc.name], your old fella is amazing...</i>\"");
		output("  Your dick twitches and swells, as if to fulfill her words, and your pelvis rocks into Sheila's as you begin to ejaculate.  \"<i>No... pull out, or you'll get me pregnant,</i>\" she cries, when ");
		//[(single cox)
		if (pc.cockTotal() == 1) output("your dick twitches inside her");
		//(multicox)
		else output("she spies the first bead of [pc.cumNoun] exiting");
		if (pc.cockTotal() == 2) output(" your lonely, unused prick");
		if (pc.cockTotal() > 2) output(" each of your lonely, unused pricks");
		output(".");
		
		output("\n\n\"<i>You do it!  You're on top!</i>\" you retort, suppressing your climax for a scant few seconds with willpower alone.");
		
		output("\n\nSheila attempts to lift her still-spasming pussy off of your shaft, but wracked by her own orgasm, goes tense again.  \"<i>F-fuck it!  Can't!  Come inside me, [pc.name]!</i>\"  At the invitation, your walls break down, and your penis sprays your held-back semen into your lover");
		//[(multi)]
		if (pc.cockTotal() > 1) output(", to be joined shortly by a messy external eruption that covers her thighs and belly in blotches of gooey [pc.cumColor]");
		output(".");
		//[(big skeet)
		if (pc.cumQ() >= 1000) output("  It rolls out of you in waves, packing Sheila's cunt until she fills to capacity and the excess is forced out, sliming your own crotch.");
		...
		pc.orgasm();
		pc.slowStatGain("l", -2);
		if (pc.cor() >= 90 && sheilaCorruption() >= 90) {
			clearMenu();
		...
		/*if (cor() > sheilaCorruption()) {
			dynStats("cor", -5);
			sheilaCorruption(5);
		}
		else if (cor() < sheilaCorruption()) {
			dynStats("cor", 5);
			sheilaCorruption(-5);
		}*/
	}
	//(if no cockFit 32 and yes vag)
		...
		//[(horse)]
		if (pc.isTaur()) output("You turn around and straddle a leg, squatting to bring your [pc.vagina] atop her crotch.  \"<i>Push up,</i>\" you order, and Sheila obeys eagerly, twisting her body to connect her excited pussy with yours.");
		//(else if naga)
		else if (pc.isNaga()) output("You slide it under her and lift gently; Sheila plays along with a disquieted expression, rolling over and raising her pussy into the air, and you press your own against her butt as your tail slides between her legs.  \"<i>See, you just need to be more... flexible,</i>\" you quip.");
		//(else if not horse and not naga)
		else output("You straddle her legs with your own, pressing her pussy into yours; Sheila gasps as the hot flesh connects and you tentatively rock your hips, dragging your lower lips across.");
		...
		//(sheila corruption > 30)
		if (sheilaCorruption() < 30) output(" even though they're small and humble");
		//(else sheila corruption > 60)
		else if (sheilaCorruption() > 60) output("; I worry that they're getting too mountainous");
		output(".</i>\"  A hand drifts up to her nipple, pinching it as her mouth hangs open.  She rolls it in her palm, then presses the breast against her ribcage, and her shiver of arousal transmits to you through");
		if (pc.isNaga()) {
			output(" her taut buttcheek");
			output(".");
		
			output("\n\nYou smile at her insecurity, and slide your hands under her chest to cup her tits.  \"<i>Do you like having your breasts touched?  Does it make you feel womanly?</i>\"  Sheila answers by writhing in your grip, moaning again and pushing her ass against your [pc.vagina].");
			...
			output(" her heated, slippery vagina.");
		...
		output("\n\n\"<i>[pc.name],</i>\" Sheila pants, \"<i>I'm gonna come soon if you keep doing that!</i>\"");
		output("\n\nIn answer, you thrust even harder against her, crossing her netherlips furiously with your");
		//[(naga) 
		if (pc.isNaga()) output(" tail, drawing it up the hot opening and coiling it so you can return the warm, lube-slicked length for another pass even as you hump her butt.");
		else output(" cunt, grinding her like a sex toy in search of your own climax.");
		output("  \"<i>Do it!</i>\" you order, anxious to feel her pussy convulsing against you, anxious to feel your own squeezing her.  \"<i>Come with your lesbian lover!</i>\"");
		...
		//[(pc corr >= 90 or > sheila corruption)
		if (pc.cor() >= 90 || pc.cor() > sheilaCorruption()) {
			output("  Her vagina doesn't wait for romance, spasming and opening wide to try to suck you in, pulling at your ");
			if (pc.isNaga()) output("scales");
			else output("pussy");
			output(" until you can take no more of the heat and come as well.");
			//[(naga)]
			if (pc.isNaga()) output("  Her breasts swell in your hands as she sucks in her breath sharply, feeling your pussy drool its juice onto her ass.");
		}
		//(else if PC corr < sheila corruption)
		else if (pc.cor() < sheilaCorruption()) {
			output("  Her vagina doesn't wait for symbolism, dumping a wave of orgasm that splashes almost audibly against you.  Your pussy answers with your own climax, and begins wetting the woman underneath");
			if (pc.isNaga()) output("; it feels like her breasts are pulling away from you as she tries to push her ass further into your quivering quim");
			output(".");
		}
		else {
			output("  Her vagina drools and spasms, not interested in gestures so much as release, and coats ");
			if (pc.isNaga()) output("your tail");
			else output("your own");
			output(" with a fresh coat of her hot fluids which trigger the expected response from you.");
		}
		//(if cock)
		if (pc.hasCock()) {
			output("  Your neglected ");
			if (pc.cockTotal() == 1) output("prick quakes and spurts");
			else output("pricks quake and spurt");
			output(" weakly, drooling semen that pools on her skin.");
		}
		//[(not horse)
	...
	//(else if no cockFit 32 and no vag)
		...
		output("Her hand lowers to her crotch and slides her middle finger into her pussy.  \"<i>I don't know what's wrong with me,</i>\" Sheila says, nervously.  \"<i>I'm not even sure why I get hot when I think of you... you don't have anything I can use.</i>\"");
		
		output("\n\nYou smile at the admission");
		if (pc.hasCock()) output(", stroking your oversized tool despite her dismissal");
		output(".  \"<i>But you do?</i>\"");
		...
		if (pc.hasCock()) {
			...
			//(mans)
			if (!pc.isTaur()) output("falls into your lap");
			//(horse)
			else output("drifts between your legs");
			output(", then crawls its way to your [pc.cockBiggest].  She begins stroking it, torpidly at first, but with increasing excitement as a drop of pre-cum slides down and wets her palm.  \"<i>What's wrong, goblin girl?</i>\" you ask.  \"<i>Gotta get some sperm no matter what?</i>\"");
		
			output("Sheila winces as your probing fingers send a shudder through her.  \"<i>Shut up, you tosser,</i>\" she retorts, yanking on your dick slightly harder in her irritation.");
		}
		//(new PG)
		...
		//[(pc corr >= 90 or > sheila corruption)]
		if (pc.cor() >= 90 || pc.cor() > sheilaCorruption()) output("  Her vagina heats up, squeezing your fingers so hard that you nearly can't pull out to rub her again, and you have a clear view of her breasts swelling up - arguable proof of your prowess, considering what she told you earlier.  Her people must be terrible at faking orgasm.");
		//(pc corr < sheila corruption)
		else if (pc.cor() < sheilaCorruption()) output("  Her vagina gushes with clear fluid, filling the bowl made by your hand with warmth, and you think you can see her breasts tightening up slightly as her back arches, though it's hard to be sure with how she's shaking.");
		else output("  Her pussy quivers around you, and you draw the fingers out and pop them into Sheila's mouth.  The woman sucks on them greedily, and you raise an eyebrow at her.  She blushes again, but it's evidently not the first time she's done the same thing!");
		...
		//[(cocks)
		if (pc.hasCock()) {
			output("\n\nYou reclaim your hand and rest it atop Sheila's, forcing the climax-addled woman to stroke again; she gasps apologetically and resumes with jerky, weak motions.  Irritated and almost at your peak, you up the pace, getting your hips into it until you're practically fucking her palm.  You tilt your pelvis toward Sheila, angling your prick at her face as you prepare to blow your load; she recoils slightly at first, but guiltily moves her mouth to your [pc.cockHeadBiggest], kissing the tip and playing her tongue over the hole.  The tingle of nerves pushes you over the brink, and you grab her head and hold it as you squirt seed past her lips.");
			//[(big skeet)
			if (pc.cumQ() > 1000) output("  Her mouth fills quickly, and she writhes in your grip, managing to turn her head.  \"<i>Oh god, no way!</i>\" she coughs, spitting up enough semen to coat her chin as your ongoing orgasm spatters against her cheek.");
			//[(mega skeet)
			if (pc.cumQ() >= 2000) output("  Not satisfied with that alone, you turn her head the other way for your finish, ejaculating on the rest of her face and completing her gooey, [pc.cumColor] beard as she squirms.");
			//[(multicock)
			if (pc.cockTotal() > 1) output("  The other strokes not coming from this cock do their part to decorate your lover as well, turning her hair into a slimy mess.");
			output("  She escapes from your hand as you spend yourself, leaning away and gagging up seed onto the dry grass.");
		}
		output("\n\nThe woman falls weakly backward");
		//(cock.length > 0)
		if (pc.hasCock()) output(", her face still covered in globby [pc.cumColor] smears");
		output(".  \"<i>Fuck...</i>\" she groans.  \"<i>You're a jerk, [pc.name].");
		//[(cock.length > 0)
		if (pc.hasCock()) output("  I can't believe you called me a goblin, you arsehole.");
		output("</i>\"");
		
		output("\n\nYou ");
		//[(cock.length > 0)
		if (pc.hasCock()) output("can't believe how much she acts like one sometimes... but you ");
		output("just laugh and tug her ear; she flicks it and pulls away from you, backhanding you softly in the gut.  \"<i>Knock it off!</i>\"");
		
		output("\n\n\"<i>Are you really so worried about it?</i>\" you ask.  No answer comes");
		//[(cock.length > 0)
		if (pc.hasCock()) output(", even after Sheila finishes wiping her face clean and stares at you");
		output(", so you just content yourself with lying next to the woman for a while.");
...
private function sheilaXPThreeSexyTimeGuardDuty():void {
	clearOutput();
	output("You fold your arms and frown at Sheila, but consent to help.  She brightens, then looks confused when you make no further motion.  \"<i>So...</i>\" she begins, \"<i>what should I do?</i>\"");
	...
	//[(PC level >=8)
	if (pc.level >= 8) output(" - which, you suppose, must include imps even though they're so far beneath you they might as well be underground");
	output(".  You stand there for so long that you nearly slip into a daydream, and are only roused by the noise of a leather belt being carefully unfastened.  Focused again, you can hear Sheila's ragged breathing and her thighs rubbing together, then a hand sliding against skin and fabric... seems like she's made her decision.");
	...
	if (pc.slut() < 40) {
		...
		output("\n\nThis continues for several minutes, the wet sounds gradually drowning out all the others, increasing in tempo and volume, until suddenly they stop.  \"<i>Nnnn...</i>\" Sheila moans, quietly.  \"<i>Oh god, [pc.name], you're amazing!</i>\"");
	...
	//(else lib >= 40)
		...
		//[(cock.length > 0)
		if (pc.hasCock()) output("shift your pelvis forward");
		//(cock.length == 0)
		else output("stick out your tongue playfully");
		output(".  The girl giggles at that, relaxing and finally letting herself get into the masturbation.");
		
		output("\n\n\"<i>You're right, mate,</i>\" she says, with a sigh and a shiver as she begins massaging her breast once more.  \"<i>I'd love to get one of those soon, but my supplier is holding out on me.</i>\"  Sheila colors a bit as she realizes what she just implied, but shakes it off and ups the pace again, arching her back as her fingers search out the best spots inside her.  \"<i>Ahh, ahh, god... gonna come soon, [pc.name]!</i>\"");
...
private function fuckBuddySheilaMeetingMaybeLater():void {
	clearOutput();
	output("You tell her you haven't the time right now, but squeeze her knee as you get up again.");
...
private function fuckBuddySheilaMeetingJustHangOut():void {
	clearOutput();
	output("In answer, you reach down to pinch her butt and then ");
	//[(not horse or drider)
	if (!pc.isBiped()) output("scoot your own forward");
	else output("fold your [pc.legs] under you");
	output(" to lay in the bush's shade.");
...
private function fuckBuddySheilaMeetingChat():void {
	clearOutput();
	output("\"<i>Feeling talkative?</i>\" Sheila asks.  \"<i>Have at yer.</i>\"");
	...
	if (flags["COC.SHEILA_JOEYS"] > 0) addButton(4, "Kids", fuckBuddySheilaMeetingChatKids);
}

//[XP4 - Chat - Family&Friends]
private function fuckBuddySheilaMeetingChatFamilyAndFriends():void {
	clearOutput();
	output("\"<i>Them?  They're doing fine.</i>\"  Sheila stares at you obtusely, trying to will you into changing the subject; you stare right back.  She sighs.  \"<i>No go, huh?</i>\"");
	...
	if (pc.slut() >= 50) {
		...
		output("\n\nThe woman squirms as you slip a hand around her waist.  \"<i>Dammit, [pc.name]... what do you want me to say?  That I'd spend all bloody day hiding in the tall grass with you if I didn't have work to do?  I'd think that would be obvious by the way I put up with your crap, you bastard.</i>\"");
...
private function fuckBuddySheilaMeetingChatOldLoves():void {
	clearOutput();
	output("\"<i>[pc.name], I don't want to talk about that,</i>\" Sheila says, \"<i>it leaves a bad taste in my mouth.</i>\"");
	
	output("\n\nYou offer to tell her about your own history, as an enticement, but she shakes her head.  \"<i>I don't want to talk about that either!  Look, no matter how cosmopolitan, nobody really wants to hear about the dicks " + pc.mf("his","her") + " girl's had growing out of her face.  I mean, how many is the right answer?</i>\"");
	...
	output("\n\nThe woman stifles a laugh.  \"<i>[pc.name], that's not funny.  I... guess I would, though it'll take a while since I feel so strongly.  Deadset, there's not much to know; I promise.  Ask again after we've been together so much we have the same accent - after ten years of happy marriage, maybe.</i>\"");
	...
	if (pc.slut() < 50) {
		...
		output("\n\nThe woman colors a bit and looks down.  \"<i>Er...</i>\" she mumbles, \"<i>it means you're great, [pc.name].  Top shelf.  My favorite " + pc.mf("bloke","sheila") + ".</i>\"");
	...
	//(else lib >=50)
		...
		if (sheilaCorruption() >= 60) output(" - though it's probably meant to be a defensive gesture, all it does is push her tits together under her white shirt and frame them in light-brown skin");
		output(".  \"<i>[pc.name], you horny whacker, I'm not gonna marry you.  Having to bull my way through a horde of monsters to see you and stand watch in shifts just to sleep together at night may be romantic once, but it's no kind of thing to do every day for the rest of my life.</i>\"");
		...
		//[(corr < 30)
		if (pc.cor() < 30) output("; you lay a hand on her shoulder reassuringly");
		output(".  Eventually she pulls away again, nose slightly red.  \"<i>Sorry, mate; didn't mean to have a tantrum on you.</i>\"");
...
private function fuckBuddySheilaMeetingChatWhyHide():void {
	...
	//(if T'A)
	if (flags["COC.TEL_ADRE_KNOWN"] >= 1) {
		output("  After all, the city in the desert seems to be bustling and bristling with weapons to resist the demons, whatever her feelings may be about it.");
...
private function fuckBuddySheilaMeetingChatLiveWithMe():void {
	clearOutput();
	output("You ask Sheila to move in with you at your camp, and she turns bright red.");
...
private function fuckBuddySheilaMeetingChatKids():void {
	clearOutput();
	//if joeycount = 1
	if (flags["COC.SHEILA_JOEYS"] == 1) {
		output("\"<i>Ah?</i>\" Sheila says, turning toward you sheepishly.");
		...
		if (pc.mfn("", "f", "") == "f") {
			output("I don't know how he'll react when he sees a pretty face like yours, though; he doesn't know that I'm-");
			//[(PC is herm, fem)
			if (pc.hasVagina()) {
			...
			output(" you guess, teasing her as her blush deepens.");
			output("\n\n\"<i>[pc.name], stop it.</i>\"");
		}
		//else if PC masculine or andro, no new PG)
		else {
			output("I'm not sure if he intends to shake your hand or punch you, though.</i>\"");
	...
	//if 1 < joeycount < 10
	else if (flags["COC.SHEILA_JOEYS"] < 10) {
		output("\"<i>So how are the kids?</i>\"");
		...
		if (pc.slut() > 50) output(" and tight, hot cunt");
		output(" so well...  \"<i>How long does it take them to grow up?</i>\"");
		...
		output("\n\nSheila laughs.  \"<i>No, mate; much older than that.  I'm not going to tell you how much, though.</i>\"  Her eyes narrow in mock suspicion, and she smiles broadly.  \"<i>A " + pc.mf("gentleman", "lady") + " doesn't ask a" + pc.mf("", "nother") + " lady's age, after all.</i>\"");
		
		//[(lib >=50)
		if (pc.slut() >= 50) {
			output("\n\n\"<i>But I'm no " + pc.mf("gentleman", "lady") + ",</i>\" you retort, leaning in to tickle her ear with your words.  \"<i>Though, I was only asking because your skin is so smooth and your nipples so perky...</i>\"  You nip at her lobe.  \"<i>And your pussy still squeezes like it's your first time.</i>\"");
			
			output("\n\n\"<i>[pc.name]!</i>\" Sheila shrieks, pulling back and slapping you across the face.  \"<i>You nasty root rat!</i>\"");
	...
	//if joeycount >= 10
		...
		output("\"<i>Kids doing all right?</i>\"");
		...
		if (pc.slut() < 50) {
			...
			//[(silly)]
			if (silly) {
				output(" and clutching the life-sized [pc.name] bodypillow I made");
				output(".  But it's not going to happen right now.</i>\"");
		...
		//(else lib >=50)
			...
			output("\n\n\"<i>[pc.name], trust you to never change.  Do... you really want to make love?</i>\"  Her care-worn eyes look into yours, regaining a bit of their former enthusiasm, and her skin heats up under your fingers.");
			//apply small lib-based lust damage and display [Sex] choices with [Rest For Now] instead of [Back]
			applyDamage(new TypeCollection( { tease : 20 + target.libido() / 10 + 5 } ), sheila, target);
...
private function fuckBuddySheilaMeetingChatKidsRest4Now():void {
	clearOutput();
	output("\"<i>Always,</i>\" you answer, \"<i>but if you need to relax and sleep for a bit, I'm here for that too.</i>\"  You draw your hand back out, patting her on the cheek.");
	
	output("\n\nSheila kisses you on the forehead and then lays her head against your ");
	if (pc.isTaur()) output("back");
	else output("lap");
	output(".  \"<i>You're sweet... thank you.  I will take you up on both of those offers - but maybe just the nap this arvo.  Would you wake me up in about an hour?</i>\"");
	
	output("\n\nYou nod and she dozes off; her face looks much more placid in sleep.  After an hour you gently nudge her awake, and she picks herself up and leaves you with a hug.  \"<i>Love you, [pc.name].</i>\"");
	//corruption down
	pc.cor( -1);
...
private function sheilaXP4Sex(display:Boolean = false):void {
	if (display) {
		clearOutput();
		output("You slip an arm around the bushranger and lean in.  \"<i>I was thinking we could have some sex... feel up to it?</i>\"");
	...
	clearMenu();
	addDisabledButton(0, "69WithWang", "69WithWang", "This scene requires you to have cock.");
	addDisabledButton(1, "69WithCooch", "69WithCooch", "This scene requires you to have vagina.");
	addDisabledButton(2, "Fuck Her", "Fuck Her", "This scene requires you to have fitting cock.");
	if (pc.hasGenitals()) {
		if (pc.hasCock()) addButton(0, "69WithWang", consensualSheila69, true);
		if (pc.hasVagina()) addButton(1, "69WithCooch", consensualSheila69, false);
	}
	//[Consensual - Vaginal Sex] (req. cock <= 32 area):
	if (pc.hasCock() && pc.cockThatFits(sheila.vaginalCapacity()) >= 0) addButton(2, "Fuck Her", consentacleVagSexForKangarooSlutBitches);
	//da scene
...
private function consensualSheila69(cock:Boolean = true):void {
	...
	if (cock) {
		if (pc.cockThatFits(32) >= 0) {
			output("  Though, she smiles openly when you produce your [pc.cock " + x + "].");
			...
			//(all big cox > 32)
			output("\n\nIncreasingly wide eyes offset her thin, pressed lips as you reveal [pc.eachCock], the voluminous flesh swaying softly to the breeze.");
			output("\n\n\"<i>[pc.name]... you really do take it too far.  There's no way I can... I mean, the size of it!</i>\"  Even despite a point-blank refusal, Sheila's eyes don't leave your crotch, drinking in the sight of your dick");
			if (pc.cockTotal() > 1) output("s");
			output(" as she rubs her small, smooth labia vigorously.  \"<i>What do you expect me to do with one of those?</i>\"");
		}
	}
	//(vag)
	else {
		output("  When you finally drop your bottoms, revealing your wet " + pc.vaginaDescript(0) + ", she smiles softly.");
		...
		if (pc.clitLength >= 4) output(" and bring your " + pc.clitDescript() + " to its erect state.  \"<i>... And quite a stem,</i>\" your lover concludes.  \"<i>You sure that's not a dick?</i>\"");
		else output(", producing a slight sigh and a wet finger.  \"<i>And I'm the eager one, right?</i>\" your lover asks, wryly - though her finger is just as wet.");
	}
	...
	if (cock && pc.cockThatFits(sheila.vaginalCapacity()) < 0) output("  \"<i>[pc.name], I'm telling you it won't fit!</i>\"");
	else output("  \"<i>Not talking, then?  Please yourself.</i>\"");
	output("  Her commentary cuts off as you ");
	if (!pc.isTaur()) output("turn around and hang your ");
	else output("straddle her and squat down to dangle your ");
	if (cock) output("pre-seeping shaft");
	else output("wet pussy");
	output(" over her head, waggling it provocatively as you slip your fingers into her.");
	
	output("\n\n\"<i>Ah... ah... you could have just told me,</i>\" Sheila gasps, ");
	//[(cox)
	if (cock) {
		output("wrapping a hand around your shaft");
		//(big) 
		if (pc.cockThatFits(sheila.vaginalCapacity()) < 0) output(" and guiding it to bend comfortably against her chest, drooling a trail of pre-cum as she slides it into position");
		output(".");
	}
	//(vag)
	else output("returning the favor and plunging into your " + pc.vaginaDescript(0) + " with her hand.");
	output("  She pumps you once, eliciting a trickle of fluid, and then speaks once more.  \"<i>Bring it lower...</i>\"");
	
	output("\n\nYou oblige, ");
	if (!pc.isTaur()) output("flattening your back");
	else output("squatting lower");
	output(" so your ");
	if (cock) {
		output("cock sinks down");
		//[(small cock) 
		if (pc.cockThatFits(sheila.vaginalCapacity()) >= 0) output(" into her warm, waiting mouth");
		else output("; she lets go of it to grab her breasts instead, trying to push them around the sensitive shaft as she runs her tongue up the underside");
		output(".");
	}
	else {
		output("pussy dangles just over her face; she ");
		if (pc.clitLength >= 4) output("nips and sucks at the tip of your clit, rolling it with her tongue");
		else output("parts your labia with her tongue, stroking them gently and tickling your little clitoris");
		output(".");
	}
	output("  Not to be outdone in creativity, technique, or audacity, you ");
	if (!pc.isTaur()) output("lean down and bury your own tongue all the way inside Sheila, forcing her labia open with your mouth");
	else output("reach as far as you can to bring both hands into play, stroking and pulling Sheila's labia apart with one as you tease her exposed clitoris or slip inside her vagina with the other");
	output("; your lover stops licking and you can feel her mouth slip away from your sensitive skin as her head rolls back.");
	...
	//[(ladyparts)
	if (!cock) {
		output("slipping her tongue back into you");
		//[(bigclit)
		if (pc.clitLength >= 4) output(" and stroking your " + pc.clitDescript() + " rapidly with one hand");
		output(".");
	}
	//(smallcox)
	else if (pc.cockThatFits(sheila.vaginalCapacity()) >= 0) output("taking you into her mouth once again and licking away a neglected drop of pre-cum.");
	else output("stroking your cock with one hand as she pushes an erect nipple against its side to trace a line.");
	...
	if (!pc.isTaur()) {
		...
		if (cock && pc.cockThatFits(sheila.vaginalCapacity()) < 0) output("shaking, teasing nipples and heaving chest");
		else output("mouth, humming in delight");
		output(".");
	}
	//(horse)
	else {
		...
		if (cock && pc.cockThatFits(sheila.vaginalCapacity()) < 0) output("hard nipples");
		else output("throat");
		output(".");
	}
	//merge horse with rider
	//[(big cox)
	if (cock && pc.cockThatFits(sheila.vaginalCapacity()) < 0) output("\n\n\"<i>[pc.name]!  I'm coming!</i>\" Sheila cries.");
	else output("\n\n\"<i>Mmmh!  Mm mmnnn!</i>\" Sheila moans, muffled by your body.  The import doesn't hit you until a second later.");
	output("  Her vagina convulses, ");
	//(pc corr >= 90 or > sheila corruption
	if (pc.cor() >= 90 || pc.cor() > flags["COC.SHEILA_CORRUPTION"]) {
		output("trying to pull your ");
		if (!pc.isTaur()) output("[pc.tongue]");
		else output("fingers");
		output(" in ravenously.  You can literally feel her internal muscles rippling and undulating, stroking you as if they could coax out an ejaculation.  Sheila sucks in her breath as she climaxes, filling her chest; you can feel her tits pushing harder and harder against you.");
	}
	//(PC corr < sheila corruption)
	else if (pc.cor() < flags["COC.SHEILA_CORRUPTION"]) {
		output("pushing out a gush of fluid.  For what seems like several minutes, her pussy deluges your ");
		if (!pc.isTaur()) output("mouth, filling you with her taste.");
		else output("hands, completely soaking them in her orgasm.");
		output("   Her thighs tense, forcing her pelvis into the air and her cunt toward you as her chest slumps; slowly, you feel the perky nipples pressing into your stomach pull away.");
	}
	//(else if equal but below 90)
	else if (pc.cor() == flags["COC.SHEILA_CORRUPTION"] && pc.cor() < 90) {
		output("drooling a trickle of fluid and licking at you as she screams her ecstasy into your crotch.");
	}
	output("  Sheila jams her face upward, ");
	//[(big cox)
	if (cock && pc.cockThatFits(sheila.vaginalCapacity()) < 0) output("pressing her lips and tongue to the base of your shaft as she masturbates it vigorously.");
	///(small cox)
	else if (cock) output("sinking your [pc.cockHead " + pc.cockThatFits(sheila.vaginalCapacity()) + "] into the warm recesses of her throat where her muscles caress and coax it.");
	else output("penetrating you deeply with her tongue as her hands reach up to stroke your clit furiously.");
	output("  Your body gives up under her frenzied assault, laying your crotch onto the woman's face as muscle control abandons you.");
	
	//[(if cock-focus)]
	if (cock) {
		output("\n\nStroke after stroke of semen passes down [pc.eachCock], ");
		//[(cock fits)
		if (pc.cockThatFits(sheila.vaginalCapacity()) >= 0) {
			output("pouring out into your lover's waiting throat.  She gags and you can feel the gooey seed oozing back against your [pc.cockHead " + pc.cockThatFits(sheila.vaginalCapacity()) + "]");
			//[(small cum)
			if (pc.cumQ() < 250) output("; you withdraw from her, allowing her throat to force your ejaculate back up and over her lips and cheeks with a splutter.");
			//(big cum)
			else if (pc.cumQ() < 2000) output(", but your relentless orgasm forces it down again, dumping more jism into her throat.  You can feel her hands clawing at your [pc.butt] as you relieve your buildup.");
			//[(mega cum)
			else output(".  Soon, her visibly-rounding stomach fills and the tip of your dick is once again washed in backflow; the next spurts force the [pc.cumNoun] all the way up, and she begins drooling the slimy semen-saliva blend from her mouth and nose, hands all the more frantic in her alarm.");
		}
		//(else doesn't fit)
		else {
			output(" squirting onto and between your intertwined bodies to soak your [pc.chest] and neck, and Sheila's softly pumping crotch.");
			//(big skeet)
			if (pc.cumQ() > 1000) output("  A wellspring of seed, your body continues producing until the fuzz on her thighs and tail is completely matted down with the [pc.cumColor] [pc.cumNoun] drooling between you.");
			//(multicocks)
			if (pc.cockTotal() > 1) {
				output("  Your other cock");
				if (pc.cockTotal() == 2) output(" spasms");
				else output("s spasm");
				output(" as well, adding their own issue to the unholy mess covering the poor woman.");
			}
		}
	}
	//(else if vag-focus)
		...
		if (pc.wetness() <= 2) output("trickle out, and are quickly sucked up by her greedy mouth");
		else if (pc.wetness() <= 4) output("gush freely, leaving her face slick");
		else output("overflow, soaking her hair even as she shuts her eyes tight against the cataract");
		output(".  As you climax, you wrap around her head, squeezing so hard that you flatten her ears against her skull.  Her hands latch onto your butt, trying to pull you off, but having already spent herself on her throes of passion, she's no match for yours, still ongoing, and you hold tight until your body relaxes.");
	}
	
	output("\n\nSpent, you roll off of Sheila");
	//[(smallcox)
	if (cock && pc.cockThatFits(sheila.vaginalCapacity()) >= 0) output(", pulling free of her throat.  She immediately turns over as well and forces herself to her hands and knees, then gags and coughs up a large amount of your jism.  When she's dumped enough to clear her passageway, she collapses forward, smearing the mess onto her chest as she spends a few minutes to catch her breath.  Once she's got it, she turns back over and sits up, strands of your slime linking her breasts to the grass.");
	//(else not smallcox)
	else output(".  She sits up, wiping sweat and stray fluids away from her eyes.");
	...
	//[(corruption threshold warning if sheila corruption >= 80 and PC corr > sheila corruption and sheila xp =/= 3)
	else if ((pc.cor() > 90 || pc.cor() > sheilaCorruption()) && sheilaCorruption() >= 80 && flags["COC.SHEILA_XP"] != 3) {
		sheilaCorruptionUpdate();
		/*if (cor() > 90 || cor() > sheilaCorruption()) {
			dynStats("cor", -10);
			sheilaCorruption(10);
		}
		else if (cor() < sheilaCorruption()) {
		...
		output("\n\nYour climax-fogged brain settles on Sheila's breasts, as requested.  They are definitely looking enormous and squeezable, and you reach out a hand to rub one without a thought.  Sheila quivers as you touch her flushed skin, but grabs your wrist.  \"<i>Dammit, [pc.name], stop!  How much fucking taint are you toting, you slut?</i>\"  Having gained your attention, she lets go.  \"<i>I'm sorry for that, but I can't keep being with you if you contaminate me like this!  I have responsibilities and mates counting on me, and already all I can think about is pushing them down and turning them into captive dongs to stuff me whenever I want!</i>\"");
	...
	output("\n\n\"<i>Gods damn, [pc.name],</i>\" Sheila coughs, staring at you.  Still slack-muscled from your orgasm, you look over at her.  ");
	if (cock && pc.balls > 0) {
		output("What you see manages to pull an explosive laugh from you, tired as you are.  Sheila gives you the stink-eye until you've recovered enough to explain the source of your mirth: on her forehead, making a perfect ");
		if (pc.balls == 2) output("infinity");
		else output("double infinity");
		output(" shape against the rest of her skin, is an imprint of your [pc.balls].  The kangaroo girl rolls her eyes at the juvenile display and resumes her harangue.  ");
	}
	output("\"<i>You could give a girl a bloody warning before that sort of behavior.  I thought you were going to kill me!</i>\"");
	...
	pc.girlCumInMouth(sheila);
	sheilaCorruptionUpdate();
	/*if (cor() >= 90 || cor() > flags["COC.SHEILA_CORRUPTION"]) {
		dynStats("cor", -10);
		sheilaCorruption(10);
	}
	...
	//[(sheila xp = 3)
	if (flags["COC.SHEILA_XP"] == 3) {
		output("\"<i>[pc.name]... thanks.  Even being covered in your mess is better than being stuck outside all night, too scared to touch myself and too horny to run.</i>\"  Sheila kisses you on the cheek.  \"<i>My sloppy hero, eh?  I'm okay now; I can make it 'til morning when the gate reopens.</i>\"");
	...
	//(else if sheila xp =/= 3)
		...
		output("\"<i>Forget it.  With the way you worked me, I forgive you.  Just be careful, you wanker.</i>\"  Sheila closes her eyes and pulls your arm around her shoulder");
		if (cock && pc.cockThatFits(sheila.vaginalCapacity()) >= 0) output(", smearing your dangling hand into the mess on her tits");
		output(".  The two of you lie like that for a while, until she gets up.  \"<i>I should really go... need to cure my catch.  Not to mention I'll have to clean up before I can even put my damn clothes back on.</i>\"  Sheila picks up her stuff.  \"<i>Hehe... see you soon.</i>\"");
	}
...
private function consentacleVagSexForKangarooSlutBitches():void {
	...
	if (sheilaCorruption() <= 30) output("pert");
	else output("jiggly");
	output(" breasts as she fiddles with your own clothing.  She manages to remove the top of your [pc.gear], exposing your [pc.chest]; she then returns your favor by ");
	if (!pc.hasFuckableNipples()) output("squeezing and fondling");
	else output("plunging two fingers into");
	output(" your own " + pc.nippleDescript(0) + "s.  You shiver as she paws at the sensitive nubs on your chest, but don't let up your assault, moving your mouth down to tickle one of hers with your teeth and tongue.  Soon she's quaking and twitching beneath your ministrations; rubbing at her crotch with one hand as she half-heartedly tries to tease you back with the other.");
	
	output("\n\n\"<i>Please... I can't take any more,</i>\" she whines.  \"<i>Stop pashing on my nips and put it in, mate.</i>\"  The hand at her crotch begins trying to unfasten her belt while the other grabs and fondles her breast; you smirk and complete your own undressing quickly with both hands.  As [pc.multiCocks] emerges, engorged with blood, from your clothing, she switches from undressing back to rubbing, breathing heavily and stroking herself through the fabric as she stares at your ");
	//[(if wangaroo)
	if (pc.cocks[x].cType == GLOBAL.TYPE_KANGAROO) output("tapered, fleshy penis.  \"<i>Wow... you never see those.</i>\"");
	...
	output("\n\nYou grin.  ");
	//[(no centaur)
	if (!pc.isTaur()) output("Taking hold of your " + pc.cockDescript(x));
	//(centard)
	else output("Attempting to maneuver your " + pc.cockDescript(x) + " into position");
	output(", you rub the head along her dripping snatch, smearing her fluid over it.  ");
	//[(non-cent)
	if (!pc.isTaur()) output("Gripping her fuzzy thighs next, you lift her bottom and line");
	//(hoers)
	else output("She lifts her bottom and lines");
	output(" her slippery passage up with your cock.  \"<i>Here it comes!</i>\" you cry.  With that announcement, you thrust into her, parting her lips with your swollen [pc.cockHead " + x + "].  She moans in relief as you finally enter her, but said relief is short-lived as you begin pumping at her slick vagina.  Gasping weakly, she lies back on the ground; ");
	if (sheilaCorruption() <= 30) output("her perky breasts rock back and forth");
	else output("her jiggling breasts flatten into perfect domes and undulate on her chest");
	output(" as you draw in and out of her warm, moist pussy.");
	...
	//[(non-cent)]
	if (!pc.isTaur()) {
		output("You pause briefly, adjusting your angle and leaning down to kiss her deeply once again and help caress her ");
		if (sheilaCorruption() <= 30) output("firm");
		else output("pillowy");
		output(" tits, then resume thrusting from your new position; her nipples now brush against your [pc.chest] as you pump.  ");
	}
	//(if balls/vag)
	if (pc.balls > 0 || pc.hasVagina()) {
		output("Her stubby tail curls inward toward you, rubbing up against your ");
		if (pc.balls > 0) output("[pc.balls]");
		else output(pc.vaginaDescript(0));
		output(" and tickling you as she writhes in pleasure.  ");
		//(if vag)
		if (pc.balls == 0 && pc.hasVagina()) output("Your cunt twitches and drools a bit of fluid on the furry intruder.  ");
	}
	output("You fuck harder, stuffing Sheila with such force that her ");
	if (sheilaCorruption() < 30) output("perky, erect nipples trace long arcs against your skin");
	else output("jiggly tits nearly slap into her face");
	output(".  \"<i>Ohhh,</i>\" she moans, \"<i>YES!</i>\"  At that moment you feel her vagina clamp around your [pc.cock " + x + "], wringing it frantically as she reaches her climax.  You continue pumping as best you're able, but you can't stand up to the siege of her spasming pussy for long; ");
	
	//[(pc corr >= 90 or > sheila corruption) 
	if (pc.cor() >= 90 || pc.cor() > flags["COC.SHEILA_CORRUPTION"]) {
		output("something builds to a painful intensity in your crotch.  It feels like two mind-blowing orgasms arriving together and fighting over which one gets to come out first.  You rise toward climax quickly and then are drawn back in moments of maddening frustration, over and over.  In response you intensify your thrusts even more, sawing in and out of the girl at a pace that would be blistering if the two of you weren't leaking enough pre-cum and pussy juice to put out a brushfire.  Eventually the pleasure overwhelms the strange reticence of your orgasm; you cry out loudly as you peak, and squeeze a tiny trickle of [pc.cumNoun] into her grasping vagina.");
		//[(multi)
		if (pc.cockTotal() > 1 ) {
			output("  Your other ");
			if (pc.cockTotal() == 2) output("dick erupts");
			else output("dicks erupt");
			output(" as well, festooning her crotch with [pc.cumVisc] strings.");
		}
		output("  The intense sensation begins to ascend your dick slowly; your body works itself into overdrive and your partner's moans increase sympathetically as it nears the tip.  Finally, it exits your body with an enormous squeeze, alongside a jet of jism.  Sheila gives a complementary gasp as this pulse enters her, and her pussy convulses in a second orgasm, causing her back to arch.  Her breasts seem to jiggle more wildly than before as she thrashes on the ground in the grip of her pleasure.");
	}
	//(PC corruption < sheila corruption)
	else if (pc.cor() < flags["COC.SHEILA_CORRUPTION"]) {
		output("your resistance is condemned to die in failure as Sheila lets out a quavering scream of pleasure and wraps her arms and legs around you, pinning you inside.  Her vagina convulses around your cock powerfully; you could swear you feel yourself stretching longer and longer as her rippling pussy pulls you deeper in.  Just when you think that your dick must be a mile long, her whole body tenses up and her fingernails dig into your back.  Wearing a madwoman's expression, Sheila stares open-mouthed at you as her muscles lock and her pussy quivers out its fitful orgasm; even her ");
		if (sheilaCorruption() < 30) output("perky");
		else output("pillowy");
		output(" breasts seem to be held taut, compressed and motionless.  Suddenly she rams her hips into yours violently and throws her head back, and before you can issue a word of protest, her cunt wrings your " + pc.cockDescript(x) + " one last time with almost supernatural heat; your mind clouds over and a wracking orgasm shoots to the base of your spine, echoing back with the first wave of your ejaculation.");
	}
	//(else if equal but below 90)
	else {
		output("your orgasm quickly supersedes your control as she wrings you and rubs her nipples against your [pc.chest].  [pc.EachCock] begins to twitch as it unloads its first squirt of semen into her spasming pussy");
		if (pc.cockTotal() > 1) output(" or onto her thighs and belly");
		output(".");
	}
	...
	//[(non-dog cock)
	if (!pc.hasKnot(x)) {
		output("You oblige shakingly, pulling your [pc.cockNoun " + x + "] out of her clenching pussy with a wet, sucking noise.  A trickle of semen drools from between her lips as you unburden yourself of the rest of your load, squirting it onto her groin");
		//[(big skeet)
		if (pc.cumQ() > 500) output(", breasts, and face");
		output(".");
		//[(Huge skeet)
		if (pc.cumQ() >= 1500) output("  More and more globs of semen shoot from the end of [pc.EachCock], pasting Sheila to the dry grass as you paint her body [pc.comColor].");
	}
	//(dog cock)
	else {
		output("You attempt to oblige her, but your swollen knot sticks firmly in her squeezing pussy, locking you into place.  After a few fruitless tugs, you give yourself up to your orgasm with an apology, slumping against her heaving chest and helplessly dumping the rest of your load into the twitching woman. ");
		//[(big skeet)
		if (pc.cumQ() >= 1000) {
			output("Her belly begins to round as your overproductive gonads push out your massive store of semen");
			//[(huge skeet)
			if (pc.cumQ() >= 3000) output(", until it's stretched as far as possible and each successive jet forces out squirts of [pc.cum] from the tight seal her pussy makes around the knot");
			output(". ");
		}
		//[(multi)
		if (pc.cockTotal() > 2) output("Your other cocks erupt as well, soaking her crotch and matting down the fur on her thighs with [pc.comColor]. ");
	}
	output("Both of your orgasms finish up");
	//(dog-c) 
	if (pc.hasKnot(x)) output(" and you finally manage to pull out of her");
	output("; the two of you lie there breathing raggedly for several minutes.  Sheila breaks the silence first.");
	...
	//[(corruption threshold warning if sheila corruption >= 80 and PC corr > sheila corruption and sheila xp =/= 3)
		...
		output("\n\n\"<i>God dammit [pc.name], look how big my pointers are!</i>\"");
	...
	//[(sheila xp = 3)
	if (flags["COC.SHEILA_XP"] == 3) {
		output("  Sheila looks over, thankfulness written on every inch of her brow.  \"<i>[pc.name]... I really appreciate what you did.  I'd rather be knocked up than caught by some nightmare because I was too horny to hide properly until the colony opened - er, not that I want to be knocked up by you, sweet as you are.</i>\"");
	...
	//(else sheila xp =/= 3)
		...
		//[(norm skeet)
		if (pc.cumQ() < 500) output("as she strokes the " + pc.skinFurScales()  + " of your chest");
		else output("as she wipes off as much jizz as she can");
		output(", then get up and collect your gear.  \"<i>Um... see you later, ok?</i>\" she says.");
		processTime(25 + rand(10));
		clearMenu();
		addButton(0, "Next", mainGameMenu);
	}
...
private function sheilaMutualMasturbation():void {
	...
	if (pc.hasCock()) {
		if ((pc.isTaur() && pc.tallness * (5 / 6.) < pc.cocks[pc.longestCockIndex()].cLength()) || (!pc.isTaur() && pc.hasCock())) choices[choices.length] = "cock";
	}
	if (pc.hasVagina() && !pc.isTaur()) choices[choices.length] = "vagina";
	if (pc.hasFuckableNipples()) choices[choices.length] = "nipplecunt";
	if (!pc.hasGenitals() || choices.length == 0) choices[choices.length] = "ass";
	
	if (choices.length == 0) 
	{
		output("ERROR - SHEILA MASTURBATE BROKE, SON");
		clearMenu();
		addButton(0, "Next", mainGameMenu);
		return;
	}
	...
	output("You snatch Sheila's hat and pack, tossing them away, then slide a hand into the girl's shirt, slipping one of her nipples between two fingers as you nibble on her neck, and use the gentle pressure to push her onto her back.  She sighs, moving a hand under her clothes to caress yours.  \"<i>Ah,</i>\" she pants, letting out a soft puff of air, \"<i>what're we gonna do on the grass, [pc.name]");
	if (silly) output("-chan");
	output("?</i>\"");
	
	output("\n\nYou offer no answer, withdrawing from her shirt and turning your attention to her belt, which you unfasten.  With it out of the way, you unbutton her shorts and pull them down, panties and all, slipping them right off her legs.  Sheila shifts nervously and squeezes her knees together as you tug her paws free of the bottoms and cast the latter aside.  Briefly, you slide your hand into the open space between her thighs to stroke her pussy once, but then you're up again, peeling off her vest and pushing her shirt over her head, getting in a good grope on her breasts as you strip her.  With Sheila in the buff and waiting eagerly, you stand up and shuck your own [pc.gear], dropping it carelessly.");
	...
	//[(horse)
	if (pc.isTaur()) output(", folding your [pc.legs] under you,");
	output(" and begin ");
	//[(adequate-length cock)
	if (target == "cock") {
		output("stroking your [pc.cock], pointing it at her as you smear pre-cum onto your fingers");
	}
	//(non-horse vag)
	else if (target == "cunt") output(" sliding your hand to your [pc.vagina], parting your lips with two fingers");
	//(gross-ass nipcunts)
	else if (target == "nipplecunt") output("plunging your fingers into one of your awkward, distended nipples");
	//(Unsexed non-centaur only)
	else if (target == "ass" && !pc.isTaur()) output("licking a finger and teasing your asshole with shallow strokes, gently circling the ring");
	//(centaur who hasn't got shit)
	else output("sucking a single digit salaciously, fingerfucking your mouth and coating it with saliva");
	output(".  \"<i>Oh... are you going to touch me, or just play with that?</i>\" Sheila asks, grinning.");
	...
	if (target == "cock") output("begin stroking your shaft, drooling pre-cum onto her skin and ");
	//(non-horse vag)
	else if (target == "cunt") output("demonstrate by slipping your fingers into your own [pc.vagina] and ");
	...
	if (target == "cock") output("You return your hand to your [pc.cock], pumping vigorously to catch up to your lover, spurred onward by the smooth skin she presses into your lips.");
	//(else if nonhorse vag)
	else if (target == "cunt") output("You slide your hand between your [pc.legs] again, intent on recovering the march Sheila's stolen on you, and begin rubbing your [pc.clit] vigorously while sighing into her breast.");
	//(else if RNG disgusting nipplecunts)
	else if (target == "nipplecunt") output("Your hand returns to your chest, slipping inside the sensitive flesh of your nipple, and you groan into Sheila's as a trickle of moisture falls from your chest.");
	//(else if non-centaur asshole)
	else if (!pc.isTaur()) output("Your hand reaches back to your [pc.asshole], neglected for too long, and a finger spears your pucker, stroking the nerves inside as you hurry to bring your arousal in line with Sheila's.");
	///(else if fucking nothing)
	else {
		output("You wish you had something to play with as you watch your lover writhe under you");
		//[(silly), even just a paperclip].]
		if (silly) output(", even just a paperclip");
		output(".");
	}
	
	output("\n\nSheila places a hand alongside your head even as she gasps and shoves her fingers deep inside her pussy.  \"<i>[pc.name], as long as you'll touch me like this, I'll be your star; I'll beg on my knees!</i>\"  Her hand moves irregularly now, as her hips jerk and swivel");
	//[(if fucking nothing)
	if (target != "cock" && target != "cunt" && target != "nipplecunt") output(", and yours joins her, slipping into her entrance alongside");
	output(".  \"<i>Just... say I'm always yours!</i>\"  She turns you toward her face again, locking you in a kiss, and your ");
	//[(if cock)
	if (target == "cock") {
		if (pc.cockTotal() == 1) output("prick begins");
		else output("pricks begin");
		output(" to twitch, ejaculating a stroke onto Sheila's midriff and wrist.  The girl groans in your mouth at the new heat, and her body shakes in orgasm.");
	}
	//(vag)
	else if (target == "cunt") {
		output("pussy convulses, ");
		if (pc.wetness() < 4) output("drooling");
		else output("squirting");
		output(" around your hand.  As your body shakes and your lips rattle against Sheila's, she shivers and her own orgasm begins.");
	}
	//(nipcunts)
	else if (target == "nipplecunt") {
		output("nipple constricts around your fingers in climax, oozing lubrication onto your partner.  As the drops from your " + pc.breastDescript(0) + " hit her own, Sheila's back arches again and her own orgasm begins.");
	}
	//(jack shit)
	else output("fingers are pulled into her vagina by her muscles as her orgasm begins.");
	//[( pc corr >= 90 or > sheila corruption) 
	if (pc.cor() >= 90 || pc.cor() > flags["COC.SHEILA_CORRUPTION"]) {
	...
	//(else if equal corr but under 90)
		...
		output("She breaks the kiss, drawing her hand from her pussy, and holds your head in front of her; her eyes soften with affection as she shudders.");
		//[(not jack shit)
		if (target == "cunt" || target == "cock" || target == "nipplecunt") output("  Emptying your reserves, you allow yourself to fall into her arms, resting your head on her chest.");
	}
	...
	if (target == "cunt" || target == "cock" || target == "nipplecunt") pc.orgasm();
	else pc.lust(50);
	if (pc.cor() >= 90 && sheilaCorruption() >= 90) {
		processTime(25 + rand(10));
		clearMenu();
	...
	//(else no corruption warning triggered)
		...
		output("\n\nSheila strokes your " + pc.hairDescript() + " as you lie ");
		if (target == "cock") output("there, staring at the load you left in her bellybutton");
		else output("together");
		output(".  \"<i>Oh, I had so much fun, mate.");
		//[(if sex =U and sillymode)
		if (!pc.hasGenitals() && silly) output("  I love you, my Ken doll.");
		output("</i>\"");
		//[(if sheila xp = 3)
		if (flags["COC.SHEILA_XP"] == 3) {
...
private function sheilaCorruptionWarningsChooseFondle():void {
	clearOutput();
	output("Fuck it.  You move your hand right to her entrance.  \"<i>Don't be like that...</i>\" you say softly into her neck.  She shivers under your touch as her resistance begins to subside, and you slide two fingers into her gently.  \"<i>No,</i>\" she says quietly, and then, as you slip in, \"<i>NO!</i>\"  Violently coming to herself, she pushes you away and scrambles to her feet.");
	
	output("\n\n\"<i>That's it, [pc.name]!  Bloody well listen when I speak!  I can NOT do this again with you while you are like this or I will fall over the edge and never come back!  I need to get myself under control and I don't need you making it harder, so rack off if you don't get that!</i>\"");
	...
	output(".  Even as she goes, you can still see the edges of her amazing, swaying rack around the sides of her torso.");
	
	output("\n\nBitch tease.");
	//set sheila xp to -3 and sheila corruption to 90, PC corr - 10
	flags["COC.SHEILA_XP"] = -3;
	flags["COC.SHEILA_CORRUPTION"] = 90;
	pc.cor( -10);
	processTime(25 + rand(10));
	clearMenu();
	addButton(0, "Next", mainGameMenu);
...
private function sheilaCorruptionWarningListenToHer():void {
	...
	pc.cor( -10);
	flags["COC.SHEILA_CLOCK"] = timeAsStamp + 60 * 4;
	processTime(25 + rand(10));
	clearMenu();
	addButton(0, "Next", mainGameMenu);
}
...
internal function sheilaGotWhomped():void {
	clearOutput();
	output("Sheila collapses, ");
	//[(HP)
	if (enemy.HP() <= 1) output("completely spread-eagled from her injuries");
	///(lust)
	else output("rubbing her thighs together and smearing around the moisture from her pussy");
	output(".  As you stand over her, she forces herself to her elbows and tries to scoot away from you, fear written on her face.  \"<i>I'm sorry... please, no...");
	//[(sheila xp > -4)
	if (flags["COC.SHEILA_XP"] > -4) output("  I'll forgive you for everything else, but please, not this!");
	output("</i>\" she implores.");
	//[(sheila corruption >= 90)
	if (sheilaCorruption() >= 90) output("  \"<i>I-if you touch me again... god, I can't even think straight anymore!  Don't do this!</i>\"");
	//(lust >= 30)
	...
	clearMenu();
	if (pc.lust() >= 33) {
		if (pc.hasCock() && pc.cockThatFits(sheila.vaginalCapacity()) >= 0) addButton(0, "Fuck Pussy", rapeSheilasCooter);
		if (pc.hasCock()) {
			if ((!pc.isTaur() && pc.shortestCockLength() <= pc.tallness * .35) || (pc.isTaur() && pc.tallness * 1.2 <= pc.shortestCockLength() && pc.shortestCockLength() <= pc.tallness * 1.55))
			{
				addButton(1, "Forced BJ", forcedSheilaOral, true);
			}
		}
		if (pc.hasVagina()) addButton(2, "Forced Lick", forcedSheilaOral, false);
	}
...
private function letSheilaGo():void {
	if (flags["COC.SHEILA_XP"] >= -3) flags["COC.SHEILA_XP"] = -1;
	processTime(20 + rand(10));
...
private function rapeSheilasCooter():void {
	...
	output("You grab the girl by her fuzzy leg; she tries to kick away from you, but you pull her back, pinning her");
	//[(horse)
	if (pc.isTaur()) output(" by planting your forehooves just above her hips");
	output(".  Reaching down, you take the neck of her shirt in your hand, then rip it open, exposing her ");
	//[(sheila corruption < 30)
	if (sheilaCorruption() < 30) output("tiny ");
	//sheila corruption > 60)
	else if (sheilaCorruption() > 60) output("huge, soft ");
	output("breasts, which jiggle from the force.  She tries to cover herself with her arms, but you grab and pull them aside, revealing her chest again.  As you appraise her naked flesh, she looks away, shame coloring her cheek.  Her mouth tightens and her eyes shut as you squeeze her tits roughly, flicking the nipples until they're erect.");
	...
	//[(monster HP < 1)
	if (sheila.HP() <= 1) output("sliding it between her labia and producing a damp spot, then ");
	output("dragging the wet fabric around her vulva.");
	
	output("\n\nSatisfying yourself as to her readiness, you unfasten your [pc.gear] to extract your [pc.cock " + x + "]; the [pc.cockHead " + x + "] drips with pre-cum as you pull it loose, and you lay it on her crotch, letting the goo soak the white fabric until you can see her light-brown skin through it.  She squirms under it, bringing up another droplet with her motion, and you place the tip at the entrance to her pussy, pressing eagerly against the fabric.  \"<i>Please...</i>\" she whimpers, \"<i>I'm begging you; don't.</i>\"");
	
	//fuck-fork
	//[(corruption < 50)
	if (pc.cor() < 50) {
		output("\n\nYou frown at the girl.  She still wants to escape, even though her body is eager for this?  You can feel her wet pussy twitching at your [pc.cockHead " + x + "].  Dragging the oozing tip along her soaked panties provokes a shiver and another bloom of fluid from the girl, but she bites her lip and refuses to meet your eye.");
		...
		output("\n\nHer knees clasp around you until she remembers her injunction against skin contact and forces them apart again; her hands, which she had reached out to you briefly, are pulled back as her arms fold tightly against her chest, pressing her breasts together.  \"<i>S-sorry!</i>\" she cries, and you feel her drooling through the panties as she spasms.  At your limit as well, you slide your [pc.cockHead " + x + "] down to the entrance to her vagina and push it partway in, rubbing the tip against the fabric to work the nerves and cross the last step to your own orgasm.");
		
		output("\n\n\"<i>Take it out!</i>\" the girl wails, still shaking with climax.  \"<i>D-don't come in me!</i>\"");
		
		output("\n\n\"<i>Shut up,</i>\" you retort, through gritted teeth, \"<i>I gave you what you wanted, now do the same for me!</i>\"  Your [pc.cock " + x + "] twitches and your muscles quake as the first stroke of semen erupts; it takes all your effort to hold yourself up and avoid sinking onto your lover while the permeable barrier of her panties collects its toll from your spilling [pc.cum], thickening with the goo you pump into the girl.  She reddens as your warm seed squirts through the fabric to invade her body, but makes no further protest.");
		//[(big skeet)
		if (pc.cumQ() >= 500) output("  Soon you've filled her as fully as the shallow penetration will allow, and the panties begin to press back against your cock, oozing onto you whatever semen the porous dam can't contain.");
		//(mega skeet)
		else if (pc.cumQ() >= 1500) output("  It sags and bows around the tip of your spear, pulsing with each new stroke as the hemlines leak jizz onto her legs and butt.");
		output("  Idly, you imagine what it must look like inside, each squirt having its momentum arrested by the cloth, drooling lazily into her until the next one displaces it a bit further forward, and wonder if it'll be enough to get her pregnant.  You pull away from the girl, leaving her gasping and once again averting her gaze.");
		
		output("\n\nShe attempts to stand, holding her midriff and looking at you unhappily.  ");
		//[(big skeet)
		if (pc.cumQ() >= 500) output("So much cum fills her pussy that it slips out, once again restoring the opacity, though not the dryness, of her panties.  ");
		output("You return her gaze, turning your nose up dismissively.  Conscious of her nudity, she pulls up her shorts and buttons her vest to conceal herself as you leave her behind.");
		...
		output("\n\n\"<i>No.</i>\"  Tensing your arm, you pull harder, and the sound of ripping fabric fills the space between you as her panties tear in half.  Casting the ruined garment away, you resume your position, jabbing at her vulva with your [pc.cock " + x + "]; it drools lubrication over your tip as you nose into it, reinforcing your decision.");
		
		output("\n\nThe girl's struggles resume as you finally push in, spurred on by the intimate contact");
		if (pc.cockVolume(x) > 32) output(" and the pain of taking a cock slightly too large for her");
		output(", and she actually rears back to scratch at you with her fingernails.  You catch her hand and hold it, and she responds by spitting at you!  \"<i>Fuck you, arsehole!</i>\" Sheila shrieks.  \"<i>Take it out!</i>\"  Reaching down, you slap her in the face.");
		...
		if (sheilaCorruption() > 10) output(" jiggles her breasts and");
		output(" clenches her jaw.  A bloom of red colors her cheeks, one brighter than the other from the blow... though she still refuses to look at you.  Doesn't matter; having sex.  You begin to pump her pussy vigorously, the wetness enough to allow quick, smooth thrusts without any warmup.  \"<i>You're a godawful liar,</i>\" you grunt");
		//[(not centaur)
		if (!pc.isTaur()) output(", leaning down to kiss her long neck and collarbone");
		output(".  Sheila quivers again, still looking away, as you continue.  \"<i>I can actually feel you stroking me with your vagina already.  Are you going to come soon?  Will you tell me how much you're hating this when you do, and wrap your legs around me to punish me by holding me inside?</i>\"  The girl's blush deepens, and she bites her lip in embarrassment.  Confidently, you release her hand");
		//[(nothors)
		if (!pc.isTaur()) output(", transferring yours to her breast while brushing her torn shirt to the side");
		output("; without struggling further, she slowly lowers it");
		//[(nothors)
		if (!pc.isTaur()) output(" to rest atop your own");
		output(", confirming your suspicion.  Her eyes close and her chin raises resignedly");
		//[(nothors)
		if (!pc.isTaur()) output(" as you triumphantly squeeze the flesh in your hand and kiss her under the jaw");
		output(".");
		...
		//[(mans)
		if (!pc.isTaur()) output("place a hand along her face, tuning it toward you, and kiss her on the lips.");
		else output("bend down, lowering your torso until you can caress her cheek.");
		output("  \"<i>Of course I won't,</i>\" you answer sweetly.  \"<i>What would be the point?</i>\"  Sheila winces as you hilt yourself, sinking your [pc.cockNounSimple " + x + "] so far into her that your crotch meets hers.  You shudder");
		//[(nonhors)
		if (!pc.isTaur()) output(" and squeeze her breast");
		output(" as your climax arrives and the first of your seed pours into her grasping pussy.  While you fill her, her back arches");
		//[(nonhors) 
		if (!pc.isTaur()) output(" and she presses your hand uncomfortably");
		output(", gasping as her own orgasm begins.  Her knees wrap around your sides, satisfying your predicition, and her vagina");
		//[(pc corr >= 90 or > sheila corruption)
		if (pc.cor() >= 90 || sheilaCorruption() < pc.cor()) {
			output(" pulls greedily at your [pc.cock " + x + "], drinking up your semen as fast as you can squirt it.  She sucks in breath as she climaxes, finally opening her eyes as her head tilts back");
			//[(nonhors)
			if (!pc.isTaur()) output("; you can almost feel her breast heating up and swelling with blood under your hand, as the nipple pokes into your palm");
			output(".");
		}
		//(PC corr < sheila corruption)
		else if (pc.cor() < sheilaCorruption()) {
			output(" floods you with juices, dumping so much around your twitching cock that you feel like you're lowering it into a hot spring.");
			//[(nonhors)
			if (!pc.isTaur()) output("  She squeezes your hand so hard that you swear you can feel her breast compressing under it as she screams her pleasure.");
		}
		//(else equal but < 90)
		else {
			output(" drools eagerly as you mingle your juices with hers, until the fluid begins to thicken with white seed.");
		}
		//[(big skeet)
		if (pc.cumQ() >= 500) output("  Overabundant as always, your semen quickly fills her to capacity and begins to squirt out around your shaft, staining her thighs.");
		//[(multicock)
		if (pc.cockTotal() > 1 ) {
			output("  Your other ");
			if (pc.cockTotal() == 2) output("dick spits");
			else output("dicks spit");
			output(" as well, leaving trails all over her light-brown skin.");
		}
...
private function forcedSheilaOral(dick:Boolean = true):void {
	...
	//set sheila xp = -4
	flags["COC.SHEILA_XP"] = -4;
	output("You lean down and grab the girl by her shirt-front, raising her to her knees; her hands automatically go to yours.");
	...
	if (sheilaCorruption() >= 50) output("soft, pillowy");
	else output("small, perky");
	output(" breast.  \"<i>You don't want to rip your clothes, do you?  It must be difficult to get nice fabric like this out here in the middle of nowhere.</i>\"");
	...
	if (sheilaCorruption() < 20) output("little round things that trace gentle curves under her loose shirt.");
	//(else sheila corruption < 50)
	else if (sheilaCorruption() < 50) output("healthy mounds that fill out her shirt.");
	else output("swollen globes that bulge over her v-neck and strain at the seams of her shirt.");
	output("  Smiling at the sight, you grab the garment's hem and pull it up roughly.  Sheila blinks dumbly as you expose her erect nipples to the air, then blushes and covers them with her hands.");
	
	output("\n\nStill smiling, you unfasten your [pc.gear] and reveal your crotch.  ");
	//[(cock.length > 0)
	if (dick) output("[pc.EachCock] flops free, half-erect and swelling steadily with blood as you appraise the woman's body.");
	//(vag present)
	else output("Your [pc.vagina] drools slightly, labia parting and [pc.clit] poking up; you imagine what you want to do with it.");
	output("  Sheila watches your face as you slip into a fantasy, half-playing with her nipples - but just when you start to get into your daydream, she turns and tries to scramble to her feet!");
	...
	if (dick) {
		output("\n\nYou seize the girl by her hair, cutting off her escape and depositing her in front of you once again.  Still holding her tight, you guide her shut mouth to the tip of your [pc.cock " + x + "].  \"<i>Oral,</i>\" you demand tersely, grabbing your shaft and smearing her lips with the pre-cum you're beginning to drip.  Sheila's eyes close like her mouth and she tries to pull away, but you shake her until she's looking at you again and point at your cock insistently.  \"<i>Oral,</i>\" you repeat, pulling her hair harder.  \"<i>Or we can slip you out of those pants instead, if you prefer...</i>\"");
		...
		if (pc.cocks[x].thickness() <= 6) {
			output("parting her lips to pop neatly between them.  It feels good, but one-sided, and you order Sheila to get to work.");
		}
		else {
			output("jabbing at her undersized mouth with your enormous [pc.cockHead " + x + "].  \"<i>It's too thick!</i>\" she complains, trying to turn away.");
		...
		output("  The reticent woman brings her lips into play, sucking gingerly, ");
		//start length fork, no new PG
		//[(dick length <= 8)
		if (pc.cocks[x].cLength() <= 8) {
			output("and the warm embrace of her mouth is welcome stimulation.");
			...
			output(", and the other hesitantly makes its way to her breasts as she gets into the sex.  The woman tweaks her nipple, softly at first but with mounting passion as her attentions on your dick become more lavish.  Catching this from the corner of your eye, you move ");
			if (!pc.isNaga()) output("a [pc.foot]");
			else output("the tip of your tail");
			output(" between her knees to her crotch, rubbing her pussy through her shorts; Sheila hums into your shaft as a wet spot appears on the cloth, and begins grinding her hips against the pressure.  You answer her enthusiasm by taking her head in your hands and stroking the surface of her long, pointed ears affectionately with your thumbs.  She flushes, then sinks down on your cock, rubbing the nerves on the slit with the back of her tongue and pulling you closer to orgasm.");
			...
			if (pc.balls > 0) output(" and your [pc.balls] tighten up");
			output(".  She pushes against you with her hands, trying to get free, but you just hold her against your crotch as you lean over her head.");
		}
		//(else dick length > 8)
		else {
			output("but this sort of half-assed attention won't get you off any time before sundown.");
			...
			//[(cock width <= 6)
			if (pc.cocks[x].cockThickness <= 6) {
				output("you grab her by both ears and force your [pc.cock " + x + "] down her throat!  You can feel her esophageal muscles rippling around you as she gags and retches, and you pull it out again to be sure she's not going to throw up on your dick.  Nothing seems forthcoming, so you drag her head forward again, once more sliding into the convulsing warmth of her throat.  A groan escapes you as it closes around your crown; her squeezing passageway feels for all the world like a tight cunt, and your eager hips begin to use it as exactly that, pumping in and out of Sheila as she shudders and struggles weakly.");
		...
		//(else if corr < 70 or PC cannot use Infest at will)
			...
			//[(big skeet)
			if (pc.cumQ() >= 1000) output(", and soon you can feel it lapping at the end of your shaft");
			output(".");
			//[(mega skeet)
			if (pc.cumQ() >= 3000) output("  Your body's not done with that, though, and keeps pushing out cum until it spurts around your prick and out Sheila's nose in little squirts.");
			output("  Emptying yourself, you pull Sheila's mouth away from your [pc.cock " + x + "] and drag the slimy member on her chest, smearing her tits with spit and semen and wiping away the last of the mess on her shirt.");
			
			output("\n\nYou pull the soiled garment down over Sheila's breasts");
			//[(big+ skeet)
			if (pc.cumQ() >= 1000) output(", though it's so wet with seed now that you can see them almost as well through it");
			output(".  \"<i>This really is a nice fabric.  You should take better care of it.</i>\"  With that parting shot, you leave the queasy-looking woman to digest your load and the humiliation.");
		}
		//end worm fork
		...
		//[(sheila corruption >= 30)
		if (sheilaCorruption() >= 30) output("; her breasts jiggle pleasingly as they settle on her chest, filling you with the urge to play with them");
		output(".  Straddling her, you lower your [pc.vagOrAss] to her face.  \"<i>Lick,</i>\" you order, rubbing your [pc.butt] on Sheila's nose");
		//[(not horse)
		if (!pc.isTaur()) output(" as you fondle her tits");
		output(".");
		...
		if ((pc.hasVagina() && pc.vaginalCapacity() >= 100) || (!pc.hasVagina() && pc.analCapacity() >= 100)) output("  She keeps hitting the same spot again, too - she's not even bothering to adjust her angle and touch the rest of your stretched opening.");
		...
		if (flags["COC.HELL_HOUND_MASTER_QUEST"] > 0) output("  I know a nice boy in the mountains who keeps dogs, and I'm sure he'd love to have a womb like yours for their seed.  By this time tomorrow, you could have a cock in both holes, filling you with hellhound babies since you can't please a person.");
		output("</i>\"");
		
		output("\n\nYou can feel her face heating up under your [pc.butt] as she flushes with shame; seems like you struck a nerve.  You ");
		//[(no horse)
		if (!pc.isTaur()) output("pinch her nipple for emphasis, then grind your hips across her face, mauling her nose with your ass.  \"<i>I could fuck a tree and it wouldn't be as wooden as you in the sack");
		//[(PC has found corrupt glade)
		if (flags["COC.EXPLORED_FOREST"] >= 40) output(" - in fact, there are quite a few I've seen who look like better lovers than you.  Maybe I should carry you to the forest and tie you to a nice pussy-shaped giant flower to give you lessons");
		output(".  ");
		//[(minotaur addiction score =/= 0%)
		if (flags["COC.MINOTAUR_CUM_ADDICTION_TRACKER"] > 0 || pc.hasPerk("Minotaur Cum Addict")) {
			output("Perhaps I'll turn you over to a minotaur; you're bound to get better at using your tongue when you're addicted to their amazing seed.  Would you like that?  The first thing you'd see in the morning would be a huge flared cock, right in front of you, dripping with heavenly precum and waiting for your mouth... or your cunt.  You could be just another minotaur-fucking slut of the plains.  ");
		}
		output("Or I could take you home and give you personal training myself, whenever I feel like it, and invite travelers to do the same");
		//[(PC has Izma with cock)
		if (izmaFollower() && flags["COC.IZMA_NO_COCK"] != 1) output("; I personally know fifteen inches of fat, orange dick and four heavy balls that'd love to get in at your other end while I ride your face");
		output(".</i>\"");
		
		output("\n\nYour [pc.vagOrAss] squeezes around Sheila's hesitant tongue, turned on more by the spectacle of selling her body than by any of her poor efforts.  She squirms under you, rubbing her thighs together as you paint the worst pictures of her future you can imagine, and you climax");
		//[(PC has met Lumi or Lynette)
		if (flags["COC.GOBLIN_SALON_FOUND"] > 0 || flags["COC.LUMI_MET"] > 0) {
			output(" while imagining her turned over to the goblins for experiments, forced to drink potion after potion - most would be dedicated to growing cocks on her or increasing semen production, of course");
		}
		output(".  Your hole clenches and drools on her, and the woman's rude tongue retreats from the pressure, leaving you painfully empty.");
		//[(no horse)
		if (!pc.isTaur()) output("  Even squeezing her breasts viciously can't compel her to reinsert it, though you amuse yourself trying.");
...
private function sheilaGetsRapedWithADildont():void {
	...
	output("\n\n\"<i>Nnnnn,</i>\" she moans, humming into your " + pc.skinFurScales() + ".  ");
	//[(PC corr >= 90 and sheila corr >= 90)
	if (pc.cor() >= 90 || pc.cor() > sheilaCorruption()) {
		output("Your face heats as she kisses you sloppily, and not all of the temperature is from her mouth.  It feels like she's pulling energy right out of you, and you can swear that her breasts are growing.");
	}
	//(else PC corr < sheila corruption)
	else if (pc.cor() < sheilaCorruption()) {
		output("Her lips heat up and your body tingles; you can see her breasts shrinking as she shivers and slobbers on your face - or so it seems from the angle.");
	}
	//(else if magically equal but < 90)
	else output("The kiss is wet and indecent as she shudders through her anal orgasm; her lips don't stay in one place, instead tracking drool all over your face.");
	...
	//(cock)
	if (pc.hasCock()) output("  \"<i>Perhaps next time you'd like to try a real cock in there?</i>\"  Even exhausted and messed up on goblin drugs, she has the presence of mind to shake her head at that.  You laugh.");
...
internal function getBeatUpBySheila():void {
	clearOutput();
	output("You slump down before Sheila, breathing heavily; the woman ");
	if (enemy.lustQ() < 75) {
		output("takes the opportunity to grab her catch and take off running!  Even if you wanted to, you couldn't catch her now... you collapse, head swimming in ");
		if (pc.HP() <= 1) output("colorful spots");
		else output("fantasies of her, stark naked but for her hat and waiting with her legs open");
		output(".");
	...
	//(else monster lust >= 75)
		...
		output("stares at you, desire and nerve at war in her face.  Her thighs rub together involuntarily as she tries and fails to will herself away... slowly, she takes a step toward you.");
		output("\n\n\"<i>[pc.name]... you're not a demon?</i>\" Sheila asks, with smoldering eyes.");
...
private function sheilaLossRapeRouter():void {
	...
	if ((!pc.hasCock() || rand(3) == 0)) sheilaForcedOralGeneric();
...
private function sheilaCowgirlRapesYou():void {
	...
	processTime(30 + rand(10));
	var x:int = pc.cockThatFits(sheila.vaginalCapacity());
	output("Sheila pushes insistently, and you allow her to force you to the ground and then onto your back");
	if (pc.isTaur()) output("; your [pc.legs] stick awkwardly out to one side");
	output(".  She blushes furiously and won't meet your eyes, but it doesn't stop her from stripping off her vest and shirt in one smooth motion, revealing her " + sheilaCup() + " chest.  \"<i>T-touch me,</i>\" she demands, \"<i>rub me.</i>\"  Almost as an afterthought, she adds \"<i>... please.</i>\"");
	
	output("\n\nYou move your hands to her smooth skin, cupping a ");
	if (sheilaCorruption() < 30) output("petit ");
	else if (sheilaCorruption() > 70) output("massive ");
	...
	if (pc.isTaur()) output("pulls your legs apart");
	else output("unfastens and strips off your [pc.gear]");
	output(", revealing your [pc.multiCocks]; ");
	//[(if no cock fit 32)
	if (x < 0) {
		output("her eyes widen as [pc.eachCock] bobs free, half-erect monster meat that fills her with frustration.");
		...
		//[(horse)
		if (pc.isTaur()) output(", or at least as flat as you can get it");
		output(", then sits on your stomach");
		if (pc.isTaur()) output(", sliding her long legs between yours");
		output(".  She scoots forward slowly, and gasps as her pussy comes to rest against your hot, blood-engorged shaft.");
		
		output("\n\n\"<i>Ahh... ah.  N-not bad,</i>\" she sighs.  She wraps her thighs lightly around your [pc.cock] and begins to shift up and down, stroking herself.  The space between her legs is nearly the perfect size, and when she squeezes you, you moan as well.  \"<i>So,</i>\" Sheila whispers, shuddering, \"<i>enjoying this, are you?</i>\"");
		
		output("\n\nYou pant in agreement as your hips begin to aid the stimulation by thrusting; your ");
		if (pc.isTaur()) output("forelegs fold over her torso affectionately");
		else output("hands search out her breasts, caressing and squeezing");
		output(" as her gasps and the picture in your head both intensify.  Sheila grabs one with her own hand, pulling it up to her mouth to smother it in kisses");
		if (!pc.isTaur()) output("and suck on your fingers");
		output(".  \"<i>Oh... [pc.name]...</i>\"");
		
		output("\n\nYou grunt a response, too involved in thrusting between her warm, juice-slicked thighs to form sentences.  \"<i>[pc.name]... I-I'm... coming!</i>\" Sheila shrieks.");
		...
		output("Sheila shivers and leans back, hair dangling, as her hips buck and her thighs squeeze your shaft.  ");
		//(PC corr >=90 or > sheila corruption)
		if (pc.cor() >= 90 || sheilaCorruption() < pc.cor()) {
			output("You can feel your prick heating up where her pussy rubs against it... it almost feels like the rest is cooling down at the same time.  The woman atop you arches her back and sucks in her breath sharply, and her tits seem to claim your field of vision, swelling up - though that could just be the angle.");
		}
		else if (pc.cor() < sheilaCorruption()) output("Her orgasm wets your prick, coating it with fluid as she comes.  Sheila wraps herself tighter in your limbs, squeezing her breasts against her chest, and you think you can feel them get a little smaller as your head fuzzes over with her excess emotion.");
		else output("Her hips lock up when she comes, leaving you to try and finish yourself as she heats up around your shaft.");
		
		//[(sens >=50)
		if (pc.slut() >= 50) {
			output("\n\nThe warm, smooth, wetness of her thighs is enough to bring out your own climax, and [pc.eachCock] unburdens itself, forcing globs of semen into the air to rain down on your entwined bodies.  Sheila laughs as you coat her midriff with your mess, and playfully strokes your sensitized [pc.cock] with one hand, sending shivers through you as she helps force your [pc.cumNoun] up your shaft.");
			//[(mega skeet)
			if (pc.cumQ() >= 2000) output("  The giggles and the strokes stop long before your orgasm, though; eventually Sheila just lies there, dumbfounded, as you deposit layer after layer of [pc.cumColor] lattice on her skin.  \"<i>Bloody hell,</i>\" she mumbles vaguely, either stone-sober or drunk from the sight of it.");
			output("  Once the rain of semen peters out, she pulls apart from you, slimy and dripping.");
		...
		//(else sens < 50)
			...
			output("\n\nYour partner parts from you and stands up, redressing as you lie on your back.  She looks at you once more and rubs your [pc.cock] one last time, right below the glans, before taking off with her bounding run.");
			//[(HP loss)
	...
	//(else if cock fit 32)
		...
		output("as you lean over to catch her expression, you can see her eyes glimmer with desire.  Sheila notices you from her peripheral vision, and her face reddens again.  She turns her back to you.");
		...
		//[(multicock)]
		if (pc.cockTotal() > 1) output("separating out and ");
		output("stroking your [pc.cock " + x + "].  A drop of precum forms, and she touches it with the index finger of her free hand, sending a tingle through you; she pulls the finger away, drawing a string of slick moisture out.  Sheila passes the finger between her breasts, wiping the fluid away on her chest and shivering.  The display is too much for you, and you reach forward, navigating your hands around her waist quietly until you can reach the buckle of her belt.  The girl sighs as you begin to unfasten it, but an errant thought brings her back to reality - she grabs your hands abruptly.");
		...
		//(else lib >= 25)
			...
			if (pc.isTaur()) output(", pushing your legs apart");
			output("; looking into your face, her frown evolves into a mischievous grin.  Sheila about-faces, turning her back to you.  \"<i>Blame yourself for this; it's rude to stare,</i>\" she giggles, hovering over your [pc.cockHead " + x + "].  The woman sinks down with an audible sigh, parting her slick, smooth labia with your tip and swallowing your " + num2Text(Math.round(pc.cocks[x].cLength())) + " inches.  \"<i>Ohh...</i>\" Sheila moans.  She slips a hand down to your crotch to check how much of your prick is left to go, and starts a bit when she realizes she's taken the entire shaft on the first stroke!");
			...
			if (!pc.isTaur()) output(", and your hands go to her waist, fairly pushing her up and slamming her back down in your eagerness");
			output(".  She rolls her head as the fuck gets wetter and faster, dangling her hair over her shoulders as her back muscles shift and flex with her strokes; her ears flick whenever she descends and rubs your neck against one of the sweet spots on her vulva.");
			...
			//(PC corr >= 90 or > sheila corruption)
			if (pc.cor() >= 90 || pc.cor() > sheilaCorruption()) output("You can feel her cum-slick inner walls pulling at you as she begins to orgasm, and your mind clears as you blow your load inside her, wondering when she'll likewise return to sense.");
			//(else PC corr < sheila corruption)
			else if (pc.cor() < sheilaCorruption()) output("Her pussy gushes with her orgasm, pushing out your semen and soaking her inner lips even as she continues riding your cock, and your mind clouds as you think of little except sneaking as much of your cum inside her as possible.");
			//else equal but < 90)
			else output("Even as she comes mindlessly something makes you hold in your own moans, wondering how long it will take her to notice her pussy being filled with your egg-hunting sperm.");
			//[(small cum)]
			if (pc.cumQ() <= 250) {
				output("  Sheila continues moving her hips even as she comes, lost in bliss; on one particularly lofty ascent, your cock flops out, squirting a line of jizz along ");
				if (pc.hasTail()) output("your tail");
				else output("the grass");
				output(", and her eyes go wide.");
			}
			//(big cum)
			else {
				output("  With how much Sheila is still moving her hips, it's only a matter of time before she notices the mounting pressure in her cunt... on one particularly deep push, she forces out a squirt of [pc.cumColor], and her eyes go wide as she pulls off and lets you finish on the grass.");
			}
			...
			output(" you groan.  \"<i>Can't believe you didn't notice...</i>\"");
	...
	sheilaCorruptionUpdate();
	/*if (cor() > sheilaCorruption() || cor() >= 90) {
		dynStats("cor", -10);
		sheilaCorruption(10);
	}
	else if (sheilaCorruption() > cor()) {
		dynStats("cor", 10);
		sheilaCorruption(-10);
	}*/
}
...
private function sheilaForcedOralGeneric():void {
	clearOutput();
	output("Sheila kisses you once more, forcing her tongue into your mouth, and slides a hand into your [pc.gear]; you stare wonderingly into her affectionate, hungry eyes at the sudden change.  It isn't until ");
	if (pc.hasCock()) output("her hand slips around your [pc.cock]");
	else if (pc.hasVagina()) output("her fingers slip between your labia");
	else output("she rubs your bare crotch for a few seconds");
	output(" that her reverie breaks; her face colors and she looks away, averting her gaze in shame.  She draws her hand back out guiltily, then begins to doff her shorts and panties.  \"<i>S-sorry... didn't mean to get familiar.  I'll have this over with quickly.</i>\"");
	
	output("\n\n\"<i>That's-</i>\"  Your words are cut off as she suddenly climbs atop you, bringing her pussy to eye level");
	if (pc.physique() < 30) output("; her weight bears you slowly to the ground");
	output(".  Sheila rubs the smooth slit on your [pc.face], trying to part her labia on you, and shivers a bit when your lips brush hers.");
	...
	if (pc.hairType == GLOBAL.HAIR_TYPE_TENTACLES) output(" and flushing as your stingers release their payloads into her palm");
	output(".  You obediently stick out your tongue");
	if (pc.slut() > 60) output(", motivated more by whimsy than the meager, almost playful pain,");
	output(" and shallowly penetrate Sheila's vagina with it.");
	
	output("\n\nThe woman jerks a bit as you enter her, but sighs in relaxation.  \"<i>That's good... come on mate, lick me more.</i>\"  She pushes her cunt against your [pc.face], trying to take your tongue deeper.");
	//[(if d-mon/dragon/naga or other future long tongue)
	if (pc.hasTongueFlag(GLOBAL.FLAG_LONG)) {
		output("  A sly look crosses your face at her request; she notices it but has no time to dismount before your long tongue shoots into her, stroking the nerves around her entrance as you unspool.  \"<i>Ah-ahhh!</i>\" she moans, \"<i>What the hell... is with your mouth?</i>\"");
	...
	//[(PC corr >= 90 or > sheila corruption)]
	if (pc.cor() >= 90 || pc.cor() > sheilaCorruption()) {
		output("strangling your tongue with muscles attempting to lock onto it, trying to pull it in.");
		//[(demon/dragon tongue)
		if (pc.hasTongueFlag(GLOBAL.FLAG_LONG)) output("  As tired as you are, you simply go along with it, allowing her rippling walls to slowly draw your tip deeper inside until it bumps against her cervix; she shivers and kisses the top of your head as you groggily run your tongue over the entrance to her womb.");
		else output("  However, it doesn't get very far; you lazily flop it around inside her as she works through her arousal.");
		output("  Sheila groans loudly and your vision fades; through the fog, you can swear you feel her breasts growing larger against your forehead.");
	}
	//(else PC corr < sheila corruption)
	else if (pc.cor() < sheilaCorruption()) {
		output("filling your mouth with her liquid love.  Your lips and throat heat up as you're forced to drink it from the nearness of her pussy; as she bucks her hips weakly into your face, trying to draw out her climax, you increasingly wish that you could return the favor, ");
		if (pc.hasCock()) output("face-fucking her and blowing a load into her mouth, then holding her jaw shut until she swallows it unhappily.");
		else if (pc.hasVagina()) output("smearing your gash on her face until you come and drown her in your squirting girl-jizz.");
		else output("forcing her to tongue your asshole until you come, writhing and pressing her head into the grass with your [pc.butt].");
	}
	else output("and she pets your head affectionately as her walls squeeze your tongue.  \"<i>Oh, mate...</i>\" she whispers lovingly, completely forgetting the struggle that led to your tryst.");
	
	processTime((pc.hasTongueFlag(GLOBAL.FLAG_LONG) ? 60 : 35) + rand(10));
	...
	//[(str < 61)
	if (pc.physique() < 30) output("climbing off and kneeling next to you.");
	else output("letting herself down your body until she can put her long legs on the ground once more.");
	output("  She looks into your eyes contentedly, rubbing your cheek... but then starts and looks away, guilty once more.  \"<i>Shit...</i>\" she mumbles.  \"<i>I did it again.  Even after I promised myself I wouldn't...</i>\"");
	...
	if (pc.hasTongueFlag(GLOBAL.FLAG_LONG)) pc.energy( -15);
	pc.lust(pc.libido() / 3 + 30);
	if (pc.lustQ() >= 100) pc.slowStatGain("l", 1);
	sheilaCorruptionUpdate();
	/*
	if (cor() >= 90 || cor() > sheilaCorruption()) {
		dynStats("cor", -10);
...
private function normalSheilaPregNotifNumberOne():void {
	...
	if (pc.cor() < 50) output("  A hand placed around her shoulder seems to steady her a bit.");
...
private function normalSheilaPregNotifNumberOneYepIssue():void {
	clearOutput();
	output("\"<i>I see,</i>\" Sheila says, nonplussed.  \"<i>I'm sorry you feel that way...  Truth be told, I'm not keen myself on the idea of having a kid, but what's out is out.</i>\"");
...
private function normalSheilaPregNotifREPEATEDED():void {
	clearOutput();
	output("Sheila is here as you step into the plains, with a grass-wrapped parcel of food open on her lap and her hat sitting upturned next to her, and waves you over.");
	
	output("\n\nAs you sit, she proffers the snack, smiling, then closes her eyes.  The woman breathes deeply and regularly as you pick and eat a morsel from her gift.  Just as you wonder if you should wake her, she speaks.  \"<i>[pc.name]...  I, ah... made another addition to your posterity.  Came out ");
	if (rand(10) < 7) output("healthy.  It's eating a lot... frankly, it's a little hellraiser with a strong grip, and it cries well, too.</i>\"");
	else output("weak... we thought we were going to lose it for a bit.  It wouldn't drink from the nurse unless I was there, so I had to stay up with it all night myself.  Feeling a bit stuffed.</i>\"");
	output("  She looks over at you, giving you the benefit of her fatigue-lined eyes.");
	
	output("\n\nSheila slips her hand into yours");
	if (pc.cor() < 40) output(", and you give it a squeeze");
...
private function normalSheilaPregNotifREPEATEDEDLetHerGo():void {
	clearOutput();
	output("Nothing you can do about that - even if you wanted to watch your baby for her, she's keeping it hidden in a hole in the ground.  You hand her the parcel back");
	if (pc.cor() < 40) output(", imploring her to eat the rest and keep up her strength");
	output(".  Sheila smiles weakly and takes it, folding the grass around again.  \"<i>Thanks, [pc.name].  I'll see you soon... ish.</i>\"");
...
private function normalSheilaPregNotifREPEATEDEDWalkWithHer():void {
	clearOutput();
	output("You pick up her forgotten hat and rise as well, carefully refolding the grass around Sheila's uneaten lunch and tucking it back into her pack for her.  She takes her hat back from you distantly.");
	...
	output(" next to her, easily keeping pace with the tired woman, and she raises her face to you inquiringly.  \"<i>What's up, [pc.name]?</i>\"");
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOut():void {
	clearOutput();
	output("You pick up her forgotten hat and set it on her head, then fold her lunch back into its grass wrapper and tuck it into her pack.");
	...
	//(else naga)
		...
		output("\n\n\"<i>Pretty good for old [pc.name], making circles in the dust,</i>\" she retorts, shoulders hunched under the weight of so many animals.  \"<i>I might even get to sleep in tomorrow with this much.</i>\"  She wraps her arms around you in a hug, then looks in your eyes.");
		
		output("\n\nYou quirk an eyebrow at her as she affects tiredness, yawning theatrically.  \"<i>Oh no, [pc.name], what are you doing?  Don't hypnotize me too...</i>\"  Sheila breaks the embrace and sits down, laying your catch and her pack beside her, then turns a hopeful face on you.  \"<i>Not to be cheeky but... care to join me for a nap?</i>\"");
	...
	//less QQ, more pew pew
	else if (pc.canFly() && (pc.hasKeyItem("Bow") || pc.hasBowWeaponAvailable())) {
		output("\n\n\"<i>Just wander around and flush game; point out animals you see.</i>\"");
		
		output("\n\nSheila looks a little confused, but glumly nods.  \"<i>Alright, [pc.name]; we'll do it your way.</i>\"  She begins to walk, then turns back when you don't follow.  \"<i>Are you com-</i>\"");
		...
		output("\n\n\"<i>Wow, [pc.name],</i>\" she says, coyly handing you back your arrows.  \"<i>That was impressive - like having my own guardian angel.  I've got more than enough to take home now, so do you want to... do anything else together?</i>\"");
	...
	//(else spd >= 80 and Runner perk)
		...
		if (pc.slut() < 50) output("you pinch her ear, provoking an annoyed flick from it.");
		else output("you slap her on the ass, grabbing a handful and taking her mind off her own misery.");
		output("  \"<i>Dammit, [pc.name]... not now,</i>\" she frowns, pulling away from you.");
		...
		if (!pc.isGoo()) output("run");
		else output("glide");
		output(", making a long loop around to the rear of the wary animal, then begin to systematically drive it closer to your partner, who has strung a line to the base of a shrub and is gesturing at you to force the hare into it.");
		...
		output("\n\n\"<i>[pc.name], you're pretty fast,</i>\" she says, warmly.");
	...
	if (hours < 21 && hours > 4) {
		...
		addDisabledButton(1, "To Camp", "To Camp", "You can invite her only if you have nothing very wrong there.");
		if (!(flags["COC.FUCK_FLOWER_LEVEL"] >= 4 && flags["COC.FUCK_FLOWER_KILLED"] == undefined) && flags["COC.VAPULA_FOLLOWER"] != 1)
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDWELP():void {
	clearOutput();
	output("\"<i>Welp,</i>\" you say softly.");
	//[(corr <50)
	if (pc.cor() < 50) output("  You set a hand on Sheila's shoulder to steady her a bit as the tears start to fall.");
	output("  \"<i>It'll be alright.</i>\"");
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE():void {
	clearOutput();
	output("With a wan smile, you pull the woman to her feet.  She doesn't resist, but continues looking into your face disconsolately.  \"<i>I'm so stonkered I can't even think what to do first...</i>\"");
	
	output("\n\n\"<i>Well, first is probably finding us a place to hide,</i>\" you suggest.");
	
	output("\n\nSheila nods and looks around for a while, moping.  Suddenly, her eyes widen with wakefulness as the full import of your words hits her.  \"<i>Um... [pc.name], you said... us?</i>\" she asks, flushing.");
	...
	if (pc.isNaga()) output("coil");
	else output("seat");
	output(" yourself, then grabs your arms and pulls them around her waist.");
	
	//forks in order of priority, no new pg at start
	//[(full gooey body)
	if (pc.isGoo()) {
		output("  She considers for a moment, rubbing away some of the slime you've deposited on her shorts, and pulls free from your slimy skin again.  \"<i>Um... do you... would you be offended if I took my kit off, so it doesn't get ruined?</i>\"");
		if (pc.slut() >= 40) output("  The leer you display is so prurient that she seems on the verge of reconsidering, but you compose yourself and");
		else output("  You");
		output(" give assent with little more than a broad smile.  Sheila pulls her shirt over her head, revealing her " + sheilaCup() + " breasts, but raises her face to you hesitantly when her hands get to her belt.");
		output("\n\nYou roll your eyes skyward.  \"<i>We have kids together.  I've seen it.</i>\"");
		
		output("\n\nSheila's blush deepens, but she nods and finishes undressing, pulling down her shorts and panties to expose her smooth pussy, then puts the clothing in a pile.  She turns her back to you and scoots in once more, leaving a tingle that spurs you to hold her closer when her flushed, embarrassed skin touches yours. \"<i>This feels good,</i>\" she says, \"<i>like a warm bath.</i>\"  Sheila giggles nervously as you instinctively pull her naked body deeper into your own.  \"<i>H-hey, [pc.name].  Don't swallow me, mate.</i>\"  You bubble happily as you relax around her, covering most of the exposed skin below her collarbone with a thin layer of yourself.");
		//(apply libido AND sens-based lust damage)
		pc.lust(pc.libido() / 5 + pc.cor() / 3);
	}
	//(naga)
	else if (pc.isNaga()) {
		...
		if (pc.slut() >= 40) output(" - but tighten the one sliding across her breasts just slightly.  Sheila's neck turns red and her ears twitch as you rub her nipples, and you plant a kiss on her nape");
		output(".  \"<i>Ahh,</i>\" she sighs, shifting in your coils.  \"<i>Hold it right there...</i>\"");
		//(apply libido-based lust damage if lib >= 40)
		if (pc.libido() >= 40) pc.lust(pc.libido() / 4);
	}
	//(horse and >=5ft)
	else if (pc.isTaur() && pc.tallness >= 60) {
		output("\n\nStrapped for space, her long feet still poke a good way out of the grass canopy, and she gets back up, tucking them under her body.  \"<i>Uh... [pc.name], not to be forward but...</i>\"  Her eyes dart toward your broad back, and you sigh and fold your legs as comfortably as you can before nodding.  She giggles and climbs atop you, then spreads herself generally around.");
		...
		if (pc.libido() >= 40) output("... or would be, if you weren't distracted by the nipples poking into you and the thighs squeezing your flank");
		output(".");
		//(apply sens-based lust damage if sens >=40)
		if (pc.libido() >= 40) pc.lust(pc.libido() / 4);
	}
	//(kitsune tails)
	else if (pc.hasTail() && pc.hasTailFlag(GLOBAL.FLAG_LONG) && pc.hasTailFlag(GLOBAL.FLAG_PREHENSILE) && pc.hasTailFlag(GLOBAL.FLAG_FURRED) && pc.hasTailFlag(GLOBAL.FLAG_FLUFFY) && pc.tailCount >= 6) {
		output("\n\nYou wrap your [pc.tails] around her as she settles in position, and Sheila wriggles against them.  \"<i>This is... weird,</i>\" she says, \"<i>but good.  Like warm, soft sleeping bag. </i>\"  With an unseen smile, you tighten the loops around her");
		if (pc.slut() >= 40) output(", sliding across her breasts just slightly.  Sheila's neck turns red and her ears twitch as you rub her nipples, and you plant a kiss on her nape");
		output(".  \"<i>Ahh,</i>\" she sighs, shifting in your coils.  \"<i>Hold it right there...</i>\"");
		//(apply libido-based lust damage if lib >= 40)
		if (pc.libido() >= 40) pc.lust(pc.libido() / 4);
	}
	//(harpy - minimum score for birdlegs at least)
	else if (pc.skinType == GLOBAL.SKIN_TYPE_FEATHERS) {
	...
	//(bees with boots)
	else if (pc.hasLegFlag(GLOBAL.FLAG_CHITINOUS)) {
		output("\n\nNo sooner does she get her head down than it's back up again.  You look a question at her.");
		output("\n\n\"<i>Your legs are so bloody hard and cold, [pc.name],</i>\" she clarifies brutally, tapping on the chitin digging into her back.");
	...
	//(eight-legged freaks)
	else if (pc.isDrider() && (pc.hasTail(GLOBAL.TYPE_DRIDER) || pc.hasTail(GLOBAL.TYPE_ARACHNID))) {
		...
		output("\n\n\"<i>[pc.name]... wow,</i>\" she mumbles, snuggling against you and looking over her shoulder.  \"<i>Do you cover all your girlfriends in sticky white strings?</i>\"");
		...
		pc.tailVenom -= 10;
		if (pc.tailVenom < 0) pc.tailVenom = 0;
	}
	//(else)
	...
	if (!pc.isGoo()) output("blood");
	else output("goo");
	output(" flowing, unsure if you'll be able to get to sleep, but Sheila has no such problems.  Within minutes, the woman is out, head nodding forward.  You suppose that means the first watch is yours... it passes slowly, despite the scenery, but eventually you nudge your partner awake when you can stay up no longer.");
	...
	if (!pc.isTaur()) output("  She grudgingly repositions, allowing you to rest against her.");
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE2():void {
	...
	pc.energy(20);
	pc.HP(pc.maxHP() / 4);
	processTime(timeUntil(5 * 60));
	output("Your sleep is fitful, but not totally useless, and you yawn and stir a few hours later when Sheila wakes you");
	if (pc.isGoo()) output("; she's already dressed and appears to have just been outside");
	output(".");
	...
	if (pc.lust() < 30) output("  You give her a hand, then help her load up.  She gives you a peck on the cheek before she departs.  \"<i>Thanks, love.</i>\"");
	if (pc.lust() >= 30) {
		output("  Part of you is actually sad to see your cozy hovel being disassembled if it means her warm body won't be pressed up against yours anymore, and your hands begin to rove over her as she works.");
		
		output("\n\n\"<i>[pc.name], stop,</i>\" she laughs.");
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDEREBUTLEAVEHERBE():void {
	clearOutput();
	output("You relent, allowing the woman to free herself with a peck on your cheek.  She loads up her catch, and then turns to you.  \"<i>Thanks, [pc.name].  Owe you one.</i>\"");
...
private function normalSheilaPregNotifREPEATEDEDHelpABitchOutTOCAMP():void {
	clearOutput();
	output("Wordlessly, you lift the bundle of game animals off of Sheila's shoulders and sling it over your own.  She looks up at you, confused enough to forget about her tears.  \"<i>What're you doing?</i>\"");
	...
	//[(int >=60)]
	if (pc.intelligence() >= 30) output("\"<i>The baby will be fine,</i>\" you venture, hazarding a guess.  \"<i>It's what the nurses are there for.</i>\"");
	else output("You slip an arm around her, and ask if she's okay.");
	output("  She nods mutely and pulls the food out of the fire.");
	//[(if Jojo present)
	if (flags["COC.JOJO_NIGHT_WATCH"] == 1 && flags["COC.JOJO_IN_CAMP"] == 1) output("  She extends one bird to you and another to Jojo.  He raises his hand and shakes his head, but politely.");
	else if (flags["COC.ANEMONE_WATCH"] == 1 && flags["COC.ANEMONE_KID"] == 1) output("  Sheila offers you one of the birds, then extends another to your odd tenant.  The anemone's mouth tightens and she shakes her head vigorously, blushing a deep blue.");
	output("  You eat in silence until the food is gone, and Sheila clears her throat.");
	...
	//[(if no watch)
	if (!(flags["COC.JOJO_NIGHT_WATCH"] = 1 && flags["COC.JOJO_IN_CAMP"] == 1) && !(flags["COC.ANEMONE_WATCH"] == 1 && flags["COC.ANEMONE_KID"] == 1) && !((pc.accessory is TamWolf) || (pc.accessory is TamWolfDamaged) || pc.hasStatusEffect("Varmint Buddy"))) output("\n\nShe gets to her feet as you grin, then makes her way over to your bedroll.  It doesn't take her long to fall asleep; you amuse yourself watching her toss and turn... and drool on your pillow.");
	//(if watch)
	else {
		output("\n\nYou grin and get up, and Sheila tails you to your bed.  ");
		if (flags["COC.SLEEP_WITH"] != undefined && flags["COC.SLEEP_WITH"] != "") output("She looks with some distaste at the nearby sleeper, and steps past.  You pick up the bedroll and follow her, reasoning that if she keeps her promise to be gone before dawn, it shouldn't raise any questions.  \"<i>Making friends, I see,</i>\" Sheila mutters... but as you unroll the bed again, her eyes zero in on it like a drowning man's on a raft, and she says nothing else.  ");
		output("She lies down first, allowing you to cozy up to her as far as you dare.  Just as you're about to drift off, you feel Sheila take your hand and pull your arm over her body.");
	}
...
private function sheilaGoesDemon():void {
	...
	if (flags["COC.SHEILA_XP"] < -2) output("always so bloody eager to fuck me!");
	else output("me best mate!");
	output("  Help out!</i>\"");
	...
	if (!CombatManager.inCombat) {
		...
		output("\n\n");
		if (enemy.isDefeated()) CombatManager.genericVictory();
...
private function demonSheilaEncounter():void {
	clearOutput();
	output("A shadow drops out of a low tree behind you with a rustle.  \"<i>Hey, lover; been a while... how about spending some time with me?</i>\"");
	...
	addDisabledButton(1, "LetHerFuck", "Let Her Fuck", "This option requires you to have sufficient arousal.");
	if (pc.lust() >= 33) addButton(1, "LetHerFuck", sheilaLetHerFuckYou);
	addButton(2, "Resist", demonSheilaResist);
	//run simply returns to camp and should be the default spacebar option
	addButton(14, "Run", function():*{ processTime(10 + rand(10)); mainGameMenu(); } );
}

//[Demon Sheila - Resist]
private function demonSheilaResist():void {
	clearOutput();
	output("You back away from the demon and raise your [pc.mainWeapon] with a dour glower.  Sheila purses her lips at you, then says, \"<i>Not in the mood, eh mate?  I'm so horny though... I bet I could get you into it.  Let me show you how much I love you...</i>\"  Her ears twitch furiously, belying her intentions as she");
	//[(sheila corruption < 30)
	if (sheilaCorruption() < 30) output(" thrusts her black spade forward and smears some of the constantly-oozing fluid from its tip down the length with her hand");
	else output(" kneads her breasts seductively, pinching her nipples");
	output(".  Your posture softens a bit as she reaches out to you");
	if (sheilaCorruption() < 30) output(", tilting her palm to dribble a handful of strong-scented dark oils");
	output(", but as she giggles sadistically and starts to approach, you tense up again.  It's pretty clear by the way she swaggers dismissively that she isn't taking the rejection well!");
	//go to fight
	processTime(1);
	clearMenu();
	addButton(0, "Next", sheilaCombatStart);
...
private function demonSheilaTalk():void {
	clearOutput();
	output("Questions nag at you, and you ask Sheila if she wouldn't mind answering a few of them.  She struts over and ");
	//[(not horse)
	if (!pc.isTaur()) output("takes your arm in hers, then slips the other hand into your armor");
	//[(horse)
	else output("between your legs");
	output(" to touch your ");
	if (pc.hasCock()) output("[pc.cock]");
	else if (pc.hasVagina()) output("[pc.vagina]");
	else output("smooth skin");
	//[(lib <20)
	if (pc.slut() < 20) output(", giggling as your expression petrifies");
	output(".  \"<i>Anything for you; I'll just find something to occupy my attention.  What's on your mind, love?</i>\"");
	//plus lust depending on sens
	applyDamage(new TypeCollection( { psionic : 5 + target.libido() / 10 } ), sheila, pc);
...
private function tellSheilaDemonToFuckOff():void {
	clearOutput();
	output("\"<i>Oh,</i>\" she says.  \"<i>Don't be like that.  You're my best mate, you know.  My beloved.  I'll never let you go, ever.</i>\"  Her eyes gloss over a bit, ");
	//[(RNG decides if dis bitch crazy, bias toward sane)
	if (rand(10) < 3) {
		output("and she blurts, \"<i>I know I can make you understand.  You can't go until you say it back!</i>\"  You pull her hand free and back away from her, but she advances again, trying to grab you!");
		processTime(1);
		clearMenu();
		addButton(0, "Next", sheilaCombatStart);
		return;
...
private function askDemonSheilaAboutDahChange():void {
	clearOutput();
	//[(if sheilacite >= 1)]
	if (flags["COC.SHEILA_CITE"] >= 1) {
		output("Sheila blushes deeply.  \"<i>Silly.  You were there.</i>\"  She looks at you, pupils dancing as she remembers the lovemaking session that led to her current form.  \"<i>I suppose that's not what you're asking, though.</i>\"");
		...
		if (pc.hasCock()) output(", and you feel a monument of your own being erected");
		output(".  \"<i>Every squeeze of my pussy compressed my focus, too, until my mind was clearer than the juice you made me flow with.  I realized my purpose there, with you.</i>\"");
...
private function askDemonSheilaAboutHerColony():void {
	clearOutput();
	output("\"<i>That place?  Do you wanna go there?</i>\"  Sheila smiles duplicitously, then lifts your chin and steals a kiss before you can pull away.  \"<i>Got no qualms about showing you the way to that nest of prudes, but...</i>\"  Her eyes darken.  \"<i>There's nothing left now.  A shame you didn't insist earlier, they could have stood to learn what a ripper " + pc.mf("man", "woman") + " is.</i>\"");
	
	output("\n\nShe giggles.  \"<i>The first thing I did when I got my new gift was to go home and share it with my loved ones, of course.  It felt incredible, [pc.name]!  When we climaxed and they drank of me, it pulled away all my lust and longing and left me full of happiness and complete oneness.  And every time one drained me dry, I would fill back up again, to pour myself into another.  Thanks to you opening the door, I was able to express my feelings for them like I never could.</i>\"  The girl leans her head against you and closes her eyes.  \"<i>Especially to the captain.</i>\"");
	
	//(corr <30)
	if (pc.cor() < 30) output("\n\nHorrified");
	else output("\n\nInterested despite yourself");
	output(", you ask what happened to him.  Sheila smiles serenely before answering.  \"<i>Unsurprisingly, his prick was all too eager to fill me with sperm after the countless sermons he preached about it being every woman's duty to expand the clan.  He barely lasted three minutes when I climbed atop him.  Oh, that little squirt was the most satisfying one.  Watching him melt under me drove home how much power I've always had... I wish I could have known when I was just an awkward little girl, sitting through his browbeating lectures on loyalty and servitude.</i>\"");
	...
	//(corr >=75)
	if (pc.cor() >= 75) output("harvested that day, you sigh wistfully");
	else output("stolen that day, you wince");
	output(".  Sheila, peeking at you with one eye from under her bangs, interprets your expression correctly.  \"<i>Wondering where the others went, love?</i>\"  She releases her grip on your arm and holds out the hand that's not exploring your [pc.gear], displaying several rings.  \"<i>The rings are my closest.  I kept them for sentimental reasons.  The rest, well... I ate them.</i>\"");
	
	output("\n\nAs you stare at her, she opens both eyes and meets your gaze defiantly.  Flashes of lavender dance in her cold irises, inclining you to believe her words.  \"<i>They were delicious, [pc.name].  Every one I consumed made me feel more powerful, bigger, more important, more like a sex goddess.  By the time I got to the colony's inner sanctum, I was knocking people over with a touch.  Weaker blokes were soaking their dacks with precum when they saw in my eyes what I wanted to do to them.  On that note...</i>\"  She looks harder at you, and you can feel a fantasy making its way into your mind, one where ");
	//[(cock)
	if (pc.hasCock()) output("she ties you down and rides you until she's so full of your spunk that she can push it back out just by pressing on her stomach");
	else output("she binds your hands and sits on you, teasing your helpless body with her oozing, throbbing spade until she comes all over you, spraying dark slimy fluid from it");
	output(".  You ");
	if (pc.cor() < 50) output("break eye contact, blushing");
	else output("stare into her shining eyes, getting hotter and hotter as the vision plays out to the very end");
	output(".");
	...
	applyDamage(new TypeCollection( { psionic : 5 + target.libido() / 3 } ), sheila, pc);
	processTime(3);
	clearMenu();
...
private function talkToDemonSheilaAboutWhatNow():void {
	...
	output("\"<i>What will I do now?  What else, my silly love?</i>\"  Sheila looks out over the plains.  \"<i>I'm always hot for you.  We could have a bit of a naughty, and make each other feel good.</i>\"");
	...
	//(lib>=30)
	if (pc.slut() >= 30) output("\n\nYour eyebrows rise.  \"<i>Got your attention, did I?  But,</i>\"");
	else output("\n\nYour mouth hardens.  \"<i>You're right,</i>\"");
	output(" Sheila says, \"<i>I'm going to save that one for our honeymoon, instead.</i>\"  Her eyes twinkle with infatuation.  \"<i>I could even pull the taint of this land out of your body and use it to make mine more lewd for you, my " + pc.mf("knight-champion","great lady") + "...</i>\"");
	...
	//(corr<50)
	if (pc.cor() < 50) {
		output("\"<i>I don't think I will, though, my sooky mate... I'd rather stain you in my colors, even if it costs my body to do it.  Or, would you like me more if my proportions were modest?  You, my confident, courtly " + pc.mf("lord", "dame") + ", and I, your worshipful, tomboyish " + pc.mf("squire", "lady-in-waiting") + ", secretly in love with you and game to follow you anywhere and do anything for you to make you acknowledge me and accept ");
		if (pc.hasCock()) output("my pussy");
		else output("the dick I grow for you");
		output("?  </i>Anything?<i>");
	}
	else {
		output("\"<i>... would you like that?  If my pointers were so big that all I could do was wait for you patiently as your sex-toy, your possession; growing whatever new parts you need to complement yours, relying on you and welcoming you home at the end of the day, inviting you to rest in the bosom you created after we lie together and make our babies... would that gratify you?  Would you love me then?");
	}
	output("</i>\"");
	
	output("\n\nSheila releases your face and looks forward again.  \"<i>I want you.  I want children with you too, if you'll ");
	if (pc.hasCock()) output("give them to me");
	else output("have them for me");
	output(".  I dunno what could be simpler.</i>\"");
	
	output("\n\nShe's so fixated on having children now, more so than anyone you've met");
	//[(PC has been propositioned by Amily)]
	if (flags["COC.AMILY_OFFER_ACCEPTED"] > 0) output(" save one.  Actually, the two are a bit reminiscent of each other..");
	output(".  Yet, any offspring a demon makes will be imps, right?");
	...
	if (pc.cor() < 50) output("; even though you hide it, I've seen it lurking");
	output(".  Feel like flying your colors for a while?</i>\"");
	
	//plus libido-based lust damage, output choices [Another Question][Let Her Fuck(req lust >=30)][Resist]
	applyDamage(new TypeCollection( { psionic : 5 + target.libido() / 15 } ), sheila, pc);
	processTime(3);
	clearMenu();
	addButton(0,"Another Q.",demonSheilaTalkAnotherQuestion);
	if (pc.lust() >= 33) addButton(1,"LetHerFuck",sheilaLetHerFuckYou);
...
private function demonSheilaTalkAnotherQuestion():void {
	...
	//(if lust <=99)
	if (pc.lustQ() <= 99) {
		output("Sheila reaches farther down, ");
		//(cock)
		if (pc.hasCock()) output("slipping more of her finger and thumb around your penis");
		else if (pc.hasVagina()) output("parting your lips more");
		else output("rubbing your bare groin");
		output(" even as she smiles encouragingly.  \"<i>Sure, mate.  Amuse yourself.  I certainly intend to.</i>\"");
		//plus sens-based lust damage, display talk options again
		applyDamage(new TypeCollection( { tease : 5 + target.libido() / 20 } ), sheila, pc);
		processTime(1);
		clearMenu();
		addButton(0, "The Change", askDemonSheilaAboutDahChange);
		addButton(1, "Her Colony", askDemonSheilaAboutHerColony);
...
private function demonImpChildren():void {
	clearOutput();
	output("\"<i>[pc.name]!</i>\"  A swishing in the grass behind you is the only prelude you get to a nude Sheila pressing herself against you in a hug, ");
	//(sheila corruption < 40)
	if (sheilaCorruption() < 40) output("her perky nipples rubbing into your arm as she snuggles up");
	else output("her enormous, pendulous breasts practically parting around you to wrap you up alongside her arms");
	output(".");
	
	output("\n\n\"<i>Guess what?  I had your baby!  He was the most darling little thing.</i>\"");
	
	output("\n\nShe takes your look of surprise as permission to elaborate.  \"<i>Mmm, why the wide eyes, mate?  Our people always pushed them out fast.  Except instead of tucking him into a snuggly against my tits, I tucked him into somewhere a bit... lower.</i>\"  She hugs your arm closer, fairly rubbing her pussy on the back of your hand and leaving a trail of her lube as she slides her own fingers into your [pc.gear].  \"<i>He has his " + pc.mf("daddy", "other mommy") + "'s cock.");
	//(if no cock)
	if (!pc.hasCock()) output("  Or, had...</i>\" she finishes with a frown as she reaches your bare groin.  \"<i>Where'd you leave that magnificent old fella?  I was lookin' forward to another go.</i>\"  She pulls her hand out, then leans toward you with a pout.  \"<i>I'm so horny, too...</i>\"");
	//(if cock)
	else {
		output("  Still, it's not the same as having you inside me...</i>\"  She squeezes the neck of your [pc.cock] and then pulls her hand away, slipping it between her own legs.  \"<i>I was thinking you could help me make another, and maybe up the population of men worth a damn around here.</i>\"  Reclining, she spreads her thighs, pulling apart her small, smooth labia ");
		//[(sheila corruption <=80)]
		if (sheilaCorruption() <= 80) output("with her fingers to make sure the invitation is all the way open");
		//(else)
		else output("with her split tail while she cradles her massive tits to make sure you see the open invitation");
		output(".  Impatiently, she rubs her stubby, drooling, phallic spade against the hole, accentuating the imagery.  You can feel the blood rushing to your groin as she gives herself shallow, teasing strokes.");
	...
	addDisabledButton(1, "Knock Up", "Knock Up", "This option requires you to have cock.");
	if (pc.hasCock()) addButton(1, "Knock Up", pregDemonSheilaKnockUp);
...
private function pregDemonSheilaOtherSex():void {
	clearOutput();
	output("Her eyebrows knit as you propose an alternative.  ");
	//[(if cock)
	if (pc.hasCock()) {
		output("\"<i>Well now, I think I get a say, don't I?  I was really hoping to have your seed inside me... it's practically worth bottling.</i>\"  She ");
		if (sheilaCorruption() >= 80) output("heaves onto her feet, heavy breasts swaying");
		else output("nimbly hops to her clawed feet");
		output(", and runs her hands distractingly along the curves of her hips.  \"<i>You can do what you like with my body, love... if you can convince me.  Of course, I'll claim the same privilege!</i>\"  Sheila jumps at you, leaving no doubt what kind of 'convincing' she expects to do!");
		processTime(1);
		clearMenu();
		addButton(0, "Next", sheilaCombatStart);
	}
	//(if no cock)
		...
		//go to fight if cock, else plus lust and present victory sex menu
		 applyDamage(new TypeCollection( { tease : 5 + target.libido() / 10 } ), sheila, pc);
		 processTime(1);
...
private function pregDemonSheilaKnockUp():void {
	clearOutput();
	output("As she smears the entrance to her snatch with the slippery slime drooling out of her slitted spade, [pc.eachCock] hardens painfully.  The eye-fucking she's giving you while teasing her breasts doesn't help matters, and soon you're so erect that you have to expose your shaft just to relieve the pressure.  With the bulk of your dick bobbing free of your [pc.gear], Sheila waggles her eyebrows at you.  \"<i>So... yes?</i>\"");
	//output demon Sheila cock victory sex scene consonant with player's length and suppress gems/xp/item gain
	processTime(2);
	clearMenu();
	if (pc.cockThatFits(sheila.vaginalCapacity()) >= 0) addButton(0, "Next", missionaryForThePurposesOfCreatingImpsWithSheila);
...
//[You lower your [pc.mainWeapon], unable to continue the fight.  ]
...
internal function loseToSheila(consensual:Boolean = false):void {
	...
	if (pc.HP() <= 1 && !consensual) {
		output("Your erstwhile opponent's eyes glimmer with excitement as you collapse from your injuries, and she runs over to you.  The demon strips off your [pc.gear] eagerly, but you can't stay awake for the fun.  Consciousness slips away and you pass out.");
		//--Next--
		...
		if (pc.hasCock() && pc.cockThatFits(sheila.vaginalCapacity()) >= 0) 
			choices[choices.length] = loseToNormalSheilaAndGetRidden;
		//Loss - tail-spade sounding for fuckhueg cock (for cockarea > 56)
		if (pc.hasCock() && pc.biggestCockVolume() >= sheila.vaginalCapacity())
...
private function loseToDemonSheila():void {
	...
	output("\n\nStill shaken, you stand and let Sheila do the same.  She narrows her eyes at you before speaking again.  \"<i>Be more careful, mate.  The idea of my " + pc.mf("boy", "girl") + " being fucked by trash makes me ill.</i>\"  Saying nothing more, the woman turns and leaves you to walk back to your camp.");
...
private function loseToNormalSheilaAndGetRidden():void {
	clearOutput();
	var x:int = pc.cockThatFits(sheila.vaginalCapacity());
	output("Sheila pushes you onto the ground");
	if (pc.isTaur()) output(" and you're forced to put your forelegs down for support as you come to rest on your haunches");
	else output(", spreading your legs.  The demon unfastens your [pc.gear] and strips it from your bottoms easily");
	output(", exposing your [pc.cocks].");
	
	output("\n\n");
	//[(multi)
	if (pc.cockTotal() > 1) output("\"<i>All of these look ripper, but... I want this one today,</i>\" she says, selecting your [pc.cock " + x + "].  ");
	output("She wraps her hand around it and begins pumping up and down, squeezing out a drop of precum that she smears onto her palm.  \"<i>You're so eager,</i>\" Sheila giggles.  \"<i>Are you already about to burst?  That's no good... I wanted to play a bit more.</i>\"");
	
	output("\n\nThe demon releases your dick, watching raptly as it bobs and twitches, then scoots up to you and");
	if (pc.isTaur()) output(", draping your forelegs over her shoulders,");
	output(" presses her " + sheilaCup() + " breasts");
	if (sheilaCorruption() < 100) output(" against you, dragging her nipples against yours.  ");
	else output(" against you so forcefully that they part around your chest.  ");
	output("\"<i>Such a " + pc.mf("handsome bloke", "pretty sheila") + ", with such a nice dong,</i>\" she whispers, \"<i>I'm going to have to take you for a ride.  Try not to come as soon as you get in!</i>\"");
	
	output("\n\nSheila grins viciously and sinks down on your [pc.cock " + x + "], sliding it into her pussy at a painstaking pace");
	//[(multi)
	if (pc.cockTotal() > 1) output(" and slipping the unused remainder of your manhood between her thighs");
	output("; you can feel every new inch of heat as she lowers her body.  The woman moans, gyrating her hips, and [pc.eachCock] twitches in painful erectness.");
	...
	if (pc.balls > 0) {
		...
		output("\n\nBehind the demon, her black, stubby spade dangles ominously above your [pc.sack] and its brown neighbor wraps around it, squeezing and stroking.  A drizzle of dark, oily secretion falls from the end; as it lands, your [pc.balls] begin to heat up painfully.  Your stomach turns as another drop of hot slime drools from your demonic lover onto your tender nuts; it feels like something is inside them, trying to push outwards in all directions.");
		//(not horse)
		if (!pc.isTaur()) output("  Sheila takes your face in her hands and presses it into her breasts as she reassures you.");
		...
		output("\n\n\"<i>Ahaha,</i>\" Sheila laughs, \"<i>got lots of little spermies for my pussy now?</i>\"  Indeed, you feel pent-up just looking at your newly-enlarged [pc.balls]... like if you don't blow your load soon, it's going to start leaking from your pores!  Your [pc.cock " + x + "] hardens again, more from desperate necessity than from pure arousal; your demon hums in pleasure as it fills her vagina, and she begins to raise and lower her hips, slapping her ass against your sensitive [pc.sack] and whispering to you.  \"<i>[pc.name]... I'm getting off on just the idea of you flooding me with all that semen.</i>\"");
		
		output("\n\nYou groan in agreement as she slides your turgid shaft back into her pussy on a particularly well-angled stroke, dragging the [pc.cockHead " + x + "] against her vaginal wall and forcing out a thick stream of lubricant.  \"<i>God,</i>\" the demon gasps, \"<i>even your prostate is working overtime, mate!  Your precum feels like an ejaculation by itself... I can't wait for you to come any longer!  Give your seed to me, now!</i>\"");
		output("\n\nSheila's pussy spasms around you and she looks into your eyes; you feel a wave of compulsion from them, and arousal flows into you from each place your " + pc.skin() + " touches her body.  Unable to withstand her orgasm-fueled power, your [pc.balls] pull close to your shaft and you let go of your climax, grabbing her hips and dumping semen into her cunt");
		if (pc.cockTotal() > 1) output(" and lap");
		output(" just like she's dumping feelings into your head.  \"<i>Lover!</i>\" she screams.  \"<i>Pack me with your babies!</i>\"");
		
		output("\n\nAs if you could help it at this point... your [pc.cock " + x + "] fills her pussy with thick spunk and the pressure begins forcing it into her womb; as Sheila's mouth hangs open in awe, your balls shift upwards again and a second orgasm begins!  You force her down to the base of your cock using your ");
		if (pc.isTaur()) output("forelegs");
		else output("hands");
		output(", displacing a glob of cum from her pussy that's soon followed by others as you ejaculate once more.  The jizz is so dense with sperm that it's thick, almost colloidal; it jiggles on her skin, barely sliding downward as your hips ram against hers.  It takes nearly ");
		//(normal skeet)
		if (pc.cumQ() < 250) {
			output("ten minutes and two");
			processTime(15 + rand(5));
			pc.orgasm();
			pc.orgasm();
		}
		//(big skeet)
			...
			output("thirty minutes and four");
			...
			output("fifty minutes and eight");
		...
		output(" more orgasms, but finally your balls empty of their demonically-induced load and your lover pulls away and stands up.");
		...
		flags["COC.INCUBATION_SHIELA"] = timeAsStamp + 72 * 60;
		pc.ballSizeRaw += Math.PI;
		if (pc.ballSizeRaw < 10) pc.ballSizeRaw += Math.PI;
	...
	//(else if no balls)
		...
		output("\n\n\"<i>Oi, [pc.name],</i>\" Sheila says, pausing now that she's reached the base of your [pc.cock " + x + "].  You look up at her, waiting for the other shoe to drop.  \"<i>Where do you suppose your seed comes from?</i>\"");
		output("\n\n\"<i>... What?</i>\" you ask.");
		...
		if (pc.isTaur()) output("You force her downward with your forelegs to");
		else output("You grab her hips and");
		output(" help her impale herself on your tool.");
		
		output("\n\nThe demon shudders as your [pc.sheath] bumps up against her vulva.  \"<i>Y-yeah, but I never paid attention to growing dongers without balls and that kinda arcane stuff, mate... didn't think I'd end up fucking a strange " + pc.race() + " like you until you stuck it in me.  I always wanted to marry a normal bloke like my dad.</i>\"");
		...
		//[(corr < 70)]
		if (pc.cor() < 70) {
			output("\n\nYou stiffen in embarrassment.  Sheila smiles and plants a kiss on your " + pc.skinFurScales() + ", then looks at you innocently.  \"<i>Don't be mad, mate.  I just wanted our time together to last.</i>\"");
			output("\n\n\"<i>Is that so?</i>\" you reply, beginning to fuck the demon harder to make up the arousal she frittered away.  Sheila moans as you work your [pc.cock " + x + "] into her hole vigorously, dragging it against the entrance and smearing your intermingled fluids on her groin.");
		...
		if (pc.cockTotal() > 1) output(" and lap");
		//[(big skeet)
		if (pc.cumQ() >= 1000) output("; she quickly reaches capacity and pulls off, jerking your messy, slimy [pc.cock " + x + "] and letting you squirt the last of your strokes all over her face and chest");
		if (pc.cumQ() >= 6000) output(".  So much is left that she's practically underwater by the time you're finished; she giggles and bubbles form in the oozing liquid covering her lips");
		output(".");
...
private function tailSpadeSoundingForFuckHugeDongsWithDemonSheila():void {
	...
	clearOutput();
	output("The demon opens your [pc.gear] and undresses you, inspecting your crotch.  \"<i>Going a bit overboard, aren't you, [pc.name]?</i>\" she taunts, rubbing your [pc.cock " + x + "] with her knee.  \"<i>I'm not quite magical enough to take this monster - maybe after a few months of fucking everything I met, I'd be that loose.</i>\"  Sheila giggles at you.");
	
	output("\n\nYour prick hardens in response to the stimulation, rising to attention, and she traces a finger up it longingly.  \"<i>Not that I wouldn't like to have this beaut inside me... maybe we can do the next-best thing.</i>\"  She pulls it down until it's parallel to the ground, then turns around and slips the [pc.cockHead " + x + "] between her thighs, rubbing it against her pussy and mingling her fluid with the precum that oozes out.  \"<i>Nice and slick,</i>\" she confirms, fingering your urethra and sending a shiver through you.");
	
	output("\n\nSheila takes a half-step forward and bends over, letting your dick out from between her legs; her brown tail catches it, lassoing the shaft just behind the crown.  The smooth skin feels quite nice, and Sheila looks over her shoulder at you.  \"<i>Enjoying the view?</i>\" she asks, shaking her round ass and swinging your [pc.cock " + x + "] side to side with it.  Another drop of precum answers her.");
	
	output("\n\n\"<i>Hold on tight,</i>\" she warns.  Before you can ask what she means, Sheila raises her black spade to the tip of your dick, smearing its oozing secretions onto your slit.  It burns a lot; all you can do is bite your lip and not call out.  Carefully watching your expression, the demon presses her spade insistently into the exit of your urethra, working it wider and inundating your [pc.cockHead " + x + "] with her dark oils.  You can feel it nearly penetrating you now - Sheila shivers and squeezes her breasts as the widest part of the spade slips into your hole.  As soon as it passes the ring, she smiles wickedly.  \"<i>Here it comes!</i>\"");
...
private function clitSwellingDemonSheilaClitSoundingAnal():void {
	...
	var x:int = pc.biggestCockIndex();
	//(no horse)
	if (!pc.isTaur()) output("Sheila places her hand in the small of your back and leans in, drawing closer to your face even as she pulls your waist inward.");
	else output("Sheila sets her hand atop your withers, pressing down gently as she looks into your eyes.");
	output("  \"<i>Why don't you relax, [pc.name]?  Lie down and let me take care of you...</i>\"  Her eyelids lower and her mouth spreads in a sly smile, and for just a moment you lose your nerve, looking into her devilish expression.  \"<i>Don't worry, love,</i>\" she says, straightening her face.  \"<i>I promise you'll enjoy this.</i>\"");
	
	output("\n\nBefogged but still reserved, you nonetheless allow Sheila to lower you to the grass and remove your [pc.gear].  She ");
	//[(not horse)
	if (!pc.isTaur()) output("turns around, then straddles your groin and looks over her shoulder.");
	//(Equus ferus caballus)
	else output("circles you until she reaches your rump, then turns around and kneels down herself, looking at you over her shoulder.");
	output("  Her black, spaded tail lifts from between her thighs and snakes toward your pussy,");
	//[(if manparts)
	if (pc.hasCock()) {
		output(" caressing ");
		if (pc.balls > 0) output("your [pc.balls]");
		else output("the underside of your [pc.cock " + x + "], before");
	}
	output(" forcing your lips apart and lingering on your rapidly-swelling " + pc.clitDescript() + ".  \"<i>Oh, my.  What's with this ");
	if (pc.clitLength <= 6) output("lewd little");
	else output("grotesque");
	output(" thing doing growing from a pretty girl like you?  It's just the perfect size for some fun.</i>\"");
	
	output("\n\nYour eyes open in alarm at the same time as her mouth does in pleasure; she jams the slit of her spade onto your button");
	if (pc.clitLength > 6) output(", taking it several inches deep");
	output(".  Your head swims as pleasure from your engulfed clitoris overwhelms you, then toes the line to genuine pain.  The copious demonic fluid backing up inside her tail begins to leak around the tip, coating your clit and drooling onto your labia; your muscles tighten as the heat escalates past comfortable levels, and you begin shifting violently, trying to pull your swollen chick-stick free from the demon's anatomy.  \"<i>Ah, ah, god, [pc.name], stop!  Don't thrash so!  You're teasing me too fast!</i>\"  Sheila's words are lost on you, sunken as you are into a hell of oversensitized skin and constant stimulation.  It feels like the nerves exposed to the torment are even multiplying, as if skin you weren't even feeling before suddenly appears to be immersed into the tainted slime.");
	output("\n\nYour eyes roll in temporary shock as her tail squeezes around your " + pc.clitDescript() + " tighter and tighter; the siege lifts slightly, allowing you to focus with supreme effort and see that the demon is similarly affected!  Her head sags between her shoulders, even as she places two hands on the ground in front of her to steady herself.  The two tails on her butt whip spastically, squeezing another wave of painful pleasure into your crotch.  Staring, you can only watch as Sheila's thrashing black spade describes a clipped arc, a long, thick rod of flesh growing out of it to join your bodies.");
	
	//[(<=6")
	if (pc.clitLength <= 6) output("\n\nAwareness comes to you slowly; that flesh is your clitoris, gratuitously mutated by Sheila's fluids!");
	else output("Though you're by now accustomed to the sight of your clitoris, more like a cock than a dainty feminine bump, this is something else altogether!  The huge thing poking into her tail is almost twice the thickness of the slim growth you remember and bulges with blood vessels; as you watch it, transfixed, a trickle of Sheila's oily, dark secretion slides down it, then is absorbed into some flesh near your groin, adding another layer of width with a pulse!");
	output("  \"<i>Fuck, it's so tight!</i>\" the demon gasps, now resting on her elbows with her chest and face pressed to the grass.  Her tail jerks again, yanking your stalk this way and that and releasing more of her backed-up fluids.  They slide all the way to the bottom of your rod before sinking into your skin.");
	
	output("\n\nThe burning has tapered off near the end of your clitoris, now more properly a lever than a button; only the base still tingles with the barely-bearable sensation that you realize to be new growth.  \"<i>Bloody hell, [pc.name],</i>\" Sheila calls, forcing her elbows to straighten under her again.  \"<i>If you get much thicker, you'll split me off another tail.</i>\"  Your mouth only opens dumbly at this.");
	
	//divide scene here by clit size
	//(clit<=6" initially)
	if (pc.clitLength <= 6) {
		output("\n\nShe sits up on her knees, then turns to face you.  ");
		if (!pc.isTaur()) output("Taking your face into her hand, she closes your jaw and then leans in to plant a kiss on your chin.");
		else output("Her eyes glitter with mirth.");
		output("  \"<i>Why the look, mate?  Seems like you're saturated for now, and just in time.  This... is...</i>\"  She pulls her tail up from the end of your clitoris, then slides it back down, slowly.  \"<i>... perfect!</i>\"  As her tight hole swallows your sensitive distaff staff once again, your eyes roll and her head drops, dragging her hair");
		//[(horse)
		if (pc.isTaur()) output(" through the grass");
		else if (pc.biggestTitSize() >= 1) output(" between your " + pc.allBreastsDescript());
		else output(" along your flat chest");
		output(".  \"<i>Ffuck, even your girl parts are amazing.</i>\"  Sheila plants kiss after kiss on your ");
		if (!pc.isTaur()) output("[pc.chest]");
		else output(pc.buttDescript());
		output(" as she shallowly pumps your " + pc.clitDescript() + " in and out of her spade.  Your pelvis twitches uselessly; with her tail waving in the air instead of braced against her, controlling the pace of the fuck is beyond you.  \"<i>Naughty sheila,</i>\" she says, squeezing your wiggling hip with a hand.  You think privately that she could just as easily be talking about herself.  \"<i>Am I not going fast enough for you?  Let me speed up.</i>\"");
		...
		if (pc.cor() < 50) {
			...
			if (pc.wetness() >= 4) output("demon's legs");
			else output("grass");
			output(", and your clit swells even further inside your tormented partner.  Sheila groans, \"<i>Fuck you, [pc.name]...</i>\"");
			
			output("\n\nIrritated, she wraps her other frantic tail around the still-straining black one and squeezes, finally managing to get some of her climax past the ring of tight skin at the end; with the fluid forming a bridge, you feel the tension release as a wave of her black, slippery tail-cum forces her slit wide open and gushes out to soak your clit and then your pussy underneath.  \"<i>Ah!  Ah!  Finally!</i>\" she cries, great gouts of agitated slime sliding down your " + pc.clitDescript() + " to bubble and lick at your pussy, as if trying to get inside.");
		}
		//(else PC corr>=50)
		else {
			output("her tail quakes, then pushes her pussy into yours.  \"<i>God, yes!</i>\"  A spatter of hot fluid hits your " + pc.vaginaDescript(0) + " as your demon lover climaxes, and another begins to drool around the tip of your clit, still suspended inside her writhing tail.  The sudden labial heat is too much for you, and you answer her with a climax of your own, ");
			if (pc.wetness() >= 4) output("drizzling");
			else output("copiously soaking");
			output(" her pussy with your own girl-jizz.  It feels like the nearness of her own cunt pulls your orgasm into obscene lengths, and you come repeatedly, drooling more fluid onto her wanton pussy as she grinds its lips against yours.  She rubs her breasts as she strokes her convulsing vagina against your own, pinching and pulling them until you swear you can see them lengthen.");
		}
	...
	//(else if clit >6" initially)
		...
		//[(horse)
		if (pc.isTaur()) output(" and rolls onto her back");
		output(", then wraps her tails around your thickened clit.  They twist and pull, sending spirals of pleasure along the numerous nerves, and begin gently but insistently tugging you toward her tan butt.  Sheila directs the tip right between her cheeks, and you can feel her using it to rub the ring around her hole.  \"<i>I think this should be about the right size for you, love,</i>\" she teases, spearing herself on your " + pc.clitDescript() + ".  Inch after inch slides into her slowly, maddening you with warmth, until ");
		if (pc.clitLength <= 12) output("all of it is in her bowels and her hot cunt rests against yours");
		else output("she's taken as much as she can");
		output(".  She half-smiles and half-winces at you.  \"<i>Don't usually take it in this hole, but I wanted to feel your pussy going crazy for me.</i>\"  ");
		if (pc.clitLength <= 12) output("Her hips shift, dragging her lips across yours and stirring your clit through her asshole even as it squeezes you; your pussy twitches at the stimulation.");
		else {
			output("Her tails, one still swollen from being stretched and tormented by your growing clit, slip into you abruptly, bringing their mild burning sensation with them; if you hadn't been saturated with it already, you'd probably pass out.");
			//[(if virgin vagina)
			if (pc.vaginas[0].hymen) output("  \"<i>What's this?</i>\" Sheila teases.  \"<i>Why, your virtue is still intact, little girl!  I wouldn't want anything to happen to it!</i>\"  She pulls her tails out a bit, contenting herself with simply teasing your lips and driving you mad for penetration.");
			output("  The demon shifts around your clit, sending a wave through you that makes your lips wrap around the invaders.");
		}
		
		output("\n\nSheila places her long-taloned feet against ");
		//(not naga)
		if (!pc.isNaga()) output("your thighs, spreading your [pc.legs] wide and then hooking them over top to touch ");
		output("the ground next to your ass, and uses the leverage to begin pumping your clit out of and into her asshole, caressing your " + pc.vaginaDescript(0) + " every time she slides home.  \"<i>I could get used to this, love.  What say I keep you dosed up like this and you become my dildo?  I'll ride you with a different hole every time!</i>\"  Your eyes roll back in your head, at least partially from the irregular pace at which she's stroking your clit.  Sheila grabs her breasts as she slows down drastically once again, mashing them with her hands and sinking onto you an inch at a time.  \"<i>Fuck... I love your junk, [pc.name].  Maybe I should catch a goblin, too, and make her tease my pussy while you fill my ass.</i>\"  She raises back up and ");
		//[(clit>12")
		if (pc.clitLength > 12) output("pulls her black tail free, dripping with your juice, then ");
		output("begins to work her drooling spade along your clit again, smearing her goo along your chunky chick-stick.  You can feel the edges of the stretched hole in her tip working on your skin, opening and closing ever so slightly as her copious tainted lube oozes out.  Your " + pc.clitDescript() + " must be completely saturated already, because the slippery goo lingers on the surface instead of wicking into it.  \"<i>Gonna g-get you nice and wet, love...  Ohh,</i>\" Sheila says, affected to distraction by the pleasure of tracing the slick end of her stubby tail along you.");
		
		output("\n\nFinally coating you to satisfaction with the oily secretions, Sheila begins pumping again.  The heated friction from before evaporates, replaced with a dull, distributed warmth as she slides her asshole along your clit.  With less resistance, she's able to up the tempo to frantic, scraping along the ground so fast that she begins to tear up the grass.  You bear up as much as you can under the onslaught of pumping and gyrating hips, but eventually the sensation is too much, and your nerves take over as your climax begins.  Your " + pc.vaginaDescript(0) + " spasms, drooling onto her ");
		if (pc.clitLength <= 12) output("pussy; her smooth, small labia greedily lap at the fluid when they touch, brushing against yours and drawing further twitches and squirts.");
		else output("teasing tails; they shift from stroking your labia to holding them open, allowing your pussy to squirt as it pleases.");
		output("  Suddenly she stops mid-stroke as her asshole quivers, sending shivers up your orgasm-sensitized clitoris.  \"<i>Fffuck!  So soon!  I'm coming too, [pc.name]!</i>\"  Sheila sinks down, burying your " + pc.clitDescript() + " into her asshole.");
		//(clit>12")
		if (pc.clitLength <= 12) {
			output("  Her tails sink into you as she squirms, parting your lips and pushing their way inside.");
			//[output hymen check, but don't remove virgin flag yet]  
			if (pc.vaginas[0].hymen) output("  <b>You've lost your virginity!</b>");
			output("  Buried in your pussy, the spade begins drooling its black fluids in greater and greater volume as Sheila is wracked by her orgasm.");
		}
		
		//(PC corr<50)
		if (pc.cor() < 50) {
			output("  The demon's pussy gushes and pours with unnatural levels of fluids, soaking your crotch as a miniature cataract makes its way out of her lower lips.  Sheila spills herself onto you for at least a minute, producing more hot fluid than you thought possible, until she looks utterly dehydrated - even her breasts seem to be smaller and less jiggly.");
		}
		else output("  The demon's skin seems to parch as she grinds into you relentlessly, evaporating your fluids so fast that most of them don't even have time to fall.  Her hot flesh lays heavily across your netherlips, prolonging your climax and coaxing out more fem-spray than you thought was in you.  Even your mouth seems to dry as she drinks your essence up.");
		
		output("\n\n\"<i>Ahh, [pc.name],</i>\" Sheila says, after a minute to recover.  \"<i>It feels good in my butt.</i>\"  Slowly, she pulls apart from you, letting your clit out of her backdoor at an agonizing pace, and stands up.");
		//[(if clit >12" and virgin vag)
		if (pc.clitLength > 12 && pc.hasVirginVagina()) {
			output("  \"<i>Sorry about your hymen, love, but I just couldn't resist making a sweet little slut like you into my girl.  I hope you'll remember me - I only wish I had a proper donger so I could give you my baby, too.</i>\"");
		}
		output("  Even as tired as you are, what you see on the demon girl makes you laugh.  She starts at this, then follows the direction of your gaze; ");
		if (!pc.isTaur()) output("her tits are covered by grass stains from her frenetic pumping.");
		else output("craning around shows her that her asscheeks are completely green and wet with torn grass from slamming into you so violently and repeatedly.");
		output("  \"<i>... Oh, god dammit,</i>\" Sheila gripes.  She walks over and forces your face into them with a long, slow shake, then giggles as a joke comes to her.  \"<i>There, [pc.name]; I've stained you with my colors!</i>\"  She plants a long kiss on your grass-smeared forehead, then drops you back to the ground, straightens up, and departs.  Exhausted, you slip into a doze, wondering if you'll be able to fit your swollen clit back into your clothing when you wake or if you'll have to walk back to camp with it sticking out.  It slowly deflates as you sleep, but does retain a bit of additional length.");
	}
	...
	if (pc.clitLength > 12 || pc.cor() < 50) {
		pc.cuntChange(0, 20, false, false, false);
		pc.loadInCunt();
	}
	pc.clitLength++;
	//if (pc.findPerk(PerkLib.BigClit) >= 0) pc.clitLength += .5;
	if (pc.cor() < 50) {
		pc.cor(10);
		sheilaCorruption(-10);
	}
...
private function aintGotNoGenderAndKangarooRaped():void {
	clearOutput();
	output("Sheila sashays over to you and brazenly sticks her hand in your featureless crotch.  \"<i>It's too bad you've mutilated yourself like this, love... or did you do it just to tease me?  You know, it doesn't bug me that much.</i>\"");
	...
	output("\n\n\"<i>I'll see you later, [pc.name]... think about me.</i>\"  You slip into a brief, tired doze as her lingering magic guarantees fulfillment of her request.");
	
	//big lib-based lust gain, med lib gain if lust hits 100, pass 1 hour
	applyDamage(new TypeCollection( { tease : 20 + target.libido() / 4 } ), sheila, pc);
...
internal function beatUpDemonSheila(showOutput:Boolean = true):void {
	if (showOutput) {
		clearOutput();
		output("\"<i>Ahh, you win, mate.  I give up.</i>\"  Sheila slumps to the ground, ");
		if (enemy.HP() <= 1) output("and her wounds begin to close with a soft lilac glow.  \"<i>So fuckin' strong.  I don't suppose you've got any sperm for me, do you?  I'd love to have some of your kids...</i>\"  She looks at you beseechingly, gently fingering herself.");
		//(lust)
		else output("frigging herself vigorously.  \"<i>Do with me what you will, lover; those were the terms.  Won't you take my body to satisfy yourself, you brute?</i>\"  The audacious woman sticks her tongue out, as if to goad you into treating her roughly.");
	}
	...
	addDisabledButton(0, "Missionary", "Missionary", "This scene requires you to have fitting cock.");
	addDisabledButton(1, "AnalHateFuck", "Anal Hate Fuck", "This scene requires you to have fitting cock.");
	addDisabledButton(2, "UseHerThighs", "UseHerThighs", "This scene requires you to have overly large cock.");
	addDisabledButton(3, "Clit-Fuck", "Clit-Fuck", "This scene requires you to have vagina.");
	...
	if (pc.hasCock() && pc.cockThatFits(sheila.vaginalCapacity()) >= 0) addButton(0, "Missionary", missionaryForThePurposesOfCreatingImpsWithSheila);
	...
	if (pc.hasCock() && pc.cockThatFits(sheila.analCapacity()) >= 0) addButton(1, "AnalHateFuck", analHateFuck);
	//Win - [Big Dick+Thighs] (usable for dicks with cockarea > 56) (all 10 of them)
	//penis envy, go!
	if (pc.hasCock() && pc.biggestCockVolume() > sheila.vaginalCapacity()) addButton(2, "UseHerThighs", bigDickAndThighs);
	//Win - [Penetration, in Spades] - for self-loathing vaginas
	if (pc.hasVagina()) addButton(3, "Clit-Fuck", winAgainstDemoNSheilaForVaginas);
}
...
private function missionaryForThePurposesOfCreatingImpsWithSheila():void {
	clearOutput();
	output("You unfasten your [pc.gear] and strip down completely, as Sheila's eyes drink up your bare flesh.  Denuded, you caress your cock with several strokes, producing a drop of precum.");
	...
	if (pc.cor() < 40) output("Your ardor subsides a bit at her performance, but seeing your look of contrition, ");
	else output("Your scowl hardens at her shameless playacting, and ");
	output("she breaks into laughter.  \"<i>I'm just having you on, love.  You know my body's yours for the using.</i>\"");
	
	output("\n\nShe tosses her hair out of her face and extends her hand to you.  \"<i>No need to be coy about wanting me, my best mate.</i>\"  When you place your own hand in hers grudgingly, she pulls you forward until you're positioned atop her.  You can feel a gentle pressure on the underside of your [pc.cock " + x + "] as one of her slick tails guides it into position.  \"<i>This is where you belong, after all.  Welcome home.</i>\"  She wraps her arms around your ");
	if (!pc.isTaur()) output("shoulders");
	else output("flanks");
	output(" and her pupils dance in excitement; for a moment you even lose yourself in her enthusiasm, forgetting her demonhood and seeing only an earnest, adoring girl under you.  \"<i>Now, push!</i>\"");
	
	output("\n\nSheila pulls you down gently and a warmth envelops your [pc.cockHead " + x + "] as it nudges past her inflamed labia.  Her command rings in your ears, and coupled with the sensation, it's as much as you can do to slip in at your own pace and enjoy yourself instead of obeying her every word.  Inch by inch you slide yourself into her pussy as she bites her lip impatiently.  \"<i>You're such a tease, [pc.name].  You know how much I want you in me, and you make me wait for it every time.</i>\"  You begin pumping her slowly; her back arches ");
	//(sheila corruption >=40)
	if (sheilaCorruption() >= 40) output("and her soft breasts jiggle ");
	output("as a twinge of sensation from your thrust provokes a gasp and forces her to reposition herself, to better find it again.  \"<i>That's the spot!  Your dick is the <b>best!</b>  Pound me, you fucking cum-hose!</i>\"");
	
	output("\n\nRestraint evaporates as she reaches up and kisses your chest, dancing her lips across your " + pc.skinFurScales() + " and leaving a trail of tingling nerves behind.  Your ");
	...
	if (pc.isTaur()) {
		output(", practically lifting herself off of the ground] as she impales her cunt on your [pc.cock " + x + "] to prevent you from pulling out.");
	}
	//[(silly)
	if (silly) output("  \"<i>Make me feel like I'm fifteen years old again and playin' with the dog-morphs in the field!</i>\"  Heavens to Betsy!");
	else output("  \"<i>This is what you're here for!  Don't spill a single drop!</i>\"");
	...
	if (sheilaCorruption() < 40) output("tight");
	else output("soft");
	output(" chest against you");
	if (pc.isTaur()) output(" as you slump forward");
	output(".  Your cock ");
	if (pc.cockTotal() > 1) {
		output(" and its ");
		if (pc.cockTotal() == 2) output("brother");
		else output("brethren");
		output(" swell and squirt");
	}
	else output("swells and squirts");
	output(", pouring your semen into Sheila's ravenous pussy");
	if (pc.cockTotal() > 1) output(" and coating her crotch with a glaze of white");
	output("; her eyes glow and it briefly feels as though her body is sucking the ejaculate right out of you");
	if (pc.cumQ() >= 500) output(", until she fills up and the rest of your orgasm is drooled out at its own lazy pace");
	output(".  She kisses you");
	if (pc.isTaur()) output("r forelegs");
	output(" lovingly and deeply");
	if (pc.cockTotal() > 1 || pc.cumQ() >= 500) output(" as the spare seed dribbles from her crotch");
	output(", then breaks away and looks up into your face.");
	
	output("\n\n\"<i>[pc.name], you ");
	if (pc.cumQ() >= 500 || pc.cockTotal() > 1) output("spilled some.  I can't have your baby if you're fucking the grass.  And you ");
	output("came too soon; I wasn't done.</i>\"");
	...
	if (rand(2) <= 0) {
		output("I used a bit of power to help you finish.  Don't hate me for it; your orgasm face is just so cute when you're giving up your seed to me.</i>\"  The woman places her hand on her stomach, below her navel.  \"<i>Thank you for this.  I hope it takes, so I can raise a big strong imp to be just as " + pc.mf("productive as his father","sexy as his mother") + ".</i>\"  She kisses you softly again, and you roll off with a groan.  \"<i>Your sperm are the best,</i>\" Sheila whispers, \"<i>so come see me again soon.  I'll be wantin' more.</i>\"  Your demon stands up and winks at you, then walks off, tails swishing happily as a trickle of white runs down the brown skin of her naked thigh.");
		//(end scene)
		processTime(10);
		pc.orgasm();
		pc.slowStatGain("l", -1);
	...
	//(else if RNG doesn't end scene)
		...
		output(".  Muscles weakening again, you lower yourself onto her once more and allow her to stroke your ");
		if (!pc.isTaur()) output("back");
		else output("sides");
		output(" as she cools down.");
		
		output("\n\n\"<i>That was amazing, [pc.name],</i>\" she says, finally.  A dirty look and a poke in her side answers, irritated as you are that she pressed another orgasm out of you before you were ready.");
...
private function analHateFuck():void {
	clearOutput();
	output("Fucking bitch, playing with herself like nothing's the matter and you didn't just show her concrete proof of your superiority.  Part of you feels like punishing the insolent woman yourself, pounding her insides and dumping your load without a whit of regard for her own pleasure... but another part is thinking it might be fun to see her completely debased, to hammer home how pathetic she is - something like reducing her to a cock-sleeve for your personal cum-slave.");
	
	//[Tear Her Up][Call Jojo]
	processTime(1);
	clearMenu();
	addButton(0, "TearHerUp", sheilaAnalHateFuckAGoGo);
	addDisabledButton(1, "Call Jojo", "Call Jojo", "This option requires you to have corrupt Jojo at camp.");
	if (flags["COC.JOJO_IN_CAMP_CORRUPT"] == 1) addButton(1, "Call Jojo", analHateFucksWithJojo);
}
...
private function sheilaAnalHateFuckAGoGo():void {
	clearOutput();
	var x:int = pc.cockThatFits(sheila.analCapacity());
	output("Your [pc.cock " + x + "] gets hard in your [pc.gear] as you watch her diddle her pussy, but you set your jaw to resist giving her what she wants - a better idea is taking shape in your head.  \"<i>Turn around,</i>\" you demand.");
	
	output("\n\nSheila bites her lip and blushes, then turns her back to you, pressing her chest into the ground and waving her ass in the air, twin tails bobbing over her round cheeks and drooling, eager cunt.  \"<i>Heehee, I'm ready for you, mate... look how wet I am!</i>\"  You strip, releasing your [pc.multiCocks], and approach.  She hums in anticipation as you rub against her smooth labia, smearing her moisture onto your [pc.cockHead " + x + "].  \"<i>Come on; time to put it in already!  Don't tease me!</i>\"");
	
	output("\n\nYour smile widens, unseen, as you silently agree.  ");
	if (!pc.isTaur()) output("Slipping one hand around your [pc.cock " + x + "]");
	else output("Straddling her");
	output(", you pull your hips back and line the [pc.cockHead " + x + "] up, then thrust - right into her asshole.  Her head raises and her teeth grit as you force the slick tip past her ring.  \"<i>[pc.name], what the fuck?!  That hurts!  Go easy if you're going to use that hole; I'm not good at stretching my body yet!</i>\"");
	
	output("\n\n\"<i>No.</i>\"  You ");
	//(not centaur)
	if (!pc.isTaur()) output("grab her thrashing legs, using them for leverage as you ");
	output("force your dick into the angry woman.  \"<i>You talk a good game about how you love being full of cock, but when I shove one in you, you get all bitchy and act like you're not ready; why don't you just shut up and get fucked, slut?</i>\"");
	...
	if (sheilaCorruption() >= 90) output("; her enormous, cushiony breasts spread under her as your prick forces her weight heavily onto them");
	output(".  She pulls up tufts and pounds the earth impotently with her fists while you sink in as far as you can.  \"<i>Fuck you!  What the fuck is wrong with you, arsehole?!  Let me go, I don't want anything to do with you anymore!</i>\"");
	...
	output("\n\n\"<i>[pc.name], stop playing!  I need you to fuck me, not fuck with me!</i>\" Sheila shouts, trying to get her legs under her to continue the sex with just her own hips.");
	...
	output("\n\n\"<i>[pc.name]!  Finish in my pussy, please!  I want to raise your baby!  You're the only one that suits me!</i>\"");
	
	processTime(15);
	//if corruption >= 80 and PC has worms, present choices 
	if (pc.cor() >= 80 && pc.hasStatusEffect("Infested")) {
		clearMenu();
		//[No][Worms Suit You], else auto-output text from [No]
		addButton(0, "NoWorms", sheilaAnalHateFuckAGoGoNO);
...
private function sheilaAnalHateFuckAGoGoNO():void {
	clearOutput();
	var x:int = pc.cockThatFits(sheila.analCapacity());
	output("\"<i>Go to hell.</i>\"  You bury yourself in the demon's ass as your dick begins to spasm.  Her tails writhe as the first trickle sinks into her.  \"<i>What?  Say something, I dare you.</i>\"");
	
	output("\n\nLosers don't get to decide how they get fucked.  If she has a problem with that, she can do better.  With surprising grace, she buttons her lip and takes it, allowing you to unburden yourself.  Her hot guts feel heavenly as you smear your [pc.cum] into them, and she even makes small little gasps as you release each new stroke.  Her tight asshole fills quickly, and you feel the semen lapping at the tip of your cock.");
	if (pc.cumQ() >= 500) output("  Your overproduction floods her insides, and Sheila groans as it begins to leak back out, drooling over her pussy.  \"<i>Hey, you got your wish,</i>\" you tease.  The wanton woman tries to tilt her ass higher, to tip more of your overflow into her cunt, but you laugh and slap her hard on the hip, sending her to one knee again.");
	
	output("\n\n\"<i>Fucking... bastard,</i>\" Sheila groans, as you pull your spent [pc.cock " + x + "] from her with a sucking noise.  Her tails droop over her abused hole, parting the ");
	if (pc.cumQ() < 250) output("trickle");
	else if (pc.cumQ() < 1000) output("stream");
	else output("waterfall");
	output(" of semen made when her body pushes out some of the foreign substance.");
...
private function sheilaAnalHateFuckAGoGoGETYOUSOMEWORMS():void {
	clearOutput();
	var x:int = pc.cockThatFits(sheila.analCapacity());
	output("\"<i>Mmm...</i>\" you answer, noncommittally.  Your ");
	if (pc.balls > 0) output("[pc.balls]");
	else output("insides");
	output(" squirm, giving you pause, and you pop your [pc.cock " + x + "] out.  \"<i>Actually... I don't think I suit you at all.</i>\"");
	...
	output("\n\n\"<i>Mmm...</i>\" she hums, reaching eagerly for your [pc.cockHead " + x + "] with her finger as if getting it inside were the only thing that mattered.  You back up again, and she crawls over and onto you, rubbing her slimy cunt against your [pc.legs].  Her strokes trigger a small orgasm in her, and as her pussy sprays a load of worms onto your " + pc.skinFurScales() + " she kisses you affectionately, looking right into your eyes; you're flooded with nebulous mental images of fucking a slender, sickly pale Sheila as both of you are covered and hidden from the sunlight by a massive colony, of her riding you with her slithering, wormy pussy until the semen spitting from your [pc.cock " + x + "] leads the parasites within right to your slit, and, most disturbingly, of Sheila stretching your slick urethra wide open and climbing into your penis herself!  Shaking it off, you quickly break contact, pick up your stuff, and depart, leaving the infested woman behind... but the visions don't go away.");
...
private function analHateFucksWithJojo():void {
	clearOutput();
	output("Watching her sit there and play with herself like nothing's the matter and you didn't just demonstrate her weakness for all the world to see touches a nerve deep inside you.  You scowl and focus your thoughts to call your mousey ex-monk, bent on showing this demon how a proper mendicant should behave.");
	
	output("\n\nYour servant eventually comes sprinting across the grasslands toward the beacon of your summons, and falls to his face before you, panting in exhaustion for some while before he can speak.  \"<i>Yes, [pc.master]; how may I please you?</i>\"");
	...
	output("\n\nYour little mouse pliantly seats himself on the grass to the rear of Sheila, then scoots forward until his throbbing prick is pressing against her lower back.  The demon shivers in anticipation as the glaze of precum smears along her spine, and you can see her tails wreathing the monk's rod.  His face pinches as she attempts to wring his cock out, but he still checks to make sure he has followed orders.  \"<i>M-[pc.master], is this.. ok?</i>\"");
	
	output("\n\n\"<i>Yes, very good.  Now, place your hands on her ass and lift her into position above your dick.</i>\"");
	output("\n\nJojo does so, relieved as her tails release his pecker and the woman leans forward to angle her pussy toward him welcomingly, then uses one of his hands to aim himself toward her crotch.  You bend down to Sheila's face, taking her chin in one hand.  \"<i>Now, supplicate yourself to your [pc.master].</i>\"");
	
	output("\n\n\"<i>Haha...</i>\"  Sheila composes her expression into one of mock-gravity, and recites, \"<i>Please, [pc.master], may I have your minion's semen in my pussy, to make you a baby?</i>\"  Her eyes are fairly lit at the prospect; it's evident she regards this as no more than a game.");
	...
	output("\n\nHer face fills with defiant bitterness as she digests this last statement, looking at the ground.  \"<i>Please... [pc.master],</i>\" she says, finally, \"<i>will you have your slave come in my unworthy pussy instead?</i>\"  She's still obviously resisting the lesson.");
...
private function analHateFucksWithJojoNo(clear:Boolean):void {
	...
	output("\n\n\"<i>Y-yes, [pc.master],</i>\" he pants, \"<i>what shall I do now?</i>\"");
...
private function jojoRuinsTheAnalHateFuck(clear:Boolean = true):void {
	...
	output("Your mouth spreads in a sickly smile as you contemplate granting her request anyway.  \"<i>Jojo...</i>\"");
	
	output("\n\nHis ears perk up.  \"<i>Yes, [pc.master]?</i>\"");
	...
	if (sheilaCorruption() >= 90) output(", mashing her tits together as they thicken");
	output(".  The mouse himself seems to be growing, in fact.  His feet kick and thrash, sprouting dangerous-looking nails as his legs stretch prodigiously.  As his body mass increases, his white-furred skin begins to burst and peel like old rags, revealing new, dark flesh underneath.");
	...
	pc.cor(10);
	flags["COC.SHEILA_DISABLED"] = 4;
	flags["COC.JOJO_DEAD_OR_GONE"] = 1;
	processTime(45);
	if (CombatManager.inCombat) {
		output("\n\n");
		CombatManager.genericVictory();
...
private function bigDickAndThighs():void {
	clearOutput();
	output("As Sheila diddles herself, you ");
	//[(not horse)
	if (!pc.isTaur()) {
		output("doff your [pc.gear]; the demon's eyebrows climb as you haul out [pc.multiCocks]");
		if (pc.cockTotal() > 2) output(" one at a time, practically stacking them atop each other");
		output(".");
	}
	//(horse)
	else {
		output("slowly draw nearer to her, your [pc.cocks] bobbing with each step.");
	}
	output("\n\n\"<i>Oh no...</i>\" she says, blushing.  \"<i>[pc.name], you're massive... are you really going to try to put something like that in my poor little pussy?</i>\"");
	...
	if (pc.cor() < 40) {
		...
		output(" toward her and aim underneath her cunt to rub your [pc.cockHead " + pc.biggestCockIndex() + "] against her palm, smearing a glob of precum on it.  You can see her shivering wordlessly, sliding her fingers together to play with the hot, clingy fluid, making translucent webbing between them.");
		//[(multi)
		if (pc.cockTotal() > 1) {
			if (pc.cockTotal() == 2) output("  Both");
			else output("  All");
			output(" of them... slide them here, love,</i>\" the demon insists.  You line the remainder of your manhood up and force it between her thighs as well.");
		}
		output("  She takes the tip of your [pc.cock " + pc.biggestCockIndex() + "] in her hand and pulls gently, coaxing your forward; you close the distance until [pc.eachCock] is peeking through her legs and your crotch is pressed against hers.");
		
		output("\n\n\"<i>I'm going to borrow a bit of your sensation...</i>\" Sheila announces quietly.  ");
		//[(not horse and not < 5"0)
		if (!pc.isTaur() && pc.tallness >= 60) output("She unbends and kisses you on the cheek, and your face feels a bit fuzzy where her lips touch.  ");
		...
		output("\n\n\"<i>We'll see who finishes first.</i>\"  Resting herself against your [pc.cocks] for support, she brings her other hand up and touches you; almost the full sensation is there this time, and you sign in happiness.  Sheila begins stroking with the new hand as well as the old, grabbing ");
		if (pc.cockTotal() == 1) output("your dick with both");
		else output("a prick in each");
		output(" and angling upward so the precum drools downward and lubes her fingers.");
	}
	//(else corr >= 40)
	else if (pc.cor() >= 40) {
		...
		output("\n\nPfft.  You angle your [pc.cock " + pc.biggestCockIndex() + "] toward her vagina and thrust, heedless of the woman's pleasure.  She tries to focus and spread her pussy open; indeed you can see it widen several inches before you mercilessly slam your [pc.cockHead " + pc.biggestCockIndex() + "] into it.  Sheila gasps, then grits her teeth as you batter her entrance, demanding entry but being denied at each push.");
		
		output("\n\n\"<i>Useless,</i>\" you gripe.  \"<i>What kind of sex demon can't even open up her pussy?</i>\"");
		
		output("\n\n\"<i>Goddamnit, [pc.name]; I'm trying, but it's bloody hard yakka with you touching all my fucking nerves and setting me off,</i>\" she fires back, shaking while you twist and roll your [pc.cockHead " + pc.biggestCockIndex() + "] inside her hot, stretched hole, smearing it with slippery precum.  Her tails hang down and wrap around your shaft, trying to hold it still; defiantly, you push and shift your hips harder.  Elbows buckling, Sheila drops to the ground.  Her head hangs in defeat as you smugly torment her vulnerable womanhood.  \"<i>Nnnn... fuck...</i>\"");
		
		output("\n\nThe two tails release their grip on your [pc.cock " + pc.biggestCockIndex() + "] and arch above it, pushing it suddenly out of alignment so your next thrust slips it between her thighs and bumps it up against her midriff.  The demon's hands dart to it, holding it in place and preventing you from pulling back out.  \"<i>Sorry, mate... can't focus like that...</i>\" she mumbles, looking over her shoulder at you.  \"<i>I'll make it up to you, though... promise.</i>\"  Her fingers search out your pre-bubbling urethra, dancing over it and sending a tingle up your tool.");
		...
		if (pc.cockTotal() > 1) {
			...
			if (pc.cockTotal() == 2) output(", you slide it through her legs between your first cock and her cunt, plowing through her entrance like a furrow; she quakes and you can feel it contracting against your skin.  ");
			else output("s, you push them through the woman's legs one by one, smearing warm precum on her thighs as you do; her pussy squeezes against the top of your [pc.cock " + pc.biggestCockIndex() + "] as you coat her skin with your fluids.  ");
		}
		else output("\n\n");
		output("Sheila scoots backward, choking up on monster cock until her ass is pressed against your crotch, and then wraps her hands around and begins to masturbate you, using your drooling fluid and her own wetness for lubrication.");
		
		output("\n\nShe seems to get off on it as much as you do; as she strokes, her tongue hangs out slightly and she begins rubbing your [pc.cocks] between her " + sheilaCup() + " breasts, smearing your secretions down her entire torso.  Her skin is hot and feels good, but not as vivid as you'd expect.  The demon's pussy contracts wildly again, despite being at rest against your shaft, and you come to a realization: she's actually siphoning the pleasure from you!");
	}
	//end corr split
	output("\n\nThe woman quickly becomes lost in her own little world, searching out the nerves on your prick");
	if (pc.cockTotal() > 1) output("s");
	output(" and teasing each one; you thrust weakly into her hand, helping guide her hands, but your lonely tip wants for stimulation.  As the feeling becomes so acute that you prepare to say something, Sheila uncannily anticipates you and scoots her pussy along your shaft, popping your [pc.cockHead " + pc.biggestCockIndex() + "] into her mouth.  Her tongue dabs at your slit, tasting your fluid, and her hands ");
	if (pc.cockTotal() > 2) output("relinquish your other pricks and ");
	if (pc.cockTotal() == 2) output("relinquish your other prick and ");
	output("focus on squeezing more drops out of you.  She fairly coats her lips with it, rubbing your hole against them before popping the whole [pc.cockHead " + pc.biggestCockIndex() + "] back into her mouth.  A moan escapes you just as another one returns from Sheila's throat, transmitted through your [pc.cock " + pc.biggestCockIndex() + "].");
	
	output("\n\nAs her attentions refocus on your [pc.cockHead " + pc.biggestCockIndex() + "], a pang of loss arises from the middle of your [pc.cocks] - and is quickly smothered with her chest.");
	//[(sheila corruption < 50)
	if (sheilaCorruption() < 50) output("  Though her modest breasts can't even begin to encompass your girth, Sheila rubs her cleavage along your skin with gusto while she works your tip with her tongue.");
	else if (sheilaCorruption() < 150) output("  Her large " + sheilaCup() + " breasts encompass your girth easily, and she pushes the soft, flushed skin together as she slides it up and down your cock.");
	else {
		output("  The demon's ridiculously enormous breasts easily cover the entire length of your cock");
		if (pc.cockTotal() > 1) output("s");
		output(" falling between her crotch and her mouth, making her whole body feel like a huge, wet fuckhole");
		if (pc.cockTotal() > 1) output(" built for " + num2Text(pc.cockTotal()) + " pricks");
		output(".");
	}
	...
	if (sheilaCorruption() >= 150) {
		output("Her enormous breasts dangle all the way to the ground, forming a second, softer hole to push into with your swollen prick");
		if (pc.cockTotal() > 1) output("s");
		output(".  ");
	}
	output("\"<i>Y-yes,</i>\" she mumbles, \"<i>gonna fuck her... harder...</i>\"");
	
	output("\n\nThe apparent answer to your question passes you by as you obliviously force your tool");
	if (pc.cockTotal() > 1) output("s");
	output(" through the vise of her legs; losing yourself further and further to pleasure, you don't even notice when Sheila's hips begin to buck in time with your own, pressing her mons down against your ");
	if (pc.cockTotal() > 1) output("topmost ");
	output("dick.  With an enormous, final thrust, you mash your crotch up against hers and begin to ejaculate.  ");
	if (pc.cor() < 50) output("The sensation seems to reawaken Sheila; she angles her body up to press her pussy close and your embezzled sensation floods back into you - along with an unexpected gift.  You can feel your... no, <i>her</i> demonic pussy convulsing repeatedly, trying to suck up sperm in vain as her own orgasm comes off, and she looks at you with sly eyes.  \"<i>A little reciprocity for my love,</i>\" the woman coos, \"<i>to show you how much I care.</i>\"");
	else output("\"<i>Oh god!</i>\" Sheila shrieks.  \"<i>I'm coming!  Jizz is going everywhere!</i>\"  Indeed, as your body releases streams of cum, Sheila directs them by aiming [pc.eachCock], covering herself and moaning as her hands press out as much as possible.");
	
	output("\n\nFinished, you pull your sensitive, soaked shaft");
	if (pc.cockTotal() > 1) output("s");
	output(" from between her still-squeezing thighs with a shudder; Sheila shudders with you until you finally draw free and break skin contact, then lowers herself carefully to the ground.  The demon turns toward you, exposing a face and chest splattered with your semen.  She stares at your deflating prick");
	if (pc.cockTotal() > 1) output("s");
	output(", looking almost wistful to see ");
	...
	output("\n\nSheila giggles at that and raises her eyes, meeting your gaze.  \"<i>But it's only yours I love, [pc.name].  I might take it off you as long as you promise ");
	if (!pc.hasVagina()) output("to grow a nice pussy for me so that ");
	output("I can put it in you any time I want.</i>\"  She sticks her tongue out at you, and the seed dripping from her lips oozes onto it.");
...
private function winAgainstDemoNSheilaForVaginas():void {
	clearOutput();
	output("You drop onto the woman's lap without hesitation, ");
	if (!pc.isTaur()) output("mauling her tits roughly with your hands, ");
	output("and grind your pussy desperately against her midriff.  She slips two fingers into you obligingly, but you ");
	if (!pc.isTaur()) output("grab her face, plunging one thumb into the corner of her mouth in your haste, and");
	else output("turn your head and");
	output(" stare right into her eyes.  Focusing as hard as you can on your fantasy, you try to transmit your desire for something more to the demon by sheer force of will.  A sly smile parts her lips as she stares back - could this actually work?");
	...
	output("\n\nSuccess!  You can feel something firming up in her lap underneath your [pc.butt], and you ");
	if (!pc.isTaur()) {
		output("crawl away on your hands");
		//[(tail or naga)
		if (pc.hasTail() || pc.isNaga()) output(" and raise your tail");
	}
	else output("clamber to your hooves and bend your forelegs");
	output(" to present your eager [pc.vagina].  Sheila sits up and places her hands on your [pc.hips], then slides her pelvis up, teasing your lower lips with something bulbous and warm.  They sting slightly as it smears its slime along them, a not-entirely-pleasant sensation.");
	...
	output("  Oh, god, it burns!  You shake violently as your pussy is assaulted by the fluid from Sheila's black tail, waves of pain twisting through it and forcing its walls to clamp down on the invader.  The demon gasps as you squeeze and leans forward onto your back");
	if (sheilaCorruption() > 0) output(", squashing her tits into you");
	output(".  \"<i>Calm down!</i>\" she barks, grabbing your arms and pulling them toward her.  With hands held behind you like this, you can do little to check her enthusiasm... but Sheila doesn't begin thrusting yet.  \"<i>It'll fade soon, love, and you'll start feeling it,</i>\" she promises, kissing your ");
	if (!pc.isTaur()) output("neck");
	else output("withers");
	output(" as her strange, still shaft continues drooling into your pussy.  Your groin heats up in response to the pain, and her words ring hollow.  The burning doesn't seem to be getting any better; you look back over your shoulder at her, fear and hurt evident on your tightened features.  She smiles nervously at you, but the flow of goo doesn't stop.  It feels wetter and slicker in your pussy every second, and soon you can feel the slime begin to seep back out of your [pc.vagina].  Trickle after trickle of fluid escapes you, falling to the ground underneath.");
	
	output("\n\n\"<i>Here it comes,</i>\" Sheila assures you.  The trickle intensifies as more of her secretion is pushed from you, and miraculously the pain seems to lessen as it comes out, landing with soft 'pat' sounds.  Your confusion must be written on your face, as Sheila leans in conspiratorially.  \"<i>That's not me doing it anymore, love.  It's you.</i>\"  To demonstrate, she pulls free of you - and the soft sounds of liquid hitting the ground only multiply!  She slides her hand in front of your pussy, cupping you gently, then pulls it away and holds it in front of your face.  It's overflowing with clear, female juices.  \"<i>Look how wet you are, my little whore,</i>\" Sheila says, spreading her fingers and letting the girl-lube trickle between them.  As you process this, she rests herself on your [pc.butt] and slips her black, spaded tail into you again; the passage is easy and marked with none of the discomfort from before, only a pleasant heat that sets your heart racing.");
	...
	output("\n\nYour [pc.vagina] convulses in climax at the thought of being so debased - ");
	//[(lib <20)
	if (pc.slut() < 20) output("even as proud as you are, ");
	output("there's something appealing about the picture, something powerful in being cherished and used in equal measure.  Sheila screams and releases your hair, digging her fingers into your hips, spraying the inside of your pussy with a thick, oozing heat that spreads out and then dulls as your own pouring juices force her cum out.  Peering between your legs, you can see it falling to the ground; viscous black sludge that bubbles and steams as if it were trying to reach back up to your sex.");
	if (pc.cor() < 50) {
		output("  She pumps and pumps again, forcing more of the muck into your [pc.vagina] even as she calls you the most filthy things she can think of");
		if (sheilaCorruption() > 0) output("; her tits wobble violently as she thrusts, and then still as she arches her back and jams her spade all the way in");
		output(".  Sheila howls to the sky as she pours her demonic slime into you in one final, solid stream.  You can feel it pushing eagerly through even your copious fluids until it's at the entrance to your womb... and then inside.");
		//[fertility up if not capped; tingle deep within body, instinctively you know you have become blah blah]
		if (pc.fertility() < 50) {
			output("  There is a tingle deep inside you, making you more fertile.");
			pc.fertilityRaw++;
		}
	}
	else {
		output("  She stops pumping, falling against your back and panting as the last of her goo seeps out and is ejected around her shaft by your fountainhead of girl-cum.  You are clearer-headed than before as your orgasm dwindles, though, illogically, you could swear you felt her tits growing against you.");
	}
	output("\n\n\"<i>God damn,</i>\" she says, rubbing your [pc.butt] and withdrawing her spade, to produce a splat as your pussy forces out the excess black slime.  \"<i>I love you so much, [pc.name].</i>\"  You both collapse; your [pc.vagina] continues to drool its dark-tinged lube for some time as you lay face-down with Sheila resting atop you, but eventually returns to nearly normal.");
...
public function badEndScarredBlade():void {
	output("Y collapse from your injuries and realize what's the scarred blade's going to do. It's GOING FOR YOU! You scream as the saber thrusts towards you.");
	if (silly) output("\n\n[DATA EXPUNGED]", false, false);
	//if (marbleFollower()) output("\n\n\"<i>NOOOOO! Sweetie!</i>\" Marble cries.");
	
	flags["COC.SCARRED_BLADE_STATUS"] = 1;
	badEnd();
}

public function breakScarredBlade():void {
	output("The scarred blade falls to the ground, inanimate. You walk over to grab the saber to find out it's still moving faintly. The saber needs to be broken to ensure it poses no further danger and remains usable.\n\n");
	if (pc.physique() >= 40) output("You grab the saber by the blade and bend it with all your might. The metal groans and the lethicite cracks. You repeatedly twist the blade until one segment of the saber finally snaps off.");
	else {
		output("You grab the saber by the blade and attempt to bend it with all your might. The blade refuses to bend despite your efforts. ");
		//if (followerHel()) output("You call Helia over. \"<i>Hey, lover! Need any help?</i>\" she asks. You ask her if she can break the scarred blade. \"<i>Yes, lover. Hold the sword out.</i>\" Helia instructs. You hold out the scarred blade, Hel raises her scimitar up and slashes it cleanly through the scarred blade, cutting off the segment of the blade.");
		//else if (followerKiha()) output("You call Kiha over. \"<i>Yes?</i>\"");
		//else if (marbleFollower()) output("You call Marble over. \"<i>What is it you need, sweetie?</i>\" she asks. You ask her if she can break the scarred blade. \"<i>Yes, sweetie. I'll get my hammer.</i>\"");
		output("You throw the sword towards the tree and it lodges partway into the tree. With the additional leverage, you push and pull the sword to the side repeatedly. The metal groans and the lethicite cracks. After a few minutes of bending, one segment of the saber finally breaks off.");
	}
...
public function rebellingScarredBlade(wieldAttempt:Boolean = false):void {
	if (!wieldAttempt) output("<b>The scratched sword you carry jerks wildly like a bucking horse, and, tilting hilt-downward, slides itself right out of its scabbard.  Before you can pick it up and re-sheathe it, it lashes out at your hand, cutting you and landing with the point out.  Even when you try to circle it and grab the handle, the uncanny saber spins its edge around to fend you off.  Sighing with irritation, you abandon it for now.</b>\n\n");
	else output("As soon as you try to wield the sword, it jerks wildly like a bucking horse. You quickly put it back into your pouches before it can do harm to you.");
	if (!wieldAttempt) {
		applyDamage(new CoCScarredBladeAnimated().meleeDamage(), new CoCScarredBladeAnimated(), pc);
		pc.meleeWeapon = new EmptySlot();
		pc.inventory.push(new CoCScarredBlade());
		flags["COC.SCARRED_BLADE_STATUS"] = 1;
	}
	CombatManager.newGroundCombat();
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(new CoCScarredBladeAnimated());
	CombatManager.victoryScene(breakScarredBlade);
	CombatManager.lossScene(badEndScarredBlade);
	CombatManager.displayLocation("SCARRED BLADE");
	clearMenu();
	addButton(0, "Fight", CombatManager.beginCombat, undefined, "Fight", "Fight the saber and try to break it. This is going to be a risky battle and a loss can mean your demise.");
}

public function findScarredBlade():void {
	output("A nearby flash of light on metal catches your eye.  Drawing closer to it, you find the blade you abandoned before sticking point-down in the dirt.  The tainted saber leans toward you, presenting its hilt almost pleadingly.  Take up the sword again?");
	clearMenu();
	addButton(0, "Yes", takeScarredBlade);
	addButton(1, "No", leaveScarredBlade);
}

private function takeScarredBlade():void {
	clearOutput();
	output("You grab the bloodthirsty saber and pull it from the ground. ");
	flags["COC.SCARRED_BLADE_STATUS"] = 0;
	
	itemScreen = mainGameMenu;
	lootScreen = leaveScarredBlade;
...
private function leaveScarredBlade():void {
	clearOutput();
	output("You choose not to take the saber, leaving it to rust. ");
	
	if (!(pc.meleeWeapon is CoCScarredBlade || pc.hasItemByType(CoCScarredBlade))) flags["COC.SCARRED_BLADE_STATUS"] = -1;
