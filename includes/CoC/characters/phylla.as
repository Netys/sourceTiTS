...
{
	return flags["COC.ANT_WAIFU"] > 0;
}

public function antColonyEncounter():void
{
	//WAIFU GET!
	trace("ANT WINS: " + flags["COC.ANT_ARENA_WINS"] + " ANT LOSSES: " + flags["COC.ANT_ARENA_LOSSES"]);
	if (int(flags["COC.ANT_ARENA_WINS"]) - int(flags["COC.ANT_ARENA_LOSSES"]) >= 2 && flags["COC.ANT_ARENA_WINS"] >= 4 && pc.hasGenitals()) {
		if (flags["COC.PHYLLA_STAY_HOME"] > 0) bumpIntoTheAntColonyAfterStayHomePhylla();
		else antGirlGoodEnd();
	}
	else if (flags["COC.PC_READY_FOR_ANT_COLONY_CHALLENGE"] == 1)
		antColonyChallenge();
	else if (flags["COC.PHYLLA_SAVED"] == 1)
		enterTheColony();
...
{
	return new CockClass(50).volume();
}
...
{
	clearOutput();
	output("While traversing an unfamiliar part of this desert wasteland, ");
	if (pc.cor() > 75 && pc.lust() > 50) output(" looking for something to slake your unquenchable lusts, ");
	output("you come across an abandoned merchant's cart.  The cart looks eerily disheveled, with a majority of its contents strewn about the sand.  Whoever or whatever deserted this thing did so in a hurry, and for seemingly no reason. Curious, you decide to inspect it for anything worthwhile.");
	...
	//[Play Hero]
	clearMenu();
	addButton(0, "Play Hero", playPhyllaHero);
	addButton(1, "Keep Hidden", keepPhyllaHidden);
}

//►[Keep Hidden]
private function keepPhyllaHidden():void
{
	flags["COC.ANT_COLONY_KEPT_HIDDEN"] = 1;
	...
	//►(If Over 41 - Male)
		...
		//output(images.showImage("ants-desert-male-firstencounter"));
		output("You know exactly what's going to happen once the demons 'subdue' their prey.  The growing throbbing in your loins begs for release, and you quickly and quietly take off your [pc.gear], making sure the demons don't notice you.  You might as well watch the show as you're not gonna be able to leave unnoticed until the demons are done anyways.");
		output("\n\nWatching from your hiding spot you start to stroke your " + pc.cockDescript(0) + ".");
		//Start Dick Size Check
		var x:Number = pc.biggestCockIndex();
		//If player can auto-fallatio:
		if (pc.cocks[x].cLength() >= 20) output("  Licking the head of your [cock " + (x + 1) + "] in anticipation of what's to come only adds to your growing lust.");
		//If player cock size tiny:
		else if (pc.cockVolume(x) < cockVolume(6)) output("  Though your cock isn't the most impressive, it's 'got it where it counts' and you intend to make the most of it.  You lovingly tease yourself until you're fully erect; running your fingers down the ridge causes a bead of pre-cum to form on your tip.");
		//If player cock size normal:
		else if (pc.cockVolume(x) < cockVolume(12)) output("  You hold your cock like the hilt of a sword and start to pump your shaft slowly.  Your breathing becomes heavier and labored, as you stroke yourself with mounting passion.");
		//If player cock size large:
		else if (pc.cockVolume(x) < cockVolume(30)) output("  The fear of having your cock give away your position quickly passes through your mind, but it's too late now.  The only way out of this is to cum, so you might as well enjoy it.  You firmly grasp your cock and start jacking off, hoping that you'll finish sooner rather than later.");
		//If player cock size MASSIVE:
		else output("  The towering meat log jutting from your crotch gives away your 'hiding' spot, but the demons don't seem to mind, as they're too busy with their new toy.  You doubt even if they did care that any of them would be able to satisfy you. You wrap your arms around your fully erect cock and start to pump, hoping to release your load soon.");
		...
		output("\n\nWatching from the rocky outcrop that hides you, you stroke [pc.eachCock] with mounting excitement, running your hand up and down the shaft and picking up speed as you watch the demons cum one after another.  Seeing this girl get used as a cum depository is way hotter then what you imagined it would be.");
		...
		output("\n\nThe hermaphrodite struggles to cram itself inside the ant-girl and eventually succeeds.  With each stomach-contorting thrust you hear the ant's muffled moans of pain and ecstasy from between the legs of the female demon.  The hermaphrodite quickens his thrusts and, in turn, you keep speed.  With near-precision timing, you and the demon finish together.  [pc.EachCock] throbs and a wave of euphoria washes over you.  Groaning in pleasure you release your ");
		//[no new PG, Start Cum Check
		if (pc.cumQ() < 100) output("sizable");
		else if (pc.cumQ() < 1000) output("huge");
		else if (pc.cumQ() < 10000) output("massive");
		else output("titanic");
		output(" load.  Once you can refocus your eyes, you see the hermaphrodite pull out, and as it does a burst of cum sprays out of the ant-morph's vagina as her stomach deflates slightly.  She's been filled with so much corrupt seed that it's stretched her belly to the point it resembles a third-trimester pregnancy.");
	...
	//►If Over 41 - Female
		...
		//output(images.showImage("ants-desert-female-firstencounter"));
		output("Watching from your hiding spot, you consider what's going to happen and feel your pussy immediately moisten.  Your practiced hands move with deftness, slipping into your [pc.gear].");
		output("\n\nFeeling your heat through your [pc.gear], you quickly and quietly strip, every now and then glancing down to make sure you're not seen.  Once you're completely nude, you find a nice vantage point to view the impending rape.  By the time you've gotten yourself ready, you realize the demons have already started.");
		output("\n\nRunning your hands over your breasts, you pinch your [pc.nipple] and give a soft coo, careful to keep your voice down.  Alas, your body's reactions start to overwhelm your senses and you slowly begin to forget that you're even trying to hide.");

		//Unique Prerequisites
		//If PC is lactating:
		if (pc.lactationQ() > 100) output("  You feel your warm [pc.milkNoun] flow out of your hardened arola, as your body begs to release its [pc.milkFlavor] nectar.");
		//If PC has dick(s):
		if (pc.hasCock()) output("  Feeling [pc.eachCock] throb between your [pc.legs], you run one of your hands down your chest and start to stroke lovingly.");

		output("\n\nBlood still concentrating on your crotch, you set your fingers atop your vagina, resting lightly on your [pc.clit], then rub the lips of your labia until your juices start to dribble down your legs.  In a slow circular motion you start to massage your clit as you watch the scene unfold before you.");
...
//►[Play Hero]
private function playPhyllaHero():void
{
	clearOutput();
	//►Introduction to Combat
	output("As the demons bear down on the ant-girl, you burst from your hiding place, raising your [pc.mainWeapon] to the air and uttering an impressive war cry.  Nobody, ant or otherwise, is getting raped if you have any say in the matter!");
	output("\n\nYou are now fighting demons!");
	
	CombatManager.newGroundCombat();
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(new CoCDemonPack());
	CombatManager.victoryScene(consolePhylla);
...
private function enterTheColony():void
{
	...
	flags["COC.PC_READY_FOR_ANT_COLONY_CHALLENGE"] = 1;
	processTime(15 + rand(5));
	addNextButton();
}
...
private function antColonyChallenge():void
{
	...
	if (flags["COC.MET_ANT_ARENA"] == 0) {
		...
		else if (pc.physique() > 30) output("\n\n\"<i>Well, you certainly look the part.  But you're going to need more than brute strength to win here.</i>\"");
		//Else if intelligence over 50:
		else if (pc.intelligence() > 30) output("\n\nYou make a witty quip about how he might be making puppy eyes at someone who saved him from a horde of giant demon cocks, too, but he doesn't receive it well.");
		output("  After a moment he looks down and scribbles something on his chart.");
		output("\n\n\"<i>So, we're ready to start when you are.</i>\"");
		flags["COC.MET_ANT_ARENA"] = 1;
	}
	//Introduction Subsequent Times
	else {
		output("Your feet bring you back to the ant colony and the guard motions you down the only lit tunnel once more.  You enter the colosseum, and as you step out into the cavern, you are greeted, again, by a thin male ant-morph holding a clipboard.  He looks up at you.");
	...
	addButton(1, "Leave", leaveAntColony);
}
...
{
	clearOutput();
	output("Deciding to better prepare yourself first, you inform the thin fight manager that you will return later.  You leave the colony, heading back to camp.");
	processTime(5);
	addNextButton();
}
...
private function antColiseumFight():void
{
	...
	//►[Fight #3]
		...
		//(Gnoll - Intro)
		if (int(flags["COC.MET_ANT_ARENA_GNOLL"]) == 0) output("With due ceremony, the manager leads you down to one of the staging areas for the arena.  You watch through the bars as the stadium fills far past capacity.  There must be a thousand ants here... maybe even the whole colony.  Gazing out into the crowd, you pick out the royal seating and in it, Princess Phylla.  The princess is in much finer attire than you're used to seeing - her chest is actually covered by a fine red and blue dress and her hair is combed, framing her face nicely.  Her mother is nowhere to be seen, and you can tell that Phylla is taking full advantage.  She waves and cheers for you just like the crowd is and, perhaps by her orchestration, you hear the crowd pick up a chant in your name.  Seeing Phylla chant with them makes you feel invigorated.  As the gates raise, you charge out into the center of the arena, roaring your finest battle cry; your opponent is taken somewhat aback but finds resolve and braces for your charge.");
		//►(Gnoll - Intro Repeat - for people that lost once)
		else output("You move through the network of tunnels and caves and come to the colosseum once more.  You are greeted by the event manager and he ushers you down to one of the staging areas for the arena.  The noise intensifies as the colosseum fills to the brim with spectators; Phylla herself is up in her box waving to you.  You look across the arena floor and the gnoll at the other side of the arena seems just as excited to fight as you are.  The gates open and you charge each other!");
		...
		IncrementFlag("COC.MET_ANT_ARENA_GNOLL");
	}
...
{
	clearOutput();
	output("After your defeat in the arena, you set off to leave but are stopped.  Princess Phylla is standing with a troupe of armed guards by the exit, holding a vial of clear liquid.  She doesn't look like she wants to give it to you but she extends it to you all the same.");
	...
	// (+1 Loss Score)
	IncrementFlag("COC.ANT_ARENA_LOSSES");
	if (flags["COC.ANT_ARENA_LOSSES"] > 3) {
		antastrophyBadEnd();
		return;
	}
	output("\n\n");
...
{
	output("As you exit the arena, amidst the cheers and roars of the crowd, you are met by Princess Phylla and slightly fewer guards than the last time.  She sees you coming and her face lights up, then runs over to you and starts doting over you. You smile and let her do her thing.  Once she's done, she looks deep into your eyes.");
...
{
	output("After your staggering defeat and subsequent humiliation in the arena two guards approach you as you try to leave.  One of them holds out a small vial.  Princess Phylla is off in the corner of the room crying, and you try to move closer to her but one of the guards steps in front of you.  \"<i>Drink!</i>\" he commands, uncorking the vial.");
...
public function phyllaPCBeatsGnoll():void
{
	...
	//►(Gnoll - Win Repeat) Standard Gnoll Win Scene. +
		...
		output("This isn't the first time you've accomplished this, but it still brings the crowd to their feet.  You simply stand in the center of the arena, taking it all in.  Phylla makes her way to you, blushing as the rowdy audience follows her with catcalls and demands for her to give her champion a kiss.  She tends to you as always and, when she's done, takes your hand in hers and looks at you expectantly.  You smile at her, brushing the hair from her face with your other hand; she colors deeply at the contact as two guards appear to lead you both off.  Having no more reason to stay, you allow your guide to return you to the surface.");
	}
	//(+1 Win Score)
	IncrementFlag("COC.ANT_ARENA_WINS");
	output("\n\n");
...
{
	clearOutput();
	output("As you go to leave the arena queen Chylla and four bodyguards approach you.");
	...
	output("\n\n\"<i>Welcome to your new home [pc.name]. Your meals are to be delivered every 4 hours. You will eat, every morsel.  Is that understood?</i>\"");
...
private function antGirlGoodEnd():void
{
	...
	processTime(15 + rand(5));
	clearMenu();
	if (pc.hasCock()) addButton(0, "Use Penis", gigititigitigitigitigityAntGirl);
	else addDisabledButton(0, "Use Penis");
	if (pc.hasVagina()) addButton(1, "Use Vagina", femalePhyllaFirstFuckGooooo);
	else addDisabledButton(1, "Use Vagina");
...
private function refuseAntSex():void {
	clearOutput();
	output("Your mind finally fires up; she's not worth your time. You quickly dart off towards the door, leaving Phylla heart-broken.");
	...
	flags["COC.ANTS_PC_FAILED_PHYLLA"] = 1;
...
{
	clearOutput();
	output("Sporting a painfully obvious tent, your body betrays its desire for sexual gratification and it causes Phylla to smile with interest.  \"<i>I-I didn't think I had that ability to... to...  I mean I'm not like you, so I thought...</i>\" she begins, but trails off as she takes in the rough, obscured shape given off by your bulge.");
	var x:int = pc.cockThatFits(phyllaCapacity());
	...
	if (pc.cocks[x].cLength() < 36) {
		...
		if (pc.cockTotal() > 1) output("\n\nShe probably won't be able to handle ONE, let alone all of your [pc.multiCocks]!");
	}
	//***Dick(s) size more than 36 inches, but less than 72 inches:
	else if (pc.cocks[x].cLength() < 72) {
		output("\n\nThe constant bobbing and twitching of your [pc.multiCocks] is apparently not obvious enough to her that your trousers aren't built to accommodate the size of your loins.  You grimace in response as the fabric of your pants become painfully constrictive and unyielding. Snapping out of her inexperienced musings, the Ant Morph sees your obvious predicament.");
		output("\n\nIn an effort to get your loins free she rushes in, all four of her arms working feverishly to help spring you free.  After a quick flurry of motions, your [pc.multiCocks] snap out of your britches; the size of which apparently intimidates the Ant Morph, whose eyes go wide.  \"<i>H-Ho- How am I... wow...</i>\"");
		//PC has more than one dick:
		if (pc.cockTotal() > 1) output("\n\nShe probably won't be able to handle ONE, let alone all of your [pc.multiCocks]!");
	}
	//***Dick(s) size more than 72 inches:
	else {
		output("\n\nThe fabric of your pants simply cannot contain your [pc.multiCocks] any longer as you frantically tug and yank at the button, trying to get your loins free before you injure yourself or ruin a good pair of trousers. Phylla's absent mindedness comes to an abrupt end when she sees your face flush with red and fury as you desperately work to free yourself.");
	...
	if (pc.cocks[x].cLength() >= 72) {
		...
		//PC has more than one dick:
		if (pc.cockTotal() > 1) output("\n\nOverwhelmed by the sight of ONE of your members, Phylla arguably goes catatonic when she sees all of what your packing.  She probably won't be able to handle the one, let alone all of your [pc.multiCocks]!");
		//Tentacle dick check for Dick Size Huge and Massive
		//One penis longer than 8 feet, or two or more dicks that are 5 feet long:
		if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) > 0) {
			output("\n\nAt your whim, you command your tentacle dick");
			if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) > 1) output("s");
			output(" forward and wrap them tightly around Phylla, taking her hand and tugging her in close.");
		}
		...
		if (pc.cockTotal() > 1) output("s");
		output(" from around her and hang it in front of her face, grinning your antics on your face as Phylla stares at ");
		if (pc.cockTotal() == 1) output("it");
		else output("them");
		output(", slightly intimidated.");
		...
		if (pc.cockTotal() > 1) output("s");
		output(" looking for something more sizeable.  You can tell Phylla is more scared of what might happen to her than what might happen to you.");

		//END DICK CHECK
	}
	clearMenu();
	//If PC has dick(s) that will fit: Jump to - Regular Male Scene Continuation
	if (pc.cockVolume(x) <= phyllaCapacity()) {
...
{
	clearOutput();
	output("Phylla stares down at your [pc.multiCocks] and starts nervously rubbing her hands together.  \"<i>It's my first time... I mean, I hope it isn't going to be bad for you...</i>\" she says, before trailing off; uncertain of her sexual prowess.");
	...
	//PC is goo morph:
	if (pc.isGoo()) output(", leaving traces of your slimy lust along her body for her to wear, allowing your detached bits of mass to ignite her sexual fires");
	output(".");
	...
	output("\n\nSimply dragging your " + pc.cockDescript(x) + " along the entrance her of moist pussy causes her to moan and wiggle under you. Parting her pink little lips with the head of your cock, you finally hit the sweet spot for penetration. With ");
	if (pc.cor() < 75) output("a gentle amount of force, ");
	else output("an uncaring, brutish amount of force, ");
	output("you push past the tight opening of Phylla's folds.  You hear Phylla inhale sharply as she stretches to accommodate your width. In what seems to be a prolonged amount of time, you finally manage to ease the head of your " + pc.cockDescript(x) + " into her.  The act takes a loud and pained turn as Phylla moans and shifts in obvious discomfort.  You feel her squeeze her vaginal walls tightly around your cock as her considerable strength almost cuts off blood flow.  After a moment, Phylla realizes how much pressure she's exerting and her body relaxes, realizing the potential for injuring her partner.  Sighing heavily, she moves her hips down to take even more of you in. As you slide further in, her eyes flash with pain as she bites down on her lip, it becomes glaringly clear that something has yielded inside of Phylla.");
	...
	if (pc.cor() < 75) addButton(0, "Next", phyllaFirstTimePureBabiesFuckEnding);
...
private function phyllaFirstTimePureBabiesFuckEnding():void
{
	...
	var x:int = pc.cockThatFits(phyllaCapacity());
	if (x < 0) x = pc.smallestCockIndex();
	output("For a while, you allow Phylla to slowly adapt to your presence inside of her and the stretching of her vaginal walls; something she reacts to with soft moans of pleasurable appreciation.  She wears her obvious discomfort at being forcefully stretched by your " + pc.cockDescript(x) + ", but you see she can hardly complain as her body cradles yours in an attempt to keep you where you are.  Once you feel she's comfortable you start slowly pumping away at her, gradually building your own rut.");
	output("\n\nYou feel the smaller set of her hands move between your [pc.legs] and start to fondle your ");
	if (pc.balls > 0) output("[pc.balls]");
	else output("sensitive taint");
	output(".  The moans that escape her lips pick up in both volume and stress as she knowingly encourages your lustful efforts on.  The working of her hips in concert with your thrusts affirms her readiness for your full force, giving you the all clear to pin her down and free your sexual aggression on her tight little love hole; slamming your cock into her, picking up the force and tempo behind each thrust.");
	...
	if (pc.cumQ() >= 100 && pc.cumQ() < 1000) output("  Truly amazing considering how much your loads usually contain.  You think you see a little bulge in her stomach that wasn't there before.");
	//If cum milliliters over 1000:
	else if (pc.cumQ() >= 1000) output("  As you look down at her nethers your mind struggles to understand just how she could... then looking up her body, you see.  Her belly is bloated with your sperm, almost to the point of already looking pregnant.");
	...
	if (pc.canOvipositSpider()) {
		...
		if (pc.intelligence() / 20 + rand(20) + 1 < 16) {
			//Persuasion failure:
			output("\n\nPhylla ponders for a moment and shakes her head.  \"<i>I'm sorry [pc.name], I'm not comfortable with that right now.");
			//(If player has already impregnated Phylla:
			if (flags["COC.PHYLLA_DRIDER_INCUBATION"] != undefined) output("  I just can't hold anything else inside me. I'm sorry! Please don't be mad... I mean I will!  Just a-after... this batch...");
		...
		//Persuasion success:
		{
			...
			if (pc.cor() >= 75) output("  Little does poor Phylla know, you're going to just use her as a baby maker to ease the tension of your own sacks.");
			...
			pc.dumpEggs();
			if (flags["COC.PHYLLA_DRIDER_INCUBATION"] == undefined) flags["COC.PHYLLA_DRIDER_INCUBATION"] = 8 * 24 * 60; //Supposed to be eight days, not eight hours
		}
...
{
	clearOutput();
	output("For a few moments longer, you pump yourself hard against her rump in an attempt to \"break\" your new fucktoy in.  Once you're satisfied that she will be able to handle what is to come, you take to sliding yourself along against her fuckhole as you grab her by the hair.  The backside of her abdomen grinds along your ");
	if (pc.tallness >= 72) output("chest");
	else output("belly");
	output(", but it will only serve to help you.  You will need a brace for what you're about to pull and her rigid abdomen will do just fine.  Phylla makes a confused noise, glancing over her shoulder with a look of concern growing on her face.");
	output("\n\n\"<i>W-What are you doing?</i>\" she asks nervously. \"<i>This isn't how I imag~</i>\"");
	var x:int = pc.cockThatFits(phyllaCapacity());
	if (x < 0) x = pc.smallestCockIndex();

	output("\n\nWithout letting her finish, you plunge your " + pc.cockDescript(x) + " into her, causing her to yell out in sheer surprise and discomfort.  \"<i>This is how <b>I</b> mate!</i>\" you inform her, telling her that she won't EVER forget her first time with you.  Her mouth goes open slightly before your subsequent gyrations put an end to whatever she was going to sputter out.  Smirking like a mad jester at the position you have Phylla in, you begin to work your hips as you dominate her from behind.  You hear your ");
	if (pc.balls == 0) output("thighs");
	else output("[pc.balls]");
	output(" slap loudly off of her rump with every aggressive thrust against her clit.  She wails out in frustration at the lack of intimacy that you apparently have in mind.  The moaning that is mixed into her pathetic protests, however, suggests that the naive little woman is getting off on being treated so roughly; a hard slap on the ass affirms this as she whines out in annoyed pleasure.  You feel the growing heat from her pussy as it gets wetter and wetter, continuing your pressure on her abdomen as you use it to support your brutal fucking of her cunt.");
	//PC has Goo Body:
	if (pc.isGoo()) output("  Perhaps the poor dear could use some tender attention, you ponder to yourself.  Reducing the solidity of your body, you work her abdomen deep into your sticky, goopy body, re-solidifying once she's firmly in there and lowering your hands down to her nipples, pinching them hard as you continue your pumping of her rump.");
	...
	var y:int = pc.cockThatFits2(phyllaCapacity());
	if (pc.cockTotal() < 2) y = -1;
	if (y >= 0) {
		output("\n\nIntent on increasing your own pleasures, you decide to work ");
		if (pc.cockTotal() > 2) output("one of your free cocks");
		else output("your free cock");
		output(" against her tight ass. Feeling this new intruder, she turns back to you.");
		...
		if (pc.biggestCockVolume() > phyllaCapacity()) {
			output("\n\nSeeing as how her hands aren't busy and she's certainly not using that mouth for anything useful; you reach down and guide one of your cocks so it presses against her down turned stomach.  You tell her to press it between her breasts and start sucking at the head.  Though this doesn't stop you from pounding away at her backside. She quickly grabs your [pc.cock " + pc.biggestCockIndex() + "] with her smaller hands and guides for a better position between her breasts.  She uses her upper hands to press her breasts together.  You immediately feel the effect of her B-Cups as with each thrust they warm the shaft of your long cock.");
		}
	}
	output("\n\nYou maintain your vice-like pressure on her abdomen as you press onward, forcing your member");
	if (y >= 0) output("s");
	output(" deep within her.  You further your power and control over Phylla, in a way that seems to convey to Phylla to let it all out, don't hold back, to have her completely resign herself to you.  The unconscious move unleashes a loud series of lust-filled gibberish from Phylla.  You feel her body fold to your will as she abandons her shyness and restraint, mashing her rump against you with the intent to rut along with you like two primal animals intertwined in savage mating.  You can't see it, but you taste her hunger to be bred; you can feel and visualize her attractive, beet red little face wearing her passion as she clenches her teeth, your instincts and consciousness completely taking her over and commanding her to hump and fuck your cock until she gets what her cunt hungers for.");
	...
	if (pc.cockTotal() > 1) output("s twitch, ");
	else output(" twitches, ");
	output("ushering forth your corrupted seed into her womb");
	if (y >= 0) output(" and her ass.");
	else output(", and onto the bedspread below.");
	output("  As her pulsing cunt works to suck the semen out of your ");
	if (pc.balls > 0) output("balls");
	else output("shaft");
	output(", Phylla retaliates with her own orgasm, flooding your groin with her sticky girl cum.  You glance down to your joined nethers and find a massive amount of Phylla's cum has pooled around your knees and more seems to be flowing every second.  Clearly, she's a squirter.");
...
{
	clearOutput();
	output("Sadly, as you gaze down at your [pc.multiCocks], you realize that trying to penetrate Phylla with your freakishly large prick");
	if (pc.cockTotal() > 1) output("s");
	output(" isn't going to work.  Phylla's eyes begin to tear up as she also comes to the realization that her first time is a non-starter.");
	...
	output("\n\n\"<i>W-What are you doing?</i>\" she asks, somewhat upset that she won't be getting the traditional first time sex.  Arcing your [pc.multiCocks], you ");
	//Dick Size Over 48 but Under 69 inches:
	if (pc.longestCockLength() < 69) {
		output("angle your hips so that ");
		if (pc.cockTotal() == 1) output("it points");
		else output("they point");
		output(" at the ceiling, placing them out of the way for what you have in mind.");
	}
	//Dick size Over 70 inches::
		...
		output(" aimed out the doorway.");
	}
	output("  You wickedly beckon Phylla over to you with a hooked finger.  Phylla looks at you quizzically for a moment before it dawns on her.  Her eyes widen and cooing she scrambles over to you.");
	...
	if (pc.cockTotal() == 1) output(" switches and bobs");
	else output("s switch and bob");
	output(", loudly brushing against the ");
	if (pc.longestCockLength() < 69) output("ceiling");
	else output("wall in the hallway");
	output(".");
	...
	//If Snake Tongue:
	if (pc.tongueType == GLOBAL.TYPE_SNAKE) output("  You run the split of your forked mouth muscle from the tip of her clit to the base.  As your textured feeler makes its way down, you wrap around the rest of her long clitorus, like a boa constrictor.  Once your split reaches the hood of her clit you roll and flick the tip of your tongue rapidly.  Pulling at your hair she gasps for breath in between teeth grinding moans.");
	...
	output("\n\nAt this rate it won't be long before she reaches her peak, so you might as well make the finish one to remember.  Driving your fingers against her clit and teasing her labia you begin to rub at a lightning pace.  Phylla almost instantly loses her balance and stumbles backwards onto your [pc.multiCocks]");
	if (pc.isGoo()) output(", tumbling out of your mass as she tears free");
	output(".  She holds one of her hands out in an attempt to catch her breath.  Ignoring her, you dive in and continue your unrelenting assault as her legs twitch uncontrollably over your head, like a dog having its stomach petted.  Screaming out, Phylla seemingly sputters a garbled string of nonsense until she takes your head in a vice like grip between her thighs.  Feeling her vaginal muscles convulse and tighten on your tongue you notice the signaling of climax.  With one final body contracting pulse Phylla arches her back and roars in orgasmic bliss.  You open your mouth in eager anticipation as her orgasm washes over you.  Her girl cum seems almost never ending as pulse after pulse of it squirt out of her.  You swallow at least two mouthfuls before turning your head, only to feel more splash on your cheek.  Still holding you firmly in between her legs, your Ant Morph lover continues to grind and spurt out juices all over your [pc.face].");
	...
	if (pc.cockTotal() > 1) output("s");
	output(" and pants heavily, having just gone through the equivalent of an orgasm triathlon.  As you sigh, you can hear her giggle in what sounds like disbelief.  \"<i>Heh... I didn't think I had it in me to do... that,</i>\" she coos, staring mischievously up at you.  \"<i>Just... could you shrink ");
	if (pc.cockTotal() == 1) output("this monster");
	else output("these monsters");
	output(" down?  I mean, I'm not complaining!  But I... just... I mean... you... inside me,</i>\" she shyly remarks, obviously wanting something more... traditional.");
	//(Player lust increases to 100)
	pc.lust(pc.lustMax(), true);
	flags["COC.DIDNT_FUCK_PHYLLA_ON_RECRUITMENT"] = 1;
...
{
	clearOutput();
	output("Eager to get to some good old fashion fun, you direct your thoughts to Phylla, telling her to 'remove her clothing'.  After all of the pieces of her royal attire are removed, she closes in and rubs her naked body against yours.  Her fingers move fluidly as she slowly traces her ever hardening nipples along your chest and along the ridges of your back, sending a sensual chill up and down your spine.  Your [pc.nipples] harden in response, and with surprising speed she shifts her body downward and locks her mouth around your nipple, firmly suckling at your hardened areola.");
	//If PC Lactating:
	if (pc.lactationQ() > 250) output("\n\nYou can feel her suck some of the [pc.milkNoun] from your breasts, slating her weak thirst in the process as she becomes immensely aroused at the sensation of \"nursing\" from her lover's " + pc.breastDescript(0) + ".  Weird... you can taste the nutritious warmth of your nectar as it flows down her throat and into her stomach.  Drinking and tasting one's own breast milk could definitely weird someone out, but the foreignness of the act is an odd turn-on for you, and you let her continue to suckle.");
	...
	if (!pc.hasCock()) addButton(0, "Next", girlFiller);
...
private function girlFiller():void
{
	...
	output("\"<i>I-I've... never,</i>\" she stammers, but you just want her to experience this before she ruins the moment.");
...
private function femalePhyllaFirstTimePlusCock():void
{
	...
	output(" hold of ");
	//One dick only:
	if (pc.cockTotal() == 1) output("your " + pc.cockDescript(0) + ".");
	else output("your " + pc.cockDescript(0) + " leaving the lucky member to bask in affectionate glory as the rest of your manhood laments being left out.  \"<i>Don't worry, I have four hands...</i>\" Phylla coos as she works your shaft.");
	...
	if (pc.smallestCockArea() > phyllaCapacity()) {
		//Dicks too Big:
		output("\n\nDespite your best efforts to try and get into a comfortable position to scissor, your painfully erect  [pc.multiCocks] prevent you from doing so, given that they are now compressed against the ceiling.  While the display of rainbows is pretty to watch on your [pc.multiCocks], the crushing pain and the unyielding nature of your [pc.multiCocks] is too much.  ");

		//Horse: (Cont from ...is too much.)
		if (pc.isTaur()) output("Seeing your impending predicament, Phylla quickly scrambles with all four of her arms to collect a bunch of cushions.  She stacks them in a makeshift 'resting place' and helps you into a comfortable position as your cocks precede to grow uncontrollably.");
		//Drider body:
		else if (pc.isDrider()) output("Phylla immediately realizes this, and flips you over so that you are resting on your head and neck, ducking in past your [pc.multiCocks] and coming to rest on the border between your spider carapace and soft clit.");
		//Goo body:
		else if (pc.isGoo()) output("Phylla immediately realizes this, and flips you over so that you are resting on your head and neck, and she works her way through your goopy mass, connecting her clit with yours in little time or effort. \"<i>This is a little weird... but I like it...</i>\" she flirts. Hardening your mass slightly, you snugly secure Phylla against your body, telling her that things \"might\" get rough.");
		//Human/Harpy/Naga legs:
		else output("Phylla immediately realizes this, and flips you over so that you are resting on your head and neck, and curls your [pc.legs] towards her.");

		output("  Your [pc.multiCocks] now resting comfortably out of the open room, Phylla positions herself to scissor your pussy.");
	...
	//If PC has an appropriately sized dick(s):
		...
		output("\n\n[pc.EachCock] hungers for attention, having been let go by Phylla when you shifted into a scissoring position.  Reaching down to start taking care of yourself; Phylla interrupts you abruptly.  \"<i>Oh, I'm sorry! I mean... I forgot... just feels... so gooood,</i>\" she coos between moans of ecstasy.  After a moment she seems to forget as waves of pleasure wash over her.  You make a playful throat clearing noise and gesture to your [pc.multiCocks].  Phylla quickly snaps back into reality and takes hold of ");
		//Tentacle dick check
		if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) > 0) {
			//PC has (a) Tentacle dick(s):
			output("your tentacle dick");
			if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) > 1) output("s");
			output(".  Clearly she's never seen anything like it before as she doesn't know what to do.  Laughing, you tell Phylla she doesn't need to do anything except hold still.  You snake your tentacle cock");
			if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) > 1) output("s");
			output(" up her body, making sure to caress her most tender places on their way.  She screams out loudly in shock only to realize what happened when she looks down.  \"<i>God, you scared me!</i>\" she yells, embarrassed that she got frightened half to death by your dick");
			if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) > 1) output("s");
			output(".  Laughing at her display, you motion to your prick");
			if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) > 1) output("s and wiggle them ");
			else output(" and wiggle it ");
			output("in front of her, like a worm to a bird in her face.  \"<i>Aw, does someone need some attention?</i>\" Phylla teases, taking ");
			//PC has:
			//one tentacle dick:
			if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) == 1) output("your cock into her mouth as she continues to rock her hips into yours.");
			//two:
			else if (pc.cockTotal(GLOBAL.TYPE_TENTACLE) == 2) output("your cocks into her mouth and between her breasts as she continues to rock her hips into yours.");
			else  output("your cocks into her mouth, between her breasts, and along her body.  The ones on her body tightly wrap around her insectile joints, using her muscular curves to get you off.");
			...
			if (pc.cockTotal() == 2 && (pc.cockVolume(pc.smallestCockIndex2()) + pc.cockVolume(pc.smallestCockIndex()) <= phyllaCapacity()))
				output("two of your [pc.multiCocks], stroking them apologetically as she scissors you.  You feel her almost insatiable appetite for all parts of your body.  You feel Phylla's mind slowly becoming lost in pleasure.");
			//TODO: "pc.cocks[pc.smallestCockIndex()] < 4" changed to cockLength. Not sure if it is the appropriate attribute.
			else if (pc.cocks[pc.smallestCockIndex()].cLength() < 4) output("your " + pc.cockDescript(pc.smallestCockIndex()) + ", stroking it apologetically as she scissors you.  You feel her almost insatiable appetite for all parts of your body.  You feel Phylla's mind slowly becoming lost in pleasure.");
			//PC only has a dick(s) that('s) (are) more than 4 inches in width:
			else output("your " + pc.cockDescript(pc.smallestCockIndex()) + ", but she simply can't fit any number of her hands around the entirety of your glans; instead, she takes to caressing the head of your " + pc.cockDescript(pc.biggestCockIndex()) + ".");
...
{
	clearOutput();
	output("You smile at her and tell her you would love for her to join you at your camp.  Her face brightens like the sun and she quickly gathers the very few possessions she owns - mostly clothing, the pillows, and some jewelry.  Together you promptly leave the colony and head back to camp.");
	output("\n\n(<b>Phylla has moved in!  She can be found in the lovers tab!</b>)");
	flags["COC.ANT_WAIFU"] = 1;
	addNextButton();
}
...
public function introductionToPhyllaFollower():void
{
	...
	if (flags["COC.PHYLLA_CAMP_VISITS"] == 0) {
		...
		if (flags["COC.DIDNT_FUCK_PHYLLA_ON_RECRUITMENT"] == 1 && pc.hasCock()) {
			output("\n\nYou raise your eyebrow and ask her how she can be pregnant if you never stuck it in.");
			output("\n\nShe takes a deep breath and continues.");
			output("\n\n\"<i>I don't need semen to have children; I just need certain fluids.</i>\"  Again she blushes and half covers her face with a pillow.  She looks as if she's not going to continue until she looks down between your [pc.legs].  \"<i>You were kind of leaking them...</i>\"  All she can muster is a deeper blush...");
		}
		else if (!pc.hasCock()) {
			output("\n\nYou raise your eyebrow and ask her how she can be pregnant if neither of you have a penis.");
			output("\n\nShe takes a deep breath and continues.");
			output("\n\n\"<i>I don't need semen to have children; I just need certain fluids.</i>\"  Again she blushes and half covers her face with a pillow.  She looks as if she's not going to continue until she looks down between your [pc.legs].  \"<i>You know the kind of fluids I'm talking about...  I mean, they're sweet and we shared them... they come from our... I mean, your...</i>\"  All she can muster is to point at where your vagina is located...");
		}
	...
	if (flags["COC.PHYLLA_GEMS_HUNTED_TODAY"] != days && flags["COC.ANT_KIDS"] > 0) addButton(5, "Find Gems", phyllaDigsForGems);
	else addDisabledButton(5, "Find Gems");
	
	addButton(14, "Back", campLoversMenu);
	IncrementFlag("COC.PHYLLA_CAMP_VISITS");
}

private function phyllaSexMenu():void
{
	clearMenu();
	addDisabledButton(0, "Get BJ", "Get BJ", "This option requires you to have cock.");
	addDisabledButton(1, "Fuck Her", "Fuck Her", "This option requires you to have cock.");
	addDisabledButton(2, "Lesbian Sex", "Lesbian Sex", "This option requires you to have vagina.");
	addDisabledButton(3, "Orgy (Male)", "Orgy (Male)", "This option requires you to have cock, high corruption and at least 10 ant children.");
	addDisabledButton(4, "Orgy (Female)", "Orgy (Female)", "This option requires you to have vagina, high corruption and at least 10 ant children.");
	addDisabledButton(5, "Oviposit", "Oviposit", "This option requires you to have drider ovipositor and eggs.");
	...
	//Sex > [Egg Phylla]
	if (pc.canOvipositSpider()) addButton(5, "Oviposit", eggDatBitch);
...
{
	clearOutput();
	flags["COC.TALKED_WITH_PHYLLA_ABOUT_HISTORY"] = 1;
	output("You relax on one of the many available pillows and ask Phylla about her race's history. She pulls a pillow up next to yours and cuddles in.  \"<i>It's not a very happy story, are you sure you want to hear it?</i>\" You nod and move your hand to hold one of hers.");
	...
	//[If corruption more than 80]
		...
		if (int(flags["COC.FACTORY_SHUTDOWN"]) <= 0) {
...
{
	clearOutput();
	output("You've always wondered how Phylla can project thoughts and images of her life into your mind. Seeing as how she's not busy, you ask her about it.  She tilts her head, trying to think of a way to explain it.  You can see this is taking some thought as she stays silent for almost a minute.");
	...
	//If corruption more than 60:
		...
		if (pc.cor() >= 80) {
			...
			if (flags["COC.PHYLLA_EGG_LAYING"] > 0) output(", sending jets of milk shooting out onto the floor");
			output(".  With a loud moan she orgasms, covering her smaller two forearms, her inner thighs, and the floor below her in thick, sweet-smelling, girl cum.");
	...
	processTime(9);
	flags["COC.PHYLLA_INHERITED_KNOWLEDGE"] = 1;
	addNextButton();
}
...
private function phyllasLifePastAndFuture():void
{
	clearOutput();
	//(If Izma at Camp)
	if (izmaFollower() && int(flags["COC.PHYLLA_IZMA_TALK"]) == 0) {
		flags["COC.PHYLLA_IZMA_TALK"] = 1;
		output("Phylla seems to be a little distant today as you sit down to talk.  You point it out to her and she cautiously asks about Izma.");
		...
		if (pc.cor() < 40) {
			...
			if (flags["COC.PHYLLA_INHERITED_KNOWLEDGE"] == 1) {
		...
		output("You take a seat on one of the many fluffy cushions in her room and she proceeds to cuddle in close, pressing her head against your chest and wrapping her abdomen around you.  She then looks up at you and asks what's on your mind.");
...
{
	clearOutput();
	IncrementFlag("COC.PHYLLA_BLOWJOBS");
	if (flags["COC.PHYLLA_BLOWJOBS"] == 1) {
		output("Turning to Phylla, you can't help but smile as you flash her a seductive look, making sure you have her undivided attention before glancing down at the bulge in your pants.  Raising a hand to her mouth in sultry intrigue, Phylla giggles and shoots you a devilish look back.  Turning towards the bed, Phylla presumptuously begins to disrobe...  close!  But not what you had in mind.");
	...
	//Subsequent Blowjobs:
		...
		output("Approaching Phylla, you cross your arms and lean against the wall of her bedchamber while sporting a tell-tale tent in your pants.  Phylla stares down at your bulge and picks up immediately on your intentions.");
		output("\n\n\"<i>Y-You gonna ask me nicely, or let your penis");
		if (pc.cockTotal() > 1) output("es");
		output(" do all the convincing?</i>\"  She teases.  Shrugging, you inform her that whatever works for her works for you.");
	}
	//Continued...
	//If Phylla is not Laying Eggs:
	...
	if (pc.cocks[x].cLength() < 60) {
		...
		if (flags["COC.PHYLLA_BLOWJOBS"] == 1) {
			...
			if (pc.cockTotal() > 1) output("s");
			output(" from your pants, but apparently doesn't realize just how big ");
			if (pc.cockTotal() == 1) output("this");
			else output("these");
			output(" battering ram");
			if (pc.cockTotal() > 1) output("s");
			output(" can get.  Chuckling at her ignorance, you tell Phylla she'd better back up.  Phylla rolls her eyes and looks up at you, giving you an unamused look.");

			output("\n\n\"<i>I-It can't be THAT big, [pc.name]...</i>\"  She chiddingly remarks, causing you to raise an eyebrow in eager amusement.  You know she's not ready to meet the challenge you're about to unleash upon her.");
			output("\n\nYou work to release yourself from your clothing.  Once you're completely free you watch as Phylla's eyes widen in horror.  Smiling, you start to stroke your [pc.multiCocks] and work ");
			if (pc.cockTotal() == 1) output("it");
			else output("them");
			output(" up to a full raging erection, achieving your maximum state of \"<i>readiness</i>\" in no time at all.");

			output("\n\nDumbfounded, Phylla stares in awe at how big your [pc.multiCocks] can really get; you can see the look of complete intimidation on her face and who could blame her?  You've seen ship cannons that pale in comparison to your man meat.");
			//If multiple cocks:
			if (pc.cockTotal() > 1) {
				output("\n\nShe's staring down a mighty series of threatening fleshy spears.");
			}
			output("\n\n\"<i>I don't know if I can... wow...</i>\" She sputters out.  Laughing out loud, you grin down at her and inform her that, in this instance, her slight ignorance IS bliss.  Twitching the muscles in your [pc.multiCocks] to bob a bit in her face you let her know that you rose to the challenge; now it's time for her to \"<i>fulfill</i>\" her end of the bargain: the cock end that is.");
		}
		//Dick size more than 60 inches (subsequent blowjobs):
		else {
			output("\n\n\"<i>Okay, you're gonna have to be careful so we don't have a repeat of ");
			if (flags["COC.PHYLLA_BLOWJOBS"] == 2) output("last time");
			else output("our first time");
			output(".  I'll help to free ");
			if (pc.cockTotal() == 1) output("that monster");
			else output("those monsters");
			output(", but p-please be careful about my valuables.</i>\"  Phylla asks nervously.");
			output("\n\nYou tell her that as long as she's helping, there shouldn't be any \"<i>accidents.</i>\"  With synchronous effort, your hulking [pc.multiCocks] ");
			if (pc.cockTotal() == 1) output("is");
			else output("are");
			output(" freed once more, ready and raring to \"re-acquaint\" with Phylla.  Taking the hint, Phylla closes in and takes the ");
			//Dick size less than 16 inches:
			if (pc.cocks[x].cLength() < 16) output("base");
			else output("middle");
			output(" of your shaft against her tongue.  You hear a mix of a giggle and hum as she drags her tongue towards the head of your " + pc.cockDescript(x) + ".  Phylla teases your shaft as she reaches the peak and suckles at your urethral opening.  You feel her drawing up as much pre-cum as she can and lapping it up like she's dying of thirst.  She continues this for a bit, until it becomes apparent that she's either not too sure of what else to do, or that she's never heard of how to properly fellate someone.");
			output("\n\nWith a chuckle, you pull your " + pc.cockDescript(x) + " away from her mouth and ask her to look up at you. Clearly embarrassed at your realization of her skills, she begins to tremor softly.  \"<i>S-sorry! I mean, I've never done this!  This act is uncommon for my people.  Such things are not proper for a queen to be...  taught, or learn.</i>\"  She apologizes, begging your forgiveness with her body language and demeanor.");
		}
	}
	clearMenu();
	//(If Corruption less than 75 transitions to - Pure BJ Ending)
	if (pc.cor() < 75) addButton(0, "Next", purePhyllaBJOver);
...
private function purePhyllaBJOver():void
{
	clearOutput();
	var x:int = pc.smallestCockIndex();
	output("You quickly and softly tell her to calm down, she needn't worry; you'll be more than happy to help her learn how as surely as she would help you if your situations were reversed.  At the mention of counter positions she nods cheerfully.");
	output("\n\n\"<i>You... know I'd d-do anything for you... I'll get good, I will!</i>\"");

	output("\n\nHappy and secure that you're willing to tolerate her lack of experience, Phylla lowers herself back down to your " + pc.cockDescript(x) + " and gives you a tender lick on your head, slurping in all the pre-cum you've been accumulating.  You opt to help her with a more direct approach; you lower your hand down to her chin and tilt her head up so her eyes meet yours.  The tension in Phylla's jaw is fierce, and you tell her that she's putting too much thought and effort into this: just relax. You move your thumb over her lips and part her mouth slightly.  A calm washes over her as your directive registers.");
	output("\n\nPeacefully, you tell her to open her mouth just a bit and to let your " + pc.cockDescript(x) + " slide into her mouth, advising her to watch the teeth.  Once she's got it in there, you tell her she can massage your head with her tongue and lips to begin with.  Eventually she'll work her way up to experimenting with the interior of her mouth.  Then once she gets that down, she can take you into her throat, but that's a more 'advanced technique.'");
	output("Taking your directions, Phylla takes the head of your cock along the top of her wet and humid tongue, leisurely batting around before trapping it with her lips.");
	...
	if (pc.cockTotal() > 1) {
		...
		else output("the rest of your dicks");
		output("; if she would be \"<i>so kind.</i>\" With gusto, her four hands shoot up and take your ");
		if (pc.cockTotal() == 2) output("cocks into their firm grips");
		else output("cock into their firm grips");
		output(".  Phylla mumbles something but absentmindedly realizes she still has your cock in her mouth and gives up on whatever it was she was trying to say.");
	}
	//Vagina:
	if (pc.hasVagina()) {
		output("\n\nYou ask Phylla if she can take care of your womanhood while she's at it.  Phylla moves one of her small arms in between your legs and rubs against your " + pc.clitDescript() + ".  She caresses your labia in between her fingers before inserting them inside of you.  Phylla begins working her wrist diligently in conjunction with her head bobbing.");
	}
...
{
	clearOutput();
	output("You pass on her offer, not wanting to know what THAT sensation");
	//If PC can't Autofellatio:
	if (!pc.canAutoFellate()) output(" or anything like it would feel like.");
	else output(" feels like with Phylla's mind linked to your own.");
...
private function pureBJEnding(linked:Boolean = true):void
{
	...
	if (!linked) output(".");
	else if (int(flags["COC.TIMES_LINKED_BJ_SUCK"]) == 0) {
		output(", ");
		//If PC linked, first time occurring:
		output("leading to you jesting through the link and calling her a damn dirty cheater.");
	}
	if (flags["COC.TIMES_LINKED_BJ_SUCK"] > 0) output("  You can feel her relying less and less on the link with every subsequent time; there's no doubt in your mind that she will no longer \"<i>cheat</i>\" her way into making you cum before you know it.");
	var x:int = pc.smallestCockIndex();
	output("\n\nShe giggles up at you as she bobs back and forth along your shaft, sensing the telltale signs of your pending orgasm with her mouth.  When you feel the moment of climax finally arrive, you grab onto the back of her head with your hands. She jumps at the unexpected feeling, but before she can react you roar, thrusting your hips forward.  Phylla, like a pro, times her motions with your release, drawing a steady stream of semen out of your rod.  With each spasm of your cock you shoot your cum into her mouth.  She doesn't relent until she's got a mouthful, pulling free of your " + pc.cockDescript(x) + " as ");
	if (pc.cumQ() < 250) output("thin");
	else if (pc.cumQ() > 1000) output("thick");
	output(" strands of cum hang wildly between her lips and your prick.");
	//If cum volume over 1000ml::
	if (pc.cumQ() > 1000) output("  Though she might have a mouthful, you're not finished yet!  You continue to unload the remainder of your hot load on her breasts and stomach.");
	if (pc.cockTotal() > 1) {
		//If multi cock::
		output("  Your other cock");
		if (pc.cockTotal() > 2) output("s shower");
		else output(" showers");
		output(" her hands and body in your [pc.cum] as she continues to pump ");
		if (pc.cockTotal() == 2) output("on each of ");
		output("them lovingly.");
	}

	output("\n\nWith a heavy sigh, and significantly lighter ");
	if (pc.balls > 0) output("balls");
	else output("in the pelvis");
	output(", you rub Phylla's hair playfully");
	if (flags["COC.PHYLLA_BLOWJOBS"] != 1) output(".");
	//If first time:
	else output(" and congratulate her on her first time.");
...
{
	clearOutput();
	output("Crossing your arms, you tell Phylla to spit your load out.  As you dress yourself back up in your [pc.gear], you hear Phylla frantically dig out a hole in the floor.  She playfully leans over and spits the salty payload into it.  She makes a soft hum when she's finished, before quickly covering it up again.  \"<i>Thank you, [pc.name].  Next time I'll do better, I promise.</i>\"");
	addNextButton();
}
...
private function corruptPhyllaEndings():void
{
	clearOutput();
	var x:int = pc.smallestCockIndex();
	output("\"<i>A slut that can't suck a dick... how can this get any worse?</i>\"  You ponder to yourself while rubbing your nasal bridge. You tell the innocent little ant that she'll learn how to suck a mean cock, even if it kills her.  You're knee deep at this point - might as well turn her into something salvageable.  Commanding her attention, you lift her face up and hover your " + pc.cockDescript(x) + " close to her lips, smothering her lips with your precum.  She attempts to turn away but you react before she can.  Holding her head in place you comment on how this is what you want from her.");
	...
	//If first time:
	if (flags["COC.PHYLLA_BLOWJOBS"] == 1) output("achieved the desired effect.");
	//Every other time:
	else output("improved since the last time.");
	...
	output("\n\nPhylla groans at the rough treatment, not used to being treated so coarsely as your " + pc.cockDescript(x) + " lunges back and forth in her mouth, grinding along her cheek and tongue. Your cock seems to have a mind of its own as you somehow manage to wrap her tongue around yourself, knowing full well Phylla lacks the technique required to do it on her own.  The little queen is about to receive a rude awakening about pleasing her master.");
	output("\n\nMoan after muffled moan escapes from Phylla's mouth as you continue your callous fucking of her delicate little face.  Your little slut is trying her best to keep up with your thrusts but is ultimately failing.  Perhaps she needs a little motivation?  Withdrawing your " + pc.cockDescript(x) + ", you tell her she doesn't get to suck your dick anymore unless she fingers her tight little hole. Desperate to fulfill your desires, she quickly darts a hand to her pussy and fingers that little fuckhole like her life depends on it.  Within seconds you see her long clit protruding, begging to be touched.");
	...
	if (pc.cockTotal() > 1) {
		...
		output(" if she would be \"<i>so kind</i>\".  With gusto, her four hands shoot up and take your cocks into their firm grips.  Phylla mumbles something but absentmindedly realizes she still has your cock in her mouth and gives up on whatever it was she was trying to say.");
	}
	//Vagina:
	if (pc.hasVagina()) {
		output("\n\nYou ask Phylla if she can take care of your womanhood while she's at it.  Phylla moves one of her small arms in between your legs and rubs against your " + pc.clitDescript() + ".  She caresses your labia in between her fingers before inserting them inside of you.  Phylla begins working her wrist diligently in conjunction with her head bobbing.");
	...
	//Continuation:
	output("\n\nCongratulating her for her hasty heeding of your wishes, you ram your " + pc.cockDescript(x) + " back into her mouth and continue the face fucking once more.  Phylla picks up the effort as she starts working her head and tongue in tandem with your hips.  Soon she's driving you perilously close to an orgasm.  With one last directive, you ask Phylla if she's ready to receive your seed.  Though since you continue your brutal fucking of her mouth, she has no opportunity to reply.");
	...
	else if (pc.cumQ() < 100) output("respectable");
	else output("massive");
	output(" facial, covering her petite features in cum.");
	...
	if (silly) {
		...
		output(".  Somehow, you succeed your fortitude roll and remain conscious without passing out even after plummeting to the ground from sheer exhaustion.  You and Phylla can't help but slowly drift off to sleep in a pool of your cum to dream about the bizarre pleasures you two have indulged in.");
	}
	...
	if (!silly) {
		output("\n\nPhylla works your " + pc.cockDescript(x) + " the entire time you cum, as if pleading for just a little more to come out.  You grab her hands, signaling you've have just about enough.  Coming down from your climax, you thank your slut for the oral and sit down on the bed and start gathering your scattered clothing.");
	...
	output("\n\n...Can you? You muse again, thinking about it.  After a few moments of helping Phylla to the pile of, now very wet, cushions she calls a bed, you finish donning your [pc.gear] and head back to the surface.");
...
private function dickPhylla():void
{
	...
	clearOutput();
	output("You give Phylla a devious look that denotes you didn't come here to just talk.  She looks a little surprised and embarrassed for you as you start removing your [pc.gear].  Noticing her watching, you pull each article of clothing off a little slower, letting her lust build.  You seductively drop your armor, completely revealing yourself to her.  You can see her eyes widen as she visibly feasts on your features.");
	//(NO BJ experience)
	if (int(flags["COC.PHYLLA_BLOWJOBS"]) == 0) {
		...
		output("\n\nCutting her off, you gently place your hand on the back of her head as [pc.eachCock] hardens even further.  With a slight pressure you force Phylla's head down onto the head of [pc.oneCock]. At this point she gets the picture.");
	}
	...
	output("\n\nHer eyes widen in surprise as your " + pc.cockDescript(x) + " fills her.  She howls a blissful scream as she drools forth her girl fluids, coating your cock ");
	if (pc.balls > 0) output("and balls ");
	output("in her sexual liquids.");
	...
	if (pc.cockTotal() >= 2 && pc.cockTotal() <= 5) {
		if (pc.hasVagina()) {
			...
			output("\n\nYou look at Phylla and shrug at her, telling her you aren't sure if you can do that, given that you have some... 'members' that are feeling a little left out.  With a flirty yet annoyed expression,  your lover reaches down and takes hold of your unloved  [pc.multiCocks].  She coos as she starts to pump ");
	...
	output("\n\nHolding out just isn't feasible anymore, and with one more great slam into her, you get as deep as you can and pin [pc.oneCock] inside Phylla as you release the hot seed of life.  As your grip tightens around her abdomen, you hear her let out a gleeful cry of satisfaction.  The quivering mass of meat spear causes her to orgasm as well, washing your lower body in her warm sweet-smelling girl cum.  You feel her transmit the feeling of her insides swallowing your load, as her pussy's spasming starts to subside.");
...
private function lesbianFisting():void
{
	...
	output("\n\nWithout saying a word, you extend your hand and beckon her over to you with a hooked finger. Raising your eyebrows seductively, you make your intent crystal clear for her.  She quickly scampers off the bed and kneels in between your [pc.hips].  Her upper two hands hold your legs apart, resting on your inner knees.  Her smaller set of hands gently begin to massage your inner [pc.legs] right on the sides of your pussy lips, driving pleasure through your thighs.  It's not long before beads of lubricant start to flow out of your quickly moistening labia, a slight moan of sexual excitement slipping past your lips as Phylla's tongue starts to lick the lubricant coming out of you, pausing only briefly to taste your juices in her mouth like a fine wine.  You fall forward as her mouth finds your cilt, wrapping your hands around her head to encourage her to continue.  Your legs attempt to wrap around her head but her upper arms' tremendous strength prevents them, leaving you \"<i>hopelessly</i>\" wide open for her assault.  Her lower hands stop massaging you, and for a moment leave you to wonder what's going on down there.  As you're about to ask why she stopped, her mouth plays erotically with your clit and one of her smaller hands spreads you wide open.");

	//If PC has cocks:
	if (pc.hasCock()) output("\n\nNot wanting to deprive you of what your body painfully wants, Phylla grabs hold of [pc.oneCock] and starts pumping away as she does wonders on your female anatomy with her mouth.");

	output("\n\nMoaning loudly into the air, the aroma of earth, sweat, and sex fills your nostrils.  You can't help but feel a little embarrassed to be so fully exposed to her, but that quickly passes as one of her fingers slips along your labia and brushes past the opening of your " + pc.vaginaDescript() + ".  Guided by the wetness of your ever eager pussy, her hard fingertips slide forward into your body.  You moan again at this slow insertion.  Encouraged, Phylla rubs up and down the walls of your vagina, leisurely at first but with mounting vigor, searching for your G-spot.");
...
{
	clearOutput();
	output("Phylla looks completely taken aback when you suggest you want to have sex with her.  Obviously self conscious about how she looks right now, she covers her enlarged breasts with all four of her hands as she sits back in her seat.");
	...
	if (pc.cor() < 50) output("don't care how she looks right now. Besides, some creatures around here appreciate a little carnal love; it helps with the birthing process.");
	//If Corruption is more than 50:
	else output("could care less for her pathetic attempts at concealing her pregnant form; you're horny, and she's going to put out, regardless of her insecurities.");

	output("\n\nYou walk over to her and ");
	if (pc.cor() < 50) output("lovingly");
	else output("apathetically");
	output(" pull her hands away from her breasts.  She resists you at first, but once she sees there's no stopping you she shyly resigns herself to you.");
	...
	//(If PC has balls:
	if (pc.balls > 0) output(" your balls slap against the smoothness of her pelvis.");
	else output(" your knees grind along the edge of her makeshift throne.");
...
{
	clearOutput();
	output("Phylla looks completely taken aback when you suggest that you want to have sex with her.  You can tell she is self conscious about the way she looks right now by the way she covers her enlarged breasts with all four of her hands as she sits back in her seat.");
	...
	if (pc.cor() <= 50) output("  You probe her mind and are almost taken aback.\"<i>NO!  Just please keep going!</i>\"  You hear her voice ring out in your head.");
	...
	//If Corruption is over 50:
		...
		output("\n\nSensing that she is nearing an orgasm, you pull back and wrestle out from betwixt her legs.  Finally free, you see she's sprawled out on the bed, her massive abdomen hanging off to the side.  Settling yourself over the top of her, you slide your " + pc.clitDescript() + " along her nose, intent on getting your scent entrenched in her body, wanting to make her crave your pussy, something that occurs as she takes a long whiff of air and moans like a whore in heat.");

		//If PC has loose pussy:
		if (pc.vaginas[0].looseness() >= 4) output("\n\nYour loose pussy lips cause your juices to drip onto Phylla's face as you hover over her, allowing her to lather in the wetness of your arousal.");
	...
	//EAT PUSSY:
	output("\n\nLike a starving animal, Phylla's mouth waters as she takes hold of your thighs and spreads your legs apart.  You grab hold of her hair and shove her head inbetween your legs, impacting your " + pc.clitDescript() + " and causing you to moan slightly.  She certainly isn't shy now, you think, as her tongue starts to work its way into you.  You can \"<i>hear</i>\" Phylla giggle back in response. Thoughts of grinding your pussy across her mouth and face flood your mind, and instinct takes over as your hips start to rock in response to Phylla's eager attack.  Running your hands through her hair,  you grind Phylla's pretty face harder and deeper into your soaking " + pc.vaginaDescript() + ".  The poor ant can only respond by moaning fiercely as she eagerly sucks and kisses your love button, completely overwhelmed by the desire that has overtaken her.");

	//If PC has (a) Dick(s), that are less than 42 inches:
	if (pc.cockTotal() > 0 && pc.cockThatFits(42, "length") >= 0) {
		output("\n\n[pc.EachCock] begs for your touch, and with no hesitation you grab a hold of ");
		if (pc.cockTotal() > 1) output("them");
		else output("it");
		output(", stroking ");
		if (pc.cockTotal() == 1) output("it");
		else if (pc.cockTotal() == 2) output("them");
		else output("as many as you can");
		output(" as fiercely as your body will let you.");
	}

	output("\n\nAs you feel your stiff " + pc.clitDescript() + " glide along her lips, you can begin to taste bits of her almost primal hunger for you.  As well, you sense that the growing contractions in her abdomen are getting quicker and stronger.");

	//If Corruption is less than 50:
	if (pc.cor() < 50) output("\n\nYou'll need to finish up quickly; you don't want to keep the children waiting to get her egg while you two are \"<i>busy.</i>\"");
	//If Corruption is more than 50:
	else output("\n\nIf she lays an egg before you're done with her, not only will you be furious, but you'll have to scare your kids away.  " + pc.mf("Daddy", "Momma") + " needs to get " + pc.mf("his", "her") + " rocks off first before any annoying kids get in the way.");
	...
	output("\n\nYour " + pc.clitDescript() + " comes to rest on her lips again as her head starts to bob between her own strong legs.  Returning the sentiment, you dart in-between her legs, tongue first.  You hit that sweet tasting little pussy of hers with force.  Phylla moans, your intentions crystal clear as she bears down on your pussy hard, intently working for your sexual release.  She reaches up with her lower set of arms and gives both your [pc.nipples] a firm squeeze, the twist of which is nothing short of exquisite.  Muffled moans of bliss are unintelligible, but in your shared mind-state, you can hear one another screaming words of encouragement and direction.");

	//If PC has (a) Dick(s), that are less than 42 inches:
	if (pc.cockTotal() > 0 && pc.cockThatFits(42, "length") >= 0) output("\n\nWithout warning, Phylla begins to alternate between giving your " + pc.clitDescript() + " oral and sucking your [pc.multiCocks]; the dual stimulation overwhelms you as she devilishly teases your sexes.");
	...
	if (pc.cockTotal() > 0 && pc.cockThatFits(42, "length") >= 0) {
		...
		if (pc.cockThatFits2(42) >= 0) output("two of ");
		output("your [pc.multiCocks] into her mouth and mashes ");
		if (pc.cockThatFits2(42) >= 0) output("them");
		else output("it");
		output(" against the back of her throat.  Unable to contain the pleasure from this, you howl and blow your hot load into her throat.");
	}
	else if (pc.hasCock()) output("\n\nFeeling your own [pc.multiCocks] about to erupt, you quickly roll off your lover.  Just in time, too - you fire your jizzum up at the ceiling, coating it quite well before your salty load falls back down on top of the two of you.");

	//PC dick is out in the hallway:
	if (pc.biggestCockVolume() >= cockVolume(500)) {
		output("\n\nYour [pc.multiCocks] twitches with orgasmic release, bulging as your load works its way through your urethra");
		if (pc.cockTotal() > 1) output("s");
		output(".  With a heavy heave, you feel the hot and [pc.cumVisc] ejaculate launch from your dick, hitting the interior decorations as you do so; what exactly you hit, you can't say... but you'll likely be tripping over your mess on the way out.");
	}

	output("\n\nYou struggle to breathe for a moment as a massive amount of warm girl cum seems to flood from Phylla's vagina to completely cover your face and soak most of your " + pc.hairDescript() + ".  While you catch your breath, you can hear Phylla noisily slurping up your fluids before compressing her face against your " + pc.clitDescript() + " in an eager effort to lather her face in your juices for just a little longer.  Contentment and fulfilled desire - not only your own, but Phylla's as well - spreads like wildfire through your brain.  You grin to yourself, satisfied at the effect you've had on Phylla during your session.");
	...
	if (pc.cor() < 75) {
		...
		output("\n\nUp the hallway, you can hear the patter of soft feet; one of your many children must be coming to fetch the egg.  Acting quickly, you grab your [pc.gear] and position it so that it obstructs the view of your naked bodies.");
...
{
	IncrementFlag("COC.TIMES_CORRUPT_MALE_ANT_ORGY");
	clearOutput();

	output("You tell Phylla to call in your strongest and most virile children for 'inspection.'");

	//First Time:
	if (flags["COC.TIMES_CORRUPT_MALE_ANT_ORGY"] == 1) {
	...
	if (flags["COC.PHYLLA_EGG_LAYING"] > 0) output("large, lactating ");
	else output("small ");
	output("orbs.");
	...
	output("\n\nIt's then that you realize you're still completely clothed!  How inconsiderate of you.  You quickly shed your [pc.gear].  Finding your own cock");
	if (pc.cockTotal() > 1) output("s");
	output(" stiff and longing for immersion into this orgy, you just need to find an opening.  Walking over to Phylla, you tap the ant getting a tit fuck on the shoulder.  Without a second thought, he stands aside for you, letting his balls slip out of Phylla's mouth with an audible pop.  You grin down at Phylla, seeing that she's now completely lost and just enjoying herself.  It's at this moment that you feel shadows looming behind you.  Before you can react, hands reach down and start lovingly stroking [pc.oneCock].");

	//If player has two-three dicks:
	if (pc.cockTotal() > 1 && pc.cockTotal() <= 3) {
	...
	output("\n\nHer mouth hangs open as if begging to be filled by [pc.oneCock].  You didn't come here to get a blow job - you want her cunt.  You want your corrupt dick inside her.  You want to make her have to beg for release.");
	...
	var x:int = pc.cockThatFits(phyllaCapacity());
	if (x < 0) x = pc.smallestCockIndex();
	output("\n\nWhen you've felt your mind completely sync up with everyone else in the room, it's like being drunk on sexual euphoria.  You almost lose yourself in it for a moment, but focusing your mind, you snap your eyes open. You see Phylla in front of you looking like a drugged slut; she runs all four of her hands over your " + pc.cockDescript(x) + " at the entrance of her flooded cunt while her children play with her nipples and suck each other off.  Aggressively, you grab her arms and pin her down while simultaneously shoving your cock into her.");
	...
	output("\n\nOne of your brood leans down and starts to feverishly lick the base of Phylla's clit as your cock slides in and out of her. His warm saliva coupled with Phylla's natural lubricant warms your cock and sends even more pleasure throughout your " + pc.cockDescript(x) + ".");
	...
	if (pc.cockTotal() > 1) output("s");
	output(" sucked, stroked, inserted, and finally orgasming so many times that you lose track. Hours pass and you eventually pass out sitting atop Phylla's throne watching the writhing mass before you.");
...
{
	clearOutput();
	IncrementFlag("COC.TIMES_CORRUPT_FEMALE_ANT_ORGY");
	//(First Time)
	if (flags["COC.TIMES_CORRUPT_FEMALE_ANT_ORGY"] == 1) {
		output("She gives you a confused look, but does as you command. Tilting her head back and closing her eyes you watch as she silently 'communicates' to her children.");
		output("\n\n\"<i>I did what you asked. Five of my~ I mean our... finest warriors are on their way here.</i>\"");
		output("\n\nYou give her a nod, hoping that 'the finest warriors' will also have the finest cocks. Your mind quickly wanders to how it's going to feel having five dicks to do with what you please. As you thoughts lead you down the familiar path you can feel your [pc.vagina] moisten");
		if (pc.hasCock()) output(" and [pc.eachCock] harden");
		output(" in anticipation. Snapping you out of your daydream, five males walk into the room holding themselves like veteran soldiers.");
	}
	//(Subsequent Times)
	else output("She nods weakly, knowing full well what 'an inspection' means by now. Looking down at the floor she closes her eyes and concentrates. After a moment you hear the familiar sound of footsteps rushing to her room. By your guess you would assume the children must be as excited for this as you are. Once they enter the room you can see your hypothesis is indeed correct.  All five of the muscular males are already completely nude and their 10-14 inch cocks are standing erect in anticipation.");
	...
	if (pc.cockTotal() > 0) {
		...
		if (pc.cockTotal() > 1) output("s");
		output(".  You feel your load");
		if (pc.cockTotal() > 1) output("s");
		output(" splash into your [pc.chest] and chest as ");
		if (pc.cockTotal() == 1) output("it");
		else output("they");
		output(" unload.");
	}
...
private function phyllaLaysEggsToggle():void
{
	...
	processTime(5);
	//(Lay Eggs)
	if (int(flags["COC.PHYLLA_EGG_LAYING"]) == 0) {
		flags["COC.PHYLLA_EGG_LAYING"] = 1;
		output("Phylla seems confused as to what you're thinking as you gaze deeply into her eyes.  You gather all four of her hands between the two of yours and tell her that children are wanted.  Not just one or two; you want as many as she can muster.");
		//If you've talked to her about history:
		if (flags["COC.TALKED_WITH_PHYLLA_ABOUT_HISTORY"] > 0) output("  You tell her you want a colony like those she spoke of in the times before the corruption.");
	...
	//(Stop Laying Eggs)
		...
		flags["COC.PHYLLA_EGG_LAYING"] = 0;
		output("You sigh to yourself; you never really wanted this conversation but you knew it might happen someday.  You make a stern face and put a determined fatherly tone in your voice.  Phylla immediately senses you mean business and you have her full attention.  You try to tell her as gently as possible that you don't think it is safe for her to keep having children.");
...
{
	clearOutput();
	output("You tilt your head slightly.  You thought she said she could start to lay any time after the first time you had sex with her.");
	...
	//First Time:
	if (int(flags["COC.ANTS_BIRTHED_FROM_LICKING"]) == 0) {
		...
		output("\n\n\"<i>This is the beginning of a whole new life for me...  I mean, for us.  I'll always remember this, [pc.name]!</i>\"  Phylla cries, cradling the egg close to her, proud to be a mother.  \"<i>I... I love you.</i>\"");
	...
	flags["COC.PHYLLA_COOLDOWN"] = 12;
	IncrementFlag("COC.ANTS_BIRTHED_FROM_LICKING");
	if (flags["COC.ANT_KIDS"] < 5000) flags["COC.ANT_KIDS"] = int(flags["COC.ANT_KIDS"]) + 5;
	processTime(25 + rand(10));
	addNextButton();
}
...
private function dontLickAntButt():void
{
	...
	if (flags["COC.ANT_KIDS"] > 1) output("; the children can tend to the eggs and I can really use the rest");
	output(".  You should get back to your duties...  B-but I wouldn't mind if you came back later...</i>\"");
	...
	flags["COC.PHYLLA_COOLDOWN"] = 6;
	IncrementFlag("COC.ANT_KIDS");
	processTime(25 + rand(10));
	addNextButton();
}
...
private function phyllaLaysSomeDriderEggs():void
{
	...
	if (pc.cor() < 75) {
		...
		if (flags["COC.PHYLLA_EGG_LAYING"] > 0) output("both eggs are just fine on their own");
		else output("the egg is just fine on its own");
		output(", you redirect your focus to helping Phylla through the labor.  Each subsequent egg that finds its way to freedom causes Phylla's once pain filled screams to turn into pleasurable moans of euphoric delight as her pussy becomes overwhelmed by the stimulation of each of your eggs working their way out.  At long last, the final egg slithers out of her vagina and lands gently on the bedding.  Her ordeal over, Phylla breathes a long sigh of relief.");
		//First Time:
		if (int(flags["COC.PHYLLA_TIMES_DRIDER_EGG_LAYED"]) == 0) output("\n\n\"<i>I-I've never given birth like that b-before.</i>\"  She manages between gasps for air while rubbing her now normal-sized belly.\n\n\"<i>I mean!  I want to, it was just different, usually I... use my...</i>\"  She shyly gestures to her abdomen.  \"<i>You'll have to do that to me again sometime.</i>\"  You feel a maternal warmth radiating from Phylla.  It's clear to you she just enjoys being a mother; it doesn't really matter to her what her children look like.");
		//Subsequent Times:
	...
	//PC has more than 75 corruption:
		...
		pc.lust(45);
		processTime(25 + rand(10));
		if (pc.canOvipositSpider() && pc.eggs >= 10) {
	...
	IncrementFlag("COC.PHYLLA_TIMES_DRIDER_EGG_LAYED");
	flags["COC.PHYLLA_DRIDER_BABIES_COUNT"] = int(flags["COC.PHYLLA_DRIDER_BABIES_COUNT"]) + 5 + rand(4);
}
...
{
	clearOutput();
	output("No longer able to control yourself, you rip free of your [pc.gear] and charge up to Phylla's bed, catching her by surprise as she looks up at you.  With a seductive smile, you pin her down and slather your ovipositor against her fuck hole.");
...
{
	clearOutput();
	output("Working up all of your self control, you decide that Phylla could use the rest.  You wink at Phylla as you leave, telling her that you'll be back to fuck her brains out shortly... once she feeds your children.  She only musters the strength to smile and mutter something about motherhood but you're already halfway out the door.");
...
private function phyllaDigsForGems():void
{
	clearOutput();
	var kidsMod:int = 0;
	if (flags["COC.ANT_KIDS"] > 10) kidsMod++;
	if (flags["COC.ANT_KIDS"] > 50) kidsMod++;
	if (flags["COC.ANT_KIDS"] > 150) kidsMod++;
	if (flags["COC.ANT_KIDS"] > 300) kidsMod++;
	if (flags["COC.ANT_KIDS"] > 600) kidsMod++;
	if (flags["COC.ANT_KIDS"] > 1000) kidsMod++;
	if (flags["COC.ANT_KIDS"] > 2000) kidsMod++;
	//Success
	if (flags["COC.PHYLLA_GEMS_HUNTED_TODAY"] != days && rand(20) + kidsMod > 10) {
		var gems:int = 0;
		...
		//If Phylla IS NOT Laying Eggs
		if (int(flags["COC.PHYLLA_EGG_LAYING"]) == 0) {
			output("You ask Phylla is she's found any gems while digging out her colony.  She nods happily and runs over to a small stone chest and rifles though it.  After a moment, she runs back over to you and holds up all four of her hands.");
	...
	//Failure
		...
		output("You ask Phylla if she's found any more gems while digging.  \"<i>No, I'm sorry.  I haven't.  Not since the last time you asked.  I'll do better!  I mean, I know you like them.  They're rare... even down here.  I'll do better...</i>\"");
		//If corruption is less  than 50
	...
	flags["COC.PHYLLA_GEMS_HUNTED_TODAY"] = days;
...
private function eggDatBitch():void
{
	clearOutput();
	//PC is a Female/Herm Drider:
	if (pc.canOvipositSpider()) output("While Phylla appears to be sexually sated, the heaviness in your spider abdomen begs for release.\n\n");
	//First Time:
	if (int(flags["COC.TIMES_EGG_IMPREGNATING_PHYLLA"]) == 0) {
		output("Turning to Phylla, you tell her that you can help to make her wishes of being a brood-mother come true.  She looks at you a bit taken back, as if what you already did wasn't enough already.");
	...
	//Persuasion Success
		...
		if (flags["COC.PHYLLA_DRIDER_INCUBATION"] == undefined) flags["COC.PHYLLA_DRIDER_INCUBATION"] = 8 * 24 * 60; //Supposed to be eight days, not eight hours
		IncrementFlag("COC.TIMES_EGG_IMPREGNATING_PHYLLA");
		pc.orgasm();
		pc.dumpEggs();
	}
	processTime(25 + rand(10));
