import classes.Characters.CoC.CoCHelspawn;
...
//New Variable: " + flags["COC.HELSPAWN_NAME"] + ", the name of Hel’s daughter
...
public function championRef():String {
	if (int(flags["COC.HELSPAWN_DADDY"]) == 0) return pc.mf("Daddy", "Mom");
	else return pc.short;
}

public function helspawnFollower():Boolean {
	return flags["COC.HELSPAWN_AGE"] == 3;
}

public function helPregnant():Boolean {
	return flags["COC.HEL_PREGNANCY_INCUBATION"] != undefined;
}
...
internal function heliasAppearanceScreen():void {
	clearOutput();
	//spriteSelect(68);
	//output(images.showImage("monster-hel"));
	output("Hel the salamander stands seven feet tall, with pale skin and thick, bright-red scales covering her arms and legs, though she has a normal human torso and face.  A fiery tail swishes gaily behind her, blazing with a ");
	if (flags["COC.HEL_BONUS_POINTS"] < 150) output("bright orange glow");
	else output("bright white glow");
	output(" that lets off a pleasant heat, though it never seems to burn you.  Hel is wearing her scale bikini and a leather thong, and using her scimitar as a weapon");
	//if Isabella is cool: 
	if (flags["COCHEL_ISABELLA_THREESOME_ENABLED"] >= 1) output("; she’s got her blue bandanna wrapped around her head, mostly obscured by her fiery hair");
	output(".  She has a human face, with bright red eyes, gentle, feminine features and a smattering of pale scales on her cheeks, like freckles.  Hel has long, bright-red hair bound in a pony-tail that hangs down her back.  She has wide-flared hips and a soft, squishy butt.  Her two reptilian legs are visibly adorned with scales and claws, ending in soft, leathery soles.");
	if (flags["COC.HELSPAWN_NAME"] != undefined) {
		output("  A dark trio of scars run down Hel’s thighs, left by " + possessive(flags["COC.HELSPAWN_NAME"]) + " youthful claws.");
	}
	else {
		switch (helPregnancyEvent()) {
			case 1: //She's pregnant, but no special text yet
					break;
			case 2: output("  Hel's just starting to show a little bulge of pregnancy.");
					break;
			case 3: output("  Hel’s belly is starting to look fairly bloated, swelling with her child.");
					break;
			case 4: output("  Hel’s belly is positively gravid, full of a little salamander child.");
					break;
			default: //She's not pregnant, check if she's ready for it
					if (flags["COC.HEL_BONUS_POINTS"] >= 150) output("  Hel’s long, reptilian tail is currently burning white-hot, signaling her body’s ready for motherhood.");
		}
	}
...
//" + flags["COC.HELSPAWN_NAME"] + "’s Appearance Screen
private function helSpawnsAppearanceScreen():void {
	clearOutput();
	if (int(flags["COC.HELSPAWN_DADDY"]) == 0) output("Your ");
	else output("Hel’s ");
	output("salamander daughter, " + flags["COC.HELSPAWN_NAME"] + ", stands about six-foot-six tall, with pale skin and thick, ");
	if (int(flags["COC.HELSPAWN_DADDY"]) == 1) output("chitin-black");
	else output("bright-red");
	output(" scales covering her arms and legs, though she has a normal human face and torso.  A long leathery tail swishes happily behind her, surrounded by a dim fiery aura that emits a pleasant heat, though thankfully it isn’t hot to the touch, seemingly incapable of harming you.  " + flags["COC.HELSPAWN_NAME"] + " is wearing ");
	if (flags["COC.HELSPAWN_PERSONALITY"] >= 50) output("a scale bikini, just like her mother’s");
	else output("a short skirt, thigh-high boots, and a sky-blue blouse, in stark contrast to her mother’s sluttier attire");
	if (flags["COC.HEL_ISABELLA_THREESOME_ENABLED"] >= 1) output(", with a brightly-colored scarf wrapped around her neck");
	output(". She has a human face, with ");
	if (int(flags["COC.HELSPAWN_DADDY"]) == 0) output("golden");
	else if (int(flags["COC.HELSPAWN_DADDY"]) == 2) output("bright green");
	else output("crimson");
	output(" eyes, girlish features, and a smattering of pale red scales on her cheeks, like freckles.  Her short, dark-red hair cuts off just below her ");
	if (int(flags["COC.HELSPAWN_DADDY"]) == 2) output("puppy-dog");
	else output("reptilian");
	output(" ears, and it’s adorned with a pretty pink bow.  She has surprisingly large hips, and a big, squishy butt behind her.  Her reptilian legs are covered with thick, dark scales, ending in short claws atop soft, leathery soles.");
	output("\n\n" + flags["COC.HELSPAWN_NAME"] + " has a pair of perky, palmable C-cup breasts, each with a sensitive half-inch nipple at its tip.");
...
public function heliaBonusPointsAward():void {
	flags["COC.HELIA_KIDS_CHAT"] = 1;
	//spriteSelect(68);
	output("\n<b>That night...</b>\n");
	output("Your sleep is disturbed by a gentle tugging on your [pc.gear].  You groggily swat at whatever's nudging at you, groaning about the hour until your eyes finally open wide enough to recognize Helia kneeling over you, a deeply distraught look in her eyes.  \"<i>[pc.name]? [pc.name] wake up, I need to talk to you. Please?</i>\"");
	
	output("\n\nYou nod, rubbing the sleep out of your eyes as you stagger up onto your [pc.feet].  You note that Hel's wrapped her old cloak tightly around herself, held closed so that nothing but her face is visible.  You start to ask what's up, but Hel shushes you, saying she needs to talk to you privately.  ");
	//{If Hel's the only one at camp: 
	if (companionsCount() == 1) output("You cock an eyebrow, saying that you're already alone, but she shakes her head, saying she needs somewhere private, away from prying eyes.  ");
	output("You nod your assent, and let her help you up and lead you away from camp.  You follow Hel through the darkness, stumbling after her until you're hidden away beneath the old ruined wall a stone's throw from the perimeter.  Safely tucked away beneath the shadow of the wall, Hel takes your hands in hers and hesitantly, says, \"<i>Look, I don't... this isn't easy for me, okay?  But I have to tell you something, [pc.name].  And it's important, alright?  To me, anyway. I wouldn't bring it up if I could deal with it myself, honest.  But I can't and... and I'm afraid, [pc.name].</i>\"");
	...
	output("\n\n\"<i>I don't just like you, [pc.name] - I mean, I do.  Like you, I mean.  But it's... it's more than that, you know?  Sure, I've said the word, but I say 'love' to a lot of people, a lot of things.  I love your ");
	if (pc.hasCock()) output(pc.multiCockDescript());
	if (pc.hasVagina() && pc.hasCock()) output(" and your [pc.chest] and your [pc.vagina]");
	if (!pc.hasCock() && pc.hasVagina()) output(" [pc.vagina] and [pc.chest]");
	output(" and everything else about you.  But... but that doesn't mean anything.  It doesn't.  I say I love minotaur dicks, and centaurs, and those two fox pricks at the bar filling both my holes, and I love beer and fighting and ramming my tail up peoples' assholes.  But that's not real love, right?  Love is - oh, god, I'm making a mess of this.  Again. I keep doing this; it always works out so much better in my head.</i>\"");
	
	output("\n\n\"<i>I guess what I'm trying to say is... I love you, [pc.name].  I really, really do.  Not fake, shitty, stupid love; not me saying it in the heat of the moment.  I've been thinking about this for a while, now.  You've been so good to me [pc.name], better than I deserve.  You saved my family, you've given me a place to live, and more kindness than I could ever have imagined when I jumped you in the plains so very long ago.</i>\"");
	
	output("\n\n\"<i>That's not what I wanted to tell you, though.  I'd have sat on that for years if I could have, but something's changed.  I've talked to my father, and he says it's normal.  He just... just chuckled and shook his head, gave me this knowing look.  But I'm still afraid, [pc.name].  I can't deal with this on my own, but... but I have to know, before we go any further: do you love me, too?  And please, please don't just say yes because it's what I want to hear.  I promise I won't run off or throw a tantrum if you say no; I liked where we were before I opened my big stupid mouth.  So, what do you say, [pc.name]? Do you love me?</i>\"");
	clearMenu();
...
private function noYouDontLoveHeliaYouMonster():void {
	clearOutput();
	//spriteSelect(68);
	output("As much as you enjoy the sex, as much as you like Hel, you can't say you feel that way towards her.  You grasp her shoulders, telling her as gently as possible how you feel.  She nods slowly as you speak, rubbing at her eyes by the time you're finished.");
	output("\n\n\"<i>Well, thanks for being honest, [pc.name].  I... I really do appreciate it.  But, maybe you can still help me with my problem.  I'll understand if not, but still, I have to ask: ");
	//[HelLove to NO]
	flags["COC.HEL_LOVE"] = -1;
	heliaLoveFollowup();
}
...
private function yesHeliaILoveYourButtHoleReaming():void {
	clearOutput();
	//spriteSelect(68);
	output("Smiling, you tell her yes.  The salamander cries out with joy, jumping into your arms and kissing you, squeezing you so tightly that you feel like you're about to black out by the time she breaks the kiss, stepping back with chest heaving.");
	output("\n\n\"<i>Wh-why didn't you tell me before, anyway?</i>\" she asks, nuzzling her cheek into your [pc.chest].  \"<i>Could have saved me all this trouble.</i>\"");
	output("\n\nYou shrug, saying that you never really felt like you had the chance until now... and it's a lot cuter when she bumbles through a confession.  Helia gives you a gentle punch on the shoulder, but softens the blow with another series of kisses along your [pc.chest] and neck, holding you close as the two of you revel in the newly-revealed love you share.  After a few minutes, though, Hel steps back and says, \"<i>I love you, [pc.name], with all my heart.  You're the person I want to spend the rest of my life with, if you'll have me.  But I still have to tell you what's wrong. The reason I brought this all up: ");
	//[HelLove to YES]
	flags["COC.HEL_LOVE"] = 1;
	heliaLoveFollowup();
}
...
private function heliaLoveFollowup():void {
	//spriteSelect(68);
	output("You're not from here, so you probably don't know much about salamanders.  Basically, we're effectively infertile.  I can get filled with gallons of cum day in and day out, and chances are none of it will take inside me.  I've always thought I could slut around as much as I wanted, and I'd never have to worry about the consequences.  But... that's not forever, I guess.  When a salamander girl finds the person she wants to be with forever, ");
	//if PC is female/herm:
	if (pc.hasVagina()) output("even another woman, it seems, ");
	output("we... change.</i>\"");
	
	output("\n\nHel reaches up and undoes the clasp on her cloak.  When it falls away, your eyes are drawn immediately to her tail - once burning with a blood-red hellfire, her leathery tail is now surrounded by a pale radiance, shimmering like white gold.  Your eyes widen, but Hel flinches back as you try to approach.  \"<i>It's called the Pale Flame, [pc.name].  Just like it looks.  When a 'mander girl finds the person she wants to be with, our bodies change.  While I'm like this... I'm as fertile as a goblin, and... and I </i>want<i> to be.  I want... I need... a child, [pc.name].  I want to be pumped full of cum until I'm bloated, to bear a little salamander for us to raise and love.  ");
	//If HelLove = No:
	if (flags["COC.HEL_LOVE"] == -1) output("Even if you don't love me, you'd love our child, right?  You can fuck me full of kids, or even... even get someone else to, I don't care.  I just </i>need<i> it, [pc.name].  More than anything else in the whole wide world.");
	else {
		output("I love you, [pc.name].  I love you so much.  I want to share this with you.  ");
		if (pc.hasCock()) output("You'll fuck me full of kids, right?  Please?  Give me your seed, [pc.name].  Give it all to me until I'm fucking gravid with your salamander babies.");
		else output("Even if you can't fuck me full of babies... you could grow a dick!  There's plenty of things in this god-forsaken world that do that. O-or if you don't want to, I bet I can get one of the fox-girls, or a minotaur, or something.  You know I can find a dick somewhere, just say the word.");
	}
	output("  So what do you say, [pc.name]?  Let's have a kid!</i>\"");
	clearMenu();
	if (pc.hasCock() && pc.cockThatFits(heliaCapacity()) >= 0) addButton(0,"Have A Kid",haveAKid);
	else if (pc.hasCock()) output("  <b>Unfortunately, you're too big to squeeze inside Helia to do the business yourself.  You might need to shrink down some.</b>");
...
internal function haveAKid():void {
	...
	flags["COC.HEL_PREGNANCY_INCUBATION"] = 400 * 60 + timeAsStamp;
	output("You tell Hel that you're in this with her, that you want to give her that child she seems so desperate for.  She beams at you, smiling from eye to eye before leaping into your arms, pressing her lips hard to yours.  You kiss her back, wrapping your arms around her hips to support her as her powerful legs wrap around your waist; you push her up against the ruined wall, hands searching across her taut, hot flesh until you toss her bikini top aside, letting her hefty tits free.  \"<i>Oh god yes,</i>\" she moans as you trail kisses from her lips, down her neck to her stiffening nipple.  \"<i>I want this so much, more than anything.  Give it to me, [pc.name].  Don't hold back!</i>\"  Your fingers sink into her pliant flesh as you suckle on her exposed teat, groping her other tit and soft ass as she moans and squirms in your arms.  Clumsily, Hel's claws brush down your body, peeling off your [pc.gear] until your " + pc.cockDescript(0) + " flops into her lap.  She locks her scaled fingers around your manhood, roughly stroking you until you're stiff as diamonds in her grasp.");
	
	output("\n\nYou shudder as her fingers work your " + pc.cockDescript(0) + ", but don't let up on your end for a second.  You brush and knead Hel's nipple between your teeth, letting your hands drift down to her wide hips and gropable ass, slowly stripping her of her scale bottom and pulling it off her legs.  With your lover bare and naked, you slip down between her legs, letting her hook them over your shoulder to give you a good view of her dripping cunt.  Your tongue laps across her labia, drawing a long, lewd moan from Hel.  She runs her fingers through your [pc.hair], urging you onward; at her lusty moans, you dig in, sucking on her prominent clit and drilling your tongue between her inner folds.  You gasp into her when Hel's lengthy tail wraps around your shoulders, the pale flame soothingly warm on your " + pc.skinFurScales() + " as her leathery appendage works its way down to the " + pc.cockDescript(0) + " dangling between your [pc.legs].  You groan with sudden need as the tip of her tail brushes your most sensitive flesh, tickling ");
	if (pc.balls > 0) output("your [pc.balls]");
	else output("the base of your prick");
	output(" before coiling around the shaft with serpentine dexterity.  Her tail contracts and squeezes, undulating across your " + pc.cockDescript(0) + " until it finally massages a thick blob of pre from your " + pc.cockHead() + ", dribbling out to stain the wasteland ground.");
	
	output("\n\nSuddenly, Helia grabs your shoulders and shoves you down.  You flop onto your back, tail-encased prick standing straight up like a flagpole as Hel straddles your [pc.hips], vigorously fingering herself as her tail lines your cock up with her drooling womanhood.  Your breath catches as your " + pc.cockHead() + " brushes her boiling juices, slickening your entry as she slides down your pole.  You grab Hel's flared hips, guiding her in as she envelopes your " + pc.cockDescript(0) + " in the roiling inferno inside her.  Her tail unravels from around your throbbing shaft as she descends, tantalizingly slowly; she takes the better part of a minute before her groin is cozily joined to yours, her hungry maw kissing the very base of your lusty prick.");
...
private function HaveAHellKidPartII():void {
	...
	output("\n\n\"<i>That was... amazing.  God, it's like we were made for each other.  The way you move, the way you fill me and fuck me and caress me.  I'm... so glad I found you, [pc.name].  It seems like a lifetime ago, like I was another person back then.  Maybe I was, but... there's nowhere I'd rather be, nowhere on Mareth or beyond than with you, the one I love.  Oh god, I'm so in love with you, [pc.name].  So very much in love,</i>\" she says, running her fingers through your [pc.hair].  You lean up and kiss her, wrapping your arms around your lover as your lips press against hers, tasting the sweet touch of hers.");
	
	output("\n\n\"<i>Oh, shit,</i>\" Hel says, breaking the kiss and staring wide-eyed into the morning sky.");
	
	output("\n\n\"<i>What's wrong?</i>\"");
	
	output("\n\n\"<i>I-I really am going to be a mother.  I can feel it, [pc.name].  Your seed taking inside me, the dawn of a new life.  I just hope... I hope this wasn't a mistake, [pc.name].</i>\"");
	...
	output("\n\n\"<i>Yeah, I know.  Forget about it, I'm fine.  Really.  It's just that this happened all so suddenly, you know?  This isn't exactly where I saw myself a year ago.  Not that I'm complaining,</i>\" she adds, turning back to you with another kiss, squeezing your shoulder.  \"<i>I just hope I'm ready, I guess.  Until the Pale Flame took, I'd never really thought about being a mother.  Never really wanted it.  But last night, it was like I was possessed, like I couldn't control myself.  I just... needed you.  And there you were, like always, ready to help.  I love you with all my heart, [pc.name]");
	if (flags["COC.HEL_LOVE"] == 1) output(", and I'm so very, very glad you love me back.");
	else output(", even if you can't say the same.  And that's alright; it doesn't change my heart, and it never will. I love you");
	output(".</i>\"");
	
	output("\n\nThe two of you snuggle up for a good long while, letting the edge of your sexual exhaustion subside as you watch the sunrise, content to lie in each others' arms.  After a blissful eternity, though, you both rise and collect the scattered bits of Hel's scale mail and your [pc.gear], laughing and teasing as you redress, exchanging the occasional kiss or caress until the pair of you are clothed, your weapons secured.  Finally, Helia slips into your arms, planting a long kiss upon your lips.  \"<i>I'm glad we're in this together, my love.  I'm going to... to need you during this.</i>\"");
	
	output("\n\nYou tell her to rest easy: that you'll be there for her every step of the way.  She smiles ");
	if (pc.tallness >= 90) output("up ");
	else if (pc.tallness <= 72) output("down ");
	output("at you, holding you tight for a long moment before stepping away, still holding your hands.  \"<i>This is all new to me, [pc.name].  I never pictured myself as a mother, with a mate and a stable, safe home - or as stable and safe as anything these days - but with you by my side, there's nothing I can't take on.</i>\"");
...
private function getAnotherDad():void {
	...
	if (!pc.hasCock()) {
		output("You tell Helia you'd love to a share a child with her, but you're not... properly equipped for the endeavor.  \"<i>That's fine!  I can... I can wait, a little.  I-if you want to go grow one, I mean.  If not, then we can find someone with a cock.  ");
		if (flags["COC.TEL_ADRE_KNOWN"] >= 1) output("There's Miko and Mai from the bar.  Mai's said she wanted a kid, but can't take care of one... she'd probably be willing to fuck one into me!  If that's not alright, then... lemme think.  ");
		output("Uh, maybe not a minotaur... they always plug more minotaurs, and I don't want a bull coming out of my twat.  Uh, maybe I could track down one of the spider boys from the swamp and jump on </i>his<i> dick.  They're pretty cute, right?  Dunno how that'd affect a child, though.  Maybe he'd end up with like, extra eyes, or chitin?  Still, better than an imp or some shit.  So what do you think?  Wanna grow a dick, or leave the knocking-up to someone else?</i>\"");
		//{Options in next section}
	}
	//[Another Dad] (PC has a dick)
	else {
		output("\"<i>W-wha?</i>\" Hel says, cocking an eyebrow.  \"<i>You sure, [pc.name]?  I-if you don't want to be the dad, I guess I understand.  But why?  Am I not... no, nevermind.  I don't want to know.  Okay, uh, we can find someone with a cock.  ");
		if (flags["COC.TEL_ADRE_KNOWN"] >= 1) output("There's Miko and Mai from the bar.  Mai's said she wanted a kid, but can't take care of one... she'd probably be willing to fuck one into me!  If that's not alright, then... lemme think.  ");
		output("Uh, maybe not a minotaur... they always plug more minotaurs, and I don't want a bull coming out of my twat.  Uh, maybe I could track down one of the spider boys from the swamp and jump on </i>his<i> dick.  They're pretty cute, right?  Dunno how that'd affect a child, though.  Maybe he'd end up with like, extra eyes, or chitin?  Still, better than an imp or some shit.  So what do you think?</i>\"");
	}
	clearMenu();
	//{If Tel'Adre has been discovered: [Mai]}
	if (flags["COC.TEL_ADRE_KNOWN"] >= 1) addButton(0, "Mai", maiWouldBeTheBestInseminator);
	//[Spiderboy]
	addButton(1, "Spiderboy", spiderboyWouldBeBestDad);
	//[I will] (If PC has a dick)
	if (pc.hasCock() && pc.cockThatFits(heliaCapacity()) >= 0) addButton(2, "I Will", haveAKid);
	else if (!pc.hasCock()) addButton(2, "I Will", growingDicks4Hel);
	addButton(3, "Not Now", noKidsHel);
}
...
private function maiWouldBeTheBestInseminator():void {
	clearOutput();
	//spriteSelect(68);
	output("You tell Hel that you think Mai would make a lovely father.  Helia nods her agreement, saying, \"<i>Yeah, I agree.  She's a beauty, and I'm sure our child will be stunning... you wouldn't mind if she visited, right?  I mean, you and I will be raising our kid - and he'll be ours for sure - but I'm sure Mai will want to at least visit her kid.</i>\"");
	...
	flags["COC.HEL_PREGNANCY_INCUBATION"] = 400 * 60 + timeAsStamp;
	flags["COC.HEL_NTR_TRACKER"] = 1;
	flags["COC.HELSPAWN_DADDY"] = 2;
	addNextButton();
}
...
private function spiderboyWouldBeBestDad():void {
	clearOutput();
	//spriteSelect(68);
	output("You tell Helia to go find a spider boy to jump.  She beams at you, and skips off toward the swamp calling, \"<i>Thank you, thank you thank you, [pc.name]!</i>\" over her shoulder as she goes.  You suppose the next time you see her, Hel's probably going to be pregnant with the child you'll be helping to raise.");
	flags["COC.HEL_PREGNANCY_INCUBATION"] = 400 * 60 + timeAsStamp;
	flags["COC.HEL_NTR_TRACKER"] = 1;
	flags["COC.HELSPAWN_DADDY"] = 1;
	addNextButton();
}
...
private function growingDicks4Hel():void {
	clearOutput();
	//spriteSelect(68);
	output("You tell Hel to give you a little while, that you'll go find something to grow a cock for her.");
	output("\n\n\"<i>Thank you!  Thank you so much, [pc.name].  I really, really wanted for it to be our child. I'll wait, but hurry.  I don't know how much longer I can stand this... this need!</i>\"");
...
private function noKidsHel():void {
	clearOutput();
	//spriteSelect(68);
	output("You shake you head and say no, you don't want to have children.  Not right now, anyway.  ");
	if (pc.hasCock() && pc.cockThatFits(heliaCapacity()) >= 0) output("You couldn't if you wanted to anyway - you're too big to fit.  ");
	output("As the words leave your lips, you can see Hel's shoulders slump, a crestfallen look spreading across her face.  \"<i>A-are you sure?  Please, [pc.name], I really, really want a child.  For </i>us<i> to have one.</i>\"");
	output("\n\nYou tell her that no, at least for now you aren't interested.  She begs and pleads for several minutes, but you hold your ground.  Finally, she relents.  \"<i>Alright, [pc.name].  I... I'll respect that, I guess.  Dad gave me some herbs, said as long as I take them, I should go back to normal.  If that's what you want, I'll start on them.  Just tell me if - when - you're ready, [pc.name].  I will be.</i>\"");
...
public function helGotKnockedUp():void {
	...
	if (int(flags["COC.HELSPAWN_DADDY"]) == 2) output("warm ");
	else output("wan ");
	output("smile as you approach, scooting over to allow you to sit beside her.  She almost seems distant, though she's quick to wrap her tail around your waist, hugging you close against her.");
...
private function sureHelGimmeSpidahBoyDetails():void {
	...
	if (pc.biggestCockVolume() >= cockVolume(20)) output(", though still nothing compared to you, lover mine");
...
private function dontTellMeAboutSpiderboy():void {
	clearOutput();
	//spriteSelect(68);
	output("\"<i>So, let's just say I came away from that encounter carrying enough cute spiderboy cum to fill a bucket.  If I'm not carrying his child now, then I'm hopeless.  But... you know, I think I can feel it, [pc.name].  I shouldn't be able to, but I can.  Oh, god,</i>\" Hel says, looking pointedly away from you, staring wide-eyed into the morning sky.");
	
	output("\n\n\"<i>What's wrong?</i>\"");
	
	output("\n\n\"<i>I-I really am going to be a mother.  I can feel it, [pc.name].  The spiderboy's seed taking inside me, the dawn of a new life.  I just hope... I hope this wasn't a mistake, [pc.name].</i>\"");
	
	output("\n\nYou put an arm around the mother-to-be's shoulder, saying, \"<i>I thought this was what you wanted, Hel.  You were-</i>\"");
	
	output("\n\n\"<i>Yeah, I know.  Forget about it, I'm fine.  Really.  It's just that this happened all so suddenly, you know?  This isn't exactly where I saw myself a year ago.  Not that I'm complaining,</i>\" she adds, turning back to you with a kiss, squeezing you with her tail.  \"<i>I just hope I'm ready, I guess.  Until the Pale Flame took, I'd never really thought about being a mother.  Never really wanted it.  But last night, it was like I was possessed, like I couldn't control myself.  I just... needed you.  Well, maybe not your sperm - not literally, I mean - but it was like my body screaming 'You've found [him], [he]'s the one, this is a keeper!  Time to propagate!' I-I can't help but feel like this wasn't entirely my choice, you know?  Like I was acting on instinct, like an animal, not a person.  But at the same time, if I've made a mistake, then I have you here with me, to help me like you always do.  I love you with all my heart, [pc.name]");
	if (flags["COC.HEL_LOVE"] == 1) output(", and I'm so very, very glad you love me back.");
	else output(", even if you can't say the same.  And that's alright; it doesn't change my heart, and it never will. I love you.");
	output("</i>\"");
...
private function dontTellMeAboutMai():void {
	clearOutput();
	//spriteSelect(68);
	output("\"<i>So, let's just say I came away from that encounter carrying enough foxy herm cum to fill a bucket.  If I'm not carrying her child now, then I'm hopeless.  But... you know, I think I can feel it, [pc.name].  I shouldn't be able to, but I can.  Oh, god,</i>\" Hel says, looking pointedly away from you, staring wide-eyed into the morning sky.");
	
	output("\n\n\"<i>What's wrong?</i>\"");
	
	output("\n\n\"<i>I-I really am going to be a mother.  I can feel it, [pc.name].  Mai's seed taking inside me, the dawn of a new life.  I just hope... I hope this wasn't a mistake, [pc.name].</i>\"");
	
	output("\n\nYou put an arm around the mother-to-be's shoulder, saying, \"<i>I thought this was what you wanted, Hel.  You were-</i>\"");
	
	output("\n\n\"<i>Yeah, I know.  Forget about it, I'm fine.  Really.  It's just that this happened all so suddenly, you know?  This isn't exactly where I saw myself a year ago.  Not that I'm complaining,</i>\" she adds, turning back to you with a kiss, squeezing you with her tail.  \"<i>I just hope I'm ready, I guess.  Until the Pale Flame took, I'd never really thought about being a mother.  Never really wanted it.  But last night, it was like I was possessed, like I couldn't control myself.  I just... needed you.  Well, maybe not your sperm - not literally, I mean - but it was like my body screaming 'You've found " + pc.mf("him","her") + ", " + pc.mf("he","she") + "'s the one, this is a keeper!  Time to propagate!'  I-I can't help but feel like this wasn't entirely my choice, you know? Like I was acting on instinct, like an animal, not a person.  But at the same time, if I've made a mistake, then I have you here with me, to help me like you always do.  I love you with all my heart, [pc.name]");
	//if HelLove is Yes:
	if (flags["COC.HEL_LOVE"] == 1) output(", and I'm so very, very glad you love me back.");
	else output(", even if you can't say the same. And that's alright; it doesn't change my heart, and it never will. I love you.");
	output("</i>\"");
...
public function bulgyCampNotice():void {
	clearOutput();
	//spriteSelect(68);
	output("As you're walking through camp, your eyes wander over toward Helia, sunning herself on a stone near the edge of camp.  You can just see that her belly's starting to bulge out from under her, and Hel's hands lie protectively over her full womb, absently rubbing the bulge of her stomach.");
	addNextButton();
}
...
public function heliaSwollenNotice():void {
	clearOutput();
	//spriteSelect(68);
	output("You note that Hel's wandering aimlessly around camp, one hand over her belly as she mumbles to herself.  You could swear she was cursing her now quite swollen belly, but suddenly she gives a girlishly happy cry and waves you over.\n");
	output("\n\"<i>Come feel, [pc.name], quick! It's kicking!</i>\"\n");
	...
	output("\n\n\"<i>Feels like she's gonna kick her way out of there sometimes,</i>\" Hel says, chuckling.  You help her sit down, both of you running your hands along the surface of her belly, responding to the little pushes her child's making.  \"<i>She's going to be a fighter, [pc.name], let me tell you.</i>\"");
...
private function youWantABoy():void {
	clearOutput();
	//spriteSelect(68);
	output("\"<i>Is that so? Yeah, I can see it.  ");
	if (pc.hasCock() && !pc.hasVagina()) output("If I were a guy, I'd want a big strong son to hang out with, too.  Take him fishing, teach him how to fight the way you do... you'll make a great dad, my love.  I'm sure you will.");
	else output("Would be nice to have a man around here, you know?  I miss hanging around the boys back home, watching 'em strut like peacocks for every passing girl.");
	output(" And any son of mine is going to be a real lady killer, mark my words.  We're going to have to fight off whole hordes of goblin sluts, all looking for a piece of our handsome little boy before you know it.</i>\"");
...
private function youWantAGirl():void {
	clearOutput();
	//spriteSelect(68);
	output("\"<i>Yeah, a girl would be pretty great.  ");
	//if PC is male:
	if (pc.hasCock() && !pc.hasVagina()) {
		output("I dunno if you had any sisters growing up, [pc.name], but let me warn you: a little girl, especially a little salamandress, is going to be a hell of a handful.  But I can just see you when she's all grown up, packing a big old sword and threatening every boy that wants a piece of her: 'Treat her right or you'll have the CHAMPION to deal with.  Rawr.'");
	}
	else output("We're going to be a gaggle of tittering girls before you know it, though.  Salamanders grow up so fast, [pc.name]...  I just hope she doesn't turn out like me, you know?  I don't know if I could stomach seeing my little girl becoming a wanton slut like her mom.  I might get jealous, after all...");
	output("  Any daughter of ours is going to be a real beauty, mark my words. She'll make a succubus look like a toad before she's out of swaddling.</i>\"");
...
internal function heliaTalkSeven():void {
	clearOutput();
	//spriteSelect(68);
	flags["COC.HELIA_TALK_SEVEN"] = 1;
	output("You take Hel with you to a secluded spot at the edge of camp and sit with her, asking how she's doing as you run a hand across her pregnant belly.  She nuzzles against you, wrapping her lengthy tail around your waist as her cheek nestles into the nape of your neck.  \"<i>It's... not like I expected, [pc.name].  Whenever I used to think of being a mother, I always sort of skipped the whole 'carry the little bastards around in your belly' part.  I just feel so... full, I guess?  Like I just ate a horse, and the horse is inviting its friends to come party at night whenever I try to sleep.  Heh, this little firebrand just loves to kick and squirm at night, let me tell you.  Speaking of which-</i>\"  Hel takes your hand in hers and places it over her belly, letting you feel the subtle little movements inside her as your growing child pushes out toward the world.");
...
private function encouragePregalia():void {
	clearOutput();
	//spriteSelect(68);
	output("You squeeze Hel's shoulder and tell her that she's going to be a great mother, and that her child would do well to learn from a beautiful, powerful warrior like dear old mom.  ");
	if (flags["COC.HEL_LOVE"] == 1) output("You love Hel");
	else output("Hel's your friend");
	output(", and if her child follows in her footsteps, so much the better.  Your lover manages a half-hearted laugh, blushing at your words.  Her tail tip brushes your cheek as its owner cuddles up against you, smiling.  \"<i>Thanks, lover mine.  I just hope I - we - can do right by the kid.  That's all.</i>\"");
...
private function helsLifestyle():void {
	clearOutput();
	//spriteSelect(68);
	output("Being very serious for a moment, you tell Hel that if she's worried about her terrible choices rubbing off on her kid, maybe she ought to reconsider some of the things she's done: be less of a slut, drink less, be less of a bloodthirsty maniac in battle.  Slowly, Hel nods to the rhythm of your words.  \"<i>Yeah, maybe... maybe I should.  I don't know if I can change though, [pc.name].  I'm not as young as I was - or at least, I don't feel it - and I've been living the way I have been for years.  I don't know if I could just give up the shit I do: the partying and the drinking, the fighting and the fucking.  It's part of who I am, ");
	if (flags["COC.HEL_LOVE"] == 1) output("and I hope that's part of why you love me");
	else output("even though I know that's probably part of why you can't love me back");
	output(".  I'll try and rein it in, but maybe... it'd be better if you were the one who raised my child, [pc.name].  God knows I'm the least qualified person to do it.</i>\"");
...
private function nameDatHelspawn():void {
	clearOutput();
	//spriteSelect(68);
	output("You and Helia share a laugh, looking down at ");
	if (int(flags["COC.HELSPAWN_DADDY"]) == 0) output("your ");
	else output("her ");
	output("little girl as she suckles.  After a few happy, blissful minutes though, Hel turns to you with a question:");
	output("\n\n\"<i>So... what do we name her, love?  I honestly hadn't put that much thought into girl names.  Kind of expected a boy, I guess, but... it's up to you, [pc.name].  What do you think?</i>\"");
...
private function applyHelspawnName():void {
	...
	if (InCollection(this.userInterface.textInput.text, "", "Hellgirl", "Kid", "Phoenix", "Savin", "Helia", "Mini-doofus"))
	{
		clearOutput();
		if (this.userInterface.textInput.text == "") output("<b>You must select a name.</b>");
		else if (this.userInterface.textInput.text == "Hellgirl") output("\"<i>Hey, that's copyright infringement.  Probably.</i>\"");
		else if (this.userInterface.textInput.text == "Kid") output("\"<i>Wow, what are you, five?</i>\"");
		else if (this.userInterface.textInput.text == "Phoenix") output("\"<i>Oh hell no.  You're not naming my little girl after one of those... things!  Sorry Kiri, but still!</i>\"");
		else if (this.userInterface.textInput.text == "Savin") output("\"<i>That's a boy's name, dumbass.</i>\"");
		else if (this.userInterface.textInput.text == "Helia") output("\"<i>My favorite name!  Except it's kinda taken, love.  Don't want things to get too confusing around here, do you?</i>\"");
		else if (this.userInterface.textInput.text == "Mini-doofus") output("\"<i>Oh yeah, Kiha'll get a laugh out of that.  You ass.</i>\"");
		output("<b>You must give her a name.</b>");
		this.displayInput();
		this.userInterface.textInput.text = "";
		clearMenu();
		addButton(0, "Next", applyHelspawnName);
	...
	flags["COC.HELSPAWN_NAME"] = StringUtil.capitalize(this.userInterface.textInput.text);
	this.removeInput();
	
	//[Name Field]
	//Easter Egg Names:
	clearOutput();
	if (flags["COC.HELSPAWN_NAME"] == "Helspawn") output("\"<i>That's what I was gonna call her!  Except not at all.  God dammit, seriously, [pc.name]?</i>\"\n\n");
	if (flags["COC.HELSPAWN_NAME"] == "Jayne") output("\"<i>That sounds like a hero's name... I like it.  A name that demands respect!</i>\"\n\n");
	if (flags["COC.HELSPAWN_NAME"] == "Hesper") output("\"<i>Ah, the Evening Star.  She'll be the star of my life, alright...</i>\"\n\n");
	if (flags["COC.HELSPAWN_NAME"] == "Kiri") output("\"<i>Aw, that's great, lover.  Sis'll shit her pants when she hears her niece is named after her!  Gonna get confusing, though...</i>\"\n\n");
	if (flags["COC.HELSPAWN_NAME"] == "Mai") output("\"<i>Aw, that's great, lover.  Mai'll be honored, I know it.  And it is a great name...</i>\"\n\n");
	if (flags["COC.HELSPAWN_NAME"] == "Tanis") output("\<i>I don't... I don't know if I want her to have that name, lover.  It's got a lot of baggage, but... but it does my mother an honor.  More than I could ever give her.</i>\"\n\n");
	//Syn: 
	if (flags["COC.HELSPAWN_NAME"] == "Syn") output("\"Awesome.  I was hoping you'd choose that one, lover mine.</i>\"\n\n");
	if (flags["COC.HELSPAWN_NAME"] == "Chara") output("\"<i>Awesome.  You liked my ideas, after all!</i>\"\n\n");
	
	output("\"<i>So... " + flags["COC.HELSPAWN_NAME"] + " it is.  That's a good name, my love.  A strong name.  ");
	if (int(flags["COC.HELSPAWN_DADDY"]) == 0) output("Our");
	else output("my");
	output(" little girl's going to grow up big and strong with a name like that... and with you around to guide her,</i>\" Hel says, kissing your cheek.  \"<i>Why don't you give me a minute to catch my breath and get the little one situated, and let's talk.  Alright?</i>\"");
	
	output("\n\nYou nod and help Helia to her feet, still holding " + flags["COC.HELSPAWN_NAME"] + " to her breast.  Your lover gives you a wink before walking bow-legged back toward her part of camp, and the little crib she's built beside her hammock.");
	helSpawnsSetup();
	addNextButton();
}
...
public function helSpawnsSetup():void {
	...
	flags["COC.HELSPAWN_PERSONALITY"] = 50;
	if (int(flags["COC.HELSPAWN_DADDY"]) == 2) flags["COC.HELSPAWN_PERSONALITY"] += 10;
	//>If Spiderbro is daddy: +10 HelspawnChaste
	if (int(flags["COC.HELSPAWN_DADDY"]) == 1) flags["COC.HELSPAWN_PERSONALITY"] -= 10;
	if (int(flags["COC.HELSPAWN_DADDY"]) == 0) {
		//>If Corrupt (66+ Corr) PC is daddy: +10 Helspawn Slutty
		if (pc.cor() >= 66) flags["COC.HELSPAWN_PERSONALITY"] += 10;
		//>If Pure (33> Corr) PC is daddy: +10 Helspawn Chaste
		if (pc.cor() <= 33) flags["COC.HELSPAWN_PERSONALITY"] -= 10;
	}
	flags["COC.HELSPAWN_AGE"] = 1;
	flags["COC.HELSPAWN_GROWUP_COUNTER"] = 1;
	flags["COC.HEL_PREGNANCY_INCUBATION"] = undefined; //Clear Pregnancy
...
internal function heliaTalkEight():void {
	clearOutput();
	flags["COC.HEL_TALK_EIGHT"] = 1;
	//spriteSelect(68);
	output("Watching your little daughter crawl around, you and Hel sit down together arm in arm, smiling as the little salamander girl plays with bits and pieces of Hel's gear.");
	...
	output(" little girl's going to be a big girl damn soon.  Let's enjoy it while we can, eh?</i>\"");
	
	output("\n\nYou nod to your lover, and the both of you walk over to spend some quality time playing with " + flags["COC.HELSPAWN_NAME"] + ".");
	addNextButton(returnToCampUseOneHour);
}
...
internal function playWithYourKid():void {
	clearOutput();
	//spriteSelect(68);
	output("Picking " + flags["COC.HELSPAWN_NAME"] + " up off the ground, you spin the giggling little girl around, laughing as your ");
	if (int(flags["COC.HELSPAWN_DADDY"]) != 0) output("ward ");
	else output("daughter ");
	output("cries out in surprise and glee.  Soon, you're bouncing her, then down on the ground crawling around with her, playing peek-a-boo and a dozen other games as you and " + flags["COC.HELSPAWN_NAME"] + " laugh and hug.  Helia herself slips down with you before you know it, holding her daughter close as she tries to communicate, teaching the ever-growing girl a few words at a time.");
	
	output("\n\n\"<i>Mommy!</i>\" " + flags["COC.HELSPAWN_NAME"] + " eventually cries, bopping her mother on the nose. Hel laughs, but it turns into a sigh as her daughter reaches down and fiddles with her top, obviously hungry.");
	
	output("\n\n\"<i>We've got to wean this one before she bites my tits off,</i>\" Hel groans as " + flags["COC.HELSPAWN_NAME"] + " latches on, starting to suckle.  \"<i>Those teeth came in fast...</i>\"");
	
	output("\n\nLaughing, you rustle " + possessive(flags["COC.HELSPAWN_NAME"]) + " fiery hair and leave mother and daughter to finish the meal.");
	addNextButton(returnToCampUseOneHour);
}
...
public function helSpawnGraduation():void {
	...
	//spriteSelect(68);
	flags["COC.HELSPAWN_GROWUP_COUNTER"] = 1;
	flags["COC.HELSPAWN_AGE"]++;
	output("You wake up to a scream, high-pitched and terrified.  You bolt up out of bed, tangling up in your bedroll for a minute before stumbling over to Hel and... " + flags["COC.HELSPAWN_NAME"] + "?");
	output("\n\nYou do a double take as you lay eyes on the ");
	if (int(flags["COC.HELSPAWN_DADDY"]) == 1) output("black");
	else output("red");
	output("-scaled figure curled up next to Helia; Hel herself is looking wide-eyed down at the drowsy figure on her breast, just barely opening her eyes.  \"<i>Y-you're... you're huge!</i>\" Hel manages to gasp out, pushing herself back and away from her daughter.");
	...
	output("\n\n\"<i>Ugh, what's wrong?</i>\" " + flags["COC.HELSPAWN_NAME"] + " groans, looking up at Hel, then to you.  \"<i>" + championRef() + ", why's mom screaming?</i>\"");
	...
	output("\n\nYou have to admit, the change is sudden and drastic... but the girl's scales, her eyes, they're certainly " + possessive(flags["COC.HELSPAWN_NAME"]) + ".  ");
	if (int(flags["COC.HELSPAWN_DADDY"]) == 2) output("And the puppy ears, now more wolf-like, are a dead giveaway.  Utterly unique to her.  ");
	output("You sit down beside the obviously confused girl and stroke her hair, telling her it's all right, her mother's just startled, that's all.");
	
	output("\n\n\"<i>It's just me, mom,</i>\" " + flags["COC.HELSPAWN_NAME"] + " says, sliding over to sit beside her mother, who's still wide-eyed with shock.  Your girl tries for a few moments to talk to Hel, but seeing that venture prove fruitless - Hel seems utterly insensate, paralyzed by shock - she instead curls back up beside her mother, head rested on Helia's shoulder.  Eventually, warily, Hel reaches out a trembling hand to brush her daughter's cheek.");
...
public function helspawnDiscoversBooze():void {
	clearOutput();
	//spriteSelect(68);
	flags["COC.HELSPAWN_DISCOVER_BOOZE"] = 1;
	output("As you're moving through camp, you notice Hel's away from her little nook - probably off visiting the family, seeing as her cloak's gone.  You wouldn't have given her part of camp a second look, except for a sudden whiff of brimstone and burning brewery on the wind.  Cocking an eyebrow, you creep over to Hel's still, quiet as ");
	if (pc.felineScore() < 5) output("a cat");
	else output("the cat you are");
	output(", and poke your head around the iron body of the still.");
	
	output("\n\nFlopped down on the ground with her mouth right under the spigot is " + flags["COC.HELSPAWN_NAME"] + ", using her tail to keep the tap open and flowing into her waiting maw as she lazes beneath it.  When you loudly clear your throat, she flails around a moment, letting the tap go as she scrambles to her feet - only to fall drunkenly on her ass.  \"<i>Uh...</i>\" she groans, wiping the booze off her cheeks as she hiccups drunkenly.  \"<i>Hi there, " + championRef() + ".</i>\"");
	clearMenu();
...
private function scoldHelSpawn():void {
	clearOutput();
	//spriteSelect(68);
	output("You cross your arms over your chest and ask your little salamander exactly what she thinks she's doing.");
	...
	output("\n\n\"<i>Alright, " + championRef() + ",</i>\" " + flags["COC.HELSPAWN_NAME"] + " says, curling up beside you, head resting on your [pc.chest].");
	//{HelspawnChaste +10}
	flags["COC.HELSPAWN_PERSONALITY"] -= 10;
	addNextButton(returnToCampUseOneHour);
}
...
private function encourageHelspawn():void {
	clearOutput();
	//spriteSelect(68);
	output("\"<i>Move over, you,</i>\" you say, sliding down beside your little salamander as you pop the tap open again.  \"<i>Gotta pace yourself, see,</i>\" you add, taking a long draught before closing it off again, savoring the potent taste of Hel's mighty brew.");
	output("\n\n\"<i>Wha...?</i>\" " + flags["COC.HELSPAWN_NAME"] + " starts to say, before catching on as you drink, inviting her to join you.");
	...
	output("\n\n\"<i>Uh...</i>\" you groan, slumping over onto your [pc.ass].  " + possessive(flags["COC.HELSPAWN_NAME"]) + " right behind you, collapsing on top of you in a pile of ");
	if (flags["COC.HELSPAWN_DADDY"] != 1) output("red ");
	else output("black ");
	output("scales and the smell of a burning brewery.");
	...
	output("\n\n\"<i>I saved some for ya, mom,</i>\" " + flags["COC.HELSPAWN_NAME"] + " groans, retching.");
	
	output("\n\n\"<i>Oh. Oh that's just great, [pc.name].  Good job.</i>\"");
...
public function helSpawnChoosesAFightingStyle():void {
	clearOutput();
	//spriteSelect(68);
	output("As you make your way back to camp, you begin to hear the sounds of fighting from ahead.  Readying your [pc.mainWeapon], you pick up the pace until your campsite comes into view.  You relax, seeing that it's only " + flags["COC.HELSPAWN_NAME"] + " and her mother.  Helia's standing near the center of camp, ");
	if (companionsCount() > 3) output("apparently having cleared out most of your other followers for the moment, ");
	output("her sword held at a guard pose between herself and a training dummy. " + flags["COC.HELSPAWN_NAME"] + " stands a few feet behind her mother, watching intently as Hel paces around the dummy, breathing hard.");
...
private function snipermanders():void {
	clearOutput();
	//spriteSelect(68);
	flags["COC.HELSPAWN_WEAPON"] = "bow";
	output("You tell Hel to take a break: you've got this handled. Grabbing your bow and quiver from over your shoulder, you hand them over to ");
	if (int(flags["COC.HELSPAWN_DADDY"]) == 0) output("your");
	else output("Hel's");
	output(" eager daughter. " + flags["COC.HELSPAWN_NAME"] + " takes them up with surprising reverence, holding them as gingerly as glass.  Grinning, you clasp her shoulder and tell her how to put the quiver on, and get her stance right for shooting.  It takes her a moment to get set up: her scaled, reptilian legs don't naturally stand in the shooting pose you've adopted");
	if (pc.hasLegFlag(GLOBAL.FLAG_DIGITIGRADE)) output(", even with your own digitigrade legs");
	output(", and you find yourself having to correct her grip on the haft several times.  But, finally, you get her ready to shoot.");
	
	output("\n\nYou slip behind " + flags["COC.HELSPAWN_NAME"] + ", putting your arms on hers as you guide her through nocking an arrow, head resting on her fist, a lone finger outstretched toward the dessicated dummy.");
	...
	//If Rath is @ camp:
	if (flags["COC.RATHAZUL_IN_CAMP"] == 1) output("  You hear your old alchemist friend suddenly putting up a ruckus as the sounds of breaking glass echo throughout camp.  " + flags["COC.HELSPAWN_NAME"] + " looks up at you nervously, but you ruffle her fiery hair and tell her to try again. Rath is probably just fine.");
	//else if Valeria/Latexy is at camp: 
	else if (flags["COC.VALARIA_AT_CAMP"] == 1) output("  You hear a sudden yelp of pain from across camp. Valeria slithers up to you with an irritated look on her gooey face, pointing an accusing finger at an arrow sticking out of her tit.  \"<i>Dammit, [pc.name], I'm a googirl, not a pin cushion!</i>\"  You wave her off, and tell " + flags["COC.HELSPAWN_NAME"] + " to try again.");
	//else if (latexGooFollower()) output("  You hear a sudden yelp of pain from across camp.  " + flags["COC.GOO_NAME"] + " slithers up to you with an irritated look on her gooey face, pointing an accusing finger at an arrow sticking out of her tit.  \"<i>Dammit, [pc.name], I'm a googirl, not a pin cushion!</i>\"  You wave her off, and tell " + flags["COC.HELSPAWN_NAME"] + " to try again.");
	
	output("\n\n\"<i>I-I dunno, " + championRef() + ",</i>\" " + flags["COC.HELSPAWN_NAME"] + " whines, biting her lip as she traces the arrow's path with her eyes.  \"<i>Maybe mom was right. I should just stick to a sword. That's easy....</i>\"");
	
	output("\n\nYou sigh and tell her to try again.  She can't expect to be perfect on the first try, after all.  It takes a little convincing, but soon you've got " + flags["COC.HELSPAWN_NAME"] + " back into form and nocking an arrow.  You put your hands on hers, helping the young salamander take aim, drawing down on her straw-stuffed target.  Barely audible over " + possessive(flags["COC.HELSPAWN_NAME"]) + " nervous breathing, you whisper, \"<i>Take it easy.  Relax.... Good.  Take aim... now breathe out.  Exhale and loose.</i>\"");
	
	output("\n\nAgain, " + flags["COC.HELSPAWN_NAME"] + " lets an arrow fly - and this one strikes true.  You watch the missile streak across camp, slamming into the training dummy's crotch with deadly force.  It sways, buckles, and falls to pieces.");
	
	output("\n\n\"<i>Ouch,</i>\" Hel laughs, sipping on her flask behind you.  \"<i>What'd that poor dummy ever do to you?</i>\"");
	
	output("\n\n\"<i>I did it!</i>\" " + flags["COC.HELSPAWN_NAME"] + " shouts, pumping her fist in the air. Her mother reaches over to ruffle her fiery hair, and you both shower her with congratulations.");

	output("\n\nBefore she can get cocky, though, you push the bow back into her hands.  \"<i>Lesson 2, kiddo: one shot is never enough.</i>\"");
	
	output("\n\nBy the time " + flags["COC.HELSPAWN_NAME"] + " has her shooting stance back, her mother's already set the dummy back up, ready for round two.  It's several hours long before the three of you quit: time spent drilling " + flags["COC.HELSPAWN_NAME"] + " on stance and aim, even going a round with her and a pair of daggers to remind her that enemies will eventually close in if she's not careful.  She learns quickly, though, and soon she hardly needs your help to plant an arrow in the target's chest.");
	
	output("\n\nA few hours like this every night, and ");
	if (int(flags["COC.HELSPAWN_DADDY"]) == 0) output("your ");
	else output("Hel's ");
	output("daughter will be a warrior worthy of her parent");
	if (int(flags["COC.HELSPAWN_DADDY"]) == 0) output("s");
	output("."); 
	addNextButton(returnToCampUseOneHour);
}
...
private function swordAndBoardmander():void {
	clearOutput();
	//spriteSelect(68);
	flags["COC.HELSPAWN_WEAPON"] = "scimitar and shield";
	output("You tell Hel to take a break: you've got this handled.  You grab some kindling and string from your supplies, and a few minutes later you've got a pair of small, vaguely sword-like practice weapons pieces together.  Tossing one to the young salamander, you tell her to come get some.");
	
	output("\n\n" + flags["COC.HELSPAWN_NAME"] + " gives the blade a few practice swings before grabbing the wooden blade in both hands and charging at you.  You put your guard up, catching her first overhand blow and turning it aside, nearly toppling the girl over.  She rolls with it, spinning out of your guard and trying a strike from the side.  You parry it, catch an opening, and give her a nice wallop on the shoulder.  A real blade would have nearly cut her arm off, but " + flags["COC.HELSPAWN_NAME"] + " merely stumbles back, yelping in pain.");
	...
	output("\n\n" + flags["COC.HELSPAWN_NAME"] + " picks herself up and grips her blade.  This time she's a little more cautious, approaching slowly, pausing just out of reach.  You hide a grin and bring your own sword up, the tips of your blades nearly touching.  \"<i>Good.  No need to be reckless, kiddo.  Don't just charge in at the first opportunity; you'll get yourself killed.</i>\"");
	
	output("\n\n\"<i>Get " + pc.mf("him","her") + ", " + flags["COC.HELSPAWN_NAME"] + "!</i>\" Hel calls from the sidelines, sipping on a mug.  \"<i>Show " + pc.mf("him","her") + " what a salamander can do!</i>\"");
	
	if (int(flags["COC.HELSPAWN_DADDY"]) == 0) output("\n\nYour");
	else output("\n\nHer");
	output(" daughter braces herself, drawing a breath just a second before she launches her attack, giving herself away.  You parry three quick blows, stepping back as " + flags["COC.HELSPAWN_NAME"] + " leaps into another overhand strike.  You dodge, spinning out of the attack and slicing her right across her bare belly.");
	
	output("\n\n\"<i>Gah!</i>\" she cries, stumbling to her knees.  \"<i>I-I can't...</i>\"");
	
	output("\n\nYou pull " + flags["COC.HELSPAWN_NAME"] + " to her feet and ruffle her hair.  \"<i>C'mon kiddo, don't give up.  Here,</i>\" you say, grabbing a dinner plate and some leather straps from your kit.  In a moment, you've got a rudimentary shield cobbled together.  You fit it over the young salamander's free arm and give her a few pointers on stance.  Nervously, she brings her makeshift shield to bear against you, fidgeting under the heft of it as you make a few slow, easy to parry swings for her to get used to it.  Steadily you pick up the pace, swinging your training sword faster and harder, forcing " + flags["COC.HELSPAWN_NAME"] + " to parry and block, retreating against your onslaught.  You keep it up for a full minute before relenting, letting the kid seize the initiative.  She swings at you, rolling with each parry you make, cleaving into another swing, turning your ripostes aside with her shield and forcing you back.");
	output("\n\nYou wince in pain as " + flags["COC.HELSPAWN_NAME"] + " manages to nick you, turning one of your blows aside with her shield and striking through the opening left in your guard.  \"<i>Alright, alright,</i>\" you laugh, rubbing your shoulder.  \"<i>Good job, " + flags["COC.HELSPAWN_NAME"] + ". You're getting the hang of that shield!</i>\"");
	
	output("\n\n\"<i>Thanks, " + championRef() + "!</i>\" the little salamander grins, hugging you.");
	
	output("\n\n\"<i>Alright, alright,</i>\" Hel says, grabbing the dummy sword from your hand.  \"<i>Now it's time to fight a REAL warrior, " + flags["COC.HELSPAWN_NAME"] + ". Come get some!</i>\"");
	
	output("\n\nIt's several hours long before the three of you quit: time spent drilling " + flags["COC.HELSPAWN_NAME"] + " on stance and parrying, trading off with Helia every few rounds to keep ");
	if (int(flags["COC.HELSPAWN_DADDY"]) != 0) output("her");
	else output("your");
	output(" daughter on her toes.  She learns quickly, though, and soon she hardly needs reminding to keep her shield up and stance braced against Helia's overwhelming assaults.");
...
private function dasBarbarimander():void {
	clearOutput();
	//spriteSelect(68);
	flags["COC.HELSPAWN_WEAPON"] = "scimitar";
	if (pc.credits >= 2000) {
		output("You sigh and dig out a handful of gems.  Helia beams at you, planting a quick kiss on your cheek before grabbing " + flags["COC.HELSPAWN_NAME"] + " and the loot.  \"<i>Thanks for the loan, lover mine.  C'mon, sweetie, we're going for a trip to town.</i>\"");
...
public function helspawnIsASlut():void {
	...
	//}
	//spriteSelect(68);
	flags["COC.HELSPAWN_FUCK_INTERRUPTUS"] = 1;
	output("\nSomething's moving in your camp.");
	
	output("\n\nYour eyes flicker open as a scaled foot moves past your face, hushed breathing barely concealing a giggle.  Another foot, this time a shimmering blue-black, moves past.  You [pc.readyWeapon] as the intruder passes by, ready to defend yourself and your loved ones.  The moment they're out of reach, you roll onto your [pc.feet], looking around for signs of a massed demon attack.  Instead, you see the outline of " + flags["COC.HELSPAWN_NAME"] + " in the dying embers of the fire light, leading what looks like a spider girl by the hand toward her bed.");
	
	output("\n\nYou creep closer, trying to get a closer look at what your ");
	if (int(flags["COC.HELSPAWN_DADDY"]) == 0) output("daughter ");
	else output("ward ");
	output("is doing.  Well, it doesn't take a genius to figure that out, really, but still, you have to be sure.  You slide right up to them, just out of their lines of sight, as " + flags["COC.HELSPAWN_NAME"] + " turns around, letting the spider-kin unclasp her bra and grab her tits, squeezing the small mounds as " + flags["COC.HELSPAWN_NAME"] + " suppresses a moan, reaching back to slip a hand into the spider's crotch, grabbing her - his dick.");
	
	output("\n\nWell then. You suppose you ought to go stop " + flags["COC.HELSPAWN_NAME"] + " from making a whore of herself... but then again, she's old enough to make her own mistakes by now.");
	if (pc.cor() >= 50) output("  And by mistake you clearly mean old enough to get her ass fucked in by a cute little spider trap... you contemplate jumping in on it, but with a sigh figure it's " + possessive(flags["COC.HELSPAWN_NAME"]) + " catch.  Let her enjoy it.");
	clearMenu();
...
private function helspawnDoNothing():void {
	clearOutput();
	//spriteSelect(68);
	output("You turn around and head back to your bed.  As soon as you're under your blanket, your ears are assaulted with the quiet moans and grunts of pleasure coming from " + possessive(flags["COC.HELSPAWN_NAME"]) + " bed as she and her first little boyfriend get it on.  You can practically hear the tail-pegging from here.  How cute!");
	//{HelspawnSlutty +10}
	flags["COC.HELSPAWN_PERSONALITY"] += 10;
	addNextButton();
}
...
private function helSpawnStopFucking():void {
	clearOutput();
	//spriteSelect(68);
	output("You sigh and step out of the shadows, grabbing the feminine little spider boy by the shoulder and yanking him away from " + flags["COC.HELSPAWN_NAME"] + ".  \"<i>And just what the hell do you two think you're doing?</i>\" you yell, scowling ");
	if (pc.tallness >= 90) output("up ");
	else if (pc.tallness <= 72) output("down ");
	output("at your ");
	if (int(flags["COC.HELSPAWN_DADDY"]) == 0) output("daughter ");
	else output("ward ");
	output(". " + flags["COC.HELSPAWN_NAME"] + " gives a shrill cry of surprise and tumbles away from you, trying to cover herself as best she can; the spider trap squirms in your unrelenting grasp, one set of arms covering his rapidly-wilting erection as the other flails around, trying to get you off of him.  Not happening.");
	
	output("\n\n\"<i>You!</i>\" you say, pointing an accusing finger at " + flags["COC.HELSPAWN_NAME"] + ".  \"<i>What the hell, kid?  Who the fuck is this?</i>\"");
	
	output("\n\n\"<i>That's, uh.... Alex, meet ");
	if (int(flags["COC.HELSPAWN_DADDY"]) == 0) output("my dad");
	else output("[pc.name]");
	output(". " + championRef() + ", Alex.</i>\"");
	...
	output("\n\n\"<i>" + championRef() + "...</i>\" " + flags["COC.HELSPAWN_NAME"] + " groans as you grab the spider boy and toss him out of camp, watching with hawk-like eyes as he slinks back to his jungle.  When he's gone, you sigh and walk over to " + flags["COC.HELSPAWN_NAME"] + ", wiping a tear from her cheek.  Putting an arm around her shoulder, you give her a long, long talk about not being a whore.");
	
	output("\n\n\"<i>B-but mom...</i>\"");
	
	output("\n\n\"<i>That's why you </i>shouldn't<i> be bringing men home.  Or women.  Or anything else, kiddo.");
	//If No Hel Love:
	if (flags["COC.HEL_LOVE"] != 0) output("  You don't want to be a whore like your mother, do you?  The girl that everyone's had a ride with.");
	//if HelLove:
	if (flags["COC.HEL_LOVE"] == 1) output("  I'm not going to a raise a whore, " + flags["COC.HELSPAWN_NAME"] + ".  Your mother's made mistakes, sure, but...</i>\" you sigh.  \"<i>");
	output("  You don't have to be that person, " + flags["COC.HELSPAWN_NAME"] + ".  Don't just give yourself out to the first person that asks, you know?  Save it for someone you really care about.  Trust me, it'll be worth it.</i>\"");
...
public function helspawnAllGrownUp():void {
	clearOutput();
	//spriteSelect(68);
	output("You wake up the next morning, and soon find your eyes drawn to where " + flags["COC.HELSPAWN_NAME"] + " lies, barely an arm's reach from her mother's hammock, still snoring peacefully.  You sigh wistfully as you get up and get dressed, still thinking of the night before and your girl's little misadventure with the spider.  God, she's grown up fast.  Faster than you could have imagined - or her mother, for that matter.  Helia, too, is still reeling from her daughter's rapid, mutagenic growth.  But there's nothing to be done about that, now, and it seems she's reached what passes for adulthood in this strange world.  She's had the body of a woman for some time now, and it seems the mind and desires of one as well.");
	
	output("\n\nA part of you is sad to see her all grown up, and now you know it's time to treat your ");
	
	if (int(flags["COC.HELSPAWN_DADDY"]) == 0) output("daughter ");
	else output("ward ");
	output("like a woman.  The time to raise her, to shape her development, is over.  You can only hope you've done a good enough job to keep her safe in this hellish world; to give her sound judgment and the abilities she'll need to protect herself in the years and adventures to come.");
	
	output("\n\n<b>" + flags["COC.HELSPAWN_NAME"] + " has been added to the Followers menu!</b>");
	flags["COC.HELSPAWN_AGE"] = 3;
	flags["COC.HELSPAWN_GROWUP_COUNTER"] = 0;
	addNextButton(returnToCampUseOneHour);
}
...
public function helspawnsMainMenu():void {
	clearOutput();
	output("You call " + flags["COC.HELSPAWN_NAME"] + " over to you and she comes running, ");
	if (pc.tallness <= 72) output("grabbing you off your [pc.feet] and clutching you in a tight hug");
	else output("throwing her arms around you and hugging you tightly");
	output(".  Laughing despite yourself, you ruffle her fiery hair and ask her if she’s got a minute.");
...
private function hugHelspawn():void {
	...
	if (pc.tallness <= 72) {
		output("You reach up and pull " + flags["COC.HELSPAWN_NAME"] + " down to your level, wrapping your arms tight around her slender shoulders.  Grinning from ear to ear, the towering girl squeezes you right back, her powerful muscles just about crushing your smaller body.  Her tail slips around your [pc.hips], the warm leather binding you to your beloved ");
		if (int(flags["COC.HELSPAWN_DADDY"]) == 0) output("daughter ");
		else output("ward ");
		output("as she nuzzles up against you.");
...
private function talkToHelspawn():void {
	...
	if (int(flags["COC.HAD_FIRST_HELSPAWN_TALK"]) == 0) {
		...
		//{if Sluttymander:
		if (flags["COC.HELSPAWN_PERSONALITY"] >= 50) output("  Even if she wants to be just like mom, she shouldn't be bringing monsters home.  Some of them may be dangerous, and she's not a warrior quite like you and Helia yet.");
		else output("  She should know better than to bring monsters back.  She's not her mother; lust doesn't have to dominate her life like that.");
		...
		//if (int(flags["COC.HELSPAWN_DADDY"]) == 0) output("Of course, as her father, you could always say it's your duty to keep her happy...");
		//else output("Of course, as her adoptive father, you could always say that you love her in a different, more intimate way.");
		//output("  She wouldn't need monsters with you around.");
		clearMenu();
		addButton(0, "Stop Fucking", dontFuckAlex);
		addButton(1, "Her Boyfriend", helSpawnBoyfriend);
		addButton(2, "Incest", incestWithHelspawn);
		//if (silly && flags["COC.HELSPAWN_PERSONALITY"] >= 50) addButton(3, "Both", whyNotBoth, null, "Both", "Why don't we have both?")
	}
	//Talk 2
	//{Kiha must be at camp}
	else if (temp <= 0 && followerKiha()) {
		output("\"<i>Oh hey, " + championRef() + "!  You gotta come see what I was making!</i>\"");
		output("\n\nShe grabs you by the [pc.gear], pulling you back toward where she had been standing, near the center of camp.  A large cast-iron cauldron’s sitting on your firepit, full of a dark-green substance that’s bubbling along.  Your dusky dragoness lover, Kiha, is sitting on the edge of the pot, apparently immune to the scalding heat, a long wooden ladle grasped between her dexterous claws.  She grins up at you as you and your daughter wander over.  \"<i>Hey, Doofus, come for an early taste?</i>\"");
		output("\n\n\"<i>C'mon, " + championRef() + ", try it!</i>\" " + flags["COC.HELSPAWN_NAME"] + " adds happily.  \"<i>It's aunt Kiha's favorite recipe.</i>\"");
	...
	//Talk 3
	//{Needs Rath at camp]
	else if (temp <= 1 && flags["COC.RATHAZUL_IN_CAMP"] == 1) {
		output("\"<i>" + flags["COC.HELSPAWN_NAME"] + "!</i>\" you hear a ragged old voice call, \"<i>Get back here this instant!  I'm not done with you yet!</i>\"");
		
		output("\n\n\"<i>Coming!</i>\" " + flags["COC.HELSPAWN_NAME"] + " calls back as Rathazul shuffles into view, waving around what looks like a tiny hammer.  Whispering, " + flags["COC.HELSPAWN_NAME"] + " says, \"<i>Could you come with me, " + championRef() + "? Rathazul weirds me out.</i>\"");
		
		output("\n\nChuckling, you nod and follow " + flags["COC.HELSPAWN_NAME"] + " as she returns to Rath's makeshift laboratory and seats herself on one of his tables.  Grumbling about kids, the old rat proceeds to rap his little hammer on her knee - nearly getting kicked in the face for his trouble - before asking her to hold her breath as he listens to her heartbeat.  She's nearly blue in the face before he nods approvingly and tells her she can go. " + flags["COC.HELSPAWN_NAME"] + " goes running, happy to get away as Rath turns to you, shaking his head.");
		
		output("\n\n\"<i>Just checking up on her, [pc.name].  Her meteoric growth is of some concern - and scientific interest, I might add, if I could determine what caused it.  If I could mass-produce it.  ");
		if ((amilyFollower() && !amilyCorrupt())) output("Little Amily's solution was makeshift at best, and the children are beyond my reach to study.  Still.  ");
		output("Imagine whole legions of children reared and raised in the time it takes a goblin to do the same. We might have a fighting chance for once, with numbers to match the demons.</i>\"");
		
		output("\n\n\"<i>But I suppose that's still out of reach, [pc.name].  She's finished growing, and there's no sign of the exact chemical needed to synthesize the compound.  I'll keep checking up on her for health, though perhaps one day I'll find the means to make accelerated growth safe and widespread.  An old man can hope, yes?</i>\"");
		addNextButton(returnToCampUseOneHour);
	}
	//Talk 4
	//{Bath Slut w/ DD or HHH must be at camp}
	else if (temp <= 2 && milkSlave() && flags["COC.MILK_SIZE"] > 0) {
		output("\"<i>Hey, have you seen " + flags["COC.MILK_NAME"] + " around anywhere, " + championRef() + "?</i>\" " + flags["COC.HELSPAWN_NAME"] + " asks, looking around the camp.  \"<i>Mom asked me to milk her, but I haven't seen her around.  She wouldn't have run off, would she?</i>\"");
		if (flags["COC.MILK_SIZE"] == 1) output("\n\nYou tell her it'd be more of a wobble, but");
		else output("\n\nYou doubt she would have; " + flags["COC.MILK_NAME"] + " seems to have a fancy for you, but");
		output(" it wouldn't hurt to check around.  She does tend to wander into the traps around camp from time to time.  Offering " + flags["COC.HELSPAWN_NAME"] + " your hand, you start searching the camp for your missing milk maid.  The two of you make a circuit around camp, checking the traps and behind various rocks and hollows, to no avail.  Finally, as you're starting to get worried about the dusky maid, you happen to look into the steel pool in her part of camp.  Sure enough, you spy " + flags["COC.MILK_NAME"] + " curled up at the bottom of the pool, cuddled up with a ragged blanket to catch a quick nap.");
		output("\n\n\"<i>Aww,</i>\" " + flags["COC.HELSPAWN_NAME"] + " grins, looking down at the napping " + flags["COC.MILK_NAME"] + ".  \"<i>Guess I can milk her later, then.</i>\"");
		clearMenu();
	...
	//{Isabella must be at camp}
		...
		if (pc.race() != "human") output(" and you yourself have suffered as well");
		output(".  You smile as Isabella seats herself beside " + flags["COC.HELSPAWN_NAME"] + ", joining their voices together for the final chorus that leaves them both shaking as " + flags["COC.HELSPAWN_NAME"] + " strums the last, desperate notes, nearly clawing through the strings in her passion.");
		
		output("\"<i>");
		if (!isabellaAccent()) output("Sehr gut");
		else output("Very good");
		output(" little " + flags["COC.HELSPAWN_NAME"] + ",</i>\" Isabella says, pulling the young 'mander into a great big hug that threatens to smother her betwixt the cowgirl's massive bosoms.  Grinning, you congratulate " + flags["COC.HELSPAWN_NAME"] + " on her performance, showering your girl in praise before you depart, leaving her to Isabella's instruction.  As you walk away, you can't help but notice Helia standing a short ways off, rubbing her eyes.");
		
		output("\n\nWhen you approach, your lover smiles at you, saying, \"<i>I always wanted to be a bard when I was a little girl.  I'm... I'm glad " + possessive(flags["COC.HELSPAWN_NAME"]) + " getting the chance, at least.</i>\"");
		addNextButton(returnToCampUseOneHour);
	}
	else {
		output("Unfortunately, there doesn't seem to be anything in particular to talk about at the moment.  The two of you spend the time in companionable quiet, making smalltalk.");
		addNextButton(returnToCampUseOneHour);
	}
}
...
private function dontFuckAlex():void {
	clearOutput();
	output("Stroking " + possessive(flags["COC.HELSPAWN_NAME"]) + " hair, you try to tell her that there's a better way, that she doesn't have to just fuck everything she comes across.  Her mother's that way, sure, but she could do so much better, so much more with herself than giving in to constant lust.");
	//{If Sluttymander:}
	if (flags["COC.HELSPAWN_PERSONALITY"] >= 50) {
		output("\n\n" + flags["COC.HELSPAWN_NAME"] + " scoffs, shaking her head.  \"<i>But I </i>like<i> mom's way.  I love jilling off, the feeling of my cunt getting stretched and my ass being torn apart by the big toys I buy in town.  That's who I am, " + championRef() + ".  I'm like mom, and I don't think... no, I know I don't want to change.  I love sex, even if you stopped me and Alex from it.  I want to feel a gang of minotaurs raping me, I want to jump on a drider's cock and ride him down.  I want to do the things I heard mom doing, and that's just what I'm going to do. I'm sorry.</i>\"");
		
		output("\n\nYou try to say something, but " + flags["COC.HELSPAWN_NAME"] + " steps away, calling back, \"<i>I love you, " + championRef() + ", but I can't be what you want me to be.  I'm going to be a slut like mom, and there's nothing you can do to stop me!  It's who I was born and raised to be.</i>\"");
		
		output("\n\nYou may have made a horrible mistake.");
	}
	//{If Chastemander:}
	else {
		output("\n\n" + flags["COC.HELSPAWN_NAME"] + " nods slowly, taking in your words.  \"<i>I... I know, " + championRef() + ".  I'm sorry I brought Alex home last night.  It won't happen again.  I don't... I love mother, but I don't want to be like her.  A whore.  She's a great woman, but the things she does... they disgust me.</i>\"");
...
private function incestWithHelspawn():void {
	clearOutput();
	output("You cup " + possessive(flags["COC.HELSPAWN_NAME"]) + " cheek, turning her to you... only to have her leap you, straddling your [pc.legs] as her powerful arms wrap around your neck.  Her thin lips press hard to yours, slender tongue probing against yours as her svelte body presses against you.  You respond in kind, grabbing her big ass and squeezing, kneading the delicious curves as she starts to grind against you, breath hot and heavy on your " + pc.skinFurScales() + ".  \"<i>I said I love you, " + championRef() + ",</i>\" she grins, so close you can practically feel the beat of her heart through her perky breasts.  \"<i>Glad to know you feel the same way.</i>\"");
...
private function ohGodEwKihaAndHelspawnSuckAtCooking():void {
	clearOutput();
	output("You spew the stew onto the ground and grab a nearby waterskin, trying to flush the taste from your mouth.  Kiha and " + flags["COC.HELSPAWN_NAME"] + " look on with horror as you wipe your mouth and begin to try and coherently explain just how god awful whatever that... STUFF... you just put in your mouth was.  Shock turns to anger before you’re halfway through admonishing the pair of scaly redheads.  Kiha scowls at you and snatches the ladle from you.");
	output("\n\n\"<i>Oh, what the fuck do you know anyway, you big ass!  It's perfectly fine, isn't it " + flags["COC.HELSPAWN_NAME"] + "?</i>\" she growls, spooning up a mouthful of the stuff before you can stop her.  A heartbeat after she swallows, Kiha goes completely stiff; her eyes growing as wide as saucers as they water.  She collapses backwards, falling onto her back with a muted <i>THUD</i>.");
	
	output("\n\n\"<i>K-Kiha?</i>\" " + flags["COC.HELSPAWN_NAME"] + " yelps, watching the dragoness collapse into a pile of limp scales and leather.  \"<i>Are you okay?  Is she okay, " + championRef() + "?</i>\"");
	...
	output("\n\nYou and " + flags["COC.HELSPAWN_NAME"] + " are soon working away, stirring up your own special blend of your rations and the spices she and Kiha gathered for the first attempt.  Once you've got the basics in, you step back and hand the ladle over, telling the young 'mander to go wild.  Grinning like a kid, she goes to work, mixing in a wild combination of ingredients.  The cauldron's full to bursting by the time she's done, and no sooner does she ladle out the first bowel of it than Kiha sits up, groaning.");
	
	output("\n\n\"<i>The fuck happened?</i>\" the dragoness hisses, rubbing her temple.  \"<i>Feel like I got hit with a hammer.</i>\"");
	
	output("\n\n\"<i>You, uh, fell off the edge, aunt Kiha.  And spilled all the stew.  Here, we remade it for you!</i>\" " + flags["COC.HELSPAWN_NAME"] + " lies, presenting \"aunt Kiha\" with a bowl full of an aromatic brown goop, full of carrots and spices floating atop it like boats on a placid lake.  At least it smells better.");
	...
	output("\n\nYou just shake your head and grab a bowl, sitting down with the scaly ladies as you enjoy your lunch, trying to ignore the little shit-eating grin " + possessive(flags["COC.HELSPAWN_NAME"]) + " sporting all the while.");
	//pc.refillHunger(25);
	addNextButton(returnToCampUseOneHour);
}
...
private function umYum():void {
	...
	//if (pc.hunger > 25)
	//{
		output("\n\nCringing, you set the offered bowl back down, squeaking out that it's delicious, thank you very much, but you have something to take care of right now; maybe you can have more later.  Kiha rolls her eyes and shoos you off before she and " + flags["COC.HELSPAWN_NAME"] + " spoon out their lunch. \n\nAs you're wandering off looking for somewhere to hurl, ");
		//pc.refillHunger(5);
	//}
	//else
	//{
		...
		//if (rand(3) == 0)
		//{
			////Player vomits. Poor pc. Damages hunger by 15-25.
			...
			//pc.takeDamage(pc.maxHP() / 4);
			//pc.damageHunger(rand(10) + 15);
			//dynStats("lib", -10);
			//dynStats("lust", -100, "resisted", false);
		//}
		//output("You set the finished bowl down and ")
	//}
	output("you hear the tell-tale groans and gagging of a pair of scaly ladies who've just realized what kind of abomination they've created.");
...
private function helSpawnSureMilkHerLater():void {
	clearOutput();
	output("You chuckle, telling her to let the poor girl sleep.  There'll be plenty of milk later.");
	addNextButton(returnToCampUseOneHour);
}
...
private function helSpawnMilkHerNow():void {
	clearOutput();
	output("\"<i>Oh, no.  You're not getting out of your chores that easy, kiddo.  In you go,</i>\" you say, giving her a little push toward the edge of the pool.");
	
	output("\n\n" + flags["COC.HELSPAWN_NAME"] + " gives a rebellious huff as she clambers down, slipping down to the bottom and giving " + flags["COC.MILK_NAME"] + " a gentle nudge.  \"<i>C'mon, cutey, wake up.  Time to relieve some pressure.</i>\"");
	
	output("\n\nYawning powerfully, " + flags["COC.MILK_NAME"] + " rises to her knees and rubs the sleep from her eyes");
	if (flags["COC.MILK_SIZE"] == 2) output(", already working to unfasten her shift");
	output(".  \"<i>Suckle?</i>\" she asks, turning her full teats toward " + flags["COC.HELSPAWN_NAME"] + ".");
	
	//If Sluttymander:
	if (flags["COC.HELSPAWN_PERSONALITY"] >= 50) output("\n\n\"<i>Wouldn't have it any other way!</i>\" the salamander answers with an eager grin, easing herself into " + flags["COC.MILK_NAME"] + "'s arms and wrapping her lips around one of her prominent teats. " + flags["COC.MILK_NAME"] + " gives a pleasured little shudder as " + flags["COC.HELSPAWN_NAME"] + " starts to suckle like a babe, drinking down the dusky maid's seemingly endless supply of milk.");
	
	//If Chastemander:
	else output("\n\n\"<i>Sorry, " + flags["COC.MILK_NAME"] + ", you know I'm not really into that.  Just turn around and sit back, alright?</i>\"  With a sigh, " + flags["COC.MILK_NAME"] + " does as she's asked, leaning back into " + possessive(flags["COC.HELSPAWN_NAME"]) + " arms as the young salamander goes to work, clawed fingers gently caressing her prominent nubs until a white flow springs fort, splashing her knees as the dusky maid gives a little gasp of pleasure.");
	
	output("\n\n<i>They seem to get on all right</i>, you think as you watch the pair of them.  It's nice to have someone else around to help keep " + flags["COC.MILK_NAME"] + "'s production under control.  Poor thing just never stops lactating.");
	addNextButton(returnToCampUseOneHour);
}
...
private function sparHelspawn():void {
	clearOutput();
	output("You ask " + flags["COC.HELSPAWN_NAME"] + " if she's up for some battle practice, and she answers with an eager nod as she grabs her weapon.");
	//If Sluttymander: 
	if (flags["COC.HELSPAWN_PERSONALITY"] >= 50) output("\n\n\"<i>Ready to get your shit kicked in, old " + pc.mf("man", "lady") + "?</i>\" she grins, drawing her weapon.");
	else output("\n\n\"<i>Just go easy on me, okay?  I'm still new at this...</i>\" she says, stepping back as she draws her weapon.");
	
	CombatManager.newGroundCombat();
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(new CoCHelspawn());
	CombatManager.victoryScene(beatUpYourDaughter);
	CombatManager.lossScene(loseSparringToDaughter);
	CombatManager.displayLocation(String(flags["COC.HELSPAWN_NAME"]).toUpperCase());
...
internal function loseSparringToDaughter():void {
	//if Sluttymander:
	if (flags["COC.HELSPAWN_PERSONALITY"] >= 50) {
		output("As you stumble back, ");
		if (pc.lustQ() >= 100) output("succumbing to your own lusts");
		else output("unable to withstand her unending hail of attacks");
		output(", " + flags["COC.HELSPAWN_NAME"] + " quickly sweeps your [pc.legs] out from under you, dropping you right on your ass.  You collapse with a grunt, weapon tumbling out of hand, and a moment later " + possessive(flags["COC.HELSPAWN_NAME"]) + " on you, straddling you with her powerful legs, pushing you down into the dirt.");
		...
		output("\n\nYou chuckle and stumble to your [pc.feet], pausing to ruffle her hair and collect your gear.");
	...
	//Else If Chastemander:
		...
		output(", " + flags["COC.HELSPAWN_NAME"] + " grabs your arm, catching you before you can fall.  \"<i>Easy there, " + championRef() + ", I got you,</i>\" she says, pulling you into a quick hug.");
		...
		if (pc.HP() <= 1) output("recover from that ass kicking");
		else output("dunk your head");
		output(".");
	}
...
internal function beatUpYourDaughter():void {
	...
	if (flags["COC.HELSPAWN_PERSONALITY"] >= 50 && enemy.lustQ() >= 100) {
		output("\"<i>N-no more...</i>\" the slutty little salamander moans, slumping down to the ground, arms wrapping around herself.  \"<i>Fuck, you're sexy... so horny...</i>\" she groans, hands slipping down to her soaked bikini bottom.");
		...
		//If No Incest: 
		if (int(flags["COC.HELSPAWN_INCEST"]) == 0) output("  You wash your hands of the defeated slut and head back to camp, leaving her to work through her tension herself.");
		else {
			output("  You reach down and give your lovely, lusty daughter a pat on her expansive rear, telling her she'll always look her best with her ass in the air, begging for ");
			if (!pc.hasCock()) output("sex");
			else output("your cock");
			output("... and that if she's lucky, you might tend to her when you've cooled off.");
		}
	}
	else {
		output(flags["COC.HELSPAWN_NAME"] + " stumbles back, dropping her weapon and waving her arms, \"<i>I give, I give.</i>\"");
		output("\n\nWith an approving nod, you lower your [pc.mainWeapon], telling her she did a fine job.  Not many people can stand up to you for as long as she did, after all.  She grins a little, but winces when you try and get close");
		if (enemy.HP() <= 1) output(", rubbing her many bruises");
		else output(", blushing brightly after your slutty display");
		output(".  You chuckle and ruffle her hair, \"<i>C'mon, kiddo, let's get some food in you.</i>\"");
...
public function maiVisitsHerKids():void {
	clearOutput();
	//spriteSelect(68);
	output("As you head back into camp, you notice Hel and " + flags["COC.HELSPAWN_NAME"] + " are sitting around the cook fire in the center of camp, with your foster daughter balanced precariously on the lap of her own father, Mai the fox-girl.");
	...
	output("\n\n\"<i>Hi, [pc.name],</i>\" Mai says with a demure smile, arms wrapped around " + possessive(flags["COC.HELSPAWN_NAME"]) + " belly, trying to hold the daughter that's near a foot taller than she is.  Finally, laughing, she lets the young salamander go.  " + flags["COC.HELSPAWN_NAME"] + " drops down to sit beside her mother and father, and quickly has both a smooth and scaly hand stroking her hair.");
	//If Chastemander:
	if (flags["COC.HELSPAWN_PERSONALITY"] < 50) output("\n\n" + flags["COC.HELSPAWN_NAME"] + " happily leans her head against Mai's leg, accepting the affection with her tail swishing gaily behind her, foxy ears twitching as Mai brushes them.");
	else output("\n\n" + flags["COC.HELSPAWN_NAME"] + " leans back, pulling out a flask and taking a deep swill before passing it to her father, who takes it with a knowing smile and knocks it back like a champ.");
	...
	output("\n\n\"<i>Heeeeeyyyyyy,</i>\" " + flags["COC.HELSPAWN_NAME"] + " whines, crossing her arms with a huff.");
	...
	output("\n\n" + flags["COC.HELSPAWN_NAME"] + " lights up as Mai slips the gold chain around her daughter's neck.  \"<i>Something to remember me by when I'm not around,</i>\" she says with a wink, letting her kid skip off to find a mirror.");
	
	output("\n\n\"<i>And you, [pc.name],</i>\" she says, turning to you, \"<i>Thanks for taking care of " + flags["COC.HELSPAWN_NAME"] + ".  I-I mean, I know I was really making her for you and Hel anyway, but still... she's a good kid, and I love her anyway.  Keep her safe, alright?</i>\"");
	
	output("\n\nYou promise that you will, and with a quick nod, Mai runs after " + flags["COC.HELSPAWN_NAME"] + " to say goodbye for the day.  As she leaves, Helia grins, holding you tight against her.  \"<i>She's a good girl, [pc.name].  Couldn't have picked a better father for our girl.</i>\"");
	addNextButton(returnToCampUseOneHour);
}
...
public function spiderBrosGift():void {
	clearOutput();
	//spriteSelect(68);
	flags["COC.SPIDER_BRO_GIFT"] = 1;
	output("As you're getting ready to head out for the day, you notice " + flags["COC.HELSPAWN_NAME"] + " prancing around camp with a long, brightly-colored scarf wrapped around her neck, standing painfully in contrast with her midnight-black scales and pale flesh.");
	...
	//if Isabella: 
	if (isabellaFollower()) output(", or aunt Isabella might have knitted me a new one, I don't know");
	output(".</i>\"");
...
public function hakonAndKiriComeVisit():void {
	clearOutput();
	//spriteSelect(68);
	flags["COC.HAKON_AND_KIRI_VISIT"] = 1;
	output("As you're returning to camp, you notice Helia running around like a chicken with her head cut off, fussing about damn near everything in her part of the camp");
	if (companionsCount() > 1) output(" and everyone else's too, much to their chagrin");
	output(". As she's furtively polishing off her giant still, trying to get the dingy old thing to shine, you approach and clear your throat. She gives a sharp yelp and spins around, but seems to relax as she sees it's just you.");
	output("\n\n\"<i>H-hey, [pc.name],</i>\" she says nervously.  \"<i>Just in time.  I was about to go looking for you.  Uh, so...</i>\"");
	...
	output("\n\nIt hardly matters, as a few minutes later, you hear a cry of unabashed glee from the edge of camp, and turn to see " + flags["COC.HELSPAWN_NAME"] + " running up to meet a pair of oncoming figures.  You wave as Hel's father swoops his granddaughter up into a great bear hug, hefting the half-breed 'mander off her feet in his powerful arms.  " + flags["COC.HELSPAWN_NAME"] + " squeals with delight as she's practically carried into camp, until Hakon sets her down and hugs her mother, pulling Hel into a tight hug before she can finish saying hello.");
	
	output("\n\n\"<i>Hi, [pc.name],</i>\" Kiri says, fluttering over to plant a peck on your cheek.  \"<i>Long time no see.</i>\"");
	
	output("\n\n\"<i>Too right.  Put 'er there, " + pc.mf("son", "girl") + ",</i>\" Hakon says, extending a hand to you, grinning wide as he lets his daughter and granddaughter go.  You shake the old salamander's hand, nearly wincing as his powerful grip throttles your wrist vigorously, practically walking you back into the heart of the camp.");
	
	output("\n\nTurning to " + flags["COC.HELSPAWN_NAME"] + ", Hakon smiles with pride.  \"<i>Gods and demons, girl, you've grown!  What have you two been feeding her, huh?</i>\"");
	
	output("\n\n\"<i>And you should see how strong she is!</i>\" Hel grins.  \"<i>Go on, " + flags["COC.HELSPAWN_NAME"] + ", show the old scales what you've got.</i>\"");
	
	output("\n\n\"<i>Old scales!?  I'll show you old, you little brat.  Come on, hit me!</i>\" Hakon says, just before " + flags["COC.HELSPAWN_NAME"] + " punches him right in the chest, knocking him right on his ass.  He tumbles back with a grunt, shaking his head with a wry laugh.  \"<i>Ha!  Maybe these scales are getting old after all.  This one's gonna be a hell of a fighter, though.  Should take her hunting with us one of these days.  What do you say, kid?</i>\"");
	
	output("\n\n\"<i>Sure!  ");
	if (flags["COC.HELSPAWN_WEAPON"] == "scimitar and shield" || flags["COC.HELSPAWN_WEAPON"] == "bow") output(possessive(championRef()) + " been teaching me how to fight.");
	else output("Mom's been teaching me how to fight.");
	output("  I'm not as good as mom and " + championRef() + " yet, but I'd love to come along.</i>\"");
	
	output("\n\n\"<i>That's the spirit.  Now we just have to convince your aunt Kiri to come along.  And ");
	if (int(flags["COC.HELSPAWN_DADDY"]) == 0) output("your old man");
	else output("[pc.name]");
	output(", too.</i>\"");
...
private function goHuntingBitches():void {
	clearOutput();
	//spriteSelect(68);
	output("\"<i>Sure,</i>\" you say, quickly earning approving nods from Hakon and Helia.");
...
private function noHuntingBitches():void {
	clearOutput();
	//spriteSelect(68);
	output("\"<i>I'll pass,</i>\" you say with a laugh, earning a shrug from the salamanders.");
