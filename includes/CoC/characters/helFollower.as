...
public function helCapacity():Number {
	return cockVolume(85);
}

public function helAnalCapacity():Number {
	var anal:int = 85;
	if (flags["COC.HELIA_ANAL_TRAINING"] >= 1) anal += 100;
	if (flags["COC.HELIA_ANAL_TRAINING"] >= 2) anal += 300;
	return cockVolume(anal);
}
...
public function helPregnancyEvent():int {
	if (flags["COC.HEL_PREGNANCY_INCUBATION"] == undefined) return 0;
	if (flags["COC.HEL_PREGNANCY_INCUBATION"] <= 100 * 60 + timeAsStamp) return 4;
	if (flags["COC.HEL_PREGNANCY_INCUBATION"] <= 200 * 60 + timeAsStamp) return 3;
	if (flags["COC.HEL_PREGNANCY_INCUBATION"] <= 300 * 60 + timeAsStamp) return 2;
	return 1;
}

public function isHeliaBirthday():Boolean {
	return days >= 7 * 4 * 7 && days <= 8 * 4 * 7;
}
...
public function heliaFollowerIntro():void {
	...
	output("\n\n\"<i>So, [pc.name], I've been thinking,</i>\" Helia murmers, still avoiding your gaze.  \"<i>I just... I guess I just wanted to say thanks.  For helping me ");
	if (flags["COC.HARPY_QUEEN_EXECUTED"] > 0) output("kill");
	else output("bring down");
	output(" the Harpy Queen... and getting my sister out of that shithole.  For giving me a chance to meet my dad.  For everything.</i>\"  You start to tell her that you're happy to help, but Hel cuts you off, speaking quickly: <i>\"Kiri, Dad, and I have been trying to make it work, living out on the plains - we really have - but Dad's not in any shape to fight and Kiri's no good at it; the plains are just too dangerous for them and Dad's been having nightmares about mom, and...\"</i> she pauses to take a gasping breath, unaccustomed to talking so quickly.");
	
	output("\n\nShe slows down, finally managing to look your way.  \"<i>So we scraped some gems together, and we're going to try and get a place in that old city - Tel'Adre.  And, well, I was wondering...</i>\" she sighs and runs her scaly hands through her hair, searching for the words she wants to say.  \"<i>Look, [pc.name], you're my best friend, bar none.  In the time we've been together, I've had more fun than the rest of my life combined.  Maybe I'm crazy, but...  I think we're good together, you know?  So, if you think the same - and I understand if you just wanna keep things the way it's been, I do... but maybe you'd like to move in with us?  With me...</i>\"");
...
private function iCantLetFireButtsRapeMyCampsButt():void {
	...
	if (flags["COC.HEL_FOXY_FOURSOME_WARNED"] > 0 || flags["COC.HEL_EDRYN_OFFER"] > 0) output("like usual");
	output(", and maybe you and I can spend some quality time together.  Alright?\"</i>");
	
	output("\n\nYou nod and tell your fiery lover you'll be sure to do just that.");
	flags["COC.HELIA_FOLLOWER_DISABLED"] = 1;
...
private function comeToCampHeliaIWantTailInButt():void {
	...
	output("\n\nHel goes wide-eyed at the suggestion, taken aback by your solution.  <i>\"I-I dunno, [pc.name].  Gods know I wanna be with you, I'd give anything to have you close... But I don't like the idea of Dad and Kiri being all by themselves.  They need me, you know?  And I need them.\"</i>");
...
private function afterMoveInBoningAnalFireTail():void {
	...
	output("\n\nYou and Hel disentangle from your post-coitus repose, redressing together as you tease and flirt, giving Hel's big breasts a playful squeeze as she swats your [pc.butt] with her warm tail.  After you're both clothed, you draw the salamander into another long kiss, breaking it only to ask what you can do to help her get settled in.");
	...
	flags["COC.HEL_FOLLOWER_LEVEL"] = 2;
...
private function justFriendsWithAnalTailWaifu():void {
	clearOutput();
	output("Awkwardly, you spend the next few minutes saying that, while you appreciate the offer and her affections, you're more interested in just being friends with Hel, as you have been for some time.");
	...
	output(" to give you a little peck on the cheek.  <i>\"Well, that's that, then.  I guess.  Well, see you around, [pc.name].\"</i>");
	
	output("\n\n<i>\"Sure will.\"</i>");
	
	output("\n\nYou wave as Hel retreats back toward her own home.");
	flags["COC.HELIA_FOLLOWER_DISABLED"] = .5;
	addNextButton();
}
...
public function helFollowersIntro():void {
	...
	if (followerKiha() && int(flags["COC.HEL_INTROS_LEVEL"]) < 1) {
		flags["COC.HEL_INTROS_LEVEL"] = 1;
		output("You pace around camp, awaiting the return of your new companion.  After several minutes, you notice Kiha sitting behind you, eyeing you with an eyebrow cocked.  Suddenly self-conscious, you stop and face the dragon-girl.");
		output("\n\nKiha smirks slightly. <i>\"What the hell are you doing, doofus?  Expecting someone?\"</i>");
		
		output("\n\nWell, yeah, actually.");
		
		output("\n\n<i>\"Oh?  Is that right?  Well, come on then, [pc.name], spill it!  Who's c- oh you've gotta be shitting me.\"</i>");
		...
		output("\n\n<i>\"I'd ask you the same thing, but...  Well, I think we both know [pc.name] here is into polyamory.  Isn't that right, lover mine?\"</i>");
		output("\n\n<i>\"Hey!  You get your whore hands off my [pc.name] right this instant!\"</i>");
		...
		output("\n\n<i>\"Gah!\"</i> the dragoness yelps, rubbing her now-red butt and flushing slightly.  Hel laughs heartily, until Kiha finally says, <i>\"Friends, huh?  Well, I guess if [pc.name] trusts you, I can...  at least be civil, I guess...  I'm Kiha.\"</i>  She steps up and, reservedly, shakes Hel's hand.");
	...
	//If Isabella 
	else if (isabellaFollower() && int(flags["COC.HEL_INTROS_LEVEL"]) < 2) {
		flags["COC.HEL_INTROS_LEVEL"] = 2;
		//is at Camp (She and Hel are cool)
		if (flags["COC.HEL_ISABELLA_THREESOME_ENABLED"] > 0) {
			output("Showing Hel around, you eventually come to the part of your camp inhabited by the towering cowgirl Isabella.  When you arrive, you find Isabella reclining in her armchair, humming a sweet melody as she cleans her tower shield.");
			...
			output("\n\n<i>\"Mmhm,\"</i> the salamander nods, using the motion to further nestle her head between Isabella's massive mammaries until she practically disappears between them.  Muffled by titflesh, she adds, <i>\"[pc.name] invited me to stick around, so...  Looks like I won't have to walk so far to get my favorite milk!\"</i>");
		...
		//If Isabella is at Camp (And she and Hel are NOT cool)
			...
			output("\n\n<i>\"[pc.name]!?\"</i> They both blurt at once, surprised by your intervention.");
			...
			output("\n\nIsabella makes an indignant huff and turns her nose up at the salamander.  <i>\"Do not listen to this little liar, [pc.name].  I found it in ze hands of ze gnolls, und most certainly did not 'steal' it.\"</i>");
			...
			output("\n\n\"<i>Yeah,</i>\" Hel answers with a little scoff, <i>\"And, I guess if you're [pc.name]'s friend...  you're probably alright.  Although I was winning, you impertinent bitch.\"</i>");
			...
			if (isabellaAccent()) output("<i>\"You are... velcome,\"</i> Isabella says before collecting some of the scattered belongings from the ground.  You continue your tour, now that the girls are...  not going to murder each other in the middle of the night, at least.");
			else output("<i>\"You're... welcome,\"</i> Isabella says before collecting some of the scattered belongings from the ground.  You continue your tour, now that the girls are...  not going to murder each other in the middle of the night, at least.");
			flags["COC.HEL_ISABELLA_THREESOME_ENABLED"] = 1;
		}
	}
	//If Rath is in Camp
	else if (int(flags["COC.HEL_INTROS_LEVEL"]) < 3 && flags["COC.RATHAZUL_IN_CAMP"] == 1) {
		flags["COC.HEL_INTROS_LEVEL"] = 3;
		output("You take Hel over to the small section of camp Rathazul has cordoned off for his 'laboratory,' surrounding himself with glass tubes and beakers and other, stranger instruments.  You poke through the array of equipment to find old Rath sitting in front of some experiment or another, furiously scribbling notes.  With a light cough, you alert him to your presence.");
		...
		output("\n\n\"<i>Goodness gracious, you certainly collect people, don't you, [pc.name]?</i>\"");
	...
	//If Bimbo Sophie is at Camp:
	else if (int(flags["COC.HEL_INTROS_LEVEL"]) < 4 && bimboSophie() && int(flags["COC.FOLLOWER_AT_FARM_SOPHIE"]) == 0) {
		flags["COC.HEL_INTROS_LEVEL"] = 4;
		output("As you help Hel string up her hammock between a few of the rocks inside your perimeter, you hear the tell-tale flapping of useless wings and a clattering of claws on the hard-packed dirt.  You brace for impact as your bimbo harpy prances up to you, planting a big, wet kiss on your cheek");
		//{PC lust goes up if not immunized to luststick}
		output(".");
		
		output("\n\n<i>\"Yay!  My special little cutie is back!</i>\"  Sophie exclaims, giggling as she wraps her feathered arms around you.  \"<i>Is my " + pc.mf("hunk","babe") + " interested in like, some 'fun'?\"</i>");
		
		output("\n\nYou're about to give your answer when suddenly Sophie is yanked off you, Hel's muscular arms locked around her neck.  \"<i>You keep your claws off my [pc.name], you feather slut!</i>\"  Hel growls, compressing Sophie's neck until the harpy bimbo squirms and gags, her flush face turning a deep blue.");
		
		output("\n\n\"<i>Hey, hey, HEY!  Break it up!</i>\"  You snap, harshly enough to make Hel drop the dumb blonde.");
		
		output("\n\n<i>\"What... what the FUCK is this thing doing here, [pc.name]?\"</i>  Hel yells, pointing an accusing finger at Sophie, who's sitting in a hapless pile on the ground, swaying slightly as her big chest heaves.  You try to explain, but Hel's having none of it - she seems <b>pissed</b>. <i>\"Look, [pc.name], I don't know why you have a harpy in your camp, and I don't care.  Just...  just get rid of her, alright?  I can't...  I refuse to be around this thing!\"</i>");
		...
		if (pc.intelligence() >= 40 || pc.intelligence() / 5 + rand(20) + 1 > 10) {
			output("Or, maybe there's a way to make this work...");
		...
		flags["COC.HEL_INTROS_LEVEL"] = 9001;
		mainGameMenu();
		return;
	}
	clearMenu();
...
private function bimboSophieGetsBooted4Firebutt():void {
	clearOutput();
	output("You sigh and pick Sophie up, slinging the harpy over your shoulders.  <i>\"Yaaaay~\"</i> she cheers.  </i>\"I'm going for a ride!\"</i>");
	...
	if (flags["COC.UNKNOWN_FLAG_NUMBER_00326"] >= 3) {
		...
		flags["COC.MINO_SONS_HAVE_SOPHIE"] = 1;
	}
...
private function bootHelOutForBimboSophie():void {
	clearOutput();
	output("<i>\"Nope, you get out,\"</i> you answer, scowling at the salamander.");
	
	output("\n\n<i>\"What.\"</i>  She says, deadpan.");
	
	output("\n\n<i>\"You.  Get.  Out.\"</i>");
	
	output("\n\n<i>\"What the fuck, [pc.name]?  Y-you're choosing a BIMBO over ME!?\"</i>");
	
	output("\n\n<i>\"Yeah.  Now get the fuck out.\"</i>");
	
	output("\n\n<i>\"I... well fine!  Fuck you anyway, [pc.name].  Fuck you!\"</i>");
...
private function workItOutWithSophieAndFireTits():void {
	clearOutput();
	output("You cross your arms and tell Hel to deal with it, remarking that you won't just dump Sophie out in the wilderness - she can't take care of herself - but you still want Hel around.");
	output("\n\n<i>\"I... But WHY, [pc.name]?  It's a harpy. They're evil!\"</i>");
	...
	output("\n\nHel sighs, and rolls her eyes.  <i>\"I dunno, [pc.name]. I don't like having her around, but - hey, get off - I guess if you're sure she's harmle- DAMMIT woman get your tits out of my face - I guess I can live with he- okay, okay, gimme a minute to settle in and I'll fuck ya already!  Damn!\"</i>");
	output("\n\nWell, maybe they'll get along after all...");
	flags["COC.KEEP_HELIA_AND_SOPHIE"] = 1;
	addNextButton();
}
...
public function angryHelAndIzzyCampHelHereFirst():void {
	clearOutput();
	output("Showing Isabella around, you eventually come to the chaotic, cluttered part of camp inhabited by Hel the salamander, who's currently sitting on her hammock sharpening her scimitar.");
	output("\n\nYou only have a moment to remember Hel's disdain for the cowgirl before...");
	...
	if (isabellaAccent()) output("V");
	else output("W");
	output("hat are YOU doing here?!\"</i>");
...
public function heliaFollowerMenu(display:Boolean = true):void {
	...
	if (int(flags["COC.HELIA_ANAL_TRAINING_OFFERED"]) == 0 && display && pc.biggestCockVolume() > heliaAnalCapacity()) {
		heliaAnalTrainingPrompt();
		return;
	}
	if (display) output("You call your salamander lover over, and in a few moments Hel walks your way, hips and tail swaying gaily as she moves.  She wraps an arm around your shoulders, pressing her soft, warm body against yours, and grins.  <i>\"Heyya, lover mine.  You need anything?\"</i>");
	clearMenu();
	...
	//}
	addButton(0, "Appearance", heliasAppearanceScreen, null, "Appearance", "Examine Helia's appearance.");
	if (pc.lust() >= 33) {
		addButton(1, "Sex", heliaRoughSex, null, "Sex", "Initiate sex with Helia.");
		addButton(2, "Threesomes", heliaThreesomes, null, "Threesomes", "Invite someone for threesomes activity with Helia!");
	}
	...
	if (flags["COC.HEL_LOVE"] == 1 || flags["COC.HEL_LOVE"] == -1) {
		if (pc.hasCock() && pc.cockThatFits(heliaCapacity()) >= 0 && pc.lust() >= 33 &&
				!helPregnant() && int(flags["COC.HELSPAWN_AGE"]) == 0) addButton(7, "Have A Kid", haveAKid, null, "Have A Kid", "Get Helia pregnant and start a family with her.");
	}
	addButton(14,"Back", campLoversMenu)
}

private function heliaOptions():void {
	if (helPregnancyEvent() >= 3 && int(flags["COC.HELIA_TALK_SEVEN"]) == 0) {
		heliaTalkSeven();
		return;
	}
	if (flags["COC.HELSPAWN_AGE"] == 1 && int(flags["COC.HEL_TALK_EIGHT"]) == 0) {
		heliaTalkEight();
		return;
	}
	clearMenu();
	addButton(0, "Discuss", talkToHel, null, "Discuss", "Talk to her about random topics.");
	if (hours >= 21) addButton(1, "Cuddle", hugASmokeyTail, null, "Cuddle", "Cuddle with Helia and sleep with her.");
	else {
		if (flags["COC.SLEEP_WITH"] == "Helia") addButton(1, "NoSleepWith", dontSleepWithHelia, null, "No Sleep With", "Stop sleeping with Helia, for now.");
		addButton(2, "Hug", hugASmokeyTail, null, "Hug", "Give that salamander bitch a hug. Bitches love hugs.");
	}
	if (flags["COC.HELIA_ANAL_TRAINING_OFFERED"] > 0 && flags["COC.HELIA_ANAL_TRAINING"] < 2 && pc.biggestCockVolume() > heliaAnalCapacity() && pc.hasItem(new CoCGoblinAle())) addButton(3, "Anal Train", heliaGapeSceneChoices);
	//addButton(4, "Futafication", talkAboutFuta, null, null, null, "See if she's in the mood to grow a new penis.");
	addButton(5, "Bathe", takeABath, null, "Bathe", "Swim in stream with Helia.");
	if (flags["COC.HELSPAWN_AGE"] == 1) addButton(7, flags["COC.HELSPAWN_NAME"], playWithYourKid, null, flags["COC.HELSPAWN_NAME"], "Spend some time with your salamander child.");
	if (flags["COC.HEL_GUARDING"] == 0) addButton(8, "GuardCamp", helGuardToggle, null, "Guard Camp", "Request her to guard the camp every night.");
	else addButton(8, "NoGuarding", helGuardToggle, null, "No Guarding", "Request her to stop guarding the camp.");
	addButton(14, "Back", heliaFollowerMenu);
}

private function dontSleepWithHelia():void {
	clearOutput();
	output("You decide not to sleep with Helia at night, for now.");
...
private function sparWithHeliaFirebuttsAreHot():void {
	clearOutput();
	output("Giving Hel a playful punch on the shoulder, you ask the salamander-girl if she'd be up for a little battle practice.");
	output("\n\n<i>\"Oh?  Well, it's certainly been awhile since you and I fought out on the plains...  Alright, let's do it, [pc.name]!  But heads up, I might just need to have my way with you after I push your face in the dirt!\"</i>");
	output("\n\nYou [pc.readyWeapon] and prepare for battle!");
	addNextButton(startHelCombat, true);
}

//Hel Whips [pc.name]'s Ass
internal function loseToSparringHeliaLikeAButtRapedChump():void {
	clearOutput();
	//If HP loss) 
	if (pc.HP() <= 1) output("You collapse on the ground, overwhelmed by pain and exhaustion caused by the berserker's onslaught.");
	else output("Your arousal is too great, and your mind can no longer focus on anything but a desperate need for release.  Lust robs you of your will, and with buckling knees you collapse.");
	
	output("\n\nYou look up to see Hel looming over you, slowly removing her bikini, revealing her ample breasts and a glistening cunt.  <i>\"What's the matter, [pc.name]?\"</i> she teases, tossing her leather thong onto your face, \"<i>Pick yourself up!  Fight me more!  Show me what my Champion can do!  COME ON!</i>\"");
	...
	output("\n\n<i>\"But, to the victor go the spoils,\"</i> she says, pushing her body against yours.  You can feel the dampness between her thighs, soaking into your [pc.gear].  She really gets off on violence, doesn't she?  <i>\"Come on, [pc.name], I'll even let you choose how you get me off...\"</i>");
...
internal function PCBeatsUpSalamanderSparring():void {
	clearOutput();
	//(If HP loss) 
	if (enemy.HP() <= 1) output("Unable to withstand your onslaught, the salamander collapses to a knee, barely supporting her weight on her sword.");
	else output("Panting heavily, knees snaking, she collapses, heavily leaning upon her sword.");
	
	output("\n\n<i>\"Come on, [pc.name],\"</i> Hel groans, swaying unsteadily.  ");
	if (enemy.HP() <= 1) output("<i>\"Kick my ass a little harder, why don't you?\"</i>");
	else output("<i>\"Turn me on till I can't think straight, why don't you?\"</i>");
	output("  You chuckle and offer her a hand up. The blushing Salamander takes it as you pull her into a tight embrace.  The cool wetness between her thighs a potent reminder of how much your lover seems to get off on violence.  Since she's so turned on, you could easily turn this into some rough loving.");
...
private function talkToHel():void {
	...
	if (int(flags["COC.FOLLOWER_HEL_TALKS"]) == 0) {
		output("You run a hand through Hel's hair and ask the recent addition to your camp if she'd like to talk.");
		output("\n\n<i>\"Sounds good to me, " + pc.mf("bud","babe") + ",\"</i> Hel grins, leading you by the arm to a rock near her hammock.  The two of you sit yourselves down, with Hel locking her fingers through yours in her lap.  <i>\"So what's on your mind, lover mine?\"</i>");
		
		output("\n\nFirst, you ask her how she's settling in.  She smiles at the question, <i>\"It's good to be here, [pc.name].  It's nice to know someone's got my back while I sleep, that I have someone who can take care of me if I get sick or hurt...  But most of all, I'm loving being so close to my best friend.\"</i>  She leans over and plants a little kiss on your cheek.");
		...
		//(If Toughness <50: 
		if (pc.tolerance() < 80 && pc.willpower() < 50) output("You cough and gag, eyes misting as the pure-grain booze burns your throat worse than hot coals.  Hel laughs riotously, slapping you on the back hard enough to cause booze to snort out through your nose.");
		else  output("You knock back the stein easily, your super-human endurance keeping you from choking on the powerful brew.  Impressed, Hel gives you a high-five and throws her head back in a laugh.");
		...
		output("\n\nHel nestles her head on your [pc.chest], wrapping her tail around your waist and curling up in your lap.  Smiling at your salamander, you wrap your arms around her and finish off your stein as Hel cuddles up.");
	}
	//Hel Talk 2
	else if (flags["COC.FOLLOWER_HEL_TALKS"] == 1) {
		output("Sitting Hel down, you ask if she's got any stories she'd like to share.  After all, she's been adventuring for years - surely she's got some fun tales to tell.");
	...
	//Hel Talk 3(C)
	else if (flags["COC.FOLLOWER_HEL_TALKS"] == 2) {
		output("Just as you're starting to speak, Hel suddenly throws her arms around your shoulders and pulls you into her lap.  You yelp in surprise as your salamander lover holds you tight, running a hand through your [pc.hair] as she settles onto the big rock near her hammock.  Giving a surprisingly girlish giggle, Hel nuzzles you, slipping her muscular legs and tail around your waist.");
		output("\n\n<i>\"Mine,\"</i> she purrs, nipping and kissing along the nape of your neck.");
	}
	//Hel Talk 4(C)
	else if (flags["COC.FOLLOWER_HEL_TALKS"] == 3) {
		//Hel Talk 4(C)
		output("You ask Hel if she has a few minutes to spare.");
		...
		output("\n\nThough she keeps her sword out, Hel walks tantalizingly close to you, her wide, swaying hips occasionally brushing against you, or letting her tail wrap around your [pc.legs] between steps.  As the two of you make it out on the plains, your lover begins to whistle, belting out a jaunty, soaring tune as she bats aside clumps of prairie grass with the flat of her scimitar.");
		...
		output("\n\nYou slip your hands around Hel's waist and let her draw you into a long, tender kiss.  She pulls you tight against her, her large, soft breasts pressing against your [pc.chest].  You cup her cheek, kiss her again, and let out a little gasp as Hel wraps her tail around you, holding you to her as she nestles her brow against yours, her red eyes gazing deeply into yours.");
		
		output("\n\n<i>\"Mmm, this is nice,\"</i> she whispers.  You smile and pull her down; you flop onto your back, bringing Hel with you, letting her nestle against your [pc.chest].  <i>\"Ah!  Even better,\"</i> she chuckles, cuddling up around your body.  You run a hand through Hel's long, red hair.  She purrs like a cat, happily moving her scalp for you to get a better angle.");
		
		output("\n\n<i>\"Hey, [pc.name],\"</i> Hel says after a long while, shifting to bring her face over yours.  By way of answer, you lean up and give her a little kiss.  She grins.  <i>\"I dunno if I've told you this lately, but...  Well,\"</i> she laughs, stroking your cheek, <i>\"there's no one I'd rather lie in the sun with than you.\"</i>");
		output("\n\nYou pull Hel down into another long, tongue-filled kiss.");
	}
	//Hel Talk 5(C)
	else if (flags["COC.FOLLOWER_HEL_TALKS"] == 4) {
		output("<i>\"Hey, [pc.name],\"</i> Hel says, slipping out of your grasp with a come-hither wag of her finger, <i>\"wanna have a drink with me?\"</i>");
		...
		if (!helPregnant()) output(", her eyes crossing a little.  Looks like she was dipping in before you got here...");
		else output(".");
		...
		{ 
			...
			//if Kiha is in camp: 
			if (followerKiha()) output("  From across camp, you hear a certain dusky dragoness shout <i>\"HEY! That's MY trick, firebutt!\"</i>");
		
			output("\n\nHel giggles, and spews another fireball into the sky.  <i>\"Heh.  Hey, this is pretty fun.  C'mere, [pc.name], try it!\"</i>");
			...
			if (followerKiha()) output("further pissing off Kiha, who throws a rock at Hel, and ");
			output("shooting up like a beacon over the wasteland.");
		}
		else {
		...
		output("into a hug, mashing your face into her warm, soft bosom.  You struggle for air, finally bursting out of her tight cleavage.  Hel chuckles, running her hand through your [pc.hair] as her fiery tail wags gaily behind her.");
	...
	//Hel Talk 6 (Needs Isabella and Kiha at camp; at least 1 gem)(C)
	else if (flags["COC.FOLLOWER_HEL_TALKS"] == 5 && pc.credits >= 10 && isabellaFollower() && followerKiha()) {
		//output(images.showImage("helia-follower-poker"));
		var gems:int = 0;
		output("<i>\"Hey, [pc.name],\"</i> Hel says with a sly grin.  <i>\"Me, Izzy, and spitfire were just playing a little game.  Wanna deal in?\"</i>");
		...
		if (!isabellaAccent()) output(", obviously only somewhat interested");
		output(".");
		...
		//{Make an INT check}
		if (rand(20) + 1 + pc.intelligence() / 5 >= 15) {
			//[Successful]
		...
		if (pc.credits / 10 + gems < 0) gems = pc.credits / 10;
		if (gems < 0) output("\n\nYou lost " + gems + " gems in the game!");
		else output("\n\nYou gained " + gems + " gems in the game!");
		pc.credits += gems * 10;
	}
	else {
		flags["COC.FOLLOWER_HEL_TALKS"] = 1;
		talkToHel();
		return;
	}

	if (flags["COC.FOLLOWER_HEL_TALKS"] == undefined) flags["COC.FOLLOWER_HEL_TALKS"] = 0;
	flags["COC.FOLLOWER_HEL_TALKS"]++;
...
private function shutUpHelTalks():void {
	clearOutput();
	output("Quickly you interject before Helia can continue with the story; you've heard quite enough.  <i>\"Whaaaat?  We were just getting to the good part...\"</i>  Hel whines, rolling her eyes.  <i>\"Hey, you're the one who asked!\"</i>");
	...
	if (pc.hasCock()) {
		...
		output("\n\nSuddenly, Hel flips herself into your lap, straddling your [pc.legs].  <i>\"Don't worry though, lover... Your dick's still the best!\"</i>  She plants a quick kiss on your lips before bounding off to another part of camp.");
	}
	//{If PC is cockelless: 
	else {
...
private function hugASmokeyTail():void {
	...
	if (hours >= 21) {
		output("As the sun sets over the camp, you see Helia standing over her hammock, stretching and yawning, ready to turn in for the night.  You approach her, sliding your arms around her supple waist and burying your face in her soft crimson locks, holding your lover close.  Hel giggles girlishly as you give her a long hug, nuzzling into the nape of her neck.");
		output("\n\nWith a bit of effort, Hel turns around in your embrace and starts to fiddle with your [pc.gear], slowly pulling it off, leaving your bare flesh pressed against her own.  You breathe in the woodsmoke scent of her hair; rub your " + pc.skinFurScales() + " along her smooth flesh; gasp lightly as her long tail wraps lovingly around your [pc.legs], drawing you even closer, letting your face rest against her yielding chest.");
		...
		if (pc.hasTail()) output("s");
		output(" intertwined.  <i>\"Oh, this is nice,\"</i> Hel laughs, running her long, scaled fingers through your hair.  <i>\"So, you wanna stay with me tonight, lover mine?  I'd appreciate the company....\"</i>");

		output("\n\nBefore you can give an answer, Hel presses her lips to yours, her breath coming hot against your face as her hands run across your back and [pc.butt].");
		
		output("\n\nHel smiles prettily as you give your assent.  <i>\"On a night like this, there's no one I'd rather be with, [pc.name]...\"</i>\n\n");
		flags["COC.SLEEP_WITH"] = "Helia";
		clearMenu();
	...
	//[If PC is <6ft Tall]
		...
		output("\n\n<i>\"Oh, [pc.name],\"</i> Hel sighs happily, holding you back against her bosom.  <i>\"My cute little [pc.name].\"</i>");
	}
...
public function sleepyNightMareHel():void {
	output("\nYou awake, finding yourself covered in a sheen of sweat.  Groggily, you peel your eyes open as the dangerously warm body beside you squirms in your grip, moaning quietly as her tail thrashes around your [pc.legs].");
	
	output("\n\n<i>\"M-mooom,\"</i> Hel breathes, just loud enough to hear, her entire body twitching, recoiling from some imagined horror.");
	
	output("\n\nGently, you stroke her cheek, giving her sleeping mind what comfort you can.  After a few moments, Hel relaxes in your arms.  She rolls over, nuzzling her cheek into your [pc.chest] and, murmuring your name, begins to snore peacefully.\n");
}


//Rough Sex
//Into Text
private function heliaRoughSex(display:Boolean = true):void {
	if (display) {
		clearOutput();
		output("<i>\"Mmm, need to blow off some steam, lover?\"</i>  Hel grins, reaching around to undo the straps of her scale bikini.  <i>\"Well, I'm all for that!\"</i>");
	}
	var buttons:int = 0;
	//(Display Options: 
	...
	if (pc.hasCock() && pc.lust() >= 33) {
		...
		if (pc.cockThatFits(heliaCapacity()) >= 0)
		{	
			addButton(buttons, "FuckVag", beatUpHelAndStealHerWalletFromHerVagina, null, "FuckVag", "Penetrate her vaginally.");
			buttons++;
		}
		//85 ass capacity
		if (pc.cockThatFits(heliaAnalCapacity()) >= 0)
		{
			addButton(buttons, "Anal", fuckHelsAss, null, "Anal", "Penetrate her anally.");
			buttons++;
		}
		if (buttons < 14) {
			addButton(buttons,"Get Blown", helBlowsYou, null, "Get Blown", "Have her suck you off..");
			buttons++;
		}
		if (pc.cockThatFits(heliaCapacity()) >= 0 && pc.cockThatFits2(heliaCapacity()) >= 0) 
		{
			addButton(buttons,"DoublePen", dpHel, null, "DoublePen", "Fill both of her holes with your cocks.");
			buttons++;
		}
		if (buttons < 14) {
			addButton(buttons, "Tail Wank", helTailWanksYourDickBecauseSheLovesYouDesuDesuHoraHora, null, "Tail Wank", "Have her jerk you off using her tail.");
			buttons++;
		}
	}
	if (pc.hasVagina() && pc.lust() >= 33) {
		addButton(buttons, "GetLicked", getLickedByHel, null, "GetLicked", "Have her lick your vagina.");
		buttons++;
	}
	if (pc.lust() >= 33) {
		addButton(buttons, "TailPeg", helTailPegging, null, "TailPeg", "Have her peg your ass.");
		buttons++;
	}
	//if (pc.lust >= 33 && flags[kFLAGS.HELIA_FUTA] > 0) {
	//	addButton(buttons, "Get Fucked", helScene.getFucked, null, null, null, "Have her penetrate you anally with her dick.");
	//	buttons++;
	//}
	//Morph-based: [Possession] [Mount Her] [Hanging 69] [Coil Her Up] [Tentafuck])
	if (pc.lust() >= 33 && pc.isTaur()) 
	{
		if (pc.hasCock()) 
		{
			if (pc.cockThatFits(heliaCapacity()) >= 0) {
		...
		if (pc.hasKeyItem("Centaur Pole") && pc.hasVagina()) {
			addButton(buttons, "CentaurToy", femtaurPlusCampHel);
			buttons++;	
		}
	}
	if (pc.lust() >= 33 && pc.hasPerk("Incorporeality") && izmaFollower() && int(flags["COC.IZMA_NO_COCK"]) == 0) {
		addButton(buttons, "Possess", heliaCampPossession);
		buttons++;
	}
	if (pc.lust() >= 33 && pc.cockTotal(GLOBAL.TYPE_TENTACLE) > 6) {
		addButton(buttons, "Tentacles", heliaFollowerTentafuck);
		buttons++;
	}
	if (pc.lust() >= 33 && pc.isNaga()) 
	{
...
private function heliaCampPossession():void {
	clearOutput();
	//output(images.showImage("helia-follower-sex-possession"));
	output("With a coy grin, you let your ghostly attributes go transparent, and ask Hel if she'd mind taking an extra passenger for a little bit.");
	
	output("\n\n\"<i>Oh, that's too spooky for me, lover mi - HEY!</i>\" she yelps as you dive into her.  Your lover flails around for a minute before settling down, grudgingly allowing you to take the wheels.  Internally, you hear her say: \"<i>Alright, alright, I trust you. Have fun, [pc.name].</i>\"  Now having assumed direct control of the salamander, you spend a few quick minutes getting adjusted to your slutty new body.  You run your scaled, clawed hands across Hel's smooth, taut body, giving your gropable ass and big, soft tits each a squeeze.  To your delight, even that slight stimulation is enough to cause a little trickle to escape the band of Hel's bikini bottom, staining your thighs with her lubricant.  Slutty body indeed.");
...
private function possessIzma():void {
	...
	output("\n\n\"<i>No,</i>\" you say, grabbing Izma by the hair and forcing her shark-like face into your cleavage.  \"<i>You don't get to cum.  Weak, slutty little whores don't get to cum without their [pc.master].</i>\"");
...
private function inCampHelNagaLuv():void {
	clearOutput();
	//output(images.showImage("helia-follower-sex-nagamale"));
	var x:int = pc.cockThatFits(heliaCapacity());
	if (x < 0) x = pc.smallestCockIndex();
	output("You slither over to the salamander and circle around her, your serpentine lower body trailing around her legs as you sweep Helia into a tight embrace.  She grins wolfishly, pressing her lips to yours; you let her tongue enter your mouth, entwining with your tongue, letting her distract herself as you coil more and more of your snake-body around the salamander.  By the time Hel realizes what you're up to, there isn't much she can do about it - you grab her arms and press them to her sides as you bind her up past the waist, wrapping your lover in your lengthy, serpentine form.");
	...
	output("\n\nWell, alrighty then.  Popping [pc.oneCock] out of your genital slit, you give it a little stroke; though it hardly needs any encouragement to get hard, with the femlube soaking into you and Hel's big tits resting atop your binding body.  You release her from your naga-hug, and Hel grunts as she flops onto her back, her legs spread perfectly for you.  In between her soaked thighs you slither, plopping your " + pc.cockDescript(x) + " onto her twat and sliding yourself between her lips in a single, fluid motion.  Hel rolls her head back and moans as you enter her, digging the very tips of her claws into the " + pc.skinFurScales() + " of your back.");
	...
	output("\n\nYou reach up and grab your lover's shoulders, pulling her face down onto your [pc.chest] before rolling over, constricting her legs in a layer of naga tail, binding her with your tip and cock buried inside her.  Hel gasps, but after a moment begins to help you, guiding your lengthy, serpentine body around and around your entwined bodies until the two of you are utterly bound together, locked in intimate embrace.  You lie back, using your tail as a warm, comfy bed; Hel cuddles up atop you, managing to squirm until her arms are wrapped tightly around you.  With a kiss, you slip your tongue into Hel's mouth as you begin to move your body, rolling your hips into hers and wriggling your tail, doubling Hel's pleasure.  She gasps and moans, starting to fully tongue-fuck you as you ream her holes.");
	
	output("\n\nUnable to move much in your serpentine embrace, Hel begins to rapidly contract her vaginal and anal muscles, squeezing and grinding against your " + pc.cockDescript(x) + " and tip.  Content to close your eyes and bask in the pleasures of your lover, ready to lose yourself in pleasure.  You relax, leaning back on yourself as you buck your hips into your lover's sweet hot box, flick your tail inside her ass, and play your tongue around her mouth, tasting her warm saliva and the aftertaste of beer.");
	
	output("\n\nYou cum easily, only a small grunt escaping your lips as Hel's delicious cunt brings you over, pulling your " + pc.cockDescript(x) + " deep into her and tightening your hold around her.  The sudden squeeze is the last hurrah as you empty your ");
	if (pc.balls > 0) output("[pc.balls]");
	else output("load");
	output(" well into her womb.  Your tail spasms as you cum, running wild inside Hel's rectum until she cums too, yelping into your open mouth as she climaxes a moment after you, girlcum squirting out of her twat until your [pc.hips] are thoroughly soaked.");
	
	output("\n\nHaving cum, you stroke Hel's cheek and kiss her again, this time a short, sweet peck on the lips that leaves her smiling lovingly at you.  Hel nuzzles against your [pc.chest], content to snuggle in your reptilian hug; you run your fingers through her thick, rich red hair, and yawn as her comforting body heat flows through you.");
...
private function nagaCoilForHelCampWithGirls():void {
	clearOutput();
	//output(images.showImage("helia-follower-sex-nagafemale"));
	output("You slither over to the salamander and circle around her, your serpentine lower body trailing around her legs as you sweep Helia into a tight embrace.  She grins wolfishly, pressing her lips to yours; you let her tongue enter your mouth, entwining with your tongue, letting her distract herself as you coil more and more of your snake-body around the salamander.  By the time Hel realizes what you're up to, there isn't much she can do about it -- you grab her arms and press them to her sides as you bind her up past the waist, wrapping your lover in your lengthy, serpentine self.");
	
	output("\n\n\"<i>Awwww,</i>\" she says, leaning over to give you a kiss on the cheek, \"<i>Nagas give the best damn hugs...</i>\"");
	
	output("\n\nYou slip inside your coil, pressing your [pc.chest] against Hel and giving her a long, tongue-entwining kiss.  After a few minutes, though, you break the kiss and slither downwards, cupping one of Helia's large, soft breasts in your hands and putting your lips to it.  She lets out a soft gasp as you gently nip at her teat.  \"<i>Gah! R-right there, [pc.name].  Oh, that's nice.  That's niiiice,</i>\" she moans, pulling one of her arms free to stroke your hair and give you a gentle nudge into her yielding flesh.  You eagerly suckle at her sensitive bud, licking and biting from one breast to the other until Hel is positively trembling with sensation.");
...
private function centaurMountsCampHel():void {
	...
	if (x < 0) x = pc.smallestCockIndex();
	clearOutput();
	//output(images.showImage("helia-follower-sex-mount"));
	output("You trot around your lover, your powerful centaur frame brushing lightly against her.  Hel grins, reaching out to pet your flanks and running her claws across your " + pc.skinFurScales() + ".  \"<i>You've got a helluva body,</i>\" she smiles, and with an invitation from you, swings up onto your back, wrapping her arms tightly around your waist as you break out into a canter around camp.");
	
	output("\n\nShe nudges you toward her part of camp.  When you arrive, Hel leaps off and grabs some ropes from her pack.  You raise an eyebrow, but allow her to tie them around your equine half, creating a makeshift harness underneath you.  Once done, Hel lowers herself to her knees and, pushing aside your [pc.gear], takes hold of your " + pc.cockDescript(x) + ".  She gives it an experimental stroke, running her warm, leathery palm and scaled fingers across your length until you let out a little whinny and stamp your hooves.");
	
	output("\n\nHel pats your flank with her free hand, but doesn't let up, instead adding her long, slender tongue to the mix.  She flicks it along the [pc.cockHead " + x + "] of your " + pc.cockDescript(x) + ", sending shivers of pleasure up your rigid shaft.  You ache to do something here, to add to your own pleasure or return it to your partner, ");
	//if no boobs: 
	if (pc.biggestTitSize() < 3) output(" but your body is too big, and you must content yourself to endure Hel's ministrations");
	else output(" but all you can do is take hold of your [pc.chest] and tease and tweak your [pc.nipples], getting what pleasure you can from your sensitive teats");
	output(".");
	
	output("\n\nAfter a few mind-hazing minutes of salamander hand-job, Hel takes the [pc.cockHead " + x + "] of your cock into her mouth, slurping up the tip of your " + pc.cockDescript(x) + ".  Your breath catches in your throat as the first inches of your cockmeat are engulfed in the hot, wet embrace of Helia's mouth.  Her lizard-tongue wastes no time in flicking across your [pc.cockHead " + x + "], the forked tip playing across your urethra, drawing forth a thick dollop of pre that soon smears her cheeks with white.  She pumps your cock into her mouth, licking and sucking on your crown as her pistoning hands force thicker globs of sticky precum out of you, eagerly swallowing every drop - but only after savoring it, rolling each drop around in her mouth until her entire gob is a sticky mess, thick webs of seed coating her lips each time she swallows another appetizing taste.");
	
	output("\n\nThen as quickly as it began, Hel's blowjob comes to a halt.  The salamander pops your prick out of her maw and, giving it one last kiss on the [pc.cockHead " + x + "], lets it fall aside as she moves.  You lean around yourself awkwardly as your lover ducks beneath your horse-half, clambering into her harness.  Hel disappears from view, completely obscured by your own body; all you can feel of her is her weight pulling down on your back and her powerful scaled legs hooking around your thighs, spreading wide as she grabs your cock again.  You groan pleasurably as the tip brushes her lower lips, the heat of her inhuman pussy radiating out to set your nerves ablaze.");
	...
	if (pc.cockVolume(x) > heliaCapacity()) output("she can cram in, the rest of your length twitching between her legs");
	else output("you have to offer her");
	output(".  Finally, though, you manage to settle down, your breath coming ragged as Hel's vaginal muscles clamp and release, gently massaging your shaft with expert movements, always keeping you on edge until...");
	
	output("\n\nTHWACK!");
	
	output("\n\nYou start forward as a rough tail-slap lands on your flank.  You reflexively step forward, your equine muscles acting on instinct...  which only serves to rock Hel off and down again on your " + pc.cockDescript(x) + ", her impromptu harness moving her like a swing. You arch your back as the movement hammers Hel down on your cock like a free-swinging onahole, but the moment of pleasure lurches out of control when she gives you another hard tail-swat, and another; your insticts carry you forward until you reach a full canter. Shudders and twitches wrack your body as your own motions rock Hel on and off your [pc.cock], slamming her back down on your shaft again and again.");
...
private function femtaurPlusCampHel():void {
	clearOutput();
	//output(images.showImage("helia-follower-sex-femtaur"));
	output("You trot around your lover, your powerful centaur frame brushing lightly against her. Hel grins, reaching out to pet your flanks and running her claws across your " + pc.skinFurScales() + ".  \"<i>You've got a helluva body,</i>\" she smiles, and with an invitation from you, swings up onto your back, wrapping her arms tightly around your waist as you break out into a canter around camp.  She gives you a gentle nudge toward a certain pile of your gear, and it doesn't take but a moment for you to notice she's urging you toward the special toys Whitney gave you for your equine-like body's needs.");
	
	output("\n\nYou quickly set up the imp-like centaur pole, its thick horse-shaft easily adjusted to the right height for your [pc.vagina].  You stand for a moment in the presence of the horsecocked imp, its huge hooked nose and wide-brimmed hat casting a maleficent shadow across your back as you line yourself with its plasticy manhood.  As the lips of your femininity brush against the stiff flat head of the false horsecock, Hel slips back along your lean frame, resting her belly and full breasts on your centaur back, her muscular arms wrapping tight around your chest.");
	
	output("\n\nA small gasp escapes your lips as you feel a tiny, red-hot point pressing against your [pc.asshole] as the tip of Hel's tail comes to rest atop the plastic horsecock, brushing across your most sensitive parts with tantalizing slowness.  \"<i>Come on, pretty girl. Let's give both your holes a thrill!</i>\" Hel laughs, patting your side encouragingly.  Slowly, you start to back up, shivering as you feel the two intruders press against your twin holes.  You clop back, a shudder wracking your body as the tip of Hel's tail pierces your [pc.asshole] and the pole's cock spreads your womanly lips, entering your [pc.vagina].");
	
	output("\n\nYou pause a moment, reveling in the intense fullness, the gaping half-pain of the two thick shafts stretching your holes.  A long moment passes by as your body tries to adjust to the tail and dildo stuffing you; only as your holes stop spasming do you start to move again, backing slowly up toward the pole, letting more and more plastic and scale into you.  It takes nearly a minute for you to finally stuff the entirety of the shaft's length into your cunt, its massive length mirrored by Hel's thick tail now stuffed wholly into your ass, stretching you wide as the appendage twitches and squirms, teasing your inner walls.  As your [pc.butt] presses against the towering Centaur Pole, you start to move, but a quick swat on the flank from your lover stops you.");
	...
	output("\n\nYou yelp as the imp-statue's hands suddenly reach out and grasp your [pc.hips].  Your eyes go wide as you feel the thick dildo withdraw from your cunt, the imp's hips pulling back.  You look back, you and Helia both gazing over your shoulders as the magic statue readies itself to join in the fun.  To your surprise though, it withdraws until just the tip remains inside you.  Its rubber cock seems to part, and a second, equally-sized cock appears above it, as if from nowhere.");
...
private function heliaFollowerTentafuck():void {
	clearOutput();
	//output(images.showImage("helia-follower-sex-tentacles"));
	output("With a flourish you pull your [pc.gear] off, exposing your writhing mass of tentacle-cocks for Hel`s enjoyment.  \"<i>Wow, think you got enough of th--urf!</i>\" the salamander starts to say before you shut her up with one of your tentacle pricks, lashing forward and plugging her mouth.  Hel staggers back, flailing around before grabbing the cock you've shoved down her throat and giving it a rough tug.  Just as she nearly succeeds in pulling it out, your other wriggling members wrap around her thick thighs; you pull her in toward you, grabbing her shoulders as your pricks peel off her bikini bottom to reveal the smooth face of Helia's crotch.");
	...
	if (pc.balls > 0) output("[pc.balls]");
	else output("wriggling tentacles");
	output(" feel painfully empty and white slush pours from all her holes, painting the ground white beneath her dangling feet.");
...
private function boxWithInCampHel():void {
	...
	output("\n\nYou toss your [pc.gear] aside and assume your own battle stance, your powerful muscles tightening beneath your " + pc.skinFurScales() + ", ready to spring into action.  You move slowly, cautiously; you and Hel begin to circle each other like predators observing their prey, each watching the other for any momentary lapse, any split-second vulnerabilities.  Suddenly, Hel lashes out, a quick one-two jab at you, narrowly blocked in your guard.  You grind your feet into the ground, deflecting her inhumanly-strong blows as best you can.  She launches a third strike, but you duck aside, spinning into her guard and delivering a hard jab to her gut.  The salamander recoils, swinging her fiery tail at you as she gets some distance, staggering back from the hit.  You exchange another quick flurry of blows, punches connecting, blocked, or dodged in rapid succession.  The fight drags on, neither of you able to land a telling blow, your strikes deflected or dodged, your prowess in battle nearly matched.");
	pc.energy( -5 - rand(20));
	//If INT check succeeds: 
	if (pc.intelligence()/20 + 1 + rand(20) >= 13) {
		...
		output("\n\n\"<i>Alright, alright,</i>\" Hel groans, settling down beneath you.  \"<i>You win... this time!</i>\"  Her smirk widens even more as your eyes jump wide; the familiar, naughty feel of clawed hand grabbing at your [pc.butt].  Hel licks her lips with a slow, sultry motion and asks, \"<i>Now then, lover mine, to the victor go the spoils...</i>\"");
	...
	else if (pc.physique()/20 + 1 + rand(20) >= 13) {
		output("\n\nYou tire of the battle, though, and decide to end it swiftly.  You put your well-trained muscles to good use, lashing out with a single, powerful blow that stuns the salamander, knocking her back.  You follow through with a trio of hard punches to the ribs, pounding Hel until she gasps for breath and stumbles, collapsing in a heap.  You're on her in an instant, pinning her down with your [pc.legs], putting your whole weight down on her so that no matter how hard she struggles, she cannot push you off.");
		
		output("\n\n\"<i>Alright, alright,</i>\" Hel groans, settling down beneath you.  \"<i>You win... this time!</i>\"");
		output("\n\nYour eyes jump wide as you feel a familiar clawed hand grab a handful of your [pc.butt].  Hel licks her lips with a slow, sultry motion and asks, \"<i>Now then, lover mine, to the victor go the spoils...</i>\"");
		//Display Rough Sex menu
		pc.slowStatGain("p", 2);
		heliaRoughSex(false);
		addButton(14, "Leave", returnToCampUseOneHour);
	...
	//If PC fails the Check:  
		...
		output("\n\nAs you desperately fend off the salamander's flurry of blows, though, you feel yourself wearing down.  Her punches start getting through your guard, scaled knuckles hammering into your flanks and gut, too many and too hard to withstand.  Eventually, battered and bruised, a particularly hard blow throwing you to the ground.  You land with a groan, and a moment later feel Hel's soft, leathery sole pressing down on your [pc.chest].");
		output("\n\n\"<i>Ha! Not today, [pc.name]! You're good, lover, but you're gonna need to practice a lot more to beat me!  Now then.... what shall the victor do with her spoils?</i>\"");
		//[Proceed to Lose to Hel sex scenes]
		//dynStats("tou", 2);
		pc.slowStatGain("r", 2);
		if (pc.lust() >= 33 && pc.hasCock() && rand(2) == 0) {
			if (pc.cockThatFits(helCapacity()) >= 0) {
				muddyLizardFeet();
				return;
			}
		}
...
private function muddyLizardFeet():void {
	clearOutput();
	//output(images.showImage("helia-follower-muddylizardfeet"));
	output("Suddenly, Hel lunges forward, grabbing you by the waist and tossing you both to the ground. With an \"<i>OOPH!</i>\" both of you go tumbling, rolling hard as Hel's toss carries you down a steep slope.  With a splash, you find yourself rolling in the river near camp, the water only a few inches deep, just enough to fill your ears as your salamander lover pins you down, grinning wolfishly at you.");
	
	output("\n\n\"<i>You're mine, [pc.name]!</i>\" she laughs, struggling to stand, one soft sole planted firmly on your chest, pressing you down into the muddy soil beside the stream.  You groan as the salamander looms over you, hands planted firmly on her wide hips.  \"<i>Now... what should I do with my weak little prize, hmm? Should I... oh, why. hello there,</i>\" Hel chuckles as your [pc.cock " + pc.biggestCockIndex() + "] stiffens unconsciously, so used to the touch of Helia's body signaling imminent pleasure that her holding you down in the mud sparks a rather confused boner.");
	
	output("\n\nHel grins down at you and shifts her weight onto the foot that had been on your chest, her other coming up between your [pc.legs], the smooth flats of her claws caressing the underside of your turgid cock, quickly bringing it to full stiffness.  She coos merrily as her claws stroke the stiff shaft of your manhood, leaving thick, slimy trails of salamander-warmed mud trickling down your prick.  Seeing your reaction - or at least, that of your rock-hard [pc.cock " + pc.biggestCockIndex() + "], Hel presses down ever so slightly, moving her heel to the base of your cock and flattening it against your belly.  A little gasp escapes your mouth as your lover applies the tiniest amount of pressure, leaning in to tighly press your rod against you.");
	...
	if (pc.balls > 0) output("her heel coming to caress your taut sack");
	else if (pc.hasVagina()) output("her heel brushing against the nub of your [pc.clit]");
	else output("her heel going so low as to brush your taint, making your [pc.asshole] clench in surprise before she moves back up your length");
	output(".  You groan lustily as her toes and soft sole jerk you off, gently urging the tight knot of pleasure in your gut to fruition.  Cupping one of her hefty tits, Hel licks her lips as the first thick smears of pre leak from your cock's [pc.cockHead " + pc.biggestCockIndex() + "], smearing onto her long-clawed toes.");
	
	output("\n\nJust as you're about to bust your nut onto Hel's foot, though, the sultry salamander ceases her sensual movements.  With a wry grin, she grabs your shoulders and pulls you up into a sitting position before swinging around behind you, wrapping her powerful thighs around your waist.  Wrapping her arms tightly around your [pc.chest], fingers playfully toying with your [pc.nipples], Hel trails kisses down your neck and shoulders, nibbling at the sensitive skin even as both her feet lock around your still-muddy [pc.cock " + pc.biggestCockIndex() + "].  You gasp at the sudden warm embrace of her soft soles, shuddering as her soil-slicked feet start to work up your length, massaging your cock between her arches.");
	
	output("\n\n\"<i>Oh, [pc.name],</i>\" she moans, squeezing your chest against herself, shifting to let your head rest against her big, soft breasts.  She leans down to kiss you, her lizard-like tongue slithering into your mouth to play across your own.  You return her kiss, reaching down to caress her leathery-scaled legs, brushing your fingers up along her thighs until her breath catches in her throat.  Just as Hel starts to pick up the pace, bringing you closer and closer to the edge of bliss, you twist around in her grip, shifting to face the salamander.  Hel gasps, but quiets instantly when you take her in your arms and kiss her, pushing her back into the muck with the intensity of your passion.");
	
	output("\n\nHel's legs wrap back around your [pc.hips], pulling you down against her, your slick cock grazing the hot slit of her pussy, just enough to part her lips ever so slightly.  Your lover breaks the kiss that binds you just long enough to moan your name, throwing her head back and clutching to you as you finally enter her, your cock spearing into her fiery depths with a slow, unyielding thrust.  You groan and moan, breath coming ragged as your [pc.cock " + pc.biggestCockIndex() + "] slides into the salamander's cunt.  You forge ahead, pressing your manhood into your lover until ");
	if (pc.biggestCockVolume() > heliaCapacity()) output("you feel your [pc.cockHead " + pc.biggestCockIndex() + "] against the lips of her cervix, as fully into her as you can go");
	else output("your groin meets Helia's, your [pc.cock] wholly enveloped within her");
	output(".  Before you can think to withdraw, Hel tightens her hold on you, whispering huskily, \"<i>Just leave it... just for a second, [pc.name].  Oh, God,</i>\" she moans, her eyes closing as she savors the feeling of you buried so deep inside her, your slick prick twitching as her walls massage it, milking it of the precum that freely flows from your tip.");
	...
	output("\n\nGrinning, you start to build up to a rougher pace, starting to fuck Hel with greater speed and power.  Your rhythm intensifies, as does the salamaner's grip on you, until you feel her claws digging into your back.  \"<i>Yeah, come on, [pc.name]!  Just like that!  Just... like... that!</i>\"");
	
	output("\n\nBefore you can blink, Hel shifts her grip on you and rolls you over, suddenly planting you back in the mud, leaving her on top, straddling your [pc.hips], cock half-buried inside her.  She gives you a wink as she lifts up, letting your [pc.cockHead " + pc.biggestCockIndex() + "] taste the cool air.  You shudder with the sudden frigid sensation along your manhood as the salamander turns around, whispering, \"<i>I'm not ready for this to end just yet....</i>\"  She comes back down a second later, reaching back to spread her cheeks so that your [pc.cock " + pc.biggestCockIndex() + "] slides readily into her taut, muscular butt, trapped behind her as she gently grinds on you.  You groan, brought down from your near-orgasmic high by the sudden cold; Hel's butt is nothing compared to the sweltering embrace of her inner depths.");
	
	output("\n\nWinking at you over her shoulder, Hel grabs your [pc.legs] for support as she starts to move, dragging your still-slick cock between her ass-cheeks.  She wastes no time with slowness, immediately bucking up and down on your fuckpole with all the speed she can muster, rutting like mad on top of you.  She's panting lustily in moments, her long tail swinging around to tease her empty twat, slipping a few inches in at the bottom of each bounce.  Unwilling to simply sit passively after you nearly had the both of you cumming buckets, you lean up and grab the slutty salamander's ass, sinking your fingers into her plush butt so hard she yelps.  You give her a little swat, chuckling as her flesh jiggles - and the impact shakes through her, making her clench around your prick.");
	
	output("\n\nBefore you can try that again, though, Hel takes your hands in hers and guides them up to the soft mounds of her mammaries, letting you get something real to sink your fingers into.  You squeeze her tits roughly, teasing her stiff teats with a pinch or twist between each thrust, forcing little gasps and moans from her throat.  Soon, Hel's tongue is lolling out of her mouth, her back arching as she undulates atop you, squeezing hard on your [pc.cock " + pc.biggestCockIndex() + "] as you lean up to press your [pc.chest] into her back.");
	
	output("\n\n\"<i>Oh, shit,</i>\" she groans, \"<i>Just fuck my tits, gimme a boobgasm, c'mon! [pc.name]!</i>\" she cries, throwing her head back, nearly ready to cum just from your savaging her breasts and the tender ministrations of her own tail.  Well, you can't have that yet!");
	
	output("\n\nStill holding Hel by the tits, you roll her over face-down into the mud.  You grab her hips, holding her still enough to get a few more rough thrusts into her asscheeks before you decide on the real prize.  At the apex of a thrust, you angle down, the tip of your prick battering into Hel's dark star.  She gasps, clenching down on the crown of your cock; but she's a regular buttslut, and another thrust brings you inside her.  Helia cries out, screaming her pleasure as you bury your thick prick deep into her backside.  \"<i>Oh, FUCK yeah,</i>\" she finally manages to say as your hips push against her butt, cock fully inside her.  \"<i>G-God yes, [pc.name], fuck my ass... you know I love it... Love you...</i>\"");
	
	output("\n\nYou run your fingers through her red tresses, as gently as you can even as you withdraw from her ass, taking yourself half way out before starting back inside.  A few more thrusts and you're righteously fucking your piece of hot salamander ass, pounding away at Hel's butthole with swift, powerful strokes that each force a cry from her lips, or a sultry moan of your name, and how much she loves you and the cock buried up her ass.  You let one of your hands drift upwards, taking her by the shoulder, helping her up onto all fours - all the better to hammer her ass.  To your delight, though, she grabs your hand and brings it to her mouth, showering it with kisses and gentle bites, suckling from your digits like little cocks, her tongue wrapping around them and squeezing with every hammerfall that brings your [pc.cock " + pc.biggestCockIndex() + "] pounding back into her.");
	
	output("\n\n\"<i>N-not yet,</i>\" Hel groans, \"<i>Just a little longer...</i>\"");
	
	output("\n\nYou grin down at her, but the salamander twists in your grip, popping off your cock so quickly that a thick pool of pre leaks out of her backside.  She rolls onto her back and grabs your [pc.legs], pulling you onto her so that you straddle her big, soft bust, your cock flopping into the slick mud coating her breasts.  She grins up at you, white fangs gleaming on a darkened face as she cups her tits, pushing them together around your throbbing shaft, so close to release that you want to scream.");
	
	output("\n\nHelia plants a dainty kiss on your [pc.cockHead " + pc.biggestCockIndex() + "] before starting to move, dragging her breasts up and down your muddy shaft, compressing the soft, yielding orbs around your turgid rod.  She's going slow, at first, letting you down easy from your orgasmic heights.  Letting your heart calm, you rest your hands on her shoulders, caressing her cheek and chest as her hands guide the heavy orbs of her tits to your pleasure. \"<i>Mmm, [pc.name],</i>\" she whispers, letting her eyes flutter closed.");
	...
	output("\n\nYou brush Helia's mud-stained tresses aside, cupping her cheek as you ask her if she's ready to bring this to an end.  She smiles, kissing your fingers as you help her up onto her knees, resting her in your lap, legs astride your [pc.hips].  High on her knees, your salamander lover shifts to press your prick against the entrance to her eager cunt, her juices flowing so freely that they nearly wash the muck from your [pc.cockHead " + pc.biggestCockIndex() + "] before you finally take her hips and guide her down, pulling her onto your [pc.cock " + pc.biggestCockIndex() + "].  She shudders as you enter her, breathing ragged gasps into your ear until you've given her all she'll get, leaving her panting and begging for more.  Instead, you take her chin between your fingers and make her look you in the eye as you kiss her, pushing your lips to hers. \"<i>[pc.name]...</i>\" she breaths, letting her tongue caress your own as she starts to move, slowly drawing herself off of you and back down again, taking slow, measured bounces as the passionate kiss persists.");
	
	output("\n\nMinutes pass, your tongue long entwined with Helia's, her hips steadily undulating atop your [pc.cock " + pc.biggestCockIndex() + "], her sizzling vaginal embrace bringing you closer and closer to the peak you've nearly crested for the better part of an hour, riding you for all you're worth as her muscles milk your prick of every last drop of pre.  This time there's no one trying to stop it; only loving caresses and gentle lovemaking, your slick bodies gliding across each other as you climb tantalizingly close to your limit.");
	
	output("\n\n\"<i>I love you, [pc.name],</i>\" Hel whispers, just speaking your name as her voice catches in her throat, her claws digging into your back.  You hold your lover close as the first trembles of orgasm hit her, holding her quivering cheek to your chest as she moans her pleasure, almost whimpering as her hot slit squeezes around you, milking you until finally you can feel sweet, blessed release.  You let out a bestial grunt as the first thick streams of seed burst out of you, a powerful spurt that paints the walls of Helia's womb white, filling her with your cum.  Squirt after squirt shoot into her until thick rivulets of spunk roll out around your [pc.cock], smearing into the mud and sweat that coats your two bodies.");
	
	output("\n\n\"<i>God, yes,</i>\" Hel moans, holding you tightly as you stuff her with your seed, \"<i>Just like that, [pc.name].  Oh, God, yes... Oh, [pc.name]....</i>\"");
	...
	output("\n\nShe gives you a gentle, quick kiss before nestling her head on your [pc.chest], eyes closed and breath calm.  \"<i>Can we just stay like this... just for... just for a few minutes?</i>\"");
...
private function takeABath():void {
	clearOutput();
	//{First Time intro}
	if (int(flags["COC.HEL_CAN_SWIM"]) == 0) {
		output("Noticing Hel smells particularly strongly of beer and sex - even for her - you ask your salamander lover if she'd like to join you for a dip in the stream.  Rather than with her usual energy, though, Hel's reply is to look quickly away and say, \"<i>Uh, not today, [pc.name]. I'm, uh, not feeling in the swimming mood.</i>\"");
	...
	//{Repeat Intro (If you taught Hel to swin; else, play 1st Time)}
		...
		//output(images.showImage("helia-swim"));
		output("You ruffle Hel's hair and ask if the fiery salamander's up for a dip.");
		...
		output("\n\nAfter a few playful minutes of splashing and laughing, the two of you wade back to a small rocky outcropping, sheltering you from wandering eyes.  Hel quickly curls up beside you, her head resting on your [pc.chest] and her warm tail lying across your lap.  You wrap an arm around her shoulders, holding your lover close.  Your lake begins to turn into a sauna, the steam rising in thick clouds all around you until all you can see is the salamander and a foot of water in front of you.  So concealed, you choose that moment to give Hel a long kiss on the brow, running your fingers through her long hair and down her shoulders.");
		
		output("\n\n\"<i>Mmm, that's nice...</i>\" she moans, nuzzling against you. \"<i>Don't stop....</i>\"");
		//Swim With Hel -- While Wearing Valeria
		if (pc.armor is CoCGooArmor && pc.lust() >= 33 && 9999 == 0) {
...
private function teachHelToSwim():void {
	...
	else output("\n\n\"<i>I-I dunno, [pc.name].  I mean, me being a firebutt... and water...  I feel like I'm gonna boil!</i>\"  You sigh, and tell Hel she'll be fine... she's just got to try.");
...
public function heliaThreesomes():void {
	clearOutput();
	output("You slip an arm around Hel's waist and ask if she's up for some... group activities?");
	output("\n\nShe grins, her serpentine tongue flicking hungrily across her lips.  \"<i>Oh, you do know what I like, don't you?  So, who do you wanna bring in on the fun, [pc.name]?</i>\"");
...
//needs wings or 50+ str, dick, added as an option \"<i>Carry her to camp</i>\" when you defeat Hel while she is in Fuckbuddy mode.
	...
	//[Kiha]
	clearMenu();
	if (vapulaSlave() && pc.lust() >= 33 && pc.hasCock()) {
		if (pc.cockThatFits(heliaCapacity()) >= 0) {
			addButton(1, "Vapula", heliaAndVapula);
		}
		else output("\n\nYou're too big to fuck Helia and Vapula with your cock.");
	}
	if (followerKiha() && pc.lust() >= 33 && pc.hasGenitals()) {
		addButton(0, "Kiha", campHelAndKihaThreeSome);
	}
	if ((pc.armor is CoCGooArmor || flags["COC.VALARIA_AT_CAMP"] == 1) && pc.lust() >= 33/* && pc.hasVagina()*/) {
		addButton(2, "Valeria", helAndValeriaCampThreesomes);
	}
	if (pc.lust() >= 33 && bimboSophie())
	{
...
private function heliaAndVapula():void {
	clearOutput();
	var x:int = pc.cockThatFits(heliaCapacity());
	var y:int = pc.cockThatFits2(heliaCapacity());
	output("You gab Hel and start for the edge of camp, where a certain succubus dwells....");
	...
	output("\n\nYou feel hands wandering across your body, Hel slowly, sensuously, pulling off your [pc.gear].  Soon, the two of you are bare, clothes and weapons discarded all around the succubus.  You push Hel over, grinding your hard " + pc.cockDescript(x) + " between her legs.  As you grope the salamander girl's body, you gesture at Vapula and tell her to come closer so that you can stroke her, too.  You begin caressing each other, spending a few moments in languid pleasure until your " + pc.cockDescript(x) + " looks ready to explode.");
	...
	output("\n\nThis libertine orgy cannot last indefinitely, and even your girls are trembling convulsively, ready to reach their climax. You manage to slide your " + pc.cockDescript(x) + " up the reptile girl's twat a few more times before blasting her with your spooge.  You fill Hel's interior, pouring cum like a torrent into her womb");
	//[IF cum production massive: 
	if (pc.cumQ() >= 750) output(" until her belly swells and starts spurting ropes of jism back out of her love-tunnel");
	output(".  Your junk palpitates and bulges as globs push their way out of your shaft before being absorbed by her needy pussy, and her skin and scales seem to literally boil with consumed lust.  At the peak of your orgasm, you stare at your girls' faces: Hel's is nothing but complete, insensate bliss, while Vapula is literally crying in ecstasy, tears of joy streaming down her cheeks.");
	
	output("\n\nSeeing how much pleasure you're giving them is quite touching, and you give each of them a long, ardent kiss, letting each girl know how the other tastes like.  At last, when your " + pc.cockDescript(x) + " has unloaded its last rope of jism, you pull out and break the kiss.  Hel pants and sighs, utterly drained; you see her tail popping out of Vapula's ass, and the released succubus just falls on her back, cunt exposed and mouth open in blissful exhaustion.");
	...
	if (!followerHel()) {
		output("  You kiss her and ask if she'll be able to find her way back to her home.");
...
private function campHelAndKihaThreeSome():void {
	...
	output("\n\n\"<i>Dammit, slut, lemme go! [pc.name]!</i>\"  Kiha growls, finally breaking free of Hel's grasp... and falling right into yours.  You grab the dragoness as she falls, and pull her right into a kiss, slipping your tongue into her mouth.  Kiha writhes for a moment, but soon relaxes, nearly melting in your embrace.  Her forked tongue plays across your own, her full red lips pressing passionately against you.  Behind her, Helia puts a hand on Kiha's shoulder, pressing her huge tits into the dragon-girl's back.");
	
	output("\n\n\"<i>What do you say to a little fun, huh? Show [pc.name] what two sets of scales can do...</i>\"");
	
	output("\n\nKiha looks to you, eyebrow arched questioningly.  You nod, and tell her it'll be fun.");
	
	output("\n\n\"<i>Well, if it's okay with [pc.name]...  I guess I could grace you with the sexual prowess of a dragon.</i>\"");
	...
	else if (pc.hasCock()) addButton(0,"Next",dudeHeliaAndKihaThreeSome);
	else if (pc.hasVagina()) addButton(0, "Next", girlsThreesomeHelAndKiha);
	else addNextButton(); // failsafe
}

//Female Kiha + Helia Scene
private function girlsThreesomeHelAndKiha():void {
	clearOutput();
	output("You slowly pull off your [pc.gear], the cool wind stiffening your [pc.nipples] as your sex is laid bare.  Hel and Kiha both step close, leaning in so their breasts press against your own [pc.chest]; scaled hands play across your thighs and hips, caressing your curves and groping your [pc.butt].  A tiny gasp escapes your lips as a pair of long, leathery tails wrap around your [pc.legs], coiling around you as the girls kiss and lick your bare breasts, each finding a sensitive nipple to suckle from.  You groan lewdly as the lizard girls suck gently on your teats, one tail finally slithering up to tease your [pc.clit], the other parting the cheeks of your backside, its very tip pressing against the dark star of your rear.");
	
	output("\n\n\"<i>Why do </i>I<i> have to take her ass?</i>\" Kiha gripes, glowering at Hel.");
	
	output("\n\n\"<i>Wha? I thought I was saving it for you...</i>\"");
	
	output("\n\nYou scowl and grab both girls' red hair, pushing them back down on your [pc.nipples].");
	//{If PC is Lactating:}
	if (pc.lactationQ() >= 50) output("  A shudder rocks you as the first trickles of milk seep from your teats, a small stream soon flowing out of you into the twin lizards' mouths.  Their cheeks bulge between swallows, but both coo happily as they suckle, their forked tongues lapping up every drop you have to offer.");
	//{If PC has a dick:}
	if (pc.hasCock()) output("  Even as the girls worship your [pc.chest], you feel a sudden pressure on [pc.oneCock].  Hel's tail deviates from your cunt for a moment, instead wrapping lovingly around your prick.  A groan escapes your lips as her hot leathery tail coils around your manhood before looping back to tease your clitty, ensuring that your dick won't be left out of the fun.  You smile and stroke her hair, eager to get to the main course.");
	...
	output("\n\nYou grab Kiha's horns to steady yourself as your cunt and ass spasm around the thick tail-cocks crammed inside you, your [pc.legs] wobbling weakly after the double-penetration.  Even as you recover, though, the dragoness mashes Hel's face into her belly, commanding, \"<i>Come on, firebutt, clean it off!</i>\"");
...
private function dudeHeliaAndKihaThreeSome():void {
	clearOutput();
	//output(images.showImage("helia-follower-threesome-kiha"));
	...
	output("\n\nBefore the dragon-girl can retort, Helia curls her flexible foot around your cockhead, eliciting a gasp of pleasure from you as your [pc.cockHead] is enveloped in warm, soft, leathery salamander foot. Matching Hel's actions, Kiha locks your shaft in the arches of her feet and starts to move them in sync, using them together like a tender onahole.  You relax in Kiha's embrace, her arms drifting down to wrap around your [pc.chest] as her feet massage your prick.  Your head rests lightly on her dusky bosom, her stiff nipples just beside your ears, your head rising and falling with each of her slow, measured breaths.  Her heartbeat echoes powerfully in her chest, the strong muscle resounding in your ear, the drumbeat nearly mesmerizing, certainly soothing, in its regularity... its power.  You almost feel like drifting off to sleep, though the three feet working tirelessly on your prick ensure that you remain alert enough to enjoy the cushiony embrace of your lovers' feet.");
	
	output("\n\nTenderly, Kiha plants a kiss in the nape of your neck, her long forked tongue tickling your " + pc.skinFurScales() + ".  You turn and return her kisses, locking lips with the dragoness, your tongues entwining to the rhythm of the feet servicing your [pc.cock].");
	...
	output("\n\nKiha gives the salamander a grave look, but soon softens, nuzzling her chin in your " + pc.hairDescript() + ".  Hel sighs, but leans down to give you each a quick kiss on the head; her foot never misses a beat, though, and combined with Kiha's sensual massaging, you realize you're dangerously close to the edge again.  You grit your teeth and let loose an animalistic growl, the only forewarning the distracted lizards get before the first burst issues forth from your [pc.cockHead], smearing the sole of Hel's foot with creamy seed.  The salamander makes a happy gasp as another squirt of cum smears her clawed toes, a particularly strong arc fountaining so high that it smears her thighs.");
...
private function helXValeriaFemalePC():void {
	clearOutput();
	//if (pc.hasVagina()) output(images.showImage("helia-follower-threesome-valeria-female"));
	//else output(images.showImage("helia-follower-threesome-valeria-male"));
	output("In the blink of an eye, Valeria slithers around Hel's body, almost completely encasing her in thick blue goo, leaving only her head and her resplendent reptilian rack bare.  Hel grins at you, putting a hand to her crotch and taking a handful of goo between her fingers.  With a bit of effort, she shapes a " + (pc.hasVagina() ? "pair of thick blue pricks " : "thick blue prick ") + "out of her groin, each shimmering in the sunlight as Hel strokes Valeria's new appendages, coaxing out a drop of slimey pre from both tips.  Valeria's face reappears on Hel's flat belly a moment after she's done, looking down to her groin with glee as she further shapes the erect pillars of slime, morphing them into a thick knotty doggy-dick on top, and a huge-flared horsecock on bottom.  You lick your lips as the dog-dick's knot inflates enticingly.");
	enum.clear();
	enum.push("[pc.butt]");
	if (pc.hasCock()) enum.push("your [pc.cock]");
	if (pc.hasVagina()) enum.push("and the slick slit of your cunt");
	output("\n\nThe Valeria-covered Hel takes you by the shoulder and bends you down, pushing you up against a nearby rock to expose your " + enum.toString() + ".  You dig your fingers into the stone as Hel moves behind you, her clawed hands running roughly along your thighs, hips, and ass, groping and caressing your tender skin.  Her fiery tail brushes your [pc.legs], coiling around before reaching up to tease your " + (pc.hasVagina() ? "[pc.clit]" : pc.hasCock() ? "[pc.cock]" : "[pc.butt]") + ", her tip running circles around your bud, sending thunderbolts of pleasure arcing through your body.  A look over your shoulder shows the gooey salamander spreading her legs and stroking her twin blue rods with one hand, the other squeezing your [pc.butt], pulling your cheeks apart to reveal your [pc.asshole].");
	...
	output("\n\nAgain, you feel the cool, wet tip of a cock push against your [pc.ass], demanding entrance as the thick, flared horsemeat below finds the lips of your womanhood, passing your vulva to taste the tiny trickle of lubricant your eager body is already making.  \"<i>Mmm, </i>delicious<i>,</i>\" Valeria says, a grin spreading across the face on Hel's belly.  \"<i>Oh, this is going to be great...</i>\"");
	
	output("\n\nYou grit your teeth and try to relax, urging your body to allow the foreign intruders in.  Your efforts seem futile, though, as both cocks slip right in, seemingly without effort, the gooey pricks becoming so thin and so slick that your body can't deny them - only to inflate again once they've gained a foothold inside you.  You cry aloud as Valeria expands inside you, her cocks growing... and growing... thickening and thickening until " + (pc.hasVagina() ? "both your holes stretch " : "your anus stretches") + " pleasurably, forced to take more and more goo until you feel as if you're going to burst, filled with more cock than you ever could have taken before.  Hel, too, cries out as the armor-goo pushes a pair of mirror cocks into her, mimicking her movements and the " + (pc.hasVagina() ? "cunt" : "ass") + "-stretching size of the members she's giving you.");
	
	output("\n\nBoth of you take a moment to recover, trying to let your bodies adjust to the titanic cyan pillars that are forcing your " + (pc.hasVagina() ? "holes" : "hole") + " agape.  Soon, though, Hel adjusts her grip on your [pc.hips] and starts to move, slowly pulling back the few inches of length inside you, withdrawing until only the very tip and flare remain within.  You groan lewdly as Hel pushes back in, slowly, gently filling you back up with her cocks, pouring gallons of goo into your " + (pc.hasVagina() ? "womb and " : "") + "rectum until her hips finally push into your [pc.butt], both your mouths locked in silent 'O's of pleasure as Valeria's goo stretches all " + (pc.hasVagina() ? "four" : "three") + " holes wide.");
	
	output("\n\nAnd then you feel the knot.");
	
	output("\n\nYou grit your teeth as the gooey knot half-inside your [pc.ass] strains your hole, starting to seriously inflate.  Hel pulls back before you have to scream in pain, suffering the same punishment on her end.  The goo-cocks withdraw a few inches, saving you from the knot and leaving only a single ring of prepuce in your well-stretched " + (pc.hasVagina() ? "twat" : "ass") + ".  \"<i>Shit, this knot...</i>\" Hel groans, starting to make shorter, quicker thrusts into you, the huge bulge bouncing against your backdoor.  \"<i>Gotta... gotta get the fucker in.  I want it so bad....  Brace yourself, lover!</i>\"");
	
	output("\n\nYou gulp and steel yourself, fingers digging into the stone as you try desperately to relax your sphincter; the gooey dog-cock Hel's sporting slips into you, sliding further and further in until the fully-swelled knot reaches your hole. Valeria giggles as the thick knot tries to force its way in, but the goo does not relent its shape or form, staying rock-hard and unyielding.  Moaning, you wiggle your [pc.hips] and push back against the knot, trying desperately to take it, to finally get the damnable thing in.  But it's too big, too hard; your efforts seem in vain, only serving to get the titanic sphere lodged half-way in you, stretching you until you can't help but cry out in pain and pleasure, your [pc.ass] pulled apart well beyond its limit.");
	pc.buttChange(cockVolume(30), true, true, false);
	if (pc.hasVagina()) pc.cuntChange(0, cockVolume(20), true, true, false);
	
	output("\n\nWith a loud, wet POP, the knot finally pushes in. You scream with pleasure as the wad of goo finally batters its way inside you, a huge gush of blue seed spurting out of Valeria's cockheads and into your " + (pc.hasVagina() ? "womb and " : "") + "gut with it.  Just as Hel finally knots you, the horsecock's flared head rams through your cervix, spewing slime freely as Valeria cums, both she and Helia crying out as they orgasm in unison, driven mad with ecstasy by the knotting of their cocks.  You, too, can feel orgasm mounting, " + (pc.hasVagina() ? "both your holes " : "your hole ") + "being roughly savaged by the double penetration of Valeria's gooey pillars.  A few short, near-panicked thrusts of the two huge members and the orgasm comes.");
	//pc.slimeFeed();

	output("\n\nGoo pours into you, cold fluid sloshing in your belly" + (pc.hasVagina() ? " and womb" : "") + ", more and more pouring in until you're completely gravid, your own orgasm serving only to milk more and more slimey cum from the googirl.  Hel bucks her hips wildly into you, pinned into short, maddeningly fast strokes by the thick knot still locked in your [pc.ass].  " + (pc.hasVagina() ? "A great spurt of femcum sprays from your [vag] " : "") + (pc.hasCock() && pc.hasVagina() ? "while " : "") + (pc.hasCock() ? (pc.hasVagina() ? "r" : "R") + "opes of jism shoot from your [pc.cock]" : "") + ", largely soaked up by the gooey horsecock; Valeria laughs, grabbing your [pc.nipples] and giving them a hard tweak, urging more and more of your fluids out, her pricks battering into you.");
	
	enum.clear();
	enum.push("goo");
	if (pc.hasCock()) enum.push("cum");
	if (pc.hasVagina()) enum.push("femcum");
	output("\n\nFinally, the orgasm subsides, and you're left trembling in a pool of " + enum.toString() + ", " + (pc.hasVagina() ? "lube leaking freely out of your cunt " : "") + (pc.hasCock() && pc.hasVagina() ? "while " : "") + (pc.hasCock() ? (pc.hasVagina() ? "cum still drips from your cock " : "your cum still dripping from your cock ") : "") + "and into Valeria.  Sighing contentedly, Hel reaches around to give you a long kiss before she pulls out... or tries to, anyway.  The knot, however, remains firmly in place.");
...
private function helGuardToggle():void {
	...
	if (int(flags["COC.HEL_GUARDING"]) == 0) {
		output("You ask Hel if she wouldn't mind watching the perimeter for a few hours during the night.");
		output("\n\n\"<i>Sure thing, [pc.name]!  I've got your back!</i>\" she says, her tail reaching around to give you a pop on the [pc.butt].");
		flags["COC.HEL_GUARDING"] = 1;
	}
	//[If Hel IS guarding Camp]
	else {
		output("You tell Hel she doesn't need to keep watch at night anymore.");
		output("\n\n\"<i>Is that a fact?  Well, I guess that gives us more time to party at night!</i>\"");
		flags["COC.HEL_GUARDING"] = 0;
	}
	clearMenu();
...
private function heliaAnalTrainingPrompt():void {
	clearOutput();
	flags["COC.HELIA_ANAL_TRAINING_OFFERED"] = 1;
	output("“<i>Hey there, lover mine. Got a minute?</i>” Helia asks with dangerous glint in her eyes.");
...
private function giveHeliaAnalTraining():void {
	clearOutput();
	output("You hand Helia a bottle of goblin ale and question if she’s sure this won’t make her loose.");
	...
	output("\n\nYou catch her meaning and strip to free [pc.eachCock], ");
	if (pc.cockTotal() > 1) output("selecting the biggest and heaviest to stroke as you near her");
	else output("grabbing it and stroking it as you near her");
	output(", admiring the hefty weight slowly and imagining it violating her stretchy sphincter in the same way as her toy. She growls and lashes her tail around your [pc.leg] to drag you closer, impatiently stabbing four fingers into her twat at a time with such force that her juices occasionally splatter out onto the dirt and you, now that you’ve come close. She helps to guide your [pc.cock " + pc.biggestCockIndex() + "] onto her face with her free hand, loudly moaning into it as her hips spread wider.");
	output("\n\nThe sexual excitement of having your ");
	if (pc.tallness < 72) output("big, ");
	output("scaley girlfriend worshipping your cock from below wars with your marvel at the size of the insertion she’s taking. It shouldn’t be possible for her hips to widen around a big, butt-fucking plug, but they are. Though she isn’t turning green or sprouting elfin ears, Helia’s body does seem to be gaining the elastic properties goblins are known for. She whimpers and cries out, lurching downward a full six inches as the knot vanishes into her belly, rounding her body with a plainly visible paunch. Her fist slips into her twat, and she cries, “<i>It’s soooo biiiiiggggg!</i>” Gushes of femspunk pour out of her relatively neglected cunt with each climactic convulsion, quivering beneath your cock as she lewdly licks as much of your length as her body can reach.");
	...
	if (pc.balls > 0) output("She tilts forward to press her chin into your [pc.sack] and begins to lather your [pc.balls] with frothy, warm, salamander spit. One hand hefts a weighty nut as she works, caressing it as if it would somehow coax the cum out of it. ");
	output("Helia slides her palms along either side of your urethra, letting her fingers handle the central, sensitive bit on the underside, following in the path of slick saliva she left for them to follow. She rubs her nose into the sensitive area just below your [pc.cockHead " + pc.biggestCockIndex() + "]. You grunt as pre-cum begins to pour in response, your bloated boner flexing atop Helia’s head as you feel a dangerously powerful orgasm welling up within you.");
	
	output("\n\nThe drunken salamander wiggles her stuffed backdoor happily around the impaling object as she strokes you faster and faster, talking into the bottom of your [pc.cock " + pc.biggestCockIndex() + "], begging it to dump a huge load all over, to drench her with gallons of spermy jism. As if you could hold back after such a lurid request. Your muscles clench powerfully");
	if (pc.balls > 0) output(", hard enough to lift your nuts in their [pc.sack]");
	output(", and you erupt, cumming a steamy hot load of spunk straight onto Helia’s face, head, and back.");
	if (pc.cumQ() < 250) output(" You run out before long, but it’s still enough to make her a white painted mess. She licks the excess from her upper lip and shudders happily.");
	else if (pc.cumQ() < 2000) output(" You spurt rope after rope onto her, actually drenching her hair with alabaster moisture. Her back and face look equally smothered with spooge when you finish. She’s happy to wipe the goop out of her eyes and shovel it into her mouth, swallowing it down with a satisfied smile.");
	else {
		output(" You pour ivory waves of spooge across her body, soaking her from head to toe with the fruit of your loins, liberally basting her with baby-batter. She groans as the excess washes out around her knees");
		if (pc.cumQ() >= 3000) output(", but you keep cumming, cascading fresh waves onto her until there’s not a single part of her that isn’t stained and dripping with your salty outflow");
		output(". Helia licks her hands clean, wipes her eyes and devours that as well, and then gives you a dripping-wet, sensuous smile.");
	}
	...
	flags["COC.HELIA_ANAL_TRAINING"] = 1;
	if (pc.biggestCockVolume() < helAnalCapacity()) {
		output("then maybe I can handle you, my hung " + pc.mf("stud","breeder") + ".");
	}
	else {
		output("then maybe we can see about trying something bigger.");
	}
	output("</i>”");
...
private function heliaGapeSceneChoices():void {
	if (int(flags["COC.HELIA_ANAL_TRAINING"]) == 0) giveHeliaAnalTraining();
	else if (flags["COC.HELIA_ANAL_TRAINING"] == 1) heliaAnalTrainingPartTwo();
}
...
private function heliaAnalTrainingPartTwo():void {
	pc.destroyItem(new CoCGoblinAle());
	clearOutput();
	output("Pulling out another bottle of ale you dangle it before the slutty salamander, saying, “<i>A present for my favorite buttslut.</i>”");
	...
	output("\n\nYou do a take from the dildo to her face and back again. How in the gods’ names is she going to take such an expansive member up her ass? You’ve wanted to put your [pc.cock " + pc.biggestCockIndex() + "] up there for some time, but seeing something mimicking your unholy size in person really drives home what an absurd insertion this is going to be. It’s not too late - you could stop her now.");
	output("\n\nDo you stop Helia?");
	//[Yes] [No]
	clearMenu();
...
private function noGapeSlutForHelia():void {
	clearOutput();
	output("You step up and kiss Helia, telling her that she doesn’t have to this for you, at least not right now.");
	...
	output("\n\nYour [pc.feet] hit the ground when the confident salamander drops you, and she says, “<i>Your call. I’m gonna pour these into my still to give it a little extra kick. Don’t fret, the transformative shit should burn off right quick.</i>” She turns and depart, her tail giving you a heat slap on the [pc.butt]. “<i>Catch ya later, [pc.name].</i>”");
	addNextButton(returnToCampUseOneHour);
}
...
private function yesMakeHeliaAGapeSlut():void {
	clearOutput();
	flags["COC.HELIA_ANAL_TRAINING"] = 2;
	output("Helia hooks a finger into your [pc.gear] and begins to tug you out of it, a task made difficult due to your ludicrous size. “<i>Don’t think I’ll let you go without draining this monster, [pc.name].</i>” Inch after inch of semi-hard cock is pulled into Helia’s firm grip, expanding with every passing moment, thickening into a truly monstrous dong. She caress the [pc.cockHead " + pc.biggestCockIndex() + "], even going so far as to playfully finger the supersized cumslit at its apex, her eyes playfully flicking up to meet your own. “<i>Now, sit back and enjoy the show.</i>”");
	...
	output("\n\nThe goblin contraption down below chooses that moment to spin up with a high-pitched whine, visible rocking and rattling around as the rest of the reservoir is rapidly empty straight into Helia’s newly-pliant asshole. She giggles as she’s filled with the elasticity inducing mutagen and rubs her pussy, cumming noisily as her belly button pops out to form an outtie. Ladyspunk pours in an unchecked river from between her legs, washing over the balls. Clouds of dust kick up as her spasming feet settle down, the medial ring long devoured by her hungry butthole. Helia starts to thumb at her clit midway through the anal massacre, eventually curling her tail around to stuff it straight up her wanton hole. Steam fogs the air around her as she lewdly tailfucks herself, chanting, “<i>Fucking yess! Fuck me, [pc.name]! Fuck my ass! Spitroast me on that cock!</i>”");
	output("\n\nYou jack off with hungry strokes as you listen to her orgasmic chatter, spurred on by the slutty salamanders smutty mouth. You’re dripping pre-cum from [pc.eachCock] quite readily by the time the steam clears and you see Helia standing there, her lower torso distorted by the veiny intrudor. She’s rubbing her belly and sighing quite happily, but she does have enough sense to crook a finger in your direction, panting, “<i>Bring that bad boy over here, " + pc.mf("stud","’stud’") + ". I’m gonna suck out the cream filling.</i>”");
	output("\n\nStaggering up, still stroking your [pc.cock " + pc.biggestCockIndex() + "], you ");
	if (pc.physique() < 40) output("strain to ");
	output("heft it up, pressing the [pc.cockHead " + pc.biggestCockIndex() + "] on Helia’s thirsty lips. She sandwitches the " + num2Text(Math.round(pc.cocks[pc.biggestCockIndex()].cLength())) + "-thick shaft in between her tits and left arm while her right tickles the sensitive underside. Her tongue slides around your urethra, collecting every single drop of pre-cum and stimulating nerve endings you didn’t even know you had. You groan and flex powerfully, preparing to dump a thick load into the hot salamander’s mouth. She spreads her lips around your cum-slit and kisses your cock hard, sucking powerfully, drawing the bubbling tide of wide straight out of your [pc.balls] with her hungry movements.");
	...
	if (pc.cumQ() < 1000) output(", ignoring the way your sheer volume of spunk leaks from the corners of her mouth and nose, her belly gradually rounding with cum in addition to thick dildo");
	output(". The slutty salamander eventually pulls it off and ");
	if (pc.cumQ() < 2000) output("squeezes a last few dollops onto her face, letting them stain her with your scent.");
	else output("allows you to hose her down with your ongoing waves of salty jism, soaking her in your sexual scent.");
	output(" Helia burps. “<i>Fuck me that was awesome! Whoah, don’t mind me, but I, uh... I think I’m gonna stay here for a little while, maybe just rock up and down till I come down off this buzz. She blushes and begins to finger her snatch again. Just let me... just let me get used to.</i>” She shoos you away and goes back to toying with herself, getting used to her new “flexibility”.");
...
public function heliasBirthday():void {
	clearOutput();
	output("Looking around camp, you see just about the strangest thing you've laid eyes on since coming to this accursed world: Helia the salamander, normally wearing her sluttiness on her shoulder with little more than an armored bikin, is striding toward you with hips a-swaying, wrapped up in a brilliant low-cut red evening gown.  Though backless and cut off just below the knees, it's by far the most conservative thing you've ever seen the sexy salamander wearing.  You can't even see a weapon on her, save her razor-like claws and swishing, fiery tail.");
	output("\n\n\"<i>Hey, lover mine,</i>\" she says, slipping into your arms to press her lips to yours. \"<i>Hey, do you know what day it is?</i>\"");
	
	output("\n\nThe heat of your lover's body pressed to yours, the feeling of her supple leathery scales caressing your " + pc.skinFurScales() + " makes your mind skip a beat before you mumble, \"<i>Uh, Saturday?</i>\"");
	
	output("\n\nHel rolls her eyes, overly dramatic.  \"<i>Ugh, come on, [pc.name].  I asked you last week to come to town with me.  Dad and Kiri are probably waiting for us.</i>\"");
	...
	flags["COC.HELIA_BIRTHDAY_OFFERED"] = 1;
	clearMenu();
...
private function stayHomeFromHeliaParty():void {
	clearOutput();
	output("\"<i>Aww, lame,</i>\" Hel groans, sighing.  \"<i>Fiiiiiine.  I guess I'll just go have awesome fun and party hard without you. Don't stick too hard into that mud, lover,</i>\" she chuckles, giving you a reassuringly affectionate kiss on the cheek before trucking off.");
	addNextButton();
}
...
private function helPartyMenu():void {
	...
	clearMenu();
	//heliaDrinks
	if (int(flags["COC.HELIA_BDAY_DRINKS"]) == 0) addButton(0, "Drinks", heliaDrinks);
	//heliaHakonAndKiri
	if (int(flags["COC.HELIA_BDAY_HAKON_AND_KIRI"]) == 0) addButton(1, "Hakon 'n Kiri", heliaHakonAndKiri);
	//heliaPhoenixes
	if (int(flags["COC.HELIA_BDAY_PHOENIXES"]) == 0) addButton(2, "Phoenixes", heliaPhoenixes);
	//heliaAndFoxTwins
	if (int(flags["COC.HELIA_BDAY_FOX_TWINS"]) == 0) addButton(3, "Fox Twins", heliaAndFoxTwins);
	//leaveWithoutSex
	addButton(4, "Leave", leaveWithoutSex);
	//leaveWithGirls
	if (int(flags["COC.HELIA_BDAY_FOX_TWINS"]) > 0) addButton(5, "Leave w/Girls", leaveWithGirls);
}

//Go with Her
private function goWithHelia():void {
	clearOutput();
	output("\"<i>Sounds like fun.  Let's go,</i>\" you say, offering your arm to Hel.  She grins and takes it, holding herself close as she takes the lead, steady paces quickly drawing you out into the vast deserts of Mareth beside your lover.");
	//{If Kiha, Isabella, and/or Valeria are at camp:}
	if (isabellaFollower() || followerKiha() || flags["COC.VALARIA_AT_CAMP"] == 1) {
		output("  Looking over your shoulder as you depart, though, you notice that camp looks particularly... empty. Perhaps your other camp-mate");
		if (companionsCount() > 1) output("s are");
		else output(" is");
		output(" out, too? You shrug. You and Hel aren't the only ones with lives, you suppose.");
	}
	...
	//if less than 7' PC: 
	if (pc.tallness < 84) output("sweeping you off your feet");
	else output("hanging off you");
	output(" as you approach the great gates.  The only one on gate duty today is Urta, the grey-coated fox curled up under a small awning beside the gate with a huge canteen resting beside you.  Her centauress partner is noticeably missing, but Urta still gives you a wave and a grin you can't quite read.");
	
	output("\n\n\"<i>Hey, cutie,</i>\" she says with a wink, pushing open the door for you.");
	//If PC did UrtaQuest: 
	output("\n\n");
	if (flags["COC.URTA_MET_HEL"] == 1) output("From the quick look she and the salamander exchange, you get the feeling they've met before. Weird. Still, ");
	output("Urta gives you a small nod as Hel leads you into town, past the half-empty guard post and into the bustling main street.  The market way's particularly busy in the late afternoon, bustling with activity as merchants hawk their wares and the first of the street-walkers start to come out, standing in seductive poses in the shade of the alleys.  More than a few send catcalls your way, a few even offering discounts for couples.");
	...
	if (pc.tallness <= 72) output(", her head resting on your shoulder");
	output(".  \"<i>Hey, thanks for coming, [pc.name],</i>\" she says quietly, squeezing your arm, \"<i>Kiri's done nothing but talk about you since the tower.  I think someone has a crush...</i>\"");
	...
	enum.clear();
	enum.push("Edryn");
	if (flags["COC.VALARIA_AT_CAMP"] == 1) enum.push("Valeria");
	if (followerKiha()) enum.push("Kiha");
	if (isabellaFollower()) enum.push("Isabella");
	list.push("a handful of her wide-hipped and winged half-sisters, the phoenixes");
	output(enum.toString() + ". You turn to Helia and cock a grin as you see the surprise etched on her face, followed by the growing smile.  \"<i>Happy birthday, Helia!</i>\" they all shout out at once, raising mugs frothing over with beer.");
/*
	addToList("Edryn");
	...
	output(outputList() + ". You turn to Helia and cock a grin as you see the surprise etched on her face, followed by the growing smile.  \"<i>Happy birthday, Helia!</i>\" they all shout out at once, raising mugs frothing over with beer.");
*/	
	...
	if (flags["COC.HARPY_QUEEN_EXECUTED"] > 0) output("who forgave us for the queen's death");
	else output("who wanted to be friends still, even after the Tower");
	output(" and... it sort of became a thing.  Hope you don't mind!</i>\"");
	...
	output("\n\nKiri slips out of the crowd to plant a quick kiss on your cheek, followed by Hakon vigorously shaking your hand, great claws scoring your " + pc.skinFurScales() + " as he thanks you again for the rescue, and for ");
	if (flags["COC.HARPY_QUEEN_EXECUTED"] > 0) output("finally killing that bitch of a queen");
	else output("setting the queen right, but leaving her alive for when it's finally time to bring the fight to Lethice");
	output(".  You nod at his thanks, and his hand is soon replaced by Helia's as she grabs you on her way toward the gigantic cake.  The towering white confectionary seems more than big enough to feed everyone in the group, a huge extravagance on the part of her family.  You hear a quiet whisper about ");
	if (flags["COC.FREED_VALA"] == 1) output("a certain waitress fairy having... supplied the icing herself");
	else output("a certain minotaur chef having supplied the icing himself");
	output(" as Hel is handed an oversized knife and urged to blow out the candles atop it, arranged into a perfect 23.");
	...
	if (flags["COC.HEL_FOXY_FOURSOME_WARNED"] == 1) output("a pair of lovely fox-eared herms");
	else output("Miko and Mai");
	output(" burst out, their big breasts covered in nothing but tiny pink pasties and an all-too-small thong sheathing their red pricks.  Arm in arm with tail wagging furiously, they cry out \"<i>Happy birthday, Hel!</i>\" before hopping out and slipping into the salamander's arms.  \"<i>Ladies,</i>\" Hel says, grinning like an idiot, a moment before she's pushed into a waiting chair behind her.  One of the girl's busts is quickly shoved into her face as the rest of the group laughs and starts passing out another round of drinks.");
...
private function heliaHakonAndKiri():void {
	clearOutput();
	output("You meander over to the veteran salamander and his half-harpy daughter, and quickly find yourself surrounded by fluttering red feathers as Kiri practically jumps on you, hugging you.  \"<i>Hey, [pc.name], long time no see,</i>\" she says, smiling demurely as she leads you back to where she and her father were standing, removed from the real bustle of the party.");
	
	output("\n\n\"<i>How're you holding up, " + pc.mf("son","kid") + "?</i>\" Hakon says, clapping you on the shoulder and passing you another beer.  \"<i>Hel tells me you're quite a handful.  Treating my girl right, I hope?</i>\"");
	
	output("\n\nYou falter at that, not quite sure how to address your lover's father in that regard, but the old salamander just laughs and gives you another rough shoulder-clap.  \"<i>Just teasing, " + pc.mf("boy","girl"));
	output(".  We salamanders are a bit more open about that sort of thing, though, if you haven't noticed.  Helia's regaled us with more than a few of her 'stories,' let me tell you. Isn't that right, sweetheart?</i>\"");
	...
	flags["COC.HELIA_BDAY_HAKON_AND_KIRI"] = 1;
	helPartyMenu();
}
...
private function heliaPhoenixes():void {
	clearOutput();
	output("You approach the small squad of phoenixes milling around the cake, mostly talking amongst themselves.  They nod as you approach, a bit wary of the person who recently kicked their asses inside out.  Still, they nod respectfully as you approach.");
	
	output("\n\n\"<i>Evening, [pc.name],</i>\" their leader says, snapping you a crisp salute.  \"<i>Fine party we've got here.</i>\"");
	...
	if (flags["COC.HARPY_QUEEN_EXECUTED"] > 0) output("dead");
	else output("deposed");
	output(", he thought it would be good to try and at least... know each other.  I'm not complaining.  Things like this party... never would have seen anything like this under Queen Calais.  It's nice to see new things, meet new people.</i>\"");
	
	output("\n\nYou see a few glances going toward Edryn and the fox twins");
	if (followerKiha() && isabellaFollower()) output(", then to the fiery dragoness and the ultra-busty cowgirl talking in the corner");
	output(", and you figure you know what they mean.");
	flags["COC.HELIA_BDAY_PHOENIXES"] = 1;
	helPartyMenu();
}
...
private function heliaAndFoxTwins():void {
	clearOutput();
	output("You approach the foxy strippers, still well into giving Helia a dual lap-dance, their red puppy peckers poking well out of their tiny thongs.  They grin as you approach, and Hel pulls you into her lap, and you quickly find yourself with a pair of big, soft breasts to rest your head on, courtesy of ");
	if (flags["COC.HEL_FOXY_FOURSOME_WARNED"] == 1) output("Mai");
	else output("one of the foxes");
	output(".  Hel plants a deep, serpent-tongue filled kiss on your lips, nuzzling her cheek on your [pc.chest].");

	output("\n\n\"<i>Not bad for a surprise party, huh, [pc.name]?</i>\"  Hel laughs, spreading her legs wide enough for the other twin to slip in, giving you a lusty grin as her hand rests on her crotch, rubbing lightly on her cock.");
	
	output("\n\nYou nod to your lover and sit back, resting your head in the pillowy valley of the fox-girl's cleavage.  Hel strokes your cheek, her tail reaching around to brush along your [pc.butt].  \"<i>Not a bad party at all... Thanks for coming, lover mine.  Didn't expect my visit to turn out like this, but... I'm glad you came.  Wouldn't be a birthday without you, [pc.name].  Nobody I'd rather spend it with.</i>\"");

	output("\n\nThe fox girls give a jealous huff, and Hel just grins and pulls them in close, a cheek pressed hard to each of her breasts.  \"<i>Don't worry, girls, I've got plenty of affection to go around.  Just say the word, [pc.name], and we'll get out of here.  And if you wanna take the girls, well... who'd say no to that?</i>\"");
	//{Unlocks Leave & Leave w/ Girls options}
	flags["COC.HELIA_BDAY_FOX_TWINS"] = 1;
	helPartyMenu();
}
...
private function leaveWithoutSex():void {
	clearOutput();
	output("You decide you've had about enough of the party for now. You slip over to Hel and tell her you're ready to head out when she is.");
	...
	output("\n\nWhen you get to the door, Hel holds you close, snuggling against you as you walk.  \"<i>Thanks for coming, [pc.name].  I love you.</i>\"");
	addNextButton();
}
...
private function leaveWithGirls():void {
	...
	else if (pc.hasVagina()) addNextButton(foxyFluffGirlsFuckSex);
	else if (pc.hasCock()) addNextButton(foxyFluffsFoursomeAsMale);
	else addNextButton();
}

private function helAndSluttyHarpy():void
{
	//output(images.showImage("hel-follower-threesome-sophie"));
...
private function helAndSluttyHarpyMale():void
{
	...
	output("You take Helia’s hand and lead her through the camp in an attempt to find someone to include in your threesome. After some searching you see Sophie sitting on a rock and get a wonderful idea. You ask Hel if she would be ok with a threesome with the busty harpy. Her smile turns to a serious frown for a moment.");
	...
	output("\n\n<i>“Oww, you stupid bird, watch what you're do-mpph”</i> Helia growls, but is cut off at the end when Sophie sits directly on her face. You can hear your fiery lover trying to continue her complaints beneath the harpy's massive assflesh, but her voice is so muffled you can’t make out the words. Sophie crooks a finger to beckon you over, pulling aside the salamander’s bikini bottom. You move over to Hel, stripping out of your [pc.gear] as you go, and kneel between her legs");

	output("\n\nYou slowly begin to inch your [pc.cock] Into Helia’s love furnace. Her complaints turn into a drawn out moan as you slide inside of her, her body shivering slightly. The shudders nearly set your bimbo slut off, and you see rivulets of her juices flowing over Hel’s chin, down her neck, and pooling on the ground. You gingerly pull up her chainmail bikini and palm one of her breasts, circling it around the nipple as you plant a kiss on the other.");
	...
	if (pc.biggestCockLength() >= 8) output(" no longer be hilted inside");
	else output(" slip out of");
	output(" her sodden cunt, and hear Hel gasp for air. Before the birdslut can comment on the matter, you spin her around and push her down onto Helia, leaving them face to face.");

	output("\n\n<i>“Gah... stupid harpy,”</i> Helia says between breaths, but she makes no further action to remove her. You slip your [pc.cock] back into Helia’s burning tunnel and begin thrusting slowly in and out. The transitions between her insides and the cool air make you feel like coming immediately, but the pleasurable moans that arise from your lover make the resistance worth it. ");
	...
	{
		output("\n\nYour bimbo’s pussy is beginning to look awfully neglected, and you can’t have that. On the next rock of your hips you pull out of Hel with a pop and transition your [pc.cock] to Sophie’s sopping box.");

		output("\n\n<i>“H-hey, don’t leave me hanging like that!”</i> Helia yells as Sophie breaks the kiss to scream in orgasm from the sudden penetration. The bimbo harpie’s cunt feels shockingly cool compared to Hel’s, and allows you to get a better hold on yourself despite the extreme tightness. After a few thrusts you pull out and transition to Hel again, trading between the two every few seconds. Eventually you just slide your [pc.cock] between the two pleasure-drunk girl’s pussies, rubbing into the sopping wet cleft between the two girls combined cunts..");

		output("\n\nThe combination of both hot and cool rubbing is too much to handle, and soon causes the pressure in your");
		if (pc.balls > 0) output(" [pc.balls]");
		else output(" groin");
		output(" to boil over. White jissom blasts from your [pc.cock], coating Helia and Sophie’s stomachs, as well as the underside of the pair’s tits.");
		if (pc.vaginas.length > 0) output(" Your unused [pc.vagina] cums with your cock in a show of sympathetic orgasm, coating your thighs with femcum.");
	}
	// 2-3 cocks
	else if (pc.cocks.length <= 3)
	{
		...
		if (pc.cumQ() < 500) output(" with your cum");
		else
		{
			output(" full of cum");
		 	if (pc.cumQ() <= 1000)
		 	{
		 		output(" until it overflows");
		 		if (pc.cumQ() >= 5000) output(", pooling around your knees");
		 	}
		}
		output(".");
		if (pc.cocks.length == 3) output(" Your unused cock splatters the two with hot, white jizz.");
		if (pc.vaginas.length > 0) output(" Your unused [pc.vagina] cums with your cocks in a show of sympathetic orgasm, coating your thighs with femcum.");
	}
	// > 4 cocks
	else
	{
		output("\n\nYou realize that there's a lot of holes on offer between the two sluts; luckily you have just the right thing to solve the problem, cocks! You unleash [pc.eachCock] upon the three holes that are still open. One [cock biggest2] snakes under the one currently pounding into Helia and slips inside her");
		if (int(flags["COC.HELIA_ANAL_TRAINING"]) == 0) output(" tight");
		else if (flags["COC.HELIA_ANAL_TRAINING"] == 1) output(" well-trained");
		else if (flags["COC.HELIA_ANAL_TRAINING"] >= 2) output(" gaped");
		output(" asshole. She gasps at the sudden double penetration, and you reciprocate because of the furnace-like heat of both her holes. Your [cock biggest3] lines up with Sophie’s slutty pussy and drives home while your [pc.cock " + pc.smallestCockIndex() + "] does the same to her ass."); // Haha look at me being a lazy shit.
		...
		output(". The two girls seem to share your problem, their teeth clenched and clinging tightly to each other trying to resist. Seeing their clearly shared pleasure in the moment puts you over the edge, and you let loose the torrent of liquid lust from [pc.eachCock]. Sophie cums as the tsunami of jizz floods her levies, the copious amount spilling back out of her holes and pooling around your knees.");
		if (pc.cocks.length > 4) output(" Your extra cocks spray pearly cum all over the nearby ground.");
		if (pc.vaginas.length > 0) output(" Your unused [pc.vagina] cums with your cocks in sympathetic orgasm, coating your thighs with femcum.");
	}
	// Merge
...
private function helAndSluttyHarpyFemale():void
{
	...
	output("\n\n<i>“Mm, is there something you wanted babe?”</i> the lusty harpy asks as she squishes her chest against your [pc.chest]. She reaches around, and draws soft circles around your shoulder blades with one of her fingers. Your lust rises at her touch, and you inform her that there most certainly is something you want.");
	...
	output("\n\n<i>“Mmmm, her pussy is, like, fiery hot. I love it!”</i> Sophie exclaims before burying her face into the salamander girl’s cunt with all the fervor she can muster. Hel grasps your [pc.hips] in what might have been an attempt to throw you off. Instead a shiver runs through her body causing her to pull your netherlips tighter to her face.");
	...
	output("\n\nYou hoist the bimbo harpy's massive ass up closer to your face and slide your tongue down her moist vulva, settling on her puffy clit at the end and slathering it with your mouth's liquid affection. Her sweet juices dribble over your chin and slide down your [pc.chest] and stomach. Your front half is completely drenched in harpy juices, but you don’t care. Helia’s ministrations are beginning to get to you.");

	output("\n\n<i>“Ooooh yeahh, thats like, perfect [pc.name], keep licking mama Sophie like that”</i> Sophie coos between moans, as if you needed to be told. Helia seems to notice that you’re getting more into it, and ups her game. You moan loudly as Helia’s long tongue slips deeper into your cunt, licking your walls as it goes. Her mouth sucks on your hole and her bottom lip brushes against your [pc.clit].");
	...
	if (pc.cocks.length >= 1)
	{
	output("The blasts spatter against the underside of Sophie’s gigantic melons and slide off Hel’s sides like an overfull cream pastry.");
	}
...
output("You walk up to Helia and ask if she would be up for fucking you with that wondrous tail of hers. She smiles slyly and wraps her arms around you. You take her response as an unspoken yes, hugging her back and pulling her off to find a better place for your loving.");
...
output("\n\n<i>“Go away, stupid harpy! I’m going to have some fun alone with [pc.name].”</i>");
