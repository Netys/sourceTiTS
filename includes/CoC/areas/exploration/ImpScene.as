...
{		
	...
	////Level modifier
	//if (player.level < 20) impChooser += pc.level;
	//else impChooser += 20;
...
//public function impLordEncounter():void {
	...
	//if (flags[kFLAGS.IMP_LORD_MALEHERM_PROGRESS] != 1) {
		//output("A large corrupted imp crosses your path. He flashes a cruel smile your way.  No way around it, you ready your " + pc.weaponName + " for the fight.");
...
//public function defeatImpLord():void {
	...
	//else {
		...
		////Leave // Rape]
		//menu();
		//if (pc.lust >= 33 && flags[kFLAGS.SFW_MODE] <= 0) addButton(0, "Sex", sexAnImpLord);
		//addButton(4,"Leave",cleanupAfterCombat);
	//}
//}
//public function loseToAnImpLord():void {
	//clearOutput();
	//if(pc.hasVagina() && (pc.gender == 2 || rand(2) == 0) && flags[kFLAGS.SFW_MODE] <= 0) getRapedAsAGirl();
	//else if(pc.hasCock() && flags[kFLAGS.SFW_MODE] <= 0) loseToImpLord();
	//else {
...
//private function sexAnImpLord():void {
	//clearOutput();
	//output("You grin evilly and walk towards the defeated corrupted creature.  He doesn't take notice of you even though you're only inches away from him.  You remove your [armor] slowly, enjoying the show the imp is giving you.  But soon it's time for you to have fun too.");
	////(No line break)
	////if(pc doesn't have centaur legs)
	//if(!pc.isTaur()) output("  You grab his hands, removing them from his " + monster.cockDescriptShort(0) + ". This gets his attention immediately, and you grin widely, pinning him to the ground.");
	...
	//if(pc.lust >= 33 && flags[kFLAGS.SFW_MODE] <= 0) {
		//if (pc.hasCock() && pc.cockThatFits(monster.analCapacity()) >= 0) {
			//addButton(0,"FuckHisAss",impLordBumPlug);
		//} else addDisabledButton(0, "FuckHisAss", "Requirements: cock not too large.");
		//if (pc.hasCock()) {
			//addButton(1,"Get Blown",getBlownByAnImpLord);
		//} else addDisabledButton(1, "Get Blown", "Requirements: cock.");
		//if (pc.hasVagina()) {
			//addButton(2,"Ride Cock",femaleVagRape);
		//} else addDisabledButton(1, "Ride Cock", "Requirements: vagina.");
		//
		//if(pc.findPerk(PerkLib.Feeder) >= 0 && monster.short != "imp overlord" && monster.short != "imp warlord") addButton(3,"Breastfeed",feederBreastfeedRape);
		//if (pc.hasVagina() && pc.biggestTitSize() >= 4 && pc.armor is LustyMaidensArmor) addButton(4, "B.Titfuck", (pc.armor as LustyMaidensArmor).lustyMaidenPaizuri);
	//}
...
//private function impLordBumPlug():void {
	...
	//var x:int = pc.cockThatFits(monster.analCapacity());
	...
	//}
	//output("  You pull the imp's head towards your [hips], forcing his lips against the base of your [pc.cock " + x + "].  The imp quickly gets the idea and begins to lick and suckle at your " + pc.cockHead(x) + " expertly.");
	//
	...
	//if(pc.balls > 0)
	//{
		//output("  You feel the slapping of your [balls] against the imp's, a sensation that only spurs you on, causing you to slam into him over and over wildly.");
	//}
	//if(pc.biggestTitSize() >= 1 && pc.lactationQ() >= 200) {
		//output("  You give a desperate groan, and grab one of your [chest] roughly.  You pinch and massage your ");
		//if(pc.averageNipplesPerBreast() > 1) output("[nipples]");
		//else output("[nipple]");
		//output(", making you moan in ecstasy and leak milk, quickly soaking your chest.");
	//}
	////if(pc has Fertility perk && pc.balls >= 4)
	//if(pc.balls >= 4) {
		//output("\n\nAs much as you would love to continue the pleasure you can't last any longer.  You howl in intense pleasure and cum.  Your [balls] tighten against your body, and empty their contents. Your [pc.cock " + x + "] pulses and spasms, releasing wave and wave of semen into the greater imp's belly.  The pleasure is intense, and almost painful as your cum.");
		////if(pc is a herm)
		//if(pc.gender == 3) output("\n\nAs your [pc.cock " + x + "] erupts, your " + vaginaDescript(0) + " tenses up tightly, spasming, desperate to be filled.  After a moment your girl juice begins to soak your inner thighs.  Your legs begin to tremble from the intensity of it all, and you question if you'll be able to make it back to your camp after this.");
	//
...
//private function getBlownByAnImpLord():void {
	...
	//output("\n\n");
	////if(cock thickness < 7)
	//if(pc.cocks[pc.biggestCockIndex()].cockThickness < 7) {
		//output("The imp begins to swallow more and more of your length, taking several inches before pulling back.  He twists his head and tongue worships your [cock biggest] with his mouth.  His tight mouth tightens more and relaxes as he swallows more of your precum.");
		//if(pc.balls > 0) output("  His hands move down towards your [balls].");
		//output("\n\n");
		////if(balls > 0)
		//if(pc.balls > 0) {
			//output("The demon gropes and massages your [sack] roughly, forcing a large squirt of precum to shoot down his throat.  He swallows the treat happily and continues his cruel groping of your [balls].  His hands work wonders of your [sack], milking your [balls] in a way you didn't know possible.  It's clear he's done this many times before and is an expert of pleasuring males.  You chuckle between your moans, you definitely made a good choice with this one.\n\n");
		//}
		////else if(balls == 0)
	...
	//if(pc.cumQ() < 500) output("  You cum hard, easily filling the imp's hot mouth.  He swallows your load just as easily, grinning as he suckles your [cockHead biggest] happily.  He suckles for a few minutes, getting the last few drops of seed before letting your [cock biggest] drop from his mouth with a soft pop.");
	////if(cumHigh) 
	//else if(pc.cumQ() < 1000) output("  You cum painfully hard, filling the demon's mouth beyond what it can hold.  Surprisingly the imp manages to swallow almost all of your spunk anyways, only allowing a little bit of his meal to dribble past his lips.  He pulls back and gives your [cock biggest] a few last licks, cleaning up any left over seed.");
...
//public function feederBreastfeedRape():void {
	...
	//if(pc.tallness > 48 && pc.tallness < 60 && !pc.isTaur()) {
		...
		////if(pc has a penis)
		//if(pc.hasCock()) {
			//output("  [EachCock] twitches violently from the stimulation of your [asshole].  Pre-cum begins to dribble out of your " + cockDescript(0) + ".  You can't help but pant desperately as the warm pleasurable sensation of arousal fills your whole being.");
		//}
		//if(pc.hasVagina()) {
			//output("  The intense stimulation of your [chest] is causing your [vagina] to become wet with girl juice... so much so that your femcum has started to seep down your taint towards your ass.  It's probably a good thing, as it's now become a lubricant for the greater imp's " + monster.cockDescriptShort(0) + ".");
		//}
		//
...
//private function getRapedAsAGirl():void {
	...
	//output(", draping his " + monster.cockDescriptShort(0) + " across your wet crotch.  You groan, and unintentionally thrust against the magnificent tool between your legs.  The imp chuckles evilly as you coat his " + monster.cockDescriptShort(0) + " in your girl juice, but he doesn't wait long before he slowly presses his head down against your [vagina].  His head slowly spreads your lips; the pleasure is unmistakable, and forces a loud moan from your lips.");
	//
