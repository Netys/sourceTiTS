...
public function GoblinSalonEnter():void {
	clearOutput();
	clearMenu();
	output("While exploring the mountain, you find a cleverly concealed doorway.  From inside you can hear the sound of blades being sharpened.  Do you enter the doorway?");
	addButton(0, "Yes", salonGreeting);
	addButton(1, "No", function():*{ processTime(15 + rand(5)); clearMenu(); mainGameMenu(); } );
}

public function salonGreeting():void{
	if (flags["COC.GOBLIN_SALON_FOUND"] > 0)
	{
		hairDresserRepeatGreeting();
	}
	else
	{
		flags["COC.GOBLIN_SALON_FOUND"] = 1;
		hairDresserGreeting();
	}
}
...
private function salonFavoritesPaymentMenu():void {
	clearMenu();
	if (flags["COC.LYNNETTE_PREGNANCY_CYCLE"] >= 4 && pc.hasCock()) addButton(5, "Fuck Goblin", fuckLynnette);
	if (pc.hasCock()) addButton(0, "Goblin Blow", goblinHairDresserFacefuck);
	...
	if (flags["COC.UNKNOWN_FLAG_NUMBER_00142"] > 0) addButton(8, "Buy MinoCum", buyMinoCum);
...
private function salonPaymentMenu():void {
	clearMenu();
	if(flags["COC.LYNNETTE_PREGNANCY_CYCLE"] >= 4 && pc.hasCock()) addButton(5,"Fuck Goblin",fuckLynnette);
	if (pc.hasCock()) addButton(0, "Goblin Blow", goblinHairDresserFacefuck, null, "Goblin Blow", "Let Lynnette suck you off.");
	addButton(1, "Canine", gloryholeDoggie, null, "Canine", "Suck that knotted cock.");
	addButton(2, "Imp", gloryholeImp, null, "Imp", "Suck that demonic cock. Judging from the constant bobbing, someone must have been trying hard to hover at the right height.");
	addButton(3, "Minotaur", gloryholeMinotaur, null, "Minotaur", "Suck that huge minotaur cock!");
	addButton(4, "Incubus", gloryholeIncubus, null, "Incubus", "Suck that incubus cock. It gives off that pleasant spicy scent.");
	if (flags["COC.UNKNOWN_FLAG_NUMBER_00142"] > 0) {
		if (pc.credits >= 600) addButton(8, "Buy MinoCum", buyMinoCum, null, "Buy MinoCum", "Buy a bottle of minotaur cum for 60 gems?");
		else addDisabledButton(8, "Buy MinoCum", "Buy MinoCum", "You have not enough gems.");
	}
...
private function buyMinoCum():void {
	...
	clearOutput();
	pc.credits -= 60 * 10;
	output("You happily give Lynnette 60 gems and pick up the bottle full of glistening, heavenly cum.  ");
	var back:Function = null;
	if (flags["COC.SALON_PAID"] == 0)
		back = salonGreeting;
	else
		back = salonPurchaseMenu;
	
	itemScreen = back;
	lootScreen = back;
	useItemFunction = back;
	itemCollect([new CoCMinotaurCum()]);
}
		
public function salonPurchaseMenu():void {
	flags["COC.SALON_PAID"] = 1;
	//spriteSelect(38);
	
	clearMenu();
	
	addDisabledButton(0, "Cut Short", "Cut Short", "Your hair are already shorter!");
	addDisabledButton(1, "Cut Medium", "Cut Medium", "Your hair are already shorter!");
	addDisabledButton(2, "Cut Long", "Cut Long", "Your hair are already shorter!");
	addDisabledButton(3, "Lengthen", "Lengthen", "Your hair are already obscenely long!");
	addDisabledButton(4, "Remove Hair", "Remove Hair", "You have no hair to remove!");
	
	if (pc.hairLength > 2) addButton(0, "Cut Short", cutShort);
	if (pc.hairLength > 13) addButton(1, "Cut Medium", cutMedium);
	if (pc.hairLength >= 26) addButton(2, "Cut Long", cutLong);
	if (pc.hairLength < pc.tallness) addButton(3, "Lengthen", hairGrow);
	if (pc.hairLength > 0) addButton(4, "Remove Hair", removeHair);
	
	addButton(7, "Beard Options", beardMenu);
	
	if (pc.hasLivingHair()) {
		addDisabledButton(0, "Cut Short", "Cut Short", "Your living hair are beyound goblin's expertise!");
		addDisabledButton(1, "Cut Medium", "Cut Medium", "Your living hair are beyound goblin's expertise!");
		addDisabledButton(2, "Cut Long", "Cut Long", "Your living hair are beyound goblin's expertise!");
		addDisabledButton(3, "Lengthen", "Lengthen", "Your living hair are beyound goblin's expertise!");
		addDisabledButton(4, "Remove Hair", "Remove Hair", "Your living hair are beyound goblin's expertise!");
		addDisabledButton(7, "Beard Options", "Beard Options", "Your living hair are beyound goblin's expertise!");
	}
	
	addButton(5, "Buy Products", dyeMenu);
	
	if (flags["COC.UNKNOWN_FLAG_NUMBER_00142"] > 0) {
		if (pc.credits >= 600) addButton(6, "Buy MinoCum", buyMinoCum, null, "Buy MinoCum", "Buy a bottle of minotaur cum for 60 gems?");
		else addDisabledButton(6, "Buy MinoCum", "Buy MinoCum", "You have not enough gems.");
	}
	
	addDisabledButton(8, "Mud Facial", "Mud Facial", "This facial is supposed to enhance the softness of your face and enhance its femininity greatly. You are already as femine as you can be.");
	addDisabledButton(9, "Sand Facial", "Sand Facial", "The goblins promise this facial will give you a rough, handsome look thanks to their special, timeless sands. You are already as masculine as you can be.");
	//Enable mud facial
	if (pc.femininity < pc.femininityMax())
		addButton(8, "Mud Facial", mudFacial, null, "Mud Facial", "This facial is supposed to enhance the softness of your face and enhance its femininity greatly.");
	
	if (pc.femininity > pc.femininityMin())
		addButton(9, "Sand Facial", sandFacial, null, "Sand Facial", "The goblins promise this facial will give you a rough, handsome look thanks to their special, timeless sands.");
	
	addButton(14, "Leave", function():*{ processTime(10 + rand(10)); mainGameMenu(); } );
}

private function hairDresserGreeting():void {
	flags["COC.LYNNETTE_BABY_COUNT"] = 0; // init var
	flags["COC.LYNNETTE_CARRYING_COUNT"] = 0;
	flags["COC.LYNNETTE_APPROVAL"] = 0;
	flags["COC.LYNNETTE_ANNOUNCED_APPROVAL"] = 0;
	//spriteSelect(38);
	clearOutput();
	//output(images.showImage("location-salon"));
	output("You step inside the cave, and are greeted by a sight you did not expect.  The cave's floor is covered with smooth wood panelling and the walls are nearly entirely covered with hanging mirrors.  The few stalactites have hooks drilled into them, from which hang hundreds of scissors, shears, razors, combs, and other hairstyling impliments.  It reminds you of the hair-cutter's shop in your hometown.");
	...
	output("   \"<i>I apologize for my daughters,</i>\" she says as she presses herself against you.  \"<i>They're a bunch of brainless hussies for the most part.  My name is Lynnette, and welcome to my salon!  You want your hair cut or lengthened?  We've got you covered, and we don't ask for much - just a shot of cum.");
	if(pc.cockTotal() == 0) {
		output("  You look like you don't got any of your own, but we've got glory holes in the back if you need to get some.  Just don't swallow too much, ok?</i>\"\n\nShe shows you to the back of the cave, which is boarded-up.  There are about 20 holes in boards, and most are empty.  While you watch, a few new dicks slide in, and just as quickly the goblin's daughters commence sucking and fucking them.  There are only a few you could take a crack at - do you blow one (and if so which one)?");
	}
	else {
		output("  I'll even do you the favor of letting you blow it in my mouth, I've already got a bun in the oven.  So what do you say?  Want a spooge and a haircut?  Or would you rather go get your payment from one of the gloryholes in the back, you " + pc.mf("kinky boy", "naughty girl") + "?</i>\"\n\n");
		output("In the very back of the salon you can see a boarded-up wall with holes cut in it, some of which are currently plugged by various monstrous penises.  Do you let the goblin blow you, or do you go suck your payment from one of them?");
	}
...
private function hairDresserRepeatGreeting():void {	
	...
	if(rand(5) == 0 && (pc.isDependant(Creature.DEPENDANT_CUM) || pc.hasPerk("Minotaur Cum Addict") || flags["COC.MINOTAUR_CUM_ADDICTION_STATE"] > 0)) {
		minotaurCumBukkakeInSalon();	
		return;
	}
	//Had babies announcement!
	if(flags["COC.LYNNETTE_PREGNANCY_CYCLE"] == 4 && flags["COC.LYNNETTE_CARRYING_COUNT"] > 0) {
		output("As soon as you enter the Salon, Lynnette is beaming ");
		if(pc.tallness >= 48) output("up ");
		output("at you. \"<i>Hey there, honey. Come for some more service or to check up on the kids?</i>\" She pats her now-flat midsection. \"<i>I popped them out last night. We had " + num2Text(flags["COC.LYNNETTE_CARRYING_COUNT"]) + " kids... all girls, of course.</i>\" She tucks a lock of beautifully styled hair behind an elfin ear. \"<i>Now you be sure and let me know when you want to contribute to my baby-bank again.</i>\"");
		if(flags["COC.LYNNETTE_BABY_COUNT"] > 0) output("\n\nA few new, green faces can be seen looking your way and blushing as they work the gloryholes, probably from a previous litter. They grow up so fast.");
		flags["COC.LYNNETTE_BABY_COUNT"] += flags["COC.LYNNETTE_CARRYING_COUNT"];
		flags["COC.LYNNETTE_CARRYING_COUNT"] = 0;
		//If favorite!
		if(flags["COC.LYNNETTE_ANNOUNCED_APPROVAL"] == 1) {
			favoriteSalonMenu();
			return;
		}
		else {
	...
	{
		if(flags["COC.LYNNETTE_MET_UNPREGNANT"] != 1) {
			flags["COC.LYNNETTE_MET_UNPREGNANT"] = 1;
			output("Lynnette's familiar face greets you at the door to her salon once more, and she waves you in with a beatific smile plastered upon her face. Clinging to her like a second skin, the businessgoblin's dress highlights her surprisingly flat midsection; she's not pregnant!");
	...
	//Standard repeats
		...
		output("Lynnette the goblin answers the door and lets you in, waving you deeper into her shop.  Her shining black dress barely contains her fertile-hips and jiggling chest as she greets you, \"<i>Welcome back, honey!  ");
		if(pc.cockTotal() == 0) {
			output("I see you still can't make payment for our services on your own.  No worries, the glory holes in the back are pretty popular with the demons and monsters, just go back there and catch some spooge and we'll see about helping with your hair, ok?</i>\"\n\n(There are a number of dicks in the glory hole, which do you want to deal with (if you're willing at all)?");
		}
		else {
			output("Will you be getting our normal services?  Just one BJ per hair treatment!  Or would you rather get it from the gloryholes in back?</i>\"");
		}
	}
	salonPaymentMenu();
}
...
private function gloryholeDoggie():void {
	...
	if(pc.slut() < 33) {
		output("It's a struggle to force your lips apart and take the strange thing in your mouth.  You feel like a freak, but make yourself get to work servicing the male who'll be covering your hair treatment today.  It tastes salty and sweaty and has a potent musky scent that excites you in spite of your better judgment.  You find yourself starting to get into it as you lick and slurp, humming with pleasure.  You reach up to touch the knot and the dog-cock jumps in your mouth, spurting a wad of thick cum into your throat.  You pull back most of the way and pump the cock, collecting the jism in your mouth.\n\n");
		pc.lust(15);
		pc.cor(0.25);
	}
	else if (pc.slut() < 66) {
		output("You open wide and take as much of the pointed dick as you can into your mouth.  A few times you nearly gag yourself, but you manage to get most of the way down to the thick swollen knot.  You start sliding back and forth, breathing in the musky animalistic scent of your 'client' and become aroused by the situation more and more.  You reach up and caress the knot, squeezing gently and stroking the dick until it unloads a thick wad of cum into your mouth.  You pull back and catch the fruits of your labor in your mouth, milking the dick with your hands until it's finished and the dick pops back through the hole.\n\n");
		pc.lust(25);
		pc.cor(0.25);
	}
	else {
		output("You throw yourself onto the swollen dog-cock and easily engulf the whole thing, spreading your lips wide around the thick knot.  You can feel it bouncing the uvula in your throat around but you manage to supress your gag reflex like a good slut while you work the sweaty dick, giving it a thorough tongue bath.  You wrap your lips around the base of the knot, feeling it start to stretch your jaws apart.  You pull back before it can lock you like that, and squeeze the knot in your hands.  It cums, unloading thick wads of gooey cum into your mouth.\n\n");
		pc.lust(30);
		pc.cor(0.3);
	}
...
private function gloryholeMinotaur(): void {
	...
	output("It's the largest available member in the room by a considerable margin, and from your position on your knees, it looks even more massive; it's over two feet long, and has three rings of prepuce spaced around its length. You can just barely span your entire hand around its thickness. You open your mouth wide and lean forward, taking the thick, spongy head into your mouth. The taste is incredibly strong, and its musk is as thick as the minotaur's dick itself. You moan in pleasure, and start bobbing your head back and forth, taking more and more of the dick into your mouth. You stroke the rest of the cock in time with your sucking. You can hear muffled grunting and snorting coming from the other side of the wall, obvious sounds of approval from your partner.\n\n");
	output("Driven on by the minotaur's reactions, you take more and more of the minotaur cock into your mouth. As you work your mouth around the first ring, you open up the top section of your [pc.gear], freeing up your [pc.fullChest]. With each pass, you're able to take more and more of the bull dick down your throat. ");
	if(pc.hasBreasts()) output("You caress your [pc.breasts] while moaning around the dick. ");
	output("When you get your mouth around the second ring, the minotaur's cock throbs sharply, and his cum explodes in your mouth. Your eyes bulge in shock, and you pull off the cock as quickly as you can. You swallow the first several shots of thick, rich minotaur cum, while the rest shoots all over your face and tits. You ");
	if(pc.hasBreasts()) output("hold up ");
	else output("arch your back to expose ");
	output("your [pc.chest] as cum rains down upon you. By the time his orgasm stops and his dick goes limp, your face and [pc.chest] are covered in a thick layer of spooge.\n\n");
	output("Your head spins from the minotaur's musk, and you idly mop up and swallow the cum on your " + pc.face() + ". A goblin aide comes in with a bowl, and gently scrapes the cum off your tits with a smooth, flat rock. Once you're cleaned up and you're dressed, the aide leads you back to Lynnette.\n\n");
	pc.lust(33);
	pc.cor(1);
...
private function goblinHairDresserFacefuck():void {
	//spriteSelect(38);
	//output(images.showImage("lynnette-blowjob"));
	clearOutput();
	output("Lynnette licks her lips and practically tears her way into your [pc.gear], having your crotch exposed in seconds.  Your [pc.cock] flops out immediately, slapping her on the nose as it grows hard.  She wraps both hands around you and begins pumping with practiced ease, flicking her tongue over your crown and wrapping her lips ");
	if(pc.cocks[0].thickness() >= 4) output("around as much of you as she can");
	else output("around your first few inches");
	output(".  She slips her tongue down your urethra, and arches her back, popping the straps on her dress and letting her oversized tits engulf your shaft.  Milk sputters and dribbles from the lactating goblin, lubricating her breasts and hands as she pleasures every inch of you.\n\n");
	output("It's impossible to hold back while receiving such expertly given stimulation, and you blow your load, bulging her cheeks out like a squirrel's.");
	if(pc.cumQ() >= 250) output(" She can't hold it all and suddenly starts swallowing as runnels of it escape her nose and a wave of it rolls down her lips.");
	output("  In time it ends, and she pops back, spitting most of the cum into a funnel.  It washes down a pipe and you have to wonder if there's some horny goblin girl at the other end with her cunt spread wide.");
...
private function hairDressingMainMenu():void {
	//spriteSelect(38);
	clearOutput();
	output("Lynnette offers and explains their options, \"<i>So what'll it be hun?  We could cut it down or give you a lengthening treatment. Or you can get a hair-dye to use on your own.  Just remember to come back in a few days for a touchup.</i>\"");
	if(flags["COC.UNKNOWN_FLAG_NUMBER_00142"] > 0) {
...
private function cutShort():void {
	...
	output("Lynnette and her daughters crowd around you with razor-sharp scissors, effortlessly paring down your [pc.hair].  When they've finished, you're left with ");
	pc.hairLength = 1;
	output(pc.hairDescript(true) + ".");
...
private function hairGrow():void {
	...
	output("Lynnette grabs a bottle and squirts a white fluid into your hair.  You really hope it isn't your payment.  But it must not be, as within short order you feel the added weight of ");
	temp = rand(3) + 3;
	//flags[kFLAGS.HAIR_GROWTH_STOPPED_BECAUSE_LIZARD] = 0;
	pc.hairLength += temp;
	output(num2Text(temp) + " more inches of [pc.hairColor] hair.");
...
private function removeHair():void {
	...
	output("You tell Lynnette that you'd like to have your hair removed.\n\n");
	output("Lynnette instructs you to take a seat and instructs her daughters to pare down your hair down to short length. They effortlessly cut your hair to short length. Next, Lynnette applies a special cream all over your [pc.hair].  Your hair starts to stiffen and falls out.  She gives your head a good cleaning afterwards.\n\n");
	output("<b>You no longer have a hair!</b>");
	pc.hairLength = 0;
...
private function buyDye(itype:ItemSlotClass):void{
	clearOutput();
...
private function dyeMenu():void {
	//spriteSelect(38);
	clearOutput();
	output("Lynnette pulls open a cabinet in the corner, displaying a wide array of exotic hair-dyes.  Which kind do you want?");
	clearMenu();
	addButton(0, "Blue", buyDye, new CoCDyeBlue());
	addButton(1, "Orange", buyDye, new CoCDyeOrange());
...
private function beardMenu():void {
	clearOutput();
	clearMenu();
	output("\<i>I can help you with your beard-related needs,</i>\" Lynnette says.")
	if (pc.hasBeard() && pc.beardLength > 0.5) addButton(0, "Cut Beard", cutBeard);
	if (pc.hasBeard() && pc.beardLength < 6) addButton(1, "Lengthen Beard", growBeard, 0);
	if (!pc.hasBeard()) addButton(1, "Grow Beard", growBeard, 1);
	if (pc.hasBeard() && pc.beardLength >= 0.5) addButton(2, "Beard Style", changeBeardStyle);
	if (pc.hasBeard()) addButton(3, "Remove Beard", removeBeard);
	addButton(14, "Back", hairDressingMainMenu);
}
...
private function cutBeard():void {
	clearOutput();
	clearMenu();
	output("Lynnette and her daughters crowd around you with razor-sharp scissors, effortlessly paring down your [pc.beard].  ");
	pc.beardLength = 0.5;
...
private function growBeard(mode:int = 0):void {
	...
	if (mode == 1){
		if (pc.mf("m", "f") == "f") {
			output("Lynnette stares at you. \"<i>Don't you think you'll look strange being a bearded lady?</i>\" she asks.\n\n");
			output("You insist her that you really want a beard, whether you're a man or a woman.\n\n");
			output("\"<i>Well... I'll get started now,</i>\" she says.\n\n");
		}
		
		output("Lynnette grabs a bottle and squirts a white fluid onto your chin and cheeks.  You really hope it isn't your payment.  But it must not be, as within short order your new beard sprouts! ");
		pc.beardLength = 0.2;
		if (temp >= 2) output("es");
		output(" of " + pc.hairColor + " beard.\n\n", false);
		output("\"<i>I'll let you choose your style before you leave,</i>\" she says.\n\n");
		changeBeardStyle();
		return;
	}
	//Grow existing beard.
	else {
		output("Lynnette grabs a bottle and squirts a white fluid onto your chin and cheeks.  You really hope it isn't your payment.  But it must not be, as within short order you feel the added weight of ");		
		var temp:Number = 5 + rand(5);
		temp /= 5
		pc.beardLength += temp;
		output(num2Text(temp) + " more inch");
		if (temp >= 2) output("es");
		output(" of " + pc.hairColor + " beard.");	
	}
	addButton(0, "Next", function():*{ processTime(20 + rand(10)); mainGameMenu(); } );
...
private function changeBeardStyle():void {
	clearOutput();
	clearMenu();
	output("What beard style would you like?");
	addButton(0, "Normal", chooseBeardStyleFinalize, 0);
	addButton(1, "Braided", chooseBeardStyleFinalize, 1);
	addButton(2, "Curly", chooseBeardStyleFinalize, 2);
	addButton(3, "Untamed", chooseBeardStyleFinalize, 3);
	addButton(4, "Anchor", chooseBeardStyleFinalize, 4);
	addButton(5, "Mutton Chops", chooseBeardStyleFinalize, 5);
	addButton(6, "Boxed", chooseBeardStyleFinalize, 6);
	addButton(7, "Chin Strap", chooseBeardStyleFinalize, 7);
	addButton(8, "Mustache", chooseBeardStyleFinalize, 8);
	addButton(9, "Handlebars", chooseBeardStyleFinalize, 9);
	addButton(10, "Goatee", chooseBeardStyleFinalize, 10);
...
private function chooseBeardStyleFinalize(choiceStyle:int = 0):void {
	...
	output("You tell Lynnette that you'd like to have your beard style changed to what you've indicated.\n\n")
	output("Lynnette and her daughters begin to mess with your beard with razor-sharp scissors and white fluid while they work to change your beard into what you've wanted.\n\n");
	pc.beardStyle = choiceStyle;
	output("After a while, you now have " + pc.beard() + "!");
...
private function removeBeard():void {
	...
	output("You tell Lynnette that you'd like to have your beard removed.\n\n", true);
	output("Lynnette instructs you to take a seat and she applies a special cream all over your " + pc.beard() +".  Your beard starts to stiffen and falls out.  She gives your chin a good cleaning afterwards.\n\n");
...
private function minotaurCumBukkakeInSalon():void {
	...
	//pc.slimeFeed();
	output("As the salon door swings closed behind you, a familiar, heavenly scent catches your nose and wicks into your brain, flooding you with need and molten-hot lust.  Lynnette saunters over with her lips slightly pursed and her body jiggling, but you brush her aside.  She's completely forgotten as you close in on the source of your olfactory bliss.  Your sigh dreamily while your pupils slowly dilate from the familiar chemicals pounding through your bloodstream");
	if(pc.hasVagina()) output(" and puffing up your [pc.vagina] with liquid arousal.\n\n");
	else if(pc.hasCock()) output(" and turning [pc.eachCock] into a turgid, pulsating mass.\n\n");
	else output("\n\n");
	
	output("You push your way past a few more staring goblin sluts, tearing off your [pc.gear] as you go.  The animal part of your brain recognizes that such needless trapping would just get in the way of all the thick, dripping, minotaur spunk just waiting to pump inside you.  Another door swings closed behind you while the scent drags you heedlessly deeper into the salon, closing in on the secluded glory-holes the goblins keep hidden away in the back.  You drool spittle down your neck while you lose yourself in the memory of that taste on your tongue, letting your body seek it out on autopilot.");
	if(pc.hasVagina() || pc.hasCock()) {
		...
		if(pc.hasCock()) {
			if(pc.cumQ() < 100) output("pre-cum ");
			else if(pc.cumQ() < 500) output("pre-cum a few inches wide ");
			else output("pre-cum over a foot wide ");
		}
		output("winds over the rough-hewn floor behind you, clearly marking your passage to the overpowering musk.");
	}
	output("\n\n");
	
	output("Time passes in a slow, torturous manner, but you turn the corner and sigh.  Salvation is at hand – you've reached the glory-holes, and to your delight there's nearly a dozen hard, dripping, delicious... mmmm... minotaur cocks crammed through the holes, some bulging out around the seams and leaking with need.  You saunter forwards, hips swaying sensually as the drug-like desire of potent minotaur musk pulls you ever closer.  Mewling happily, you take one of them in hand and stroke along its length, giggling as it pulses and leaks a stream of heavenly goo down your arm.   You lick it from your arm in one long, languid motion before pursing your lips around the minotaur's flared tip and sucking it hard as you quest for more of its heady ambrosia.\n\n"); 
	
	output("You hear a deep, strangled sigh as more and more delicious pre floods your mouth, lighting your senses up with a fireworks show of pleasure");
	if(pc.hasGenitals()) output(" and increasing the size of the puddle you're leaving on the floor");
	output(".  In between gulps of the fragrant fluid, you suck the swollen rod deeper and deeper into your mouth, gleefully suppressing your gag reflex as it pushes aside your uvula.  You work your throat, feverishly swallowing to suck it down.  A flare briefly distends your neck as it's taken deeper and deeper inside your form until your lips are puckered through the hole and you're sniffing the minotaur's pheromone-laced crotch.\n\n");
	
	output("The beast pulls back and starts pounding the hole, letting out deep, rumbling sighs of pleasure with each throat abusing glory-hole fuck.  If it weren't for his constant, bubbling preseed turning your throat to a slippery rut-hole you'd be rubbed raw, but instead you continue to lean forward, sniffing at his matted pubes every time he crushes them against your dick-stretched lips.  The other horny studs grunt with displeasure, but you reach out with your hands and grab hold two eager brutes' throbbing horsecocks.  Three out-of-sync heartbeats hammer through your hands and throat while you do your best to get them all off.\n\n"); 
	
	output("You let your eyes drift closed and lash your tongue back and forth across the minotaur's lowest medial ring, feeling his flare bulge larger inside you in response.  The beast-man batters at the wall, grunting in equal measures of passion and pent-up desire as his cock's tip stretches wide and locks itself inside you.  His urethra bulges in rhythmic contractions, stretching your lips around the already swollen shaft and signaling to your musk-addled mind that you're about to get what you desire.  You hum with blissful delight as your belly begins to gurgle, accepting long bursts of sticky minotaur drug.\n\n");
	
	output("Tied down by the cum-spurting flare locked in your gullet, you pump on the other two dicks with feverish speed and sway on your [pc.feet] as the narcotic spooge intoxicates your already-addled mind.  You can feel each muscular contraction pulsing through the bestial shaft while it finishes depositing the heavy, sticky load, and your eyes cross from the viscous inebriation that's pooling in your belly.  Drizzles of pre-cum soak into your arms and palms, drawn out from the frenzied pumping of your fists.  They won't come from just a hand though.  They need something... tighter.\n\n");
	
	output("You start moaning in drug-induced bliss, but your vocalized pleasure is interrupted by the squelching slurp of the softening shaft being pulled from your dick-puckered lips.  It drips a rope of cum over your mouth and chin as it pulls free from the wall, leaving behind one vacancy among the swarm of ready minotaur dicks.  You lick up your stud's leavings and purr in bliss, reaching through the hole to cup the departing minotaur's balls teasingly.  He grunts and walks out of your grip – sated for now.  You curl your fingers into a come-hither gesture, retract your arm through the sweat-lubricated glory-hole, and wait for one of the other minotaurs to come over.\n\n");
	
	output("The beast who kept your left hand so busy repositions himself at the now-vacant opening, and you decide to reward him for moving so quickly.  You lick the last of the salty cream from your lips and muse that he isn't the only one getting a reward, but the monstrous cow-man doesn't need to know that.");
	if(pc.biggestTitSize() >= 2) {
		output("  You wrap the pillowy flesh of your [pc.fullChest] around the new member, pleasantly surprised by its girth and wide, already-flared tip.  Maybe you could have gotten him off with your hands after all?  ");
		if(pc.biggestTitSize() < 6) output("Even so, you can't quite get your breasts the entire way around him, so you make up for it by pressing it harder into you with your busy hands.  ");
		output("Runnels of pre cover the shaft and squish wetly between your tits, turning your body into a slip-n-slide that reeks of hot, sticky minotaur sex.");
	}
	else {
		output("You crush the member tightly against your chest, smearing your torso with the copious pre until you're like a hot, wet slip-n-slide of minotaur sex.  You make up for it by wrapping your arms around his prodigious thickness and squeeze him tightly with your hands, stroking him off with your whole body with enough tightness to make the poor mino' think he's in some poor adventurer's asshole.");
	}
	output("\n\n");
	
	output("You lower and raise yourself, bouncing up and down on your [pc.legs] to enhance the ");
	if(pc.biggestTitSize() >= 2) output("tit-fucking ");
	else output("full-body handjob ");
	output("you're giving out.  In such a position, you're given the perfect view to watch as your strokes draw forth large bubbles of pre, and before you can lose your high, you latch onto the minotaur's vulnerable urethra and suck, tonguing in wide circles around it since you can't open wide enough to accommodate his flare.  Of course, all the attention just makes him flare wider, not just at the tip, but through the whole shaft.  ");
	if(pc.biggestTitSize() >= 2) output("The sudden girth change sends an enticing ripple through your [pc.fullChest] that's pleasurable enough to make you moan into his steadily-widening urethra.  ");
	output("You pull off and bounce faster, lost to your lust and the haze of sex-musk permeating the air, intent on seeing just how much this huge stud can spray onto you.\n\n");

	output("The minotaur does not disappoint.  His hole dilates from the size of the approaching cum-blast, and you sink down his shaft slowly until it's aimed directly at your face.  You close your eyes and feel the first explosion splatter over your [pc.hair] and forehead.  The next takes you full in the face, making it difficult to breathe through the mask of drug-like goo, but a few quick licks gives you a fix and makes it easy to breathe again.  On and on, the minotaur pumps fat ropes of spooge over your body until you're a syrupy, sticky mess that reeks of minotaur pheromones so strongly that dizziness overwhelms you and you fall free of the still-orgasming mino-cock, taking a few final blasts of seed on your [pc.chest] and crotch.  Your hands instinctively shovel a few loads into your [pc.vagOrAss]");
	if (pc.hasVagina()) {
		output(" while the animal part of your brain hopes it makes you pregnant with an equally girthy son");
		pc.loadInCunt(new CoCMinotaur());
	} else pc.loadInAss(new CoCMinotaur());
	output(".\n\n");
	//ADD PREG CHECK
	//Preggers chance!
	//pc.knockUp(PregnancyStore.PREGNANCY_MINOTAUR, PregnancyStore.INCUBATION_MINOTAUR, 70);
	pc.loadInMouth(new CoCMinotaur());

	output("Giggling, you stagger over to the next cock in line and turn around, possessed with the idea of taking its spooge in the most direct way possible – anally.   You pull your butt-cheeks apart and lean back, surprising one of the horny beasts with the warmth of your [pc.asshole] as you slowly relax, spreading over his flare.  He actually squirts ropes of something inside of you, but you've been around minotaurs enough to know that it can't be cum, at least not yet.  The slippery gouts of preseed make it nice and easy to rock back and spear yourself on the first few inches, ");
	if(pc.analCapacity() < 80) {
		output("delighting in the drug-numbed pain of stretching yourself beyond your normal capacity.");
	}
	else if(pc.analCapacity() < 140) output("delighting in the feeling of perfect fullness.");
	else output("delighting in realizing that you could take far larger than even this virile specimen!");
	//(buttchange here: 90)
	pc.buttChange(90, true, false);
	...
	output("The strength goes completely out of your [pc.legs], but you manage to hold yourself up long enough for your stud to flex his cock inside you and fill up your backdoor with more potent addiction.  You slide off, nerveless and still orgasming as jizz rains on your back from the abandoned cock, rolling off you to add to the ever-widening puddle of fluids on the floor.  Smiling vacantly and addled on more minotaur-cum than any one person has reason to handle, you stumble up on all fours and crawl to the next one.  More...");
	//[NEXT]
	pc.orgasm();
	pc.libido(2);
	pc.cor(2);
	...
	clearMenu();
	addButton(0, "Next", minotaurSalonFollowUp);
}
private function minotaurSalonFollowUp():void {
	//spriteSelect(38);
	clearOutput();	
	if(IncrementFlag("COC.UNKNOWN_FLAG_NUMBER_00142") == 1) {
		//Unlock mino cum purchase
		output("Lynnette slaps your face, waking you from your stupor.  What?  Where are you?  You look around and realize you're strapped into a barber's chair and caked with white goop, but why?\n\n");
		output("The goblin answers before you can vocalize your query, \"<i>Baby, you're so cum-hungry you make my daughters look like chaste virgins!  I haven't seen anyone go to town on minotaurs like that since my mother passed away, Marae guide her soul.  Normally we don't have that much use for the minotaurs outside of a bit of recreational drug-play, but you milked out so much cum that we can start selling it!  Isn't that great!?</i>\"\n\n");
		output("You nod, a little shocked by your behavior, but already licking your lips in anticipation.  Lynnette smiles and says, \"<i>You can buy some for the usual price, plus 60 gems, but for right now, how about some hair service?  It's on the house!</i>\"");
	}
	else output("You're woken up with a slap to the face, and still muzzy from your cum-induced glory-hole orgy, you stagger up to your [pc.feet].  Lynnette shakes her stinging palm and apologizes, \"<i>Sorry I have to keep doing that, but you're damned hard to wake after you go on these binges!  Anyways, you've earned some hair treatment if you want it.  Of course you could always buy more of your favorite fluid...</i>\"\n\n");
	//Menu
	output("Lynnette offers and explains their options, \"<i>So what'll it be hun?  We could cut it down or give you a lengthening treatment. Or you can get a hair-dye to use on your own.  Just remember to come back in a few days for a touchup.</i>\"");
	salonPurchaseMenu();
}
...
private function mudFacial():void {
	//spriteSelect(38);
	clearOutput();
	output("You sit back in a comfortable chair and pull on a lever to recline it.  The goblins buzz around you, gathering up 'special mud'.  You close your eyes, letting them plaster your [pc.face] with the stuff in hopes that it will improve your complexion as much as you've been promised.  A pair of cucumber slices are laid out on your eyes, obscuring your view.\n\n");

	output("With that finished, the crowd of busty, green-skinned women disperses to leave you in peace.  Time drags on, but eventually the mud hardens and cracks.  As if on cue, tiny hands emerge with wet rags to scrub your face clean.  Once they've finished, you feel like a whole new you!");
...
private function sandFacial():void {
	//spriteSelect(38);
	clearOutput();
	output("You sit back in a comfortable chair and pull on a lever to recline it.  The goblins buzz around you, gathering up 'special sand'.  You close your eyes, letting them splatter your [pc.face] with the rough, textured goop.  It doesn't feel very good, but that won't matter if it makes you as handsome as it's supposed to.\n\n");

	output("After a while the goblin girls come back and clean the stuff from your face.");
...
private function fuckLynnette():void {
	clearOutput();
	//output(images.showImage("lynnette-fuck"));
	//Checks to see if you've cum withint hte past 24 hours.
	if(IncrementFlag("COC.LYNNETTE_FUCK_COUNTER", false) == 0) {
		output("At your suggestion, Lynnette's eyelashes flutter dangerously low. She gives you a smokey look and asks, \"<i>Is that so?</i>\" She circles around you, looking you up and down with eyes that seem to bore right through your [pc.gear]. She must see something she likes, because she dips forward, parting her weighty melons around your ");
		if(pc.tallness >= 72) output("[pc.leg]");
		else if(pc.tallness >= 55) output("[pc.hips]");
		else output("side");
		output(" so that she can slip a manicured hand inside your underclothes. She handles [pc.eachCock] ");
		if(pc.cockTotal() > 1) output("in turn ");
		output("with the skill of a practiced snake-charmer before ");
		if(pc.balls == 0) output("pressing down near the [pc.sheath]");
	...
	//Repeat cum within 24 hours check
		...
		output(". Lynnette strokes a few times to make sure you're suitably engorged and then shifts lower, squeezing ");
		if(pc.balls == 0) output("around your [sheath] to get a feel for how pent-up you are");
	...
	output("\n\n\"<i>Mount me,</i>\" the goblin growls as she displays her nude body to you, \"<i>I want to feel you inside, [pc.name].  I want to make you cum for me.</i>\"  She smiles lecherously.  \"<i>You WILL cum for me.</i>\"  The glittering look of promise in her eyes leaves no room for doubt in your heart.  She's going to wind up with a cuntful of cum, and all you can do is provide it for her.");
	...
	output(" begun to bead clear droplets of precum. They run down the underside of your distended, eager cumvein");
	if(pc.cockTotal() > 1) output("s");
	output(" and race down towards your ");
	if(pc.balls > 0) output("[pc.balls]");
	else output("[pc.sheath]");
	output(", leaving a glossy, sexual shine in their wake, and they're only coming on faster and faster the longer you gaze upon Lynnette's emerald twat and inhale her prick-stiffening scent.");
	...
	output("\n\nYour thrusts transform from powerful lunges into a frenzied jackhammering so powerful that the impact ripples along her considerable thighs and ass.  Even her squirting tits shake and jostle with the pussy-plowing impacts as you mount the goblin like a rutting beast, battering your [pc.cockHead " + x + "] ");
	if(pc.cockVolume(x) > 24) output("up against her slightly-yielding cervix");
	else output("in as close to the cervix as you possibly can");
	output(", grunting and growling as you lose yourself in the feel of her soaked, velvet tunnel.  Your focus collapses entirely down to the feel of that slick tunnel around you and how far you can push into it, how strongly you can make it climax and squeeze around you.");
	output("\n\nLooking up at you in between reason-deadening orgasms, Lynnette purses her considerably puffy, cock-sucking lips to lick them.  She reaches up with both arms to grab you firmly about the neck, surprising you with her strength as she pulls you down, guiding your mouth to her moistened puckers for an eager kiss.  Her lips slip and slide across yours as you nuzzle mouth to mouth, your hips still working, and you trade saliva, sparring your tongues back and forth between your paired oral orifices.  One of her hands grabs your [pc.hair] in a tight grip, breaking the kiss long enough for her to growl, \"<i>Cum.  Now.</i>\"");
	
	output("\n\nYour body has been ready to go off for some time now, and the words act like an irresistible trigger for your climax, causing you to lurch your [pc.cock " + x + "] ");
	if(pc.cockVolume(x) <= 90) output("completely inside once more");
	else output("as deeply inside as you can");
	output(" and hold it there.");
	if(pc.cockTotal() > 1) {
		output("  Your other member");
		if(pc.cockTotal() > 2) output("s are");
		else output(" is");
		output(" spasming ");
		if(pc.longestCockLength() <= 12) output("on the greedy green slut's belly");
		else output("in the milk-stained valley between her tits, absolutely buried in squishy-soft breast");
		output(", jizzing in wild spurts.  The cum mixes with all the goblin-cream into a sticky-slick morass that paints Lynnette white.");
	}
	output("  Screaming with relief, you can feel the cum rushing out from your [pc.balls], through your middle, and down the length of your quim-buried tool, distending it as blobs of cum roll through the honeyed folds to explode against the well-endowed goblin's cervix. The touch of cum at the womb's entrance sends your lover back into another body-shaking climax; her eyes roll back, her pussy clamps down, and most importantly, her cervix dilates into a wide-open hole, sucking the cum straight into her womb as fast as you can shoot it.");
	output("\n\nTiny hands latch onto your shoulders as you pour your distilled lusts into the lucky woman's womb, digging deeply into your [pc.skinFurScales]. Lynnette's lips crash back against yours, her tongue burrowing into your mouth before she traps your lower lip, biting it painfully hard. Her eyes roll back, and her body shakes with so much pleasure that she almost looks to be having a seizure.  Your cock doesn't care about that any more than her sperm-hungry uterus does, the two organs far too busy exchanging genetic material to care what state their owners are in.");
	...
	else if(pc.cumQ() < 6000) output("\n\nThere's so much cum flowing out of your [pc.cockHead " + x + "] that Lynnette's once-taut tummy immediately plumps up with spermy weight, filled into a jiggly paunch by your first few blasts.  As you continue to flood her womb with your slurry of baby-making goodness, her belly continues to rise up between you, eventually arcing up into a gravid dome.  Somehow, her body holds it all inside, and only a small trickle of spunk leaks out of her luscious slit as you finish.");
...
public function lynnetteApproval(arg:int = 0):Number {
	if(arg != 0) flags["COC.LYNNETTE_APPROVAL"] += arg;
	if(flags["COC.LYNNETTE_APPROVAL"] < -100) flags["COC.LYNNETTE_APPROVAL"] = -100;
	else if(flags["COC.LYNNETTE_APPROVAL"] > 100) flags["COC.LYNNETTE_APPROVAL"] = 100;
