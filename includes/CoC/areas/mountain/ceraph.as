...
import classes.Items.Armor.CoCSeductiveArmor;
...
{
	clearMenu();
	addButton(0, "Next", function():*{ processTime(10 + rand(10)); mainGameMenu(); } ); // failsafe
	//If havent met her
	if (int(flags["COC.PC_MET_CERAPH"]) == 0) {
		//spriteSelect(7);
		showName("\nGOBLIN");
		ceraphFirstTime();
		return;
	...
	//If fetished level 1
	if (flags["COC.PC_FETISH"] == 1) {
		output("Though you spend an hour exploring, you don't find anything of note.");
		return;
	}
	//fetished level 2
...
private function ceraphFirstTime():void
{
	clearOutput();
	//spriteSelect(88);
	output("A cute goblin steps out of the bushes, holding a pair of green earrings up to the light.  She politely asks, \"<i>Hey cutey!  I've got some spare earrings I made the other day.  I was wondering if you want them?  As you can see, my ears are already full, and my daughters are a bunch of ungrateful cunts.</i>\"\n\n");

	output("The goblin's ears are covered in a dazzling array of piercings of all varieties.  She isn't dressed nearly as slutty as most of the goblins you've seen.\n\n");
	output("Do you let her put the earrings in your ears?");
	flags["COC.PC_MET_CERAPH"] = 1;
	if (flags["COC.PC_FETISH"] == undefined) flags["COC.PC_FETISH"] = 0;
	clearMenu();
	addButton(0, "Yes", ceraphFirstTimeVolunteer);
	addButton(1, "No", ceraphFirstTimeDecline);
}
...
private function ceraphFirstTimeDecline():void
{
	clearOutput();
	//spriteSelect(7);
	output("You decline, informing the petite girl you have no interest in her earrings.  She shrugs and laughs, her form distorting as it grows taller, twisting as her skin lightens, her breasts bulge, and a pair of cute horns breaks out of the skin of her forehead.  She pockets the earrings and stretches, revealing a glistening demon-snatch capped with a modest cock growing from where her clit should be.\n\n");

	output("The demon introduces herself, \"<i>Well if you want to do this the hard way, that's fine by me.  The ones that fight the hardest break down into the most submissive slaves.  Just remember that this is the beginning of your life as Ceraph the Omnibus' slave-" + pc.mf("boy", "girl") + ".</i>\"\n\n");
...
private function ceraphFirstTimeVolunteer():void
{
	clearOutput();
	//spriteSelect(7);
	output("The goblin motions for you to bend over, and you do.  She holds the earrings up on your ears, getting them aligned just right.  ");
	if (pc.earsPierced > 0) output("Miraculously, your old earrings just fall out.  ");
	output("The earrings snap into place with a painful and definitive *click*, and dizziness hits you so hard that you nearly fall.  The goblin catches you, steadying you as you blink a few times and adjust to the sensation.\n\n");

	output("When you feel steady again, you look up and see that the tiny goblin has transformed into a nude, hermaphroditic demon!  She grins knowingly and caresses your body.  You step back and she says, \"<i>Awww, don't look so shocked dear.  You just took your first step into becoming one of my pets!  I'm Ceraph, and we're going to be getting VERY familiar with each other.</i>\"\n\n");
	output("Her forwardness brings a blush to your face and kindles a bonfire in your loins.  Somehow the thought of being nude in front of her is far more arousing than it ought to be.  You keep trying to banish the thought, but all you can think of is your nudity and how much hornier it's making you.\n\n");

	output("Ceraph breaks your reverie with a chuckle, \"<i>Oh baby, I see you're starting to figure it out.  Let me break it down for you.  <b>Those earrings have given you a fetish for exhibitionism.</b>  Being exposed and humiliated makes you hornier than a dog in heat, and even if you found a way to remove them, it's too late – they've forever linked nudity and humiliation with desire and lust in your easy little mind.  Get used to it and I'll be back to introduce you to another fun little fetish.</i>\"\n\n");
	
	if (pc.exhibitionism() < 66) output("The slutty demon walks away with a sexy sway, and you're too confused and turned on by your predicament to pursue her.  It looks like you'll have to do with a little extra arousal whenever you're exposed in public.");
	else if (!pc.isExposed()) output("You don't actually feel any diffferent, though...");
	else output("You can't help but laugh - you are already hardcore exhibitionist, walking around exposed!");
...
private function ceraphFirstTimeRepeat():void
{
	clearOutput();
	//spriteSelect(7);
	output("Ceraph saunters out of the bushes and gives you a disapproving look, \"<i>I tried this the easy way.  Now we're doing it the hard way.</i>\"\n\n");

	output("She uncurls a whip that lights aflame and says, \"<i>Time for your spanking!</i>\"\n\n");
...
public function CoCCeraphPCVictory():void
{
	...
	if (flags["COC.CERAPH_RAPE_COUNT"] >= 4 && !ceraphIsFollower()) 
	{
		submissiveCeraphOffer();
		return;
	}
	
	if (inCombat() && enemy.HP() <= 1) output("Ceraph collapses in a beaten, bloody heap.\n\n");
	else output("Ceraph masturbates futilely, too driven by desire to fight.\n\n");
	
	if (inCombat()) addButton(14, "Leave", function():* { processTime(20 + rand(10)); CombatManager.genericVictory(); } );
...
internal function buttRapeCeraph():void
{
	...
	var x:Number = pc.cockThatFits(ceraph.analCapacity());
	if (x < 0) x = 0;
	clearOutput();
	output("Completely defeated, the haughty demoness lies face down in the dirt, ");
	if (ceraph.HP() <= 1) output("her tail hanging limp between her legs.  She spits a wad of bloody saliva and says, \"<i>What's the matter?  Beat a girl senseless and lose your nerve?  I thought you were going to rape me.</i>\"");
	...
	if (ceraph.HP() <= 1) output("  Cunt-juice splatters over your [pc.legs] as Ceraph's now-energetic tail plunges into her sodden box, pumping into it in time with the penetration of her asshole.");
...
internal function maleFuckCeraphsPussy():void
{
	...
	if (pc.cockVolume(x) <= ceraph.vaginalCapacity()) {
		//[Male] Fuck 'Dat Pussah
		//[lust]
		if (!inCombat() || ceraph.lustQ() > 99) {
		...
		if (pc.cockVolume(x) < cockVolume(40)) {
...
internal function rideCeraphsCockLikeaBAWSSexclamation11eleven():void
{
	...
	if (pc.vaginalCapacity() < ceraph.biggestCockVolume()) output("unable to take the entire exquisite cock inside you.  The nubs squirm and wiggle, practically vibrating and massaging your internal muscles, and you're unable to stop yourself from bouncing on top of her, spearing yourself repeatedly upon the thick rod.\n\n");
	...
	pc.cuntChange(0, ceraph.biggestCockVolume());
	output("\n\n");
...
/*MISC NOTES
 ...
 //L1 Ear Piercings
 pc.earsPShort = "green gem-stone ear-studs";
 pc.earsPLhort = "Green gem-stone ear-studs";
 ...
 //L3 Cock
 pc.cocks[0].pShort = "diamond cock-stud";
 pc.cocks[0].pLong = "Diamond cock-stud";
...
public function CoCCeraphPCLoss():void
{
	...
	flags["COC.CERAPH_RAPE_COUNT"] = 0;
	//spriteSelect(7);
	if (int(flags["COC.PC_FETISH"]) == 0) loseToCeraphAndGetPiercingsInEarsOrSomethingSomehow();
	else if (flags["COC.PC_FETISH"] == 1) loseToCeraphAnGetPierced2();
	else if (flags["COC.PC_FETISH"] == 2) getPiercedByCeraphLoss3();
	else {
...
private function loseToCeraphAndGetPiercingsInEarsOrSomethingSomehow():void
{
	clearOutput();
	//spriteSelect(7);
	output("Ceraph advances on your ");
	if (pc.HP() <= 1) output("prone ");
	else output("lust-consumed ");
	output("form, whispering calmly as she pulls a pair of gleaming, green piercings from a pouch, \"<i>Oh, don't worry baby, you're going to love this so much.  These piercings are special, and they'll help you learn to enjoy the things I'm going to teach you.</i>\"\n\n");

	if (pc.earsPierced > 0) output("With a dismissive gesture, Ceraph murmurs words of dark magic and the piercings in your ears drop to the ground, leaving behind smooth, unbroken flesh.  ");
	//(BUSINESS AS USUAL GENTLEMEN)
	output("The demon places a hand on your forehead and rubs your temples.  Strength drains from your body with every touch, until you're left helpless and paralyzed.  She snaps an earring into your left ear, and dizziness washes over you.  A moment later she's piercing the other side, and the now-familiar vertigo that accompanies it seems to come and go quicker than before.  You hope she doesn't take your [pc.gear] while you're paralyzed, leaving you to roam the realm totally exposed.  Confusion and waves of new desire battle in your mind as you try to come to grips with the odd thought.\n\n");
	...
	output(" as the new thoughts and desires settle themselves inside you.  She gives you a gentle pat and explains, \"<i>It's ok baby, you're an exhibitionist now.  The moment I put those cursed earrings in your cute little ears, they slid their magic deep inside you, teaching you how to be a good " + pc.mf("boy", "girl") + " and get turned on by exposing yourself.</i>\"\n\n");
	
	if (pc.exhibitionism() < 66) output("Judging by how hot the idea is getting you, Ceraph is right – <b>whether you like it or not, you're an exhibitionist now.</b>");
	else if (!pc.isExposed()) output("You don't actually feel any diffferent, though...");
	else output("You can't help but laugh - you are already hardcore exhibitionist, walking around exposed!");
...
private function loseToCeraphAnGetPierced2():void
{
	clearOutput();
	//spriteSelect(7);
	output("Ceraph giggles as she closes in on you once again.  Unable to fight back, you lie there as she once again massages your temples, using her magic to paralyze your body.  She's all too happy to build up the suspense as she pulls out a pair of shining black studs, \"<i>Oh, did you want to know what these are going to teach you?  Well, how about I slide them into your ");
	if (pc.hasFuckableNipples()) output("slut");
	else if (pc.nippleLength(pc.biggestTitRow()) < 1) output("cute");
	else output("tight");
	output("-nipples and you tell me all about your fetishes and which one makes you the hottest.  OK?</i>\"\n\n");

	//If already pierced
	if (pc.nipplesPierced > 0) output("With a dismissive gesture, Ceraph murmurs words of dark magic, and the piercings in your nipples drop to the ground, leaving behind smooth, unbroken flesh.  ");
	//Business as usual!
	output("The demon doesn't give you a chance to reply, instead she focuses on aligning the business ends of the piercings with your sensitive nipple-flesh.  Your right " + pc.nippleDescript(0) + " is pierced in one smooth motion, nearly making you scream in pain.  As she fastens it on, you feel goosebumps spread over your body in a wave.  The second piercing doesn't seem to hurt as bad, but the sensation of spreading goosebumps is far more noticeable.  Your eyes dart wildly, afraid at what new kind of fetish Ceraph has forced upon you.\n\n");

	output("She smiles down at you and teases, \"<i>Still haven't figured it out?  I'll give you a hint:  Isn't being paralyzed nearly the same as being tied down?</i>\"\n\n");

	output("Your body goes beet-red as it suddenly tries to struggle against her invisible restraints.  It really is!  You nearly cry as the restraint turns you on more and more.  Ceraph's cursed piercings have given you a fetish for being restrained.  You nearly faint when you think of the tentacle monsters, and you know you have no hope of resisting if they ever bind you up.  <b>Somehow you'll have to get used to being a bondage fetishist...</b>");
	processTime(15);
	pc.lust(25);
	pc.cor(5);
	pc.nipplesPierced = 1;
...
private function getPiercedByCeraphLoss3():void
{
	clearOutput();
	//spriteSelect(7);
	output("The demoness pulls out a diamond-studded piercing and closes in on you, her cock hard, her pussy moist, and her hips swaying seductively as she advances.  Ceraph gives you a serious look and warns you, \"<i>You know how close you are to being my pet, don't you?  You just need this piercing and a little time to get used to it.  Then if you're good, I'll strip you naked, tie you down, and let you lick me out.  We can even let the imps watch!</i>\"\n\n");

	output("The idea turns you on immensely, and you pant and gasp as ");
	if (pc.hasCock()) {
		output("pre-cum oozes from [pc.eachCock].");
	}
	...
	//[dicks]
	if (pc.hasCock()) {
		output("grabs your " + pc.cockDescript(0));
		if (pc.cocks[0].pierced > 0) output(", the old piercing clattering to the ground as it slides out of your flesh, ");
	...
	output("As she finishes, you realize she didn't even try to restrain you!  Feeling a second wind, you twist out of her grasp before she can take advantage of your new piercing and lash out with a vicious punch.  Ceraph doesn't even try to move.  The demon watches calmly as your fist gets closer and closer, then stops, the punch turning into a gentle caress.  Horrified, you try to smack her with a " + pc.leg() + ", but all you manage to do is stroke it along her inner thigh.\n\n");

	output("\"<i>So, I take it you like it?  You'll never be able to raise a hand in anger again.  I guess if you want to win fights you'll have to tease your foes into submission with that luscious body.  I suppose that might be hard to do when you're getting off on exposing yourself and cumming from the thought of being tied down,</i>\" she laughs.\n\n");

	output("You tremble in impotent fury at the violation as the demoness flounces away.  You'd pursue her, but between her ideas and exposing your crotch to the entire area, you need to cum more than anything.  You scurry back to camp, too horny to think straight and your new piercing aching just enough to keep you from forgetting about it.");
	processTime(15);
	pc.lust(25);
	pc.cor(5);
	CombatManager.genericLoss();
...
private function finalCeraphEncounterChoiceNo():void
{
	clearOutput();
	//spriteSelect(7);
	output("You tell Ceraph no, emphatically, over and over.  She gives you a smirk and says, \"<i>Whatever.  It's only a matter of time.  You'll join me soon, pet.</i>\"\n\n");

	output("With that declaration she departs, leaving you confused and horny.");
	processTime(2);
	clearMenu();
	addButton(0, "Next", function():*{ processTime(10 + rand(10)); mainGameMenu(); } );
}
...
private function finalCeraphEncounterStartFight():void
{
	clearOutput();
	//spriteSelect(7);
	output("You tell Ceraph no, and let her know ");
	if (pc.cor() <= 50) output("you'll stop her devilish scheme");
	else output("you'll be HER " + pc.mf("master", "mistress"));
	output(".  She snarls and drops the harness, preparing to defend herself.  It looks like you've got a fight!");
	clearMenu();
	addButton(0, "Next", fightCeraph);
}
...
private function agreeToBecomeCeraphsFuckpetBondageToyBitchSlut():void
{
	clearOutput();
	//spriteSelect(7);
	output("At this point, you know she's right.  The desires Ceraph has awakened inside you will never go away.  In spite of the hassle they've caused, you've come to love being so uncontrollably aroused by such humiliating treatment.");
	if (pc.hasVagina()) output("  Your " + pc.vaginaDescript(0) + " tingles as you wonder how many piercings and fetishes she could fit into your labia.");
	else if (pc.cockTotal() > 0) output("  Your " + pc.cockDescript(0) + " grows rock hard as you wonder if she'll give you a Jacob's ladder full of new fetishes and thoughts.");
	else output("  Your body hums with arousal as you wonder if she'll give you a cock and cunt, pierce them both, and fill you with new fetishes and sexy submissive thoughts.");
	...
	output("She spends a few moments undressing you, then gives your " + pc.buttDescript() + " a few smacks to suitably redden it.  Horrified at the treatment but more turned on than ever, you endure the humiliation, crawling after your mistress as she leads you to a new, sexy life on an impromptu leash.  You lose track of time, just focusing on putting one hand in front of the other.  Multiple times you catch glimpses of demons and monsters from the fringe of your vision, but none of them seem keen on approaching your mistress or her new slave, you.  Every time you feel their eyes on your naked [pc.skinFurScales], you're rocked with sexual pleasure.\n\n");

	output("Absorbed in your own thoughts, you barely notice the light dim or the solid thunk of a door closing behind you.  Happy moans bring you out of your dazed state.  You've arrived at Ceraph's lair.  The demon is barely paying attention to you, holding on to your leash as she walks around greeting her other pets.  She ruffles the hair of a tiny boy wearing naught but doggie-ears and an inventive looking butt-plug that gives him the appearance of having a tail.  Ceraph tickles the crotch of a girl with more than a few demonic features.  She's tied to the wall, with two vibrators strapped hilt-deep in her abused nipple-cunts.  Her original sex is drooling but totally unfilled as she squirms in her straps while Ceraph touches her.\n\n");

	output("There are many more slaves just like those two throughout Ceraph's home.  Some are little more than furniture, some she treats like artwork, and others she uses for her sexual enjoyment.  She announces, \"<i>Pets, meet your new " + pc.mf("brother", "sister") + ".  I think I'll just call " + pc.mf("him", "her") + " 'pet'.  " + pc.mf("He", "She") + " needs to learn her place and we're going to be teaching and piercing " + pc.mf("him", "her") + " all week.</i>\"\n\n");

	output("The whip is uncurled from around your neck, and you look up, unsure of what to do.  Ceraph coos, \"<i>Good pet.  Just stay like that and pout for me.</i>\"\n\n");

	output("Not seeing the harm and seeking to endear yourself to your mistress, you thrust your lower lip out.  Your obediance is rewarded by a sharp snap of dazzling pain.  Licking experimentally, you feel the smooth band that loops over the center of your lip. ");
	//if pierced there
	if (pc.lipPierced > 0) output("The previous piercing has vanished!  ");
	output("Mistress is so kind to give you such a pretty piercing!  The ease you find yourself calling her mistress shocks you, but that's what she is, isn't she?  She's your mistress and owner, and you are her pet.  You catch her looking at you expectantly and you realize she's waiting for something.\n\n");

	output("You lick your new lip-ring nervously and say, \"<i>Thank you mistress, pet is so happy to be pierced again!</i>\"\n\n");

	output("What the hell?  Did you just call yourself pet?  You struggle internally, thinking, \"<i>When did pet start thinking of " + pc.mf("him", "her") + "self as pet?</i>\"  A tingle of new arousal shivers through you – Mistress Ceraph has made it impossible for you to think or call yourself anything other than pet!  What else is she going to do to you?\n\n");

	output("\"<i>Oh it's so cute watching my pets and toys truly learn their new names.  Now come here pet, a good pet wears a collar so everyone knows who " + pc.mf("he", "she") + " belongs to,</i>\" says Mistress as she pulls a thick black collar off a hook on the wall.  Compliant, you allow her to fasten it around your neck, waggling your ");
	if (!pc.hasTail()) output("backside");
	else output("tail");
	output(" back and forth to show Mistress how happy you are.  Obedience slowly crowds your mind, rolling over your other thoughts and concerns inexorably.  Your worries, cares, and troubles slide away, replaced with an all-consuming need to please.");
	addNextButton(agreeToBeABitchPt2);
}
...
private function agreeToBeABitchPt2():void
{
	clearOutput();
	//spriteSelect(7);
	output("Months pass in pure bliss.  Since adoption into Mistress Ceraph's harem, pet gained over a dozen piercings and nearly twice as many fetishes.  Pet learned to love spanking, and developed the ability to cum just from having " + pc.mf("his", "her") + " nipples pinched or being paddled.  Ceraph enjoyed that for a while, but she did eventually tire, and pet was given a fetish for orgasm denial");
	if (pc.cockTotal() > 0) output(" along with a magical cock-ring");
	output(".  Two weeks of drug and bondage induced edging nearly broke pet's mind, but Mistress was nice and let " + pc.mf("him", "her") + " cum over and over after getting a new piercing and worshiping her feet for a few hours.\n\n");

	output("The champion that left Ingnam so long ago is totally unrecognizable, body and soul.  The spirit that once burned bright with the desire to save the innocents of the village has been warped and twisted beyond repair.  The once-champion known only as 'pet', not even worthy of a proper name, spends all " + pc.mf("his", "her") + " time lusting after Mistress Ceraph, taking part in her depraved orgies and willingly submitting to any kinks or debaucheries that are presented to " + pc.mf("him", "her") + ".  Pet's life of individuality and choice is over, all that remains is pleasure and pain.");
...
private function level3CeraphOfferNo():void
{
	clearOutput();
	//spriteSelect(7);
	output("You let Ceraph know that you refuse to let her violate your body and mind in such a way.  She shrugs nonchalantly and tugs on the crop, stretching it into a whip that bursts into flames.\n\n");
	output("\"<i>Mortals always want to do things the hard way,</i>\" she sighs.\n\n");
	output("<b>You've got a fight on your hands!</b>");
...
private function yesOHGODYESPIERCELEVEL3():void
{
	clearOutput();
	//spriteSelect(7);
	output("You nod, biting your lip anxiously and undoing your armor to give her better access to your body.  The demoness pulls out a diamond-studded piercing and closes in on you, her cock hard, her pussy moist, and her hips swaying seductively as she advances.  Ceraph gives you a serious look and warns you, \"<i>You know how close you are to being my pet, don't you?  You just need this piercing and a little time to get used to it.  Then if you're good I'll strip you naked, tie you down, and let you lick me out.  We can even let the imps watch!</i>\"\n\n");
	output("The idea turns you on immensely, and you pant and gasp as ");
	if (pc.hasCock()) {
		output("pre-cum oozes from [pc.eachCock].");
	}
	else if (pc.hasVagina()) {
		output("feminine moisture drools from between your lips and your " + pc.clitDescript() + " turns into a hard button.");
	}
	else output("your body aches for release.");
	...
	if (pc.cockTotal() > 0) {
		...
		if (pc.cocks[0].pierced > 0)
			output(", the old piercing clattering to the ground as it slides out of your flesh,");
		output(" and snaps the diamond stud through your sensitive flesh, making your vision haze red in pain.");
		pc.cocks[0].pierced = 1;
		pc.cocks[0].pShortDesc = "seamless, diamond cock-stud";
		pc.cocks[0].pLongDesc = "Seamless, diamond cock-stud";
	}
	//[cunts]
	else if (pc.hasVagina()) {
		output("spreads your lips");
		if (pc.vaginas[0].clitPierced > 0) output(", the old piercing clattering to the ground as it slides out of your flesh,");
		output(" and gets ahold of the flesh around the base of your " + pc.clitDescript() + ".  With practiced ease, she snaps the piercing closed, attaching the diamond stud to you while the pain fuzzes your vision red.");
		pc.vaginas[0].clitPierced = 1;
		pc.vaginas[0].clitPShort = "seamless, diamond clit-stud";
		pc.vaginas[0].clitPLong = "seamless, diamond clit-stud";
	}
	else {
		output("gives you a mildly annoyed look before she snaps the diamond stud into your eye-brow, piercing it.  ");
		if (pc.eyebrowPierced > 0) {
			output("She discards your old jewelry like a piece of garbage.  ");
		}
		output("It hurts more than it ought to, fuzzing your vision red.");
		pc.eyebrowPierced = 1;
		pc.eyebrowPShort = "seamless, diamond eyebrow-stud";
		pc.eyebrowPLong = "seamless, diamond eyebrow-stud";
	}
	output("  As she finishes, you feel grateful that she's so willing to lavish you with such sexy jewelry and teach you how to enjoy these wonderful new experiences.  You give her a wet kiss, wondering what she's done to you, but in the end you don't really care.  Somehow you know it's going to make you feel even more like a horny, hot, and obedient sex-pot for your demonic mistress.\n\n");
	output("\"<i>So, I take it you like it?  You'll never be able to raise a hand in anger again.  I guess if you want to win fights you'll have to tease your foes into submission with that luscious body.  I suppose that might be hard to do when you're getting off on exposing yourself and cumming from the thought of being tied down,</i>\" she laughs.\n\n");
	output("You tremble with barely restrained lust as the demoness flounces away.  You'd pursue her, but between her ideas and exposing your crotch to the entire area, you need to cum more than anything in the world.  You scurry back to camp, too horny to think straight and your new piercing aching just enough to keep you from forgetting about it.");
	flags["COC.PC_FETISH"] = 3;
...
private function hugeCorruptionForceFuckCeraph():void
{
	...
	if (ceraph.HP() <= 1) output("beaten, prone");
	...
	output(pc.cockDescript(x) + ", feeling the blood pump into it.  The thick, penile flesh inflates to the ludicrous size it adopts every time you get hard.  Ceraph's eyes widen into dinner plate-sized saucers of shock as she beholds the behemoth that sprouts from your loins.  She actually tries to scrabble away in the dirt, but she fails miserably.  ");
	if (ceraph.HP() <= 1) output("Her hands are too busy starting to pump away at her nodule-covered demon-dick to help her crawl away.");
	else output("Both her hands are too busy at her groin to help her escape.");
	output("\n\n");

	output("You drop the swollen mass of dick-flesh on top her, pinning her under the weight of your lust.  Ceraph gasps in shock and pain, but her hard nipples are pressing tightly into your " + pc.cockDescript(x) + "'s underside.  The slut is loving it – could she secretly be a submissive?  It doesn't matter one way or the other; this demon is going to pay.  You shimmy back, dragging the elephantine fuck-stick over the defeated demon's body until the bloated tip is resting on her purplish demon-snatch.  She screams, \"<i>OH FUCK NO!  That would kill me!  Please, don't!</i>\"\n\n");

	output("Please?  She said the magic words.  ");
	if (pc.statusEffectv1("Exgartuan") == 1) output("You pat your " + pc.cockDescript(x) + " and say, \"<i>A little help please, Exgartuan?</i>\"  ");
	else output("You blink your eyes closed and focus your corruptive powers in your crotch.  ");
	...
	if (pc.cocks[x].cLength() > 84) output("You've still got a lot more dick to get wet, and you start pushing the rest through her, using her like a plus-sized, organic onahole.  ");
		...
		output("You sigh happily, rubbing your ");
		if (!pc.hasSheath()) output("groin");
		else output("sheath");
		output(" against her slippery, split slit, before you pull back and start fucking her like a plus-sized, organic onahole.  ");
	}
	...
	else if (pc.cumQ() < 1000) output("You squirt your copious cum everywhere, splattering the dirt and dripping white goo onto Ceraph's face, forehead, and hair.  By the time you finish she's got a nice glaze going on.");
	else output("You expel huge waves of seed in what can only be described as eruptions.  They splatter into the dirt, forming thick puddles that are joined together with each successful splash of goo.  In between blasts, a steady stream leaks from the tip, travels down the underside, and catches on Ceraph's lip.  The spunk-flow is pulled over her face and into her hair by gravity.  She's glazed [pc.cumColor] and dripping into a puddle by the time you finish.");
	output("\n\n");

	output("You push the used fuck-toy off your " + pc.cockDescript(0), false);
	if (pc.cockTotal() == 2) output(" and marvel at the [pc.cum] your other cock dumped into the puddle");
	else if (pc.cockTotal() > 2) output(" and marvel at the [pc.cum] your other cocks dumped into the puddle");
	...
	output("  That's rarer than you know, so I'm feeling a little generous.  ");
	if (flags["COC.PC_FETISH"] > 0) output("I'll remove some of my magic from you if you want, and I'll even let you keep the piercing.  Would you like that?");
	else output("I'd be willing to give you a few extra gems to keep you motivated to do something like this again.  I LIKE surprises. How about it?");
	output("</i>\"\n\n");
	//if (pc.statusEffectv1(StatusEffects.Exgartuan) == 1) {
		//output("Exgartuan mumbles, \"<i>Yeah I love me too.  Now while you two bitches kiss and make up, I'm gonna take a nap.</i>\"\n\n");
		////Put Exgartuan to bed.  Awww he's so cute!
		//pc.addStatusValue(StatusEffects.Exgartuan, 2, 5);
	//}
	output("(Do you accept Ceraph's Offer?)");
	//Y/N – remove 1 fetish level or +10 gems
	clearMenu();
	addButton(0, "Yes", ceraphsNiceOffer);
...
private function ceraphsNiceOffer():void
{
	clearOutput();
	//spriteSelect(7);
	if (flags["COC.PC_FETISH"] > 0) {
		output("Ceraph touches your forehead, and it feels like her claws are raking through your brain, plowing through the surface of your thoughts.  She pulls away, leaving you feeling a little dazed.  You blink and mentally go over your fetishes - ");
		if (flags["COC.PC_FETISH"] == 1) output("your exhibitionism is gone!");
		else if (flags["COC.PC_FETISH"] == 2) output("your bondage fetish is gone!");
		else output("your pacifism is gone!");
		flags["COC.PC_FETISH"]--;
	}
	else {
		output("Ceraph shakes some of the jism out of her hair and reaches under the tangled mass.  Her hand comes out with a small pouch of gems, and she tosses them to you.  You open it up and are surprised to find 10 gems inside.");
		pc.credits += 100;
	}
...
public function friendlyNeighborhoodSpiderManCeraph():void
{
	clearOutput();
	//spriteSelect(7);
	output("While exploring, a sudden compulsion to strip naked runs through your body, ");
	if (pc.cor() > 66) output("and without a second thought you disrobe, tossing your [pc.gear] to the side.  It thumps heavily in the dry mountain dust.");
	else if (pc.cor() > 33) output("and you squirm and writhe within your [pc.gear], the material itching on your skin.  Ultimately, you manage to resist.");
	else output("and you stifle a laugh at the ridiculous thought, ignoring your fingers while they scratch at the itch under your gear.");
	output("  Your [pc.skin] burns hot, raging from fetish-enhanced lust and daydreams, but you shake your head in time to see a pair of inky black orbs rising from behind a nearby boulder.\n\n");

	output("You wrench your gaze from the treacherous, alien eyes and glance lower, but it doesn't help.  There's nothing but flawless, sweat-moistened demon-skin, jutting purple breasts, and a crotch that combines the finest attributes of both genders.  It's the demon, Ceraph!\n\n");

	output("\"<i>Oh my, does someone like what they see?  ");
	if (pc.cor() > 66) output("I can see just how hot I'm making you, I'm so glad you gave in and got undressed for me");
	else output("You can't take your eyes off me.  It's a shame you didn't get undressed.  Didn't anyone tell you this mountain has a dress code?  Or should I say, an undress code");
	output(",</i>\" she says.\n\n");

	output("You [pc.readyWeapon], but Ceraph just throws her head back and laughs in your face.  \"<i>");
	if (pc.cor() > 66) output("Are you going to try to fight me in the buff?  If you're going to do that then you can go ahead and get dressed, I wouldn't want to accidentally leave a scar in the wrong place.");
	else output("Are you going to try to fight me?  If you're going to do that, I suppose we can.  You do seem to need some practice at indulging your fetishes.");
	output("</i>\"\n\n");

	output("The demoness lifts one heeled foot high onto a boulder and winks at you, letting her whip rub up and down her hard, nodule-studded shaft.  Her tail rises languidly between her perfect, lissomelegs, rubbing the glistening wet delta of her sex enticingly.  The demon coos, \"<i>Oh, I do love putting on a show for my future pets.  Tell me, did you really come up here to fight?  I'd rather lie back in the sunlight, rubbing my fingers over my glistening skin and showing everyone just how wondrous sex with me would be.  You can even go if you want, or maybe you'd have something worth trading with me?</i>\"");
	pc.lust(5 +pc.cor() / 10 + pc.libido() / 20);
	//[Fight] [Trade] [Run]
	clearMenu();
	addButton(0, "Fight", fightCeraph);
...
private function runFromCeraphsNiceDeal():void
{
	...
	//spriteSelect(7);
	output("You ");
	if (pc.cor() > 66) output("grab your equipment and ");
	output("beat a hasty retreat from the trickster of fetishes.  Her masturbatory moans chase you down the mountainside back towards your camp, spiking the already-burning furnace of your lust.");
	//(+10 lust + 10lust/piercing)
	pc.lust(10 + flags["COC.PC_FETISH"] * 10);
	processTime(5);
	clearMenu();
	addButton(0, "Next", function():*{ processTime(10 + rand(10)); mainGameMenu(); } );
}
...
private function tradeCeraphSomething():void
{
	clearOutput();
	//spriteSelect(7);
	output("Ceraph looks you up and down like an appraiser examining an antique, never stopping her languid masturbation.  You stand there, hands on your hips, waiting for her to finish while you try not to stare too long at her beading pre-cum or dripping twat-juice.\n\n");
	clearMenu();
	//(BIMBO LIQUER) 
	if (pc.hasItem(new CoCBimboLiqueur())) {
		output("She finishes and smiles widely.  \"<i>Is that bimbo liqueur?  That stuff is so hard to find, even for a demon like me.  I tell you what, I'll release you from ");
		if (flags["COC.PC_FETISH"] > 1) output("all the fetishes");
		else output("the fetish");
		output(" I've placed upon you if you'll trade me that stuff.  There's a naughty boy in my harem whose quick wit I've begun to tire of.");
		if (pc.hasGenitals() || pc.biggestTitSize() > 2) output("  Or if you'd rather keep it, I could take one of your endowments for my own use and remove a single fetish.  It's always fun giving one of my old pets a new part to play with.");
		output("  Maybe you just need some better equipment though?  I have this sexy armor I'd love to loan you, pet.</i>\"\n\n");
		addButton(0, "Liqueur", tradeCeraphBimboLiqueur);
		if (pc.hasGenitals() || pc.biggestTitSize() > 2) addButton(1, "Endowment", tradeCeraphCrotchityBits);
	}
	//(No Bimbo Liquer) 
	else if (pc.hasGenitals() || pc.biggestTitSize() > 2) {
		output("She finishes and perks up, leering lewdly at you.  \"<i>I tell you what.  Let me take one of your endowments to use on my pets and I'll get rid of ");
		if (flags["COC.PC_FETISH"] == 1) output("your fetish for you.</i>\"\n\n");
		else output("one of your fetishes for you.</i>\"\n\n");
		addButton(1, "Endowment", tradeCeraphCrotchityBits);
	}
	//(NO TRADE)
	else {
		output("She finishes and frowns.  \"<i>I don't think you've got a single thing worth trading.");
		if (flags["COC.PC_FETISH"] < 2) {
			addButton(1, "Armor", getCeraphsArmor);
			output("Don't fret your " + pc.mf("pretty little head", "charming little head") + ", Mistress Ceraph is nothing if not generous to her pets.  I can give you some beautiful armor to entice your foes with, wouldn't that be lovely?");
		}
		output("</i>\"\n\n");
	}
...
private function getCeraphsArmor():void
{
	clearOutput();
	//spriteSelect(7);
	output("Ceraph smiles knowingly at your response.  \"<i>You'll find this to be quite sexy.  Just be careful putting it on.  If you don't fit it right it'll pinch,</i>\" the demoness instructs while tossing you a set of armor.\n\n");
	//(Get trapped armor and go home)
	processTime(10 + rand(10));
	quickLoot(new CoCSeductiveArmor());
}
...
private function tradeCeraphCrotchityBits():void
{
	...
	if (pc.statusEffectv1("Exgartuan") == 1) {
		output("A disembodied voice roars out, \"<i>FUCK NO!  You are NOT giving me to that crazy bitch!</i>\"  Exgartuan doesn't seem to want to run the risk of being taken by Ceraph, and he completely assumes control of your " + pc.legs() + " to make you flee with all due haste.  He taunts, \"<i>Rip off your own little dick, why doncha!  You'd look better with just a pussy anyhow ya dried out old sow!</i>\"\n\n");
		output("Ceraph seems perturbed but doesn't bother to pursue you.");
		pc.lust( -20);
		addNextButton();
		return;
	}
	...
	while (temp > 0) {
		...
		if (pc.breastRows[temp].breastRating > 2) choices[choices.length] = 3;
	}
	//Think about it...
	output("Ceraph grins lecherously and pulls her tail from her drooling snatch.  \"<i>Really?  Well this should be fun.  Let me just figure out what I want...</i>\"  Her gaze is quite piercing.\n\n");
	//Pick a winner!
	choice = choices[rand(choices.length)];

	if (choice == 0) output("FUCK AN ERROR HAPPENED GET FENOXO ON THE HORN!");
	//(DICKS)
	else if (choice == 1) {
		temp2 = rand(pc.cockTotal());
		output("She pipes up, \"<i>");
		if (pc.cocks[temp2].cType == GLOBAL.TYPE_EQUINE) output("Oooh, is that a horse-cock?  Those are quite nice.  I love the feel of a flare inside me, or even better, plunging one deep into a slut's poor, abused asshole.");
		else if (pc.cocks[temp2].cType == GLOBAL.TYPE_CANINE) output("Oooh, is that a dog-cock?  Those are quite nice.  It's oh-so-fun to enhance the knots on those and then watch them puff up enough to distend some poor slave's gut.");
		else if (pc.cocks[temp2].cType == GLOBAL.TYPE_DEMONIC) output("Oooh, is that a demon-cock?  As you can see I'm QUITE familiar with those, but it might be fun to replace one of my girl's tongues with it.");
		else if (pc.cocks[temp2].cType == GLOBAL.TYPE_FELINE) output("Oooh, is that a kitty-dick?  The spines on those feel so good while they rub down all your walls or catch your prostate.  Meow.");
		else if (pc.cocks[temp2].cType == GLOBAL.TYPE_SNAKE) output("Wow, you've got a snake-dick?  I haven't seen one of those in a while.  I love the bulbous, knot-like texture.  Maybe I'll graft it onto a chair and make one of my boys sit on it for a few days.");
		else if (pc.cocks[temp2].cType == GLOBAL.TYPE_TENTACLE) output("Oooh, a tentacle cock?  How lucky to find one of those on you!  I've been meaning to add another poor tentacle-slave.");
		else if (pc.cocks[temp2].cType == GLOBAL.TYPE_ANEMONE) output("Oooh, an anemone-cock?  Simply marvelous!  I might have to crank up the strength of the venom and use it myself!");
		else if (pc.cocks[temp2].cType == GLOBAL.TYPE_HUMAN) output("Oooh, a normal, uncorrupted cock.  How utterly droll!  You'd think these would be more common, but it's quite a treat to come across one.");
		else output("Oooh... What is it?");
		if (pc.cockTotal() > 1) {
			output("  Of course that's not the only prick you're packing, hrmmm?  Maybe I should take ");
			if (pc.cockTotal() == 2) output("the other one");
			else output("one of the others");
		}
		output(".");
		output("</i>\"\n\n");
	}
	//(Cunts) 
		...
		output("The demon's gaze drops down between your " + pc.legs() + ", and she remarks, \"<i>");
		if (pc.wetness() >= 5) output("My my, what a juicy box you have there.  Tell me, doesn't it bother you to be soaking in your own lubricants all the time?  Perhaps I'll put it on one of my slaves and mount her on the wall.  Then I can simply slip in and lube up before giving one of my other pets a proper lesson.");
		else if (pc.wetness() >= 3) output("My my, what a nice wet pussy you have there.  I really feel that you have a perfect blend of moisture and practicality down there.  I might have to snatch that snatch and give it to one of my drier pets.");
		else if (pc.looseness() > 4) output("My my, what a terrifically gaped hole you have there.  Do minotaurs even feel anything while they rail you?  Perhaps I could use that when I'm playing with Shenandoah.  She's had a few too many incubi drafts and can be a little difficult to handle.");
		else output("My my, such a pretty pussy.  I bet it would be fun to replace one of my noisier pet's mouth with.  She never did learn not to bite.");
		output("</i>\"\n\n");
	}
	//(Tits) 
	else {
		output("Ceraph's hands reach up to cup the swell of your " + pc.biggestBreastDescript() + " as she asks, \"<i>");
		if (pc.biggestTitSize() > 14) output("Oh wow... just wow.  How do you walk?  With melons like this you could tit-fuck three or four people at once!  Absolutely exquisite.");
		else if (pc.biggestTitSize() > 6) output("Tell me dear, are they real?  I mean, were they always this size, or did you have to get a little bit of work done?");
		else output("Well, these are a bit small.  Have you ever considered trying some Gro Plus?  I hear it's all the rage with Lethice's lackeys nowadays.");
		output("</i>\"\n\n");
	}
	output("A long, silent moment passes before Ceraph makes up her mind.  She points at your ");
	if (choice == 1) {
		output("[pc.cocksLight] ");
	}
	else if (choice == 2) output("vagina ");
	else output("breasts ");
	output("and declares, \"<i>I think I'll have ");
	if (choice <= 2) output("one ");
	else output("two ");
	output("of those.  Now just hold still.</i>\"\n\n");

	output("Do you let her take it?");
	flags["COC.CERAPH_EXCHANGE_TARGET"] = choice;
	processTime(5);
	clearMenu();
	addButton(0, "Yes", giveUpYourBallsOrWhateverForLessFetishes);
	addButton(1, "No", bailBeforeCeraphYoinksYourNads);
}
...
private function bailBeforeCeraphYoinksYourNads():void
{
	clearOutput();
	//spriteSelect(7);
	output("You let her know that you've changed your mind and take off before Ceraph can try to take your beloved body parts.  You hear her laugh and tease as you run, yelling, \"<i>What, can't handle the thought of an Omnibus touching your fun bits?  Poor baby!</i>\"\n\n");
	processTime(1);
	clearMenu();
	addButton(0, "Next", function():*{ processTime(10 + rand(10)); mainGameMenu(); } );
}
...
private function giveUpYourBallsOrWhateverForLessFetishes():void
{
	...
	//spriteSelect(7);
	var x:Number = 0;
	output("You hold still, dreading this moment but knowing all-too well that you'd have to give something up to get her touch out of your mind.  ");
	//(Cock)
	if (flags["COC.CERAPH_EXCHANGE_TARGET"] == 1) {
		//Pick a dick at random!
		x = rand(pc.cockTotal());
		output("Ceraph's fingers feel unexpectedly hot as they brush your " + pc.cockDescript(x) + ", as if she had just come out of a heated tub.  You don't have long to ponder that little detail as they pinch into a tight ring");
	...
	//(Pussy)
		...
		output("You work your jaw in consternation, trying to dredge up a response or stop her, but before you can do anything, she pushes your old " + pc.vaginaDescript(0) + " down on her throbbing demon-shaft.  It knocks the strength out of your " + pc.legs() + ", and you lie there moaning while Ceraph prances off, masturbating herself with your disembodied pussy.  Eventually you cum to the feeling of demon-cum spurting inside you, but it's a strange, empty orgasm.");
		pc.removeVagina(0, 1);
	...
	//(Tits) 
		...
		output("Ceraph turns and prances off through the rocks, tweaking your nipples HARD every time you start to make a move to follow her.  It's no use.  She vanishes before long, leaving you to deal with the slowly dwindling sensation of her cock in your tits.\n\n");
...
public function ceraphBodyPartDreams():void
{
	...
	//dicks
	temp = flags["COC.CERAPH_COCKS_GIVEN"];
	while (temp > 0) {
		temp--;
		choices[choices.length] = 1;
	}
	//pussies
	temp = flags["COC.CERAPH_VAGS_GIVEN"];
	while (temp > 0) {
		temp--;
		choices[choices.length] = 2;
	}
	temp = flags["COC.CERAPH_BREASTS_GIVEN"];
	while (temp > 0) {
		temp--;
	...
	if (choice == 2) {
		...
		if (subChoice == 0) {
			output("You can feel an odd sensation in your nethers during your dreams, and as the warm, slick feeling grows you recognize it for what it is – cum dripping into your pussy.  ");
			if (!pc.hasVagina()) output("You don't have a vagina any more, do you?");
			else output("No, that doesn't feel right – it isn't your vagina.");
			output("  It must be the sex that Ceraph stole from you!  You feel tight... enclosed somehow, but the ever present trickle of bodily fluids into your imprisoned nethers slowly brings about your own arousal.\n\n");

			output("Choruses of voices moan with building, peaking pleasure, and the ebb and flow of sexual slime into your parted pussy increases.  You moan in your sleep, feeling wet, obscenely wet, as you're used as a cum-drain for some massive orgy.  The cum from dozens of Ceraph's pets drools into you, filling you with warm, masculine slime and slippery pussy-fluid.  Helpless and imprisoned, you can do nothing but pant with desire until your womb is utterly packed with spooge and your gaping twat is brimming with spunk.\n\n");

			output("You're a useless cum-dump, there only to slurp up the leavings from Ceraph's more treasured pets, but it makes you oh-so horny.  You pant in your sleep, hopelessly aroused but denied release by the fading dream.");
			//(+4 sens, +200 lust)
		...
		//PUSSY FLESHLIGHT
			...
			output("A sudden prodding at your vagina's entrance nearly rouses you from your sleep.  ");
			if (!pc.hasVagina()) output("Wait... you don't have a pussy anymore.  ");
			else output("Wait... your pussy doesn't feel like that!  ");
			output("You must be picking up on feelings from the cunt Ceraph took away from you!  Warm demonic cum bubbles at your entrance, slicking the engorging vulva with demonic taint.  You swoon from the heat of it, and the demoness wastes no time, quickly inserting her entire length.\n\n");

			output("The nubby nodules that line her shape press on your walls, stroking the innermost folds of your long-lost nethers.  Somehow you get a visual of the scene – Ceraph, lying back in bed, your pussy-flesh clutched in her hand like a perverse sex-toy.  She starts pumping it on herself, using a part of your body like a simple masturbation tool.  The room is empty, and she wastes no time in vocalizing her pleasure, \"<i>Ohhh fuck you've got a nice pussy " + pc.short + ".  By Lethice's lewd tits, I hope you can feel how good your cooch is at devouring a demon's dick.  Next time we meet you should... Ah.. ahhh... ohhhh umm... submit to... ahhh AHHH!</i>\"\n\n");

			output("Throughout Ceraph's speech she continually increased her pace, jackhammering your pussy up and down on her bulging dick.  The nodules start to enlarge, puffing up around her base and then rippling, wave-like towards the slowly thickening head.  A squirt of hot cum shoots into your depths, sliming them with the demoness' corrupt cream, but it feels so good... so very good.  Your pussy clamps down on the invader, imprinting the swollen nodules against its soft inner walls, and you cum as hard as if you were riding her yourself.\n\n");

			output("The vision and feelings slowly fade, letting you out of their grip and leaving faint after-images of Ceraph cleaning you with her tongue.");
			...
			//Vagina Wall-Cubby
			output("MMMmmm, something is filling you just - ahhhh... right.  You shift position in your blankets, but it doesn't seem to move the insistent hardness lodged up your twat.  Gods, it feels so good to be so full.  It's like having three or four hard dicks inside you all at once, keeping you stretched and open, the cunt-honey dripping toward the floor.  The only time they move is when a wave of contractions pulses through your female flesh, squeezing about the invaders.  Languidly, you work your muscles faster, wanting to feel greater pleasures.  You secretly wish that whatever it is would fuck you.");
			output("\n\nWithout any warning, the multitude of rigid intruders moves, one of their number sliding out.  Your poor, stretched-out vaginal flesh slowly tightens up around the remainder, but it isn't as tight or enjoyable as it was a moment ago.  Under your covers, your [pc.hips] rock back and forth encouragingly, but it doesn't help at all.  Another rod vanishes from your vaginal embrace, setting off fire bursts of pleasure followed by aching hollowness.  You're so empty that your lips don't even close around the rest of the thick shifts.  They just gape, luridly opening wider as if to beg for more.");
			...
			output("\n\n\"<i>Oh [pc.name], your pussy makes such a good cubbyhole for my toys.  Still, it gets so wet.  I could do with some relief,</i>\" an unseen, feminine voice muses, \"<i>I'm so glad I was playing with some gro+ earlier...</i>\"");
		...
		//Tit Volleyball
			...
			output("You see Ceraph, the conniving omnibus, standing in a hazy pink void, dressed in a tiny purple bikini.  She smiles to herself as she holds out a fleshy orb.  It looks soft, jiggly... and alarmingly familiar.  As you spy a rounded areola, you realize the demon is holding one of your old breasts! She tosses the plump tit ponderously for a moment, before raising it to her mouth and... blowing?");
			...
			if (silly) output("horsecocks ");
			else output("demonic cocks ");
			output("rise from the sand, forming makeshift poles for the twisted volleyball net.");
			...
			if (silly) output(" and what possible ramifications it has for your mental health");
			output(", until you dismiss it as corruption and toss over, entering a peaceful, dreamless sleep.");
			pc.lust(5 + pc.libido() / 5);
		}
	}
	//COCKUUUU
		...
		if (subChoice <= 2) {
			...
			output("The process is repeated twice more, each more painful and arousing than the last.  Just as you're nearly driven to wakefulness, a cool salve is applied to the oh-so-sensitive member, smothering the pain with blissful cool.  Ceraph's nails tease their way through the new substance, setting you on edge.  While you're ready for an explosive climax, it never comes.  That's the last thing your phantom cock feels that night, and you drift into a peaceful, if horny, slumber.");
		...
		//Cock Sandwich
			...
			//dynStats("lus", 5 + pc.lib / 5, "cor", .5);
			//pc.hoursSinceCum += 100;
