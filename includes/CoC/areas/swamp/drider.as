...
public function driderEncounter():void
{
	...
	if (flags["COC.DRIDER_MET"] == 1) {
		...
		output("Her tone makes it quite clear you'll have to run or fight your way out of this!");
	}
	//repeat intro:
	else {
		output("Bushwhacking your way through the dense foliage of the swamp, it soon becomes clear that you've wandered into spider territory.  Webbing hangs from trees and the dark, dank air hides ambushes well.  Diligence and wariness pay off, for you become aware of a skittering too near to be simple background noise.\n\n");

		output("As far as you're aware, it's unlikely you'll be able to scare your stalker off, and so you simply wait for them to show themselves.  You search with your eyes and, noisily clearing your throat, make it clear you're aware of their presence.  Soft chuckling chimes around you, seeming to originate from several places in the scrub, but nothing appears.  You turn about and call again, [pc.readyingWeapon].  A gentle tap on the shoulder answers you, but when you look the vista is equally as desolate as the first time.  And where the hell is that laughing coming from?\n\n");

		output("Spinning again, you lose your patience and loudly demand your ambusher present itself.  A soft tug on your ");
		if (pc.hasTail()) output("[pc.tail] ");
		else output("[pc.hair] ");
		output("pulls you back halfway, but as you open your mouth to yell, the ground gives a quiet thump.  Just barely in your periphery, you see a white apparition with a yawning mouth stretching out to bite, and your reflexes take over, dropping your weight and rolling you away from it.\n\n");

		output("\"<i>Clever little pet,</i>\" the phantom spits, resolving into the shape of a drider as your eyes focus and your adrenaline haze clears.  Raising a cupped hand to her ghoulishly grinning mouth, she laughs into it; a minute shivering in the air reveals the taut threads of silk attached to her finger.  The vibrations travel down them, shaking several silk-tied leaves and producing the raspy, laughing audience from before.  \"<i>It'd be so much easier on you if you'd just walked into the trap, sweetness.</i>\"\n\n");

		output("You've yet to meet a wild drider that let you walk away without some sadistic display of power, and this one looks to be no different.");
	}
...
public function defeatDriderIntro():void
{
	//spriteSelect(77);
	clearOutput();
	output("All eight of the drider's black and [enemy.hairColor] striped legs collapse under her weight while she struggles ");
	if (enemy.HP() <= 1) output("to try to continue the fight, her movements slowed by pain and weakness.");
	else output("to masturbate her swollen pussy-lips and the thick, juicy demon-cock that's sprouted in place of her clit.  She's so focused on getting off that she utterly gives up on the idea of fighting back, let alone dominating you as she seemed to plan originally.");
	output("  The spider-herm lies in a miserable, defeated heap.");
	
	clearMenu();
	addButton(14, "Leave", function():*{ processTime(20 + rand(10)); CombatManager.genericVictory(); } );
	
	addDisabledButton(0, "Butt Fuck", "Butt Fuck", "This scene requires you to have fitting cock and sufficient arousal.");
	addDisabledButton(1, "Fuck Pussy", "Fuck Pussy", "This scene requires you to have fitting cock and sufficient arousal.");
	addDisabledButton(2, "Bondage Fuck", "Bondage Fuck", "This scene requires you to have fitting cock, spider silk and sufficient arousal.");
	addDisabledButton(3, "FuckSpinner", "Fuck Spinneret", "This scene requires you to have fitting cock and sufficient arousal.");
	addDisabledButton(4, "Ride Cock", "Ride Cock", "This scene requires you to have vagina and sufficient arousal.");
	addDisabledButton(5, "Ride Ovi", "Ride Ovipositor", "This scene requires you to have vagina and sufficient arousal.");
	addDisabledButton(6, "RideOviAnal", "Ride Ovipositor Anally", "This scene requires you to have sufficient arousal.");
	
	if (pc.lust() < 33) return;
	
	output("\n\nWhat do you do?");
	
	if (pc.cockThatFits(enemy.analCapacity()) >= 0)
		addButton(0, "Butt Fuck", buttFuckADriderOhBaby);
	if (pc.cockThatFits(enemy.vaginalCapacity()) >= 0) {
		addButton(1, "Fuck Pussy", winDriderPCDickInSpiderCunt, null, "Fuck Pussy", "You could fuck her pussy, though you'd be within easy reach of her lips.  If she gets any crazy ideas, it'd be hard to stop her.");
		if (pc.hasTail(GLOBAL.TYPE_ARACHNID) || pc.hasTail(GLOBAL.TYPE_DRIDER) || pc.hasItem(new CoCToughSpiderSilk()))
			addButton(2, "Bondage Fuck", driderVagSmartFuck, null, "Bondage Fuck", "You could bind her up with some webbing for some bondage.  Her lips are dangerous, after all.");
	}
	if (pc.cockThatFits(12) >= 0)
		addButton(3, "FuckSpinner", victoryVSDriderStickDickInSpinneret, null, "Fuck Spinneret", "You could fuck her spinneret.");
	if (pc.hasVagina()) {
		addButton(4, "Ride Cock", winVSDriderTakeItsCockInCunt, null, "Ride Cock", "You could ride the drider's retractible demon-cock.");
		addButton(5, "Ride Ovi", victoryVSDriderRideOviVaginal, null, "Ride Ovipositor", "You could ride the drider's ovipositor.");
	}
	addButton(6, "RideOviAnal", victoryVSDriderRideOviAnal, null, "Ride Ovipositor Anally", "You could always ride her ovipositor anally...");
}
...
public function loseToDrider():void
{
	...
	var select:Array = [];
	if (pc.hasCock()) select[select.length] = 0;
	if (pc.hasVagina()) select[select.length] = 1;
	//Genderless scene has low chance for gendered folks
	if (!pc.hasGenitals() || (pc.hasGenitals() && rand(3) == 0)) select[select.length] = 2;
...
private function winDriderPCDickInSpiderCunt():void
{
	...
	if (enemy.lustQ() > 99) {
		...
		output("she looks ");
		if (pc.tallness > 72) output("up ");
		output("at you with a look of disgust in her " + enemy.eyeColor + " eyes.  She folds her arms across her many-bosomed, quadratically endowed chest with disdain");
		if (enemy.nipplesPierced > 0) output(", the chains jingling loudly as she does so");
		output(".  In spite of keeping a stiff upper lip about it, her lower ones visibly quiver while you strip.");
	}
	output("  You saunter over to the limp-legged drider and run your hands down her inhuman, arachnid body, exploring the smooth, shiny carapace.\n\n");

	output("The corrupted monster-girl sighs and shudders at your touches, as if she can somehow feel your finger-tips through the rock-hard exoskeleton.  From the way she's trying to hold in her exhalations, you can only guess that it must feel wonderful, so you keep it up, circling and touching every part of her exotic anatomy.  By the time you've circled around her entire body, she's ");
	if (enemy.HP() <= 1) output("giving up little, lewd moans");
	else output("breathing out lurid moans");
	output(" of pleasure and her ");
	if (enemy.nipplesPierced > 0) output("chain-pierced-");
	output("nipples and clit look as hard as her natural body-armor.  You boldly grope one of her ");
	if (enemy.nipplesPierced > 0) output("pierced ");
	output("breasts, feeling the supple, soft flesh shift while you roughly palm it and rub your thumb down a dark nipple.  You let the sensitive bud go and watch it spring back up, enjoying the drider's gasp of surprised pleasure for the brief moment before she muffles herself.\n\n");

	output("Still fondling one of the four nicely-sized tits, you push the spider-bitch back, admiring the drider's flexibility as she leans back over her inhuman lower body, giving you an even better view of her intoxicatingly sweet-smelling pussy.  You take [pc.oneCock] in hand and rub the [pc.cockHead " + x + "] along the slick lips of her pussy, shuddering slightly as her wetness rubs the sensitive glans.  Gasping, you slowly spread her molten-hot pussy around your girth, reveling in the tight embrace of her slick walls.  The drider squirms in your grip with her breasts bouncing deliciously and whole body trembling.  Spreading as easily as her cunt, the drider's venom-glossed lips slowly open into a wordless 'o' of rapturous pleasure.\n\n");

	output("Half an inch at a time, you sink ever-deeper inside the seemingly bottomless pit that is the spider-girl's steamy sex.  Reveling in the taming of the aggressive predator's sticky snatch, you postpone the eventual hilting as long as possible.  For her part, she doesn't seem to mind you having your way with her.  All eight of her spindly legs are scrabbling pitifully in the dirt, doing little more than nervelessly convulsing in uncontrolled pleasure.  The spider-woman's arms close behind your back, the sharp tips of her nails trying to claw deep furrows into your " + pc.skinFurScales() + ".  At long last, your waist presses against her own to feel the oozing slipperiness of her nethers.  Your [pc.cock " + x + "] is totally buried in her tunnel, and now that you're holding still, you can feel the tiny folds inside her vagina slowly moving and gliding along your length, stroking your [pc.cock " + x + "] like dozens of tongues.\n\n");

	output("Compelled by your body's instincts, you draw back your [pc.hips] to expose the bulk of your pussy-soaked shaft.  The spider-woman moans disappointedly, her voice filled with so much wanton desire that your [pc.cockHead " + x + "] twitches in response.  You fondle the drider's lower pair of breasts with both hands, circling and squeezing the delectable mounds while you begin to repeatedly impale her squelching pussy, watching the upper row of pale breasts bounce and jiggle above your busy fingers.");
	if (enemy.nipplesPierced > 0) output("  The chains binding her nipples make a delightful clinking noise with each squeeze and stroke.");
	output("\n\n", false);

	output("Meanwhile, the drider's swollen clit prods at your loins with each penetrating thrust, leaving lines of sticky spider-juice all over your crotch.  ");
	if (pc.cockTotal() > 1) {
		output("Your extra penis");
		if (pc.cockTotal() > 2) output("es fan out to the side, forgotten while you give yourself over to the silken caresses of her glorious vagina.");
		else output(" sticks out to the side, forgotten while you give yourself over to the silken caresses of her glorious vagina.");
	}
	else output("Your [pc.cock " + x + "] seems to bulge inside her, giving itself over to the silken caresses of her glorious vagina.");
	output("  The more you fuck the arachnid hussy's wanton slit, the more animated she becomes, until she finally breaks her wordless silence by crying, \"<i>Yes " + pc.mf("master", "mistress") + ", dominate me!  I've been such a naughty bitch.  Use me like your cheap whore!  Fuck my holes full of cum, and leave me defiled in a puddle of spunk-laced mud!</i>\"\n\n");

	output("Invigorated by the rising tide of desire inside her, the drider pulls her torso up, crushing her tits into your palms so hard that her flesh bulges out through your fingers.  You smile down at the slutty spider and squeeze them hard enough to give her a taste of pain in response.  She pants, and her four " + enemy.eyeColor + "-hued eyes gaze at you, so full of desire that you have a hard time not cumming on the spot while you lock onto those half-lidded, luminous orbs.  You let go of her breasts and reach back to squeeze the smooth skin of her butt, feeling the joint between her human and arachnid halves just under her curvy derriere.\n\n");

	output("Heaving mightily, you use the submissive spider's ass-cheeks as hand-holds for a fast, hard fuck.  Her four ");
	if (enemy.nipplesPierced > 0) output("pierced");
	else output("white-skinned");
	output(" breasts mash against your [pc.fullChest], sweaty skin sliding back and forth and drawing gasps from each of you every time one of her nipples brushes one of yours.  She coos, \"<i>Please fuck my slutty drider-cunt harder, " + pc.mf("master", "mistress") + "!  It was so naughty and cruel when I tried to rape you.  Don't let me cum... deny me over and over until I'm your good little spider-pet that'll do anything for release!</i>\"  She emphasizes every sentence by clamping her quim down on your [pc.cock " + x + "], the dozens of lubricant-soaked folds rolling back and forth, milking you until you cannot help but ejaculate inside her.\n\n");

	output("Your back arches in orgasmic pleasure, fingers squeezing her butt tightly enough that your pale lover's derriere will show your hand-prints for some time.  She shudders slightly as your [pc.cock " + x + "] is forced as far inside her lewdly-textured vagina as possible, milking your penis with repeated strokes while your urethra bulges with the first load.  You release what feels like a torrent into her passage; the heavy spurt is quickly swallowed by the drider, disappearing deep inside her as if drawn away on an unseen current.  The sudden suction and increase in the movement of her folds makes your next rope of jizz feel even bigger, but it too disappears into the drider's seemingly-bottomless body.");
	if (pc.cumQ() > 1000) output("  Amazingly, you actually manage to cum enough to fill her massive womb, and after the first few squirts, the cum slowly bubbles out around your shaft with each fresh deposit of jism.");
	output("\n\n");

	output("While you're distracted by the repeated contrast of tension and relief emanating from [pc.eachCock], the drider leans up and kisses your neck, humming against your throat as you squirt the last of your orgasmic goop into her vagina.  You slump into her arms, spent by the effort of fucking her so fast and cumming so hard.  As you lie there, her lips sucking hard enough to leave hickeys on your neck with every crushing, passionate kiss, you feel the patterns of glossy fluid tingling on your " + pc.skin() + ".  A jolt of fear runs up your spine, and you try to push away from her envenomed affections.  It doesn't work!  You can barely lift your arms, let alone pull out of the amorous arachnid's embrace!\n\n");

	output("\"<i>Mmmmm,</i>\" hums the drider into your collarbone, smearing more of her toxic lip-gloss onto you as she goes.  \"<i>Such a good " + pc.mf("master", "mistress") + "!  You punished me so well!  Now just relax... I've been dripping out a special venom that'll help you teach me the rest of my lesson, and I need to make sure that every... single... drop... slips inside that sexy bloodstream of yours.</i>\"\n\n");

	output("You wiggle futilely, using the last of your strength to try to break free, but it's no use - she's too strong, or the venom has made you too weak.  Dimly, you're aware of your [pc.cock " + x + "] pulsating inside her, hard as iron and burning hot with lust, even though you just came.  The spider-woman continues to kiss you, and with every peck of her lips, you feel yourself sagging deeper into her embrace until you lay your head down on her breast, your only movements the slight twitches of your body as it tries to deal with the poison-induced need to copulate.  You feel the warmth of her breath on your neck when she stops, cradling you in her arms.\n\n");

	output("The drider cocks her head to the side so you can see her luminous " + enemy.eyeColor + " eyes boring into you while she whispers, \"<i>Don't try to fight it, dear.  While there's enough of my normal venom in there to keep your [pc.cock " + x + "] hard enough to cum a few more times for me, the real magic is in the secondary chemicals my demonic friends were kind enough to teach me how to excrete.  You can feel it relaxing your muscles can't you?  Well, the beauty is that it isn't just your muscles relaxing... your mind is getting nice and relaxed too.  So open and pliable that you'll soon be a horny puddle of putty for me to play with.</i>\"\n\n");

	output("Gods, she's right!  You started to nod while she was talking, automatically agreeing with every sibilant syllable.  Trying to shake your head in denial, you only succeed at bouncing the jiggling tit-flesh beneath you.  You have to resist!  You beat her!  She can't do this!\n\n");

	output("As if reading your thoughts, the seductive spider answers, \"<i>Oh dear, don't struggle.  You won, remember?  You held me down and fucked me hard.  You mounted me like a fuck-hungry stud that does all his thinking with the big fat cock between his legs, and we both loved it.  You won't deny that will you?  It's the truth.</i>\"\n\n");

	if (pc.cor() > 80 && pc.libido() > 50) output("Of course it's the truth, the cunt's clearly got a good grasp on the obvious. Unwilling to lose your grip on dominance, however, you try to keep struggling to get the upper hand.\n\n");

	else output("No, that's not right.  You're not some rutting beast, you're the champion.  Sure, you've got your [pc.cock " + x + "] locked hilt-deep inside her squirming slit, but that doesn't mean you're some kind of " + pc.mf("stud", "slut") + " who jumps on a cunt without a second thought, does it?  You blink your eyes while you try to think.  She does have a point though, if you hadn't been in such a hurry for some sex, you wouldn't be sagging into her tits and feeling so awfully relaxed.\n\n");

	output("The drider croons, \"<i>Come on, don't delude yourself.  If you weren't such a dominating, rape-hungry beast you wouldn't be nuzzling against my tits and giving me little nods of agreement.  Honestly, how could you ever think of yourself as anything different when your brain just does whatever your dick tells it to?  You just got off and might have even knocked me up, but your penis is even harder than before.</i>\"\n\n");

	if (pc.cor() > 80 && pc.libido() > 50) output("You grumble out a denial.  You're not dumb, you just like to take charge and fuck. There's no reason to resist jumping on a nice wet pussy and making it squirt while you pack it full... but you can't quite get your limbs to move.\n\n");

	else output("You mumble out a denial.  You're not a beast... you just like to take charge and fuck.  It's just so hard to resist jumping on a nice wet pussy and making it squirt while you pack it full... no!  That's not what...\n\n");

	output("\"<i>Oooh, are you leaking out more pre-cum already?  You're insatiable!  I bet you start thinking about your next fuck before you even finish getting off, don't you?  You can deny it all you want, but I felt your dick puff up a little bit more as soon as I said it.  No matter what you think, your cock is a dumb, happy little tattle-tale that always tells the girls exactly what you're thinking.</i>\"");
	//if (pc.statusAffectv1(StatusAffects.Exgartuan) == 1) output("  Exgartuan mumbles telepathicaly, \"<i>Fuck yes I do, but I wanna see how this plays out.</i>\"");
	output("  She adds, \"<i>Face the music - there's nothing you love more than defeating someone and letting your dick tell you what to do.  You dumb... domineering... beast...</i>\"\n\n");

	output("Gods!  It's getting so hard to think with that tight cunt clutching at your cock and the soft, feminine enunciations constantly interrupting your thoughts.  How can she concentrate with the stink of sex hanging so thick in the air?  Your [pc.cock " + x + "] is so horny, why aren't you fucking her yet?  You shudder weakly and remember ");
	if (pc.cor() > 80 && pc.libido() > 50) output("that it's because of her own venom paralyzing your muscles so effectively.\n\n");
	else output("there was something wrong about what she was saying... something important.\n\n");

	output("The drider pauses to gaze into your eyes with all four of hers.  She runs a hand through your hair while making gentle shushing noises at you before she resumes her diatribe.  \"<i>There there, I know how eager you are to fuck me into my place.  I'm just another wanton harlot waiting to be forced on her back and turned into a fresh cock-sleeve for you, aren't I?  You recognize the absolute truth of my words.  Every time I talk you can feel the words bouncing around inside that empty head of yours.</i>\"\n\n");

	output("You nod, actually moving your head against her pillowy breasts as you feel your strength starting to return.  Why is she wasting so much time telling you things you already know?  This bitch's cunt is like a white-hot furnace of lust, and your cock is so tired of having to sit still.  You can't wait to recover from the venom and fuck her so hard she'll black out from the ensuing orgasm.  Then you'll fuck her a few more times until your cock finally gets tired of her sloppy, cream-pied pussy.\n\n");

	output("\"<i>Oooh, that dick is just getting harder and harder isn't it?  I bet it feels like it's about to burst and cum, even though you're not able to punish me like you want to yet.  Maybe you secretly get turned on by being denied?  Oh!  I just felt your dick jump!  So you want to go around, letting your dick tell you to dominate every horny cunt you see, but the thing that turns you on more than anything else is having a woman turn the tables on you, isn't it?</i>\"\n\n");

	output("You moan, \"<i>N-no...</i>\" but a small burst of pre-cum chooses that moment to erupt from your [pc.cock " + x + "], still nestled inside the squirming spider-cunt.  It feels heavenly, and while you try to struggle in your weakened state, the warm embrace of her pussy's many cock-milking folds is turning your penis into a cum-drooling behemoth on the edge of exploding.  Oh gods, is she going to do it?  Damn, you want to put her in her place, but you're so weak right now.  Even worse, her pussy has you so close, so very close.\n\n");

	output("The chitinous, oh-so-right bitch pulls your head up to gaze deeply into her eyes.  \"<i>You have a horny, naughty cock, and you always listen to it.  This rebellious member gets you into so much trouble, but you'll never stop listening to it.  You'll keep chasing every wet pussy you see, because you're just a horny animal.  A rutting, horny animal.  But I know your secret.  Your dick really wants a girl to turn the tables on you, paralyze you, and tease your cock mercilessly.  It feels so good to let me touch your body, stroke it, tease it, and squeeze it.  You won't even fight any more.  My pussy has your cock so close, so very close, and you're going to cum when I tell you.  You'll blow your load on command for me, my servile, secretly submissive mount.</i>\"\n\n");

	output("A plaintive whining, almost exactly like that of an animal, vocalizes subconsciously.  Your [pc.cock " + x + "] feels afire, constantly leaking pre-cum while you're held immobile in your spidery mistress' arms; you lick your lips and start to beg, mumbling out your desire to cum for her, anything for her, so long as she lets your dick orgasm again and again.  You want to climax so bad, but you want her to control your cock so much more.  It's always getting you in trouble, and it feels so good to let her use you as her cum-drooling mount.\n\n");

	output("The spider-woman giggles and surprises you by kissing you on the lips, holding you against her while her narrow tongue smears her venom all over your lips and tongue, staining them purple.  You exhale happily through your nose, feeling perfectly, utterly relaxed and yet aroused beyond any reasonable measure.  The squeezing caresses around your dick keep you happily squirting heavy dollops of pre-cum.  You've no doubt it must look like some kind of over-stuffed sausage by this point, bloated beyond reason.\n\n");

	output("When she pulls away, her face is covered in a huge blush, and she's panting out loud and interrupting her words with quiet moans as she talks, \"<i>That's a good " + pc.mf("boy", "girl") + ".  Even though you're a stupid pussy-slave, bound to let your cock lead you into every hungry snatch you see, you know how to obey a smarter woman's cunt, don't you?  Let me hear you beg like the animal you are.  Whine for mistress while her cunt fucks your dick, show me with every fiber of your being how much you love to submit, to obey, and to beg to be allowed to cum.</i>\"\n\n");

	output("She lets your head hang limply, and all you can do is whine, and try to look up at her with the most pathetic expression in your eyes.  You're beaten - she's so much smarter than you and she's got you by your dick.  Why didn't you think?  ", false);

	if (pc.cor() > 80 && pc.libido() > 50) output("Gods it was a bad idea not to bind her lips... ");
	else output("Gods this was a bad idea... ");
	output("still, at least she's controlling your rebellious dick, teaching the little prick a lesson while letting you feel so good.\n\n");

	output("The drider moans louder and responds, \"<i>Yes, oooh, good " + pc.mf("boy", "girl") + ".  You're so hard, so very hard.  You're oozing out cum even though you're not allowed to fuck me, and you love it.  I bet you don't even mind not getting to orgasm - so long as you're helpless and hilt-deep in a cunt, you're happy, aren't you?  Oohhh, I'm gonna... gonna... cum soon.  Luckily for you, I want you to squirt inside me.  I want that cock creaming my pussy as soon as I... ohhhh... so good... s-soon as I command it...  You'll do that for me, won't you?  You'll obey me completely, utterly, and pump every drop of your jism into my soon-to-be-pregnant pussy, right?</i>\"\n\n");

	output("You mewl out an affirmation, forgetting how to speak with all the venom pumping through you and the mind-crushing levels of arousal pouring out of your [pc.cock " + x + "].\n\n");

	output("The spider-woman shudders from head to all eight of her 'toes', her four eyelids closing while her mouth gasps out, \"<i>Cum for me.</i>\"\n\n");

	output("The first spurt of pent-up seed splatters the spider's womb before you even realize she gave you permission, painting the squirming walls white.  You growl and cum, releasing spurt after spurt of spunk while her pussy seems to drink it all down");
	if (pc.cumQ() > 1000) output(", even though you've packed it full once already");
	output(".  Her legs skitter and tremble, her pussy convulses, and your dick gets the squeezing, sucking orgasm of a lifetime.  It feels like you're pumping out two or three orgasms worth of spooge, each dick-stretching explosion of cream dilating your slit almost painfully wide.");
	if (pc.cumQ() > 1000) {
		output("  The stuff dribbles out around your tool along with a mix of girl-lube, too much even for her prodigious pussy to contain.");
		if (pc.cumQ() > 2000) output("  The flow thickens into a river, and soon the puddle of cum looks more like a pond, turning the dirt into a soup of muddy cum.");
	}
	output("\n\n");

	output("While it seems to go on and on, your orgasm eventually ends, leaving you to hang limp while your still-hard dick is massaged by a much wetter spider-cunt.  The " + enemy.eyeColor + "-haired beauty dreamily says, \"<i>Mmmm... good job, pet.  Now, let that nice hard dick do the thinking for you and keep fucking my hot cunt until it goes dry.  Maybe a few hours after you finish, you'll be able to remember that you used to be a " + pc.mf("man", "woman") + " before you became my livestock.</i>\"\n\n");

	output("The blissful arachnid rolls onto her side with you still locked inside her and falls asleep, her " + enemy.hairColor + " tresses ");
	if (pc.cumQ() <= 1000) output("splayed across the ground");
	else output("soaking in the white puddle of spooge");
	output(".  Even unconscious, her vagina continues its rhythmic contractions around your cock, working you towards another burst of orgasmic pleasure while you wait for your strength to come back...\n\n");
	//[NEXT]
	pc.orgasm();
	...
	clearMenu();
	addButton(0, "Next", winDriderPCDickInSpiderCunt2);
}
...
private function winDriderPCDickInSpiderCunt2():void
{
	...
	//spriteSelect(77);
	clearOutput();
	output("<b>Sometime Later...</b>\n");

	output("Pistoning your hips violently, you continue to ravage the unconscious spider, fucking her in a brutal, beast-like manner that centers entirely upon your own pleasure.  You cum, dumping a fresh batch of jizz into her already-stuffed twat, grinning when a few more rivulets of ooze roll over her inflamed pussy-lips to drip into the growing puddle.  Still hard, [pc.eachCock] continues to twitch, hungry for more, and you don't deny it.  You grab the arachnid bitch and stuff your dick into her mouth, shivering while her venom seeps into your dick, keeping you hard and ready to blow.  Whatever special, immobilizing venom she had earlier, it seems exhausted, and the only response your body has to the onslaught of toxin is to erupt, spouting a huge load of semen into her mouth.\n\n");

	output("A low growl rips through your throat as you pull back and plaster her face white, coating the caked-on residue of a few previous loads with fresh moisture.  You shift position and force your [pc.cock " + x + "] between her two pairs of breasts, sandwiching it in there for the titty-fuck of a lifetime.  Pistoning back and forth, humping the glorious orbs relentlessly, you give yourself over completely to your sore dick, letting your world dissolve into the feelings of each of the spider-woman's orifices around your member.\n\n");

	output("<b>Even later...</b>\n");

	output("You collapse into the dirt, not even caring that it stinks of dried cum and girl-jizz.  You've completely emptied out every speck of sperm from your [pc.balls], and you even kept going for a while after that.  It wasn't until [pc.eachCock] went soft at last that you found enough peace to lie down.  ");
	if (pc.cor() > 80 && pc.libido() > 50) output("The longer you sit there, the more your strength returns to you, and the more the submissiveness placed into you by her venom recedes. Still, you've tired yourself out too much to leave at the moment.");
	else output("The longer you sit there, the more you remember of yourself, and the more you forget the animal, the rut-beast that the spider-woman placed inside you.");
	output("  You shiver and wrap your arms around her, snuggling against her while a primal, buried part of your brain insists that she's YOURS.  You fall asleep like that, resting dreamlessly even through your mate's waking and departure.\n\n");
	//hideUpDown();
	//monster.lust = 98;
	//monster.HP = 2;
...
private function victoryVSDriderRideOviAnal():void
{
	...
	output("The spider's look of ");
	if (enemy.HP() <= 1) {
		output("hate fades slightly when she sees you eyeing her ovipositor, and with a sigh of annoyance, allows the segmented organ to slowly slide out from her body.  It grows to just over two feet in length, swaying sinuously in the air, though you can tell it isn't completely engorged yet.  Still, your [pc.asshole]");
		if (pc.hasCock()) output(" and [pc.eachCock] twitch");
		else output(" twitches");
		output(" at the thought of trying to pack all of that meaty, alien pole inside you.\n\n");
	}
	...
	output("You lean over and grab it in your hands, letting it pulse and squirm in response to your touches.  The copious slime it's covered in lets your fingers easily slide down its length.  Each stroke of the odd, feminine member elicits a pleasured groan from your defeated opponent, and you quickly surmise that she must like this just as much as you do using your own genitals.  Pumping and stroking it, you watch with perverse fascination as the ridges grow firmer, hardening in your hand.  A viscous green slime bubbles out of the wide slot at its tip as the spider-girl's moans reach a crescendo.  It's as lubricated as it's going to get.\n\n");

	output("Climbing over the drider's bulbous body, you bend over to present your [pc.ass] to the twitching ovipositor, beckoning her to slide inside your twitching [pc.asshole] as you stand atop its owner.  The sprawled-out spider looks up past all of her body to watch you, leaving you waiting.  Nothing happens at first.  You're just there, ass in the air and waiting to be violated, encouraging it on your own terms, but she doesn't do anything at all.  She simply watches you.  It isn't until a scowl of irritation makes its way across your face that she acts.\n\n");

	output("The ovipositor lurches over, bending and flexing to reach up to your exposed hole, barely touching you while it explores your cheeks.   You pant and grab your [pc.ass], pulling yourself wide to expose the clenching, dark circle that is your pucker.  It rubs across your hand before finally finding its way into your anal cleavage, ridges sliding between your backside and flopping against your ring while it blindly tries to get adjusted.  The moist, dripping slime it exudes quickly turns your ass into a green-glazed slip-'n-slide.  A sudden shock of sensation alerts you that yes, the ovipositor has finally found your asshole.\n\n");

	output("The tentacle-like tube prods forward, leaking ooze inside you as the first ring contracts to squeeze inside you.  You gasp when it pops into place, barely adjusting before the next ridge slides into the slightly-loosened sphincter.");
	pc.buttChange(30, true, true, false);
	output("  Each segment somehow feels larger than the one preceding it, though it seems impossible given how widely stretched you feel by the time the fourth is inside you.");
	if (pc.hasCock()) {
		output("  The warm organ brushes past your prostate, the slippery caress making [pc.eachCock] dribble a ");
		if (pc.cumQ() < 500) output("drop");
		else output("stream");
		output(" of pre-cum down its length.");
	}
	if (pc.hasVagina()) {
		output("  Your [pc.vagina] quivers");
		if (pc.wetness() >= 3) output(" and drips");
		output(" sympathetically with the sensation, and your [pc.clit] peeks from the wet lips.");
	}
	output("  Yet you are not denied the anal pleasure you crave, as the widening bands now filling you are each pushing out harder than the one before.  At the stimulation, her own demonic clit swells as if posessed, nodulates, and hardens into dick form, poking up from her pussy and drooling a slime of pre-cum onto her abdomen.  The second ring rolls over your anal g-spot and you gasp, nearly losing your grip.  The third knocks your " + pc.legs() + " out from under you, and you collapse atop your partner, squeezing oozing dick");
	if (pc.hasCock()) output("s");
	output(" between you");
	if (pc.clitLength >= 4 && pc.hasVagina()) output(", and pressing your monster chick-stick into it; the gooey heat sends a tingle of shock up your spine and your pussy convulses");
	output(".");
	pc.buttChange(50, true, true, false);
	output("\n\n");

	output("Shivering, you glance back up at the defeated woman; she has a passionate, prideful look plastered on her face, though it seems to waver each time another inch of ovipositor pushes past your stretched-out ring.\n\n");

	//([not mans]
	if (!pc.hasCock()) output("\"<i>I never would've thought you were an anal-pregnancy slut,</i>\" hisses the spider in pleasure, flexing her organ harder while it curls through your guts, so deep inside you that you wonder how much further she can go.");
	else output("\"<i>Just because I have an exoskeleton doesn't mean I can't feel you dripping on me.  I thought I was the corrupt bitch here, but you... you take the cake.</i>\"");
	output("\n\n");

	output("With a final hard shove, the last few segments pop inside you");
	if (pc.hasCock()) output(", nearly crushing your prostate from the tight squeeze");
	output(".  ");
	if (!pc.hasCock() && pc.hasVagina()) output("You gasp and dribble a few strands of girl-goo, the contractions of your achingly empty pussy intensifying.  ");
	if (pc.hasCock()) output("You gasp and squirt a few weak ropes of cum, having a miniature orgasm on the spot.  ");
	output("Writhing happily, you sigh ", false);
	if (pc.hasCock()) output("while your [pc.cocksLight] dribbles white, wasted goo onto the black and " + enemy.hairColor + " striped carapace below");
	else if (pc.hasVagina()) output("while your [pc.vagina] dribbles fem-slime onto the black and " + enemy.hairColor + " striped carapace below");
	else output("while your fingers tweak your [pc.nipples] and your starved sphincter sucks at its slimy, squirming stuffing");
	output(".  Without warning, the ovipositor dilates, pulling your [pc.asshole] even wider.  An explosion of warmth goes off in your guts, and you realize that your term as an incubator has begun with a sense of perverse delight.\n\n");

	output("The large, round egg pushes its way up your butt, squeezes past your ring");
	if (pc.hasCock()) output(" and your prostate");
	output(", and travels deep inside you, bulging your belly the slightest bit once it's deposited.  With a groan of deep satisfaction, you start humping the fluid-slicked chitin beneath you, barely registering the long, seemingly unending series of eggs that infiltrate your ass");
	if (pc.hasCock()) output(" and milk your prostate as they push past");
	output(".\n\n");

	output("The stream of slime only spreads as the stimulation of the sliding spheres slewing into your sphincter sets off");
	if (pc.hasGenitals()) {
		output(", simultaneously, ");
		if (pc.hasCock()) {
			output("a storm of semen-squirting spurts from your shaft");
			if (pc.cockTotal() > 1) output("s");
		}
		else if (pc.hasVagina()) output("a siege of spray-scattering spasms from your snatch");
		output(" and");
	}
	output(" squirts of slippery spooge from her shuddering spunk-stick.  While you shriek, squirm and shiver shamelessly, you see the spider-slut suspended in the same, similarly a slave to the sensation of shoving and slamming her stalk into your straining sacrum and stuffing you with her strange spawn.\n\n");

	output("Gradually, you're lifted off the carapace by your inflating abdomen.  Your belly gurgles, making you burp as you're filled with enough eggs to make you feel uncomfortable and compressed.  The " + pc.skinFurScales() + " of your stomach is drum-tight by the time you slide off the wilting ovipositor, and the bottom portion of it is soaked in the last of your partner's orgasm.  You look positively pregnant as you stagger upright and make for your garments, trying to ignore the exhausted drider's jibes.\n\n");

	output("By the time you finish getting dressed, she's fallen into a fitful slumber.  What an ordeal!\n\n");
	//GET KNOCKED UP
	//pc.buttKnockUp(PregnancyStore.PREGNANCY_DRIDER_EGGS, PregnancyStore.INCUBATION_DRIDER - 200, 10); //Butt pregnancy goes faster
	pc.loadInAss(enemy);
	//pc.slimeFeed();
	pc.orgasm();
	pc.cor(0.5);
...
private function loseVSDriderGetAnalledOvi():void
{
	//spriteSelect(77);
	clearOutput();
	output("You fall to the ground, defeated by the agile arachnid-woman as she slowly advances towards you, her engorged clit growing larger and larger as it throbs in your direction.  You've managed to find yourself crumpled face down in the dirt, your [pc.ass] in full display of the aroused drider while she grins lustfully at your broken figure.  Grunting, you begin to pull yourself off the ground, only to hear your attacker 'tsk' before feeling the warm stickiness of her web launch upon your limbs, gluing your hands to your ankles and leaving you completely vulnerable.  You struggle against the bindings, but to no avail.  Lifting your head, you turn to see the drider stroking her now fully-enlarged demon cock, replacing the hardened clit that once adorned her moist pussy.\n\n");
	output("\"<i>You just had to make it harder for yourself, didn't you?</i>\"\n\n");

	output("You grunt, too exhausted to move any further.  She chuckles at your disinterest, bending down to rest her thick member in between your cheeks and slowly grind along the cleft of your ass.  You feel the tiny nubs of her demonic cock rub along your eager asshole, pre-cum lubricating your hot entrance as she moans in unrequited pleasure.  Gritting your teeth, you force yourself to ");
	if (pc.HP() <= 1) output("relax in order to reduce the discomfort");
	else output("calm your racing thoughts and resist showing how eager you are");
	output(" as she begins to enter inside of you.\n\n");

	output("Inch by inch, the bumps and curves of her nine-inch demonic dick massage your puckered hole as she plunges further in, working slowly to savor the feeling of being inside of you.  With a hearty grunt she fails to control her lust, driving in at full force and filling you with her corrupted member.");
	pc.buttChange(enemy.cockVolume(0), true, true, false);
	output("\n\n");

	output("\"<i>Ungh, if I knew your ass was this eager to s-swallow my cock I wouldn't have wasted my time tying you up...</i>\"  She gasps, clearly the more impassioned of the two of you as she manages to bottom out inside of your [pc.ass].  You have very little time to regain your breath as the horny drider-woman begins to thrust in and out of your [pc.asshole] with fevered abandon, tongue lolling out of her open mouth while you're forced to bounce against her crotch, your ");
	if (pc.balls > 0) output("[pc.balls]");
	else if (pc.hasVagina()) output("[pc.vagina]");
	else output("taint");
	output(" slapping against her slavering cunt.  She grabs on to your [pc.hips], using you as leverage to pump her pole inside of you while you jerk against the ground, eyes closed tight while your cheek rubs against the cold dirt.\n\n");

	output("After what seems like hours of the lustful spider-taur plunging in and out of your now loosened hole, the horny herm lessens her speed, appearing to give in to exhaustion.  She slaps your asscheeks and slowly removes herself from inside you, your original feeling of fullness now being replaced by the pleasure of her nubby cock grinding against your entrance as it moves against your inner walls.  At this point you're already numbed to the sensation of her pleasurable rod, though the emptiness of your insides seems to wake you up from your enthused trance.\n\n");

	output("Your vigor renewed, you start to move against your binds once more, only to pause once you feel the sensation of a larger, more intimidating organ rub along your back.  You try to move around to observe the new invader, though you freeze when you notice the slim spider-legs of your opponent surround you on all sides - almost like a prison.\n\n");

	output("\"<i>Well, aah, enough of my fun,</i>\" the drider pants, groping her pillowy breasts and stroking herself to maintain her stiff erection.  \"<i>Time to get down to business.</i>\"\n\n");

	output("Shifting your head, you notice a menacing ovipositor reaching from underneath the arachnid-woman's abdomen - the ribbed, tentacle-like appendage thick and twitching with excited intention.  The lengthy organ bulges with her rounded eggs, its slick opening drooling a slimy liquid that pools onto the curve of your back.  You stifle a moan as you feel her thickness poking against you, the segmented ovipositor tracing around your puckered anus to relax the unprepared hole.  Feeling something drip onto your face, you look over your shoulder to see the hot and bothered drider woman groping and massaging her distended member, pre-cum spurting from the tip and landing on your nose.  Her scent fills your nostrils, the feeling of helplessness doing little to aid your struggle.\n\n");

	output("With a more gentle movement of her body, the arachnid-morph begins to ease into your [pc.asshole], the ridges of her ovipositor grinding against your opening.  The bulging appendage is far thicker than the drider's demonic dick, and the constant jerking and shuddering of the egg-laying organ causes your body to drag along the moist dirt to keep up with the movements of the cock-like extension.  The drider's length feels as if it stretches on infinitely, the arachnid-tentacle dipping further and further inside until it stops inside of you, beginning to expand uncomfortably against your inner walls.  Your mouth hangs open; the feeling of the growing stalk as it tenses is driving you insane.  The carapaced body of the drider lowers onto you, her weight resting upon your back while the woman groans in pleasure.\n\n");

	output("\"<i>A-ah, here they come!</i>\"\n\n");

	output("One by one, the round protrusions of the ovipositor enter your ");
	if (pc.HP() <= 1) output("abused");
	else output("hungry");
	output(" asshole, her eggs squeezing along and filling you up inside.  You yelp as the seemingly endless onslaught of spherical objects force their way inside, the drider effortlessly pumping in and out of you to help the process along.  The arachnid-woman pants and curses while she jerks off her nubbed member, using both of her hands to double the pleasure of being inside of you.  You're pushed and shoved against the ground while your attacker slams against your [pc.ass] with wild abandon, and with one final push she's sent over the edge, shuddering in orgasmic delight as ropes of cum fly out of her thick length and land ");
	if (pc.tallness < 66) output("in front of you");
	else output("on your face");
	output(", her pussy juices squirting alongside them.  As she calms down, the herm-drider's spunk lessens in force, instead drooling out of her softening cock and landing onto your ");
	if (pc.tallness < 66) output(pc.face() + ", sliding down across your cheeks and along your lips");
	else output("chest, trickling between your [pc.nipples]");
	output(".  She pulls out her ovipositor slowly, though the bulge in your stomach remains as evidence of her work.");
	pc.buttChange(50, true, true, false);
	output("  With a giggle, she removes herself from you and walks in front of your defeated form, bending down to kiss you gently on your lips and licking a pearl of semen from your face.  Your skin tingles with the brush of her venomous lips.  \"<i>Thanks for the fuck, pet!  Here's hoping my babies won't keep you for too long,</i>\" the arachnid-woman purrs, before turning around and walking towards the hazy embrace of the fog.  \"<i>Toodle-oo~!</i>\"\n\n");
	output("...\n\n");
	output("A few hours later and you wake up to the sound of loud croaking and the smell of the bog.  You try moving your limbs, finding that your bindings had partially dissolved since the encounter. Painfully removing yourself from the ground, you begin your journey back to camp - carrying a bit more than you were before.\n\n");
	//GET KNOCKED UP
	//pc.buttKnockUp(PregnancyStore.PREGNANCY_DRIDER_EGGS, PregnancyStore.INCUBATION_DRIDER - 200, 10); //Butt pregnancy goes faster
...
private function loseVSDriderAndVaginallyImpreggleOvi():void
{
	...
	output("The drider ignores you at first, lifting her head and glancing from side to side.  It's unclear why - perhaps she is worried that another predator will have been attracted by the sounds of combat, and attempt to steal her prey away.  Whatever her reasons she's apparently satisfied with what she sees.  Her gaze returns to you, eyes twinkling maliciously.  \"<i>Aren't you <b>precocious</b>,</i>\" she chuckles, walking slowly towards your prone body.  \"<i>Such an aggressive little morsel.</i>\"  A single strong spider leg places itself on your chest, pushing you ");
	if (pc.isBiped() || pc.isGoo()) output("down onto your back");
	else output("over, onto your side and then to your back");
	output(".  It seems that, regardless of their slenderness, she can still put an impressive amount of weight behind her legs.  They ");
	if (!pc.isNaga()) output("pull your own " + pc.feet() + " apart");
	else output("scrape inquisitively at your scales");
	output(", her body climbing atop you.  Before you can realize her plans her spinneret is at work, ");
	if (!pc.isNaga()) {
		output("coating your ");
		if (pc.isTaur()) output("rear ");
		output(pc.legs() + " and pinning you in that prone, separated position");
	}
	else output("spraying webbing across your tail and weighing it down to pin it against the earth");
	output(".  She turns with the eerie speed of the arachnids of Mareth, quickly performing the same acts to your ");
	if (!pc.isTaur()) output("wrists");
	else output("forelegs");
	output(" and leaving you pinned and spread-eagled on the ground.  \"<i>That should stem that.</i>\"\n\n");

	output("Turning, she makes as though to bite you, before pausing.  A thought runs through her head, and if the wicked curve of her dark lips are any indication, she likes it.  She gathers webbing on her fingers and bends down, inspecting your features carefully.  Placing said fingers on your chin she forces it open, quickly and expertly weaving sticky strands to connect your jaw and your chest.  The strands are just short enough to keep your chin held down, while strong enough that you can't seem to pull at them.  ");
	if (!pc.isTaur()) output("Her fingers then move to the sides of your head, lacing webbing through your hair and sticking it firmly to the ground. More vulnerable than ever, you can't lift your head without ripping your hair from your scalp in chunks.");
	else {
		output("Her fingers then reach behind your head, pulling your upper body off of the damp dirt and stretching sticky strands between your skull and your immobile forelegs.  ");
		if (pc.biggestTitSize() > 4) output("With pain and then a pop, your " + pc.allBreastsDescript() + " squash out to the sides as she presses your torso into your knees.  ");
		output("Reaching back for more webbing, she ties your arms behind your back in a bundle of white thread.  Now pretzeled into an uncomfortable curled shape, you can't pull your torso away from your forelegs without wrenching your head off of your neck.  You feel completely helpless.");
	}
	output("  \"<i>Now, isn't that just precious,</i>\" the drider chuckles again.  She leans in low, her fingers trickling over your thighs like the steps of... well, of small spiders.  You gurgle in response, mouth held open uncomfortably.  She roams slowly up your body.");
	if (pc.hasCock() && enemy.nipplesPierced > 0) output("  Chains brush over [pc.eachCock] as her chest passes your crotch.");
	output("  Her fangs trail teasingly along your belly, her fingers crawling up your sides.");
	if (pc.biggestTitSize() > 3) output("  Venom dribbles down the cleavage of your " + pc.biggestBreastDescript() + " as she approaches your vulnerable face.  Her own quartet of matronly tits comes to rest on them as her torso appears over their horizon.");
	output("\n\n");

	output("Drawing ever closer, at last her eyes blink slowly in front of yours.  Her lips seem to be pouting, then smirking, then - oh.  She's swirling something around in her mouth.  Finally it opens up above yours, her fangs extending.  Though you flinch in preparation for a bite, none comes - instead venom drips off her teeth and lips, every drop felt as it lands on your tongue.  You gurgle once more in protestation at this torture, but her hand pets the skin of your forehead maternally to remind you of your powerlessness.  Sadistic delight plays across her eyes as she watches your restrained struggles.  As the venom seeps, a heat grows.  Though subtle at first, you soon recognize it: lust.  Raw, burning need.  The poison coursing through you makes little effect on your heart, but a world of difference to your crotch.");
	if (pc.hasCock()) output("  Worse still, its blood-swollen protuberance is met with no reaction from the drider.");
	output("  The slow drip serves no purpose - at least, none that serves you.  She is not teasing your body, nor whispering delicious torments.  All it does is force you to tremble more and more as she watches, knowing the effect that her venom will have. For no reason beyond her own sadistic amusement, she forces your body to put on the display of a helpless victim losing more and more control of their lust. You feel like a toy.\n\n");

	output("And the more you drink, the less you mind that.\n\n");

	output("Eventually she decides you've had enough, or perhaps grows impatient.  Her mouth closes, and her body rises.  Yours, incapable of either, merely burns.  The entirety of your body desires her, and your mind has a difficult time coming up with counterarguments.  Venom flows through you, commandeering your sensibilities and trading them in for a desire thus far unquenched.  She stands tall above you, straddling your body with her arachnid behind.  Something moist and ridged stretches out, caressing your thighs.  The degree to which you hope it's a cock is nearly immeasurable.  \"<i>I think you'll make a delectable little nest, morsel,</i>\" the drider croons.  \"<i>Let's start.</i>\"  Though you can't shift your head down to look at what she has in her behind, you can feel it.  It slaps messily against your thighs, searching out your [pc.vagina].  The caress the tentacle gives your slit is mockingly affectionate, but any illusions of gentility that may have persisted are soon ended by the thrust that delves greedily and deeply into your [pc.vagina].  If there is any small mercy, it's that the insatiable lust flooding your mind may wind up sated after all.\n\n");

	output("The thick, vile appendage twists inside of you as it advances.  Slick and obscene in its girth, each individual segmented ridge is palpable against your walls.  The prick practically breathes inside of you, pulsing with life and the ravenous desires that embody a phallus.  As it pushes it twists about in an attempt to wriggle in deeper, inadvertently showing the flexibility of its form.  \"<i>How many eggs have you lain, snack?</i>\" the drider coos.  Her fingers rub her sizeable swollen clit forebodingly.  It grows increasingly large, acquiring veins and bulges where no clit should.  \"<i>How swollen has your belly grown?</i>\"  Her tongue flicks across her venomous full lips.  \"<i>Time to enjoy it again.</i>\"  You can't see the tentacle shoved into your [pc.vagina], but you are increasingly certain that it is in not, in fact, a dick.");
	pc.cuntChange(0, 50, true, true, false);
	output("  With her body in the way, you cannot clearly see what transpires in your loins, yet a sight emerges before your eyes that dominates your mind. Her clit has finished growing, and is no longer a clit.\n\n");

	output("From her crotch a cock has erupted, demonic and terrible.  It drips with sweat and musk, dribbling onto your collarbone.  The position you're in has never seemed more ominous - or more exciting.  Her cockhead smacks against your lower lip, pre-cum bouncing off and lightly spattering on your forehead.  Venom-addled, breathing in her fragrant spunk sends a quake up your body.  Inadvertently your hips start to seize, your [pc.vagina] shuddering in surprising pleasure.");
	if (pc.hasCock()) output("  Even your [pc.cock] spurts some pre-cum of its own.", false);
	output("  There's some shame in having a small orgasm from nothing more than the scent of sperm, but you place the blame solely on your intoxicated state.\n\n");
...
private function loseVSDriderAndGetFuckedANDAnalImpreggegity():void
{
	...
	if (pc.lustQ() > 99) output("Unable to resist the desire coursing through your body, you strip off your [pc.gear] and start stroking [pc.eachCock], displaying it in the hope that it will draw the drider's attention, while at the same time just trying to get yourself off and attain release.");
		...
		output("member");
		if (pc.cockTotal() > 1) output("s");
		output(" to the air.  She squeezes [pc.oneCock] in her hand and pumps it, smiling while you stiffen and twitch precipitously in her grasp.");
	}
	output("  With a strength beyond what her lithe frame would imply, the spider-woman's shiny, exoskeletal arms slip under your shoulders and lift you up, hugging you into both pairs of breasts");
	if (enemy.nipplesPierced > 0) output(", the crossed chains digging into your [pc.chest]");
	output(".  The warmth of her nodule-covered cock pulsates on your belly, smearing you with a trail of pre-cum while she arranges your entwined bodies.\n\n");

	output("Lines of webbing burst from the arachnid abdomen, snapping into tree limbs before pulling taut.  The drider's eight legs clitter and clack, four of them folding around your back while your world rotates 90 degrees.  The two of you are pulled into the trees by the sticky strands, hanging sideways, almost upside-down, in the leaves.  The four-eyed monster-girl blinks asynchronously and smiles, shifting you slightly to bring [pc.oneCock] against her moist lips.  You exhale, an intense, animalistic thrill shivering through you from the feeling of having your [pc.cock " + x + "] lodged in a woman's entrance.  She leans down and bites into your neck, releasing a surge of toxin into your bloodstream.  It boils like fire, making you twitch and squirm in her many-limbed clutches.  The pain fades quickly, but in its place it leaves behind a molten, liquid lust, filling [pc.eachCock] to its absolute maximum size, as hard as it can possibly be.\n\n");

	output("The " + enemy.hairColor + "-haired dominatrix pulls back, a single drop of blood clinging to one of her fangs while she hisses, \"<i>Mmm, fresh meat is always so tasty... not to mention... firm...</i>\"\n\n");

	output("You look at her with fear, but your [pc.hips] start to twitch against her, rubbing your [pc.cockHead " + x + "] up and down the slippery slit.  A slow exhale of pleasure escapes the spider's purple lips while her legs squeeze tighter against you, trying to push your [pc.cock " + x + "] home.  The drider gasps when at last, you slip between her juicy outer labia, slurping deep into her cunny with a lurid squelch.  Finally held within her depths, your muscles all slacken, save for those you need to pump your hips against her, futilely trying to fuck her hot quim even though you have zero leverage.\n\n");

	output("\"<i>So eager!  I'm not going to let you fuck me, but I do want your sperm.  After all, I'll need to fertilize some more eggs.</i>\"\n\n");

	output("Wait, eggs?  While you try to chew over the thought, you're battling with your raging, venom-enhanced libido in order to think about something other than how good her pussy feels.  Fuck it!  You go back to grinding against your mistress, nuzzling at her neck while she twists her spider-half around you, holding you tightly in her chitin and bringing her abdomen up behind you.  A weird, wet appendage rubs over your exposed [pc.ass], slapping at your cheeks until it finds the cleft between them.  Sliding deeper, it bumps into your sphincter, spurts something warm, wet, and sticky against it, then pushes through.");
	pc.buttChange(8, true, true, false);
	output("  You shudder from the anal violation and twist against the legs holding you in place, but with your [pc.cock " + x + "] locked inside her, it's a useless endeavor.  It isn't shaped like a dick, and besides, that's currently dripping on your belly.  Oh gods... she mentioned eggs.\n\n");

	output("The strange organ keeps forcing its way deeper, slipping the steadily-widening, rib-like segments of its bulk into your [pc.asshole].  ");
	output("  Once it gets deep enough to push on your prostate, an upsurge of pleasure hits you hard enough to make [pc.eachCock] dribble.  You moan against the drider's neck, pulsing pathetically inside the spider-woman's slit.  You know you'll soon be giving her exactly what she wants - all of it - and there's nothing you can do about it.\n\n");

	output("Another inch slides inside you, filling you uncomfortably full.  You shiver, clutching at your spidery mistress while she prepares both to deposit and to withdraw from the bank of you.   While you hang there, on the very edge of orgasm, it stops going deeper.  You've been given a reprieve, but it doesn't last long.  The drider kisses your forehead, her venomy lips spiking the sensitivity of your skin once again.  Suddenly, the fat tube in your [pc.asshole] is dilating, stretching you wider.  Goo races up it, pumping into your guts, paving the way for the bigger bulk of the coming egg.  It stops at your pucker, battering against your muscles' resistance until it pops inside you, slamming into your prostate so hard that a huge rope of dick-milk erupts into the spider-woman's wet vagina.");
	pc.buttChange(50, true, true, false);
	output("\n\n");

	output("The egg travels further up your butt, eventually flowing out the end of her stalk-like organ to rest deep in your digestive tract, ensconced in a bubble of slime.  Before you can adjust to this new sensation, another egg slips through your now-widened ring.  It squishes past your anal g-spot, rubbing it just as hard as the first, but before it even finishes traveling up your intestines, a third egg is forced inside you.  [pc.EachCock] begins to cum wildly, squirting and leaking a torrent of jizz.  The first few ropes blast out with incredible force, hosing down her cunt");
	if (pc.cockTotal() > 1) output(" and belly");
	output(".  Then, the constant application and release of pressure on your prostate utterly destroys the orgasmic timing of your muscles' contractions, making [pc.eachCock] release a weak, but constant trickle of cum.\n\n");

	output("You quickly lose track of how many of the drider's eggs have gone inside you.  Feeling the numerous bulges in your abdomen, you give up entirely.  Your ass is going to be full of eggs and you've probably cum enough for her to make a whole 'nother batch.  Shuddering while the ovipositor is withdrawn, you sag, exhausted, barely realizing that the drider is hosing down your [pc.chest] with her arachnid herm-spunk, the nodules ringing her dick pulsing in time with the blasts of thick spooge.  Some sticks to your chin, but you're so drained from your rough treatment that you barely react.\n\n");
	output("\"<i>Shhh, shhh... it's all right.  You'll be such a good mother for my young, and an even better father for the eggs you just fertilized.  Maybe if we ever meet again you can tell me how they looked when they were born.  For now, rest up.  I want you to take good care of our babies,</i>\" instructs the drider while she lowers herself towards the ground.  She sets you down on a soft spot, giving you a blanket of moss to recover in.  You struggle to stay awake but quickly slump into a deep slumber.\n\n");
	//(slimefeed)
	//pc.slimeFeed();
	//GET KNOCKED UP
	//pc.buttKnockUp(PregnancyStore.PREGNANCY_DRIDER_EGGS, PregnancyStore.INCUBATION_DRIDER - 200, 10); //Butt pregnancy goes faster
...
private function buttFuckADriderOhBaby():void
{
	...
	//spriteSelect(77);
	clearOutput();
	output("As soon as the drider-girl goes down, you leap into action.  Before she has a chance to recover, you run over and leap onto her back, balancing shakily as the drider stumbles back up onto her many legs, trying to shake you off.  You grab onto the drider's hair for support, pulling tight each time you're nearly displaced.  Yelping in agony, she soon learns her lesson and settles down, submitting to your dominance.\n\n");

	output("Now that she's calmed down, you give her a hard shove, putting her on her drider-belly so you're not nearly thrown off her every other moment.  With that done, you put your hands on her human back, pushing her forward and exposing her human-like bubble-butt, seated just above where woman meets spider. Greedily, you pull her cheeks apart, revealing the tiny star of her puckered asshole.  You pull your [pc.cock " + x + "] out of your [pc.gear], already hardening from the thought of you pounding her tight little backdoor.\n\n");

	output("\"<i>Oh, you want that hole, do you? How very kinky,</i>\" the drider chides, but you aren't fooled by her tone вЂ“ you can see her anus relaxing, preparing itself for your arrival.  With a grin, you grab your [pc.cock " + x + "] and push it against her tight hole, leaning into the penetration until you feel it slip past her sphincter and into her hot, snug passage.  As you slide more and more of your cockmeat into her, you reach up from her cheeks and grope her breasts, squeezing her pert nipples between your fingers as you finally bottom out.  Since you can go no further up her butt, you start to piston your hips forward, slapping your hips to her ass with short, forceful strokes.\n\n");

	output("You're pleased to see the drider's tongue loll out of her mouth, her eyes rolled up into her head. Clearly she's enjoying this!  Encouraged, you start to fuck her butt harder, hammering into her ass with every bit of force your powerful muscles can summon up.  You're rewarded with a sharp, high moan from the drider.  Once she starts, she doesn't stop.  A symphony of moans, grunts, and pleasured screams erupt from the spider-girl, all in concert with your rough anal ravishing.\n\n");

	output("Without warning, the drider cums.  She cries out and grabs her fully-engaged corrupted cock, hosing down the trees in front of her with spider-cum as her butt squeezes down on your [pc.cock " + x + "], milking your dick with her anal passage.  You pick up the tempo of your fucking to break-pelvis speeds, ramming your cock into her backdoor and letting loose a torrent of your spunk, shooting it deep inside her");
	//(if Vagina: 
	if (pc.hasVagina()) {
		output(" while your unused cunt spurts girlcum across her carapace");
		if (pc.cockTotal() > 1) output(" and");
	}
	else if (pc.cockTotal() > 1) output(" as");
	if (pc.cockTotal() > 1) {
		output(" your spare cock");
		if (pc.cockTotal() == 2) output(" coats");
		else output("s coat");
		output(" her human back with a thick white layer of spooge");
	}
	output(".\n\n");

	output("Finally done, you pull out of the drider, collapsing on her spider-back with a contented sigh.  Completely insensate after the anal ravaging you just gave her, the drider poses no threat as you leisurely gather [pc.eachCock] back into your [pc.gear] and lower yourself off her cum-soaked abdomen.  You give her a final, playful ass slap as you dismount before heading back to camp.\n\n");
...
private function driderVagSmartFuck():void
{
	...
	output("You keep your distance from the downed spidermorph, still wary of her dripping, toxin-coated lips despite her weakened state and your own rising desire to breed her.  Trailing your eyes over her sweaty, splayed body you feel a familiar rush below the belt, your hardening [cock " + y + "] pressing against the inside of your [pc.gear] as her breasts rise and fall seductively with each panting breath.  Desire threatens to overpower your caution as your gaze lingers on the vulnerable woman before you, her netherlips ");
	if (enemy.HP() <= 1) output("parted slightly and glistening with beads of sweat from the adrenaline of battle.");
	else output("stuffed with her fingers, femcum streaming down the front of her spider-half as she desperately plays with herself.");
	output("  The need to bury your aching length inside that puffy snatch is almost unbearable, but you don't want to risk exposing yourself to any of her tricks, especially considering how close you'd be to her mouth as you take her.  Racking your brain for a solution, the mass of webbing that covers the canopy around this area catches your eye.");

	if (pc.hasTail(GLOBAL.TYPE_ARACHNID) || pc.hasTail(GLOBAL.TYPE_DRIDER)) {
		output("\n\nA plan quickly forms in your mind and you feel your spinnerets twitch with anticipation as you ");
		if (pc.isDrider()) output("scuttle");
		else output("saunter");
		output(" over to the collapsed drider.");
		driderTiedUpPartTwo(false);
	}
	//(Champion has some Tough Spider Silk on them)
	else if (pc.hasItem(new CoCToughSpiderSilk())) {
		...
		//[Yes] [No]
		clearMenu();
		addButton(0, "Yes", driderTiedUpPartTwo, true);
		addButton(1, "No", dontSilkRapeDrider);
	}
}
...
private function dontSilkRapeDrider():void
{
	//spriteSelect(77);
	clearOutput();
	output("Deciding against wasting the valuable material for little more than a quick fuck, you ponder your other options for using the defeated girl.\n\n");
	//(Return to "win" menu)
	clearMenu();
	addButton(0, "Next", defeatDriderIntro);
}
...
private function driderTiedUpPartTwo(useItem:Boolean):void
{
	...
	output("\n\nHer firm behind continually bumps against your throbbing shaft as she struggles against your attentions, rapidly bringing you to full hardness and making you wince as you press against the inside of your [pc.gear].  You can feel a damp patch spreading against your tip, a result of the pre-cum being forced out of you with every grinding movement the drider makes against your crotch.  Unable to resist, you quickly drag one hand down her body, streaking a path through the mixture of sweat and swamp-water still clinging to her toned muscles, until finally driving it between your bodies to free your aching member.  You nearly gasp as it springs free, wetly slapping against her taut rear and driving her to another fit of panic as she squirms away from your thick meat, apparently terrified that you're going to stuff her tight ass.");
	...
	if (!useItem) output("Shifting yourself, you withdraw your hand from her quivering slit, reaching back to collect another handful of stretchy silk.");
	else output("Reaching back into your pack, you grab the remainder of the bundle of silk that you tied the girl's hands with.  Looking over the mass of white-grey strands, you smile as you see that there seems to be more than enough left for what you have in mind.");

	//New Page
	clearMenu();
	addButton(0, "Next", driderTiedUpPartThree);
}
...
private function driderTiedUpPartThree():void
{
	...
	//dynStats("sen", -1);
	clearMenu();
	addButton(0, "Next", driderTiedUpPartFour);
}
...
private function driderTiedUpPartFour():void
{
	...
	//(if (30 =< corruption < 70)
		...
		output("You make sure she's comfortable before checking her for anything that could help your quest.  She stirs slightly as your hands caress her, eyes fluttering drunkenly as she tries to focus on you.  Something in the way her eyes bore into you fills you with a sense of shame at having taken advantage of her and you quickly sever her bonds, reasoning that she'll still be too weak to try anything.");
