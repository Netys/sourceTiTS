import classes.Characters.CoC.CoCChameleonGirl;
...
public function showChameleon():void
{
	showName("\nCHAMELEON");
}
...
public function encounterChameleon():void
{
	...
	if (int(flags["COC.TIMES_MET_CHAMELEON"]) == 0) {
		...
		//ENCOUNTER INTRO; REPEAT (Z edited)
		output("You work your way through the dense foliage of the bog, pushing aside branches and slogging through the thick mud in search of something new.  Feeling exhausted, you slow down and look for a place to rest; a small clearing with shallow water and firmer ground seems to fit the bill, and you sit back against a tree to catch your breath.  You're so soaked by now that you hardly notice the murky water beneath you and the slick mud on the trunk seeping into your [pc.gear].");
		output("\n\nA sudden scrape sounds behind you!  You spin around in time to see the familiar shape of a chameleon girl peeling off the trees.  She approaches you with a menacing glare in her eyes, growling \"<i>This is my bog!  Get out now or... or face the consequences!</i>\"  You [pc.readyWeapon] and prepare to defend yourself.");
	}
...
public function loseToChameleonGirl():void
{
	...
	//-Lose by lust
	if (pc.lustQ() >= 100) {
		output("Losing control to your own growing arousal, you fall to your knees and desperately start working to get at your needy body beneath your [pc.gear].");
	}
	//Lose by HP
	else output("Too weak to keep fighting, you stagger back and fall to your knees.");
	...
	if(!pc.hasGenitals()) {
		...
		if(pc.HP() > 0) applyDamage(new TypeCollection( { truedamage : 10 + rand(10) } ), enemy, pc);
	...
	{
		var options:Array = [];
		if (pc.isHerm()) options.push(loseToChameleonWithCockAnBallsAnCunt);
		if (pc.hasCock()) options.push(dudesLoseToChamChams);
		if (pc.hasVagina()) options.push(loseToChamChamWithPCCunt);
...
private function loseToChameleonWithCockAnBallsAnCunt():void
{
	...
	output("\n\n\"<i>Um, strip for me!</i>\" she commands, as forcefully as she can manage.  Still, it almost sounds like a question.  Too weak to do anything but play along, you comply, shedding your [pc.gear].");

	output("\n\nAs you set the last bit of your [pc.gear] aside in the water, you're suddenly struck in your chest and pushed to the ground.  The chameleon girl's wide foot keeps you pinned down, and she looks at you almost apologetically.  She shakes the worry from her face, and tries to appear stern once again.  \"<i>I'll... I'll show you who's the boss around here!</i>\"  Her tail darts towards you and uncoils at its end.  The tip starts probing around your " + pc.vaginaDescript(0) + ", brushing across your " + pc.clitDescript() + " as it feels around for your entrance.");
...
private function dudesLoseToChamChams():void
{
	...
	showChameleon();
	output("\n\nRecognizing that you're no danger, her skin shifts back into its natural " + enemy.skinTone + " coloration as she walks towards you.  She swings her hips seductively as she splashes slowly through the water and licks her lips with a loud smack of her tongue, then reaches out and forcefully pulls your [pc.gear] above your waist before you can even try to resist her advances, and pushes you back against a tree.  With a few quick pulls of fabric she's stripped her own clothes, and she tosses her thong over a low-hanging branch.  The other band of silk she uses to quickly bind your wrists.  Were you in any shape to fight back you could pull free, but you're too ");
	if (pc.lustQ() >= 100) output("overwhelmed by lust");
	else output("battered");
	output(" to even rip the makeshift binding.");

	output("\n\nShe makes short work of your [pc.gear] in her haste to see her prize, then stands up and, still looking down, lets her tongue fall out of her mouth and find its way to [pc.oneCock].  In an amazing display of control, she manages to wrap the warm pink appendage around your man-meat, sliding the slick length of her warm tongue up and down your shaft almost as if she were massaging you.  You shudder softly as she tickles the underside of your cock with just the tip of her tongue, trailing sticky saliva around its head.  Your member is soon erect and throbbing with need; satisfied, she slurps the slender length of muscle back into her mouth.");
...
public function defeatChameleonGirl():void
{
	...
	//-Win by lust
	if (enemy.lustQ() >= 100) {
		output("Unable to control her arousal, the chameleon girl collapses to her knees and begins masturbating underneath her thong, having lost all capacity to fight you; she moans and throws her head back as her hand splashes in and out of the water she's kneeling in.  Her skin returns to its usual " + enemy.skinTone + " and then keeps going, shifting closer and closer to pink as her moans increase in both volume and volubility.");
		if (pc.lust() >= 33) output("  Do you help the horny girl to get you both off?  Or maybe you could punish her bad attitude with something from your bag...");
	}
	//-Win by HP
	else {
		output("Too weak to continue fighting, the chameleon girl drops to her knees, exhausted.  Her skin returns to its usual " + enemy.skinTone + ", unable to maintain the camouflage.");
		if (pc.lust() >= 33) output("  Do you use the girl in her weakened state to get yourself off?  Or maybe you could punish her bad attitude with an item from your bags...");
	}
	//(Display Options: [Fuck Her Face] [Pussy Rub] [Herm Style Pussyrub] [Incubi Draft] [Succubi Milk] [Lust&Sens Drafts])
	
	clearMenu();
	addDisabledButton(0, "Use Dick", "Use Dick", "This option requires you to have cock.");
	addDisabledButton(1, "Use Pussy", "Use Pussy", "This option requires you to have vagina.");
	addDisabledButton(2, "SuccMilk", "Use Succubi Milk", "This option requires you to have cock and a bottle of the Succubi Milk.");
	addDisabledButton(3, "LustD", "Use Lust Draft", "This option requires you to have a bottle of the Lust Draft.");
	...
	if (pc.hasCock()) addButton(0, "Use Dick", manFucksChameleonWithBiggishWang);
	if (pc.isHerm()) addButton(1, "Use Pussy", fuckDatChameleonAsACoolGuyGirlHerm); // those two are basically the same with focus on vagina, and for herm having extra vag only scene makes no sense
	else if (pc.hasVagina()) addButton(1, "Use Pussy", femaleHasWinSexWithChamCham);
	
	if (pc.hasCock() && (pc.hasItem(new CoCSucMilk()) || pc.hasItem(new CoCSucMilkPure()))) addButton(2, "SuccMilk", giveTheChameleonASuccubiMilk);
	if (pc.hasItem(new CoCLustDraft()) || pc.hasItem(new CoCFuckDraft())) addButton(3, "LustD", doseDatChameleonWithLustAndSensitivityDrafts);
}
...
private function fuckDatChameleonAsACoolGuyGirlHerm():void
{
	...
	if (pc.cor() < 50) output("and guiltily ");
	output("roll her onto her back and get up to collect your things, rinsing yourself with the water and shaking the mud out before putting your [pc.gear] back on and heading back to camp.");
	pc.orgasm();
...
private function manFucksChameleonWithBiggishWang():void
{
	...
	if (pc.balls > 0) output("your [pc.balls] clench and ");
	output("the semen erupts from the end of your " + pc.cockDescript(x) + " spilling out hot spunk over her folded tongue.  She gasps audibly and arches her back as the thick ropes of your jizz coat her tongue, shoving her long fingers as far as she possibly can into her dripping pussy, and flashes so brightly pink she nearly glows.");
	if (pc.cumQ() > 750) output("  The torrent of cum bursting from your cock just doesn't stop, and her eyes roll back into her head as your semen coats her face and runs off her tongue, drenching her breasts and belly in white.  Her head falls back and her tongue goes limp as she's sent into ecstasy by the flood.");
	...
	if (pc.cumQ() > 750) output("  Even the murky water around her is turned white as the pools of your cum on her body run off into it.");
	output("  Satisfied, you put your [pc.gear] back on and head back to camp.");
...
private function femaleHasWinSexWithChamCham():void
{
	clearOutput();
	showChameleon();
	output("You shed your [pc.gear] and approach the fallen chameleon girl");
	if (enemy.lustQ() >= 100) output(", her moans of pleasure growing more desperate as you approach");
	output(".  You look her up and down, following the smooth curves of her thin body with your eyes.  Squatting down beside her, you take her weak body by the shoulders in one arm and trail your fingertips lightly up her torso with the other, feeling the soft skin of her belly.");
	if (enemy.lustQ() >= 100) output("  She coos lustily at the sensation.");
	else output("  Her eyes widen, and she looks from your hand to your face and back again.");
	output("  When your hand reaches the silk cloth covering her chest, you pull it off with a light tug and cast it off to the side.  You cup her small breast in your palm, and as you squeeze it her skin tone changes perceptibly, flashing to a much brighter shade of the colors it was.");
	...
	if (pc.cor() < 50) output("and somewhat guiltily ");
	output("roll her onto her back and get up to collect your things, shaking out the mud and water before putting your [pc.gear] back on and heading to camp.");
	//send player back to camp, reset hours since cum, add gems and time
	pc.orgasm();
	output("\n\n");
	processTime(15 + rand(5));
...
private function giveTheChameleonASuccubiMilk():void
{
	clearOutput();
	showChameleon();
	output("You find a bottle of Succubi Milk tucked away in a pouch.  A wicked smile comes to your face as you imagine filling her so full of the stuff that her tits grow to the point where she can't even move, but you ultimately decide that just one dose will suffice for now.");
	...
	if (pc.hasTail() && pc.hasTailFlag(GLOBAL.FLAG_PREHENSILE)) output(" with your tail,");
	else output(" by straddling her,");
	output(" your cock aimed for her pillowy tits.  She pushes and squirms beneath you, but cannot get enough leverage to move you.  You take her breasts in your hands and shimmy up her torso towards them, squeezing them around your " + pc.cockDescript(x) + ".");
	...
	if (pc.hasItem(new CoCSucMilkPure())) pc.destroyItem(new CoCSucMilkPure());
	else {
...
private function doseDatChameleonWithLustAndSensitivityDrafts():void
{
	clearOutput();
	showChameleon();
	output("You dig through your bags and come across a particularly tortuous combination of fluids.  With a Lust Draft in one hand and a Sensitivity Draft in the other, you return to the chameleon girl, swirling them and smiling at her menacingly.  She starts to protest, but you cut her off by shoving the opening of the Lust Draft vial into her open mouth.");
	...
	output("\n\nToo turned on to just sit there, she falls forward, catching herself on you, then looks up, pleading with her eyes.  You laugh mirthlessly at her grovelling form, and tell her to beg if she wants you.  \"<i>Please!</i>\" she squeals, her eyes watering, \"<i>I need this!</i>\"  She tugs insistently at your [pc.gear], begging for you to fill her and crying out that she can't stand to feel so empty.  You grin, and wonder if the sensitivity draft has started to take effect.  Two fingers run lightly up her forearm produce shivers at the obviously amplified sensation, and a tweak of her nipple elicits a shrill shriek.  \"<i>Why does it feel so good?!</i>\"  She puts her face into your lap and begs, \"<i>I don't even know what's going on, please, just fuck me, pleasepleaseplease...</i>\" trailing off as she grinds her pussy into you.");
