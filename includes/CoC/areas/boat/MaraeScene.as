...
public function encounterMarae():void {
	...
	if(flags["COC.FACTORY_SHUTDOWN"] == undefined) {
		...
		if(flags["COC.MET_MARAE"]  == undefined) {
			...
			if(pc.cor() > 66) output("The woman bellows, \"<i>Begone demon.  You tread on the precipice of damnation.</i>\"  The tree's eyes flash, and you find yourself rowing back to camp.  The compulsion wears off in time, making you wonder just what that tree-woman was!");
			//Explain the dungeon scenario
				...
				output("You take a step back, amazed to find such a creature, apparently uncorrupted.  ");
				if(pc.libido() + pc.cor() > 80) output("Your eyes can't help but take note of the tree-woman's shapely breasts, and wonder if they feel like tits or wood.  ");
				...
				if(pc.libido() + pc.cor() > 80) {
					output("You could leave, but the desire to feel her breast will not go away.  What do you do?");
		...
		//Second meeting
			...
			else
			{
				...
				if(flags["COC.MARAE_QUEST_START"] >= 1) {
					output("Marae reminds you, \"<i>You need to disable the demonic factory!  It's located in the foothills of the mountain.  Please, I do not know how long I can resist.</i>\"");
					doNext(returnToCampUseOneHour);
				}
				//If not
				else {
...
private function maraeBadEnd():void {
	...
	output("She approaches you, her breasts swinging pendulously and dripping sap.   Mesmerized by her swaying mammaries, you watch until she mashes you into them with an enormous hug.  A hand traces down your chest to your groin");
	if(!pc.hasGenitals()) output(" where it pauses in momentary confusion");
	...
	//if(pc.statusAffectv3(StatusAffects.Marble) > 0) output("Any thought or need to drink Marble's milk vanishes from your mind and body.  ");
...
private function maraeStealLethicite():void {
	...
	//(FAIL)
		...
		output("\"<i>Awwww, what a nasty deceitful little " + pc.mf("boy", "girl"));
		...
		{
			output("She extends a hand expectantly, watching with detached concentration while a tentacle lowers from the tree into her palm.   Hips swaying sexually, she advances, peeling back the tentacle's outer layer.  It opens up to reveal a wet, gummy mouth.  She giggles and bumps the opening against your ");
			if(pc.cocks.length > 1) output("largest ");
			output(pc.cockDescript(pc.biggestCockIndex()));
			...
			output("\"<i>Oh that simply won't do,</i>\" Marae whispers, cupping your ");
			if(pc.balls > 0) output(pc.ballsDescript());
			else {
				if(pc.vaginas.length > 0) output(pc.vaginaDescript(0));
				else output("crotch");
			}
		...
		//FEM)
			...
			output("She extends a hand expectantly, watching with detached concentration while a tentacle lowers from the tree into her palm.  A swift slash of her free hand cuts your [pc.gear] free, exposing your ");
			if(!pc.hasGenitals()) {
				output("hairless crotch.  She holds the tentacle back a moment and raises her free hand.  It begins to glow and shimmer as she points to your groin.  Warmth explodes in your crotch as a wriggling wet gash opens up - <b>your new vagina</b>.  ");
				pc.createVagina();
			}
...
public function level2MaraeEncounter():void {
	...
	output("While rowing about the lake");
	if(pc.PQ() > 70) output(" with ease");
	output(", a familiar island resolves itself in the center of the lake.  There's a familiar tree perched atop it, though the wriggling tentacles' silhouette against the sky are a cruel reminder that this island will no longer be the peaceful haven it once was.  ");
	if(pc.cor() < 33) output("Knowing all too well what kinds of horrors await");
	else if(pc.cor() < 66) output("Knowing all too well what kinds of sexual escapades lurk ahead");
	else output("Worried about enjoying the pleasures of the island overmuch");
	...
	output("Squirming roots are crawling over the boat's underside.  They dig into every crease and crevice, binding the vessel tightly in place.  Just as you start to ponder swimming for the shore, the boat shifts, ");
	if(pc.RQ() >= 50) output("nearly ");
	output("dropping you flat on your ass before you catch yourself and sit down");
	output(".  The old dinghy's cutting through the water with amazing speed, leaving foot-high waves in its wake.  Marae's island grows larger with each passing second, almost taunting you with your inability to get away.   By now the root-like vines have crept over the gunwales, and they wriggle at you, seemingly in warning.  Getting in the water is definitely not a good idea right now.	The bone-chilling scrape of sand on wood grates at your ears.  You've arrived.  There's no point in putting off the inevitable.  You straighten up your [pc.gear]");
	if(pc.hasWeapon()) output(", adjust your [pc.mainWeapon],");
	...
	if(pc.hasKeyItem("Marae's Lethicite") >= 0) {
		output("Welcome back, sneak-thief.  What kind of " + pc.mf("gentleman","lady") + " is offered sex and then ransacks a god's soul?  Honestly, that right-right theeeeeree-oh yeah right there-is true depravity.</i>");
	}
	//(FAIL-STOLE)
	else if(pc.hasPerk("Marae's Gift: Stud") || pc.hasPerk("Marae's Gift: Fertility")) {
		output("Hey there [pc.name].  I didn't think I'd manage to snag you again so soon.  Are you enjoying my gifts?  I've been feeling kind of lonely without anyone around here to play with.</i>");
	}
	//(Left Like a Bitch)
	else {
		output("Well, look who came back!  I thought you were too afraid of a good time to come up here and fuck around with little ol' Marae.   I was actually going to let you row away if you were too scared to come here.  No, don't even glance back now, I've changed my mind.</i>");
	}
	output("\"\n\n");

	output("Featureless white irises glare at you from the goddess' lust-lidded eyes.  She commands you, \"<i>Come here.  It's time for a ");
	if(pc.hasPerk("Marae's Gift: Stud") || pc.hasPerk("Marae's Gift: Fertility")) output("second ");
	output("dose of Marae's tender affections.</i>\"\n\n");
	
	if(!pc.canFly()) {//Cant fly?  Stuck for sex! Or fight!
		output("You don't see any escape! If you like, you can attempt to fight her, but really?");
		addDisabledButton(1, "Fly Away", "Fly Away", "No sky for earth worms!"); }
	else {//Can fly?  Choice to run
		output("You don't think she's counted on your wings.  If you tried to fly you could probably get out of the reach of her tentacles in short order.");
...
private function MaraeIIStageII():void {
	...
	if(pc.hasVagina() && ! pc.hasCock()) {
		...
		output("Paralyzed by ");
		if(pc.lustQ() > 80) output("lust");
		else if(pc.cor() > 50) output("indecision");
		else output("worry");
		output(", you don't manage a single backwards step before the warm bulge is rubbing ");
		if(pc.tallness > 48) output("at your crotch");
		else output("against your belly");
		...
		output("Marae grunts and pulls on ");
		if(pc.hasHorns()) output("your horns");
		else output("your " + pc.hairDescript());
		output(", shoving her thick clit-cock deep inside your throat.  You reflexively swallow down the bulging fuck-meat and ");
		if(pc.cor() < 33) output("struggle to ");
		else if(pc.cor() < 66) output("work to ");
		else output("easily ");
	...
	//Marae uses a tree-tentacle to 'milk' male PC's.  Oral or Vajayjay? Not sure.
		...
		output("You watch, standing stock-still and paralyzed with ");
		if(pc.lustQ() > 80) output("lust");
		else if(pc.cor() > 50) output("indecision");
		else output("worry");
		...
		output("Vines whip out, sliding under your [pc.gear], undoing clasps, and removing it until you stand naked and exposed to Marae.  She smirks and crooks her finger at you in a 'come hither' gesture.  You lurch forward, as if pulled on an invisible string until you're standing inches away from her, your " + pc.cockDescript(pc.biggestCockIndex()) + " rubbing her belly");
		if(pc.cockTotal() == 2) output(" while your other dangles against her thigh");
		else if(pc.cockTotal() > 2) output(" while your others dangle against her thighs");
		...
		output("You grunt and pump your hips, shameless as you give in to the squeezing, textured tentacle");
		if(pc.cockTotal() > 1) output("s");
		output(".  Cum boils out from your ");
		if(pc.balls == 0) output("body");
		else output("balls");
		output(", but it's quickly devoured by Marae's tree-based tentacle-beast without a sound.  ");
		if(pc.cumQ() > 500) {
			output("The thick bulges of spooge actually distort the vines");
			if(pc.cockTotal() > 1) output("s");
			output(", letting you get the barest glimpse of white through the over-stretched tentacle-tube");
			if(pc.cockTotal() > 1) output("s");
			output(".  ");
		}
	...
	//[Hermz]  Marae grows vinecawks for DP under her flowercunt and sexes.
		...
		output("The goddess closes with deliberately slow, hip-swaying steps that make your " + pc.cockDescript(pc.biggestCockIndex()) + " tremble and leave no doubt that she's the goddess of fertility.  Marae giggles and wraps her arms around you, planting a firm, wet kiss on your lips while she undoes your [pc.gear].  Your gear hits the ground with a dull thump, and then her taut green nipples are pressed against your own");
		if(pc.hasFuckableNipples()) output(", slipping inside your cunt-like nipples with ease.  You moan into her ear at the unexpected penetration and shiver from the tingling shocks of pleasure in your " + pc.breastDescript(0) + ".");
		else output(".");
		output("  Hammering in your chest, your heart beats fast enough to flush your whole body when your " + pc.cockDescript(pc.biggestCockIndex()) + " manages to slip between Marae's legs.  It doesn't penetrate, merely trapping itself between her fluid-lubed thighs");
		if(pc.cockTotal() == 2) output(" while your other cock rubs over her surface");
		else if(pc.cockTotal() > 2) output(" while your other cocks rubs over her surface");
		output(".\n\n");
		...
		output("She's easy to push down into the soft grasses of the island, and her legs part to allow you better access.  Your " + pc.cockDescript(pc.biggestCockIndex()) + " doesn't need to be told what to do, and it slips into her waiting wetness as if it was made for her.  ");
		...
		else output("She feels perfect.  A velvet vice of hot, slippery wetness clutches tightly around your " + pc.cockDescript(pc.biggestCockIndex()) + ".  It almost feels like it's actually gripping you, cradling your cock in her ambrosia-slicked box.");
		if(pc.cockTotal() > 1) {
			if(pc.cockTotal() > 2) output("  Another ");
			else output("Your other [pc.cock " + pc.biggestCockIndex2() + "] prods at her tight pucker, and with a slight adjustment, you're able to line it up.  It's wet!  Inch after inch slides in with incredible ease, violating her slippery butthole until you've completely double-penetrated her.");
		}
		output("\n\n");

		if(pc.vaginas[0].wetness() < 2) output("Sticky wetness glistens between your thighs");
		...
		output("You happily thrust forwards, ramming your " + pc.cockDescript(pc.biggestCockIndex()) + " into her cunt with such force that a wet slap echoes over the lake and her fluids splatter your abdomen.  ");
		if(pc.cockTotal() > 1) output("Her asshole is squelching and dripping from your " + pc.cockDescript(pc.biggestCockIndex2()) + ", actually squirting more lubricant than her pussy from the violent fucking!  ");
		...
		output("Cum boils out of your ");
		if(pc.balls > 0) output("rapidly contracting balls");
		else output("tentacle-squeezed prostate");
		output(" and erupts into Marae's womb.  Your hips rock forward, grazing her cervix with your " + pc.cockHead(0) + " to better fill her uterus.  ");
		if(pc.cockTotal() > 1) output("The " + pc.cockDescript(pc.biggestCockIndex2()) + " in her ass spasms and explodes with its brother, glazing her slippery colon with a coating of syrupy spunk.  ");
		if(pc.cockTotal() > 2) {
			output("Neglected but orgasming, ");
			if(pc.cockTotal() > 3) output("the remainder of ");
			output("your ");
			if(pc.cockTotal() > 3) output(pc.cocksDescriptLight());
			else output(pc.cockDescript(pc.biggestCockIndex3()));
			output(" does its best to coat Marae's thighs with whiteness.  ");
		}
		output("The goop from inside you never seems to end, and you pump Marae's belly up with it until she looks a little pregnant.  ");
		if(pc.cumQ() < 500) output("The orgasm is so much more massive than normal, and you wonder if her magic enhanced it.  ");
		if(pc.cumQ() > 1000) output("She's actually surprised when you keep fountaining more seed into her.  Her nipples start to squirt out the excess seed, but you keep cumming until she's squirted enough to soak herself with your jism.  ");
...
private function MaraePt2RoundIIIPrizes():void {
	...
	if(pc.hasCock() && !pc.hasVagina()) {
		...
		//(NON STUD)
		else output("You might be a little sore.  I did some work to make sure you'll be a perfect breeding stud for me.  No tiny cum-shots for you!  You'll squirt out enough to knock up anyone, and I even touched up your seed so it'll get through most contraceptives.  Aren't I the nicest?</i>\"  ");
		//(CONTINUED)
	...
	//[Chickzillas]
		...
		if(pc.hasPerk("Marae's Gift: Fertility")) {
			...
			output("\"<i>I just made your butt-hole a little more welcoming for all the boys that are going to be fucking you.  I mean, once your cunt is full they need somewhere else to stick it right?  If anything the bee-girls should appreciate this.  I know they're kinky and like to use that side,</i>\" Marae confirms.  ");
		}
		//(NOT BREEDER)
		else {
			output("You might feel a little sore.  I gave your little womb a makeover to make sure you'll be nice and fertile for all the boys out there.  You're going to serve me so well.  So many died fighting the demons, and you'll be popping out kids from every dick that gets anywhere near your little birth-hole.</i>\"  ");
		}
		...
		if(pc.hasPerk("Marae's Gift: Fertility")) {
			output("<b>(New Perk Gained: Marae's Gift – Buttslut)</b>");
			...
			output("<b>(New Perk Gained: Marae's Gift – Fertility)</b>");
			pc.createPerk("Marae's Gift: Fertility", 0, 0, 0, 0, "Greatly increases fertility and halves base pregnancy speed.");
		}
	}
	//[HERMS]
		...
		if(!pc.hasPerk("Marae's Gift: Fertility") && !pc.hasPerk("Marae's Gift: Stud")) {
			...
			//(RANDOM 2)
				...
				output("You might be a little sore.  I did some work to make sure you'll be a perfect breeding stud for me.  No tiny cum-shots for you!  You'll squirt out enough to knock up anyone, and I even touched up your seed so it'll get through most contraceptives.  Aren't I the nicest?</i>\"  ");
			pc.createPerk("Marae's Gift: Stud", 0, 0, 0, 0, "Increases your cum production and potency greatly.");
			}
			...
			if(pc.hasPerk("Marae's Gift: Fertility")) output("<b>(New Perk Gained: Marae's Gift - Fertility)</b>");
			else output("<b>New Perk Gained: Marae's Gift - Stud)</b>");
		}
		//(HAZ BREEDER)
