import classes.Characters.CoC.CoCSharkGirl;
...
private function sharkGirlBadEndCounterTimePassedNotifyGrapple():* { timeChangeListeners.push(sharkGirlBadEndCounterTimePassedNotify); }

//[Explore Lake]
public function sharkGirlEncounter(exploreLoc:Number = 1):void {
	//spriteSelect(70);
	clearOutput();
	clearMenu();
	
	//Set 'PC met Sharkgirls' for Izma stuff
	if (flags["COC.SHARK_GIRLS_MET"] == undefined) flags["COC.SHARK_GIRLS_MET"] = 1;
	if (flags["COC.SHARK_GIRLS_BAD_END_CD"] == undefined) flags["COC.SHARK_GIRLS_BAD_END_CD"] = 0;
	
	if (flags["COC.SHARK_GIRLS_BAD_END_CD"] > 0) trace("Shark Girl Bad End counter: " + flags["COC.SHARK_GIRLS_BAD_END_CD"]);
	if(flags["COC.SHARK_GIRLS_BAD_END_CD"] >= 7 && pc.hasCock()) {
		sharkBadEnd();
		return;
	}
	...
	//Rowboat
	if(exploreLoc == 1) {
		output("While rowing the boat across the lake you spy a shark fin heading your way.  Worried it might damage the small boat, you hastily row back to shore, jumping out of the boat.  The shark shows no signs of slowing, and the fin disappears just before coming ashore.  ");
	}
	//Lake
	else {
		output("You wander around the sandy shores of the lake before raising your eyebrow at a strange sight: A solitary fin, circling around in the water. You take a step back in surprise when the fin suddenly starts barrelling toward the shore at inhuman speeds.  ");
	}
	
	output("A grey blur bursts from the water and lands on the ground a few feet away from you.\n\n");
	output("It's a woman – a peculiarly corrupted woman, with shiny grey skin, silver hair, and a fin positioned between her shoulder blades. She's wearing some rather revealing black swimwear. The girl looks up at you and grins widely, showing rows of knife-like teeth. \"<i>Wanna play? Heads up though, I play 'rough'!</i>\"\n\n");
	...
	//}
	output("You're fighting a shark girl!");
	
	CombatManager.newGroundCombat();
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(new CoCSharkGirl());
	CombatManager.victoryScene(CoCSharkGirldefeated);
	CombatManager.lossScene(CoCSharkGirlwon);
	CombatManager.displayLocation("SHARK\nGIRL");
...
internal function sharkWinChoices():void {
	...
	addButton(14, "Leave", CombatManager.genericVictory, null);
	addDisabledButton(0, "Use Dick", "Use Dick", "You need sufficient arousal. And dick.");
	addDisabledButton(1, "Pussy w/69", "Pussy w/69", "You need sufficient arousal. And vagina.");
	if (pc.isNaga())
		addDisabledButton(1, "Pussy Oral", "Pussy Oral", "You need sufficient arousal. And vagina.");
	addDisabledButton(2, "Dildo Rape", "Dildo Rape", "You need sufficient arousal. And Deluxe Dildo. And some bits to play for yourself.");
	...
	if (pc.lust() >= 33 && pc.hasGenitals()) {
		...
		if (pc.hasVagina() && pc.isNaga())
			addButton(1, "Pussy Oral", sharkgirlSixtyNine);
		if(pc.hasKeyItem("Deluxe Dildo"))
			addButton(2, "Dildo Rape", sharkGirlGetsDildoed);
	}
}
...
private function sharkgirlDickFuck():void {
	...
	if(pc.isNaga()) {
		x = pc.cockThatFits(enemy.analCapacity());
...
internal function sharkLossRape():void {
	...
	//Genderless:
	if(!pc.hasGenitals()) {
		output("You slump down in defeat, too ", false);
		if(pc.HP() <= 1) output("hurt ", false);
		else output("horny ", false);
		output("to fight on.\n\n", false);
		output("The shark girl does a little victory dance, swaying her hips to and fro before moving over to you. She quickly removes your [pc.gear], but her smile fades to a blank expression when she notices you lack any genitalia. \"<i>What the...</i>\" she mumbles, poking you in the groin. Finding you completely useless, she growls in frustration and stomps on your face in anger. The sudden pain makes you pass out.\n\n", false);
	...
	if(pc.hasVagina() && (!pc.hasCock() || rand(2) == 0)) {
		...
		output("The shark girl giggles and moves over to you, tugging at your [pc.gear]  impatiently. Her tail swishes around and smacks your " + pc.buttDescript() + ". \"<i>You're gonna make me very happy, you hear? Otherwise...</i>\" she opens her mouth wide and you see her fangs glinting menacingly in the light. You gulp hard and nod, bringing a smile from the shark girl.\n\n", false); 
		output("Wasting no time, she removes her skimpy swimwear and your own gear.  ", false);
		//[if herm]
		if(pc.isHerm()) output("Seeing your " + pc.cockDescript(0) + " puts a smile on the shark girl's face as she takes a firm grip on your erection. \"<i>Well, you're just full of surprises, aren't you? Maybe I'll give this bad boy a whirl sometime. For now though...</i>\"  ", false);
		output("Her gaze drifts over to your " + pc.vaginaDescript(0) + " and she licks her lips in delight. \"<i>Now that's what I'm looking for! Tell you what dear, you get me wet and I might just give you some pleasure too.</i>\"\n\n", false);
		
		output("She roughly grabs you by the hair and pulls your face into her drooling cunt, your tongue instinctively probing into her. \"<i>Mmm... don't you dare stop licking you dumb bitch, if you know what's good for you,</i>\" she orders. You reply by speeding up your tongue work. You're a little ashamed to admit it, but her dominant command makes you feel rather hot and bothered.\n\n", false);
		output("The shark girl eventually sighs happily and relaxes her grip on your hair, pulling your head away a few inches. \"<i>Not bad bitch, not bad. Now get on your back.</i>\" You obey your mistress's command and flop onto your back. A sense of joy fills you as she positions her crotch in front of your face and moves her own head between your legs. You quickly resume eating her out, and this time she joins in the feast. It's not too long before the two of you orgasm, spraying girl-cum onto each other's faces.\n\n", false);
		output("The shark girl stands to leave and winks at you before diving back into the water. You eventually pass out from the exertion.\n\n", false);
	...
	//Male:
		...
		output("You feel the shark girl's bare foot press against your chest and she roughly pushes you onto your back. \"<i>Oh man, I can't even remember the last time I had an actual man...</i>\" the shark girl says, pulling your pants down to your ankles. Seeing your stiff erection, your opponent smirks and wets her lips before taking your entire " + pc.cockDescript(0) + " into her mouth. The feeling is heavenly, her long tongue slithering around your shaft.\n\n", false);
		output("But before you can begin to really enjoy it, she pulls her head away, visible strands of saliva still linking her mouth and your " + pc.cockDescript(0) + ". The shark girl quickly maneuvers herself so that she's straddling your cock and presses herself down, the two of you gasping sharply from the sensation. \"<i>Hmm, good boy... You make me cum first, and I won't bite you. Deal?</i>\" You nod, though given that peculiar feelers inside her cunt are massaging your cock, you don't know how long you can really hold out.\n\n", false);
...
public function sharkGirlGetsDildoed():void {
	clearOutput();
	output("You grin from ear to ear, revealing enough teeth to make even the defeated shark-girl shiver.  Advancing upon the prone monster-woman, you ", false);
	if(pc.hasWeapon()) output("put away your [pc.mainWeapon] and ", false);
	output("draw out a glistening pink dildo from your pouches, as if it were a weapon.   She looks up at you, at once knowing your intent.  ", false);
	if(enemy.lust() > 99) output("Her legs spread invitingly wide and she tears off her bikini bottom in a lusty frenzy.  You hold the dildo over her and give it a tiny squeeze, wringing out a few drops of pink fluid from the special dildo.  They land on her exposed cunt and in seconds she's writhing underneath you, humping the air.", false);
	else output("She holds her legs together defiantly, but you pry them open, tear off her bikini bottom and expose her bald nether-lips.   You hold the dildo over her and squeeze, squirting a gush of pink fluid onto her exposed cunt.  The effect is immediate, and in moments she's humping the air and wet with her own fluids.", false);
	output("\n\n", false);
	
	output("'<i>What a wonderful toy</i>,' you muse as you kneel between your ", false);
	if(pc.cor() > 50) output("latest ", false);
	output("victim's legs, inhaling her scent.  It smells surprisingly salty, reminding you of the sea.   Sand-papery skin brushes against your ankle.  You don't even have to look down to recognize the feeling of her tail curling around your leg and thigh.  Before she can use it to her advantage, you gently part her outer folds with one hand and insert the artificial member with the other.  The shark-girl's tail immediately drops away as her body shivers and relaxes, barely managing a few twitches.\n\n", false);
	
	output("The sea-smell from her groin intensifies and you feel the dildo bloat up in your hand.  She's orgasmed already!\n\n", false);
	
	output("You slap her left ass-cheek hard, the rough texture of her skin giving the perfect *crack* of skin on skin as you scold her, \"<i>You're not supposed to cum so hard!  Now you're really going to get stretched, skank!</i>\"\n\n", false);
	
	output("The aquatic monster-woman whimpers, feeling herself getting stretched wider and wider by the growing toy.   Intent on punishing her for trying to rape you AND for cumming too soon, you grab the dildo in a double-handed grip and force it in to the hilt.   Her tail flops and thrashes as the dildo sinks in until you bottom out.  She flops about like a dying fish, though perhaps the only thing about her 'dying' is a few brain cells from the bliss her cunny's feeling.  Satisfied with the degree of penetration, you grab it around the base and yank back on it, struggling with how tightly it's lodged in her cunt.\n\n", false);
	
	output("You wiggle it back and forth, working it partway out before ramming it back in.  Moaning and rocking underneath you, the shark-girl's eyes cross as she tries to fuck your hands.  Both her hands climb up under the thin black fabric of her top and wrap around her modest breasts, squeezing and caressing them.  She gasps when her fingers find her nipples, and starts drooling down her chin.\n\n", false);
	output("Her salty pussy clamps down hard, but the slick juices pouring from her hole makes it easier and easier to work the bloated sex-toy in and out of her love-hole.   Tired of the slut's moans and gasps of pleasure, you decide to finish her off.   You pull the dildo out, and let out a gasp of surprise when her twitching cunt gapes open obscenely, allowing you to see the entire depth of her love-tunnel, all the way to her cervix.   She groans at the sudden loss of sensation, but before she has a chance to jam a finger into her needy hole, you squeeze the perverted toy HARD, wringing a thick gush of pinkish fluid directly into her baby-maker.\n\n", false);
	
	output("You plunge the jiggling sex-toy back into her stretched cunt, pistoning the fluid directly into her womb and setting off an orgiastic cry with enough volume to make you cringe.  She trembles madly and you have to sit down on her tail to keep the tip of it from cracking you upside your head.  The dildo in your hands puffs up wider and wider until it pops itself out, exposing the pink-coated inner flesh of the shark-woman's cunt.  You can watch the muscles rippling and squeezing, though thanks to her stretching, you don't think she'll be able to squeeze anything smaller than a minotaur.  After a few more seconds her body goes still, save for the occasional twitch, and you realize she's passed out.\n\n", false);
	
	output("Damn that was hot!  You'll need to sate yourself once you get back to camp for sure.  Maybe you should give this dildo a whirl?  It still smells of your victim.\n\n", false);
	
	pc.lust(20 + pc.libido() / 5 + pc.cor() / 10);
	CombatManager.genericVictory();
}

public function CoCSharkGirldefeated():void
{
	sharkWinChoices();
}

public function CoCSharkGirlwon():void
{
	sharkLossRape();
}