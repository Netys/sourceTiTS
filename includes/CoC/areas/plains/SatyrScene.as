import classes.Characters.CoC.CoCSatyr;
...
public function satyrEncounter(location:int = 0):void {
	clearOutput();
	//spriteSelect(98);
	if(rand(2) == 0 || pc.isPregnant() || !pc.hasGenitals()) {
		output("As you cross the ");
		if (location == 0) output("grassy plains");
		else output("sodden expanse of the swamp");
		output(", you hear lewd bellowings and drunken curses.  From out of the expanse of green comes a humanoid figure with a set of goat-like horns curling from his head.  Seeing you, he lets out a randy bleat and charges, naked erection jabbing before him!");
	...
	//Pregnant PCs can't get this encounter.
		...
		output("You wander through the ");
		if (location == 0) output("grassy plains");
		else output("sodden expanse of the swamp");
		output(" when you hear strange music emanating not far from where you are.  Do you investigate?");
		//[Yes][No]
		//if(location == 0) doYesNo(createCallBackFunction(consensualSatyrFuck,0), returnToCampUseOneHour);
		//else doYesNo(createCallBackFunction(consensualSatyrFuck,0), returnToCampUseOneHour);
		clearMenu();
		addButton(0, "Yes", consensualSatyrFuck);
		addButton(1, "No", function():*{ processTime(10 + rand(10)); mainGameMenu(); } );
	}
}


private function startSatyrCombat():void {
	CombatManager.newGroundCombat();
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(new CoCSatyr());
	CombatManager.victoryScene(defeatASatyr);
	CombatManager.lossScene(loseToSatyr);
...
private function consensualSatyrFuck(loc:int = 0):void {
	...
	//(+Lust)
	pc.imbibeAlcohol(50);
	applyDamage(new TypeCollection( { drug : 5 + 20 * (100 - pc.tolerance()) / 100 } ), pc, pc, "minimal");
	...
	addButton(14, "Leave", leavePartySatyr);
}
...
private function keepDrinking():void {
	clearOutput();
	//spriteSelect(98);
	output("You grin at the satyr's encouragement and continue drinking, setting on a slower pace so you won't spill any more; shortly you pass him the empty skin and ask for more.");
	...
	pc.imbibeAlcohol(50);
	applyDamage(new TypeCollection( { drug : 5 + 50 * (100 - pc.tolerance()) / 100 } ), pc, pc, "minimal");
	...
	output("\n\nYou can't resist the soft melody and begin to feel sleepy; you stop your drinking and eating to yawn... now that you've stopped, you feel awfully hot.  You strip off your [pc.gear] without a second thought, not even bothering to protect your modesty.  Once you feel the cool air against your " + pc.skinFurScales() + " you sigh and lay to relax for a nap.");
	...
	if (pc.hasVagina()) {
		...
		pc.loadInCunt(new CoCSatyr(), pc.findEmptyPregnancySlot(1));
	}
	else {
	...
	output("\n\nYou wake up you don't know how many hours later, head spinning in pain at the heavy drinking you did earlier.  Come to think of it... all the bottles and dishes of food seem to be gone.  The only thing left are puddles of warm cum all around you and on you.  You get up, dazed, only to feel a strange feeling inside your [pc.vagOrAss]; you double over as pain explodes from your [pc.vagOrAss] and gasp as a flood of semen dribbles from your used ");
	if(pc.hasVagina()) output("vagina");
	else output("ass");
	output(".  This must be the work of that satyr!  Mentally, you remind yourself to watch out for him next time.  You clean yourself up as best as you can and redress, then wobble your way towards your camp, trying to stifle the pain, in your head and elsewhere, along the way.");
	//(8 hours lost) (PC is pregnant (either vagina or ass) with a satyr, slimefeed)
	//satyrPreggo();
	
	processTime(3 * 60 + rand(120));
	clearMenu();
...
private function leavePartySatyr():void {
	clearOutput();
	//spriteSelect(98);
	output("You thank the satyr for his generosity, but you don't think you can handle this kind of booze, so you get up and start on your way back to your camp.");
...
private function trickZeSatyr():void {
	clearOutput();
	//spriteSelect(98);
	output("You come up with a plan and pretend to start drinking again; once you notice the satyr is distracted, you quickly spill most of your drink on the floor and return an empty skin to him.");
	...
	pc.credits += (10 + rand(10)) * 10;
	processTime(50 + rand(30));
	clearMenu();
	addButton(0, "Next", mainGameMenu);
}
//[=Skip Foreplay=]
private function skipForeplay():void {
	clearOutput();
	//spriteSelect(98);
	output("You smirk and crawl towards the satyr, discarding the skin of alcohol and knocking over several dishes and bottle in your way.  Once you're close enough, you roughly grab at his massive shaft and begin stroking it.  \"<i>We both know where this is headed...</i>\" you whisper, \"<i>so why not skip the foreplay?</i>\"");
	
	output("\n\nThe satyr looks surprised, then grins.  \"<i>Very well, if you insist...</i>\" he purrs, reaching out to grab and push you to the ground, tearing roughly at your [pc.gear] until you are naked.");
	
	//Play appropriate willing sex scene//
	clearMenu();
	addButton(0, "Next", willinglyBoneSatyr);
}

private function drinkAndSex():void {
	clearOutput();
	//spriteSelect(98);
	output("You grin at the satyr's encouragement and continue drinking, setting on a slower pace so you won't spill any more.  You tell him that you'd like to have sex with him.");
	
	output("\n\nThe satyr gladly takes the empty skin and puts it away.  He nods in agreement, giving you the signal to slowly strip your [pc.gear] off and present your ");
	if (pc.hasCock()) output("[pc.cock]");
	if (pc.hasCock() && pc.hasVagina()) output(", ");
	if (pc.hasVagina()) output("[pc.vagina]");
	if (pc.hasCock() || pc.hasVagina()) output(" and ");
	output("[pc.butt] to the satyr.  He looks up and down at you.  You waste no time caressing his cock and at the same time, he caresses your [pc.butt].");
	clearMenu();
	addButton(0, "Next", willinglyBoneSatyr);
}
...
internal function loseToSatyr():void {
	...
	else if(pc.isBiped()) output("spreads your [pc.legs]");
	else output("pins you down");
	output(" and reaches forward, probing for a vagina.  ");
	if(pc.hasVagina()) output("Upon finding one, he grins lustfully, his already impressively erect cock rising to full flag.");
	else output("Finding you lacking that trait, he grunts disdainfully and probes roughly for an asshole, which he locates right between your buttcheeks where it belongs, then jerks himself with several strokes to help coax his masculinity to full erection.");
	output("He grabs your [pc.ass] and roughly squeezes your cheeks, kneeling down while lifting you up so he can impale your [pc.vagOrAss].");
	...
	if((pc.hasVagina() && pc.vaginalCapacity() < enemy.cockVolume(0)) || (!pc.hasVagina() && pc.analCapacity() < enemy.cockVolume(0))) output(", not even bothered by the fact that his cock doesn't even fit inside you");
	output(".");
	//[cunt/buttchange]
	if (pc.hasVagina()) {
	...
	output("\n\nYou scream as your [pc.vagOrAss] clenches, strangling the satyr's cock in a vice-like grip.");
	if(pc.hasCock()) output("  [pc.EachCock] throbs as it send strings of cum flying straight into the satyr's chest, painting it white.");
	if(pc.hasVagina()) output("  A veritable flood of girlcum escapes the small space between your vaginal walls and the satyr's massive shaft, covering his balls and legs with your female fluids.");
	...
	output("; your belly begins to swell and bulge as he pumps liters of semen into you.");
	...
	if(pc.cor() < 50) output("shamefully ");
	else output("shamelessly ");
	output(" gather and re-dress in your garments and head back to camp, cum still dribbling from you as you go.\n\n");
...
internal function defeatASatyr():void {
	...
	addDisabledButton(0, "FuckHisButt", "Fuck His Butt", "This scene requires you to have fitting cock.");
...
private function femaleTakesAdvantageOfSatyr():void {
	clearOutput();
	//spriteSelect(98);
	output("You eye his massive shaft speculatively, then decide against it.  Why should he receive the true pleasures of your cunt when he attacked you so rudely?  No, if there will be anyone taking pleasure from this, it will be you alone.  With that in mind, you cast aside your [pc.gear] in the most imperious manner you can muster, until you are standing stark naked.");
	output("\n\nThe satyr bleats as his eyes widen in expectation, setting his gaze squarely on your [pc.vagina]; he starts panting as his massive cock grows even harder, hoping for the release that he was denied.");
	output("\n\nYou saunter forward and topple him over onto his back.  He lets out a noise like a goat that's been struck, even as you move to pin him to the ground, swivelling your hips until you are practically sitting on his face with your [pc.vagina] over his mouth.  You grind your crotch into his face and demand he lick you, taking hold of the copious hair around his privates and pulling to suggest what refusal may mean.");
	
	output("\n\nToo horny to fight back or protest, the satyr wastes no time in burying his face into you, licking and kissing your netherlips, nipping at your [pc.clit]");
	if(pc.hasCock()) {
	...
	output("\n\nYou quiver and gyrate as his tongue slurps and licks its lewd way into your most intimate of places, sending delicious waves of pleasure shuddering and rippling through you.  Deciding to 'encourage' him to work harder, you tighten the grip of your [pc.legs] and begin to stroke his shaft, dragging your clenched hands up and down in a rough handjob for the satyr's oversized male tool.");
	...
	if(pc.hasCock()) output(", your own cock belching its usual load across his stomach and the surroundings");
	output(".");
...
private function malesTakeAdvantageOfSatyrs():void {
	...
	output("\n\nThe moment your fingers slide inside the helpless satyr he bleats, not in horror, but in delight.  His insistent struggles to get away from you make a complete U-turn, and instead he struggles to get more of your hand inside his gripping hole; his cock throbs and grows even harder, spurting pre on the ground below, as his balls churn and slosh eager for the prostate massage you'll be giving him with your own [pc.cock " + x + "].  You can't help but laugh at the satyr's change of heart; what a slut.");
	...
	//Low Cum Amount
	if(pc.cumQ() < 250) output("  He tries to milk you for far more than you can give him, and at one point you feel like his anal muscles are going to tear your dick off, until finally they go lax and lets you slip out.");
	//(Medium Cum Amount)
	else if(pc.cumQ() < 1000) output("  You dump far more than you usually do; the satyr's clenching asshole makes sure to drain you of every little drop of cum you have until finally it goes lax and lets you slip out, leaving the goat-man with a pudgy belly.");
	//(High Cum Amount)
	else output("  You shoot jet after jet of cum, far faster than the satyr's ass can milk you for it, and still he doesn't seem to stop trying.  His rippling muscles draw your cum inside his bowels in big bubbles.  When his ass finally goes lax, you're propelled halfway out of him by the force of your own backflowing seed; the satyr's ass looks like a wreck, and his belly is overflowing with so much spunk you're surprised it didn't come out of his mouth.");
...
private function willinglyBoneSatyr():void {
	clearOutput();
	//spriteSelect(98);
	output("The satyr eyes you up and down hungrily; his hands move to grope your [pc.chest], gently tweaking your [pc.nipples], then he moves down towards your ");
	if(pc.hasCock()) output("hardening maleness, giving it a few strokes to bring it to full mast, then he moves on to your ");
	if(pc.balls > 0) output("[pc.balls], rolling each of your orbs around with reverence at the life-giving load they produce; finally he addresses your ");
	//((if PC has a pussy)
	if(pc.hasVagina()) output("[pc.vagina], spreading your labia wide to see the interior of your rapidly moistening walls.");
	else output("[pc.ass], spreading your cheeks apart to gaze at your winking rosebud.");

	output("\"<i>Wonderful!  With a [pc.vagOrAss] like this I'm sure you'll make a great " + pc.mf("father", "mother") + " for our children,</i>\" he says, leaning closer to sniff at your ");
	if(pc.hasVagina()) output("[pc.vagina], licking all over your netherlips and tasting your juices.");
	else output("[pc.butt], licking all around your hole to prepare it for what's coming.");
	...
	output("\n\nYou cry out as his massive manhood fills you, wrapping your [pc.legs] around your caprine lover and pulling yourself fiercely against him, imploring him to hold you tight and fuck you senseless.  The satyr begins trembling and bucking lightly against you, slapping his balls on your [pc.butt] softly with each small thrust.  Yet he resists letting his lust dominate him and smiles at you.  \"<i>I said I want you to remember me... and no matter how much I want to unload into you right now, this is exactly what I'm going to do.</i>\"");
	...
	if(!pc.hasVagina()) output("clenching butt");
	else output("spasming vagina");
	output(".  It's like someone shoved a hose into you and opened the nozzle, filling you with an endless stream of hot spooge; you can feel it pooling inside you, inflating your belly with his gigantic load.");
	...
	if(pc.hasVagina()) output("pussy gushes fluids down his massive manhood");
	if(pc.isHerm()) output(" and ");
	if(pc.hasCock()) output("cock spurts cream across your intertwined forms");
	output(".  Moaning softly and heaving in great lungfuls of breath, you sink back down onto him, feeling well and truly sated.");
	...
	output("\n\nThe satyr gets up on unsteady hooves, visibly shaken after this session with you.  \"<i>Well, sorry for not sticking around.  But I have places to get and people to fuck, so I guess we'll bump into each other some other time.</i>\"  The goat-man gives your [pc.butt] a friendly pat and begins packing away whatever leftovers he manages to find; then turns to blow you a kiss and dashes off.");
...
//private function satyrPreggo():void {
	//if (pc.hasVagina()) {
		//pc.knockUp(PregnancyStore.PREGNANCY_SATYR, PregnancyStore.INCUBATION_SATYR); //Satyrs can now fertilize eggs for ovipositing players
	//}
	//else {
		//pc.buttKnockUp(PregnancyStore.PREGNANCY_SATYR, PregnancyStore.INCUBATION_SATYR, 1, 1);
	//}
//}
