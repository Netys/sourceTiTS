import classes.Characters.CoC.CoCPhoenix;
...
public function encounterPhoenix():void {
	if (flags["COC.PHOENIX_ENCOUNTERED"] == undefined) encounterPhoenixFirstTime();
	else encounterPhoenixRepeat();
}

public function encounterPhoenixFirstTime():void {
	clearOutput();
	output("Strolling along the mountain path, you realise that you are quite close to the tower where you and Hel fought the Harpy Queen and her little army of hybrids. Pausing for a moment, you are relieved to hear a distinct lack of noise, which hopefully means that no-one's tried to move back into the stronghold after ");
	if (flags["COC.HARPY_QUEEN_EXECUTED"] == 1) output("her Highness became an ex-harpy.");
	else output("you evicted her Majesty from her seat of power.");
	output("\n\nA little smile crosses your features as you remember Hel's reaction to seeing her father again and you continue on your way, a slight bounce in your " + (pc.legCount > 1 ? "step": "slither") + ". However, less than half a minute later you stop in your tracks when a cascade of rocks suddenly starts rolling down a slope to your right. It's at this point that you realise that the mountain is hardly ever quite this quiet, there's almost always some harpy screeching in the distance or, more often than not, right next to you. Fearing an ambush, you keep your eyes fixed on the peak of the small ridge, gripping your [pc.mainWeapon] tightly as you slow to a snail's pace. Your head darts from side to side as you try to keep watch on every possible hiding place at once, wondering if your previous idea about the tower being abandoned was completely wrong. What if those harpies you drove out decided to re-occupy their old roost? Images of you suddenly being swarmed by a mass of furious, bitter bird-girls enter your head and your jaw clenches slightly, [pc.legs] starting to move a little faster as you try to distance yourself from the area as quickly as you can manage.");
...
private function fightPhoenix():void {
	CombatManager.newGroundCombat();
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(new CoCPhoenix());
	CombatManager.victoryScene(CoCPhoenixPCVictory);
	CombatManager.lossScene(CoCPhoenixPCLoss);
	CombatManager.displayLocation("PHOENIX");
...
//VICTORY!
public function CoCPhoenixPCVictory():void {
	flags["COC.PHOENIX_HP_LOSS_COUNTER"] = undefined; //Reset counter if you win.
	output("With one final grunt, the phoenix collapses against a nearby rock, barely able to support herself. The once-proud soldier has been reduced to a " + (enemy.lustQ() > 99 ? "dazed, lust-crazed slut, desperately pulling at her clothing in a mad attempt to expose herself": "a beaten, battered heap; completely unable to resist your advances") + ". ");
	...
	addDisabledButton(1, "Fuck Ass", "Fuck Ass", "This scene requires you to have fitting cock and sufficient arousal.");
	addDisabledButton(2, "Get Wanked", "Get Wanked", "This scene requires you to have cock and sufficient arousal.");
	addDisabledButton(3, "Ride Anal", "Ride Anal", "This scene requires you to have sufficient arousal.");
	addDisabledButton(4, "Ride Vaginal", "Ride Vaginal", "This scene requires you to have vagina and sufficient arousal.");
	
	if (pc.lust() >= 33) {
		if (pc.hasCock()) {
			if (pc.cockThatFits(enemy.vaginalCapacity()) >= 0) addButton(0, "Missionary", missionaryWithPhoenix);
			if (pc.cockThatFits(enemy.analCapacity()) >= 0) addButton(1, "Fuck Ass", fuckPhoenixsButt);
			addButton(2, "Get Wanked", getWanked);
		}
		addButton(3, "Ride Anal", rideAnal);
		if (pc.hasVagina()) addButton(4, "Ride Vaginal", rideVaginal);
	}
...
public function missionaryWithPhoenix():void {
	...
	output("You charge at the defenseless phoenix, quickly disarming her before throwing the defeated girl to the ground, leaving her moaning in the dirt on her back. She grunts, shaking her head vigourously to try and clear her mind, before looking up at you with grim, fierce eyes. ");
	output("\n\n\"<i>I’ll never submit! I am a noble phoenix, not some-</i>\" Completely ignoring her you yank her chain shirt up, revealing the heaving, plush D-cups beneath " + (enemy.lustQ() > 99 ? " nipples already standing at attention in the cool air": "") + ". The phoenix gasps at the sudden exposure and turns her head away, squeezing her eyes closed, determined to not look you in the eye as you take your pleasure.");
	output("\n\nKneeling before her you tear away her loincloth, forcing her legs apart to reveal her " + (enemy.lustQ() > 99 ? " slick pussy and half-rigid cock, clearly aroused despite her insistence on not enjoying herself": " tight slit and slowly engorging lizard-shaft, apparently her current condition is more enticing than she wants to admit") + ". You run a pair of fingers along her cleft, trailing it all the way up to the tip of her stirring member before circling around her tip and sliding back down. The phoenix whines pitifully as her prick reacts accordingly, twitching as it hardens at your teasing touch before you bring your fingers back to her slickening entrance and ease them between her soft lips. She groans, gritting her teeth as you go deeper and deeper inside her, wiggling against her walls whilst she soaks your hand with arousal.");
	output("\n\nHer neck cranes as she struggles to control herself; slow, heavy breathing accentuating her tits as they tremble gently with each mouthful of air. With a grin you whip your fingers out of her and, as she writhes at the sudden emptiness, force them into her gasping mouth. Eyes shooting wide open, she sputters and shakes her head, but you don’t let up until she’s had a good, long taste, wrapping her reptilian tongue around your dripping digits to make sure she gets a full sampling of her own flavour. The way her tongue constricts your sodden fingers floods your groin with warmth and you feel your [pc.cocksLight] starting to thicken as she slurps at her own spunk.");
	...
	clearMenu();
	addButton(0, "Next", missionaryWithPhoenixPart2);
}
...
public function missionaryWithPhoenixPart2():void {
	...
	output("\n\nShe cums first" + (y == -1 ? "": ", the rapid pace of your twin shafts too much for her to bear") + ", gripping your shoulders hard enough for her claws pierce your armour. He eyes squeeze shut as her body gives in, not wanting to see you as you claim the girl as yours. Despite herself, a deep groan rumbles in her throat as the pleasure overwhelms her, her chest heaving upwards as she tries to futilely fight one last time against your grip.");
	output("\n\nYou moan in kind as her spasming pussy " + (y == -1 ? "": "and quivering sphincter") + " tightens around your [pc.cock]" + (y == -1 ? "": "and [cock 2]") + ". Her hips start to buck as her member erupts, her legs pistoning her upwards to your body as hot jizz pumps between you both with every twitch of her sensitive shaft. ");
	output("\n\nWith her writhing form pressing against you so fully, her muscles squeezing down so hard, you can’t help but blow your load. You bite down on her hard little nip, moaning richly into her breast as your hips hammer the girl's pelvis. Thick, rich spunk surges through your cock" + (y == -1 ? "": "s") + ", [pc.cumVisc] ropes of warm seed shooting into her wet tunnel " + (y == -1 ? "": "and deliciously tight asshole") + ". It floods her body, even as her hips squirm in an attempt to escape the feeling of being defiled by you. Her voice explodes into a wailing moan as you continue to shoot inside her, staining her quivering walls [pc.cumColor], all the while holding her wrists firmly down as her entire body undulates against you; a mixture of orgasmic pleasure and a desperate need to escape your grip.");
	...
	output("\n\nEventually, your orgasm finished, leaving you panting atop the phoenix girl. Finally satisfied, you release the her wrists and heave yourself off her, glancing down to see that she’s passed out from the nigh-unending orgasm. Her leaking, cum-stuffed hole" + (y == -1 ? "": "s") + " still quiver" + (y == -1 ? "s": "") + " around your shaft" + (y == -1 ? "": "s") + ", her thick, purple lizard dick half-erect and spurting cum onto her chest in time with her breathing. Her tits glisten slightly in the sunlight as they rise and fall, sweat, saliva and cum clinging to the wobbling orbs. Grinning, you withdraw yourself from the girl, pulling your cock" + (y == -1 ? "": "s") + " out of her ravaged entrance" + (y == -1 ? "": "s") + ", triggering a rush of your semen as her widened snatch " + (y == -1 ? "": "and ass") + " lose the plugs that held the deluge back. You can still see her labia twitching as your seed drips down her cheeks, the [pc.cumVisc] [pc.cumNoun] swiftly pooling between her thighs.");
	output("\n\nYou wipe your cock" + (y == -1 ? "": "s") + " on some of the girl's less-stained feathers before tucking " + (y == -1 ? "it": "them") + " away, snatching a gempurse from her discarded armour before starting on your journey back to camp, whistling all the way.");
	output("\n\n");
...
public function fuckPhoenixsButt():void {
	...
	if (x < 0) x = 0;
	output("With your mind made up, you're definitely in control. You tell the phoenix that you're going to fuck her ass. You quickly yank her scimitar away so she's defenseless and loincloth off to expose her tailhole and firm butt-cheeks. You give her a hard shove on the grounds and step on her back like you're dominating. She gives you a glare and yells, \"<i>I'll never submit to you! I never will!</i>\"");
	output("\n\n");
	//if (pc.armorName == "goo armor") output("The goo opens up to reveal your " + pc.cockDescript(x) + ". ");
	//else 
	if(!pc.isCrotchExposed()) output("You take off the bottom half of your [pc.gear] to reveal your " + pc.cockDescript(x) + ". ");
	output("You grab her by her rump and lift her tail, caress her butt-cheeks and tell her to get ready for the ordeal ahead.");
	output("\n\nYou push your " + pc.cockDescript(x) + " into her rear entrance, inch by inch until " + (pc.cocks[x].cLength() <= 12 ? "you hilt into her": "you can push in no more") + ".");
	pc.cockChange();
	output("  You begin to thrust back and forth with abandon as each thrust brings pleasure to the phoenix.  Precum leaks from her reptilian cock and femjuice drips from her vagina. She gives you a glare and yells, \"<i>I still w— yes, fuck my ass harder!</i>\"");
	...
	output("\n\n");
	//if (pc.armorName == "goo armor") output("The goo closes up around your nether regions.  ");
	//else 
	if(!pc.isCrotchExposed()) output("You take the time to re-dress your lower half.  ");
	output("Satisfied, you make your way back to your camp.");
	output("\n\n");
	processTime(15 + rand(5));
	pc.orgasm();
	CombatManager.genericVictory();
...
public function rideVaginal():void {
	clearOutput();
	output("You grab the defenseless phoenix and throw the hermaphrodite on her back. She grunts, looking up at you with grim, fierce eyes. \"<i>I’ll never submit! I am a proud warrior, not some--</i>\" Yeah, whatever. You rip her chain shirt open, revealing the large, soft globes of her D-cups beneath. The phoenix gasps at the sudden exposure and turns her head away, determined not to look you in the eye as you take your pleasure. You ignore her temporary defiance and grab her cock.");
	output("\n\n\"<i>Heeeey,</i>\" the phoenix whines squirming to get out of your grip.");
	output("\n\nYou maintain your grasp on her long, purple lizard dick and tell her that you’re doing her a favor: you’re going to let her fuck your " + pc.vaginaDescript() + ". She stops struggling at the invitation.");
	...
	if (!pc.isPregnant() && pc.pregnancyIncubationBonusMother() >= 2) output("You assure her she will be soon");
	...
	pc.cuntChange(0, enemy.cockVolume(), true, true, true);
...
public function rideAnal():void {
	clearOutput();
	output("You grab the defenseless phoenix and throw the hermaphrodite on her back. She grunts, looking up at you with grim, fierce eyes. \"<i>I’ll never submit! I am a proud warrior, not some--</i>\" Yeah, whatever. You rip her chain shirt open, revealing the large, soft globes of her D-cups beneath. The phoenix gasps at the sudden exposure and turns her head away, determined not to look you in the eye as you take your pleasure. Gripping the warrior by her hefty boobs, you tell the phoenix that it’s her lucky goddamn day: you’re going to let her fuck your ass.");
	...
	pc.buttChange(enemy.cockVolume(), true, true, true);
	output("\n\nYou grunt as she bottoms out inside you, leaving you with a feeling of intense fullness and warmth. You grin down at the phoenix girl, pleased to see the look of rapture on her face as your ass muscles squeeze down on her stiff lizard-cock. You feel her hands digging into your [pc.hips], and in return you give her soft breasts a playful squeeze. You start to rock your hips, letting an inch or two of her dick spill out of you before your stretched [pc.asshole] sucks it back up.");
...
//LOSS! GET RAPED!
public function CoCPhoenixPCLoss():void {
	clearOutput();
	if (pc.HP() <= 1) {
		if (IncrementFlag("COC.PHOENIX_HP_LOSS_COUNTER") >= 4) {
			phoenixBadEnd();
			return;
		}
	}
	output("You collapse to the rocky ground, far too " + (pc.lustQ() > 99 ? "turned on": "drained") + " to continue the fight. The phoenix smirks as you drop, carefully shedding her tattered clothing to expose her lush hips and breasts before making her way over to you, swaying sensuously with each step. ");
	...
	if (pc.hasCock() && rand(2) == 0) {
		addButton(0, "Next", cockwielderLossScenePart1);
	}
	else {
		addButton(0, "Next", cockwielderLossScenePart1);
	}
...
public function cockwielderLossScenePart2():void {
	...
	pc.buttChange(100, true, true, true);
	output("The tip of her flexing limb flicks against your prostate and you whine, unable to move your lower body to escape the sensation, your [pc.cock] too tightly held to let you release.");
	...
	output("\n\nHowever, just before her lips fully remove themselves from your flagging cock, the mass of semen inside her starting to dribble out and down your already soaked shaft, she suddenly drives herself back down you and re-reams your ass with her still-burning tail. You gasp as your [pc.cock] jumps back to attention inside her, the sheer amount of semen sloshing around meaning her tight walls find almost no purchase on you.");
...
public function getRapedInTheAss():void {
	clearOutput();
	output("The phoenix shoves you down on the ground. You struggle to get up but the girl just pins you down. \"<i>I'm going to ravage your ass,</i>\" she says with a smirk.");
	output("\n\nShe slowly pulls off the bottom half of your [pc.gear], surveying your [pc.butt]. She commands you to get into doggy position. You have no choice but to obey. You get on all fours. You can only prepare for whatever's coming.");
	...
	if (pc.analVirgin) output("\"<i>I see you've got virgin ass. Not anymore once I start!</i>\" She gives you a smug grin.");
	output("\n\n\"<i>Get ready! I'm going to pound you so <b>HARD</b>!</i>\" She announces. She grabs your [pc.butt] and moves her hips back. Suddenly, she slams her entire cock length into your [pc.asshole], causing you to yelp in pain.");
	pc.buttChange(enemy.cockVolume(), true, true, true);
	output("\n\n\"<i>After all this... You really deserve this. Now enjoy it,</i>\" she says. She hammers repeatedly into your [pc.asshole], stretching it.");
	output("\n\nShe gives your [pc.butt] a hard spank. <b>SPANK!</b> She spanks your butt-cheeks again. <b>SPANK!</b> The repeatedly spankings cause your backside to turn red and sore.");
	//if (flags[kFLAGS.WATERSPORTS_ENABLED] >= 1 && rand(2) == 0) {
		//output("\n\nShe stops and pulls out. \"<i>I'll give you something you'll never forget,</i>\" she says.  She aims her cock towards you and... By Marae! She's peeing all over you! Her urine feels warmer than usual; this could be attributed to the fiery nature of the phoenix. Eventually, she does stop peeing and gets back to fucking your [pc.asshole].");
	//}
	...
	if (pc.hasCock()) output(" shoot ropes of [pc.cum] from your [pc.multiCocks]");
	if (pc.hasCock() && pc.hasVagina()) output(" while you also");
	if (pc.hasVagina()) {
		if (pc.wetness() < 4) output(" leak [pc.girlCum]");
		else output(" squirt [pc.girlCum] everywhere");
	}
	if (!pc.hasGenitals()) output("r body spasms with pleasure");
	output(". \"<i>I hope you've learned your lesson,</i>\" she says as she pulls her cock out of your leaking backdoor. She wipes her cock clean on your [pc.butt], before she moves her loincloth back in place. \"<i>See you around!</i>\" The phoenix says with a wink as she walks off.");
	//dynStats("str", -1);
	output("\n\n");
	processTime(15 + rand(5));
	pc.orgasm();
...
public function phoenixBadEnd():void {
	clearOutput();
	output("\"<i>Seriously? I've beaten you several times in a row? I guess... I've made a final decision; you must die. I'm sorry but I have to,</i>\" the phoenix says.");
