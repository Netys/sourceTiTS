...
public function basiliskGreeting():void {
	...
	CombatManager.setHostileCharacters(new CoCBasilisk());
	CombatManager.victoryScene(defeatBasilisk);
	CombatManager.lossScene(loseToBasilisk);
	CombatManager.displayLocation("BASILISK");
	
	if(IncrementFlag("COC.MET_BASILISK") == 1) {
		output("You are carefully trailblazing up a steep pass in the jagged mountain peaks when a furious screech from high above you makes you start.\n\n");

		output("\"<i>Thiieeeeeeeeef, filthy thieeeeeeeeef!</i>\"  A harpy flaps into view over the ridge, her face a picture of rage.  You hastily assume a fighting stance- but the harpy completely ignores you, her gaze sliding over you and onto the rock wall behind you.  \"<i>You can hide but you can't run, sneak thiieeeeeeeef!  When I find you, I'm going to tear your naaaaaaaaasty little eyes out!</i>\"  She flies on, screaming with anger, her head moving back and forth in jerky movements, scrying the landscape. You stare blankly in bafflement at this display, but are glad that a harpy's attention is on someone else for a change. Marveling at your fortune, you decide to press on.\n\n");

		output("At the top of the pass you take a moment to catch your breath, and notice something white lying on the ground, sheltered by a rock formation.  Is that a harpy egg?  What is it doing away from a nest?  Something on the rocks catches your eye: a tiny pool of grey, split by a vertical black slit.  You look closer at it to try to make sense of it.  It's");
		
		if (CoCBasilisk.canLookIntoEyes()) {
			output("... Some lizard-like creature's eyes! The basilisk, seeing that his camouflage isn't working, slides from his hiding place and advances upon you, his deadly eyes and sharp claws glinting coldly in the sunlight.\n\n");
		} else {
			output(" part of something bigger... but that's not important.  What is important is that you keep staring into that depthless pool of grey.  If you keep looking into it, you are quite sure you will be able to see forever; like two mirrors held up against each other, it's just a case of looking hard enough.  Is there even anything outside of the grey pool?  You don't think there is, and why would you even want to find out?  Everything to be seen and known is there, in those bottomless depths...\n\n");
			
			output("Using every vestige of your willpower, you tear your gaze away from the terrible, paralyzing sight.  Panting and feeling groggy, you desperately hold the rock formation in the corner of your eye. A tall, thin bipedal shape disengages from the stone against which he had been camouflaging himself, and stalks predatorily towards you.  With small, quick glances you glean fleeting impressions of grey-green scales, a tightly muscled yellow underbelly, cruelly curved index claws, a whip like tail. The creature moves its snub head towards yours suddenly, trying to catch your gaze with his deadly grey eyes again.  You recoil and ready yourself to fight it as best you can.\n\n");
			//(spd loss)
			CoCBasilisk.basiliskSpeed(pc, 5);
		}
	}
	//Standard encounter:
	else {
		output("You notice a large boulder ahead.  There is something curiously shaped about it. A small, wet grey shape on it catches your eye...\n\n");

		output("You look away in the nick of time,");
...
public function defeatBasilisk():void {
	...
	clearMenu();
	//Player HP victory: 
	if(enemy.HP() <= 1) output("Unable to stand anymore, the basilisk shakily sinks down on one knee, drops its head and looks at the ground, evidently demonstrating submission.\n\n");
	//Player Lust victory: 
	else output("No longer able to control its raging erection, the basilisk closes its eyes and sinks to one knee.  It would probably be attempting to signal its submission to you if it weren't furiously masturbating its long, purple cock, which has emerged straining from the creature's genital slit.\n\n");
	
	addButton(14, "Leave", function():*{ processTime(15 + rand(10)); CombatManager.genericVictory(); } );
	
	addDisabledButton(0, "Tongue", "Tongue",  "This scene requires you to have genitals and high arousal.");
	addDisabledButton(1, "Ass", "Ass",  "This scene requires you to have fitting cock, high corruption and high arousal.");
	

	//If victory and Player Lust above 30: 
	if(pc.lust() >= 33 && pc.hasGenitals()) {
		output("  Certain that the creature won't dare try and turn its eyes on you again, you take your time to look the tall reptile over directly for the first time.  Perhaps you could use it to satisfy your baser urges. If so, what part of it do you choose?\n\n");
		//[Tongue][pc.ass]
		addButton(0, "Tongue", tongueBasiliskSmex);
		if (pc.cockThatFits(enemy.analCapacity()) >= 0 && (pc.cor() >= 66)) addButton(1, "Ass", defeatBasiliskAndAnal);
		
		//if(pc.canOvipositSpider()) eggs = driderPCEggLaysBasilisk;
		//if(pc.canOvipositBee() && pc.gender > 0) eggs = layBeeEggsInABasilisk;
	}
}
...
private function tongueBasiliskSmex():void {
	//spriteSelect(75);
	clearOutput();
	output("You tap your jaw as you stare down at the defeated reptile, smiling at its oddly formal gesture of surrender.  You'd very much like to take some satisfaction from the basilisk, extract a bit of pleasurable payment for what it tried to do to you: but the more you get to look at it properly, the more difficult doing that seems. The tall, thin creature is all angles, tough scales and pointed edges. You don't like the thought of putting your genitals anywhere near its sickle claws or curved fangs.\n\n");

	output("It is as you are taking in this latter article that the basilisk swallows nervously, and you get a glimpse inside its mouth at a long, sticky, pale purple tongue, another gift of its reptilian ancestors. It's the only part you've seen of it so far which looks... soft.  Pliable.  Your smile widens and you step forward, slowly loosening your [pc.gear].\n\n");
...
private function basiliskHasVagEggStuff():void {
	//spriteSelect(75);
	//pc.slimeFeed();
	output("The basilisk is breathing heavily as it tears your [pc.gear] from your body, its warm exhalations rolling over your naked flesh.  It seems to be having difficulty controlling itself; from your frozen gaze you can see it constantly shifting its dreadful slit eyes back to your frame as it searches through your pockets with claws that tremble.  Eventually it throws down your attire and stares back into your eyes.  There is something else in there now; a pulsing lust, hints of red at the edges of that great, grey sea, a rapacious tide gathering.  You wish you could look away but there is more chance of you moving mountains.");
	//(Heat: 
	if(pc.hasStatusEffect("Heat")) output("  You are more aware than ever of an invisible scent simmering off you, of your wet vagina clenching and wetting itself in anticipation, your body begging this male creature to fulfil its genetic objective upon you.  Your eyes have betrayed you, your body is betraying you, and whatever else you are is a tiny, ignored voice screaming in between.");
	output("  The basilisk suddenly breaks away and kneels down in front of you.  Out of sight of your petrified eyes you cannot see what it is doing; however a moment later, you can feel, as a warm, sticky sensation slavers over your abdomen.");
	//(egg preg:
	if(pc.hasPregnancyOfType("CoCOviElixEggs")) output("  The basilisk licks your bulging belly hungrily, pushing against and testing for the eggs you are carrying.  Your sensitive cargo shifts around under its hungry attention; you'd squirm, but that is, of course, impossible.");
	//(heat or perk:
	if(pc.hasStatusEffect("Heat") || pc.hasPerk("Oviposition") || pc.hasPerk("Basilisk Womb")) output("  The basilisk licks your belly hungrily, its sticky tongue crawling like a warm tentacle across your sensitive underside.  You'd squirm, but that is impossible.  The creature is making you feel everything it is forcing upon you.");
	output("\n\n");
	
	output("The basilisk gets up and again stares back into your eyes and you feel its will press indomitably against your pliable mind.  At the very edge of your vision, underneath the creature you can see a hint of shiny purple; the creature's cock has slid out of its genital slit.  How long it is and what it looks like you cannot see, but as with another raspy moan you finally accept what is about to happen, there is no doubt you are going to get to know about it very personally.\n\n");
	
	output("The creature suddenly raps something out in its strange, dry tongue, and you feel something - something deep and red - flinch in your mind.  Suddenly, you are wet, wetter than you've ever been, your pussy slavering so badly you can hear the pitter-patter of your juices hitting the ground beneath you.  The basilisk says something else, more softly this time as it slides in close, its long claws reaching around to clutch your [pc.ass].  With a kind of horror you feel your limbs move involuntarily, your arms reaching around the thing's thin, muscled back, your bottom half slackening until you are supported entirely by the wiry strength of the lizard.  You expose your crotch in complete submission as the basilisk walks forward, and you feel the head of its cock teasingly touch your dripping, treasonous cunt.  As soon as your limbs are where it wants them to be, they lock in position again; you heave at them desperately, but once again your ability to do anything with your own body is simply not there.  You are less of a statue and more of a fuck toy, an extension of the basilisk's lust-maddened will.  Your jailer keeps walking until you feel the rough surface of a boulder against your back, and using this support the basilisk pushes itself straight into your [pc.vagina].");
	pc.cuntChange(0,enemy.cockVolume(0),true,true,false);
	output("\n\n");
	
	output("Once it has you pinioned, the uncontrollable rut your body has instilled in the basilisk really takes hold and it begins thrusting against you with abandon, its long, thin reptilian cock sliding in and out of your eager, slavering cunt, its hot breath pushing against your face and shoulders.  ");
	//<(Tight: 
	if(pc.looseness() < 4) output("Though it is not girthy, it is a perfect fit for your tight hole, and its long length coupled with your drawn up position has you panting as it touches your deepest, most sensitive depths.");
	//(Loose: 
	else output("At first the sensation is not great, its thin penis quickly lost in your vast, accommodating twat, but then, in between breaths, the basilisk barks out more harsh words. Once again you feel that helpless, red flex in your mind, and suddenly your vagina tightens around it, beginning to eagerly milk the reptile.  You desperately wish it didn't and at the same time are hopelessly glad it did; the sensation of your walls pushing and pulling the long, smooth prick in loving synchronisation is unbearably pleasurable.");
	output("  The rock wall the basilisk is fucking you against grates your back and ass and, lost in its daze the creature's claws dig into your flesh, but these discomforts only serve to heighten the pleasure the creature is forcing you to feel.  Its hard stomach beats a steady rhythm against your own as you slather its thighs and crotch with girl cum with the first of many involuntary orgasms, your [pc.clit] twitching eagerly for more.\n\n");

	output("The basilisk's rut means it cannot last as long as your hormones crave; although it must have been having its way with you for at least half an hour, it seems all too soon to your supine body when the creature tenses against you, throws its snub head back and with a harsh, dry call begins to fuck you for all it is worth, slamming your paralyzed frame again and again into the rock wall until you feel bruised and dizzy, before it tenses and pours itself into you.  You feel warmth spread through you as basilisk jizz floods through your cervix and womb, and you reaching a final bone-tingling plateau of pleasure, made all the more intense by the fact you cannot writhe or cry out; everything is locked up inside of you.\n\n"); 

	output("The basilisk collects itself against you while its penis drools its last, sliding its abdomen up and down your body, apparently enjoying the feeling of your soft skin against its own leathery hide- or maybe just enjoying the fact you can't do anything about it.  Eventually however it withdraws from your womanhood, trailing your mingled fluids as it goes.  It steps languidly back, looks into your eyes with dopey, post-coital satisfaction, and then licks your face with long, tender strokes with its sticky tongue. Again, whether it does this through affection or simply because it can, you can't fathom; you suppose, as you mutely accept the wet, ticklish saliva being lavished all over your face, it doesn't make much difference from where you're standing.\n\n"); 

	output("Finally, it whispers something dryly.  You were very much hoping that it would let you go once it had taken its pleasure, but that is evidently not the case.  It makes you stand still, as you were before; you feel its seed trickle down your leg as your body is guided to attention.  It hisses one final afterthought to you, and then leaves, a certain swagger in its stalking gait.  Once again, you are petrified, naked, utterly helpless. A backwash of erotic images from your past involuntarily rise up and assault your senses...\n\n");

	output("After about an hour of being forced to stand still and savor your own shameful memories, you find with great relief you can begin to move your toe again.  Hard part's over, now.  Eventually with some effort you manage to work power into each corner of your body and finally shake free of the basilisk's curse; quickly, you rub the remnants of sticky saliva off your face and redress before anything else finds you, before groggily picking your way back to camp.  The cum still oozing from your quim and the occasional twinging memory mean that you aren't going to be able to shake free of the experience as easily as you'd like.");
	//(preg check, or change preg to basilisk if egg)
	//pc.knockUp(PregnancyStore.PREGNANCY_BASILISK, PregnancyStore.INCUBATION_BASILISK);
	pc.loadInCunt(enemy);
	//Egg change - 100% chance
	//if (pc.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS) {
		...
		//pc.knockUpForce(PregnancyStore.PREGNANCY_BASILISK, PregnancyStore.INCUBATION_BASILISK - 150); //Convert Ovi Elixir eggs to Basilisk eggs
	//}
...
private function basiliskAdvantageHarpy():void {
	//spriteSelect(75);
	output("Time stretches by at an agonizingly slow pace as you stand there, a bizarre, motionless flesh statue.  You have no way of measuring how much time is passing; the sun is not in your direct line of vision.   You try to move any and every part of yourself in turn, but it is hopeless.  Your body is a cage, and you begin to hate the basilisk less because it paralyzed you and more because it left your mind entirely aware of it.  Every so often another unbidden backwash of erotic memories overwhelms your senses, keeping you helplessly aroused and reminded of who did this to you.  Coupled with the unscratchable itches and the aching in your limbs the experience is one of sensational hell.\n\n");
	output("You hear a shrill cry from above you, half eagle scream and half mocking, female laughter.  With a fluttering flap of feathers, a harpy lands at your side before proceeding to stalk around you, taking in your helpless, frozen form with stiff, jerky movements.  You reflect bitterly that if the big bottomed bird woman had turned up fifteen minutes ago she would probably have scared the basilisk off.  As it is, you are going to have to take whatever she can throw at you... with a stiff upper lip, as it were.\n\n");
		
	//Male/Herm: 
	if(pc.hasCock()) {
		output("The harpy's eyes zero in on your erect cock greedily.  Stepping back from you she raises her head and lets out a screech which echoes around the mountains; as answering calls roll back to her she closes in, threads her arms around your neck and scalp and kisses you roughly. She pushes her golden lips against yours and squeezes her rough bird tongue into your mouth.  You feel your lips tingle and you raggedly moan against the savage frenching, her hot breath pushing down your throat as she circles your still tongue with her own, before exploring further down towards your tonsils.  By the time she has finished with you your whole body feels like it is glowing red from the effect of her lipstick, [pc.eachCock] straining.\n\n");

		output("You wonder vaguely how she and her no-doubt-soon-to-arrive sisters are going to take advantage of you as you are; you find out a moment later when with no preamble whatsoever the harpy shoves you roughly in the chest.  ");
		if(pc.isBiped()) output("You teeter horribly on your frozen heels for a moment and then fall onto your back like a collapsing statue.  ");
		else output("You feel yourself slowly and horribly lose your balance before flopping onto your side like a collapsing statue.  ");
		output("Unable to brace yourself, you bang your head painfully; as you are lying there dazed, you feel something build at the back of your mind.  Involuntary sensations prickle your skin and groin as, once again, the intermittent mental backwash that the basilisk's hypnosis has forced upon your mind hits you.  Memory after memory of sexually-charged encounters, daydreams or fantasies crowd your consciousness.  It is made worse, much worse by the pheromones the harpy has pushed into your mouth; your body rides the chemical glow at the same time as imaginings of soft skin, tight muscle and musk overload your senses.  You groan raggedly as [pc.eachCock] bulges and leaks pre-cum, almost screaming for attention.  When you finally, woozily come to your senses, you find that all vision has been blotted out by a big, wobbly harpy bum, her moistening lips rubbing impatiently against your mouth.\n\n");

		output("The bang on the head you took, the harpy lipsticks trilling in your bloodstream and the involuntary tide of erotic memories which ebbs and flows over you mean the next couple of hours goes by for you in a haze of forceful sex.  You can't control your cock");
		if(pc.cockTotal() > 1) output("s");
		output(", your sex drive or what you are thinking about; you are a prisoner of lust and you quickly subside under the sexual concussion, hoping eventually the harpies and your own body will stop fucking you.  It does sink in around your second involuntary orgasm, your aching [pc.cock] spurting ribbon after ribbon of jizz into a harpy's clenching warmth, that the two arriving harpies are not happy with the first one; with your mouth and hands frozen you can't pleasure them with anything but your manhood, which leaves two of them fighting each other for time with your groin whilst a third grinds frustratedly at your face, mainly using your nose to rub at her inner walls and clit, forcing her juices down your nostrils and making you cough raggedly, struggling to breathe.\n\n");

		//Single cock: 
		if(pc.cockTotal() == 1) output("Their inability to properly satisfy themselves on your frame raises their tempers and frustration to the point where the three of them are physically fighting each other, treading all over you as they scream, bite and tear, their feathers flying all over the place.  Eventually the biggest of the trio drives the other two off, flapping and screeching into the mountains, before throwing herself onto her scratched and battered prize, her eyes lit up and wild.  She fucks you with a gusto born of bloodlust, her petit breasts bouncing up and down as she slams her powerful thighs into your [pc.hips], picking up the pace.  She quickly forces you to another achingly pleasurable peak, your [pc.cock] spurting more of your seed deep into her, and then just keeps on working you. Your cock seems incapable of going soft.  You feel the involuntary erotic backwash build in your skull again...\n\n");
		//Multicock: 
		else {
			output("The three of them do eventually work out a compromise however, once they discover you have more than one manpole to your name.  One pulls your [pc.cock] forwards painfully until it is almost pointing towards your face before squatting over it, whilst another clutches at your " + pc.cockDescript(1) + " as she spreads her legs and works her way inwards until she is spearing herself on you. You can't see them - you can't see anything except pink wobbly flesh and lavender feathers - but you can hear them shift impatiently around each other as they find a position which is comfortable to them both, their inner walls rubbing you from every direction as they move.  The third continues to thrust her needy sex into your face as best she can", false);
			if(pc.cockTotal() >= 3) output(", crowded out by the bodies of the others despite her transparent desire for the unoccupied man meat flopping against them,");
			output(" as the other two begin to push and pull your cocks into them, their overcharged libidos taking hold.  Your first cock feels like it is being pulled off your body at the same time as its end is being pumped; the sensation of being doubly and brutally fucked like this is unbearably pleasurable and with a series of ragged gasps you tumble helplessly over a sweat-beading peak, your two cocks spurting jizz deep into the two harpies.  The third harpy coos as she feels your hot breath on her gaping vagina, and she begins to buck her frustrated sex against your face faster.  Trapped in the harpies' clenching holes and incapable of going soft, your cocks continue to get worked as if nothing happened.  You feel the involuntary erotic backwash build in your skull again and with a lost moan you fall comatose, incapable of even computing how fucked you are anymore.\n\n");
		}

		output("Eventually you notice that your dick is no longer trapped in sucking wet, and coming out of your daze you find yourself without harpy companions, leaving your petrified form in a mingled pool of harpy juices and your own jizz.  After another ten or twenty minutes of being forced to lie there and marinate in your own shameful memories, you find with great relief you can begin to move your fingers again.  Eventually with some effort you manage to work power into each corner of your body and finally shake free of the basilisk's curse; quickly, you pick yourself up and redress before anything else finds you and woozily begin to make your way back down the mountain.  The whole experience feels like it may have been a lucid sex nightmare to your sluggish mind and you could almost believe it- if you didn't reek of sticky harpy sex and your own musk.");
	...
	//Genderless: 
		...
		output("You feel something build at the back of your mind.  Involuntary sensations prickle your skin as, once again, the intermittent mental backwash that the basilisk's hypnosis has forced upon your mind hits you.  Memory after memory of sexually-charged encounters, daydreams or fantasies crowd your consciousness.  It is made worse, much worse by the pheromones the harpy has pushed into your mouth; your body rides the chemical glow at the same time as imaginings of soft skin, tight muscle and musk overload your senses.  The denial the petrification has forced upon you is becoming unbearable.\n\n");
		...
		output("The harpy manages to cum twice on your hand, gobbling with excitement as she spatters your arm with her juices.  During this time you are forced to ride the potent cocktail of hypnotic sexual compulsion and the harpy's golden lipstick again and again, until you feel you would have collapsed in a pool of steaming sex long ago if your knees would only allow it.  Once she is finished with you the harpy clambers down, taking care to wipe her leaking twat on your naked front as she does so, before flapping off with a winsome smirk, entirely ignoring your own plight.\n\n");
...
private function basiliskAdvantageGoblin():void {
	...
	//Unsexed: 
		...
		output("\"<i>A " + pc.race() + "!</i>\" she yelps.  Hesitantly at first, then with increasing confidence as how vulnerable you are sinks in, she does a round circuit of you, taking in every angle of your frozen, helpless flesh.\n\n");

		output("\"<i>Get caught with our pants down by a basilisk, did we?</i>\" the goblin purrs.  \"<i>Well, not to worry. I'll take good c- hey, what gives?!</i>\" From behind you, you feel her hands thread their way around your " + pc.legs() + " before feeling and slapping all around your featureless groin.  \"<i>This is fucking bullshit!</i>\" she howls.  \"<i>I get a toy that can't stop me all to my lonesome, and it's some colossal jerkoff who thinks it's funny to have no sex!</i>\"  She stomps around to your front and glares at you, simmering with rage.  \"<i>I bet you think you're a real smartass, you bastard... you bitch... you... whatever!  Ooh, I'll fix you!</i>\"  With a look of complete disgust she storms off.  Having faced down this bizarre rant, you allow yourself to feel a tiny bit of relief.  You were worried for a moment there she'd go for your ass... a short distance away, you hear the goblin calling to someone.\n\n");

		output("\"<i>Hey you!  Yeah you, shit-for-brains!  Wanna free assfuck?  Come and get it then, you ugly, dumb muscle-bound moron!</i>\"  A moment later you feel a slap on your calf as the goblin runs past you, giggling.  \"<i>Have fun, smartass!</i>\"  A huge, angry bellow vibrates the air around you and the earth shakes as something big approaches you from behind.  You strain with every sinew of your being to escape, but you are, as ever, glued in place.  You manage a raspy moan as the enraged minotaur grasps you roughly by both arms, his animal musk filling your nostrils. <i>Fucking goblins.</i>\n\n");

		output("The huge bull-man is not one to look a gift fuck in the mouth.  Without bothering to take you in, bar a long wet sniff of your [pc.hair], he sticks his cock between your ass cheeks.  He grunts as he forces his head past your sphincter, squirting pre-cum into your passage as he does so.  Your body is incapable of clenching instinctively against the invasion, but there is no escaping how huge the dong feels as he impatiently forces your ass open.");
		pc.buttChange(60,true,true,false);
		if(pc.analCapacity() < 60) output("  The giant cock stretches you out painfully, and everything else blots out as your body attempts to accommodate the beast.  As he begins to thrust more of his length up you, he grunts and beads more of his drugged pre-cum, lubricating your anus.  This thankfully makes his cock easier to take, but also increases the pace of his thrusting as your hole becomes more receptive to it.");
		//V loose/Buttslut: 
		else output("  Your well-worn ass is a perfect fit for the giant cock and accepts it eagerly, every bit as welcoming as a moist vagina.  As the minotaur rubs against your tender inner walls he grunts and beads more of his drugged pre-cum, turning your hole into a helplessly wet, clenching ass cunt.");
		output("  You begin to pant from exertion and the overpowering sensation of the fuck as you feel first one ring and then a second push past your sphincter, then out again, then in and out as the minotaur picks up the pace.  As rut takes hold of him, he picks you up by your arms and uses his strength to force you up and down his dick, using you as a cocksleeve to sate his animalistic desires.  Your ass is slick with his pre-cum by now however, and the drugged slime has bumped you upwards into a hazy high; only the height of his downward thrust when his dick is almost completely buried in your bowels brings you out of it, the painful intensity of it dragging you down of your cloud with a gasp.\n\n");

		output("Eventually the minotaur's balls swell against your [pc.ass] and with a long, satisfied moo, he reaches his peak, holding you down so his cum jets forth deep inside you.  The sensual high it has already instilled in you amplifies by tenfold as the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your anus helplessly milking the creature for all it can get.\n\n");

		output("The minotaur holds onto you until he has finished spurting his last into you, before abruptly setting you back down on your frozen feet, pulling his slimy, receding member out of your abused anus and with a satisfied snort, takes his leave.  You feel his cum drooling out of you and down your [pc.hips], but in your hazy, druggy state the feeling is almost sensual.");
		//(Addict: 
		//if(flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] > 0 || pc.findPerk(PerkLib.MinotaurCumAddict) >= 0) output("  You're incredibly frustrated that you can't clench yourself and hold the magical substance deep inside you, so you can savor the wonderful, soft elation it blossoms inside of you for as long as you can.", false);
		output("\n\n");
		
		output("Eventually, after another thirty or so minutes of being forced to stand there and savor the cum trickling down your legs, you find with great relief you can begin to move your fingers again. With some effort you manage to work power into each corner of your body and finally shake free of the basilisk's curse; quickly, you shake the aching out of your " + pc.legs() + " and re-dress before anything else finds you and woozily begin to make your way back down the mountain, trying to ignore the feeling of ooze dripping out of you.");
		pc.orgasm();
		//pc.slimeFeed();
	}
	//Female: 
		...
		output("\"<i>Get caught with our pants down by a basilisk, did we slut?</i>\" she purrs.  \"<i>Well, not to worry.  I'll take good care of you.</i>\"  You somehow doubt her good intentions, and your suspicions are confirmed when, after completing her sauntering circumference of you, she shoves your bottom half as hard as she can. You fall slowly like a wooden board; being unable to brace yourself, the sensation of dropping is horrible, and, once your skull connects with the ground, painful.  Before you can clear your swimming vision you feel the goblin's soft, dense weight on your chest, and then her eager lips upon yours, sucking and tonguing every inch of your frozen mouth that she can reach.  A tingling sensation spreads from your mouth downwards as her pheromone-laced lipstick takes effect.  By the time she has finished with your mouth and worked her way downwards, your paralyzed body feels warm and receptive, and your [pc.vagina] is wet with need.  You feel small fingers caressing your mons before slipping their way inside, circling your [pc.clit] before testing your depths.  You wish you could cringe away or even thrust yourself forward to try and make the goblin better address your growing need, but that is impossible; you stare in front of you, petrified, as the fingers continue their lazy tour of your sex until your juices are running like a river in spring.\n\n"); 

		output("\"<i>You are well up for this, aren't you hun?</i>\" coos the goblin as she begins to flick your sopping clit, sounding delighted with the involuntary heat she has stoked within you.  \"<i>It's such a shame that I don't have the big cock you obviously want and need.  Don't worry though; I've got the next best thing!</i>\"  You hear her rummage around in her satchel, which is followed by the sounds of her wetly sucking on something with obvious enthusiasm.  You shrink inwardly as you make a strong guess as to what she's holding and what she intends to do with it.  Sure enough, a moment later you feel a dildo pushing unceremoniously into your [pc.vagina].");
		pc.cuntChange(0,30,true,true,false);
		//[(tight)
		if(pc.vaginalCapacity() < 30) output("  You didn't get the wet end, and you groan as you feel the thing begin to puff up as it reacts with your eager juices until it is pushing almost painfully against your inner walls.");
		else output("  You didn't get the wet end, and you groan as you feel the thing begin to puff up as it reacts with your eager juices until it fills your well-used passage almost perfectly.");
		output("  You feel the goblin crawl on top of you, pulling the dildo upwards so it rubs against your [pc.clit] and then with a squealing giggle impales herself on the other end.  Her lustful movements translate through the artificial cock buried in your dripping sex, sending tremors and waves of pleasure to your very core.  A gush of female fluid splatters over your own sex as the goblin works herself right down the shaft connecting you until she is squelching against you wetly.  The goblin twists, grinding and scissoring her thighs, the hard bud of her clit rubbing back and forth over your own.  By now your passage feels as stuffed as it ever has been, crammed totally full of the squishy expanded double-dong.  Every motion the tiny slut makes is amplified directly into the fuck-stick plugging your [pc.vagina].  Judging by how wonderful it feels rubbing and twisting against your sensitive walls, the aphrodisiac it is leaking into you is definitely having an effect.  You whine at the goblin twists herself and her dildo around, rubbing and riding you so effectively, the involuntary tide of erotic memories building again...\n\n");

		output("After cumming over and over again in tandem with the dominant little slut she eventually takes mercy upon you, leaving you lying in a pool of mingled girl cum, dazed by the hypnotic backwash and the drugs which have left you so hazy and sensitive.  After another ten or twenty minutes of being forced to lie there and marinate in your own shameful memories, you find with great relief you can begin to move your fingers again.  Eventually with some effort you manage to work power into each corner of your body and finally shake free of the basilisk's curse; quickly, you pick yourself up and redress before anything else finds you and woozily begin to make your way back down the mountain.  The smell of horny goblin on you is a lingering reminder of what just happened to you.\n\n");
...
private function basiliskAdvantageMinotaur():void {
	...
	output("Eventually the minotaur's balls swell against your [pc.ass] and with a long, satisfied moo, it reaches its peak, holding you down so its cum jets forth deep inside you.  ");
	if(pc.hasCock() && !pc.hasVagina()) output("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously ejaculate, ropes of your own cum spattering on the ground and your own nerveless " + pc.feet() + ".");
	else if(!pc.hasCock() && pc.hasVagina()) output("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your juices spattering your thighs and calves.");
	else if(pc.isHerm()) output("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, ropes of your own jizz spattering on the ground and girl cum dripping down your thighs.");
	else output("The sensual high it has already instilled in you amplifies by tenfold as the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your anus helplessly milking the creature for all it can get.");
	output("\n\n");	
...
//private function driderPCEggLaysBasilisk():void {
	//spriteSelect(75);
	//clearOutput();
	//output("Leaning back on your carapace-clad abdomen, you try and look over your defeated opponent.  Even slumped over in ");
	//if(monster.HP() <= 1) output("exhaustion");
	//else output("arousal");
	//output(", the basilisk is still striving to meet your eyes with its own.  Clenching your teeth, you avert your eyes and stomp forward blindly, colliding roughly with the spined lizard, knocking him flat on his back on the rocky ground.");
	//
	...
	//else output("cunt");
	//output(" up against his half-open mouth, smearing your free-running juices across his scaled face.  You wait a few seconds, just enjoying the feeling of his hot breath against your oversensitive sex, before you state your intentions in an imperious tone.  \"<i>Lick.</i>\"");
	...
	//if(pc.hasCock()) output("wrap tightly around your waiting prick");
	//else output("burrow deep into your needy slit");
	//output(".  You coo in delight at the sensation, and begin to grind yourself against his face as you wait for your egg-laying to begin.");
	//output("\n\nThe monster's stomach bloats as a final rush of goo fills his bowels in preparation for your ");
	//if(pc.fertilizedEggs() == 0) output("un");
	//output("fertilized orbs, providing you with a moment's warning before the ecstasy-inducing ripple of an egg sliding deep into him rolls your eyes backwards in pleasure.  You twitch and squirm atop the bound basilisk, mashing your ");
	//if(pc.hasCock()) output("dick");
	//else output("cunt");
	//output(" against his face while your egg-laden tube pumps away, each pulse forcing another egg through his squeezing ring and sending trills of intoxicating euphoria up your spine.  You lose yourself in the act, your heartbeat racing in time with contractions of your ovipositor, spraying ");
	//if(pc.hasCock()) output("semen");
	//else output("femcum");
	//output(" across your partner's face and neck.");
	...
	////(if corruption >= 50 
	//if(cor() >= 50) output(" before heading off towards your camp. The harpies will probably free him from your webs... eventually.");
	//else output(".  You reach up and slice him free from the webbing, carefully placing him on the ground below.  The blindfold stays, though.  You're kind, not stupid.  With your good deed for the day complete, you gather yourself up and head back to camp.");
	//pc.dumpEggs();
	//pc.orgasm();
	//cleanupAfterCombat();
//}
...
//private function layBeeEggsInABasilisk():void {
	//spriteSelect(75);
	//clearOutput();
	//output("Completely destroyed, the basilisk lays face down on the ground, struggling pathetically to get up.  You giggle at the attempt as you ");
	//if(pc.wingType == WING_TYPE_BEE_LIKE_SMALL || pc.wingType == WING_TYPE_BEE_LIKE_LARGE ) output("buzz lightly over to the basilisk's writhing body");
	//else output("saunter arrogantly over to the basilisk's crumpled up figure");
	//output(" and lightly nudge him in his ribs with your [pc.foot].  His eyes are closed from to the ");
	//if(monster.HP() <= 1) output("pain");
	//else output("arousal");
	//output(" he's currently suffering through, but you know they won't stay that way for long.  You zealously search through your bags for some extra undergarments and wrap them around the lizard beast's scaly visage; with a quick afterthought, you reach for his talons and tie them to his face, atop his eyes in mock mourning for what is about to happen.  You let out another perverse giggle and wonder vaguely if there is anything more you can do to make this situation more humiliating for the beast.");
	//
	////[(if pc is horse or unsexed)
	//if(pc.isTaur() || pc.gender == 0) output("\n\nYou drool messily on your hand and cup it to the basilisk's hole; he hisses in displeasure - though he must have had an inkling of what was coming, the visceral sensation of having his asshole lubed for entry is probably sending his imagination careening off in reckless, tormenting anticipation.");
	//
	//else if(pc.hasCock()) {
		//output("\n\nYou remove just enough of your [pc.gear] to expose yourself and grab hold of your increasing length");
		//if(pc.totalCocks()) output("s");
		//output("; rubbing enthusiastically, you start at the base but end in a gentle stroke at the sensitive tip.  With you doing your best to milk as much pre as you can, the viscous ooze that dribbles out of your dong");
		//if(pc.totalCocks()) output("s");
		//output(" lands with soft pitters on the overgrown lizard's back, the intense mountain sun baking the gunk onto the scales.  You sigh in languor, but remember you're doing this for a reason.");
	//}
	////(else if pc has vag:
	//else output("\n\nYou tap a finger against your cheek, then have an idea.  Making quick work of your [pc.gear] you dip your fingers into your personal honey jar, flicking the contents enticingly but quickly, to get some lube to dribble out onto your hands.  Once a decent amount has been deposited into your waiting palm, you slap the viscous concoction under the basilisk's nostrils and smear it lasciviously all around.  The basilisk coughs as his way of breathing is momentarily interrupted, but you ignore it in favor of reaching for more honey to rub.");
	//
	//output("\n\nYour victim squirms at the sensations forced upon him, but as weak as he is, another firm [pc.foot] to his ribs makes him rethink his resistance.  Your overburdened abdomen throbs, reminding you of your purpose, and you tilt the basilisk's hips up toward the sky for a better angle to insert yourself; a sigh of relief escapes your lips as you release your ovipositor from your bee abdomen.  You take your time arranging yourself ");
	//if(pc.isTaur()) output("above");
	//else output("behind");
	//output(" the desperately-squirming lizard, delighting in the fear-scented air around the beast as your ovipositor slaps his cheeks in search of his tight little black hole.  Aside from the difference in feel, you know once you hit the sweet spot by the rigid jolt from the basilisk as his worst fear is confirmed.  You openly laugh this time, and enjoy the feel of the lizard squirming under you as your bulbous appendage painstakingly, slowly works its way inside his anus.  Dry and tight, you really have to force your way, but the feeling is amazing, and it just keeps getting better.  It probably doesn't feel too great for the basilisk, but that's not exactly your problem.  The slit at the end of your ovipositor widens, leaking golden liquid, and the squirming basilisk halts its struggles and relaxes as the narcotics in the slime work their way into the bloodstream.  You begin humming in a fashion similar to the giant bees you now resemble as you enjoy the blissful sensation of the eggs progressing through the long tubular organ, stretching the reptile's pucker to accommodate your ");
	//if(pc.fertilizedEggs()) output("fertilized offspring");
	//else output("burdensome load");
	//output(".");
	//
	...
	//if(pc.isTaur() || pc.gender == 0) {
		//output("\n\nThere's nothing to distract you from the pleasure; ");
		////[(U)
		//if(pc.gender == 0) output("no needy cock or cunt demanding a caress - except the basilisk's, and you care not a bit about that one.");
		//else if(pc.isTaur()) output("even if you wanted to reach your genitals, your arms resolutely refuse to bend that way, so you focus on the slippery, silky feelings travelling along your ovipositor.");
	//}
	//if(pc.hasCock()) {
		//output("\n\nYour seeping member");
		//if(pc.cockTotal() > 1) output("s");
