...
public function encounterGooGirl():void
{
	clearOutput();
	//spriteSelect(69);
	output("As you walk around the lake, you notice a pale red light pulsing in the ", false);
	if (!flags["COC.FACTORY_SHUTDOWN"] == 2) output("sapphire ", false);
	else output("murky ", false);
	...
	output(gooGirlColor() + " slime body sculpting itself into a humanoid shape. The girl curiously tilts her head to one side, as if trying to figure out why you're backing away, before she happily surges forward!", false);
...
private function fightGooGirl():void {
	...
	CombatManager.victoryScene(beatUpGooGerl);
	CombatManager.lossScene(getBeatByGooGirl);
...
//
		...
		//output("(<b>Gained New Perk: Slime Core - Moisture craving builds at a greatly reduced rate.</b>)", false);
...
public function gooGirlBadEnd2():void
{
	...
	output("Gripping her in your gentle hands, you lower the girl to the junction of your groin.  ", false);
	if (pc.hasCock()) output("A pseudopod extends from your crotch, a gooey penis the size of a minotaur. It snakes through the air toward the girl, its tip flaring to expose a wide, urethra-like opening.  ", false);
...
public function getBeatByGooGirl():void
{
	...
	if (flags["COC.GOOGIRL_CONSECUTIVE_LOSSES"] == undefined) flags["COC.GOOGIRL_CONSECUTIVE_LOSSES"] = 0;
	flags["COC.GOOGIRL_CONSECUTIVE_LOSSES"]++;
	...
	if (flags["COC.TIMES_FUCKED_NORMAL_GOOS"] == undefined) flags["COC.TIMES_FUCKED_NORMAL_GOOS"] = 0;
	flags["COC.TIMES_FUCKED_NORMAL_GOOS"]++;
}
...
public function dudeLoseToGooGal():void
{
	...
	output("Just in time, the thin tongue pistoning its way into your abdomen opens, the tube-like appendage mirroring the thirsty suckling of her " + gooGirlColor(5) + " mouth. Your body feels like it melts just a bit as your potent seed is greedily slurped up her tongue and bursts into her mouth with enough force to kick against the back of her maw, nearly blowing clear out of her head! Pumping her slimy fist inside your rear, the goo-girl milks your prostate for every drop of cum in your body.  ", false);
...
public function hermLoseToGooGal():void
{
	...
	if (pc.milkQ() >= 100) output("  The mouths sucking at your " + pc.allBreastsDescript() + " ripple against your flesh until milk sprays from your mammaries, her body suckling the entirety of your lactate-heavy chest, cleavage to teat.", false);
	output("  Your spurting ecstasy floods the girl's jelly body, coloring her " + gooGirlColor(3) + " body white as she drinks deeply, coaxing a few seconds of gasping spasms into long, seed-spewing minutes of climaxing euphoria.", false);
...
public function femaleLoseToGooGal():void
{
	...
	if (pc.isPregnant()) {
		...
		output("Thick trails of milk squirt into the slime's hands and down her arms, filling the girl's " + gooGirlColor() + " body with your creamy milk.", false);
		if (pc.lactationQ() <= 1000) output("  She drains you in no time, hands hungrily sucking for more in goopy kisses on your overtaxed nipples.", false);
		//[medium milk only: 
		else if (pc.lactationQ() <= 2000) output("  She drinks gratefully, nursing you with her permeable body until the pout of her belly sloshes heavily with the pale ivory of your motherly bounty.", false);
		else output("  You bite your lip and let go, torrents of rich, pearl fluid rushing through her arms and inflating the surprised girl before your eyes. The milk gushing into her fills the goo from base to breast, giving the once-" + gooGirlColor(5) + " girl an opalescent shine, her over-filled body bloated as if she were carrying triplets.", false);
	...
	//[If the player isn't pregnant]
		...
		output("You try to stop the girl's progress, frantically, wagging your legs and clenching your inner walls together, but your strength has left you and her " + gooGirlColor() + " skin is much too slippery to gain ground against. A moment later, all thoughts of stopping the invading goo are lost to the waves of gushing heat pouring into your body as her neck feeds into your aching cunt, filling your uterus with the sentient slime. She braces her hands at the widening mouth of your sex before slipping those in as well, working her arms and shoulders into your now gaping body.  ", false);
		//[Small vaginal capacity: 
		if (pc.vaginalCapacity() <= 30) output("She bottoms out quickly, filling every inch of your pussy with her squished form, wriggling inside of you happily, her membrane molding to your birth canals every crevasse.", false);
		...
		//if (pc.pregnancyIncubation >= 1) output("  Everything she pumped into you runs back out after.  For some reason or another, it didn't take.", false);
		//else {
...
//Victory – Male
public function gooMaleRape1():void { gooMaleRape(1); }
public function gooMaleRape2():void { gooMaleRape(2); }
