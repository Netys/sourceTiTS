...
public function kaijuMeeting():void {
	clearOutput();
	if(flags["COC.KAIJU_MEETINGS"] == undefined) {
		output("Your explorations take you to a small island you haven't seen before.  It appears to be a large, smooth rock hill jutting out of the water.  Do you explore it?");
		//[Yes/No]
		clearMenu();
		addButton(0,"Yes",meetDatKaijuYo);
		addButton(1,"Leave",noMeetingKaijus);
	}
	else {
		if(flags["COC.FACTORY_SHUTDOWN"] == 2 && !kaijuCock()) kaijuGrowsWangus();
		else if(flags["COC.KAIJU_BAD_END_COUNTER"] >= 5) kaijuBadEndToyBOOSH();
		else repeatKaijuEncounter();
	}
}

private function kaijuCock():Boolean {
	return (flags["COC.KAIJU_COCK"] == 1);
}

//[If no]
private function noMeetingKaijus():void {
	clearOutput();
	output("You continue rowing on, away from the hilly island.");
...
private function meetDatKaijuYo():void {
	...
	if (pc.hasGenitals()) addButton(1, "Flirt", flirtWithKaiju);
	else addDisabledButton(1, "Flirt", "Flirt", "You have no interest in flirt. You don't even have any genutals.");
...
private function insultTheKaijuFirstMeeting():void {
	...
	if(kaijuCock()) {
		...
		output("In perhaps not your brightest idea, you decide to make fun of the giant green girl in front of you, mocking her obvious insecurity over her figure.  The word 'fat' barely exits your mouth before her face goes red in anger and you realize your mistake.");
		...
		if(getKnownFireMagic() != null) {
			output("\n\nSensing her desires, you try to quickly think of how to avoid pleasing the giant's large cock.  It occurs to you that you could use the magic fire to attack her.  Do you cast it?");
			//[Yes][No]
...
private function corruptKaijuInsertion():void {
	clearOutput();
	//spriteSelect(103);
	output("She practically shoves your face into her dark green cock-head, your nose snug against the long slit.  It smells of the sexual corruption of the lake, of demonic jizz and tainted aphrodisiacs.  \"<i>Well go on,</i>\" she says, \"<i>lick it!</i>\"");
...
private function complimentKaiju():void {
	clearOutput();
	//spriteSelect(103);
	output("You decide to compliment her figure, stating it is proportional and very pleasing.");
...
private function stopItPlease():void {
	clearOutput();
	//spriteSelect(103);
	output("You cry out and wave the inquisitive hand away.  You tell her that you mean no offense, but at such a size and strength disparity you are afraid of what a grip from such a woman could do accidentally.  She seems a bit saddened at that, but makes no further attempt to grab you.");
...
private function letKaijuHaveWayWithYou():void {
	clearOutput();
	//spriteSelect(103);
	output("You make no moves as the hand firmly but gently picks you up and brings you upwards towards her ample bosom.  She holds you tightly to a breast, nearly smothering you in tit flesh.  Her perky nipple seems massive up close as it practically bounces off your head when she begins grinding you up and down her gargantuan breast");
	//[only apply if player has dick]
	if (pc.hasCock()) {
		output(", [pc.eachCock] stiffening ")
		if(pc.isCrotchGarbed()) output("in your [pc.lowerGarmentOuter] ");
		output("from the stimulation as your whole body is used in what seems more and more like a massive boob job");
	}
	output(".");
	if(pc.hasVagina()) output("  Your [pc.vagina] moistens from your body's growing arousal.");
	
	if(pc.isCrotchGarbed()) {
		output("\n\n\"<i>Oh my, it seems someone is enjoying my special hug,</i>\" the giant beauty says noticing your rising arousal.  The lusty giant removes your [pc.lowerGarmentOuter] revealing your ");
		if(pc.isHerm()) output("[pc.cocks] and [pc.vagina]");
		else if(pc.hasCock()) output("[pc.cocks]");
		else if(pc.hasVagina()) output("[pc.vagina]");
		else output("bare groin");
	}
	output(".  \"<i>Let's have some fun!</i>\"");
...
private function flirtWithKaiju():void {
	clearOutput();
	//spriteSelect(103);
	output("You decide to try and flirt up the giantess.  You tell her that her figure is perfect and statuesque, and that her curves are quite magnificent to behold.  It seems to work if her spreading look of pleasure is any indication.");
	...
	if(pc.hasCock()) {
		...
		output(" throbbing from the sexual stimulants of her aphrodisiac moisture and the pressing walls of her cunt flesh, pre forming at your tip");
		if(pc.cockTotal() > 1) output("s");
		output(".");
	}
	...
	if(pc.libido() < 50) {
		...
		//[If PC has cock]
		if(pc.hasCock()) output("  [pc.eachCock] spasms as you release your load onto her pussy walls, making them just a bit more slick with your contribution.");
		if(pc.hasVagina()) output("  Your own pussy releases its own feminine juices, soaking your thighs even further in wetness.");
	...
	if(kaijuCock()) {
		...
		if(pc.hasGenitals()) {
			...
			if(pc.biggestTitSize() >= 1) output("  You press your [chest] against her cock flesh as well, giving her penis as much a boob job as such a behemoth prick can be given.");
			if(pc.lactationQ() >= 200) output("  Milk spurts and flows from your [pc.nipples], making your breast slick and shiny and helping you slide more easily along the throbbing meat-pole.");
		}
		...
		if(pc.hasGenitals()) {
			...
			output("  Undeterred, you continue grinding upon the spurting pole until you reach your own smaller, far less impressive orgasm, releasing your own ");
			if(pc.hasVagina()) output("feminine juices");
			if(pc.isHerm()) output(" and ");
			if(pc.hasCock()) output("thick cum");
			output(".  You look out upon the lake to see much of it has taken on a remarkably more milky white color.  Even for a giant that was a prodigious amount of cum she unleashed!");
...
private function repeatKaijuEncounter():void {
	...
	if(!kaijuCock())
		...
		addDisabledButton(5, "Give Cock", "Give Cock", "Considering her size, you need a LOT of Incubi Drafts.");
	...
	else addDisabledButton(3, "UrethraFuck", "Urethra Fuck", "She lacks cock required for this scene.");
...
private function kaijuRepeatBoobHug():void {
	clearOutput();
	//spriteSelect(103);
	output("You decide it would be fun to play with her titanic titties for a while.");
	...
	//[only apply if player has dick]
	if(pc.hasCock()) output(", [pc.eachCock] stiffening" + (pc.isCrotchGarbed() ? " in your [pc.lowerGarment]" : "") + " from the stimulation as your whole body is used in what seems more and more like a massive boob job");
	output(".");
	if(pc.hasVagina()) output("  Your [pc.vagina] moistens from your body's growing arousal.");
	
	output("\n\n\"<i>Oh my, it seems someone is enjoying my special hug,</i>\" the giant beauty says noticing your rising arousal.");
	if(pc.isCrotchGarbed()) {
		output("  The lusty giant removes your [pc.lowerGarmentOuter] revealing your ");
		if(pc.hasCock()) output(pc.cocksDescriptLight());
		if(pc.isHerm()) output(" and ");
		if(pc.hasVagina()) output("[pc.vagina]");
		if (!pc.hasGenitals()) output("bare groin");
	}
	output(".  \"<i>Let's have some fun!</i>\"");
	...
	if(pc.hasCock()) {
		...
		if(pc.cockTotal() > 1) output("s");
		if(pc.balls > 0) output(" and [pc.balls]");
		output(" rest on her perky pink naughty bit.  She coos in delight as her hand slowly rubs your private parts along her firm nipple.  She begins to bounce ever so slightly, which at her size means a verifiable mountain of boob begins smacking heavily against your lower body.  \"<i>Oh yes,</i>\" she says, moaning in ecstasy, \"<i>fuck my breast!</i>\"  You don't really seem to have a choice in the matter as she picks up the pace, almost violently slamming you against her smooth green skin and stiff pink nipple.  Despite the relatively soft flesh of her mammary, it is still hundreds of pounds of tit flesh.  As if unsatisfied with just smothering you in boob she takes one green pinkie finger and begins to spank you in a manner she probably believes to be quite soft but lands quite hard on your [pc.butt].  A small number of spankings begin to leave your ass red and tingling, each strike pushing your [pc.cocksLight] further into her soft breast and firm nipple.  \"<i>Please, cum on me!</i>\" she commands, and almost as if on cue your body obeys, jizz flying and coating her breast with as much sperm as your body can produce.");
		if(pc.balls > 0) output("  Your [pc.balls] begin to ache as you slowly empty your load onto green flesh.");
		pc.orgasm();
	}
	//[if player has vagina]
	...
	output("\n\n\"<i>My, that was a mighty big hug.  I think I'll need another nap. Come visit me again some time,</i>\" she says, slowly putting you down in your boat.  You slowly roll away as the jolly green giantess giggles and slowly wades off.");
	//[Giant turtle girl now encounter-able at Boat and Swamp, corruption increases slightly]
	flags["COC.KAIJU_BAD_END_COUNTER"]++;
	doNext(returnToCampUseOneHour);
}
...
private function fuckThisGiantYouDumbCunt():void {
	clearOutput();
	//spriteSelect(103);
	output("You decide it’d be fun to fuck the giantess, even if at her size you’re really more of a living dildo than a rutting stud.  She seems pretty happy with the thought if her spreading look of pleasure is any indication.");
	...
	//[If PC has breasts]
	if(pc.biggestTitSize() >= 1) {
		output("\n\nYour [pc.chest] slide across the slick yet firm vaginal flesh of her surprisingly dexterous cunt, your [pc.nipples] stiffening from the constant teasing massage.");
	}
	//[If PC has cock]
	if(pc.hasCock()) {
		output("\n\nYour hard cock");
		if(pc.totalCocks() > 1) output("s are");
		else output(" is");
		output(" throbbing from the sexual stimulants of her aphrodisiac moisture and the pressing walls of her cunt flesh, pre forming at your tip");
	...
	if(pc.libido() >= 50) {
		...
		if(kaijuCock()) {
			...
			//[If vagina and/or cock is present]
			if(pc.hasGenitals()) {
				output("You grind your privates against her member, determined to get yourself off as well.  ");
			}
			//[If PC has boobs]
			if(pc.biggestTitSize() >= 1) output("You press your [fullChest] against her cock flesh as well, giving her penis as much a boob job as such a behemoth prick can be given.");
			if(pc.lactationQ() >= 200) output("  Milk spurts and flows from your [nipples], making your breast slick and shiny and helping you slide more easily along the throbbing meat-pole.");
...
private function urethraFuckDatGiantCock():void {
	clearOutput();
	//spriteSelect(103);
	output("You look at her with an inquisitive stare.  You’re interested in that big green cock of hers, but how can you use such a giant tool?");
	...
	output("\n\n\"<i>Gods above [pc.name] but you look like a whore!  By Marae you’re turning me on!</i>\"");
	
	output("\n\nShe quickly has you positioned in front of her cock once more, this time feet first so she can keep your head free.  You can see your legs slowly slide down into her green cock, her cum-slit swallowing you up to your [pc.hips].  You can feel the walls of her cock head push in all around you, squeezing your [pc.butt]");
	if(pc.hasCock()) output(" and your " + pc.cocksDescriptLight());
	output(".  \"<i>Wow, you look absolutely gorgeous in my cock!</i>\" the giantess says with glee, perverse desire in her eyes as she shoves you in further up to your [pc.chest], her trembling, squeezing cock seeming to give you an almost relaxing massage.  You can barely believe it, but this unnatural giantess has as much control over her urethral walls as the most whorish succubus has over her cunt!  Only your arms and head are free of her cock as her unusual member starts squeezing and rippling all along your body.  It seems her dick wants to give you a happy ending.");
	
	output("\n\nJudging by the lustful cooing sounds, the jolly green girl is nearing her limit, her free hand snaking down to gently stroke her giant cock.  \"<i>Going to cum I’m going to cum going to cum!</i>\" she shouts as she reaches her own orgasm before you.  Suddenly her dick widens at the base as her cum shot heads your way!  With your body lodged tightly in the way, the corrupted turtle sperm has nowhere to go but in, your [pc.asshole] suddenly filling up as your bowels are flooded with cum!");
	if(pc.hasCock()) output("  So much jizz fills your ass it can’t help but to press in on your prostate, making [pc.eachCock] stiffen with a seriously growing urge.");
	if(pc.hasVagina()) output("  Your ass isn’t alone in the feeling of being flooded as your [pc.vagina] and womb are packed with the hermaphrodite’s sperm.");
	output("  You manage to reach your own orgasmic climax right before the pressure finally pops you out of her cock like a cork from a wine bottle.  You black out somewhere above the lake.");
...
private function leaveRepeatKaiju():void {
	clearOutput();
	//spriteSelect(103);
	output("You politely decline any options and bid the green girl goodbye as you row away.");
	doNext(returnToCampUseOneHour);
}
...
private function talkToKaiju():void {
	clearOutput();
	//spriteSelect(103);
	flags["COC.KAIJU_TALK_CYCLE"]++;
	flags["COC.KAIJU_BAD_END_COUNTER"]--;
	output("\"<i>Oh?  You want to talk?</i>\" she says, a bit taken by surprise, \"<i>What about?</i>\"");
	//(One of the available lines of dialog plays.)
	if(flags["COC.KAIJU_TALK_CYCLE"] == 1) {
		//Dialog one: 
	...
	//Dialog two: 
	else if(flags["COC.KAIJU_TALK_CYCLE"] == 2) {
		...
		if(flags["COC.FACTORY_SHUTDOWN"] == 1) output("  But the flow of yummy fluids seems to have been stopped.");
		//[If the Factory was shut down and Marae corrupted]
		else if(kaijuCock()) output("  But the pouring of liquids has been stopped for some reason, but not before there was one final, huge burst of tainted fluids. It was so strong and changed me even further, making me grow this,</i>\" she says as she rises further out of the lake's waters, revealing a large green cock.");
		
		output("\n\nYou thank the giantess for her explanation as you row away.");
	}
	else if(flags["COC.KAIJU_TALK_CYCLE"] >= 3) {
		//Dialog three: 
...
private function peekAtSomePhatAssKaijuButt():void {
	...
	output(".  She pumps her clenched fist furiously as deep into herself as she can reach, going faster and faster as she quickly reaches orgasm, her pussy juices squirting out around her fist, soaking her forearm in the feminine fluid.  She slowly pulls her hand out with an audible 'Plop' and brings both hands back to her breasts, massaging her giant sized fuck pillows until two thick steady streams of milk begin flowing out her nipples and down her breasts.");
	//[Lust is increased]
	//dynStats("lus", 10 + pc.lib / 3);
...
private function noKaijuPeepShows():void {
	clearOutput();
	//spriteSelect(103);
	output("\"<i>My goodness, it's so much better with a captivated audience!</i>\" she says, breathing heavily.  You thank her for the show as she places you back into your boat and giving it a push.  You row away, considering perhaps coming back for another show.");
...
private function dontGetFutaTurtlesOffToday():void {
	clearOutput();
	//spriteSelect(103);
	output("You shake your head no, politely responding that you do not want to get her off right now.  \"<i>Oh, okay. I understand,</i>\" the giantess states, looking somewhat crestfallen.  You row away, leaving her and her new addition to themselves for the time being.");
	doNext(returnToCampUseOneHour);
}
...
private function mockDatTurtleGirl():void {
	...
	if(pc.balls > 0 && pc.ballSize() >= 6) output("Your [pc.balls] rest on the hermaphrodite's hand, bouncing loudly with each thunderous stroke from the speed and force she is employing to get herself off.  ");
	//[end balls] [If player has cock]
	if(pc.hasCock()) output("You can feel your [pc.armor] getting tighter as [pc.eachCock] begins to stiffen under your clothing from all the constant pressure of the cock squeezing down on your member.  ");
	//[If player has vagina]
...
private function helpNewFutaKaijuGetOff():void {
	...
	//[If PC has cock]
	if(pc.hasCock()) output("  [pc.eachCock] quickly hardens now as it is rubbed up against the giant's massive shaft.");
	if(pc.hasVagina()) output("  The warm heat that radiates off of the titanic member seems to seep into your nether region, making your [vagina] leak out moisture onto the massive prick.");
	output("  She squirms under your touch, falling on her back and wriggling almost uncontrollably.  You're quite surprised at the power you hold over such a behemoth.");
	...
	if(pc.cor() < 50 && pc.libido() < 75) output("  You knead the cock head with your hands, massaging it quickly and roughly as you grind up against it, rock back and forth on the giant member.  Some of the pre dribbles down and you begin to use it as sticky massage oil, working it into the cock flesh.  The cooing and lustful moaning coming from the giantess increases in intensity with your every movement.");
...
private function yesTurnKaijuFuta():void {
	...
	while (counter > 0) {
		counter--;
		if(pc.hasItemByName("IncubiD")) pc.destroyItemByName("IncubiD");
		else if(pc.hasItemByName("P.Draft")) pc.destroyItemByName("P.Draft");
	}
	output("You wouldn’t have offered if you didn’t want to see a giant green futa-cock dangling between her legs.  Your declaration seems to have deepened her blush to almost beet red, but the lustful look in her eyes betrays her desire.");
	...
	output("\n\nWait, she wants you in her dick? The hell kind of minotaur was she spying on?");
	output("\n\nBefore you can comment or protest or even question the logic of her plan she has already stripped you of your [pc.gear] and positioned you in front of her crotch, face to face with her cum-slit.  A giant sized glob of pre has already formed at the head of her dick, her newly formed cock apparently a bit of a leaky faucet.  \"<i>Geronimo!</i>\" she yells as she pushes you face first into her goo drizzling tip, your head sliding into her dick with surprising ease, though your shoulders seem to be a different manner.  She pushes and twists you about, gently adding pressure until you finally pop right in all the way to your waist.  It’s surprisingly slippery in her cock, lubed by sweat and precum, and you slide with some ease even further into her dick until you are in up to your ankles despite the tightness of her passage, keeping your arms pinned to your sides.  You wiggle your feet a bit outside her dick before the perverted giant uses them to pull you most of the way back out before shoving you in there once more.  From your vantage point you can almost see all the way into her balls, and to your horror you see a new giant glob of pre heading your way!  You try to shout or move about to get the green herm’s attention, but your voice is muffled and your body constrained by the tight cock flesh all around you.  You take a deep breath and hold it right before getting a face full of musky sweet precum, the scent in your nose reminding you of salty...watermelon? You barely have time to register the scent before you are yanked out of her cock and can breathe again, her pre dripping off your face.");
	output("\n\n\"<i>Sorry about that, it only just occurred to me that might pose a problem.  But I have an even better idea, and this way I can see your cute face!</i>\" she says, blushing in delight at the view of your precum dripping face.  \"<i>Gods above [pc.name] but you look like a whore!  By Marae you’re turning me on!</i>\"");
	...
	if(pc.hasCock()) output(" and your " + pc.cocksDescriptLight());
	output(".  \"<i>Wow, you look absolutely gorgeous in my cock!</i>\" the giantess says with glee, perverse desire in her eyes as she shoves you in further up to your [chest], her trembling, squeezing cock seeming to give you an almost relaxing massage.  You can barely believe it, but this unnatural giantess has as much control over her urethral walls as the most whorish succubus has over her cunt!  Only your arms and head are free of her cock as her unusual member starts squeezing and rippling all along your body.  It seems her new dick wants to give you a happy ending.");
	output("\n\nJudging by the lustful cooing sounds, the jolly green girl is nearing her limit, her free hand snaking down to gently stroke her giant cock.  \"<i>Going to cum I’m going to cum going to cum!</i>\" she shouts as she reaches her own orgasm before you.  Suddenly her dick widens at the base as her second cum shot heads your way!  With your body lodged tightly in the way, the corrupted turtle sperm has nowhere to go but in, your [pc.butt] suddenly filling up as your bowels are flooded with cum!");
	//[If PC has a cock]
	if(pc.hasCock()) output("  So much jizz fills your ass it can’t help but to press in on your prostate, making [pc.eachCock] stiffen with a seriously growing urge.");
	//[If PC has a vagina]
	if(pc.hasVagina()) output("  Your ass isn’t alone in the feeling of being flooded as your [vagina] and womb are packed with the hermaphrodite’s sperm.");
...
private function flyAwayFromBadEnd():void {
	...
	//spriteSelect(103);
	flags["COC.KAIJU_DISABLED"] = 1;
	output("Flapping your wings at max speed you beat a hasty retreat! You've decided to avoid her in future...");
	doNext(returnToCampUseOneHour);
}
...
private function badEndPartTwo():void {
	...
	if(kaijuCock()) {
		...
		pc.buttChange(500,true,true,false); // 500? is actually possible to get such capacity... some nice potential for hyper
	...
	output("\n\nThe first planty-cock hits your ass like a switch, spanking your [pc.butt] till your cheeks are blushing red before the tip snuggles up to your puckered sphincter.  It wriggles wildly, working itself up in ecstatic anticipation, or at least the plant-like version of anticipation, drippy vine-pre-seed lubing up your hole before pushing forth with all its might, meeting only a little resistance before the pressure and fluid mixture eases it into your [pc.butt].");
	pc.buttChange(30,true,true,false);
	output("  Inches slide in at a time as it spasms inside of you, attempting to hit every sensitive spot your ass may have as it slowly tries to stretch you out.\n\n");
	
	//[If PC has a cock] 
	if(pc.hasCock()) output("[pc.eachCock] stiffens as the tree tendril pushes against your prostate, forcing a thick glob of pre up your shaft.  ");
	if(pc.hasVagina()) output("As the first vine-cock works its way up your rectum a second has found its way up to your snatch!  The plant-like head rubs against your folds almost lovingly, tenderly, before the head starts to prod past your pink lips, butting up against your cunt as part of the shaft rears up to rub against your clit, sending shockwaves through your body, and the vine-cock chooses this moment to plunge into your [pc.vagina].  Now you have two cocks ramming into your holes, pressing up against your walls as if to feel each other through you.  The plant dick in your pussy slides up and down through your folds, each thrust out going ever so slowly, only to ram back in at full speed, more inches sliding in every time until the head is bumping up against your cervix, leaky pre-seed slowly filling up your cunt to leak out between your thighs.  ");
	output("  The next sinuous cock threads itself between your legs, up your belly, past your chest to lightly poke at your lips.  You open your mouth to shout when the cock in your [pc.asshole] hits a sensitive spot and the cock at your mouth slides right in, dripping seed into your mouth, your tongue suddenly awash in a salty, yet pleasantly sweet flavor.");
	if(pc.biggestTitSize() >= 1) output("  Another tendril snakes its way up your abdomen to nestle between your [chest], slicking your mounds with pre as it slides between your tits, giving itself a titty-fuck with your titties!");
