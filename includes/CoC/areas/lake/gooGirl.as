...
public function gooGirlColor(arg:int = 1):String {
	var factory:* = flags["COC.FACTORY_SHUTDOWN"];
	switch(arg) {
		case 2:
			if (factory == undefined) return "azure";
			else if (factory == 2) return "plum";
			else return "crystalline";
		case 3:
			if (factory == undefined) return "cerulean";
			else if (factory == 2) return "violet";
			else return "clear";
		case 4://[teal/lavender/glassy] 
			if (factory == undefined) return "teal";
			else if (factory == 2) return "lavender";
			else return "glassy";
		case 5://[sapphire/amethyst/diamond]
			if (factory == undefined) return "sapphire";
			else if (factory == 2) return "amethyst";
			else return "diamond";
		case 6://[lapis/periwinkle/pure]
			if (factory == undefined) return "sapphire";
			else if (factory == 2) return "amethyst";
			else return "diamond";
		case 7://[blue berry/grape/crystal]
			if (factory == undefined) return "blueberry";
			else if (factory == 2) return "grape";
			else return "crystal";
		case 8://[aquamarine/plum/transparent]
			if (factory == undefined) return "aquamarine";
			else if (factory=== 2) return "plum";
			else return "transparent";
		case 9://[an aquamarine/a lilac/a translucent]
			if (factory == undefined) return "an aquamarine";
			else if (factory == 2) return "a plum";
			else return "a translucent";
		case 10://[blueberries/grapes/strawberries]
			if (factory == undefined) return "blueberries";
			else if (factory == 2) return "grapes";
			else return "strawberries";
		case 11://[cerulean tint/violet tint/clear body]
			if (factory == undefined) return "cerulean tint";
			else if (factory == 2) return "violet tint";
			else return "clear body";
		default://blue, purple, or crystal
 			if (factory == undefined) return "blue";
			else if (factory == 2) return "purple";
			else return "crystal";
	}
}
...
public function encounterGooGirl():void
{
	clearOutput();
	//spriteSelect(69);
	output("As you walk around the lake, you notice a pale red light pulsing in the ");
	if (!flags["COC.FACTORY_SHUTDOWN"] == 2) output("sapphire ");
	else output("murky ");
	output("waters. You pause, trying to figure out what the shape might be. Just under the surface of the water, there appears to be a fist-sized heart shedding a crimson glow. Leaning closer, you gaze down into your reflection only to find your face rising up with pursed lips, trying to kiss you! You jerk backwards and the pseudo-head quivers, resolving its face into a gooey-looking girl, her ");
	output(gooGirlColor() + " slime body sculpting itself into a humanoid shape. The girl curiously tilts her head to one side, as if trying to figure out why you're backing away, before she happily surges forward!");
...
private function fightGooGirl():void {
	CombatManager.newGroundCombat();
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(new CoCGooGirl());
	CombatManager.victoryScene(CoCGooGirlPCVictory);
	CombatManager.lossScene(CoCGooGirlPCLoss);
	CombatManager.displayLocation("GOO GIRL");
...
{
	//if (rand(4) == 0 && pc.findStatusAffect(StatusAffects.SlimeCraving) >= 0 && pc.findPerk(PerkLib.SlimeCore) < 0 && pc.isGoo() && pc.gooScore() >= 4) {
		//output("\n\nAs the goo-girl slithers away, into the lake's placid waves, you notice she seems to have left behind a small blob. Upon investigation, it appears to be a tiny, ruby heart, encased in a slimy " + gooGirlColor(8) + " membrane. As you reach to pick it up, the jelly ball quivers and pulses with a warm, cheerful light. Your fingers close on it and the nucleus slides through your palm, into your body!\n\n");
//
		//output("There is a momentary pressure in your chest and a few memories that are not your own flicker before your eyes. The dizzying sight passes and the slime core settles within your body, imprinted with your personality and experiences. There is a comforting calmness from your new nucleus and you feel as though, with your new memories, you will be better able to manage your body's fluid requirements.\n\n");
		////(Reduces Fluid Addiction to a 24 hour intake requirement).
		//output("(<b>Gained New Perk: Slime Core - Moisture craving builds at a greatly reduced rate.</b>)");
		//pc.createPerk(PerkLib.SlimeCore, 0, 0, 0, 0);
	//}
}
...
public function slimeBadEnd():void { //Another gooey bad end; you should have drunk more fluids
	...
	clearMenu();
	addButton(0, "Next", slimeBadEnd2);
}

public function slimeBadEnd2():void {
	clearOutput();
	output("One year later...");
...
//DEFEAT
//===============
public function CoCGooGirlPCLoss():void
{
	//if (doSFWloss()) return; //No rape in SFW mode.
	IncrementFlag("COC.GOOGIRL_CONSECUTIVE_LOSSES");
	IncrementFlag("COC.TIMES_FUCKED_NORMAL_GOOS");
	if (flags["COC.GOOGIRL_CONSECUTIVE_LOSSES"] >= 5 && pc.gooScore() >= 4) gooGirlBadEnd();
	else if (!pc.hasGenitals()) genderlessLoseToGooGal();
	else if (pc.isHerm()) hermLoseToGooGal();	
	else if (pc.hasCock()) dudeLoseToGooGal();
	else if (pc.hasVagina()) femaleLoseToGooGal();
}
...
public function dudeLoseToGooGal():void
{
	...
	output("Reeling, you stumble backwards, trying and failing to shake the dizziness from your eyes. The " + gooGirlColor() + " girl nods, emboldened, and leaps at you, her gushing body splashing heavily against your chest, knocking you to the ground. Your landing takes the breath from your throat and you struggle weakly as the " + gooGirlColor(5) + " muck below the girl's knees flows around your limbs, sucking your hands and feet into her slimy membrane with a slurping schlick. Trying to pull free, you stretch her body this way and that, but the goo holds fast, bonding you to her slippery form like warm, elastic shackles. The slime gurgles with pleasure at your struggles, her body rippling with your weak movements, her mouth curled into a happy grin. She gives you a wicked wink and rubs her hands over your hips, her dripping fingers slipping under your [pc.gear] and spreading across your flesh like a second skin. With squeezing tickles her groping digits massage her sludgy muck into your " + pc.skin() + ". Pursing her mouth into a cute pucker, she grips your vestments between her lips and deftly pulls them aside just enough to let your [pc.cock " + x + "] flop free.\n\n");

	output("The heat from the slime girl's engulfing second skin begins seeping into your core, leaving you panting while beads of sweat trickle down your brow. She notices your discomfort and lowers her head to nudge your veiny phallus with her tiny, vestigial nose. The girl follows up with a playful kiss, cobalt lips pliant and moist. The fire around your skin goes from uncomfortable to suffocating as your cock surges to life, and the girl pouts at the look of dismay on your face. Her eyes light up with an idea and she opens her mouth as wide as your fist, revealing a thin, " + gooGirlColor(2) + " tongue swirling behind her lips. The goo-girl presses the tip of her tongue at the tip of your head and a wonderful flash of cold floods your groin, chilly shivers soothing your overheated body like an icy glass of water in the desert. Your voice wavers as you laugh in relief, her wet, whirling tongue lapping along your pulsing shaft, growing longer and more flexible with each passing moment. After a few minutes of her licking, the girl's tongue has grown long enough to wrap around your [pc.cock " + x + "] several times. Using it like a slimy tendril, she strokes up and down your manhood in tightening coils of " + gooGirlColor(5) + ", slimy drool giving your cock an almost indigo appearance.\n\n");

	output("As she jerks you off with her tentacle-tongue, the goo-girl's body temperature slowly equalizes, her tongue warming and her body cooling to match your body heat. Her wobbling chest moves in time with your breaths and her skin ripples at each of your heart beats, subtle mimicry synchronizing her form to yours. As you gradually relax, she gives you a second wink and unravels her prehensile tongue from your shaft, raising its thin, pointed tip to the crest of your bulb. She teasingly circles your urethra a few times, making you bite your lower lip, before sliding her tongue into the narrow cleft, pressing into your organ almost effortlessly. You marvel at how... right... it feels. There is no pain from the penetration, her gooey body sculpting the intruder to fit perfectly, just large enough to give you the sensation of a uniquely exciting pressure. Reflexively, your body tries to expel the snaking interloper, clenching rhythmically in trembling throbs that curl your fingers into quivering fists. At the sight of your pulsing erection, the girl lowers her head over your [pc.cock " + x + "], and in one, long quaff, seals her mouth shut with a slurping gulp around your base. She stares at you, curiously, studying your expression as she bobs her head up and down in time to your quickening heart rate, syrupy clumps of the goo's hair falling over one of her eyes.\n\n");

	//[If the player has balls:
	if (pc.balls > 0) output("The lithe layer of ooze that creeps across your body wraps around your [pc.balls], its silken touch like the caressing tips of dozens of fingers, nestling against your swollen testicles, cradling them to feel the rush of cum inflating your sac at the building suction of her mouth.  Even with your [pc.gear] still on, it feels like she's licking your bulging pouch with her sylphlike hands.  The slime's gooey grip presses against your scrotum with steady strokes, rolling your overinflated balls in their pouch as if the silent girl were playing some mischievous game. You shake your head as your [pc.balls] bloat fuller and fuller at the coy girl's sport.\n\n");

	output("Through the " + gooGirlColor(2) + " hue of her semi-transparent face, you can see your cock throbbing at the gurgling eddies of her oozing mouth and the lively tongue-fucking she's giving your slit. Her pace has quickened to the point where her lips and nose slap against your groin with every heartbeat, a " + gooGirlColor(5) + " blur that feeds blissful inches of ooze into your urethra, flesh parting like a pussy before her penetration.  The second skin of the goo-girl's creeping hands reach the junction between your ass cheeks and you clench down as hard as you can, reluctant to let the spunky slime explore your body any deeper. The forceful girl will not be dissuaded, however, and her fingers grow thinner and thinner until one slides past your sealed sphincter like a key, inflating itself from within to open your ass to her curiosity. You moan as the oozing touch of her long, searching fingers edge their way past the plush, folded flesh of your anus, velvety walls parting as her hands swell to fit your bumpy nethers, first one finger, then another. She presses deeper until you feel what seems to be a fully formed hand snaking through your colon, up to the elbow, just as her devious tongue reaches the end of your urethra. Utterly penetrated, you are little more than putty in the hands and mouth of the goo-girl and the last of your resistance melts away.\n\n");

	output("She clenches her rippling body, mouth squeezing tighter than a fist and softer than a pussy on your sensitive organ. Your abdomen bulges outward as her gooey arm grows more solid inside your ass, filling you with a painless pressure that mirrors the straw-like tongue that you can see impatiently wriggling in your shaft. Her head ignites with the heat of anticipation and your muscles unclench, orgasm washing over you at last.");
	if (pc.balls > 0) output("  Your scrotum lurches with the force of your load, balls slapping against her cobalt chin as thick cum churns against the obstructing tongue plugging your release. Some chemical in her saliva seems to be reacting inside your [pc.balls], the gurgling ejaculate multiplying, semen filling your pouch and bloating it before your eyes. The rubbing pressure on your prostate just worsens the condition and, for a moment, you fear your balls will swell too large for you to move!\n\n");

	output("Just in time, the thin tongue pistoning its way into your abdomen opens, the tube-like appendage mirroring the thirsty suckling of her " + gooGirlColor(5) + " mouth. Your body feels like it melts just a bit as your potent seed is greedily slurped up her tongue and bursts into her mouth with enough force to kick against the back of her maw, nearly blowing clear out of her head! Pumping her slimy fist inside your rear, the goo-girl milks your prostate for every drop of cum in your body.  ");
	//[small to medium cum amounts:
	if (pc.cumQ() <= 500) output("A ball of pearl fluid builds up in the girl's head, turning her curious eyes white for a moment before she makes an exaggerated swallowing motion and the huge lump rolls down her throat, neck bulging outward as it drops into her belly, surrounding the gleaming ruby heart in a halo of ivory sperm.");
	else output("The girl's expression goes from pleased triumph to mild panic when your orgasm shows no sign of flagging, load after load racing into her body. She quickly snakes her tongue out of your urethra and tries to pull off of your [pc.cock " + x + "], but her arm is too deep in your ass and she finds herself stuck! You rest your hands on the " + gooGirlColor(4) + " sludge of her goopy hair, fingers sinking into her slime as she tries to make room for your gushing, ivory torrent.  You clench your eyes in pleasure and surrender yourself to the feeling, your groin a font of creamy seed.  When you finally slow and open your eyes, you're a little surprised to find that the girl has become almost entirely white!  Her translucent skin retains a bit of the lake's " + enemy.skinTone + " hue, but top to bottom, the goo-girl's body swirls with your cum, leaving her with the fattened appearance of pale pregnancy.");
	output("  Her arm slides out of your rump and the goo-girl practically rolls into the lake, splashing into the depths lazily. It sure didn't feel like you lost that fight, you reflect while taking delicious swallows of cold air.\n\n");
	//pc.slimeFeed();
	processTime(25 + rand(10));
	pc.orgasm();
	//dynStats("sen", 4);
...
public function femaleLoseToGooGal():void
{
	...
	//[Pregnancy continued]
	if (pc.isPregnant()) {
		output("The fluid woman squeezes her goopy breasts against yours and the globes jiggle with their liquid weight. The " + gooGirlColor() + " girl wriggles against you with an ecstatic expression, delighting in the quivering vibrations of your body. She presses her palms to your breasts and the warm goo engulfs your " + pc.nippleDescript(0) + " with a wet plop. She plays with your tits, rolling them in her hands while the slurping goop of her body sucks at your swollen mammaries. She bends down to pepper your pulsing tit-flesh with tiny, moist kisses that leave " + gooGirlColor() + " lip prints on your " + pc.skin() + ". Rocking her body against yours, the girl's expression mimicks the helpless, agonized bliss of your face, her cheeks even flushing indigo to imitate your blush. She darts in and gives you a deep, full-bodied kiss of pregnant joy and your " + pc.allBreastsDescript() + " burst with creamy milk, even as your climax leaves your body as quivering as the goo-girl's. Feeding on your sexual release, she arches her back and her hearts flicker wildly, her body losing some of its focus as it appears to melt under her orgasmic heat.\n\n");

		output("Thick trails of milk squirt into the slime's hands and down her arms, filling the girl's " + gooGirlColor() + " body with your creamy milk.");
		if (pc.lactationQ() <= 1000) output("  She drains you in no time, hands hungrily sucking for more in goopy kisses on your overtaxed nipples.");
		//[medium milk only: 
		else if (pc.lactationQ() <= 2000) output("  She drinks gratefully, nursing you with her permeable body until the pout of her belly sloshes heavily with the pale ivory of your motherly bounty.");
		else output("  You bite your lip and let go, torrents of rich, pearl fluid rushing through her arms and inflating the surprised girl before your eyes. The milk gushing into her fills the goo from base to breast, giving the once-" + gooGirlColor(5) + " girl an opalescent shine, her over-filled body bloated as if she were carrying triplets.");
		output("  She gives you a fond bop on the chin and a loving stroke on your belly before sloshing back into the lake, her seething heat still clinging to your body like a lingering hug.\n\n");
	}
	//[If the player isn't pregnant]
	else {
		output("The goo-girl flashes a broad smile and tickles under your chin to get your attention. Apparently trying to communicate her intent, she puffs out her cheeks and pats her belly, jiggling her amorphous frame in the process. Next, she taps your nose and pats your belly, slapping your " + pc.skin() + " with a moist splat of her hand. Your expression remains blank and she sighs soundlessly, turning cold in annoyance. The slime presses you on your back and lifts your " + pc.legs() + " into the air. In a splattering motion, the goo lets her lower half sink into the thick sludge of her feet until only her upper torso remains. Then, with impatient force, she descends on your " + pc.vaginaDescript(0) + ", her yielding breasts rolling across your lower body.\n\n");

		output("The girl's face finds your all-too-eager pussy swiftly enough, her tongue tracing every fleshy fold of your labia while the warming slime of her lips suck at the moisture of your rapidly lubricating slit. She pushes a little harder, forcing the oozing length of her tongue inside of you, its pulsing surface expanding and contracting, leaving you squirming and gasping, biting at one of your knuckles while the goo prods the limits of your womanhood. Seemingly satisfied, she screws her face into an expression of intense focus and slaps her head down, between your thighs. The delicate features of her nose and face press against your lower lips with an insistence that practically melts you before, shockingly, her head begins to squeeze into your " + pc.vaginaDescript(0) + "!\n\n");

		output("You try to stop the girl's progress, frantically, wagging your legs and clenching your inner walls together, but your strength has left you and her " + gooGirlColor() + " skin is much too slippery to gain ground against. A moment later, all thoughts of stopping the invading goo are lost to the waves of gushing heat pouring into your body as her neck feeds into your aching cunt, filling your uterus with the sentient slime. She braces her hands at the widening mouth of your sex before slipping those in as well, working her arms and shoulders into your now gaping body.  ");
		//[Small vaginal capacity: 
		if (pc.vaginalCapacity() <= 30) output("She bottoms out quickly, filling every inch of your pussy with her squished form, wriggling inside of you happily, her membrane molding to your birth canals every crevasse.");
		else output("More and more of her ooze thrusts into you, shivers of overstimulation tearing through your mind like fists through wet parchment. Your eyes roll aimlessly as the pressure makes it difficult to breathe. With a squelching slurp, the last of the girl's form is pulled inside your slavering pussy.");
		output("  Your body is wildly distorted by the intruder, some aspect of her slimy coating anesthetizing your anticipated pain with tooth-grinding pleasure. You stroke your bloated form, thoughts turning to sweet daydreams of pregnancy, your body full of a pulsing, liquid life, dependent upon you and squirming with eager desire.\n\n");

		output("When the girl begins sliding back out, it's all you can do not to vocalize your disappointment with a feeble moan. Splashing in an expanding puddle, the girl gradually molds her features back into a humanoid shape, but you notice she's smaller than before. Touching your belly, you can feel a jiggling orb still inside of you. You nearly bite your tongue when the straggler presses up against your cervix and forces its way into your womb with wriggling suction. Your body clenches in orgasm, but the slime's fluid body works past your sphincter-tight barrier and distends your gut with its occupancy. You shake in the squirting throes of your climax and the reduced woman rubs her cheek against your goo-slick breasts affectionately. Dimly, you notice that her heart-shaped core has been split right down the center, the other half, presumably, inside your body. With a wave goodbye, she plunges back into the lake, leaving you hot, wet, and pregnant- or what passes for pregnancy in asexual creatures.\n\n");
		//if (pc.pregnancyIncubation >= 1) output("  Everything she pumped into you runs back out after.  For some reason or another, it didn't take.");
		//else {
			//pc.knockUp(PregnancyStore.PREGNANCY_GOO_GIRL, PregnancyStore.INCUBATION_GOO_GIRL, 1, 1);
		//}
		
		pc.tryKnockUp(enemy, 0);
	}
	processTime(25 + rand(10));
	pc.orgasm();
	//dynStats("sen", 4);
...
public function gooPregVagBirth(pregSlot:int):void
{
...
//VICTORY
public function CoCGooGirlPCVictory():void
{
	flags["COC.GOOGIRL_CONSECUTIVE_LOSSES"] = 0;
	clearOutput();
	output("The excitement of your scuffle proves too much for the goo-girl to keep up with and she collapses into the slime of her lower torso, her skin wiggling as she struggles to maintain cohesion. Her expression is one of disappointment, and she looks at you with big, hopeful eyes, reaching out a hand, as if to offer an apology for her over-exuberance.\n\n");

	//Victory – Neuter
	if (!pc.hasGenitals() || pc.lust() < 33) {
		output("The goo-girl, while an unusual creature, seems unable to communicate and clearly has nothing of value.  Of no particular use in your quest, you shoo the dripping blob back to the shore. She seems disappointed at first, but bounces back quickly enough, spotting movement in the lake.  She splashes in and takes off at top speed, " + gooGirlColor(9) + " blur while you take your leave.\n\n");
		CombatManager.genericVictory();
	}
	else {
		output("With the goo-girl defeated, her unusual body is at your mercy.  What do you do?\n\n");
		//var valeria:Function = kGAMECLASS.valeria.valeriaAndGooThreeStuff;
		//if (pc.armorName != "goo armor" || pc.isButtPregnant() || pc.isPregnant()) valeria = null;
		//var eggs:Function =null;
		//if (pc.canOvipositBee()) eggs = layBeeEggsInGoo;
		clearMenu();
		
		if(pc.hasCock() && pc.smallestCockLength() < 24)
			addButton(0, "DickSex", gooMaleRape, 2);
		else  addDisabledButton(0, "DickSex", "Dick Sex", "You need small enough cock.");
		
		if(pc.hasCock() && pc.smallestCockLength() >= 24)
			addButton(1, "BigDickSex", gooMaleRape, 1);
		else  addDisabledButton(1, "BigDickSex", "Big Dick Sex", "You need large enough cock.");
		
		if(pc.isHerm())
			addButton(2, "Herm Sex", victoryHermSex);
		else  addDisabledButton(2, "Herm Sex", "Herm Sex", "You need both cock and vagina.");
		
		if(pc.hasVagina())
			addButton(3, "Fuck", normalFemaleRapesAGooGirl);
		else  addDisabledButton(3, "Fuck", "Fuck", "You need vagina.");
		
		if(pc.hasVagina() && (flags["COC.PC_FETISH"] >= 1 || pc.exhibitionism() > 80))
			addButton(4, "Exhib.Fuck", exhibitionismGooGirlVictoryRape);
		else  addDisabledButton(4, "Exhib.Fuck", "Exihibitionism", "You need vagina and exhibitionism streak.");
		...
		//corrupt chances
		//if ((flags["COC.GOO_TFED_MEAN"] == 0 && flags["COC.GOO_TFED_NICE"] == 0) && flags["COC.TIMES_FUCKED_NORMAL_GOOS"] >= 2) {
			//if (cor() < 50 && (pc.hasItem(consumables.SUCMILK) || pc.hasItem(consumables.P_S_MLK)) && (pc.hasItem(consumables.BLACKEG) || pc.hasItem(consumables.L_BLKEG))) {
			...
			//else if (flags[kFLAGS.PC_KNOWS_ABOUT_BLACK_EGGS] > 0) {
				...
				//}
				//if ((pc.hasItem(consumables.SUCMILK) || pc.hasItem(consumables.P_S_MLK)) && (pc.hasItem(consumables.BLACKEG) || pc.hasItem(consumables.L_BLKEG))) {
					//output("  Good thing you have those handy!");
		...
		//if (valeria != null) {
			...
			//if (flags[kFLAGS.TIMES_VALERIA_GOO_THREESOMED] == 0) output("Do you offer a threesome with the girl to Valeria? It could get a little weird....");
			//else output("Do you offer a threesome with the girl to Valeria? She'll likely try flood with you with more sloshing, shuddering pleasure than your body can handle.");
		//}
...
public function gooMaleRape(type:Number = 1):void
{
	...
	clearOutput();
	var x:Number = pc.biggestCockIndex();
	if (pc.longestCockLength() >= 24 && pc.shortestCockLength() < 24) {
		if (type == 1) x = pc.longestCockIndex();
		else x = pc.shortestCockIndex();
	}
	output("The girl wriggles in place, staring hopefully at you from her oozing puddle, her arms folded under her chest, eagerly waiting for your next move. Your skin still prickles from the lingering heat of her touch, impatient desire bubbling in your chest. You descend on the girl and she proudly puffs out her breast, presenting herself to you.\n\n");
	//[Dick under 2 ft] 
	if (pc.cocks[x].cLength() < 24) {
		output("Dropping to the goo-girl's level, you roughly grab her pulsing breasts, fingers sinking an inch into the slime before her surface tension gives you something to grip. Her membrane squishes between your fingers with lewd squelches as you squeeze harder, rolling the " + gooGirlColor() + " orbs in your palms, her " + gooGirlColor(2) + " nipples bulging outward the harder you press. She noiselessly coos at your stimulation and shivers of delight ripple through her body. Stroking your body with soft hands, the goo-girl leaves " + gooGirlColor(9) + " film of warm slime across your [pc.gear] that quickly leaves you too hot to stay clothed. You strip and the buxom slime flashes a happy smile of victory.\n\n");

		output("Noticing your hardening length, the goo-girl claps her hands in gushing pleasure and gently pushes you back onto your " + pc.buttDescript() + ". Rubbing her moist cheek against yours, she slides down your " + pc.chestDesc() + " and perches over your groin. Slowly, she moves back and forth, stroking your shaft with the perky wetness of her stiff nipples. She hovers over your manhood for a moment before dropping down, her sloping cleavage stroking your cock between her slimy pillows with a slurping squelch. Pressing her balled-up fists to either side, she squeezes your [pc.cock " + x + "] between her breasts and begins to slowly pump your stiffening organ. The natural lubrication of her gooey skin makes the tit-fuck pleasantly slick, softness sucking your flesh with wet gulps as she rolls from base to tip, your cockhead popping between her cleavage at a brisk pace, cooled by the air for only a moment before it is swallowed once more by the seething heat of her chest. Through her transparent skin, you can see your [pc.cock " + x + "] being massaged by the gooey embrace of her tits. Your body trembles to the precipice of an orgasm, muscle spasms and a shortness of breath makes each pump of your slime-oiled hardness more exquisite than the last, as if the girl's murky breasts were pulling the air from your lungs.\n\n");
...
public function victoryRapeAGooGalAsFeeder():void
{
	IncrementFlag("COC.TIMES_FUCKED_NORMAL_GOOS");
	clearOutput();
	output("The slimy girl wobbles back and forth, curiously eyeing you with her mouth turned up in an expectant smile. Her expression slowly widens into one of alarm, as your breath comes in quickened gasps. The need is building in your chest, setting your nipples aflame with the pressure within your breasts. Reading the blossoming heat from your body, the goo-girl seems to guess your intent and the playful petulance gives way to a wary caution. She shakes her head, puffing her cheeks and patting her belly as if to say 'no thank you, I am full.' A lop-sided grin creeps up the side of your face, a trickle of drool sliding out the corner of your mouth. She thinks she's got a choice in the matter, you muse. How cute. Advancing on the girl, you gently stroke her " + gooGirlColor(4) + " cheek with the back of your hand, her fluid membrane slick and warm on your skin. Yes, she'll do quite nicely.\n\n");

	output("Stripping your [pc.gear] to the waist, you settle on your ass, guiding the slime to a sitting position in front of you, her gaze darting back and forth, debating if she still has a chance to run. Before she can weigh the option any further, you seize her head with both hands and brace your thumbs on either side of her jaw. With a bit of pressure, her gelatinous head squishes between your fingers, mouth opening under the force of your grip while her eyes fill with silent panic. Your chest heaves in the open air, the heat of the goo-girl's muck feeding into the furnace your thumping heart has made of your flesh. Unbidden, pale cream bubbles along the sensitive surface of your nipples, beading like sweat on your skin. You pull the girl's head against your leaking mammary, guiding her " + gooGirlColor(4) + " lips into a suckling seal around your areola, squelching slime lapping at your trembling mound. A pressure builds just beyond your ribs and you grit your teeth, clenching your eyes shut as you push past the resistance, squeezing your thighs together. With a sweet, trembling wave of blissful release, your breast gushes into the girl's mouth, a torrent released from the swollen bounty of your heavy peak. Your fingers dig into the girl's head as you press her pliant softness into the yielding swell of your " + pc.allBreastsDescript() + ". The girl sucks gently, but every licking ripple of the slime's molded mouth excites your tingling breast with the familiar, surging sensations that creeps into your mind, clawing out all rational thought and leaving behind only the primal need to nurse.\n\n");

	output("The slime swallows, creamy milk floating down her neck and into her belly as she pops her lips off of your breast. \"<i>Not enough,</i>\" you pant, mouth twisting into a snarl. Sliding your thumbs from her jaw to her mouth, you hook your digits into the sides of her lips and pull outward, stretching her mouth wider and wider. Thrusting your chest forward, almost violently, you press your breasts together, between the sides of your arms, rubbing your nipples against each other. Jamming them into her stretched mouth, you begin jerking her head back and forth, fucking her face with your nipples. She gasps and the action seals her mouth as tight as a pussy around your intruding peaks, " + pc.allBreastsDescript() + " squirting increasing loads with every thrust. On some level, you delight in the knowledge that the girl doesn't need to breathe as you have no desire to relent your milk-heavy assault, orbs throbbing with every pink-tipped smack against her " + gooGirlColor(5) + " face. After a few minutes of your pounding, she's given up struggling against you, rounded eyes relenting to a vacant expression as you use her head as a slimy milking machine.\n\n");

	output("Again and again, you fill her cheeks with your glistening warmth, and it's all she can do to swallow as quickly as possible, her throat bulging with the alabaster loads rolling into her over-full belly. Overwhelmed, the goo-girl's ruby heart rolls helplessly in the fattening slime's body, unable to digest the sheer volume of lactate you're pumping into her. Running out of places to store it, the girl's own breasts begin to inflate, her " + gooGirlColor(2) + " nipples leaking in bubbling rivulets to mirror your own aching mounds. She cycles through cup-sizes faster than you can list them, the burgeoning promise of her curved bounty weighing her down until her chest bounces against the puddle of her feet with every jiggling gulp. The " + gooGirlColor(8) + " hair sliding across your fingers begins to writhe and twist, gooey strands merging and rising to the sides of her head forming two little horns on either side of her head, her body morphing into a shape more appropriate for the sloshing udders her chest has become under the suckling you're forcing on the girl. You've fed her so much that she's become a milk cow, you realize with a shuddering pleasure that touches the addiction center of your brain so intensely that you feel yourself creaming in a sequence of full-body contractions, muscles clenching with shivering rapture. It's just so right, you muse, panting through the coiling suffocation of your orgasm, to awaken in others the NEED to be fed. How sweet to nurse and to be used! And how much sweeter to teach others the mind-erasing desire to be little more than a beast, addicted to the lust of being milked!  ");
	if (pc.hasVagina()) output("Even without stimulation, your clit practically vibrates under your climax, squeezing your body to its limits until everything fades into a pearl haze and you lose yourself to the euphoria.\n\n");
	else if (pc.hasCock()) if (pc.hasVagina()) output("Even without stimulation, your cock practically vibrates under your climax, squeezing your body to its limits until everything fades into a pearl haze and you lose yourself to the euphoria.\n\n");
	else if (pc.hasVagina()) output("Even without stimulation, your tingling anus practically vibrates under your climax, squeezing to its limits until everything fades into a pearl haze and you lose yourself to the euphoria.\n\n");

	output("When you regain consciousness, your breasts finally feel empty, milked dry by your rough treatment of the goo-girl. Shaking your head, you look for a sign of the girl and spot something a short distance off in the lake. You almost don't recognize her at first, until you notice the tiny, jiggling horns poking out of the sides of her head. The cow-slime has apparently captured a shark girl and an anemone, her pale " + gooGirlColor() + " membrane wrapping around their bodies like giant fists. She's holding them to each giant breast, kneading their faces against her quivering membrane as her periwinkle nipples gush with your milk, soaking their heads and filling their senses with your addictive fragrance. In short order, they wrap their mouths around the " + gooGirlColor(5) + " faucets and begin to drink deeply, the cow-goo's face a silent mirror of your previous climax. Two more girls being taught the joys of lactation, you murmur, touching yourself with a gleeful shudder. Forcing yourself to master the desire to keep watching, you clean yourself up and head out, pleased gurgles fading into the distance.\n\n");
	coreDropChance();
	processTime(25 + rand(10));
	pc.orgasm();
...
public function normalFemaleRapesAGooGirl():void
{
	IncrementFlag("COC.TIMES_FUCKED_NORMAL_GOOS");
	clearOutput();
	output("The slimy girl wobbles back and forth, curiously eyeing you with her mouth turned up in an expectant smile. You don't doubt that she'd enjoy playing with your body given half a chance, but your victory in the scuffle has given you a rare opportunity to try out something that been lurking in the back of your head for a while. With a devious smile, you remove your [pc.gear] until your bare flesh shivers at the cool wind blowing over the surface of the lake. You hook a beckoning finger to the goo-girl and she shrugs, a willing grin on her face. Splashing toward you, the girl's puddle laps at your " + pc.feet() + ", " + gooGirlColor(2) + " muck lapping around you until you hold a palm up to stop her. Reaching down, you wrap your hands under her arms and haul her upward, plump legs forming under her waist as she grows taller and taller until you're eye to eye with the slime. Placing your fingers on her hips, you turn the girl around until her jiggling butt is pressed against your " + pc.hipDescript() + ", warm goop wobbling with your every movement. Taking a deep breath, you shake out your shoulders and swallow. Here goes nothing.\n\n");

	output("With your hands wrapped around the goo-girl's waist, you take a step forward. The pliant ooze of the girl's body squelches at the pressure of your legs straining against her membrane, spilling out to the side of her hips and she perks her head up, unfamiliar with what you're trying to do. You push harder, keeping her in place as you move against her, more forcefully this time. Her body squeaks with the slapping pressure against her yielding form and she furrows her brow, pointing at her mouth and between her legs. You just smile and pull her against you harder and harder, her expression becoming pinched with the force of your attempt to move through her. Finally, her mouth gapes in discomfort and her membrane parts, gooey legs becoming permeable and squishing around " + pc.legs() + ", engulfing your lower body in her " + gooGirlColor(4) + " slime. Your " + pc.vaginaDescript(0) + " sinks into the muck of the girl's body and her heat washes over you in a wonderful embrace of fluid tightness that caresses as much as it gurgles across your sensitive flesh. Wet and soft, the inside of the girl's gelatinous form sculpts itself around yours like a slimy mold. As delightful as the moist embrace is for you, it seems to be getting the goo off even more, the rest of her membrane trembling, unable to maintain firmness in the assault of your invading body. You easily push your arms into her shoulders and down the dripping shafts of her " + gooGirlColor(8) + " limbs, fingers siding into hers like moist gloves of gelatin.  Your " + pc.allBreastsDescript() + " goosh against her back and the tiny red heart in her chest retreats into her head as your orbs sink into the translucent girl's sapphire torso and rest within her wobbling breasts, her tits cupping your flush mounds like huge, warm mouths, her " + gooGirlColor(5) + " nipples tweaking yours ever so slightly as they pop into the pert nubs. You take another deep breath and release it, savoring the sensation as her back slowly closes up around your shoulder blades, sealing you to the neck in the goo-girl's body.\n\n");

	output("The sensations are so strange all over your body that it is overwhelming at first. You have to steady your breathing to keep the heat and tightness from subsuming your mind, so you take inventory of your body, focusing the cresting ecstasy of complete immersion down to more manageable chunks. The slime embracing your " + pc.clitDescript() + " vibrates ever so slightly, teasing you so lightly it is maddening, your fingers twitching with the desire to rub yourself more vigorously. The goo around your pussy and ass is less coy, pressing inward with every heartbeat, penetrating you by inches as the bunch at your feet rolls upward to fill every nook and crevice of your curves with its oily " + gooGirlColor(2) + ". The goo inside you is just thick enough to make it difficult for you to take deep breaths, but soft enough to let your body squeeze and clench with gushing spasms that leave you swaying in place, not daring to move too much. Your breasts feel weightless, suspended in the thick fluid of the goo's body, leaving you light-headed and almost giddy, unable to help yourself as you play with your chest, wobbling the " + gooGirlColor(2) + " mammaries and giggling at the endless ripples that radiate over your chest, the motion sending curtains of heat rolling over your sensitive " + pc.skin() + ".\n\n");

	output("Taking slow, shallow gulps of air, you finally master yourself and are able to move, albeit slowly. At your first step, the goo-girl sucks up the slime pooling at your feet and thickens the jiggling gel-encasement around you. Her head pokes out next to your shoulder, her expression one of eye-clenching bliss at the vibrations of your body inside her. She can feel every twitch of your over-stimulated flesh, every pulse of blood through your veins, and every muscle contraction of your building orgasm. The trivial sensation you take for granted are transmitted through the tightly-fitting body-suit you've made of her. A sudden resolve strikes you, and you launch yourself forward, breaking into a swift run across the shore of the lake. Next to yours, the girl's head silently moans at the muscles thrumming within her body. You clench your bobbing breasts with one hand, squeezing your nipples through the goo-suit and the shimmering, slick membrane begins to bead with droplets of sapphire sweat, your ministrations overwhelming the girl's ability to maintain consistency. The heat of her orgasm feeding into your own, it's all you can do to keep your " + pc.legs() + " from slipping out from underneath you.  Your free arm lifts of its own accord and you realize with a start that she's not completely helpless as she moves your hand to your crotch, bracing your thumb firmly against your pulsing clit. You let out a soft gasp and try to pull your hand away, but the goo-glove controls your limb as firmly as if it were hers. She slides one finger after another into your goo-stuffed cunt and the penetrating firmness pushes her slimy-soft dildo deeper against your inner walls.\n\n");

	output("When she gets to the third finger, you lose your balance and tumble to the ground, rolling in the sand and ending up on your back as the goo-girl digs your hand into your pussy as deeply as she can. Trying to fight back, you squeeze your nipples in a pinch so tight it is just shy of painful. Wearing each others' bodies, the two of you roll your hands up and down, tweaking, stroking, pumping, teasing, and pinching every sensitive spot, trying to work the other to a release but succeeding only in pleasuring both of you. You turn your head to the goo-girl next to you and murmur \"<i>fuck it,</i>\" before pressing in to a deep, gooey kiss with the girl. The goo suit stops fighting against you immediately and the two of you jill yourselves frantically, redoubling your efforts, cooperatively this time. Your climax is almost immediate, the goo-girl rippling wildly as you clench down, muscles spasming in a full body shudder that floods your mind and tints your vision with a pinkish " + gooGirlColor(4) + ".\n\n");

	output("You aren't sure how much time has passed when you finally regain your senses. The goo-girl nuzzles your cheek affectionately, perhaps thanking you for the unique experience. Slowly, regretfully, she flows off of you, suckling ooze lapping at your limbs with slurping kisses all over your body. When the two of you are finally separated, you notice that she's maintained your proportions exactly, a " + gooGirlColor(3) + " mirror of your body, jiggling alluringly. You sigh as she flows back into the lapping waves of the lake, missing the heat of her embrace already. Admittedly, you don't know if trying to wear her for an extended length of time would've been a good idea- you'd never get anything done! Retracing your steps, you recover your [pc.gear] and head back to camp.\n\n");
	coreDropChance();
	processTime(25 + rand(10));
	pc.orgasm();
	//pc.slimeFeed();
...
public function layBeeEggsInGoo():void
{
	...
	output("\n\nThis continues on for quite some time with the honey googirl greedily sucking down everything it can from your ovipositor, stroking it and squeezing your abdomen to work out all of its sweet fluid and eggs.  When you finally decide enough is enough, you retract your black tube and rise back up to your [pc.feet].  Turning to look at the goo, you see her idly rubbing at her distended honey-colored belly, imitating a pregnant mother before she slides back down the lakeshore; your numerous eggs bounce around inside her infused body.  She turns one last time to you and waves before sinking below the surface.\n\n");
	//pc.dumpEggs();
	processTime(25 + rand(10));
	pc.orgasm();
	CombatManager.genericVictory();
