...
public function sandWitchSceneEncounter():void {
	//spriteSelect(50);
	clearOutput();
	clearMenu();
	output("A strange woman seems to appear from the dunes themselves.  She identifies herself as a sand witch, and politely asks if she can cast a spell on you.");
	//if (flags[kFLAGS.CODEX_ENTRY_SANDWITCHES] <= 0) {
		//flags[kFLAGS.CODEX_ENTRY_SANDWITCHES] = 1;
		//output("\n\n<b>New codex entry unlocked: Sand Witches!</b>\n\n")
	//}
	//if (pc.statusAffectv1(StatusAffects.Exgartuan) == 1 && pc.cockVolume(0) > 100 && pc.statusAffectv2(StatusAffects.Exgartuan) == 0) {
		//output("\n\nThe [pc.gear] covering your lower half hits the ground, as if yanked down by magic.  Your " + cockDescript(0) + " pulsates darkly, growing rigid in seconds as the demon within you takes over.  It barks, \"<i>Fuck, how about I cast my spell on you baby?</i>\"\n\n");
		//output("The sandwitch ");
		//if (cor() < 50)
			//output("and you both turn crimson", false);
		//else output("turns crimson", false);
		//output(" as you yank your [pc.gear] back into place.  You're in charge here, not some possessed appendage!   Exgartuan yells something snide, but it's muffled too much to understand.  You look up in time to sidestep an attack from the Sand Witch.  It looks like you'll have to fight her!");
		//startCombat(new SandWitch());
	//}	else 
	addButton(0, "Yes", allowSandWitchMagic);
	addButton(1, "No", refuseSandWitchMagic);
}
...
private function allowSandWitchMagic():void {
	...
	if (pc.hairColor == "sandy blonde") {
		output("She smiles wickedly and intones, \"<i>Tresed eht retaw llahs klim ruoy.</i>\"\n\n");
		...
		else if (pc.biggestTitSize() >= 1 && pc.biggestTitSize() <= 2) {
		...
		if (pc.nipplesPerBreast < 1) {
			output("Two dark spots appear on your chest, rapidly forming into sensitive nipples.  ", false);
			pc.nipplesPerBreast = 1;
			//dynStats("sen", 2, "lus", 1);
			pc.lust(1);
		}
		if (pc.isLactating()) {
			output("A strong pressure builds in your chest, painful in its intensity.  You yank down your top as ");
			if (pc.milkRate <= 20)
				output("powerful jets of milk spray from your nipples, spraying thick streams over the desert sands.  You moan at the sensation and squeeze your tits, hosing down the tainted earth with an offering of your milk.  You blush as the milk ends, quite embarassed with your increased milk production.  ");
			else if (pc.milkRate <= 26)
			...
			output("A pleasurable release suddenly erupts from your nipples!  Streams of milk are spraying from your breasts, soaking into the sand immediately.  It stops all too soon, though the witch assures you that you can lactate quite often now.  ");
			if (!pc.canLactate()) pc.milkMultiplier = 75;
			//dynStats("lib", .5, "sen", 1, "lus", 10);
			pc.libido(0.5);
			pc.lust(10);
		}
		output("The sand-witch smiles and thanks you for your offering.  You notice her dress is damp in four spots on the front.  ");
		if (flags["COC.SAND_WITCH_SAW_NAKED"] == undefined)
			output("You wonder at what her robes conceal as she vanishes into the dunes.");
		if (flags["COC.SAND_WITCH_SAW_NAKED"] == 1) {
			if (pc.cor() <= 33)
				output("You are glad to avoid servicing her again as she vanishes into the dunes.");
			else if (pc.cor() <= 66)
				output("You wonder if you should've resisted and tried for some sex as she departs.");
			else output("You wish you had said no, so you could fuck with her and her magnificent quartet of breasts some more.");
		}
	}
	else
	{
		output("She smiles wickedly and intones, \"<i>nuf erutuf rof riah ydnas, nus tresed eht sa ydnas.</i>\"\n\nYou feel a tingling in your scalp, and realize your hair has become a sandy blonde!", false);
...
private function refuseSandWitchMagic():void {
	...
	clearOutput();
	clearMenu();
	output("With an inarticulate scream of rage, the Sand Witch attacks!");

	CombatManager.newGroundCombat();
	CombatManager.setFriendlyCharacters(pc);
	CombatManager.setHostileCharacters(new CoCSandWitch());
	CombatManager.victoryScene(CoCSandWitchDefeated);
	CombatManager.lossScene(CoCSandWitchWon);
	CombatManager.displayLocation("SAND WITCH");
...
internal function sandwitchRape():void {
	...
	if(pc.lustQ() > 99) {
		...
		pc.orgasm();
		output("Your wobbly legs give out underneath you as your body's will to fight evaporates.\n\n", true);
		if(pc.hairColor.indexOf("sandy blonde") != -1) {
			output("She smiles wickedly and intones, \"<i>Tresed eht retaw llahs klim ruoy.</i>\"\n\n");
			if (pc.breastRows.length == 0 || pc.biggestTitSize() == 0) {
				output("You grow a perfectly rounded pair of C-cup breasts!  ");
				if (pc.breastRows.length == 0) pc.createBreastRow();
...
private function sandWitchBadEndPartThree():void {
	...
	//if (pc.findPerk(PerkLib.MarblesMilk) >= 0) {
		...
		//output("Trying to figure out why you feel so incredibly weak, you think back on what might have caused this.  When was the last time you had some of Marble's milk?  Feeling your body start to wither away, the realization hits you.  You chuckle softly at your captor as the world starts to go dark; looks like you'll be the one to have the last laugh...\n\n");
		//output("The last thing you hear is the Sand Witch screaming \"<i>What the hell is going on?  I ALWAYS get what I want!</i>\"");
	//}
	//else {
...
private function sandwitchRaped():void {
	...
	output("Even as you rip her sand-colored robes from her body, her eyes hold power and defiance. Her chest holds four large breasts, each leaking milk upon the sands.  ", false); 
	//Malllllleeeee (or has penis)
	if(pc.hasCock()) {
		//Not so corrupt
		if(pc.cor() < 50) {
			output("You smile at the sand witch, trying to seem enticing. A tiny smile plays across her face even though she supports a worried frown. She reaches up to you, and you allow her to gently pull you down to her. You marvel at her multiple breasts, each perfectly rounded. Each has a nipple approximately two inches long, enough to suck if you wanted to.  ", false);
			output("Your attention wanders down to the damp sand underneath her legs, and her musky scent hits your nose. Which do you go after?\n\n", false);
		...
		else 
		{
			...
			if(pc.cockTotal() == 1) {
				//DOGGIES
				if(pc.hasKnot()) knotSandwitch();
				else {
					output("You place your " + pc.foot() + " in the wet sand, pulling her hair and making her look you in the eyes. The sand witch looks shocked at your display of power, only turning you on more. You undo your [pc.gear] and grope her nethers, only to find a surprise. She has " + num2Text(pc.cocks.length + 1) + " pussies and each seems to be made to fit you exactly. Pushing her to the ground, you bring the seeping head of your " + pc.cockDescript(0) + " to one of her nethers, slowly easing into her depths.  ", false);
					output("Thrusting deep, you hit bottom, and marvel at the sensations as you feel the head of your " + pc.cockDescript(0) + " rub against the end of her slick cunts, only making you lust after her more. As though responding to your will, your " + pc.cockDescript(0) + " grows just a tiny bit more in length and thickness.  ");
					output("With a satisfied growl of lust from yourself, and a screech of pain from the sand witch, you break past her cervix and into her womb. Pulling out, you rub yourself against her wet outer lips, bringing yourself closer and closer to your peak as the sand witch struggles, her eyes turning wide with fright as she starts to understand you hold more power than her.  ");
					output("With a bestial cry of lust, you shove your shaft deep within one of her vaginas, past her cervix and into her womb where you blast your potent, corrupted, demon-tainted seed.  ", false);
					output("\n\nThe sand witch screams in savage denial as you plant your seed deep within her body, your corruption no doubt making it extra potent. You gaze into her eyes, reveling in her expression of horror. Without uttering a word, you continue to fuck her, making sure your seed is well-entrenched within her womb, feeling it splatter and bubble around your " + pc.cockDescript(0) + ".  You wonder if she will remember the pleasure her body felt today, and if she will be more likely to drop to her knees before you than to fight... ", false);
					output("Finally finished, you pull your " + pc.cockDescript(0) + " from one of her well-used slits, your " + pc.cockDescript(0) + " drooling cum even now. With a grin, you walk away, happy with your little encounter.  ", false);
					pc.orgasm();
				}
			}
			//Multiprick
			if(pc.cockTotal() > 1) {
				...
				output("Using the corruption in your blood, you place your " + pc.foot() + " upon the budding shoots, and wipe them from existence. The sand witch looks shocked at your display of power, only turning you on more. You undo your loin cloth and grope her nethers, only to find a surprise. She has " + num2Text(pc.cocks.length + 1) + " loose wet cunts and each seems to be made to fit you exactly. Pushing her to the ground, you bring the seeping heads of your [pc.cocksLight] to her nethers, slowly easing into her depths.  ", false);
				output("Thrusting deep, you hit bottom, and marvel at the sensations as you feel the heads of your [pc.cocksLight] rub against the end of her wet pussies, only making your desire to penetrate increase. As though responding to your will, your [pc.cocksLight] grow just a tiny bit more in length and thickness.  ", false);
				output("With a satisfied growl of lust from yourself, and a screech of pain from the sand witch, you break past her cervixes and into her wombs. Pulling the heads of your [pc.cocksLight] from her wombs, you rub and rut at the outside of her cunts, bringing yourself closer and closer to your peak as the sand witch struggles, her eyes turning wide with fright as she starts to understand you hold more power than her.  ", false);
				output("With a bestial cry of lust, you shove your shafts deep within her vaginas, past her cervices and into her womb where you blast your potent, corrupted, demon-tainted seed.  ", false);
				//A few dicks...
				output("\n\nThe sand witch screams in savage denial as you plant your seed deep within her body, your corruption no doubt making it extra potent. You gaze into her eyes, reveling in her expression of horror. Without uttering a word, you continue to fuck her, making sure your seed is well-entrenched within her wombs, feeling it splatter and bubble around your [pc.cocksLight].  You wonder if she will remember the pleasure her body felt today, and if she will be more likely to drop to her knees before you than to fight... ", false);
	...
	//Femfem
		...
		output("You smile at the sand witch, trying to seem enticing. A tiny smile plays across her face even though she supports a worried frown. She reaches up to you, and you allow her to gently pull you down to her. You marvel at her multiple breasts, each perfectly rounded. Each has a nipple approximately two inches long, enough to suck if you wanted to.  ", false);
		output("Your attention wanders down to the damp sand underneath her legs, and her musky scent hits your nose.\n\n", false);
		sandwitchBewbs();
	}
}
...
private function sandwitchBewbs():void {
	//spriteSelect(50);
	clearOutput();
	output("You move yourself atop the defeated sand witch, and bring your lips to her breasts.  The moment the first drop of milk touches your tongue, you drink, hungrily, needfully, demandingly. You grope and massage her breasts as she leans back and cries out, her eyes wide in surprise. You guzzle and drink her sweet cream, the small seedlings underneath you appearing to rustle and move as you drink the alabaster flow once intended for the desert sands.  ", false);
	output("Warmth fills your gluttonous belly, and the sand witch places a hand to your stomach, rubbing it lightly as you drink from her breasts, moving on when the one you're drinking from is drained.  ", false);
	output("You burp, and flush scarlet, but the sand witch lays a hand against your face, leans close, and kisses your lips sweetly. Her tongue snakes out and dives into your mouth. She pulls your face against hers, grinding her breasts against your chest. You can feel fresh milk dribbling against your " + pc.skinFurScales() + " and the scents are driving you wild with lust.  ", false);
	output("She breaks the kiss and smiles at you, whispering. Your eyes slowly close as you grow drowsy; you realize too late that she's casting a spell. You fall asleep, the sand witch's milk in your stomach churning, gurgling.  ", false);
	output("You wake up in the desert, immediately grabbing at your body to see if anything's changed...\n\n");
	//pc.slimeFeed();
	//pc.refillHunger(35);
	//dynStats("lus", 25);
	pc.lust(25);
...
private function sandwitchSex():void {
	//spriteSelect(50);
	clearOutput();
	output("With a charming, disarming smile, you reach between her legs and touch against the slick lips of her pussies. She has ", false);
	...
	if(pc.balls >= 2) output("balls ache and your ", false);
	output(pc.cockDescript(0) + " throbs with need, even as you empty your lust into the defeated sand witch.", false);
	output("\n\nWith a satisfied smirk, you lay atop your prize, your " + pc.cockDescript(0) + " still twitching within her depths. One of the sand witch's hands rubs at your head, teasing your " + pc.skinFurScales() + ". Like a fresh youth, you fall asleep, " + pc.cockDescript(0) + " buried deep in the sand witch.  ", false);
	output("With a start you wake up, only to find the sand witch still passed out beside you.  Almost thinking it was a dream, you touch your groin tenderly and rub softly. There's a faint trace of soreness, and you remember the ferocity with which you mashed your pelvises together.  You get up and get ready.  ", false);
	pc.orgasm();
...
private function sandwitchCentaurBoning():void {
	//spriteSelect(50);
	pc.orgasm();
	output("The Sand Witch is panting before you, her clothes in disarray ", false);
	//[win via HP]
	if(enemy.HP() <= 1) output("and torn in many places. ", false);
	//[win via lust]
	else output("and her fingers squeezing her nipples as lust overwhelms her. ", false);
	//[has breasts]
	if(pc.biggestTitSize() > 3) {
		output("As you approach, her eyes drift up to your " + pc.allBreastsDescript() + " and she ", false);
		//[win via HP]
		if(enemy.HP() <= 1) output("is unable to stop herself from slowly licking her lips. ", false);
		//[win via lust]
		else output("sensually licks her parted lips. ", false);
	}
	//[lactating]
	if(pc.isLactating()) {
		output("Her eyes widen when she sees the beads of milk already dripping from your " + pc.nippleDescript(0) + "s as her hand drops between her legs and she begins to slowly masturbate.", false);
	}
	output("\n\n", false);
	//[Corruption < 70]
	if(pc.cor() < 70) {
		output("You smile as you approach", false);
		//[has breasts]
		if(pc.biggestTitSize() > 0) output("; your " + pc.nippleDescript(0) + "s  become stiff", false);
		//[cock only]
		if(pc.hasCock() && !pc.hasVagina()) output(" and your " + pc.cockDescript(0) + " hardens.", false);
		//[cunt only]
		if(pc.hasVagina() && !pc.hasCock()) output(" and your " + pc.vaginaDescript(0) + " begins to drip with moisture.", false);
		//[cock and cunt]
		if(pc.hasVagina() && pc.hasCock()) output(" and your " + pc.cockDescript(0) + " hardens as your " + pc.vaginaDescript(0) + " begins to drip with moisture.", false);
		if(!pc.hasGenitals()) output(" her.", false);  
		output(" She seems hesitant at first, but soon approaches and begins to run her hands along your stomach and your " + pc.hipDescript(),false);
		//[has breasts]
		if(pc.biggestTitSize() > 0) output(", eyes fixed on your " + pc.allBreastsDescript(), false);
		output(".\n\n", false);
		//[GoTo: SW_3 and return] 
		if(SWCentaurMore(3)) return;
		output("\n\n", false);
		output("Delicate hands reach around to ", false);
		//[has breasts]
		if(pc.biggestTitSize() > 0) output("fondle your " + pc.allBreastsDescript() + " and ", false);
		output("tease your " + pc.nippleDescript(0) + "s, and you can feel her grinding herself against your strong shoulders. ", false);
		//[orb penetrated player during combat] 
		output("With a start you realize that the witch's orb is still inside you as it suddenly begins to pulse within your " + pc.vagOrAss() + ", causing you to cry out and hasten your pace across the dunes. ", false);
		output("You lose track of time and location as you ride, feeling her orgasm over and over against you, her milk pouring down your back and spilling onto the sands. ", false);
		//[has breasts]
		if(pc.biggestTitSize() > 0) {
			//[lactating]
			if(pc.isLactating()) {
				output("Your own " + pc.nippleDescript(0) + "s  are leaking as well, ", false);
				//[light-med lactation]
				if(pc.milkQ() < 1500) output("small streams running out ", false);
				//[heavy lactation] 
				else output("torrents gushing out ", false);
				output("under her expert hands. ", false);
			}
		}
		output("You can somehow feel her arousal pouring through her milk and into your skin, filling you with her own sensations and making your entire body vibrate like a sexual tuning fork.  As the sensations grow to be overwhelming, you feel yourself stumbling and slowing until you finally come to a stop. Panting, you listen to the sand witch's voice whispering sweet nothings into your ear... but the nothings seem to be having an effect.\n\n", false);
		//[GoTo: SW-1]
		if(SWCentaurMore(1)) return;
	}
	//[Corruption > 70]
	else {
		output("When she sees the look on your face, the sand witch comes to her senses. Quickly gathering herself up, she tries to run in fear.  You grin and out-pace her easily, galloping around her and steering her in circles, occasionally knocking her over for fun.  When she starts to slow down, you grab her sand-colored robes and ", false);
		//[STR > 50]
		if(pc.PQ() >= 50) output("easily tear them off of her. ", false);
		//[STR < 50]
		else output("drag her a bit until they come off. ", false);
		output("Underneath the robes is a beautiful body with a quartet of breasts, even now leaking milk down her chest and stomach.  The streams run over her abs and split as they pour down the cracks of her twin set of pussies.  Realizing she cannot outrun you, she finally turns and stands her ground, staring at you with a mixture of fear and anger that only serves to increase your lust.\n\n", false);
		//[GoTo: SW-2]
...
private function SWCentaurMore(argument:Number):Boolean {
	...
	//[SW_1]
	if(argument == 1) {
		output("You realize too late that she has been casting a spell for the past few minutes. Before you can react, everything grows dark.", false);
	...
	if(argument == 2) {
		...
		if(pc.cockTotal() > 0 || (pc.isHerm() && rand(2) == 0)) {
			output("Finished with your games, pre-cum starts pooling under your throbbing " + pc.cockDescript(0) + ". ", false);
			//[largest cock is wide]
			if(pc.cocks[0].thickness() >= 3) {
				if(pc.hasKnot(0)) output("Seeing its sheer size combined with your swollen knot causes her to gasp in terror and try to run again.  ", false);
				else output("Seeing its sheer size causes her to gasp in fear and turn to run again.  ", false);
			}
			output("You grab the witch and knock her down into the sand, quickly lowering your " + pc.cockDescript(0) + " to be against her buttocks. ", false);
			//[1 cock, non-tentacle]
			if(pc.cockTotal() == 1 && pc.cockTotal(GLOBAL.TYPE_TENTACLE) == 0) output("With a single thrust, you push deep into one of her cunts, ", false);
			//[1 cock, tentacle]
			if(pc.cockTotal() == 1 && pc.cockTotal(GLOBAL.TYPE_TENTACLE) == 1) output("Your " + pc.cockDescript(0) + " caresses her anus teasingly, causing her to whimper in a mixture of arousal and denial.  With a single savage thrust, you push past her clenched muscles, ", false);
			//[two cocks] 
			...
			//[2+ cocks]
			if(pc.cockTotal() >= 2) {
				output("The feeling of your [pc.cocksLight] touching each other through the narrow layers of her body causes you to grunt in pleasure, experiencing the sensation anew during every thrust. ", false);
			}
			//[largest cock is long, non-tentacle]
			if(pc.cocks[0].cLength() >= 12) {
				if(pc.cockTotal() == 1) output("Her body surrenders more and more as your [pc.cocksLight] presses against her cervix. ", false);
				else output("Her body surrenders more and more as your [pc.cocksLight]  press against her cervixes. ", false);
				output("Her mouth opens in a soundless and agonizing cry when you finally push past. ", false);
			}
			//[1 cock, tentacle] 
			if(pc.cockTotal(GLOBAL.TYPE_TENTACLE) == 1 && pc.cockTotal() == 1) {
				output("Her body surrenders more and more as your " + pc.cockDescript(0) + " pushes deep into her bowels, snaking its way further into her body as it fucks her insides. ", false);
			}
			output("It feels like you have been pushing yourself into her for hours", false);
			//[largest cock is wide and/or long]
			if(pc.cocks[0].cLength() >= 12 || pc.cocks[0].thickness() >= 3) {
				output("; her cries have long since died down to whimpers. ", false);
			}
			//[largest cock is not wide and/or long]
			else output(", and her cries have long since turned into groans of pleasure. ", false);
			output("You cannot hold off your orgasm any longer. Your [pc.cocksLight] explode", false);
			if(pc.cockTotal() == 1) output("s", false);
			output(", ", false);
			//[large cum production] 
			if(pc.cumQ() >= 250) output("gushing massive amounts of your cum ", false);
			//[regular cum production] 
			else output("pushing your sperm ", false);
			output("deep into her ", false);
			//[1 cock, tentacle]
			if(pc.cockTotal() == 1 && pc.cockTotal(GLOBAL.TYPE_TENTACLE) == 1) output("anus", false);
			else {
				//[largest cock is not wide and/or long]
				if(pc.cocks[0].cLength() >= 12 || pc.cocks[0].cockThickness >= 3) {
					//[1 cock]
					if(pc.cockTotal() == 1) output("cunt", false);
					//[2+ cocks]
					if(pc.cockTotal() >= 2) output("cunts", false);
				}
				//[largest cock is wide and/or long]
				else {
					output("womb", false);
					if(pc.cockTotal() >= 3) output(" and bowels", false);
				}
			}
			output(".  With a satisfied groan, you pull out and let your [pc.cocksLight] dribble the last remnants of your cum over the ravished witch. Satisfied, you ride off into the desert.", false);
		}
		//[% no cocks]
		else {
			output("Finished with your games ", false);
			//[has cunt] 
			if(pc.hasVagina()) output("and your " + pc.vaginaDescript(0) + " dripping with desire", false);
			output(", you push the witch unceremoniously to the ground and deliver a slap to her breasts.  She cries out in pain as milk splashes out onto the sand.  A cruel smile is brought to your face and you start slapping them even harder, alternatively smacking her pair of cunts for good measure.  Surprisingly, her cries of pain begin to turn into moans of pleasure with every slap.  She might even be pushing into the blows slightly, though it is difficult to tell.  After a sizable pool of milk and her juices has drained into the sands beneath her, you reach down and start to drive your fingers hard into one of the sand witch's cunts.  The first few thrusts are enough to violently bring her to orgasm.  You trot forward slowly until you are over her and the witch suddenly hops up. Driving her face into your ", false);
			//[has cunt]
			if(pc.hasVagina()) {
				output(pc.vaginaDescript(0) + ", she fingers herself even harder than you had been a moment ago.  Her lips lock unto your " + pc.clitDescript() + " and she suckles on it, drawing a pained gasp from you and causing your pussy juices to gush over her face. You feel ", false);
				//[cunt size is small]
				if(pc.vaginalCapacity() < 10) output("one of her fingers ", false);
				//[cunt size is normal]
				else if(pc.vaginalCapacity() < 20) output("several of her fingers ", false);
				//[cunt size is large] 
				else output("a fist ", false);
				output("pushing deep into your " + pc.vaginaDescript(0) + ", and you're surprised as ", false);
				//[anus size is small]
				if(pc.analCapacity() < 10) output("her tongue ", false);
				//[anus size is normal]
				else if(pc.analCapacity() < 20) output("two of her fingers ", false);
				//[anus size is large]
				else output("a fist ", false);
				output("penetrates your " + pc.assholeDescript(), false);
				output(".", false);
			}
			//[does not has cunt]
			else {
				output(pc.assholeDescript() + ", she fingers herself even harder than you had been a moment ago.  Her tongue drives into your " + pc.assholeDescript() + ", licking enthusiastically as she squeezes her nipples. ", false);
				//[anus size is small]
				if(pc.analCapacity() < 10) output("The sensations are intense, and you could almost swear that her tongue is tracing a pattern as her mouth mutters strange sounds. ", false);
				//[anus size is normal] 
				else if(pc.analCapacity() < 20) output("Unsatisfied with just her buffet, you soon feel her fingers thrust hard into you and hear her muttering what sounds like obscene words of depraved lust. ", false);
				//[anus size is large]
				else output("Unsatisfied with just her buffet, the witch grabs one of her breasts and squirts milk into your massive hole.  You realize she was lubricating it even as you feel her entire hand driving into you.  The sensations are too intense, and your screams of pleasure drown out her lustful muttering. ", false);
				output("Her mumbling finishes even as an orgasm overwhelms you.", false); 
				output("\n\n", false);
				//[SW_1]
				SWCentaurMore(1);
	...
	//[SW_3]
	if(argument == 3) {
		output("As she begins to seem less skittish, you reach your hands into her robe and push it off of her shoulders, revealing a beautiful body underneath.  A pair of pussies is betraying the sand witch's arousal - her thighs are already dripping wet.  You begin to tease her four breasts as you lower your mouth to her lips for a passionate kiss.  Breaking off the kiss and grinning, you ", false);
		//[STR > 50]
		if(pc.PQ() > 50) output("pick her up and toss her onto your back. ", false);
		//[STR < 50]
		else output("offer her your hand and help her onto your back. ", false);
		output("Your strong hooves carry you across the clear desert at a light trot.  Without her robes to protect her, you can feel the witch's bare pussies grinding against your back as she presses her quad of breasts against your shoulders.  Her moans intensify more and more as you pick up the pace.", false);
	}
	return false;
}
...
private function knotSandwitch():void {
   ...
   output("You begin to buck into her at a frenzied pace, your body's need to release your motivation.  At first, your knot swelling is an extra sensation, a tightness that squeezes deliciously as more and more blood is forced inside by sheer stimulation.\n\n", false);
	...
	output("You grunt and push hard, desperate to force the bulbous cock-flesh inside, and you're rewarded as it pops in with a wet squelch. The tightening pressure around the base of your " + pc.cockDescript(0) + " pushes you over the edge, and the cum begins to shoot in her tight hole, with your " + pc.cockDescript(0) + " trapped inside!\n\n", false);
  	//ORGAZMO
  	if(pc.cumQ() < 25) output("The cum slicks your dick as you continue trying to hump, easing the over-sensation of your dick while you're glued together by the bulbous knot locked in her snatch.", false);
    if(pc.cumQ() >= 25 && pc.cumQ() < 100) output("Your cum expands her pussy, stretching her out and leaving your " + pc.cockDescript(0) + " swimming in your spunk, trapped within her tight-stretched walls by your massive knot.", false);
    if(pc.cumQ() >= 100 && pc.cumQ() < 500) output("Your cum begins to pump into her corked body, filling her hole to the brim.  Surely the pressure is piling the jizz deep into her womb, and after what feels like forever, the overfull spunk shoves back on your knot, a little dribbling out around the edges of her taut feminine folds as her pussy reaches its limit.", false);
    if(pc.cumQ() >= 500) output("Your cum spurts into her in rapid jets, coating the inside of her walls briefly white as more continues to pour into her. Her cunt begins to fill, and as she strugles to relieve the pressure that continues to pump into her, she only succeeds in spurring your orgasm to a new height.  You continue to blast within her, your knot holding her fast as she squirms, each shot of cum from your prick managing to squirt a tiny bit of jizz from your locked genitals.", false);
	pc.orgasm();
}
...
public function beatSandwitch():void {
	...
	if (enemy.lust() > 99) output("You smile in satisfaction as the " + enemy.short + " drops down on all fours and begins masturbating feverishly.\n\n");
	else output("You smile in satisfaction as the " + enemy.short + " drops down on all fours and struggles to rise.\n\n");
	if (pc.lust() < 33)
	{
		processTime(10 + rand(10));
		addButton(0, "Next", CombatManager.genericVictory);
	...
	output(" Sadly you realize your own needs have not been met.  Of course, you could always fuck the witch...  Of course, just taunting, teasing, and humiliating her for her arrogance would be equally amusing, <b>but it would give her plenty of time to turn the tables...</b>\n\n");
...
public function missingoSex():void {
	//spriteSelect(50);
	clearOutput();
	output("You stride proudly towards your latest conquest, the prone witch starting at you with a look of pure and delightful helplessness. She backs away on all fours, but a simple \"Stop.\" uttered from your lips is enough to freeze her in place. You look do/n at her, your \" + multiCockDescript(0) + \" throbbing within your clothes. What clothes? Oh, those clothes th$t you happen to be tearing from your body, much to her dismay.\n\n");

	output("Clearly, this bitch needs the dick. And you're just the ^&a*pion to give it to her. You lick your lips and drop to your @nees, taking hold of her le-s and forcibly spreading them apart. Looking down, you see that her body is quite ready for you, her exposed fuckhole pouted and moist. You cannot help but laugh a- the situation, the motions making your \" + multiCockDescript(0) + \" bounce in the open air as you co-vulse with laugh-er. Her res$$tance only serves to arouse you, making you savor that choice ass before you claim it for your own.\n\n");

	output("You want to embarrass her further, complete her humiliation upon the sands she once thought to be her domain. You tear her robes into little shreds, exposing her four generously sized breasts. Your eyes follow her dri-^ling nipples, the pupils bouncing as they do. \"Wh-what are you going to do? Rape me?!\" cries the witch, her tone equal parts terrified and wishful. Her thighs attempt to close, though you're sure it's more t$ grind them together than any atte-pt at preserving her modesty. Leaning down, y-u open up to let your tongue lap at her swollen clit. She arches her back and moans, thrusting her hips towa-ds you as bes- she can.\n\n");

	output("0)w is t$e time. But no, she won't get the dick. She hasn't EAR*ED the dick. She isn't even wor$h your \" + pc.vaginaDescript(0) + \" yet. No n- no, you need -o warm that ho_ cunt up. You whistle to g-t her attention, and as you do, you present your -i%-s ^a_=e* up to*--h-r. Her eyes wi_en, the wo$an's jaw going slack. \"N-no.. not t-e whole thing.\" you nod _n respo-se to her plea. \"The WHO-E THING?!\"\n\n");

	output("Now she begins to panic. She shake- her head rapidly as you draw bac- your %s99=, and thrust them towards her >pen sl1t...");
...
private function missingoSex3():void {
	//spriteSelect(50);
	clearOutput();
	output("_at_r@_ly, you %i-+. Ho@-@@ly, her h_le c_n't take @_ch _ po_ndi_g. Yo_ can t_ll t-is b_ loo_ing _t her pr_ne,',, v@^@erable b@dy, #ipe _or y-ur a_use. Y_ur *i- isn't ev|n en_ug_ to do this! Y_ur-i*))s fall sho_t, la@d@ng i( the sa_d, sp_as_i_g he- bo^y wit@ a @ave @f the -tuff. She gasps in su!2!i!e, ba_el_ a?le to k_ep it fr_m g_tti_g into h_r m_uth. @er @it@ har_ly fa_e as well, bei@g c@_t_d ge_ero_sl_, ^er milk mi$ing w--h it. T_e re_ult_ng co@cocti@n sti-ks to her c_est, l@a_in@ it q_ite t_e m_s_.\n\n");
	
	output("\"@ @a@'t b@lie^e y_u j@st _rie_ th_t.\" s@e sa_s @?th a )_=f. \"Yo_'re @oi(g to g_t a_d in all s))ts o& u_c_mfo_table pla_es ^& yo_ ke-_p t@at u@.\"\n\n");
	
	output("\"@_i@_...\" s_e says, ##o@%7g @t yo_ s_@_y. \"W_@ d_n't @o_ (ull b=c( an_ @r_ _h_@ a_@in....\"\n\n");
...
private function missingoSex6():void {
	...
	clearOutput();
	pc.orgasm();
	output("You successfully restore your game. Everything's intact. At least, you thought it was. You must have messed up the password somewhere along the line, but oddly enough, the only difference is the clock being set an hour ahead and a few extra gems and EXP in your pocket.\n\n");

	output("It is like fucking christmas up in here.");
...
//private function ovipositSandWitches():void {
	//clearOutput();
	//output("As you glance down at the ");
	//if(enemy.HP() <= 1) output("bruised and battered");
	//else output("lusty");
	//output(" sand witch, you feel the familiar tug of arousal in your loins.  Yet your current... situation offers quite the opportunity to relieve some tension through the use of your 'other' sexual appendage.  Licking your lips in anticipation, you lower yourself down to the obstinate spell caster, who immediately reacts with ");
	//if(enemy.HP() <= 1) output("fright as she clamors to get away.");
	//else output("an uneasy curiosity to see what exactly you have planned.");
	//
	...
	//output(" for her viewing consumption.  Making quite the persuasive argument of non-hostility as you do, the sand witch finally relents, removing her cloak and exposing her four-breasted, ");
	//if(pc.totalCocks() > 1) output(num2Text(pc.cockTotal() + 1));
	//else output("two");
	//output("-sexed form, allowing the sun to glisten off her bare body.");
	//
	...
	//if(pc.canOvipositBee()) output("bee-morphs");
	//else output("driders");
	//output(" can do to their partners and unsure if this is really what she wants.  The poor girl seems to need a little 'encouragement'... of course, you could just skip all that regardless if you really wanted.");
	//
...
//private function eggwitchForeplay():void {
	//clearOutput();
	//output("Knowing how the desert witch feels at this moment, you decide to help ease her into receiving your young.  Slowly, you slide your fingers up the uneasy woman's legs, the sensation of your hands along her delicate frame soliciting a pitiful gasp from the sand witch as she receives the caress.  Gradually working upwards, your palms curve and twist along the blonde beauty's thighs, all the while causing her to shudder from the stimulus, her ");
	//if(pc.totalCocks() > 1) output(num2Text(pc.cockTotal() + 1));
	//else output("two");
	//output(" clits all hard from the apparent promise of your carnal attentions.  Positioned between her thighs, your hands rest and you wait for the eager gaze of her eyes, the imploring of your continued progress.  The sand witch doesn't disappoint; her needy eyes beseech you to splay her legs open and get intimately acquainted with her naughty bits.");
	//
...
//private function laySomeEggsInThatWitchFinally():void {
	...
	//output("Before long the intense need to culminate the act grows too lucid to ignore, and with a hum of satisfaction you finally bore your ovipositor through the tight ");
	//if(cunt) output("folds of her vulva");
	//else output("ring of her anus");
	//output(".  Like something out of a fever dream, the bizarre appendage you wield burrows further and further into her ");
	//if(cunt) output("sex");
	//else output("ass");
	//output(", the ");
	//if(cunt) output("ribbed contours of her pussy");
	//else output("tight fit of her innards");
	//output(" enrapturing your egg-tube with every passing inch.  You begin the process of lubricating the antsy witch's ");
	//if(cunt) output("love-");
	//else output("anal ");
	//output("canal for the delivery of your young; the witch barely contains her moans and utter glee at being filled with such a hot, humid substance.  In perverted delight the sand witch coos between kisses, her moans betraying a curiosity to explore just how pleasant mating this way can truly be.");
	//
	...
	//if(cunt) output("opening of your lover's cervix");
	//else output("deepest recesses of her sphincter");
	//output(".  The joining of oviduct to ");
	//if(cunt) output("womb");
	//else output("anus");
	//output(" now complete, you prepare yourself for the inevitable convulsion of orgasm along your bizarre appendage, grinning down at the sand witch with a smirk that can only convey that the fun's about to begin.  With a sudden, violent quake your ovipositor rumbles with the first of many eggs, squeezing your unborn brood down your shaft and pushing ");
	//if(cunt) output("at the waiting lips of the sand witch's vagina");
	//else output(" through her rectal walls");
	//output(".  With a meticulous push, the forerunner of your young manages to infiltrate her passage, working its way valiantly into her depths with every convulsion.");
	//
	...
	//if(cunt) output("into her waiting womb");
	//else output("as far as you can safely tuck it");
	//output(", nestled and sound until the scramble for life can begin.");
	//
	//output("\n\n\"<i>More... give me more...</i>\" the sand witch whimpers, caressing your ");
	////(Bee, non-horse:
	//if(pc.canOvipositBee() && !pc.isTaur()) output("torso");
	//else output("pelvis");
	//output(" to spur you on.  Of course you don't have a problem with fulfilling her wishes; the neglect of your throbbing sex");
	//if(pc.cockTotal() > 1 || pc.gender > 2) output("es");
	//output(" is the only point of dissatisfaction, a point you make clear when you ");
	//if(pc.hasCock()) output("take hold of your [pc.cocksLight] and bring it");
	//else output("bring your " + vaginaDescript(0));
	//output(" to her face.  The cute stare of her half-glazed eyes turns to the lunge of lips toward your sex, the sand witch driven like a woman possessed in her efforts to get the next batch going...");
	//
	//output("\n\nFor what feels like an eternity you sit atop the pliant sorceress, having her tend to your sex");
	//if(pc.cockTotal() > 1 || pc.gender == 3) output("es");
	//output(" while you slowly pump her full of eggs, her girlish little moans resonating through your body as she orally stimulates your ");
	//if(pc.hasCock()) output(multiCockDescriptLight());
	//if(pc.gender == 3) output(" and ");
	//if(pc.hasVagina()) output("your " + vaginaDescript(0));
	//output(".  At long last her enthusiastic efforts bring you to a more 'traditional' release, lathering ");
	//var facial:int = rand(2);
	//if(facial == 0) output("her facial features with proof of your climax");
	//else if(facial == 1) output("the interior of her mouth with sweltering streams of your cum");
...
//public function sammitchBirthsDriders():void {
	...
	//output("As you struggle with the loose footing of the hot desert sand, a weak howl reaches your ears.  Curiously, you stop and listen, trying to locate the potential source of the sound.  Yet all you can really hear is the wind against the sand-blasted, desolate terrain, giving you cause to shrug it off and continue on your merry way.");
	//
	...
	//if(cor() < 50) output("Intending to help");
	//else output("Curious");
	//output(", you slide down the dune that you are on and come to a stop next to the distressed woman.  However, upon seeing her face you realize that this is the very sand witch that you had a hot and steamy tumble with!");
	//
	...
	//if(cor() < 60) helpZeWithBirfBabies();
...
//private function helpZeWithBirfBabies():void {
	...
	//
		//output("\n\n\"<i>Please, not too fast!</i>\" she calls out from behind you, clearly trying her best to not have the labors of your lust right here and now.  Rising to your [pc.feet], you attempt to recall which way to go, only to realize that it doesn't matter.  Concentrating on the camp, you yell at the sand witch to hold on, then ");
		//if(pc.isDrider()) output("skitter");
		//else output("gallop");
		//output(" across the dry desert sea...");
	//}
...
//private function sandwitchBirthsYourMonstrosities():void {
	...
	//if(cor() < 60) output(", with a quick kiss on the lips");
	//output(", whisper in her ear that you'll see her again...");
	//
	////[(corr >= 60)
...
//public function witchBirfsSomeBees():void {
	...
	//if(silly()) output(", echoing tiny little cries of \"<i>@</i>\"");
	//output("; likely it was just waiting for someone to find their mother before taking off in the general direction of the forest.  The weak voice of the desert vixen fills the air as she speaks to you.  \"<i>That... was the best.  I can't believe I'm a mother!</i>\"  She gives you a look of appreciation for showing her how pleasurable being a host can be.  Seeing she needs her rest, you give a nod and turn to leave... only to feel a hand grasp at your [pc.leg].  \"<i>I would be disappointed if you didn't come around and 'say hello' more often; keep that in mind " + pc.mf("handsome","beautiful") + ".</i>\"  She coos, before drifting off to sleep.  She'll be fine in this shady part of the desert while she rests, the dune currently obstructing the sun and keeping her from being burned from the sun's rays.");
	//
...
private function sandwitchSpanking():void {
	clearOutput();
	clearMenu();
	output("Feeling euphoric after your clear victory, you lazily circle the panting witch and laugh at her.  \"<i>Look at you,</i>\" you taunt, \"<i>A mighty witch of the sands, laid low by a helpless traveller.");
	if(enemy.HP() <= 1) output("  You can't even stand up!");
	else output("  You can't even stop masturbating, can you?");
	output("</i>\"  You kick her hand away, and she tumbles down into a distraught heap, crying softly");
	if(enemy.lust() > 99) output(" but still masturbating");
	output(".  It's time for some fun!");
	...
	if(pc.cor() < 33) output("and though you feel kind of bad about using her this way, she likely would have done worse to you.");
	else if(pc.cor() < 66) output("and though you aren't sure you ought to be using her this way, she likely would have done worse to you.");
	else output("and though you used to find things like this distasteful, you don't really give a shit anymore.");
	output("  You tweak and tease with a goofy smile, laughing as you aim one of the tits up into the witch's face, splattering her with her own alabaster juices.  You're so involved in it that you fail to notice the increasingly angry gaze that settles over your captive's face.  Her tits are just so goddamn fun to tease that you don't really waste any time paying attention to her from the neck up.  That's a mistake.");
	output("\n\nToo late, you see the witch's hands glowing, and before you can react, a club of solid stone whomps you upside the head, flinging you wholesale onto your side.  You try to stagger up, but your balance isn't working, and you fall back down, ears ringing.  The witch, seemingly recovered, is towering over you.  She screams, \"<i>My milk is not a toy!</i>\" and throws her arms up over her head.  From below, a torso-sized block of sandstone rumbles up out of the dune, striking you square in the plexus and lifting you up, [pc.legs] and arms dangling uselessly.  A few whispered incantations later and your wrists and [pc.feet] are sucked into the block, binding you into a helpless, restrained pose.");
	output("\n\n\"<i>Now who is laid low?!</i>\" she snaps with magic crackling in the air.  \"<i>You need to be taught a lesson, off-worlder!</i>\"  The witch grabs at the bottom of your [pc.armor] and yanks it down, exposing your [pc.butt] to the hot desert air.  ");
	if(pc.hasCock()) output("Worse still, [pc.eachCock] is now unprotected and pressed against the rough stone by your body-weight.  It's less than pleasant.  ");
	else if(pc.hasVagina()) output("Worse still, your [pc.vagina] is pressed up against the rough stone.  It's less than pleasant.  ");
	output("You bristle at your helplessness");
	//if(pc.findPerk(PerkLib.FireLord) >= 0 || pc.findPerk(PerkLib.Hellfire) >= 0 || pc.findPerk(PerkLib.Dragonfire) >= 0) output(", breathing out a gout of flame in rage, but you can't even direct the blast properly like this.  All it does is melt some of the sand below into glass");else 
	output(", but there's nothing you can do");
	output(".  Fingers tickle the " + pc.skin() + " of your bottom, and then, without warning, your [pc.butt] is slapped!  The sound of the hit rings out across the desert, hard enough to make moisture bead at the corners of your eyes.");
	output("\n\nThe smirking sorceress laughs, \"<i>You are too prideful for your own good.  Now, I will teach you respect for our order, one spank at a time.</i>\"  She doesn't even wait before hitting you again, bringing her hand down on your unprotected ass without hesitation.  You squirm from pain, trying to escape, to wiggle free, but there's no use - you're tied down with stone restraints that you can't hope to escape.  All you can do is writhe uselessly against your bindings while your [pc.butt] is tanned bright red, tingling with residual pain even when it isn't being hit.");
	...
	if(pc.cor() < 50) output("She sounds almost fanatical in her dedication to her order.");
	else output("What the hell is she on about?  Does she actually think her little torture session is going change your attitude?");
	output("\n\nPicking up the pieces of her tattered robe, the sand witch makes to leave.  She stops after a few paces and turns back your way, \"<i>Perhaps this will ease the wait.</i>\"  Your eyes widen in anger when she makes familiar motions, but there's nothing you can do.  Soon, smooth stone orbs are sliding into through your cheeks and into your [pc.butt]");
	if(pc.hasVagina()) output(" and [pc.vagina]");
	output(".  All of them are vibrating to slightly different tempos, and you get so focused on the oscillating anal symphony, that you miss the enchantress's departure.  You ride the vibrating stones to so many orgasms that you lose count.  Eventually, the magic column fades, receding into the sand.  At the same time, the lifeless stone orbs roll out of your [pc.asshole]");
	if(pc.hasVagina()) output(" and [pc.vagina]");
	output(".  Bruised and orgasmed into exhaustion, you pass out in the sand with your [pc.armor] around your ankles.  Maybe next time you won't waste your chances gloating.");
	enemy.lust(98);
	enemy.HP(2);
...
public function CoCSandWitchDefeated():void
{
	pc.removeStatusEffect("Vibrating Stone");
	beatSandwitch();
}

public function CoCSandWitchWon():void
{
	pc.removeStatusEffect("Vibrating Stone");	
	sandwitchRape();
}